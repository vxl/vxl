...
template <class T>
struct foo<T *, T *> { void bar() {}
                       };
template <class T>
struct foo<int *, T> { void baz() {}
                       };
...
class victor;
template <class T>
T dot(victor<T> const & u, victor<T> const & v);
...
template <class T>
void f(T t) { t.compiler_selected_wrong_overload(); }
void f(const A &) {}
