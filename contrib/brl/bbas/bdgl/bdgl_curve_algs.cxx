...
//: Need this temporary cross product for some reason
//: The one in vnl seems to go into recursive self-destruction
//JLM - change back after we see what is going on.
static vnl_double_3 temp_cross_3d(vnl_double_3& v0, vnl_double_3& v1)
{
  vnl_double_3 v1xv2;
  v1xv2.x() = v0.y()*v1.z()-v1.y()*v0.z();
  v1xv2.y() = v0.z()*v1.x()-v0.x()*v1.z();
  v1xv2.z() = v0.x()*v1.y()-v0.y()*v1.x();
  return v1xv2;
}
//:Intersect an infinite line with a line formed by the two 
//input points , p0 and p1.  It is assumed that the two lines
//do intersect.
static bool intersect_crossing(vnl_double_3& line_coefs, 
                               ...
                               vnl_double_3& inter)
{
  //uform the line from p0 and p1
  vnl_double_3 lv01 = temp_cross_3d(p0, p1);

  //Find the intersection point
  inter = temp_cross_3d(lv01, line_coefs);
  //Check sanity of the intersection
  if(fabs(inter.z())<bdgl_curve_algs::tol)
    return false;
  return true;
}

//-------------------------------------------------------------
//: intersect an infinite line with the digital curve.  If there
//    is no intersection return false. Note that the line can intersect 
                                     ...
                                     vcl_vector<vgl_point_2d<double> >& pts)
{
  ...
  for (double t=dt; t<=1.0; t+=dt)
  {
    ...
    if(vcl_fabs(sign0)<bdgl_curve_algs::tol||
       fabs(sign1)<bdgl_curve_algs::tol||sign0*sign1<=0) //we have crossed or
                                                         // are on the line
      {
        vnl_double_3 inter;
