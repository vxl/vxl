...
vcl_vector<double> min_distances;
 for(int k=0;k<clusters_[i].curve_cluster_.size();k++)
 {
	...
	for(int l=0;l<clusters_[j].curve_cluster_.size();l++)
	{
	  distances.clear();
	  double dist=0;
		for(int m=0;m<clusters_[j].curve_cluster_.size();m++)
		{
	  		if(l!=m)
			{
...
double bdgl_curve_clustering::compute_likelihood_distance(int i,int j)
{
  ...
  if(clusters_[i].curve_cluster_.size()>1)
  {
   for(unsigned int k=0;k<clusters_[i].curve_cluster_.size()-1; k++)
   {
  ...
  {
   for(unsigned int k=0;k<clusters_[j].curve_cluster_.size()-1; ++k)
   {
