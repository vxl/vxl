// This is core/vgui/bgui_displaybase_tableau.cxx
...
// \date   14 Sep 99
// \brief  See bgui_displaybase_tableau.h for a description of this file.
...
bool bgui_displaybase_tableau_selection_callback::select(unsigned)
{
  return false;
}

bool bgui_displaybase_tableau_selection_callback::deselect(unsigned)
{
  return false;
}

bool bgui_displaybase_tableau_selection_callback::deselect_all()
{
  return false;
}
...
bgui_displaybase_tableau::bgui_displaybase_tableau()
{
...
void bgui_displaybase_tableau::set_selection_callback(bgui_displaybase_tableau_selection_callback* cb)
{
  cb_ = cb;
}


void bgui_displaybase_tableau::add(vgui_soview* object)
{
...
void bgui_displaybase_tableau::remove(vgui_soview* object)
{
...
//
void bgui_displaybase_tableau::clear()
{
  highlighted = 0;
  deselect_all();
...
void bgui_displaybase_tableau::draw_soviews_render()
{
  if (debug)
  ...
  if (vgui_style_factory::use_factory)
  {
         ...
         s_iter != styles_copy.end(); ++s_iter)
    {
           ...
           so_iter != soviews.end(); ++so_iter)
      {
        ...
        if (i != objects.end())
        {
          if (!is_selected(so->get_id()) && !is_highlighted(so->get_id()))
            so->draw();
        }
      }
    }
...
void bgui_displaybase_tableau::draw_soviews_select()
{
...
bool bgui_displaybase_tableau::handle(const vgui_event& e)
{
  if (e.type == vgui_DRAW)
  {
...
bool bgui_displaybase_tableau::is_selected(unsigned id)
{
  vcl_vector<unsigned>::iterator result = vcl_find(selections.begin(), selections.end(), id);
  return result != selections.end();
}

vcl_vector<unsigned> const & bgui_displaybase_tableau::get_selected() const
{
  return selections;
}

vcl_vector<vgui_soview*> bgui_displaybase_tableau::get_selected_soviews() const
{
...
vcl_vector<vgui_soview*> const & bgui_displaybase_tableau::get_all() const
{
  return objects;
}

vcl_vector<unsigned> const bgui_displaybase_tableau::get_all_ids() const
{
...
bool bgui_displaybase_tableau::select(unsigned id)
{
  vcl_vector<unsigned>::iterator result = vcl_find(selections.begin(), selections.end(), id);
...
bool bgui_displaybase_tableau::deselect(unsigned id)
{
  vcl_vector<unsigned>::iterator result = vcl_find(selections.begin(), selections.end(), id);
  if (result != selections.end())
  {
...
bool bgui_displaybase_tableau::deselect_all()
{
...
bool bgui_displaybase_tableau::is_highlighted(unsigned id)
{
  return id == highlighted;
}

unsigned bgui_displaybase_tableau::get_highlighted()
{
  return highlighted;
}

vgui_soview* bgui_displaybase_tableau::get_highlighted_soview()
{
    return vgui_soview::id_to_object(highlighted);
}

bool bgui_displaybase_tableau::highlight(unsigned id)
{
  highlighted = id;
  return true;
}

vgui_soview* bgui_displaybase_tableau::contains_hit(vcl_vector<unsigned> names)
{
       ...
       i != objects.end() ; ++i)
  {
    ...
    //vcl_cerr << "bgui_displaybase_tableau::contains_hit soview_id" << soview_id << vcl_endl;
