// This is core/vgui/bgui_easy2D_tableau.cxx
#ifdef VCL_NEEDS_PRAGMA_INTERFACE
#pragma implementation
#endif
//:
// \file
// \brief  See bgui_easy2D_tableau.h for a description of this file.
...
#include <bgui/bgui_displaylist2D_tableau.h>
...
bgui_easy2D_tableau::bgui_easy2D_tableau(const char* n):
  image_slot(this),
  name_(n)
{
  fg[0] = 1.0f;
...
bgui_easy2D_tableau::bgui_easy2D_tableau(vgui_image_tableau_sptr const& i, const char* n) :
  image_slot(this,i),
  image_image(i),
  name_(n)
{
...
bgui_easy2D_tableau::bgui_easy2D_tableau(vgui_tableau_sptr const& i, const char* n) :
  image_slot(this,i),
  name_(n)
{
  fg[0] = 1.0f;
...
bool bgui_easy2D_tableau::handle(vgui_event const& e) {
  if (image_slot) {
  ...
  return bgui_displaylist2D_tableau::handle(e);
}

vcl_string bgui_easy2D_tableau::file_name() const {
  if (image_slot)
...
vcl_string bgui_easy2D_tableau::pretty_name() const {
  if (image_slot)
    return type_name() + "[" + name_ + ",i=" + image_slot->file_name() + "]";
  else
...
vcl_string bgui_easy2D_tableau::type_name() const {
  return "bgui_easy2D_tableau";
}

//: Set the child tableau to be the given image_tableau.
void bgui_easy2D_tableau::set_image(vcl_string const& fn)
{
  image_image->set_image(fn.c_str());
}

//: Set the child tableau to be the given tableau.
void bgui_easy2D_tableau::set_child(vgui_tableau_sptr const& i) {
...
vgui_soview2D_lineseg* bgui_easy2D_tableau::add_line(float x0, float y0, float x1, float y1) {
...
vgui_soview2D_infinite_line* bgui_easy2D_tableau::add_infinite_line(float a, float b, float c) {
...
vgui_soview2D_circle* bgui_easy2D_tableau::add_circle(float x, float y, float r) {
...
vgui_soview2D_ellipse* bgui_easy2D_tableau::add_ellipse(float x, float y, float w, float h, float phi){
...
vgui_soview2D_point* bgui_easy2D_tableau::add_point_3dv(double const p[3]) {
  return add_point(p[0]/p[2], p[1]/p[2]);
}

//: Add a line with the given projective start and end points.
vgui_soview2D_lineseg* bgui_easy2D_tableau::add_line_3dv_3dv(double const p[3], double const q[3]) {
  return add_line(p[0]/p[2], p[1]/p[2],
                  q[0]/q[2], q[1]/q[2]);
}

//: Add an infinite line with the given projective coordinates.
vgui_soview2D_infinite_line* bgui_easy2D_tableau::add_infinite_line_3dv(double const l[3]) {
  return add_infinite_line(l[0], l[1], l[2]);
}

//: Add a circle with the given centre (in projective coords) and radius to the display.
vgui_soview2D_circle* bgui_easy2D_tableau::add_circle_3dv(double const point[3], float r) {
  return add_circle(point[0]/point[2], point[1]/point[2], r);
}

//: Add a linestrip with the given n vertices to the display.
vgui_soview2D_linestrip* bgui_easy2D_tableau::add_linestrip(unsigned n, float const *x, float const *y) {
...
vgui_soview2D_polygon* bgui_easy2D_tableau::add_polygon(unsigned n, float const *x, float const *y) {
...
void bgui_easy2D_tableau::print_psfile(vcl_string filename, int reduction_factor, bool print_geom_objs, int wd, int ht)
{
  ...
  if (get_image_tableau() && wd*ht > 0)
  {
    ...
    {
      if (debug)
        vcl_cerr << "bgui_easy2D_tableau::print_psfile printing greyscale image to"
    ...
    {
      if (debug)
        vcl_cerr << "bgui_easy2D_tableau::print_psfile printing color image to "
                 << filename.c_str() << vcl_endl;
  ...
  if (debug)
    vcl_cerr << "bgui_easy2D_tableau: Printing geometric objects\n";
  ...
  for (vcl_vector<vgui_soview*>::iterator i = all_objs.begin(); i != all_objs.end(); ++i)
  {
    ...
    if (sv->type_name() == "vgui_soview2D_point")
    {
      ...
      if (debug)
        vcl_cerr << "  bgui_easy2D_tableau: Adding a point at "
                 << pt->x << ", " << pt->y << vcl_endl;
    }
    ...
    else if (sv->type_name() == "vgui_soview2D_circle")
    {
      ...
      if (debug)
        vcl_cerr << "  bgui_easy2D_tableau: Adding circle, center " << circ->x << ", "
    ...
    else if (sv->type_name() == "vgui_soview2D_lineseg")
    {
      ...
      if (debug)
        vcl_cerr << " bgui_easy2D_tableau: Adding line between " << line->x0 << ", "
    ...
    else if (sv->type_name() == "vgui_soview2D_linestrip")
    {
      ...
      if (debug)
        vcl_cerr<< " bgui_easy2D_tableau: Adding linestrip \n";
    }
    else if (sv->type_name() == "vgui_soview2D_polygon")
    {
      ...
      if (debug)
        vcl_cerr<< " bgui_easy2D_tableau: Adding polygon \n";
    }
    else
