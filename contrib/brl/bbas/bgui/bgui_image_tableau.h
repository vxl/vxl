...
public:
  ...
  void image_line(const float col_start, const float row_start, const float col_end, const float row_end,
                  vcl_vector<double>& line_pos, vcl_vector<double>& vals);

  // : Extract a line of pixel values return color if available
  void image_line(const float col_start, const float row_start, const float col_end, const float row_end,
                  vcl_vector<double>& line_pos, vcl_vector<vcl_vector<double> >& vals);
  ...
  bool mouse_pos(float& mouse_u, float& mouse_v)
  {
    mouse_u = mouse_pos_[0],
