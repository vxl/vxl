...
public:
  ...
  bgui_vtol2D_tableau(vgui_image_tableau_sptr const& it, const char* n = "unnamed");

  bgui_vtol2D_tableau(vgui_tableau_sptr const& t, const char* n = "unnamed");
  ...
  bgui_vtol_soview2D_vertex * add_vertex(vtol_vertex_2d_sptr const& v, const vgui_style_sptr& style = NULL);

  bgui_vtol_soview2D_edge * add_edge(vtol_edge_2d_sptr const& e, const vgui_style_sptr& style = NULL);

  bgui_vtol_soview2D_edge_group * add_edge_group(vcl_vector<vtol_edge_2d_sptr>& edges,
                                                 const vgui_style_sptr& style = NULL);

  bgui_vtol_soview2D_face * add_face(vtol_face_2d_sptr const& f, const vgui_style_sptr& style = NULL);

  // : display methods for vectors of topology classes (not grouped)
  void add_topology_object(vtol_topology_object_sptr const& tos, const vgui_style_sptr& style = NULL);

  void add_topology_objects(vcl_vector<vtol_topology_object_sptr> const& tos, const vgui_style_sptr& style = NULL);

  void add_edges(vcl_vector<vtol_edge_2d_sptr> const & edges, bool verts = false, const vgui_style_sptr& style = NULL);

  void add_faces(vcl_vector<vtol_face_2d_sptr> const & faces, bool verts = false, const vgui_style_sptr& style = NULL);
  ...
  void set_vtol_topology_object_style(vtol_topology_object_sptr tos, const vgui_style_sptr& style = NULL);
