...
void buildScene(SoGroup *root)
{
  ...
  for (int i=0; i<4; ++i){
    ...
    int coord_num = 0;
    for ( double t=0.0; t<=1.0; t+=0.001, ++coord_num) {  
      line_coords->point.set1Value(coord_num, i*vcl_sin(t*10*(5-i)), i*vcl_cos(t*10*(5-i)), 4*t+i);
      vertex_material->diffuseColor.set1Value(coord_num, double(i)/4.0 , 1.0 - t, t);
    } 
