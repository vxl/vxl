...
double bil_chi_sq_dist(const bil_bin hist1[], const bil_bin hist2[])
{
  ...
  for( int i = 0; i < NBINS; ++i )
    {
    dist += (hist1[i].weight - hist2[i].weight) * (hist1[i].weight - hist2[i].weight)
      / (hist1[i].weight + hist2[i].weight + 2e-16);
    }
...
double bil_gray_EMD(const bil_bin dirt[], const bil_bin hole[])
{
  ...
  while( true )
    ...
    if( leftoverdirt == 0.0 )
      ...
      ++i;
      while( dirt[i].weight == 0.0 && i < NBINS )
        {
        ++i;
        }
    ...
    if( leftoverhole == 0.0 )
      ...
      ++j;
      while( hole[j].weight == 0.0 && j < NBINS )
        {
        ++j;
        }
