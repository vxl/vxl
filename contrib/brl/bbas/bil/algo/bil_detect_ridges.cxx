...
for(unsigned j =0; j < nj; j++) {
  for(unsigned i =0; i < ni; i++) {
    ...
    Ix_plus_e(i,j)  = vil_bilin_interp_safe(x_ind_p , y_ind_p , xdata , ni , nj , x_istep , x_jstep);
    Iy_plus_e(i,j)  = vil_bilin_interp_safe(x_ind_p , y_ind_p , ydata , ni , nj , y_istep , y_jstep);
    Ix_minus_e(i,j) = vil_bilin_interp_safe(x_ind_m , y_ind_m , xdata , ni , nj , x_istep , x_jstep);
    Iy_minus_e(i,j) = vil_bilin_interp_safe(x_ind_m , y_ind_m , ydata , ni , nj , y_istep , y_jstep);
  }
}
