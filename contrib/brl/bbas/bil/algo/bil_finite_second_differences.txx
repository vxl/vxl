...
#define IMVAL(im, i, j, p, ni, \
              nj) (im(static_cast<int>(i) < 0 ? 0 : (i >= ni ? ni - 1 : i), \
                      static_cast<int>(j) < 0 ? 0 : (j >= nj ? nj - 1 : j), \
                      p) )
#define DX 1
#define DY 1
template <class T>
    ...
    for( int j = 0; j < static_cast<int>(nj); ++j )
      ...
      for( int i = 0; i < static_cast<int>(ni); ++i )
        ...
        dxy(i, j, p) = ( IMVAL(src_im, i + 1, j + 1, p, ni, nj) + IMVAL(src_im, i - 1, j - 1, p, ni, nj)
                         - IMVAL(src_im, i - 1, j + 1, p, ni,
                                 nj) - IMVAL(src_im, i + 1, j - 1, p, ni, nj) ) / (4 * DX * DY);
        }
      }
    }
}
