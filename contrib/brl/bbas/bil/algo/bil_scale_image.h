...
public:
  // : Constructor
  bil_scale_image(unsigned int num_levels = 3, unsigned int num_octaves = 0, float init_scale = 1.0f,
                  int first_octave = 0);

  // : Constructor - Make a Gaussian scale space from this image
  bil_scale_image(const vil_image_view<T>& image, unsigned int num_levels = 3, unsigned int num_octaves = 0,
  ...
  vil_image_view<T> neighbors(int octave, unsigned int level, unsigned int i, unsigned int j) const;
