...
// : Constructor
bil_raw_image_istream::bil_raw_image_istream()
  : index_(INIT_INDEX),
  ni_(0), nj_(0),
...
// : Constructor
bil_raw_image_istream::bil_raw_image_istream(const vcl_string& glob)
  : index_(INIT_INDEX),
  ni_(0), nj_(0),
  format_(VIL_PIXEL_FORMAT_UNKNOWN),
  current_frame_(NULL),
...
bool
bil_raw_image_istream::open(const vcl_string& rawFile)
{
...
bil_raw_image_istream::current_frame()
{
  ...
  if( is_valid() )
    ...
    if( !current_frame_ )
      ...
      vcl_cout << "The location is " << loc << " and image size is " << ni_ << " " << nj_ << " " << pixel_size_
               << vcl_endl;
      ...
      if( pixel_size_ == 24 )
        ...
        current_frame_ = new vil_image_view<vxl_byte>(mem_chunk,
                                                      (vxl_byte *) mem_chunk->data(), ni_, nj_, 3, 3, 3 * ni_, 1);
        }
      else if( pixel_size_ == 8 )
        {
        raw_.read( (char *) mem_chunk->data(), imgSize );
        current_frame_ = new vil_image_view<vxl_byte>(mem_chunk,
                                                      (vxl_byte *) mem_chunk->data(), ni_, nj_, 1, 1, ni_, ni_ * nj_);
        }
      else if( pixel_size_ == 16 )
        {
        raw_.read( (char *) mem_chunk->data(), imgSize );
        current_frame_ = new vil_image_view<unsigned short>(mem_chunk,
                                                            (unsigned short *) mem_chunk->data(), ni_, nj_, 1, 1, ni_,
                                                            ni_ * nj_);
        }
      //  vj Modification
      // read timestamp
...
bool
bil_raw_image_istream::seek_frame(unsigned int frame_nr)
{
