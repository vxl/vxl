...
public:
  ...
  bmsh3d_edge * find_other_E(const bmsh3d_vertex* inputV, const bmsh3d_edge* inputE) const;

  // : Given a vertex V and a halfedge of this face incident to V, find the other halfedge of this face incident to V.
  bmsh3d_halfedge * find_other_HE(const bmsh3d_vertex* inputV, const bmsh3d_halfedge* inputHE) const;
  ...
  bmsh3d_edge * find_next_E(const bmsh3d_vertex* inputV, const bmsh3d_edge* inputE) const;
  ...
  bmsh3d_halfedge * find_next_HE(const bmsh3d_vertex* inputV, const bmsh3d_halfedge* inputHE) const;
  ...
  void set_orientation(bmsh3d_halfedge* new_start_he, bmsh3d_vertex*   new_next_v);
...
void _delete_HE_chain(bmsh3d_halfedge * & he_head, vcl_vector<bmsh3d_edge *>& incident_edge_list);

// : Given the face, current halfedge, and current eV, find the next halfedge given in the vector<>.
bmsh3d_halfedge * _find_next_halfedge(bmsh3d_halfedge* HE, bmsh3d_vertex* eV, vcl_vector<bmsh3d_halfedge *>& inc_hes);

// : Assume the mesh face is planar and compute a 2D planar coordinate for it.
void get_2d_coord(const vcl_vector<bmsh3d_vertex *>& vertices, vgl_vector_3d<double>& N, vgl_vector_3d<double>& AX,
                  vgl_vector_3d<double>& AY);

// : Return ordered set of vertices in 2D (x,y) coord.
void get_2d_polygon(const vcl_vector<bmsh3d_vertex *>& vertices, vcl_vector<double>& xs, vcl_vector<double>& ys);
...
vgl_point_2d<double> get_2d_proj_pt(vgl_point_3d<double> P, const vgl_point_3d<double>& A,
                                    const vgl_vector_3d<double>& AX, const vgl_vector_3d<double>& AY);
...
vgl_vector_3d<double> compute_normal(const vgl_point_3d<double>& C, const bmsh3d_edge* E, const bmsh3d_vertex* Es);
