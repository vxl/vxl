...
bmsh3d_mesh_mc::bmsh3d_mesh_mc(bmsh3d_mesh* mesh) : bmsh3d_mesh()
{
  ...
  for( v_it = vertices.begin(); v_it != vertices.end(); v_it++ )
    ...
    bmsh3d_vertex* new_vertex = new bmsh3d_vertex(vertex->get_pt().x(), vertex->get_pt().y(),
                                                  vertex->get_pt().z(), vertex->id() );
                     ...
                     bmsh3d_face_mc * & F1, bmsh3d_face_mc * & F2)
{
  ...
  for( ; index != E2_index; )
    ...
    vcl_cout << inc_edges[index]->id() << "from " << inc_edges[index]->sV()->id() << " to"
             << inc_edges[index]->eV()->id() << vcl_endl;
    if( ++index == inc_edges.size() )
      {
      index = 0;
      }
