// This is basic/bnl/algo/bnl_sparse_lm.cxx
...
// ctor
void bnl_sparse_lm::init(bnl_sparse_lst_sqr_function* f)
{
  f_ = f;

  // If changing these defaults, check the help comments in bnl_sparse_lm.h,
...
bool bnl_sparse_lm::minimize(vnl_vector<double>& a, vnl_vector<double>& b, bool use_gradient)
{
  ...
  if (size_a+size_b > size_e) {
    vcl_cerr << "bnl_sparse_lm: Number of unknowns("<<size_a+size_b<<") greater than number of data ("<<size_e<<")\n";
    failure_code_ = ERROR_DODGY_INPUT;
    return false;
  }

  if (int(a.size()) != size_a) {
    vcl_cerr << "bnl_sparse_lm: Input vector \"a\" length ("<<a.size()<<") not equal to num parameters in \"a\" ("<<size_a<<")\n";
    failure_code_ = ERROR_DODGY_INPUT;
    return false;
  }

  if (int(b.size()) != size_b) {
    vcl_cerr << "bnl_sparse_lm: Input vector \"b\" length ("<<b.size()<<") not equal to num parameters in \"b\" ("<<size_b<<")\n";
    failure_code_ = ERROR_DODGY_INPUT;
    return false;
  }
  ...
  const bnl_crs_index& crs = f_->residual_indices(); 
  ...
  for(num_iterations_=0; num_iterations_<(unsigned int)maxfev && !stop; ++num_iterations_)
  {
    ...
    while(true)
    {
      ...
      for(unsigned int j=0; j<f_->number_of_b(); ++j){
        ...
        bnl_crs_index::sparse_vector col = crs.sparse_col(j);
      ...
      for(unsigned int i=0; i<f_->number_of_a(); ++i){
        ...
        bnl_crs_index::sparse_vector row_i = crs.sparse_row(i);
        ...
        for(unsigned int h=i+1; h<f_->number_of_a(); ++h){
          bnl_crs_index::sparse_vector row_h = crs.sparse_row(h);
      ...
      // check for convergence in parameters (change in parameters is below a tolerence)
...
void bnl_sparse_lm::diagnose_outcome() const
{
...
void bnl_sparse_lm::diagnose_outcome(vcl_ostream& s) const
{
#define whoami "bnl_sparse_lm"
  //if (!verbose_) return;
...
vnl_matrix<double> const& bnl_sparse_lm::get_JtJ()
{
  return inv_covar_;
}
