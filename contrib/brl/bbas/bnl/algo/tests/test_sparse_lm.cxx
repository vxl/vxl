...
class bundle_2d : public bnl_sparse_lst_sqr_function
{
 public:
  bundle_2d(unsigned int num_cam, unsigned int num_pts,
   ...
   : bnl_sparse_lst_sqr_function(num_cam,2,num_pts,2,xmask,1,g), data_(data) {}
...
MAIN( test_sparse_lm )
{
   ...
   bundle_2d gen_func(3,20,proj,mask,bnl_sparse_lst_sqr_function::use_gradient);
   gen_func.f(a,b,proj);
   ...
   // test 2D bundle adjustment with all data and no noise
   {
     ...
     bundle_2d my_func(3,20,proj,mask,bnl_sparse_lst_sqr_function::use_gradient);

     bnl_sparse_lm slm(my_func);
     slm.minimize(pa,pb);
   ...
   // test 2D bundle adjustment with missing data and no noise
   {
     // Start all cameras at (0,0) and all points at (1,1)
     vnl_vector<double> pa(6,0.0), pb(40,1.0);
     bundle_2d my_func(3,20,proj2,mask,bnl_sparse_lst_sqr_function::use_gradient);
   ...
   // add uniform random noise to each measurment
