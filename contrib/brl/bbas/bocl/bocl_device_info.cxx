...
bocl_device_info::bocl_device_info(cl_device_id* device)
{
  ...
  status = clGetDeviceInfo(*device_, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(vcl_size_t), (void *)&max_work_group_size_,
                           NULL);
  ...
  status = clGetDeviceInfo(*device_, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(cl_uint),  (void *)&max_dimensions_,
                           NULL);
  ...
  status = clGetDeviceInfo(*device_, CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(vcl_size_t) * 3,
                           (void *)max_work_item_sizes_, NULL);
  ...
  status =
    clGetDeviceInfo(*device_, CL_DEVICE_MAX_MEM_ALLOC_SIZE, sizeof(cl_ulong), (void *)&max_mem_alloc_size_, NULL);
  ...
  status =
    clGetDeviceInfo(*device_, CL_DEVICE_MAX_PARAMETER_SIZE, sizeof(cl_ulong), (void *)&max_parameter_size_, NULL);
  ...
  status = clGetDeviceInfo(*device_, CL_DEVICE_PREFERRED_VECTOR_WIDTH_SHORT, sizeof(cl_uint),
                           (void *)&vector_width_short_, NULL);
  ...
  status = clGetDeviceInfo(*device_, CL_DEVICE_PREFERRED_VECTOR_WIDTH_FLOAT, sizeof(cl_uint),
                           (void *)&vector_width_float_, NULL);
  ...
  status =
    clGetDeviceInfo(*device_, CL_DEVICE_IMAGE2D_MAX_WIDTH, sizeof(vcl_size_t), (void *)&image2d_max_width_, NULL);
  if( !check_val(status, CL_SUCCESS, "clGetDeviceInfo CL_DEVICE_IMAGE2D_MAX_WIDTH failed.") )
    {
    return;
    }
  // get device image2d max height
  status = clGetDeviceInfo(*device_, CL_DEVICE_IMAGE2D_MAX_HEIGHT, sizeof(vcl_size_t), (void *)&image2d_max_height_,
                           NULL);
  ...
  status =
    clGetDeviceInfo(*device_, CL_DEVICE_IMAGE3D_MAX_WIDTH, sizeof(vcl_size_t), (void *)&image3d_max_width_, NULL);
  if( !check_val(status, CL_SUCCESS, "clGetDeviceInfo CL_DEVICE_IMAGE3D_MAX_WIDTH failed.") )
    {
    return;
    }
  // get device image3d max height
  status = clGetDeviceInfo(*device_, CL_DEVICE_IMAGE3D_MAX_HEIGHT, sizeof(vcl_size_t), (void *)&image3d_max_height_,
                           NULL);
  ...
  // get device image3d max width
  status =
    clGetDeviceInfo(*device_, CL_DEVICE_IMAGE3D_MAX_DEPTH, sizeof(vcl_size_t), (void *)&image3d_max_depth_, NULL);
...
vcl_ostream & operator <<(vcl_ostream & s, bocl_device_info& info)
{
  ...
  s << " Number of compute units: " << info.max_compute_units_ << '\n'
    ...
    << " Max 3D image dim: (" << info.image3d_max_width_ << ',' << info.image3d_max_height_ << ','
    << info.image3d_max_depth_ << ")\n"
  ...
  if( info.is_nvidia_device_ )
    ...
    s << " NVIDIA Specific Device Properties: \n"
      << "   CL_DEVICE_COMPUTE_CAPABILITY_NV: " << info.compute_capability_major_ << "."
      << info.compute_capability_minor_ << '\n'
      ...
      << ( (info.integrated_memory_ == CL_TRUE) ? "CL_TRUE" : "CL_FALSE") << '\n'
    ;
    }
