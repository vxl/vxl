                          ...
                          vcl_size_t* global_offsets)
{
  ...
  status = clEnqueueNDRangeKernel(cmd_queue, kernel_, dim, global_offsets, global_threads, local_threads, 0, NULL,
                                  &ceEvent_);
...
{
  unsigned long size;
  int           status = clGetKernelWorkGroupInfo(kernel_, (*device_), CL_KERNEL_LOCAL_MEM_SIZE, sizeof(size),
                                                  (void *) &size, NULL);
...
{
  vcl_size_t size;
  int        status = clGetKernelWorkGroupInfo(kernel_, (*device_), CL_KERNEL_WORK_GROUP_SIZE, sizeof(size),
                                               (void *) &size, NULL);

  if( !check_val(status, CL_SUCCESS,
                 "bocl_kernel::private_mem_size()  failed (" + id_ + ") " + error_to_string(status) ) )
    {
    return 0;
    }
  else
...
vcl_string bocl_kernel::program_binaries()
{
  ...
  if( !check_val(status, CL_SUCCESS,
                 "bocl_kernel::program_binaries() numDevices failed (" + id_ + ") " + error_to_string(status) ) )
    {
    return "";
    }
  ...
  if( !check_val(status, CL_SUCCESS,
                 "bocl_kernel::program_binaries() binary_sizes failed (" + id_ + ") " + error_to_string(status) ) )
    {
    return "";
    }
  ...
  if( !check_val(status, CL_SUCCESS,
                 "bocl_kernel::program_binaries()  failed (" + id_ + ") " + error_to_string(status) ) )
    {
    return "";
    }
