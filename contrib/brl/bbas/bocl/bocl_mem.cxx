...
bool bocl_mem::create_buffer(const cl_mem_flags& flags)
{
  ...
  if( !check_val(status, MEM_FAILURE,
                 "clCreateBuffer failed: " + this->id_  + ", with status: " + error_to_string(status) ) )
    {
    return MEM_FAILURE;
    }
  return MEM_SUCCESS;
...
bool bocl_mem::init_gpu_buffer(void const* init_val, vcl_size_t value_size, cl_command_queue& cmd_queue)
{
  ...
  delete[] init_buff;
  if( !check_val(status, MEM_FAILURE,
                 "clEnqueueWriteBuffer (INIT BUFFER) failed: " + this->id_ + error_to_string(status) ) )
    {
    return MEM_FAILURE;
    }
  return MEM_SUCCESS;
