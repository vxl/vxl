...
acal_planar_feature_matcher::load_solved_images(
    std::map<size_t, std::string>& solved_image_names,
    std::string const& img_cam_dir,
    std::string const& ext)
{
  if(solved_image_names.size() == 0){
    std::cout << "No solved images to match" << std::endl;
    return false;
  }
  for(std::map<size_t, std::string>::iterator iit = solved_image_names.begin();
      iit != solved_image_names.end(); ++iit)
  {
    size_t idx = iit->first;
    std::string ipath = img_cam_dir + iit->second + ext;
    vil_image_view<vxl_byte> view = vil_load(ipath.c_str());
    if(view.ni() == 0){
      std::cout << "Couldn't load image from " << ipath << std::endl;
      return false;
    }
    solved_inames_[idx]=iit->second;
    solved_images_[idx]=view;
  }
  return true;
}
...
acal_planar_feature_matcher::load_solved_affine_cams(
    std::string const& img_cam_dir,
    std::string const& corrected_affine_cam_postfix)
{
  for(std::map<size_t, std::string>::iterator sit = solved_inames_.begin();
      sit != solved_inames_.end(); ++sit)
  {
    std::string affine_cam_path = img_cam_dir + sit->second + corrected_affine_cam_postfix;
...
{
  double r = params_.patch_radius_;
  double s = params_.pt_spacing_;
...
{
  size_t n = track_3d_pts_.size();
  if(n == 0){
    std::cout << "no focus 3-d points" << std::endl;
    return false;
  }
  planar_patches_.resize(n);
  double r = params_.patch_radius_;
  double s = params_.pt_spacing_;
  for(size_t i = 0; i<n; ++i)
  {
    std::map<size_t, vil_image_view<vxl_byte> > patches;
    const vgl_point_3d<double>& p3d = track_3d_pts_[i];
    size_t nc = track_cams_.size();
    for(size_t c = 0; c<nc; ++c)
    {
      size_t cidx = track_cams_[c];
      if(solved_images_.count(cidx) == 0 || solved_cams_.count(cidx) == 0){
        std::cout << "camera index " << cidx << " not in solved images or cameras" << std::endl;
        return false;
      }
      const vil_image_view<vxl_byte>& img = solved_images_[cidx];
      const vpgl_affine_camera<double>& cam = solved_cams_[cidx];
      vil_image_view<vxl_byte> patch;
      this->backproject_patch(img, cam, p3d, patch);
      patches[cidx]=patch;
    }
    planar_patches_[i]=patches;
  }
  return true;
}
...
acal_planar_feature_matcher::save_planar_patches(std::string const& patch_dir)
{
  ...
  std::map<size_t, vil_image_view<vxl_byte> >& patches = planar_patches_[0];
  for(std::map<size_t, vil_image_view<vxl_byte> >::iterator pit = patches.begin();
        pit != patches.end(); ++pit)
  {
    size_t pidx = pit->first;
    if(solved_inames_.count(pidx) == 0){
      std::cout << "image id " << pidx << " doesn't exist in solved inames" << std::endl;
      return false;
    }
    std::string iname = solved_inames_[pidx];
    std::string idir = patch_dir + iname + "/";
  ...
  for(size_t i = 0; i<n; ++i)
  {
    std::map<size_t, vil_image_view<vxl_byte> >& temp = planar_patches_[i];
    ...
    vpgl_affine_camera<double> const& cam)
{
  ...
  cam_translations_.clear();
  int ni = img.ni(), nj = img.nj();
  double coarse_r = static_cast<double>(params_.coarse_search_radius_);
  double  fine_r = static_cast<double>(params_.fine_search_radius_);
  double coarse_inc = static_cast<double>(params_.coarse_search_increment_);
  double fine_inc = static_cast<double>(params_.fine_search_increment_);
  for(size_t i = 0; i<n; ++i)
  {
    std::map<size_t, vil_image_view<vxl_byte> >& temp = planar_patches_[i];
    const vgl_point_3d<double>& p3d = track_3d_pts_[i];
...
acal_planar_feature_matcher::compute_scores_stats_median_trans()
{
  ...
  for(std::vector<std::map<size_t, std::pair<vgl_vector_2d<double>, double> > > ::iterator sit = cam_translations_.begin();
      sit != cam_translations_.end(); ++sit)
  {
  ...
  if(n == 1){
    median_translation_.set(u_trans[0], v_trans[0]);
    match_score_stats_.median_score_ = scores[0];
    return;
  }

  size_t mid = n/2;
  median_translation_.set(u_trans[mid], v_trans[mid]);
  match_score_stats_.median_score_ = scores[mid];

  match_score_stats_.min_score_ = min_score;   match_score_stats_.max_score_ = max_score;
  match_score_stats_.mean_score_ = mean_score; match_score_stats_.score_std_dev_ = score_std_dev;
}
...
{
  for(std::vector<std::map<size_t, std::pair<vgl_vector_2d<double>, double> > >::iterator trit =  cam_translations_.begin();
      trit != cam_translations_.end(); ++trit)
  {
