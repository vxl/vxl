...
// : Constructor
bpgl_bundle_rolling_shutter_adj_lsqr::bpgl_bundle_rolling_shutter_adj_lsqr(const vcl_vector<vpgl_calibration_matrix<
                                                                                              double> >& K,
  ...
  // Aij.fill(0.0);
    ...
    double dvddx =
       ...
       * KR(1,
            0) - p[1]
       * KR(2,
            0) ) * v_k / rolling_rate_ / (p[2] * p[2] - p[2] * KRd[1] / rolling_rate_ + p[1] * KRd[2] / rolling_rate_);
    ...
    double dvddy =
       ...
       * KR(1,
            1) - p[1]
       * KR(2,
            1) ) * v_k / rolling_rate_ / (p[2] * p[2] - p[2] * KRd[1] / rolling_rate_ + p[1] * KRd[2] / rolling_rate_);
    ...
    double dvddz =
       ...
       * KR(1,
            2) - p[1]
       * KR(2,
            2) ) * v_k / rolling_rate_ / (p[2] * p[2] - p[2] * KRd[1] / rolling_rate_ + p[1] * KRd[2] / rolling_rate_);
    ...
    if( om != 0.0 )
      ...
      drdox = domdox * t * vcl_sin(om * t) / (om * om) * oc2 -   domdox * vcl_sin(om * t) / (om * om) * oc
        + vcl_sin(om * t) / om * docdox
        + domdox * t * vcl_cos(om * t) / om * oc       - 2 * domdox * (1 - vcl_cos(om * t) ) / (om * om * om) * oc2
        + (1 - vcl_cos(om * t) ) / (om * om) * doc2dox;

      drdoy = domdoy * t * vcl_sin(om * t) / (om * om) * oc2 -   domdoy * vcl_sin(om * t) / (om * om) * oc + vcl_sin(
          om * t) / om * docdoy
        + domdoy * t * vcl_cos(om * t) / om * oc       - 2 * domdoy * (1 - vcl_cos(om * t) ) / (om * om * om) * oc2
        + (1 - vcl_cos(om * t) ) / (om * om) * doc2doy;

      drdoz = domdoz * t * vcl_sin(om * t) / (om * om) * oc2 -   domdoz * vcl_sin(om * t) / (om * om) * oc + vcl_sin(
          om * t) / om * docdoz
        + domdoz * t * vcl_cos(om * t) / om * oc       - 2 * domdoz * (1 - vcl_cos(om * t) ) / (om * om * om) * oc2
        + (1 - vcl_cos(om * t) ) / (om * om) * doc2doz;

      dddox = -3 * domdox * (vcl_sin(om * t) + om * t) / (om * om * om * om) * (oc2 * d) + (vcl_sin(om * t) + om * t)
        / (om * om * om) * (doc2dox * d)
        - domdox * t * (vcl_sin(om * t) ) / (om * om) * oc * d + (domdox * t * vcl_cos(om * t) + domdox * t)
        * (oc2 * d) / (om * om * om)
        + 2 * domdox * (1 - vcl_cos(om * t) ) / (om * om * om) * (oc * d) - (1 - vcl_cos(om * t) ) / (om * om)
        * docdox * d;

      dddoy = -3 * domdoy * (vcl_sin(om * t) + om * t) / (om * om * om * om) * (oc2 * d) + (vcl_sin(om * t) + om * t)
        / (om * om * om) * (doc2doy * d)
        - domdoy * t * (vcl_sin(om * t) ) / (om * om) * oc * d + (domdoy * t * vcl_cos(om * t) + domdoy * t)
        * (oc2 * d) / (om * om * om)
        + 2 * domdoy * (1 - vcl_cos(om * t) ) / (om * om * om) * (oc * d) - (1 - vcl_cos(om * t) ) / (om * om)
        * docdoy * d;

      dddoz = -3 * domdoz * (vcl_sin(om * t) + om * t) / (om * om * om * om) * (oc2 * d) + (vcl_sin(om * t) + om * t)
        / (om * om * om) * (doc2doz * d)
        - domdoz * t * (vcl_sin(om * t) ) / (om * om) * oc * d + (domdoz * t * vcl_cos(om * t) + domdoz * t)
        * (oc2 * d) / (om * om * om)
        + 2 * domdoz * (1 - vcl_cos(om * t) ) / (om * om * om) * (oc * d) - (1 - vcl_cos(om * t) ) / (om * om)
        * docdoz * d;
      }
    ...
    if( om == 0.0 )
      ...
      {
      dtddx = (vcl_sin(om * t) + om * t) / (om * om * om) * oc2 * ddddx - (1 - vcl_cos(om * t) ) / (om * om)
        * (oc * ddddx) + ddddx * t;
      dtddy = (vcl_sin(om * t) + om * t) / (om * om * om) * oc2 * ddddy - (1 - vcl_cos(om * t) ) / (om * om)
        * (oc * ddddy) + ddddy * t;
      dtddz = (vcl_sin(om * t) + om * t) / (om * om * om) * oc2 * ddddz - (1 - vcl_cos(om * t) ) / (om * om)
        * (oc * ddddz) + ddddz * t;
      }
