...
public:
  ...
  bpgl_bundle_rolling_shutter_adj_lsqr(const vcl_vector<vpgl_calibration_matrix<double> >& K,
                                       ...
                                       const vcl_vector<vcl_vector<bool> >& mask, double rolling_rate,
  ...
  bpgl_bundle_rolling_shutter_adj_lsqr(const vcl_vector<vpgl_calibration_matrix<double> >& K,
                                       ...
                                       const vcl_vector<vcl_vector<bool> >& mask, bool use_confidence_weights = true);
  ...
  virtual void jac_blocks(vnl_vector<double> const& a, vnl_vector<double> const& b, vcl_vector<vnl_matrix<double> >& A,
                          vcl_vector<vnl_matrix<double> >& B);

  // : compute the Jacobian Aij
  void jac_Aij(vnl_double_3x4 const& Pi, vnl_double_3x3 const& K, vnl_vector<double> const& ai,
               vnl_vector<double> const& bj, double v_k, vnl_matrix<double>& Aij);

  // : compute the Jacobian Bij
  void jac_Bij(vnl_double_3x4  Pi, vnl_vector<double> const& ai, vnl_vector<double> const& bj, double v_k,
...
public:
  ...
  // : Bundle Adjust
  bool optimize(vcl_vector<vpgl_perspective_camera<double> >& cameras, vcl_vector<vnl_vector<double> > & motion,
                double & r, vcl_vector<vgl_point_3d<double> >& world_points,
                const vcl_vector<vgl_point_2d<double> >& image_points, const vcl_vector<vcl_vector<bool> >& mask);

  // : Write cameras and points to a file in VRML 2.0 for debugging
  static void write_vrml(const vcl_string& filename, vcl_vector<vpgl_perspective_camera<double> >& cameras,
