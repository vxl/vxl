...
public:
    ...
    : vnl_cost_function(2), cest_(cest), img_e_(img_e)
  {
    ...
    if( cam->type_name() == "vpgl_perspective_camera" )
      ...
      vpgl_perspective_camera<double> cam_centered(cam_->get_calibration(), vgl_point_3d<double>(0.0, 0.0,
                                                                                                 0.0),
                                                   cam_->get_rotation() );
      double im_center_x = 0.5 * (double)img_e.ni();
      double im_center_y = 0.5 * (double)img_e.nj();

      vgl_line_3d_2_points<double> ln_center =
        cam_centered.backproject(vgl_point_2d<double>(im_center_x, im_center_y) );
      ...
      {
      vcl_cout << "bvxm_expected_edge_functor::apply() -- The camera type: " << cam->type_name()
               << " is not implemented yet!" << vcl_endl;
      }
  }
  ...
  double f(const vnl_vector<double>& x)
  {
    if( cam_ )
      ...
      vpgl_camera_double_sptr curr_cam_sptr = new vpgl_perspective_camera<double>(
          cam_->get_calibration(), curr_center, cam_->get_rotation() );
      vpgl_perspective_camera<double>* curr_cam =
        static_cast<vpgl_perspective_camera<double> *>(curr_cam_sptr.as_pointer() );
      ...
      vcl_cout << "x[0]: " << x[0] << " x[1]: " << x[1] << " ctr: " << curr_center << " score: " << curr_score
