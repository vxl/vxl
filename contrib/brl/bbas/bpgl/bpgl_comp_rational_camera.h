...
public:
  ...
  bpgl_comp_rational_camera(vnl_matrix_fixed<T, 3, 3> const& M, vpgl_rational_camera<T> const& rcam);

  // : Constructor from translation only
  bpgl_comp_rational_camera(const T tu, const T tv, vpgl_rational_camera<T> const& rcam);

  // : Constructor from translation rotation only (first rotate then translate)
  bpgl_comp_rational_camera(const T tu, const T tv, const T angle_in_radians, vpgl_rational_camera<T> const& rcam);

  // : Constructor with all transform parameters (scale,then rotate,then trans)
  // Note, scale factors are non-negative
  bpgl_comp_rational_camera(const T tu, const T tv, const T angle_in_radians, const T su, const T sv,
  ...
  void set_all(const T tu, const T tv, const T angle_in_radians, const T su, const T sv);
