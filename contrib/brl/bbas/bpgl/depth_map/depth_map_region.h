...
{
public:
  enum orientation { HORIZONTAL, FRONT_PARALLEL, SLANTED_RIGHT, SLANTED_LEFT, POROUS, INFINT, NON_PLANAR, GROUND_PLANE,
                     VERTICAL };
  // : default constructor
  depth_map_region();
  // : standard constructor for a plane that can be moved along the camera ray
  depth_map_region(vsol_polygon_2d_sptr const& region, vgl_plane_3d<double> const& region_plane,
                   double const& min_depth, double const& max_depth, vcl_string const& name,
                   depth_map_region::orientation orient, unsigned const& land_id = 0, double const& height = -1.0,
                   bool const& is_ref = false);

  // : constructor for a fixed plane, e.g. the ground plane
  depth_map_region(vsol_polygon_2d_sptr const& region, vgl_plane_3d<double> const& region_plane, vcl_string const& name,
                   depth_map_region::orientation orient, unsigned const& land_id = 0);

  // : constructor for a region of infinite distance
  depth_map_region(vsol_polygon_2d_sptr const& region, vcl_string name);
  ...
  static vsol_polygon_3d_sptr region_2d_to_3d(vsol_polygon_2d_sptr const& region_2d,
                                              vgl_vector_3d<double> const& region_normal, double depth, // plane is moved along ray to depth
  ...
  static vgl_vector_3d<double> perp_ortho_dir(vpgl_perspective_camera<double> const& cam);
  ...
  bool set_ground_plane_max_depth(double max_depth, vpgl_perspective_camera<double> const& cam,
                                  double proximity_scale_factor);

  // : compute the homography between the image and 3d region plane coords
  bool img_to_region_plane(vpgl_perspective_camera<double> const& cam, vgl_h_matrix_2d<double>& H) const;

  // : update the input depth image with *this* region
  // Currently assumes disjoint regions so that sorting on depth
  // is not required.
  bool update_depth_image(vil_image_view<float>& depth_image, vpgl_perspective_camera<double> const& cam,
