...
depth_map_scene::depth_map_scene(unsigned ni, unsigned nj,
...
void depth_map_scene::add_ortho_perp_region(vsol_polygon_2d_sptr const& region,
...
void depth_map_scene::set_ground_plane_max_depth(double max_depth,
...
vil_image_view<float> depth_map_scene::depth_map(unsigned log2_downsample_ratio)
{
...
vil_image_view<float> depth_map_scene::depth_map(vcl_string region_name, unsigned log2_downsample_ratio,
                                                 double gp_dist_cutoff)
{
  double ratio = vcl_pow(2.0, static_cast<double>(log2_downsample_ratio) );
  double dni = static_cast<double>(ni_) / ratio,
    dnj = static_cast<double>(nj_) / ratio;
...
bool depth_map_scene::match(vil_image_view<float> const& depth_img, vil_image_view<float> const& vis_img,
                            unsigned level, float& score)
{
...
#ifdef DEBUG
            vcl_cout << " region: " << scene.scene_regions()[r]->name() << " depth mean: " << mean << " std dev: "
                     << std_dev << " score prev: " << score << vcl_endl;
#endif
  ...
  for( unsigned r = 0; r < scene.scene_regions().size(); r++ )
    {
    vcl_cout << "region: " << scene.scene_regions()[r]->name() << ' ' << region_order[r].first << ' '
             << region_order[r].second << '\n';
    }
#endif
  for( unsigned r = 0; r < this->scene_regions().size(); ++r )
    {
...
bool depth_map_scene::match_with_ground(vil_image_view<float> const& depth_img, vil_image_view<float> const& vis_img,
                                        unsigned level, float ground_depth_std_dev, float& score)
{
  ...
  for( unsigned r = 0; r < this->scene_regions().size(); r++ )
    ...
    if( this->scene_regions()[r]->active() )
      ...
      region_min_depths.push_back(vcl_pair<float, int>(this->scene_regions()[r]->min_depth(),
                                                       this->scene_regions()[r]->order() ) );
#if 0
  ...
  for( unsigned r = 0; r < region_imgs.size(); r++ )
    ...
    float norm_val = 1.0 / (4 * std_dev);
#if 0
    vcl_cout << " region: " << scene.scene_regions()[r]->name() << " depth mean: " << mean << " std dev: " << std_dev
             << " score prev: " << score << vcl_endl;
#endif
    float den = denominator(std_dev);
