...
public:
  ...
  icam_cost_func( const vil_image_view<float>& source_img, const vil_image_view<float>& dest_img,
                  const icam_depth_transform& dt, unsigned nbins = 16);
  ...
  double error(vnl_vector_fixed<double, 3> rodrigues, vgl_vector_3d<double> trans, double min_allowed_overlap = 0.01);

  // : A set of error values for a range of camera parameter values; the \p param_index indicates which parameter is being varied.
  vcl_vector<double> error(vnl_vector<double> const& x, unsigned param_index, double pmin, double pmax, double pinc);

  // : The joint probability histogram for source and destination pixel values for a given camera transformation.
  vbl_array_2d<double> joint_probability(vnl_vector_fixed<double, 3> rodrigues, vgl_vector_3d<double> trans);

  // : Joint probability for a given mapped source image
  vbl_array_2d<double> joint_probability(vil_image_view<float> const& map_dest, vil_image_view<float> const& map_mask);

  // : Mutual information between the destination and mapped source image
  double mutual_info(vnl_vector_fixed<double, 3> rodrigues, vgl_vector_3d<double> trans,
                     double min_allowed_overlap = 0.01);

  // : Mutual information leaving out the destination entropy
  double entropy_diff(vnl_vector_fixed<double, 3> rodrigues, vgl_vector_3d<double> trans,
                      double min_allowed_overlap = 0.01);

  // : Mutual information for a source image mapped to the destination frame
  double mutual_info(vil_image_view<float> const& map_dest, vil_image_view<float> const& map_mask,
                     double min_allowed_overlap = 0.01);
  ...
  void samples(vnl_vector_fixed<double, 3> rodrigues, vgl_vector_3d<double> trans, vnl_vector<double>& trans_source,
               vnl_vector<double>& mask, vnl_vector<double>& dest);

  // : the source image mapped to the destination frame (useful for debug)
  vil_image_view<float> mapped_dest(vnl_vector_fixed<double, 3> rodrigues, vgl_vector_3d<double> trans);
...
protected:
  ...
  unsigned              n_samples_;
  unsigned              nbins_;
  vbl_array_2d<double> joint_probability(vnl_vector<double> const& samples, vnl_vector<double> const& mask);
