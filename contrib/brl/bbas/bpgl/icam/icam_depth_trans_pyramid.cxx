...
unsigned icam_depth_trans_pyramid::required_levels(unsigned ni, unsigned nj, unsigned min_size)
{
  ...
  if( nj < ni ) {m = nj; }
  // required levels
  double   nlevls = 1.0 + vcl_log(mns / m) / vcl_log(0.5);
  unsigned ncheck = static_cast<unsigned>(nlevls);
  double   temp = static_cast<unsigned>(vcl_pow(0.5, double(ncheck - 1) ) * m);
...
vnl_matrix_fixed<double, 3, 3> icam_depth_trans_pyramid::K_to(unsigned level)
{
...
icam_depth_trans_pyramid::icam_depth_trans_pyramid(icam_depth_transform& base_transform,
                                                   unsigned n_levels)
{
  this->init(base_transform, n_levels);
}

icam_depth_trans_pyramid::icam_depth_trans_pyramid(vnl_matrix_fixed<double, 3, 3> const& K,
...
icam_depth_trans_pyramid::icam_depth_trans_pyramid(vnl_matrix_fixed<double, 3, 3> const& K_from,
