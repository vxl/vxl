...
public:
  ...
  icam_depth_trans_pyramid(icam_depth_transform & base_transform, unsigned n_levels);
  icam_depth_trans_pyramid(vnl_matrix_fixed<double, 3, 3> const& K, vil_image_view<double> const& depth,
                           vgl_rotation_3d<double> const& rot, vgl_vector_3d<double> const& trans, unsigned n_levels,
                           bool adjust_to_fl = false);
  icam_depth_trans_pyramid(vnl_matrix_fixed<double, 3, 3> const& K_from, vnl_matrix_fixed<double, 3, 3> const& K_to,
                           vil_image_view<double> const& depth, vgl_rotation_3d<double> const& rot,
                           vgl_vector_3d<double> const& trans, unsigned n_levels, bool adjust_to_fl = false);
  icam_depth_trans_pyramid(vnl_matrix_fixed<double, 3, 3> const& K_from, double to_fl, double to_pu, double to_pv,
                           vil_image_view<double> const& depth, vgl_rotation_3d<double> const& rot,
                           vgl_vector_3d<double> const& trans, unsigned n_levels, bool adjust_to_fl = false);
  ...
  static unsigned required_levels(unsigned ni, unsigned nj, unsigned min_size = 127);
  ...
  icam_depth_transform depth_trans(unsigned level, bool smooth_map = false, double smooth_sigma = 1.0);
  ...
  {return depth_pyramid_(level); }
private:
  void init(icam_depth_transform& base_transform, unsigned n_levels);
