...
public:
  ...
  icam_depth_transform(vnl_matrix_fixed<double, 3, 3> const& K, vil_image_view<double> const& depth,
                       vgl_rotation_3d<double> const& rot, vgl_vector_3d<double> const& trans,
                       bool adjust_to_fl = false);
  // : Construct with separate from and to calibration matrices
  icam_depth_transform(vnl_matrix_fixed<double, 3, 3> const& K_from, vnl_matrix_fixed<double, 3, 3> const& K_to,
                       vil_image_view<double> const& depth, vgl_rotation_3d<double> const& rot,
                       vgl_vector_3d<double> const& trans, bool adjust_to_fl = false);
  // : Construct with separate from and to calibration matrices
  icam_depth_transform(vnl_matrix_fixed<double, 3, 3> const& K_from, double to_fl, double to_pu, double to_pv,
                       vil_image_view<double> const& depth, vgl_rotation_3d<double> const& rot,
                       vgl_vector_3d<double> const& trans, bool adjust_to_fl = false);
...
private:
  ...
  void set_k(vnl_matrix_fixed<double, 3, 3> const& K_from, vnl_matrix_fixed<double, 3, 3> const& K_to);
