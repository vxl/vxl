...
public:
  ...
  icam_minimizer( const vil_image_view<float>& source_img, const vil_image_view<float>& dest_img,
                  const icam_depth_transform& dt, icam_minimizer_params const& params, bool verbose = false);

  // : Constructor, when source image is not known yet
  icam_minimizer(const vil_image_view<float>& dest_img, const icam_depth_transform& dt,
                 icam_minimizer_params const& params, bool verbose = false);
  ...
  double polar_inc(unsigned level, unsigned& nsteps, double polar_range = vnl_math::pi);

  // : principal ray iterator for initialized search
  principal_ray_scan initialized_pray_scan(unsigned initial_level, unsigned search_level, unsigned& n_pts);

  // : polar angle increment for initialized search
  void initialized_polar_inc(unsigned initial_level, unsigned search_level, unsigned& nsteps, double& polar_range,
                             double& polar_inc);

  // : Run the minimization starting at input values
  void minimize(vgl_rotation_3d<double>& rot, vgl_vector_3d<double>& trans);

  // : Run the minimization, adjusting focal length, starting at input values
  void minimize(vgl_rotation_3d<double>& rot, vgl_vector_3d<double>& trans, double to_fl);

  // : Run powell minimization on rotation only using entropy diff for a level
  void minimize_rot(vgl_rotation_3d<double>& rot, vgl_vector_3d<double> const& trans, unsigned level,
  ...
  bool rot_search(vgl_vector_3d<double> const& trans, vgl_rotation_3d<double>& initial_rot, unsigned n_axis_steps,
                  double axis_cone_half_angle, unsigned n_polar_steps, double polar_range, unsigned level,
                  double min_allowed_overlap, vgl_rotation_3d<double>& min_rot, double& min_cost,
  ...
  virtual bool exhaustive_rotation_search(vgl_vector_3d<double> const& trans, unsigned level,
                                          double min_allowed_overlap, vgl_rotation_3d<double>& min_rot,
                                          double& min_cost, double& min_overlap_fraction, bool setup, bool finish);

  // : search for rotation about a given initial value, given the camera translation
  // This virtual method is implemented in both C++ and in OpenCL
  // setup and finish are particular to OpenCL to signal setup and
  // finish of GPU context and buffers
  virtual bool initialized_rot_search(vgl_vector_3d<double> const& trans, vgl_rotation_3d<double>& initial_rot,
                                      unsigned initial_level, unsigned search_level, double min_allowed_overlap,
                                      vgl_rotation_3d<double>& min_rot, double& min_cost, double& min_overlap_fraction,
                                      bool setup, bool finish);

  bool refine_minimum(int mx, int my, int mz, unsigned level, double min_allowed_overlap,
                      vgl_vector_3d<double>& min_trans, double& min_cost);

  bool exhaustive_camera_search(vgl_box_3d<double> const& trans_box, vgl_vector_3d<double> const& trans_steps,
                                unsigned level, double min_allowed_overlap, vgl_vector_3d<double>& min_trans,
                                vgl_rotation_3d<double>& min_rot, double& min_cost, double& min_overlap_fraction);

  bool  pyramid_camera_search(vgl_vector_3d<double> const &
                              start_trans, vgl_rotation_3d<double> const &
                              start_rotation, vgl_vector_3d<double> const &
                              start_step_delta, unsigned start_level, unsigned final_level, double min_allowed_overlap,
                              bool refine, vgl_vector_3d<double>& min_trans, vgl_rotation_3d<double>& min_rot,
                              double& min_cost, double& min_overlap_fraction);

  // : the full search function
  bool camera_search( vgl_box_3d<double> const& trans_box, vgl_vector_3d<double> const& trans_steps,
                      unsigned final_level, double min_allowed_overlap, bool refine, vgl_vector_3d<double>& min_trans,
                      vgl_rotation_3d<double>& min_rot, double& min_error, double& min_overlap);

  // : find the smallest cost in the camera box search that is also a local minimum.
  //  Return false if no local minima occur
  bool smallest_local_minimum(double nbhd_cost_threshold, double& min_cost, vgl_vector_3d<double>& min_trans,
                              vgl_rotation_3d<double>& min_rot, int& ix_min, int& iy_min, int& iz_min);

  // ===============debug functions ===========

  // : the average intensity difference for a given rotation, translation and level
  double error(vgl_rotation_3d<double>& rot, vgl_vector_3d<double>& trans, unsigned level);

  // : the average intensity difference for a given rotation, translation and level over a set of parameter values
  vcl_vector<double> error(vgl_rotation_3d<double>& rot, vgl_vector_3d<double>& trans, unsigned level,
                           unsigned param_index, double pmin, double pmax, double pinc);

  // : source images mapped to destination camera for a set of parameter values
  vcl_vector<vil_image_view<float> > views(vgl_rotation_3d<double>& rot, vgl_vector_3d<double>& trans, unsigned level,
                                           unsigned param_index, double pmin, double pmax, double pinc);

  // :source image mapped to destination by specified source cam
  vil_image_view<float> view(vgl_rotation_3d<double>& rot, vgl_vector_3d<double>& trans, unsigned level);

  // :mask for source image mapped to destination by specified source cam
  vil_image_view<float> mask(vgl_rotation_3d<double>& rot, vgl_vector_3d<double>& trans, unsigned level);
  ...
  bool box_search_vrml(vcl_string const& vrml_file, vgl_vector_3d<double> const& trans =
  ...
  void print_axis_search_info(unsigned level, vgl_rotation_3d<double> const& actual,
                              vgl_rotation_3d<double> const& init = vgl_rotation_3d<double>(), bool top_level = false);

  void print_polar_search_info(unsigned level, vgl_rotation_3d<double> const& actual,
                               vgl_rotation_3d<double> const& init = vgl_rotation_3d<double>(), bool top_level = false);

protected:
  void set_origin_step_delta(vgl_box_3d<double> const& trans_box, vgl_vector_3d<double> const& trans_steps);

  // |<--- k --->|
  // [   |   |   | * |   |   |   ]
  // |dx |
  // |< --initial step delta --->|
  void reduce_search_box(vgl_vector_3d<double> const& center_trans, vgl_vector_3d<double> const& initial_step_delta);
