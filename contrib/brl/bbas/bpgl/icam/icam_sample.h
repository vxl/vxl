...
public:
  ...
  static void sample( unsigned int ni_dest,  unsigned int nj_dest, vil_image_view<float> const& source,
                      icam_depth_transform const& dt, vnl_vector<double>& samples, vnl_vector<double>& mask,
                      unsigned& n_samples);
  ...
  static void resample(unsigned int ni_dest,  unsigned int nj_dest, vil_image_view<float> const& source,
                       icam_depth_transform const& dt, vil_image_view<float>& dest, vil_image_view<float>& mask,
                       unsigned& n_samples);

  // : samples from externally processed images
  static void sample(vil_image_view<float> const& map_dest, vil_image_view<float> const& map_mask,
                     vnl_vector<double>& samples, vnl_vector<double>& mask, unsigned& n_samples);
