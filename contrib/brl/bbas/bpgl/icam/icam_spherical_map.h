...
public:
  ...
  bool closest_camera(vgl_ray_3d<double> const& sph_ray, vgl_point_3d<double> const& p, unsigned& cam_index,
                      double& dot_prod, double& u, double& v) const;
  ...
  bool map_sphere(double angle_exponent = 0.0, bool use_image = true, vxl_byte fg_r = 0, vxl_byte fg_g = 255,
                  vxl_byte fg_b = 0, vxl_byte bk_r = 20, vxl_byte bk_g = 20, vxl_byte bk_b = 20);
  ...
  bool render_map(vil_image_view<vxl_byte> const& backgnd, vpgl_camera_double_sptr const& cam, double theta_off,
                  double phi_off, vil_image_view<vxl_byte>& img, double scale = 1.0, float back_r = 0.0f,
