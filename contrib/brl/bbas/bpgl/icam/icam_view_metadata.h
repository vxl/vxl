...
public:
  ...
  icam_view_metadata(vcl_string const& exp_img, vcl_string const& dt_path);

  virtual ~icam_view_metadata() {}

  virtual void register_image(vil_image_view<float> const& source_img, vpgl_camera_double_sptr camera,
                              icam_minimizer_params const& params);

  virtual void refine_camera(vil_image_view<float> const& source_img, vpgl_camera_double_sptr camera,
                             icam_minimizer_params const& params);

  double cost() { return cost_; }

  // : for debug purposes
  void mapped_image(vil_image_view<float> const& source_img, vpgl_camera_double_sptr camera,
                    vgl_rotation_3d<double>& rot, vgl_vector_3d<double>& trans, unsigned level,
                    icam_minimizer_params const& params, vil_image_view<float>& act_dest,
...
bool load_image(vil_image_view_base_sptr const& base_img, vil_image_view<T> *& image)
{
  if( base_img->pixel_format() == VIL_PIXEL_FORMAT_FLOAT )
    ...
    {
    vcl_cout << "icam_register_image_process -- image type " << base_img->pixel_format() << " is not supported!"
             << vcl_endl;
    return false;
    }
  return true;
