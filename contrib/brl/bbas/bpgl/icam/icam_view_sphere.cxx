  ...
  while( it_imgs != images.end() )
    ...
    if( images[uid].size() > 0 )
      ...
      if( depth_images.find(uid) != depth_images.end() )
        ...
        {
        vcl_cout << "icam_view_sphere::set_images -- ERROR! There is a missing depth image for image id=" << uid
                 << vcl_endl;
        }
      }
    it_imgs++;
                                    ...
                                    icam_minimizer_params const& params)
{
  ...
  if( view_sphere_->view_point(viewpoint_id, vp) )
    ...
    if( data )
      {
      vpgl_perspective_camera<double>* vp_cam =
        static_cast<vpgl_perspective_camera<double> *>(vp->camera().as_pointer() );
