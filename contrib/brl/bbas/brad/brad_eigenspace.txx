...
template <class T>
bool brad_eigenspace<T>::compute_covariance_matrix(vcl_vector<vil_image_resource_sptr> const& rescs)
{
...
template <class T>
bool brad_eigenspace<T>::compute_covariance_matrix_rand(vcl_vector<vil_image_resource_sptr> const& rescs, double frac,
...
template <class T>
bool brad_eigenspace<T>::compute_covariance_matrix_blocked(vcl_vector<vil_image_resource_sptr> const& rescs,
                                                           unsigned nit, unsigned njt)
{
  unsigned n = funct_.size();
...
template <class T>
bool brad_eigenspace<T>::compute_eigensystem()
{
  if( !covar_valid_ )
    {
    return false;
...
template <class T>
bool brad_eigenspace<T>::compute_eigenimage(vil_image_resource_sptr const& resc,
...
template <class T>
bool brad_eigenspace<T>::compute_eigenimage_pixel(vil_image_view<float> const& input,
...
template <class T>
bool brad_eigenspace<T>::classify_image(vil_image_resource_sptr const& resc,
                                        bsta_joint_histogram_3d<float> const& no_atmos,
                                        bsta_joint_histogram_3d<float> const& atmos,
                                        unsigned nit, unsigned njt,
                                        vcl_string const& output_path)
...
template <class T>
bool brad_eigenspace<T>::classify_image_pixel(vil_image_view<float> const& image,
                                              bsta_joint_histogram_3d<float> const& no_atmos,
                                              bsta_joint_histogram_3d<float> const& atmos,
                                              float prob_ratio,
...
template <class T>
bool brad_eigenspace<T>::init_histogram(vil_image_resource_sptr const& resc, unsigned nbins,
              ...
              bool& done)
{
  ...
  if( rni % sbi != 0 )
    {
    nbi--;              // discard partial blocks
    }
  unsigned nbj = bif->n_block_j();
  if( rnj % sbj != 0 )
    {
    nbj--;              // discard partial blocks
    }
  view.set_size(ni, nj);
  if( first )
    {
...
template <class T>
bool brad_eigenspace<T>::update_histogram(vil_image_resource_sptr const& resc,
                                          bsta_joint_histogram_3d<float>& hist)
{
  if( !eigensystem_valid_ )
    {
...
template <class T>
bool brad_eigenspace<T>::init_histogram(vcl_vector<vil_image_resource_sptr> const& rescs,
...
template <class T>
bool brad_eigenspace<T>::update_histogram(vcl_vector<vil_image_resource_sptr> const& rescs,
                                          bsta_joint_histogram_3d<float>& hist)
{
  for( vcl_vector<vil_image_resource_sptr>::const_iterator rit = rescs.begin();
       rit != rescs.end(); ++rit )
...
template <class T>
bool brad_eigenspace<T>::init_histogram_rand(vcl_vector<vil_image_resource_sptr> const& rescs,
                                             unsigned nbins,
                                             bsta_joint_histogram_3d<float>& hist,
...
template <class T>
bool brad_eigenspace<T>::init_histogram_blocked(vcl_vector<vil_image_resource_sptr> const& rescs,
                                                unsigned nbins,
                                                bsta_joint_histogram_3d<float>& hist,
                                                unsigned nit, unsigned njt)
{
...
template <class T>
bool brad_eigenspace<T>::update_histogram_rand(vcl_vector<vil_image_resource_sptr> const& rescs,
                                               bsta_joint_histogram_3d<float>& hist,
                                               double frac, unsigned nit, unsigned njt)
{
  if( !eigensystem_valid_ )
  ...
  for( unsigned t = 0; t < ntiles; ++t )
    ...
    double rd = (njd - njtd - 1.0) * (vcl_rand() / (RAND_MAX + 1.0) );
    double cd = (nid - nitd - 1.0) * (vcl_rand() / (RAND_MAX + 1.0) );
    if( rd < 0 ) {rd = 0.0; } else if( rd > njd - njtd - 1.0 )
      {
      rd = njd - njtd - 1.0;
      }
    if( cd < 0 ) {cd = 0.0; } else if( cd > nid - nitd - 1.0 )
      {
      cd = nid - nitd - 1.0;
      }
    unsigned                 j0 = static_cast<unsigned>(rd), i0 = static_cast<unsigned>(cd);
...
template <class T>
bool brad_eigenspace<T>::update_histogram_blocked(vcl_vector<vil_image_resource_sptr> const& rescs,
                                                  bsta_joint_histogram_3d<float>& hist,
                                                  unsigned nit, unsigned njt)
{
  for( vcl_vector<vil_image_resource_sptr>::const_iterator rit = rescs.begin();
