...
bool brad_estimate_shadow_prob_density(vil_image_view<float> const& radiance_image, brad_image_metadata const& mdata,
                                       brad_atmospheric_parameters const& atm_params,
                                       vil_image_view<float> & shadow_prob_density)
{
  ...
  double shadow_rad = brad_expected_radiance_chavez(mean_reflectance, vgl_vector_3d<double>(0, 0,
                                                                                            1), mdata_nosun,
                                                    atm_params);
  double radiance_var = brad_radiance_variance_chavez(0.2, vgl_vector_3d<double>(0, 0,
                                                                                 1), mdata_nosun, atm_params, reflectance_sigma * reflectance_sigma, optical_depth_sigma * optical_depth_sigma, skylight_sigma * skylight_sigma,
                                                      airlight_sigma * airlight_sigma);
...
bool brad_estimate_shadow_prob(vil_image_view<float> const& radiance_image, brad_image_metadata const& mdata,
                               brad_atmospheric_parameters const& atm_params, vil_image_view<float> & shadow_prob)
{
