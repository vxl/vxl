...
bool brad_estimate_atmospheric_parameters(vil_image_view<float> const& radiance, brad_image_metadata const& mdata,
...
bool brad_estimate_atmospheric_parameters(vil_image_view<float> const& radiance, brad_image_metadata const& mdata,
                                          float mean_reflectance, brad_atmospheric_parameters & params,
                                          bool constrain_atmospheric_params)
{
  ...
  if( radiance_mean < airlight )
    {
    vcl_cout << "ERROR: radiance mean is less than the airlight; radiance_mean=" << radiance_mean << " airlight="
             << airlight << vcl_endl;
    return false;
    }
  ...
  double optical_depth =
    -vcl_log(vnl_math::pi / (mean_reflectance * mdata.sun_irradiance_ * vcl_sin(deg2rad * mdata.sun_elevation_) )
             * (radiance_mean - airlight) );
  ...
  vcl_cout << "airlight = " << airlight << " optical_depth = " << optical_depth << " skylight = " << skylight
           << vcl_endl;
...
bool brad_estimate_atmospheric_parameters_multi(vil_image_view<float> const& radiance, brad_image_metadata const& mdata,
                                                brad_atmospheric_parameters & params)
{
  unsigned int ni = radiance.ni();
  unsigned int nj = radiance.nj();
                                                ...
                                                bool constrain_atmospheric_params)
{
  ...
  for( unsigned p = 0; p < np; p++ )
    ...
    if( radiance_mean < airlight )
      {
      vcl_cerr << "brad_image_atmospherics_est::brad_estimate_atmospheric_parameters_multi : radiance_mean "
               << radiance_mean << " is smaller than airlight " << airlight << "\n!";
      return false;
      }
    // fix skylight
    double skylight = 0.0;
    ...
    double optical_depth =
      -vcl_log(vnl_math::pi
               / (mean_reflectance * mdata.sun_irradiance_values_[p] * vcl_sin(deg2rad * mdata.sun_elevation_) )
               * (radiance_mean - airlight) );
...
bool brad_undo_reflectance_estimate(vil_image_view<float> const& reflectance, brad_image_metadata const& mdata,
                                    brad_atmospheric_parameters const& atm_params, vil_image_view<float> & radiance)
{
  ...
  double Lsat_horizontal = T_view * (mdata.sun_irradiance_ * sun_dot_norm * T_sun + atm_params.skylight_)
    / vnl_math::pi + atm_params.airlight_;
...
bool brad_estimate_reflectance_image(vil_image_view<float> const& radiance, brad_image_metadata const& mdata,
                                     brad_atmospheric_parameters const& atm_params, vil_image_view<float> & reflectance)
{
  // calculate atmosphere transmittance value
  double sun_el_rads = mdata.sun_elevation_ * vnl_math::pi_over_180;
  double sat_el_rads = mdata.view_elevation_ * vnl_math::pi_over_180;
...
bool brad_estimate_reflectance_image_multi(vil_image_view<float> const& radiance, brad_image_metadata const& mdata,
                                           brad_atmospheric_parameters const& atm_params,
                                           vil_image_view<float> & reflectance_multi)
{
  // calculate atmosphere transmittance value
  double   sun_el_rads = mdata.sun_elevation_  * vnl_math::pi_over_180;
  double   sat_el_rads = mdata.view_elevation_ * vnl_math::pi_over_180;
  ...
  for( unsigned p = 0; p < np; p++ )
    ...
    // ideal Lambertian reflector, surface normal = [0,0,1]
    double sun_dot_norm = vcl_sin(sun_el_rads);
    double Lsat_horizontal = T_view
      * (mdata.sun_irradiance_values_[p] * sun_dot_norm * T_sun + atm_params.skylight_multi_[p]) / vnl_math::pi
      + atm_params.airlight_multi_[p];
    // stretch radiance values such that airlight value maps to 0, and ideal reflector maps to 1
    for( unsigned j = 0; j < nj; j++ )
      {
      for( unsigned i = 0; i < ni; i++ )
