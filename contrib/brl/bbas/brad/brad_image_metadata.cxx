...
bool brad_image_metadata::parse_from_imd(vcl_string const& filename)
{
  ...
  for( ; awk; ++awk )
    ...
    if( tag.compare("satId") == 0 )
      ...
      satellite_name_ =
        satellite_name_.substr(satellite_name_.find_first_of("\"") + 1, satellite_name_.find_last_of("\"") - 1);
      continue;
      }
    if( tag.compare("LLLon") == 0 || tag.compare("URLon") == 0 || tag.compare("ULLon") == 0 ||
        tag.compare("LRLon") == 0 )
      {
      int corner_pos = 0;
    ...
    if( tag.compare("LLLat") == 0 || tag.compare("URLat") == 0 || tag.compare("ULLat") == 0 ||
        tag.compare("LRLat") == 0 )
      {
      int corner_pos = 0;
    ...
    if( tag.compare("LLHAE") == 0 || tag.compare("URHAE") == 0 || tag.compare("ULHAE") == 0 ||
        tag.compare("LRHAE") == 0 )                                                                                          // CAUTION: height above ELLIPSOID (not mean sea level/ geoid)
      {
      linestr >> tag;
      double z;
      linestr >> z;
  ...
  vcl_cout << "  cloud coverage percentage : " << cloud_coverage_percentage_ << " band: " << band_
           << " number of bands: " << n_bands_ << vcl_endl;
...
bool brad_image_metadata::parse_from_pvl_only(vcl_string const& filename)
{
  ...
  for( ; awk; ++awk )
    ...
    // image footprint
    if( (linestr.str().find("BEGIN_GROUP") != vcl_string::npos &&
         linestr.str().find("upperRightCorner") != vcl_string::npos) ||
        (linestr.str().find("BEGIN_GROUP") != vcl_string::npos &&
         linestr.str().find("upperLeftCorner") != vcl_string::npos) ||
        (linestr.str().find("BEGIN_GROUP") != vcl_string::npos &&
         linestr.str().find("lowerRightCorner") != vcl_string::npos) ||
        (linestr.str().find("BEGIN_GROUP") != vcl_string::npos &&
         linestr.str().find("lowerLeftCorner") != vcl_string::npos) )
      {
      int corner_pos = 0;
      // ugh
  ...
  if( !parsed_sun_irradiance )
    ...
    if( img_info.find("IKONOS") != vcl_string::npos || nitf_filename.find("IK") != vcl_string::npos )
      ...
      satellite_name_ = "IKONOS";
             ...
             satellite_name_.compare("OV-5") == 0 )                                                                                                      // OZGE TODO: check this one
      ...
      if( n_bands_ == 1 )
        ...
        return false;
             ...
             satellite_name_.compare("WV01") == 0 )
      {
      ...
      satellite_name_ = "WorldView";
             ...
             satellite_name_.compare("WV02") == 0 )
      {
      ...
      vcl_cerr
        <<
      "WARNING satellite name is DigitalGlobe but cannot determine which satellite!! so sun irradiance values are not set properly (esp. for multi-spectral images)"
        << vcl_endl;
      }
    else
      {
      vcl_cerr << "ERROR Could not find known satellite name in: " << img_info << " from NITF " << nitf_filename
               << vcl_endl;;
...
bool brad_image_metadata::parse_from_meta_file(vcl_string const& meta_file)
{
  ...
  if( satellite_name_.find("IKNOOS") != vcl_string::npos )
    ...
    satellite_name_ = "IKNOOS";
           ...
           satellite_name_.find("GEOEYE1") != vcl_string::npos )
    {
    // these values are from http://apollomapping.com/wp-content/user_uploads/2011/09/GeoEye1_Radiance_at_Aperture.pdf
    solar_irrad = 1617;
    satellite_name_ = "GeoEye-1";
    ...
    {
    vcl_cerr << "ERROR Could not find known satellite name in: " << satellite_name_ << " from metafile " << meta_file
             << vcl_endl;;
