...
float brad_phongs_model::val(float view_elev, float view_azim, float sun_elev, float sun_azim)
{
  ...
  double       specular_term = ks_ * vcl_pow(vcl_fabs(dot_product<double>(reflected_light_vector,
                                                                          view_dir) ), (double)gamma_);
...
float brad_phongs_model::val(vnl_double_3 view_dir, float sun_elev, float sun_azim)
{
  ...
  double diffuse_term  = kd_ * vcl_fabs(dot_product<double>(normal_, sun_dir) );
  double specular_term = ks_
    * vcl_pow(vcl_fabs(dot_product<double>(reflected_light_vector, view_dir) ), (double)gamma_);
  return float(diffuse_term + specular_term);
}

brad_phongs_model_approx::brad_phongs_model_approx(float kd, float ks, float gamma, float normal_elev,
                                                   float normal_azim)
{
  kd_ = vcl_fabs(kd);
  ks_ = vcl_fabs(ks);
...
brad_phongs_model_est::brad_phongs_model_est(double sun_elev,
                                             ...
                                             bool with_grad) : vnl_least_squares_function(5,
                                                                                          obs.size(),
...
void brad_phongs_model_est::f(vnl_vector<double> const& x, vnl_vector<double>& y)
{
  ...
  vnl_double_3 light_vector(vcl_sin(sun_elev_) * vcl_cos(sun_azim_), vcl_sin(sun_elev_) * vcl_sin(
                              sun_azim_), vcl_cos(sun_elev_) );
  vnl_identity_3x3 I;
...
void brad_phongs_model_est::gradf(vnl_vector<double> const& x, vnl_matrix<double> & J)
{
  ...
  vnl_double_3 lv(vcl_sin(sun_elev_) * vcl_cos(sun_azim_), vcl_sin(sun_elev_) * vcl_sin(sun_azim_), vcl_cos(
                    sun_elev_) );
  vnl_identity_3x3 I;
  ...
  for( unsigned i = 0; i < viewing_dirs_.size(); ++i )
    ...
    J[i][3] = vcl_fabs(x[0]) * dot_product<double>(nvdt, lv) * vcl_sqrt(obs_weights_[i])
      + vcl_fabs(x[1]) * x[2]
      * vcl_pow(dp_rlv_vv, x[2] - 1) * dot_product<double>(householder_xform_dt_lv, view_vector) * vcl_sqrt(
        obs_weights_[i]);

    J[i][4] = vcl_fabs(x[0]) * dot_product<double>(nvdp, lv) * vcl_sqrt(obs_weights_[i])
      + vcl_fabs(x[1]) * x[2]
      * vcl_pow(dp_rlv_vv, x[2] - 1) * dot_product<double>(householder_xform_dp_lv, view_vector) * vcl_sqrt(
        obs_weights_[i]);
    }
}
...
{
  vnl_double_3 normal_vector(vcl_sin(x[3]) * vcl_cos(x[4]), vcl_sin(x[3]) * vcl_sin(x[4]), vcl_cos(x[3]) );
  vnl_double_3 light_vector(vcl_sin(sun_elev_) * vcl_cos(sun_azim_), vcl_sin(sun_elev_) * vcl_sin(sun_azim_), vcl_cos(
                              sun_elev_) );
  double diffuse_term  = vcl_fabs(x[0]) * vcl_fabs(dot_product<double>(normal_vector, light_vector) );
  ...
  for( unsigned i = 0; i < viewing_dirs_.size(); ++i )
    ...
    half_vector = half_vector / half_vector.magnitude();
    double dp = vcl_fabs(dot_product<double>(normal_vector, half_vector) );
    y[i] = (diffuse_term + vcl_fabs(x[1]) * vcl_pow(dp, x[2]) - obs_[i]) * vcl_sqrt(obs_weights_[i]);
    }
}
...
#if 0
    J[i][3] = vcl_fabs(x[0]) * dot_product<double>(nvdt, lv) * vcl_sqrt(obs_weights_[i])
      + vcl_fabs(x[1]) * vcl_pow(dp_rlv_vv, x[2]) * vcl_log(dp_rlv_vv) * dot_product<double>(householder_xform_dt_lv,
                                                                                             view_vector) * vcl_sqrt(
        obs_weights_[i]);
...
#else // 0
    J[i][3] = vcl_fabs(x[0]) * dot_product<double>(nvdt, lv) * vcl_sqrt(obs_weights_[i])
      + vcl_fabs(x[1]) * x[2] * vcl_pow(dp, x[2] - 1) * dot_product<double>(nvdt, half_vector) * vcl_sqrt(
        obs_weights_[i]);

    J[i][4] = vcl_fabs(x[0]) * dot_product<double>(nvdp, lv) * vcl_sqrt(obs_weights_[i])
      + vcl_fabs(x[1]) * x[2] * vcl_pow(dp, x[2] - 1) * dot_product<double>(nvdp, half_vector) * vcl_sqrt(
        obs_weights_[i]);
#endif // 0
    }
}
