// This is bsol_intrinsic_curve_3d.cxx
...
bool bsol_intrinsic_curve_3d::operator==(const vsol_spatial_object_3d& obj) const
{
	return
		...
		? *this == (bsol_intrinsic_curve_3d const&) (bsol_intrinsic_curve_3d const&) obj
		: false;
}
...
void bsol_intrinsic_curve_3d::computeProperties ()
{
	...
	for (int i=1; i<size(); i++) {
		...
		double dL = sqrt (cur_dx*cur_dx + cur_dy*cur_dy + cur_dz*cur_dz);
		s_.push_back(dL);
		length += dL;
	...
	for (int i=2; i<size(); i++) {
		...
		double curvature = hypot (phis, vcl_sin(phi_[i])*thetas);
	...
	for (int i=1; i<size(); i++) {
	   ...
	   double theta = vcl_atan2 (cur_dy, cur_dx);
		...
		if (dL > ZERO_TOLERANCE) {
			...
			K = (d2y*cdx-d2x*cdy) / pow (cdx*cdx+cdy*cdy,3/2);
		}
		else {
			K = 0;
		}
...
bool bsol_intrinsic_curve_3d::LoadCON3File (vcl_string fileName)
{
	...
	if ((fp = fopen(fileName.c_str(), "r")) == NULL) {
		fprintf (stderr, "ERROR: Can't open input .con3 file %s.\n", fileName.c_str());
		return false; 
	}
	...
	fscanf (fp, "CONTOUR\n");
	fscanf (fp, "%s\n", buffer);
	if (strcmp(buffer, "OPEN")==0) //the same
		isOpen_ = true;
	else //"CLOSE"
		isOpen_ = false;
	int numPoints;
...
bool bsol_intrinsic_curve_3d::SaveCON3File (vcl_string fileName)
{
	...
	if ((fp = fopen(fileName.c_str(), "w")) == NULL) {
		fprintf (stderr, "ERROR(%s): Can't open output .con3 file %s.\n", fileName.c_str());
		return false; 
	}
