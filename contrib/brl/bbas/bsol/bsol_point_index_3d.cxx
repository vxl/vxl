  ...
  for( int ro = -row_radius; ro <= row_radius; ro++ )
    ...
    for( int co = -col_radius; co <= col_radius; co++ )
      ...
      for( int so = -slab_radius; so <= slab_radius; so++ )
        ...
        vcl_vector<vsol_point_3d_sptr>& points_in_cell =
          (*(point_array_[r][c]) )[s];
...
vsol_box_3d_sptr bsol_point_index_3d::index_cell(const int row, const int col, const int slab)
{
...
vsol_box_3d_sptr bsol_point_index_3d::index_cell(const double x, const double y, const double z)
{
