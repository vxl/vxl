...
public:
  ...
  vcl_vector<T> & weights() { return weights_; }
  ...
  vcl_vector<int> & assignments() { return assignments_; }
  ...
  typename vcl_vector<typename bsta_parzen_sphere<T,
                                                  n>::vector_type>::const_iterator samples_begin() const { return
                                                                                                             bsta_parzen
                                                                                                             <T,
    n>::samples_.begin(); }
  typename vcl_vector<typename bsta_parzen_sphere<T,
                                                  n>::vector_type>::const_iterator samples_end() const { return
                                                                                                           bsta_parzen<T,
                                                                                                                       n>
    ::samples_.end(); }
  ...
  {
    return bsta_parzen<T, n>::samples_.size() == weights_.size() &&
...
template <class T, unsigned n>
bool bsta_sample_set_fit_distribution(const bsta_sample_set<T, n>& set, bsta_mixture<bsta_num_obs<bsta_gaussian_full<T,
                                                                                                                     n> > >& out)
{
  if( !set.check_initializations() )
    {
...
template <class T>
T bsta_sample_set_log_likelihood(const bsta_sample_set<T, 1>& set, bsta_num_obs<bsta_gaussian_sphere<T, 1> >& dist,
                                 T total_weight)
{
  if( !set.size() )
    {
    vcl_cout << "Error in - bsta_sample_set<T,n>::bsta_sample_set_log_likelihood() : assignments not initialized!\n";
    return T(0);
    }
  ...
  for( unsigned i = 0; i < set.size(); i++ )
    {
    if( vcl_sqrt(dist.sqr_mahalanobis_dist(set.sample(i) ) ) < 3 )// we don't want zero to be logged
      {
      T p = T(vcl_log(dist.prob_density(set.sample(i) ) ) );
      T pw = T(vcl_log(p_dist) );
...
//  (bsta_num_obs class contains total weight of samples assigned to this distribution)
template <class T, unsigned n>
T bsta_sample_set_log_likelihood(const bsta_sample_set<T, n>& set, bsta_num_obs<bsta_gaussian_full<T, n> >& dist,
                                 T total_weight)
{
  if( !set.size() )
    {
...
//  (bsta_num_obs class contains total weight of samples assigned to this distribution)
template <class T>
T bsta_sample_set_log_likelihood(const bsta_sample_set<T, 2>& set, bsta_num_obs<bsta_gaussian_sphere<T, 1> >& dist0,
                                 T w0, bsta_num_obs<bsta_gaussian_sphere<T, 1> >& dist1, T w1, T& w_sum)
{
  if( !set.size() )
    {
    vcl_cout << "Error in - bsta_sample_set<T,n>::bsta_sample_set_log_likelihood() : set is empty!\n";
    return T(0);
    }
  ...
  for( unsigned i = 0; i < set.size(); i++ )
    ...
    T d0_sqrt = vcl_sqrt(d0);
    T d1_sqrt = vcl_sqrt(d1);
  ...
  for( unsigned m = 0; m < mode_cnt; m++ )
    ...
    for( unsigned i = 0; i < set.size(); i++ )
      ...
      if( set.assignment(i) == m )
        {
        points.push_back(vcl_pair<T, T>(T(set.sample(i)[0]), T(set.sample(i)[1]) ) );
        }
      }
    if( points.size() > 0 )
      {
