...
bsta_sigma_normalizer::bsta_sigma_normalizer(float under_estimation_probability,
                                             unsigned int N_PRECOMPUTED) : N_PRECOMPUTED_(N_PRECOMPUTED), unbias_const_(
    N_PRECOMPUTED + 1, 0.0f)
{
  class gammainc_error_fn : public vnl_least_squares_function
  {
public:
    gammainc_error_fn(unsigned int ndof, float under_estimation_prob) : vnl_least_squares_function(1, 1, no_gradient),
      ndof_(ndof), p_(under_estimation_prob) {}
...
private:
  ...
  if( under_estimation_probability < 1e-4 )
    {
    vcl_cout << "error : boxm_sigma_normalizer : under_estimation_probability " << under_estimation_probability
             << " too low" << vcl_endl;
    return;
    }
...
float bsta_sigma_normalizer::normalization_factor(float number_of_observations) const
{
  ...
  float norm_factor = (normalization_factor_int( (unsigned int)nobs_floor) * floor_weight)
    + (normalization_factor_int( (unsigned int)nobs_ceil) * (1.0f - floor_weight) );
...
float bsta_sigma_normalizer::normalization_factor_int(unsigned int number_of_observations) const
{
  ...
  static const float m = (unbias_const_[N_PRECOMPUTED_] - unbias_const_[N_PRECOMPUTED_ - 5])
    / (1.0f / N_PRECOMPUTED_ - 1.0f / (N_PRECOMPUTED_ - 5) );
...
void vsl_b_write(vsl_b_ostream & /*os*/, bsta_sigma_normalizer const & /*bit_scene*/) {}
// : Binary write boxm2_scene pointer to stream
void vsl_b_write(vsl_b_ostream & /*os*/, bsta_sigma_normalizer * const & /*ph*/) {}
// : Binary write boxm2_scene smart pointer to stream
void vsl_b_write(vsl_b_ostream & /*os*/, bsta_sigma_normalizer_sptr &) {}
// : Binary write boxm2_scene smart pointer to stream
void vsl_b_write(vsl_b_ostream & /*os*/, bsta_sigma_normalizer_sptr const &) {}

// : Binary load boxm scene from stream.
void vsl_b_read(vsl_b_istream & /*is*/, bsta_sigma_normalizer & /*bit_scene*/) {}
// : Binary load boxm scene pointer from stream.
void vsl_b_read(vsl_b_istream & /*is*/, bsta_sigma_normalizer * /*ph*/) {}
