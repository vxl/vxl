...
void test_sample_set(T epsilon, const vcl_string& type_name)
{
  ...
  bool                                                     return_value = bsta_sample_set_fit_distribution<T>(ss,
                                                                                                              out_dist);
  TEST( (type_name + ": mean shift fit distribution to sample set method").c_str(), return_value, true);
  TEST( (type_name + ": mean shift fit distribution to sample set method").c_str(),
        out_dist.num_components(), ss.mode_cnt() );
  ...
  vcl_cout << "likelihood for ss: " << likelihood << '\n';
}
...
void test_sample_set(T epsilon, const vcl_string& type_name)
{
  ...
  TEST_NEAR( (type_name + ": mean shift sample set total_weight method").c_str(),
             ss.total_weight(), total_weight, epsilon);
  ...
  TEST_NEAR( (type_name + ": mean shift sample set mean of the mode method").c_str(), dummy2.magnitude(), T(0),
             epsilon);
  ...
  TEST_NEAR( (type_name + ": mean shift sample set mean of the mode method").c_str(), dummy3.magnitude(), T(0),
             epsilon);
  ...
  bool                                                   return_value = bsta_sample_set_fit_distribution<T, n>(ss,
                                                                                                               out_dist);
  TEST( (type_name + ": mean shift fit distribution to sample set method").c_str(), return_value, true);
  TEST( (type_name + ": mean shift fit distribution to sample set method").c_str(),
        out_dist.num_components(), ss.mode_cnt() );
  ...
  for( unsigned mi = 0; mi < ss.mode_cnt(); mi++ )
    ...
    TEST( (type_name + ": mean shift mode addition to random distribution").c_str(),
          rand_dist.insert(gauss_d_n, 1.0f / ss.mode_cnt() ), true);
    }
  ...
  for( unsigned mi = 0; mi < rand_dist.num_components(); mi++ )
    ...
    vcl_cout << "data likelihood of random dist by component: " << mi << ": " << likelihood << '\n';
    }
}
...
void test_mean_shift_algo(T epsilon, const vcl_string& type_name)
{
  ...
  for( unsigned i = 1; i < n; i++ )
    {
    sd1[i] = T(vcl_pow(rng.drand32() / 1000, 2) ); sd2[i] = T(vcl_pow(rng.drand32() / 1000, 2) );
    sd3[i] = T(vcl_pow(rng.drand32() / 1000, 2) );
    }
