...
// constructor
  bsta_int_histogram_2d::bsta_int_histogram_2d(const unsigned int nbins_x, const unsigned int nbins_y)
  {
    nbins_x_ = nbins_x;
    nbins_y_ = nbins_y;
	float diag_lgth = sqrt( static_cast<float>(nbins_x_*nbins_x_) + static_cast<float>(nbins_y_*nbins_y_));
...
// smooth the histogram with a 2D parzan window (which is a gaussian filter)
												...
												bsta_int_histogram_1d &phist_y ) 
  {
	...
	float diag_lgth = sqrt(1.0f + (inverse_slope*inverse_slope));
	...
	float dxintcpt = sqrt(1 + (slope*slope));
	float dyintcpt = sqrt(1 + (inverse_slope*inverse_slope));
	unsigned int xbox = static_cast<unsigned int>(ceil(dxintcpt)); 
	unsigned int ybox = static_cast<unsigned int>(ceil(dyintcpt)); 
	...
	for (unsigned int i=0; i<diag_; i++)
	{
		...
		float xaxis_proj = sqrt((i*i) + ((i*slope)+(i*slope)));
		float yaxis_proj = sqrt((i*i) + ((i/slope)+(i/slope)));
		...
		{
			unsigned int xindex = static_cast<int>(floor(xpos));		// integer projection onto 2D axis
			unsigned int yindex = static_cast<int>(floor(ypos));
								   ...
								   bool front)
{
	...
	float diag_lgth = sqrt(1.0f + (newslope*newslope));
