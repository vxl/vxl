...
{
public:
  bool operator()( const vcl_pair<float, T>& left_pair,
...
template <class T>
bool bsta_sampler<T>::sample(const bsta_joint_histogram<float>& jh, unsigned cnt, vcl_vector<vcl_pair<float,
                                                                                                      float> >& out)
{
  // vnl_random rng;
                             ...
                             vnl_random & rng)
{
  ...
  for( unsigned i = 0; i < cnt; i++ )
    ...
    if( r >= last_val )
      ...
      vcl_pair<float, float>                         out_sample(jh.min_a() + (r_id.first + 1) * jh.delta_a(),
                                                                jh.min_b() + (r_id.second + 1) * jh.delta_b() );
      out.push_back(out_sample);
      }
    }
...
template <class T>
bool bsta_sampler<T>::sample_in_likelihood_order(const bsta_joint_histogram<float>& jh, unsigned cnt,
                                                 vcl_vector<vcl_pair<float, float> >& out)
{
  ...
  for( unsigned i = 0; i < size; i++ )
    ...
    vcl_pair<float, float> out_p(jh.min_a() + (arr[i].second.first + 1) * jh.delta_a(),
                                 jh.min_b() + (arr[i].second.second + 1) * jh.delta_b() );
    out.push_back(out_p);
    }
