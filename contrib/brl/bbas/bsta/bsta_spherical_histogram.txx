#ifndef bsta_spherical_histogram_txx_
...
template <class T>
bsta_spherical_histogram<T>::bsta_spherical_histogram(unsigned n_azimuth, unsigned n_elevation,
...
template <class T>
void bsta_spherical_histogram<T>::center(int linear_index, T& az_center, T& el_center)
{
...
template <class T>
void bsta_spherical_histogram<T>::convert_to_cartesian(T azimuth, T elevation, T& x, T& y, T& z) const
{
...
template <class T>
void bsta_spherical_histogram<T>::convert_to_spherical(T x, T y, T z, T& azimuth, T& elevation) const
{
...
template <class T>
void bsta_spherical_histogram<T>::azimuth_interval(int azimuth_index, T& azimuth_start, T& azimuth_range) const
{
  if( n_azimuth_ == 0 )
    {
    azimuth_start = T(0);
...
template <class T>
void bsta_spherical_histogram<T>::elevation_interval(int elevation_index, T& elevation_start, T& elevation_range) const
{
  if( n_elevation_ == 0 )
    {
    elevation_start = T(0);
...
template <class T>
void bsta_spherical_histogram<T>::upcount(T azimuth, T elevation, T mag)
{
...
template <class T>
void bsta_spherical_histogram<T>::upcount_weighted_by_area(T azimuth, T elevation, T mag)
{
  int    el_indx = elevation_index(elevation);
...
template <class T>
vcl_vector<int> bsta_spherical_histogram<T>::bins_intersecting_cone(T center_az, T center_el, T cone_half_angle)
{
  vcl_vector<int> ret;
  T               xc, yc, zc;
...
template <class T>
void bsta_spherical_histogram<T>::write_counts_with_interval(vcl_ostream& os, int azimuth_index,
                                                             int elevation_index)
{
...
template <class T>
void bsta_spherical_histogram<T>::write_counts_with_center(vcl_ostream& os, int azimuth_index,
                                                           int elevation_index)
{
  int lidx = linear_index(azimuth_index, elevation_index);
  T   cnts = counts_[lidx];
...
#endif // bsta_spherical_histogram_txx_
