...
bsta_spherical_histogram<T>::bsta_spherical_histogram()
  : total_counts_valid_(false), total_counts_(0), n_azimuth_(0), n_elevation_(0), 
    azimuth_start_(0), azimuth_range_(0), elevation_start_(0), elevation_range_(0),
    units_(DEG), az_branch_cut_(B_180_180), el_poles_(B_90_90)
{
}
...
bsta_spherical_histogram(unsigned n_azimuth, unsigned n_elevation,
  ...
  : total_counts_valid_(false), total_counts_(0), n_azimuth_(n_azimuth), 
    n_elevation_(n_elevation),azimuth_start_(azimuth_start), 
    azimuth_range_(azimuth_range), elevation_start_(elevation_start), 
    elevation_range_(elevation_range), units_(units),
...
{
  //internal representation is the standard spherical coordiante system
...
int bsta_spherical_histogram<T>::elevation_index(T elevation) const {
  ...
  //standardized elevation corrdinates range from
...
vcl_ostream& operator<<(vcl_ostream& s, bsta_spherical_histogram<T> const& h)
{
  ...
  s << "n_bins_azimuth: " << h.n_azimuth() << " n_bins_elevation: " 
    << h.n_elevation() << '\n';
  s << "azimuth(start: " << h.azimuth_start() <<" interval: " << h.azimuth_range()<< " )\n"; 
  s << "elevation(start: "<< h.elevation_start()<<" interval: " << h.elevation_range()<<" )\n"; 
