               ...
               float width = 600.0f, float height = 600.0f, float margin = 40.0f, int font_size = 30)
{
  ...
  if( false )
    {
    pl.set_margin(margin);          // TODO
    }
  if( false )
    {
    pl.set_font_size(font_size);          // TODO
    }
  T min = h.min();
  T max = h.max();
  T delta = h.delta();
                              ...
                              float width = 600.0f, float height = 600.0f, float margin = 40.0f, int font_size = 30)
{
  ...
  for( float vala = (float)mina; vala < maxa; vala += delta_a )
    ...
    t->set_location( (float)(300.0f + (radius) * vcl_cos(vala + vnl_math::pi * 0.01) ),
                     (float)(300.0f + (radius) * -vcl_sin(vala + vnl_math::pi * 0.01) ) );
    pl.add_element(t);
    }
