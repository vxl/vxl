...
public:
  ...
  static void equiform_uniform(const unsigned n_trans, vcl_vector<vgl_h_matrix_2d<T> >& transform_set, const T dx,
                               const T dy, const T dtheta = (T)0, const T ds = (T)0);

  // : generate a set of uniformly distributed affine transformations.
  // skew is always zero
  static void zero_skew_affine_uniform(const unsigned n_trans, vcl_vector<vgl_h_matrix_2d<T> >& transform_set,
                                       const T dx, const T dy, const T dtheta = (T)0, const T ds = (T)0,
                                       const T daspect = (T)0);

  // : generate a set of uniformly distributed affine transformations on a specified set of intervals
  static void zero_skew_affine_uniform_interval(const unsigned n_trans, vcl_vector<vgl_h_matrix_2d<T> >& trans_set,
                                                const T xmin, const T xmax, const T ymin, const T ymax,
                                                const T theta_min, const T theta_max, const T scale_min,
                                                const T scale_max, const T aspect_min, const T aspect_max);
