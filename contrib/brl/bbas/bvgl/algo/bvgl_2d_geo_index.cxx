...
void bvgl_2d_geo_index::write_to_kml_node(vcl_ofstream& ofs, bvgl_2d_geo_index_node_sptr n,
                                          unsigned const& current_depth, unsigned const& depth, vcl_string explanation)
{
  if( !n )
    {
    return;
...
void bvgl_2d_geo_index::write_to_kml(bvgl_2d_geo_index_node_sptr root, unsigned const& depth,
                                     vcl_string const& kml_file, vcl_string explanation)
{
...
void bvgl_2d_geo_index::write_to_kml_node(vcl_ofstream& ofs, bvgl_2d_geo_index_node_sptr n,
                                          unsigned const& current_depth, unsigned const& depth,
                                          vpgl_lvcs_sptr const& lvcs, vcl_string explanation)
{
  if( !n )
    {
    return;
  ...
  if( current_depth == depth )
    ...
    lvcs->local_to_global(n->extent_.min_point().x(),
                          n->extent_.min_point().y(), 0.0, vpgl_lvcs::wgs84, min_lon, min_lat, gz);
    lvcs->local_to_global(n->extent_.max_point().x(),
                          n->extent_.max_point().y(), 0.0, vpgl_lvcs::wgs84, max_lon, max_lat, gz);
    vnl_double_2 ul, ll, lr, ur;
...
void bvgl_2d_geo_index::write_to_kml(bvgl_2d_geo_index_node_sptr root, unsigned const& depth,
                                     vcl_string const& kml_file, vpgl_lvcs_sptr const& lvcs, vcl_string explanation)
{
  vcl_ofstream ofs(kml_file.c_str() );
  bkml_write::open_document(ofs);
...
void bvgl_2d_geo_index::get_leaves(bvgl_2d_geo_index_node_sptr root, vcl_vector<bvgl_2d_geo_index_node_sptr>& leaves,
                                   vcl_vector<vgl_point_2d<float> > const& line)
{
...
void bvgl_2d_geo_index::get_leaf(bvgl_2d_geo_index_node_sptr root, bvgl_2d_geo_index_node_sptr& leaf,
...
void bvgl_2d_geo_index::get_leaf(bvgl_2d_geo_index_node_sptr root, bvgl_2d_geo_index_node_sptr& leaf,
                                 vgl_point_2d<float> const& point)
{
