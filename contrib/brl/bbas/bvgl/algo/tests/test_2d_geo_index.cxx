...
static void test_2d_geo_index()
{
  ...
  vcl_cout << " size of the leaf node: " << leaves[0]->extent_.width() << 'x' << leaves[0]->extent_.height()
           << vcl_endl;
  ...
  poly1.new_sheet();
  poly1.push_back(vgl_point_2d<float>(0.55, 0.65) );  poly1.push_back(vgl_point_2d<float>(0.55, 0.40) );
  poly1.push_back(vgl_point_2d<float>(0.80, 0.40) );
  ...
  for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
    {
    bvgl_2d_geo_index_node<vcl_vector<int> >* leaf_ptr =
      dynamic_cast<bvgl_2d_geo_index_node<vcl_vector<int> > *>(leaves[l_idx].ptr() );
  ...
  for( unsigned i = 0; i < leaves.size(); i++ )
    ...
    bvgl_2d_geo_index_node<vcl_vector<int> >* leaf_ptr =
      dynamic_cast<bvgl_2d_geo_index_node<vcl_vector<int> > *>(leaves[i].ptr() );
