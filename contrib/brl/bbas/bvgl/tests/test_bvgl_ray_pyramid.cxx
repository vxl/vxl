  ...
  for(int i=0; i<4; ++i) {
    for(int j=0; j<4; ++j) {
      ...
      if( vcl_fabs(ray.direction().x() - out1GT[idx] ) > 1e-5f ) {
        vcl_cout<<" img1 No match at "<<i<<','<<j<<vcl_endl;
        vcl_cout<<"gt "<<out1GT[idx]<<" calc "<<ray.direction().x()<<vcl_endl;
      }
      idx+=4; 
    }
  }
  ...
  for(int j=0; j<2; ++j) {
    for(int i=0; i<2; ++i) {
      ...
      if( vcl_fabs(ray.direction().x() - out2GT[idx] ) > 1e-5f ) {
        vcl_cout<<" img2 No match at "<<i<<','<<j<<vcl_endl;
        vcl_cout<<"gt "<<out2GT[idx]<<" calc "<<ray.direction().x()<<vcl_endl;
      }
      idx+=4; 
    }
  }
