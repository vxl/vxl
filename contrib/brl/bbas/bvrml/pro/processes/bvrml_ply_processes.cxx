...
bool bvrml_load_points_ply(const vcl_string & ply_file, float dist_thres, vcl_vector<point_pair> & data);
...
bool bvrml_filtered_ply_process(bprb_func_process& pro)
{
  ...
  if( nearest )
    ...
    avg_LE /= cnt;
    avg_CE /= cnt;
    vcl_cout << "there were " << cnt << " nearest pts, avg dist: " << avg_dist << " avg LE: " << avg_LE
             << " avg CE: " << avg_CE << vcl_endl;
    }
