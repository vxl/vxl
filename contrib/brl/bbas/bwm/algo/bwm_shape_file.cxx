...
void bwm_shape_file::print()
{
  if (handle_) {
    ...
    vcl_cout << "NUMBER OF OBJECTS:" << nEntities_ << vcl_endl;
    vcl_cout << "BOUNDING BOX:" << bb_ << vcl_endl;

    for (int i=0; i<nEntities_; i++) {
      SHPObject* obj = SHPReadObject( handle_, i );
      vcl_cout << i << "th Object -----" << vcl_endl;
      vcl_cout << "SHAPE TYPE:" << obj->nSHPType << vcl_endl;
      vcl_cout << "SHAPE ID:" << obj->nShapeId << vcl_endl;
      vcl_cout << "NUM PARTS:" << obj->nParts << vcl_endl;
      vcl_cout << "NUM Vertices:" << obj->nVertices << vcl_endl;
      for (int j=0; j<obj->nVertices; j++) {
        double x = obj->padfX[j];
        double y = obj->padfY[j];
        double z = obj->padfZ[j];
