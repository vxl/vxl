...
//: test if the site exists in the set of correspondences and
//   if so return the associated 3-d point
...
vcl_ostream& operator<<(vcl_ostream& s, bwm_3d_corr const& c)
{
  ...
  while (iter != mlist.end()) {
    s <<  "Site[ " << iter->first << " ]:( X: " << iter->second.x() << " Y: " << iter->second.y() << " Z: " << iter->second.z() << " )\n";
      iter++;
    }
