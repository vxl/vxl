    ...
    for( fit = object_->facemap().begin(); fit != object_->facemap().end(); fit++ )
      ...
      for( unsigned j = 0; j < face->vertices().size(); j++ )
        ...
        lvcs->global_to_local(v->pt().x(), v->pt().y(),
                              v->pt().z(), vpgl_lvcs::wgs84, x, y, z, vpgl_lvcs::DEG, vpgl_lvcs::METERS);
                                                    ...
                                                    vcl_string material_name)
{
  ...
  os << "\n        </float_array>\n";
  os << "        <technique_common>\n";
  os << "          <accessor source=\"#" << geometry_position_array_id.c_str() << "\" count=\"" << nverts
     << "\" stride=\"3\">\n";
  ...
  os << "\n        </float_array>\n";
  os << "        <technique_common>\n";
  os << "          <accessor source=\"#" << geometry_uv_array_id.c_str() << "\" count=\"" << ncorners
     << "\" stride=\"2\">\n";
