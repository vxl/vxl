...
void bwm_observer_cam::set_ground_plane(double x1, double y1, double x2, double y2)
{
  ...
  if( !intersect_ray_and_plane(vgl_point_2d<double>(x1, y1), xy_plane,
                               world_point1) ||
                                          ...
                                          vsol_point_2d_sptr new_pt)
{
  ...
  if( (select_list.size() == 1) &&
    ...
    bwm_plane_fitting_lsf lsf(a, b, c, d, (new vsol_point_2d(
                                             master_img_pt) ), new_pt, bwm_observer_mgr::BWM_MASTER_OBSERVER, this);
...
void bwm_observer_cam::create_circular_polygon(vcl_vector<vsol_point_2d_sptr> ps_list,
                                               ...
                                               int num_sect, double & r, vgl_point_2d<double> & c)
{
  ...
  for( int i = 0; i < num_sect; i++ )
    {
    x = cx + r * vcl_sin(rho * i);
    y = cy + r * vcl_cos(rho * i);
...
bool bwm_observer_cam::find_intersection_points(vgl_point_2d<double> const img_point1,
                                                ...
                                                /* end points of the first polygon segment*/ vgl_point_3d<double>& l1,
                                                vgl_point_3d<double>& l2,
                                                vgl_point_3d<double>& point2,
                                                /* end points of the second polygon segment*/ vgl_point_3d<double>& l3,
...
void bwm_observer_cam::extrude_face(vsol_point_2d_sptr pt)
{
  ...
  if( !poly )
    {
    vcl_cerr << "Please select a face (only one) to extrude!\n";
    return;
    }
  ...
  if( vertices.size()  != 2 )
    {
    vcl_cerr << "Please select 2 vertices on the face to extrude!\n";
    return;
    }
  ...
  if( index1 == index2 )
    {
    vcl_cerr << "the edge vertices are the same vertex, something terribly wrong!\n";
    return;
    }
    ...
    if( next_index != index2 )
      {
      vcl_cerr << "backprojected vertices are not adjacent\n";
      return;
      }
    }
  else
...
#  ifdef CAM_DEBUG
  vcl_cout << "angle " << a * vnl_math::deg_per_rad << '\n';
#  endif
...
void bwm_observer_cam::project_shadow()
{
  ...
  for( unsigned s = 0; s < select_list.size(); s++ )
    ...
    if( select_list[s]->type_name().compare("bgui_vsol_soview2D_line_seg") == 0 )
      ...
      vsol_point_3d_sptr pnew =
        new vsol_point_3d(base_point->x() - len * vcl_tan(sun_elev_angle_) * vcl_cos(sun_azim_angle_),
                          base_point->y() - len * vcl_tan(sun_elev_angle_)
                          * vcl_sin(sun_azim_angle_),
                          base_point->z() );
...
void bwm_observer_cam::set_horizon()
{
  ...
  if( select_list.size() == 1 )
    ...
    if( select_list[0]->type_name().compare("bgui_vsol_soview2D_polyline") == 0 )
      ...
      if( pline->size() != 2 )
        {
        vcl_cout << "polyline must have exactly 2 vertices to be a horizon line\n";
        return;
        }
      vsol_point_2d_sptr p0 = pline->p0(), p1 = pline->p1();
      horizon_ = new vsol_line_2d(p0, p1);
      vcl_cout << "got the horizon\n";
      }
    }
}
                                  ...
                                  double height)
{
  ...
  switch( orient )
    ...
    case 1:
      scene_.add_region(
        p->sptr(), np, min_depth, max_depth, name, depth_map_region::FRONT_PARALLEL, order, land_id, height);
      break;
    case 2:
      scene_.add_region(
        p->sptr(), np, min_depth, max_depth, name, depth_map_region::SLANTED_RIGHT, order, land_id, height);
      break;
    case 3:
      scene_.add_region(
        p->sptr(), np, min_depth, max_depth, name, depth_map_region::SLANTED_LEFT, order, land_id, height);
      break;
    case 4:
...
void bwm_observer_cam::save_weight_params(vcl_string const& path)
{
  ...
  ofs << "Note: 1. for all objects, the summation of weight in the last column should be equal to 1"
      << " (average = " << 1
    / float(!scene_.sky().empty() + !scene_.ground_plane().empty() + scene_.scene_regions().size() ) << ")\n"
