...
void bwm_project_meshes(vcl_vector<vcl_string> paths, vpgl_camera<double>* cam,
...
public:
  ...
  bwm_observer_cam(bgui_image_tableau_sptr const& img, vpgl_camera<double> * camera, vcl_string cam_path)
    ...
    proj_plane_(vgl_plane_3d<double>(0, 0, 1, 0) ), extrude_mode_(false), show_geo_position_(false), focal_length_(
      3000.0), cam_height_(1.6), horizon_(0),
    horizon_soview_(0)
  {}
  ...
  void move_ground_plane(vgl_plane_3d<double> master_plane, vsol_point_2d_sptr new_pt);
  ...
  void backproj_point(vsol_point_2d_sptr p2d, vsol_point_3d_sptr& p3d, vgl_plane_3d<double> proj_plane);
  ...
  void backproj_poly(vsol_polygon_2d_sptr poly2d, vsol_polygon_3d_sptr& poly3d, double dist);

  virtual void proj_point(vgl_point_3d<double> world_pt, vgl_point_2d<double> & image_pt);

  virtual void proj_line(vsol_line_3d_sptr line_3d, vsol_line_2d_sptr & line_2d);

  virtual void proj_poly(vsol_polygon_3d_sptr poly3d, vsol_polygon_2d_sptr& poly2d);

#if 0
  void proj_poly(vcl_vector<bmsh3d_vertex *> verts, vcl_vector<vgl_point_2d<double> > & projections);

#endif

  bool intersect(float x1, float y1, float x2, float y2);

  bool intersect(bwm_observable_sptr obj, unsigned face_id, float x1, float y1, float x2, float y2);

  bool intersect(bwm_observable_sptr obj, float img_x, float img_y, unsigned face_id, vgl_point_3d<double> & pt3d);

  bool find_intersection_point(vgl_point_2d<double> img_point, vsol_polygon_3d_sptr poly3d,
  ...
  void divide_face(bwm_observable_sptr obs, unsigned face_id, float x1, float y1, float x2, float y2);
  ...
  void create_circular_polygon(vcl_vector<vsol_point_2d_sptr> ps_list, vsol_polygon_3d_sptr & circle, int num_sect,
                               double & r, vgl_point_2d<double> & c);
  ...
  static void project_meshes(vcl_vector<vcl_string> paths, vpgl_camera<double>* cam,
                             vcl_vector<vgl_polygon<double> > & poly_2d_list);
  ...
  void add_region(vcl_string name, double min_depth, double max_depth, unsigned order, unsigned orient,
                  unsigned land_id, double height);

  void add_vertical_depth_region(double min_depth, double max_depth, vcl_string name);
...
protected:
  ...
  bool intersect_ray_and_box(vgl_box_3d<double> box, vgl_point_2d<double> img_point, vgl_point_3d<double> & point);

  bool backproj_poly(vsol_polygon_2d_sptr poly2d, vsol_polygon_3d_sptr& poly3d, vgl_plane_3d<double> proj_plane);

  bool find_intersection_points(vgl_point_2d<double> const img_point1, vgl_point_2d<double> const img_point2,
                                vsol_polygon_3d_sptr poly3d, vgl_point_3d<double>& point1, vgl_point_3d<double>& l1,
                                vgl_point_3d<double>& l2, vgl_point_3d<double>& point2, vgl_point_3d<double>& l3,
