...
bwm_observer_img::bwm_observer_img(bgui_image_tableau_sptr const& img, vcl_string name, vcl_string image_path,
                                   bool display_image_path)
  : bgui_vsol2D_tableau(img), lock_vgui_status_(false), vgui_status_on_(false), draw_mode_(MODE_2D_POLY), img_tab_(img),
  viewer_(0),
  change_type_("change"), show_image_path_(false), start_x_(0), start_y_(0), moving_p_(0), moving_v_(0), moving_vertex_(
    false),
...
void bwm_observer_img::set_draw_mode(BWM_2D_DRAW_MODE mode)
{
  ...
  if( mode != draw_mode_ )
    ...
    for( vcl_map<unsigned, bgui_vsol_soview2D *>::iterator it = obj_list.begin();
         it != obj_list.end(); it++ )
      {
    ...
    for( vcl_map<unsigned, vcl_vector<bwm_soview2D_vertex *> >::iterator it = vert_list.begin();
         it != vert_list.end(); it++ )
      {
      vcl_vector<bwm_soview2D_vertex *> v_list = it->second;
...
void bwm_observer_img::copy()
{
  ...
  if( select_list.size() > 1 )
    {
    vcl_cerr << "Please select only one object to copy\n";
    return;
    }
...
void bwm_observer_img::paste(float x, float y)
{
  if( !copy_obj_ )
    {
    vcl_cerr << "No object is selected to paste, COPY first\n";
    return;
    }
...
void bwm_observer_img::delete_vertex(vgui_soview* vertex)
{
  ...
  if( v )
    ...
    if( obj->type_name().compare(POLYGON_TYPE) == 0 )
      ...
      if( i >= poly2d->size() )
        {
        vcl_cerr << "The index is invalid [" << i << " of " << poly2d->size() << '\n';
        return;
        }
...
void bwm_observer_img::display_reg_seg(vcl_vector<vsol_digital_curve_2d_sptr> const& search_edges,
...
void bwm_observer_img::create_mask()
{
  ...
  for( vcl_map<unsigned int, vsol_polygon_2d_sptr>::iterator pit = mask_polys_.begin();
  ...
  for( vcl_map<unsigned int, vsol_polygon_2d_sptr>::iterator pit = mask_dontcare_polys_.begin();
...
{
  return ground_truth_->create_mask_from_objs(img_tab_->get_image_resource()->ni(),
                                              img_tab_->get_image_resource()->nj(), "change");
}
