...
public:
  ...
  void intensity_profile(float start_col, float start_row, float end_col, float end_row);

  virtual void proj_poly(vsol_polygon_3d_sptr poly3d, vsol_polygon_2d_sptr& poly2d) = 0;

  virtual void proj_line(vsol_line_3d_sptr line_3d, vsol_line_2d_sptr & line_2d) = 0;

  virtual void proj_point(vgl_point_3d<double> world_pt, vgl_point_2d<double> & image_pt) = 0;

  virtual void backproj_poly(vsol_polygon_2d_sptr poly2d, vsol_polygon_3d_sptr& poly3d) = 0;
...
protected:
  ...
  void draw_mesh(bwm_observable_sptr observable, vcl_map<unsigned, bgui_vsol_soview2D *> & poly_list,
                 vcl_vector<bwm_soview2D_vertex *> & vertx_list, vcl_vector<vsol_point_2d_sptr> & vertx_xy_list);

  void draw_vertices(bwm_observable_sptr observable, vcl_map<unsigned, bgui_vsol_soview2D *> list, bool selectable,
                     vcl_vector<bwm_soview2D_vertex *> & vertx_list, vcl_vector<vsol_point_2d_sptr> & vertx_xy_list);
