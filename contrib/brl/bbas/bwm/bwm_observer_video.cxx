...
bool bwm_observer_video::add_match_at_vertex()
{
  ...
  if( !sov )
    {
    vcl_cerr << "In bwm_observer_video::add_video_corr_at_vertex() -"
             << " no vertex selected\n";
    return false;
    }
  ...
  if( !tracked_corr_ )
    {
    vcl_map<unsigned, bwm_video_corr_sptr>::iterator cit = video_corrs_.end();
...
void bwm_observer_video::display_projected_3d_point(bwm_video_corr_sptr const& corr)
{
...
void bwm_observer_video::clear_corr_display_map()
{
  ...
  for( ; fit != corr_soview_map_.end(); ++fit )
    {
    vcl_map<unsigned, bwm_soview2D_cross *>::iterator cit =
      (*fit).second.begin();
...
{
  vcl_map<unsigned, vgui_soview2D_point *>::iterator wit =
    world_pt_map_.begin();
...
bool bwm_observer_video::extract_neighborhoods(unsigned nhd_radius_x, unsigned nhd_radius_y,
...
bool bwm_observer_video::extract_histograms(vcl_vector<bsta_histogram<float> >& hists)
{
  video_istr_->seek_frame(0);
  while( true )
    {
...
bool bwm_observer_video::save_as_image_list(vcl_string const& path)
{
...
bool bwm_observer_video::intersect_ray_and_plane(vgl_point_2d<double> img_point,
