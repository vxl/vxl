...
class bwm_observer_video : public bwm_observer_cam
{
 public:
  ...
  bwm_observer_video(bgui_image_tableau_sptr const& img,
    ...
    time_interval_(0.0f), video_istr_(0), cam_istr_(0), prev_frame_(0)
    {init();}

  bwm_observer_video(bgui_image_tableau_sptr const& img)
    : bwm_observer_cam(img), play_video_(false), time_interval_(0.0f),
    video_istr_(0), cam_istr_(0), prev_frame_(0)
    {init();}
 ...
 protected:
  ...
  //: the frame number currently the previous frame
  unsigned prev_frame_;
