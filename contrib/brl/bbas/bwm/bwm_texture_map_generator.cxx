...
vgl_vector_3d<double> bwm_texture_map_generator::compute_face_normal_lvcs(bmsh3d_face* face, vpgl_lvcs lvcs)
{
  ...
  lvcs.global_to_local(centroid.x(), centroid.y(),
                       centroid.z(), vpgl_lvcs::wgs84, cx, cy, cz, vpgl_lvcs::DEG, vpgl_lvcs::METERS);

  double         vx, vy, vz;
  bmsh3d_vertex* last_v = (bmsh3d_vertex *) verts.back();
  lvcs.global_to_local(last_v->pt().x(), last_v->pt().y(),
                       last_v->pt().z(), vpgl_lvcs::wgs84, vx, vy, vz, vpgl_lvcs::DEG, vpgl_lvcs::METERS);
  vgl_vector_3d<double> a(vx - cx, vy - cy, vz - cz);
  for( unsigned i = 0; i < verts.size(); i++ )
    {
    bmsh3d_vertex* v = (bmsh3d_vertex *) verts[i];
    lvcs.global_to_local(v->pt().x(), v->pt().y(),
                         v->pt().z(), vpgl_lvcs::wgs84, vx, vy, vz, vpgl_lvcs::DEG, vpgl_lvcs::METERS);
