...
int main(int argc, char* * argv)
{
  ...
  if( argc != 9 )
    ...
    vcl_cout
      <<
    "usage: bwm_3d_fixed_transform -site_files <a txt file with the name of site xml files at each line> -gps <3d point file> -in_cam_prefix <uses glob: prefix*_cam.txt> -out_cam_dir <transform each input cam by the same fixed R|t and write into this folder>\n";
    return -1;
    }
  ...
  vcl_cout << "there are: " << n_pts
           << " corrs total in all the site files,expect to find this many 3d points in the gps file\n";
  ...
  if( pts_3d.size() != n_pts )
    {
    vcl_cerr
      << "input gps file and the number of correspondences in the input site files are not consistent! Exiting!\n";
    return -1;
    }
