...
static void
write_vrml_cameras(vcl_ofstream& str, vcl_vector<vpgl_perspective_camera<double> > & cams, double rad,
                   vcl_set<int> const& bad_cams)
{
  str << "#VRML V2.0 utf8\n"
      << "Background {\n"
      << "  skyColor [ 0 0 0 ]\n"
...
{
  os << "Transform {\n"
     << "  translation " << bounding_box.centroid_x() << ' ' << bounding_box.centroid_y() << ' '
     << bounding_box.centroid_z() << '\n'
     << "     children[\n"
     << "     Shape {\n"
     << "         appearance Appearance{\n"
     << "                         material Material{\n"
                      ...
                      vcl_vector<vpgl_perspective_camera<double> > & cams)
{
  ...
  if( 2 * up < cams.size() )// flip the world
    ...
    for( unsigned i = 0; i < cams.size(); ++i )
      ...
      new_cams[i] = vpgl_perspective_camera<double>(
          new_cams[i].get_calibration(), rot_cami, new_cams[i].get_translation() );
      }
    }
  for( unsigned i = 0; i < cams.size(); ++i )
    {
                            ...
                            vcl_map<unsigned, unsigned>&      view_count_map )
{
  ...
  for( unsigned i = 0; i < corrs.size(); ++i )
    ...
    for( vcl_map<unsigned, vgl_point_2d<double> >::iterator iter = matches.begin();
                  ...
                  float                       filter_thresh)
{
  ...
  for( vcl_map<unsigned, double>::iterator ve_itr = view_error_map.begin();
...
{
  // Get Inputs
  vul_arg<vcl_string> bundlerfile("-bundler", "Output file of bundler (required)",  "");
  vul_arg<vcl_string> img_dir("-img_dir",     "list of images filenames (required)", "");
  ...
  vul_arg<bool>       draw_box("-draw_box",
                               "Draw Bounding Box around points within 2*(standard deviation) from the center of scene",
                               true);
  vul_arg<bool>  filter("-filter_cams", "Filter camera based on Reprojection error of 3d correspondences", false);
  vul_arg<float> filter_thresh("-filter_thresh",
                               "Threshold for average rms value for a given view. Units are pixels", .75);
  ...
  for( unsigned i = 0; i < num_cams; ++i )
    ...
    if( !bad_cams.count(i) )
      ...
      // save cam file
      char filename[1024];
      vcl_sprintf(filename, "%s/camera%05d.txt", krt_dir.c_str(), cnt2);
  ...
  vcl_cout
           <<
  "Bounding Box containing points which are [-3,3]sigma about x and y and [-1,5]-z_sigma about the scene center: "
           << bounding_box2 << '\n'
  ...
  // --------------------------------------------------------------------------
  int good_cam = 0;
  while( bad_cams.count(good_cam) > 0 )
    {
    good_cam++;
    }
  ...
  for( vcl_set<int>::iterator iter = bad_cams.begin();
