...
public:
  ...
  bwm_reg_matcher(vcl_vector<vsol_digital_curve_2d_sptr> const& model_edges, unsigned search_col_origin,
                  unsigned search_row_origin, unsigned search_cols, unsigned search_rows,
  ...
  bool match(int& tcol, int& trow, double distance_threshold, double angle_threshold, double min_probability);

  // : Filter out model edges whose samples  don't lie within the specified distance of some search edge point
  bool close_edges(double filter_distance, double angle_threshold, unsigned min_curve_length,
