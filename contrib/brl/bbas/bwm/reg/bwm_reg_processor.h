...
public:
  ...
  bwm_reg_processor(vil_image_resource_sptr const& model_image, vpgl_rational_camera<double> const& model_cam,
                    vgl_point_3d<double> const& world_point, vgl_plane_3d<double> const& world_plane,
                    vil_image_resource_sptr const& search_image, vpgl_rational_camera<double> const& search_cam);
  ...
  bwm_reg_processor(vil_image_resource_sptr const& model_image, vpgl_rational_camera<double> const& model_cam,
                    vil_image_resource_sptr const& other_mode_image, vpgl_rational_camera<double> const& other_mode_cam,
                    vgl_point_3d<double> const& world_point, vgl_plane_3d<double> const& world_plane,
                    vil_image_resource_sptr const& search_image, vpgl_rational_camera<double> const& search_cam);
  ~bwm_reg_processor() {}
  ...
  bool match(double radius, double proj_error, double distance_threshold, double angle_threshold,
             unsigned min_curve_length, double min_probability, float model_noise_threshold,
             float search_noise_threshold, int& tcol, int& trow);
...
protected:
  ...
  bool scale(bwm_reg_image_sptr& image1, bwm_reg_image_sptr& image2, float& sigma1, float& sigma2);

  // : Filter irrelevant edges
  bool filter(double model_radius, double proj_error, double filter_distance, double angle_threshold,
              unsigned min_curve_length, float model_noise_threshold, float other_noise_threshold);
