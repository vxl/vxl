...
// : Constructor
bwm_video_cam_ostream::bwm_video_cam_ostream()
  : index_(0),
  dir_(),
  name_format_(),
  file_format_("vsl")
...
bwm_video_cam_ostream::bwm_video_cam_ostream(const vcl_string& directory,
...
// : Open the stream
bool
bwm_video_cam_ostream::open(const vcl_string& directory,
                            const vcl_string& name_format,
...
bool
bwm_video_cam_ostream::is_open() const
{
  return dir_ != "";
}
...
vcl_string
bwm_video_cam_ostream::next_file_name() const
{
  return dir_ + '/' + "camera"
...
bool
bwm_video_cam_ostream::write_camera(const vpgl_perspective_camera<double>*  cam)
{
  if( !cam )
    {
    return false;
...
// \retval false if the image could not be written
bool
bwm_video_cam_ostream::write_camera(const vpgl_proj_camera<double>*  cam)
{
  if( !cam )
    {
    return false;
