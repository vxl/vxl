...
public:
  ...
  bwm_video_cam_ostream(const vcl_string& directory, const vcl_string& name_format = "%05d",
                        const unsigned int init_index = 0);

  // : Destructor
  ~bwm_video_cam_ostream() { close(); }
  // : Open the stream
  bool open(const vcl_string& directory, const vcl_string& name_format = "%05d", const unsigned int init_index = 0);
