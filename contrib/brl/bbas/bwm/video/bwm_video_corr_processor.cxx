...
bwm_video_corr_lsqr_cost_func::bwm_video_corr_lsqr_cost_func(vil_image_view<float> const& base_image,
...
bwm_video_corr_cost_function::bwm_video_corr_cost_function(vil_image_view<float> const& base_image,
                                                           unsigned match_radius,
                                                           vcl_vector<float> corr_window_ab)
...
bool bwm_video_corr_processor::open_camera_istream(vcl_string const& camera_path)
{
  if( camera_path == "" )
    {
    return false;
  ...
  if( open )
    {
    vcl_cout << "in bwm_video_corr_processor::open_camera_istream() -- input stream: " << camera_path
             << " is opened!\n";
...
bool bwm_video_corr_processor::open_camera_ostream(vcl_string const& camera_path)
{
  if( camera_path == "" )
    {
    return false;
  ...
  if( open )
    {
    vcl_cout << "in bwm_video_corr_processor::open_camera_ostream() -- output stream: " << camera_path
             << " is opened!\n";
    }
  else
    {
...
void bwm_video_corr_processor::set_world_pts(vcl_vector<vgl_point_3d<double> > const& pts)
{
...
void bwm_video_corr_processor::write_cameras_txt(vcl_string const& cam_txt_dir,
                                                 vcl_vector<vpgl_perspective_camera<double> > const& cameras)
{
  char filename[1024];
    ...
    for( unsigned i = 0; i < cameras.size(); i++ )
      ...
      if( ofile )
        ...
        ofile << cameras[i].get_calibration().get_matrix() << '\n'
              ...
              << cameras[i].get_translation().x() << ' ' << cameras[i].get_translation().y() << ' '
              << cameras[i].get_translation().z() << '\n';
        }
      }
    }
}

bool bwm_video_corr_processor::interpolate_cameras(vcl_vector<vpgl_perspective_camera<double> > known_cams,
...
bool bwm_video_corr_processor::initialize_world_pts_and_cameras(vpgl_calibration_matrix<double> const& K,
...
void bwm_video_corr_processor::compute_ab_corr_windows(unsigned match_radius,
...
void bwm_video_corr_processor::set_correspondences(vcl_vector<bwm_video_corr_sptr> const& corrs)
{
  corrs_ = corrs;
}

void bwm_video_corr_processor::exhaustive_init(vnl_vector<double>& position,
                                               unsigned win_radius,
...
bool bwm_video_corr_processor::find_missing_correspondences(unsigned win_radius,
...
void bwm_video_corr_processor::print_frame_alignment_quality(unsigned start_frame, unsigned end_frame)
{
  unsigned n = corrs_.size();
