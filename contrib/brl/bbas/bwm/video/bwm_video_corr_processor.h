...
public:
  ...
  bwm_video_corr_lsqr_cost_func(vil_image_view<float> const& base_image, unsigned match_radius,
...
public:
  ...
  bwm_video_corr_cost_function(vil_image_view<float> const& base_image, unsigned match_radius,
                               vcl_vector<float> corr_window_ab);
...
public:
  ...
  bool initialize_world_pts_and_cameras(vpgl_calibration_matrix<double> const& K, double initial_depth);
  ...
  bool find_missing_correspondences(unsigned win_radius, unsigned search_radius, bool use_lmq = true);
...
protected:
  ...
  void mask(unsigned& min_frame, unsigned& max_frame, vcl_vector<vcl_vector<bool> >& mask);

  // : extract the float pixel windows for start and end frames used to find correspondences
  void compute_ab_corr_windows(unsigned match_radius, vcl_vector<bool> const& mask_a, vcl_vector<bool> const& mask_b);
  ...
  bool find_missing_corrs(unsigned frame_index_a, vcl_vector<bool> mask_a, unsigned frame_index_b,
                          vcl_vector<bool> mask_b, unsigned frame_index_x, unsigned win_radius, unsigned search_radius,
                          bool use_lmq);
  ...
  bool interpolate_cameras(vcl_vector<vpgl_perspective_camera<double> > kcams, vcl_vector<bool> unknown,
...
// at the start and end of the search is reported
  void exhaustive_init(vnl_vector<double>& unknowns, unsigned win_radius, unsigned search_radius,
                       vil_image_view<float> const& base, vcl_vector<float> corr_win, double& start_error,
                       double& end_error);
