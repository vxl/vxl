...
static bool process_camera_from_photo_overlay(vcl_string const& params_path, vcl_string const& output_cam_path)
{
  ...
  if( ofile )
    ...
    ofile << camera.get_calibration().get_matrix() << '\n'
          ...
          << camera.get_translation().x() << ' ' << camera.get_translation().y() << ' '
          << camera.get_translation().z() << '\n';
    }
...
int main(int argc, char* * argv)
{
  ...
  vul_arg<bool> dont_use_H(arglist, "-no_H",
                           "don't find 3D to 3D transformation to map bundle adjusted world points back to gt world points",
                           false);
  ...
  if( camera() )    // run the process that reads camera parameters and creates perspective camera
    ...
    if( dont_use_H() )
      ...
      {
      if( !process(site_path(), cal_matrix_path(), initial_depth(), out_site_path(), out_cam_dir(),
                   out_proj_cam_dir() ) )
        {
        return -1;
        }
