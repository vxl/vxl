                             ...
                             vcl_vector<float> & corr_int)
{
  for( unsigned i = 0; i < img_files.size(); ++i )
    ...
    if( vil_image_view<unsigned short> * img_byte = dynamic_cast<vil_image_view<unsigned short> *>(img_ptr.ptr() ) )
      ...
      vil_convert_stretch_range_limited<unsigned short>(*img_byte, *floatimg, (unsigned short)29000,
                                                        (unsigned short) 34000, 0.0f, 1.0f);
...
int main(int argc, char* * argv)
{
  ...
  vcl_string cam_glob = vul_file::dirname(camdir() ) + "/*.???";
  vcl_string obj_glob = vul_file::dirname(objdir() ) + "/*.???";
  ...
  while( img_file_it && cam_file_it )
    ...
    vcl_string camname(cam_file_it() );
  ...
  while( obj_file_it )
    {
    vcl_string objname(obj_file_it() );
  ...
  for( unsigned j = 0; j < mean_ref_intensity.size(); ++j )
    ...
    stdev_ref_intensity[j] =
      vcl_sqrt( (stdev_ref_intensity[j] / mean_ref_count[j] - mean_ref_intensity[j] * mean_ref_intensity[j]) );
  ...
  for( unsigned i = 0; i < img_files.size(); ++i )
    ...
    if( vil_image_view<unsigned short> * img_byte = dynamic_cast<vil_image_view<unsigned short> *>(img_ptr.ptr() ) )
      {
      vil_image_view<float>* floatimg = new vil_image_view<float>(img_byte->ni(), img_byte->nj(), 1);
      vil_convert_stretch_range_limited<unsigned short>(*img_byte, *floatimg, (unsigned short)5500,
                                                        (unsigned short) 6600, 0.0f, 1.0f);
      ...
      if( count >= 2 )
        ...
        vil_convert_stretch_range_limited<float>(*floatimg, *img_byte, 0.0f, 1.0f, (unsigned short)0,
                                                 (unsigned short)(256 * 256 - 1) );
