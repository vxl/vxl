...
void imesh_imls_surface::compute_unweighed_rec(const vcl_auto_ptr<imesh_kd_tree_node>& node)
{
  const unsigned int& i = node->index_;
  ...
  while( !remain.empty() && -remain.back().val_ > lambda_ * sums.I )
    ...
    if( current->is_leaf() )
      ...
      typedef integral_data (* F) (const vgl_point_3d<double> &, const vgl_point_3d<double> &,
                                   const vgl_point_3d<double> &, const vgl_point_3d<double> &, double, double, double,
                                   double);
...
//  call triangle_quadrature to first split an arbitrary triangle
//  \a eps2 is epsilon^2
imesh_imls_surface::integral_data imesh_imls_surface::split_triangle_quadrature_with_deriv(
