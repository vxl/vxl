...
void imesh_project_verts(const vcl_vector<vgl_point_3d<double> >& verts3d, const vpgl_proj_camera<double>& camera,
                         vcl_vector<vgl_point_2d<double> >& verts2d);

// : project the 3D vertices into 2D using the camera
void imesh_project_verts(const imesh_vertex_array<3>& verts3d, const vpgl_proj_camera<double>& camera,
                         vcl_vector<vgl_point_2d<double> >& verts2d);

// : project the 3D vertices into 2D vertices and depths using the camera
void imesh_project_verts(const vcl_vector<vgl_point_3d<double> >& verts3d, const vpgl_proj_camera<double>& camera,
                         vcl_vector<vgl_point_2d<double> >& verts2d, vcl_vector<double>& depths);
...
void imesh_distort_verts(const vcl_vector<vgl_point_2d<double> >& in_verts, const vpgl_lens_distortion<double>& lens,
...
void imesh_project(const imesh_mesh& mesh, const vpgl_proj_camera<double>& camera,
                   const vpgl_lens_distortion<double>& lens, vil_image_view<bool>& image);
...
void imesh_project(const imesh_mesh& mesh, const vcl_vector<vgl_vector_3d<double> >& normals,
                   const vpgl_proj_camera<double>& camera, const vpgl_lens_distortion<double>& lens,
                   vil_image_view<bool>& image, vgl_box_2d<unsigned int>* bbox = 0);
...
void imesh_render_triangle_interp(const vgl_point_2d<double>& v1, const vgl_point_2d<double>& v2,
                                  const vgl_point_2d<double>& v3, const double& i1, const double& i2, const double& i3,
...
void imesh_render_triangles_interp(const imesh_regular_face_array<3>& tris,
                                   const vcl_vector<vgl_point_2d<double> >& img_verts, const vcl_vector<double>& vals,
                                   vil_image_view<double>& image);

// : Triangulates the faces and then calls imesh_render_triangles_interp
void imesh_render_faces_interp(const imesh_mesh& mesh, const vcl_vector<vgl_point_2d<double> >& img_verts,
                               const vcl_vector<double>& vals, vil_image_view<double>& image);
...
int imesh_project_onto_mesh(const imesh_mesh& mesh, const vcl_vector<vgl_vector_3d<double> >& normals,
                            const vcl_vector<vgl_point_2d<double> >& verts2d,
                            const vpgl_perspective_camera<double>& camera, const vgl_point_2d<double>& pt_2d,
                            vgl_point_3d<double>& pt_3d);

// : project a texture point onto a mesh face index with barycentric coordinates
//  Returns the index of the intersected triangle, or -1 for no intersection
int imesh_project_texture_to_barycentric(const imesh_mesh& mesh, const vgl_point_2d<double>& pt_2d,
...
bool imesh_project_texture_to_barycentric(const imesh_mesh& mesh, const vcl_vector<vgl_point_2d<double> >& pts_2d,
                                          vcl_vector<vgl_point_2d<double> >& pts_uv, vcl_vector<unsigned long>& idxs,
...
vnl_matrix_fixed<double, 3, 3> imesh_affine_map(const vgl_point_2d<double>& a1, const vgl_point_2d<double>& b1,
                                                const vgl_point_2d<double>& c1, const vgl_point_2d<double>& a2,
                                                const vgl_point_2d<double>& b2, const vgl_point_2d<double>& c2);

// : project barycentric coordinates with an index to texture space
//  \param idx is the face index
vgl_point_2d<double> imesh_project_barycentric_to_texture(const imesh_mesh& mesh, const vgl_point_2d<double>& pt_uv,
                                                          unsigned int idx);

// : project barycentric coordinates with an index the mesh surface (3D)
//  \param idx is the face index
vgl_point_3d<double> imesh_project_barycentric_to_mesh(const imesh_mesh& mesh, const vgl_point_2d<double>& pt_uv,
                                                       unsigned int idx);
...
void imesh_project_onto_mesh(const imesh_mesh& mesh, const vcl_vector<vgl_vector_3d<double> >& normals,
                             const vpgl_perspective_camera<double>& camera,
                             const vcl_vector<vgl_point_2d<double> >& pts_2d, vcl_vector<unsigned int>& idx_2d,
                             vcl_vector<vgl_point_3d<double> >& pts_3d);

// : back project an image point onto the mesh using the camera
//  The resulting point is in barycentric coordinates for the returned triangle
//  Returns the index of the intersected triangle, or -1 for no intersection
int imesh_project_onto_mesh_barycentric(const imesh_mesh& mesh, const vcl_vector<vgl_vector_3d<double> >& normals,
                                        const vcl_vector<vgl_point_2d<double> >& verts2d,
                                        const vpgl_perspective_camera<double>& camera,
                                        const vgl_point_2d<double>& pt_img, vgl_point_2d<double>& pt_bary);
...
//  Returns the index of the intersected triangle, or -1 for no intersection
int imesh_project_onto_mesh_texture(const imesh_mesh& mesh, const vcl_vector<vgl_point_2d<double> >& verts2d,
                                    const vpgl_perspective_camera<double>& camera, const vgl_point_2d<double>& pt_img,
                                    vgl_point_2d<double>& pt_uv);
