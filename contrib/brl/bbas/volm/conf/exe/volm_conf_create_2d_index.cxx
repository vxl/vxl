...
int main(int argc, char* * argv)
{
  ...
  vul_arg<vcl_string> geo_folder("-hypo", "folder to read the geo hypotheses", "");
  vul_arg<vcl_string> land_index_folder("-land", "folder to read land map index", "");
  vul_arg<vcl_string> out_folder("-out", "folder to put created index", "");
  vul_arg_parse(argc, argv);
  vcl_cout << "argc: " << argc << vcl_endl;
  // input check
  if( tile_id() == 9999 || geo_folder().compare("") == 0 || land_index_folder().compare("") == 0 ||
      out_folder().compare("") == 0 )
           {  vul_arg_display_usage_and_exit();  return volm_io::EXE_ARGUMENT_ERROR;  }

  vcl_stringstream log_file;
  log_file << out_folder() << "/log_tile_" << tile_id() << "_leaf_" << min_leaf_id() << "_to_" << max_leaf_id()
           << ".xml";
  vcl_stringstream log;

  // create index
  vcl_cout << "-------------- Start to create 2d configurational index for tile " << tile_id() << vcl_endl;
  ...
  vcl_cout << "\t " << indexer->loc_leaves().size() << " location leaves are loaded from geo location folder: "
           << geo_folder() << vcl_endl;
  ...
  log << "indexing " << num_locs << " locations consumes " << t.all() / (1000.0 * 60.0) << " seconds (" << t.all()
  / (1000.0 * 60.0 * 60.0) << " minutes)" << vcl_endl;
