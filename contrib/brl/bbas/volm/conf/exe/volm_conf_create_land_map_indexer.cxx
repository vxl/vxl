...
int main(int argc, char* * argv)
{
  ...
  vul_arg<unsigned>   is_osm_junction("-junction",
                                      "option to put OSM road intersection into database (default is true)", 1);
  ...
  bvgl_2d_geo_index_node_sptr root = bvgl_2d_geo_index::construct_tree<volm_conf_land_map_indexer_sptr>(
      tiles[tile_id()].bbox_double(), min_size(), roi_poly);
  ...
  vcl_cout << "2D bvgl geo index is created with min size: " << min_size() << " and " << leaves.size()
           << " leaves (depth " << tree_depth << ") are inside ROI.\n";
  ...
  vcl_cout << "read " << osm.num_locs() << " OSM location points, " << osm.num_roads() << " OSM roads and "
  ...
  for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
    ...
    for( unsigned i = 0; i < build_polys.size(); i++ )
      ...
      if( !leaf_ptr->contents_->add_locations(poly, land_id, build_heights[i]) )
        {
        log << "ERROR: adding location from URGENT building: " << build_polys[i].second << " failed." << vcl_endl;
        error(log_file, log.str() );
        return volm_io::EXE_ARGUMENT_ERROR;
        }
#endif
    ...
    vcl_cout << "\t   " << leaf_ptr->contents_->nlocs() << " locations (" << leaf_ptr->contents_->nland_type()
             << " land types) are added after loading URGENT data"
             << vcl_flush << vcl_endl;
    ...
    vcl_cout << "\t   " << leaf_ptr->contents_->nlocs() << " locations (" << leaf_ptr->contents_->nland_type()
             << " land types) are added after loading SME data"
             << vcl_flush << vcl_endl;
    ...
    if( is_osm_pts() ) // add OSM location points
      ...
      vcl_cout << "\t   " << leaf_ptr->contents_->nlocs() << " locations (" << leaf_ptr->contents_->nland_type()
               << " land types) are added after loading OSM points"
               << vcl_flush << vcl_endl;
      }
    ...
    if( is_osm_road() )  // add OSM location roads
      ...
      for( unsigned r_idx = 0; r_idx < n_lines; r_idx++ )
        ...
        //  continue;
        leaf_ptr->contents_->add_locations(osm.loc_lines()[r_idx]->line(),
                                           osm.loc_lines()[r_idx]->prop().id_, -1.0, road_density() );
        }
      vcl_cout << "\t   " << leaf_ptr->contents_->nlocs() << " locations (" << leaf_ptr->contents_->nland_type()
               << " land types) are added after loading OSM roads"
               << vcl_flush << vcl_endl;
      }
    ...
    if( is_osm_regions() )  // add OSM location regions
      ...
      vcl_cout << "\t   " << leaf_ptr->contents_->nlocs() << " locations (" << leaf_ptr->contents_->nland_type()
               << " land types) are added after loading OSM regions"
               << vcl_flush << vcl_endl;
      }
