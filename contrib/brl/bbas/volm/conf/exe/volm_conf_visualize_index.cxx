...
int main(int argc, char* * argv)
{
  ...
  if( !vul_file::exists(loc_file_pre.str() + ".txt") )
    {
    vcl_cerr << "ERROR: can not find geo location index for tile " << tile_id() << " -- " << loc_file_pre.str()
             << ".txt\n";
    return 1;
    }
  float                    min_size;
  ...
  if( !leaf )
    {
    vcl_cerr << "ERROR: the geo index: " << geo_hypo_folder() << " does not contain any hyp close to " << loc_lon()
             << ", " << loc_lat() << '\n';
    return 1;
    }
  vcl_cout << "location " << loc_lon() << ", " << loc_lat() << " is in leaf " << leaf->extent_
           << " and closest location is: " << leaf->hyps_->locs_[hyp_id].x() << ", "
           << leaf->hyps_->locs_[hyp_id].y() << vcl_endl;
  ...
  kml_file << vul_file::strip_extension(bin_file_name) << '_' << vcl_setprecision(6) << vcl_fixed
           << leaf->hyps_->locs_[hyp_id].x()
           << '_' << vcl_setprecision(6) << vcl_fixed << leaf->hyps_->locs_[hyp_id].y() << ".kml";
  volm_conf_object::write_to_kml(leaf->hyps_->locs_[hyp_id].x(), leaf->hyps_->locs_[hyp_id].y(), values,
                                 kml_file.str() );
