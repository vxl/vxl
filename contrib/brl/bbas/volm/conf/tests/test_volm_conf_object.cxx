...
static void test_volm_conf_object()
{
  ...
  float theta =
    (vcl_atan2(pt.y(), pt.x() ) < 0) ? (float)vcl_atan2(pt.y(), pt.x() ) + vnl_math::twopi : (float)vcl_atan2(
      pt.y(), pt.x() );
  ...
  vcl_cout << "size of its member: " << sizeof(obj_sptr->theta() ) + sizeof(obj_sptr->dist() )
  + sizeof(obj_sptr->land() ) + sizeof(obj_sptr->height() ) << " bytes" << vcl_endl;
