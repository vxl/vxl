...
static void test_volm_conf_query_from_tag_file()
{
  ...
  bool                 success = volm_io::read_conf_query_tags(xml_file, floor_height, dms, world_region, img_ni,
                                                               img_nj, query_name);
  ...
  for( unsigned i = 0; i < conf_objs.size(); i++ )
    ...
    for( vcl_map<vcl_string, volm_conf_object_sptr>::iterator mit = conf_objs[i].begin();  mit != conf_objs[i].end();
         ++mit )
      {
      vcl_cout << "\t\t obj name: " << mit->first << "\t\t";  mit->second->print(vcl_cout);
      vcl_cout << "\t\t  distance tolerance: " << d_tol[i][mit->first].first << " to " << d_tol[i][mit->first].second
               << vcl_endl;
      }
    }
