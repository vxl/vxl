...
static void test_volm_conf_score()
{
  ...
  TEST_NEAR("Testing angular value to north",
            score_sptr->theta_to_north(), vnl_math::pi_over_2 / vnl_math::pi_over_180, EPISLON);
  ...
  float buffer_capacity = 200000.0f * 2.0f * sizeof(volm_conf_score)
    / (1024.0f * 1024.0f * 1024.0f);                                                           // buffer size that can store 200000 objects
  ...
  for( unsigned k = 0; k < data_size; k++ )
    ...
    for( unsigned i = 0; i < size; i++ )
      {
      landmarks.push_back(volm_conf_object(rnd.drand32(0.0f, vnl_math::twopi), rnd.drand32(0.0f, 200.0f),
                                           rnd.drand32(0.0f, 32.f), rnd.lrand32(0, 255) ) );
      }
  ...
  for( unsigned k = 0; k < data_size; k++ )
    ...
    if( k < 10 )
      ...
      vcl_cout << "   input landmarks " << k << ": " << scores[k].landmarks().size() << ", loaded landmarks " << k
               << ": " << scores_in[k].landmarks().size() << vcl_endl;
      }
