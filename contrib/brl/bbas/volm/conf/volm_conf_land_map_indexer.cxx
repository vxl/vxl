...
volm_conf_land_map_indexer::volm_conf_land_map_indexer(double const& min_lon, double const& max_lon,
                                                       double const& min_lat, double const& max_lat,
...
volm_conf_land_map_indexer::volm_conf_land_map_indexer(float  const& min_lon, float  const& max_lon,
                                                       float  const& min_lat, float  const& max_lat,
                                                       float  const& density)
{
...
bool volm_conf_land_map_indexer::add_locations(vil_image_view<vxl_byte> const& image, vpgl_geo_camera* geocam,
                                               vcl_string const& img_type)
{
  ...
  vcl_cout << "min_x: " << bbox_.min_x() << " min_y: " << bbox_.max_y() << ", min_u: " << min_uu << ", min_v: "
           << min_vv << vcl_endl;
  vcl_cout << "max_x: " << bbox_.max_x() << " max_y: " << bbox_.min_y() << ", max_u: " << max_uu << ", max_v: "
           << max_vv << vcl_endl;
  ...
  for( vcl_set<unsigned char>::iterator sit = land_types.begin(); sit != land_types.end();  ++sit )
    ...
    if( img_type == "nlcd" || img_type == "NLCD" )
      ...
      {
      vcl_cout << "In volm_conf_land_map_indexer, unknown image type: " << img_type
               << ", only nlcd/NLCD/geocover allowed" << vcl_endl;
      return false;
      }
    ...
    for( unsigned e_idx = 0; e_idx < sampled_edge_locs.size(); e_idx++ )
      {
      for( vcl_vector<vsol_point_2d_sptr>::iterator vit = sampled_edge_locs[e_idx].begin();
           vit != sampled_edge_locs[e_idx].end(); ++vit )
        {
...
bool volm_conf_land_map_indexer::add_locations(vcl_vector<vgl_point_2d<double> > const& locs, unsigned char const& land,
                                               double const& height, double const& density)
{
...
bool volm_conf_land_map_indexer::add_locations(vcl_vector<vcl_vector<vgl_point_2d<double> > > const& lines,
                                               vcl_vector<unsigned char> const& lines_prop)
{
  if( lines.size() != lines_prop.size() )
    {
    vcl_cerr
      <<
    " In volm_conf_land_map_indexer::add_location: number of defined line properties is unequal to number of lines in the network"
      << vcl_endl;
    return false;
    }
  ...
  for( unsigned r_idx = 0; r_idx < n_lines_in; r_idx++ )
    ...
    if( !volm_io_tools::search_junctions(curr_line, curr_prop, net, net_props, cross_pts, cross_props,
                                         cross_geo_props) )
      {
      vcl_cerr << "In volm_conf_land_map_indexer::add_location: find line intersection failed for line " << r_idx
               << vcl_endl;
      return false;
      }
    ...
    for( unsigned i = 0; i < cross_pts.size(); i++ )
      ...
      if( vcl_find(land_locs_[cross_props[i].id_].begin(), land_locs_[cross_props[i].id_].end(),
                   vgl_point_3d<double>(lon, lat, -1.0) ) == land_locs_[cross_props[i].id_].end() )
        {
...
void volm_conf_land_map_indexer::upsample_location_list(vcl_vector<vsol_point_2d_sptr> const& in_locs,
                                                        vcl_vector<vsol_point_2d_sptr>& out_locs, double const& density)
{
  out_locs.clear();
...
void volm_conf_land_map_indexer::upsample_region_boundary(vgl_polygon<double> const& poly,
                                                          vcl_vector<vsol_point_2d_sptr>& out_locs,
                                                          double const& density)
{
  out_locs.clear();
...
void volm_conf_land_map_indexer::write_out_kml_locs(vcl_ofstream& ofs,
                                                    vcl_vector<vgl_point_3d<double> > const& locations,
                                                    unsigned char land_id,
...
bool volm_conf_land_map_indexer::write_out_kml(vcl_string const& kml_file, unsigned char const& land_id,
                                               double const& size,  bool const& is_write_as_dot) const
{
  if( land_locs_.find(land_id) == land_locs_.end() )
    {
    vcl_cout << "In volm_conf_land_map_indexer: no land id " << (int)land_id << " in database, kml won't be created"
             << vcl_endl;
    return false;
    }
...
bool volm_conf_land_map_indexer::write_out_kml(vcl_string const& kml_file, double const& size,
                                               bool const& is_write_as_dot) const
{
  if( land_locs_.empty() )
    {
...
void volm_conf_land_map_indexer::b_read(vsl_b_istream& is)
{
  ...
  switch( ver )
    ...
    default:
      vcl_cerr << "I/O ERROR: volm_conf_land_map_indexer::b_read(vsl_b_istream&): Unknown version number " << ver
               << vcl_endl;
