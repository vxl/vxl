...
public:
  ...
  volm_conf_land_map_indexer(double const& min_lon, double const& max_lon, double const& min_lat, double const& max_lat,
                             double const& density);
  volm_conf_land_map_indexer(float  const& min_lon, float  const& max_lon, float  const& min_lat, float  const& max_lat,
                             float  const& density);
  ...
  bool add_locations(vil_image_view<vxl_byte> const& image, vpgl_geo_camera* camera,
                     vcl_string const& img_type = "nlcd");
  ...
  bool add_locations(vcl_vector<vgl_point_2d<double> > const& locs, unsigned char const& land,
                     double const& height = -1.0, double const& density = -1.0);
  ...
  bool add_locations(vcl_vector<vcl_vector<vgl_point_2d<double> > > const& lines,
  ...
  void upsample_location_list(vcl_vector<vsol_point_2d_sptr> const& in_locs, vcl_vector<vsol_point_2d_sptr>& out_locs,
  ...
  void upsample_region_boundary(vgl_polygon<double> const& poly, vcl_vector<vsol_point_2d_sptr>& out_locs,
                                double const& density = -1.0);
  ...
  bool write_out_kml(vcl_string const& kml_file, unsigned char const& land_id, double const& size = 1E-5,
                     bool const& is_write_as_dot = false) const;
...
private:
  ...
  void write_out_kml_locs(vcl_ofstream& ofs, vcl_vector<vgl_point_3d<double> > const& locations, unsigned char land_id,
