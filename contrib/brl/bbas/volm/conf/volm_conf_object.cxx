               ...
               vcl_string const& kml_file)
{
  ...
  for( unsigned i = 0; i < n; i++ )
    ...
    loc_name << i << "_" << vcl_setprecision(4) << loc_lon << '_' << vcl_setprecision(4) << loc_lat << '_'
             << vcl_setprecision(6) << values[i].height()
...
::b_read(vsl_b_istream& is)
{
  ...
  if( ver == 1 )
    ...
    {
    vcl_cout << "volm_conf_object: binary read -- unknown binary io version: " << (int)ver
             << ", most updated version is " << (int)this->version()
             << vcl_flush << vcl_endl;
    return;
    }
}
