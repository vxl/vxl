...
bool volm_conf_query::create_conf_object()
{
  ...
  if( tol_in_pixel_ == 0 )
    ...
    for( unsigned cam_id = 0;  cam_id < ncam_;  cam_id++ )
      ...
      for( vcl_map<vcl_string, vcl_pair<unsigned, unsigned> >::iterator mit = conf_pixels.begin();
           mit != conf_pixels.end();  ++mit )
        {
        float min_dist = conf_objs[mit->first]->dist() * 0.8;
        float max_dist = conf_objs[mit->first]->dist() * 1.2;
        conf_dist_tol.insert(vcl_pair<vcl_string, vcl_pair<float, float> >(mit->first,
                                                                           vcl_pair<float, float>(min_dist,
                                                                                                  max_dist) ) );
        }
...
#  endif
    ...
    pt_pixels.insert(vcl_pair<float, vcl_pair<unsigned, unsigned> >(dist,
                                                                    vcl_pair<unsigned, unsigned>( (unsigned)x,
                                                                                                  (unsigned)y) ) );
    }
...
bool volm_conf_query::visualize_ref_objs(vcl_string const& in_file, vcl_string const& out_folder)
{
  ...
  for( unsigned cam_id = 0; cam_id < ncam_; cam_id++ )
    ...
    for( vcl_map<vcl_string, vcl_pair<unsigned, unsigned> >::iterator mit = conf_objects_pixels_[cam_id].begin();
         mit != conf_objects_pixels_[cam_id].end(); ++mit )
      {
      ...
      this->plot_dot_into_image(img, vgl_point_2d<double>( (double)mit->second.first,
                                                           (double)mit->second.second), r, g, b, width);
      }
