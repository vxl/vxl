...
int main(int argc,  char* * argv)
{
  ...
  vul_arg<bool> random("-random",
                       "if exists create output prob maps randomly, otherwise run the land type matcher");
  vul_arg<vcl_string> category_file("-cat",
                                    "category file that contains one line for the land type of the camera for the query ",
                                    "");
  vul_arg<vcl_string> category_gt_file("-cat_gt",
                                       "category gt file of multiple images, use image id to get gt loc of this one ",
                                       "");
  ...
  vul_arg<vcl_string> maps_folder("-maps",
                                  "the classification map images where each pixel has the land id (type is unsigned char)",
                                  "");
  vul_arg<vcl_string> radii_string("-rad",
                                   "e.g. pass 100_500_1000 for radii of 100, 500 and 1000 meter to construct descriptors at each location",
                                   "");
  ...
  if( match() )    // run the matcher
    ...
    else if( world_str().compare("Desert") == 0 )
      {
      tiles = volm_tile::generate_p1_wr1_tiles();
      }
    else
      {
      vcl_cout << "ERROR: unknown ROI region: " << world_str()
               << ".  Available regions are: Coast, Desert, Chile, India, Jordan, Philippines, Taiwan\n";
    ...
    if( category_gt_file().compare("") == 0 || out_folder().compare("") == 0 || geo_hypo_folder().compare("") == 0 ||
        tile_id() < 0 || img_id() < 0 || desc_index_folder().compare("") == 0 || NLCD_folder().compare("") == 0 )
      {
      vcl_cerr << "EXE_ARGUMENT_ERROR!\n";
      volm_io::write_status(out_folder(), volm_io::EXE_ARGUMENT_ERROR);
      vul_arg_display_usage_and_exit();
    ...
    if( land_name().compare("") != 0 &&
        volm_osm_category_io::volm_land_table_name.find(land_name() ) ==
    ...
    float gt_score;
    m->create_prob_map(geo_hypo_folder(), out_folder(), tile_id(),
                       tiles[tile_id()], query_img_info[img_info_id].first, gt_score);
