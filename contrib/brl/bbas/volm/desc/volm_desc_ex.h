...
public:
  volm_object() {}
  volm_object(double const& dist, depth_map_region::orientation const& orient, unsigned const& land,
...
public:
  ...
  volm_desc_ex(depth_map_scene_sptr const& dms, vcl_vector<double> const& radius, unsigned const& norients = 3,
  ...
  volm_desc_ex(vcl_vector<unsigned char> const& index_dst, vcl_vector<unsigned char> const& index_combined,
               vcl_vector<double> depth_interval, vcl_vector<double> const& radius, unsigned const& norients = 3,
               unsigned const& nlands = volm_osm_category_io::volm_land_table.size(),
               unsigned char const& initial_mag = 0);
  ...
  volm_desc_ex(unsigned ndists, unsigned norients, unsigned nlands, vcl_vector<double>& radius) :
    ndists_(ndists), norients_(norients), nlands_(nlands), radius_(radius)
  {
    nbins_ = ndists_ * nlands_; h_.resize(
      nbins_); initialize_bin(0);
  }
  ...
  void set_count(double const& dist, unsigned const& orient, unsigned const& land, unsigned char const& count);
