...
volm_desc_ex_2d_indexer::volm_desc_ex_2d_indexer(vcl_string const& input_folder,
                                                 vcl_string const& out_index_folder,
                                                 vcl_vector<double> const& radius,
                                                 unsigned const& nlands) : volm_desc_indexer(out_index_folder), nlands_(
    nlands)
{
...
bool volm_desc_ex_2d_indexer::get_next()
{
  ...
  for( unsigned i = 0; i < classification_maps_.size(); i++ )
    ...
    if( vgl_intersection(classification_maps_[i].bbox, extent).area() > 0 )
      ...
      if( !classification_maps_[i].img_r )
        ...
        classification_maps_[i].ni = classification_maps_[i].img_r->ni();
        classification_maps_[i].nj = classification_maps_[i].img_r->nj();
        }
  ...
  vcl_cout << "!!!!!!! there are: " << current_leaf_maps_.size()
           << " classification maps that intersect the current leaf!\n";
  vcl_cout.flush();
  return true;
}
...
bool volm_desc_ex_2d_indexer::extract(double lat, double lon, double elev, vcl_vector<unsigned char>& values)
{
  ...
  for( vcl_map<unsigned, vcl_vector<vcl_pair<int, int> > >::iterator it = imgs_and_corners.begin();
       it != imgs_and_corners.end(); it++ )
    {
