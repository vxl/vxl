...
public:
  ...
  volm_desc_ex_land_only(unsigned ndists, unsigned nlands, vcl_vector<double>& radius) :
    ndists_(ndists), nlands_(nlands),
    radius_(radius)
  {
    nbins_ = ndists_ * nlands_; h_.resize(nbins_); initialize_bin(0);
    name_ = "existence land only descriptor";
  }

  // : Constructor from depth map scene
  volm_desc_ex_land_only(depth_map_scene_sptr const& dms, vcl_vector<double> const& radius,
  ...
  volm_desc_ex_land_only(vcl_vector<unsigned char> const& index_dst, vcl_vector<unsigned char> const& index_combined,
                         vcl_vector<double> depth_interval, vcl_vector<double> const& radius,
                         unsigned const& nlands = volm_label_table::compute_number_of_labels(),
                         unsigned char const& initial_mag = 0);
