                                ...
                                unsigned const& tile_id)
{
  ...
  if( !params.read_params_file(index_file_name_pre.str() ) )
    {
    vcl_cerr << " ERROR: can not open index parameter file: "
             << vul_file::strip_extension(index_file_name_pre.str() ) << ".params\n";
    return false;
    }
  ...
  for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
    {
    vcl_string index_file = index_file_name_pre.str() + "_" + leaves[l_idx]->get_string() + "_"
      + this->get_index_type_str() + ".bin";
...
bool volm_desc_matcher::create_random_prob_map(vnl_random& rng, vcl_string const& geo_hypo_folder,
                                               vcl_string const& out_folder, unsigned tile_id, volm_tile& tile)
{
                                              ...
                                              vcl_string const& world_str)
{
  ...
  vcl_cout << " there are " << cand_map.size() << " candidate regions given threshold = " << threshold
