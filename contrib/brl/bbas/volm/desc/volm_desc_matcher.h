...
public:
  ...
  bool matcher(volm_desc_sptr const& query, vcl_string const& geo_hypo_folder, vcl_string const& desc_index_folder,
               float buffer_capacity, unsigned const& tile_id);

  // : write the matcher scores
  bool write_out(vcl_string const& out_folder, unsigned const& tile_id);

  // : generate probability map
  bool create_prob_map(vcl_string const& geo_hypo_folder, vcl_string const& out_folder, unsigned const& tile_id,
                       volm_tile tile, vgl_point_3d<double> const& gt_loc, float& gt_score);
  ...
  bool create_random_prob_map(vnl_random& rng, vcl_string const& geo_hypo_folder, vcl_string const& out_folder,
                              unsigned tile_id, volm_tile& tile);

  // : generate scaled probability map
  static bool create_scaled_prob_map(vcl_string const& out_folder, volm_tile tile, unsigned const& tile_id,
                                     float const& ku, float const& kl, float const& thres_value);

  // : generate candidate list
  static bool create_candidate_list(vcl_string const& map_root, vcl_string const& cand_root, unsigned const& threshold,
                                    unsigned const& top_size, float const& ku, float const& kl,
                                    float const& thres_value, vcl_string const& query_name, vcl_string const& world_id);
