...
int main(int argc, char* * argv)
{
  ...
  for( vcl_vector<depth_map_region_sptr>::iterator rit = sky_reg.begin(); rit != sky_reg.end(); ++rit )
    {
    vcl_cout << "\tname = " <<  (*rit)->name() << "\t orient = " << (*rit)->orient_type() << "\t NLCD = "
             << (*rit)->land_id()
  ...
  for( vcl_vector<depth_map_region_sptr>::iterator rit = grd_reg.begin(); rit != grd_reg.end(); ++rit )
    {
    vcl_cout << "\tname = " << (*rit)->name() << "\t orient_id = " << (*rit)->orient_type() << " orient = "
             << (*rit)->orient_string( (unsigned char)(*rit)->orient_type() )
             << "\t land_id = " << (*rit)->land_id() << "\t land_name = "
             << volm_label_table::land_string( (unsigned char)(*rit)->land_id() )
             << "\t order = " << (*rit)->order() << vcl_endl;
    }
  vcl_cout << " ----------------- OBJECTS ----------------------\n";
  ...
  for( vcl_vector<depth_map_region_sptr>::iterator rit = regs.begin(); rit != regs.end(); ++rit )
    {
    vcl_cout << "name = " << (*rit)->name() << ",\t orient = " << (*rit)->orient_type() << "\t orient_string = "
             << (*rit)->orient_string( (unsigned char)(*rit)->orient_type() )
  ...
  if( !volm_io::read_camera(cam_file(), dm->ni(), dm->nj(), heading, heading_dev, tilt, tilt_dev, roll, roll_dev, tfov,
                            top_fov_dev, altitude, lat, lon) )
    {
