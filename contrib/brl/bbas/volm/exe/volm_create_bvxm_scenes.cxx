...
int main(int argc, char* * argv)
{
  ...
  vul_arg<float>      height("-height",
                             "the amount to be added on top of the terrain height to create the scene in meters", 0.0f);

  vul_arg_parse(argc, argv);

  // check input
  if( in_folder().compare("") == 0 || in_poly().compare("") == 0 || out_folder().compare("") == 0 ||
      world_root().compare("") == 0 )
    {
  ...
  vpgl_lvcs_sptr       lvcs_ll =
    new vpgl_lvcs(ll.y(), ll.x(), 0.0, vpgl_lvcs::wgs84, vpgl_lvcs::DEG, vpgl_lvcs::METERS);
  vpgl_lvcs_sptr lvcs_ur =
    new vpgl_lvcs(ur.y(), ur.x(), 0.0, vpgl_lvcs::wgs84, vpgl_lvcs::DEG, vpgl_lvcs::METERS);
  ...
  scale_ur_x -= ur.x();
  scale_ur_y -= ur.y();
  vcl_cout << " pre-defined world_size is " << world_size_input() << " and voxel size is " << voxel_size()
           << " actual world size is " << world_size << vcl_endl;
  ...
  scale_set.insert(scale_ur_x);  scale_set.insert(scale_ur_y);  scale_set.insert(scale_ll_x);  scale_set.insert(
    scale_ll_y);
  double min_size = *scale_set.begin();
  vcl_cout << " given maximum allowed world size " << world_size_input() << " the min_size for geo index is: "
           << min_size << vcl_endl;
  ...
  vcl_cout << "the geo-index quad tree for scene has " << leaves.size() << " leaves and depth is " << tree_depth
           << vcl_endl;
  ...
  for( unsigned i = 0; i < leaves.size(); i++ )
    ...
    // construct lvcs
    vpgl_lvcs_sptr lvcs = new vpgl_lvcs(lower_left.y(),
                                        lower_left.x(), min, vpgl_lvcs::wgs84, vpgl_lvcs::DEG, vpgl_lvcs::METERS);
