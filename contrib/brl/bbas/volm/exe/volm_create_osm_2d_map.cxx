...
int main(int argc, char* * argv)
{
  ...
  for( unsigned r_idx = 0; r_idx < num_roads; r_idx++ )
    ...
    if( !volm_io_tools::expend_line(line_img, width, img_poly) )
      {
      vcl_cout << " expending osm line in tile " << tile_id() << ", leaf " << leaf_bbox_geo << " failed given width "
               << width << vcl_endl;
      return false;
      }
    // update the label
...
int main(int argc, char* * argv)
{
  ...
  if( geo_folder().compare("") == 0 || out_folder().compare("") == 0 || world_id() == 0 ||
      urban_folder().compare("") == 0 )
    {
  ...
  for( unsigned i = 0; i < tile_size; i++ )
    ...
    vcl_stringstream kml;
    kml << out_folder() << "/p1b_wr" << world_id() << "_tile_" << i << "_depth_" << tree_depth << ".kml";
...
int main(int argc, char* * argv)
{
  ...
  vul_arg<int>        leaf_idx("-leaf",
                               "leaf id of geo index for current tile (negative value will loop over all leaves", -1);
  vul_arg<vcl_string> class_img_folder("-class-img", "classification image from satellite modeling", "");
  vul_arg<bool>       is_satellite_building("-is-sat-building",
                                            "option to ingest satellite classification buildings into 2d land map",
                                            false);
  vul_arg<bool> is_osm_region("-is-osm-region", "option to ingest all osm regions", false);
  vul_arg<bool> is_osm_building("-is-osm-building", "option to ingest osm buildings into classification map",
                                false);
  vul_arg<bool> is_osm_road("-is-osm-road", "option to ingest all osm road and junctions into classification map",
                            false);
    ...
    for( unsigned l_idx = 0; l_idx < (unsigned)leaves.size(); l_idx++ )
      ...
      if( is_satellite_building() )
        ...
        if( sat_img_infos.size() != 0 )
          {
          vcl_cout << "there are " << sat_img_infos.size()
                   << " satellite class images intersect with current 2d map leaf: " << leaf_bbox_geo << vcl_endl;
          vcl_cout << "\t they are: \n";
            ...
            for( unsigned i = 0; i < ni; i++ )
              ...
              for( unsigned j = 0; j < nj; j++ )
                ...
                for( unsigned c_idx = 0; (c_idx < leaf_strings.size() && !is_pixel_found); c_idx++ )
                  ...
                  unsigned uu = (unsigned)vcl_floor(u + 0.5);
                  unsigned vv = (unsigned)vcl_floor(v + 0.5);
                  if( uu > 0 && vv > 0 && uu < class_img_map[leaf_strings[c_idx]].ni() &&
                      vv < class_img_map[leaf_strings[c_idx]].nj() )
                    {
                    is_pixel_found = true;  // to avoid handling overlap region twice
                    // only ingest buildings satellite classification map
                    if( class_img_map[leaf_strings[c_idx]] (uu,
                                                            vv) ==
                        volm_osm_category_io::volm_land_table_name["building"].id_ )
                      {
...
#  endif
      ...
      if( is_osm_road() )
        ...
        for( unsigned r_idx = 0; r_idx < num_roads; r_idx++ )
          ...
          if( !volm_io_tools::expend_line(line_img, width, img_poly) )
            {
            log << " expending osm line in tile " << t_idx << ", leaf " << leaf->extent_ << " failed given width "
                << width << vcl_endl;
            error(log_file.str(), log.str() );
            return false;
            }
        ...
        for( unsigned r_idx = 0; r_idx < n_road_in; r_idx++ )
          ...
          if( !volm_io_tools::search_junctions(curr_rd, curr_rd_prop, net, net_props, cross_pts, cross_props,
                                               cross_geo_props) )
            {
            log << "ERROR: find road junction for tile " << t_idx << " leaf " << leaf_bbox_geo << " road " << r_idx
                << " failed\n";
            error(log_file.str(), log.str() );
            return false;
            }
...
int main(int argc, char* * argv)
{
  ...
  // input check
  if( nlcd_folder().compare("") == 0 || lidar_folder().compare("") == 0 || osm_folder().compare("") == 0 ||
      roi_kml().compare("") == 0 ||
  ...
  else if( world_str() == "chile" || world_str() == "india" || world_str() == "jordan" ||
           world_str() == "philippines" || world_str() == "taiwan" ||
           world_str() == "Chile" || world_str() == "India" || world_str() == "Jordan" ||
           world_str() == "Philippines" || world_str() == "Taiwan" )
    {
    phase_str = "b";
    }
  ...
  else if( world_str() == "Coast" || world_str() == "coast" )
    {
    world_id = 2;
    }
  else if( world_str() == "Chile" || world_str() == "chile" )
    {
    world_id = 1;
    }
  else if( world_str() == "India" || world_str() == "india" )
    {
    world_id = 2;
    }
  else if( world_str() == "Jordan" || world_str() == "jordan" )
    {
    world_id = 3;
    }
  else if( world_str() == "Philippines" || world_str() == "philippines" )
    {
    world_id = 4;
    }
  else if( world_str() == "Taiwan" || world_str() == "taiwan" )
    {
    world_id = 5;
    }
  ...
  unsigned                  tile_size = tiles.size();
  volm_geo_index2_node_sptr root = volm_geo_index2::construct_tree<volm_osm_object_ids_sptr>(tiles[t_id],
                                                                                             min_size(), roi_poly);
  vcl_cout << "Create geo index with min_size: " << min_size() << vcl_endl;

  unsigned         tree_depth = volm_geo_index2::depth(root);
  vcl_stringstream kml;
  kml << out_folder() << "/p1a_" << world_str() << "_tile_" << t_id << "_depth_" << tree_depth << ".kml";
  ...
  if( info_tmp.size() != tile_size )
    {
    log << "ERROR: mismatch in create tiles " << tiles.size() << " and loaded " << info_tmp.size()
  ...
  // load open street map binary
  vcl_stringstream osm_file;
  osm_file << osm_folder() << "/p1" << phase_str << "_wr" << world_id << "_tile_" << t_id << "_osm.bin";
  if( !vul_file::exists(osm_file.str() ) )
    {
    log << "ERROR: can not find osm binary file: " << osm_file.str() << '\n';  error(log_file, log.str() );
    return false;
  ...
  vcl_cout << "read " << osm.num_locs() << " OSM loc points, " << osm.num_roads() << " OSM roads and "
           << osm.num_regions() << " OSM regions" << vcl_endl;
  ...
  for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
    ...
    vcl_cout << "leaf " << l_idx << " has geo boundary " << leaf_bbox_geo << " --> image size " << out_img.ni()
             << 'x' << out_img.nj() << vcl_endl;
...
#  if 1
    ...
    for( unsigned r_idx = 0; r_idx < num_roads; r_idx++ )
      ...
      if( !volm_io_tools::expend_line(line_img, width, img_poly) )
        {
        log << " expending osm line in tile " << t_id << ", leaf " << leaf->extent_ << " failed given width "
            << width << vcl_endl;
        error(log_file, log.str() );
        return false;
        }
    ...
    for( unsigned r_idx = 0; r_idx < n_road_in; r_idx++ )
      ...
      if( !volm_io_tools::search_junctions(curr_rd, curr_rd_prop, net, net_props, cross_pts, cross_props,
                                           cross_geo_props) )
        {
        log << "ERROR: find road junction for tile " << t_id << " leaf " << leaf_bbox_geo << " road " << r_idx
            << " failed\n";
        error(log_file, log.str() );
        return false;
        }
        ...
        for( int ii = cx - radius; ii < cx + radius; ii++ )
          ...
          for( int jj = cy - radius; jj < cy + radius; jj++ )
            ...
            if( ii >= 0 && jj >= 0 && ii < out_img.ni() && jj < out_img.nj() )
              {
              if( curr_level >=
                  level_img(ii, jj) && out_img(ii, jj) != volm_osm_category_io::volm_land_table_name["building"].id_ )
                {
                out_img(ii, jj)   = curr_id;
                level_img(ii, jj) = curr_level;
                }
...
#    if 1
    ...
    for( unsigned ii = 0; ii < build_polys.size(); ii++ )
      ...
      for( unsigned jj = 0; jj < build_polys[ii].first[0].size(); jj++ )
        {
        double lx, ly, lz;
        lvcs->global_to_local(build_polys[ii].first[0][jj].x(),
                              build_polys[ii].first[0][jj].y(), 0.0, vpgl_lvcs::wgs84, lx, ly, lz);
        double i = lx - leaf_bbox.min_x();  double j = leaf_bbox.max_y() - ly;
        int    x = (int)i;  int y = (int)j;
        if( x >= 0 && y >= 0 && x < out_img.ni() && y < out_img.nj() )
          {
...
#    endif
    ...
    for( vcl_vector<vcl_pair<unsigned, unsigned> >::iterator vit = pier_pixels.begin();  vit != pier_pixels.end();
         ++vit )
      {
