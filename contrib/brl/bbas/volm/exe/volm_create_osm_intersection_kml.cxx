...
int main(int argc, char* * argv)
{
  ...
  volm_geo_index2_node_sptr root = volm_geo_index2::construct_tree<volm_osm_object_ids_sptr>(bbox,
                                                                                             min_size() );
  ...
  for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
    ...
    vcl_cout << " leaf " << l_idx << ", " << leaf_bbox_geo << " has " << roads_in_leaf.size() << " roads inside"
             << vcl_endl;
    ...
    for( unsigned i_rdx = 0; i_rdx < n_road_in; i_rdx++ )
      ...
      if( cross_geo_props.size() != cross_pts.size() )
        {
        log << "ERROR: number of intersection, " << cross_pts.size()
            << ", differs from the number of intersection types" << cross_geo_props.size() << "!\n";
        error(log_file, log.str() );
        return volm_io::EXE_ARGUMENT_ERROR;
        }
    ...
    vcl_cout << "After leaf " << l_idx << ", there are " << all_cross_pts.size() << " \"" << key()
             << "\" intersections formed" << vcl_flush << vcl_endl;
    } // loop over leaves
      // put into kml file
...
int main(int argc, char* * argv)
{
  ...
  for( unsigned t_idx = 0; t_idx < tile_size; t_idx++ )
    ...
    for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
      ...
      vcl_cout <<  leaves.size() << " leaves in total, handling " << l_idx << " leaf with " << roads_in_leaf.size()
               << " roads in it ";
      ...
      for( unsigned i_rdx = 0; i_rdx < n_road_in; i_rdx++ )
        ...
        if( !volm_io_tools::search_junctions(curr_rd, curr_rd_prop, net, net_props, cross_pts, cross_props,
                                             cross_geo_props) )
          {
          log << "ERROR: find road junction for tile " << t_idx << " leaf " << leaf_bbox_geo << " road " << i_rdx
              << " failed\n";
          error(log_file.str(), log.str() );
          return volm_io::EXE_ARGUMENT_ERROR;
          }
