...
static void write_polys_to_kml(vcl_ofstream& ofs, vcl_string const& key, vcl_string const& value,
                               vcl_map<vcl_pair<vcl_string, vcl_string>, volm_land_layer> const& osm_land_table,
                               vcl_vector<vgl_polygon<double> > osm_polys, vcl_vector<vcl_vector<vcl_pair<vcl_string,
                                                                                                          vcl_string> > > osm_poly_keys, unsigned char const& r, unsigned char const& g,
                               unsigned char const& b);

static bool find_key_value_from_tags(vcl_string const& key, vcl_string const& value, vcl_vector<vcl_pair<vcl_string,
                                                                                                         vcl_string> >& keys,
                                     vcl_string& name);
...
int main(int argc, char* * argv)
{
  ...
  vul_arg<vcl_string> key("-key", "the key of desired osm category, write \"all\" (default) to parse all categories",
                          "all");
  vul_arg<vcl_string> value("-value",
                            "the value of desired osm category, write \"all\" (default) to parse all categories",
                            "all");
  vul_arg<vcl_string> geometry("-geo",
                               "the geometry of desired osm category, can be point/line/poly, write \"all\" (default) to parse all geometries",
                               "all");
  ...
  if( geometry().compare("point") == 0 )
    {
    vcl_cout << " parsing all points having key \"" << key() << "\" and value \"" << value() << "\" in osm to kml..."
             << vcl_endl;
    ...
    {
    vcl_cout << " parsing all roads having key \"" << key() << "\" and value \"" << value() << "\" in osm to kml..."
             << vcl_endl;
    volm_osm_parser::parse_lines(osm_lines, osm_line_keys, osm_file() );
    write_lines_to_kml(ofs, key(), value(), osm_land_table, osm_lines, osm_line_keys, (unsigned char)rgb_r(),
                       (unsigned char)rgb_g(), (unsigned char)rgb_b() );
    }
  else if( geometry().compare("poly") == 0 )
    {
    vcl_cout << " parsing all regions having key \"" << key() << "\" and value \"" << value()
             << "\" in osm to kml..." << vcl_endl;
    volm_osm_parser::parse_polygons(osm_polys, osm_poly_keys, osm_file() );
    write_polys_to_kml(ofs, key(), value(), osm_land_table, osm_polys, osm_poly_keys, (unsigned char)rgb_r(),
                       (unsigned char)rgb_g(), (unsigned char)rgb_b() );
    }
  else
    {
    vcl_cout << " parsing location points having key \"" << key() << "\" and value \"" << value()
             << "\" in osm to kml..." << vcl_endl;
    volm_osm_parser::parse_points(osm_pts, osm_pt_keys, osm_file() );
    vcl_cout << osm_pts.size() << " points are parsed from osm file" << vcl_flush << vcl_endl;
    vcl_cout << " parsing lines having key \"" << key() << "\" and value \"" << value() << "\" in osm to kml..."
             << vcl_endl;
    volm_osm_parser::parse_lines(osm_lines, osm_line_keys, osm_file() );
    vcl_cout << osm_lines.size() << " lines are parsed from osm file" << vcl_flush << vcl_endl;
    vcl_cout << " parsing regions having key \"" << key() << "\" and value \"" << value() << "\" in osm to kml..."
             << vcl_endl;
    volm_osm_parser::parse_polygons(osm_polys, osm_poly_keys, osm_file() );
...
#if 0
  bool found = false;
  for( vcl_vector<vcl_pair<vcl_string, vcl_string> >::iterator vit = curr_keys.begin();
       (vit != curr_keys.end() && !found); ++vit )
    {
                              ...
                              vcl_string& name)
{
  ...
  for( vcl_vector<vcl_pair<vcl_string, vcl_string> >::iterator vit = tags.begin(); ( vit != tags.end() && !found);
       ++vit )
    {
