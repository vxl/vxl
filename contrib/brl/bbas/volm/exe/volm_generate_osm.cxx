...
int main(int argc, char* * argv)
{
  ...
  vul_arg<double>     min_size("-min-size",
                               "minimum leaf size (in degree), note the root corresponds to a tile with 1 degree in size",
                               0.5);
  ...
  if( read() )
    ...
    double                    min_size;
    volm_geo_index2_node_sptr root = volm_geo_index2::read_and_construct<volm_osm_object_ids_sptr>(
        file_name.str(), min_size);
    // obtain all leaves
    ...
    for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
      {
      volm_geo_index2_node<volm_osm_object_ids_sptr>* leaf_ptr =
        dynamic_cast<volm_geo_index2_node<volm_osm_object_ids_sptr> *>(leaves[l_idx].ptr() );
  ...
  for( unsigned p_idx = 0; p_idx < num_pts; p_idx++ )
    ...
    volm_geo_index2_node<volm_osm_object_ids_sptr>* leaf_ptr =
      dynamic_cast<volm_geo_index2_node<volm_osm_object_ids_sptr> *>(leaf.ptr() );
