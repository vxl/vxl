...
void prepare_site_file(vcl_string const& site_file, vcl_vector<vcl_string>& lidar_full,
                       vcl_vector<vcl_string>& lidar_name, vcl_vector<vcl_string>& img_full,
                       vcl_vector<vcl_string>& img_name)
{
...
void collect_lidar_pixels(vpgl_lvcs_sptr lvcs, vcl_string const& mask_name, vcl_string const& img_name,
                          vcl_vector<vcl_pair<int, int> >& pixels, vcl_vector<vcl_pair<double, double> >& local_coords)
{
...
bool collect_img_pixels(vpgl_lvcs_sptr lvcs, int utm_zone, vcl_string const& mask_name, vcl_string const& img_tfw_name,
                        vcl_vector<vcl_pair<double, double> >& local_coords, vcl_vector<vcl_pair<int,
                                                                                                 int> >& img_pixels,
                        int n_size)
{
  vil_image_view<vxl_byte> mask = vil_load(mask_name.c_str() );
  ...
  vcl_cout << " collected: " << img_pixels.size() << " pixels from aerial image after neighborhood enlargement by: "
           << n_size << " x " << n_size << '\n';
  return true;
}
...
bool read_training_img(vcl_string txt_file, vcl_vector<vcl_string>& names, vcl_vector<vcl_pair<vcl_string,
                                                                                               vcl_string> >& lidar_imgs,
                       vcl_vector<vcl_pair<vcl_string, vcl_string> >& imgs)
{
...
int main(int argc,  char* * argv)
{
  ...
  vul_arg<vcl_string> in_poly("-poly",
                              "a folder for a set of polys for the category in question as kml files, to be used for testing or training",
                              "");
  ...
  vul_arg<vcl_string> train("-train",
                            "load the specified sdet classifier instance and add to its training data using the info in img_folder()",
                            "");
  vul_arg<int> neighborhood_size("-n",
                                 "size of the neighborhood around a pixel to collect training examples, in pixels, e.g. 5",
                                 5);
  vul_arg<vcl_string> compute("-compute",
                              "load the specified classifier instance and compute textons and save the dictionary file",
                              "");
  vul_arg<vcl_string> test("-test",
                           "load the specified classifier instance and classify pixels in images in imgs folder specified in test_imgs.txt file",
                           "");
  ...
  if( find_imgs() )
    ...
    vcl_cout << " first point: " << polys[0].first[0][0].x() << " " << polys[0].first[0][0].y() << " # of points: "
             << polys[0].first[0].size() << vcl_endl;
      ...
      for( unsigned i = 0; i < imgs.size(); i++ )
        ...
        for( unsigned j = 0; j < polys.size(); j++ )
          ...
          if( imgs[i].intersects(polys[j].first) )
            {
            vcl_cout << imgs[i].name << " intersects: " << polys[j].second << " it's bbox: " << imgs[i].bbox
                     << vcl_endl;
            vcl_stringstream file;
            file << out_folder() << "\\bbox_" << polys[j].second << "_img_" << imgs[i].name << ".kml";
      ...
      for( unsigned i = 0; i < lidar_imgs.size(); i++ )
        ...
        for( unsigned j = 0; j < polys.size(); j++ )
          ...
          if( lidar_imgs[i].intersects(polys[j].first) )
            {
            vcl_cout << lidar_imgs[i].name << " intersects: " << polys[j].second << " it's bbox: "
                     << lidar_imgs[i].bbox << vcl_endl;
            vcl_stringstream file;
            file << out_folder() << "\\bbox_" << polys[j].second << "_lidar_img_" << lidar_imgs[i].name << ".kml";
    ...
    for( unsigned j = 0; j < polys.size(); j++ )
      {
      vcl_stringstream site_file;
      site_file << in_poly() << "site_" << polys[j].second << "_zone_" << utm_zone() << ".xml";
  ...
  if( train().compare("") != 0 )
    ...
    for( unsigned ii = 0; ii < names.size(); ii++ )
      ...
      if( !collect_img_pixels(lvcs, utm_zone(), in_poly() + imgs[ii].second, tfw_name, local_coords, img_pixels,
                              neighborhood_size() ) )
        {
      ...
      if( !lidar_c.compute_filter_bank_float_img(filter_folder(), lidar_folder() + lidar_imgs[ii].first + ".tif",
                                                 lidar_max() ) )
        {
        return 0;
        }
      ...
      if( !naip_c.compute_filter_bank_color_img(filter_folder(),
                                                img_folder() + imgs[ii].first + "/" + imgs[ii].first + ".tif") )
        {
        return 0;
        }
