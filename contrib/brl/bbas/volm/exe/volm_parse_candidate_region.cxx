...
static void write_out_polygon(vcl_string const& out_folder, vcl_string const& region_str,
                              vcl_string const& leaf_str_pre, vgl_polygon<double> const& poly);
...
int main(int argc, char* * argv)
{
  ...
  if( !is_kml() )  // create candidate polygon for each tile
    ...
    unsigned            n_out, n_in;
    vgl_polygon<double> poly_all = volm_candidate_region_parser::parse_polygon_with_inner(cand_file(),
                                                                                          region_name(), outer, inner, n_out,
                                                                                          n_in);
    ...
    {
    vcl_cout << "============  Start to visualize the candidate region for tile " << tile_id() << ", leaf "
             << leaf_id() << " ============\n";
...
void write_out_polygon(vcl_string const& out_folder, vcl_string const& region_str, vcl_string const& leaf_str_pre,
