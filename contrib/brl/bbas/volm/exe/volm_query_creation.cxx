...
int main(int argc, char** argv)
{
  ...
  vcl_cout << " for spherical surface, point angle = " << point_angle() << " degree, "
           ...
           << " \t" << query->headings().size() << " headings = " << query->headings()[0] << " +/- " << query->head_d_ << " increment: " << query->head_inc_ << '\n'
           << " \t" << query->tilts().size() << " tilts = " << query->tilts()[0] << " +/- " << query->tilt_d_ << " increments: " << query->tilt_inc_ << '\n'
           << " \t" << query->rolls().size() << " rolls = " << query->rolls()[0] << " +/- " << query->roll_d_ << " increments: " << query->roll_inc_ << '\n'
           << " \t" << query->top_fovs().size() << " top_fov = " << query->top_fovs()[0] << " +/- " << query->tfov_d_ << " increment: " << query->tfov_inc_ << '\n'
           << " For each camera hypothesis, generated query_size is " << query->get_query_size() << " byte" << vcl_endl;
