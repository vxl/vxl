...
static bool find_nearest_building(unsigned const& i, unsigned const& j,
                                  int const& radius,
                                  vcl_map<unsigned, vcl_pair<vcl_vector<unsigned>,
                                                             vcl_vector<unsigned> > > const& buildings,
...
static bool obtain_buildings(vil_image_view<vxl_byte> const& c_img, unsigned const& start_ni, unsigned const& start_nj,
                             unsigned const& end_ni,   unsigned const& end_nj, vcl_map<unsigned,
                                                                                       vcl_pair<vcl_vector<unsigned>,
                                                                                                vcl_vector<unsigned> > >& buildings, vcl_map<unsigned,
                                                                                                                                             vcl_pair<vcl_vector<unsigned>, vcl_vector<unsigned> > >& flat_regions,
                             vcl_vector<vcl_pair<unsigned, unsigned> >& non_buildings);

static bool refine_building_by_median(vil_image_view<float> const& h_img, vcl_map<unsigned,
                                                                                  vcl_pair<vcl_vector<unsigned>,
                                                                                           vcl_vector<unsigned> > > const& buildings,
                                      vil_image_view<float>& refined_img);

static bool refine_building_by_median_divided(vil_image_view<float> const& h_img, vil_image_view<vxl_byte> const& c_img,
                                              vcl_map<unsigned, vcl_pair<vcl_vector<unsigned>,
                                                                         vcl_vector<unsigned> > > const& buildings,
                                              unsigned const& size,
                                              vil_image_view<float>& refined_img);

static bool refine_building(float const& ratio, float const& height_threshold, vil_image_view<float> const& h_img,
                            vil_image_view<vxl_byte> & refined_c_img, vil_image_view<float>& refined_h_img);

static float neighbor_height(vcl_map<vcl_pair<unsigned, unsigned>, float> const& window_height, unsigned const& w_idx_i,
...
int main(int argc, char* * argv)
{
  ...
  for( vul_file_iterator fn = file_glob.c_str(); fn; ++fn )
    {
    volm_geo_index2_node_sptr class_root = volm_geo_index2::read_and_construct<volm_osm_object_ids_sptr>(
        fn(), class_map_min_size);
  ...
  if( class_leaves.empty() )
    {
    log << "ERROR: can not find any overlapped 2d class map for current height map: " << height_img_name() << " -- "
        << h_bbox << '\n';
    error(log_file.str(), log.str() );
    return volm_io::EXE_ARGUMENT_ERROR;
    }
  ...
  for( unsigned i = 0; i < ni; i++ )
    ...
    for( unsigned j = 0; j < nj; j++ )
      ...
      for( unsigned l_idx = 0; l_idx < num_leaves; l_idx++ )
        ...
        if( uu > 0 && vv > 0 && uu < class_img_map[class_leaves[l_idx]->get_string()].ni() &&
            vv < class_img_map[class_leaves[l_idx]->get_string()].nj() )
          {
...
#  if 0
  vcl_string c_img_before_refine = vul_file::strip_extension(refined_c_img_name)
    + "_cropped.tif";
  ...
  for( unsigned w_idx_i = 0; w_idx_i < num_w_i; w_idx_i++ )
    ...
    for( unsigned w_idx_j = 0; w_idx_j < num_w_j; w_idx_j++ )
      ...
      if( !obtain_buildings(c_img, start_ni, start_nj, end_ni, end_nj, buildings, flat_regions, non_buildings) )
        {
        log << "error: building extraction failed for window (" << start_ni << 'x' << start_nj << ") (to " << end_ni
            << 'x' << end_nj << ")\n";
        error(log_file.str(), log.str() );
        return volm_io::EXE_ARGUMENT_ERROR;
        }
...
#  endif
      ...
      vcl_pair<vcl_pair<unsigned, unsigned>, vcl_vector<vcl_pair<unsigned, unsigned> > > tmp_pair(vcl_pair<unsigned, unsigned>(
                                                                                                    w_idx_i,
                                                                                                    w_idx_j),
                                                                                                  non_buildings);
      ...
      for( vcl_map<unsigned, vcl_pair<vcl_vector<unsigned>, vcl_vector<unsigned> > >::iterator mit =
             flat_regions.begin();
      ...
      if( flat_heights.size() != 0 )
        ...
        window_min_height.insert(vcl_pair<vcl_pair<unsigned, unsigned>,
                                          float>(vcl_pair<unsigned, unsigned>(w_idx_i, w_idx_j), height_median) );
        }
  ...
  for( unsigned w_idx_i = 0; w_idx_i < num_w_i; w_idx_i++ )
    ...
    for( unsigned w_idx_j = 0; w_idx_j < num_w_j; w_idx_j++ )
      ...
      vcl_map<vcl_pair<unsigned, unsigned>,
              float>::iterator mit = window_min_height.find(vcl_pair<unsigned, unsigned>(w_idx_i, w_idx_j) );
      ...
      vcl_map<vcl_pair<unsigned, unsigned>,
              vcl_vector<vcl_pair<unsigned,
                                  unsigned> > >::iterator r_mit = non_building_regions.find(vcl_pair<unsigned, unsigned>(
                                                                                              w_idx_i,
                                                                                              w_idx_j) );
    ...
    for( unsigned w_idx_j = 0; w_idx_j < num_w_j; w_idx_j++ )
      ...
      for( unsigned i = start_ni; i < end_ni; i++ )
        ...
        for( unsigned j = start_nj; j < end_nj; j++ )
          ...
          if( (refined_h_img(i,
                             j) - grd_height) > tall_building_thres() &&
              refined_c_img(i, j) == volm_osm_category_io::volm_land_table_name["building"].id_ )
            {
                                      ...
                                      vil_image_view<float>& refined_img)
{
  ...
  for( vcl_map<unsigned, vcl_pair<vcl_vector<unsigned>, vcl_vector<unsigned> > >::const_iterator mit =
         buildings.begin();
                             ...
                             unsigned const& w_idx_j, unsigned const& w_size)
{
  ...
  for( int radius = 1; (radius < (int)w_size && !found_neigh_height); radius++ )
    ...
    for( unsigned c = 0; c < num_nbrs; c++ )
      ...
      vcl_map<vcl_pair<unsigned, unsigned>,
              float>::const_iterator mit = window_height.find(vcl_pair<int, int>(nbr_i, nbr_j) );
                            ...
                            vil_image_view<float>& refined_h_img)
{
  ...
  for( int i = 0; i < ni; i++ )
    ...
    for( int j = 0; j < nj; j++ )
      ...
      if( refined_c_img(i,
                        j) == volm_osm_category_io::volm_land_table_name["palm_tree"].id_ ||
          refined_c_img(i, j) == volm_osm_category_io::volm_land_table_name["tree"].id_ )
        {
        continue;
        }
