...
int main(int argc, char* * argv)
{
  ...
  vul_arg<vcl_string> out_filename("-out-file", "name of the txt file which the sorted results will be written",
                                   "./urban_sort_scene.txt");
  ...
  if( vul_file::exists(tree_txt) )
    ...
    double square_size =
      (bbox_rect.width() >= bbox_rect.height() ) ? bbox_rect.width() : bbox_rect.height();
    ...
    vpgl_lvcs_sptr       lvcs_ll = new vpgl_lvcs(ll.y(),
                                                 ll.x(), 0.0, vpgl_lvcs::wgs84, vpgl_lvcs::DEG, vpgl_lvcs::METERS);
    vpgl_lvcs_sptr lvcs_ur = new vpgl_lvcs(ur.y(),
                                           ur.x(), 0.0, vpgl_lvcs::wgs84, vpgl_lvcs::DEG, vpgl_lvcs::METERS);
  ...
  for( vul_file_iterator fn = file_glob.c_str(); fn; ++fn )
    ...
    if( (lvcs_ori_lat - lat_min) * (lvcs_ori_lat - lat_min) > 1e-5 ||
        (lvcs_ori_lon - lon_min) * (lvcs_ori_lon - lon_min) > 1e-5 )
      {
      log << " ERROR: mismatch for leaf extent " << bbox_geo << " with lvcs origin (" << lvcs_ori_lon << ", "
          << lvcs_ori_lat << ") for scene " << scene_id << '\n';
      error(log_file.str(), log.str() );
      return volm_io::EXE_ARGUMENT_ERROR;
      }
