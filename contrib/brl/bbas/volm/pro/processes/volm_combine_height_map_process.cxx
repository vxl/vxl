...
bool volm_combine_height_map_process(bprb_func_process& pro)
{
  ...
  double square_size =
    (bbox_extend.width() >= bbox_extend.height() ) ? bbox_extend.width() : bbox_extend.height();
  ...
  for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
    ...
    filename << "HeightMap_" << hemisphere << vcl_setprecision(12) << lat_min << direction << vcl_setprecision(12)
             << lon_min
    ...
    out_img.fill(-1.0f);
    vcl_cout << "\tleaf " << l_idx << " has geo boundary " << leaf_bbox_wgs << " --> image size " << out_img.ni()
...
bool volm_combine_height_map_process3(bprb_func_process& pro)
{
  ...
  for( unsigned i = 0; i < o_ni; i++ )
    ...
    for( unsigned j = 0; j < o_nj; j++ )
      ...
      bool found = false;
      for( vcl_vector<volm_img_info>::iterator vit = overlap_infos.begin(); (vit != overlap_infos.end() && !found);
           ++vit )
        {
