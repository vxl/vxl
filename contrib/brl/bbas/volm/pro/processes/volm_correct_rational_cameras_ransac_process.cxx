...
bool volm_correct_rational_cameras_ransac_process(bprb_func_process& pro)
{
  if( pro.n_inputs() < 3 )
    {
    vcl_cout << "volm_correct_rational_cameras_ransac_process: The number of inputs should be 3" << vcl_endl;
    return false;
    }
  ...
  while( !ifs.eof() )
    ...
    vcl_string out_cam_name = output_path + "/" + vul_file::strip_extension(vul_file::strip_directory(cam_name) )
      + "_corrected.rpb";
  ...
  if( !n )
    {
    vcl_cerr << "In vpgl_correct_rational_cameras_process2 - 0 correspondences in file: " << input_cams
  ...
  if( cnt_exists == 0 )
    ...
    {
    vcl_cerr << pro.name() << ": all cameras have been corrected, exit without any corrections!\n";
    return false;
    }
  else
    {
  ...
  if( cams.size() == 1 )
    {
    vcl_cerr << pro.name() << ": only 1 cameras are found, exit without any corrections!\n";
    return false;
    }
  // if (cams.size() == 1) {
  ...
  for( unsigned i = 0; i < n; i++ )
    ...
    // vcl_cout << "after adjustment 2d correspondence: " << i <<  " gives a 3D intersection point: " << intersection << vcl_endl;
  ...
  if( !cam_trans.size() )
    ...
    vcl_cout << "out of " << n << " correspondences " << cam_trans.size()
             << " of them yielded corrections! exit without any corrections!\n";
    return false;
    }
  ...
  if( !vpgl_rational_adjust_multiple_pts::adjust_lev_marq(cams, cam_weights, corrs_inliers, cam_trans_inliers,
                                                          intersections) )
    {
...
bool volm_correct_rational_cameras_ransac_process2(bprb_func_process& pro)
{
  ...
  if( enforce_existing && cnt_exists < 2 )
    {
    vcl_cerr << " Enforcing condition to have 2 pre-existing corrected cameras! EXITING since there is: "
             << cnt_exists << " cameras.\n";
    return false;
    }
  ...
  for( unsigned i = 0; i < cam_trans.size(); i++ )
    {
    vcl_cout << i << " --> " << inlier_cnts[i] << vcl_endl;
    if( max < inlier_cnts[i] )
      {
      max = inlier_cnts[i];
      max_i = i;
  ...
  vcl_cout << '\n';
  vcl_cout << vcl_endl << " using translations of 1 pt with the most inliers: " << max_i << "\n";
  for( unsigned k = 0; k < cams.size(); k++ )
    {
