...
bool volm_correct_rational_cameras_ransac_process(bprb_func_process& pro)
{
  ...
  //{
    ...
    if (img_path.compare("") == 0) {
      vcl_cerr << "Cannot locate: " << cam_name << " in satellite resources! exiting!\n";
      return false;
    }
    vpgl_rational_camera<double> *ratcam = load_cam_from_nitf(img_path);
    if ( !ratcam ) {
      vcl_cerr << "Failed to load rational camera from file" << img_path << '\n';
      return false;
    }
  ...
  if (!vpgl_rational_adjust_multiple_pts::adjust_lev_marq(cams, corrs_inliers, cam_trans_inliers, intersections))
  {
