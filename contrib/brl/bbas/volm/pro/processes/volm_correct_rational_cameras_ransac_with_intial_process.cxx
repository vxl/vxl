...
bool obtain_relative_diameter(volm_satellite_resources_sptr const& sat_res, vcl_vector<vcl_string> const& camera_names,
...
bool initial_point_by_overlap_region(volm_satellite_resources_sptr const& sat_res,
                                     vcl_vector<vcl_string> const& camera_names, vcl_string const& dem_folder,
...
bool volm_correct_rational_cameras_ransac_with_intial_process(bprb_func_process& pro)
{
  ...
  if( !n )
    ...
    vcl_cerr << pro.name() << ": 0 correspondences in file: " << input_txt
             << "! returning without correcting any cams!\n";
    return false;
    }
  ...
  if( enforce_existing && cnt_exist < 2 )
    {
    vcl_cerr << pro.name()
             << ": Enforcing condition to have 2 pre-existing corrected cameras! EXITING since there is: " << cnt_exist
             << " cameras.\n";
    return false;
    }
  ...
  vcl_cout << "out of " << cam_trans.size() << " valid correspondences, " << max
           << " of them yield constant translations using " << pix_rad << " pixel radius" << vcl_endl;
  ...
  for( unsigned i = 0; i < intersections.size(); i++ )
    {
    vcl_cout << "after adjustment 3D intersection point: " << vcl_setprecision(12) << intersections[i].y() << ","
             << vcl_setprecision(12) << intersections[i].x()
...
vpgl_rational_camera<double> * volm_correct_rational_cameras_ransac_with_initial_process_globals::load_cam_from_nitf(
  vcl_string const& nitf_img_path)
{
...
bool volm_correct_rational_cameras_ransac_with_initial_process_globals::obtain_relative_diameter(
  ...
  double& relative_diameter)
{
  ...
  vgl_box_2d<double> overlap_region = volm_correct_rational_cameras_ransac_with_initial_process_globals::intersection(
      img_footprints);
...
bool volm_correct_rational_cameras_ransac_with_initial_process_globals::initial_point_by_overlap_region(
  volm_satellite_resources_sptr const& sat_res,
  vcl_vector<vcl_string> const& camera_names,
  ...
  double& zmax)
{
  ...
  if( !volm_io_tools::find_min_max_height(lower_left, upper_right, infos, min_elev, max_elev) )
    {
    vcl_cerr << "can not find elevation for the overlap region " << overlap_region.min_point() << ", "
             << overlap_region.max_point() << '\n';
    return false;
    }
  zmin = min_elev - 10;
  zmax = max_elev + 10;
...
vgl_box_2d<double> volm_correct_rational_cameras_ransac_with_initial_process_globals::intersection(
