...
bool volm_query_satellite_resources_process(bprb_func_process& pro)
{
  ...
  if( !pick_seed )
    {
    out = res->query_print_to_file(lower_left_lon, lower_left_lat, upper_right_lon, upper_right_lat, cnt, out_file,
                                   band, gsd_thres);
    pro.set_output_val<unsigned>(0, cnt);
    }
  else
    {
    out = res->query_seeds_print_to_file(lower_left_lon, lower_left_lat, upper_right_lon, upper_right_lat, n_seeds, cnt,
                                         out_file, band, gsd_thres);
    pro.set_output_val<unsigned>(0, cnt);
    }
  return out;
}
...
bool volm_pick_nadir_resource_pair_process(bprb_func_process& pro)
{
  ...
  for( vcl_map<double, vcl_pair<vcl_string, vcl_string>, vcl_greater<double> >::iterator mit = pairs.begin();
       mit != pairs.end(); ++mit )
    {
...
bool volm_find_satellite_pairs_process(bprb_func_process& pro)
{
  ...
  out = res->query_pairs_print_to_file(lower_left_lon, lower_left_lat, upper_right_lon, upper_right_lat, GSD_thres, cnt,
                                       out_file, sat_name);
  pro.set_output_val<unsigned>(0, cnt);
  return out;
}
