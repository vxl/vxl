  ...
  while( ply_it )
    ...
    for( ; it != bmesh->facemap().end(); it++ )
      ...
      for( unsigned j = 0; j < vertices.size(); j++ )
        {
        vcl_cout << "vertex[" << j << "]: x:" << vertices[j]->get_pt().x() << " y: " << vertices[j]->get_pt().y()
                 << " z: " << vertices[j]->get_pt().z() << vcl_endl;
...
bool volm_generate_height_map_plot_process(bprb_func_process& pro)
{
  ...
  vcl_cout << " !!!!!!!!!! minimum difference value: " << min_val_act << " max difference val: " << max_val_act
           << vcl_endl;
