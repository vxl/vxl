...
bool volm_map_segments_process(bprb_func_process& pro)
{
  ...
  vcl_cout << "!!!!!!!!!!! input height img ni: " << input_height_image_sptr->ni() << "  nj: "
           << input_height_image_sptr->nj() << vcl_endl;
  ...
  img(img_sptr);                                     // input segments
  vcl_map<vcl_pair<unsigned char, vcl_pair<unsigned char, unsigned char> >,
          vcl_pair<vcl_vector<vcl_pair<int, int> >, vil_rgb<vxl_byte> > > segment_map;
  vcl_map<vcl_pair<unsigned char, vcl_pair<unsigned char, unsigned char> >,
          vcl_pair<vcl_vector<vcl_pair<int, int> >, vil_rgb<vxl_byte> > >::iterator iter;
  for( unsigned i = 0; i < img.ni(); i++ )
    {
    for( unsigned j = 0; j < img.nj(); j++ )
      {
      vcl_pair<unsigned char, vcl_pair<unsigned char, unsigned char> > seg_color(img(i,
                                                                                     j).r, vcl_pair<unsigned char, unsigned char>(img(
                                                                                                                                    i,
                                                                                                                                    j).g, img(
                                                                                                                                    i,
                                                                                                                                    j).b) );
  ...
  if( hit )
    ...
    for( unsigned i = 0; i < img_lines.size(); i++ )
      ...
      vcl_pair<unsigned char, vcl_pair<unsigned char, unsigned char> > seg_color(segment_color.r,
                                                                                 vcl_pair<unsigned char, unsigned char>(
                                                                                   segment_color.g, segment_color.b) );
...
bool volm_map_osm_onto_image_process(bprb_func_process& pro)
{
  ...
  vpgl_local_rational_camera<double>* cam_local_rat =
    dynamic_cast<vpgl_local_rational_camera<double> *>(sat_cam.ptr() );
      ...
      for( unsigned j = 0; j < pts.size(); j++ )
        ...
        if( uu < height_img_sptr->ni() && vv < height_img_sptr->nj() )
          ...
          if( iuu >= 0 && ivv >= 0 && iuu < sat_img_sptr->ni() && ivv < sat_img_sptr->nj() )
            {
            vcl_cout << "line " << i << ": pt [" << pts[j].x() << ',' << pts[j].y() << ',' << elev << " --> " << iuu
                     << ',' << ivv << vcl_endl;
          ...
          for( it.reset(); it.next();  )
            ...
            for( int x = it.startx(); x <= it.endx(); ++x )
              ...
              if( x >= 0 && y >= 0 && x < (int)out_img.ni() && y < (int)out_img.nj() )
                ...
                else if( band_name.compare("g") == 0 )
                  {
                  out_img(x, y).g = 255;
                  }
                else if( band_name.compare("b") == 0 )
                  {
                  out_img(x, y).b = 255;
                  }
                }
...
bool volm_map_osm_onto_image_process2(bprb_func_process& pro)
{
  ...
  unsigned                                       osm_id = 0;
  vcl_map<vcl_string, volm_land_layer>::iterator iter = volm_osm_category_io::volm_land_table_name.find(
      osm_category_name);
  if( iter == volm_osm_category_io::volm_land_table_name.end() )
    {
    vcl_cerr << "string: " << osm_category_name
             << " is not a valid category name listed in volm_osm_category_io::volm_land_table_name!\n";
    return false;
    }
  else
    {
  ...
  if( !ostr )
    ...
    {
    vcl_cout << "there are " << sos.size() << " vsol objects, appending to binary file: " << bin_filename
             << " which currently has: " << sos_in.size() << " objects" << vcl_endl;
