...
bool project_dem_to_sat(vpgl_geo_camera const& dem_cam, vpgl_rational_camera<double>* sat_cam, int const& dem_i,
                        int const& dem_j,  float const& dem_height, int const& sat_ni, int const& sat_nj,
                        int& sat_img_i,    int& sat_img_j);

// function to project dem image pixel to ortho image pixel
bool project_dem_to_ortho(vpgl_geo_camera const& dem_cam, vpgl_geo_camera* img_cam, int const& dem_i,  int const& dem_j,
                          float const& dem_height, int const& img_ni, int const& img_nj, int& img_i,    int& img_j);
...
bool volm_project_dem_to_sat_img_process(bprb_func_process& pro)
{
  ...
  if( !sat_img )
    {
    vcl_cout << pro.name() << ": The image pixel format, " << sat_img_sptr->pixel_format() << " is not supported!"
             << vcl_endl;
    return false;
    }
  ...
  if( !dem_view )
    ...
    if( !dem_view_int )
      ...
      if( !dem_view_byte )
        {
        vcl_cout << pro.name() << ": The dem image pixel format, " << dem_view_base->pixel_format()
                 << " is not supported!" << vcl_endl;
        return false;
        }
      else
        {
  ...
  // start projection
  vcl_cout << "Projecting [" << dem_ni << 'x' << dem_nj << "] dem image to satellite image having size [" << sat_ni
           << 'x' << sat_nj << "]\n";
...
bool volm_project_dem_to_sat_img_process_globals::project_dem_to_sat(vpgl_geo_camera const& dem_cam,
                                                                     vpgl_rational_camera<double>* sat_cam,
                                                                     int const& dem_i,  int const& dem_j,
                                                                     float const& dem_height,
...
// function to project dem image pixel to ortho image pixel
bool volm_project_dem_to_sat_img_process_globals::project_dem_to_ortho(vpgl_geo_camera const& dem_cam,
                                                                       vpgl_geo_camera* img_cam,
                                                                       int const& dem_i,  int const& dem_j,
                                                                       float const& dem_height,
