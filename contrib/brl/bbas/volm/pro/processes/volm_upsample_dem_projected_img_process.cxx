...
bool volm_upsample_dem_projected_img_process(bprb_func_process& pro)
{
  ...
  if( !in_img )
    {
    vcl_cout << pro.name() << ": The input image pixel format, " << in_img_sptr->pixel_format()
  ...
  vcl_cout << "Start to interpolate the [" << ni << 'x' << nj << "] image given " << pixels.size()
           << " valid pixels and " << num_neighbors << " nearest neighbors";
  ...
  for( unsigned i = 0; i < ni; i++ )
    ...
    for( unsigned j = 0; j < nj; j++ )
      ...
      for( unsigned n_idx = 0; n_idx < 4; n_idx++ )
        {
        float dist =
          (float)vcl_sqrt( (neigh_points[n_idx][0] - i) * (neigh_points[n_idx][0] - i) + (neigh_points[n_idx][1] - j)
                           * (neigh_points[n_idx][1] - j) );
        dist_all  += dist;
