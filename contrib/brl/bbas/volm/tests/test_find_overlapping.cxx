...
static volm_satellite_resources_sptr create_test_sat_res()
{
  ...
  for( unsigned i = 0; i < res_sptr->resources_.size(); i++ )
    ...
    satellite_footprint.add(vgl_point_2d<double>(res_sptr->resources_[i].meta_->lower_left_.x(),
                                                 res_sptr->resources_[i].meta_->lower_left_.y() ) );
    satellite_footprint.add(vgl_point_2d<double>(res_sptr->resources_[i].meta_->upper_right_.x(),
                                                 res_sptr->resources_[i].meta_->upper_right_.y() ) );
