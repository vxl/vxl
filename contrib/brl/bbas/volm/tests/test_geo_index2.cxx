...
static void test_geo_index2()
{
  ...
  vgl_box_2d<double>   bbox(ref_pt.x() - 0.5 * min_size, ref_pt.x() + 0.5 * min_size,
                            ref_pt.y() - 0.5 * min_size, ref_pt.y() + 0.5 * min_size);
  ...
  for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
    ...
    if( ptr = dynamic_cast<volm_geo_index2_node<volm_loc_hyp_sptr> *>(leaves[l_idx].ptr() ) )
      ...
      ptr->contents_->add(leaves[l_idx]->extent_.min_point().x() + 0.5 * min_size,
                          leaves[l_idx]->extent_.min_point().y() + 0.5 * min_size, 1.6);
      ptr->contents_->add(leaves[l_idx]->extent_.min_point().x() + 0.25 * min_size,
                          leaves[l_idx]->extent_.min_point().y() + 0.25 * min_size, 1.6);
      }
    }
  ...
  for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
    ...
    volm_geo_index2_node<volm_loc_hyp_sptr>* ptr =
      dynamic_cast<volm_geo_index2_node<volm_loc_hyp_sptr> *>(leaves[l_idx].ptr() );
  ...
  for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
    {
    ;
    if( volm_geo_index2_node<volm_loc_hyp_sptr>* ptr =
          dynamic_cast<volm_geo_index2_node<volm_loc_hyp_sptr> *>(leaves[l_idx].ptr() ) )
      {
      if( !ptr->contents_ )
        {
    ...
    else if( volm_geo_index2_node<volm_loc_hyp>* ptr =
               dynamic_cast<volm_geo_index2_node<volm_loc_hyp> *>(leaves[l_idx].ptr() ) )
      {
      ptr->contents_.add(leaves[l_idx]->extent_.min_point().x() + 0.5 * min_size,
                         leaves[l_idx]->extent_.min_point().y() + 0.5 * min_size, 2.0);
      }
    }
  for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
    {
