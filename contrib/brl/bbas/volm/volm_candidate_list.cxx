...
bool volm_candidate_list::top_locations(vcl_vector<vgl_point_2d<int> >& top_locs, vcl_vector<unsigned>& top_loc_scores,
                                        unsigned const& size, unsigned const& sh_idx)
{
  if( sh_idx > n_sheet_ )
    {
    return false;
...
bool volm_candidate_list::top_locations(vcl_vector<vgl_point_2d<double> >& top_locs,
                                        vcl_vector<unsigned>& top_loc_scores, volm_tile& tile, unsigned const& size,
                                        unsigned const& sh_idx)
{
...
bool volm_candidate_list::img_to_golbal(unsigned const& sh_idx, volm_tile& tile,
                                        vcl_vector<vgl_point_2d<double> >& region_global)
{
  if( sh_idx > n_sheet_ )
    {
    return false;
...
{
  assert(
    sheet_id <= hull_poly_.size() &&
    " in candidate list, the polygon sheet id is larger than the size of created polygon ");
                                            ...
                                            unsigned const& rank)
{
  if( region.size() != 0 )
    ...
    for( vcl_vector<vgl_point_2d<double> >::iterator vit = region.begin(); vit != region.end(); ++vit )
      {
      str << "                " << vcl_setprecision(12) << vit->x() << ',' << vcl_setprecision(12) << vit->y()
          << ",0\n";
      }
    str << "                " << vcl_setprecision(12) << region[0].x() << ',' << vcl_setprecision(12)
        << region[0].y() << ",0\n";
    str << "                </coordinates>\n"
        << "              </LinearRing>\n"
        << "            </outerBoundaryIs>\n"
                                            ...
                                            unsigned const& rank)
{
  if( region.size() != 0 )
    ...
    for( unsigned l_idx = 0; l_idx < landmarks.size(); l_idx++ )
      {
      str << "          <Placemark>\n"
          << "            <name>" << volm_osm_category_io::volm_land_table[landmark_types[l_idx]].name_ << "_"
          << rank << "</name>\n"
          << "            <Point>\n"
          << "              <altitudeMode>relativeToGround</altitudeMode>\n"
          << "              <coordinates>\n"
...
bool volm_candidate_list::generate_pin_point_circle(vgl_point_2d<double> const& center, double const& radius,
...
bool volm_candidate_list::generate_heading_direction(vgl_point_2d<double> const& center, float const& heading_angle,
                                                     float const& length, float const& right_fov,
...
bool volm_candidate_list::inside_candidate_region(vgl_polygon<double> const& cand_poly, double const& lon,
                                                  double const& lat)
{
...
bool volm_candidate_list::inside_candidate_region(vgl_polygon<double> const& cand_poly_in,
                                                  vgl_polygon<double> const& cand_poly_out, double const& lon,
                                                  double const& lat)
{
