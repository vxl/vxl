...
public:
  ...
  volm_candidate_list() {}
  // : constructor
  volm_candidate_list(vil_image_view<vxl_byte> const& image, unsigned threshold);
  // : destructor
  ~volm_candidate_list() {}
  ...
  bool top_locations(vcl_vector<vcl_vector<vgl_point_2d<int> > >& top_locs,
                     vcl_vector<vcl_vector<unsigned> >& top_loc_scores);

  // : for each sheet, return top number of pixels which have high scores
  bool top_locations(vcl_vector<vcl_vector<vgl_point_2d<int> > >& top_locs,
                     vcl_vector<vcl_vector<unsigned> >& top_loc_scores, unsigned const& size);

  // : return the top number of pixels which have highest scores, for sheet sh_idx
  bool top_locations(vcl_vector<vgl_point_2d<int> >& top_locs, vcl_vector<unsigned>& top_loc_scores,
                     unsigned const& size, unsigned const& sh_idx);

  // : return the top number of locs which have highest scores, for sheet sh_idx
  bool top_locations(vcl_vector<vgl_point_2d<double> >& top_locs, vcl_vector<unsigned>& top_loc_scores, volm_tile& tile,
                     unsigned const& size, unsigned const& sh_idx);
  ...
  static void write_kml_regions(vcl_ofstream& str, vcl_vector<vgl_point_2d<double> >& region,
                                vcl_vector<vgl_point_2d<double> >& top_locs, vcl_vector<cam_angles>& camera,
                                vcl_vector<double>& right_fov, float const& likelihood, unsigned const& rank);

  static void write_kml_regions(vcl_ofstream& str, vcl_vector<vgl_point_2d<double> >& region,
                                vgl_point_2d<double>& top_loc, cam_angles const& camera, double const& right_fov,
                                float const& likelihood, unsigned const& rank);

  // : write a candidate region associated with its favored heading direction (represented as a line)
  static void write_kml_regions(vcl_ofstream& str, vcl_vector<vgl_point_2d<double> >& region,
                                vgl_point_2d<double>& top_loc, vcl_vector<vgl_point_2d<double> >& heading,
                                vcl_vector<vgl_point_2d<double> >& viewing,
                                vcl_vector<vgl_point_2d<double> >& landmarks, vcl_vector<unsigned char>& landmark_types,
                                float const& likelihood, unsigned const& rank);

  // : generate a circular region given a circular center and radius
  static bool generate_pin_point_circle(vgl_point_2d<double> const& center, double const& radius,
  ...
  static bool generate_heading_direction(vgl_point_2d<double> const& center, float const& heading_angle,
  ...
  static bool inside_candidate_region(vgl_polygon<double> const& cand_poly_in, vgl_polygon<double> const& cand_poly_out,
