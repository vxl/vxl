...
void volm_candidate_region_parser::charData(const XML_Char* s, int len)
{
  ...
  if( last_tag == KML_LON_TAG )
    ...
    if( cord_tag_ == KML_POLYOB_TAG )
      ...
      {
      vcl_cout << "WARNING: unknown shape tag: " << cord_tag_ << " (name: " << current_name_ << ") won't be parsed"
               << vcl_endl;
      }
    last_tag = "";
    cord_tag_ = "";
...
vcl_vector<vgl_point_3d<double> > volm_candidate_region_parser::parse_points(vcl_string const& kml_file,
...
vcl_vector<vcl_vector<vgl_point_3d<double> > > volm_candidate_region_parser::parse_lines(vcl_string const& kml_file,
                                                                                         vcl_string const& name)
{
...
vgl_polygon<double> volm_candidate_region_parser::parse_polygon_with_inner(vcl_string const& kml_file,
                                                                           vcl_string const& name,
