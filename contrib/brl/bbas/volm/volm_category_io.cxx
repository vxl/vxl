...
bool volm_osm_category_io::load_category_table(vcl_string const& filename,
                                               vcl_map<vcl_pair<vcl_string, vcl_string>,
                                                       volm_land_layer> & land_category_table)
{
  ...
  while( !ifs.eof() )
    ...
    land_category_table[vcl_pair < vcl_string,
                        vcl_string > (tag, value)] = volm_land_layer( (unsigned char)id, name, (unsigned char)level,
                                                                      width, id_color);
    }
...
bool volm_osm_category_io::load_road_width_table(vcl_string const& filename, vcl_map<vcl_pair<vcl_string, vcl_string>,
                                                                                     float>& osm_road_width)
{
  if( !vul_file::exists(filename) )
    {
    return false;
...
vcl_map<int, volm_land_layer> create_nlcd_to_volm_table()
{
  ...
  m[volm_osm_category_io::NLCD_EMERGENT_WETLAND] =
    volm_land_layer(14, "Emergent_Herbaceous_Wetlands", 0, 0.0, color(14) );
...
vcl_map<int, volm_land_layer> create_geo_cover_to_volm_table()
{
  ...
  m[volm_osm_category_io::GEO_URBAN]               =
    volm_land_layer(4, "Developed/Medium_Intensity", 0, 0.0, color(4) );
...
bool volm_osm_category_io::load_road_junction_table(vcl_string const& filename, vcl_map<vcl_pair<int, int>,
                                                                                        volm_land_layer>& road_junction_table)
{
  if( !vul_file::exists(filename) )
    {
    return false;
  ...
  while( !ifs.eof() )
    ...
    id_color = color(id);
    road_junction_table[vcl_pair < int, int > (id1, id2)] = volm_land_layer( (unsigned char)id, name,
                                                                             (unsigned char)level, width, id_color);
    if( id1 != id2 )
      {
      road_junction_table[vcl_pair < int, int > (id2, id1)] = volm_land_layer( (unsigned char)id, name,
                                                                               (unsigned char)level, width, id_color);
      }
    }
...
vcl_map<vcl_pair<int, int>, volm_land_layer> load_osm_road_junction_table()
{
  ...
  while( !ifs.eof() )
    ...
    ifs >> id;   ifs >> name;  ifs >> level;  ifs >> width;
    id_color = color(id);
    m[vcl_pair < int, int > (id1, id2)] = volm_land_layer( (unsigned char)id, name, (unsigned char)level, width,
                                                           id_color);
    if( id1 != id2 )
      {
      m[vcl_pair < int, int > (id2, id1)] = volm_land_layer( (unsigned char)id, name, (unsigned char)level, width,
                                                             id_color);
      }
    }
...
vcl_vector<vcl_string>                       volm_osm_category_io::volm_category_name_table =
  create_volm_land_layer_name_table();
vcl_map<int,
        double> volm_osm_category_io::geo_land_hyp_increments  =
  create_geo_cover_hyp_increments_for_roads();
vcl_map<vcl_string,
        volm_land_layer> volm_osm_category_io::tag_to_volm_land_table = load_tag_to_volm_land_table();
