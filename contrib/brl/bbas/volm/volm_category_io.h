...
public:
  ...
  volm_land_layer(unsigned char const& id, vcl_string const& name, unsigned char const& level, double const& width)
    ...
    color_(bvrml_color::heatmap_classic[id][0], bvrml_color::heatmap_classic[id][1],
...
public:
  enum nlcd_values { NLCD_WATER = 11, NLCD_ICE_SNOW = 12, NLCD_SAND = 31, NLCD_INVALID = 0,
                     NLCD_DEVELOPED_LOW = 22, NLCD_DEVELOPED_MED = 23, NLCD_DEVELOPED_HIGH = 24,
                     NLCD_DEVELOPED_OPEN = 21,
  ...
  static bool load_category_table(vcl_string const& filename, vcl_map<vcl_pair<vcl_string, vcl_string>,
  ...
  static bool load_road_junction_table(vcl_string const& filename, vcl_map<vcl_pair<int, int>,
                                                                           volm_land_layer>& road_junction_table);

  // : table to define default width of road loaded from open street map
  static bool load_road_width_table(vcl_string const& filename, vcl_map<vcl_pair<vcl_string, vcl_string>,
                                                                        float>& road_width_table);
