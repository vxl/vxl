...
{
public:
  volm_geo_index_node(vgl_box_2d<double> const& extent, volm_geo_index_node_sptr& parent) : extent_(extent), parent_(
      parent) {}
  volm_geo_index_node(vgl_box_2d<double> const& extent) : extent_(extent), parent_(0) {}
  ~volm_geo_index_node();
  vcl_string get_string();

  vcl_string get_hyp_name(vcl_string const& geo_index_name_pre)
  {
    return geo_index_name_pre + "_" + this->get_string()
           + ".bin";
  }
  vcl_string get_index_name(vcl_string const& geo_index_name_pre)
  {
    return geo_index_name_pre + "_"
           + this->get_string() + "_index.bin";
  }
...
public:
  ...
  static void write_to_kml_node(vcl_ofstream& ofs, volm_geo_index_node_sptr n, unsigned current_depth, unsigned depth,
