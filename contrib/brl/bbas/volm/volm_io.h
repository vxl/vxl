...
public:
  ...
  enum label_values { INVALID = 0, SAND = 31, WATER = 11, DEVELOPED_LOW = 22, DEVELOPED_MED = 23, DEVELOPED_HIGH = 24,
                      DEVELOPED_OPEN = 21, WETLAND = 95, WOODY_WETLAND = 90, BUILDING = 100, FORT = 106, PIER = 114,
                      BUILDING_TALL = 119 };
...
public:
  ...
  volm_category_attribute(vcl_string lnd, vcl_string ori, unsigned is_active) : lnd_(lnd), ori_(ori), is_active_(
      is_active) {}
...
public:
  ...
  static void size(unsigned char& fallback_size)
  {
    fallback_size =
      (unsigned char)volm_fallback_label::fallback_id[0].size();
  }
  static void print_id(unsigned char id)
  {
    vcl_cout << '[';
    for( vcl_vector<unsigned char>::iterator vit = volm_fallback_label::fallback_id[id].begin();
         vit != volm_fallback_label::fallback_id[id].end(); ++vit )
      {
  ...
  {
    vcl_cout << '[';
    for( vcl_vector<float>::iterator vit = volm_fallback_label::fallback_weight[id].begin();
         vit != volm_fallback_label::fallback_weight[id].end(); ++vit )
      {
...
public:
  ...
  enum VOLM_ERROR_CODES { SUCCESS, EXE_ARGUMENT_ERROR, EXE_RUNNING, CAM_FILE_IO_ERROR, MATCHER_EXE_STARTED,
                          MATCHER_EXE_FINISHED, MATCHER_EXE_FAILED,
  ...
  static bool write_status(vcl_string out_folder, int status_code, int percent = 0, vcl_string log_message = "",
                           vcl_string status_file = "status.xml");
  ...
  static bool read_camera(vcl_string kml_file, unsigned const& ni, unsigned const& nj, double& heading,
                          double& heading_dev, double& tilt,      double& tilt_dev, double& roll,      double& roll_dev,
                          double& top_fov,   double& top_fov_dev, double& altitude, double& lat, double& lon);

  static bool read_labelme(vcl_string xml_file, vcl_string category_file, depth_map_scene_sptr& depth_scene,
                           vcl_string& img_category);

  // : parser for reading xml tags to create depth_map_scene
  static bool read_query_tags(vcl_string xml_file, depth_map_scene_sptr& depth_scene, vcl_vector<volm_weight>& weights,
                              vcl_string& world_region, unsigned& img_ni, unsigned& img_nj, vcl_string& query_name);

  // : parser for reading xml tags for landmark configuration matcher
  static bool read_conf_query_tags(vcl_string xml_file, float const& floor_height, depth_map_scene_sptr& depth_scene,
                                   vcl_string& world_region, unsigned& img_ni, unsigned& img_nj,
  ...
  static unsigned char scale_score_to_1_255_sig(float const& kl, float const & ku, float const& threshold,
                                                float const& score);
  ...
  static int read_gt_file(vcl_string gt_file, vcl_vector<vcl_pair<vgl_point_3d<double>, vcl_pair<vcl_pair<vcl_string,
                                                                                                          int>,
                                                                                                 vcl_string> > >& samples);
  ...
  static bool read_building_file(vcl_string file, vcl_vector<vcl_pair<vgl_polygon<double>,
  ...
  static bool write_sme_kml_type(vcl_string file, vcl_string type_name, vcl_vector<vcl_pair<vgl_point_2d<double>,
                                                                                            int> >& objects);
...
public:
  ...
  static bool write_top_matches(vcl_multiset<vcl_pair<float, volm_rationale>, std::greater<vcl_pair<float,
                                                                                                    volm_rationale> > >& top_matches,
                                vcl_string& filename);

  static bool read_top_matches(vcl_multiset<vcl_pair<float, volm_rationale>, std::greater<vcl_pair<float,
                                                                                                   volm_rationale> > >& top_matches,
                               vcl_string& filename);
...
public:
  ...
  volm_score(unsigned const& leaf_id, unsigned const& hypo_id, float const& max_score, unsigned const& max_cam_id,
...
public:
  volm_weight() {}
  volm_weight(vcl_string const& w_name, vcl_string const& w_typ, float const& w_ori, float const& w_lnd,
              float const& w_ord, float const& w_dst, float const w_obj)
