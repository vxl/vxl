...
bool volm_io_tools::load_naip_img(vcl_string const& img_folder, vcl_string const& name, vpgl_lvcs_sptr& lvcs,
                                  volm_img_info& info, bool load_resource)
{
  ...
  if( utm_zone != zone_max )
    {
    vcl_cout << "!!!!!!!!!!!!!!!!!!!!!!!!!WARNING! img: " << img_name
             << " has min and max points in different UTM zones, using zone of min point!\n";
                                  ...
                                  bool load_cam_from_tfw, vcl_string const& cam_tfw_file)
{
  ...
  vcl_cout << " zone of lidar img: " << img_file << ": " << utm_zone << " from lower left corner: " << lon << ','
           << lat << "!\n";
...
void volm_io_tools::load_imgs(vcl_string const& folder, vcl_vector<volm_img_info>& infos, bool load_image_resource,
                              bool is_cam_global, bool load_cam_from_tfw)
{
  vcl_string in_dir = folder + "/*.tif";  // sometimes .tif is written .tiff
...
bool volm_io_tools::get_location_nlcd(vcl_vector<volm_img_info>& NLCD_imgs, double lat, double lon, double elev,
                                      unsigned char& label)
{
  bool found_it = false;
...
volm_geo_index2_node_sptr volm_io_tools::read_osm_data_and_tree(vcl_string geoindex_filename_pre,
                                                                vcl_string osm_bin_filename, volm_osm_objects& osm_objs,
                                                                double& min_size)
{
  ...
  for( unsigned l_idx = 0; l_idx < leaves.size(); l_idx++ )
    ...
    volm_geo_index2_node<volm_osm_object_ids_sptr>* ptr =
      dynamic_cast<volm_geo_index2_node<volm_osm_object_ids_sptr> *>(leaves[l_idx].ptr() );
...
bool volm_io_tools::load_satellite_height_map(vcl_string const& filename, volm_img_info& info,
                                              bool const& load_cam_from_file)
{
  info.img_name = filename;
  info.name = vul_file::strip_directory(info.img_name);
  info.name = vul_file::strip_extension(info.name);
  ...
  if( utm_zone_ur != utm_zone )
    {
    vcl_cout << " warning: satellite height image " << info.name << " crosses two utm zone: " << utm_zone << ", "
             << utm_zone_ur << vcl_endl;
    }
  unsigned northing = 1;
  ...
  if( upper_right.x() * lower_left.x() < 0 )
    {
    vcl_cout << " warning: satellite height image " << info.name << " crosses the Equator (set it to be northing)"
             << vcl_endl;
    }
...
bool volm_io_tools::load_satellite_height_imgs(vcl_string const& folder, vcl_vector<volm_img_info>& infos,
                                               bool const& load_cam_from_file, vcl_string const& keywords)
{
...
void crop_and_find_min_max(vcl_vector<volm_img_info>& infos, unsigned img_id, int i0, int j0, int crop_ni, int crop_nj,
...
bool volm_io_tools::find_min_max_height(vgl_point_2d<double>& lower_left, vgl_point_2d<double>& upper_right,
                                        vcl_vector<volm_img_info>& infos, double& min, double& max)
{
...
bool find_intersect(vgl_box_2d<double> const& bbox, vgl_point_2d<double> const& s, vgl_point_2d<double> e,
...
bool volm_io_tools::line_inside_the_box(vgl_box_2d<double> const& bbox, vcl_vector<vgl_point_2d<double> > const& line,
                                        vcl_vector<vgl_point_2d<double> >& road)
{
...
void form_line_segment_from_pts(vcl_vector<vgl_point_2d<double> > const& road,
...
{
  vgl_line_segment_3d<double> l1(vgl_point_3d<double>(seg.point1().x(), seg.point1().y(), 0.0), vgl_point_3d<double>(
                                   seg.point2().x(), seg.point2().y(), 0.0) );
    ...
    for( unsigned r_idx = 0; r_idx < n_rds; r_idx++ )
      ...
      for( unsigned p_idx = 0; p_idx < pt.size(); p_idx++ )
        ...
        if( vcl_find(cross_pts.begin(), cross_pts.end(), pt[p_idx]) == cross_pts.end() )
          {
          cross_pts.push_back(pt[p_idx]);  cross_props.push_back(prop[p_idx]);  cross_geo_props.push_back(
            volm_osm_category_io::volm_land_table_name["4_way"]);
          }
        }
      }
    }
