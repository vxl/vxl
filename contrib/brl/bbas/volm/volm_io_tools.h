...
public:
  ...
  static bool load_naip_img(vcl_string const& img_folder, vcl_string const& name, vpgl_lvcs_sptr& lvcs,
                            volm_img_info& info, bool load_resource = false);
  ...
  static int load_lidar_img(vcl_string img_file, volm_img_info& info, bool load_resource = false,
                            bool is_cam_global = false, bool load_cam_from_tfw = false,
  ...
  static bool get_location_nlcd(vcl_vector<volm_img_info>& infos, double lat, double lon, double elev,
                                unsigned char& label);
  ...
  static volm_geo_index2_node_sptr read_osm_data_and_tree(vcl_string geoindex_filename_pre, vcl_string osm_bin_filename,
                                                          volm_osm_objects& osm_objs, double& min_size);
  ...
  static bool load_satellite_height_map(vcl_string const& filename, volm_img_info& info,
                                        bool const& load_cam_from_file = false);
  ...
  static bool load_satellite_height_imgs(vcl_string const& folder, vcl_vector<volm_img_info>& infos,
                                         bool const& load_cam_from_file = true, vcl_string const& keywords = "");
  ...
  static bool find_min_max_height(vgl_point_2d<double>& lower_left, vgl_point_2d<double>& upper_right,
  ...
  static bool line_inside_the_box(vgl_box_2d<double> const& bbox, vcl_vector<vgl_point_2d<double> > const& line,
                                  vcl_vector<vgl_point_2d<double> >& line_in);
  ...
  static bool search_junctions(vcl_vector<vgl_point_2d<double> > const& road, volm_land_layer const& road_prop,
                               ...
                               vcl_vector<volm_land_layer> net_props, vcl_vector<vgl_point_2d<double> >& cross_pts,
                               vcl_vector<volm_land_layer>& cross_props, vcl_vector<volm_land_layer>& cross_geo_props);
