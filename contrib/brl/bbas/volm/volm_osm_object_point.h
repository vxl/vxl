...
public:
  // : default constructor
  volm_osm_object_point() : prop_(0, "invalid", 0, 0.0, vil_rgb<vxl_byte>(0, 0, 0) ), loc_(vgl_point_2d<double>(0.0,
                                                                                                                0.0) ) {}

  ////: constructors
  volm_osm_object_point(volm_land_layer prop, vgl_point_2d<double> loc) : prop_(prop), loc_(loc) {}
  volm_osm_object_point(volm_land_layer prop, vgl_point_3d<double> loc) : prop_(prop),
    loc_(vgl_point_2d<double>(loc.x(), loc.y() ) ) {}
  volm_osm_object_point(volm_land_layer prop, double lon, double lat) : prop_(prop),
    loc_(vgl_point_2d<double>(lon, lat) ) {}
