...
volm_osm_objects::volm_osm_objects(vcl_string const& osm_file, vcl_string const& osm_to_volm_file)
{
  ...
  for( unsigned i = 0; i < num_pts; i++ )
    ...
    for( vcl_vector<vcl_pair<vcl_string, vcl_string> >::iterator vit = curr_keys.begin();
         (vit != curr_keys.end() && !pier_found); ++vit )
      {
      if( vit->first == "man_made" && vit->second == "pier" )
        ...
        if( mit != osm_land_table.end() )
          {
          pier_found = true;  volm_land_layer prop = mit->second;
  ...
  for( unsigned i = 0; i < num_polys; i++ )
    ...
    for( vcl_vector<vcl_pair<vcl_string, vcl_string> >::iterator vit = curr_keys.begin();
      ...
      if( vit->first == "man_made" && vit->second == "pier" )
        ...
        if( mit != osm_land_table.end() )
          {
          pier_found = true;  volm_osm_object_polygon_sptr loc_poly = new volm_osm_object_polygon(mit->second,
                                                                                                  osm_polys[i]);
