...
void volm_osm_parser::startElement(const XML_Char* name, const XML_Char* * atts)
{
  ...
  if( vcl_strcmp(name, OSM_NODE) == 0 )
    ...
    if( atts )
      ...
      // insert 3d points
      nodes_3d_.insert(vcl_pair<unsigned long long, vgl_point_3d<double> >(node_id, vgl_point_3d<double>(lon, lat,
                                                                                                         alt) ) );
      }
    }
                                   ...
                                   vcl_string const& osm_file)
{
  ...
  vcl_map<unsigned long long,
          vcl_vector<vcl_pair<vcl_string, vcl_string> > >::iterator mit = parser->node_keys_.begin();
...
vcl_vector<unsigned long long> get_line_from_way_id(unsigned long long const& way_id,
                                                    vcl_vector<vcl_pair<unsigned long long,
                                                                        vcl_vector<unsigned long long> > >& ways)
{
                                     ...
                                     vcl_string const& osm_file)
{
  ...
  for( vcl_map<unsigned long long, vcl_string>::iterator mit = relation_type.begin(); mit != relation_type.end();
       ++mit )
    {
    ...
    for( unsigned w_idx = 0; w_idx < (unsigned)way_ids.size(); w_idx++ )
      {
      ways.push_back(vcl_pair<unsigned long long, vcl_vector<unsigned long long> >(way_ids[w_idx],
                                                                                   parser->ways_[way_ids[w_idx]]) );
      }
    /*for (unsigned w_idx = 0; w_idx < (unsigned)way_ids.size(); w_idx++)
