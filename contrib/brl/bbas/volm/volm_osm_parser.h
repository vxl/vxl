...
public:
  ...
  static void parse_points(vcl_vector<vgl_point_2d<double> >& points, vcl_vector<vcl_vector<vcl_pair<vcl_string,
                                                                                                     vcl_string> > >& keys,
  ...
  static void parse_points_3d(vcl_vector<vgl_point_3d<double> >& points, vcl_string const& osm_file);
  ...
  static void parse_lines(vcl_vector<vcl_vector<vgl_point_2d<double> > >& lines,
                          vcl_vector<vcl_vector<vcl_pair<vcl_string, vcl_string> > >& keys, vcl_string const& osm_file);

  // : parse polygons from open street map (polygon can be from ways and relation elements)
  static void parse_polygons(vcl_vector<vgl_polygon<double> >& polys, vcl_vector<vcl_vector<vcl_pair<vcl_string,
                                                                                                     vcl_string> > >& keys,
                             vcl_string const& osm_file);
