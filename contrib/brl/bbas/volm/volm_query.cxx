  ...
  : cam_space_(cam_space), invalid_( (unsigned char)255), sph_depth_(sph), sph_(sph_shell)
{
  ...
  if( use_default_ )
    ...
    volm_io::read_camera(cam_kml_file, ni_, nj_, head, head_d, tilt, tilt_d, roll, roll_d, tfov, tfov_d, altitude, lat,
                         lon);
    ...
    for( unsigned j = 0; j < rolls_.size(); ++j )
      ...
      for( unsigned k = 0; k < top_fov_.size(); ++k )
        ...
        for( unsigned h = 0; h < headings_.size(); ++h )
          ...
          vpgl_perspective_camera<double> cam = bpgl_camera_utils::camera_from_kml( (double)ni_, (double)nj_, right_f,
                                                                                    top_f, altitude_, head, tilt, roll);
...
#  if 0
              vcl_cout << "checking ground plane consistency for: " << dm_->ground_plane()[i]->name()
                       << " min depth is: " << dm_->ground_plane()[i]->min_depth()
...
#  endif
            ...
            if( success )
              ...
              camera_strings_.push_back(bpgl_camera_utils::get_string( (double)ni_, (double)nj_, right_f, top_f,
                                                                       altitude_, head, tilt, roll) );
              }
            else
              {
#  if 0
              vcl_cout
                       <<
              "WARNING: following camera hypothesis is NOT consistent with defined ground plane in the query image and ignored\n"
                       << " \t heading = " << head << ", tilt = " << tilt << ", roll = " << roll << ", top_fov = "
                       << top_f << ", right_fov = " << right_f << vcl_endl;
#  endif
              }
            }
          else
    ...
    for( unsigned p_idx = 0; p_idx < query_size_; ++p_idx )
      ...
      if( cam.is_behind_camera(vgl_homg_point_3d<double>(qp) ) )
        ...
        if( u > (double)ni_ || v > (double)nj_ || u < TOL || v < TOL )     // container point qp is outside camera viewing volume
          ...
          min_dist = this->fetch_depth(u, v, order, max_dist, obj_id, grd_fallback_id, grd_fallback_wgt, is_ground,
                                       is_sky, is_object, depth_img);
...
void volm_query::draw_viewing_volume(vcl_string const& fname, vpgl_perspective_camera<double> cam, float r, float g,
                                     float b)
{
  vcl_ofstream ofs(fname.c_str(), vcl_ios::app);
...
void volm_query::draw_polygon(vil_image_view<vil_rgb<vxl_byte> >& img, vgl_polygon<double> const& poly,
                                 ...
                                 vcl_string value_type)
{
  ...
  if( value_type == "orientation" )
    ...
    {
    vcl_cerr << "WARNING: given image type " << value_type
             << " is not found in volm_query::depth_rgb_image, generate depth image instead\n";
    for( unsigned pidx = 0; pidx < query_size_; ++pidx )
      {
      if( values[pidx] < 255 )
        {
