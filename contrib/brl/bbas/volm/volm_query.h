...
public:
  ...
  volm_query(volm_camera_space_sptr cam_space, vcl_string const& label_xml_file, vcl_string const& category_file,
             volm_spherical_container_sptr const& sph, volm_spherical_shell_container_sptr const& sph_shell);

  // : constructor from depth map scene
  volm_query(volm_camera_space_sptr cam_space, vcl_string const& depth_map_scene_file,
             volm_spherical_shell_container_sptr const& sph_shell, volm_spherical_container_sptr const& sph);

  // : constructor from a binary file of data members
  volm_query(vcl_string const& query_file, volm_camera_space_sptr cam_space, vcl_string const& depth_map_scene_file,
             volm_spherical_shell_container_sptr const& sph_shell, volm_spherical_container_sptr const& sph);
  ...
  unsigned get_cam_num() const
  {
    return (unsigned)cam_space_->
           valid_indices().size();
  }
  unsigned get_obj_order_num() const
  {
    return (unsigned)order_index_[0].size();
  }
  ...
  void depth_rgb_image(vcl_vector<unsigned char> const& values, unsigned const& cam_id,
                       vil_image_view<vil_rgb<vxl_byte> >& out_img, vcl_string value_type = "depth");
  ...
  static void draw_polygon(vil_image_view<vil_rgb<vxl_byte> >& img, vgl_polygon<double> const& poly,
...
protected:
  ...
  unsigned char fetch_depth(double const& u, double const& v, unsigned char& order, unsigned char& max_dist,
                            unsigned& object_id, vcl_vector<unsigned char>& grd_fallback_id,
                            vcl_vector<float>& grd_fallback_wgt, bool& is_ground, bool& is_sky, bool& is_object,
  ...
  void draw_dot(vil_image_view<vil_rgb<vxl_byte> >& img, vgl_point_3d<double> const& world_point, vil_rgb<vxl_byte>,
