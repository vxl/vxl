  ...
  if( eliminate_same_ )
    ...
    for( unsigned i = 0; i < resources_.size(); i++ )
      ...
      if( resources_[i].meta_->satellite_name_.compare(res.meta_->satellite_name_) == 0 &&
        ...
        vcl_cout << " band of resources: " <<  resources_[i].meta_->band_ << " band trying to add: "
                 << res.meta_->band_ << vcl_flush << vcl_endl;
        return;
        }
      }
    }
...
volm_satellite_resources::volm_satellite_resources(vgl_box_2d<double>& bbox, double min_size,
                                                   bool eliminate_same) : min_size_(min_size), bbox_(bbox),
  eliminate_same_(eliminate_same)
{
  this->construct_tree();
}
...
void volm_satellite_resources::add_resources(unsigned start, unsigned end)
{
  ...
  for( unsigned i = start; i < end; i++ )
    ...
    satellite_footprint.add(vgl_point_2d<double>(resources_[i].meta_->lower_left_.x(),
                                                 resources_[i].meta_->lower_left_.y() ) );
    satellite_footprint.add(vgl_point_2d<double>(resources_[i].meta_->upper_right_.x(),
                                                 resources_[i].meta_->upper_right_.y() ) );
    volm_geo_index2::get_leaves(root_, leaves, satellite_footprint);
    for( unsigned j = 0; j < leaves.size(); j++ )
      {
      volm_geo_index2_node<vcl_vector<unsigned> >* leaf_ptr =
        dynamic_cast<volm_geo_index2_node<vcl_vector<unsigned> > *>(leaves[j].ptr() );
...
void volm_satellite_resources::query(double lower_left_lon, double lower_left_lat, double upper_right_lon,
                                     double upper_right_lat, const vcl_string& band_str, vcl_vector<unsigned>& ids,
                                     double gsd_thres)
{
  ...
  for( unsigned i = 0; i < leaves.size(); i++ )
    {
    volm_geo_index2_node<vcl_vector<unsigned> >* leaf_ptr =
      dynamic_cast<volm_geo_index2_node<vcl_vector<unsigned> > *>(leaves[i].ptr() );
    ...
    for( unsigned k = 0; k < leaf_ptr->contents_.size(); k++ )
      ...
      sat_box.add(vgl_point_2d<double>(resources_[res_id].meta_->lower_left_.x(),
                                       resources_[res_id].meta_->lower_left_.y() ) );
      sat_box.add(vgl_point_2d<double>(resources_[res_id].meta_->upper_right_.x(),
                                       resources_[res_id].meta_->upper_right_.y() ) );
...
bool volm_satellite_resources::query_print_to_file(double lower_left_lon, double lower_left_lat, double upper_right_lon,
                                                   double upper_right_lat, unsigned& cnt, vcl_string& out_file,
                                                   const vcl_string& band_str, double gsd_thres)
{
...
bool volm_satellite_resources::query_seeds_print_to_file(double lower_left_lon, double lower_left_lat,
                                                         double upper_right_lon, double upper_right_lat, int n_seeds,
                                                         unsigned& cnt, vcl_string& out_file, vcl_string& band_str,
                                                         double gsd_thres)
{
  ...
  vcl_cout << "using bbox for scene: " << lower_lon << " " << lower_lat << " " << upper_lon << " " << upper_lat
           << vcl_endl;
  ...
  for( unsigned i = 0; i < ids.size(); i++ )
    ...
    if( resources_[ids[i]].meta_->cloud_coverage_percentage_ < 1 )
      {
      vcl_map<vcl_string, vcl_vector<unsigned> >::iterator iter = possible_seeds.find(
          resources_[ids[i]].meta_->satellite_name_);
  ...
  for( vcl_map<vcl_string, vcl_vector<unsigned> >::iterator iter = possible_seeds.begin(); iter != possible_seeds.end();
       iter++ )
    {
    vcl_cout << iter->first << "\n";
...
// : get a list of ids in the resources_ list that overlap the given rectangular region
unsigned volm_satellite_resources::query_pairs(double lower_left_lon, double lower_left_lat, double upper_right_lon,
                                               double upper_right_lat, vcl_string& sat_name, float GSD_thres,
                                               vcl_vector<vcl_pair<unsigned, unsigned> >& ids)
{
  ...
  for( unsigned i = 0; i < ids2.size(); i++ )
    ...
    for( unsigned j = i + 1; j < ids2.size(); j++ )
      ...
      vcl_cout << " azi: " << resources_[ids2[i]].meta_->sun_azimuth_ << " elev: "
               << resources_[ids2[i]].meta_->sun_elevation_ << " res: " << resources_[ids2[i]].meta_->gsd_ << vcl_endl;
      vcl_cout << resources_[ids2[j]].name_ << ": ";
      resources_[ids2[j]].meta_->print_time();
      vcl_cout << " azi: " << resources_[ids2[j]].meta_->sun_azimuth_ << " elev: "
               << resources_[ids2[j]].meta_->sun_elevation_ << " res: " << resources_[ids2[j]].meta_->gsd_ << vcl_endl;
...
bool volm_satellite_resources::query_pairs_print_to_file(double lower_left_lon, double lower_left_lat,
                                                         double upper_right_lon, double upper_right_lat,
                                                         float GSD_thres, unsigned& cnt, vcl_string& out_file,
                                                         vcl_string& sat_name)
{
  vcl_vector<vcl_pair<unsigned, unsigned> > ids;
  ...
  if( !ofs )
    {
    vcl_cerr << "In volm_satellite_resources::query_pairs_print_to_file() -- cannot open file: " << out_file
             << vcl_endl;
    return false;
    }
  for( unsigned i = 0; i < ids.size(); i++ )
...
void
volm_satellite_resources::convert_to_global_footprints(vcl_vector<vgl_polygon<double> >& footprints,
                                                       const vpgl_lvcs_sptr& lvcs,
                                                       const vcl_vector<vgl_polygon<double> >& lvcs_footprints,
...
void
volm_satellite_resources::convert_to_local_footprints(vpgl_lvcs_sptr& lvcs,
                                                      ...
                                                      float downsample_factor)
{
  ...
  for( vcl_vector<vgl_polygon<double> >::const_iterator it = footprints.begin(); it != footprints.end(); ++it )
    ...
    for( unsigned int p = 0; p < footprint[0].size(); ++p )
      ...
      double x, y, z;
      lvcs->global_to_local(lon, lat, 0, lvcs->get_cs_name(), x, y, z, lvcs->geo_angle_unit(),
                            lvcs->local_length_unit() );
...
void
volm_satellite_resources::compute_footprints_heatmap(vil_image_view<unsigned>& heatmap,
                                                     vgl_box_2d<double>& image_window,
                                                     const vcl_vector<vgl_polygon<double> >& footprints)
{
...
void
volm_satellite_resources::query_resources(vcl_vector<vgl_polygon<double> >& footprints,
                                          vcl_vector<unsigned>& footprint_ids,
                                          volm_satellite_resources_sptr res, const vcl_string& kml_file,
                                          const vcl_string& band, double gsd_thres)
{
...
void
volm_satellite_resources::highly_overlapping_resources(vcl_vector<vcl_string>& overlapping_res,
                                                       volm_satellite_resources_sptr res,
                                                       const vcl_string& kml_file, float downsample_factor,
                                                       const vcl_string& band, double gsd_thres)
{
  // query_resources
...
// #define DEBUG_HIGHLY_OVERLAPPING_SAT_RESOURCES
  ...
  for( size_t i = 0; i < overlapping_ids.size(); ++i )
    {
    overlapping_ids[i] = i;
    }
...
void
volm_satellite_resources::highly_intersecting_resources(vcl_vector<vcl_string>& overlapping_res,
                                                        volm_satellite_resources_sptr res,
                                                        const vcl_string& kml_file, int k, int l,
                                                        const vcl_string& band, double gsd_thres)
{
  // query_resources
  vcl_vector<vgl_polygon<double> > footprints;
...
volm_satellite_resources::highly_intersecting_resources(vcl_vector<unsigned>& overlapping_ids,
                                                        const vcl_vector<vgl_polygon<double> >& footprints, unsigned k,
                                                        unsigned l)
{
  double max_area = 0.0;
...
bool volm_satellite_resources::same_time(volm_satellite_resource const& res_a, volm_satellite_resource const& res_b,
                                         float const& diff_in_sec)
{
...
bool volm_satellite_resources::same_view(volm_satellite_resource const& res_a, volm_satellite_resource const& res_b,
                                         double const & tol)
{
...
bool volm_satellite_resources::same_extent(volm_satellite_resource const& res_a, volm_satellite_resource const& res_b,
                                           double const& tol)
{
