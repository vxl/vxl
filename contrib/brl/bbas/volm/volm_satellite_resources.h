...
public:
  ...
  void query(double lower_left_lon, double lower_left_lat, double upper_right_lon, double upper_right_lat,
             const vcl_string& band_str, vcl_vector<unsigned>& ids, double gsd_thres);

  // : query the resources in the given box and output the full paths to the given file
  bool query_print_to_file(double lower_left_lon, double lower_left_lat, double upper_right_lon, double upper_right_lat,
                           unsigned& cnt, vcl_string& out_file, const vcl_string& band_str, double gsd_thres);
  ...
  bool query_seeds_print_to_file(double lower_left_lon, double lower_left_lat, double upper_right_lon,
                                 double upper_right_lat, int n_seeds, unsigned& cnt, vcl_string& out_file,
                                 vcl_string& band_str, double gsd_thres);

  // : get a list of pairs of ids in the resources_ list that are taken a few minutes apart from each other
  unsigned query_pairs(double lower_left_lon, double lower_left_lat, double upper_right_lon, double upper_right_lat,
                       vcl_string& sat_name, float GSD_threshold, vcl_vector<vcl_pair<unsigned, unsigned> >& ids);

  // : query the resources in the given box and output the full paths of pairs to the given file
  bool query_pairs_print_to_file(double lower_left_lon, double lower_left_lat, double upper_right_lon,
                                 double upper_right_lat, float GSD_threshold, unsigned& cnt, vcl_string& out_file,
                                 vcl_string& sat_name);

  void rasterize(const vgl_polygon<double> & bounds, vil_image_view<bool> & mask);

  void rasterize(const vgl_box_2d<double> & bbox_clipped, const vgl_polygon<double> & bounds,
  ...
  void query_resources(vcl_vector<vgl_polygon<double> >& footprints, vcl_vector<unsigned>& footprint_ids,
                       volm_satellite_resources_sptr res, const vcl_string& kml_file, const vcl_string& band = "PAN",
                       double gsd_thres = 10.0);

  void highly_overlapping_resources(vcl_vector<vcl_string>& overlapping_res, volm_satellite_resources_sptr res,
                                    const vcl_string& kml_file, float downsample_factor, const vcl_string& band = "PAN",
                                    double gsd_thres = 10.0);

  void highly_overlapping_resources(vcl_vector<unsigned>& overlapping_ids,
  ...
  void highly_intersecting_resources(vcl_vector<vcl_string>& overlapping_res, volm_satellite_resources_sptr res,
                                     const vcl_string& kml_file, int k = 3, int l = 5, const vcl_string& band = "PAN",
                                     double gsd_thres = 10.0);

  void highly_intersecting_resources(vcl_vector<unsigned>& overlapping_ids,
                                     const vcl_vector<vgl_polygon<double> >& footprints, unsigned k = 3,
                                     unsigned l = 5);
...
protected:
  ...
  bool same_time(volm_satellite_resource const& res_a, volm_satellite_resource const& res_b,
                 float const& t_diff_in_sec = 10);
