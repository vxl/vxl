  ...
  : depth_offset_(0), ds_(d_solid_ang), vmin_(voxel_min)
{
  ...
  unsigned int i = 0;
  for( vcl_map<double, unsigned int>::iterator iter = depth_offset_map_.begin(); iter != depth_offset_map_.end();
       iter++ )
    {
    ...
    if( i > 252 )
      ...
      vcl_cerr
        <<
      "In volm_spherical_container::volm_spherical_container() -- number of depth intervals is larger than 253! Current indexing scheme does not support this container extent of "
        << dmax_ << "!\n";
      break;
      }
    }
...
void volm_spherical_container::draw_template_painted(vcl_string vrml_file_name, double dmin,
                                                     vcl_vector<unsigned int>& ids, float r, float g, float b,
                                                     float trans)
{
  vcl_ofstream ofs(vrml_file_name.c_str() );
  // write the header
...
void volm_spherical_container::draw_template_vis_prob(vcl_string vrml_file_name, double dmin, vcl_vector<char>& ids)
{
  ...
  if( voxels_.size() != ids.size() )
    ...
    vcl_cerr
      <<
    "In volm_spherical_container::draw_template_vis_prob() -- passed vector is not of the same size with voxels vector!\n";
    return;
    }
