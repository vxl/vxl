...
{
public:
  volm_spherical_layers(vpgl_perspective_camera<double> const& cam, depth_map_scene_sptr const& dm_scene,
                        double altitude, volm_spherical_container_sptr const& sph_vol,
                        volm_spherical_shell_container_sptr const& sph_shell, unsigned char invalid,
                        unsigned char default_sky_order, double d_threshold, unsigned log_downsample_ratio);
  volm_spherical_layers(depth_map_scene_sptr const& dm_scene, double altitude,
...
private:
  unsigned char fetch_depth(double const& u, double const& v, vcl_vector<depth_map_region_sptr> const& depth_regions,
                            vcl_vector<depth_map_region_sptr> const& ground_plane,
                            vcl_vector<depth_map_region_sptr> const& sky, unsigned char& order, unsigned char& max_dist,
                            unsigned& object_id,  unsigned char& grd_nlcd, bool& is_ground,  bool& is_sky,
