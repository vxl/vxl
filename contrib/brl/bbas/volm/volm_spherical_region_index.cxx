...
volm_spherical_region_index::volm_spherical_region_index(vcl_map<vcl_string, vcl_string> & index_file_paths,
...
volm_spherical_region_index::volm_spherical_region_index(float * boxes, int num_depth_regions,
                                                         int num_orientation_regions, int num_nlcd_regions,
                                                         int sky_regions)
{
  unsigned count = 0;
  ...
  for( int i = 0; i < num_depth_regions; ++i, ++count )
    ...
    box2d.set(boxes[count * 6 + 3], boxes[count * 6 + 4], this->check_phi_bounds(
                boxes[count * 6 + 0]), this->check_phi_bounds(boxes[count * 6 + 1]),
              this->check_phi_bounds(boxes[count * 6 + 2]) );
...
volm_spherical_region_index::volm_spherical_region_index(vcl_map<vcl_string,
                                                                 vcl_vector<unsigned char> > & index_buffers,
