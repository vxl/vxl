...
// constructor
volm_spherical_shell_container::volm_spherical_shell_container(double radius, float cap_angle, float point_angle,
...
volm_spherical_shell_container::volm_spherical_shell_container(double point_angle, double min_theta,
                                                               double max_theta)
{
...
void volm_spherical_shell_container::draw_template(vcl_string vrml_file_name, vcl_vector<unsigned char>& values,
...
void volm_spherical_shell_container::panaroma_img(vil_image_view<vil_rgb<vxl_byte> >& img,
...
void volm_spherical_shell_container::panaroma_img_class_labels(vil_image_view<vil_rgb<vxl_byte> >& img,
                                                               vcl_vector<unsigned char>& values)
{
  assert(values.size() == usph_->size() );
  vcl_vector<vsph_sph_point_3d> sph_pts = this->sph_points();
...
void volm_spherical_shell_container::panaroma_img_orientations(vil_image_view<vil_rgb<vxl_byte> >& img,
                                                               vcl_vector<unsigned char>& values)
{
  assert(values.size() == usph_->size() );
  vcl_vector<vsph_sph_point_3d> sph_pts = this->sph_points();
...
void volm_spherical_shell_container::panaroma_images_from_combined(vil_image_view<vil_rgb<vxl_byte> >& img_orientation,
                                                                   ...
                                                                   vcl_vector<unsigned char>& values)
{
  ...
  for( unsigned i = 0; i < sph_pts.size(); i++ )
    ...
    if( values[i] == 253 )  // invalid
      ...
      if( volm_orient_table::ori_index_colors.find( (int)orientation_value) ==
          volm_orient_table::ori_index_colors.end() )
        {
