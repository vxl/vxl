...
volm_tile::volm_tile(float lat, float lon, char hemisphere, char direction, float scale_i, float scale_j, unsigned ni,
                     unsigned nj) :
  lat_(lat), lon_(lon), hemisphere_(hemisphere), direction_(direction), scale_i_(scale_i), scale_j_(scale_j), ni_(ni),
  nj_(nj)
{
...
volm_tile::volm_tile(vcl_string file_name, unsigned ni, unsigned nj) : ni_(ni), nj_(nj)
{
  ...
  if( hemisphere_ ==
      'N' )
    {
    vcl_cout << " upper left corner in the image is: " << hemisphere_ << lat_ + scale_i_ << direction_ << lon_
             << vcl_endl;
    }
  else
    {
  ...
  if( direction_ ==
      'W' )
    {
    vcl_cout << " lower right corner in the image is: " << hemisphere_ << lat_ << direction_ << lon_ - scale_j_
             << vcl_endl;
    }
  else
    {
...
vcl_vector<volm_tile> volm_tile::generate_p2a_wr8_tiles()
{
  ...
  tiles.push_back(volm_tile(38, 147, 'S', 'E', scale_i, scale_j, ni, nj) );
...
bool volm_tile::global_to_img(double lon, double lat, unsigned& i, unsigned& j)
{
  ...
  if( j == this->nj_ )
    {
    j--;                           // v may be larger than nj+0.5 due to the floating point precision
    }
  return true;
}
...
void volm_tile::get_uncertainty_region(float lambda_i, float lambda_j, float cutoff, vbl_array_2d<bool>& mask,
...
void volm_tile::mark_uncertainty_region(int i, int j, float score, vbl_array_2d<bool>& mask,
                                        vbl_array_2d<float>& kernel, vil_image_view<unsigned int>& img)
{
  unsigned nrows = (unsigned)mask.rows();
  unsigned ncols = (unsigned)mask.cols();
...
void volm_tile::mark_uncertainty_region(int i, int j, float score, vbl_array_2d<bool>& mask,
                                        vbl_array_2d<float>& kernel, vil_image_view<vxl_byte>& img)
{
  unsigned nrows = (unsigned)mask.rows();
  unsigned ncols = (unsigned)mask.cols();
