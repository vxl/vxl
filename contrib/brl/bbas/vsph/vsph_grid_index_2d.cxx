...
static void add(vcl_vector<vcl_pair<vsph_sph_point_2d, int> > const& vec_to_add, vcl_vector<vcl_pair<vsph_sph_point_2d,
                                                                                                     int> >& result)
{
                              ...
                              unsigned& ph_idx, int& id,  double tol) const
{
  ...
  if( spts.size() )
    {
    for( vcl_vector<vcl_pair<vsph_sph_point_2d, int> >::const_iterator sit = spts.begin();
         (sit != spts.end() ) && !found; ++sit )
      {
