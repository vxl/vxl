...
public:
  ...
  // break the ambiguity
  vsph_sph_box_2d(vsph_sph_point_2d const& pa, vsph_sph_point_2d const& pb, vsph_sph_point_2d const& pc);

  ~vsph_sph_box_2d() {}

  void set(double min_theta, double max_theta, double a_phi, double b_phi, double c_phi, bool in_radians = true);
  ...
  vsph_sph_box_2d transform(double t_theta, double t_phi, double scale, bool in_radians) const;

  vsph_sph_box_2d transform(double t_theta, double t_phi, double scale, double theta_c, double phi_c,
  ...
  bool sub_divide(vcl_vector<vsph_sph_box_2d>& sub_boxes, double min_ang = 0.035) const;

  // : decompose box into approximately planar quadrilaterals
  void planar_quads(vcl_vector<vgl_vector_3d<double> >& verts, vcl_vector<vcl_vector<int> >& quads,
                    double tol = 0.01) const;

  // : display the box as a set of planar quadrilaterals in vrml
  void display_box(vcl_ostream& os, float r, float g, float b, double tol = 0.01, double factor = 1.0) const;

  // : display a set of boxes
  static void display_boxes(vcl_string const& path, vcl_vector<vsph_sph_box_2d> const& boxes,
                            vcl_vector<vcl_vector<float> > colors, double tol = 0.01, double factor = 1.0);
...
bool intersection(vsph_sph_box_2d const& b1, vsph_sph_box_2d const& b2, vcl_vector<vsph_sph_box_2d>& boxes);
