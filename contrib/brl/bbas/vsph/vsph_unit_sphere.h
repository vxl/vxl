...
public:
  ...
  static vsph_sph_point_2d spher_coord(vgl_vector_3d<double> const& cp, bool in_radians = true);
  ...
  void display_data(vcl_string const & path, vcl_vector<double> const& data,
                    vsph_sph_box_2d const& mask = vsph_sph_box_2d() ) const;

  // : display a color distribution on the unit sphere
  void display_color(vcl_string const & path, vcl_vector<vcl_vector<float> > const& cdata, vcl_vector<float> const& skip_color =
                       vcl_vector<float>(3, -1.0f),
                     vsph_sph_box_2d const& mask = vsph_sph_box_2d() ) const;

  // : display a set of axis aligned boxes on the sphere
  void display_boxes(vcl_string const & path, vcl_vector<vsph_sph_box_2d> const& boxes);
