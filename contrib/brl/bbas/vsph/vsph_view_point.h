...
public:
  ...
  vsph_view_point(vsph_sph_point_3d const& p, vpgl_camera_double_sptr cam, T_data* metadata) : cam_(cam),
    spher_coord_(p), metadata_(metadata) {}
  ...
  {
    os << " vsph_view_point: camera=" << cam_->type_name() << ",coordinates="
       << spher_coord_ << ",data=" << *metadata_ << " " << vcl_endl;
  }

  void relative_transf(vpgl_camera_double_sptr const& cam, vgl_rotation_3d<double>& rel_rot,
  ...
  switch( version )
    ...
    case 1:
      ...
      if( cam )
        ...
        {
        vcl_cout << "vsph_view_point<T>::b_write -- Camera type:" << cam_->type_name() << " is not supported yet!"
                 << vcl_endl;
        }
    }
}
...
template <class T>
void vsph_view_point<T>::relative_transf(vpgl_camera_double_sptr const& cam,
