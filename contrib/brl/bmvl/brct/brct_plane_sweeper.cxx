...
brct_plane_sweeper::brct_plane_sweeper(brct_plane_sweeper_params& sp)
{
...
//:compute the projections of left and right images that overlap
                             ...
                             bool reset_flags)
{
  ...
  if(!overlapping_projections(z, imgs, proj_corners))
    return false;
  vcl_cout << "Correlating corners at z"  << z << "\n";
...
//:Map an image from one camera to the other, assuming all the pixels lie 
//on the plane at z.  Put the result into the coordinate frame of the target
//camera (to camera)
...
void brct_plane_sweeper::init_harris_match(const int from_cam)
{
  ...
  vcl_cout << "\nTotal points in point index = " << temp.size() << "\n";
  pindx_.clear_marks();
