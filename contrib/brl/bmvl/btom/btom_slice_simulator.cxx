...
// This is brl/bmvv/btom/btom_slice_simulator.cxx
#include <vcl_cmath.h>
										 ...
										 vil_memory_image_of<float> & reconst)
{
  ...
  int theta_max = 360;
  int tmax = (int)sqrt(float((xmax)*(xmax)+(ymax)*(ymax)));
  int tt = (tmax-1)/2;
  ...
  float linear_cyl = sqrt(float(ncyl_));
  ...
  for(float t = -tt; t<tt; t++)
    for(float th = 0; th<theta_max; th++)
      {
        ...
        for(vcl_vector<btom_gauss_cylinder_sptr>::iterator cit = cyls.begin();
            cit != cyls.end(); cit++)
			{
				...
				total_density += (*cit)->radon_transform(th,t);}
			
        sinogram(int(th),int(t+tt)) = total_density;
      }
    ...
    for(int x=-xt; x<xt; x++)
      {
        ...
        for(vcl_vector<btom_gauss_cylinder_sptr>::iterator cit = cyls.begin();
            cit != cyls.end(); cit++)
          intensity += (*cit)->cylinder_intensity(x,y);
      
        reconst(x+xt,y+yt) = intensity;
      }
}
