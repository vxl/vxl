...
public:
  ...
  // : Return a string representation of the default value
  virtual vcl_string type_str() const
  {
    return typeid(vcl_string("dummy") ) ==
           typeid(default_) ? "string" : typeid(default_).name();
  }
...
public:
  ...
  bprb_choice_param_type(const vcl_string& name, const vcl_string& desc,
    ...
    : bprb_param_type<unsigned>(name, desc, def_val, 0, static_cast<unsigned>(choices.size() - 1) ),
    choices_(choices) {}
...
public:
  ...
  void print_def_XML(const vcl_string& root_tag, const vcl_string& xml_path);

  // : prints the currently used parameter values to an XML document
  void print_current_XML(const vcl_string& root_tag, const vcl_string& xml_path);
