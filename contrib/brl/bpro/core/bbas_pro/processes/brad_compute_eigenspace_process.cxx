...
bool eigen_training_process(bbas_1d_array_string const& paths, double frac,
  ...
  if(frac >= 1.0){
    if(!espace.compute_covariance_matrix_blocked(resources, nit, njt))
      return false;
  }else
    if(!espace.compute_covariance_matrix_rand(resources, frac, nit, njt)) return false;
  
  if(!espace.compute_eigensystem())
    return false;
  return true;
}
}
