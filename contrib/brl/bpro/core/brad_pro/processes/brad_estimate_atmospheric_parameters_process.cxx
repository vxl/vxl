...
bool brad_estimate_atmospheric_parameters_process(bprb_func_process& pro)
{
  ...
  if( mean_reflectance > 0 )
    ...
    if( radiance_img->nplanes() == 1 )
      {
      succeed = brad_estimate_atmospheric_parameters(*radiance_img, *mdata, mean_reflectance, *atm_params,
                                                     constrain_atmospheric_params);
      }
    else
      {
      succeed = brad_estimate_atmospheric_parameters_multi(*radiance_img, *mdata, mean_reflectance, *atm_params,
                                                           constrain_atmospheric_params);
      }
    }
  else
