...
bool sdet_add_to_filter_bank_process(bprb_func_process& pro)
{
  ...
  if( vil_image_view<vxl_byte>* img_ptr = dynamic_cast<vil_image_view<vxl_byte> *>(img_sptr.ptr() ) )
    ...
    {
    vcl_cout << "Error: " << pro.name() << " -- The image pixel format: " << img_sptr->pixel_format()
             << " is not supported!\n";
    return false;
    }
...
bool sdet_add_responses_to_filter_bank_process(bprb_func_process& pro)
{
  ...
  if( img_sptr->pixel_format() != VIL_PIXEL_FORMAT_FLOAT )
    ...
    vcl_cerr
      <<
    " In sdet_add_responses_to_filter_bank_process(): the input image format is not FLOAT!.. it is assumed the input image is properly scaled to [0,1]!\n";
    return false;
    }
