...
bool sdet_texture_classifier_roc_process(bprb_func_process& pro)
{
  ...
  double thresholds[9] =
   {0.01, 0.001, 0.0001, 0.00001, 0.000001, 0.0000001, 0.00000001, 0.000000001, 0.0000000001};
...
bool sdet_texture_classifier_roc_process2(bprb_func_process& pro)
{
  ...
  if( !in_img )
    {
    vcl_cerr << pro.name() << ": unsupported image pixel format: " << in_img_sptr->pixel_format()
             << ", only float allowed!\n";
    return false;
    }
  if( in_img->nplanes() != 1 )
  ...
  vcl_cout << '\n';
  vcl_cout << positives.size() << " positive regions are loaded and " << pos_pixels.size()
           << " pixels are positive and "
