...
bool sdet_texture_classify_satellite_clouds_process(bprb_func_process& pro)
{
  ...
  if( cat_id_map.find(first_category) == cat_id_map.end() )
    {
    vcl_cout << pro.name() << ": can not find the input first category " << first_category
             << " among all categories!" << vcl_endl;
    return false;
    }
  ...
  if( !roi )
    {
    vcl_cerr << "cannot crop from image with size: " << image->ni() << " " << image->nj() << " at position (" << i
             << ", " << j << ") of size (" << width << ", " << height << ") with margin: " << invalid << vcl_endl;
    return false;
    }
...
bool sdet_texture_classify_satellite_clouds_process2(bprb_func_process& pro)
{
  ...
  if( cat_id_map.find(first_category) == cat_id_map.end() )
    {
    vcl_cerr << pro.name() << ": can not find the input first category " << first_category
  ...
  if( (i + width) >= image->ni() || (j + height) >= image->nj() )
    ...
    vcl_cerr << pro.name() << ": can not crop from image with size " << image->ni() << " " << image->nj()
             << " at position (" << i << ", " << j << ") of size (" << width << ", " << height << ") with margin "
             << invalid_margin << vcl_endl;
    return false;
    }
