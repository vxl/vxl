...
bool vil_EO_IR_combine_process(bprb_func_process& pro)
{
  ...
  for( ; eo_iter != eo_rgba_view->end(); ++eo_iter, ++ir_iter, ++out_iter )
    {
    (*out_iter) = vil_rgb<vxl_byte>(
        (unsigned char)(0.75 * (float)eo_iter->R() + 0.25 * (float)(*ir_iter) ), eo_iter->G(),
        (unsigned char)vcl_fabs(0.75 * (float)eo_iter->G() - 0.25 * (float)(*ir_iter) ) );
    }
