...
//:
// \file
// \brief Dialate a binary image using a disk structural element
...
bool vil_image_dilate_disk_process(bprb_func_process& pro)
{
    ...
    if( pro.n_inputs() < n_inputs_)
    {
        vcl_cerr << "In vil_image_dilate_disk_process(.): "
                 << "Must provide at least 1 input" << vcl_endl;
        return false;
    }
    ...
    {
        vcl_cerr << "In vil_image_dilate_disk_process(.)" << vcl_endl
                 << "\tMust provide boolean image." << vcl_endl;
        return false;
    }
    ...
    {
        vcl_cerr << "In vil_image_dilate_disk_process(.)" << vcl_endl
                 << "\tInput image must be a single plane." << vcl_endl;
        return false;
    }
