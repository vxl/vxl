...
bool vil_nitf_remove_margin_process(bprb_func_process& pro)
{
  ...
  for( unsigned sub_idx_i = 1; (sub_idx_i <= 10 && !found);  sub_idx_i++ )
    ...
    for( unsigned sub_idx_j = 1; (sub_idx_j <= 10 && !found);  sub_idx_j++ )
      ...
      if( !roi )
        {
        vcl_cerr << pro.name() << " can not crop from image with size " << ni << 'x' << nj << " at position ("
                 << sub_i0 << ',' << sub_j0
                 << ") of size (" << sni << ',' << snj << ")!\n";
        return false;
        }
        ...
        for( int i = (int)(img.ni() - 1); (i >= 0 && !found); i-- )
          ...
          for( int j = (int)(img.nj() - 1); (j >= 0 && !found); j-- )
            ...
            if( img(i, j, p) != 0 )
              ...
              vcl_cout << "image has valid pixel: sub_i0: " << sub_i0 << ", sub_j0: " << sub_j0 << ", sni: " << sni
                       << ", snj: " << snj << vcl_endl;
              vcl_cout << " valid pixel is caught at pixel (" << i << ',' << j << ',' << p << ")" << vcl_endl;
              vcl_string out_folder =
                "d:/work/Dropbox/pyscripts/expt_scripts/src/11jul10003056-m1bs-500081228070_01_p001/";
