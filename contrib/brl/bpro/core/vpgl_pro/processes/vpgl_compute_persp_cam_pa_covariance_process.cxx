...
//            vpgl_perturb_persp_cam_orient_process
//      
...
{
  if (pro.n_inputs()< 3) {
    vcl_cout << "vpgl_compute_persp_cam_pa_covariance_process: The input number should be 3" << vcl_endl;
    return false;
  }
  ...
  if (!ofs.is_open()) {
    vcl_cerr << "Failed to open file " << filename << vcl_endl;
    return false;
  }
  ofs << cov;
  ofs.close();
