...
bool vpgl_convert_local_rational_to_generic_process(bprb_func_process& pro)
{
  if (pro.n_inputs()!= pro.input_types().size()) {
    vcl_cout << "vpgl_convert_local_rational_to_generic_process: The input number should be " << pro.input_types().size() << vcl_endl;
    return false;
  }
  // get the inputs
  ...
  if (!lrat_cam) {
    vcl_cerr << "Error: camera is not a vpgl_local_rational_camera" << vcl_endl;
    return false;
  }
