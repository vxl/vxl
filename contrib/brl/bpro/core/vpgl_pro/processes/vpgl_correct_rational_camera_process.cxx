...
bool vpgl_correct_rational_camera_process(bprb_func_process& pro)
{
  if( pro.n_inputs() != 3 )
    {
    vcl_cout << "vpgl_correct_rational_camera_process: The number of inputs should be 3, not " << pro.n_inputs()
             << vcl_endl;
    return false;
    }
  ...
  if( !cam_local_rat )
    ...
    if( !cam_rational )
      ...
      {
      vcl_cout << "In vpgl_correct_rational_camera_process() - correcting rational camera.. with off_u: "
               << gt_offset_u << " off_v: " << gt_offset_v << "\n";
...
bool vpgl_get_rational_camera_offsets_process(bprb_func_process& pro)
{
  if( pro.n_inputs() != 1 )
    {
    vcl_cout << "vpgl_get_rational_camera_offsets_process: The number of inputs should be 3, not " << pro.n_inputs()
             << vcl_endl;
    return false;
    }
