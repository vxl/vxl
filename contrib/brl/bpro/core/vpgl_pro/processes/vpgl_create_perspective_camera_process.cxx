...
bool vpgl_create_perspective_camera_process4(bprb_func_process& pro)
{
  ...
  vcl_cout << "theta_c: " << theta_c * rad_to_deg << " theta_n: " << theta_n * rad_to_deg << " theta dif: "
           << (theta_c - theta_n) * rad_to_deg << vcl_endl;
  double theta = (theta_n - theta_c) / 2.0;
  vcl_cout << "theta: " << theta << " which is: " << theta * rad_to_deg << " in degrees, using 5 degree which is: "
           << 5 * vnl_math::pi / 180.0 << " in radians!\n";
...
bool vpgl_create_perspective_camera_from_kml_process(bprb_func_process& pro)
{
  ...
  vpgl_perspective_camera<double> out_cam = bpgl_camera_utils::camera_from_kml( (double)ni, (double)nj, right_fov,
                                                                                top_fov, 1.6, heading, tilt, roll);
