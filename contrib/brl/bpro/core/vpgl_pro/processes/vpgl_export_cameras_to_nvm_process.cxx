...
bool vpgl_export_cameras_to_nvm_process(bprb_func_process& pro)
{
  if( pro.n_inputs() != pro.input_types().size() )
    {
    vcl_cout << "vpgl_export_cameras_to_nvm_process: The number of inputs should be " << pro.input_types().size()
             << vcl_endl;
    return false;
    }
  int        i = 0;
  ...
  for( unsigned k = 0; k < cams.size(); k++ )
    ...
    ofile << img_iter() << " " << f << " " << q.r() << " " << q.x() << " " << q.y() << " " << q.z() << " " << cc.x()
          << " " << cc.y() << " " << cc.z() << " " << 0 << " " << 0 << vcl_endl;
