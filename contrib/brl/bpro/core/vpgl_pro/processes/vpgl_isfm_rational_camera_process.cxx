...
bool vpgl_isfm_rational_camera_process(bprb_func_process& pro)
{
  ...
  while( iter() )
    {
    vpgl_local_rational_camera<double> * ratcam =
      read_local_rational_camera<double>(seedcamdir + "/" + iter.filename() );
    if( !ratcam )
      {
  ...
  while( iter2() )
    {
    vpgl_local_rational_camera<double> * ratcam = read_local_rational_camera<double>(
        nonseedcamdir + "/" + iter2.filename() );
    if( !ratcam )
      {
