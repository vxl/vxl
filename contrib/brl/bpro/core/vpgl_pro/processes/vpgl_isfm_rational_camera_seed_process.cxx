...
bool obtain_relative_diameter(double const& ll_lon, double const& ll_lat, double const& ur_lon, double const& ur_lat,
                              double& relative_diameter);
...
bool initial_point_by_overlap_region(double const& ll_lon, double const& ll_lat, double const& ur_lon,
                                     double const& ur_lat, vcl_vector<vcl_pair<vil_image_view_base_sptr,
                                                                               vpgl_geo_camera *> >& dem_infos,
                                     vgl_point_3d<double>& init_pt, double& zmin, double& zmax,
                                     double const& height_diff = 20.0);
...
bool vpgl_isfm_rational_camera_seed_process(bprb_func_process& pro)
{
  ...
  if( !n )
    {
    vcl_cerr << pro.name() << ": 0 correspondences in file: " << input_txt
             << "! returning without correcting any camera!\n";
    return false;
    }
  ...
  while( !ifs.eof() )
    ...
    vcl_string out_cam_file = out_folder + vul_file::strip_extension(vul_file::basename(cam_file) )
      + "_local_corrected.rpb";
  ...
  if( enforce_existing && cnt_exist < 2 )
    {
    vcl_cerr << pro.name()
             << ": enforcing condition to have at least 2 pre-existing corrected cameras! EXITING since there is: "
             << cnt_exist << " cameras.\n";
    return false;
    }
  ...
  double               zmin, zmax;
  if( !initial_point_by_overlap_region(lower_left_lon, lower_left_lat, upper_right_lon, upper_right_lat, dem_infos,
                                       initial_pt, zmin, zmax, height_diff) )
    {
  ...
  vcl_cout << "height value (using " << height_diff << " meter height difference): [" << zmin << ',' << zmax << ']'
           << vcl_endl;
  ...
  for( unsigned i = 0; i < n; i++ )
    ...
    if( !vpgl_rational_adjust_onept::adjust_with_weights(cams, cam_weights, corrs_i, initial_pt, zmin, zmax,
                                                         cam_trans_i, intersection, relative_diameter) )
      {
      continue;
#if 1
  ...
  vcl_cout << "out of " << n << " correspondences " << cam_trans.size()
           << " of them back-projected to 3-d world point successfully:";
  ...
  if( !cam_trans.size() )
    {
    vcl_cout << "out of " << n << " correspondences " << cam_trans.size()
             << " of them yielded corrections! exit without any correction!\n";
    return false;
    }
  ...
  vcl_cout << "out of " << cam_trans.size() << " valid correspondences, " << max
           << " of them yield constant translations using " << pixel_radius << " pixel radius" << vcl_endl;
  ...
  if( inlier_ratio < 0.1 )
    ...
    vcl_cout << pro.name()
             <<
      ": less than 10% of correspondence yield constant translations due to bad correspondence, correction failed"
             << vcl_endl;
    return false;
    }
  ...
  for( unsigned k = 0; k < cams.size(); k++ )
    {
    vcl_cout << "camera " << k << " --> offset_u: " << cam_trans[max_i][k].x() << " offset_v: "
             << cam_trans[max_i][k].y() << vcl_endl;
    }
  for( unsigned k = 0; k < cams.size(); k++ )
    {
    double u_off, v_off;
  ...
  if( !vpgl_rational_adjust_multiple_pts::adjust_lev_marq(cams, cam_weights, corrs_inliers, initial_pt, zmin, zmax,
                                                          cam_trans_inliers, intersections, relative_diameter) )
    {
...
#if 0
bool vpgl_isfm_rational_camera_seed_process_globals::initial_point_by_overlap_region(
  vcl_vector<vcl_string> const& sat_res,
  vcl_vector<vcl_pair<vil_image_view_base_sptr, vpgl_geo_camera *> >& dem_infos,
  vgl_point_3d<double>& init_pt,
  double& zmin, double& zmax,
  double const& height_diff)
{
  ...
  if( !vpgl_isfm_rational_camera_seed_process_globals::find_min_max_height(overlap_region.min_x(),
                                                                           overlap_region.min_y(),
                                                                           overlap_region.max_x(),
                                                                           overlap_region.max_y(),
...
bool vpgl_isfm_rational_camera_seed_process_globals::initial_point_by_overlap_region(double const& ll_lon,
                                                                                     double const& ll_lat,
                                                                                     double const& ur_lon,
                                                                                     double const& ur_lat,
                                                                                     vcl_vector<vcl_pair<vil_image_view_base_sptr, vpgl_geo_camera *> >& dem_infos,
...
bool vpgl_isfm_rational_camera_seed_process_globals::obtain_relative_diameter(double const& ll_lon,
                                                                              double const& ll_lat,
                                                                              double const& ur_lon,
                                                                              double const& ur_lat,
                                                                              double& relative_diameter)
...
vgl_box_2d<double> vpgl_isfm_rational_camera_seed_process_globals::intersection(
  vcl_vector<vgl_box_2d<double> > const& boxes)
{
  if( boxes.size() == 2 )
    {
...
bool vpgl_isfm_rational_camera_seed_process_globals::find_min_max_height(double const& ll_lon, double const& ll_lat,
                                                                         ...
                                                                         double& min, double& max)
{
  ...
  if( corners[0].first == corners[1].first )
    ...
    vpgl_isfm_rational_camera_seed_process_globals::crop_and_find_min_max(infos, corners[0].first, i0, j0, crop_ni,
                                                                          crop_nj, min, max);
    return true;
    }
  ...
  if( corners[0].first == corners[2].first && corners[1].first == corners[3].first )
    ...
    vpgl_isfm_rational_camera_seed_process_globals::crop_and_find_min_max(infos, corners[1].first, i0, j0, crop_ni,
                                                                          crop_nj, min, max);
    return true;
    }
  ...
  crop_nj = corners[2].second.second + 1;
  vpgl_isfm_rational_camera_seed_process_globals::crop_and_find_min_max(infos, corners[2].first, i0, j0, crop_ni,
                                                                        crop_nj, min, max);
  ...
  vpgl_isfm_rational_camera_seed_process_globals::crop_and_find_min_max(infos, corners[3].first, i0, j0, crop_ni,
                                                                        crop_nj, min, max);
  return true;
}

void vpgl_isfm_rational_camera_seed_process_globals::crop_and_find_min_max(vcl_vector<vcl_pair<vil_image_view_base_sptr,
                                                                                               vpgl_geo_camera *> >& infos,
