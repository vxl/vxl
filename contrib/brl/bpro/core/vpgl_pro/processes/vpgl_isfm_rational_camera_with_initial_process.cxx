...
bool vpgl_isfm_rational_camera_with_initial_process(bprb_func_process& pro)
{
  ...
  for( unsigned i = 0; i < n_seeds; i++ )
    ...
    if( !ratcam )
      {
      vcl_cerr << pro.name() << ": failed to load local seed rational camera " << i << " from file: "
               << seed_cam_files[i] << "!!\n";
      return false;
      }
  ...
  for( unsigned i = 0; i < n_track; i++ )
    ...
    if( !vpgl_rational_adjust_onept::adjust_with_weights(cams, weights, corrs, initial_pt, zmin, zmax, cam_trans_i,
                                                         intersection, relative_diameter) )
      {
      continue;
#if 1
  ...
  for( unsigned k = 0; k < cam_trans.size(); k++ )
    ...
    vcl_cout << "image point: [" << img_points[k].x() << ',' << img_points[k].y()
             ...
             << "], world point: [" << vcl_setprecision(10) << reconstructed_pts[k].x() << ','
             << vcl_setprecision(10) << reconstructed_pts[k].y() << ','
...
bool vpgl_isfm_rational_camera_with_initial_process_globals::find_min_max_height(double const& ll_lon,
                                                                                 double const& ll_lat,
                                                                                 double const& ur_lon,
                                                                                 double const& ur_lat,
                                                                                 ...
                                                                                 double& min, double& max)
{
  ...
  if( corners[0].first == corners[1].first )
    ...
    vpgl_isfm_rational_camera_with_initial_process_globals::crop_and_find_min_max(infos, corners[0].first, i0, j0,
                                                                                  crop_ni, crop_nj, min, max);
    return true;
    }
  ...
  if( corners[0].first == corners[2].first && corners[1].first == corners[3].first )
    ...
    vpgl_isfm_rational_camera_with_initial_process_globals::crop_and_find_min_max(infos, corners[1].first, i0, j0,
                                                                                  crop_ni, crop_nj, min, max);
    return true;
    }
  ...
  crop_nj = corners[2].second.second + 1;
  vpgl_isfm_rational_camera_with_initial_process_globals::crop_and_find_min_max(infos, corners[2].first, i0, j0,
                                                                                crop_ni, crop_nj, min, max);
  ...
  vpgl_isfm_rational_camera_with_initial_process_globals::crop_and_find_min_max(infos, corners[3].first, i0, j0,
                                                                                crop_ni, crop_nj, min, max);
  return true;
}

void vpgl_isfm_rational_camera_with_initial_process_globals::crop_and_find_min_max(
  vcl_vector<vcl_pair<vil_image_view_base_sptr, vpgl_geo_camera *> >& infos,
  unsigned const& img_id,
  int const& i0, int const& j0, int const& crop_ni, int const& crop_nj,
  double& min, double& max)
{
...
bool vpgl_isfm_rational_camera_with_initial_process_globals::initial_point_by_overlap_region(double const& ll_lon,
                                                                                             double const& ll_lat,
                                                                                             double const& ur_lon,
                                                                                             double const& ur_lat,
                                                                                             vcl_vector<vcl_pair<vil_image_view_base_sptr, vpgl_geo_camera *> >& dem_infos,
                                                                                             ...
                                                                                             double const& height_diff)
{
  ...
  if( !vpgl_isfm_rational_camera_with_initial_process_globals::find_min_max_height(overlap_region.min_x(),
...
bool vpgl_isfm_rational_camera_with_initial_process_globals::obtain_relative_diameter(double const& ll_lon,
                                                                                      double const& ll_lat,
                                                                                      double const& ur_lon,
                                                                                      double const& ur_lat,
                                                                                      double& relative_diameter)
...
vgl_box_2d<double> vpgl_isfm_rational_camera_with_initial_process_globals::intersection(
