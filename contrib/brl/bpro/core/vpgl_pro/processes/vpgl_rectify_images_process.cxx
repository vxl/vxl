...
void out_image_size(unsigned ni, unsigned nj, vnl_matrix_fixed<double, 3, 3>& H1, double& min_i, double& min_j,
...
void warp_bilinear(vil_image_view<float>& img, vnl_matrix_fixed<double, 3, 3>& H, vil_image_view<float>& out_img,
                   double mini, double minj)
{
...
bool vpgl_affine_rectify_images_process(bprb_func_process& pro)
{
  ...
  vcl_cout << " using z = " << z
           <<
  " as local ground plane height and will sample points on this plane randomly! the mid point z height would be: "
           << 0.5 * height + min_z << '\n';
...
bool vpgl_construct_height_map_process(bprb_func_process& pro)
{
  ...
  double                height = max_z - min_z;
  vil_image_view<float> out_map(width, depth), out_map_img1(width, depth), out_map_disp1(width, depth), out_map_img2(
    width, depth);
  out_map.fill(min_z);
