...
public:
  // : Constructor
  bapl_bbf_box();
  bapl_bbf_box( const vnl_vector_fixed<double, 128>& min_point, const vnl_vector_fixed<double, 128>& max_point );
...
double bapl_bbf_dist_sq( const bapl_keypoint_sptr p, const bapl_bbf_box& b );
...
public:
  // : Constructor
  bapl_bbf_tree( const vcl_vector<bapl_keypoint_sptr>& points, int points_per_leaf = 4 );

  // : Return an estimate of the n closest points to the query point
  void n_nearest( const bapl_keypoint_sptr query_point, vcl_vector<bapl_keypoint_sptr>& closest_points,
                  vcl_vector<int>& closest_indices, int n = 1, int max_search_nodes = -1 );

  // : Return an estimate of the n closest points to the query point
  // \param n is the number of nearest nodes to return
  // \param max_search_nodes is the number of nodes to examine (-1 mean all)
  void n_nearest( const bapl_keypoint_sptr query_point, vcl_vector<bapl_keypoint_sptr>& closest_points, int n = 1,
                  int max_search_nodes = -1);

private:
  // : Build the tree
  bapl_bbf_node_sptr build_tree( int points_per_leaf, const bapl_bbf_box& outer_box, int depth,
  ...
  // : Update
  void update_closest( const bapl_keypoint_sptr query_point, int n, bapl_bbf_node_sptr p,
                       vcl_vector<int>& closest_indices, vcl_vector<double>& sq_distances, int & num_found );

  // : See if the current leaf contains the NN neighbors
  bool bounded_at_leaf( const bapl_keypoint_sptr query_point, int n, bapl_bbf_node_sptr current,
