...
bool bapl_keypoint_extractor( const vil_image_resource_sptr & image, vcl_vector<bapl_keypoint_sptr> & keypoints,
                              float curve_ratio = 10.0f, bool verbose = true);

// : Find the peaks in the DoG pyramid
void bapl_dog_peaks( vcl_vector<vgl_point_3d<float> >& peak_pts, bapl_lowe_pyramid_set_sptr pyramid_set,
...
public:
  ...
  void orient_at( float x, float y, float scale, const vil_image_view<float> & grad_orient,
                  const vil_image_view<float> & grad_mag, vcl_vector<float> & orientations  );
