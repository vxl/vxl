...
{
  os << "Lowe keypoint: (" << location_i_ << ',' << location_j_ << ") scale=" << scale_ << " orientation="
     << orientation_ << vcl_endl;
...
bapl_lowe_keypoint_sptr read_from_file(vcl_istream& ifs, int len, int img_width, int img_height)
{
  ...
  if( len != 128 )    // not possible to use this method for any other sized descriptor
    ...
    vcl_cerr
      <<
    "WARNING descriptor size is not 128, this method will work if len < 128 and fill the rest with zeros, otherwise it will break!\n";
    // return 0;
    }
