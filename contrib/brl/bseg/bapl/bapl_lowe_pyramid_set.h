...
public:
  ...
  bapl_lowe_pyramid_set( const vil_image_resource_sptr& image, unsigned octave_size = 3, unsigned num_octaves = 0,
                         bool verbose = true);

  // : Accessor for the Gaussian pyramid
  const vil_image_view<float> & gauss_at( float scale, float * actual_scale = 0, float * rel_scale = 0 ) const;

  // : Accessor for the Difference of Gaussians pyramid
  const vil_image_view<float> & dog_at( float scale, float * actual_scale = 0, float * rel_scale = 0 ) const;

  // : Accessor for the Gradient orientation pyramid
  const vil_image_view<float> & grad_orient_at( float scale, float * actual_scale = 0, float * rel_scale = 0 ) const;

  // : Accessor for the Gradient magnitude pyramid
  const vil_image_view<float> & grad_mag_at( float scale, float * actual_scale = 0, float * rel_scale = 0 ) const;
...
protected:
  ...
  const vil_image_view<float> & pyramid_at( const bapl_lowe_pyramid<float> & pyramid, float scale,
                                            float * actual_scale = 0, float * rel_scale = 0 ) const;
