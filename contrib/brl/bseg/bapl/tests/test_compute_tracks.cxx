...
static void test_compute_tracks(int argc, char* argv[])
{
  ...
  while( !ifsi.eof() )
    ...
    for( int i = 0; i < cnt; i++ )
      ...
      if( kid1 >= (int)img_keypoints[id1]->keys_.size() || kid2 >= (int)img_keypoints[id2]->keys_.size() )
        {
        vcl_cout << "In bapl_load_match_process() -- Mismatch in keypoint vector size for: " << id1 << " or for: "
                 << id2 << " and the match file!\n";
        return;
        }
  ...
  for( unsigned i = 0; i < paths.size(); i++ )
    ...
    while( !line.eof() )
      ...
      bapl_key_match km; km.first = img_keypoints[left_id]->keys_[kid1];
      km.second = img_keypoints[right_id]->keys_[kid2];
