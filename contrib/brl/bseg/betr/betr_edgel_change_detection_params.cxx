...
std::ostream&  operator<<(std::ostream& s, betr_edgel_change_detection_params const& ecdp){
  betr_params_sptr params_ptr = new betr_edgel_change_detection_params(ecdp);
...
std::istream&  operator>>(std::istream& s, betr_edgel_change_detection_params& ecdp){
  ...
  if(good)
    ...
    std::cout << "Failed to parse JSON file for betr_edgel_change_detection_params" << std::endl;
  return s;
}
