...
public:
  ...
  static bool next_trace_point(unsigned int& i, unsigned int& j, int& dir, const unsigned int * & p, unsigned int value,
                               unsigned int ni, unsigned int nj, vcl_ptrdiff_t istep, vcl_ptrdiff_t jstep);

  // : trace a single boundary starting and location (i,j)
  static bool trace_boundary(vcl_vector<vgl_point_2d<double> >& pts, unsigned int value,
                             const vil_image_view<unsigned int>& labels, vil_image_view<bool>& visited, unsigned int i,
                             unsigned int j);
  ...
  static vcl_vector<vgl_polygon<double> > trace_boundaries(const vil_image_view<unsigned int>& labels,
  ...
  static unsigned int link_boundary_edges(const vil_image_view<unsigned int>& labels,
                                          ...
                                          vcl_vector<bmdl_edge>& edges, vcl_vector<bmdl_region>& regions);
  ...
  static void roof_subtract_hole(const imesh_vertex_array<3>& verts, vcl_vector<unsigned int>& face,
  ...
  // i.e. (x,y) -> (x,-y)
  static void mesh_lidar(const vcl_vector<vgl_polygon<double> >& boundaries, const vil_image_view<unsigned int>& labels,
                         const vil_image_view<double>& heights, const vil_image_view<double>& ground, imesh_mesh& mesh);

  // : construct a mesh out of data and labels using linked edges
  // The coordinate system is flipped over the x-axis to make it right handed
  // i.e. (x,y) -> (x,-y)
  static void mesh_lidar(const vcl_vector<bmdl_edge>& edges, const vcl_vector<bmdl_region>& regions,
                         unsigned int num_joints, const vil_image_view<unsigned int>& labels,
                         const vil_image_view<double>& heights, const vil_image_view<double>& ground, imesh_mesh& mesh);
