...
bmdl_modeling_process::bmdl_modeling_process()
{
  ...
  if (!parameters()->add( "Vegetation Threshold" , PARAM_VEG_THRESH , 1.0f ))
    vcl_cerr << "ERROR: Adding parameters in bmdl_classify_process\n";
...
bool bmdl_modeling_process::execute()
{
  ...
  if (!modeling(first_ret, last_ret, ground_img, output_path,
    x_dim, y_dim, x_over, y_over, gthresh, vthresh, athresh, hres)) {
                                     ...
                                     float gthresh, float vthresh, float athresh, float hres)
{
  ...
  while (j_orig < lidar_first->nj()) {
    ...
    while (i_orig < lidar_first->ni()) {
      ...
      classify_process.classify(first_roi, last_roi, ground_roi, label_img, height_img, gthresh, vthresh, athresh, hres);
