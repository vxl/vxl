...
public:
  ...
  boct_tree<T_loc, T_data> * clone_and_intersect(boct_tree_cell<T_loc, T_data>* subtree_root,
                                                 short parent_tree_root_level, vgl_box_3d<double> local_crop_box);
  ...
  boct_tree<T_loc, vnl_vector_fixed<T_data, DIM> > * clone_to_vector()
  {
    ...
    boct_tree_cell<T_loc, vnl_vector_fixed<T_data, DIM> >* cloned_root = temp_tree.construct_tree(cloned_cells,
                                                                                                  this->number_levels() );
    boct_tree<T_loc, vnl_vector_fixed<T_data, DIM> >* tree = new boct_tree<T_loc, vnl_vector_fixed<T_data, DIM> >(
        cloned_root,  this->number_levels() );
    tree->set_bbox(this->bounding_box() );
    return tree;
  }
  ...
  boct_tree_cell<T_loc, T_data> * locate_point_global(const vgl_point_3d<double>& p, boct_tree_cell<T_loc,
                                                                                                    T_data>* current_cell);
  ...
  void locate_region_cells_global(const vgl_box_3d<double>& r, vcl_vector<boct_tree_cell<T_loc, T_data> *> & leaves,
                                  short level);

  // : Returns all leaf cells entirely contained in 3d region in global coordinates
  //  Code assumes boxes have the same centroid
  void locate_leaves_in_hollow_region_global(const vgl_box_3d<double>& outer_r, const vgl_box_3d<double>& inner_r,
  ...
  void change_leaves_in_global_region_leaves_global(const vgl_box_3d<double>& r, const T_data & val)
  {
    ...
    double                          radius = vcl_sqrt(
        r.width() * r.width() + r.depth() * r.depth() + r.height() * r.height() ) + 1e-7;
  ...
  {
    return vgl_point_3d<double>(cell->code_.x_loc_ / max_val_, cell->code_.y_loc_ / max_val_,
                                cell->code_.z_loc_ / max_val_);
  }
  ...
  {
    return vgl_point_3d<double>(global_bbox_.min_x() + ( (cell->code_.x_loc_ / max_val_) + cell_length(
                                                           cell) / 2.0) * global_bbox_.width(),
                                global_bbox_.min_y() + ( (cell->code_.y_loc_ / max_val_) + cell_length(
                                                           cell) / 2.0) * global_bbox_.height(),
                                global_bbox_.min_z() + ( (cell->code_.z_loc_ / max_val_) + cell_length(
                                                           cell) / 2.0) * global_bbox_.depth() );
  }
...
public:  // should be made private
  ...
  boct_tree_cell<T_loc, T_data> * construct_tree(vcl_vector<boct_tree_cell<T_loc, T_data> >& leaf_nodes,
                                                 short num_levels);
...
// : Binary write
template <class T_loc, class T_data>
void vsl_b_write(vsl_b_ostream & os, const bool save_internal_nodes, boct_tree<T_loc, T_data>& tree)
{
  tree.b_write(os,
               save_internal_nodes);
}
