...
boct_tree<T_loc, T_data>::boct_tree(short num_levels, short init_levels) : num_levels_(num_levels), root_level_(
    num_levels - 1), max_val_( (double)(1 << root_level_) ), root_(0)
{
...
template <class T_loc, class T_data>
boct_tree<T_loc, T_data>::boct_tree(T_data data, short num_levels, short init_levels) : num_levels_(num_levels),
  root_level_(num_levels - 1), max_val_( (double)(1 << root_level_) ), root_(0)
{
  // root is allocated at (max_level_-1) with code [0,0,0]
  boct_loc_code<T_loc> code;
  if( init_levels > 0 )
...
template <class T_loc, class T_data>
boct_tree_cell<T_loc, T_data> * boct_tree<T_loc, T_data>::construct_tree(vcl_vector<boct_tree_cell<T_loc,
                                                                                                   T_data> >& leaf_nodes,
                                                                         short num_levels)
{
...
template <class T_loc, class T_data>
boct_tree<T_loc, T_data> * boct_tree<T_loc, T_data>::clone_subtree(boct_tree_cell<T_loc, T_data>* subtree_root,
                                                                   short parent_tree_root_level)
{
  ...
  if( shift_level > 0 )
    {
    shift_code.set_code( ( (1 << (parent_tree_root_level - subtree_root->level() ) ) - 1),
                         ( (1 << (parent_tree_root_level - subtree_root->level() ) ) - 1),
                         ( (1 << (parent_tree_root_level - subtree_root->level() ) ) - 1) );
                                                                         ...
                                                                         vgl_box_3d<double> local_crop_box)
{
  ...
  if( shift_level > 0 )
    {
    shift_code.set_code( ( (1 << (subtree_root->level() ) ) - 1), ( (1 << (subtree_root->level() ) ) - 1),
                         ( (1 << (subtree_root->level() ) ) - 1) );
    boct_tree_cell<T_loc, T_data>* root = subtree_root->clone_and_intersect(0, &shift_code, local_crop_box,
                                                                            parent_tree_root_level);
    boct_tree<T_loc, T_data>* tree = new boct_tree<T_loc, T_data>(root, subtree_root->level() + 1 );
    return tree;
    }
  else if( shift_level == 0 )
...
template <class T_loc, class T_data>
boct_tree_cell<T_loc, T_data> * boct_tree<T_loc, T_data>::locate_point_global(const vgl_point_3d<double>& p,
                                                                              boct_tree_cell<T_loc, T_data>* curr_cell)
{
...
template <class T_loc, class T_data>
boct_tree_cell<T_loc, T_data> * boct_tree<T_loc, T_data>::locate_point_at_level(const vgl_point_3d<double>& p,
                                                                                short level, bool check_out_of_bounds)
{
  short curr_level = root_level_;
                                                                        ...
                                                                        bool check_out_of_bounds)
{
  ...
  while( !(xorcode->x_loc_ & (1 << level_x) ) && level_x )
    {
    level_x--;
    }

  while( !(xorcode->y_loc_ & (1 << level_y) ) && level_y > level_x )
    {
    level_y--;
    }

  while( !(xorcode->z_loc_ & (1 << level_z) ) && level_z > level_y )
    {
    level_z--;
    }
...
template <class T_loc, class T_data>
void boct_tree<T_loc, T_data>::locate_region_leaves_global(const vgl_box_3d<double>& r, vcl_vector<boct_tree_cell<T_loc,
                                                                                                                  T_data> *> & leaves)
{
...
template <class T_loc, class T_data>
void boct_tree<T_loc, T_data>::locate_region_cells_global(const vgl_box_3d<double>& r, vcl_vector<boct_tree_cell<T_loc,
                                                                                                                 T_data> *> & leaves,
                                                          short level)
{
  boct_tree_cell<T_loc, T_data>*              root = locate_region_global(r);
...
template <class T_loc, class T_data>
void boct_tree<T_loc, T_data>::locate_leaves_in_hollow_region_global(const vgl_box_3d<double>& outer_r,
                                                                     const vgl_box_3d<double>& inner_r,
                                                                     vcl_vector<boct_tree_cell<T_loc,
                                                                                               T_data> *> & leaves)
{
  ...
  for( ; it != all_leaves.end(); ++it )
    {
    if( (!vgl_intersection(cell_bounding_box(*it),
                           outer_r).is_empty() ) && (vgl_intersection(cell_bounding_box(*it), inner_r).is_empty() ) )
      {
      leaves.push_back(*it);
      }
    }
...
vgl_box_3d<double> boct_tree<T_loc, T_data>::cell_bounding_box_canonical(boct_tree_cell<T_loc, T_data> * const cell)
{
  ...
  vgl_point_3d<double> local_origin(cell->code_.x_loc_ * onebymaxval, cell->code_.y_loc_ * onebymaxval,
                                    cell->code_.z_loc_ * onebymaxval);
