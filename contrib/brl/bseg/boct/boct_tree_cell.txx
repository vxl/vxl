...
long unsigned boct_tree_cell<T_loc, T_data>::n_created_ = 0;
template <class T_loc, class T_data>
long unsigned boct_tree_cell<T_loc, T_data>::n_destroyed_ = 0;
#endif
...
vgl_box_3d<double> boct_tree_cell<T_loc, T_data>::local_bounding_box(short root_level)
{
  ...
  vgl_point_3d<double> cell_max_corner(cell_origin_x + cell_length, cell_origin_y + cell_length,
                                       cell_origin_z + cell_length);
...
template <class T_loc, class T_data>
boct_tree_cell<T_loc, T_data> * boct_tree_cell<T_loc, T_data>::clone_and_intersect(boct_tree_cell<T_loc,
                                                                                                  T_data>* parent,
                                                                                   ...
                                                                                   short root_level)
{
  ...
  if( !inters.is_empty() )
    ...
    if( this->is_leaf() )
      ...
      for( unsigned i = 0; i < 8; i++ )
        {
        boct_tree_cell<T_loc, T_data>* c = this->children_[i].clone_and_intersect(cell, shift_code, local_crop_box,
                                                                                  root_level);
        cell->children_[i] = *c;
        c->children_ = NULL;
        delete c;
...
template <class T_loc, class T_data>
boct_tree_cell<T_loc, T_data> * boct_tree_cell<T_loc, T_data>::traverse_to_level(boct_loc_code<T_loc> * code,
                                                                                 short level)
{
  if( level < 0 )
    {
...
bool boct_tree_cell<T_loc, T_data>::insert_subtree( boct_tree_cell<T_loc, T_data> *& subtree)
{
  ...
  if( !this->is_leaf() )
    {
    vcl_cout
    << "ERROR! boct_tree_cell<T_loc,T_data>::insert_subtree -- subtree cannot be inserted, the node is not leaf"
    << vcl_endl;
    return false;
    }
...
template <class T_loc, class T_data>
void boct_tree_cell<T_loc, T_data>::leaf_children_at_level(vcl_vector<boct_tree_cell<T_loc, T_data> *>& v,
                                                           short target_level)
{
...
template <class T_loc, class T_data>
void boct_tree_cell<T_loc, T_data>::children_at_level(vcl_vector<boct_tree_cell<T_loc, T_data> *>& v,
                                                      short target_level)
{
  short curr_level = code_.level;
