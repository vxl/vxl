...
MAIN( test_binary_io )
{
  ...
  boct_tree<vgl_point_3d<double> > *tree_in=new boct_tree<vgl_point_3d<double> >(6,3);
  tree_in->split();
  vcl_vector<boct_tree_cell<vgl_point_3d<double> >*> leaves = tree_in->leaf_cells();
  int num_leaves = leaves.size();
  for (unsigned i=0; i<leaves.size(); i++) {
    boct_tree_cell<vgl_point_3d<double> >* leaf = static_cast<boct_tree_cell<vgl_point_3d<double> >*>(leaves[i]);
  ...
  boct_tree<vgl_point_3d<double> > *tree_out=new boct_tree<vgl_point_3d<double> >();
