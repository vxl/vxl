                               ...
                               vcl_vector<boct_tree_cell<short, vgl_point_3d<double> > *> & neighbors)
{
  ...
  vgl_box_3d<double> cellbox(cell->code_.get_point(max_level), cellsize, cellsize, cellsize,
  ...
  for( unsigned i = 0; i < leaf_nodes.size(); i++ )
    ...
    vgl_box_3d<double> neighborcellbox(leaf_nodes[i]->code_.get_point(
                                         max_level), neighborcellsize, neighborcellsize, neighborcellsize,
                                       vgl_box_3d<double>::min_pos);
...
static void test_find_neighbors()
{
  ...
  gt_code.set_code( (cell->get_code().x_loc_ | 1 << (cell->level() ) ), cell->get_code().y_loc_,
  ...
  gt_code_x_low.set_code( (cell_xlow->get_code().x_loc_ - (1 << (cell_xlow->level() ) ) ),
                          cell->get_code().y_loc_, cell->get_code().z_loc_);
