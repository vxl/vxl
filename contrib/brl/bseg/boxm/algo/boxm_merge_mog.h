...
public:
  ...
  bool kl_merge_scene(boxm_scene<boct_tree<short, mix_gauss_type> > & app_scene,  boxm_scene<boct_tree<short,
                                                                                                       gauss_type> > & gauss_app);
  ...
  static bool mpm_merge_scene(boxm_scene < boct_tree<short, mix_gauss_type> app_scene,  boxm_scene < boct_tree<short,
                                                                                                               gauss_type> gauss_app);
#endif
...
bool compute_differential_entropy(boxm_scene<boct_tree<short, bsta_num_obs<bsta_gauss_sf1> > >& scene,
                                  boxm_scene<boct_tree<short, float> >& scene_out);

// : Computes expected color of a voxel as mean/occupancy
bool compute_expected_color(boxm_scene<boct_tree<short, boxm_sample<BOXM_APM_MOG_GREY> > >& scene,
                            boxm_scene<boct_tree<short, float> >& scene_out, float grey_offset);
