...
boxm_scene<boct_tree<short, bool> > * boxm_explore_cells_at_level(boxm_scene<boct_tree<short, T_data> > * scene_in,
                                                                  short level)
{
  // parameters and structure of the output scene are the same as those of the input scene
  boxm_scene<boct_tree<short, bool> > * scene_out = new boxm_scene<boct_tree<short, bool> >(
      scene_in->lvcs(), scene_in->origin(), scene_in->block_dim(), scene_in->world_dim(),
  ...
  for( it_in.begin(); !it_in.end(); ++it_in )
    ...
    for( ; cells_it != all_cells.end(); cells_it++ )
      ...
      if( level == 0 )
        ...
        shift_code.set_code(cell_in->get_code().x_loc_ >> 1, cell_in->get_code().y_loc_ >> 1,
                            cell_in->get_code().z_loc_ >> 1);
