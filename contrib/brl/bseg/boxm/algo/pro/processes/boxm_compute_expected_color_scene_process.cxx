...
bool boxm_compute_expected_color_scene_process(bprb_func_process& pro)
{
  ...
  boxm_scene<boct_tree<short, float> > * mean_color_scene = new boxm_scene<boct_tree<short, float> >(
      scene->lvcs(), scene->origin(), scene->block_dim(), scene->world_dim(), scene->max_level(), scene->init_level() );
