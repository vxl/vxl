...
public:
  // : "default" constructor
  boxm_change_probability_functor(vil_image_view<typename boxm_apm_traits<APM>::obs_datatype> const & image,
                                  vil_image_view<typename boxm_apm_traits<APM>::obs_datatype> & change_prob,
                         ...
                         bool /*use_black_background*/ = false)
{
  ...
  boxm_aux_scene<T_loc, T_data, boxm_rt_sample<sample_datatype> > aux_scene(&scene,
                                                                            boxm_aux_traits<BOXM_AUX_NULL>::storage_subdir(),
                                                                            boxm_aux_scene<T_loc, T_data,
                                                                                           boxm_rt_sample<sample_datatype> >::LOAD);
  typedef boxm_change_probability_functor<T_data::apm_type, boxm_rt_sample<sample_datatype> > chng_functor;
  boxm_raytrace_function<chng_functor, T_loc, T_data, boxm_rt_sample<sample_datatype> > raytracer(scene, aux_scene,
                                                                                                  cam.ptr(),
                                                                                                  change_prob.ni(),
                                                                                                  change_prob.nj() );
  chng_functor functor(image, change_prob,
                       mask,
                       change_prob.ni(), change_prob.nj(), true, false);
