...
boxm_edge_tangent_updater<T_loc, APM, AUX>::boxm_edge_tangent_updater(boxm_scene<boct_tree<T_loc,
  ...
  : image_ids_(image_ids), use_ransac_(use_ransac), ransac_ortho_thres_(ransac_ortho_thres), ransac_volume_ratio_(
    ransac_volume_ratio), ransac_consensus_cnt_(ransac_concensus_cnt), scene_(scene)
{}
...
bool boxm_edge_tangent_updater<T_loc, APM, AUX>::add_cells()
{
  ...
  while( !iter.end() )
    ...
    for( unsigned i = 0; i < cells.size(); ++i )
      ...
      // initialize the cell
      boxm_inf_line_sample<AUX> data(vgl_infinite_line_3d<AUX>(vgl_vector_2d<AUX>(0, 0),
                                                               vgl_vector_3d<AUX>(1, 1, 1) ) );
      cell->set_data(data);
      ...
      if( aux_samples.size() > 1 )
        ...
        if( planes.size() > 1 )
          ...
          if( use_ransac_ )
            ...
            if( boxm_plane_ransac<AUX>(aux_samples, weights, line, residual, bb, ransac_consensus_cnt_,
                                       ransac_ortho_thres_, ransac_volume_ratio_) )
              {
              ...
              {
              boxm_inf_line_sample<AUX> sample(vgl_infinite_line_3d<AUX>(vgl_vector_2d<AUX>(-10000,
                                                                                            -10000), vgl_vector_3d<AUX>(
                                                                           0, 0, 1) ) );
...
template <class T_loc, class APM, class AUX>
boxm_edge_tangent_refine_updates<T_loc, APM,
                                 AUX>::boxm_edge_tangent_refine_updates(boxm_scene<boct_tree<T_loc,
                                                                                             boxm_inf_line_sample<APM> > > & scene,
                                                                        int consensus_cnt,
    ...
    for( unsigned i = 0; i < cells.size(); ++i )
      ...
      if( cell_value.residual_ < 1.0 )
        ...
        for( unsigned k = 0; k < edge_images_.size(); k++ )
          ...
          if( dist < 10.0 )
            {
            if( vcl_abs(angle_of_min - line_angle) < vnl_math::pi / 16.0 ||
                vcl_abs(angle_of_min - line_angle2) < vnl_math::pi / 16.0 )
              {
              cnt++;
              }
            }
