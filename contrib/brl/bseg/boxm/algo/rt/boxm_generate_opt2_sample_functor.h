                                ...
                                bool black_background = false)
{
  ...
  if( model_prior <= 0.0f )
    {
    vcl_cerr << "error: boxm_generate_opt2_samples : alt_appearance_priors sum to " << 1.0f - model_prior
             << " >= 1.0!\n";
    }
  ...
  if( n_alt > 0 )
    ...
    for( ; img_it != obs.end(); ++img_it, ++alt_prob_it )
      ...
      for( unsigned int a = 0; a < n_alt; ++a )
        {
        const float alt_prob_density = boxm_apm_traits<T_data::apm_type>::apm_processor::prob_density(
            alt_appearance_models[a], *img_it);
  ...
  boxm_raytrace_function<pass_2, T_loc, T_data, sample_datatype> raytracer_2(scene, aux_scene, cam.ptr(),
                                                                             obs.ni(), obs.nj() );
  pass_2 pass_2_functor(obs, beta_denom_img, model_prior,
                        alt_prob_img);
