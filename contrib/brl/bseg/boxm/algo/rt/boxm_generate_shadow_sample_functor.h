...
public:
  ...
  boxm_generate_shadow_sample_functor_pass_2(vil_image_view<typename boxm_apm_traits<APM>::obs_datatype> const& image,
                                             vil_image_view<float> const& Beta_denom, float shadow_prior,
                                             vil_image_view<float> const& shadow_density)
    : obs_(image), Beta_denom_(Beta_denom), vis_img_(image.ni(), image.nj(), 1), pre_img_(image.ni(), image.nj(), 1),
    alpha_integral_(image.ni(), image.nj(), 1), shadow_prior_(shadow_prior), shadow_density_(shadow_density)
  {
                                    ...
                                    vcl_string iname)
{
  ...
  boxm_aux_scene<T_loc, T_data,  sample_datatype> aux_scene(&scene, iname, boxm_aux_scene<T_loc, T_data,
                                                                                          sample_datatype>::CLONE);
                                    ...
                                    bool black_background = false)
{
  ...
  boxm_raytrace_function<pass_2, T_loc, T_data, sample_datatype> raytracer_2(scene, aux_scene, cam.ptr(),
                                                                             obs.ni(), obs.nj() );
  pass_2 pass_2_functor(obs, Beta_denom_img, shadow_prior,
                        shadow_density);
