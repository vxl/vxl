                                              ...
                                              vpgl_camera_double_sptr camera)
{
  ...
  vgl_box_3d<double> box_uncertainty(-uncertainty, -uncertainty, -uncertainty, uncertainty, uncertainty,
                                     uncertainty);
                           ...
                           unsigned num_observations)
{
  ...
  if( rpc_shift_3d_flag )
    {
    vpgl_local_rational_camera<double> * cam_input_temp =
                           ...
                           unsigned num_observations)
{
      ...
      for( int m = offset_search_size + 1; m < ni - offset_search_size - 1; m++ )
        ...
        for( int n = offset_search_size + 1; n < nj - offset_search_size - 1; n++ )
          ...
          if( edge_image(m, n, 0) > -1 && edge_image(m, n, 1) > -1 && edge_image(m, n, 2) > -1 )
            {
            if( expected_edge_image(m - u, n - v,
                                    0) > 0 &&
                expected_edge_image(m - u, n - v, 1) > 0 && expected_edge_image(m - u, n - v, 2) > 0 )
              {
              // vcl_cout<<'.';
              ...
              float dist1 =
                vcl_sqrt( (dx * sintheta1 * sintheta1 + dy * sintheta1 * costheta1)
                          * (dx * sintheta1 * sintheta1 + dy * sintheta1 * costheta1)
                          + (dy * costheta1 * costheta1 + dx * sintheta1 * costheta1)
                          * (dy * costheta1 * costheta1 + dx * sintheta1 * costheta1) );
              float dist2 =
                vcl_sqrt( (dx * sintheta2 * sintheta2 + dy * sintheta2 * costheta2)
                          * (dx * sintheta2 * sintheta2 + dy * sintheta2 * costheta2)
                          + (dy * costheta2 * costheta2 + dx * sintheta2 * costheta2)
                          * (dy * costheta2 * costheta2 + dx * sintheta2 * costheta2) );
    ...
    for( double v = -subpixel_offset_limit; v <= subpixel_offset_limit; )
      ...
      for( int m = offset_search_size; m < ni - offset_search_size; m++ )
        ...
        for( int n = offset_search_size; n < nj - offset_search_size; n++ )
          ...
          if( edge_image(m, n, 0) != -1 || edge_image(m, n, 1) != -1 || edge_image(m, n, 2) != -1 )
            {
            if( expected_edge_image(m - best_offset_u, n - best_offset_v,
                                    0) > 0 &&
                expected_edge_image(m - best_offset_u, n - best_offset_v,
                                    1) > 0 && expected_edge_image(m - best_offset_u, n - best_offset_v, 2) > 0 )
              {
              double dx = expected_edge_image(m - best_offset_u, n - best_offset_v, 0) + best_offset_u + u - edge_image(
                  m, n, 0);
              double dy = expected_edge_image(m - best_offset_u, n - best_offset_v, 1) + best_offset_v + v - edge_image(
                  m, n, 1);
  ...
  vcl_cout << "Estimated changes in offsets (u,v)=(" << subpixel_best_offset_u << ',' << subpixel_best_offset_v
           << ')' << vcl_endl;
