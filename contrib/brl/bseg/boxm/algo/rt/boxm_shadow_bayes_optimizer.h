...
public:
  ...
  boxm_shadow_bayes_optimizer(boxm_scene<boct_tree<T_loc, boxm_sample<APM> > > & scene,
                              vcl_vector<vcl_string> const& image_ids, float min_app_sigma, float shadow_prior,
                              float shadow_mean, float shadow_sigma, bool verbose = false, vgl_point_3d<double> debug_pt =
                                vgl_point_3d<double>(0, 0,
                                                     0) );
