#ifndef boxm_shadow_bayes_optimizer_txx_
#define boxm_shadow_bayes_optimizer_txx_
...
boxm_shadow_bayes_optimizer<T_loc, APM, AUX>::boxm_shadow_bayes_optimizer(boxm_scene<boct_tree<T_loc,
                                                                                               boxm_sample<APM> > > & scene,
                                                                          vcl_vector<vcl_string> const& image_ids,
                                                                          float min_app_sigma,
                                                                          float shadow_prior, float shadow_mean,
                                                                          float shadow_sigma,
...
bool boxm_shadow_bayes_optimizer<T_loc, APM, AUX>::optimize_cells(double damping_factor)
{
  ...
  for( unsigned int i = 0; i < image_ids_.size(); ++i )
    {
    boxm_aux_scene<T_loc, boxm_sample<APM>, aux_type> aux_scene(&scene_, image_ids_[i], boxm_aux_scene<T_loc,
                                                                                                       boxm_sample<APM>,
                                                                                                       aux_type>::LOAD);
...
// #endif
      ...
      boxm_compute_shadow_appearance<APM>(obs_vector, pre_vector, vis_vector, data.appearance_, min_app_sigma_,
                                          shadow_prior_, shadow_mean_, shadow_sigma_, print_p);
      cell->set_data(data);
      }
...
#endif // boxm_shadow_bayes_optimizer_txx_
