...
public:
  ...
  boxm_update_image_functor_pass_2(vil_image_view<typename boxm_apm_traits<APM>::obs_datatype> const& image,
    ...
    : obs_(image), norm_(normalizing_img), vis_img_(image.ni(), image.nj(), 1), pre_img_(image.ni(), image.nj(), 1),
    alpha_integral_(image.ni(), image.nj(), 1)
  {
                          ...
                          bool black_background = false)
{
  ...
  boxm_aux_scene<T_loc, T_data, sample_datatype> aux_scene(&scene,
                                                           boxm_aux_traits<BOXM_AUX_OPT_RT_GREY>::storage_subdir(),
                                                           boxm_aux_scene<T_loc, T_data, sample_datatype>::CLONE);
  typedef boxm_seg_length_functor<T_data::apm_type, sample_datatype> pass_0;
  boxm_raytrace_function<pass_0, T_loc, T_data, sample_datatype> raytracer_0(scene, aux_scene, cam.ptr(),
                                                                             obs.ni(), obs.nj() );
  vcl_cerr << "PASS 0\n";
  ...
  boxm_raytrace_function<pass_1, T_loc, T_data, sample_datatype> raytracer_1(scene, aux_scene, cam.ptr(),
                                                                             obs.ni(), obs.nj() );
  vcl_cerr << "PASS 1\n";
  ...
  vcl_cerr << "PASS 2\n";
  typedef boxm_update_image_functor_pass_2<T_data::apm_type,
                                           boxm_aux_traits<BOXM_AUX_OPT_RT_GREY>::sample_datatype> pass_2;
  boxm_raytrace_function<pass_2, T_loc, T_data, boxm_aux_traits<BOXM_AUX_OPT_RT_GREY>::sample_datatype> raytracer_2(
    scene, aux_scene, cam.ptr(), obs.ni(), obs.nj() );
  pass_2 pass_2_functor(
    obs, norm_img);
  raytracer_2.run(pass_2_functor);
  vcl_cerr << "PASS 3\n";
  typedef boxm_update_image_functor_pass_3<T_data::apm_type,
                                           boxm_aux_traits<BOXM_AUX_OPT_RT_GREY>::sample_datatype> pass_3;
  boxm_iterate_cells_function<pass_3, T_loc, T_data,
                              boxm_aux_traits<BOXM_AUX_OPT_RT_GREY>::sample_datatype> cell_tracer_3(scene, aux_scene,
                                                                                                    cam.ptr(), obs.ni(),
                                                                                                    obs.nj() );
