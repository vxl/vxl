...
bool boxm_edge_update_process(bprb_func_process& pro)
{
  ...
  if( scene_base->appearence_model() == BOXM_EDGE_LINE )
    ...
    boxm_edge_tangent_updater<short, float, float> updater(*scene, image_ids, use_ransac, ortho_thres, vol_ratio,
                                                           consensus_cnt);
    updater.add_cells();
    }
  // store output
...
bool boxm_edge_refine_updates_process(bprb_func_process& pro)
{
  ...
  for( unsigned int i = 0; i < n_cameras; ++i )
    ...
    if( !ratcam.as_pointer() )
      ...
      if( !ratcam.as_pointer() )
        {
        vcl_cout << " camera: " << img_id
                 << " is not local or global rational camera, not supporting any other type for now!\n";
        return false;
        }
      }
