...
bool boxm_generate_opt2_samples_process(bprb_func_process& pro)
{
  ...
  if( pro.n_inputs() < n_inputs_ )
    {
    vcl_cout << pro.name() << "boxm_generate_opt2_samples_process: The input number should be " << n_inputs_
             << vcl_endl;
    return false;
    }
  ...
  switch( scene->appearence_model() )
    ...
    case BOXM_APM_SIMPLE_GREY:
      ...
      if( !scene->multi_bin() )
        ...
        boxm_generate_opt2_samples<short, boxm_sample<BOXM_APM_SIMPLE_GREY>, BOXM_AUX_OPT2_GREY>(*s, camera, img,
                                                                                                 img_name,
                                                                                                 alt_appearance_priors,
                                                                                                 alt_appearance_models,
    ...
    case BOXM_APM_MOG_GREY:
      ...
      if( !scene->multi_bin() )
        ...
        boxm_scene<tree_type> * s = static_cast<boxm_scene<tree_type> *>(scene.as_pointer() );
        boxm_generate_opt2_samples<short, boxm_sample<BOXM_APM_MOG_GREY>, BOXM_AUX_OPT2_GREY>(*s, camera, img, img_name,
                                                                                              alt_appearance_priors,
                                                                                              alt_appearance_models,
                                                                                              use_black_background);
        }
