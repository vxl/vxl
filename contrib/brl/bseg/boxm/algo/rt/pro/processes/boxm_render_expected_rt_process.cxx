...
bool boxm_render_expected_rt_process(bprb_func_process& pro)
{
  ...
  if( scene_ptr->appearence_model() == BOXM_APM_MOG_GREY )
    ...
    if( !scene_ptr->multi_bin() )
      ...
      boxm_render_image_rt<short, boxm_sample<BOXM_APM_MOG_GREY> >(*scene, camera, expected, mask, -1,
    ...
    vil_image_view<unsigned char> * expected_byte = new vil_image_view<unsigned char>(expected.ni(),
                                                                                      expected.nj(),
                                                                                      expected.nplanes() );
    ...
    if( !scene_ptr->multi_bin() )
      ...
      boxm_scene<type>* scene = dynamic_cast<boxm_scene<type> *>(scene_ptr.as_pointer() );
      boxm_render_image_rt<short, boxm_sample<BOXM_APM_SIMPLE_GREY> >(*scene, camera, expected, mask, -1,
                                                                      use_black_background);
      }
    else
      {
    ...
    if( !scene_ptr->multi_bin() )
      ...
      boxm_scene<type>* scene = dynamic_cast<boxm_scene<type> *>(scene_ptr.as_pointer() );
      boxm_render_image_rt<short, boxm_sample<BOXM_APM_MOB_GREY> >(*scene, camera, expected, mask, -1,
                                                                   use_black_background);
      }
    img_mask = new vil_image_view<float>(mask);
