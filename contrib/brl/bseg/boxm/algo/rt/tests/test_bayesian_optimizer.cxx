...
static void test_bayesian_optimizer()
{
  ...
  vpgl_lvcs lvcs(33.33, 44.44, 10.0, vpgl_lvcs::wgs84, vpgl_lvcs::DEG,
                 vpgl_lvcs::METERS);
  ...
  for( unsigned i = 0; i < image_names.size(); i++ )
    {
    aux_scenes[i] = new boxm_scene<boct_tree<short, aux_type> >(scene.lvcs(), scene.origin(),
                                                                scene.block_dim(), scene.world_dim() );
    ...
    while( !iter.end() )
      ...
      boct_tree_cell<short,
                     aux_type>* cell1 = aux_tree->locate_point(vgl_point_3d<double>(0.01, 0.01, 0.01) );
      ...
      rt.seg_len_ = 0.5;
      cell2->set_data(rt);
      aux_scenes[i]->set_block(iter.index(), new boxm_block<boct_tree<short, aux_type> >(
                                 block->bounding_box(), aux_tree) );
