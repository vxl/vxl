                       ...
                       typename T_data::apm_datatype background_model, int bin)
{
  ...
  if( vpgl_rational_camera<double> const* rcam = dynamic_cast<vpgl_rational_camera<double> const *>(cam.as_pointer() ) )
    ...
    vgl_plane_3d<double> projection_plane = boxm_rational_camera_utils::boxm_find_parallel_image_plane(rcam, top,
                                                                                                       bottom,
                                                                                                       img.ni(),
                                                                                                       img.nj() );
    }
    ...
    for( unsigned i = 0; i < block_indices.size(); i++ ) // code for each block
      ...
      while( frontier_it.next() )
        ...
        for( ; cell_it != vis_cells.end(); cell_it++ )
          ...
          else if( vpgl_rational_camera<double>* rcam =
                     dynamic_cast<vpgl_rational_camera<double> *>(cam.as_pointer() ) )
            {
            boxm_rational_camera_utils::project_corners_rational_camera(corners, rcam,
                                                                        vgl_plane_3d<double>(), xverts, yverts,
                                                                        vertdists);
