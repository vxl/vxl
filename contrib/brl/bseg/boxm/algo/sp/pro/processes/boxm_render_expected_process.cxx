...
bool boxm_render_expected_process(bprb_func_process& pro)
{
  ...
  if( scene_ptr->appearence_model() == BOXM_APM_MOG_GREY )
    ...
    if( !scene_ptr->multi_bin() )
      ...
      boxm_render_image_splatting_triangle<short, boxm_sample<BOXM_APM_MOG_GREY> >(*scene, camera, expected, mask, -1,
                                                                                   true);
      }
    else
      {
      ...
      boxm_scene<type>* scene = dynamic_cast<boxm_scene<type> *>(scene_ptr.as_pointer() );
      boxm_render_image_splatting_triangle<short, boxm_sample_multi_bin<BOXM_APM_MOG_GREY> >(*scene, camera, expected,
                                                                                             mask, bin, true);
      }
