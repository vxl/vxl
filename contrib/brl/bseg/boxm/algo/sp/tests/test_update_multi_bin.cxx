...
static void test_update_multi_bin()
{
  ...
  while( !iter.end() )
    ...
    boct_tree<short,
              boxm_sample_multi_bin<BOXM_APM_MOG_GREY> > * tree =
      new boct_tree<short, boxm_sample_multi_bin<BOXM_APM_MOG_GREY> >(4, 3);
    boct_tree_cell<short,
                   boxm_sample_multi_bin<BOXM_APM_MOG_GREY> >* cel11 =
      tree->locate_point(vgl_point_3d<double>(0.01, 0.01, 0.01) );
    cel11->set_data(s2_sample);
    boct_tree_cell<short,
                   boxm_sample_multi_bin<BOXM_APM_MOG_GREY> >* cell2 =
      tree->locate_point(vgl_point_3d<double>(0.51, 0.51, 0.01) );
  ...
  for( unsigned i = 0; i < cameras.size(); i++ )
    ...
    expected.fill(0.0);
    boxm_render_image_splatting_triangle<short, boxm_sample_multi_bin<BOXM_APM_MOG_GREY> >(scene, cameras[i], expected,
                                                                                           mask, 5);
  ...
  for( unsigned i = 0; i < cameras.size(); i++ )
    {
    vcl_stringstream      ss; ss << "boxm_scene2_mb/img_new" << i << ".tif";
    vil_image_view<float> expected_new(IMAGE_U, IMAGE_V);
    boxm_render_image_splatting_triangle<short, boxm_sample_multi_bin<BOXM_APM_MOG_GREY> >(scene_new, cameras[i],
                                                                                           expected_new, mask, 3);
