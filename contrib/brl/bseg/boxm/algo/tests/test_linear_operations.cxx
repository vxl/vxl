//:
// \file Test linear combinations of blocks
...
void test_linear_operations()
{
  ...
  while ( !(it1.end() || it2.end()) ) {
    ...
    if(vcl_abs(cell1->data()- 4.0f) > 1.0e-7){
      vcl_cerr << "Data1 is incorrect: "<< cell1->data()  <<"Error: " << vcl_abs(cell1->data()- 4.0f) << vcl_endl;
      result = false;
      break;
    }
    
    if(vcl_abs(cell2->data()- 0.8f) > 1.0e-7){
      vcl_cerr << "Data2 is incorrect: " << cell2->data()<<vcl_endl;
      result = false;
      break;
    }
