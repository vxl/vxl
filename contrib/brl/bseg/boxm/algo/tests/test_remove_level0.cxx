...
void test_remove_level0()
{
  ...
  boxm_scene<boct_tree<short, float> > * scene2 = new boxm_scene<boct_tree<short, float> >(
      scene1->lvcs(), scene1->origin(), scene1->block_dim(),
      scene1->world_dim(),
      scene1->max_level(), scene1->init_level(),
      scene1->load_all_blocks(),
      scene1->save_internal_nodes() );
