...
static void test_upload_mesh()
{
  ...
  while( !iter.end() )
    ...
    boct_tree<short,
              boxm_sample<BOXM_APM_MOG_GREY> > * tree = new boct_tree<short, boxm_sample<BOXM_APM_MOG_GREY> >(6, 5);
  ...
  boct_tree_cell<short,
                 boxm_sample<BOXM_APM_MOG_GREY> >* cell0 = tree->locate_point_global(
      vert0.operator vgl_point_3d<double>() );
  TEST("Vertex 0 is in the octree", cell0->data().alpha, 0.0f);

  boct_tree_cell<short,
                 boxm_sample<BOXM_APM_MOG_GREY> >* cell1 = tree->locate_point_global(
      vert1.operator vgl_point_3d<double>() );
  TEST("Vertex 1 is in the octree", cell1->data().alpha, 0.0f);

  boct_tree_cell<short,
                 boxm_sample<BOXM_APM_MOG_GREY> >* cell2 = tree->locate_point_global(
      vert2.operator vgl_point_3d<double>() );
  TEST("Vertex 2 is in the octree", cell2->data().alpha, 0.0f);

  boct_tree_cell<short,
                 boxm_sample<BOXM_APM_MOG_GREY> >* cell = tree->locate_point_global(vgl_point_3d<double>(1, 1, 1) );
