...
public:
  ...
  boxm_raytrace_function(boxm_scene<tree_type> & scene,
    ...
    : scene_(scene), aux_scene_(null_scene_), reverse_traversal_(reverse_traversal), cam_(cam), img_i0_(i0),
    img_j0_(j0), img_ni_(ni), img_nj_(nj), debug_lvl_(0) {}
  ...
  boxm_raytrace_function(boxm_scene<tree_type> & scene,
                         ...
                         bool reverse_traversal = false,
                         unsigned int i0 = 0, unsigned int j0 = 0)
    : scene_(scene), aux_scene_(aux_scene), reverse_traversal_(reverse_traversal), cam_(cam), img_i0_(i0), img_j0_(j0),
    img_ni_(ni), img_nj_(nj), debug_lvl_(0) {}
    ...
    while( block_vis_iter.next() )
      ...
      for( unsigned i = 0; i < block_indices.size(); i++ )  // code for each block
        ...
        if( block_bb.contains(ray_origin) )
          ...
          else if( (plane_intersections[0].y() >= block_bb.min_y() ) &&
                   (plane_intersections[0].y() <= block_bb.max_y() ) &&
                   (plane_intersections[0].z() >= block_bb.min_z() ) &&
                   (plane_intersections[0].z() <= block_bb.max_z() ) &&
                   (direction.x() > epsilon) )
            {
            // ray intersects the xmin plane
            enter_pt = plane_intersections[0];
            }
          else if( (plane_intersections[1].y() >= block_bb.min_y() ) &&
                   (plane_intersections[1].y() <= block_bb.max_y() ) &&
                   (plane_intersections[1].z() >= block_bb.min_z() ) &&
                   (plane_intersections[1].z() <= block_bb.max_z() ) &&
                   (direction.x() < -epsilon) )
            {
            // ray intersects the xmax plane
            enter_pt = plane_intersections[1];
            }
          else if( (plane_intersections[2].x() >= block_bb.min_x() ) &&
                   (plane_intersections[2].x() <= block_bb.max_x() ) &&
                   (plane_intersections[2].z() >= block_bb.min_z() ) &&
                   (plane_intersections[2].z() <= block_bb.max_z() ) &&
                   (direction.y() > epsilon) )
            {
            // ray intersects the ymin plane
            enter_pt = plane_intersections[2];
            }
          else if( (plane_intersections[3].x() >= block_bb.min_x() ) &&
                   (plane_intersections[3].x() <= block_bb.max_x() ) &&
                   (plane_intersections[3].z() >= block_bb.min_z() ) &&
                   (plane_intersections[3].z() <= block_bb.max_z() ) &&
                   (direction.y() < -epsilon) )
            {
            // ray intersects the ymax plane
            enter_pt = plane_intersections[3];
            }
          else if( (plane_intersections[4].x() >= block_bb.min_x() ) &&
                   (plane_intersections[4].x() <= block_bb.max_x() ) &&
                   (plane_intersections[4].y() >= block_bb.min_y() ) &&
                   (plane_intersections[4].y() <= block_bb.max_y() ) &&
                   (direction.z() > epsilon) )
            {
        ...
        while( true )
          ...
          bool                 found_exit = boxm_utils::cube_exit_point(cell_bb, enter_pt, direction, exit_pt, lambda,
                                                                        face_id);
          if( !found_exit )
            {
        ...
        for( unsigned int i = int(img_bb.min_x() + 0.99); i <= img_bb.max_x(); ++i )
          ...
          for( unsigned int j = int(img_bb.min_y() + 0.99); j <= img_bb.max_y(); ++j )
            ...
            while( continue_trace(i - img_i0_, j - img_j0_) )
              {
              boct_tree_cell<T_loc,
                             T_aux> * curr_aux_cell = reinterpret_cast<boct_tree_cell<T_loc, T_aux> *>(&aux_cell_null);
              if( step_functor.is_aux_ )
                {
                boct_loc_code<T_loc> cell_code(curr_cell->get_code() );
                curr_aux_cell = aux_tree->get_cell(cell_code);
              ...
              continue_trace(i - img_i0_, j - img_j0_) = step_functor.step_cell(i, j, enter_pt, exit_pt, cell_val,
                                                                                aux_val);
...
protected:
  ...
  void generate_ray(float i, float j, vgl_box_3d<double> const & /*block_bb*/, vgl_point_3d<double> & ray_origin,
                    vgl_vector_3d<double> & norm_direction)
  {
