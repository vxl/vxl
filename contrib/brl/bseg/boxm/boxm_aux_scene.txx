  ...
  : aux_scene_(0)
{
  ...
  aux_scene_ = new boxm_scene<aux_tree_type>(scene->lvcs(), scene->origin(), scene->block_dim(),
                                             scene->world_dim(), scene->load_all_blocks(),
                                             scene->save_internal_nodes(), scene->save_platform_independent() );
    ...
    while( !iter.end() )
      ...
      if( scene->discover_block(iter.index().x(), iter.index().y(), iter.index().z() ) )
        ...
        if( type == LOAD )
          ...
          boxm_block<aux_tree_type>*         aux_block =
            new boxm_block<aux_tree_type>(block->bounding_box(), aux_tree );
...
template <class T_loc, class T, class T_AUX>
boct_tree_cell_reader<T_loc, T_AUX> * boxm_aux_scene<T_loc, T,
                                                     T_AUX>::get_block_incremental(vgl_point_3d<int> block_idx)
{
  if( aux_scene_ )
    {
    vcl_string path =
      aux_scene_->gen_block_path(block_idx.x(), block_idx.y(), block_idx.z() );
