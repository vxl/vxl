...
void boxm_scene<T>::write_active_block(bool unload_block)
{
  if( valid_index(active_block_) && (!load_all_blocks_) )
    ...
    vcl_cout << "block path: " << path << '\n'
             ...
             << "Internal Nodes 2: " << save_internal_nodes_ << " save_platform_independent_ "
             << save_platform_independent_ << vcl_endl;
#endif
...
void boxm_scene<T>::write_active_blocks(bool unload_block)
{
  ...
  for( ; it != active_blocks_.end(); it++ )
    ...
    if( valid_index(bidx) && (!load_all_blocks_) )
      ...
      vsl_b_ofstream os(path);
#ifdef DEBUG
      vcl_cout << "Load All blocks " << load_all_blocks_ << '\n'
               << "Internal Nodes 2: " << save_internal_nodes_ << " save_platform_independent_ "
               << save_platform_independent_ << vcl_endl;
#endif
      blocks_(x, y, z)->b_write(os, save_internal_nodes_, save_platform_independent_);
...
{
  vgl_point_3d<double> min_p(block_dim_.x() * x + origin_.x(), block_dim_.y() * y + origin_.y(),
                             block_dim_.z() * z + origin_.z() );
...
template <class T>
vcl_string boxm_scene<T>::gen_block_path(int x, int y, int z) const
{
...
bool boxm_scene<T>::load_block_and_neighbors(unsigned i, unsigned j, unsigned k)
{
  ...
  vcl_set<vgl_point_3d<int>,
          bvgl_point_3d_cmp<int> > new_active_blocks  = neighboring_blocks(vgl_point_3d<int>(i, j, k) );
  ...
  vcl_set_difference(active_blocks_.begin(), active_blocks_.end(),
                     ...
                     vcl_insert_iterator<vcl_set<vgl_point_3d<int>, bvgl_point_3d_cmp<int> > >(blocks_to_unload,
                                                                                               blocks_to_unload.begin() ),
                     cmp);

  vcl_set_difference(new_active_blocks.begin(), new_active_blocks.end(),
                     active_blocks_.begin(), active_blocks_.end(),
                     vcl_insert_iterator<vcl_set<vgl_point_3d<int>, bvgl_point_3d_cmp<int> > >(blocks_to_load,
                                                                                               blocks_to_load.begin() ),
                     cmp);
...
double  boxm_scene<T>::finest_cell_length_in_memory() const
{
  ...
  if( (vcl_abs(block_dim_.x() - block_dim_.y() ) > 1.0e-7)   || (vcl_abs(block_dim_.x() - block_dim_.z() ) > 1.0e-7) )
    {
    vcl_cerr
      << "Warning: In boxm_scene::finest_cell_length, cells aren't cubical, returning length along x direction\n";
    }
...
template <class T>
void boxm_scene<T>::leaves_in_region(vgl_box_3d<double> box, vcl_vector<boct_tree_cell<typename T::loc_type,
                                                                                       typename T::datatype> *> & cells)
{
  ...
  for( int k = min_idx.z(); k <= max_idx.z(); k++ )
    ...
    for( int j = min_idx.y(); j <= max_idx.y(); j++ )
      ...
      for( int i = min_idx.x(); i <= max_idx.x(); i++ )
        ...
        vgl_box_3d<double> local_box_exclusive(local_box.min_point().x(),
                                               local_box.min_point().y(), local_box.min_point().z(),
                                               local_box.max_point().x() - 1e-7,
                                               local_box.max_point().y() - 1e-7, local_box.max_point().z() - 1e-7);
  ...
  for( int k = min_idx.z(); k <= max_idx.z(); k++ )
    ...
    for( int j = min_idx.y(); j <= max_idx.y(); j++ )
      ...
      for( int i = min_idx.x(); i <= max_idx.x(); i++ )
        ...
        vcl_cout << ".....In tree with box dimension: " << get_block_bbox(i, j, k).width() << ", " << get_block_bbox(i,
                                                                                                                     j,
                                                                                                                     k).
          depth() << ", " << get_block_bbox(i, j, k).height()
                 << "looking for box with dim: " << box.width() << ", " << box.depth() << ", " << box.height()
                 << vcl_endl;
...
template <class T>
void boxm_scene<T>::leaves_data_in_region(vgl_box_3d<double> box, vcl_vector<boct_cell_data<typename T::loc_type,
                                                                                            typename T::datatype> > & cell_data)
{
  vgl_box_3d<double> valid_box = vgl_intersection(get_world_bbox(), box);
  cell_data.clear();
...
template <class T>
void boxm_scene<T>::cell_data_in_region(vgl_box_3d<double> box, vcl_vector<boct_cell_data<loc_type,
                                                                                          datatype> > & cell_data,
                                        unsigned level)
{
  vgl_box_3d<double> valid_box = vgl_intersection(get_world_bbox(), box);
...
template <class T>
void boxm_scene<T>::leaves_in_hollow_region(vgl_box_3d<double> outer_box, vgl_box_3d<double> inner_box,
                                            vcl_vector<boct_tree_cell<loc_type, datatype> *>& cells)
{
  ...
  for( int k = min_idx.z(); k <= max_idx.z(); k++ )
    ...
    for( int j = min_idx.y(); j <= max_idx.y(); j++ )
      ...
      for( int i = min_idx.x(); i <= max_idx.x(); i++ )
        ...
        // subtract a little from the max point because octree cell are give by a half-closed interval [...).
        // if this is not done, the endpoint may be out of bounds
        vgl_box_3d<double> local_outer_box_exclusive(local_box.min_point().x(),
                                                     local_box.min_point().y(), local_box.min_point().z(),
                                                     local_box.max_point().x() - 1e-7,
                                                     local_box.max_point().y() - 1e-7,
                                                     local_box.max_point().z() - 1e-7);
...
// : Locates and modifies the value of all cells within a 3d region, which coordinates are given in scene coordinates
template <class T>
void boxm_scene<T>::change_leaves_in_regions(vcl_vector<vgl_box_3d<double> > boxes,
                                             const vcl_vector<typename T::datatype> & all_data)
{
...
// : Locate point
template <class T>
boct_tree_cell<typename T::loc_type, typename T::datatype> * boxm_scene<T>::locate_point_in_memory(
  vgl_point_3d<double> & p, unsigned level)
{
...
boxm_cell_iterator<T> & boxm_cell_iterator<T>::begin(bool use_internal_cells)
{
  ...
  (block_iterator_.scene_->*block_loading_func_)(block_iterator_.index().x(), block_iterator_.index().y(),
                                                 block_iterator_.index().z() );
...
template <class T>
boct_tree_cell<typename boxm_cell_iterator<T>::loc_type,
               typename boxm_cell_iterator<T>::datatype> * boxm_cell_iterator<T>::operator*()
{
