...
void boxm_update(boxm_scene<boct_tree<T_loc, boxm_sample<APM> > > &scene, 
				 vil_image_view<typename psm_apm_traits<APM>::obs_mathtype> &img,
				 ...
				 bool black_background = false)
{
  ...
  boxm_update_function<boxm_update_functor_one_pass<APM>, APM> update_fn1(scene, cam, img.ni(), img.nj());
  boxm_update_functor_one_pass<APM> update_func1(img, cam);
  update_func1.set_background_model(background_apm);

  vcl_cout << "update: pass1" << vcl_endl;
  update_fn1.run(update_func1);
  // retrieve normalization image
  vil_image_view<float> norm_img(img.ni(), img.nj(), 1);
  update_func1.normalization_image(norm_img);
  
  boxm_update_function<boxm_update_functor_two_pass<APM>, APM> update_fn2(scene, cam, img.ni(), img.nj());
  boxm_update_functor_two_pass<APM> update_func2(cam, img, norm_img);
  update_func2.set_background_model(background_apm);

  vcl_cout << "update: pass2" << vcl_endl;
  update_fn2.run(update_func2);
