...
void boxm_ocl_refine_scene_cpu(boct_tree<short, boxm_sample<APM> >* tree, float prob_thresh)
{
  ...
  float max_alpha_int = (-1)*vcl_log(1.0 - prob_thresh);
  ...
  vcl_cout<<"refine time: "<<refine_time<<'\n'
          <<"num cells now: "<<cells.size()<<vcl_endl;
  ...
  vcl_cout<<'\n'
          <<"----CPU SWAPPING STATS---------------\n"
          <<"tree size (numcells): "<<numCells<<'\n'
          <<"cpu time : "<<cpu_time<<" s\n"
          <<"format correct : "<< (isCorrect ? " yes" : " no") <<'\n'
          <<"-------------------------------------"<<vcl_endl;
}
