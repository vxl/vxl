												...
												vil_image_view<float> &obs)
{
	...
	if (build_kernel_program(program_pass0_)) {
		return false;
	}
	return true;
}
...
{
  bool good = true;
  good = good && set_root_level_buffers();
  return good;
}
template<class T>
...
{
  bool good = true;
  good = good && release_root_level_buffers();
  return good;
}
template<class T>
...
bool boxm_online_update_manager<T>::set_root_level_buffers()
{
	...
	root_level_buf_ = clCreateBuffer(this->context_,
									...
									sizeof(cl_uint),
									&root_level_,&status);
	if (!this->check_val(status,CL_SUCCESS,"clCreateBuffer (root level) failed."))
		return false;
...
bool boxm_online_update_manager<T>::set_persp_camera_buffers()
{
	...
	persp_cam_buf_ = clCreateBuffer(this->context_,
									...
									3*sizeof(cl_float16),
									persp_cam_,&status);
	if (!this->check_val(status,CL_SUCCESS,"clCreateBuffer (persp_cam_buf_) failed."))
		return false;
...
bool boxm_online_update_manager<T>::release_persp_camera_buffers()
{
	...
	if (!this->check_val(status,CL_SUCCESS,"clReleaseMemObject failed (persp_cam_buf_)."))
		return false;
...
bool boxm_online_update_manager<T>::set_input_image()
{
  ...
  {
	  vcl_cerr<<"Failed allocation of image or image dimensions "<<vcl_endl;
	  return false;
  }
  return true;
...
bool boxm_online_update_manager<T>::set_input_image_buffers()
{
	...
	img_dims_buf_ = clCreateBuffer(this->context_,
								...
								sizeof(cl_uint4),
								img_dims_,&status);
	if (!this->check_val(status,CL_SUCCESS,"clCreateBuffer (imd_dims_buf_) failed."))
		return false;
...
bool boxm_online_update_manager<T>::set_offset_buffers(int offset_x,int offset_y)
{
	...
	offset_y_buf_ = clCreateBuffer(this->context_,
								...
								sizeof(cl_int),
								&offset_y_,&status);
	if (!this->check_val(status,CL_SUCCESS,"clCreateBuffer (offset_y_) failed."))
		return false;
	return true;
}
template<class T>
bool boxm_online_update_manager<T>::release_offset_buffers()
{
	...
	if (!this->check_val(status,CL_SUCCESS,"clReleaseMemObject failed (offset_y_buf_)."))
		return false;
