// This is brl/bseg/boxm/ocl/boxm_ray_bundle_trace_manager.txx
#ifndef boxm_ray_bundle_trace_manager_txx_
#define boxm_ray_bundle_trace_manager_txx_
#include "boxm_ray_bundle_trace_manager.h"
                                                     ...
                                                     unsigned int i0, unsigned int j0, bool useimage)
{
  ...
  if (n_sources == 0) {
    vcl_cerr << "Error: boxm_ray_bundle_trace_manager : must provide at least one functor source\n";
    return  false;
  }
  for (unsigned int i=0; i < n_sources; ++i) {
    if (!this->append_process_kernels(functor_source_filenames[i])) {
      vcl_cerr << "Error: boxm_ray_bundle_trace_manager : failed to load application-specific functor source\n";
      return false;
    }
  }
  if (!this->append_process_kernels(vcl_string(VCL_SOURCE_ROOT_DIR)
                                    +"/contrib/brl/bseg/boxm/ocl/ray_bundle_trace_main.cl")) {
  ...
  kernel_ = clCreateKernel(program_,"ray_bundle_trace_main",&status);
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::setup_image_cam_arrays()
{
  bool good = true;
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::clean_image_cam_arrays()
{
  bool good = true;
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::clean_raytrace()
{
  bool good = true;
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::setup_img_bb(vpgl_camera_double_sptr cam, vgl_box_3d<double> const& block_bb, vgl_box_2d<double> &img_bb, unsigned ni,unsigned nj)
{
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::setup_tree()
{
  if (!tree_)
    return false;
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::clean_tree()
{
  if (cells_)
  {
#ifdef _WIN32
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::setup_tree_results()
{
  if (!tree_)
    return false;
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::clean_tree_results()
{
  if (tree_results_)
  {
#ifdef _WIN32
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::setup_camera()
{
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::clean_camera()
{
  if (svd_UtWV_)
  {
#ifdef _WIN32
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::setup_img_dims(unsigned ni,unsigned nj)
{
  ni_=ni;
  nj_=nj;
#if defined (_WIN32)
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::clean_img_dims()
{
  if (imgdims_) {
#ifdef _WIN32
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::setup_roi_dims(unsigned min_i,unsigned max_i,unsigned min_j,unsigned max_j)
{
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::clean_roi_dims()
{
  if (roidims_) {
#ifdef _WIN32
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::setup_work_image()
{
#if defined (_WIN32)
  ray_results_ = (cl_float*)_aligned_malloc(this->work_space_ni()*this->work_space_nj()*sizeof(cl_float4), 16);
#elif defined(__APPLE__)
  ray_results_ = (cl_float*)malloc(this->work_space_ni()*this->work_space_nj()*sizeof(cl_float4));
#else
  ray_results_ = (cl_float*)memalign(16, this->work_space_ni()*this->work_space_nj()* sizeof(cl_float4));
#endif

  for (unsigned i=0;i<this->work_space_ni()*this->work_space_nj()*4;)
  {
    ray_results_[i]=0.0f;
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::clean_work_image()
{
  if (ray_results_) {
#ifdef _WIN32
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::setup_ray_origin()
{
  ...
  if (vpgl_perspective_camera<double> *pcam =
    ...
    vcl_cerr << "Error: boxm_ray_bundle_trace_manager::setup_ray_origin() : unsupported camera type\n";
    return false;
  }
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::clean_ray_origin()
{
  if (ray_origin_) {
#ifdef _WIN32
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::setup_tree_input_buffers(bool useimage)
{
  cl_int status = CL_SUCCESS;
  useimage_=useimage;
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::clean_tree_input_buffers()
{
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::setup_camera_input_buffer()
{
  cl_int status = CL_SUCCESS;
  // Create and initialize memory objects
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::clean_camera_input_buffer()
{
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::setup_roidims_input_buffer()
{
  cl_int status = CL_SUCCESS;
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::clean_roidims_input_buffer()
{
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::setup_ray_origin_buffer()
{
  cl_int status = CL_SUCCESS;
  // Create and initialize memory objects
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::clean_ray_origin_buffer()
{
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::setup_work_img_buffer()
{
  cl_int status = CL_SUCCESS;
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::clean_work_img_buffer()
{
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::setup_imgdims_buffer()
{
  cl_int status = CL_SUCCESS;
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::clean_imgdims_buffer()
{
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::setup_tree_global_bbox_buffer()
{
  cl_int status = CL_SUCCESS;
  // Create and initialize memory objects
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::clean_tree_global_bbox_buffer()
{
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::setup_image_cam_buffers()
{
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::clean_image_cam_buffers()
{
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::build_kernel_program(bool useimage)
{
  cl_int status = CL_SUCCESS;
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::create_kernel(vcl_string const& kernel_name)
{
  cl_int status = CL_SUCCESS;
...
template<class T>
int boxm_ray_bundle_trace_manager<T>::release_kernel()
{
...
template<class T>
void boxm_ray_bundle_trace_manager<T>::clear_tree_results()
{
  if (tree_results_) {
...
template<class T>
void boxm_ray_bundle_trace_manager<T>::clear_ray_results()
{
  if (ray_results_) {
...
template<class T>
vcl_size_t boxm_ray_bundle_trace_manager<T>::n_ray_groups()
{
...
template<class T>
void boxm_ray_bundle_trace_manager<T>::print_tree_input()
{
  vcl_cout << "Tree Input\n";
  if (cells_)
...
template<class T>
void boxm_ray_bundle_trace_manager<T>::print_ray_input()
{
  vcl_cout << "Ray Input\n";
...
template<class T>
void boxm_ray_bundle_trace_manager<T>::print_ray_results()
{
...
template<class T>
boxm_ray_bundle_trace_manager<T>::~boxm_ray_bundle_trace_manager()
{
  if (program_)
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::load_tree(vcl_string const& path)
{
  if (tree_)
    delete tree_;
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::write_tree(vcl_string const& path)
{
  if (!tree_)
    return false;
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>::run_block()
{
  ...
  if (!this->check_val(status,CL_SUCCESS,"clSetKernelArg failed. (local cache ptr bundle)"))
   return SDK_FAILURE;
  status = clSetKernelArg(kernel_,14,sizeof(cl_short4)*this->bundle_ni()*this->bundle_nj(),0);
  if (!this->check_val(status,CL_SUCCESS,"clSetKernelArg failed. (loc code bundle)"))
    return SDK_FAILURE;
  status = clSetKernelArg(kernel_,15,sizeof(cl_float2)*this->bundle_ni()*this->bundle_nj(),0);
  if (!this->check_val(status,CL_SUCCESS,"clSetKernelArg failed. (loc code bundle)"))
    return SDK_FAILURE;
...
template<class T>
bool boxm_ray_bundle_trace_manager<T>:: read_output_image()
{
  cl_event events[2];

  // Enqueue readBuffers
  int status = clEnqueueReadBuffer(command_queue_,work_image_buf_,CL_TRUE,
                                   0,this->work_space_ni()*this->work_space_nj()*sizeof(cl_float4),
                                   this->ray_results(),
                                   0,NULL,&events[0]);
...
#define BOXM_RAY_BUNDLE_TRACE_MANAGER_INSTANTIATE(T) \
  template class boxm_ray_bundle_trace_manager<T >
