...
bool boxm_re_render_ocl_scene_manager::set_args(unsigned pass)
{
  ...
  if (pass==4)  // update_ocl_scene_main_(opt)
  {
    ...
    if (check_val(status, CL_SUCCESS, "clSetKernelArg failed. (cell_alpha_buf_)")!=CHECK_SUCCESS)
      return false;
    status = clSetKernelArg(kernels_[pass], i++, sizeof(cl_mem), (void *)&cell_mixture_buf_);
    if (check_val(status, CL_SUCCESS, "clSetKernelArg failed. (cell_mixture_buf_)")!=CHECK_SUCCESS)
      return false;
    ...
    if (check_val(status, CL_SUCCESS, "clSetKernelArg failed. (lenbuffer_buf_)")!=CHECK_SUCCESS)
      return false;
    status = clSetKernelArg(kernels_[pass], i++, sizeof(cl_mem), (void *)&numbuffer_buf_);
    if (check_val(status, CL_SUCCESS, "clSetKernelArg failed. (numbuffer_buf_)")!=CHECK_SUCCESS)
      return false;
  ...
  { // norm image process //
    ...
    if (check_val(status, CL_SUCCESS, "clSetKernelArg failed. (image array)")!=CHECK_SUCCESS)
      return false;

    status = clSetKernelArg(kernels_[pass], 1,
                            sizeof(cl_mem), (void *)&app_density_buf_);
    if (check_val(status, CL_SUCCESS, "clSetKernelArg failed. (remote surface appearance)")!=CHECK_SUCCESS)
      return false;
  ...
  if (pass==0 || pass==1 || pass==3) // update_ocl_scene (aux data passes)
  {
    ...
    if (check_val(status,CL_SUCCESS,"clSetKernelArg failed. (factor_buf_)")!=CHECK_SUCCESS)
      return false;
    status = clSetKernelArg(kernels_[pass],i++,sizeof(cl_mem),(void *)&offset_x_buf_);
    if (check_val(status,CL_SUCCESS,"clSetKernelArg failed. (offset_x_buf_)")!=CHECK_SUCCESS)
      return false;

    status = clSetKernelArg(kernels_[pass],i++,sizeof(cl_mem),(void *)&offset_y_buf_);
    if (check_val(status,CL_SUCCESS,"clSetKernelArg failed. (offset_y_buf_)")!=CHECK_SUCCESS)
      return false;
    ...
    if (check_val(status,CL_SUCCESS,"clSetKernelArg failed. (local cache ptr bundle)")!=CHECK_SUCCESS)
      return false;
    status = clSetKernelArg(kernels_[pass],i++,sizeof(cl_int)*this->bni_*this->bnj_,0);
    if (check_val(status,CL_SUCCESS,"clSetKernelArg failed. (loc code bundle)")!=CHECK_SUCCESS)
      return false;
  ...
  if (pass==5) //ray_trace_ocl_scene(_opt)
  {
    ...
    //if (!check_val(status,CL_SUCCESS,"clSetKernelArg failed. (intensity image failed)"))
      //return SDK_FAILURE;
  ...
  if (pass==6)  //refine_main_opt
  {
    ...
    if (!check_val(status,CL_SUCCESS,"clSetKernelArg failed. (local tree buffer)"))
      return false;
    status = clSetKernelArg(kernels_[pass], i++, sizeof(cl_mem),(void*) &output_debug_buf_);
    if (!check_val(status,CL_SUCCESS,"clSetKernelArg failed. (output_Buff buffer)"+error_to_string(status)))
      return false;
  }
...
bool boxm_re_render_ocl_scene_manager::set_commandqueue()
{
  ...
  check_val(status,CL_SUCCESS,"Falied in command queue creation" + error_to_string(status))==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::read_output_image()
{
  ...
  return check_val(status,CL_SUCCESS,"clReleaseEvent failed.")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::set_scene_origin_buffers()
{
  ...
  return check_val(status,CL_SUCCESS,"clCreateBuffer (scene_origin_) failed.")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::release_scene_origin_buffers()
{
  ...
  return check_val(status,CL_SUCCESS,"clReleaseMemObject failed (scene_origin_buf_).")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::set_scene_dims_buffers()
{
  ...
  scene_dims_buf_ = clCreateBuffer(this->context_,
                                   ...
                                   sizeof(cl_int4),
                                   scene_dims_,&status);
  return check_val(status,CL_SUCCESS,"clCreateBuffer (scene_dims_) failed.")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::release_scene_dims_buffers()
{
  ...
  return check_val(status,CL_SUCCESS,"clReleaseMemObject failed (scene_dims_buf_).")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::set_block_dims_buffers()
{
  ...
  block_dims_buf_ = clCreateBuffer(this->context_,
                                   ...
                                   4*sizeof(cl_float),
                                   block_dims_,&status);
  return check_val(status,CL_SUCCESS,"clCreateBuffer (block_dims_) failed.")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::release_block_dims_buffers()
{
  ...
  return check_val(status,CL_SUCCESS,"clReleaseMemObject failed (block_dims_buf_).")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::set_mem_ptrs_buffers()
{
  ...
  output_debug_buf_ = clCreateBuffer(this->context_,
                                     ...
                                     output_debug_,
                                     &status);
  return check_val(status, CL_SUCCESS, "clCreateBuffer (cell_data) failed.")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::release_mem_ptrs_buffers()
{
  ...
  return check_val(status,CL_SUCCESS,"clReleaseMemObject failed (output_debug_buf_).")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::set_block_ptrs_buffers()
{
  ...
  block_ptrs_buf_ = clCreateBuffer(this->context_,
                                   ...
                                   block_ptrs_,&status);
  return check_val(status,CL_SUCCESS,"clCreateBuffer (block_ptrs_) failed.")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::release_block_ptrs_buffers()
{
  ...
  return check_val(status,CL_SUCCESS,"clReleaseMemObject failed (block_ptrs_buf_).")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::set_root_level_buffers()
{
  ...
  root_level_buf_ = clCreateBuffer(this->context_,
                                   ...
                                   sizeof(cl_uint),
                                   &root_level_,&status);
  return check_val(status,CL_SUCCESS,"clCreateBuffer (root level) failed.")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::release_root_level_buffers()
{
  ...
  return check_val(status,CL_SUCCESS,"clReleaseMemObject failed (root_level_buf_).")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::set_tree_buffers()
{
  ...
  cell_alpha_buf_ = clCreateBuffer(this->context_,
                                   ...
                                   cell_alpha_, &status);
  if (!check_val(status,CL_SUCCESS,"clCreateBuffer (cell alpha) failed."))
    return false;
  ...
  if (!check_val(status,CL_SUCCESS,"clCreateBuffer (cell num obs) failed."))
    return false;
...
bool boxm_re_render_ocl_scene_manager::release_tree_buffers()
{
  ...
  if (!check_val(status,CL_SUCCESS,"clReleaseMemObject failed (cell_alpha_buf_)."))
    return false;
  ...
  if (!check_val(status,CL_SUCCESS,"clReleaseMemObject failed (cell_num_obs_buf_)."))
    return false;
...
bool boxm_re_render_ocl_scene_manager::set_persp_camera_buffers()
{
  ...
  persp_cam_buf_ = clCreateBuffer(this->context_,
                                  ...
                                  3*sizeof(cl_float16),
                                  persp_cam_,&status);
  return check_val(status,CL_SUCCESS,"clCreateBuffer (persp_cam_buf_) failed.")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::release_persp_camera_buffers()
{
  ...
  return check_val(status,CL_SUCCESS,"clReleaseMemObject failed (persp_cam_buf_).")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::set_input_image_buffers()
{
  ...
  image_buf_ = clCreateBuffer(this->context_,
                              ...
                              image_,&status);
  return check_val(status,CL_SUCCESS,"clCreateBuffer (image_buf_) failed.")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::write_image_buffer()
{
  ...
  return check_val(status,CL_SUCCESS,"clFinish (writing) failed.")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::set_image_dims_buffers()
{
  ...
  img_dims_buf_ = clCreateBuffer(this->context_,
                                 ...
                                 sizeof(cl_uint4),
                                 img_dims_,&status);
  return check_val(status,CL_SUCCESS,"clCreateBuffer (imd_dims_buf_) failed.")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::release_input_image_buffers()
{
  ...
  return check_val(status,CL_SUCCESS,"clReleaseMemObject failed (img_dims_buf_).")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::set_offset_buffers(int offset_x,int offset_y,int factor)
{
  ...
  offset_y_buf_ = clCreateBuffer(this->context_,
                                 ...
                                 sizeof(cl_int),
                                 &offset_y_,&status);
  return check_val(status,CL_SUCCESS,"clCreateBuffer (offset_y_) failed.")==SDK_SUCCESS;
}
...
bool boxm_re_render_ocl_scene_manager::release_offset_buffers()
{
  ...
  return check_val(status,CL_SUCCESS,"clReleaseMemObject failed (offset_y_buf_).")==SDK_SUCCESS;
}
