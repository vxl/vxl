...
bool boxm_update_ocl_scene_manager::refine()
{
  ...
  for (int i=0; i<numbuffer_; i++) {
    ...
    int freeSpace = (startPtr >= endPtr)? startPtr-endPtr : lenbuffer_ - (endPtr-startPtr);
...
bool boxm_update_ocl_scene_manager::release_mem_ptrs_buffers()
{
  ...
  return check_val(status,CL_SUCCESS,"clReleaseMemObject failed (output_debug_buf_).")==1;
}
...
bool boxm_update_ocl_scene_manager::release_root_level_buffers()
{
  ...
  return check_val(status,CL_SUCCESS,"clReleaseMemObject failed (root_level_buf_).");
}
