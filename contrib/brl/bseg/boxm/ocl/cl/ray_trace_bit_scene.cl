                        ...
                        __constant uchar  * bit_lookup)    // input image and store vis_inf and pre_inf
{
  ...
  int4 curr_block_index = convert_int4( (entry_pt - info->origin)/block_len);
  ...
  while (!(any(curr_block_index<(int4)0) || any(curr_block_index>=(scenedims))))
  {
    ...
    float4 local_ray_o = (ray_o - info->origin)/block_len - convert_float4(curr_block_index);
    
    //entry point in local block coordinates (need to ensure that this point is actually
    //between (0,1) for xyz
    float4 block_entry_pt = (entry_pt - info->origin)/block_len - convert_float4(curr_block_index);
    ...
    //--------------------------------------------------------------------------
    // block bounding box
    cell_min = block_len * convert_float4(curr_block_index) + info->origin;
    cell_len = block_len;
    if (!intersect_cell(ray_o, ray_d, cell_min, (float4) cell_len, &tnear, &tfar))
    {
        ...
        if (tfar-tnear < block_len/100)
        {
            ...
            curr_block_index   = convert_int4((entry_pt-info->origin)/block_len);
            curr_block_index.w = 0;
        }
    ...
    else
    {
        ...
        if (any(-1*(isless(fabs(entry_pt-cell_min+cell_len),(float4)block_len/100.0f)*isless(fabs(ray_d),(float4)1e-3))))
            break;
        curr_block_index   = convert_int4(floor((entry_pt+(block_len/20.0f)*ray_d-info->origin)/block_len));
        curr_block_index.w = 0;
    }
  }
#ifdef DEPTH
