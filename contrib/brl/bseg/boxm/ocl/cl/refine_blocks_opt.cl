...
void push(ocl_stack *stack, int a) { stack->data[stack->top] = a; stack->top++; }
int pop(ocl_stack *stack) { int buff = stack->data[stack->top-1]; stack->top--; return buff; }
...
///////////////////////////////////////////////////////////////////////
void swap_eight(__local int2 *tree, int a, int b, __global float* output)
{
  ...
  for (int i=0; i<8; i++) {
    int childA = get_child_ptr(tree[a+i]);
    int childB = get_child_ptr(tree[b+i]);
...
///////////////////////////////////////////
void reformat_tree(__local int2 *tree, __global float* output)
{
  unsigned gid = get_group_id(0);
  ...
  while (!empty(&open)){
    ...
    int child_ptr = get_child_ptr(tree[currNode].y);
...
/////////////////////////////////////////////////////////////////
int refine_tree(__local  int2    *tree,
                         int      tree_size,
                         ...
                         int      block_index)
{
  ...
  while (!empty(&open)){
    ...
    int child_ptr = get_child_ptr(tree[currNode]);
    if (child_ptr < 0)
    {
      //////////////////////////////////////////////////
      ...
      int dataIndex   = get_data_ptr(tree[currNode]);
      float  alpha    = alpha_array[gid*len_buffer + dataIndex];
      float4 nobs     = convert_float4(num_obs_array[gid*len_buffer + dataIndex]);
      float8 mixture  = convert_float8(mixture_array[gid*len_buffer + dataIndex]);
      ...
      if (alpha_int > max_alpha_int && currLevel < maxLevel-1)
      {
        ...
        int2 cell = tree[currNode];
        set_child_ptr(&cell, tree_size);
        tree[currNode] = cell;

        //each child points to the currNode, has no children,
        for (int i=0; i<8; i++) {
          int4 tcell = (int4) (currNode, -1, (int)data_size+i, 0);
          tree[tree_size+i] = pack_cell(tcell);

          if (data_size+i < len_buffer) {
            int newDataIndex = gid*len_buffer + (data_size+i);
            alpha_array[newDataIndex]   = new_alpha;
            mixture_array[newDataIndex] = (uchar8) (0);
            num_obs_array[newDataIndex] = (ushort4) (0);
          }
        }
  ...
  ///////////////////////////////////////////////////////////////////
  ////REFORMAT TREE into canonical order
  ///////////////////////////////////////////////////////////////////
...
refine_main(__global  int4     *block_ptrs,     //3d block array
            ...
            __global  int2     *tree_cells,     //tree_cells
            __global  float    *alpha_array,
            __global  uchar8   *mixture_array,
            __global  ushort4  *num_obs_array,
            ...
            __local   int2      *local_tree,    //local copy of the tree
