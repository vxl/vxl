// This is brl/bseg/boxm/ocl/pro/processes/boxm_ocl_ray_bundle_expected_process.cxx
...
namespace boxm_ocl_ray_bundle_expected_process_globals
{
  const unsigned n_inputs_ = 5;
  const unsigned n_outputs_ = 2;
}

bool boxm_ocl_ray_bundle_expected_process_cons(bprb_func_process& pro)
{
  using namespace boxm_ocl_ray_bundle_expected_process_globals;
...
bool boxm_ocl_ray_bundle_expected_process(bprb_func_process& pro)
{
  using namespace boxm_ocl_ray_bundle_expected_process_globals;
  ...
  switch (scene_ptr->appearence_model())
  {
    case BOXM_APM_MOG_GREY:
    {
      ...
      if (!scene_ptr->multi_bin())
      {
        ...
        boxm_opencl_ray_bundle_expected<BOXM_APM_MOG_GREY>(*scene, camera, expected, mask, use_black_background);
      }
      else
      {
    ...
    case BOXM_APM_SIMPLE_GREY:
    {
      ...
      if (!scene_ptr->multi_bin())
      {
        ...
        boxm_scene<type>* scene = dynamic_cast<boxm_scene<type>*> (scene_ptr.as_pointer());
        boxm_opencl_ray_bundle_expected<BOXM_APM_SIMPLE_GREY>(*scene, camera, expected, mask, use_black_background);
      }
      else
      {
        vcl_cout<<"OpenCL rendering of multi-bin scenes not implemented"<<vcl_endl;
    ...
    default:
    {
      vcl_cout << "boxm_ocl_ray_bundle_expected_process: unsupported APM type" << vcl_endl;
      return false;
    }
  }
