                      ...
                      __global uint    * gl_image)    // input image and store vis_inf and pre_inf
{
  ...
  while (true)
  {
    ...
    while (true)
    {
      ...
      //// exit point
      exit_pt=local_ray_o + tfar*ray_d;exit_pt.w=0.5;
                      ...
                      __global uint  * gl_image)    // input image and store vis_inf and pre_inf
{
  ...
  gl_image[j*get_global_size(0)+i]=rgbaFloatToInt(data_return);
}
...
//__kernel
//void
//ray_trace_single_block( __global int4 * scene_dims,  // level of the root.
//                      __global float4 * scene_origin,
//                      __global float4 * block_dims,
//                      __global int * root_level,
//                      __global int4    * tree_array,
...
//
//  //// combine the following into one
//  float cellsize=1/(float)(1<<rootlevel);
//
//  int count=0;
//  while (true)
//  {
...
//}
//
#if 0 //: removing a while loop but it is way slower
__kernel
void
ray_trace_all_blocks( __global int4 * scene_dims,  // level of the root.
                      __global float4 * scene_origin,
                      ...
                      //__global uint * gl_image) // input image and store vis_inf and pre_inf
{
  ...
  int4 scenedims=(int4)(*scene_dims).xyzw;
  scenedims.w=1;
  blockdims.w=1; // for safety purposes.
  ...
  cell_max=blockdims*convert_float4(scenedims)+origin;
  int hit = intersect_cell(ray_o, ray_d, cell_min, cell_max,&tnear, &tfar);
  if (!hit || (tfar-tnear)<(cellsize*blockdims.x/10.0f))
    return;

  entry_pt=ray_o + tnear*ray_d;

  int4 curr_block_index=convert_int4((entry_pt-origin)/blockdims);
  // handling the border case;
  curr_block_index=curr_block_index+(curr_block_index==scenedims);
  //// combine the following into one
  int count=0;
  float4 debug_var=(float4)0.0f;
  short4 exit_face=(short4)(-1);
  short4 root = (short4)(0,0,0,rootlevel);
  ...
  while (true)
  {
      ...
      if(!hit) 
      {
          // went outside the block
          int4 temp=fabs(exit_pt)<(float4)cellsize/10;
          if(any(temp))
          {
              //int4 temp; 
              //temp.x=fabs(exit_pt.x)<cellsize/10?1:0;
              //temp.y=fabs(exit_pt.y)<cellsize/10?1:0;
              //temp.z=fabs(exit_pt.z)<cellsize/10?1:0;
              exit_face=convert_short4(temp*-1);
              temp.w=0;  
              new_block=1;
          }
          temp=fabs(exit_pt-(float4)1.0f)<(float4)cellsize/10;
          if(any(temp))
          {
              //int4 temp;
              //temp.x=fabs(exit_pt.x-1.0f)<cellsize/10?1:0;
              //temp.y=fabs(exit_pt.y-1.0f)<cellsize/10?1:0;
              //temp.z=fabs(exit_pt.z-1.0f)<cellsize/10?1:0;
              exit_face=convert_short4(temp*-1); temp.w=1;  
              new_block=1;
          }
          if(new_block==0)
              break;
      ...
      if(hit)
      {
          ...
          else
          {
              ...
              // required exit location code
              short4 exit_loc_code = loc_code(exit_pt, rootlevel);       
              //debug_var=(float4)neighbor_ptr-curr_cell_ptr;

              // the current cell (cells[curr_cell_ptr])is the cell reached by
              curr_cell_ptr = traverse_force(tree_array, neighbor_ptr, neighbor_code,exit_loc_code, &curr_loc_code,&global_count);
              //debug_var=(float4)count;//convert_float4(curr_loc_code);
    ...
    {
        //: at the block level
        exit_pt=(exit_pt+convert_float4(curr_block_index))*blockdims+origin;
        ...
        if (any(curr_block_index<(int4)0)|| any(curr_block_index>=(scenedims)))
            break;
        //: get the root_ptr  for the new block
        root_ptr = block_ptrs[curr_block_index.z*scenedims.x*scenedims.y
                             +curr_block_index.y*scenedims.x
                             +curr_block_index.x];

        block_origin=blockdims*convert_float4(curr_block_index)+origin;
        local_ray_o= (ray_o-block_origin)/blockdims;
        root_cell=tree_array[root_ptr];

        // canonincal bounding box of the tree
        cell_bounding_box(root, rootlevel+1, &cell_min, &cell_max);
        //debug_var=convert_float4(curr_block_index);

        if (!cell_entry_point(local_ray_o, ray_d, cell_min, cell_max, &entry_pt))
            break;
        entry_loc_code = loc_code(entry_pt, rootlevel);
        curr_loc_code=(short4)-1;
        curr_cell_ptr = traverse_force(tree_array, root_ptr, root, entry_loc_code,&curr_loc_code,&global_count);
        new_block=0;
    }
  }
  ...
  data_return.z+=(1-data_return.w)*0.5f;
   ...
   //also it is not necessary to have a full float4 as the
   //output type a single scalar float array is sufficient
  in_image[j*get_global_size(0)+i]=(float4)data_return;//convert_float4(curr_block_index);//
}
#endif // 0