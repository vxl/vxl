...
__kernel
void
test_intersect_cell(__global int4* cells, __global float2* cell_data,
                    __global int4* results)
{
  short4 code, ncode;
  int n_codes = 0;
    ...
    while (count<1000) {
      ...
      if (exit_face.x<0)
      {
        ...
        curr_cell_ptr = traverse(cells, root_ptr, root,
                                 exit_loc_code, &curr_loc_code,&global_count);
#if 0
        results[result_ptr++]=convert_int4(curr_loc_code);
#endif
      }
        ...
        int neighbor_ptr =  neighbor(cells, curr_cell_ptr, curr_loc_code,
                                     exit_face, n_levels, &neighbor_code,&global_count);
        if (neighbor_ptr<0)
          break;
