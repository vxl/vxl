...
//: Constructor
boxm_ocl_rerender_tableau::boxm_ocl_rerender_tableau()
{
    pbuffer_=0;
    ni_=640;
    nj_=480;
                                        ...
                                        vpgl_perspective_camera<double> * ext_cam)
{
  ...
  ext_cam_=new vpgl_perspective_camera<double>();
  (*ext_cam_)=(*ext_cam);

  ext_image_=ext_image;

  do_init_ocl_rerender = true;
  return true;
}

bool boxm_ocl_rerender_tableau::init_ocl()
{
  GLenum err = glewInit();
  if (GLEW_OK != err)
  {
...
//  initialize ray trace using the input camera
  ...
  good = good && ray_mgr->set_external_image_cam_buffers(ext_image_,ext_cam_);
  // delete old buffer
  if (pbuffer_) {
...
bool boxm_ocl_rerender_tableau::handle(vgui_event const &e)
{
...
float boxm_ocl_rerender_tableau::render_frame()
{
