#ifndef boxm_opt_rt_sample_txx_
#define boxm_opt_rt_sample_txx_

#include "boxm_opt_rt_sample.h"


template <class OBS_T>
void boxm_opt_rt_sample<OBS_T>::print(vcl_ostream& os) const
{
...
void vsl_b_read(vsl_b_istream & is, boxm_opt_rt_sample<T> &sample)
{
  ...
  switch (version)
  {
    case 1:
      ...
      break;
    default:
      vcl_cerr << "I/O ERROR: vsl_b_read(vsl_b_istream&, boxm_opt_rt_sample<T>&)\n"
...
#define BOXM_OPT_RT_SAMPLE_INSTANTIATE(T) \
template class boxm_opt_rt_sample<T >; \
template void vsl_b_write(vsl_b_ostream &, boxm_opt_rt_sample<T > const &); \
template void vsl_b_write(vsl_b_ostream &, boxm_opt_rt_sample<T > const *&); \
template void vsl_b_read(vsl_b_istream &, boxm_opt_rt_sample<T > &); \
template void vsl_b_read(vsl_b_istream &, boxm_opt_rt_sample<T > *&); \
template vcl_ostream& operator << (vcl_ostream&, const boxm_opt_rt_sample<T >&)
