...
public:
                        ...
                        boxm_sample<APM> &cell_value, T_aux & aux_val)
  {
    ...
    const float seg_len = (float)(s1 - s0).length();
    //: compute avergae intensity for the cell    
...
public:
  ...
  boxm_update_image_functor_pass_2(vil_image_view<typename boxm_apm_traits<APM>::obs_datatype> const& image,
    ...
    : obs_(image), norm_(normalizing_img), pre_img_(image.ni(), image.nj(), 1), vis_img_(image.ni(), image.nj(), 1), alpha_integral_(image.ni(), image.nj(), 1)
  {
    alpha_integral_.fill(0.0f);
    pre_img_.fill(0.0f);
    vis_img_.fill(1.0f);
     ...
     raytracer_1.run(pass_1_functor);
       ...
       vil_image_view<float> PI_inf(obs.ni(), obs.nj(),1);
         ...
         if (black_background) {
             ...
             vil_image_view<typename T_data::obs_datatype>::const_iterator img_it = obs.begin();
       ...
       vil_math_image_sum(pre_inf,inf_term,norm_img);
