...
bool boxm_ray_trace_manager::setup_rays()
{
  ...
  vcl_size_t nrows = ray_origin_input_.rows(), ncols = ray_origin_input_.cols();
  if (nrows != ray_dir_input_.rows() ||
      ncols != ray_dir_input_.cols()) {
    vcl_cout << "In boxm_ray_trace_manager::setup_rays()"
             << " - inconsistent array size\n";
    return false;
  }
  vcl_size_t nrays = nrows*ncols;
  ray_origin_ = NULL;
  ray_dir_ = NULL;
#if defined (_WIN32)
  ray_origin_ =
...
int boxm_ray_trace_manager::setup_ray_input_buffers()
{
  ...
  cl_int status = CL_SUCCESS;
  // Create and initialize memory objects
  ray_origin_buf_ = clCreateBuffer(context_,
                                   CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
  ...
  ray_dir_buf_ = clCreateBuffer(context_,
                                CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                nrays * sizeof(cl_float4),
                                ray_dir_,
                                &status);
...
vcl_size_t boxm_ray_trace_manager::n_ray_groups()
{
