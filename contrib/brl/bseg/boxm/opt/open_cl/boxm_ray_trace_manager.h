...
class boxm_ray_trace_manager : public boxm_opencl_manager<boxm_ray_trace_manager>
{
 ...
 public:
  ...
  size_t cell_array_size(){return cell_input_.size();}
  size_t cell_data_array_size(){return data_input_.size();}

  size_t tree_result_size() {return 2*cell_input_.size();}
  cl_int* tree_results(){return tree_results_;}
  void clear_tree_results();

  size_t n_rays(){return ray_dir_input_.rows()*ray_dir_input_.cols();}
  size_t ray_rows(){return ray_dir_input_.rows();}
  size_t ray_cols(){return ray_dir_input_.cols();}
  cl_float* ray_results(){return ray_results_;}
  void clear_ray_results();
  size_t n_ray_groups();

  vcl_string program_source(){return prog_;}
  cl_program program(){return program_;}

  void print_tree_input();
  void print_ray_input();
  void print_tree_results(){}//later
