...
bool cube_fill_value(double* xverts_2d, double* yverts_2d, boct_face_idx visible_faces, vil_image_view<float> & img,
                     float const& val);

bool boxm_alpha_seg_len(double * xverts_2d, double* yverts_2d, float* vert_distances, boct_face_idx visible_faces,
                        float alpha, vil_image_view<float> & alpha_distance);

bool boxm_depth_fill(double * xverts_2d, double* yverts_2d, float* vert_distances, boct_face_idx visible_faces,
                     vil_image_view<float> & depth_image);

template <class T, class tri_int_it_t>
bool tri_interpolated_weighted_sum(tri_int_it_t & tri_it, vil_image_view<T> const& values, T & val_sum,
                                   float & weight_sum, bool subtract)
{
  tri_it.reset();
  while( tri_it.next() )
    {
...
bool tri_interpolated_weighted_sum(tri_it_t & tri_it, vil_image_view<T1> const& weights,
                                   vil_image_view<T2> const& values, T2 & val_sum, T1 & weight_sum,
                                   bool subtract = false)
{
  tri_it.reset();
  while( tri_it.next() )
...
#if 0
template <class T, class tri_it_T>
void tri_interpolate_values(tri_it_T & tri_it, double* xvals, double* yvals, T* vals, vil_image_view<T> & img,
                            vil_image_view<float> & pix_coverage, unsigned int v0 = 0, unsigned int v1 = 1,
                            unsigned int v2 = 2)
{
...
template <class T, class tri_it_T>
void tri_interpolate_min_max(tri_it_T & tri_it, double* xvals, double* yvals, T* vals, vil_image_view<T> & min_img,
                             vil_image_view<T> & max_img, unsigned int v0 = 0, unsigned int v1 = 1, unsigned int v2 = 2)
{
  // first compute s0, s1, s2 such that  val = s0*x + s1*y + s2 for any point within the triangle
  // (no need to compute barycentric coordinates at each step)
  // subtract 0.5 from xvals and yvals, so that interpolated value at pixel x,y evaluates to coordinates x+0.5, y+0.5 (center of pixel)
...
void tri_weighted_sum(vgl_triangle_scan_iterator<double> & tri_it, vil_image_view<T1> & img,
                      vil_image_view<T2> & weights, T1 & val_sum, T2 & weight_sum)
{
  tri_it.reset();
  while( tri_it.next() )
    {
...
template <class T>
void tri_fill_value_aa(boxm_triangle_scan_iterator_aa & tri_it, vil_image_view<T> & img,
                       vil_image_view<float> & aa_weights, T val)
{
  tri_it.reset();
  while( tri_it.next() )
    {
...
template <class T>
bool cube_mean(double* xverts_2d, double* yverts_2d, float* vert_dists, boct_face_idx visible_faces,
               vil_image_view<T> const& values, T & mean)
{
...
inline void init_triangle_scan_iterator(vgl_triangle_scan_iterator<double> & tri_it, const double* xverts,
                                        const double* yverts, unsigned int v0 = 0, unsigned int v1 = 1,
                                        unsigned int v2 = 2)
{
