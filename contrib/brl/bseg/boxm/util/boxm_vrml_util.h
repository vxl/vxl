...
public:
  ...
  static void write_vrml_scene_as_spheres(vcl_ofstream& str, bvxm_voxel_scene<vnl_float_4> * scene, float threshold,
                                          int s = 1);

  static void write_vrml_scene_as_pointers(vcl_ofstream& str, bvxm_voxel_scene<vnl_float_4> * scene, float threshold,
                                           int s = 1);
...
template <class T_data>
void boxm_vrml_util::write_vrml_scene(vcl_ofstream& str, boxm_scene<boct_tree<short, T_data> > * scene,
                                      float /*threshold*/)
{
...
#if 0 // not yet working?
void bvxm_vrml_voxel_grid::write_vrml_grid(vcl_ofstream& str, bvxm_voxel_grid<bsta_num_obs<bsta_gauss_sf1> > * grid,
                                           float threshold)
{
      ...
      for( unsigned j = 0; j < (*grid_it).ny(); ++j )
        ...
        if( ( ( (*grid_it)(i, j) ).mean() > threshold) )
          ...
          str << "        "
              << ( (*grid_it)(i,
                          j) ).mean() << ' '
              << ( (*grid_it)(i, j) ).mean() << ' ' << ( (*grid_it)(i, j) ).mean() << '\n';
          }
        }
      }
    }
...
void bvxm_vrml_voxel_grid::write_vrml_grid_as_spheres(vcl_ofstream& str, bvxm_voxel_grid<vnl_float_4> * grid,
                                                      float threshold, int s)
{
...
void bvxm_vrml_voxel_grid::write_vrml_grid_as_pointers(vcl_ofstream& str, bvxm_voxel_grid<vnl_float_4> * grid,
                                                       float threshold, int s)
{
  bvxm_voxel_grid<vnl_float_4>::iterator grid_it = grid->begin();
