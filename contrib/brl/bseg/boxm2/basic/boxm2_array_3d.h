...
public:
  ...
  boxm2_array_3d(size_type n1, size_type n2, size_type n3, T* buffer) : buffer_(NULL), element_(NULL), row1_count_(0),
    row2_count_(0), row3_count_(0)
  {
    construct(n1, n2, n3, buffer);
  }

  boxm2_array_3d(size_type n1, size_type n2, size_type n3, T* buffer, T const& fill_value) : buffer_(NULL), element_(
      NULL), row1_count_(0), row2_count_(0), row3_count_(0)
  {
    construct(n1, n2, n3, buffer);
    fill(fill_value);
  }
...
VCL_TEMPLATE_EXPORT template <class T>
vcl_ostream & operator<<(vcl_ostream &, boxm2_array_3d<T> const &);

VCL_TEMPLATE_EXPORT template <class T>
vcl_istream & operator>>(vcl_istream &, boxm2_array_3d<T> &);
