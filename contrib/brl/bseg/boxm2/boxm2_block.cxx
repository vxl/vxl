                           ...
                           vgl_point_3d<double>& cell_center, double& side_length) const
{
  ...
  if( index_x >= this->trees_.get_row1_count() || index_y >= this->trees_.get_row2_count() ||
      index_z >= this->trees_.get_row3_count() )
    {
    return false;
    }
...
bool boxm2_block::data_index(vgl_point_3d<double> const& global_pt, unsigned& index, unsigned& depth,
...
vcl_vector<vgl_point_3d<double> > boxm2_block::sub_block_neighbors(vgl_point_3d<double> const& probe,
...
void boxm2_block::leaf_neighbors(vgl_point_3d<double> const& probe, double distance,
                                 vcl_vector<vgl_point_3d<double> >& nbrs, vcl_vector<double>& nbr_edge_lengths,
                                 vcl_vector<unsigned>& data_indices, bool relative_distance) const
{
  nbrs.clear();
  ...
  double                dx = v0.x() / sub_block_dim_.x(), dy = v0.y() / sub_block_dim_.y(),
    dz = v0.z() / sub_block_dim_.z();
  int ix = static_cast<int>(vcl_floor(dx) ), iy = static_cast<int>(vcl_floor(dy) ),
    iz = static_cast<int>(vcl_floor(dz) );
  ...
  for( int z = iz - dri; z <= iz + dri; ++z )
    ...
    for( int y = iy - dri; y <= iy + dri; ++y )
      ...
      for( int x = ix - dri; x <= ix + dri; ++x )
        {
        // check bounds
        if( x < int(0) || y < int(0) || z < int(0) || x >= int(sub_block_num_.x() ) || y >= int(sub_block_num_.y() ) ||
            z >= int(sub_block_num_.z() ) )
          {
          continue;
          }
        ...
        for( vcl_vector<int>::iterator iter = leafBits.begin(); iter != leafBits.end(); ++iter )
          ...
          vgl_vector_3d<double> cell_offset(cell_pos.x() * sub_block_dim_.x(),
                                            cell_pos.y() * sub_block_dim_.y(), cell_pos.z() * sub_block_dim_.z() );
...
vcl_vector<vgl_point_3d<int> >  boxm2_block::sub_blocks_intersect_box(vgl_box_3d<double> const& box) const
{
  ...
  int                            nx = static_cast<int>(sub_block_num_.x() ), ny = static_cast<int>(sub_block_num_.y() ),
    nz = static_cast<int>(sub_block_num_.z() );
  vgl_point_3d<double>  minp = box.min_point();
  vgl_vector_3d<double> vmin = (minp - local_origin_);
  double                x_min = vmin.x() / sub_block_dim_.x(), y_min = vmin.y() / sub_block_dim_.y(),
    z_min = vmin.z() / sub_block_dim_.z();
  int ix_min = static_cast<int>(vcl_floor(x_min) ),
    iy_min = static_cast<int>(vcl_floor(y_min) ), iz_min = static_cast<int>(vcl_floor(z_min) );
  ...
  double                x_max = vmax.x() / sub_block_dim_.x(), y_max = vmax.y() / sub_block_dim_.y(),
    z_max = vmax.z() / sub_block_dim_.z();
  int ix_max = static_cast<int>(vcl_floor(x_max) ), iy_max = static_cast<int>(vcl_floor(y_max) ),
    iz_max = static_cast<int>(vcl_floor(z_max) );
...
{
  return s << "Block ID=" << block.block_id() << '\n'
