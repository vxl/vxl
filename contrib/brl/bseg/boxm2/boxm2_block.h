...
// this way, n_cells_ will always remain up to date.
  ...
  {
    if( that.get_row1_count() != this->trees_.get_row1_count() ||
        that.get_row2_count() != this->trees_.get_row2_count() ||
        that.get_row3_count() != this->trees_.get_row3_count() )
      {
  ...
  boxm2_array_3d<uchar16> trees_copy()
  {
    ...
    return boxm2_array_3d<uchar16>(trees_.get_row1_count(), trees_.get_row2_count(), trees_.get_row3_count(),
                                   copy_buff);
  ...
  void leaf_neighbors(vgl_point_3d<double> const& probe, double distance, vcl_vector<vgl_point_3d<double> >& nbrs,
                      vcl_vector<double>& nbr_edge_lengths, vcl_vector<unsigned>& data_indices,
                      bool relative_distance = true) const;
