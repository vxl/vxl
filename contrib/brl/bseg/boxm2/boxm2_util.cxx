...
vil_image_view_base_sptr boxm2_util::prepare_input_image(vil_image_view_base_sptr loaded_image, bool force_grey)
{
  ...
  if( loaded_image->nplanes() == 1 )
    ...
    else if( vil_image_view<unsigned short> * img_byte =
               dynamic_cast<vil_image_view<unsigned short> *>(loaded_image.ptr() ) )
      {
      floatimg = new vil_image_view<float>(loaded_image->ni(), loaded_image->nj(), 1);
...
bool boxm2_util::verify_appearance(boxm2_scene& scene, const vcl_vector<vcl_string>& valid_types, vcl_string& data_type,
                                   int& appTypeSize )
{
                             ...
                             float& prob, float& intensity)
{
  ...
  boxm2_data_base * int_base  =
    cache->get_data_base(scene, id, boxm2_data_traits<BOXM2_MOG3_GREY>::prefix() );
...
boxm2_util::blocks_along_a_ray(boxm2_scene_sptr scene, vgl_point_3d<double> p0, vgl_point_3d<double> p1)
{
  ...
  while( t < length )
    ...
    t =
      vcl_min(vcl_min( (max_facex - p0.x() ) * (1.0 / ray_dx), (max_facey - p0.y() ) * (1.0 / ray_dy) ),
              (max_facez - p0.z() ) * (1.0 / ray_dz) ) + 1.0;
