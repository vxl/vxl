...
public:
  ...
  static vbl_array_2d<vil_image_view<vxl_byte> *>  render_upper_hemisphere(int num_az, int num_incline, double radius,
  ...
  static vpgl_perspective_camera<double> * construct_camera(double elevation, double azimuth, double radius,
                                                            unsigned ni, unsigned nj, vgl_box_3d<double> bb,
                                                            bool fit_bb = true);

  static int find_nearest_cam(vgl_vector_3d<double>& normal, vcl_vector<vpgl_perspective_camera<double> *>& cams);
  ...
  static bool verify_appearance(boxm2_scene& scene, const vcl_vector<vcl_string>& valid_types, vcl_string& data_type,
                                int& appTypeSize );

  static bool get_point_index(boxm2_scene_sptr& scene, boxm2_cache_sptr& cache, const vgl_point_3d<double>& point,
                              boxm2_block_id& bid, int& data_index, float& side_len);

  static bool query_point(boxm2_scene_sptr& scene, boxm2_cache_sptr& cache, const vgl_point_3d<double>& point,
  ...
  static vcl_vector<boxm2_block_id> order_about_a_block(boxm2_scene_sptr scene, boxm2_block_id curr_block,
                                                        double distance = vcl_numeric_limits<double>::max() );

  // : get blocks along a ray
  static vcl_vector<boxm2_block_id> blocks_along_a_ray(boxm2_scene_sptr scene, vgl_point_3d<double> p0,
