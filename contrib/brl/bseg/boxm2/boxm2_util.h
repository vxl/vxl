...
class boxm2_util
{
  public:
    ...
    static vpgl_camera_double_sptr camera_from_file(std::string camfile);
    ...
    static std::vector<std::string> files_from_dir(std::string dir, std::string ext);
    ...
    static bool copy_file(std::string file, std::string dest);

    //Will be used to specify rows and cols for JavaScript App
    static bool generate_html(int width, int height, int nrows, int ncols, std::string dest);
    static bool generate_jsfunc(vbl_array_2d<std::string> img_files, std::string dest);
    ...
    static std::vector<boxm2_block_id> order_about_a_block(boxm2_scene_sptr scene, boxm2_block_id curr_block, double distance = std::numeric_limits<double>::max());

    //: get blocks along a ray
    static std::vector<boxm2_block_id> blocks_along_a_ray(boxm2_scene_sptr scene, vgl_point_3d<double> p0, vgl_point_3d<double> p1);

    static bool get_raydirs_tfinal(std::string depthdir, std::string camsfile, vgl_point_3d<double> origin,
                                   std::vector<vil_image_view<float>*> & raydirs,
                                   std::vector<vil_image_view<float>*> & tfinal,int scale);

    static bool write_blocks_to_kml(boxm2_scene_sptr& scene, std::string kml_file, std::vector<boxm2_block_id> blks);
};
