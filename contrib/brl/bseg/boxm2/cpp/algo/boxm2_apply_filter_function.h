...
public:
  ...
  void apply_filter(boxm2_block_metadata data, boxm2_block* blk, boxm2_data_base* alphas, boxm2_data_base* responses,
                    float prob_threshold, unsigned octree_lvl);
...
private:
  ...
  float eval_alpha(boxm2_block_metadata data, const boct_bit_tree& bit_tree, const vgl_point_3d<double> & point,
                   const boxm2_array_3d<uchar16>& trees, const boxm2_data_traits<BOXM2_ALPHA>::datatype* alpha_data,
                   int curr_depth);

  // : evaluate a filter in a given neighborhood
  float eval_filter(vcl_map<vnl_vector_fixed<int, 3>, vgl_point_3d<double> > neighbors, boxm2_block_metadata data,
                    const boct_bit_tree& bit_tree,  const boxm2_array_3d<uchar16>& trees,
