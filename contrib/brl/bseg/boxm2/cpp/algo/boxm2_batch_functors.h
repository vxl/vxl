...
class boxm2_batch_update_pass1_functor
{
 public:
  ...
  bool init_data(vcl_vector<boxm2_data_base*> & datas, vil_image_view<float>* pre_img, vil_image_view<float>* vis_img)
  {
    ...
    // was:   =new boxm2_data<BOXM2_MOG3_GREY>(datas[3]->data_buffer(),datas[3]->buffer_length(),datas[3]->block_id());
...
class boxm2_batch_update_functor
{
 public:
  ...
  bool init_data(boxm2_data_base *alph, boxm2_data_base *mog, boxm2_stream_cache_sptr str_cache, bsta_sigma_normalizer_sptr n_table)
  {
    ...
    // was:   = new boxm2_data<BOXM2_MOG3_GREY>(mog->data_buffer(), mog->buffer_length(), mog->block_id());
  ...
  inline bool process_cell(int index)
  {
    ...
    for (unsigned m = 0; m < nimgs; m++) {
      ...
      // was:  = boxm2_data_traits<BOXM2_MOG3_GREY>::processor::prob_density(mog, mean_obs);
      ...
      if (index == cell_no) {
        vcl_cout << "\t m: " << m << " pre_i: " << pre_i << " vis_i: " << vis_i << " post_i: " << post_i << '\n'
                 ...
                 << "current term1: " << term1 << " term2: " << term2 << "\n";
      }
    }
    ...
    alpha *= p_q_new; // was: alpha = -vcl_log(1.0f-p_q_new)/max_obs_seg_len;
    ...
    // was: boxm2_data_traits<BOXM2_MOG3_GREY>::processor::compute_gauss_mixture_3(mog,obs, vis,n_table_,0.03f);
 ...
 private:
  boxm2_data<BOXM2_ALPHA>* alpha_data_;
  boxm2_data<APM_TYPE>* mog_data_; // was: boxm2_data<BOXM2_MOG3_GREY>* mog_data_;
