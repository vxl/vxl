                             ...
                             F functor, float tfar_max = -1.0f)
{
  ...
  float tfar =
    vcl_min(vcl_min( (max_facex - ray_ox) * (1.0f / ray_dx),
                     (max_facey - ray_oy) * (1.0f / ray_dy) ), (max_facez - ray_oz) * (1.0f / ray_dz) );
  ...
  float tblock =
    vcl_max(vcl_max( (min_facex - ray_ox) * (1.0f / ray_dx),
                     (min_facey - ray_oy) * (1.0f / ray_dy) ), (min_facez - ray_oz) * (1.0f / ray_dz) );
  ...
  while( tblock < tfar )
    ...
    float texit =
      vcl_min(vcl_min( (cell_minx - ray_ox) * (1.0f / ray_dx), (cell_miny - ray_oy) * (1.0f / ray_dy) ),
              (cell_minz - ray_oz) * (1.0f / ray_dz) );
    if( texit <= tblock )
      {
      break;                    // need this check to make sure the ray is progressing
    ...
    while( ttree < texit )
      ...
      float t1 =
        vcl_min(vcl_min( (cell_minx - lrayx) * (1.0f / ray_dx), (cell_miny - lrayy) * (1.0f / ray_dy) ),
                (cell_minz - lrayz) * (1.0f / ray_dz) );
