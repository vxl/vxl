...
public:
  ...
  bool init_data(vcl_vector<boxm2_data_base *> & datas, vil_image_view<float> * in_img,
                 vil_image_view<float> * expected, vil_image_view<float>* vis_img)
  {
    alpha_data_ =
      new boxm2_data<BOXM2_ALPHA>(datas[0]->data_buffer(), datas[0]->buffer_length(), datas[0]->block_id() );
    mog3_data_ = new boxm2_data<BOXM2_MOG3_GREY>(datas[1]->data_buffer(), datas[1]->buffer_length(),
                                                 datas[1]->block_id() );
...
public:
  ...
  {
    alpha_data_    = new boxm2_data<BOXM2_ALPHA>(datas[0]->data_buffer(), datas[0]->buffer_length(),
                                                 datas[0]->block_id() );
    mog3_data_     = new boxm2_data<BOXM2_MOG3_GREY>(datas[1]->data_buffer(),
  ...
  inline bool step_cell(float seg_len, int index, unsigned i, unsigned j, float abs_depth)
  {
    ...
    (*expected_img_)(i,
                     j) =
      (*expected_img_)(i, j) + boxm2_processor_type<BOXM2_MOG3_GREY>::type::expected_color(mog3) * curr_p;
    (*change_image_)(i,
                     j) =
      (*change_image_)(i,
                       j) + boxm2_processor_type<BOXM2_MOG3_GREY>::type::prob_density(mog3, (*in_img_)(i, j) ) * curr_p;
    // : representing 8 bytes of data aby double
    unsigned        img_index = j * ni_ + i;
    unsigned char * exp_distribution_array =
      reinterpret_cast<unsigned char *>(dist_image_->top_left_ptr() + img_index);
    ...
    if( w2 <= 0.0f )
      ...
      {
      boxm2_processor_type<BOXM2_MOG3_GREY>::type::merge_mixtures(mog3, curr_p, exp_distribution, w2,
                                                                  updated_exp_distribution);
      }
#ifdef DEBUG
    vcl_cout << '[';
  ...
  void finish()
  {
    ...
    for( unsigned j = 0; j < nj_; j++ )
      ...
      for( unsigned i = 0; i < ni_; i++ )
        ...
        ++count;
        unsigned char * exp_distribution_array =
          reinterpret_cast<unsigned char *>(dist_image_->top_left_ptr() + count);
        vnl_vector_fixed<unsigned char, 8> exp_distribution(exp_distribution_array);
        float                              pr = boxm2_processor_type<BOXM2_MOG3_GREY>::type::prob_density(
            exp_distribution, expblur(i, j) );
