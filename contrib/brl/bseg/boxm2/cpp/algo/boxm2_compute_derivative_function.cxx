  ...
  float normal_threshold, vcl_string kernel_x_name, vcl_string kernel_y_name, vcl_string kernel_z_name)
{
  ...
  boxm2_data_traits<BOXM2_ALPHA>::datatype * alpha_data =
    (boxm2_data_traits<BOXM2_ALPHA>::datatype *)alphas->data_buffer();
    ...
    for( unsigned int y = 0; y < trees.get_row2_count(); ++y )
      ...
      for( unsigned int z = 0; z < trees.get_row3_count(); ++z )
        ...
        for( iter = leafBits.begin(); iter != leafBits.end(); ++iter )
          ...
          vcl_vector<vcl_pair<vgl_point_3d<int>, vgl_point_3d<double> > > neighbor_points = this->neighbor_points(
              cellCenter, side_len, trees, 5);
          if( neighbor_points.size() != kernel_x_.size() )
            {
            continue;
            }

          vcl_vector<vcl_pair<vgl_point_3d<int>, float> > neighbors = eval_neighbors(data, bit_tree, neighbor_points,
                                                                                     trees, alpha_data,  curr_depth);
...
#ifdef DEBUG
          vcl_cout << vcl_endl
                   << "the data origin is " << data.local_origin_.x() << ' ' << data.local_origin_.y() << ' '
                   << data.local_origin_.z() << ' ' << vcl_endl
                   << "the sub dim are "    << data.sub_block_dim_.x() << ' ' << data.sub_block_dim_.y() << ' '
                   << data.sub_block_dim_.z() << ' ' << vcl_endl
                   << "cell center is " << cellCenter.x() << ' ' << cellCenter.y() << ' ' << cellCenter.z() << ' '
                   << vcl_endl;
#endif
...
vcl_vector<vcl_pair<vgl_point_3d<int>, vgl_point_3d<double> > >  boxm2_compute_derivative_function::neighbor_points(
  const vgl_point_3d<double>& cellCenter, double side_len, const boxm2_array_3d<uchar16>& trees, int nhood_size )
{
    ...
    for( int j = -(nhood_size - 1) / 2; j <= (nhood_size - 1) / 2; j++ )
      ...
      for( int k = -(nhood_size - 1) / 2; k <= (nhood_size - 1) / 2; k++ )
        ...
        if( cellCenter.x() + i * side_len < trees.get_row1_count() &&
            ...
            && cellCenter.x() + i * side_len >= 0 && cellCenter.y() + j * side_len >= 0 &&
...
vcl_vector<vcl_pair<vgl_point_3d<int>, float> >  boxm2_compute_derivative_function::eval_neighbors(
  ...
  const vcl_vector<vcl_pair<vgl_point_3d<int>, vgl_point_3d<double> > > & neighbors,
  ...
  const boxm2_data_traits<BOXM2_ALPHA>::datatype* alpha_data, int curr_depth)
{
  ...
  for( unsigned int i = 0; i < neighbors.size(); ++i )
    ...
    if( neighborTree.is_leaf(neighborBitIdx) )
      ...
      float                                    prob = 1.0f
        - (float)vcl_exp(-alpha * side_len * data.sub_block_dim_.x() );
...
vcl_vector<vcl_pair<vgl_point_3d<int>, float> > boxm2_compute_derivative_function::load_kernel(vcl_string filename)
{
  ...
  while( true )
    ...
    filter.push_back(vcl_pair<vgl_point_3d<int>,
                              float>( vgl_point_3d<int>(int(this_loc.x() ), int(this_loc.y() ), int(this_loc.z() ) ),
                                      weight) );
#ifdef DEBUG
...
float boxm2_compute_derivative_function::apply_filter(vcl_vector<vcl_pair<vgl_point_3d<int>, float> > neighbors,
                                                      vcl_vector<vcl_pair<vgl_point_3d<int>, float> > filter)
{
  float sum = 0;
