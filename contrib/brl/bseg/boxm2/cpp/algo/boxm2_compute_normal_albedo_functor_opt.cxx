                                                        ...
                                                        boxm2_data_base * normal_albedo_model)
{
  ...
  for( unsigned int n = 0; n < num_normals_; ++n )
    ...
    for( unsigned int m = 0; m < num_images_; ++m )
      ...
      double var_offset = brad_radiance_variance_chavez(0.0, normals[n], metadata_[m], atm_params_[m], reflectance_var,
                                                        optical_depth_var, skylight_var, airlight_var);
      radiance_var_offsets_[n][m] = var_offset;
      // compute scale
      double var = brad_radiance_variance_chavez(1.0, normals[n], metadata_[m], atm_params_[m], reflectance_var,
                                                 optical_depth_var, skylight_var, airlight_var);
...
bool boxm2_compute_normal_albedo_functor_opt::process_cell(unsigned int index, bool is_leaf, float side_len)
{
  ...
  if( aux0_raw.size() != num_images_ )
    {
    vcl_cerr << "ERROR: boxm2_compute_normal_albedo_functor_opt : aux0_raw.size() " << aux0_raw.size()
             << " != num_images_ " << num_images_ << '\n';
    return false;
    }
  ...
  for( unsigned int n = 0; n < num_normals_; ++n )
    {
    boxm2_compute_normal_albedo_cost_function cost_fun(radiances, vis_vals, radiance_scales_[n], radiance_offsets_[n],
                                                       radiance_var_scales_[n], radiance_var_offsets_[n]);
