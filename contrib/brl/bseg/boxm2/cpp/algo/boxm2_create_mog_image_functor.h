...
public:
  ...
  bool init_data(vcl_vector<boxm2_data_base *> & datas,
                 vbl_array_2d<typename boxm2_data_traits<APM_TYPE>::datatype> * mog_img,
                 vil_image_view<float> * vis_img)
  {
    ...
    nobs_data_ =
      new boxm2_data<BOXM2_NUM_OBS>(datas[2]->data_buffer(), datas[2]->buffer_length(), datas[2]->block_id() );
  ...
  inline bool step_cell(float seg_len, int index, unsigned i, unsigned j, float abs_depth = 0.0)
  {
    ...
    nobs[0] = (unsigned short)nobs_float[0]; nobs[1] = (unsigned short)nobs_float[1];
