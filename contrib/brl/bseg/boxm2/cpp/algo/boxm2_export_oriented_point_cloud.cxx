...
bool boxm2_export_oriented_point_cloud::export_oriented_point_cloud(boxm2_scene_sptr scene, boxm2_cache_sptr cache,
                                                                    vcl_string output_filename, bool output_aux,
                                                                    float vis_t, float nmag_t, float prob_t,
                                                                    float exp_t,
                                                                    vcl_string bb_filename)
{
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    vcl_cout << "Processing Block: " << id << " with prob t: " << prob_t << ", vis t: " << vis_t << " and nmag_t: "
             << nmag_t << " finest cell length: " << finest_cell_length << vcl_endl;
    ...
    boxm2_data_base * points = cache->get_data_base(scene, id,
                                                    boxm2_data_traits<BOXM2_POINT>::prefix(),
                                                    data_buff_length * pointTypeSize);
    boxm2_data_base * normals = cache->get_data_base(scene, id,
                                                     boxm2_data_traits<BOXM2_NORMAL>::prefix(),
                                                     data_buff_length * normalTypeSize);
    boxm2_data_base * vis = cache->get_data_base(scene, id,
                                                 boxm2_data_traits<BOXM2_VIS_SCORE>::prefix(),
                                                 data_buff_length * visTypeSize);
    boxm2_data_base * vis_sum = cache->get_data_base(scene, id, boxm2_data_traits<BOXM2_VIS_SCORE>::prefix(
                                                       "sum"), data_buff_length * visTypeSize);
    boxm2_data_base * mog = cache->get_data_base(scene, id,
                                                 boxm2_data_traits<BOXM2_MOG3_GREY>::prefix(),
                                                 data_buff_length * mogSize);
    boxm2_data_base * exp = cache->get_data_base(scene, id,
                                                 boxm2_data_traits<BOXM2_EXPECTATION>::prefix(),
                                                 data_buff_length * expTypeSize);
    boxm2_data_base * nobs = cache->get_data_base(scene, id,
                                                  boxm2_data_traits<BOXM2_NUM_OBS_SINGLE_INT>::prefix(),
                                                  data_buff_length * nobsTypeSize);
    boxm2_data_base * ray_dir_sum = cache->get_data_base(scene, id,
                                                         boxm2_data_traits<BOXM2_RAY_DIR>::prefix(),
                                                         data_buff_length * raydirTypeSize);
    boxm2_data_base * ray_dir_weighted_sum = cache->get_data_base(scene, id,
                                                                  boxm2_data_traits<BOXM2_RAY_DIR>::prefix(),
                                                                  data_buff_length * raydirTypeSize);
    ...
    if( output_filename.substr(output_filename.find_last_of(".") + 1) == "xyz" )
      {
      boxm2_export_oriented_point_cloud_function::exportPointCloudXYZ(scene, data, blk, alpha, vis, vis_sum, exp, nobs,
                                                                      points, normals, ray_dir_sum, myfile, output_aux,
                                                                      vis_t, nmag_t, prob_t, exp_t, bb_expanded);
      }
    /*   else if (output_filename.substr(output_filename.find_last_of(".") + 1) == "ply")
      ...
      {
      boxm2_export_oriented_point_cloud_function::exportPointCloudPLY(scene, data, blk, alpha, vis, points, normals,
                                                                      myfile, output_aux, vis_t, nmag_t, prob_t,
                                                                      bb_expanded, num_vertices);
      }
