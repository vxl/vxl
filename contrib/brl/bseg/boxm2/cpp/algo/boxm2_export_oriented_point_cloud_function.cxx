...
void boxm2_export_oriented_point_cloud_function::exportPointCloudXYZ(const boxm2_scene_sptr& scene,
                                                                     boxm2_block_metadata data, boxm2_block* blk,
                                                                     boxm2_data_base* alpha, boxm2_data_base* vis,
                                                                     boxm2_data_base* vis_sum, boxm2_data_base* exp,
                                                                     boxm2_data_base* nobs,
                                                                     ...
                                                                     float prob_t, float exp_t, vgl_box_3d<double> bb)
{
  boxm2_data_traits<BOXM2_ALPHA>::datatype * alpha_data =
    (boxm2_data_traits<BOXM2_ALPHA>::datatype *)alpha->data_buffer();
  boxm2_data_traits<BOXM2_POINT>::datatype * points_data =
    (boxm2_data_traits<BOXM2_POINT>::datatype *)points->data_buffer();
  boxm2_data_traits<BOXM2_NORMAL>::datatype * normals_data =
    (boxm2_data_traits<BOXM2_NORMAL>::datatype *)normals->data_buffer();
  boxm2_data_traits<BOXM2_VIS_SCORE>::datatype * vis_data =
    (boxm2_data_traits<BOXM2_VIS_SCORE>::datatype *)vis->data_buffer();
  boxm2_data_traits<BOXM2_VIS_SCORE>::datatype * vis_sum_data =
    (boxm2_data_traits<BOXM2_VIS_SCORE>::datatype *)vis_sum->data_buffer();
  boxm2_data_traits<BOXM2_EXPECTATION>::datatype * exp_data =
    (boxm2_data_traits<BOXM2_EXPECTATION>::datatype *)exp->data_buffer();
  boxm2_data_traits<BOXM2_NUM_OBS_SINGLE_INT>::datatype * nobs_data =
    (boxm2_data_traits<BOXM2_NUM_OBS_SINGLE_INT>::datatype *)nobs->data_buffer();
  boxm2_data_traits<BOXM2_RAY_DIR>::datatype * ray_dir_sum_data =
    (boxm2_data_traits<BOXM2_RAY_DIR>::datatype *)ray_dir_sum->data_buffer();
    ...
    if( normals_data[currIdx][3] >= nmag_t && vis_data[currIdx] >= vis_t && points_data[currIdx][3] != -1 )
      ...
      if( prob >= prob_t && exp_data[currIdx] >= exp_t )
        ...
        if( bb.is_empty() || bb.contains(points_data[currIdx][0], points_data[currIdx][1], points_data[currIdx][2]) )
          ...
          if( output_aux )
            {
            file <<  prob  << ' ' <<  normals_data[currIdx][3] << ' ' <<  vis_data[currIdx] << ' '
                 <<  vis_sum_data[currIdx];
            file << ' ' << ray_dir_sum_data[currIdx][0] << " " << ray_dir_sum_data[currIdx][1] << " "
                 << ray_dir_sum_data[currIdx][2] << " ";
...
void boxm2_export_oriented_point_cloud_function::exportPointCloudPLY(const boxm2_scene_sptr& scene,
                                                                     boxm2_block_metadata data, boxm2_block* blk,
                                                                     boxm2_data_base* alpha, boxm2_data_base* vis,
                                                                     boxm2_data_base* points, boxm2_data_base* normals,
                                                                     vcl_ofstream& file,
                                                                     bool output_aux, float vis_t, float nmag_t,
                                                                     float prob_t, vgl_box_3d<double> bb,
    ...
    if( normals_data[currIdx][3] >= nmag_t && vis_data[currIdx] >= vis_t && points_data[currIdx][3] != -1 )
      ...
      if( prob >= prob_t )
        ...
        if( bb.is_empty() || bb.contains(points_data[currIdx][0], points_data[currIdx][1], points_data[currIdx][2]) )
          ...
          if( output_aux )
            {
            file << ' ' << normals_data[currIdx][0] << ' ' << normals_data[currIdx][1] << ' '
                 << normals_data[currIdx][2] << ' ' << prob  << ' ' << vis_data[currIdx] << ' '
...
void boxm2_export_oriented_point_cloud_function::exportPointCloudPLY(const boxm2_scene_sptr& scene,
                                                                     ...
                                                                     boxm2_data_base* points,
                                                                     boxm2_data_base* covariances, vcl_ofstream& file,
                                                                     float prob_t, vgl_box_3d<double> bb,
                                                                     unsigned& num_vertices, bool color_using_model)
{
  boxm2_data_traits<BOXM2_POINT>::datatype * points_data =
    (boxm2_data_traits<BOXM2_POINT>::datatype *)points->data_buffer();
  boxm2_data_traits<BOXM2_COVARIANCE>::datatype * covs_data =
    (boxm2_data_traits<BOXM2_COVARIANCE>::datatype *)covariances->data_buffer();
  boxm2_data_traits<BOXM2_MOG3_GREY>::datatype * mog_data =
    (boxm2_data_traits<BOXM2_MOG3_GREY>::datatype *)mog->data_buffer();
  ...
  for( unsigned currIdx = 0; currIdx < (points->buffer_length() / pointTypeSize); currIdx++ )
    ...
    if( !calculateProbOfPoint(scene, blk, points_data[currIdx], covs_data[currIdx], mog, alpha, prob, exp_color, axes,
                              LE, CE) )
      {
      continue;
      }
...
void boxm2_export_oriented_point_cloud_function::exportColorPointCloudPLY(const boxm2_scene_sptr& scene,
                                                                          boxm2_block_metadata data, boxm2_block* blk,
                                                                          boxm2_data_base* mog, boxm2_data_base* alpha,
                                                                          vcl_string datatype,
      ...
      if( bb.is_empty() || bb.contains(points_data[currIdx][0], points_data[currIdx][1], points_data[currIdx][2]) )
        ...
        if( datatype == boxm2_data_traits<BOXM2_MOG3_GREY>::prefix() )
          ...
          boxm2_data_traits<BOXM2_MOG3_GREY>::datatype * mog_data     =
            (boxm2_data_traits<BOXM2_MOG3_GREY>::datatype *)mog->data_buffer();
          float exp_color  =
            boxm2_processor_type<BOXM2_MOG3_GREY>::type::expected_color(mog_data[currIdx]);
          int col = (int)(exp_color * 255);
          col = col > 255 ? 255 : col;
          file << col << ' ' << col << ' ' << col;
          }
          ...
          {
          boxm2_data_traits<BOXM2_GAUSS_RGB>::datatype * color_data     =
            (boxm2_data_traits<BOXM2_GAUSS_RGB>::datatype *)mog->data_buffer();
          vnl_vector_fixed<float,
                           3> exp_color =
            boxm2_processor_type<BOXM2_GAUSS_RGB>::type::expected_color(color_data[currIdx]);
...
bool boxm2_export_oriented_point_cloud_function::calculateProbOfPoint(const boxm2_scene_sptr& scene, boxm2_block * blk,
                                                                      ...
                                                                      vnl_vector_fixed<double, 3>& axes, double& LE,
                                                                      double& CE)
{
                                                                      ...
                                                                      const float& alpha, float& prob)
{
  ...
  float                               side_len =
    static_cast<float>(mdata.sub_block_dim_.x() / ( (float)(1 << depth) ) );

  prob = 1.0f - vcl_exp(-alpha * side_len);
  return true;
}

void boxm2_export_oriented_point_cloud_function::writePLYHeader(vcl_ofstream& file, unsigned num_vertices,
                                                                vcl_stringstream& ss, bool output_aux)
{
...
void boxm2_export_oriented_point_cloud_function::writePLYHeaderOnlyPoints(vcl_ofstream& file, unsigned num_vertices,
                                                                          vcl_stringstream& ss)
{
  file << "ply\nformat ascii 1.0\nelement vertex " << num_vertices
       <<
