...
void boxm2_export_stack_images_function::export_opacity_stack_images(const boxm2_scene_sptr& scene,
                                                                     boxm2_cache_sptr & cache, vcl_string outdir)
{
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    boxm2_data_base * alpha_base  =
      cache->get_data_base(nc_scene, blk_iter->first, boxm2_data_traits<BOXM2_ALPHA>::prefix() );
...
void boxm2_export_stack_images_function::export_greyscale_stack_images(const boxm2_scene_sptr& scene,
                                                                       ...
                                                                       vil3d_image_view<unsigned char> & img3d)
{
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    boxm2_data_base * int_base  =
      cache->get_data_base(nc_scene, blk_iter->first, boxm2_data_traits<BOXM2_MOG3_GREY>::prefix() );
    boxm2_data<BOXM2_MOG3_GREY> * int_data = new boxm2_data<BOXM2_MOG3_GREY>(
        int_base->data_buffer(), int_base->buffer_length(), int_base->block_id() );
          ...
          for( int ti = 0; ti < 585; ++ti )
            ...
            if( validParent )
              ...
              unsigned char intensity =
                (unsigned char)(boxm2_mog3_grey_processor::expected_color(int_data->data()[index]) * 255);
              for( int subi = 0; subi < factor; ++subi )
                {
                for( int subj = 0; subj < factor; ++subj )
                  {
...
void boxm2_export_stack_images_function::export_color_stack_images(const boxm2_scene_sptr& scene,
                                                                   boxm2_cache_sptr & cache,
                                                                   vil3d_image_view<unsigned char> & img3d)
{
  // we need to const_cast to call the new cache functions which take a (non const) scene as 1st arg.
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    boxm2_data<BOXM2_ALPHA> * alpha_data = new boxm2_data<BOXM2_ALPHA>(
        alpha_base->data_buffer(), alpha_base->buffer_length(), alpha_base->block_id() );
    boxm2_data_base * int_base  =
      cache->get_data_base(nc_scene, blk_iter->first, boxm2_data_traits<BOXM2_GAUSS_RGB>::prefix() );
    boxm2_data<BOXM2_GAUSS_RGB> * int_data = new boxm2_data<BOXM2_GAUSS_RGB>(
        int_base->data_buffer(), int_base->buffer_length(), int_base->block_id() );
