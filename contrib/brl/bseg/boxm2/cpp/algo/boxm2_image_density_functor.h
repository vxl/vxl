...
public:
  ...
  bool init_data(vcl_vector<boxm2_data_base *> & datas, vil_image_view<float> * img,
                 vil_image_view<float> * density_img, vil_image_view<float>* vis_img)
  {
    alpha_data_ =
  ...
  inline bool step_cell(float seg_len, int index, unsigned i, unsigned j, float abs_depth = 0.0)
  {
    ...
    float                             color_dens = boxm2_processor_type<APM_TYPE>::type::prob_density(
        mog3_data_->data()[index], color);
