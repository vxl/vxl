...
template <class T>
bool boxm2_refine_block_function_with_labels<T>::init_data(boxm2_block* blk, vcl_vector<boxm2_data_base *> & datas,
                                                           float prob_thresh,
...
bool boxm2_refine_block_function_with_labels<T>::refine_deterministic(vcl_vector<boxm2_data_base *>& datas)
{
  ...
  for( blk_iter = trees.begin(); blk_iter != trees.end(); ++blk_iter, ++currIndex )
    ...
    newInitCount += this->move_data(old_tree, refined_tree, alpha_cpy, alpha_sav_cpy, mog_cpy, mog_sav_cpy, flow_cpy,
                                    flow_res_cpy);
