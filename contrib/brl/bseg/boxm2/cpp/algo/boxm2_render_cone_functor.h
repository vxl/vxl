...
public:
  ...
  bool init_data(vcl_vector<boxm2_data_base *> & datas, vil_image_view<float> * expected,
                 vil_image_view<float>* vis_img)
  {
    alpha_data_   =
      new boxm2_data<BOXM2_GAMMA>(datas[0]->data_buffer(), datas[0]->buffer_length(), datas[0]->block_id() );
  ...
  inline bool step_cell(float volume, int index, unsigned i, unsigned j)
  {
    ...
    float                             voxel_int = boxm2_processor_type<BOXM2_MOG3_GREY>::type::expected_color(
        mog3_data_->data()[index]);
