                                   ...
                                   double distance_thresh, bool zero_model, float occupied_prob)
{
  ...
  for( vcl_map<boxm2_block_id, boxm2_block_metadata>::iterator iter = blocks.begin(); iter != blocks.end(); iter++ )
    ...
    boxm2_data_base* alpha_base  = boxm2_cache::instance()->get_data_base(scene, id,
                                                                          boxm2_data_traits<BOXM2_ALPHA>::prefix() );
    alpha_base->enable_write();
    boxm2_data<BOXM2_ALPHA> * alpha_data =
      new boxm2_data<BOXM2_ALPHA>(alpha_base->data_buffer(), num_cells * sizeof(float), alpha_base->block_id() );
      ...
      for( unsigned iy = 0; iy < nums.y(); ++iy )
        ...
        for( unsigned ix = 0; ix < nums.x(); ++ix )
          ...
          for( vcl_vector<int>::iterator iter = leafBits.begin(); iter != leafBits.end(); ++iter )
            ...
            vgl_vector_3d<double> cell_offset(cell_pos.x() * dims.x(), cell_pos.y() * dims.y(),
                                              cell_pos.z() * dims.z() );
...
#define BOXM2_SURFACE_DISTANCE_REFINE_INSTANTIATE(D) \
  template void boxm2_surface_distance_refine(D const &, boxm2_scene_sptr, vcl_vector<vcl_string> const &, double, bool, \
                                              float )
