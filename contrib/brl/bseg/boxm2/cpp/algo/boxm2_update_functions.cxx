                             ...
                             unsigned int roi_nj0)
{
  ...
  for( unsigned int pass_no = 0; pass_no < num_passes; ++pass_no )
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      boxm2_data_base * aux  = cache->get_data_base(scene, *id,
                                                    boxm2_data_traits<BOXM2_AUX>::prefix(),
                                                    alph->buffer_length() / alphaTypeSize * auxTypeSize);
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    {
    boxm2_block *     blk   = cache->get_block(scene, *id);
    boxm2_data_base * alph  = cache->get_data_base(scene, *id,
                                                   boxm2_data_traits<BOXM2_GAMMA>::prefix(), 0, false);
    boxm2_data_base * mog   = cache->get_data_base(scene, *id, data_type, 0, false);
    boxm2_data_base * nobs  = cache->get_data_base(scene, *id, num_obs_type, 0, false);
    int               alphaTypeSize = (int)boxm2_data_info::datasize(
        boxm2_data_traits<BOXM2_GAMMA>::prefix() );
    int               auxTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX>::prefix() );
    boxm2_data_base * aux  = cache->get_data_base(scene, *id,
                                                  boxm2_data_traits<BOXM2_AUX>::prefix(),
                                                  alph->buffer_length() / alphaTypeSize * auxTypeSize);
  ...
  for( unsigned int pass_no = 0; pass_no < num_passes; ++pass_no )
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      boxm2_data_base * mog  = cache->get_data_base(scene, *id, data_type,
                                                    alph->buffer_length() / alphaTypeSize * appTypeSize, false);
  ...
  for( unsigned int pass_no = 0; pass_no < num_passes; ++pass_no )
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      boxm2_data_base * nobs  = cache->get_data_base(scene, *id, num_obs_type,
                                                     alph->buffer_length() / alphaTypeSize * nobsTypeSize, false);
      int auxTypeSize =
        (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX>::prefix() );
      boxm2_data_base * aux  = cache->get_data_base(scene, *id,
                                                    boxm2_data_traits<BOXM2_AUX>::prefix(),
                                                    alph->buffer_length() / alphaTypeSize * auxTypeSize);
      vcl_vector<boxm2_data_base *> datas;
