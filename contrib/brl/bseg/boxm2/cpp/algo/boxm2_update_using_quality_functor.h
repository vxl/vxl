#ifndef boxm2_update_using_quality_functor_h
#define boxm2_update_using_quality_functor_h
// :
// \file
        ...
        ( ( (pre + vis * PI)
            * (*quality_img_)(i,
            ...
            + (1.0f
               - (*quality_img_)(i,
                                 j) ) )
          / ( (*norm_img_)(i, j) * (*quality_img_)(i, j) + (1.0f - (*quality_img_)(i, j) ) ) * seg_len);
      // aux[3]+=vis*(*quality_img_)(i,j)*seg_len;
      aux[3] +=
        ( (pre + vis * PI)
          * (*quality_img_)(i,
                            j)
          / ( (pre + vis * PI) * (*quality_img_)(i, j) + (1.0f - (*quality_img_)(i, j) ) ) ) * seg_len;
      }
    pre += vis * omega * PI;
    vis = vis * (1 - omega);
    (*vis_img_)(i, j) = vis;
...
private:
  ...
  vil_image_view<float> *   quality_img_;
};

template <boxm2_data_type APM_TYPE>
class boxm2_update_using_quality_functor
{
public:
  // : "default" constructor
  ...
  bool init_data(vcl_vector<boxm2_data_base *> & datas, float block_len, int max_levels)
  {
    ...
    mog3_data_ = new boxm2_data<APM_TYPE>(datas[2]->data_buffer(), datas[2]->buffer_length(), datas[2]->block_id() );
    nobs_data_ =
      new boxm2_data<BOXM2_NUM_OBS>(datas[3]->data_buffer(), datas[3]->buffer_length(), datas[3]->block_id() );
