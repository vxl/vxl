...
public:
  ...
  inline bool step_cell(float seg_len, int index, unsigned i, unsigned j, float abs_depth = 0.0)
  {
    ...
    if( (*norm_img_)(i, j) > 1e-10f )
      {
      aux[2] +=
        ( ( (pre + vis * PI) * model_prior_
            + (*alt_prob_img_)(i, j) ) / ( (*norm_img_)(i, j) * model_prior_ + (*alt_prob_img_)(i, j) ) * seg_len);
      // aux[3]+=vis*model_prior_*seg_len;
      aux[3] +=
        ( (pre + vis * PI) * model_prior_ / ( (pre + vis * PI) * model_prior_ + (*alt_prob_img_)(i, j) ) ) * seg_len;
      }
    pre += vis * omega * PI;
    vis = vis * (1 - omega);
    (*vis_img_)(i, j) = vis;
...
public:
  ...
  bool init_data(vcl_vector<boxm2_data_base *> & datas, float shadow_sigma, float block_len, int max_levels)
  {
    aux_data_ = new boxm2_data<BOXM2_AUX>(datas[0]->data_buffer(), datas[0]->buffer_length(), datas[0]->block_id() );
    alpha_data_ =
      new boxm2_data<BOXM2_ALPHA>(datas[1]->data_buffer(), datas[1]->buffer_length(), datas[1]->block_id() );
