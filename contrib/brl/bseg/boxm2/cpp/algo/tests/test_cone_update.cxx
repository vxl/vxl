...
void test_cone_update()
{
  ...
  boxm2_data_base * alph = boxm2_cache::instance()->get_data_base(scene, id,
                                                                  boxm2_data_traits<BOXM2_GAMMA>::prefix() );
  boxm2_data<BOXM2_GAMMA> * alpha_data_ = new boxm2_data<BOXM2_GAMMA>(alph->data_buffer(),
                                                                      alph->buffer_length(), alph->block_id() );
  boxm2_data_base * mog  =
    boxm2_cache::instance()->get_data_base(scene, id, boxm2_data_traits<BOXM2_MOG3_GREY>::prefix() );
  boxm2_data<BOXM2_MOG3_GREY>* mog3_data_  = new boxm2_data<BOXM2_MOG3_GREY>(mog->data_buffer(),
                                                                             mog->buffer_length(), mog->block_id() );
  ...
  // run render process
  test_success = test_success && boxm2_update_cone_image(scene, "boxm2_mog3_grey", "boxm2_num_obs", cam, &input_img,
                                                         ni, nj);
