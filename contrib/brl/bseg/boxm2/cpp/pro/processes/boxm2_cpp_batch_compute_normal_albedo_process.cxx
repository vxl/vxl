...
bool boxm2_cpp_batch_compute_normal_albedo_process(bprb_func_process& pro)
{
  ...
  for( id = blk_ids.begin(); id != blk_ids.end(); id++ )
    ...
    unsigned int      n_cells = alpha->buffer_length() / boxm2_data_info::datasize(
    ...
    unsigned int na_data_size = n_cells * boxm2_data_info::datasize(
        boxm2_data_traits<BOXM2_NORMAL_ALBEDO_ARRAY>::prefix() );
    ...
    boxm2_data_base * na_model_data  = cache->get_data_base(scene, *id,
                                                            boxm2_data_traits<BOXM2_NORMAL_ALBEDO_ARRAY>::prefix(), na_data_size,
                                                            false);
