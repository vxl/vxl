...
bool boxm2_cpp_batch_compute_shadow_model_process(bprb_func_process& pro)
{
  ...
  if( !foundDataType )
    ...
    vcl_cout
      <<
    "boxm2_cpp_batch_compute_shadow_model_process ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
      << vcl_endl;
    return false;
    }
  ...
  for( id = blk_ids.begin(); id != blk_ids.end(); id++ )
    ...
    boxm2_data_base * sunvis  = cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(
                                                       "sunvis"), 0, true);
    // vcl_cout << "buffer length of sunvis: " << sunvis->buffer_length() << '\n';
    boxm2_compute_ambient_functor data_functor;
    data_functor.init_data(sunvis, str_cache, weights, weighted_intensities,
                           float(blk->sub_block_dim().x() ), blk->max_level() );
