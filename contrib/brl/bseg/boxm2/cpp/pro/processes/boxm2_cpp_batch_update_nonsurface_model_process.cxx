...
bool boxm2_cpp_batch_update_nonsurface_model_process(bprb_func_process& pro)
{
  ...
  for (id = blk_ids.begin(); id != blk_ids.end(); id++) {
    ...
    boxm2_data_base *  histo_air  = cache->get_data_base(*id,boxm2_data_traits<BOXM2_MOG3_GREY>::prefix("histo_air"),alpha->buffer_length()*2,false);
    ...
    data_functor.init_data(histo_air,entropy_histo_air, str_cache);
    int histo_airTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_MOG3_GREY>::prefix());
    int data_buff_length  = (int)(histo_air->buffer_length()/histo_airTypeSize);
    boxm2_data_serial_iterator<boxm2_compute_nonsurface_histogram_functor>(data_buff_length,data_functor);

    cache->remove_data_base(*id,boxm2_data_traits<BOXM2_MOG3_GREY>::prefix("histo_air"));
