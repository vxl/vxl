...
bool boxm2_cpp_pre_infinity_opt2_phongs_process(bprb_func_process& pro)
{
  ...
  vcl_vector<boxm2_block_id> vis_order = scene->get_vis_blocks(
      reinterpret_cast<vpgl_generic_camera<double> *>(cam.ptr() ) );
  if( vis_order.empty() )
    {
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    boxm2_data_base * float8_phongs   = cache->get_data_base(scene, *id,
                                                             boxm2_data_traits<BOXM2_FLOAT8>::prefix(), 0, true);
    ...
    boxm2_data_base * aux0_view =
      cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(dir_identifier) );
    boxm2_data_base * aux1_view =
      cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_AUX1>::prefix(dir_identifier) );
    boxm2_data_base * aux2_view =
      cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_AUX2>::prefix(dir_identifier) );
    boxm2_data_base * aux3_view =
      cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_AUX3>::prefix(dir_identifier) );
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    boxm2_block *     blk   = cache->get_block(scene, *id);
    boxm2_data_base * alph  = cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_ALPHA>::prefix(), 0, true);
    boxm2_data_base * phongs_model_base   = cache->get_data_base(scene, *id,
                                                                 boxm2_data_traits<BOXM2_FLOAT8>::prefix(), 0, false);
    // call get_data_base method with num_bytes = 0 to read from disc
    boxm2_data_base * aux0 = cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(img_identifier) );
    boxm2_data_base * aux1 = cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_AUX1>::prefix(img_identifier) );
    boxm2_data_base * aux0_view =
    ...
    boxm2_data_base * aux = cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_AUX>::prefix(
                                                   img_identifier),
                                                 aux0->buffer_length() / aux0TypeSize * auxTypeSize, false);
...
bool boxm2_cpp_batch_update_nonray_process(bprb_func_process& pro)
{
  ...
  for( id = blk_ids.begin(); id != blk_ids.end(); ++id )
    ...
    // pass num_bytes = 0 to make sure disc is read if not already in memory
    boxm2_data_base * alph  = cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_ALPHA>::prefix(), 0, false);
    boxm2_data_base * phongs  = cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_FLOAT8>::prefix(
                                                       "phongs_model"), 0, false);
    boxm2_data_base * air  = cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(
                                                    "entropy_histo_air"), 0, false);
    boxm2_data_base * uncertainty  =
      cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_AUX1>::prefix("uncertainty"), 0, false);
    vcl_cout << "buffer length of alpha: " << alph->buffer_length() << '\n'
             << "buffer length of phongs: " << phongs->buffer_length() << vcl_endl;
