...
bool boxm2_cpp_create_norm_intensities_process(bprb_func_process& pro)
{
  ...
  if( vil_image_view<float> * input_image = dynamic_cast<vil_image_view<float> *>(float_image.ptr() ) )
    ...
    vcl_vector<boxm2_block_id> vis_order =
      scene->get_vis_blocks(reinterpret_cast<vpgl_generic_camera<double> *>(cam.ptr() ) );
    if( vis_order.empty() )
      {
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      boxm2_data_base * aux0 =
        cache->get_data_base_new(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(identifier), false);
      boxm2_data_base * aux1 =
        cache->get_data_base_new(scene, *id, boxm2_data_traits<BOXM2_AUX1>::prefix(identifier), false);
      boxm2_data_base * aux2 = cache->get_data_base_new(scene, *id, boxm2_data_traits<BOXM2_NUM_OBS_SINGLE>::prefix(
                                                          identifier), false);
...
bool boxm2_cpp_create_aux_data_process(bprb_func_process& pro)
{
  ...
  if( vil_image_view<float> * input_image = dynamic_cast<vil_image_view<float> *>(float_image.ptr() ) )
    ...
    if( !foundDataType )
      {
      vcl_cout
      << "boxm2_cpp_create_aux_data_process ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
      << vcl_endl;
      return false;
      }
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      boxm2_data_base * aux = cache->get_data_base_new(scene, *id, boxm2_data_traits<BOXM2_AUX>::prefix(
                                                         identifier), false);
...
bool boxm2_cpp_batch_update_process(bprb_func_process& pro)
{
  ...
  if( !foundDataType )
    {
    vcl_cout
    << "boxm2_cpp_batch_update_process ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
    << vcl_endl;
    return false;
    }
