...
bool boxm2_cpp_cast_3d_point_hypothesis_process(bprb_func_process& pro)
{
  ...
  vbl_array_2d<vnl_matrix_fixed<double, 3, 3> >* Rss = new vbl_array_2d<vnl_matrix_fixed<double, 3, 3> >(
      depth_img->ni(), depth_img->nj(), R);
  ...
  for( unsigned i = 0; i < depth_img->ni(); i++ )
    ...
    for( unsigned j = 0; j < depth_img->nj(); j++ )
      ...
      if( i == 0 && j == 0 )
        {
        vcl_cout << "principal ray: " << v << vcl_endl;
        vcl_cout << "generic ray: " << v_s << vcl_endl;
        vcl_cout << "R_ss: \n" << rot_s << vcl_endl;
        vcl_cout << "R_ss*v = \n" << rot_s * v << vcl_endl;
        }
      }
...
// #endif
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    boxm2_data_base * pts = cache->get_data_base_new(scene, *id, boxm2_data_traits<BOXM2_POINT>::prefix(
                                                       identifier), buf_len / alphaTypeSize * pointTypeSize, false);
    boxm2_data_base * sums = cache->get_data_base_new(scene, *id, boxm2_data_traits<BOXM2_FLOAT16>::prefix(
                                                        identifier), buf_len / alphaTypeSize * auxTypeSize, false);
...
bool boxm2_cpp_cast_3d_point_hypothesis_process2(bprb_func_process& pro)
{
  ...
  vpgl_camera_double_sptr cam = pro.get_input<vpgl_camera_double_sptr>(i++);
  vcl_string              identifier = pro.get_input<vcl_string>(i++);
  vcl_string              cov_c_file = pro.get_input<vcl_string>(i++);
  vcl_string              cov_v_file = pro.get_input<vcl_string>(i++);

  // read matrices from text file.
  vcl_ifstream ifs(cov_c_file.c_str() );
  if( !ifs.is_open() )
    {
    vcl_cerr << "Failed to open file " << cov_c_file << vcl_endl;
    return false;
    }
  ...
  if( !ifs.is_open() )
    {
    vcl_cerr << "Failed to open file " << cov_v_file << vcl_endl;
    return false;
    }
...
// #endif
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    boxm2_data_base * aux = cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_FLOAT16>::prefix(
                                                   identifier), buf_len / alphaTypeSize * auxTypeSize, true);

    // : first make sure that the database is removed from memory if it already exists
    cache->remove_data_base(scene, *id, boxm2_data_traits<BOXM2_COVARIANCE>::prefix(identifier) );

    // : now retrieve it with get_data_base_new method so that even if it exists on disc, a fresh one will be created
    boxm2_data_base * covs = cache->get_data_base_new(scene, *id, boxm2_data_traits<BOXM2_COVARIANCE>::prefix(
                                                        identifier), buf_len / alphaTypeSize * covTypeSize, false);
