...
bool boxm2_cpp_change_detection_process2(bprb_func_process& pro)
{
  ...
  if( !foundDataType )
    {
    vcl_cout
    << "BOXM2_OCL_CHANGE_DETECTION_PROCESS2 ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
    << vcl_endl;
    return false;
    }
    ...
    if( vil_image_view<float> * exp_img = dynamic_cast<vil_image_view<float> *>( exp_in_img.ptr() ) )
      ...
      for( id = vis_order.begin(); id != vis_order.end(); ++id )
        ...
        boxm2_data_base * alph =
          cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_ALPHA>::prefix() );
        ...
        cast_ray_per_block<boxm2_change_detection_with_uncertainity_functor>(cd_wu_functor, scene_info_wrapper->info,
                                                                             blk, cam, in_img->ni(), in_img->nj() );
        }
