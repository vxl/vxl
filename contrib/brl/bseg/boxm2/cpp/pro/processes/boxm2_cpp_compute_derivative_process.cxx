...
bool boxm2_cpp_compute_derivative_process(bprb_func_process& pro)
{
  ...
  if( id_x == -1 && id_y == -1 && id_z == -1 )
    ...
    {
    vcl_cout << "Running boxm2_cpp_compute_derivative_process on block: (" << id_x << ',' << id_y << ',' << id_z
             << ")..." << vcl_endl;
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    boxm2_data_base * normals    = cache->get_data_base(scene, id,
                                                        ...
                                                        alph->buffer_length() / alphaTypeSize * normalsTypeSize, false);
    ...
    boxm2_data_base * points    = cache->get_data_base(scene, id,
                                                       ...
                                                       alph->buffer_length() / alphaTypeSize * pointsTypeSize, false);

    boxm2_block_metadata data = blk_iter->second;
    boxm2_compute_derivative_function(data, blk, alph, normals, points, prob_threshold, normal_threshold,
                                      kernel_x_file_name, kernel_y_file_name, kernel_z_file_name);

#ifdef DEBUG
    vcl_size_t                                 dataSize = alph->buffer_length();
    boxm2_data_traits<BOXM2_POINT>::datatype * point_data =
      (boxm2_data_traits<BOXM2_POINT>::datatype *)points->data_buffer();
    boxm2_data_traits<BOXM2_NORMAL>::datatype * normal_data =
