...
bool boxm2_cpp_create_mog_image_process(bprb_func_process& pro)
{
  ...
  vbl_array_2d<boxm2_data_traits<BOXM2_MOG3_GREY>::datatype>* mog3_img =
    new vbl_array_2d<boxm2_data_traits<BOXM2_MOG3_GREY>::datatype>(ni, nj, mog3);
  vnl_vector_fixed<unsigned char, 2>                           mog( (unsigned char)0);
  vbl_array_2d<boxm2_data_traits<BOXM2_GAUSS_GREY>::datatype>* mog_img =
    new vbl_array_2d<boxm2_data_traits<BOXM2_GAUSS_GREY>::datatype>(ni, nj, mog);
  ...
  if( !foundDataType )
    {
    vcl_cout << "BOXM2_CPP_RENDER_PROCESS ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
             << vcl_endl;
    return false;
    }
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    boxm2_data_base * nobs  = cache->get_data_base(scene, *id, num_obs_type,
                                                   alph->buffer_length() / alphaTypeSize * nobsTypeSize, false);
  ...
  if( boxm2_data_traits<BOXM2_MOG3_GREY>::prefix() + "_" + identifier == data_type )
    ...
    for( unsigned i = 0; i < ni; i++ )
      ...
      for( unsigned j = 0; j < nj; j++ )
        ...
        float                               mu0 = ( (float)mog3[0] / 255.0f); float sigma0 = ( (float)mog3[1] / 255.0f);
        float                               w0 = ( (float)mog3[2] / 255.0f);
        float                               mu1 = ( (float)mog3[3] / 255.0f); float sigma1 = ( (float)mog3[4] / 255.0f);
        float                               w1 = ( (float)mog3[5] / 255.0f);
        float                               mu2 = ( (float)mog3[6] / 255.0f); float sigma2 = ( (float)mog3[7] / 255.0f);
        float                               w2 = 1 - w0 - w1;
