#ifndef boxm2_cache2_h_
#define boxm2_cache2_h_
//:
// \file
...
class boxm2_cache2;
typedef vbl_smart_ptr<boxm2_cache2> boxm2_cache2_sptr;

class boxm2_cache2_destroyer;

class boxm2_cache2: public vbl_ref_count
{
 public:
  void set_filesystem(BOXM2_IO_FS_TYPE fs_type) { filesystem_ = fs_type; }

  //: Use this instead of constructor
  static boxm2_cache2_sptr instance();
  static bool         exists() { return boxm2_cache2::instance_!=0; }

  //: the destructor instance to make sure memory is deallocated when the program exits
  static boxm2_cache2_destroyer destroyer_;  // it's not a pointer so C++ will make sure that its destructor will be called
  friend class boxm2_cache2_destroyer;
  ...
  virtual boxm2_data_base* get_data_base(boxm2_scene_sptr & scene, boxm2_block_id id, vcl_string type, vcl_size_t num_bytes=0, bool read_only = true) = 0;
  ...
  virtual boxm2_data_base* get_data_base_new(boxm2_scene_sptr & scene, boxm2_block_id id, vcl_string type=0, vcl_size_t num_bytes=0, bool read_only = true) = 0;
  ...
  virtual void remove_data_base(boxm2_scene_sptr & scene, boxm2_block_id id, vcl_string type)=0;

  virtual void replace_data_base(boxm2_scene_sptr & scene, boxm2_block_id id, vcl_string type, boxm2_data_base* replacement)=0;

  //: returns data pointer to data specified by ID and data_type
  template <boxm2_data_type T>
  boxm2_data<T>* get_data(boxm2_scene_sptr & scene, boxm2_block_id id, vcl_size_t num_bytes=0, bool read_only=true);
 ...
 protected:
  ...
  //: hidden constructor
  boxm2_cache2(boxm2_scene_sptr scene, BOXM2_IO_FS_TYPE fs=LOCAL) : scenes_(1,scene), filesystem_(fs) {}

  //: hidden destructor (protected so it cannot be called -- forces the class to be singleton)
  virtual ~boxm2_cache2() {}

  //: singleton instance of boxm2_cache2
  static boxm2_cache2_sptr instance_;
...
template <boxm2_data_type T>
boxm2_data<T>* boxm2_cache2::get_data(boxm2_scene_sptr & scene , boxm2_block_id id, vcl_size_t num_bytes, bool read_only)
{
...
//: Binary write boxm2_cache2 smart pointer to stream
...
//: Binary load boxm2_cache2 smart pointer from stream.
...
{
 public:
  boxm2_cache2_destroyer(boxm2_cache2_sptr s = 0);
  ~boxm2_cache2_destroyer();

  void set_singleton(boxm2_cache2_sptr s);
 private:
  boxm2_cache2_sptr s_;
};

#endif // boxm2_cache2_h_
