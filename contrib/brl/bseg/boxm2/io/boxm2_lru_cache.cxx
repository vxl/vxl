...
vcl_vector<boxm2_scene_sptr> boxm2_lru_cache::get_scenes()
{
  ...
  for( vcl_map<boxm2_scene_sptr, vcl_map<boxm2_block_id, boxm2_block *>, ltstr1>::const_iterator it =
  ...
  for( vcl_map<boxm2_scene_sptr, vcl_map<vcl_string, vcl_map<boxm2_block_id, boxm2_data_base *> >,
               ltstr1>::const_iterator it = cached_data_.begin();
...
{
  vcl_map<boxm2_scene_sptr, vcl_map<vcl_string, vcl_map<boxm2_block_id, boxm2_data_base *> >,
          ltstr1>::iterator scene_iter = cached_data_.begin();
  for( ; scene_iter != cached_data_.end(); scene_iter++ )
    ...
    for( vcl_map<vcl_string, vcl_map<boxm2_block_id, boxm2_data_base *> >::iterator iter = dmap.begin();
         iter != dmap.end(); iter++ )
      {
      for( vcl_map<boxm2_block_id, boxm2_data_base *>::iterator it = iter->second.begin(); it != iter->second.end();
           it++ )
        {
        delete it->second;
        }
  ...
  cached_data_.clear();
  vcl_map<boxm2_scene_sptr, vcl_map<boxm2_block_id, boxm2_block *>,
          ltstr1>::iterator scene_block_iter = cached_blocks_.begin();
...
boxm2_data_base * boxm2_lru_cache::get_data_base(boxm2_scene_sptr & scene, boxm2_block_id id, vcl_string type,
                                                 vcl_size_t num_bytes, bool read_only)
{
  ...
  if( num_bytes > 0 )
    ...
    if( num_bytes != byte_length )
      {
      vcl_stringstream ss;
      ss << "Attempting to retrieve " << num_bytes << " bytes for datatype " << type
         << " when actual buffer size should be " << byte_length;
...
boxm2_data_base * boxm2_lru_cache::get_data_base_new(boxm2_scene_sptr & scene, boxm2_block_id id, vcl_string type,
                                                     vcl_size_t num_bytes, bool read_only)
{
...
void boxm2_lru_cache::remove_data_base(boxm2_scene_sptr & scene, boxm2_block_id id, vcl_string type)
{
  // grab a reference to the map of cached_data_
  vcl_map<boxm2_block_id, boxm2_data_base *>& data_map = this->cached_data_map(scene, type);
  // then look for the block you're requesting
  ...
  if( rem != data_map.end() )
    ...
    if( !litter->read_only_ )
      {
      // save it
      vcl_cout << "boxm2_lru_cache::remove_data_base " << scene->xml_path() << " type " << type << ':' << id
...
void boxm2_lru_cache::replace_data_base(boxm2_scene_sptr & scene, boxm2_block_id id, vcl_string type,
                                        boxm2_data_base* replacement)
{
  // grab a reference to the map of cached_data_
...
vcl_map<boxm2_block_id, boxm2_data_base *> & boxm2_lru_cache::cached_data_map(boxm2_scene_sptr & scene,
                                                                              vcl_string prefix)
{
...
vcl_string boxm2_lru_cache::to_string()
{
  ...
  vcl_map<boxm2_scene_sptr, vcl_map<boxm2_block_id, boxm2_block *>,
          ltstr1>::iterator scene_block_iter = cached_blocks_.begin();
  vcl_map<boxm2_scene_sptr, vcl_map<vcl_string, vcl_map<boxm2_block_id, boxm2_data_base *> >,
          ltstr1>::iterator scene_data_iter = cached_data_.begin();
  for( ; scene_block_iter != cached_blocks_.end(); scene_block_iter++, scene_data_iter++ )
    ...
    vcl_map<vcl_string,
            vcl_map<boxm2_block_id, boxm2_data_base *> >::iterator dat_iter = scene_data_iter->second.begin();
