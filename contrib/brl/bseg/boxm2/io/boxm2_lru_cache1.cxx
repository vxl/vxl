...
void boxm2_lru_cache1::create(boxm2_scene_sptr scene, BOXM2_IO_FS_TYPE fs_type)
{
  if( boxm2_cache1::exists() )
    ...
    if( boxm2_cache1::instance_->get_scene()->xml_path() == scene->xml_path() )
      {
      vcl_cout << "boxm2_lru_cache1:: boxm2_cache1 singleton already created" << vcl_endl;
      }
    else
      {
      vcl_cout << "boxm2_lru_cache1:: destroying current cache and creating new one: " << scene->data_path()
               << vcl_endl;
...
boxm2_block * boxm2_lru_cache1::get_block(boxm2_block_id id)
{
  ...
  if( !loaded && scene_->block_exists(id) )
    {
    vcl_cout << "boxm2_lru_cache1::initializing empty block " << id << vcl_endl;
...
boxm2_data_base * boxm2_lru_cache1::get_data_base(boxm2_block_id id, vcl_string type, vcl_size_t num_bytes,
                                                  bool read_only)
{
  ...
  if( num_bytes > 0 )
    ...
    vcl_cout << "boxm2_lru_cache1::initializing empty data " << id
             << " type: " << type
    ...
    if( !loaded && scene_->block_exists(id) )
      {
      vcl_cout << "boxm2_lru_cache1::initializing empty data " << id << " type: " << type << vcl_endl;
...
boxm2_data_base * boxm2_lru_cache1::get_data_base_new(boxm2_block_id id, vcl_string type, vcl_size_t num_bytes,
                                                      bool read_only)
{
...
void boxm2_lru_cache1::remove_data_base(boxm2_block_id id, vcl_string type)
{
  // grab a reference to the map of cached_data_
  vcl_map<boxm2_block_id, boxm2_data_base *>& data_map =
    this->cached_data_map(type);
  ...
  if( rem != data_map.end() )
    ...
    if( !litter->read_only_ )
      {
      // save it
      vcl_cout << "boxm2_lru_cache1::remove_data_base " << type << ':' << id << "; saving to disk" << vcl_endl;
      boxm2_sio_mgr::save_block_data_base(scene_dir_, id, litter, type);
      }
    else
      {
      vcl_cout << "boxm2_lru_cache1::remove_data_base " << type << ':' << id << "; not saving to disk" << vcl_endl;
      }
    // now throw it away
    delete litter;
    data_map.erase(rem);
...
void boxm2_lru_cache1::replace_data_base(boxm2_block_id id, vcl_string type, boxm2_data_base* replacement)
{
  // grab a reference to the map of cached_data_
  vcl_map<boxm2_block_id, boxm2_data_base *>& data_map =
    this->cached_data_map(type);
...
vcl_map<boxm2_block_id, boxm2_data_base *> & boxm2_lru_cache1::cached_data_map(vcl_string prefix)
{
...
vcl_string boxm2_lru_cache1::to_string()
{
...
vcl_ostream & operator<<(vcl_ostream & s, boxm2_lru_cache1& scene)
{
