...
bool boxm2_stream_block_cache::init(boxm2_block_id id)
{
  ...
  for( unsigned i = 0; i < data_types_list_.size(); i++ )
    ...
    for( unsigned j = 0; j < identifier_list_.size(); j++ )
      {
      vcl_string filename = scene_->data_path() + data_types_list_[i] + "_" + identifier_list_[j] + "_"
        + current_block_id_.to_string() + ".bin";
    ...
    for( unsigned j = 0; j < identifier_list_.size(); j++ )
      ...
      if( vul_file::exists(filename) )
        ...
        vcl_cerr << "For " << id << ", data type: " << data_types_list_[i] + "_" + identifier_list_[j]
                 << " does not exist!\n";
        }
      }

    boxm2_data_base * data_buffer             = new boxm2_data_base(buffer, total_bytes_per_data_type,
                                                                    current_block_id_, true);
