...
char * boxm2_stream_cache_helper::get_cell(int i, vcl_size_t cell_size, boxm2_block_id id)
{
  ...
  int dif = i - index_;
  if( dif >= num_cells(cell_size) )
    {
    return 0;                                // means new buf needs to be loaded
    }
  ...
  : scene_(scene), identifier_list_(identifier_list)
{
  ...
  for( vcl_map<vcl_string, boxm2_stream_cache_datatype_helper_sptr>::iterator it = data_types_.begin();
...
#ifdef DEBUG
    vcl_cout << it->first << " will have " << it->second->buf_size_
    / vcl_pow(2.0, 20.0) << " MB buffers per identifier.\n";
#endif
    }
}
