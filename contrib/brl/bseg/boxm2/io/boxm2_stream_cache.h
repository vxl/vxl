...
public:
  ...
  boxm2_stream_cache(boxm2_scene_sptr scene, const vcl_vector<vcl_string>& data_types,
                     const vcl_vector<vcl_string>& identifier_list, float num_giga = 1.0f);
  ...
  vcl_vector<typename boxm2_data_traits<T>::datatype> get_next(boxm2_block_id id, int index = -1);

  // : random access in the stream
  template <boxm2_data_type T>
  vcl_vector<typename boxm2_data_traits<T>::datatype> get_random_i(boxm2_block_id id, unsigned int index);
...
protected:
  ...
  template <boxm2_data_type T>
  bool open_streams( boxm2_stream_cache_datatype_helper_sptr h);
