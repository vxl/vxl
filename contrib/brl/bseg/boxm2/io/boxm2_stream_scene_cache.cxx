...
:scene_(scene), data_types_(data_types),identifiers_(identifiers), blk_buffer_(0)
{
	...
	for(unsigned i = bounding_blk_ids.min_x(); i <= bounding_blk_ids.max_x(); i++){
		for(unsigned j = bounding_blk_ids.min_y(); j <= bounding_blk_ids.max_y(); j++){
			for(unsigned k = bounding_blk_ids.min_z(); k <= bounding_blk_ids.max_z(); k++)
			{
