...
boxm2_ocl_aggregate_normal_from_filter_vector::boxm2_ocl_aggregate_normal_from_filter_vector(boxm2_scene_sptr scene,
                                                                                             boxm2_opencl_cache_sptr ocl_cache, bocl_device_sptr device,
...
bool boxm2_ocl_aggregate_normal_from_filter_vector::compile_kernel(bocl_kernel & aggregate_kernel, vcl_string opts)
{
  ...
  return aggregate_kernel.create_kernel(&device_->context(),
                                        device_->device_id(), src_paths, "aggregate", opts, "aggregate");
}
...
bool boxm2_ocl_aggregate_normal_from_filter_vector::run(bool clear_cache)
{
  ...
  bocl_mem_sptr directions_buffer = ocl_cache_->alloc_mem(sizeof(cl_float4) * num_filters, directions,
                                                          "directions buffer");
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    bocl_mem * normals    = ocl_cache_->get_data(scene_, id,
    ...
    if( !good_exec )
      {
      vcl_cerr << "ERROR: boxm2_ocl_aggregate_normal_from_filter_vector::run() kernel_.execute() returned false"
               << vcl_endl;
      }
