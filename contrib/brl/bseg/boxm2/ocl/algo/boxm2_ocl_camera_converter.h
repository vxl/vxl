...
public:
  ...
  static void compute_ray_image( bocl_device_sptr & device, cl_command_queue & queue, vpgl_camera_double_sptr & cam,
                                 unsigned cl_ni, unsigned cl_nj, bocl_mem_sptr & ray_origins,
                                 bocl_mem_sptr & ray_directions, vcl_size_t i_min = 0, vcl_size_t j_min = 0,
                                 bool create_ray_o_d_buffers = true);

private:
  // : private helper method converts perspective camera to ray image
  static float convert_persp_to_generic( bocl_device_sptr & device, cl_command_queue & queue,
                                         vpgl_perspective_camera<double>* pcam, bocl_mem_sptr & ray_origins,
                                         bocl_mem_sptr & ray_directions, unsigned cl_ni, unsigned cl_nj,
                                         vcl_size_t i_min, vcl_size_t j_min, bool create_ray_o_d_buffers);
