                                                                ...
                                                                vcl_string identifier = "")
{
  ...
  if( !foundDataType )
    {
    vcl_cout
    << "yyboxm2_ocl_change_detection_process ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
    << vcl_endl;
    return false;
    }
  if( !foundNumObsType )
                                                ...
                                                vcl_size_t                startJ)
{
  ...
  bocl_mem_sptr exp_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), exp_image_buff,
  ...
  bocl_mem_sptr change_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), change_image_buff,
                                                       "change image buffer");
  ...
  bocl_mem_sptr change_exp_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), change_exp_image_buff,
                                                           "change exp image buffer");
  ...
  if( n > 1 )
    ...
    bocl_mem_sptr prob_change = new bocl_mem(
        device->context(), prob_change_buff, cl_ni * cl_nj * sizeof(float), "pass one prob change buffer");
    ...
    for( int oi = -half; oi <= half; ++oi )
      ...
      for( int oj = -half; oj <= half; ++oj )
        ...
        bocl_mem_sptr p2_vis      = new bocl_mem(
            device->context(), p2_vis_buff, cl_ni * cl_nj * sizeof(float), "pass one visibility buffer");
        ...
        bocl_mem_sptr nxn_change     = new bocl_mem(
            device->context(), nxn_change_buff, cl_ni * cl_nj * sizeof(float), "pass two change buffer");
        bocl_mem_sptr nxn_change_exp = new bocl_mem(
            device->context(), nxn_change_exp_buff, cl_ni * cl_nj * sizeof(float), "pass two change exp buffer");
...
// -------------------------------------------------
void boxm2_ocl_change_detection::full_pyramid(vil_image_view_base_sptr in_img, float* img_buff, unsigned cl_ni,
                                              unsigned cl_nj)
{
...
// of pixel values
// ------------------------------------------------------
double boxm2_ocl_change_detection::mutual_information_2d(const vnl_vector<double>& X, const vnl_vector<double>& Y,
                                                         int nbins)
{
                                              ...
                                              bool                      pmax )
{
  ...
  cl_float*     ray_origins    = new cl_float[4 * cl_ni * cl_nj];
  cl_float*     ray_directions = new cl_float[4 * cl_ni * cl_nj];
  bocl_mem_sptr ray_o_buff = new bocl_mem(
      device->context(), ray_origins,   cl_ni * cl_nj * sizeof(cl_float4), "ray_origins buffer");
  bocl_mem_sptr ray_d_buff = new bocl_mem(
      device->context(), ray_directions, cl_ni * cl_nj * sizeof(cl_float4), "ray_directions buffer");
  ...
  bocl_mem_sptr mask_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(vxl_byte), mask_buff,
                                                     "change mask image buffer");
  ...
  for( unsigned int offi = 0; offi < ois.size(); ++offi )
    ...
    for( unsigned int offj = 0; offj < ojs.size(); ++offj )
      ...
      for( id = vis_order.begin(); id != vis_order.end(); ++id )
        ...
        bocl_mem * aux0  =
          opencl_cache->get_data<BOXM2_AUX0>(scene, *id, info_buffer->data_buffer_length * auxTypeSize);
        auxTypeSize     = (int) boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX1>::prefix() );
        bocl_mem * aux1  =
          opencl_cache->get_data<BOXM2_AUX1>(scene, *id, info_buffer->data_buffer_length * auxTypeSize);
        transfer_time += (float) transfer.all();
  ...
  bocl_kernel* normalize_change_kernel =  kerns[2];
    ...
    // true vis buff
    bocl_mem_sptr vis_two = new bocl_mem(
        device->context(), true_vis, sizeof(float) * cl_ni * cl_nj, "oi=0,oj=0 visibility");
...
vcl_vector<bocl_kernel *> & boxm2_ocl_aux_pass_change::get_kernels(bocl_device_sptr device, vcl_string opts,
                                                                   bool maxdensity)
{
  // check to see if this device has compiled kernels already
  vcl_string identifier = device->device_identifier() + opts;
