                                      ...
                                      unsigned int roi_nj0)
{
  ...
  bocl_mem_sptr norm_image = new bocl_mem(
      device->context(), norm_buff, cl_ni * cl_nj * sizeof(float), "pre image buffer");
                             ...
                             unsigned int roi_nj0)
{
  ...
  for( unsigned int i = 0; i < kernels.size(); ++i )
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      bocl_mem* mog     = opencl_cache->get_data(*id, data_type, /*alpha->num_bytes()/alphaTypeSize*appTypeSize*/ 0,
                                                 false);                                                                      // info_buffer->data_buffer_length*boxm2_data_info::datasize(data_type));
      bocl_mem* num_obs = opencl_cache->get_data(*id, num_obs_type, /*alpha->num_bytes()/alphaTypeSize*nobsTypeSize*/ 0,
                                                 false);                                                                     // ,info_buffer->data_buffer_length*boxm2_data_info::datasize(num_obs_type));
#endif
