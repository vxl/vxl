...
::allocate_render_buffers(int cl_ni, int cl_nj)
{
  ...
  t_infinity_image_ =
    opencl_cache_->alloc_mem(cl_ni * cl_nj * sizeof(cl_float), t_infinity_buff_, "t_inf image buffer");
  t_infinity_image_->create_buffer(CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR);

  ray_origins_image_ = opencl_cache_->alloc_mem(cl_ni * cl_nj * sizeof(cl_float4), ray_origins_buff_,
                                                "ray_origins buffer");
  ray_origins_image_->create_buffer(CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR);

  ray_directions_image_ = opencl_cache_->alloc_mem(cl_ni * cl_nj * sizeof(cl_float4), ray_directions_buff_,
...
::render(vpgl_camera_double_sptr camera, unsigned ni, unsigned nj, float nearfactor, float farfactor)
{
  ...
  bool good_queue = check_val(status, CL_SUCCESS, "ERROR: boxm2_ocl_depth_renderer: Failed to create command queue: " + error_to_string(
                                status) );
  if( !good_queue )
    {
    return false;
  ...
  boxm2_ocl_camera_converter::compute_ray_image( device_, queue, camera, cl_ni, cl_nj, ray_origins_image_,
                                                 ray_directions_image_, 0, 0, false);

  int  statusw = clFinish(queue);
  bool good_write = check_val(statusw, CL_SUCCESS, "ERROR: boxm2_ocl_depth_renderer: Initial write to GPU failed: " + error_to_string(
                                statusw) );
  if( !good_write )
    {
    return false;
  ...
  for( vcl_vector<boxm2_block_id>::iterator id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    bool good_run = check_val(statusw, CL_SUCCESS, "ERROR: boxm2_ocl_depth_renderer: Execution of depth kernel failed: " + error_to_string(
                                statusw) );
    if( !good_run )
      {
      return false;
  ...
  bool good_norm = check_val(statusw, CL_SUCCESS, "ERROR: boxm2_ocl_depth_renderer: Execution of depth norm kernel failed: " + error_to_string(
                               statusw) );
  if( !good_norm )
    {
    return false;
  ...
  statusw = clFinish(queue);
  bool good_read = check_val(statusw, CL_SUCCESS, "ERROR: boxm2_ocl_depth_renderer: Read of depth buffers failed: " + error_to_string(
                               statusw) );
  if( !good_read )
    {
    return false;
