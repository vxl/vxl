...
::render(vpgl_camera_double_sptr camera, unsigned ni, unsigned nj, float nearfactor, float farfactor)
{
  ...
  if( camera->type_name() == "vpgl_perspective_camera" )
    {
    float f  = ( (vpgl_perspective_camera<double> *)camera.ptr() )->get_calibration().focal_length()
      * ( (vpgl_perspective_camera<double> *)camera.ptr() )->get_calibration().x_scale();
...
::compile_kernels(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, boxm2_data_type data_type)
{
  ...
  if( (data_type == BOXM2_MOG3_GREY) ||
    ...
    options += "-D RENDER ";
    options +=
      "-D RENDER_MAX -D STEP_CELL=step_cell_render_max(aux_args.mog,aux_args.alpha,data_ptr,d*linfo->block_len,vis,aux_args.expint,aux_args.maxomega)";
    ...
    {
    vcl_cerr << "ERROR: boxm2_ocl_expected_image_renderer::compile_kernels(): Unsupported Appearance model type "
             << data_type << vcl_endl;
    return false;
    }
  return true;
