...
bool boxm2_ocl_filter_scene_data::apply_filter(int index)
{
  ...
  vcl_cout << "Smoothing scene with filter: " << filter_ident.str() << " of size: " << filter->float_kernel_.size()
           << vcl_endl;
  // filter->print();
  ...
  bocl_mem_sptr filter_buffer = opencl_cache_->alloc_mem(
      sizeof(cl_float4) * filter->float_kernel_.size(), filter_coeff, "filter coefficient buffer");
  ...
  bocl_mem_sptr                                           centerX = opencl_cache_->alloc_mem(sizeof(cl_float) * 585,
                                                                                             boct_bit_tree::centerX,
                                                                                             "centersX lookup buffer");
  bocl_mem_sptr centerY = opencl_cache_->alloc_mem(sizeof(cl_float) * 585,
                                                   boct_bit_tree::centerY,
                                                   "centersY lookup buffer");
  bocl_mem_sptr centerZ = opencl_cache_->alloc_mem(sizeof(cl_float) * 585,
                                                   boct_bit_tree::centerZ,
                                                   "centersZ lookup buffer");
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    bocl_mem * alpha_new = opencl_cache_->get_data_new(scene_, id, boxm2_data_traits<BOXM2_ALPHA>::prefix(
                                                         "new"), data_size * alphaTypeSize, false);
    bocl_mem * mog_new  =
      opencl_cache_->get_data_new(scene_, id, appType_ + "_new", data_size * appTypeSize_, false);
