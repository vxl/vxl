                                                            ...
                                                            boxm2_opencl_cache_sptr  opencl_cache)
{
  ...
  for( ; iter_blks_A != blocks_A.end() || iter_blks_B != blocks_B.end(); iter_blks_A++, iter_blks_B++ )
    ...
    global_threads[0] = (unsigned) RoundUp(
        info_buffer_A->scene_dims[0] * info_buffer_A->scene_dims[1] * info_buffer_A->scene_dims[2],
        (int)local_threads[0]);
    ...
    opencl_cache->get_cpu_cache()->remove_data_base(sceneA, (*iter_blks_A),
                                                    boxm2_data_traits<BOXM2_GAUSS_RGB>::prefix() );
...
vcl_vector<bocl_kernel *> & boxm2_ocl_fuse_based_visibility::get_kernels(bocl_device_sptr device, vcl_string opts)
{
  ...
  vcl_cout << "=== boxm2_ocl_fuse_based_visibility_process::compiling kernels on device " << identifier << "==="
           << vcl_endl;
  ...
  fuse->create_kernel(&device->context(),
                      device->device_id(), src_paths, "fuse_blockwise_based_visibility", update_opts,
                      "fusion::fuse_blockwise_based_visibility");
...
vcl_vector<bocl_kernel *> & boxm2_ocl_fuse_based_orientation::get_kernels(bocl_device_sptr device, vcl_string opts)
{
  ...
  fuse->create_kernel(&device->context(),
                      device->device_id(), src_paths, "fuse_blockwise_based_orientation", update_opts,
                      "fusion::fuse_blockwise_based_orientation");
  vec_kernels.push_back(fuse);
  // store and return
  kernels_[identifier] = vec_kernels;
  return kernels_[identifier];
                                                          ...
                                                          boxm2_opencl_cache_sptr  opencl_cache)
{
  ...
  for( ; iter_blks_A != blocks_A.end() || iter_blks_B != blocks_B.end(); iter_blks_A++, iter_blks_B++ )
    ...
    bocl_mem* exp_A     = opencl_cache->get_data<BOXM2_EXPECTATION>(sceneA, *iter_blks_A, 0, true,
                                                                    "surfacedensity");
    bocl_mem*         app_A     = opencl_cache->get_data<BOXM2_GAUSS_RGB>(sceneA, *iter_blks_A, 0, false);
    boxm2_scene_info* info_buffer_A = sceneA->get_blk_metadata(*iter_blks_A);
    info_buffer_A->data_buffer_length = (int) (alpha_A->num_bytes() / alphaTypeSize);
    ...
    bocl_mem* exp_B     = opencl_cache->get_data<BOXM2_EXPECTATION>(sceneB, *iter_blks_B, 0, true,
                                                                    "surfacedensity");
    boxm2_scene_info* info_buffer_B = sceneB->get_blk_metadata(*iter_blks_B);
    info_buffer_B->data_buffer_length = (int) (alpha_B->num_bytes() / alphaTypeSize);
    bocl_mem* blk_info_B  = new bocl_mem(device->context(), info_buffer_B, sizeof(boxm2_scene_info), " Scene Info" );
...
vcl_vector<bocl_kernel *> & boxm2_ocl_fuse_surface_density::get_kernels(bocl_device_sptr device, vcl_string opts)
{
  ...
  fuse->create_kernel(&device->context(),
                      device->device_id(), src_paths, "fuse_blockwise_based_surface_density", update_opts,
                      "fusion::fuse_blockwise_based_surface_density");
  vec_kernels.push_back(fuse);
  // store and return
  kernels_[identifier] = vec_kernels;
  return kernels_[identifier];
