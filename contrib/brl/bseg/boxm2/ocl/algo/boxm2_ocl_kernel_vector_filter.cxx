...
boxm2_ocl_kernel_vector_filter::boxm2_ocl_kernel_vector_filter( bocl_device_sptr device) :
  device_(device)
{
...
bool boxm2_ocl_kernel_vector_filter::run(boxm2_scene_sptr scene, boxm2_opencl_cache_sptr opencl_cache,
                                         bvpl_kernel_vector_sptr filter_vector)
{
  ...
  bocl_mem_sptr centerX = new bocl_mem(
      device_->context(), boct_bit_tree::centerX, sizeof(cl_float) * 585, "centersX lookup buffer");
  bocl_mem_sptr centerY = new bocl_mem(
      device_->context(), boct_bit_tree::centerY, sizeof(cl_float) * 585, "centersY lookup buffer");
  bocl_mem_sptr centerZ = new bocl_mem(
      device_->context(), boct_bit_tree::centerZ, sizeof(cl_float) * 585, "centersZ lookup buffer");
  ...
  for( unsigned k = 0; k < filter_vector->kernels_.size(); k++ )
    ...
    bocl_mem * filter_buffer = new bocl_mem(
        device_->context(), filter_coeff, sizeof(cl_float4) * filter->float_kernel_.size(),
    ...
    bocl_mem_sptr filter_size_buffer = new bocl_mem(
        device_->context(), filter_size, sizeof(unsigned int), "filter_size buffer");
