                                         ...
                                         bsta_sigma_normalizer_sptr n_table )
{
  ...
  boxm2_data_base* mog_base = cache->get_data_base(scene, id,
                                                   boxm2_data_traits<BOXM2_MOG3_GREY>::prefix(), mogSize * data_buff_length,
                                                   false);
  boxm2_data<BOXM2_MOG3_GREY>* mog_data_ = new boxm2_data<BOXM2_MOG3_GREY>(
      mog_base->data_buffer(), mog_base->buffer_length(), mog_base->block_id() );
  // ----------------
  ...
  vcl_cout << "  total time: " << time << "; "
           ...
           << "  num samps: " << numSamps << " = " << (float)(numSamps / data_buff_length)
           << " samples per cell (avg)\n"
           << "  total size: " << (float)totalSize / 1024.0f / 1024.0f << " mb in " << t.all() << " ms ("
           << stream_time << " ms in stream cache)" << vcl_endl;

  // -------------------------------------------------------------------------
  // set up input buffers
  bocl_mem_sptr index_mem = new bocl_mem(
      device->context(), sampleIndex, sizeof(cl_uint) * data_buff_length, "sample index buffer");
  index_mem->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  vxl_byte*     intsBuff = &all_obs[0];
  bocl_mem_sptr sample_ints = new bocl_mem(
      device->context(), intsBuff, sizeof(char) * numSamps, "sample_intensities buffer");
  sample_ints->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  vxl_byte*     visBuff = &all_vis[0];
  bocl_mem_sptr sample_vis  = new bocl_mem(
      device->context(), visBuff, sizeof(char) * numSamps, "sample_visibilities buffer");
  sample_vis->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr num_samples = new bocl_mem(
      device->context(), &numSamps, sizeof(unsigned int), "total number of samples buffer");
                                                      ...
                                                      vcl_string       opts )
{
  ...
  kernel->create_kernel(&device->context(),
                        device->device_id(), src_paths, "batch_update_appearance", opts, "batch_paint::paint_kernel");
