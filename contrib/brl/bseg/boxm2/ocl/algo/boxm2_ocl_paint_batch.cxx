                                         ...
                                         bsta_sigma_normalizer_sptr n_table )
{
  ...
  int* sampleIndex = new int[data_buff_length]; 
  int  currIdx = 0; 
  vcl_vector<float> all_obs, all_vis; 

  //go through each cell, get stats, store
  for(int index = 0; index < data_buff_length; ++index) { 
    
    if(index%10000==0)
      vcl_cout<<"  index: "<<index<<" of "<<data_buff_length<<vcl_endl;
  ...
  //for(int index = 0; index < data_buff_length; ++index) {
    ...
    ////initialize to grey
    //boxm2_data<BOXM2_MOG3_GREY>::datatype & mog = mog_data_->data()[index];
    //mog[0] = 128; 
    //mog[1] = 128; 
    //mog[2] = 255; 
    
    ////calculate the number of samples for this cell
    //int start = sampleIndex[index]; 
    //int end   = (index==data_buff_length-1) ? all_obs.size() : sampleIndex[index+1]; 
    //int numSamples = end-start; 
    //if(numSamples <= 0) 
      //return; 
    
    ////create global float* buffers for stat calcs
    //float* obs = &all_obs[start]; //sampleInt + start; 
    //float* vis = &all_vis[start]; // + start; 
    
    ////calculatea app model
    //float mean, var; 
    //weighted_mean_var(obs, vis, numSamples, mean, var); 
    //mog[0] = (vxl_byte) (mean*255.0f); 
    //mog[1] = vcl_max( (vxl_byte) (var*255.0f), (vxl_byte) 6);
    //mog[2] = 255; 
  //}
  ...
  int totalSize = sizeof(int)*data_buff_length + (all_obs.size()+all_vis.size())*sizeof(float); 
  float time = (float) t.all(); 
  vcl_cout<<"  num cells: "<<data_buff_length<<"; "
          <<"  num samps: "<<all_obs.size()<<"\n"
          <<"  total size: "<<(float)totalSize/1024.0f/1024.0f<<" mb in "<<t.all()<<" ms ("<<stream_time<<" ms in stream cache)"<<vcl_endl;
  
  //-------------------------------------------------------------------------
  //set up input buffers
  bocl_mem_sptr index_mem = new bocl_mem(device->context(), sampleIndex, sizeof(int)*data_buff_length, "sample index buffer");
  index_mem->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  float* intsBuff = &all_obs[0]; 
  bocl_mem_sptr sample_ints = new bocl_mem(device->context(), intsBuff, sizeof(float)*all_obs.size(), "sample_intensities buffer");
  sample_ints->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);
  
  float* visBuff = &all_vis[0];
  bocl_mem_sptr sample_vis  = new bocl_mem(device->context(), visBuff, sizeof(float)*all_vis.size(), "sample_visibilities buffer");
  sample_vis->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);
  
  int numSamps = all_obs.size(); 
  bocl_mem_sptr num_samples = new bocl_mem(device->context(), &numSamps, sizeof(int), "total number of samples buffer");
