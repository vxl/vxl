                                                            ...
                                                            vcl_string const& apm_id)
{
  ...
  // raytrace kernel
    ...
    bocl_mem_sptr in_image =
      opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(cl_float4), input_buff, "input image buffer");
                                                                         ...
                                                                         vcl_string opts )
{
  ...
  kernel->create_kernel(&device->context(),
                        device->device_id(), src_paths, "seg_len_vis_main", updt_opts,
                        "online_paint_color::seg_len_vis_main");
  ...
  compress_kernel->create_kernel(&device->context(),
                                 device->device_id(), second_src_paths, "compress_rgb", compress_opts,
                                 "online_paint_color::compress_rgb");
  ...
  update_kernel->create_kernel(&device->context(),
                               device->device_id(), second_src_paths, "update_yuv_main", app_opts,
                               "online_paint_color::update_yuv_main");
...
bool boxm2_ocl_paint_online_color::reset( boxm2_scene_sptr scene, bocl_device_sptr device,
                                          boxm2_opencl_cache_sptr opencl_cache, vcl_string apm_id)
{
  int              status = 0;
  cl_command_queue queue = clCreateCommandQueue( device->context(),
                                                 *(device->device_id() ),
