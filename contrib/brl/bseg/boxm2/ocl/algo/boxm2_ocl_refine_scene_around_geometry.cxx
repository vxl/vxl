...
::compile_kernel()
{
  ...
  label_k->create_kernel(&device_->context(),
                         device_->device_id(), src_paths, "label_cells_to_refine", opts, "refinement kernel");
...
::refine_cpp()
{
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    boxm2_data_base * alph    = cache_->get_cpu_cache()->get_data_base(scene_, id,
                                                                       boxm2_data_traits<BOXM2_ALPHA>::prefix() );
    boxm2_data_base * mog     = cache_->get_cpu_cache()->get_data_base(scene_, id, app_type_);
    boxm2_data_base * labels  = cache_->get_cpu_cache()->get_data_base(scene_, id, boxm2_data_traits<BOXM2_LABEL_SHORT>::prefix(
                                                                         "refine") );
...
::label_cells_for_refinement()
{
  ...
  unsigned k = 0;
    ...
    bocl_mem_sptr filter_buffer = cache_->alloc_mem(
    ...
    for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
      ...
      bocl_mem * to_refine = cache_->get_data_new(scene_, id, boxm2_data_traits<BOXM2_LABEL_SHORT>::prefix(
                                                    "refine"), data_size * labelTypeSize, false);
