...
public:
        boxm2_ocl_refine_scene_around_geometry(boxm2_scene_sptr scene,boxm2_opencl_cache_sptr cache,
                        bocl_device_sptr device,bvpl_kernel_vector_sptr filter_vector,
                        int num_times,float p_thresh,bool refine_gpu):cache_(cache),
                        scene_(scene),
                        device_(device),
                        p_thresh_(p_thresh),
                        ...
                        num_times_(num_times)
        {
                ...
                vcl_cout<<"scene doesn't have the correct appearance type - only boxm2_gauss_rgb_view and mog6_view compact allowed!!"<<vcl_endl;
        }
                this->compile_kernel();
        }
        bool refine();
