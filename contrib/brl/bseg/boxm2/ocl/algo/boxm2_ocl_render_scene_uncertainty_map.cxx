                                                                      ...
                                                                      bool visualization )
{
  ...
  if (!visualization)
  {
    float phi_inc =  (phi_max-phi_min)/(float)(cl_ni)  ;
    float theta_inc = (theta_max-theta_min)/ (float) (cl_nj) ;
  ...
  {
    float phi_inc =  (phi_max-phi_min)/(float)(cl_nj/2)  ;
    float theta_inc = (theta_max-theta_min)/ (float) (cl_nj/2) ;
    vgl_box_3d<double> scene_bbox = scene->bounding_box();
    vgl_point_3d<double> center(scene_bbox.centroid_x(),scene_bbox.centroid_y(),scene_bbox.min_z()-(scene_bbox.depth()/100.0));

    float img_center_x = (float) cl_ni/2;
    float img_center_y = (float) cl_nj/2;

    for (unsigned j = 0 ; j < cl_nj; j++)
    {
      for (unsigned i = 0 ; i < cl_ni; i++)
      {
        float r = vcl_sqrt((j-img_center_y)*(j-img_center_y)+
                           (i-img_center_x)*(i-img_center_x));
        float theta = theta_min + theta_inc * r ;
