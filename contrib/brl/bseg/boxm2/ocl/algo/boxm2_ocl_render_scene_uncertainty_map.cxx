                                                                      ...
                                                                      vcl_string cam_dir_2)
{
  ...
  if( !foundDataType )
    ...
    vcl_cout
      <<
    "BOXM2_OCL_RENDER_SCENE_UNCERTAINTY_MAP ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
      << vcl_endl;
    return false;
    }
  if( ident.size() > 0 )
  ...
  bocl_mem_sptr alpha_integral_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), alpha_integral_buff,
                                                               "alpha integral buffer");
  ...
  bocl_mem_sptr cum_alpha_integral_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float),
                                                                   cum_alpha_integral_buff,
                                                                   " cumulative alpha integral buffer");
  ...
  vnl_float_4 x = boxm2_ocl_render_scene_uncertainty_map::compute_cubic_trajectory( phi_min, phi_max, theta_min,
                                                                                    theta_max, exp_image);
...
vcl_vector<bocl_kernel *> & boxm2_ocl_render_scene_uncertainty_map::get_kernels(bocl_device_sptr device,
                                                                                vcl_string opts)
{
  ...
  vcl_string    alpha_integral_options = options
    +
    "-D RENDER_ALPHA_INTEGRAL -D STEP_CELL=step_cell_alpha_integral(aux_args.alpha,data_ptr,d*linfo->block_len,aux_args.alpha_int)";
  ...
  vcl_string    render_options = options
    +
    "-D RENDER_USING_ALPHA_INTEGRAL -D STEP_CELL=step_cell_render_using_alpha_intergal(aux_args.mog,aux_args.alpha,data_ptr,d*linfo->block_len,aux_args.alpha_int,aux_args.alpha_int_cum,aux_args.expint)";
