                                        ...
                                        float resfarfactor )
{
  ...
  for( unsigned int i = 0; i < kernels.size(); ++i )
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      bocl_mem * aux0   = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(
                                                   view_ident), info_buffer->data_buffer_length * auxTypeSize, false);
      auxTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX1>::prefix() );
      bocl_mem * aux1   = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX1>::prefix(
                                                   view_ident), info_buffer->data_buffer_length * auxTypeSize, false);
      auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX2>::prefix() );
      bocl_mem * aux2   = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX2>::prefix(
                                                   view_ident), info_buffer->data_buffer_length * auxTypeSize, false);

      auxTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX3>::prefix() );
      bocl_mem * aux3   = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX3>::prefix(
                                                   view_ident), info_buffer->data_buffer_length * auxTypeSize, false);
...
vcl_vector<bocl_kernel *> & boxm2_ocl_update_auxQ::get_kernels(bocl_device_sptr device, vcl_string opts)
{
  ...
  convert_aux_int_float->create_kernel(&device->context(),
                                       device->device_id(), src_paths_4, "convert_aux_int_to_float", opts + " -D CONVERT_AUX ",
                                       "batch_update::convert_aux_int_to_float");
...
bool boxm2_ocl_update_PusingQ::init_product(boxm2_scene_sptr scene, boxm2_cache_sptr cache)
{
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    {
    boxm2_data_base * aux3 = cache->get_data_base(scene, *id,
                                                  boxm2_data_traits<BOXM2_AUX3>::prefix(), 0, false);
    boxm2_data_traits<BOXM2_AUX3>::datatype * aux3_data =
      reinterpret_cast<boxm2_data_traits<BOXM2_AUX3>::datatype *>( aux3->data_buffer() );
                                                  ...
                                                  vcl_string identifier)
{
  ...
  int              status = 0;
  cl_command_queue queue = clCreateCommandQueue( device->context(),
                                                 *(device->device_id() ), CL_QUEUE_PROFILING_ENABLE, &status);
  if( status != 0 )
    {
    return false;
    }
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    bocl_mem * aux3_curr   =
      opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX3>::prefix(
                               identifier + "_curr"), info_buffer->data_buffer_length * auxTypeSize, false);
    bocl_mem * aux3_prev   =
      opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX3>::prefix(
                               identifier + "_prev"), info_buffer->data_buffer_length * auxTypeSize, false);
    bocl_mem * aux0_curr   =
      opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(
                               identifier + "_curr"), info_buffer->data_buffer_length * auxTypeSize, false);
    transfer_time += (float) transfer.all();
    // set workspace
    vcl_size_t ltr[] = {64};
...

{
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    // grab an appropriately sized AUX data buffer
    int        auxTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX3>::prefix() );
    bocl_mem * aux3_product   = opencl_cache->get_data(scene, *id,
                                                       boxm2_data_traits<BOXM2_AUX3>::prefix(), info_buffer->data_buffer_length * auxTypeSize,
                                                       true);
    transfer_time += (float) transfer.all();
...
vcl_vector<bocl_kernel *> & boxm2_ocl_update_PusingQ::get_kernels(bocl_device_sptr device, vcl_string opts)
{
  ...
  apply_beta->create_kernel(&device->context(),
                            device->device_id(), non_ray_src, "apply_beta", apply_beta_opts, "update::apply_beta");
  vec_kernels.push_back(apply_beta);

  bocl_kernel* compute_product_Q = new bocl_kernel();
  vcl_string   product_q = opts + " -D PRODUCTQ";
  compute_product_Q->create_kernel(&device->context(),
                                   device->device_id(), non_ray_src, "compute_product_Q", product_q,
                                   "update::compute_product_Q");
  vec_kernels.push_back(compute_product_Q);
  bocl_kernel* update_P = new bocl_kernel();
  vcl_string   update_q_opts = opts + " -D UPDATEP";
  update_P->create_kernel(&device->context(),
                          device->device_id(), non_ray_src, "update_P_using_Q", update_q_opts,
                          "update::update_P_using_Q");
