  ...
  for (unsigned int i=0; i<kernels.size(); ++i)
  {
    ...
    for (id = vis_order.begin(); id != vis_order.end(); ++id)
    {
      ...
      else if (i==UPDATE_CELL_BASED_DISPERSION)
      {
        ...
        bocl_mem *aux0_dir_x   = opencl_cache->get_data<BOXM2_AUX0>(*id, info_buffer->data_buffer_length*auxTypeSize,false, "direction_expectation_x");

        auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX0>::prefix("direction_expectation_y"));
        bocl_mem *aux0_dir_y   = opencl_cache->get_data<BOXM2_AUX0>(*id, info_buffer->data_buffer_length*auxTypeSize,false, "direction_expectation_y");

        auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX0>::prefix("direction_expectation_z"));
        bocl_mem *aux0_dir_z   = opencl_cache->get_data<BOXM2_AUX0>(*id, info_buffer->data_buffer_length*auxTypeSize,false, "direction_expectation_z");

        auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX0>::prefix("visibility_expectation"));
        bocl_mem *aux0_vis_exp   = opencl_cache->get_data<BOXM2_AUX0>(*id, info_buffer->data_buffer_length*auxTypeSize,false, "visibility_expectation");
...
vcl_vector<bocl_kernel*>& boxm2_ocl_update_based_dispersion::get_kernels(bocl_device_sptr device, vcl_string opts, bool isRGB)
{
  ...
  vcl_cout<<"=== boxm2_ocl_update_based_dispersion_process::compiling kernels on device "<<identifier<<"==="<<vcl_endl;
