...
vcl_map<vcl_string, vcl_vector<bocl_kernel *> > boxm2_ocl_update_based_dispersion::kernels_;

// Main public method, updates color model
bool boxm2_ocl_update_based_dispersion::update(boxm2_scene_sptr              scene,
                                               ...
                                               vcl_size_t                    startJ)
{
  ...
  bocl_mem_sptr ray_o_buff = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(cl_float4), ray_origins,
                                                     "ray_origins buffer");
  bocl_mem_sptr ray_d_buff = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(cl_float4), ray_directions,
                                                     "ray_directions buffer");
  boxm2_ocl_camera_converter::compute_ray_image( device, queue, cam, cl_ni, cl_nj, ray_o_buff, ray_d_buff, startI,
                                                 startJ);
  ...
  if( cam->type_name() == "vpgl_perspective_camera" )
    ...
    float f  = ( (vpgl_perspective_camera<double> *)cam.ptr() )->get_calibration().focal_length()
      * ( (vpgl_perspective_camera<double> *)cam.ptr() )->get_calibration().x_scale();
  ...
  bocl_mem_sptr centerX = new bocl_mem(
      device->context(), boct_bit_tree::centerX, sizeof(cl_float) * 585, "centersX lookup buffer");
  bocl_mem_sptr centerY = new bocl_mem(
      device->context(), boct_bit_tree::centerY, sizeof(cl_float) * 585, "centersY lookup buffer");
  bocl_mem_sptr centerZ = new bocl_mem(
      device->context(), boct_bit_tree::centerZ, sizeof(cl_float) * 585, "centersZ lookup buffer");
  ...
  if( cam->type_name() == "vpgl_perspective_camera" )
    {
    vis_order =
      scene->get_vis_blocks_opt( (vpgl_perspective_camera<double> *)cam.ptr(), img_view->ni(), img_view->nj() );
    }
  else
    {
  ...
  for( unsigned int i = 0; i < kernels.size(); ++i )
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      bocl_mem* mog       = opencl_cache->get_data(scene, *id, data_type,
                                                   alpha->num_bytes() / alphaTypeSize * appTypeSize, false);                   // info_buffer->data_buffer_length*boxm2_data_info::datasize(data_type));
      bocl_mem* num_obs   = opencl_cache->get_data(scene, *id, num_obs_type,
                                                   alpha->num_bytes() / alphaTypeSize * nobsTypeSize, false);                  // ,info_buffer->data_buffer_length*boxm2_data_info::datasize(num_obs_type));
      ...
      if( i == UPDATE_SEGLEN )
        ...
        bocl_mem * aux2   =
          opencl_cache->get_data<BOXM2_AUX2>(scene, *id, info_buffer->data_buffer_length * auxTypeSize);
        aux2->zero_gpu_buffer(queue);
        auxTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX3>::prefix() );
        bocl_mem * aux3   =
          opencl_cache->get_data<BOXM2_AUX3>(scene, *id, info_buffer->data_buffer_length * auxTypeSize);
        ...
        bocl_mem * aux0_dir_x   = opencl_cache->get_data<BOXM2_AUX0>(scene, *id,
                                                                     info_buffer->data_buffer_length * auxTypeSize,
                                                                     false, "direction_expectation_x");

        auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX0>::prefix("direction_expectation_y") );
        bocl_mem * aux0_dir_y   = opencl_cache->get_data<BOXM2_AUX0>(scene, *id,
                                                                     info_buffer->data_buffer_length * auxTypeSize,
                                                                     false, "direction_expectation_y");

        auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX0>::prefix("direction_expectation_z") );
        bocl_mem * aux0_dir_z   = opencl_cache->get_data<BOXM2_AUX0>(scene, *id,
                                                                     info_buffer->data_buffer_length * auxTypeSize,
                                                                     false, "direction_expectation_z");

        auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX0>::prefix("visibility_expectation") );
        bocl_mem * aux0_vis_exp   = opencl_cache->get_data<BOXM2_AUX0>(scene, *id,
                                                                       info_buffer->data_buffer_length * auxTypeSize,
                                                                       false, "visibility_expectation");

        // update_alpha boolean buffer
        cl_int up_alpha[1];
        up_alpha[0] = update_alpha ? 1 : 0;
        bocl_mem_sptr up_alpha_mem = new bocl_mem(
            device->context(), up_alpha, sizeof(up_alpha), "update alpha bool buffer");
...
vcl_vector<bocl_kernel *> & boxm2_ocl_update_based_dispersion::get_kernels(bocl_device_sptr device, vcl_string opts,
                                                                           bool isRGB)
{
  ...
  vcl_cout << "=== boxm2_ocl_update_based_dispersion_process::compiling kernels on device " << identifier << "==="
           << vcl_endl;
  ...
  seg_len->create_kernel(&device->context(), device->device_id(), src_paths, "seg_len_main", seg_opts,
                         "update::seg_len");
  ...
  if( isRGB )
    ...
    comp->create_kernel(&device->context(),
                        device->device_id(), non_ray_src, "compress_rgb", comp_opts, "update::compress_rgb");
  ...
  pre_inf->create_kernel(&device->context(), device->device_id(), src_paths, "pre_inf_main", pre_opts,
                         "update::pre_inf");
  ...
  proc_img->create_kernel(&device->context(),
                          device->device_id(), non_ray_src, "proc_norm_image", proc_opts, "update::proc_norm_image");
  ...
  bayes_main->create_kernel(&device->context(),
                            device->device_id(), src_paths, "bayes_main", bayes_opt, "update::bayes_main");
  vec_kernels.push_back(bayes_main);

  // may need DIFF LIST OF SOURCES FOR THSI GUY TOO
  bocl_kernel* update = new bocl_kernel();
  vcl_string   update_opts = options + " -D UPDATE_BIT_BASED_DISPERSION";
  update->create_kernel(&device->context(),
                        device->device_id(), non_ray_src, "update_bit_scene_based_dispersion", update_opts,
                        "update::update_main");
...
bool boxm2_ocl_update_based_dispersion::validate_appearances(boxm2_scene_sptr scene,
                                                             ...
                                                             bool& isRGB)
{
  ...
  if( !foundDataType )
    {
    vcl_cout
      << "BOXM2_OPENCL_UPDATE_PROCESS ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
      << vcl_endl;
    return false;
    }
  if( !foundNumObsType )
