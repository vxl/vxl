                                          ...
                                          vcl_size_t               startJ)
{
  ...
  for( unsigned int i = 0; i < kernels.size(); ++i )
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      bocl_mem* mog       = opencl_cache->get_data(scene, *id, data_type, info_buffer->data_buffer_length * appTypeSize,
                                                   false);
      bocl_mem* num_obs   = opencl_cache->get_data(scene, *id, num_obs_type,
                                                   info_buffer->data_buffer_length * nobsTypeSize, false);
...
vcl_vector<bocl_kernel *> & boxm2_ocl_update_color::get_kernels(bocl_device_sptr device, vcl_string opts)
{
  ...
  seg_len->create_kernel(&device->context(),
                         device->device_id(), src_paths, "seg_len_main", seg_opts, "update_color::seg_len");
  ...
  proc_img->create_kernel(&device->context(),
                          device->device_id(), non_ray_src, "proc_norm_image", options,
  ...
  update->create_kernel(&device->context(),
                        device->device_id(), non_ray_src, "update_bit_scene_main", options,
                        "update_color::update_main");
