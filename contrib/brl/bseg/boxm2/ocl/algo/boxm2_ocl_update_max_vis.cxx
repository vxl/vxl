  ...
  for( unsigned int i = 0; i < kernels.size(); ++i )
    ...
    for( vcl_vector<boxm2_block_id>::iterator id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      if( i == UPDATE_SEGLEN_VIS )
        ...
        bocl_mem * aux2   = opencl_cache->get_data<BOXM2_AUX2>(scene, *id,
                                                               info_buffer->data_buffer_length * auxTypeSize, false);
        auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_VIS_SCORE>::prefix() );
        bocl_mem * aux_vis_score   = opencl_cache->get_data<BOXM2_VIS_SCORE>(scene, *id,
...
vcl_vector<bocl_kernel *> & boxm2_ocl_update_max_vis::get_kernels(bocl_device_sptr device, vcl_string opts)
{
  ...
  seg_len->create_kernel(&device->context(),
                         device->device_id(), src_paths, "vis_seg_len_main", seg_opts, "fusion::vis_seg_len_main");
  ...
  update->create_kernel(&device->context(),
                        device->device_id(), non_ray_src, "update_max_vis_score", update_opts,
                        "fusion::update_max_vis_score");
                                                        ...
                                                        float resfarfactor)
{
  ...
  for( unsigned int i = 0; i < kernels.size(); ++i )
    ...
    for( vcl_vector<boxm2_block_id>::iterator id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      bocl_mem * normal_array   = opencl_cache->get_data<BOXM2_NORMAL>(scene, *id,
                                                                       info_buffer->data_buffer_length * normalTypeSize,
                                                                       true);
      transfer_time += (float) transfer.all();
      if( i == ACCUMLATE_PER_RAY )
        ...
        bocl_mem * expectation_array   = opencl_cache->get_data<BOXM2_EXPECTATION>(scene, *id,
                                                                                   info_buffer->data_buffer_length * auxTypeSize, false,
                                                                                   "normaldot");

        auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX3>::prefix() );
        bocl_mem * vis_array   = opencl_cache->get_data<BOXM2_AUX3>(scene, *id,
                                                                    info_buffer->data_buffer_length * auxTypeSize,
                                                                    false, "normaldot");
...
vcl_vector<bocl_kernel *> & boxm2_ocl_update_cosine_angle::get_kernels(bocl_device_sptr device, vcl_string opts)
{
  ...
  // seg len pass
  bocl_kernel* seg_len = new bocl_kernel();
  vcl_string   seg_opts = options
    + " -D VIEW_NORMAL_DOT  -D STEP_CELL=step_cell_view_normal_dot(aux_args,data_ptr,llid,d)";
  seg_len->create_kernel(&device->context(),
                         device->device_id(), src_paths, "view_normal_dot_main", seg_opts, "fusion::vis_seg_len_main");
  vec_kernels.push_back(seg_len);
  ...
  update->create_kernel(&device->context(),
                        device->device_id(), non_ray_src, "update_avg_view_normal_dot", update_opts,
                        "fusion::update_avg_view_normal_dot");
  vec_kernels.push_back(update);
                                                              ...
                                                              float resfarfactor)
{
  ...
  bocl_mem_sptr vis_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), vis_buff, "vis image buffer");
  vis_image->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);
  bocl_mem_sptr cl_exp_depth_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), exp_depth_buf,
                                                             "exp depth buffer");
  cl_exp_depth_image->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);
  bocl_mem_sptr cl_std_depth_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), std_depth_buf,
                                                             "std depth buffer");
    ...
    for( vcl_vector<boxm2_block_id>::iterator id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      if( i == ACCUMLATE_PER_RAY )
        ...
        bocl_mem * aux2   = opencl_cache->get_data<BOXM2_AUX2>(scene, *id,
                                                               info_buffer->data_buffer_length * auxTypeSize, false);
        auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_EXPECTATION>::prefix() );
        bocl_mem * density_array   = opencl_cache->get_data<BOXM2_EXPECTATION>(scene, *id,
                                                                               info_buffer->data_buffer_length * auxTypeSize, false,
                                                                               "surfacedensity");
        auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX3>::prefix() );
        bocl_mem * vis_array   = opencl_cache->get_data<BOXM2_AUX3>(scene, *id,
...
vcl_vector<bocl_kernel *> & boxm2_ocl_update_surface_density::get_kernels(bocl_device_sptr device, vcl_string opts)
{
  ...
  bocl_kernel* seg_len = new bocl_kernel();
  vcl_string   seg_opts = options
    + " -D ACCUMULATE_SURFACE_DENSITY  -D STEP_CELL=step_cell_surface_density(tblock,aux_args,data_ptr,llid,d)";
  seg_len->create_kernel(&device->context(),
                         device->device_id(), src_paths, "accumulate_surface_density_main", seg_opts,
                         "fusion::accumulate_surface_density_main");
  vec_kernels.push_back(seg_len);
  ...
  update->create_kernel(&device->context(),
                        device->device_id(), non_ray_src, "update_avg_surface_density", update_opts,
                        "fusion::update_avg_surface_density");
  vec_kernels.push_back(update);
