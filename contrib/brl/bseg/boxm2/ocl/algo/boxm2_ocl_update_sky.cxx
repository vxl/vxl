...
vcl_vector<bocl_kernel *> & boxm2_ocl_update_sky::get_kernels(bocl_device_sptr device, vcl_string opts, bool isRGB)
{
  ...
  update_sky_kernel->create_kernel(&device->context(),
                                   device->device_id(), src_paths, "update_sky_main", seg_opts, "update_sky_main");
                                                    ...
                                                    vil_image_view_base_sptr skyimg)
{
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    bocl_mem * aux0_cum   = opencl_cache->get_data<BOXM2_AUX0>(scene, *id,
                                                               info_buffer->data_buffer_length * auxTypeSize, false,
                                                               "cum");
    auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX1>::prefix() );
    bocl_mem * aux1_count   = opencl_cache->get_data<BOXM2_AUX1>(scene, *id,
                                                                 info_buffer->data_buffer_length * auxTypeSize, false,
                                                                 "count");
    transfer_time += (float) transfer.all();
    kern->set_arg( blk_info );
                                         ...
                                         boxm2_opencl_cache_sptr  opencl_cache)
{
  ...
  bocl_mem_sptr chisqr_lookup = new bocl_mem(
      device->context(), chisqr_lookup_arr, sizeof(cl_float) * 10000, "chisqr cdf lookup buffer");
...
vcl_vector<bocl_kernel *> & boxm2_ocl_update_sky2::get_kernels(bocl_device_sptr device, vcl_string opts, bool isRGB)
{
  ...
  vcl_string   accum_opts = options
    + " -D UPDATE_SKY2  -D STEP_CELL=step_cell_accumulate_evidence_per_ray(aux_args,data_ptr,llid,d)";
  update_sky_accum_ray_kernel->create_kernel(&device->context(),
                                             device->device_id(), src_paths, "accumulate_evidence_sky_ray_main", accum_opts,
                                             "accumulate_evidence_sky_ray_main");
  ...
  update_sky_accum_cell_kernel->create_kernel(&device->context(),
                                              device->device_id(), non_ray_src, "accumulate_evidence_sky_cell_main", cell_accum_opts,
                                              "accumulate_evidence_sky_cell_main");
  ...
  update_sky_main_kernel->create_kernel(&device->context(),
                                        device->device_id(), non_ray_src, "update_sky_main", update_opts,
                                        "update_sky_main");
