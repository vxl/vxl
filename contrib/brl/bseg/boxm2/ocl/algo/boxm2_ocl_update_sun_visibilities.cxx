...
void boxm2_ocl_update_sun_visibilities::compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels,
                                                       vcl_string opts)
{
  ...
  vcl_string   seg_opts = options
    + " -D SEGLENVIS -D STEP_CELL=step_cell_seglen_vis(aux_args,data_ptr,llid,d*linfo->block_len) ";
  seg_len->create_kernel(&device->context(),
                         device->device_id(), src_paths, "seg_len_and_vis_main", seg_opts, "update::seg_len_vis");
  vec_kernels.push_back(seg_len);

  // may need DIFF LIST OF SOURCES FOR THSI GUY TOO
  bocl_kernel* update = new bocl_kernel();
  update->create_kernel(&device->context(),
                        device->device_id(), src_paths2, "update_visibilities_main", " -D UPDATE_SUN_VIS",
                        "update::update_visibilities_main");
                                                ...
                                                )
{
  ...
  if( kernels_.find(identifier) == kernels_.end() )
    {
    vcl_cout << "boxm2_ocl_update_sun_visibilities_process::compiling kernels on device" << identifier << "==="
             << vcl_endl;
  ...
  bocl_mem_sptr last_vis_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), last_vis_buff,
                                                         "last vis image buffer");
  ...
  vcl_vector<boxm2_block_id> vis_order =
    scene->get_vis_blocks( (vpgl_generic_camera<double> *)sun_cam.ptr() );
