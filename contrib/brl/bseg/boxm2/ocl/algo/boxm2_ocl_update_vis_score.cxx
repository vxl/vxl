...
::compile_kernels()
{
  ...
  vcl_string seg_opts = options
    + " -D RAYTRACE_PASS -D STEP_CELL=step_cell_vis_score(aux_args,data_ptr,llid,d*linfo->block_len) ";
  if( use_surface_normals_ )
    {
    seg_opts += "-D USE_SURFACE_NORMALS";
    }
  bool status = seg_len_kernel_.create_kernel(&device_->context(),
                                              device_->device_id(), src_paths, "vis_score_raytrace_main", seg_opts,
                                              "update::vis_score_raytrace");

  // may need DIFF LIST OF SOURCES FOR THSI GUY TOO
  status &= update_kernel_.create_kernel(&device_->context(),
                                         device_->device_id(), src_paths2, "update_vis_score_main", " -D UPDATE_PASS",
                                         "update::vis_score");
      ...
      vcl_string prefix_name )
{
  ...
  bocl_mem_sptr ray_o_buff =
    ocl_cache_->alloc_mem(cl_ni * cl_nj * sizeof(cl_float4), ray_origins, "ray_origins buffer");
  bocl_mem_sptr ray_d_buff = ocl_cache_->alloc_mem(cl_ni * cl_nj * sizeof(cl_float4), ray_directions,
  ...
  // set arguments
  vcl_vector<boxm2_block_id>           vis_order = scene_->get_vis_blocks(cam);
  vcl_vector<boxm2_block_id>::iterator id;
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    {
    vcl_cout << "update_vis_score: id = " << id->to_string() << vcl_endl;
    ...
    bocl_mem * aux0   = ocl_cache_->get_data<BOXM2_AUX0>(scene_, *id, info_buffer->data_buffer_length * auxTypeSize,
                                                         false);
    auxTypeSize = boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX1>::prefix() );
    bocl_mem * aux1   = ocl_cache_->get_data<BOXM2_AUX1>(scene_, *id, info_buffer->data_buffer_length * auxTypeSize,
                                                         false);
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    {
    vcl_cout << "update_vis_score2 : id = " << id->to_string() << vcl_endl;
    // choose correct render kernel
    boxm2_block_metadata mdata = scene_->get_block_metadata(*id);
