  ...
  : bytesInCache_(0), block_info_(0), device_(device)
{
  ...
  if( !check_val(status, CL_SUCCESS,
                 "boxm2_opencl_cache:: failed in command queue creation " + error_to_string(status) ) )
    {
    return;
    }
  queue_ = &q_;
...
void boxm2_opencl_cache::set_context(cl_context& context)
{
  ...
  if( !check_val(status, CL_SUCCESS,
                 "boxm2_opencl_cache::set_context failed to release old queue " + error_to_string(status) ) )
    {
    return;
    }
  ...
  q_ = clCreateCommandQueue(*context_,
                            *device_->device_id(),
                            ...
                            &status);
  if( !check_val(status, CL_SUCCESS,
                 "boxm2_opencl_cache::set_context failed in command queue creation " + error_to_string(status) ) )
    {
    return;
    }
  queue_ = &q_;
...
bool boxm2_opencl_cache::clear_cache()
{
  ...
  vcl_map<boxm2_scene_sptr, vcl_map<vcl_string, vcl_map<boxm2_block_id, bocl_mem *> >,
          ltstr1>::iterator scene_data_iter;
...
// Possible issue: if \p num_bytes is greater than 0, should it then always initialize a new data object?
bocl_mem * boxm2_opencl_cache::get_data_new(boxm2_scene_sptr scene, boxm2_block_id id, vcl_string type,
                                            vcl_size_t num_bytes, bool read_only)
{
  // push id to front of LRU list
  this->lru_push_front(vcl_pair<boxm2_scene_sptr, boxm2_block_id>(scene, id) );
...
void boxm2_opencl_cache::deep_replace_data(boxm2_scene_sptr scene, boxm2_block_id id, vcl_string type, bocl_mem* mem,
                                           bool read_only)
{
...
void boxm2_opencl_cache::deep_remove_data(boxm2_scene_sptr scene, boxm2_block_id id, vcl_string type, bool write_out)
{
...
void boxm2_opencl_cache::shallow_remove_data(boxm2_scene_sptr scene, boxm2_block_id id, vcl_string type)
{
  // find the data in this map
  vcl_map<boxm2_block_id, bocl_mem *>&          data_map = this->cached_data_map(scene, type);
  vcl_map<boxm2_block_id, bocl_mem *>::iterator iter = data_map.find(id);
...
bool boxm2_opencl_cache::lru_remove_last(vcl_pair<boxm2_scene_sptr, boxm2_block_id> & lru_id)
{
