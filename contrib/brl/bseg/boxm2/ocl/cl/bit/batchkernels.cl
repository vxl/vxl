    ...
    if(gid<info->dims.x*info->dims.y*info->dims.z)
    {
        ...
        for(int i=0; i<MAX_INNER_CELLS; i++) {
            ...
            if(validParent && tree_bit_at(local_tree, i)==0) {
                ...
                int dataIndex=buff_index*info->data_len+data_index(0, local_tree,i,bit_lookup);

                float8 hist8  = convert_float8(as_int8(hist[dataIndex]))/SEGLEN_FACTOR; 
                hist[dataIndex]=hist8;
                ...
                float chisqr1=0.0;
                if(pbC0>0)
                    chisqr1=(hist8.s0-0.125*pbC0)*(hist8.s0-0.125*pbC0)/0.125*pbC0+
                            (hist8.s1-0.47*pbC0 )*(hist8.s1-0.47*pbC0 )/0.47*pbC0 +
                            (hist8.s2-0.28*pbC0 )*(hist8.s2-0.28*pbC0 )/0.28*pbC0 +
                            (hist8.s3-0.121*pbC0)*(hist8.s3-0.121*pbC0)/0.121*pbC0;

                float chisqr2=0.0;
                if(pbC1>0)
                    chisqr2=(hist8.s4-0.42*pbC1 )*(hist8.s4-0.42*pbC1 )/0.42*pbC1 +
                            (hist8.s5-0.378*pbC1)*(hist8.s5-0.378*pbC1)/0.378*pbC1+
                            (hist8.s6-0.197*pbC1)*(hist8.s6-0.197*pbC1)/0.197*pbC1;
                //float chisqr1=0.0;
                //if(pbC0>0)
                ...
                intensity_array[dataIndex]=(uchar8)((unsigned char)floor(max_intensity*255.0f),
                                                     0,255,0,0,0,0,0);

                float chisqr=chisqr1+chisqr2;

                float P=0.0;
                if(chisqr<3.8) P=0.021*chisqr*chisqr;
                else           P=tanh(0.168*(chisqr-2));

                //get alpha value for this cell;
                alpha_array[dataIndex]=P;//-(log(1-P))/side_len;
