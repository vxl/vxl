                    ...
                    AuxArgs aux_args )
{
  ...
  while (currT < tFar)
  {
    ...
    for (int x=minCell.x; x<maxCell.x; ++x) {
      for (int y=minCell.y; y<maxCell.y; ++y) {
        for (int z=minCell.z; z<maxCell.z; ++z) {
          ...
          //visit list for BFS through tree (denotes parents that ought to be visited)
          ...
          if( intersect_volume > 0.0f ) {
            if( (*ltree).s0 == 0)
            {
              ...
              step_cell_cone(aux_args, data_ptr, intersect_volume, side_len * linfo->block_len,
            ...
            for(int currBitIndex=8*pindex + 1; currBitIndex < 8*pindex + 9; ++currBitIndex) {
              ...
              if ( intersect_volume > 0.0f ) {
                ...
                if ( tree_bit_at(ltree, currBitIndex) == 0 ) 
                {
                  ...
                  int data_ptr = data_index_cached(ltree, currBitIndex, bit_lookup, csum, &cumIndex) +
                                 data_index_root(ltree);
                  step_cell_cone(aux_args, data_ptr, intersect_volume, side_len * linfo->block_len,
                                 &intensity_norm, &weighted_int, &prob_surface);
                  total_volume += intersect_volume;
                }
    ...
    if (intensity_norm > 1e-10 && total_volume > 1e-10) {
      ...
      update_expected_int( aux_args, weighted_int/intensity_norm, sphere_occ_prob );

      //update visibility after all cells have accounted for
      update_vis( aux_args, sphere_occ_prob );
    }

    ////////////////////////////////////////////////////////////////////////////
    // Continue iterating over the cone ray
