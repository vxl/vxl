...
int main(int argc,  char* * argv)
{
  ...
  vsph_view_sphere<vsph_view_point<vcl_string> > sphere(scene->bounding_box(),
                                                        scene->bounding_box().depth() * radius() );
    ...
    for( unsigned int az_i = 0; az_i < num_az(); ++az_i )
      ...
      idstream << imgdir << imgname() << '_' << uid << ".jpg";
        ...
        if( scene->has_data_type(boxm2_data_traits<BOXM2_GAUSS_RGB>::prefix() ) )
          ...
          vil_image_view<vil_rgba<vxl_byte> >* exp_img_out =
            static_cast<vil_image_view<vil_rgba<vxl_byte> > *>(outimg.ptr() );
