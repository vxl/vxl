#include "boxm2_opencl_render_rgb_process.h"
...
bool boxm2_opencl_render_rgb_process::execute(vcl_vector<brdb_value_sptr>& input, vcl_vector<brdb_value_sptr>& output)
{
  ...
  if (!image_) {
    ...
    image_ = new bocl_mem((*context_), exp_buff, exp_img_view->size() * sizeof(cl_float4), "exp image buffer");
  ...
  for (id = vis_order.begin(); id != vis_order.end(); ++id)
  {
    ...
    bocl_mem* mog       = cache_->get_data<BOXM2_GAUSS_RGB>(*id);
...
bool boxm2_opencl_render_rgb_process::clean()
{
                                              ...
                                              vcl_string opts)
{
  ...
  bool created =  render_kernel_.create_kernel( context_,
                                                ...
                                                "render_bit_scene",   //kernel name
                                                options,              //options
                                                "boxm2 opencl render rgb"); //kernel identifier (for error checking)
  ...
  created = created && normalize_render_kernel_.create_kernel( context_,
                                                               ...
                                                               options,              //options
                                                               "normalize render kernel rgb"); //kernel identifier (for error checking)
...
bool boxm2_opencl_render_rgb_process::execute(vcl_vector<brdb_value_sptr>& input, vcl_vector<brdb_value_sptr>& output)
{
  ...
  //vis image buffer
  float* vis_buffer = new float[exp_img_view->size()];
  for (unsigned int i=0; i<exp_img_view->size(); ++i) vis_buffer[i] = 1.0f;
  if (vis_img_) delete vis_img_;
  vis_img_ = new bocl_mem((*context_), vis_buffer, exp_img_view->size()*sizeof(cl_float), "vis_image_ buffer");
  vis_img_->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);
