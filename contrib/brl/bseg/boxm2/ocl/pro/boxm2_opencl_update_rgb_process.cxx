...
bool boxm2_opencl_update_rgb_process::execute(vcl_vector<brdb_value_sptr>& input, vcl_vector<brdb_value_sptr>& output)
{
  ...
  for (unsigned int i=0; i<update_kernels_.size(); ++i)
  {
    ...
    for (id = vis_order.begin(); id != vis_order.end(); ++id)
    {
      ...
      mog_       = cache_->get_data<BOXM2_GAUSS_RGB>(*id);
      num_obs_   = cache_->get_data<BOXM2_NUM_OBS_SINGLE>(*id);
...
bool boxm2_opencl_update_rgb_process::write_input_image(vil_image_view<vil_rgba<vxl_byte> >* input_image)
{
  ...
  for (unsigned j=0;j<nj;j++)
  {
    for (unsigned i=0;i<ni;i++)
    {
      ...
      if (i<input_image->ni() && j< input_image->nj()) {
        ...
        buff[numFloats*count + 3] = (float) rgba.A() / 255.0f;
      }
      ++count;
    }
  }  
  ...
  if (!image_) {
    //create mem
    image_ = new bocl_mem((*context_), buff, ni* nj * numFloats * sizeof(cl_float), "input image buffer (float image)");
...
bool boxm2_opencl_update_rgb_process::execute(vcl_vector<brdb_value_sptr>& input, vcl_vector<brdb_value_sptr>& output)
{
  ...
  {
    //for debuggin...
    if ( i == UPDATE_COMPRESS_RGB ) {
      vcl_cout<<"SKipping compress kernel "<<vcl_endl;
      continue; 
    }
...
//Input image is a float4 (float intensities for each channel
bool boxm2_opencl_update_rgb_process::write_input_image(vil_image_view<vil_rgba<vxl_byte> >* input_image)
{
  //write to buffer (or create it)
  unsigned ni=RoundUp(input_image->ni(),8);
  ...
  for (unsigned j=0;j<nj;j++)
  {
    for (unsigned i=0;i<ni;i++)
    {
      ...
      if (i<input_image->ni() && j< input_image->nj()) {
        vil_rgba<vxl_byte> rgba = (*input_image)(i,j); 
        buff[numFloats*count + 0] = (float) rgba.R()/255.0f;
        //buff[numFloats*count + 1] = (float) rgba.R()/255.0f;
        //buff[numFloats*count + 2] = (float) rgba.R()/255.0f;
        //buff[numFloats*count + 3] = (float) rgba.R()/255.0f;
      }
      ++count;
    }
  }
  ...
  if (!image_) {
    //create mem
    image_ = new bocl_mem((*context_), buff, ni*nj * numFloats * sizeof(cl_float), "input image buffer");
    image_->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);
  }
  else {
    image_->write_to_buffer(*command_queue_);
