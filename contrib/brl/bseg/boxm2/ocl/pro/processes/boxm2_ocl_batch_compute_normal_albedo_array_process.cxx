...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels)
{
  ...
  compute_naa->create_kernel(&device->context(),
                             device->device_id(), src_paths, "batch_fit_normal_albedo_array", opts,
                             "batch_fit_normal_albedo_array");
...
bool boxm2_ocl_batch_compute_normal_albedo_array_process(bprb_func_process& pro)
{
  ...
  if( num_normals != 16 )
    {
    vcl_cerr << "ERROR: boxm2_ocl_batch_compute_normal_albedo_array_process: expecting 16 normals, got "
             << num_normals << '\n';
    return false;
    }
  // compute offsets and scales for linear radiance model
  float* radiance_scales_buff = new float[num_normals * num_images];
  float* radiance_offsets_buff = new float[num_normals * num_images];
  float* radiance_var_scales_buff = new float[num_normals * num_images];
  float* radiance_var_offsets_buff = new float[num_normals * num_images];
  for( unsigned int m = 0; m < num_images; ++m )
    ...
    for( unsigned int n = 0; n < num_normals; ++n )
      ...
      double var_offset = brad_radiance_variance_chavez(0.0, normals[n], metadata[m], atm_params[m], reflectance_var,
                                                        optical_depth_var, skylight_var, airlight_var);
      radiance_var_offsets_buff[idx] = var_offset;
      // compute scale
      double var = brad_radiance_variance_chavez(1.0, normals[n], metadata[m], atm_params[m], reflectance_var,
                                                 optical_depth_var, skylight_var, airlight_var);
  ...
  bocl_mem_sptr radiance_scales = new bocl_mem(
      device->context(), radiance_scales_buff, sizeof(float) * num_normals * num_images, "radiance scales buffer");
  radiance_scales->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr radiance_offsets = new bocl_mem(
      device->context(), radiance_offsets_buff, sizeof(float) * num_normals * num_images, "radiance offsets buffer");
  radiance_offsets->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr radiance_var_scales = new bocl_mem(
      device->context(), radiance_var_scales_buff, sizeof(float) * num_normals * num_images,
      "radiance variance scales buffer");
  radiance_var_scales->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr radiance_var_offsets = new bocl_mem(
      device->context(), radiance_var_offsets_buff, sizeof(float) * num_normals * num_images,
      "radiance variance offsets buffer");
  ...
  for( id = block_ids.begin(); id != block_ids.end(); ++id )
    ...
    bocl_mem_sptr     bocl_data_type0 = opencl_cache->alloc_mem(data_type0->buffer_length(),
                                                                data_type0->data_buffer(), "bocl data type0");
    ...
    bocl_mem_sptr     bocl_data_type1 = opencl_cache->alloc_mem(data_type1->buffer_length(),
                                                                data_type1->data_buffer(), "bocl data type1");
    ...
    bocl_mem_sptr     bocl_data_type2 = opencl_cache->alloc_mem(data_type2->buffer_length(),
                                                                data_type2->data_buffer(), "bocl data type2");
    ...
    bocl_mem_sptr     bocl_data_type3 = opencl_cache->alloc_mem(data_type3->buffer_length(),
                                                                data_type3->data_buffer(), "bocl data type3");
