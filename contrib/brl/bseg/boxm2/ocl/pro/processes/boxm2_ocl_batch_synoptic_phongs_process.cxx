...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels)
{
  ...
  compute_phongs->create_kernel(&device->context(),
                                device->device_id(), src_paths, "batch_fit_phongs_model", opts,
                                "batch_fit_phongs_model");
...
bool boxm2_ocl_batch_synoptic_phongs_process(bprb_func_process& pro)
{
  ...
  int           max_iter = 100;
  bocl_mem_sptr max_iter_mem =
    new bocl_mem(device->context(), &max_iter, sizeof(int), "Number of Max Iterations for LM");
  ...
  for( id = block_ids.begin(); id != block_ids.end(); ++id )
    ...
    bocl_mem_sptr     bocl_data_type1 = new bocl_mem(device->context(),
                                                     data_type1->data_buffer(), data_type1->buffer_length(), "");
