// This is brl/bseg/boxm2/ocl/pro/processes/boxm2_ocl_batch_uncertainty_process.cxx
#include <bprb/bprb_func_process.h>
// :
// \file
// \brief  A process for computing uncertainty in a batch process.
//
// \author Vishal Jain
// \date Mar 10, 2011
...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels)
{
  ...
  vcl_string   opts = " -D COMPUTE_UNCERTAINTY ";

  compute_cubic->create_kernel(&device->context(),
                               device->device_id(), src_paths, "compute_uncertainty", opts,
                               "kernel: compute_uncertainty");
...
bool boxm2_ocl_batch_uncertainty_process_cons(bprb_func_process& pro)
{
...
bool boxm2_ocl_batch_uncertainty_process(bprb_func_process& pro)
{
  using namespace boxm2_ocl_batch_uncertainty_process_globals;
  ...
  for( id = block_ids.begin(); id != block_ids.end(); ++id )
    ...
    boxm2_data_base * cpu_alpha  = cache->get_data_base(*id,
                                                        boxm2_data_traits<BOXM2_ALPHA>::prefix(), 0, false);
    boxm2_data_base * cubic_model_data  = cache->get_data_base(*id, boxm2_data_traits<BOXM2_FLOAT8>::prefix(
                                                                 "cubic_model"), cpu_alpha->buffer_length() * 8, false);
