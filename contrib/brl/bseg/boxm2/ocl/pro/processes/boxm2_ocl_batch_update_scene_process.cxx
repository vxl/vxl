...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts_app)
{
  ...
  compute_alpha->create_kernel(&device->context(),
                               device->device_id(), src_paths, "batch_update_alpha", opts, "batch_update_alpha");
  ...
  compute_mog->create_kernel(&device->context(),
                             device->device_id(), src_paths, "batch_update_mog", opts_mog, "batch_update_mog");
  vec_kernels.push_back(compute_mog);
}

template <boxm2_data_type type>
vcl_map<vcl_string, char *> helper(boxm2_scene_sptr scene, boxm2_block_id id, boxm2_data_traits<type> data_type,
  ...
  for( id = block_ids.begin(); id != block_ids.end(); ++id )
    ...
    for( unsigned s = 0; s < datasize; ++s )
      ...
      for( unsigned short k = 0; k < num_imgs; ++k )
        ...
        boxm2_data_traits<BOXM2_EXPECTATION>::datatype* exp_imgK =
          (boxm2_data_traits<BOXM2_EXPECTATION>::datatype *)(expectations[image_ids[k]]);
        boxm2_data_traits<BOXM2_PIXEL>::datatype* obs_imgK =
          (boxm2_data_traits<BOXM2_PIXEL>::datatype *)(pixels[image_ids[k]]);
        boxm2_data_traits<BOXM2_AUX2>::datatype* pre_exp_imgK =
          (boxm2_data_traits<BOXM2_AUX2>::datatype *)(pre_expectations[image_ids[k]]);
        boxm2_data_traits<BOXM2_AUX3>::datatype* seglen_imgK =
          (boxm2_data_traits<BOXM2_AUX3>::datatype *)(seglens[image_ids[k]]);
    ...
    bocl_mem_sptr num_samples = new bocl_mem(
        device->context(), &num_samp, sizeof(cl_uint), "total number of samples buffer");
    num_samples->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);

    bocl_mem_sptr index_mem = new bocl_mem(
        device->context(), sampleIndex, sizeof(cl_uint) * datasize, "sample index buffer");
    index_mem->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);

    bocl_mem_sptr weighted_pre_exp_sum_mem = new bocl_mem(
        device->context(), weighted_pre_exp_sum, sizeof(cl_float) * datasize, "weighted pre expectation sum buffer");
    weighted_pre_exp_sum_mem->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

    bocl_mem_sptr pre_exp_sum_mem = new bocl_mem(
        device->context(), pre_exp_sum, sizeof(cl_float) * datasize, "pre expectation sum buffer");
    pre_exp_sum_mem->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);

    bocl_mem_sptr exp_sum_mem = new bocl_mem(
        device->context(), exp_sum, sizeof(cl_float) * datasize, "expectation sum buffer");
    exp_sum_mem->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);

    bocl_mem_sptr seglen_sum_mem = new bocl_mem(
        device->context(), seglen_sum, sizeof(cl_float) * datasize, "seglen sum buffer");
    ...
    for( unsigned int i = 0; i < kernels[identifier].size(); i++ )
      ...
      if( i == COMPUTE_ALPHA )
        ...
        // allocate mem on gpu.
        bocl_mem_sptr bocl_data_type0 = opencl_cache->alloc_mem(
            exp_all.size() * sizeof(float), &exp_all[0], "expectations");
        bocl_mem_sptr bocl_data_type2 = opencl_cache->alloc_mem(
            seglen_all.size() * sizeof(float), &seglen_all[0], "seglens");
        ...
        // allocate mem on gpu.
        bocl_mem_sptr bocl_data_type0 = opencl_cache->alloc_mem(
            obs_all.size() * sizeof(unsigned char), &obs_all[0], "boxm2_mog2_rgb");
        bocl_mem_sptr bocl_data_type2 = opencl_cache->alloc_mem(
            exp_all.size() * sizeof(float), &exp_all[0], "float16_exp");
        ...
        bocl_mem* num_iter   = opencl_cache->get_data(scene, *id,
                                                      boxm2_data_traits<BOXM2_NUM_OBS>::prefix(),
                                                      alpha->num_bytes() / alphaTypeSize * niterTypeSize, false);
