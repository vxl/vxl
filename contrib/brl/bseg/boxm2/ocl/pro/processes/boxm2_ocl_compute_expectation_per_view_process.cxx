...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  seg_len->create_kernel(&device->context(),
                         device->device_id(), src_paths, "seg_len_nobs_main", seg_opts, "batch_update::seg_len_nobs");
  ...
  exp_sum->create_kernel(&device->context(),
                         device->device_id(), src_paths, "exp_sum_main", pre_opts, "batch_update::exp_sum");
  ...
  proc_img->create_kernel(&device->context(),
                          device->device_id(), non_ray_tracing_paths, "reinit_vis_image", proc_opts,
                          "update::reinit_vis_image");
  vec_kernels.push_back(proc_img);

  // compute expectation
  bocl_kernel* expectation = new bocl_kernel();
  vcl_string   expectation_opts = options
    + " -D EXPECTATION -D STEP_CELL=step_cell_expectation(aux_args,data_ptr,llid,d)";
  expectation->create_kernel(&device->context(),
                             device->device_id(), src_paths, "expectation_main", expectation_opts,
                             "batch_update::expectation");
  ...
  convert_exp_to_float->create_kernel(&device->context(),
                                      device->device_id(), non_ray_tracing_paths, "convert_exp_to_float", convert_opts2,
                                      "batch_update::convert_exp_to_float");
...
bool boxm2_ocl_compute_expectation_per_view_process(bprb_func_process& pro)
{
  ...
  if( !foundDataType )
    {
    vcl_cout << "BOXM2_OCL_COMPUTE_EXPECTATION_PER_VIEW_PROCESS ERROR: scene doesn't have BOXM2_MOG3_GREY data type"
             << vcl_endl;
    return false;
    }
  ...
  bocl_mem_sptr exp_denom_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), exp_denom_buff,
                                                          "expectation denom image buffer");
  exp_denom_image->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr pre_exp_num_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), pre_exp_num_buff,
                                                            "pre-expectation numerator image buffer");
  pre_exp_num_image->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr pi_inf_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), pi_inf_buff,
                                                       "pi infinity image buffer");
  ...
  for( unsigned int i = 0; i < kernels[identifier].size(); i++ )
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      bocl_mem * aux0   = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(
                                                   suffix), info_buffer->data_buffer_length * auxTypeSize, false);
      ...
      bocl_mem* mog       = opencl_cache->get_data(scene, *id, data_type,
                                                   alpha->num_bytes() / alphaTypeSize * appTypeSize, false);

      transfer_time += (float) transfer.all();
      if( i == COMPUTE_SEGLEN )
        {
        int       nobsTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_NUM_OBS_SINGLE_INT>::prefix() );
        bocl_mem* num_obs_single   = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_NUM_OBS_SINGLE_INT>::prefix(
                                                              suffix), info_buffer->data_buffer_length * nobsTypeSize,
                                                            false);

        auxTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_DATA_INDEX>::prefix() );
        bocl_mem* currIdx   = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_DATA_INDEX>::prefix(
                                                       suffix), info_buffer->data_buffer_length * auxTypeSize, false);
        ...
        bocl_mem* all_obs = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_PIXEL>::prefix(
                                                     suffix), total_num_rays * pixelTypeSize, false);
        // alloc expectations
        int       expTypeSize      = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_EXPECTATION>::prefix() );
        bocl_mem* all_exp = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_EXPECTATION>::prefix(
                                                     suffix), total_num_rays * expTypeSize, false);
        // alloc pre expectations
        int       preexpTypeSize      = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX2>::prefix() );
        bocl_mem* all_pre_exp = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX2>::prefix(
                                                         suffix), total_num_rays * preexpTypeSize, false);
        // alloc seglens
        int       seglenTypeSize      = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX3>::prefix() );
        bocl_mem* all_seglen = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX3>::prefix(
                                                        suffix), total_num_rays * seglenTypeSize, false);
        }
        ...
        bocl_mem* all_exp     =
          opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_EXPECTATION>::prefix(suffix) );
        ...
        bocl_mem_sptr total_num_rays_gpu = new bocl_mem(
            device->context(), &total_num_rays, sizeof(cl_uint), "total number of rays buffer");
