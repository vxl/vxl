...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  accumulate_direction_kernel->create_kernel(&device->context(),
                                             device->device_id(), src_paths, "accumulate_direction_vectors", accumulate_opts,
                                             "accumulate_direction_vectors");
  ...
  compute_dispersion->create_kernel(&device->context(),
                                    device->device_id(), src_paths, "compute_dispersion", dispersion_opts,
                                    "compute_dispersion");
...
bool boxm2_ocl_compute_expectation_view_direction_process(bprb_func_process& pro)
{
  ...
  for( vcl_vector<boxm2_block_id>::iterator id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    bocl_mem * aux_direction_expectation_x  =
      opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(
                               "direction_expectation_x"), info_buffer->data_buffer_length * auxTypeSize, false);
    auxTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX0>::prefix() );
    bocl_mem * aux_direction_expectation_y  =
      opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(
                               "direction_expectation_y"), info_buffer->data_buffer_length * auxTypeSize, false);
    auxTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX0>::prefix() );
    bocl_mem * aux_direction_expectation_z  =
      opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(
                               "direction_expectation_z"), info_buffer->data_buffer_length * auxTypeSize, false);
    auxTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX0>::prefix() );
    bocl_mem * aux_vis_expectation  =
      opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(
                               "visibility_expectation"), info_buffer->data_buffer_length * auxTypeSize, false);
    ...
    kern =  kernels[identifier][1];
      {
      auxTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX0>::prefix() );
      bocl_mem * aux_dispersion  = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(
                                                            "dispersion"), info_buffer->data_buffer_length * auxTypeSize,
                                                          false);
      kern->set_arg( blk_info );
      kern->set_arg( alpha );
      kern->set_arg( aux_direction_expectation_x );
      ...
      opencl_cache->deep_remove_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix("direction_expectation_x"),
                                     true);
      opencl_cache->deep_remove_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix("direction_expectation_y"),
                                     true);
      opencl_cache->deep_remove_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix("direction_expectation_z"),
                                     true);
