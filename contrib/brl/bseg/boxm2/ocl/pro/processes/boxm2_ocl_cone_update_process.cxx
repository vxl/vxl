...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  pass_one->create_kernel(&device->context(),
                          device->device_id(), src_paths, "pass_one", one_opts, "cone_update::pass_one");
  ...
  bayes_main->create_kernel(&device->context(),
                            device->device_id(), src_paths, "bayes_main", bayes_opt, "cone_update::bayes_main");
  vec_kernels.push_back(bayes_main);

  // may need
  bocl_kernel* update = new bocl_kernel();
  update->create_kernel(&device->context(),
                        device->device_id(), non_ray_src, "update_cone_data", options, "cone_update::update_data");
