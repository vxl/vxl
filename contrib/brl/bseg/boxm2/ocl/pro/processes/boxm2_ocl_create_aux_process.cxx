...
bool boxm2_ocl_create_aux_process(bprb_func_process& pro)
{
  ...
  for (unsigned int i=0; i<kernels[identifier].size(); ++i)
  {
    for (id = vis_order.begin(); id != vis_order.end(); ++id)
    {
      ...
      bocl_mem *aux0  = opencl_cache->get_data(*id, boxm2_data_traits<BOXM2_AUX0>::prefix(ident),0,false);
      auxTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX1>::prefix());
      bocl_mem *aux1  = opencl_cache->get_data(*id, boxm2_data_traits<BOXM2_AUX1>::prefix(ident),0,false);
      ...
      if (i==UPDATE_PRE_INF)
      {
        ...
        bocl_mem* num_obs   = opencl_cache->get_data(*id,boxm2_data_traits<BOXM2_FLOAT8>::prefix(),alpha->num_bytes()/alphaTypeSize*nobsTypeSize,false);
      ...
      else if (i==UPDATE_PREVISPOST)
      {
        ...
        bocl_mem *aux4 = opencl_cache->get_data(*id,boxm2_data_traits<BOXM2_AUX4>::prefix(ident), info_buffer->data_buffer_length*auxTypeSize,false);
