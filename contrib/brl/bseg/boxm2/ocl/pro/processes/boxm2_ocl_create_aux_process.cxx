...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  convert_aux_int_float->create_kernel(&device->context(),
                                       device->device_id(), second_pass_src, "convert_aux_and_normalize", opts + " -D CONVERT_AUX_NORMALIZE",
                                       "batch_update::convert_aux_and_normalize");
  ...
  pre_inf->create_kernel(&device->context(),
                         device->device_id(), src_paths, "pre_inf_main", pre_opts, "batch_update::pre_inf");
  vec_kernels.push_back(pre_inf);

  // push back cast_ray_bit
  bocl_kernel* aux_pre_vis = new bocl_kernel();
  vcl_string   bayes_opt = options
    + " -D AUX_PREVISPOST -D STEP_CELL=step_cell_aux_previspost(aux_args,data_ptr,llid,d) ";
  aux_pre_vis->create_kernel(&device->context(),
                             device->device_id(), second_pass_src, "aux_previspost_main", bayes_opt,
                             "batch_update::aux_previspost_main");
...
bool boxm2_ocl_create_aux_process(bprb_func_process& pro)
{
  ...
  if( !foundDataType )
    ...
    vcl_cout
      <<
    "BOXM2_OCL_UPDATE_AUX_PER_VIEW_PROCESS ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
      << vcl_endl;
    return false;
    }
  ...
  bocl_mem_sptr in_image = new bocl_mem(
      device->context(), input_buff, cl_ni * cl_nj * sizeof(float), "input image buffer");
  ...
  bocl_mem_sptr pre_image =
    new bocl_mem(device->context(), pre_buff, cl_ni * cl_nj * sizeof(float), "pre image buffer");
  pre_image->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr vis_inf_image = new bocl_mem(
      device->context(), vis_inf_buff, cl_ni * cl_nj * sizeof(float), "vis inf image buffer");
  vis_inf_image->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr pre_inf_image = new bocl_mem(
      device->context(), pre_inf_buff, cl_ni * cl_nj * sizeof(float), "pre inf image buffer");
  ...
  for( unsigned int i = 0; i < kernels[identifier].size(); ++i )
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      if( i == UPDATE_PRE_INF )
        ...
        bocl_mem* num_obs   = opencl_cache->get_data(scene, *id,
                                                     boxm2_data_traits<BOXM2_FLOAT8>::prefix(),
                                                     alpha->num_bytes() / alphaTypeSize * nobsTypeSize, false);
        ...
        bocl_mem * aux4 = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX4>::prefix(
                                                   ident), info_buffer->data_buffer_length * auxTypeSize, false);
        ...
        for( unsigned k = 0; k < 100; k++ )
          {
          vcl_cout << aux0_f[k] << ' ' << aux1_f[k] << ' ' << aux2_f[k] << ' ' << aux3_f[k] << ' ' << aux4_f[k]
                   << vcl_endl;
          }
#endif
        }
