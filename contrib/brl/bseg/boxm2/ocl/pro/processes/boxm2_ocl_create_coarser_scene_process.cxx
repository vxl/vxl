                         ...
                         boxm2_cache_sptr cache)
{
  ...
  boxm2_data_base * alpha_buffer_B = cache->get_data_base(scene, mdata.id_,
      ...
      for( unsigned k = 0; k < nk; k++ )
        ...
        for( int ti = 0; ti < 585; ti++ )
          ...
          if( validParent && curr_tree.bit_at(ti) == 0 )
            ...
            vgl_point_3d<double> pt = mdata.local_origin_ + vgl_vector_3d<double>(
                ( (double)i + cc.x() ) * sub_blk_dims.x(),
...
bool boxm2_ocl_create_coarser_scene_process(bprb_func_process& pro)
{
  ...
  for( ; iter != scene_blks.end(); iter++ )
    ...
    vgl_vector_3d<double> coarse_blk_dim(
      mdata.sub_block_dim_.x() * (double) mdata.sub_block_num_.x() / (double) coarse_blk_num.x(),
      mdata.sub_block_dim_.y() * (double) mdata.sub_block_num_.y() / (double) coarse_blk_num.y(),
      mdata.sub_block_dim_.z() * (double) mdata.sub_block_num_.z()
      / (double) coarse_blk_num.z() );
  ...
  for( ; coarse_iter != coarse_scene_blks.end(); coarse_iter++ )
    ...
    boxm2_sio_mgr::save_block_data_base(coarse_scene->data_path() + "/", (*coarse_iter), &alpha_buff,
                                        boxm2_data_traits<BOXM2_ALPHA>::prefix() );
    }
