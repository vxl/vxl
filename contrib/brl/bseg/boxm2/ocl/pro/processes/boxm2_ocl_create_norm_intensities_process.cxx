...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  convert_nobs_int_short->create_kernel(&device->context(),
                                        device->device_id(), second_pass_src, "convert_nobs_int_short", opts + " -D CONVERT_NOBS_INT_SHORT",
                                        "batch_update::convert_nobs_int_to_short");
  ...
  seg_len->create_kernel(&device->context(),
                         device->device_id(), second_pass_src, "seg_len_nobs_main", bayes_opt,
                         "batch_update::seg_len_nobs_main");
  ...
  for( unsigned int i = 0; i < kernels[identifier].size(); ++i )
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      if( i == UPDATE_CREATE_NORM )
        ...
        int nobsTypeSizeShort =
          (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_NUM_OBS_SINGLE>::prefix() );
        bocl_mem* num_obsShort   = opencl_cache->get_data(*id, num_obs_type_short,
                                                          info_buffer->data_buffer_length * nobsTypeSizeShort);
