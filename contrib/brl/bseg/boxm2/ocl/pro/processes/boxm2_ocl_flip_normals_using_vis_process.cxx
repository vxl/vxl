...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  compute_vis->create_kernel(&device->context(), device->device_id(), src_paths, "compute_vis", seg_opts,
                             "compute_vis");
  vec_kernels.push_back(compute_vis);

  bocl_kernel* decide_normal_dir = new bocl_kernel();
  decide_normal_dir->create_kernel(&device->context(),
                                   device->device_id(), src_paths, "decide_normal_dir", seg_opts, "decide_normal_dir");
...
bool boxm2_ocl_flip_normals_using_vis_process_cons(bprb_func_process& pro)
{
...
bool boxm2_ocl_flip_normals_using_vis_process(bprb_func_process& pro)
{
  ...
  bocl_mem_sptr dodecahedron_dir_lookup = new bocl_mem(
      device->context(), dodecahedron_dir, sizeof(cl_float4) * 12, "dodecahedron directions lookup buffer");
  dodecahedron_dir_lookup->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);

  cl_bool       contain_point[1];
  bocl_mem_sptr contain_point_mem = new bocl_mem(
      device->context(), contain_point, sizeof(cl_bool), "contains point buffer");
  ...
  for( unsigned int i = 0; i < kernels[identifier].size(); ++i )
    ...
    for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
      ...
      if( i == COMPUTE_VIS )
        ...
        bocl_mem * vis_sphere   =
          opencl_cache->get_data_new<BOXM2_VIS_SPHERE>(scene, blk_iter->first,
                                                       (normals->num_bytes() / normalsTypeSize) * visTypeSize, false);
        ...
        for( blk_iter_inner = blocks.begin(); blk_iter_inner != blocks.end(); ++blk_iter_inner )
          ...
          int                  alphaTypeSize =
            (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_ALPHA>::prefix() );
        ...
        bocl_mem* vis   =
          opencl_cache->get_data<BOXM2_VIS_SCORE>(scene, blk_iter->first, (normals->num_bytes() / normalsTypeSize)
                                                  * boxm2_data_info::datasize(boxm2_data_traits<
                                                                                BOXM2_VIS_SCORE>::prefix() ), false);
