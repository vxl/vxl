...
{
public:
  alpha_update_from_opinion_functor(boxm2_data_base* alpha, boxm2_data_base* aux0, boxm2_data_base* aux1,
                                    double sb_side_len)
...
#ifdef DEBUG
    vcl_cout << "index " << index << ": belief = " << belief << ", uncertainty = " << uncertainty << ", alpha = "
             << alpha << vcl_endl;
#endif
    // if(belief< 0)
      {
...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string options)
{
  ...
  options +=
    " -D STEP_CELL=step_cell_ingest_buckeye_dem(aux_args,data_ptr,(t_vox_exit-d)*linfo->block_len,t_vox_exit*linfo->block_len)";
...
bool boxm2_ocl_ingest_buckeye_dem_process(bprb_func_process& pro)
{
  ...
  if( !a1_view )
    {
    vcl_cerr
      << "Error: boxm2_ocl_ingest_buckeye_dem_process: could not cast first return image to a vil_image_view<float>\n";
    return false;
    }
  ...
  if( !a2_view )
    {
    vcl_cerr
      << "Error: boxm2_ocl_ingest_buckeye_dem_process: could not cast last return image to a vil_image_view<float>\n";
    return false;
    }
  ...
  bocl_mem_sptr a1_buff =
    new bocl_mem(device->context(), a1_img, cl_ni * cl_nj * sizeof(cl_float), "buckeye a1 buffer");
  a1_buff->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr a2_buff =
    new bocl_mem(device->context(), a2_img, cl_ni * cl_nj * sizeof(cl_float), "buckeye a2 buffer");
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    bocl_mem * aux0 = opencl_cache->get_data(scene, *id,
                                             boxm2_data_traits<BOXM2_AUX0>::prefix(), info_buffer->data_buffer_length * auxTypeSize,
                                             false);
    ...
    bocl_mem * aux1 = opencl_cache->get_data(scene, *id,
                                             boxm2_data_traits<BOXM2_AUX1>::prefix(), info_buffer->data_buffer_length * auxTypeSize,
                                             false);
    ...
    boxm2_data_base * alpha_data = cpu_cache->get_data_base(scene, *id,
                                                            boxm2_data_traits<BOXM2_ALPHA>::prefix(), 0, false);
    boxm2_data_base * aux0_data =
      cpu_cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(), 0, false);
    boxm2_data_base * aux1_data =
      cpu_cache->get_data_base(scene, *id, boxm2_data_traits<BOXM2_AUX1>::prefix(), 0, false);
    ...
    int data_len =
      int(alpha_data->buffer_length() / boxm2_data_info::datasize(boxm2_data_traits<BOXM2_ALPHA>::prefix() ) );
