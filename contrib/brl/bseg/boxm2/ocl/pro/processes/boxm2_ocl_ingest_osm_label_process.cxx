...
bool boxm2_ocl_ingest_osm_label_process(bprb_func_process& pro)
{
  ...
  vcl_cout << "casting images of size: " << z_img->ni() << " x " << z_img->nj() << " rounded up size: " << cl_ni
           << " x " << cl_nj << vcl_endl;
  ...
  vcl_cout << " alpha data size info: " << vcl_endl;
  vcl_cout << " apptypesize = " << apptypesize << vcl_endl;
  vcl_cout << " alphaTypeSize = " << alphaTypeSize << vcl_endl;
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    bocl_mem* label_data = opencl_cache->get_data(scene, *id, out_data_type,
