...
bool boxm2_ocl_kernel_filter_process_globals::compile_filter_kernel(bocl_device_sptr device,
                                                                    bocl_kernel * filter_kernel, vcl_string opts)
{
...
bool boxm2_ocl_kernel_filter_process_globals::process(bocl_device_sptr device, boxm2_scene_sptr scene,
                                                      boxm2_opencl_cache_sptr opencl_cache, bvpl_kernel_sptr filter)
{
  float gpu_time = 0.0f;
  ...
  unsigned                                                                   ci = 0;
  cl_float4*                                                                 filter_coeff =
    new cl_float4[filter->float_kernel_.size()];
  ...
  bocl_mem_sptr filter_buffer = new bocl_mem(
      device->context(), filter_coeff, sizeof(cl_float4) * filter->float_kernel_.size(), "filter coefficient buffer");
  ...
  bocl_mem_sptr filter_size_buffer = new bocl_mem(
      device->context(), filter_size, sizeof(unsigned int), "filter_size buffer");
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    bocl_mem*        filter_response =
      opencl_cache->get_data_new(scene, id, boxm2_data_traits<BOXM2_FLOAT>::prefix(filter_ident.str() ), dataSize,
                                 false);
