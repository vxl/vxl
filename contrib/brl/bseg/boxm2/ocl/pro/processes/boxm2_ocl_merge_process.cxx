...
void compile_merge_data_kernel(bocl_device_sptr device, bocl_kernel* merge_data_kernel, vcl_string option)
{
  vcl_vector<vcl_string> src_paths;
  vcl_string             source_dir = boxm2_ocl_util::ocl_src_root();
  src_paths.push_back(source_dir + "scene_info.cl");
...
bool boxm2_ocl_merge_process(bprb_func_process& pro)
{
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    bocl_mem_sptr blk_copy = new bocl_mem(
        device->context(), NULL, numTrees * sizeof(cl_uchar16), "merge trees block copy buffer");
    ...
    bocl_mem_sptr tree_sizes = new bocl_mem(
        device->context(), NULL, sizeof(cl_int) * numTrees, "merge tree sizes buffer");
  ...
  vcl_cout << " Merge GPU Time: " << gpu_time << ", transfer time: " << transfer_time << vcl_endl;
  return true;
}
