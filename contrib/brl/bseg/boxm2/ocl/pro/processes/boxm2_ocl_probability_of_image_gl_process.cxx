...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  opts +=
    " -D STEP_CELL=step_cell_compute_probability_of_intensity(aux_args.mog,aux_args.alpha,data_ptr,d*linfo->block_len,vis,aux_args.prob_image,aux_args.intensity) ";
...
bool boxm2_ocl_probability_of_image_gl_process(bprb_func_process& pro)
{
  ...
  if( !foundDataType )
    ...
    vcl_cout
      <<
    "boxm2_ocl_probability_of_image_gl_process ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
      << vcl_endl;
    return false;
    }
  if( !foundNumObsType )
  ...
  bocl_mem_sptr prob_image = new bocl_mem(
      device->context(), prob_image_buff, cl_ni * cl_nj * sizeof(float), "expected image buffer");
  ...
  vcl_vector<boxm2_block_id> vis_order =
    scene->get_vis_blocks(  (vpgl_perspective_camera<double> *)cam.ptr() );
  ...
  // normalize
  bocl_kernel* convert_to_rgba_image_kernel =  kernels[identifier][2];
    {
    convert_to_rgba_image_kernel->set_arg( prob_image.ptr() );
    convert_to_rgba_image_kernel->set_arg( img_dim.ptr() );
    convert_to_rgba_image_kernel->set_arg( exp_image.ptr() );
    convert_to_rgba_image_kernel->execute( queue, 2, local_threads, global_threads);
    clFinish(queue);
    gpu_time += convert_to_rgba_image_kernel->exec_time();
