...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  opts +=
    " -D STEP_CELL=step_cell_cubic_compute_probability_of_intensity(aux_args,data_ptr,d*linfo->block_len,vis,aux_args.prob_image) ";
...
bool boxm2_ocl_probability_of_image_wcubic_process(bprb_func_process& pro)
{
  ...
  if( !foundDataType )
    ...
    vcl_cout
      <<
    "boxm2_ocl_probability_of_image_wcubic_process ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
      << vcl_endl;
    return false;
    }
...
// : create a command queue.
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    bocl_mem* mog       = opencl_cache->get_data(scene, *id, data_type,
                                                 alpha->num_bytes() / alphaTypeSize * mogTypeSize, false);                   // info_buffer->data_buffer_length*boxm2_data_info::datasize(data_type));
