...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels)
{
  ...
  options +=
    " -D STEP_CELL=step_cell_render_depth2(tblock,linfo->block_len,aux_args.alpha,data_ptr,d*linfo->block_len,aux_args.vis,aux_args.expdepth,aux_args.expdepthsqr,aux_args.probsum,aux_args.t)";
...
bool boxm2_ocl_render_expected_depth_region_process(bprb_func_process& pro)
{
  ...
  bocl_mem_sptr prob_image = new bocl_mem(
      device->context(), prob_buff, cl_ni * cl_nj * sizeof(float), "vis x omega image buffer");
  prob_image->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr t_infinity = new bocl_mem(
      device->context(), t_infinity_buff, cl_ni * cl_nj * sizeof(float), "t infinity buffer");
  ...
  bvgl_2d_geo_index_node_sptr blk_id_tree_2d = bvgl_2d_geo_index::construct_tree<vcl_vector<boxm2_block_id> >(
      scene_bbox_2d, min_size);
  ...
  if( curr_leaf )
    {
    vcl_cout << " leaf " << curr_leaf->extent_ << " contains location: " << lon << " lat: " << lat << " elev: "
             << elev << " ( " << local_h_pt_d << vcl_endl;
    ...
    if( !found_blk )
      {
      vcl_cout << pro.name() << ": Scene does not contain location -- lon: " << lon << " lat: " << lat << " elev: "
               << elev
  ...
  vcl_cout << " Scene has location -- lon: " << lon << " lat: " << lat << " elev: " << elev << " in box "
           << curr_block << vcl_endl;
