...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string options)
{
  ...
  options +=
    " -D STEP_CELL=step_cell_render_depth2(tblock,aux_args.alpha,aux_args.mog,data_ptr,d*linfo->block_len,vis,aux_args.expdepth,aux_args.expdepthsqr,aux_args.probsum,aux_args.expint)";
...
bool boxm2_ocl_render_expected_height_map_process(bprb_func_process& pro)
{
  ...
  if( !foundDataType )
    ...
    vcl_cout
      <<
    "BOXM2_OCL_RENDER_EXPECTED_HEIGHT_MAP_PROCESS ERROR: scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
      << vcl_endl;
    return false;
    }
