...
bool boxm2_ocl_render_expected_image_naa_process(bprb_func_process& pro)
{
  ...
  if( !found_appearance )
    {
    vcl_cout << "BOXM2_OCL_RENDER_IMAGE_NAA_PROCESS ERROR: scene doesn't have BOXM2_NORMAL_ALBEDO_ARRAY data type"
             << vcl_endl;
    return false;
    }
  ...
  double       expected_radiance = brad_expected_radiance_chavez(expected_albedo, vgl_vector_3d<double>(0, 0,
                                                                                                        1), *metadata,
                                                                 *atm_params);
  background_rad_buff[0] = expected_radiance;
  bocl_mem_sptr background_rad = new bocl_mem(
      device->context(), background_rad_buff, sizeof(float), "background radiance value");
