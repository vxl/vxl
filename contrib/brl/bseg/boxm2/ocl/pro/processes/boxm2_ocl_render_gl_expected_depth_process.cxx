...
bool boxm2_ocl_render_gl_expected_depth_process(bprb_func_process& pro)
{
  ...
  bocl_mem_sptr vis_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), vis_buff,
                                                    "vis image (single float) buffer");
  ...
  bocl_mem_sptr prob_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), prob_buff,
                                                     "vis x omega image buffer");
  prob_image->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr t_infinity =
    opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), t_infinity_buff, "t infinity buffer");
  ...
  // normalize
    ...
    bocl_mem_sptr depth_scale_offset = new bocl_mem(
        device->context(), depth_scale_offset_buff, sizeof(float) * 2, "depth scale and offset buffer");
