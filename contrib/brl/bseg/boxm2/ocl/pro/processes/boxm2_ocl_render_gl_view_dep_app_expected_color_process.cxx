// This is brl/bseg/boxm2/ocl/pro/processes/boxm2_ocl_render_gl_view_dep_app_expected_color_process .cxx
// :
// \file
...
namespace boxm2_ocl_render_gl_view_dep_app_expected_color_process_globals
{
...
bool boxm2_ocl_render_gl_view_dep_app_expected_color_process_cons(bprb_func_process& pro)
{
  using namespace boxm2_ocl_render_gl_view_dep_app_expected_color_process_globals;
...
bool boxm2_ocl_render_gl_view_dep_app_expected_color_process(bprb_func_process& pro)
{
  using namespace boxm2_ocl_render_gl_view_dep_app_expected_color_process_globals;
  ...
  valid_types.push_back(boxm2_data_traits<BOXM2_GAUSS_RGB_VIEW_COMPACT>::prefix() );
  if( !boxm2_util::verify_appearance( *scene, valid_types, data_type, apptypesize ) )
    {
    vcl_cout
    << "boxm2_ocl_render_gl_view_dep_app_expected_image_process ERROR: scene doesn't have BOXM2_MOG6_VIEW data type"
    << vcl_endl;
    return false;
    }
  vcl_string options = boxm2_ocl_util::mog_options(data_type);

  vcl_cout << options << " " << data_type << " " << apptypesize << vcl_endl;
  ...
  bocl_mem_sptr exp_color = new bocl_mem(
      device->context(), buff, 4 * cl_ni * cl_nj * sizeof(float), "color im buffer (float4) buffer");
  exp_color->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  // visibility image
  float* vis_buff = new float[cl_ni * cl_nj];
  vcl_fill(vis_buff, vis_buff + cl_ni * cl_nj, 1.0f);
  bocl_mem_sptr vis_image = new bocl_mem(
      device->context(), vis_buff, cl_ni * cl_nj * sizeof(float), "vis image (single float) buffer");
  vis_image->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  float* max_omega_buff = new float[cl_ni * cl_nj];
  vcl_fill(max_omega_buff, max_omega_buff + cl_ni * cl_nj, 0.0f);
  bocl_mem_sptr max_omega_image = new bocl_mem(
      device->context(), max_omega_buff, cl_ni * cl_nj * sizeof(float), "max_omega_image image (single float) buffer");
