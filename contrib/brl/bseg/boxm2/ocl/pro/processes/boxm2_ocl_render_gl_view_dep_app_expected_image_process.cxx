// This is brl/bseg/boxm2/ocl/pro/processes/boxm2_ocl_render_gl_view_dep_app_expected_image_process.cxx
// :
// \file
...
namespace boxm2_ocl_render_gl_view_dep_app_expected_image_process_globals
{
...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  options +=
    " -D STEP_CELL=step_cell_render(aux_args.mog,aux_args.alpha,data_ptr,aux_args.app_model_weights,d*linfo->block_len,vis,aux_args.expint) ";
  ...
  ray_trace_kernel->create_kernel( &device->context(),
                                   ...
                                   options,                             // options
                                   "boxm2 opencl view dep app render"); // kernel identifier (for error checking)
...
bool boxm2_ocl_render_gl_view_dep_app_expected_image_process_cons(bprb_func_process& pro)
{
...
bool boxm2_ocl_render_gl_view_dep_app_expected_image_process(bprb_func_process& pro)
{
  using namespace boxm2_ocl_render_gl_view_dep_app_expected_image_process_globals;
  ...
  if( !boxm2_util::verify_appearance( *scene, valid_types, data_type, apptypesize ) )
    ...
    vcl_cout
      <<
    "boxm2_ocl_render_gl_view_dep_app_expected_image_process ERROR: scene doesn't have BOXM2_MOG6_VIEW or BOXM2_MOG6_VIEW_COMPACT data type"
      << vcl_endl;
    return false;
    }
  ...
  bocl_mem_sptr vis_image =
    new bocl_mem(device->context(), vis_buff, cl_ni * cl_nj * sizeof(float), "exp image buffer");
  ...
  bocl_mem_sptr max_omega_image = new bocl_mem(
      device->context(), max_omega_buff, cl_ni * cl_nj * sizeof(float), "vis image (single float) buffer");
