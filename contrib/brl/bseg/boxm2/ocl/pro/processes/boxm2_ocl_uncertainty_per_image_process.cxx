...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  vcl_string   pre_opts = options
    + " -D PREINF -D PREINF_CUBIC -D STEP_CELL=step_cell_preinf_cubic(aux_args,data_ptr,llid,d) ";
  ...
  proc_img->create_kernel(&device->context(),
                          device->device_id(), non_ray_src, "proc_norm_image", proc_norm_opts,
  ...
  vcl_string   avg_surface_empty_ratio_opt = options
    + "-D AVG_SURFACE_EMPTY_RATIO -D JOINT -D STEP_CELL=step_cell_avg_ratio_cubic(aux_args,data_ptr,llid,d) ";
  avg_surface_empty_ratio_main->create_kernel(&device->context(),
                                              device->device_id(), src_paths, "avg_surface_empty_ratio_main", avg_surface_empty_ratio_opt,
                                              "update::avg_surface_empty_ratio_main");
  ...
  convert_aux_int_float->create_kernel(&device->context(),
                                       device->device_id(), non_ray_src, "convert_aux_int_to_float", opts + " -D CONVERT_AUX ",
...
bool boxm2_ocl_uncertainty_per_image_process(bprb_func_process& pro)
{
  ...
  bocl_mem_sptr vis_inf_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), vis_inf_buff,
                                                        "vis inf image buffer");
  ...
  bocl_mem_sptr pre_inf_image = opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), pre_inf_buff,
                                                        "pre inf image buffer");
    ...
    if( i == UPDATE_SEGLEN )
      ...
      for( id = vis_order.begin(); id != vis_order.end(); ++id )
        ...
        bocl_mem * aux0   = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(
                                                     ident), info_buffer->data_buffer_length * auxTypeSize, false);
      ...
      for( id = vis_order.begin(); id != vis_order.end(); ++id )
        ...
        // data type string may contain an identifier so determine the buffer size
        bocl_mem* mog       = opencl_cache->get_data(scene, *id, data_type,
                                                     alpha->num_bytes() / alphaTypeSize * appTypeSize);                            // info_buffer->data_buffer_length*boxm2_data_info::datasize(data_type));
        bocl_mem* num_obs   = opencl_cache->get_data(scene, *id, num_obs_type,
                                                     alpha->num_bytes() / alphaTypeSize * nobsTypeSize);                           // ,info_buffer->data_buffer_length*boxm2_data_info::datasize(num_obs_type));
        // grab an appropriately sized AUX data buffer
        int        auxTypeSize = (int)boxm2_data_info::datasize(boxm2_data_traits<BOXM2_AUX0>::prefix() );
        bocl_mem * aux0   = opencl_cache->get_data(scene, *id, boxm2_data_traits<BOXM2_AUX0>::prefix(
                                                     ident), info_buffer->data_buffer_length * auxTypeSize, false);
