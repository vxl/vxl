...
bool compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  if( !pre_inf->create_kernel(&device->context(), device->device_id(), src_paths, "pre_inf_naa_main", pre_opts,
                              "update::pre_inf_naa") )
    {
    vcl_cerr << "ERROR compiling kernel\n";
    return false;
  ...
  if( !proc_img->create_kernel(&device->context(), device->device_id(), non_ray_src, "proc_norm_image", proc_opt,
                               "update::proc_norm_image") )
    {
  ...
  if( !bayes_main->create_kernel(&device->context(), device->device_id(), src_paths, "bayes_main_naa", bayes_opt,
                                 "update::bayes_naa_main") )
    {
    vcl_cerr << "ERROR compiling kernel\n";
    return false;
    }
  ...
  if( !update->create_kernel(&device->context(), device->device_id(), non_ray_src, "update_bit_scene_main_alpha_only",
                             update_opt, "update::update_main_alpha_only") )
    {
    vcl_cerr << "ERROR compiling kernel\n";
    return false;
    }
...
bool boxm2_ocl_update_alpha_naa_process(bprb_func_process& pro)
{
  ...
  if( !found_appearance )
    {
    vcl_cout << "BOXM2_OPENCL_UPDATE_ALPHA_NAA_PROCESS ERROR: scene doesn't have BOXM2_NORMAL_ALBEDO_ARRAY data type"
             << vcl_endl;
    return false;
    }
  if( !found_num_obs )
  ...
  *background_density_buffer = 1.0 / expected_dL_drho;
  ...
  bocl_mem_sptr background_density = new bocl_mem(
      device->context(), background_density_buffer, sizeof(cl_float), "background appearance density buffer");
