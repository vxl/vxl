// This is brl/bseg/boxm2/ocl/pro/processes/boxm2_ocl_update_alpha_wphongs_process.cxx
...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  src_paths.push_back(source_dir + "update_phongs_functors.cl");
  ...
  vcl_string   pre_opts = options
    + " -D PREINF -D PREINF_PHONGS -D STEP_CELL=step_cell_preinf_phongs(aux_args,data_ptr,llid,d) ";
  ...
  vcl_string   bayes_opt = options
    + " -D BAYES -D BAYES_PHONGS -D STEP_CELL=step_cell_bayes_phongs(aux_args,data_ptr,llid,d) ";
  ...
  update->create_kernel(&device->context(),
                        device->device_id(), non_ray_src, "update_bit_scene_main", options += "-D UPDATE_ALPHA ",
                        "update::update_main");
...
bool boxm2_ocl_update_alpha_wphongs_process_cons(bprb_func_process& pro)
{
  using namespace boxm2_ocl_update_alpha_wphongs_process_globals;
...
bool boxm2_ocl_update_alpha_wphongs_process(bprb_func_process& pro)
{
  using namespace boxm2_ocl_update_alpha_wphongs_process_globals;
  ...
  bocl_mem_sptr norm_image = new bocl_mem(
      device->context(), norm_buff, cl_ni * cl_nj * sizeof(float), "norm image buffer");
  ...
  for( unsigned int i = 0; i < kernels[identifier].size(); ++i )
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      bocl_mem* cubic_model       = opencl_cache->get_data(scene, *id, data_type,
