...
bool compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  if( !convert_aux_int_float->create_kernel(&device->context(), device->device_id(), src_paths,
                                            "convert_aux_int_to_float", opts + " -D CONVERT_AUX ",
  ...
  if( !seg_len->create_kernel(&device->context(), device->device_id(), src_paths, "seg_len_main", seg_opts,
                              "update::seg_len") )
    {
  ...
  if( !aux_previs_naa_kernel->create_kernel(&device->context(), device->device_id(), src_paths, "aux_previs_main_naa",
                                            aux_opt, "batch_update::aux_previs_main_naa") )
    {
...
bool boxm2_ocl_update_aux_per_view_naa_process(bprb_func_process& pro)
{
  ...
  // variances
  const double reflectance_var = boxm2_normal_albedo_array_constants::sigma_albedo
    * boxm2_normal_albedo_array_constants::sigma_albedo;
  const double airlight_var = boxm2_normal_albedo_array_constants::sigma_airlight
    * boxm2_normal_albedo_array_constants::sigma_airlight;
  const double optical_depth_var = boxm2_normal_albedo_array_constants::sigma_optical_depth
    * boxm2_normal_albedo_array_constants::sigma_optical_depth;
  const double skylight_var = boxm2_normal_albedo_array_constants::sigma_skylight
    * boxm2_normal_albedo_array_constants::sigma_skylight;
  ...
  if( !foundDataType )
    {
    vcl_cout << "BOXM2_OCL_UPDATE_AUX_PER_VIEW_NAA_PROCESS ERROR: scene doesn't have NORMAL_ALBEDO_ARRAY data type"
             << vcl_endl;
    return false;
    }
  ...
  for( unsigned n = 0; n < num_normals; ++n )
    ...
    double var_offset = brad_radiance_variance_chavez(0.0, normals[n], *metadata, *atm_params, reflectance_var,
                                                      optical_depth_var, skylight_var, airlight_var);
    radiance_var_offsets_buff[n] = var_offset;
    // compute scale
    double var = brad_radiance_variance_chavez(1.0, normals[n], *metadata, *atm_params, reflectance_var,
                                               optical_depth_var, skylight_var, airlight_var);
  ...
  vcl_cout << "allocating ray_o_buff: ni = " << cl_ni << ", nj = " << cl_nj << "  size = " << cl_ni * cl_nj
    * sizeof(cl_float4) << vcl_endl;
  ...
  bocl_mem_sptr radiance_scales = new bocl_mem(
      device->context(), radiance_scales_buff, sizeof(float) * num_normals, "radiance scales buffer");
  radiance_scales->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr radiance_offsets = new bocl_mem(
      device->context(), radiance_offsets_buff, sizeof(float) * num_normals, "radiance offsets buffer");
  radiance_offsets->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr radiance_var_scales = new bocl_mem(
      device->context(), radiance_var_scales_buff, sizeof(float) * num_normals, "radiance variance scales buffer");
  radiance_var_scales->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr radiance_var_offsets = new bocl_mem(
      device->context(), radiance_var_offsets_buff, sizeof(float) * num_normals, "radiance variance offsets buffer");
