...
void test_batch_update_kernels()
{
  ...
  for( ; iter != blk_map.end(); iter++ )
    ...
    boxm2_data_base * data_base =
      cache->get_data_base(scene, iter->first, boxm2_data_traits<BOXM2_AUX>::prefix() );
    boxm2_data<BOXM2_AUX> * aux_data = new boxm2_data<BOXM2_AUX>(data_base->data_buffer(),
                                                                 data_base->buffer_length(), data_base->block_id() );

    boxm2_data_base * hist_base =
      cache->get_data_base(scene, iter->first, boxm2_data_traits<BOXM2_BATCH_HISTOGRAM>::prefix() );
    boxm2_data<BOXM2_BATCH_HISTOGRAM> * hist_data = new boxm2_data<BOXM2_BATCH_HISTOGRAM>(
        hist_base->data_buffer(), hist_base->buffer_length(), hist_base->block_id() );
    boxm2_data_base * alpha_data_base  =
      cache->get_data_base(scene, iter->first, boxm2_data_traits<BOXM2_ALPHA>::prefix() );
