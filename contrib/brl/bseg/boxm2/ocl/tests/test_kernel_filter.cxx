...
bool test_gauss_filter()
{
  ...
  if(mgr->numGPUs()<1) {  
    vcl_cerr << "No GPU available" << vcl_endl;
    return false;
  }
...
{
  bool result = test_gauss_filter();
  TEST("Gauss response has a valid value", result, true);
  //result = test_gauss_x_filter();
  //TEST("Gauss_x response has a valid value", result, true);
