...
bool test_gauss_gradients_filter()
{
  ...
  for( unsigned k = 0; k < filter_vector->kernels_.size(); k++ )
    ...
    for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
      ...
      boxm2_data_base * response    = boxm2_cache::instance()->get_data_base(scene, id, boxm2_data_traits<BOXM2_FLOAT>::prefix(
                                                                               filter_ident.str() ) );
      boxm2_block_metadata data = blk_iter->second;

      // 3d array of trees
      const boxm2_array_3d<uchar16>&             trees = blk->trees();
      boxm2_data_traits<BOXM2_FLOAT>::datatype * response_data =
        (boxm2_data_traits<BOXM2_FLOAT>::datatype *)response->data_buffer();
        ...
        for( unsigned int y = 0; y < trees.get_row2_count(); ++y )
          ...
          for( unsigned int z = 0; z < trees.get_row3_count(); ++z )
            ...
            for( iter = leafBits.begin(); iter != leafBits.end(); ++iter )
              ...
              result = result &&
                ( (vcl_abs(response_data[currIdx] - val1) < 1e-5) || (vcl_abs(response_data[currIdx] - val2) < 1e-5) ||
                  (vcl_abs(response_data[currIdx] - 0.0f) < 1e-5) );
