...
void compute_ocl_cholesky(vnl_matrix<float>  A, vnl_vector<float>  b, vnl_vector<float>  x0, vnl_vector<float>  & x)
{
  ...
  cholesky_test.create_kernel(&device->context(),
                              device->device_id(), src_paths, "test_onl_cholesky", "", "test cholesky decomposition");

  bocl_kernel cholesky_solve_test;
  cholesky_solve_test.create_kernel(&device->context(),
                                    device->device_id(), src_paths, "test_onl_cholesky_solve", "",
...
void compute_vnl_cholesky(vnl_matrix<double> & A, vnl_vector<double>  b, vnl_vector<double>  x0,
                          vnl_vector<double>  & x)
{
