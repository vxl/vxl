                             ...
                             )
{
  ...
  bocl_kernel lm_test;
  lm_test.create_kernel(&device->context(),
                        device->device_id(), src_paths, "test_ocl_phongs_model_est", "-D PHONGS",
  ...
  bocl_mem_sptr obsbuff = new bocl_mem( device->context(), obs.data_block(),
                                        obs.size() * sizeof(float), "Vector for Obs");
  obsbuff->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr weightsbuff = new bocl_mem( device->context(), weights.data_block(),
                                            weights.size() * sizeof(float), "Vector for Weights");
  weightsbuff->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr sun_anglesbuff = new bocl_mem( device->context(),
                                               sun_angles.data_block(), 2 * sizeof(float), "Vector for Weights");
  sun_anglesbuff->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);

  bocl_mem_sptr viewdirsbuff = new bocl_mem( device->context(), viewdirs.data_block(),
                                             viewdirs.size() * sizeof(float), "Vector for Weights");
