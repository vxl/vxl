...
void mog_samples(float* samps)
{
  // mu = .25, sigma = .06
  float mode1[] =
   {0.30379690014757355, 0.28398201060058043, 0.2943911897717762, 0.29554249051465387, 0.25036897662946456,
   0.25549424398511705, 0.29891513517069201, 0.14853695403446637, 0.24113954487952871,
   0.26341571068280933,
   0.30260051941567706, 0.26134741888779478, 0.11880256142618917, 0.25101962163019631,
   0.23151293297545156,
   0.26720328010011379, 0.24732171438831776, 0.39769973987319168, 0.29549215792551908,
   0.2358631231366328,
   0.2139691167996427, 0.2823641459086581, 0.28999592189937706, 0.26198238775894345,
   0.33805484983932677};

  // mu = .75, sigma = .1
  float mode2[] =
   {0.87325394637314291, 0.69515977265953222, 0.79767398608666218, 0.80051357134604817, 0.81414255593445839,
   0.66897597424702882, 0.7406910756126307, 0.65699199445733247, 0.80719869327245219,
   0.67310697233514216,
   0.87185180539848361, 0.75278383121102233, 0.75820857425782995, 0.96191233327723635,
   0.69855343912868184 };

  // mu = .5, sigma = .15
  float mode3[] =
   {0.67146615989813541, 0.60330803063835836, 0.46633873293634936, 0.42366551340449088, 0.4707333498526044,
   0.74645874517355715, 0.47955437786233279, 0.37463417810951577, 0.38937639603429275,
   0.48731303837214823};
...
void test_weighted_em()
{
  ...
  vcl_cout << "CPP MOG-------------:\n"
           << "  mu0,sigma0,w0: " << (float) mog[0] / 255.0f << ", " << (float) mog[1] / 255.0f << ", "
           << (float)mog[2] / 255.0f << vcl_endl
           << "  mu1,sigma1,w1: " << (float) mog[3] / 255.0f << ", " << (float) mog[4] / 255.0f << ", "
           << (float)mog[5] / 255.0f << vcl_endl
           << "  mu2,sigma2,w2: " << (float) mog[6] / 255.0f << ", " << (float) mog[7] / 255.0f << ", "
           << (float)(255 - mog[2] - mog[5]) / 255.0f << vcl_endl;
  ...
  bocl_kernel em_test;
  em_test.create_kernel(&device->context(),
                        device->device_id(), src_paths, "test_weighted_em", opts, "test weighted em kernel");
  ...
  vcl_cout << "REAL MOG------------:\n"
           ...
           << (float)(255 - mog[2] - mog[5]) / 255.0f << vcl_endl
           << "OPENCL MOG-------------:\n"
           << "  mu0,sigma0,w0: " << (float) mogBuff[0] / 255.0f << ", " << (float) mogBuff[1] / 255.0f << ", "
           << (float)mogBuff[2] / 255.0f << vcl_endl
           << "  mu1,sigma1,w1: " << (float) mogBuff[3] / 255.0f << ", " << (float) mogBuff[4] / 255.0f << ", "
           << (float)mogBuff[5] / 255.0f << vcl_endl
           << "  mu2,sigma2,w2: " << (float) mogBuff[6] / 255.0f << ", " << (float) mogBuff[7] / 255.0f << ", "
           << (float)(255 - mogBuff[2] - mogBuff[5]) / 255.0f << vcl_endl;
}
