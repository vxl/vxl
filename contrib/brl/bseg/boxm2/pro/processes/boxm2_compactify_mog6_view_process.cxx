...
bool boxm2_compactify_mog6_view_process(bprb_func_process& pro)
{
  ...
  for( iter = blocks.begin(); iter != blocks.end(); iter++ )
    ...
    if( data_type == boxm2_data_traits<BOXM2_MOG6_VIEW>::prefix() )
      {
      boxm2_data_base* app =
        cache->get_data_base(scene, iter->first, boxm2_data_traits<BOXM2_MOG6_VIEW>::prefix(), 0);
      vnl_vector_fixed<float, 16>* app_buf = ( vnl_vector_fixed<float, 16> *)app->data_buffer();
      unsigned                     num_el = app->buffer_length()
        / boxm2_data_traits<BOXM2_MOG6_VIEW>::datasize();
      boxm2_data_base* app_compact =
        cache->get_data_base(scene, iter->first,
                             boxm2_data_traits<BOXM2_MOG6_VIEW_COMPACT>::prefix(),
                             num_el * boxm2_data_traits<BOXM2_MOG6_VIEW_COMPACT>::datasize() );
      vnl_vector_fixed<unsigned char,
                       16>* app_compact_buf = ( vnl_vector_fixed<unsigned char, 16> *)app_compact->data_buffer();
      ...
      {
      boxm2_data_base* app = cache->get_data_base(scene, iter->first,
                                                  boxm2_data_traits<BOXM2_GAUSS_RGB_VIEW>::prefix(), 0);
      ...
      boxm2_data_base*           app_compact =
        cache->get_data_base(scene, iter->first,
                             ...
                             num_el * boxm2_data_traits<BOXM2_GAUSS_RGB_VIEW_COMPACT>::datasize() );
