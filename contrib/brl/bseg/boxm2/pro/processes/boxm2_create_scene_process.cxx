...
bool boxm2_create_scene_and_blocks_process(bprb_func_process& pro)
{
  ...
  vcl_cout << "local coords:\nlat1,lon1,elev1: " << local_origin_x << ' ' << local_origin_y << ' ' << local_origin_z
           << vcl_endl;
  ...
  if( lx <= 0 || ly <= 0 || lz <= 0 )
    ...
    vcl_cout
      <<
    "error: negative width or height! select the upper right corner to the North-East of lower left corner of the scene!\n";
    }
  vcl_cout << "local origin of the scene: " << local_origin_x << ", " << local_origin_y << ", " << local_origin_z
           << " width: " << lx << " depth: " << ly << " height: " << lz << vcl_endl;

  boxm2_scene_sptr scene = new boxm2_scene(datapath, vgl_point_3d<double>(local_origin_x, local_origin_y,
                                                                          local_origin_z) );
  ...
  vcl_cout << "memory requirements for a block at finest resolution:\n"
           << n_subb << "=n_subblocks, " << n_bytes_subb << " bytes per sub-block, " << bytes / 1000000.0
           << " MB per block\n"
    ...
    for( int j = 0; j < n_y; ++j )
      ...
      for( int k = 0; k < n_z; ++k )
        ...
        if( blks.find(id) != blks.end() )
          {
          vcl_cout << "Problems in adding block: (" << i << ',' << j << ',' << k << ") block already exists"
                   << vcl_endl;
          return false;
          }
...
bool boxm2_create_poly_scene_and_blocks_process(bprb_func_process& pro)
{
  ...
  for( unsigned sh_idx = 0; sh_idx < n_sheets; sh_idx++ )
    ...
    for( unsigned vt_idx = 0; vt_idx < n_verts; vt_idx++ )
      ...
      lv.global_to_local(poly_deg[sh_idx][vt_idx].x(),
                         poly_deg[sh_idx][vt_idx].y(), 1.6, vpgl_lvcs::wgs84, local_x, local_y, local_z);
...
bool boxm2_distribute_scene_blocks_process(bprb_func_process& pro)
{
  ...
  if( cnt != blks.size() )
    {
    vcl_cerr << "Not all blocks are added to one of the small scene!!!! cnt: " << cnt << " blocks cnt: "
             << blks.size() << '\n';
    }
  ...
  for( double orig_x = bb.min_point().x(); orig_x <= bb.max_point().x(); orig_x += scene_dim )
    ...
    for( double orig_y = bb.min_point().y(); orig_y <= bb.max_point().y(); orig_y += scene_dim )
      ...
      small_scene_box.add(vgl_point_2d<double>(orig_x + scene_dim - scene_dim / 1000,
                                               orig_y + scene_dim - scene_dim / 1000) );
...
bool find_min_max_height(vgl_point_2d<double> const& lower_left, vgl_point_2d<double> const& upper_right,
                         vcl_vector<vil_image_view_base_sptr>& dem_views, vcl_vector<vpgl_geo_camera *>& dem_cams,
                         vcl_vector<vgl_box_2d<double> >& dem_bbox, double& min_elev, double& max_elev);

// : crop and find the min/max value
void crop_and_find_min_max(vcl_vector<vil_image_view_base_sptr>& dem_views, unsigned const& img_id, int const& i0,
                           int const& j0, int const& c_ni, int const& c_nj, double& min, double& max);
...
void boxm2_prune_scene_blocks_by_dem_process_globals::crop_and_find_min_max(
  vcl_vector<vil_image_view_base_sptr>& dem_views, unsigned const& img_id,
  int const& i0, int const& j0,
  int const& c_ni, int const& c_nj,
...
bool boxm2_prune_scene_blocks_by_dem_process_globals::find_min_max_height(vgl_point_2d<double> const& lower_left,
                                                                          vgl_point_2d<double> const& upper_right,
                                                                          vcl_vector<vil_image_view_base_sptr>& dem_views,
                                                                          vcl_vector<vpgl_geo_camera *>& dem_cams,
                                                                          vcl_vector<vgl_box_2d<double> >& dem_bbox,
                                                                          double& min_elev, double& max_elev)
{
  ...
  for( unsigned i = 0; i < corners.size(); i++ )
    {
    vcl_cout <<  " dem img id : " << corners[i].first << " pixel: " << corners[i].second.first << "x"
             << corners[i].second.second << vcl_endl;
    }
  if( corners.size() != 4 )
    {
  ...
  if( corners[0].first == corners[1].first )
    ...
    boxm2_prune_scene_blocks_by_dem_process_globals::crop_and_find_min_max(dem_views, corners[0].first, i0, j0, crop_ni,
                                                                           crop_nj, min_elev, max_elev);
    return true;
    }
  ...
  if( corners[0].first == corners[2].first && corners[1].first == corners[3].first )
    ...
    boxm2_prune_scene_blocks_by_dem_process_globals::crop_and_find_min_max(dem_views, corners[1].first, i0, j0, crop_ni,
                                                                           crop_nj, min_elev, max_elev);
    return true;
    }
  ...
  crop_nj = corners[2].second.second + 1;
  boxm2_prune_scene_blocks_by_dem_process_globals::crop_and_find_min_max(dem_views, corners[2].first, i0, j0, crop_ni,
                                                                         crop_nj, min_elev, max_elev);
  ...
  boxm2_prune_scene_blocks_by_dem_process_globals::crop_and_find_min_max(dem_views, corners[3].first, i0, j0, crop_ni,
                                                                         crop_nj, min_elev, max_elev);
...
void change_block_metadata(boxm2_block_metadata& md, volm_osm_category_io::geo_cover_values const& land_cover,
                           int const& refine_coef)
{
...
bool boxm2_change_scene_res_by_geo_cover_process(bprb_func_process& pro)
{
  ...
  for( vcl_map<boxm2_block_id, boxm2_block_metadata>::iterator mit = blks.begin(); mit != blks.end(); ++mit )
    ...
    unsigned key = (blk_id.i() + blk_id.j() ) * (blk_id.i() + blk_id.j() + 1) / 2
      + blk_id.j();
        ...
        for( unsigned j = bd_min_nj; j <= bd_max_nj && loop; j++ )
          ...
          if( (*img)(i, j) == volm_osm_category_io::GEO_URBAN )
            ...
            land_cover = volm_osm_category_io::GEO_URBAN;  loop = false;
                          ...
                          j) == volm_osm_category_io::GEO_AGRICULTURE_GENERAL ||
                   (*img)(i, j) == volm_osm_category_io::GEO_AGRICULTURE_GENERAL )
            {
