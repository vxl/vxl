...
bool upsample_dem(vil_image_resource_sptr& out_img_res, unsigned ni, unsigned nj, vil_image_view<float>* dem_view,
                  unsigned dem_ni, unsigned dem_nj)
{
...
bool boxm2_dem_to_xyz_process(bprb_func_process& pro)
{
  ...
  if( !dem_view )
    ...
    if( !dem_view_int )
      ...
      if( !dem_view_byte )
        {
        vcl_cerr << "Error: boxm2_dem_to_xyz_process: The image pixel format: " << dem_view_base->pixel_format()
                 << " is not supported!\n";
        return false;
        }
      else
...
bool boxm2_shadow_heights_to_xyz_process(bprb_func_process& pro)
{
  ...
  vcl_cout << "scene projected in the image mini: " << min_i << " minj: " << min_j << " maxi: " << max_i << " maxj: "
           << max_j << vcl_endl;
