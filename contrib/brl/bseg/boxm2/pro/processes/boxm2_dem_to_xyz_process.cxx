...
bool upsample_dem(vil_image_resource_sptr& out_img_res, unsigned ni, unsigned nj, vil_image_view<float>* dem_view, unsigned dem_ni, unsigned dem_nj) {
  ...
  vcl_cout << "ni: " << ni << " dem_ni: " << dem_ni << " block_ni: " << block_ni << vcl_endl;
  vcl_cout << "nj: " << nj << " dem_nj: " << dem_nj << " block_nj: " << block_nj << vcl_endl;
...
bool boxm2_shadow_heights_to_xyz_process_cons(bprb_func_process& pro)
{
  ...
  vcl_vector<vcl_string> input_types_(n_inputs_);
  input_types_[0] = "boxm2_scene_sptr";
  input_types_[1] = "vil_image_view_base_sptr";  // shadow height map image of the cropped ortho aerial image, heigth values are in pixels with respect to a horizontal surface at the base of the object
...
bool boxm2_shadow_heights_to_xyz_process(bprb_func_process& pro)
{
  ...
  if (gcam->rows() != nj || gcam->cols() !=ni) {
    ...
    vcl_cerr << "gcam rows: " << gcam->rows() << " cols: " << gcam->cols() << " ni: " << ni << " nj: " << nj << vcl_endl;
    return false;
  }
