...
void boxm2_texture_mesh_from_imgs(vcl_string im_dir, vcl_string cam_dir, imesh_mesh& in_mesh, vcl_map<vcl_string,
                                                                                                      imesh_mesh>& meshes);
...
void boxm2_visible_faces( vcl_vector<vpgl_perspective_camera<double> *>& cameras,
                          vcl_vector<vil_image_view<int> *>& vis_images, imesh_mesh& in_mesh);

// checks if a triangle in UV space is visible.  us and vs are double buffers of length 3
bool face_is_visible( vpgl_perspective_camera<double>* cam, vil_image_view<int>* vis_img, double* us, double* vs,
                      int face_id);
                                                                              ...
                                                                              vcl_map<vcl_string, imesh_mesh>& meshes)
{
  ...
  for( unsigned int i = 0; i < camfiles.size(); ++i )
    ...
    vcl_cout << icam->principal_axis() << vcl_endl;
    }
...
void boxm2_export_textured_mesh_process_globals::boxm2_visible_faces(
  vcl_vector<vpgl_perspective_camera<double> *>& cameras,
  vcl_vector<vil_image_view<int> *>& vis_images,
  imesh_mesh& in_mesh)
{
