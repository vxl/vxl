...
bool boxm2_export_textured_mesh_process(bprb_func_process& pro)
{
  ...
  vcl_cout<<"Scene 2d bounding box (xy) : "<<scene_xy<<'\n'
          <<"Scene 3d bounding box (xyz): "<<scene->bounding_box()<<vcl_endl;
  ...
  for (apps = app_faces.begin(); apps != app_faces.end(); ++apps)
  {
    ...
    vcl_auto_ptr<imesh_vertex_array_base> v3(vs);

    //instantiate face array
    imesh_face_array_base* fs = new imesh_face_array();
    vcl_auto_ptr<imesh_face_array_base> f3(fs);
