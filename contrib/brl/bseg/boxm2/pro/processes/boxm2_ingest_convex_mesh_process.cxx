...
bool boxm2_ingest_convex_mesh_process(bprb_func_process& pro)
{
  ...
  for( iter = blocks.begin(); iter != blocks.end(); iter++ )
    ...
    boxm2_data_base *    alpha  = cache->get_data_base(scene, iter->first,
                                                       boxm2_data_traits<BOXM2_ALPHA>::prefix(), 0, true);
    int len_buffer  = alpha->buffer_length() / 4 * boxm2_data_info::datasize(
        "boxm2_label_short");
    ...
    for( unsigned nm = 0; nm < meshes.size(); nm++ )
      ...
      if( !rbox.is_empty() )
        ...
        if( !flag )
          ...
          label_data = new boxm2_data<BOXM2_LABEL_SHORT>(label_data_base->data_buffer(),
                                                         label_data_base->buffer_length(), iter->first);
          flag = true;
          }
          ...
          for( unsigned int j = min_j; j < max_j; j++ )
            ...
            for( unsigned int k = min_k; k < max_k; k++ )
              ...
              for( unsigned t = 0; t < leaf_bits.size(); t++ )
                ...
                vgl_point_3d<double> global_cc(
                  ( (double)i + cc.x() ) * mdata.sub_block_dim_.x() + mdata.bbox().min_x(),
                  ( (double)j + cc.y() ) * mdata.sub_block_dim_.y() + mdata.bbox().min_y(),
                  ( (double)k + cc.z() ) * mdata.sub_block_dim_.z()
                  + mdata.bbox().min_z() );
                bool inside = true;
                face_it = fmap.begin();
    ...
    if( flag )
      {
      boxm2_sio_mgr::save_block_data_base(scene->data_path(), iter->first, label_data_base,
                                          boxm2_data_traits<BOXM2_LABEL_SHORT>::prefix(identifier) );
      }
    }
