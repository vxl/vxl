      ...
      for( unsigned int x = 0; x < trees.get_row1_count(); ++x )
        ...
        for( unsigned int y = 0; y < trees.get_row2_count(); ++y )
          ...
          for( unsigned int z = 0; z < trees.get_row3_count(); ++z )
            ...
            vgl_box_3d<double> tree_box(tree_min_pt, boxm2_metadata.sub_block_dim_.x(),
                                        boxm2_metadata.sub_block_dim_.y(),
                                        boxm2_metadata.sub_block_dim_.z(), vgl_box_3d<double>::min_pos);
            if( !vgl_intersection<double>(tree_box, box).is_empty() ) // if the tree intersects the box
              ...
              for( int j = 0; j < MAX_CELLS_; ++j )
                ...
                vgl_point_3d<double> cellCenter_global(
                  float(cellCenter.x() * boxm2_metadata.sub_block_dim_.x() + boxm2_metadata.local_origin_.x() ),
                  float(cellCenter.y() * boxm2_metadata.sub_block_dim_.y() + boxm2_metadata.local_origin_.y() ),
                  float(cellCenter.z() * boxm2_metadata.sub_block_dim_.z()
                        + boxm2_metadata.local_origin_.z() ) );
                vgl_box_3d<double> cell_box(cellCenter_global,
                                            boxm2_metadata.sub_block_dim_.x() * side_len,
                                            boxm2_metadata.sub_block_dim_.y() * side_len,
                                            boxm2_metadata.sub_block_dim_.z() * side_len,
