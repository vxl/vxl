...
void boxm2_texture_mesh_from_imgs(vcl_string im_dir, vcl_string cam_dir, vcl_string out_dir, imesh_mesh& in_mesh,
                                  vcl_map<vcl_string, imesh_mesh>& meshes);

// populates a vector of visibility images (by face id int)
void boxm2_visible_faces( vcl_vector<vpgl_perspective_camera<double> *>& cameras, imesh_mesh& in_mesh,  unsigned int ni,
                          unsigned int nj);

// stores a norm image (3d face norm) for each of the texture images
void compute_norm_images( vcl_vector<vpgl_perspective_camera<double> *>& cameras, imesh_mesh& in_mesh, unsigned int ni,
                          unsigned int nj);
...
bool face_is_visible( vpgl_perspective_camera<double>* cam, vil_image_view<int>* vis_img, triangle_3d& world_tri);

// matches textures
void boxm2_match_textures(vcl_vector<vpgl_perspective_camera<double> *>& cameras, vcl_vector<vcl_string>& imfiles,
                          imesh_mesh& in_mesh, vcl_map<vcl_string, vcl_vector<unsigned> >& app_faces,
...
vcl_vector<triangle_3d> get_visible_triangles(vpgl_perspective_camera<double>* cam, vil_image_view<int>* vis_img,
                                              triangle_3d& world_tri);

// given revised lists of visible images, norms, etc, return the index of the best view
int get_best_view(vcl_vector<vpgl_perspective_camera<double> *>& cams, vcl_vector<vil_image_view<int> *>& vis_images,
                  vcl_vector<vil_image_view<float> *>& normx, vcl_vector<vil_image_view<float> *>& normy,
                  vcl_vector<vil_image_view<float> *>& normz, triangle_3d& world_tri );

vgl_vector_3d<double> calc_smooth_norm( vpgl_perspective_camera<double>* cam, vil_image_view<int>* vis_img,
                                        vil_image_view<float>* normx, vil_image_view<float>* normy,
                                        vil_image_view<float>* normz, triangle_3d& world_tri);
...
void boxm2_texture_mesh_process_globals::boxm2_match_textures(vcl_vector<vpgl_perspective_camera<double> *>& cameras,
                                                              ...
                                                              vcl_map<vcl_string,
                                                                      vpgl_perspective_camera<double> *>& texture_cams)
{
  ...
  for( unsigned iface = 0; iface < nfaces; ++iface )
    ...
    int closeIdx =
      get_best_view(visible_views, vis_images, normx, normy, normz, world_tri);
  ...
  bvgl_triangle_interpolation_iterator(double * verts_x, double * verts_y, T * values, unsigned int v0 = 0,
                                       unsigned int v1 = 1, unsigned int v2 = 2);
...
void boxm2_texture_mesh_process_globals::boxm2_visible_faces( vcl_vector<vpgl_perspective_camera<double> *>& cameras,
                                                              imesh_mesh& in_mesh, unsigned int ni = 1024,
                                                              unsigned int nj = 768)
{
  imesh_face_array& in_faces = (imesh_face_array &) in_mesh.faces();
