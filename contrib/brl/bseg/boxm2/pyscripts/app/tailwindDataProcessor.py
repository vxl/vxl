...
def convertcamera(srcFileName, dstFileName):
	...
	T=[0,0,0];
	for i in range(0,3):
		T[i]= Rbae[i]*Cbae[0]+Rbae[i+3]*Cbae[1]+Rbae[i+6]*Cbae[2];
...
class TailwindPipelineRunner():
	...
	def runPipeline2(self):
			...
			steps = [
		        self.moveData,		 # Step 1. Find the PNGs in the data directory, convert them to JPGs and write them out to a new folder.
		        self.createSceneFromBox, # Step 2. Create the scene using the .nvm file produced above.
	...
	def updateScene(self, stepNum):
		...
		camsDir = os.path.abspath(os.path.join( self.config.NVMOutputDirName, "cams"))
	...
	def extractCellSize(self, stepNum):
		...
		if False == os.path.isfile(self.config.uSceneFileName):
			log("Step %d - Cell size extraction failed because the uScene file can't be found at %s." % (stepNum, self.config.uSceneFileName))
			return False, None

		dom = parse(self.config.uSceneFileName)
		xmlTag = dom.getElementsByTagName("block")
		if 0 == len(xmlTag):
			log("Step %d - No \"block\" elements could be found in the XML document %s." % (stepNum, self.config.uSceneFileName))
			return False, None

		if False == xmlTag[0].hasAttribute("dim_x"):
			log("Step %d - The XML document %s Has a \"block\" element has no \"dim_x\" attribute." % (stepNum, self.config.uSceneFileName))
			return False, None
		...
		try:
			...
			log("Step %d - The parsed value of dimX %s appears to be invalid." % (stepNum, dimXValue))
			return False, None
...
class PipelineConfig():
	...
	def isConfigComplete(self):
		...
		if self.resumeAtStep < 0 or self.resumeAtStep > 3:
			log("The \"step\" value of %d is not valid. Valid values are in the range of 0-3 (inclusive)." % self.resumeAtStep)
			isOK = False
...
if __name__ == '__main__':
	...
	try:
		opts, args = getopt.getopt(sys.argv[1:], "d:b:i:e:rs:u:", ["dryrun","bbfile=", "inputlist=", "exportDir=", "randomize", "step=", "updatePassCount="])
		print opts
