...
print 'Deprecated! See boxm2/vpgl_adaptor.py for correct access to vpgl_adaptors'
# import sys
...
def convert_to_local_coordinates2(lvcs, lat, lon, el):
    ...
    boxm2_batch.set_input_double(2, lon)
    boxm2_batch.set_input_double(3, el)
    boxm2_batch.run_process()
    (id, type) = boxm2_batch.commit_output(0)
    x = boxm2_batch.get_output_double(id)
    boxm2_batch.remove_data(id)
    (id, type) = boxm2_batch.commit_output(1)
    y = boxm2_batch.get_output_double(id)
    boxm2_batch.remove_data(id)
    (id, type) = boxm2_batch.commit_output(2)
    z = boxm2_batch.get_output_double(id)
    boxm2_batch.remove_data(id)
    return (x, y, z)
# convert lat,lon,el to local coordinates;
