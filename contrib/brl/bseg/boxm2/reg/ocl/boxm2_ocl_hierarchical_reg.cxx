...
boxm2_ocl_hierarchical_reg::boxm2_ocl_hierarchical_reg( boxm2_opencl_cache_sptr  & cache,
                                                        ...
                                                        int nbins, bool do_vary_scale) :  boxm2_ocl_reg_mutual_info(
    cache, sceneA, sceneB, device, nbins, do_vary_scale), do_vary_scale_(do_vary_scale)
{
...
bool boxm2_ocl_hierarchical_reg::exhaustive()
{
  samples_.clear();
  int          nlevels = 4;
  ...
  for( unsigned int level = 0; level < 3; level++ )
    ...
    for( unsigned int j = 0; j < numbestparticales[level] && iter != samples_sorted.rend(); j++, iter++ )
      ...
      for( unsigned int sampleno = 0; sampleno < numsamples_per_best_particle; sampleno++ )
        ...
        double minfo = this->mutual_info(curr_sample, level);
        mis.push_back(minfo);
    ...
    vcl_cout << "Mutual Infor for Max Sample" << this->max_sample() - mu_ << " is " << this->mutual_info(
      this->max_sample(), level) << vcl_endl;
