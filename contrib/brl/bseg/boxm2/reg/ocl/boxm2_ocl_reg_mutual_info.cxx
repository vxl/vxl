...
double boxm2_ocl_reg_mutual_info::mutual_info(vgl_rotation_3d<double> rot, vgl_vector_3d<double> trans, double scale,
                                              int depth)
{
  float val = 0.0f;
...
bool boxm2_ocl_reg_mutual_info::compile_kernel()
{
  ...
  return kern->create_kernel(&device_->context(), device_->device_id(), src_paths, "estimate_mi_blockwise_vol", "",
                             "MI");
}
                                                         ...
                                                         float & mi)
{
  ...
  bocl_mem * joint_histogram = new bocl_mem(
      device_->context(), joint_histogram_buff, sizeof(int) * nbins * nbins, "joint histogram" );
  ...
  vcl_vector<boxm2_block_id>           blocks_A = sceneA_->get_block_ids();
  ...
  for( ; iter_blks_A != blocks_A.end(); iter_blks_A++ )
    ...
    global_threads[0] = (unsigned) RoundUp(
        info_buffer->scene_dims[0] * info_buffer->scene_dims[1] * info_buffer->scene_dims[2], (int)local_threads[0]);
    for( iter_blks_B = blocks_B.begin(); iter_blks_B != blocks_B.end(); iter_blks_B++ )
      ...
      vgl_box_3d<float> box_B(info_buffer_B->scene_origin[0], info_buffer_B->scene_origin[1],
                              info_buffer_B->scene_origin[2],
      ...
      if( !vgl_intersection<float>(box_A_xformed, box_B).is_empty() )
        ...
        bocl_mem* blk_info_B  =
          new bocl_mem(device_->context(), info_buffer_B, sizeof(boxm2_scene_info), " Scene Info" );
  ...
  if( sum <= 0.0 )
    ...
    for( int k = 0; k < nbins; k++ )
      ...
      for( int l = 0; l < nbins; l++ )
        {
        entropyAB +=
          -(joint_histogram_float[k * nbins + l] >
            0 ? joint_histogram_float[k * nbins + l] * vcl_log(joint_histogram_float[k * nbins + l]) : 0);
        }
      }
