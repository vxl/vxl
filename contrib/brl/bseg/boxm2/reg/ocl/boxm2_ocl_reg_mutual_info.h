...
public:
  ...
  boxm2_ocl_reg_mutual_info(boxm2_opencl_cache_sptr& opencl_cache, boxm2_scene_sptr& sceneA, boxm2_scene_sptr& sceneB,
                            bocl_device_sptr device, int nbins, bool do_vary_scale = false);
  ~boxm2_ocl_reg_mutual_info();

  // : Mutual information between the destination and mapped source image
  double mutual_info(vgl_rotation_3d<double> rot, vgl_vector_3d<double> trans, double scale,  int depth  = 3 );
...
protected:
  ...
  bool boxm2_ocl_register_world(vgl_rotation_3d<double>  rot, vgl_vector_3d<double> tx, double s, int nbins, int depth,
                                float & mi);
