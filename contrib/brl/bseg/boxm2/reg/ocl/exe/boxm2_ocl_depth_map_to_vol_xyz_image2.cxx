...
int main(int argc,  char** argv)
{
  ...
  if (cnt == 0) {
    ...
    vcl_cout << "total bytes per block: " << cache->total_bytes_per_block_ << " per data: " << cache->total_bytes_per_data_["alpha"] << "\n";
  ...
  vcl_cout << "Initial value at " << x[0] << ", " << x[1] << ", " << x[2] << " is " << mi << "\n";
  ofs << x[0] << " " << x[1] << " " << mi << "\n";
  ...
  secs = t.all()/1000.0;
  vcl_cout<<"Time "<< secs << " secs " << secs/60.0 << " mins."<<vcl_endl;
  vcl_cout << "Best mi: " << best_mi <<" at x: " << best_x << " y: " << best_y << " z: " << best_z << "\n";
  vcl_cout.flush();
  ofs.close();
