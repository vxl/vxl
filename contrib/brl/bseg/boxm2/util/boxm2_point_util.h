...
public:
  ...
  static bool axis_align_scene( vcl_vector<bwm_video_corr_sptr>        & corrs, vcl_vector<CamType> & cams);
  ...
  static void calc_projection_error(vcl_vector<CamType>                & cams,
                                    ...
                                    vcl_vector<bwm_video_corr_sptr>    & corrs, vcl_map<unsigned,
                                                                                        double>           & view_error_map, vcl_map<unsigned,
                                                                                                                                    unsigned>         & view_count_map );

  // takes view error map and view count map, and sets bad cams and reports error
  static void report_error( vcl_map<unsigned, double>    & view_error_map, vcl_map<unsigned,
                                                                                   unsigned>  & view_count_map,
                            vcl_set<int>                & bad_cams, float                       filter_thresh = 1.5f);
