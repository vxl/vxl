...
// constructors
boxm2_vecf_cranium_scene::boxm2_vecf_cranium_scene(std::string const& scene_file): boxm2_vecf_articulated_scene(scene_file), source_model_exists_(true), alpha_data_(VXL_NULLPTR), app_data_(VXL_NULLPTR), nobs_data_(VXL_NULLPTR), cranium_data_(VXL_NULLPTR), intrinsic_change_(false){
...
boxm2_vecf_cranium_scene::boxm2_vecf_cranium_scene(std::string const& scene_file, std::string const& geometry_file, std::string const& params_file_name):
...
bool boxm2_vecf_cranium_scene::find_nearest_data_index(boxm2_vecf_cranium_scene::anat_type type, vgl_point_3d<double> const& probe, double cell_len, unsigned& data_indx, int& found_depth) const{
   ...
   std::vector<cell_info> ccs = blk_->cells_in_box(probe_box);
   unsigned dindx = 0;
   int depth_min = 0;
...
void  boxm2_vecf_cranium_scene::inverse_vector_field(std::vector<vgl_vector_3d<double> >& vf,
  ...
  std::cout << "computed " << cnt << " pts out of "<< nt << " for cranium vector field in " << t.real()/1000.0 << " sec.\n";
}
...
void boxm2_vecf_cranium_scene::apply_vector_field_to_target(std::vector<vgl_vector_3d<double> > const& vf,
  ...
  std::cout << "Apply cranium vector field in " << t.real()/1000.0 << " sec.\n";
}
...
void boxm2_vecf_cranium_scene::prerefine_target(boxm2_scene_sptr target_scene){
  ...
  std::cout << "count0 " << count0 << " count1 " << count1 << " count2 " << count2 << " count3 " << count3 << '\n';
