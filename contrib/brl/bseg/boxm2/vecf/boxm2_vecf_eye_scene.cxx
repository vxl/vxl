...
void boxm2_vecf_eye_scene::extract_block_data()
{
  ...
  boxm2_data_base * sphere_dist_base  =
    boxm2_cache::instance()->get_data_base(base_model_, *iter_blk, boxm2_data_traits<BOXM2_FLOAT>::prefix(
                                             "sphere_dist") );
  sphere_dist_base->enable_write();
  sphere_dist_ = new boxm2_data<BOXM2_FLOAT>(sphere_dist_base->data_buffer(),
                                             sphere_dist_base->buffer_length(), sphere_dist_base->block_id() );
...
void boxm2_vecf_eye_scene::extract_target_block_data(boxm2_scene_sptr target_scene, vcl_string const& app_id)
{
  ...
  boxm2_data_base * app_base  =
    boxm2_cache::instance()->get_data_base(target_scene, *iter_blk,
                                           boxm2_data_traits<BOXM2_MOG3_GREY>::prefix(app_id) );
  app_base->enable_write();
  target_app_data_ = new boxm2_data<BOXM2_MOG3_GREY>(app_base->data_buffer(),
                                                     app_base->buffer_length(), app_base->block_id() );

  boxm2_data_base * nobs_base  =
    boxm2_cache::instance()->get_data_base(target_scene, *iter_blk, boxm2_data_traits<BOXM2_NUM_OBS>::prefix(app_id) );
  nobs_base->enable_write();
...
boxm2_vecf_eye_scene::boxm2_vecf_eye_scene(vcl_string const& scene_file, bool initialize) :
  base_model_(new boxm2_scene(scene_file) ), alpha_data_(0), app_data_(0), nobs_data_(0), sphere_(0), sphere_dist_(0),
  iris_(0), pupil_(0)
{
...
void boxm2_vecf_eye_scene::find_cell_neigborhoods()
{
  ...
  for( unsigned i = 0; i < sphere_cell_centers_.size(); i++ )
    ...
    for( unsigned j = 0; j < temp.size(); ++j )
      ...
      double                d =
        vcl_sqrt( (p.x() - q.x() ) * (p.x() - q.x() ) + (p.y() - q.y() ) * (p.y() - q.y() ) + (p.z() - q.z() )
                  * (p.z() - q.z() ) );
...
void boxm2_vecf_eye_scene::apply_vector_field_to_target(vcl_vector<vgl_vector_3d<double> > const& vf)
{
  ...
  for( int j = 0; j < n; ++j )
    ...
    double                     sumw = gauss(dc, params_.gauss_sigma() ), sumint = app[0] * sumw,
      sumalpha = alpha0 * sumw;
