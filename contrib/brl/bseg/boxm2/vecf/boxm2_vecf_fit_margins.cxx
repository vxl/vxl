...
public:
  margin_residual_function(vcl_vector<vgl_point_2d<double> >  const& inf_pts,
    ...
    vnl_least_squares_function(7,
                               static_cast<unsigned>(inf_pts.size() + sup_pts.size() + crease_pts.size() + 2
                                                     * ( (inf_pts.size() + sup_pts.size() ) / 2) + 1),
                               vnl_least_squares_function::no_gradient), inf_pts_(inf_pts),
    sup_pts_(sup_pts),  crease_pts_(crease_pts), lat_canth_(lat_canth), med_canth_(med_canth), opr_(opr), is_right_(
      is_right), estimate_t_(estimate_t) {}
  ...
  virtual void f(vnl_vector<double> const& x, vnl_vector<double>& fx)
  {
    ...
    unsigned                 ninf = static_cast<unsigned>(inf_pts_.size() ),
      nsup = static_cast<unsigned>(sup_pts_.size() );
...
double boxm2_vecf_fit_margins::fit(vcl_ostream* outstream, bool verbose)
{
  ...
  margin_residual_function mrf(inferior_margin_pts_, superior_margin_pts_, superior_crease_pts_, lateral_canthus_,
                               medial_canthus_, opr_, is_right_, estimate_t_);
...
bool boxm2_vecf_fit_margins::plot_orbit(vcl_ostream& ostr) const
{
  ...
  vgl_vector_2d<double>    v(opr_.x_trans(), opr_.y_trans() );
  double                   tinf = opr_.lower_eyelid_tmin_, tsup = opr_.eyelid_tmin_, ct = opr_.eyelid_crease_ct_;
  unsigned                 ninf = static_cast<unsigned>(inferior_margin_pts_.size() ),
    nsup = static_cast<unsigned>(superior_margin_pts_.size() );
