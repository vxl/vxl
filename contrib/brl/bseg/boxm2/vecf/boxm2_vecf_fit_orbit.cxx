                                                ...
                                                vcl_string const& dlabel, vcl_string const& olabel)
{
  ...
  if( (pts.size() == 1) && !circ_pt )
    ...
    // compute iris radius
    double d = 0.5
      * vcl_sqrt( (p0.x() - p1.x() ) * (p0.x() - p1.x() ) + (p0.y() - p1.y() ) * (p0.y() - p1.y() ) );
...
bool boxm2_vecf_fit_orbit::left_eye_inferior_lid_thickness(vcl_string const& data_desc, double& dr)
{
  ...
  double x0 = left_params_.x_trans(), y0 = left_params_.y_trans() + left_params_.y_off_,
    z0 = left_params_.z_trans(), r = left_params_.eye_radius_;
...
bool boxm2_vecf_fit_orbit::right_eye_inferior_lid_thickness(vcl_string const& data_desc, double& dr)
{
  ...
  double x0 = right_params_.x_trans(), y0 = right_params_.y_trans() + right_params_.y_off_,
    z0 = right_params_.z_trans(), r = right_params_.eye_radius_;
  vcl_vector<double> dr_vals;
  for( vcl_vector<vgl_point_3d<double> >::const_iterator pit = pts.begin();
       pit != pts.end(); ++pit )
    {
...
bool boxm2_vecf_fit_orbit::display_orbit_vrml(vcl_ofstream& ostr, bool is_right, bool show_model)
{
  ...
  bvrml_write::write_vrml_cylinder(ostr, vgl_point_3d<double>(0.0, cy, r - 0.3),  vgl_vector_3d<double>(0.0, 0.0,
                                                                                                        1.0), irad, 0.5f, 0.0f, 0.5f,
                                   0.75f);
  bvrml_write::write_vrml_cylinder(ostr, vgl_point_3d<double>(0.0, cy, r + 0.25), vgl_vector_3d<double>(0.0, 0.0,
                                                                                                        1.0), prad, 0.1f, 0.0f, 0.0f,
                                   0.0f);
