...
void boxm2_vecf_head_model::map_to_target(boxm2_scene_sptr target_scene)
{
  ...
  for( vcl_vector<boxm2_block_id>::iterator tblk = target_blocks.begin();
    ...
    boxm2_data_base * target_color =
      boxm2_cache::instance()->get_data_base(target_scene, *tblk, boxm2_data_traits<BOXM2_GAUSS_RGB>::prefix(
                                               color_apm_id_) );
    if( !get_data(target_scene, *tblk, &target_alpha, &target_app, &target_nobs) )
      {
      vcl_cerr << "ERROR: boxm2_vecf_head_model::map_to_target(): error getting target block data block="
               << tblk->to_string() << vcl_endl;
      return;
      }
    ...
    boxm2_data_traits<BOXM2_ALPHA>::datatype * target_alpha_data =
      reinterpret_cast<boxm2_data_traits<BOXM2_ALPHA>::datatype *>(target_alpha->data_buffer() );
    boxm2_data_traits<BOXM2_MOG3_GREY>::datatype * target_app_data =
      reinterpret_cast<boxm2_data_traits<BOXM2_MOG3_GREY>::datatype *>(target_app->data_buffer() );
    boxm2_data_traits<BOXM2_NUM_OBS>::datatype * target_nobs_data =
      reinterpret_cast<boxm2_data_traits<BOXM2_NUM_OBS>::datatype *>(target_nobs->data_buffer() );
    boxm2_data_traits<BOXM2_GAUSS_RGB>::datatype * target_color_data =
      reinterpret_cast<boxm2_data_traits<BOXM2_GAUSS_RGB>::datatype *>(target_color->data_buffer() );
    ...
    for( vcl_vector<boxm2_block_id>::iterator sblk = source_blocks.begin();
      ...
      boxm2_data_base* source_color =
        boxm2_cache::instance()->get_data_base(base_model_, *sblk,
                                               boxm2_data_traits<BOXM2_GAUSS_RGB>::prefix(color_apm_id_) );
      if( !get_data(base_model_, *sblk, &source_alpha, &source_app, &source_nobs) )
        {
        vcl_cerr << "ERROR: boxm2_vecf_head_model::map_to_source(): error getting source block data block="
                 << sblk->to_string() << vcl_endl;
        return;
        }
      boxm2_data_traits<BOXM2_ALPHA>::datatype * source_alpha_data =
        reinterpret_cast<boxm2_data_traits<BOXM2_ALPHA>::datatype *>(source_alpha->data_buffer() );
      boxm2_data_traits<BOXM2_MOG3_GREY>::datatype * source_app_data =
        reinterpret_cast<boxm2_data_traits<BOXM2_MOG3_GREY>::datatype *>(source_app->data_buffer() );
      boxm2_data_traits<BOXM2_NUM_OBS>::datatype * source_nobs_data =
        reinterpret_cast<boxm2_data_traits<BOXM2_NUM_OBS>::datatype *>(source_nobs->data_buffer() );
      boxm2_data_traits<BOXM2_GAUSS_RGB>::datatype * source_color_data =
        reinterpret_cast<boxm2_data_traits<BOXM2_GAUSS_RGB>::datatype *>(source_color->data_buffer() );
          ...
          for( unsigned int z = 0; z < trees.get_row3_count(); ++z )
            ...
            for( iter = leafBits.begin(); iter != leafBits.end(); ++iter )
              ...
              if( source_blk->contains( inv_scaled_cell_center, local_tree_coords, source_cell_center, side_len ) )
                ...
                vgl_vector_3d<double> source_scene_origin = vgl_vector_3d<double>(
                    source_blk->local_origin().x(), source_blk->local_origin().y(), source_blk->local_origin().z() );
                vgl_vector_3d<double> source_center_rel = vgl_vector_3d<double>(
                    source_cell_center.x() * source_blk->sub_block_dim().x(),
                    source_cell_center.y() * source_blk->sub_block_dim().y(),
                    source_cell_center.z()
                    * source_blk->sub_block_dim().z() );
                vgl_vector_3d<double> abs_cell_center_vec =  source_scene_origin + source_center_rel;
                vgl_point_3d<double>  abs_source_cell_center = vgl_point_3d<double>(
                    abs_cell_center_vec.x(), abs_cell_center_vec.y(), abs_cell_center_vec.z() );
                ...
                if( dP.x() >= 0 ) // source point is to the right of the cell center along x
                  {
                  abs_neighbors[0].x() = abs_neighbors[1].x() = abs_neighbors[4].x() = abs_neighbors[5].x() =
                          abs_source_cell_center.x();                                                                                    // x-left neighbor is the cell center
                  abs_neighbors[2].x() = abs_neighbors[3].x() = abs_neighbors[6].x() = abs_neighbors[7].x() =
                          abs_source_cell_center.x() + cell_len_rw;                                                                                   // x-right neighbor  is the cell to the right of cell center
                  }
                else   // source point is to the left of the cell center along x
                  {
                  abs_neighbors[0].x() = abs_neighbors[1].x() = abs_neighbors[4].x() = abs_neighbors[5].x() =
                          abs_source_cell_center.x() - cell_len_rw;                                                                                    // x-left neighbor is the cell to the left of cell center
                  abs_neighbors[2].x() = abs_neighbors[3].x() = abs_neighbors[6].x() = abs_neighbors[7].x() =
                          abs_source_cell_center.x();                                                                                     // x-right neighbor is the cell center
                  }
                if( dP.y() >= 0 ) // source point is to the right of the cell center along y
                  {
                  abs_neighbors[0].y() = abs_neighbors[2].y() = abs_neighbors[4].y() = abs_neighbors[6].y() =
                          abs_source_cell_center.y();                                                                                    // y-left neighbor is the cell center
                  abs_neighbors[1].y() = abs_neighbors[3].y() = abs_neighbors[5].y() = abs_neighbors[7].y() =
                          abs_source_cell_center.y() + cell_len_rw;                                                                                   // y-right neighbor  is the cell to the right of cell center
                  }
                else   // source point is to the left of the cell center along y
                  {
                  abs_neighbors[0].y() = abs_neighbors[2].y() = abs_neighbors[4].y() = abs_neighbors[6].y() =
                          abs_source_cell_center.y() - cell_len_rw;                                                                                   // y-left neighbor is the cell to the left of cell center
                  abs_neighbors[1].y() = abs_neighbors[3].y() = abs_neighbors[5].y() = abs_neighbors[7].y() =
                          abs_source_cell_center.y();                                                                                    // y-right neighbor the cell center
                  }
                if( dP.z() >= 0 ) // source point is above the cell center along z
                  {
                  abs_neighbors[0].z() = abs_neighbors[1].z() = abs_neighbors[2].z() = abs_neighbors[3].z() =
                          abs_source_cell_center.z();                                                                                    // z-bottom neighbor is the cell center
                  abs_neighbors[4].z() = abs_neighbors[5].z() = abs_neighbors[6].z() = abs_neighbors[7].z() =
                          abs_source_cell_center.z() + cell_len_rw;                                                                                   // z-top neighbor  is the cell on top of cell center
                  }
                else   // source point is below the cell center along z
                  {
                  abs_neighbors[0].z() = abs_neighbors[1].z() = abs_neighbors[2].z() = abs_neighbors[3].z() =
                          abs_source_cell_center.z() - cell_len_rw;                                                                                   // z-bottom neighbor is the cell below cell center
                  abs_neighbors[4].z() = abs_neighbors[5].z() = abs_neighbors[6].z() = abs_neighbors[7].z() =
                          abs_source_cell_center.z();                                                                                     // z-top neighbor  is the  cell center
                  }

                if( abs_neighbors[0].x() == abs_neighbors[2].x() || abs_neighbors[0].y() == abs_neighbors[1].y() ||
                    abs_neighbors[0].z() == abs_neighbors[4].z() )
                  {
                  continue;
                  }
                ...
                for( unsigned i = 0; i < 8; i++ )
                  ...
                  vgl_vector_3d<double> nbCenter = (abs_neighbors[i] - source_blk->local_origin() )
                    / source_blk->sub_block_dim().x();
                  ...
                  if( in_bounds )
                    {
                    uchar16 nb_tree =
                      source_trees( (unsigned char) floor(nbCenter.x() ), (unsigned char) floor(nbCenter.y() ),
                                    (unsigned char) floor(nbCenter.z() ) );
                    ...
                    vgl_vector_3d<double> locCenter = nbCenter
                      - vgl_vector_3d<double>(floor(nbCenter.x() ), floor(nbCenter.y() ), floor(nbCenter.z() ) );
                    int neighborBitIndex =
                      neighbor_tree.traverse(vgl_point_3d<double>(locCenter.x(), locCenter.y(), locCenter.z() ) );
