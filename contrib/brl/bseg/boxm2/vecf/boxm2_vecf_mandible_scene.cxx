...
void boxm2_vecf_mandible_scene::extract_block_data()
{
  ...
  app_base_  = boxm2_cache::instance()->get_data_base(base_model_, *iter_blk,
  ...
  nobs_base_  = boxm2_cache::instance()->get_data_base(base_model_, *iter_blk,
  ...
  mandible_base_  = boxm2_cache::instance()->get_data_base(base_model_, *iter_blk, boxm2_data_traits<BOXM2_PIXEL>::prefix(
                                                             "mandible") );
...
#if 0
  boxm2_data_base * left_ramus_base  =
    boxm2_cache::instance()->get_data_base(base_model_, *iter_blk,
                                           boxm2_data_traits<BOXM2_PIXEL>::prefix("left_ramus") );
  left_ramus_base->enable_write();
  left_ramus_ = new boxm2_data<BOXM2_PIXEL>(left_ramus_base->data_buffer(),
                                            left_ramus_base->buffer_length(), left_ramus_base->block_id() );

  boxm2_data_base * left_angle_base  =
    boxm2_cache::instance()->get_data_base(base_model_, *iter_blk,
                                           boxm2_data_traits<BOXM2_PIXEL>::prefix("left_angle") );
  left_angle_base->enable_write();
  left_angle_ = new boxm2_data<BOXM2_PIXEL>(left_angle_base->data_buffer(),
                                            left_angle_base->buffer_length(), left_angle_base->block_id() );

  boxm2_data_base * body_base  =
    boxm2_cache::instance()->get_data_base(base_model_, *iter_blk, boxm2_data_traits<BOXM2_PIXEL>::prefix("body") );
  body_base->enable_write();
  body_ = new boxm2_data<BOXM2_PIXEL>(body_base->data_buffer(), body_base->buffer_length(), body_base->block_id() );

  boxm2_data_base * right_angle_base =
    boxm2_cache::instance()->get_data_base(base_model_, *iter_blk,
                                           boxm2_data_traits<BOXM2_PIXEL>::prefix("right_angle") );
  right_angle_base->enable_write();
  right_angle_ = new boxm2_data<BOXM2_PIXEL>(right_angle_base->data_buffer(),
                                             right_angle_base->buffer_length(), right_angle_base->block_id() );

  boxm2_data_base * right_ramus_base =
    boxm2_cache::instance()->get_data_base(base_model_, *iter_blk,
                                           boxm2_data_traits<BOXM2_PIXEL>::prefix("right_ramus") );
  right_ramus_base->enable_write();
  right_ramus_ = new boxm2_data<BOXM2_PIXEL>(right_ramus_base->data_buffer(),
                                             right_ramus_base->buffer_length(), right_ramus_base->block_id() );
#endif
}
...
void boxm2_vecf_mandible_scene::extract_target_block_data(boxm2_scene_sptr target_scene)
{
  ...
  target_alpha_base_  = boxm2_cache::instance()->get_data_base(target_scene, *iter_blk,
  ...
  target_app_base_  = boxm2_cache::instance()->get_data_base(target_scene, *iter_blk,
                                                             boxm2_data_traits<BOXM2_MOG3_GREY>::prefix() );
  ...
  target_nobs_base_  = boxm2_cache::instance()->get_data_base(target_scene, *iter_blk,
                                                              boxm2_data_traits<BOXM2_NUM_OBS>::prefix() );
...
boxm2_vecf_mandible_scene::boxm2_vecf_mandible_scene(vcl_string const& scene_file, vcl_string const& geometry_file) :
  boxm2_vecf_articulated_scene(scene_file), alpha_base_(0), app_base_(0), nobs_base_(0), mandible_base_(0),
  target_alpha_base_(0), target_app_base_(0),
  target_nobs_base_(0), left_ramus_(0), left_angle_(0), body_(0), right_angle_(0), right_ramus_(0), intrinsic_change_(
    false)
{
...
boxm2_vecf_mandible_scene::boxm2_vecf_mandible_scene(vcl_string const& scene_file, vcl_string const& geometry_file,
                                                     vcl_string const& params_file_name) :
  boxm2_vecf_articulated_scene(scene_file), alpha_base_(0), app_base_(0), nobs_base_(0), target_alpha_base_(0),
  target_app_base_(0), target_nobs_base_(0),
...
bool boxm2_vecf_mandible_scene::is_type_global(vgl_point_3d<double> const& global_pt,
                                               boxm2_vecf_mandible_scene::anat_type  type) const
{
  unsigned indx;
...
bool boxm2_vecf_mandible_scene::find_nearest_data_index(boxm2_vecf_mandible_scene::anat_type type,
                                                        vgl_point_3d<double> const& probe, double cell_len,
                                                        unsigned& data_indx, int& found_depth) const
{
...
void  boxm2_vecf_mandible_scene::inverse_vector_field(vgl_rotation_3d<double> const& rot,
                                                      ...
                                                      vcl_vector<bool>& valid) const
{
  ...
  vcl_cout << "computed " << box_cnt << " pts " << nt << " for mandible vector field in " << t.real() / 1000.0
           << " sec.\n";
}
...
//
void boxm2_vecf_mandible_scene::interpolate_vector_field(vgl_point_3d<double> const& src, unsigned sindx,
                                                             ...
                                                             vcl_vector<bool> const& valid)
{
  ...
  vcl_cout << "Apply mandible vector field to " << valid_count << " out of " << n << " cells in " << t.real()
    / 1000.0 << " sec.\n";
}
