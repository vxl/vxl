...
{
  for( vcl_map<vcl_string, vcl_pair<boxm2_vecf_orbit_params, boxm2_vecf_orbit_params> >::iterator pit =
         param_map_.begin();
    ...
    avg.medial_socket_radius_coef_ = 0.5
      * (left_params.medial_socket_radius_coef_ + right_params.medial_socket_radius_coef_);
    devs_[pid]["medial_socket_radius_coef"] = vcl_fabs(
        left_params.medial_socket_radius_coef_ - avg.medial_socket_radius_coef_);
    avg.lateral_socket_radius_coef_ = 0.5
      * (left_params.lateral_socket_radius_coef_ + right_params.lateral_socket_radius_coef_);
    devs_[pid]["lateral_socket_radius_coef"] = vcl_fabs(
        left_params.lateral_socket_radius_coef_ - avg.lateral_socket_radius_coef_);
    avg.inferior_lid_radius_offset_ = 0.5
      * (left_params.inferior_lid_radius_offset_ + right_params.inferior_lid_radius_offset_);
    devs_[pid]["inferior_lid_radius_offset"] = vcl_fabs(
        left_params.inferior_lid_radius_offset_ - avg.inferior_lid_radius_offset_);
    avg.eyelid_crease_scale_y_coef_ = 0.5
      * (left_params.eyelid_crease_scale_y_coef_ + right_params.eyelid_crease_scale_y_coef_);
    devs_[pid]["eyelid_crease_scale_y_coef"] = vcl_fabs(
        left_params.eyelid_crease_scale_y_coef_ - avg.eyelid_crease_scale_y_coef_);
...
void boxm2_vecf_orbit_param_stats::print_stats()
{
  ...
  for( vcl_map<vcl_string, boxm2_vecf_orbit_params>::iterator pit = param_avg_.begin();
    ...
    vcl_cout << ' ' << rat << ' ' << devs_[pid]["height_to_width_ratio"]
             ...
             << ' ' << pr.mid_eyelid_crease_z_ / pr.lid_sph_.radius() << ' ' << devs_[pid]["mid_eyelid_crease_z"]
    / pr.lid_sph_.radius() << '\n';
    }
}
...
{
  vcl_cout
  << "patient_id  left_inf_xyz left_sup_xyz left_sup_crease right_inf_xyz right_sup_xyz right_sup_crease_xyz \n";
  for( vcl_map<vcl_string, vcl_pair<boxm2_vecf_orbit_params, boxm2_vecf_orbit_params> >::iterator pit =
         param_map_.begin();
       pit != param_map_.end(); ++pit )
    {
