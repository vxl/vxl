...
double boxm2_vecf_orbit_params::height_to_width_ratio() const{
  ...
  return (height*vcl_cos(dphi_rad_)*scale_y())/width;
}
...
vcl_ostream&  operator<<(vcl_ostream& s, boxm2_vecf_orbit_params const& pr){
  ...
  s << "crease y_scale: " << pr.eyelid_crease_scale_y() << '\n';
  s << "phi rotation angle (deg): " << pr.dphi_rad_*(180.0/3.14159) << '\n';
  s << "inferior lid thickness: " << pr.inferior_lid_thickness() << '\n';
  //  s << "inferior_margin_t: " << pr.lower_eyelid_tmin_ << '\n';
  //  s << "superior_margin_t: " << pr.eyelid_tmin_ << '\n';
  s << "height to width ratio: " << pr.height_to_width_ratio() << '\n';
