...
public:
  ...
  boxm2_vecf_spline_field(vgl_cubic_spline_3d<double> const& spline,
                          vcl_vector<vgl_vector_3d<double> > const& field_knots) :
    generating_spline_(spline), field_knots_(field_knots)
  {
    if( static_cast<unsigned>(field_knots_.size() ) != spline.n_knots() )
      {
