  ...
  if( !from_dlib )
    ...
    if( !good )
      ...
      if( !good )
        {
        vcl_cout
        << "failed to read left inferior lid surface file  or next best choice the left_inferior_margin file\n";
        return -1;
        }
      else
        {
...
/// right eye processing
  if( !from_dlib )
    ...
    if( !good )
      ...
      if( !good )
        {
        vcl_cout
        << "failed to read right inferior lid surface file  or next best choice, the right_inferior_margin file\n";
        return -1;
        }
      else
        {
  ...
  if( do_non_lin )
    ...
    boxm2_vecf_fit_margins right_fmargs(fo.orbit_data("right_eye_inferior_margin"), fo.orbit_data(
  ...
  if( do_non_lin )
    ...
    boxm2_vecf_fit_margins left_fmargs(fo.orbit_data("left_eye_inferior_margin"), fo.orbit_data(
