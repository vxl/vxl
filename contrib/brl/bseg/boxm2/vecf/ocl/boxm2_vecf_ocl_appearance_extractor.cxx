...
void boxm2_vecf_ocl_appearance_extractor::compile_kernels()
{
  ...
  appearance_extraction_kernel->create_kernel(&device_->context(),
                                              device_->device_id(), src_paths, "map_to_source_and_extract_appearance", options,
                                              "orbit_appearance_extraction");
  ...
  mean_anatomical_appearance_kernel->create_kernel(&device_->context(),
                                                   device_->device_id(), src_paths, "map_to_source_and_extract_appearance", options_anatomy,
                                                   "orbit_mean_appearance_calc");
...
bool boxm2_vecf_ocl_appearance_extractor::extract_data(boxm2_scene_sptr scene, boxm2_block_id& block, float * & alpha,
                                                       gray_APM * & gray_app, color_APM * & color_app)
{
  ...
  boxm2_data_base* color_app_db =
    boxm2_cache::instance()->get_data_base(scene, block, color_APM_prefix + "_" + (head_model_.color_apm_id_) );
...
void boxm2_vecf_ocl_appearance_extractor::extract_orbit_appearance()
{
  ...
  if( target_blocks.size() > 1 )
    {
    vcl_cout << "visibility info cannot be used in current implementation if scene contains more than one block"
             << vcl_endl;
    current_vis_score_ = 0;
    }
  else
    {
    boxm2_data_base* vis_score_db =
      boxm2_cache::instance()->get_data_base(target_scene_, target_blocks[0], boxm2_data_traits<BOXM2_VIS_SCORE>::prefix(
                                               head_model_.color_apm_id_) );
...
void boxm2_vecf_ocl_appearance_extractor::extract_iris_appearance(bool is_right, bool extract)
{
  ...
  for( vcl_vector<boxm2_block_id>::iterator sblk = source_blocks.begin(); sblk != source_blocks.end(); ++sblk )
    ...
    for( unsigned i = 0; i < n_source_cells; ++i )
      ...
      local_reflected_p =
        vgl_point_3d<double>(-1 * local_reflected_p.x(), local_reflected_p.y(), local_reflected_p.z() );
      ...
      for( vcl_vector<boxm2_block_id>::iterator tblk = target_blocks.begin(); tblk != target_blocks.end(); ++tblk )
        ...
        if( target_blk->contains( mapped_p, local_tree_coords, target_cell_center, target_side_len ) )
          ...
          unsigned    target_data_idx, reflected_target_data_idx;
          ...
          if( extract )
            ...
            float8 appearance =
              weight_intesities(current_vis_score_[target_data_idx], current_vis_score_[reflected_target_data_idx],                                               to_float8(
                                  target_color_data[target_data_idx]),
...
void boxm2_vecf_ocl_appearance_extractor::extract_eye_appearance(bool is_right, bool extract)
{
  vul_timer                                   t;
  boxm2_vecf_composite_head_parameters const& head_params = head_model_.get_params();
  ORBIT                                       orbit, other_orbit;
  boxm2_vecf_orbit_params                     orbit_params, the_other_orbit_params;
      ...
      for( vcl_vector<boxm2_block_id>::iterator tblk = target_blocks.begin(); tblk != target_blocks.end(); ++tblk )
        ...
        if( target_blk->contains( mapped_p, local_tree_coords, target_cell_center, target_side_len ) )
          ...
          if( extract )
            ...
            {
            float8 appearance =
              weight_intesities(current_vis_score_[target_data_idx], current_vis_score_[reflected_target_data_idx],
...
bool boxm2_vecf_ocl_appearance_extractor::extract_appearance_one_pass(bool is_right)
{
  ...
  boxm2_vecf_composite_head_parameters const& head_params = head_model_.get_params();
  boxm2_vecf_ocl_orbit_scene                  orbit, other_orbit;
  boxm2_vecf_orbit_params                     orbit_params, other_orbit_params;
  ...
  vcl_cout << " mean eyelid intensity " << total_app[3][0] << " " << total_app[3][1] << " " << total_app[3][2]
           << vcl_endl;
  ...
  other_rotation_buff[0] = (float) other_R(0, 0); other_rotation_buff[3] = (float) other_R(1, 0);
  other_rotation_buff[6] = (float) other_R(2, 0);
  other_rotation_buff[1] = (float) other_R(0, 1); other_rotation_buff[4] = (float) other_R(1, 1);
  other_rotation_buff[7] = (float) other_R(2, 1);
  other_rotation_buff[2] = (float) other_R(0, 2); other_rotation_buff[5] = (float) other_R(1, 2);
  other_rotation_buff[8] = (float) other_R(2, 2);
  ...
  bocl_mem_sptr other_rotation_l = new bocl_mem(
      device_->context(), other_rotation_buff, sizeof(float) * 9, " other_rotation " );
  ...
  bocl_mem_sptr other_offset_l =
    new bocl_mem(device_->context(), other_offset_buff, sizeof(float) * 4, "other offset " );
...
//   output_cl->zero_gpu_buffer(queue_);
  global_threads[0] = (unsigned) RoundUp(
      info_buffer_source->scene_dims[0] * info_buffer_source->scene_dims[1] * info_buffer_source->scene_dims[2],
      (int)local_threads[0]);
...
//   output_cl->zero_gpu_buffer(queue_);
  ...
  vcl_cout << " max t color was determined to be " << max_t_color << " and t min and t max are  "
           << orbit_params.eyelid_tmin_ << " " << orbit_params.eyelid_tmax_ << vcl_endl;
...
#endif
float8 boxm2_vecf_ocl_appearance_extractor::weight_intesities(float& vis_A, float& vis_B, float8 int_A, float8 int_B,
                                                              float8 mean_A, float8 mean_B)
{
