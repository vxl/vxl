...
boxm2_vecf_ocl_composite_head_model::boxm2_vecf_ocl_composite_head_model( vcl_string const& head_model_path,
                                                                          vcl_string const& eye_model_path,
...
bool boxm2_vecf_ocl_composite_head_model::set_params(boxm2_vecf_articulated_params const& params)
{
  try
    {
    boxm2_vecf_composite_head_parameters const& params_ref =
      dynamic_cast<boxm2_vecf_composite_head_parameters const &>(params);
  ...
  params_.l_orbit_params_.offset_ = vgl_vector_3d<double>(eye_offset_x + eye_dist_x, eye_offset_y - y_off_orbit,
                                                          eye_offset_z_left);
  params_.r_orbit_params_.offset_ = vgl_vector_3d<double>(eye_offset_x + -eye_dist_x, eye_offset_y - y_off_orbit,
                                                          eye_offset_z_right);

  // if the intrinsic eye parameters changed, clear target
  if( left_orbit_.vfield_params_change_check(params_.l_orbit_params_) ||
      right_orbit_.vfield_params_change_check(params_.r_orbit_params_) )
    {
...
void boxm2_vecf_ocl_composite_head_model::update_gpu_target(boxm2_scene_sptr target_scene)
{
  ...
  for( vcl_vector<boxm2_block_id>::iterator tblk = target_blocks.begin();
    ...
    {
    bocl_mem* color_app_db =
      opencl_cache_->get_data(target_scene, *tblk, boxm2_data_traits<BOXM2_GAUSS_RGB>::prefix(color_apm_id_) );
    bocl_mem* vis_score    =
      opencl_cache_->get_data(target_scene, *tblk, boxm2_data_traits<BOXM2_VIS_SCORE>::prefix(color_apm_id_) );
    bocl_mem* alpha_db     =  opencl_cache_->get_data(target_scene, *tblk, boxm2_data_traits<BOXM2_ALPHA>::prefix() );
    bocl_mem* gray_app_db  =
      opencl_cache_->get_data(target_scene, *tblk, boxm2_data_traits<BOXM2_MOG3_GREY>::prefix() );
