...
bool boxm2_vecf_ocl_filter::compile_filter_kernel()
{
  ...
  return filter_kern->create_kernel(&device_->context(),
                                    device_->device_id(), src_paths, "filter_block_six_neighbors", options,
                                    "filter_scene");
}
...
bool boxm2_vecf_ocl_filter::filter(vcl_vector<float> const& weights, unsigned num_iterations)
{
  ...
  output = new bocl_mem(
      device_->context(), output_buff, sizeof(float) * info_buffer_source->data_buffer_length, "output" );
