...
boxm2_vecf_ocl_head_model::boxm2_vecf_ocl_head_model(vcl_string const& scene_file, bocl_device_sptr device,
                                                     boxm2_opencl_cache_sptr opencl_cache, vcl_string color_apm_ident) :
  boxm2_vecf_articulated_scene(scene_file, color_apm_ident),
  scale_(1.0, 1.0, 1.0), opencl_cache_(opencl_cache), device_(device), scene_transformer_(base_model_, opencl_cache_,
                                                                                          "", color_apm_id_)
{
...
void boxm2_vecf_ocl_head_model::map_to_target(boxm2_scene_sptr target_scene)
{
  ...
  vgl_vector_3d<double>   inv_scale = vgl_vector_3d<double>(1.0 / this->scale_.x(),
                                                            1.0 / this->scale_.y(), 1.0 / this->scale_.z() );
...
void boxm2_vecf_ocl_head_model::clear_target(boxm2_scene_sptr target_scene)
{
  ...
  for( vcl_vector<boxm2_block_id>::iterator tblk = target_blocks.begin();
    ...
    bocl_mem* nobs_db      =  opencl_cache_->get_data(target_scene, *tblk, boxm2_data_traits<BOXM2_NUM_OBS_SINGLE>::prefix(
                                                        color_apm_id_) );
