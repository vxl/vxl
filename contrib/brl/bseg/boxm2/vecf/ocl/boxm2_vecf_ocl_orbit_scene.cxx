...
bool boxm2_vecf_ocl_orbit_scene::compile_kernels()
{
  ...
  map_sphere_kernel->create_kernel(&device_->context(),
                                   device_->device_id(), src_paths, "map_orbit_to_target", options,
                                   "map_orbit_to_target kernel");
...
void boxm2_vecf_ocl_orbit_scene::extract_block_data()
{
  ...
  alpha_data_            = new boxm2_data<BOXM2_ALPHA>(
      (char *)source_alpha_base_->cpu_buffer(), source_alpha_base_->num_bytes(), *iter_blk);
  app_data_              = new boxm2_data<BOXM2_MOG3_GREY>(
      (char *)source_app_base_->cpu_buffer(), source_app_base_->num_bytes(), *iter_blk);
  color_app_data_        = new boxm2_data<BOXM2_GAUSS_RGB>(
      (char *)source_color_base_->cpu_buffer(), source_color_base_->num_bytes(), *iter_blk);
  nobs_data_             = new boxm2_data<BOXM2_NUM_OBS>(
      (char *)source_nobs_base_->cpu_buffer(), source_nobs_base_->num_bytes(), *iter_blk);
  source_vis_score_data_ = new boxm2_data<BOXM2_VIS_SCORE>(
      (char *)source_vis_base_->cpu_buffer(), source_vis_base_->num_bytes(), *iter_blk);

  sphere_        =
    new boxm2_data<BOXM2_PIXEL>( (char *)sphere_base_->cpu_buffer(), sphere_base_->num_bytes(), *iter_blk);
  ...
  eyelid_        =
    new boxm2_data<BOXM2_PIXEL>( (char *)eyelid_base_->cpu_buffer(), eyelid_base_->num_bytes(), *iter_blk);
  lower_eyelid_  = new boxm2_data<BOXM2_PIXEL>( (char *)lower_eyelid_base_->cpu_buffer(),
                                                lower_eyelid_base_->num_bytes(), *iter_blk);
  eyelid_crease_ = new boxm2_data<BOXM2_PIXEL>(
      (char *)eyelid_crease_base_->cpu_buffer(), eyelid_crease_base_->num_bytes(), *iter_blk);
...
void boxm2_vecf_ocl_orbit_scene::extract_target_block_data(boxm2_scene_sptr target_scene)
{
  ...
  target_alpha_data_     = new boxm2_data<BOXM2_ALPHA>(
      (char *)target_alpha_base_->cpu_buffer(), target_alpha_base_->num_bytes(), *iter_blk);
  target_app_data_       = new boxm2_data<BOXM2_MOG3_GREY>(
      (char *)target_app_base_->cpu_buffer(), target_app_base_->num_bytes(), *iter_blk);
  target_nobs_data_      = new boxm2_data<BOXM2_NUM_OBS>(
      (char *)target_nobs_base_->cpu_buffer(), target_nobs_base_->num_bytes(), *iter_blk);
  target_vis_score_data_ = new boxm2_data<BOXM2_VIS_SCORE>(
      (char *)target_vis_base_->cpu_buffer(), target_vis_base_->num_bytes(), *iter_blk);
  target_color_data_     = new boxm2_data<BOXM2_GAUSS_RGB>(
      (char *)target_color_base_->cpu_buffer(), target_color_base_->num_bytes(), *iter_blk);
...
bool boxm2_vecf_ocl_orbit_scene::update_target_gpu_buffers(boxm2_scene_sptr target_scene, boxm2_block_id id, bool write)
{
  ...
  target_vis_base_   =
    opencl_cache_->get_data(target_scene, id, boxm2_data_traits<BOXM2_VIS_SCORE>::prefix(color_apm_id_) );
  target_color_base_ =
    opencl_cache_->get_data(target_scene, id, boxm2_data_traits<BOXM2_GAUSS_RGB>::prefix(color_apm_id_) );
  if( write )
    {
    target_alpha_base_->write_to_buffer(queue_);
    target_app_base_->write_to_buffer(queue_);
...
bool boxm2_vecf_ocl_orbit_scene::update_source_gpu_buffers(bool write)
{
  ...
  source_vis_base_   = opencl_cache_->get_data(base_model_, id, boxm2_data_traits<BOXM2_VIS_SCORE>::prefix(
                                                 color_apm_id_) );
  source_color_base_ = opencl_cache_->get_data(base_model_, id, boxm2_data_traits<BOXM2_GAUSS_RGB>::prefix(
                                                 color_apm_id_) );
  ...
  lower_eyelid_base_ =
    opencl_cache_->get_data(base_model_, id, boxm2_data_traits<BOXM2_PIXEL>::prefix("lower_eyelid") );
  eyelid_crease_base_ =
    opencl_cache_->get_data(base_model_, id, boxm2_data_traits<BOXM2_PIXEL>::prefix("eyelid_crease") );
...
// Main constructor
boxm2_vecf_ocl_orbit_scene::boxm2_vecf_ocl_orbit_scene(vcl_string const& scene_file, bocl_device_sptr device,
                                                       boxm2_opencl_cache_sptr opencl_cache, bool is_single_instance,
                                                       bool is_right ) :
  boxm2_vecf_articulated_scene(scene_file), is_right_(is_right), alpha_data_(0), app_data_(0), nobs_data_(0),
  sphere_(0), iris_(0), pupil_(0), device_(device), opencl_cache_(opencl_cache)
{
  ...
  for( vcl_vector<cell_info>::iterator cit = ccs.begin();
    ...
    if( d < d_thresh )
      ...
      if( !is_type_global(cell_center, SPHERE) )
        ...
        alpha_data_->data()[indx] = -vcl_log(1.0 - ( 0.95 ) ) / static_cast<float>(this->subblock_len() )
          * blending_factor;
...
void boxm2_vecf_ocl_orbit_scene::build_iris()
{
  ...
  for( vcl_vector<vgl_point_3d<double> >::iterator cit = sphere_cell_centers_.begin();
    ...
    if( el <= iris_half_ang )
      ...
      vcl_vector<vgl_point_3d<double> >::iterator iit = vcl_find(iris_cell_centers_.begin(),
                                                                 iris_cell_centers_.end(), cell_center);
...
bool boxm2_vecf_ocl_orbit_scene::is_type_data_index(unsigned data_index,
                                                    boxm2_vecf_ocl_orbit_scene::anat_type type) const
{
...
bool boxm2_vecf_ocl_orbit_scene::is_type_global(vgl_point_3d<double> const& global_pt,
                                                boxm2_vecf_ocl_orbit_scene::anat_type  type) const
{
  unsigned indx;
...
bool boxm2_vecf_ocl_orbit_scene::find_nearest_data_index(boxm2_vecf_ocl_orbit_scene::anat_type type,
                                                         vgl_point_3d<double> const& probe, unsigned& data_indx) const
{
...
void  boxm2_vecf_ocl_orbit_scene::inverse_vector_field_eye(vgl_rotation_3d<double> const& rot,
...
void  boxm2_vecf_ocl_orbit_scene::inverse_vector_field_eyelid(double dt, vcl_vector<vgl_vector_3d<double> >& vfield,
                                                              vcl_vector<unsigned char>& valid) const
{
  vul_timer t;
...
void  boxm2_vecf_ocl_orbit_scene::inverse_vector_field_lower_eyelid(vcl_vector<vgl_vector_3d<double> >& vfield,
                                                                    vcl_vector<bool>& valid) const
{
  vul_timer t;
  unsigned  nt = static_cast<unsigned>(box_cell_centers_.size() );
...
void  boxm2_vecf_ocl_orbit_scene::inverse_vector_field_eyelid_crease(vcl_vector<vgl_vector_3d<double> >& vfield,
                                                                     vcl_vector<bool>& valid) const
{
  vul_timer t;
...
//
void boxm2_vecf_ocl_orbit_scene::interpolate_vector_field(vgl_point_3d<double> const& src, unsigned sindx,
                                                          unsigned dindx, unsigned tindx,
                                                          vcl_vector<vgl_point_3d<double> > & cell_centers,
                                                          vcl_map<unsigned,
                                                                  vcl_vector<unsigned> >& cell_neighbor_cell_index,
                                                          vcl_map<unsigned,
                                                                  vcl_vector<unsigned> >& cell_neighbor_data_index)
{
  boxm2_data_traits<BOXM2_MOG3_GREY>::datatype app;
  ...
  curr_color[0] = ( (double) color_app[0]) / 255; curr_color[2] = ( (double)color_app[2]) / 255;
  curr_color[4] = ( (double)color_app[4]) / 255;
  ...
  sumalpha /= sumw;
  sumcolor /= sumw;
  color_app[0] = (unsigned char) (sumcolor[0] * 255); color_app[2] = (unsigned char)(sumcolor[2] * 255);
  color_app[4] = (unsigned char) (sumcolor[4] * 255);
...
void boxm2_vecf_ocl_orbit_scene::apply_eyelid_crease_vector_field_to_target(
  vcl_vector<vgl_vector_3d<double> > const& vf,
  vcl_vector<bool> const& valid)
{
...
void boxm2_vecf_ocl_orbit_scene::init_eyelids()
{
  ...
  eyelid_geo_cl_       =
    new bocl_mem(device_->context(), eyelid_cl_buf_,        sizeof(float) * 21, " eyelid struct " );
  eyelid_crease_geo_cl_ = new bocl_mem(
      device_->context(), eyelid_crease_cl_buf_, sizeof(float) * 16, " eyelid crease struct " );
  lower_eyelid_geo_cl_  = new bocl_mem(
      device_->context(), lower_eyelid_cl_buf_,  sizeof(float) * 16, "lower eyelid struct " );
...
bool boxm2_vecf_ocl_orbit_scene::vfield_params_change_check(const boxm2_vecf_orbit_params & params)
{
  ...
  intrinsic_change |= fabs( (float)this->params_.sclera_intensity_              - (float)params.sclera_intensity_) >
    tol;
  intrinsic_change |=
    fabs( (float)this->params_.lower_eyelid_intensity_        - (float)params.lower_eyelid_intensity_) > tol;
  intrinsic_change |= fabs( (float)this->params_.eyelid_intensity_              - (float)params.eyelid_intensity_) >
    tol;
  intrinsic_change |= fabs(
      (float)this->params_.eyelid_crease_upper_intensity_ - (float)params.eyelid_crease_upper_intensity_) > tol;
...
void boxm2_vecf_ocl_orbit_scene::reset_buffers(bool color_only)
{
  ...
  source_color_base_ =
    opencl_cache_->get_data(base_model_, blocks[0], boxm2_data_traits<BOXM2_GAUSS_RGB>::prefix(color_apm_id_) );
...
void boxm2_vecf_ocl_orbit_scene::determine_target_box_cell_centers()
{
  ...
  vgl_box_3d<double> offset_box(source_box.centroid() + params_.offset_, source_box.width(),
                                source_box.height(), source_box.depth(), vgl_box_3d<double>::centre);
  if( target_blk_ )
    {
...
bool boxm2_vecf_ocl_orbit_scene::map_orbit_to_target_single_pass(boxm2_scene_sptr target_scene)
{
  ...
  bocl_mem_sptr blk_info_target  = new bocl_mem(
      device_->context(), info_buffer_target, sizeof(boxm2_scene_info), " Scene Info Target" );
  good_buffs &= blk_info_target->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR);
  blk_info_target->write_to_buffer(queue_);

  bocl_mem_sptr blk_info_source  = new bocl_mem(
      device_->context(), info_buffer_source, sizeof(boxm2_scene_info), " Scene Info Source" );
  ...
  bocl_mem_sptr output_mog_cl = new bocl_mem(
      device_->context(), output_mog, source_data_size * sizeof(float) * 8, "Output buf" );
  ...
  bocl_mem_sptr output_color_cl = new bocl_mem(
      device_->context(), output_color, source_data_size * sizeof(float) * 4, "Output buf" );
  good_buffs &=  output_color_cl->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR );

  float*        nb_top = new float[source_data_size * 16];
  bocl_mem_sptr nb_top_cl =
    new bocl_mem(device_->context(), nb_top, source_data_size * sizeof(float) * 16, "nb top buf" );
  good_buffs &=  nb_top_cl->create_buffer(CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR );

  float*        nb_bot = new float[source_data_size * 16];
  bocl_mem_sptr nb_bot_cl =
    new bocl_mem(device_->context(), nb_bot, source_data_size * sizeof(float) * 16, "nb top buf" );
  ...
  global_threads[0] = (unsigned) RoundUp(
      info_buffer_target->scene_dims[0] * info_buffer_target->scene_dims[1] * info_buffer_target->scene_dims[2],
      (int)local_threads[0]);
  ...
  for( unsigned i = 0, k = 0; i < source_data_size; i += 1, k += 1 )
    ...
    if( eyelid_->data()[i] && !is_right_ && output_color[4 * i] > 200 )
      ...
      if( output_mog[i * 8] != output_mog[i * 8 + 1] )
        ...
        vcl_cout << "++++++++++++++++++ " << "color val  at " << p_src << output_color[4 * i] << " "
                 << output_color[4 * i + 1] << " " << output_color[4 * i + 2] << " " << vcl_endl;
