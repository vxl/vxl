...
public:
  ...
  boxm2_vecf_ocl_orbit_scene() : alpha_data_(0), app_data_(0), nobs_data_(0), sphere_(0), iris_(0), pupil_(0),
    eyelid_(0), target_alpha_data_(0), target_app_data_(0), target_nobs_data_(0), extrinsic_only_(false),
    target_blk_(0), target_data_extracted_(false), boxm2_vecf_articulated_scene(), sigma_(0.5f), device_(0),
    opencl_cache_(0), scene_transformer_(0)
  {
  }
  ...
  boxm2_vecf_ocl_orbit_scene(vcl_string const& scene_file, bocl_device_sptr device,
                             boxm2_opencl_cache_sptr opencl_cache, bool is_single_instance = true,
                             bool is_right = false);
  ...
  void inverse_vector_field_eyelid(double dt, vcl_vector<vgl_vector_3d<double> >& vfield,
  ...
  void interpolate_vector_field(vgl_point_3d<double> const& src, unsigned sindx, unsigned dindx, unsigned tindx,
                                vcl_vector<vgl_point_3d<double> > & cell_centers, vcl_map<unsigned,
                                                                                          vcl_vector<unsigned> >& cell_neighbor_cell_index, vcl_map<unsigned,
  ...
  void apply_eye_vector_field_to_target(vcl_vector<vgl_vector_3d<double> > const& vf, vcl_vector<bool> const& valid);
