...
bool boxm2_vecf_ocl_store_nbrs::compile_kernel()
{
  ...
  return kern->create_kernel(&device_->context(),
                             device_->device_id(), src_paths, "extract_neighbors_block", options, "neighbor_info");
}
...
bool boxm2_vecf_ocl_store_nbrs::augment_1_blk()
{
  ...
  nbr_exint = opencl_cache_->get_data_new(source_scene_, *iter_blk, boxm2_data_traits<BOXM2_CHAR8>::prefix(
                                            "nbr_exint"), nbytes_nbr_exint, false);
  vcl_size_t prob_size = boxm2_data_traits<BOXM2_FLOAT8>::datasize();
  vcl_size_t nbytes_nbr_prob = prob_size * nvox;
  nbr_prob = opencl_cache_->get_data_new(source_scene_, *iter_blk, boxm2_data_traits<BOXM2_FLOAT8>::prefix(
                                           "nbr_prob"), nbytes_nbr_prob, false);
  vcl_size_t exist_size = boxm2_data_traits<BOXM2_CHAR8>::datasize();
  vcl_size_t nbytes_nbr_exist = exist_size * nvox;
  nbr_exists = opencl_cache_->get_data_new(source_scene_, *iter_blk, boxm2_data_traits<BOXM2_CHAR8>::prefix(
                                             "nbr_exist"), nbytes_nbr_exist, false);
  // set kernel args
