...
bool boxm2_vecf_ocl_transform_scene::compile_trans_interp_kernel()
{
  ...
  return trans_interp_kern->create_kernel(&device_->context(),
                                          device_->device_id(), src_paths, "transform_scene_interpolate", options,
                                          "trans_interp_scene");
}
...
bool boxm2_vecf_ocl_transform_scene::compile_trans_interp_trilin_kernel()
{
  ...
  return trans_interp_trilin_kern->create_kernel(&device_->context(),
                                                 device_->device_id(), src_paths, "warp_and_resample_trilinear_similarity", options,
                                                 "trans_interp_similarity_trilin_scene");
}
...
bool boxm2_vecf_ocl_transform_scene::compile_compute_cell_centers_kernel()
{
  ...
  return compute_cell_centers_kern->create_kernel(&device_->context(),
                                                  device_->device_id(), src_paths, "compute_cell_centers", options,
...
bool boxm2_vecf_ocl_transform_scene::compile_trans_interp_vecf_trilin_kernel()
{
  ...
  return trans_interp_vecf_trilin_kern->create_kernel(&device_->context(),
                                                      device_->device_id(), src_paths, "warp_and_resample_trilinear_vecf", options,
                                                      "trans_interp_vecf_trilin_scene");
}
                                                            ...
                                                            bool finish)
{
  ...
  if( first )
    ...
    vgl_box_3d<float> box_target(info_buffer_->scene_origin[0], info_buffer_->scene_origin[1],
                                 info_buffer_->scene_origin[2],
    ...
    global_threads[0] = (unsigned) RoundUp(
        info_buffer_->scene_dims[0] * info_buffer_->scene_dims[1] * info_buffer_->scene_dims[2], (int)local_threads[0]);
    ...
    vgl_box_3d<float> box_source(info_buffer_source->scene_origin[0], info_buffer_source->scene_origin[1],
                                 info_buffer_source->scene_origin[2],
                                 ...
                                 info_buffer_source->scene_origin[2] + info_buffer_source->scene_dims[2]
                                 * info_buffer_source->block_len);
    ...
    if( app_type_ == BOXM2_MOG3_GREY )
      ...
      {
      vcl_cout << "ERROR: boxm2_vecf_ocl_transform_scene: Unsupported appearance type for source_scene "
               << boxm2_data_info::prefix(app_type_) << '\n';
      return false;
      }
    ...
    nbr_prob   = opencl_cache_->get_data<BOXM2_FLOAT8>(source_scene_, *iter_blk_source,
                                                       32 * info_buffer_source->data_buffer_length, true, "nbr_prob");
...
bool boxm2_vecf_ocl_transform_scene::transform_1_blk_interp_trilin(boxm2_vecf_ocl_vector_field & vec_field,
                                                                   bool finish)
{
  if( !target_scene_ )
    {
  ...
  if( !vec_field.compute_inverse_transform(target_scene_, *iter_blk_target, target_pts, source_pts, queue_) )
    {
    vcl_cout
      << "ERROR: boxm2_vecf_ocl_transform_scene::transform_1_blk_interp_trilin(): Error computing inverse transform!"
      << vcl_endl;
    return false;
    }
  ...
  bocl_mem_sptr blk_info_source_loc = opencl_cache_->alloc_mem(sizeof(boxm2_scene_info), info_buffer_source,
                                                               "Scene Info");
                                                                   ...
                                                                   bool finish)
{
  ...
  bocl_mem_sptr blk_info_source_l  = new bocl_mem(
