...
public:
  // : Constructor.
  boxm2_vecf_ocl_transform_scene(boxm2_scene_sptr source_scene, boxm2_scene_sptr target_scene,
                                 boxm2_opencl_cache_sptr ocl_cache, vcl_string gray_app_id = "",
                                 vcl_string color_app_id = "", bool do_alpha = true, bool do_interp = true);

  // constructor if target scene is not known at the time of creation
  boxm2_vecf_ocl_transform_scene(boxm2_scene_sptr source_scene, boxm2_opencl_cache_sptr ocl_cache,
                                 vcl_string gray_app_id = "", vcl_string color_app_id = "", bool do_alpha = true,
                                 bool do_interp = true);
  ...
  // no interpolation
  bool transform(vgl_rotation_3d<double>  rot, vgl_vector_3d<double> trans, vgl_vector_3d<double> scale);
  ...
  //  no interpolation
  bool transform_1_blk(vgl_rotation_3d<double>  rot, vgl_vector_3d<double> trans, vgl_vector_3d<double> scale,
                       bool finish = true);

  // : transform a scene with one block and interpolate appearance and alpha
  // leave GPU buffers in place unless finish == true
  bool transform_1_blk_interp(vgl_rotation_3d<double>  rot, vgl_vector_3d<double> trans, vgl_vector_3d<double> scale,
                              bool finish = true);

  // : warps using similarty transform
  bool transform_1_blk_interp_trilin(boxm2_scene_sptr target_scene, vgl_rotation_3d<double>  rot,
                                     vgl_vector_3d<double> trans, vgl_vector_3d<double> scale, bool finish);
