...
__kernel void warp_and_resample_trilinear(__constant  float           * centerX,//0
                      ...
                      if( alpha_offset >=0  && alpha_offset < source_scene_linfo->data_len )
                        {
                          ...
                          for (unsigned i=0; i<8; i++){
                            ...
                            if (in_bounds){
                              ...
                              alphas[i] =0.0; params[i]=0.0;
                            }
                            }
                            tmp_out[0] = abs_neighbors[0].x ; tmp_out[1] = abs_neighbors[2].x;
                            tmp_out[2] = abs_neighbors[0].y ; tmp_out[3] = abs_neighbors[1].y;
                            tmp_out[4] = abs_neighbors[0].z ; tmp_out[5] = abs_neighbors[4].z;
                            tmp_out[6]= dP.y; tmp_out[7] = dP.z;
                              ...
                              uchar8 expected_int_interped  = (uchar8)((uchar)(expected_int * NORM), 32, 255, 0, 0, 0, 0, 0);
                              CONVERT_FUNC_SAT_RTE(mog_interped, expected_int_interped);
#endif

                              output[dataIndex] = out_local;
                              target_scene_alpha_array[dataIndex] = currDepth >= source_depth ? alpha_interped : alpha;
                              target_scene_mog_array[dataIndex] = currDepth >=source_depth ? mog_interped :mog_original;
