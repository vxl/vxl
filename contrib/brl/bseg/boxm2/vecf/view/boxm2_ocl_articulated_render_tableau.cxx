...
boxm2_ocl_articulated_render_tableau::boxm2_ocl_articulated_render_tableau()
{
  ...
  REG_PROCESS_FUNC_CONS(bprb_func_process, bprb_batch_process_manager,
                        boxm2_ocl_render_gl_view_dep_app_expected_color_process,
                        "boxm2OclRenderGlViewDepExpectedColorProcess");
...
float boxm2_ocl_articulated_render_tableau::render_frame()
{
  ...
  if( render_depth_ )
    ...
    if( target_scene_->has_data_type(boxm2_data_traits<BOXM2_MOG6_VIEW>::prefix() ) ||
        target_scene_->has_data_type(boxm2_data_traits<BOXM2_MOG6_VIEW_COMPACT>::prefix() ) )
      {
      good = bprb_batch_process_manager::instance()->init_process("boxm2OclRenderGlViewDepExpectedImageProcess");
      }
    else if( target_scene_->has_data_type(boxm2_data_traits<BOXM2_GAUSS_RGB_VIEW>::prefix() ) ||
             target_scene_->has_data_type(boxm2_data_traits<BOXM2_GAUSS_RGB_VIEW_COMPACT>::prefix() ) )
      {
...
bool boxm2_ocl_articulated_render_tableau::init_clgl()
{
  ...
  int status_queue = 0;
  queue_ =
    clCreateCommandQueue(device_->context(), *(device_->device_id() ), CL_QUEUE_PROFILING_ENABLE, &status_queue);
  ...
  clgl_buffer_ = clCreateFromGLBuffer(device_->context(),
                                      ...
                                      &status);
  exp_img_ =
    new bocl_mem(device_->context(),  NULL, static_cast<unsigned>(RoundUp(ni_, 8) * RoundUp(nj_,
                                                                                            8) * sizeof(GLubyte) * 4),
                 "exp image (gl) buffer");
