...
float boxm2_ocl_render_tableau::render_frame()
{
  ...
  if( render_depth_ )
    ...
    if( scene_->has_data_type(boxm2_data_traits<BOXM2_MOG6_VIEW>::prefix() ) ||
        scene_->has_data_type(boxm2_data_traits<BOXM2_MOG6_VIEW_COMPACT>::prefix() ) )
      {
      good = bprb_batch_process_manager::instance()->init_process("boxm2OclRenderGlViewDepExpectedImageProcess");
      }
    else if( scene_->has_data_type(boxm2_data_traits<BOXM2_GAUSS_RGB_VIEW>::prefix() ) ||
             scene_->has_data_type(boxm2_data_traits<BOXM2_GAUSS_RGB_VIEW_COMPACT>::prefix() ) )
      {
...
bool boxm2_ocl_render_tableau::init_clgl()
{
  ...
  clgl_buffer_ = clCreateFromGLBuffer(device_->context(),
                                      ...
                                      pbuffer_,
                                      &status);
  exp_img_ = new bocl_mem(device_->context(),  NULL, RoundUp(ni_, 8) * RoundUp(nj_,
                                                                               8) * sizeof(GLubyte) * 4,
                          "exp image (gl) buffer");
