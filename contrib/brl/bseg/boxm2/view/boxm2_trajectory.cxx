    ...
    for( double y = y_start; y < y_end; y += y_increment )
      ...
      for( double head = heading; head < heading + 360; head += heading_increment )
        ...
        vpgl_perspective_camera<double>* cam =
          new vpgl_perspective_camera<double>(bpgl_camera_utils::camera_from_kml( (double)ni, (double)nj, right_fov,
                                                                                  top_fov, alt, head, tilt, roll) );
...
boxm2_trajectory::boxm2_trajectory(vgl_box_3d<double> bb, vil_image_view<float>& x_img, vil_image_view<float>& y_img,
                                   vil_image_view<float>& z_img,
                                   unsigned ni, unsigned nj, double right_fov, double top_fov, double alt, double tilt,
                                   double roll,
                                   unsigned margin, unsigned i_start, unsigned j_start, unsigned i_inc, unsigned j_inc,
...
void boxm2_trajectory::init_cameras(double incline0, double incline1, double radius, vgl_box_3d<double> bb, unsigned ni,
                                    unsigned nj)
{
