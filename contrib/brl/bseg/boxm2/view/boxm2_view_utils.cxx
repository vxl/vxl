                                         ...
                                         double & res_angle)
{
  ...
  if( dot_product(norm_axis_vxl, norm_axis_vxl) < 1e-8 )
    ...
    {
    norm_quat_vxl =
      vnl_quaternion<double>(vnl_vector_fixed<double, 3>(norm_axis_vxl.x(), norm_axis_vxl.y(),
                                                         norm_axis_vxl.z() ), vcl_acos(-look.z() ) );
    }
  ...
  vnl_quaternion<double> rot_y_quat_vxl(vnl_vector_fixed<double, 3>(temp_v_vxl.x(), temp_v_vxl.y(),
                                                                    temp_v_vxl.z() ), vcl_acos(dot_product(new_y_vxl,
                                                                                                           v_vxl) ) );
...
void boxm2_view_utils::add_vrml_animation(vcl_ostream & os, vcl_vector<vpgl_camera_double_sptr> &  cameras)
{
  ...
  for( unsigned i = 0; i < cameras.size(); i++ )
    {
    os << orientations[i][0] << ' ' << orientations[i][1] << ' ' << orientations[i][2] << ' ' << orientations[i][3]
       << ',' << vcl_endl;
    }
  os << "]\n"
     << "}\n"
