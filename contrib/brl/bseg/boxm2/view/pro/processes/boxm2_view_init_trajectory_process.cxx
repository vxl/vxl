...
bool boxm2_view_init_regular_trajectory_process(bprb_func_process& pro)
{
  ...
  // new trjaectory
  boxm2_trajectory_sptr traj = new boxm2_trajectory(ni, nj, right_fov, top_fov, alt, heading, tilt, roll, x_start,
                                                    y_start, x_end, y_end, x_increment, y_increment, heading_increment);
...
bool boxm2_view_init_height_map_trajectory_process(bprb_func_process& pro)
{
  ...
  // new trjaectory
  boxm2_trajectory_sptr traj = new boxm2_trajectory(
      scene->bounding_box(), x_img, y_img, z_img, ni, nj, right_fov, top_fov, alt, tilt, roll, margin, i_start, j_start, i_inc, j_inc, heading_start,
      heading_increment);
