...
bool boxm2_volm_loc_hypotheses::add(boxm2_scene_sptr scene, vgl_box_3d<double>& scene_bounding_box, double lon,
                                    double lat, double elev, unsigned i, unsigned j)
{
  double lx, ly, lz;
...
void boxm2_volm_loc_hypotheses::generate_output_tile(vcl_vector<float>& scores, int uncertainty_size_i,
                                                     int uncertainty_size_j, float cut_off,
                                                     vil_image_view<unsigned int>& out)
{
...
bool boxm2_volm_loc_hypotheses::write_hypotheses(vcl_string out_file)
{
...
bool boxm2_volm_loc_hypotheses::write_hypotheses_kml(boxm2_scene_sptr scene, vcl_string kml_file)
{
  vcl_ofstream ofs(kml_file.c_str() );

  if( !ofs )
    {
    vcl_cerr << "In boxm2_volm_loc_hypotheses::write_hypotheses_kml() -- cannot open: " << kml_file << vcl_endl;
    return false;
    }
  ...
  for( unsigned i = 0; i < locs_.size(); i++ )
    ...
    vnl_double_2     ul(lat, lon); vnl_double_2 ur(lat + 0.0005, lon); vnl_double_2 ll(lat, lon - 0.0005);
    vnl_double_2     lr(lat + 0.0005, lon - 0.0005);
