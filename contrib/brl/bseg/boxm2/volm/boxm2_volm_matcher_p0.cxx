...
boxm2_volm_matcher_p0::boxm2_volm_matcher_p0(volm_camera_space_sptr cam_space, volm_spherical_region_query srq,
                                             float threshold)
...
bool boxm2_volm_matcher_p0::match(volm_spherical_region_index & index, volm_score_sptr score)
{
  ...
  for( camera_space_iterator iter = cam_space_->begin(); iter != cam_space_->end(); ++iter, ++count )
    ...
    for( int i = 0; i < q_regions.size(); ++i )
      ...
      for( unsigned k = 0; k < sizeof(attributes_to_match) / sizeof(int); ++k )
        ...
        vcl_vector<unsigned int> attribute_poly_ids = index_layer.attributed_regions_by_value(attributes_to_match[k],
                                                                                              qval);
  ...
  int   max_cam_id = -1;
  int   grndtruthindex =
    cam_space_->closest_index(cam_angles(cam_space_->roll_mid(), cam_space_->top_fovs()[0], cam_space_->head_mid(),
                                         cam_space_->tilt_mid() ) );
  vcl_cout << "INDEX of Ground Truth cam " << grndtruthindex << " and the score is " << scores[grndtruthindex].first
           << vcl_endl;
  // : selecting the top 200 cameras and the best camera location
  vcl_sort(scores.begin(), scores.end(), scorecomp_obj);
  int cnt = 0;
  for( vcl_vector<vcl_pair<float, int> >::iterator iter = scores.begin(); iter != scores.end() && cnt < 1000;
       ++iter, ++cnt )
    {
...
bool boxm2_volm_matcher_p0::match_order(volm_spherical_regions_layer & index_layer, double & score,
                                        volm_spherical_regions_layer & q_regions, camera_space_iterator & iter,
                                        vsph_sph_box_2d & imbox_xfomred)
{
