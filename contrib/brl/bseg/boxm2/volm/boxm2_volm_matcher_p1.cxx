...
bool boxm2_volm_matcher_p1::volm_matcher_p1(int const& num_locs_to_kernel)
{
  ...
  *n_obj_ = (unsigned)(query_->depth_regions() ).size();
  ...
  vcl_cout << "\t 4.1.1 Setting up query in pass 1 matcher for GPU ------> \t" << trans_query_time.all() / 1000.0
           << " seconds." << vcl_endl;
  // create queue
  ...
  depth_interval_cl_mem_ =
    new bocl_mem(gpu_->context(), depth_interval_buff_,
                 sizeof(float) * ( (unsigned)depth_interval_.size() ), " depth_interval ");
  ...
  *fallback_size_buff_ = fallback_size_;
  ...
  if( num_locs_to_kernel > 0 )
    ...
    if( layer_size_ < 20000 )
      ...
      else if( *n_cam_ < 15000 )
        {
        ni = 32;
        }
      else if( *n_cam_ < 20000 )
        {
        ni = 16;
        }
      else {ni = 16; }
  ...
  vcl_cout << "\t 4.1.3: device have total " << device_global_mem_ << " Byte (" << (float)device_global_mem_
    / (float)GBYTE << " GB) memory space\n"
           << "\t        query requires " << query_global_mem_ << " Byte (" << (float)query_global_mem_
    / (float)GBYTE << " GB)\n"
           << "\t        leave " << extra_global_mem  << " Byte (" << (float)extra_global_mem / (float)GBYTE
           << " GB) extra empty space on device\n"
           << "\t        a single index require " << per_index_mem << " Byte given " << nc << " cameras and " << no
           << " objects\n"
  ...
  while( leaf_id < leaves_.size() )
    ...
    if( !this->fill_index(ni, layer_size_, leaf_id, index_buff_, index_orient_buff_, index_land_buff_, l_id, h_id,
                          actual_n_ind) )
      {
      this->clean_query_cl_mem();
      delete depth_interval_cl_mem_;        delete depth_length_cl_mem_;
    ...
    *n_ind_ = ni;
    ...
    bocl_mem* index_cl_mem_ = new bocl_mem(
        gpu_->context(), index_buff_, sizeof(unsigned char) * ni * layer_size_, " index ");
    ...
    bocl_mem* index_combine_cl_mem_ = new bocl_mem(
        gpu_->context(), index_combine_buff_, sizeof(unsigned char) * ni * layer_size_, " index_combine ");
    ...
    bocl_mem* index_orient_cl_mem_ = new bocl_mem(
        gpu_->context(), index_orient_buff_, sizeof(unsigned char) * ni * layer_size_, " index_orient ");
    ...
    bocl_mem* index_land_cl_mem_ = new bocl_mem(
        gpu_->context(), index_land_buff_, sizeof(unsigned char) * ni * layer_size_, " index_land ");
    ...
    check_val(status, MEM_FAILURE,
              " release N_INDEX failed on device " + gpu_->device_identifier() + error_to_string(status) );
    delete index_cl_mem_;
    status = clFinish(queue_);
    check_val(status, MEM_FAILURE,
              " release INDEX failed on device " + gpu_->device_identifier() + error_to_string(status) );
    delete score_cl_mem_;
    status = clFinish(queue_);
    check_val(status, MEM_FAILURE,
              " release SCORE failed on device " + gpu_->device_identifier() + error_to_string(status) );
    delete mu_cl_mem_;
    status = clFinish(queue_);
    check_val(status, MEM_FAILURE,
              " release MU(average depth value) failed on device " + gpu_->device_identifier()
    ...
    status = clFinish(queue_);
    check_val(status, MEM_FAILURE,
              " release INDEX_ORIENT failed on device " + gpu_->device_identifier() + error_to_string(status) );
    delete index_land_cl_mem_;
    status = clFinish(queue_);
    check_val(status, MEM_FAILURE,
              " release INDEX_LAND failed on device " + gpu_->device_identifier() + error_to_string(status) );
  ...
  vcl_cout << "\t\t total time for " << total_index_num << " indices and " << *n_cam_ << " cameras -------> "
           << total_time / 1000.0 << " seconds (" << total_time << " ms)\n"
           << "\t\t GPU kernel execution ------------------> " << gpu_matcher_time / 1000.0 << " seconds ("
           << gpu_matcher_time << " ms)\n"
           << "\t\t CPU host execution --------------------> " << (total_time - gpu_matcher_time) / 1000.0
           << " seconds (" << total_time - gpu_matcher_time << " ms)" << vcl_endl;
...
#if 0
            vcl_cerr << " leaf = " << li + 1 << ", combine_file = " << leaves_[li + 1]->get_label_index_name(
              file_name_pre_.str(), "combined") << '\n';
#endif
            }
          }
        }
                                                          ...
                                                          bocl_mem*                     mu_cl_mem_)
{
  ...
  check_val(status, MEM_FAILURE,
            " release DEBUG failed on device " + device->device_identifier() + error_to_string(status) );
  delete [] debug_buff_;
  return true;
}
...
bool boxm2_volm_matcher_p1::write_matcher_result(vcl_string const& tile_fname_bin, vcl_string const& tile_fname_txt)
{
  ...
  for( unsigned i = 0; i < all_loc; i++ )
    ...
    txt_ofs << vcl_setprecision(6) << score->leaf_id_ << ' ' << score->hypo_id_ << ' ' << score->max_score_ << ' '
            << score->max_cam_id_ << '\n';
    for( unsigned jj = 0; jj < cam_ids.size(); jj++ )
      {
      txt_ofs << ' ' << cam_ids[jj];
...
bool boxm2_volm_matcher_p1::write_gt_cam_score(unsigned const& leaf_id, unsigned const& hypo_id,
                                               vcl_string const& out_fname)
{
  ...
  for( unsigned i = 0; i < score_cam_.size(); i++ )
    ...
    if( score_cam_[i].l_id_ == leaf_id && score_cam_[i].h_id_ == hypo_id )
      ...
      for( unsigned jj = 0; jj < cam_ids.size(); jj++ )
        {
        ofs << vcl_setprecision(6) << cam_ids[jj] << ' ' << cam_scores[jj] << ' '
            << query_->get_cam_string(cam_ids[jj]) << '\n';
        }
      return true;
      }
    }
...
bool boxm2_volm_matcher_p1::transfer_query()
{
  ...
  if( is_grd_reg_ )
    ...
    for( unsigned cam_id = 0; cam_id < *n_cam_; cam_id++ )
      ...
      for( unsigned vox_id = 0; vox_id < grd_land[cam_id].size(); vox_id++ )
        ...
        for( vcl_vector<unsigned char>::iterator vit = grd_land[cam_id][vox_id].begin();
             vit != grd_land[cam_id][vox_id].end(); ++vit )
          {
          vcl_cout << (int)(*vit) << ' ';
          }
        vcl_cout << "] ---> lnd_wgt = [";
        for( vcl_vector<float>::iterator vit = grd_land_wgt[cam_id][vox_id].begin();
             vit != grd_land_wgt[cam_id][vox_id].end(); ++vit )
          {
          vcl_cout << *vit << ' ';
          }
        vcl_cout << ']' << vcl_endl;
    ...
    for( unsigned cam_id = 0; cam_id < *n_cam_; cam_id++ )
      ...
      for( unsigned vox_id = vox_start; vox_id < vox_end; vox_id++ )
        {
        vcl_cout << " in buffer, cam_id = " << cam_id << ", vox_global_id = " << vox_id << ", vox_id = " << vox_id
          - vox_start << " ---> lnd_id = [";
    ...
    grd_dist_cl_mem_ =
      new bocl_mem(gpu_->context(), grd_dist_buff_, sizeof(unsigned char) * grd_vox_size, " grd_dist " );
    ...
    grd_id_offset_cl_mem_ = new bocl_mem(
        gpu_->context(), grd_id_offset_buff_, sizeof(unsigned) * (*n_cam_ + 1), " grd_offset " );
    ...
    grd_land_cl_mem_ = new bocl_mem(
        gpu_->context(), grd_land_buff_, sizeof(unsigned char) * grd_vox_size * fallback_size_, " grd_lnd ");
    ...
    grd_land_wgt_cl_mem_ = new bocl_mem(
        gpu_->context(), grd_land_wgt_buff_, sizeof(float) * grd_vox_size * fallback_size_, " grd_lnd_wgt ");
  ...
  if( is_sky_reg_ )
    ...
    sky_id_offset_cl_mem_ = new bocl_mem(
        gpu_->context(), sky_id_offset_buff_, sizeof(unsigned) * (*n_cam_ + 1), " sky_offset " );
  ...
  if( is_obj_reg_ )
    ...
    for( unsigned obj_id = 0; obj_id < *n_obj_; obj_id++ )
      ...
      for( vcl_vector<unsigned char>::iterator vit = query_->obj_land_id()[obj_id].begin();
           vit != query_->obj_land_id()[obj_id].end(); ++vit )
        {
        vcl_cout << (int)(*vit) << ' ';
        }
      vcl_cout << "] ---> lnd_wgt = [";
      for( vcl_vector<float>::iterator vit = query_->obj_land_wgt()[obj_id].begin();
           vit != query_->obj_land_wgt()[obj_id].end(); ++vit )
        {
        vcl_cout << *vit << ' ';
        }
      vcl_cout << ']' << vcl_endl;
    ...
    obj_id_offset_cl_mem_ = new bocl_mem(
        gpu_->context(), obj_id_offset_buff_, sizeof(unsigned) * (obj_offset_size + 1), " obj_offset " );
    ...
    obj_min_dist_cl_mem_ = new bocl_mem(
        gpu_->context(), obj_min_dist_buff_, sizeof(unsigned char) * (*n_obj_), " obj_min_dist " );
    ...
    obj_order_cl_mem_ =
      new bocl_mem(gpu_->context(), obj_order_buff_, sizeof(unsigned char) * (*n_obj_), " obj_order " );
    ...
    obj_land_cl_mem_ = new bocl_mem(
        gpu_->context(), obj_land_buff_, sizeof(unsigned char) * (*n_obj_) * fallback_size_, " obj_land " );
    ...
    obj_land_wgt_cl_mem_ = new bocl_mem(
        gpu_->context(), obj_land_wgt_buff_, sizeof(float) * (*n_obj_) * fallback_size_, " obj_land_wgt ");
...
bool boxm2_volm_matcher_p1::transfer_orient()
{
  ...
  obj_orient_cl_mem_ =
    new bocl_mem(gpu_->context(), obj_orient_buff_, sizeof(unsigned char) * (*n_obj_), " obj_orient ");
...
bool boxm2_volm_matcher_p1::transfer_weight()
{
  ...
  obj_wgt_attri_cl_mem_ = new bocl_mem(
      gpu_->context(), obj_wgt_attri_buff_, 4 * (*n_obj_) * sizeof(float), " obj_wgt_attri " );
  ...
  for( unsigned ind_id = 0; ind_id < n_ind; ind_id++ )
    ...
    for( unsigned cam_id = 0; cam_id < nc; cam_id++ )
      ...
      if( is_grd_reg_ && is_sky_reg_ )
        {
        score_all.push_back(
          score_sky_all[id] + score_grd_all[id] + score_ord_all[id] + score_min_all[id] + score_ori_all[id]
          + score_lnd_all[id]);
        }
      else if( !is_grd_reg_ && is_sky_reg_ )
        {
        score_all.push_back(
          score_sky_all[id] + score_ord_all[id] + score_min_all[id] + score_ori_all[id] + score_lnd_all[id]);
        }
      else if( !is_sky_reg_ && is_grd_reg_ )
        {
        score_all.push_back(
          score_grd_all[id] + score_ord_all[id] + score_min_all[id] + score_ori_all[id] + score_lnd_all[id]);
        }
      else
        {
