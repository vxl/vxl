...
public:
  ...
  // : constructor
  boxm2_volm_matcher_p1(volm_camera_space_sptr const& cam_space, volm_query_sptr const& query,
                        vcl_vector<volm_geo_index_node_sptr> const& leaves, float const& buffer_capacity,
                        vcl_string const& geo_index_folder,
                        unsigned const& tile_id, vcl_vector<float> const& depth_interval,
                        vgl_polygon<double> const& cand_poly, bocl_device_sptr gpu, bool const& is_candidate,
                        bool const& is_last_pass, vcl_string const& out_folder, float const& threshold,
                        unsigned const& max_cam_per_loc, vcl_vector<volm_weight> weights);
...
private:
  ...
  bool fill_index(unsigned const& n_ind, unsigned const& layer_size, unsigned& leaf_id, unsigned char* index_buff,
                  unsigned char* index_orient_buff, unsigned char* index_land_buff, vcl_vector<unsigned>& l_id,
                  vcl_vector<unsigned>& h_id, unsigned& actual_n_ind);
  ...
  bool execute_matcher_kernel_orient(bocl_device_sptr                  device, cl_command_queue&                  queue,
                                     vcl_vector<bocl_kernel *>        kern_vec,
                                     bocl_mem*                  n_ind_cl_mem_, bocl_mem*                  index_cl_mem_,
                                     bocl_mem*           index_orient_cl_mem_, bocl_mem*             index_land_cl_mem_,
                                     bocl_mem*                  score_cl_mem_,
                                     bocl_mem*                     mu_cl_mem_);

  // : a test function to check the kernel implementation
  bool volm_matcher_p1_test_ori(unsigned n_ind, unsigned char* index, unsigned char* index_ori,
                                unsigned char* index_lnd, float* score_buff, float* mu_buff);
