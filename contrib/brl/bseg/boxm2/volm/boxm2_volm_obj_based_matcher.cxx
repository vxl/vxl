...
bool boxm2_volm_obj_based_matcher::obj_based_matcher()
{
  ...
  *n_obj_ = (unsigned)(query_->depth_regions()).size();
  ...
  if (!this->create_queue()) {
    vcl_cerr << "\n ERROR: creating matcher queue failed." << vcl_endl;
    return false;
  }
  ...
  if (kernels_.find(identifier) == kernels_.end()) {
    ...
    //vcl_cout << "Device Info: " << vcl_endl;
    //vcl_cout << gpu_->info() << vcl_endl;
  ...
  if(query_->get_query_size() != ind_->layer_size()){
    vcl_cerr << "\n ERROR: generate query and index have different voxel size" << vcl_endl;
    return false;
  }
  ...
  if ( tol_global_mem+query_global_mem_ > avail_global_mem_) {
    vcl_cerr << "\n ERROR: no enough global memory for index, reduce the size of extra empty space" << vcl_endl;
    delete n_cam_cl_mem_;         delete n_obj_cl_mem_;
  ...
  vcl_cout << "\t\t available GLOBAL memory = " << avail_global_mem_ << " byte (" << avail_global_mem_/1073741824.0 << " GB)" << vcl_endl;
  vcl_cout << "\t\t available LOCAL memory = " << avail_local_mem_ << " Byte (" << avail_local_mem_/1024.0 << " KB)" << vcl_endl;
  vcl_cout << "\t\t required GLOBAL memory for query = " << query_global_mem_ << " Byte (" << query_global_mem_/1048576.0 << " MB)" << vcl_endl;
  vcl_cout << "\t\t required LOCAL memory for query = " << query_local_mem_ << " Byte " << vcl_endl;
  vcl_cout << "\t\t index has unit size = " << index_unit_size << " Byte" << vcl_endl;
  vcl_cout << "\t\t available GLOBAL memory for indices (leave " << (float)tol_global_mem/1073741824.0 << "GB empty space) = " << avail_index_mem << " Byte (" << avail_index_mem/1073741824.0 
  ...
  for (unsigned i = 0; i <= loop_num; i++) {
    // constucture indice, index_offset buffer and cl_mem
    ...
    *n_ind_ = end - start;
    ...
    if (!n_ind_cl_mem_->create_buffer( CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR )) {
      vcl_cerr << "\n ERROR: creating bocl_mem failed fro N_INDEX" << vcl_endl;
      delete n_cam_cl_mem_;         delete n_obj_cl_mem_;
      delete grd_id_cl_mem_;        delete grd_dist_cl_mem_;       delete grd_id_offset_cl_mem_;  delete grd_weight_cl_mem_;
      delete sky_id_cl_mem_;        delete sky_id_offset_cl_mem_;  delete sky_weight_cl_mem_;
      delete obj_id_cl_mem_;        delete obj_id_offset_cl_mem_;  delete obj_weight_cl_mem_;
    ...
    if(!this->fill_index(start, end, ind_buff, ind_offset_buff)) {
      vcl_cerr << "\n ERROR: constructing index_buff for index from " << start << " to " << end << " failed" << vcl_endl;
      delete [] ind_buff;
      return false;
    }
    ...
    if (!ind_offset_cl_mem->create_buffer( CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR )) {
      vcl_cerr << "\n ERROR: creating bocl_mem failed for INDEX_OFFSET" << vcl_endl;
      delete n_cam_cl_mem_;         delete n_obj_cl_mem_;
      delete grd_id_cl_mem_;        delete grd_dist_cl_mem_;       delete grd_id_offset_cl_mem_;  delete grd_weight_cl_mem_;
      delete sky_id_cl_mem_;        delete sky_id_offset_cl_mem_;  delete sky_weight_cl_mem_;
      delete obj_id_cl_mem_;        delete obj_id_offset_cl_mem_;  delete obj_weight_cl_mem_;
    ...
    if ( matcher_local_threads_[0] * matcher_local_threads_[1] > max_work_group_size ) {
      vcl_cerr << "\n ERROR: work_item_size [" << matcher_local_threads_[0] << "x" << matcher_local_threads_[1] << ']'
               << " is larger than device maximum work group size " << max_work_group_size << vcl_endl;
      delete n_cam_cl_mem_;         delete n_obj_cl_mem_;
      delete grd_id_cl_mem_;        delete grd_dist_cl_mem_;       delete grd_id_offset_cl_mem_;  delete grd_weight_cl_mem_;
      delete sky_id_cl_mem_;        delete sky_id_offset_cl_mem_;  delete sky_weight_cl_mem_;
      delete obj_id_cl_mem_;        delete obj_id_offset_cl_mem_;  delete obj_weight_cl_mem_;
    ...
    vcl_cout << "\t\t i = " << i << ", start = " << start << ", end = " << end 
             ...
             << "--> index_buff_size = " << ind_size << vcl_endl;
    vcl_cout << "\t\t NDRange is work_group_size = [" << matcher_local_threads_[0] << "x" << matcher_local_threads_[1] << ']'
    ...
    if (!mu_cl_mem->create_buffer( CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR )) {
      vcl_cerr << "\n ERROR: creating bocl_mem failed for MU array" << vcl_endl;
      delete n_cam_cl_mem_;         delete n_obj_cl_mem_;
      delete grd_id_cl_mem_;        delete grd_dist_cl_mem_;       delete grd_id_offset_cl_mem_;  delete grd_weight_cl_mem_;
      delete sky_id_cl_mem_;        delete sky_id_offset_cl_mem_;  delete sky_weight_cl_mem_;
      delete obj_id_cl_mem_;        delete obj_id_offset_cl_mem_;  delete obj_weight_cl_mem_;
    ...
    if (!this->execute_matcher_kernel(gpu_, queue_, kernels_[identifier][0], 
                                      ...
                                      mu_cl_mem,
                                      score_cl_mem) ) {
      vcl_cerr << "\n ERROR: matcher kernel execution failed" << vcl_endl;
      delete n_cam_cl_mem_;         delete n_obj_cl_mem_;
      delete grd_id_cl_mem_;        delete grd_dist_cl_mem_;       delete grd_id_offset_cl_mem_;  delete grd_weight_cl_mem_;
      delete sky_id_cl_mem_;        delete sky_id_offset_cl_mem_;  delete sky_weight_cl_mem_;
      delete obj_id_cl_mem_;        delete obj_id_offset_cl_mem_;  delete obj_weight_cl_mem_;
...
bool boxm2_volm_obj_based_matcher::transfer_query()
{
  ...
  *sky_weight_buff_ = query_->sky_weight();
  ...
  vcl_cout << " sky_weight = " << *sky_weight_buff_ << '\n';
                                                          ...
                                                          bocl_mem*              score_cl_mem)
{
  ...
  if (!debug_cl_mem->create_buffer( CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR )) {
    vcl_cerr << "\n ERROR: creating bocl_mem failed for DEBUG array" << vcl_endl;
  ...
  unsigned i = 0;
  while( (debug_buff[i]-12.31)*(debug_buff[i]-12.31)>0.0001 && i < 1000){
