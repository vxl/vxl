...
::conf_match_cpp(vcl_string const& index_name, bool const& use_height)
{
  ...
  for( unsigned l_idx = 0; l_idx < n_leaves; l_idx++ )
    ...
    //  return -1;
    // }
    vcl_cout << "loading index from: " << bin_file_name << " using " << buffer_capacity_ << " GB buffer" << vcl_flush
             << vcl_endl;
    volm_conf_buffer<volm_conf_object> ind(buffer_capacity_);
    if( !ind.initialize_read(bin_file_name) )
      {
      vcl_cerr << "In boxm2_volm_conf_matcher::conf_matcher: can not initialize reading index binary file: "
               << bin_file_name << "!\n";
      return -1;
      }
    ...
    vcl_string score_bin_file = score_file_pre.str() + "_" + leaf->get_string() + "_"
      + index_name + ".bin";
    volm_conf_buffer<volm_conf_score> score_idx(buffer_capacity_);
    if( !score_idx.initialize_write(score_bin_file) )
      {
      vcl_cout << "In boxm2_volm_conf_matcher::conf_matcher: can not initialize writing score binary file: "
               << score_bin_file << "!\n";
      return -1;
      }
    ...
    while( leaf->hyps_->get_next(0, 1, h_pt) )
      ...
      if( !ind.get_next(values) )
        {
        vcl_cerr << "In boxm2_volm_conf_matcher::conf_matcher: get index for location " << h_pt.x() << ", "
                 << h_pt.y() << " failed!\n";
        return -1;
        }
  ...
  for( unsigned l_idx = 0; l_idx < n_leaves; l_idx++ )
    ...
    if( !vul_file::exists(bin_file_name) )
      {
      vcl_cerr << "In boxm2_volm_conf_matcher::conf_matcher: can not find index binary file " << bin_file_name
               << " for leaf " << leaf_idx << " and index name " << index_name << "!\n";
      return 0;
      }
    vcl_cout << "loading index from: " << bin_file_name << " using " << buffer_capacity_ << " GB buffer" << vcl_flush
             << vcl_endl;
...
#  if 0
      ...
      if( is_cand_ &&
          !volm_candidate_list::inside_candidate_region(cand_poly_in_, cand_poly_out_, h_pt.x(), h_pt.y() ) )
        {
        continue;
        }
#  endif
...
::matching(vcl_vector<volm_conf_object> const& values, volm_conf_score& score, bool const& use_height)
{
  ...
  for( unsigned cam_id = 0; cam_id < ncams; cam_id++ )
    ...
    for( vcl_map<vcl_string, volm_conf_object_sptr>::iterator mit = ref_objs.begin(); mit != ref_objs.end(); ++mit )
      ...
      for( vcl_map<vcl_string, volm_conf_object_sptr>::iterator q_mit = query_map.begin();  q_mit != query_map.end();
           ++q_mit )
        {
      ...
      for( vcl_vector<volm_conf_object>::iterator vit = index_map[ref_land_id].begin();
           vit != index_map[ref_land_id].end(); ++vit )
        {
        if( use_height )
          {
      ...
      score_ref_map.insert(vcl_pair<vcl_string, vcl_pair<float, float> >(mit->first,
                                                                         vcl_pair<float, float>(score_ref_i,
                                                                                                theta_ref_i) ) );
    ...
    for( vcl_map<vcl_string, vcl_pair<float, float> >::iterator mit = score_ref_map.begin(); mit != score_ref_map.end();
         ++mit )
      {
    ...
    score_cam_map.insert(vcl_pair<unsigned, vcl_pair<float, float> >(cam_id,
                                                                     vcl_pair<float, float>(max_score_cam,
                                                                                            max_theta_cam) ) );
  ...
  for( vcl_map<unsigned, vcl_pair<float, float> >::iterator mit = score_cam_map.begin(); mit != score_cam_map.end();
       ++mit )
    {
                       ...
                       bool const& use_height)
{
  ...
  for( unsigned j = 0; j < obj_q.size(); j++ )
    ...
    float                        dist_q = obj_q[j]->dist();  float theta_q = obj_q[j]->theta();
    float                        height_q = obj_q[j]->height();
    ...
    for( vcl_vector<volm_conf_object>::iterator vit = objs_i.begin(); vit != objs_i.end(); ++vit )
      ...
      if( volm_osm_category_io::volm_land_table[land_q].name_ == "building" ||
          volm_osm_category_io::volm_land_table[land_q].name_ == "tall_building" )
        {
