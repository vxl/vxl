...
static bool generate_landmarks(vgl_point_2d<double> const& pt, vcl_vector<volm_conf_object> const& land_objs,
...
int main(int argc, char* * argv)
{
  ...
  // input check
  if( world_id() == 9999 || geo_folder().compare("") == 0 || score_folder().compare("") == 0 ||
      out_kml().compare("") == 0 )
    {
  ...
  for( unsigned t_idx = 0; t_idx < tiles.size(); t_idx++ )
    ...
    vcl_cout << "  loading and sorting scores for tile " << t_idx << " from " << loc_leaves.size() << " leaves"
             << vcl_endl;
    ...
    for( unsigned i = 0; i < loc_leaves.size(); i++ )
      ...
      vcl_string               score_bin_file = score_file_pre.str() + "_" + leaf->get_string() + "_" + index_name()
        + ".bin";
      ...
      while( leaf->hyps_->get_next(0, 1, h_pt) )
        ...
        vcl_pair<float, vcl_pair<volm_conf_score, vgl_point_2d<double> > > tmp_pair(
          score_in.score(), vcl_pair<volm_conf_score, vgl_point_2d<double> >(score_in, h_pt_2d) );
        score_map.insert(tmp_pair);
        }
      } // end of loop over leaves
    vcl_cout << "  scores in tile " << t_idx << " from " << loc_leaves.size() << " have been sorted successfully"
             << vcl_flush << vcl_endl;
    } // end of loop over tiles

  vcl_cout << "Start to generate " << num_top_locs() << " top regions from " << score_map.size()
  ...
  vcl_vector<vcl_vector<unsigned char> >
                    pin_pt_landmark_types;
  vcl_vector<float> pin_pt_max_dist;
  vcl_vector<float>
                    pin_pt_heading_mid;
  vcl_vector<float> likelihood;
  vcl_multimap<float, vcl_pair<volm_conf_score, vgl_point_2d<double> >,
               vcl_greater<float> >::iterator mit = score_map.begin();
  while( pin_pt_center.size() < num_top_locs() && mit != score_map.end() )
    ...
    float max_dist = 0.0;
    for( vcl_vector<volm_conf_object>::iterator vit = mit->second.first.landmarks().begin();
         vit != mit->second.first.landmarks().end(); ++vit )
      {
  ...
  vcl_cout << pin_pt_center.size() << " pin points are created out of " << score_map.size() <<  " matched locations"
           << vcl_endl;
  ...
  for( unsigned i = 0; i < pin_pt_center.size(); i++ )
    ...
    if( !volm_candidate_list::generate_heading_direction(pin_pt_center[i], pin_pt_heading_mid[i],
                                                         pin_pt_max_dist[i] * 1.2f, right_fov, heading_line,
                                                         cam_viewing) )
      {
    ...
    volm_candidate_list::write_kml_regions(ofs_kml, pin_pt_poly[i][0], pin_pt_center[i], heading_line, cam_viewing,
                                           pin_pt_landmarks[i], pin_pt_landmark_types[i], likelihood[i], rank++);
    }
...
int main(int argc, char* * argv)
{
  ...
  if( vul_file::exists(cand_file() ) )
    ...
    vcl_cout << "candidate regions (" << cand_out.num_sheets() << " outer sheet and " << cand_in.num_sheets()
             << " inner sheet)are loaded from file: "
             << cand_file() << "!!!!!!!!!!" << vcl_endl;
...
#  endif
    ...
    vcl_cout << "  socres in tile " << t_idx << " from " << loc_leaves.size() << " have been sorted successfully"
             << vcl_flush << vcl_endl;
    } // end of loop over tiles
...
bool generate_landmarks(vgl_point_2d<double> const& pt, vcl_vector<volm_conf_object> const& land_objs,
                        vcl_vector<vgl_point_2d<double> >& landmarks)
{
