...
int main(int argc, char* * argv)
{
  ...
  // input check
  if( tile_id() == 9999 || geo_folder().compare("") == 0 || index_name().compare("") == 0 ||
  ...
  if( query_score() )
    ...
    vcl_cout << "location " << vcl_setprecision(6) << vcl_fixed << gt_lon() << ", " << vcl_setprecision(6)
             << vcl_fixed << gt_lat()
             << " is in leaf " << leaf->extent_
             << " and closest location (" << dist << " meter away) is "
             << vcl_setprecision(6) << vcl_fixed << closest_pt.x() << ", " << vcl_setprecision(6) << vcl_fixed
             << closest_pt.y() << vcl_endl;
  ...
  if( tile_id() >= tiles.size() )
    {
    log << "ERROR: unknown tile id: " << tile_id() << ", there are " << tiles.size() << " tiles in world "
        << world_id() << "!\n";
    volm_io::write_error_log(log_file.str(), log.str() );
    return volm_io::EXE_ARGUMENT_ERROR;
    }
  ...
  vcl_cout << "---------  Start to create probability map for tile " << tile_id() << " in world " << world_id()
           << " -------------------- " << vcl_endl;
