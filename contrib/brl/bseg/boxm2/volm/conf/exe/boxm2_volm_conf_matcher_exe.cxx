...
int main(int argc, char* * argv)
{
  ...
  vul_arg<vcl_string> cand_folder("-cand-folder",
                                  "folder where the candidate regions for each geo location leaf stores", "");
  ...
  if( use_height() )
    {
    vcl_cout << "Configuration Matcher will match the height of URGENT building footprints!!!!!!!!!!!!!!!!!!!!!!!!"
             << vcl_endl;
    }
  ...
  if( !read() )        // perform configurational matcher
    ...
    if( vul_file::exists(dms_bin() ) )  // load from .vsl file
      ...
      vcl_cout << "Configuration Matcher will load query from tag xml file: " << tag_xml()
               << "!!!!!!!!!!!!!!!!!!!!!!!!" << vcl_endl;
    ...
    if( !volm_io::read_camera(cam_kml(), dms->ni(), dms->nj(), heading, heading_dev, tilt, tilt_dev, roll, roll_dev,
                              tfov, tfov_dev, altitude, lat, lon) )
      {
    ...
    vcl_cout << "--------- Operate configuration matcher on tile " << tile_id() << " and leaf " << leaf_id()
             << " ------------ " << vcl_endl;
    ...
    if( cam_string.size() < 20 )
      ...
      for( unsigned i = 0; i < conf_objs.size(); i++ )
        {
        vcl_cout << "   camera " << i << " : " << cam_string[i] << " has " << conf_objs[i].size()
                 << " configurational objects " << vcl_endl;
        }
      }
    unsigned num_locs = 0;
    vcl_cout << " Index type: " << index_name() << vcl_endl;
    vcl_cout << " Tile " << tile_id() << " has " << loc_leaves.size() << " leaves for matcher" << vcl_endl;
    vcl_cout << " Configuration matcher will operate on " << loc_leaves.size() << " leaves (leaf id: " << leaf_id()
             << ')' << vcl_endl;
    ...
    vul_timer t;
    t.mark();
    boxm2_volm_conf_matcher matcher(query, tile_id(), loc_leaves, idx_folder(), out_folder(),
                                    cand_folder(), buffer_capacity() );
    int matched_locs = 0;
    if( is_cand() )
      {
    ...
    vcl_cout << " Matching " << matched_locs << " and " << query->ncam() << " cameras per location costs " << t.all()
    / (1000.0 * 60.0) << " minutes" << vcl_endl;
...
int main(int argc, char* * argv)
{
  ...
  if( leaf_id() >= 0 && leaf_id() < (int)loc_leaves_all.size() )
    ...
    if( loc_leaves.empty() )
      {
      vcl_cout << "input leaf " << leaf_id() << ": " << loc_leaves_all[leaf_id()]
               << " is outside candidate polygon.  Exit!!!\n";
      return volm_io::SUCCESS;
      }
    }
  else
...
#  endif
    ...
    vul_timer t;
    t.mark();
    boxm2_volm_conf_matcher matcher(query, tile_id(), loc_leaves, idx_folder(),
                                    out_folder(), cand_out, cand_in, buffer_capacity() );
    ...
    vcl_cout << " Matching " << matched_locs << " and " << query->ncam() << " cameras per location consumes "
             << t.all() / (1000.0 * 60.0) << " minutes" << vcl_endl;
    return volm_io::SUCCESS;
    }
  else  // read the score for given leaf
    {
