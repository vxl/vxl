...
int main(int argc, char* * argv)
{
  ...
  vul_arg<unsigned>            norients("-nori",
                                        "number of orientation type considered, default are horizontal, vertical and infinite",
                                        0);
  vul_arg<unsigned> nlands("-nlands", "number of land types considered, default are 35 land types", 0);
  vul_arg<float>    buffer_capacity("-buffer",
                                    "buffer size used for indices, ensure the system has 2*buffer_capacity memory, unit as GB (default 2.0G)",
                                    2.0f);
  ...
  if( radius().size() == 0 )
    {
    vcl_cout << " NOTE: No distance type considered, the descriptor only consider land type and orientation"
             << vcl_endl;
    }
  if( norients() != 0 || nlands() != 0 )
    {
    vcl_cout << " WARNING: specified number of orientation types and specified number of land type considered"
             << vcl_endl;
    }
  ...
  volm_spherical_container_sptr sph = new volm_spherical_container(params.solid_angle, params.vmin,
                                                                   params.dmax);
  ...
  vcl_cout << " ============================= Start to create existence indices for tile " << tile_id()
           << " ===================== " << vcl_endl;
  ...
  if( is_land_only() )
    ...
    if( norients() == 0 && nlands() == 0 )
      {
      ex_indexer = new volm_desc_ex_land_only_indexer(index_folder(), out_index_folder(),
                                                      radius(), depth_interval, index_layer_size, buffer_capacity() );
      }
    else
      {
      ex_indexer = new volm_desc_ex_land_only_indexer(index_folder(), out_index_folder(),
    ...
    if( norients() == 0 && nlands() == 0 )
      {
      ex_indexer = new volm_desc_ex_indexer(index_folder(), out_index_folder(),
                                            radius(), depth_interval, index_layer_size, buffer_capacity() );
      }
    else
      {
  ...
  vcl_cout << " ============================= Finished to create existence indices for tile " << tile_id()
           << " ===================== " << vcl_endl;
