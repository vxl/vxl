...
int main(int argc, char* * argv)
{
  ...
  if( is_matcher() )
    ...
    if( !vul_file::exists(query_xml() ) )    // load query from vxl file and weight from weight file
      ...
      if( weights.size() != (!dms->sky().empty() + !dms->ground_plane().empty() + dms->scene_regions().size() ) )
        ...
        vcl_cout << " depth map scene size = " << !dms->sky().empty() + !dms->ground_plane().empty()
        + dms->scene_regions().size() << vcl_endl;
      ...
      if( weights.size() != dms->scene_regions().size() )
        {
        err_log << "ERROR: number of weight parameters (" << weights.size()
                << ") is inconsistent with labeled object (" << dms->scene_regions().size() << ")\n";
        error_report(err_log_file.str(), err_log.str() );
        return volm_io::EXE_ARGUMENT_ERROR;
        }
      }
    ...
    else if( world_region.compare("Desert") == 0 )
      {
      tiles = volm_tile::generate_p1_wr1_tiles();
      }
    else
      {
      err_log << "ERROR: unknown ROI region: " << world_region
              << ", check tag xml.  Available regions are: Coast, Desert, Chile, India, Jordan, Philippines, Taiwan\n";
      error_report(err_log_file.str(), err_log.str() );
      return volm_io::EXE_ARGUMENT_ERROR;
      }
    ...
    // Screen output
    vcl_cout << " =========== Start to execute existence matcher for image: " << image_name << " ==============="
             << vcl_endl;
    ...
    for( unsigned i = 0; i < obj.size(); i++ )
      {
      vcl_cout << " \t\t" << obj[i]->name() << " --- mindist = " << obj[i]->min_depth() << ", maxdist = "
               << obj[i]->max_depth()
  ...
  if( is_cand() )
    ...
    if( out_folder().compare("") == 0 )
      {
      vcl_cerr
      << "ERROR: output folder and probability map folder can not be empty when creating candidate list.  Check input"
      << vcl_endl;
      vul_arg_display_usage_and_exit();
      return volm_io::EXE_ARGUMENT_ERROR;
      }
    ...
    if( !volm_desc_matcher::create_candidate_list(out_folder(), cand_folder.str(), thresc(), top_size(), ku(), kl(),
                                                  threshold(), query_name(), world_str() ) )
      {
...
int main(int argc, char* * argv)
{
  ...
  if( is_matcher() )
    ...
    // start the matcher
    vcl_cout << " =========== Start to execute existence matcher on tile " << tile_id() << " for image: "
             << image_name.str() << " ===============" << vcl_endl;
    vcl_cout << " \t Descriptor type : " << ex_matcher->get_index_type_str() << vcl_endl;
    vcl_cout << " \t weight parameters: " << vcl_endl;
    for( unsigned i = 0; i < weights.size(); i++ )
      {
  ...
  if( is_cand() )
    ...
    vcl_cout << " =========== Start to create candidate list for image: " << image_name.str() << " ==============="
             << vcl_endl;
    ...
    vcl_cout << "\t result stored in: " << cand_folder.str() << vcl_endl;
    if( !volm_desc_matcher::create_candidate_list(prob_map_folder.str(), cand_folder.str(), thresc(), top_size(), ku(),
                                                  kl(), threshold(), test_id(), id(), world_id() ) )
      {
