...
bool best_match(vgl_polygon<double> const& poly, vcl_vector<volm_geo_index_node_sptr> const & leaves,
                vcl_string const& score_file,
...
int main(int argc, char* * argv)
{
  ...
  for( unsigned r_idx = 0; r_idx < num_regions; r_idx++ )
    ...
    vcl_cout << " \t after searching region " << r_idx << " has best maximum " << max_score << " at location "
             << vcl_setprecision(12) << best_location.x() << vcl_setprecision(12) << best_location.y()
             << " and camera ";
    best_cam.print();
    ...
    bkml_write::write_photo_overlay(ofs_kml, kml_name.str(), best_location.x(), best_location.y(),
