...
{
  vul_arg<vcl_string> scene_file("-scene", "scene xml filename prefix", "");
  vul_arg<int>        scene_id("-id", "scene id", -1);
  vul_arg<vcl_string> builds_file("-builds", "folder with files", "");
  ...
  vul_arg<vcl_string> lidar_folder("-lidar", "folder with lidar images", "");
  ...
  if( scene_file().compare("") == 0 || builds_file().compare("") == 0 || out_folder().compare("") == 0 ||
      class_folder().compare("") == 0 )
    {
  ...
  lvcs->local_to_global(scene_bbox.min_point().x(),
                        scene_bbox.min_point().y(), 0, vpgl_lvcs::wgs84, min_lon, min_lat, gz);
  lvcs->local_to_global(scene_bbox.max_point().x(),
                        scene_bbox.max_point().y(), 0, vpgl_lvcs::wgs84, max_lon, max_lat, gz);
  ...
  for( vul_file_iterator fit = glob; fit; ++fit )
    ...
    vcl_pair<vil_image_view<float>, vpgl_geo_camera *> pair(img, cam);
    lidar_imgs.push_back(pair);

    vcl_string name = vul_file::strip_directory(fit() );
  ...
  vcl_cout << "there are " << lidar_imgs.size() << " lidar images that intersect the scene!\n";
  ...
  vcl_cout << "pier color is: " << pier_color << " water id: " << (int)water_id << " sand id: " << (int)sand_id
           << " pier_id: " << (int)pier_id << "\n";
      ...
      if( sbbox.contains(objects[kk].first) )  // the scene contains this bbox
        ...
        for( int ii = i - fort_rad; ii < i + fort_rad; ii++ )
          ...
          for( int jj = j - fort_rad; jj < j + fort_rad; jj++ )
            {
            if( ii >= 0 && jj >= 0 && ii < (int)out_img_label.ni() && jj < (int)out_img_label.nj() &&
                out_img_label(ii, jj) != water_id )
              {
        ...
        if( cnt > 750 )  // water on at least 1 side
          ...
          for( int ii = i - 200; ii < i + 200; ii++ )
            ...
            for( int jj = j - 200; jj < j + 200; jj++ )
              ...
              if( out_img_label(ii,
                                jj) == dev_id1 || out_img_label(ii, jj) == dev_id2 || out_img_label(ii, jj) == dev_id3 )
                {
                cnt_dev++;
                }
              }
