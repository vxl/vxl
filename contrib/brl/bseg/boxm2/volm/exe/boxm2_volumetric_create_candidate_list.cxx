                     ...
                     vcl_vector<cam_angles>& top_cameras, vcl_vector<double>& right_fov)
{
  ...
  for( unsigned i = 0; i < top_size; i++ )
    {
    unsigned                 hypo_id;
    volm_geo_index_node_sptr leaf_node = volm_geo_index::get_closest(root, top_locs[i].y(),
                                                                     top_locs[i].x(), hypo_id);
    unsigned                              cam_id;
                       ...
                       volm_camera_space_sptr const& cam_space, unsigned ni, unsigned nj, double max_depth)
{
  ...
  bkml_write::write_photo_overlay(ofs_kml, kml_name.str(), lon, lat,
                                  cam_space->altitude(), head, tilt, roll, tfov, rfov, max_depth);
...
int main(int argc,  char* * argv)
{
  ...
  vul_arg<bool>       is_generate_top_cam_kml("-is_top_cam",
                                              "option to choose whether we retrive top cameras from generated candidate list",
                                              false);
  ...
  if( is_generate_top_cam_kml() )
    ...
    if( cam_bin().compare("") == 0 || query_img().compare("") == 0 || score_folder().compare("") == 0 ||
        dms_bin().compare("") == 0 || geo_hypo().compare("") == 0 )
      {
    ...
    for( unsigned i = 0; i < top_cams; i++ )
      ...
      vcl_cerr << " Top " << i << " locations " << lon << ',' << lat << " is in utm zone " << zone_id << " and tile "
               << tile_id << vcl_endl;
      ...
      if( !vul_file::exists(file_name_pre.str() + ".txt") )
        {
        log << " ERROR: location " << lon << ',' << lat << " is in tile " << tile_id
            << " but no geo locations for this tile, stop\n";
        volm_io::write_post_processing_log(log_file, log.str() );  vcl_cerr << log.str();
      ...
      if( leaf )
        ...
        vcl_cout << " loc_closest = " << loc_closest << " with leaf id = " << leaf->get_string() << " and hypo_id = "
                 << hypo_id << vcl_endl;
        if( gt_dist > 0 )
          {
          log << " NOTE: distance from the closest location " << loc_closest << " in geo location to location "
              << top_locs[i]
              << " is " << gt_dist << " meters\n";
          }
        }
      ...
      if( !create_camera_kml(i, loc_closest.x(), loc_closest.y(), leaves, leaf, hypo_id, score_file.str(),
                             rational_folder, cam_space, ni, nj, max_depth) )
        {
  ...
  if( out().compare("") == 0 || threshold() == 0 || world_id() == 9999 || cam_bin().compare("") == 0 ||
      query_img().compare("") == 0 ||
  ...
  log << " there are " << cand_map.size() << " candidate regions given threshold = " << threshold()
      << " (likelihood = " << thres_value << ")\n";
  vcl_cerr << log.str();
  volm_io::write_post_processing_log(log_file, log.str() );
  ...
  for( ; mit != cand_map.end(); ++mit )
    ...
    if( is_camera() )
      {
      if( !get_top_cameras(tile_idx, cam_bin(), ni, nj, score_folder(), geo_hypo_folder, top_locs, top_cameras,
                           right_fov) )
        {
    ...
    volm_candidate_list::write_kml_regions(ofs_kml, region_loc_global, top_locs, top_cameras, right_fov, likelihood,
                                           rank++);
    ...
    for( unsigned i = 0; i < top_locs.size(); i++ )
      ...
      if( rank % 500 == 0 )
        {
        vcl_cerr << " size = " << cand_map.size() << ", cnt = " << rank - 1 << " score = " << mit->first
                 << " --> tile = " << tile_idx << " sh_id = " << sh_idx
                 << " --> top_locs = " << vcl_setprecision(10) << top_locs[i].x() << ", " << top_locs[i].y()
                 << " top_score = " << top_loc_scores[i] << vcl_endl;
        }
      }
#endif
    }
  ...
  for( unsigned t_idx = 0; t_idx < n_tile; t_idx++ )
    {
    vcl_cout << " For tile " << t_idx << ", there are " << cand_lists[t_idx].num_of_regions()
             << " regions above threshold " << threshold() << vcl_endl;
    }
  // best scores
  for( unsigned t_idx = 0; t_idx < n_tile; t_idx++ )
    {
  ...
  for( ; mit != cand_map.end(); ++mit )
    {
    vcl_cout << " region score = " << mit->first << " --> tile_id = " << mit->second.first << " , sheet_id = "
             << mit->second.second << vcl_endl;
