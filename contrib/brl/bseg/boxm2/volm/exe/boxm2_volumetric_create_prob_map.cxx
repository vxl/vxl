...
{
  vul_arg<vcl_string> cam_bin("-cam", "camera psace which is use to give the camera giving the maximum score");
  ...
  vul_arg<vcl_string> world_region("-world",
                                   "ROI world region (can be desert, coast, Chile, India, Jordan, Philippines, Taiwan)",
                                   "");
  ...
  for( unsigned i = 0; i < tiles.size(); i++ )
    ...
    for( unsigned ii = 0; ii < total_ind; ii++ )
      ...
      if( tile.global_to_img(h_pt.x(), h_pt.y(), u, v) )
        {
#if 0
        vcl_cout << " total_ind = " << total_ind << " ii = " << ii << " leaf_id = " << scores[ii]->leaf_id_
                 << ", hypo_id = " << scores[ii]->hypo_id_
                 << vcl_setprecision(10) << " lon = " << h_pt.x() << " , lat = " << h_pt.y() << ", pixel = (" << u
                 << "," << v << ")"
        ...
        if( tiles[i].global_to_img(gt_lon, gt_lat, u, v) )
          ...
          if( u < tiles[i].ni() && v < tiles[i].nj() )
            ...
            log << "\t GT location: " << img_id() << ", "
                ...
                << " at location " << vcl_setprecision(12) << max_score_loc.x() << ", " << vcl_setprecision(12)
                << max_score_loc.y()
                << '\n';
            volm_io::write_post_processing_log(log_file, log.str() );
            vcl_cout << log.str();
