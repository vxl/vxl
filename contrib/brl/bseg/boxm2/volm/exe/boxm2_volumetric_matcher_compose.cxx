...
int main(int argc,  char** argv)
{
  ...
  if (!q_params.read_params_file(index_files[0])) {
    vcl_cerr << " cannot read params file for " << index_files[0] << vcl_endl;
    volm_io::write_status(out_folder(), volm_io::EXE_ARGUMENT_ERROR, 100);
    return volm_io::EXE_ARGUMENT_ERROR;
  }
  ...
  for (unsigned i = 0; i < score_files.size(); i++) {
    ...
    vcl_cout << hyp.size() << " hypotheses read from: " << hyp_files[i] << vcl_endl;
    vcl_cout << scores.size() << " scores read from: " << score_files[i] << " params start: " << params.start << " skip: " << params.skip << vcl_endl;
         ...
         if (scores[ind_idx] > current_score) {
           if (cam_ids[ind_idx] < query->get_cam_num()) {
             ...
             vcl_cout << "inserting " << scores[ind_idx] << " cam id: " << cam_ids[ind_idx] << vcl_endl;
	           ...
	           log << "score file is: " << score_files[i] << " id in the file: " << ind_idx << " hypo id: " << hyp.current_ << vcl_endl;
	           vcl_cerr << log.str();
	           volm_io::write_composer_log(out_folder(), log.str());
	         }
