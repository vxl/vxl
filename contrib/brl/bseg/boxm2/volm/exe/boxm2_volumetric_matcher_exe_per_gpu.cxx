...
int main(int argc,  char** argv)
{
  ...
  log << " generated query has size " << query->get_query_size() << vcl_endl;
  log << " The query has " << query->get_cam_num() << " cameras: " << vcl_endl;
  log << " \t" << query->headings().size() << " headings = " << query->headings()[0] << " +/- " << (query->headings().size()-1)/2 << vcl_endl;
  log << " \t" << query->tilts().size() << " tilts = " << query->tilts()[0] << " +/- " << (query->tilts().size() - 1)/2 << vcl_endl;
  log << " \t" << query->rolls().size() << " rolls = " << query->rolls()[0] << " +/- " << (query->rolls().size() - 1)/2 << vcl_endl;
  log << " \t" << query->top_fovs().size() << " top_fov = " << query->top_fovs()[0] << " +/- " << (query->top_fovs().size() - 1)/2 << vcl_endl;
  log << " Generated query_size for 1 camera is " << query->get_query_size() << " byte, "
  ...
  for (unsigned ind_i = 0; ind_i < inp_files.size(); ind_i++) {
    ...
    vcl_cout << log.str() << vcl_endl; // print log here
