...
int main(int argc, char* * argv)
{
  ...
  while( leaf_id < leaves.size() )
    ...
    vcl_string               configfile = geo_data_folder() + "/segment/" + fileprefix + "_" + hyp_leaf->get_string()
      + "_segment.bin";
    vcl_string datafile = geo_data_folder() + "/segment/" + fileprefix + "_" + hyp_leaf->get_string()
      + "_segment_data.bin";
    ...
    for( unsigned i = 0; i < hyp_leaf->hyps_->size(); i++ )
      ...
      vcl_cout << "Vals " << num_regions[0] << " " << num_regions[1] << " " << num_regions[2] << " "
               << num_regions[3] << vcl_endl;
  ...
  vcl_cout << "Total time taken is " << t.all() << " seg time " << ttime << " # of intersections " << matcher.count_
           << vcl_endl;
