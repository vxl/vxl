...
int main(int argc, char* * argv)
{
  ...
  if( !vul_file::exists(file_name_pre.str() + ".txt") )
    {
    log << " ERROR: gen_index_folder is wrong (missing last slash/ ?), no geo_index_files found in "
        << geo_hypo_folder() << '\n';
    if( do_log ) { volm_io::write_log(out_folder(), log.str() ); }
    vcl_cerr << log.str();
  ...
  vcl_cout << "\n==================================================================================================\n"
           ...
           << " The spherical shell for current query has parameters: point_angle = "
           << query->sph_shell()->point_angle()
  ...
  boxm2_volm_matcher_p1 obj_ps1_matcher(cam_space, query, leaves, buffer_capacity(), geo_index_folder(), tile_id(),
                                        depth_interval, cand_poly,
                                        mgr.gpus_[dev_id()], is_candidate, is_last_pass, out_folder(),
  ...
  if( gt_id() != 0 && gt_id() < 100 )
    ...
    if( gt_tile_id != tile_id() || gt_utm_id != zone_id() )
      ...
      vcl_cout
               <<
      "\n==================================================================================================\n"
               << "\t\t  6. write out the score for " << max_cam_per_loc() << " cameras which is higher than "
               << threshold() << "\n"
               << "\t\t     " << out_folder() << '\n'
               << "\t\t for GT location " << gt_id() << samples[gt_id()].first.y() << ", " << samples[gt_id()].first.x()
               << ", is in utm zone " << gt_utm_id << " and tile " << gt_tile_id << '\n'
               << "\t\t Associated leaf is " << leaf_gt->get_string() << "(leaf_id " << gt_l_id << ")  and "
               << " hypo_id = "
      ...
      if( !obj_ps1_matcher.write_gt_cam_score(gt_l_id, gt_h_id, gt_score_txt.str() ) )
        {
        log << " ERROR: writing output failed --> can not find ground truth leaf_id " << gt_l_id << ", hypo_id "
            << gt_h_id << '\n';
        if( do_log ) {volm_io::write_log(out_folder(), log.str() ); }
        vcl_cerr << log.str();
        volm_io::write_status(out_folder(), volm_io::EXE_ARGUMENT_ERROR, 100, "", status_xml.str() );
  ...
  if( use_ps2() )
    ...
    vcl_cout << " we will use pass 2, i.e. object based, ray based ORIENT/NLCD matcher\n"
             ...
             << " NEED TO CHECK WHETHEER WE HAVE PASS 0 MATCHER RESULT, IF SO, LOAD THE REDUCED SPACE FROM PASS 0"
             << vcl_endl;
    }
  else
    {
