...
int main(int argc, char* * argv)
{
  ...
  for( unsigned ti = 0; ti < tiles.size(); ++ti )
    ...
    for( vcl_vector<double>::iterator vit = thresholds.begin(); vit != thresholds.end(); ++vit )
      ...
      // save the image
      vcl_string out_png_name = out() + "/" + "ProbMap_" + tiles[ti].get_string() + ".tif";
  ...
  vcl_string log_fname = rational_folder.str()
    + "/matchimg.log";
  ...
  while( cnt )
    ...
    vcl_cout << " top " << (29 - cnt) << '(' << cnt << ") " << " score = " << mit_all->first << ", cam = "
             << mit_all->second.first << ", loc = " << mit_all->second.second << vcl_endl;
    }
