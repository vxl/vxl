...
int main(int argc, char* * argv)
{
  ...
  vul_arg<vcl_string> out_folder("-out",
                                 "output folder where the generate query binary, camspace binary and weight parameter stored",
                                 "");
  ...
  if( !volm_io::read_camera(cam_kml(), dm->ni(), dm->nj(), heading, heading_dev, tilt, tilt_dev, roll, roll_dev, tfov,
  ...
  volm_camera_space_sptr cam_space = new volm_camera_space(tfov, top_fov_dev, params.fov_inc, altitude,
                                                           dm->ni(), dm->nj(),
  ...
  vcl_cout << " The " << dmq->ni() << " x " << dmq->nj() << " query image has following defined depth region"
           << vcl_endl;
  if( !dmq->sky().empty() )
    {
