...
int main(int argc, char* * argv)
{
  ...
  vul_arg<vcl_string> geo_hypo_folder("-hypo", "folder prefix to read the geo hypotheses", "");
  vul_arg<vcl_string> geo_index_folder("-index", "folder prefix to read the geo index and the hypo", "");
  ...
  vul_arg<vcl_string> img("-img", "query images, to get the image size", "");                    // query -- actual query image
  ...
  // check input
  if( out().compare("") == 0 || geo_hypo_folder().compare("") == 0 || geo_index_folder().compare("") == 0 ||
      point_angle() == 0 ||
      gt_file().compare("") == 0 ||
      pass_id() > 2 ||
      img().compare("") == 0 ||
  ...
  vcl_cout << "GT location " << id() << " ---> " << samples[id()].first.x() << ", " << samples[id()].first.y()
           << ", is in tile " << tile_id << vcl_endl;
  ...
  if( !vul_file::exists(file_name_pre.str() + ".txt") )
    {
    log << "GT location " << id() << " ---> " << samples[id()].first.x() << ", " << samples[id()].first.y()
        << " is in tile " << tile_id
  ...
  if( sph_shell->get_container_size() != params.layer_size )
    {
    log << "ERROR: the spherical shell binary (" << sph_shell->get_container_size()
        << ") has different parameters setting from index ("
        << params.layer_size << ")\n";
    volm_io::write_post_processing_log(log_file, log.str() );
    vcl_cerr << log.str();
    return volm_io::EXE_ARGUMENT_ERROR;
  ...
  log << " For GT location, leaf is " << leaf_gt->get_string()
      ...
      << gt_cam_ang.get_string() << ", id = " << cam_gt_best << " ) img size " << query_img.ni() << " by "
      << query_img.nj() << '\n';
  vcl_cout << log.str();
  volm_io::write_post_processing_log(log_file, log.str() );
  ...
  vcl_stringstream file_index_name_pre;
  ...
  // write camera
  vcl_string kml_name = "BestCamera";
...
{
  vul_arg<vcl_string> gt_file("-gt_locs", "file with the gt locs of all test cases", "");
  vul_arg<vcl_string> geo_hypo_folder_pre("-hypo", "folder prefix to read the geo hypotheses", "");
...
#  if 1
  ...
  while( leaf_gt->hyps_->get_next(0, 1, h_pt) )
    ...
    if( h_id == h_gt )
      ...
      vcl_vector<unsigned char> values_ori;
      vcl_vector<unsigned char> values_lnd;
      vcl_vector<unsigned char> test_ori;
      vcl_vector<unsigned char> test_lnd;
...
#    if 0
      if( id() == 40 )
        ...
        for( unsigned ii = 0; ii < values_combine.size(); ii++ )
          {
          vcl_cout << " ii = " << ii << ", combine = " << (int)values_combine[ii]
...
#    endif
      ...
      for( unsigned i = 0; i < query_img.ni(); i++ )
        ...
        for( unsigned j = 0; j < query_img.nj(); j++ )
          {
          dst_img(i, j).r = (unsigned char)120;  dst_img(i, j).g = (unsigned char)120;
          dst_img(i, j).b = (unsigned char)120;
          ori_img(i, j).r = (unsigned char)120;  ori_img(i, j).g = (unsigned char)120;
          ori_img(i, j).b = (unsigned char)120;
          lnd_img(i, j).r = (unsigned char)120;  lnd_img(i, j).g = (unsigned char)120;
          lnd_img(i, j).b = (unsigned char)120;
          }
        }
#    ifdef DEBUG
...
#    endif
      ...
      if( id() == 36 || id() == 40 )
        ...
        for( unsigned i = 0; i < query_img.ni(); i++ )
          ...
          for( unsigned j = 0; j < query_img.nj(); j++ )
            {
            dst_gt_img(i, j).r = (unsigned char)120;  dst_gt_img(i, j).g = (unsigned char)120;
            dst_gt_img(i, j).b = (unsigned char)120;
            ori_gt_img(i, j).r = (unsigned char)120;  ori_gt_img(i, j).g = (unsigned char)120;
            ori_gt_img(i, j).b = (unsigned char)120;
            lnd_gt_img(i, j).r = (unsigned char)120;  lnd_gt_img(i, j).g = (unsigned char)120;
            lnd_gt_img(i, j).b = (unsigned char)120;
            }
          }
        query->depth_rgb_image(values_dst, gt_cam_id, dst_gt_img, "depth");
        query->depth_rgb_image(values_ori, gt_cam_id, ori_gt_img, "orientation");
