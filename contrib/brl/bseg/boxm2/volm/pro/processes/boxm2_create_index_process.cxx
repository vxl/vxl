...
//  delete subblk_dim_mem;
  ...
  vcl_cout << "\nGPU Execute time " << gpu_time << " ms = " << gpu_time / (1000.0 * 60.0) << " secs.\n"
           << "GPU Transfer time " << transfer_time << " ms = " << transfer_time / (1000.0 * 60.0) << " secs."
           << vcl_endl;
...
bool boxm2_visualize_index_process2(bprb_func_process& pro)
{
  ...
  if( !leaf )
    {
    vcl_cerr << " the geo index: " << geo_index_hyp_folder << " do not contain any hyp close to " << lat << ", "
             << lon << '\n';
    return true;
    }
  vcl_cout << "hyp " << lat << ", " << lon << " is in leaf: " << leaf->extent_ << vcl_endl;
  vcl_cout << "\t closest to hyp: " << leaf->hyps_->locs_[hyp_id].y() << ", " << leaf->hyps_->locs_[hyp_id].x()
           << vcl_endl;
