...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels)
{
  ...
  options +=
    " -D STEP_CELL=step_cell_compute_index(tblock,linfo->block_len,aux_args.alpha,data_ptr,d*linfo->block_len,aux_args.vis,aux_args.expdepth,aux_args.expdepthsqr,aux_args.probsum,aux_args.t)";
  bocl_kernel* compute_index = new bocl_kernel();

  compute_index->create_kernel(&device->context(),
                               device->device_id(), src_paths, "compute_loc_index", options, "compute_loc_index");
  ...
  norm_kernel->create_kernel(&device->context(),
                             device->device_id(), src_paths, "normalize_index_depth_kernel", options,
...
bvgl_2d_geo_index_node_sptr construct_2d_geo_index_for_blks(boxm2_scene_sptr scene)
{
  ...
  vgl_box_2d<double> scene_bbox_2d(scene_bbox.min_x(), scene_bbox.max_x(),
                                   scene_bbox.min_y(), scene_bbox.max_y() );
  bvgl_2d_geo_index_node_sptr root = bvgl_2d_geo_index::construct_tree<vcl_vector<boxm2_block_id> >(scene_bbox_2d,
                                                                                                    min_size);
  return root;
}
...
bool boxm2_create_index_process2(bprb_func_process& pro)
{
  ...
  out_sph_namet << out_index_folder << "geo_index_tile_" << tile_id << "_index_sph_shell.vrml";
  ...
  for( vcl_map<boxm2_block_id, boxm2_block_metadata>::iterator mit = blks.begin(); mit != blks.end(); ++mit )
    ...
    vgl_box_2d<double> curr_blk_bbox_2d(mit->second.bbox().min_x(),
                                        mit->second.bbox().max_x(), mit->second.bbox().min_y(),
                                        mit->second.bbox().max_y() );
  ...
  for( unsigned li = 0; li < leaves2.size(); li++ )
    ...
    while( leaves2[li]->hyps_->get_next(0, 1, h_pt) )
      ...
      if( curr_leaf )
        ...
        for( vcl_vector<boxm2_block_id>::iterator vit = curr_leaf_ptr->contents_.begin();
             vit != curr_leaf_ptr->contents_.end(); ++vit )
          {
...
//  delete subblk_dim_mem;
  ...
  vcl_cout << "GPU Transfe time " << transfer_time << " ms = " << transfer_time / (1000.0 * 60.0) << " secs. "
           << vcl_endl;
