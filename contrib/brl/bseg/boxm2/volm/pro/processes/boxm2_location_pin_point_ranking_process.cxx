...
bool boxm2_location_pin_point_ranking_process(bprb_func_process& pro)
{
  ...
  vcl_string pin_point_kml = pro.get_input<vcl_string>(in_i++);

  // parse the pin-point regions
  if (!vul_file::exists(pin_point_kml)) {
    vcl_cerr << pro.name() << ": can not find file: " << pin_point_kml << vcl_endl;
    return false;
  }
  ...
  if (!rank) {
    vcl_cerr << pro.name() << ": the location (" << lon << ", " << lat << ") is not in any pin-point region" << vcl_endl;
    return false;
  }
  // output
