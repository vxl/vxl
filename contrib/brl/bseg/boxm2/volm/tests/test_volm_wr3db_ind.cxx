...
static void test_volm_wr3db_ind()
{
  ...
  vcl_cout << "global id: " << ind->current_global_id() << " current active cache id: " << ind->current_id()
           << vcl_endl;
...
vcl_cout << "number of voxels in container: " << voxels.size() << " size of voxel array for " << data_size
         << " bytes: " << ind_size << " MB" << vcl_endl;
double inc = 10.0;
...
vcl_cout << " for 100 km size WR (in each size) for every " << inc << "m there are: " << num << " location hypotheses\n"
         << " then the index size on file is: " << num * ind_size << " MBs = " << num * ind_size / 1024 << " GBs = "
         << num * ind_size / 1048576 << " TBs.\n";
...
vcl_cout << " last layer with vmin res is at depth: " << d << " and has offset: " << offset << " end_offset: "
         << end_offset << '\n'
         << "first voxel: " << sph->get_voxels()[offset].center_ << ' ' << sph->get_voxels()[offset].resolution_ << '\n'
         << "last voxel: " << sph->get_voxels()[end_offset - 1].center_ << ' '
         << sph->get_voxels()[end_offset - 1].resolution_ << '\n'
         << "after last voxel: " << sph->get_voxels()[end_offset].center_ << ' '
         << sph->get_voxels()[end_offset].resolution_ << vcl_endl;
unsigned int size = end_offset - offset;
vcl_cout << "size is: " << size << vcl_endl;
ind_size = size * sizeof(float) * 2 * num / 1048576.0; // in MBs
vcl_cout << "if we index 2 floats for this layer each location index is of size: " << size * sizeof(float) * 2
 / 1048576.0 << " MBs, for all locs, size is: " << ind_size << " MB = " << ind_size / 1024 << " GB\n"
         << "if we index 1 floats for this layer each location index is of size: " << size * sizeof(float)
 / 1048576.0 << " MBs, for all locs, size is: " << ind_size / 2 << " MB = " << ind_size / 2048 << " GB\n"
         << "if we index 1 unsigned short for this layer, each location index is of size: " << size
 * sizeof(unsigned short) / 1048576.0 << " MBs, for all locs, size is: " << size * num * sizeof(unsigned short)
 / 1048576.0 << " MBs = " << size * num * sizeof(unsigned short) / 1048576.0 / 1024 << " GBs\n";
#endif
