...
public:
  ...
  static float pre_vis_inf( boxm2_multi_cache&              cache, const vil_image_view<float>&    img,
                            vpgl_camera_double_sptr         cam, float*                          norm_img,
                            boxm2_multi_update_helper&      helper);

private:

  // runs pre/vis on single block
  static float pre_vis_per_block(const boxm2_block_id&     id, boxm2_scene_sptr    scene,
                                 ...
                                 bocl_mem_sptr&      img_dim, bocl_mem_sptr&      ray_o_buff,
                                 bocl_mem_sptr&      ray_d_buff, bocl_mem_sptr&      tnearfarptr,
                                 bocl_mem_sptr&      cl_output, bocl_mem_sptr&      lookup,
                                 vcl_size_t*         lthreads, vcl_size_t*         gThreads);

#if 0
  // reduction step (proc norm image)
  static float pre_vis_reduce( boxm2_multi_cache&    cache, vcl_vector<float *>&   pre_imgs,
                               vcl_vector<float *>&   vis_imgs, vcl_vector<boxm2_opencl_cache1 *>& ocl_caches,
                               vcl_map<bocl_device *, float *>& pre_map, vcl_map<bocl_device *, float *>& vis_map,
                               int ni, int nj, float* norm_img   );
