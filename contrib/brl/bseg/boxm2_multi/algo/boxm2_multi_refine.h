...
private:
  static float refine_trees_per_block(const boxm2_block_id& id, boxm2_opencl_cache1* ocl_cache, cl_command_queue& queue,
                                      int numTrees, BlockMemMap&  sizebuffs, BlockMemMap&  blockCopies,
                                      bocl_mem_sptr& prob_thresh, bocl_mem_sptr& lookup, bocl_mem_sptr& cl_output );

  // : refines trees in block
  static void swap_data_per_block(boxm2_scene_sptr scene, const boxm2_block_id& id, int numTrees,
                                  boxm2_opencl_cache1* ocl_cache, cl_command_queue& queue, BlockMemMap& sizebuffs,
                                  BlockMemMap& blockCopies, BlockMemMap& newDatas, BlockIntMap& newDataSizes,
                                  bocl_mem_sptr cl_output, bocl_mem_sptr lookup, vcl_string data_type, int  apptypesize,
