                                 ...
                                 vpgl_camera_double_sptr cam)
{
  ...
  for( unsigned int i = 0; i < ocl_caches.size(); ++i )
    ...
    bocl_mem_sptr ray_o_buff =
      ocl_cache->alloc_mem(cl_ni * cl_nj * sizeof(cl_float4), ray_origins, "ray_origins buffer");
    bocl_mem_sptr ray_d_buff = ocl_cache->alloc_mem(cl_ni * cl_nj * sizeof(cl_float4), ray_directions,
    ...
    bocl_mem_sptr max_omega_image = ocl_cache->alloc_mem(cl_ni * cl_nj * sizeof(float), max_omega_buff,
                                                         "max omega buffer");
...
#  endif
  ...
  for( unsigned int i = 0; i < ocl_caches.size(); ++i )
    ...
    bocl_mem_sptr vis_image = new bocl_mem(
        device->context(), vis_buff, cl_ni * cl_nj * sizeof(float), "exp image buffer");
    vis_image->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR);
    vis_mems.push_back(vis_image);
...
boxm2_multi_render::get_kernels(bocl_device_sptr device, vcl_string opts)
{
  ...
  options +=
    " -D STEP_CELL=step_cell_render(aux_args.mog,aux_args.alpha,data_ptr,d*linfo->block_len,vis,aux_args.expint)";
