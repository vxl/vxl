...
public:
  ...
  static float store_aux( boxm2_multi_cache& cache, vil_image_view<float>& img, vpgl_camera_double_sptr cam,
...
private:
  ...
  static void store_aux_per_block(const boxm2_block_id& id, boxm2_scene_sptr    scene,
                                  boxm2_opencl_cache1* opencl_cache, cl_command_queue&   queue,
                                  bocl_kernel*        kernel, bocl_mem_sptr&      in_image, bocl_mem_sptr&      img_dim,
                                  ...
                                  vcl_size_t*         gthreads, bool                store_rgb = false);

  // Reads aux memory from GPU to CPU ram
  static void read_aux(const boxm2_block_id& id, boxm2_opencl_cache1* opencl_cache, cl_command_queue&   queue);
