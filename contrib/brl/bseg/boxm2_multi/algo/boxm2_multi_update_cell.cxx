  ...
  for( unsigned int grpId = 0; grpId < grp.size(); ++grpId )
    ...
    for( unsigned int idx = 0; idx < indices.size(); ++idx )
      ...
      transfer_time += calc_beta_per_block(id, sub_scene, ocl_cache, queues[i], data_type, kerns[0],
                                           vis_mems[i], pre_mems[i], norm_mems[i], img_dims[i], ray_os[i], ray_ds[i],
                                           tnearfarptrs[i], out_imgs[i], lookups[i], lthreads, gThreads);
                                                 ...
                                                 boxm2_multi_update_helper& helper )
{
  ...
  for( unsigned int grpId = 0; grpId < grp.size(); ++grpId )
    ...
    for( unsigned int i = 0; i < ids.size(); ++i )
      ...
      bocl_mem*  num_obs       = ocl_cache->get_data(id, num_obs_type,
...
vcl_vector<bocl_kernel *> & boxm2_multi_update_cell::get_kernels(bocl_device_sptr device, vcl_string opts)
{
  ...
  update->create_kernel(&device->context(),
                        device->device_id(), non_ray_src, "update_bit_scene_main", main_opts, "update::update_main");
