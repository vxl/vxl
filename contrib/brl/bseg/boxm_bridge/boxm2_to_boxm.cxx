...
int main(int argc, char* * argv)
{
  ...
  vgl_vector_3d<double> ww(world.width() / block_nums.x(), world.height() / block_nums.y(),
                           world.depth() / block_nums.z() );
  ...
  while( iter != blocks.end() )
    ...
    boxm2_data_base * data_base = cache->get_data_base(scene2, iter->first,
                                                       boxm2_data_traits<BOXM2_NUM_OBS>::prefix() );
    boxm2_data<BOXM2_NUM_OBS> * num_obs = new boxm2_data<BOXM2_NUM_OBS>(
        data_base->data_buffer(), data_base->buffer_length(), data_base->block_id() );

    boxm2_data_base * mog3_data_base =
      cache->get_data_base(scene2, iter->first, boxm2_data_traits<BOXM2_MOG3_GREY>::prefix() );
    boxm2_data<BOXM2_MOG3_GREY> * mog3_data = new boxm2_data<BOXM2_MOG3_GREY>(
        mog3_data_base->data_buffer(), mog3_data_base->buffer_length(), mog3_data_base->block_id() );
    boxm2_data_base * alpha_data_base  =
      cache->get_data_base(scene2, iter->first, boxm2_data_traits<BOXM2_ALPHA>::prefix() );
    boxm2_data<BOXM2_ALPHA> * alpha_data = new boxm2_data<BOXM2_ALPHA>(
        alpha_data_base->data_buffer(), alpha_data_base->buffer_length(), alpha_data_base->block_id() );
    ...
    vgl_box_3d<double> block_bb(p, subdim.x() * dim.x(), subdim.y() * dim.y(),
                                subdim.z() * dim.z(), vgl_box_3d<double>::min_pos);
