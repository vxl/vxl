...
int main(int argc, char* * argv)
{
  ...
  if( scene.appearence_model() == BOXM_APM_MOG_GREY )
    ...
    while( !iter.end() )
      ...
      for( unsigned z = 0; z < dim; z++ )
        ...
        for( unsigned y = 0; y < dim; y++ )
          ...
          for( unsigned x = 0; x < dim; x++ )
            ...
            p[1] = y * cell_dim;
            p[2] = z * cell_dim;
            vgl_box_3d<double> cell_bb(p, cell_dim, cell_dim, cell_dim,
                                       vgl_box_3d<double>::min_pos);
            boct_tree_cell<short, boxm_sample<BOXM_APM_MOG_GREY> >* node = tree->locate_point_at_level(
                cell_bb.centroid(), 3);
            if( !node )
              {
      ...
      // allocate data array
      boxm2_data_traits<BOXM2_MOG3_GREY>::datatype* data_arr =
        new  boxm2_data_traits<BOXM2_MOG3_GREY>::datatype[data_size];
      boxm2_data_traits<BOXM2_NUM_OBS>::datatype* num_obs_arr =
        new  boxm2_data_traits<BOXM2_NUM_OBS>::datatype[data_size];
          ...
          for( unsigned x = 0; x < dim; x++ )
            ...
            if( !node )
              ...
              if( n1 != n2 )
                {
                vcl_cout << x << ',' << y << ',' << z << '\n'
                         << "ERROR! The converted tree is not right, should have " << n1 << " nodes instead of "
                         << n2 << vcl_endl;
                }
              }
            }
      ...
      boxm2_data<BOXM2_MOG3_GREY> * mog_data =
        new boxm2_data<BOXM2_MOG3_GREY>(b2, data_size * sizeof(boxm2_data_traits<BOXM2_MOG3_GREY>::datatype), block_id);
      char*                       b3 = reinterpret_cast<char *>(num_obs_arr);
      boxm2_data<BOXM2_NUM_OBS> * num_data =
        new boxm2_data<BOXM2_NUM_OBS>(b3, data_size * sizeof(boxm2_data_traits<BOXM2_NUM_OBS>::datatype), block_id);
