...
bool
brec_bayesian_propagation::horizontal_pass(brec_fg_pair_density& fgp, brec_fg_bg_pair_density& fgbgp,
                                           brec_bg_pair_density& bgp)
{
  ...
  for( unsigned i = 0; i < bg_map_.ni() - 1; i++ )
    ...
    for( unsigned j = 0; j < bg_map_.nj(); j++ )
      ...
      if( !helper_function(p_b_current, p_f_current, p_b_current1, p_f_current1, p_ff, p_fb, p_bf, p_bb,
                           bg_map_(i, j) ) )
        {
        continue;
        }
...
bool
brec_bayesian_propagation::directional_pass(brec_fg_pair_density& fgp, brec_fg_bg_pair_density& fgbgp,
                                            brec_bg_pair_density& bgp, int ii, int jj)
{
  // : for now make only one horizontal pass
    ...
    for( unsigned j = 0; j < bg_map_.nj(); j++ )
      ...
      if( i == 572 && j == 335 )
        ...
        vcl_cout << "--------- 572, 335 -----------\n"
                 ...
                 << " p_h_bb: " << p_h_bb << " p_h_bf: " << p_h_bf << " p_h_fb: " << p_h_fb << " p_h_ff: " << p_h_ff
                 << vcl_endl
...
bool
brec_bayesian_propagation::vertical_pass_using_prob_density_as_bgp(brec_fg_pair_density& fgp,
                                                                   brec_fg_bg_pair_density& fgbgp)
{
  // : for now make only one horizontal pass
  for( unsigned i = 0; i < bg_map_.ni(); i++ )
...
//  jj is either -1 or 0 or 1 so can run in all 8 directions
bool
brec_bayesian_propagation::directional_pass_using_prob_density_as_bgp(brec_fg_pair_density& fgp,
                                                                      brec_fg_bg_pair_density& fgbgp, int ii, int jj)
{
  // : for now make only one horizontal pass
  for( unsigned i = 0; i < bg_map_.ni(); i++ )
    {
...
bool
brec_bayesian_propagation::pass_using_prob_density_as_bgp(brec_fg_pair_density& fgp, brec_fg_bg_pair_density& fgbgp,
      ...
      for( int k = -1; k < 2; k++ )
        ...
        for( int m = -1; m < 2; m++ )
          ...
          helper_function2(p_b_current, p_f_current, p_b_current1, p_f_current1,
                           ...
                           values[count * 4 + 0].second, values[count * 4 + 1].second, values[count * 4 + 2].second,
                           values[count * 4 + 3].second);
