...
bool brec_bg_pair_density::generate_mixture_image()
{
  ...
  if( voxel_type_ == "apm_mog_rgb" )
    ...
    bvxm_voxel_slab<grey_mog_type>* mog_image_ptr =
      dynamic_cast<bvxm_voxel_slab<grey_mog_type> *>(mog_image_sptr.ptr() );
    ...
    {
    vcl_cout << "In brec_normalize_image_process::execute() -- input appearance model: " << voxel_type_
             << " is not supported\n";
    return false;
    }
...
vil_image_view<float>
brec_bg_pair_density::prob_density_from_likelihood_map(vil_image_view<vxl_byte>& img,
                                                       vil_image_view<float>& prob_density)
{
  unsigned ni = img.ni();
  unsigned nj = img.nj();
...
bool
brec_bg_pair_density::initialize_bg_map(vil_image_view<float>& prob_density, vil_image_view<float>& map,
                                        float high_prior, float top_percentile)
{
...
#if 1
  vcl_cout << "1-" << top_percentile << "= " << 1 - top_percentile << " percentile value of prob density is: " << val
           << vcl_endl;
