...
public:
  ...
  brec_bg_pair_density(unsigned ni, unsigned nj)
    : brec_pair_density(), verbose(false), world_(0), cam_(0), voxel_type_(""),
  ...
  static bool initialize_bg_map(vil_image_view<float>& change_map, vil_image_view<float>& map, float high_prior = 0.9f,
  ...
  virtual vil_image_view<float> prob_density_from_likelihood_map(vil_image_view<vxl_byte>& img,
