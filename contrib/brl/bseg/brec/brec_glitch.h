...
public:
  ...
  static void square_glitch(int square_size, vcl_vector<vcl_pair<int, int> >& neighborhood_center,
                            vcl_vector<vcl_pair<int, int> >& neighborhood_surround);

  static vil_image_view<vxl_byte> square_glitch_mask_img(int square_size);

  static void extend_prob_to_square_region(int square_size, vil_image_view<float>& input_map,
                                           vil_image_view<float>& output_map);
