...
bool extract_gaussian_primitives(vil_image_resource_sptr img, float lambda0, float lambda1, float theta, bool bright,
                                 float cutoff_percentage, float strength_threshold, unsigned type,
  ...
  for( unsigned j = 0; j < nj; ++j )
    ...
    for( unsigned i = 0; i < ni; ++i )
      ...
      if( strength_map(i, j) > strength_threshold )
        {
        brec_part_gaussian_sptr dp = new brec_part_gaussian( (float)i, (float)j, strength_map(i,
                                                                                              j), lambda0, lambda1, theta, bright,
                                                             type);
...
brec_part_gaussian::brec_part_gaussian(float x, float y, float strength, float lambda0, float lambda1, float theta,
                                       bool bright, unsigned type)
  : brec_part_instance(0, type, brec_part_instance_kind::GAUSSIAN, x, y, strength),
  lambda0_(lambda0), lambda1_(lambda1), theta_(theta), bright_(bright), cutoff_percentage_(0.01f), lambda_(0.0f), k_(
    0.0f), fitted_weibull_(false)
{
  initialize_mask();
}
...
brec_part_gaussian::brec_part_gaussian() : brec_part_instance(0, 0, brec_part_instance_kind::GAUSSIAN, 0.0f, 0.0f,
                                                              0.0f),
  lambda0_(0), lambda1_(0), theta_(0), bright_(true), cutoff_percentage_(0.0f), lambda_(0.0f), k_(0.0f),
...
bool brec_part_gaussian::construct_bg_response_model_gauss(vil_image_view<float>& mean_img,
                                                           vil_image_view<float>& std_dev_img,
                                                           vil_image_view<float> & mu_img,
                                                           vil_image_view<float> & sigma_img)
{
  unsigned ni = mean_img.ni();
  unsigned nj = mean_img.nj();
...
bool brec_part_gaussian::construct_class_response_models(vil_image_view<float>& img,
                                                         ...
                                                         double & lambda, double & k, double & lambda_non_class,
                                                         double & k_non_class)
{
  unsigned ni = img.ni();
  unsigned nj = img.nj();
...
bool brec_part_gaussian::update_response_hist(vil_image_view<float>& img, vil_image_view<float>& fg_prob_img,
                                              vil_image_view<bool>& mask_img,
                                 ...
                                 float prior_class)
{
  ...
  // find the response img for this operator
  vil_image_view<float> op_res = brip_vil_float_ops::extrema(img, lambda0_, lambda1_, theta_ + rot_angle, bright_,
                                                             false, false);
  ...
  if( fitted_weibull_ )
    {
    vcl_cout << "using fitted weibull parameters lambda: " << lambda_ << " k: " << k_ << " lambda_nc: "
             << lambda_non_class_ << " k_nc: " << k_non_class_ << " of the part for the foreground response model!\n";
  ...
  for( unsigned j = 0; j < nj; ++j )
    ...
    for( unsigned i = 0; i < ni; ++i )
...
#endif // DEBUG
      brec_part_gaussian_sptr dp = new brec_part_gaussian( (float)i, (float)j, res, lambda0_, lambda1_,
                                                           theta_ + rot_angle, bright_, type_);
      dp->rho_c_f_ = pos_c_f;
      ...
      if( (i == 375 && j == 204) || (i == 348 && j == 221) || (i == 374 && j == 193) ||
        ...
        {
        vcl_cout << " i == " << i << " && j == " << j << " \nk_fore: " << k_fore << " lam_fore: " << lambda_fore
                 << " mu_bk: " << mu_bk << " sigma_bk: " << sigma_bk << '\n'
                 << " res: " << res << " pb: " << pb << " pdb: " << pdb << " pdf: " << pdf << " den: " << den
                 << " neu: " << neu << " posterior: " << posterior << vcl_endl;
        }
#endif
      }
    }
...
bool brec_part_gaussian::extract(vil_image_view<float>& img, vil_image_view<float>& class_prob_image, float rot_angle,
                                 vcl_vector<brec_part_instance_sptr>& instances)
{
  unsigned ni = img.ni();
  unsigned nj = img.nj();
  ...
  if( !fitted_weibull_ )
    {
    vcl_cout
      << "ERROR: Foreground response model's parameters have not been set! Run parameter fitting routine first!\n";
    return false;
    }
                                                     ...
                                                     vil_image_view<float>& std_dev_img)
{
  ...
  if( !construct_bg_response_model_gauss(mean_img, std_dev_img, mu_img, sigma_img) )
    ...
    vcl_cout
      <<
    "In brec_part_gaussian::update_foreground_posterior() - problems in construction of background response model params!\n";
    return false;
    }
