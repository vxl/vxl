...
bool brec_part_hierarchy_detector::detect(vil_image_resource_sptr img)
{
  ...
  for( unsigned i = 0; i < dumm_ins.size(); i++ )
    ...
    if( dumm_ins[i]->kind_ == brec_part_instance_kind::GAUSSIAN )
      ...
      if( !extract_gaussian_primitives(img, p->lambda0_, p->lambda1_, p->theta_, p->bright_, p->cutoff_percentage_,
                                       p->detection_threshold_, p->type_, parts_prims) )
        {
  ...
  vcl_cout << "\textracted " << parts_upper_most.size() << " of highest layer: " << highest << " parts, rtree size: "
           << rtree_upper_most->size() << vcl_endl;
...
bool brec_part_hierarchy_detector::detect(vil_image_resource_sptr img, float angle)
{
  ...
  for( unsigned i = 0; i < dumm_ins.size(); i++ )
    ...
    if( dumm_ins[i]->kind_ == brec_part_instance_kind::GAUSSIAN )
      {
      brec_part_gaussian_sptr p = dumm_ins[i]->cast_to_gaussian();
      if( !extract_gaussian_primitives(img, p->lambda0_, p->lambda1_, p->theta_ + angle, p->bright_,
                                       p->cutoff_percentage_, p->detection_threshold_, p->type_, parts_prims) )
        {
        vcl_cout << "problems in extracting gaussian primitives!!\n";
        return false;
        }
...
bool brec_part_hierarchy_detector::detect_primitives_using_trained_response_models(vil_image_view<float>& img,
                                                                                   vil_image_view<float>& fg_prob_img,
                                                                                   float angle, float prior_class)
{
  // start from the primitives
  vcl_vector<brec_part_instance_sptr> dumm_ins = h_->get_dummy_primitive_instances();
  vcl_vector<brec_part_instance_sptr> parts_prims;
...
bool brec_part_hierarchy_detector::detect_primitives_for_training(vil_image_view<float>& inp,
                                                                  vil_image_view<float>& fg_prob_img, float angle)
{
  // start from the primitives
  vcl_vector<brec_part_instance_sptr> dumm_ins = h_->get_dummy_primitive_instances();
  vcl_vector<brec_part_instance_sptr> parts_prims;
...
bool brec_part_hierarchy_detector::detect(vil_image_view<float>& img, vil_image_view<float>& fg_prob_img, float angle,
                                          unsigned rho_calculation_method, double radius, float prior_class,
                                          unsigned layer_id)
{
  ...
  for( unsigned i = 0; i < extracted_parts.size(); i++ )
    ...
    for( brec_part_hierarchy::edge_iterator eit = hp->in_edges_begin(); eit != hp->in_edges_end(); eit++ )
      ...
      if( hp == (*eit)->source()->central_part() )
        ...
        brec_part_instance_sptr hp_upper_instance = exists(hp_upper, p, ni, nj, extracted_parts_rtree,
...
void brec_part_hierarchy_detector::extract_upper_layer(vcl_vector<brec_part_instance_sptr>& extracted_parts,
                                                       Rtree_type* extracted_parts_rtree,
                                                       vcl_vector<brec_part_instance_sptr>& extracted_upper_parts,
                                                       unsigned rho_calculation_method, double radius)
{
  // for each detected part, check for the existence of each upper layer part that uses it as a central part
  for( unsigned i = 0; i < extracted_parts.size(); i++ )
    {
    ...
    for( brec_part_hierarchy::edge_iterator eit = hp->in_edges_begin(); eit != hp->in_edges_end(); eit++ )
      ...
      if( (*eit)->to_central() && hp == (*eit)->source()->central_part() )
        ...
        switch( rho_calculation_method )
          {
          case brec_detector_methods::POSTERIOR_NUMERATOR: hp_upper_instance =
            exists(hp_upper, p, extracted_parts_rtree); break;
          case brec_detector_methods::DENSITY_FOR_TRAINING: hp_upper_instance = exists_for_training(hp_upper, p,
                                                                                                    extracted_parts_rtree);
            break;
          case brec_detector_methods::POSTERIOR: hp_upper_instance = exists_using_hierarchies(hp_upper, p,
                                                                                              extracted_parts_rtree,
                                                                                              radius); break;
          default: vcl_cout << "Warning: unknown brec_detector_method encountered: " << rho_calculation_method
                            << vcl_endl; break;
          }
