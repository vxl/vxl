...
public:
  ...
  static bool  meet(vgl_box_2d<T> const& b0, vgl_box_2d<T> const& b1)
  {
    ...
    vgl_point_2d<T> max_of_mins(b0min.x() > b1min.x() ? b0min.x() : b1min.x(),
                                b0min.y() > b1min.y() ? b0min.y() : b1min.y() );
    vgl_point_2d<T> min_of_maxs(b0min.x() < b1min.x() ? b0min.x() : b1min.x(),
                                b0min.y() < b1min.y() ? b0min.y() : b1min.y() );
...
public:
  ...
  brec_part_hierarchy_detector(brec_part_hierarchy_sptr h) : h_(h), prior_c_f_(0.15f), prior_non_c_f_(0.15f),
    prior_c_b_(0.15f) {}
  ...
  brec_part_instance_sptr exists(brec_part_base_sptr upper_p, brec_part_instance_sptr central_p, unsigned ni,
                                 unsigned nj, Rtree_type* lower_rtree, float det_threshold);

  // : check for existence of upper_p with central_p as its central part and map will tell if all the other parts exist
  //  No thresholding, \return a probabilistic score
  brec_part_instance_sptr exists(brec_part_base_sptr upper_p, brec_part_instance_sptr central_p,
                                 Rtree_type* lower_rtree);

  brec_part_instance_sptr exists_for_training(brec_part_base_sptr upper_p, brec_part_instance_sptr central_p,
                                              Rtree_type* lower_rtree);

  brec_part_instance_sptr exists_using_hierarchies(brec_part_base_sptr upper_p, brec_part_instance_sptr central_p,
  ...
  void extract_upper_layer(vcl_vector<brec_part_instance_sptr>& extracted_parts, unsigned ni, unsigned nj,
  ...
  void extract_upper_layer(vcl_vector<brec_part_instance_sptr>& extracted_parts, Rtree_type* extracted_parts_rtree,
                           vcl_vector<brec_part_instance_sptr>& extracted_upper_parts,
                           unsigned rho_calculation_method = 0, double radius = 10.0);
  ...
  bool detect_primitives_using_trained_response_models(vil_image_view<float>& img, vil_image_view<float>& fg_prob_img,
                                                       float angle, float prior_class);

  bool detect(vil_image_view<float>& img, vil_image_view<float>& fg_prob_img, float angle = 0.0f,
              unsigned rho_calculation_method = brec_detector_methods::POSTERIOR_NUMERATOR, double radius = 10.0,
              float prior_class = 0.1f, unsigned layer_id = 0);
