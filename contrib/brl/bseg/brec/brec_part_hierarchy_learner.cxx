  ...
  for( i11 = 0; i11 < left_array.rows(); i11++ )
    ...
    for( j11 = 0; j11 < left_array.cols(); j11++ )
      ...
      if( left_array.get_rows()[i11][j11] )
        ...
        i22 = i11 - i1 + i2;
        j22 = j11 - j1 + j2;
        if( i22 < 0 || i22 >= (int)right_array.rows() || j22 < 0 || j22 >= (int)right_array.cols() ||
            !right_array.get_rows()[i22][j22] )
          {
          return false;
          }
        }
  ...
  for( float lambda0 = lambda_inc; lambda0 <= lambda_range; lambda0 += lambda_inc )
    ...
    for( float lambda1 = lambda_inc; lambda1 <= lambda_range; lambda1 += lambda_inc )
      ...
      for( int i = 0; i < ndirs; i++ )
        ...
        brec_part_gaussian_sptr p = new brec_part_gaussian(0.0f, 0.0f, 0.0f, lambda0, lambda1, adjusted_theta, true,
                                                           type_cnt);
        ...
        brec_part_gaussian_sptr pd = new brec_part_gaussian(0.0f, 0.0f, 0.0f,
                                                            lambda0, lambda1,
                                                            adjusted_theta, false,
                                                            type_cnt);
...
bool brec_part_hierarchy_learner::initialize_layer_n_as_pairs(brec_part_hierarchy_sptr h, unsigned layer_id,
                                                              unsigned nclasses, float radius)
{
  if( !layer_id )
    {
    return false;
      ...
      if( (*it)->layer_ == layer_id - 1 )
        ...
        for( brec_part_hierarchy::vertex_iterator it2 = h->vertices_begin(); it2 != h->vertices_end(); it2++ )
          ...
          if( (*it2)->layer_ == layer_id - 1 )
            ...
            brec_part_instance_sptr p_n = new brec_part_instance(layer_id, type_cnt_, brec_part_instance_kind::COMPOSED,
                                                                 0.0f, 0.0f, 0.0f);
            type_cnt_++;
  ...
  for( layer_n_map::iterator it = stats_layer_n_.begin();
    ...
    for( m_it = map->begin(); m_it != map->end(); m_it++ )
      ...
      for( brec_part_hierarchy::edge_iterator eit = pa.first->out_edges_begin(); eit != pa.first->out_edges_end();
           eit++ )
        {
...
bool brec_part_hierarchy_learner::layer_n_collect_stats(brec_part_hierarchy_detector_sptr hd, unsigned layer_id,
                                                        unsigned class_id)
{
  if( !layer_id )
    {
...
bool brec_part_hierarchy_learner::layer_n_fit_distributions(unsigned class_id, unsigned layer_id, unsigned M)
{
  ...
  vcl_cout << "total weight of all the data from all the pairs for class: " << class_id << " is: " << total_weight
           << vcl_endl;
  for( qit = map->begin(); qit != map->end(); qit++ )
    {
    hist_ptr d_hist = (qit->second).second.first.first;
    ...
    vcl_cout << "In layer_n_fit_distributions(" << class_id << ") - type: " << layer_n_part->type_
             << ", # of d modes: " << d_set.mode_cnt() << vcl_endl;
    ...
    vcl_cout << "In layer_n_fit_distributions(" << class_id << ") - type: " << layer_n_part->type_
             << ", # of a modes: " << a_set.mode_cnt() << vcl_endl;
    ...
    if( ok )
      ...
      for( unsigned mi = 0; mi < d_out_dist.num_components(); mi++ )
        ...
        for( unsigned mj = 0; mj < a_out_dist.num_components(); mj++ )
          ...
          double ll = bsta_sample_set_log_likelihood(*set, d_out_dist.distribution(mi), d_out_dist.weight(
                                                       mi), a_out_dist.distribution(mj), a_out_dist.weight(mj), w_sum);
          ...
          if( stats_layer_n_.size() > 1 )
            ...
            for( layer_n_map::iterator class_it = stats_layer_n_.begin(); class_it != stats_layer_n_.end(); class_it++ )
              ...
              if( class_pair_it == (*class_it).second->end() )
                {
                vcl_cout << "Error: One of the classes was not initialized for the pair: " << qit->first.first << ' '
                         << qit->first.second << '\n';
                return false;
                }
              sample_set_ptr class_set = (class_pair_it->second).second.second;
              double         class_w_sum = 0.0;
              double         class_ll = bsta_sample_set_log_likelihood(*class_set, d_out_dist.distribution(
                                                                         mi), d_out_dist.weight(
                                                                         mi), a_out_dist.distribution(
                                                                         mj), a_out_dist.weight(mj), class_w_sum);
          ...
          e2->set_model(d_out_dist.distribution(mi), a_out_dist.distribution(mj), d_out_dist.weight(
                          mi) * a_out_dist.weight(mj) );
          ...
          if( class_h->layer_cnt(layer_id) < M )    // insert directly
            ...
            for( brec_part_hierarchy::vertex_iterator v_it = class_h->vertices_begin(); v_it != class_h->vertices_end();
                 v_it++ )
              {
            ...
            if( min < ratio )
              ...
              if( !class_h->remove_vertex(*v_min_it) )
                ...
                vcl_cout
                  <<
                  "ERROR: brec_part_hierarchy_learner::layer_n_fit_distributions() -- cannot delete vertex from hierarchy!\n";
                return false;
                }
              class_h->add_vertex(p_n);
              }
...
void brec_part_hierarchy_learner::print_layer0()
{
  for( unsigned i = 0; i < stats_layer0_.size(); i++ )
    ...
    if( pi->kind_ == brec_part_instance_kind::GAUSSIAN )
      ...
      if( p->bright_ )
        {
        vcl_cout << "--- lambda0 " << p->lambda0_ << " --- lambda1 " << p->lambda1_ << " --- theta " << p->theta_
                 << " --- bright ---\n";
        }
      else
        {
...
void brec_part_hierarchy_learner::print_to_m_file_layer_n(vcl_string file_name, unsigned class_id, bool print_set)
{
  ...
  for( m_it = map->begin(); m_it != map->end(); m_it++ )
    ...
    ofs << "h = figure;\n";
    ofs << "set(h, 'Name', 'Class: " << class_id << " pair: (" << id_p.first << ": " << cp_sid << ", " << id_p.second
        << ": " << sp_sid << ") ');\n";
...
void vsl_b_write(vsl_b_ostream & /*os*/, brec_part_hierarchy_learner const & /*ph*/)
{
  vcl_cerr
    <<
    "vsl_b_write() -- Binary io, NOT IMPLEMENTED, signatures defined to use brec_part_hierarchy_learner as a brdb_value\n";
  return;
}
...
void vsl_b_read(vsl_b_istream & /*is*/, brec_part_hierarchy_learner & /*ph*/)
{
  vcl_cerr
    <<
    "vsl_b_read() -- Binary io, NOT IMPLEMENTED, signatures defined to use brec_part_hierarchy_learner as a brdb_value\n";
  return;
}
