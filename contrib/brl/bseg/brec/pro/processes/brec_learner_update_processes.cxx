...
bool brec_learner_layer0_update_stats_process(bprb_func_process& pro)
{
  ...
  if( inp_prob->pixel_format() == VIL_PIXEL_FORMAT_BOOL )
    ...
    vcl_cout << " input prob map was a byte image, after conversion min value: " << min << " max value: " << max
             << vcl_endl;
    }
    ...
    vcl_cout
      <<
      "In brec_learner_update_stats_process::execute() -- ERROR: input prob image is neither BOOL img nor FLOAT img!!\n";
    return false;
    }
...
bool brec_learner_layer0_update_posterior_stats_process(bprb_func_process& pro)
{
  ...
  if( inp_prob->pixel_format() == VIL_PIXEL_FORMAT_BOOL )
    ...
    vcl_cout
      <<
      "In brec_learner_update_posterior_stats_process::execute() -- ERROR: input prob image is neither BOOL img nor FLOAT img!!\n";
    return false;
    }
  ...
  if( !inp_mask || inp_mask->ni() != img.ni() )    // if mask is passed as zero make a bool img with all pixels true, i.e. use the whole input img
    ...
    if( inp_mask->pixel_format() != VIL_PIXEL_FORMAT_BOOL )
      ...
      vcl_cout
        <<
      "In brec_learner_update_posterior_stats_process::execute() -- ERROR: input mask image is not of type BOOL!!\n";
      return false;
      }
    mask_img = inp_mask;
    }
