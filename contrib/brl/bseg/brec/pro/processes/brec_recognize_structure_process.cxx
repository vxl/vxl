...
bool brec_recognize_structure_process(bprb_func_process& pro)
{
  ...
  for( unsigned i = 0; i < d_ins.size(); i++ )
    ...
    if( !extract_gaussian_primitives(img, gp->lambda0_, gp->lambda1_, gp->theta_, gp->bright_, gp->cutoff_percentage_,
                                     gp->detection_threshold_, gp->type_, parts_0) )
      {
      return false;
      }

    vcl_cout << "extracted " << parts_0.size() - prev_size << " primitive parts of type: " << d_ins[i]->type_
             << vcl_endl;
  ...
  brec_part_hierarchy::generate_output_img(parts_upper_most, orig_img, output_img,
...
bool brec_recognize_structure2_process(bprb_func_process& pro)
{
  ...
  vcl_cout
    <<
  "CAUTION: Input prob map is assumed to be a \"foreground\" probability map, convert it before passing to this process if necessary! (e.g. if only the background map is available)\n";
