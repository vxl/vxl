...
static void test_brec_hierarchy_detector2_simple()
{
  ...
  h1->add_dummy_primitive_instance(pi_0_0->cast_to_instance() ); h2->add_dummy_primitive_instance(
    pi_0_0->cast_to_instance() );
  h1->add_dummy_primitive_instance(pi_0_1->cast_to_instance() ); h2->add_dummy_primitive_instance(
    pi_0_1->cast_to_instance() );
  ...
  for( unsigned i = 0; i < upper_parts.size(); i++ )
    {
    vcl_cout << "detected a part at center: (" << upper_parts[i]->x_ << ", " << upper_parts[i]->y_ << ") type: "
             << upper_parts[i]->type_ << " posterior: " << upper_parts[i]->rho_c_b_ << vcl_endl;
    }
}
...
static void test_brec_hierarchy_detector2_img()
{
  ...
  vcl_string h_name =
    "C:\\projects\\roi_1\\sewage_and_vehicles_learning\\output_learning\\hierarchy_0_layer_2.xml";
