                                      ...
                                      vil_image_view<T>& mi_img )
{
  ...
  while (finder.next_4con_region(region))
  {
    ...
    if(MI != MI) {
      vcl_cout<<"mutual info is NAN "<<vcl_endl;
      vcl_cout<<"  num samps = "<<x_samps.size()<<vcl_endl;
    }
    if( vcl_fabs(MI-.6702) < .0001 ) {
      vcl_cout<<"MID BLOB samps: "<<vcl_endl;
      vcl_cout<<"  num samps = "<<x_samps.size()<<vcl_endl;
    }
    if(x_samps.size() == 0) {
      vcl_cout<<"ZERO SAMPLE SET: "<<vcl_endl;
      vcl_cout<<"   MI = "<<MI<<vcl_endl;
      vcl_cout<<"   H1 = "<<H1<<vcl_endl;
      vcl_cout<<"   H2 = "<<H2<<vcl_endl;
      vcl_cout<<"   Hxy= "<<H3<<vcl_endl;
    }
                                 ...
                                 vil_image_view<T>& mi_img )
{
  ...
  while (finder.next_4con_region(region))
  {
    ...
    if(startI > 422 && startI < 460 && startJ>280 && startJ<310) {
      vcl_cout<<"BLOB with size "<<x_samps.size()<<vcl_endl;
      vcl_cout<<"  histX (inimg) = "; 
      for(int i=0; i<x_hist.size(); ++i) vcl_cout<<x_hist[i]/magX<<" "; 
      vcl_cout<<vcl_endl;
      vcl_cout<<"  histY (expimg)= "; 
      for(int i=0; i<y_hist.size(); ++i) vcl_cout<<y_hist[i]/magY<<" "; 
      vcl_cout<<vcl_endl;
      vcl_cout<<"  KL div = "<<KL<<vcl_endl;
    }
#endif
