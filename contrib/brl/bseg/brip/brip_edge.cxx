// This is brl/bseg/brip/brip_edge.cxx
...
vil_image_view<float> brip_edge::multiply_image_with_gaussian_kernel(vil_image_view<float> img, double gaussian_sigma)
{
...
vbl_array_2d<float> brip_edge::get_spherical_gaussian_kernel(const int size, const float sigma)
{
...
void brip_edge::estimate_edge_prob_image(const vil_image_view<vxl_byte>& img_edge, vil_image_view<float>& img_edgeness, const int mask_size, const float mask_sigma)
{
...
float brip_edge::convert_edge_statistics_to_probability(float edge_statistic, float n_normal, int dof)
{
  if (dof<1) {
