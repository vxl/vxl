...
{
public:
  brip_G_Lhalf_kernel(double sigma_, double dx_ = 0.0, double dy_ = 0.0, double theta_ = 0.0) : brip_gaussian_kernel(
      sigma_, dx_, dy_, theta_) {}
  ~brip_G_Lhalf_kernel() {}
    ...
    for( int x = -khs; x <= khs; x++ )
      ...
      for( int y = -khs; y <= khs; y++ )
        ...
        if( yy >= 0 )
          ...
          top_left_[(x + khs) * istep_ + (y + khs) * jstep_] = vcl_exp(-xx * xx / (2 * ssq) ) * yy * -vcl_exp(
              -yy * yy / (2 * ssq) ) / cc;
          }
        }
      }
...
{
public:
  brip_G_Rhalf_kernel(double sigma_, double dx_ = 0.0, double dy_ = 0.0, double theta_ = 0.0) : brip_gaussian_kernel(
      sigma_, dx_, dy_, theta_) {}
  ~brip_G_Rhalf_kernel() {}
...
public:
  ...
  virtual void compute_kernel(bool separated_kernels_only = false)
  {
    ...
    for( int x = -khs; x <= khs; x++ )
      {
      K_x[x + khs] = (x - dx) * (3 * ssq - (x - dx) * (x - dx) ) * vcl_exp(-(x - dx) * (x - dx) / (2 * ssq) )
        / (sq2pisig * ssq * ssq * ssq);
      }
    for( int y = -khs; y <= khs; y++ )
      {
      K_y[y + khs] = vcl_exp(-(y - dy) * (y - dy) / (2 * ssq) ) / sq2pisig;
...
public:
  ...
  virtual void compute_kernel(bool separated_kernels_only = false)
  {
    ...
    for( int y = -khs; y <= khs; y++ )
      {
      K_y[y + khs] = (y - dy) * (3 * ssq - (y - dy) * (y - dy) ) * vcl_exp(-(y - dy) * (y - dy) / (2 * ssq) )
        / (sq2pisig * ssq * ssq * ssq);
      }
