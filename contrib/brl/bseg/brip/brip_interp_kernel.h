...
public:
  ...
  virtual void compute_kernel()
  {
    ...
    for( int x = -khs; x <= khs; x++ )
      ...
      dG_x[x
           + khs] =
        (vcl_exp(-(x + 0.5 - dx) * (x + 0.5 - dx) / (2 * ssq) )
         - vcl_exp(-(x - 0.5 - dx) * (x - 0.5 - dx) / (2 * ssq) ) ) / sq2pisig;
      }
    for( int y = -khs; y <= khs; y++ )
      {
      G_y[y + khs] = (vnl_erf( (y + 0.5 - dy) / c) - vnl_erf( (y - 0.5 - dy) / c) ) / 2.0;
...
public:
  ...
  virtual void compute_kernel()
  {
    ...
    for( int y = -khs; y <= khs; y++ )
      ...
      dG_y[y
           + khs] =
        (vcl_exp(-(y + 0.5 - dy) * (y + 0.5 - dy) / (2 * ssq) )
         - vcl_exp(-(y - 0.5 - dy) * (y - 0.5 - dy) / (2 * ssq) ) ) / sq2pisig;
      }
    for( unsigned i = 0; i < ni_; i++ )
      {
      for( unsigned j = 0; j < nj_; j++ )
...
public:
  ...
  virtual void compute_kernel()
  {
    ...
    for( int x = -khs; x <= khs; x++ )
      ...
      d2G_x[x
            + khs] =
        (-(x + 0.5 - dx) * vcl_exp(-(x + 0.5 - dx) * (x + 0.5 - dx) / (2 * ssq) ) + (x - 0.5 - dx)
         * vcl_exp(-(x - 0.5 - dx) * (x - 0.5 - dx) / (2 * ssq) ) ) / (sq2pisig * ssq);
      }
    for( int y = -khs; y <= khs; y++ )
      {
      G_y[y + khs] = (vnl_erf( (y + 0.5 - dy) / c) - vnl_erf( (y - 0.5 - dy) / c) ) / 2.0;
...
public:
  ...
  virtual void compute_kernel()
  {
    ...
    for( int y = -khs; y <= khs; y++ )
      ...
      d2G_y[y
            + khs] =
        (-(y + 0.5 - dy) * vcl_exp(-(y + 0.5 - dy) * (y + 0.5 - dy) / (2 * ssq) ) + (y - 0.5 - dy)
         * vcl_exp(-(y - 0.5 - dy) * (y - 0.5 - dy) / (2 * ssq) ) ) / (sq2pisig * ssq);
      }
    for( unsigned i = 0; i < ni_; i++ )
      {
      for( unsigned j = 0; j < nj_; j++ )
...
public:
  ...
  virtual void compute_kernel()
  {
    ...
    for( int x = -khs; x <= khs; x++ )
      ...
      d3G_x[x
            + khs] =
        ( ( (x + 0.5 - dx) * (x + 0.5 - dx) - ssq) * vcl_exp(-(x + 0.5 - dx) * (x + 0.5 - dx) / (2 * ssq) )
          - ( (x - 0.5 - dx) * (x - 0.5 - dx) - ssq) * vcl_exp(-(x - 0.5 - dx) * (x - 0.5 - dx) / (2 * ssq) ) )
        / (sq2pisig * ssq * ssq);
      }
    for( int y = -khs; y <= khs; y++ )
...
public:
  ...
  virtual void compute_kernel()
  {
    ...
    for( int y = -khs; y <= khs; y++ )
      ...
      d3G_y[y
            + khs] =
        ( ( (y + 0.5 - dy) * (y + 0.5 - dy) - ssq) * vcl_exp(-(y + 0.5 - dy) * (y + 0.5 - dy) / (2 * ssq) )
          - ( (y - 0.5 - dy) * (y - 0.5 - dy) - ssq) * vcl_exp(-(y - 0.5 - dy) * (y - 0.5 - dy) / (2 * ssq) ) )
        / (sq2pisig * ssq * ssq);
      }
    for( unsigned i = 0; i < ni_; i++ )
...
public:
  ...
  virtual void compute_kernel()
  {
    ...
    for( int x = -khs; x <= khs; x++ )
      ...
      G_x[x
          + khs] =
        ( (x - 1 - dx) * vnl_erf( (x - 1 - dx) / c) - 2 * (x - dx) * vnl_erf( (x - dx) / c) + (x + 1 - dx)
          * vnl_erf( (x + 1 - dx) / c) ) / 2.0
        + sigma
        * (vcl_exp(-(x - 1 - dx) * (x - 1 - dx) / (2 * ssq) ) - 2 * vcl_exp(-(x - dx) * (x - dx) / (2 * ssq) )
           + vcl_exp(-(x + 1 - dx) * (x + 1 - dx) / (2 * ssq) ) ) / sq2pi;
      }
    for( int y = -khs; y <= khs; y++ )
      {
      G_y[y
          + khs] =
        ( (y - 1 - dy) * vnl_erf( (y - 1 - dy) / c) - 2 * (y - dy) * vnl_erf( (y - dy) / c) + (y + 1 - dy)
          * vnl_erf( (y + 1 - dy) / c) ) / 2.0
        + sigma
        * (vcl_exp(-(y - 1 - dy) * (y - 1 - dy) / (2 * ssq) ) - 2 * vcl_exp(-(y - dy) * (y - dy) / (2 * ssq) )
           + vcl_exp(-(y + 1 - dy) * (y + 1 - dy) / (2 * ssq) ) ) / sq2pi;
      }
    for( unsigned i = 0; i < ni_; i++ )
      {
      for( unsigned j = 0; j < nj_; j++ )
  ...
  virtual void compute_kernel()
  {
    ...
    for( int x = -khs; x <= khs; x++ )
      ...
      d2G_x[x
            + khs] =
        (vcl_exp(-(x - 1 - dx) * (x - 1 - dx) / (2 * ssq) ) - 2 * vcl_exp(-(x - dx) * (x - dx) / (2 * ssq) )
         + vcl_exp(-(x + 1 - dx) * (x + 1 - dx) / (2 * ssq) ) ) / (sq2pi * sigma);
      }
    for( int y = -khs; y <= khs; y++ )
      {
      G_y[y
  ...
  virtual void compute_kernel()
  {
    ...
    for( int y = -khs; y <= khs; y++ )
      ...
      d2G_y[y
            + khs] =
        (vcl_exp(-(y - 1 - dy) * (y - 1 - dy) / (2 * ssq) ) - 2 * vcl_exp(-(y - dy) * (y - dy) / (2 * ssq) )
         + vcl_exp(-(y + 1 - dy) * (y + 1 - dy) / (2 * ssq) ) ) / (sq2pi * sigma);
      }
    for( unsigned i = 0; i < ni_; i++ )
      {
      for( unsigned j = 0; j < nj_; j++ )
...
public:
  ...
  virtual void compute_kernel()
  {
    ...
    for( int x = -khs; x <= khs; x++ )
      ...
      d3G_x[x
            + khs] =
        (-(x - 1 - dx) * vcl_exp(-(x - 1 - dx) * (x - 1 - dx) / (2 * ssq) ) + 2 * (x - dx)
         * vcl_exp(-(x - dx) * (x - dx) / (2 * ssq) ) - (x + 1 - dx)
         * vcl_exp(-(x + 1 - dx) * (x + 1 - dx) / (2 * ssq) ) ) / (sq2pi * sigma * ssq);
      }
    for( int y = -khs; y <= khs; y++ )
      {
      G_y[y
...
public:
  ...
  virtual void compute_kernel()
  {
    ...
    for( int y = -khs; y <= khs; y++ )
      ...
      d3G_y[y
            + khs] =
        (-(y - 1 - dy) * vcl_exp(-(y - 1 - dy) * (y - 1 - dy) / (2 * ssq) ) + 2 * (y - dy)
         * vcl_exp(-(y - dy) * (y - dy) / (2 * ssq) ) - (y + 1 - dy)
         * vcl_exp(-(y + 1 - dy) * (y + 1 - dy) / (2 * ssq) ) ) / (sq2pi * sigma * ssq);
      }
    for( unsigned i = 0; i < ni_; i++ )
      {
      for( unsigned j = 0; j < nj_; j++ )
