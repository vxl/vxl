...
void brip_para_cvrg::init(vil1_image const & image)
{
  ...
  vcl_cout << "xsize = " << xsize_ << " ysize_ = " << ysize_ << '\n';
...
void brip_para_cvrg::set_float_image(float **image, float val)
{
  ...
  for(x=1; x < image_.width();x++)
    {
      ptr = image[x];
      memcpy((char*)ptr, (char*)image[x-1], image_.height()*sizeof(float));
    }
}
...
{
  memcpy((char*)image2[0], (char*)image1[0],
...
// derivative in that direction.  Private.
// 
...
//----------------------------------------------------------
// -- Find the direction with maxium parallel coverage.  Return the 
//    normalized coverage value
