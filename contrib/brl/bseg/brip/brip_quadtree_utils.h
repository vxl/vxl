...
public:
  ...
  //  that of the parent.
  static void extract_nodes_from_image(vil_image_view<T> const & img, vil_image_view<bool> const & mask,
                                       vil_image_view<T> const& parent_img, vil_image_view<bool> const& parent_mask,
                                       unsigned scale, vbl_array_2d<brip_quadtree_node_base_sptr>& nodes);

// : given two layers of a pyramid of quadtree nodes, establish the parent-child links
  static void connect_children(vbl_array_2d<brip_quadtree_node_base_sptr>& nodes, unsigned scale,
...
// : Construct the quadtree from an image pyramid, with masks defined for each pyramid layer
  static
  void quadtrees_from_pyramid(vcl_vector<vil_image_view<T> > levels, vcl_vector<vil_image_view<bool> > masks,
