...
public:
  ...
  static vil1_memory_image_of<float> convolve(vil1_memory_image_of<float> const & input,
  ...
  static void non_maximum_suppression(vil1_memory_image_of<float> const & input, const int n, const float thresh,
                                      vcl_vector<float>& x_pos, vcl_vector<float>& y_pos, vcl_vector<float>& value);

  // : downsamples the input using the Bert-Adelson algorithm
  static vil1_memory_image_of<float> half_resolution(vil1_memory_image_of<float> const & input,
                                                     float filter_coef = 0.359375);

  // : downsamples the color input image using the Bert-Adelson algorithm
  static vil1_memory_image_of<vil1_rgb<unsigned char> > half_resolution(
    vil1_memory_image_of<vil1_rgb<unsigned char> > const & input, float filter_coef = 0.359375);

#if 0
  // : interpolates the input using the Bert-Adelson algorithm
  static vil1_memory_image_of<float> double_resolution(vil1_memory_image_of<float> const & input,
                                                       float filter_coef = 0.359375);

#endif

  // : subtracts image_1 from image_2
  static vil1_memory_image_of<float> difference(vil1_memory_image_of<float> const & image_1,
                                                vil1_memory_image_of<float> const & image_2);

  // : sets absolute values greater than thresh to specified level
  static vil1_memory_image_of<float> abs_clip_to_level(vil1_memory_image_of<float> const & image, const float thresh,
                                                       const float level = 0.0);

  // : The gradient using a 3x3 kernel
  static void gradient_3x3(vil1_memory_image_of<float> const & input, vil1_memory_image_of<float>& grad_x,
                           vil1_memory_image_of<float>& grad_y);

  static void hessian_3x3(vil1_memory_image_of<float> const & input, vil1_memory_image_of<float>& Ixx,
                          vil1_memory_image_of<float>& Ixy, vil1_memory_image_of<float>& Iyy);

  static vil1_memory_image_of<float> beaudet(vil1_memory_image_of<float> const & Ixx,
                                             vil1_memory_image_of<float> const & Ixy,
                                             vil1_memory_image_of<float> const & Iyy);

  // : IxIx.transpose gradient matrix elements (N = 2n+1)
  static void grad_matrix_NxN(vil1_memory_image_of<float> const & input, const int n, vil1_memory_image_of<float>& IxIx,
                              vil1_memory_image_of<float>& IxIy, vil1_memory_image_of<float>& IyIy);

  // : Computes the Harris corner measure
  static vil1_memory_image_of<float> harris(vil1_memory_image_of<float> const & IxIx,
                                            vil1_memory_image_of<float> const & IxIy,
                                            vil1_memory_image_of<float> const & IyIy, double scale = 0.04);
  ...
  static void Lucas_KanadeMotion(vil1_memory_image_of<float> & current_frame,
                                 vil1_memory_image_of<float> & previous_frame, int n, double thresh,
                                 vil1_memory_image_of<float>& vx, vil1_memory_image_of<float>& vy);
  ...
  static vil1_memory_image_of<unsigned short> convert_to_short(vil1_memory_image_of<float> const & image,
                                                               const float min_val, const float max_val);
  ...
  static void convert_to_IHS(vil1_memory_image_of<vil1_rgb<unsigned char> > const& image,
                             vil1_memory_image_of<float>& I, vil1_memory_image_of<float>& H,
                             vil1_memory_image_of<float>& S);

  // : display IHS images as RGB (not conversion from IHS to RGB)
  static void display_IHS_as_RGB(vil1_memory_image_of<float> const& I, vil1_memory_image_of<float> const& H,
  ...
  static bool fourier_transform(vil1_memory_image_of<float> const & input, vil1_memory_image_of<float>& mag,
                                vil1_memory_image_of<float>& phase);

  // : compute the inverse Fourier transform using the vnl FFT algorithm
  static
  bool inverse_fourier_transform(vil1_memory_image_of<float> const& mag, vil1_memory_image_of<float> const& phase,
                                 vil1_memory_image_of<float>& output);

  // : resize to specified dimensions, fill with zeros if output is larger
  static
  void resize(vil1_memory_image_of<float> const & input, const int width, const int height,
              vil1_memory_image_of<float>& output);

  // : resize to closest power of two larger dimensions than the input
  static
  bool resize_to_power_of_two(vil1_memory_image_of<float> const & input, vil1_memory_image_of<float>& output);

  // : filter the input image with a Gaussian blocking filter
  static bool spatial_frequency_filter(vil1_memory_image_of<float> const & input, const float dir_fx,
                                       const float dir_fy, const float f0, const float radius,
                                       const bool output_fourier_mag, vil1_memory_image_of<float> & output);
  ...
  static bool homography(vil1_memory_image_of<float> const & input, vgl_h_matrix_2d<double> const& H,
                         vil1_memory_image_of<float>& output, bool output_size_fixed = false,
                         float output_fill_value = 0.0);

  // : rotate the input image counter-clockwise about the image origin
  static
  vil1_memory_image_of<float> rotate(vil1_memory_image_of<float> const & input, const double theta_deg);

  static bool chip(vil1_memory_image_of<float> const & input, vsol_box_2d_sptr const& roi,
  ...
  static float cross_correlate(vil1_memory_image_of<float> const & image1, vil1_memory_image_of<float> const & image2,
                               const float x, const float y, const int radius = 5, const float intensity_thresh = 25.0);

  // : Cross-correlate two images using faster running sums
  static bool cross_correlate(vil1_memory_image_of<float> const & image1, vil1_memory_image_of<float> const & image2,
                              vil1_memory_image_of<float>& out, const int radius = 5,
...
private:
  ...
  static void half_resolution_1d(const float* input, int n, const float k0, const float k1, const float k2,
                                 float* output);
  ...
  static
  void ftt_fourier_2d_reorder(vnl_matrix<vcl_complex<double> > const& F1, vnl_matrix<vcl_complex<double> > & F2);

  // : Blocking filter function
  static float gaussian_blocking_filter(const float dir_fx, const float dir_fy, const float f0, const float radius,
