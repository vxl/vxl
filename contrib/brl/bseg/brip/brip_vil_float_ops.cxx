                                             ...
                                             const double fuzz)
{
  ...
  while( (float)brip_vil_gaussian( (double)radius, sigma) > fuzz )
    {
    ++radius;
    }
...
brip_vil_float_ops::absolute_value(vil_image_view<float> const& input)
{
  ...
  for( unsigned p = 0; p < np; ++p )
    ...
    for( unsigned j = 0; j < nj; ++j )
      ...
      for( unsigned i = 0; i < ni; ++i )
        {
        dest(i, j, p) = vcl_fabs(input(i, j, p) );
        }
      }
    }
  return dest;
...
//
bool brip_vil_float_ops::local_maximum(vbl_array_2d<float> const& neighborhood, int n, float& value)
{
  bool local_max = true;
...
//
void brip_vil_float_ops::interpolate_center(vbl_array_2d<float> const& neighborhood, float& dx, float& dy)
{
  dx = 0; dy = 0;
...
vil_image_view<float> brip_vil_float_ops::trace_grad_matrix_NxN(vil_image_view<float> const& input, unsigned n)
{
  unsigned ni = input.ni(), nj = input.nj();
                                                                ...
                                                                float min_scale, float max_scale, float scale_inc)
{
  ...
  vil_save_image_resource(vil_new_image_resource_of_view(
                            tr_cast_byte), "D:\\projects\\vehicle_rec_on_models\\trace_image.png");
  ...
  vil_save_image_resource(vil_new_image_resource_of_view(
                            mod_cast), "D:\\projects\\vehicle_rec_on_models\\modified_image.png");
  ...
  vcl_cout << "in difference image of normalized trace images min value: " << min_bd << " max value: " << max_bd
           << vcl_endl;
  vil_image_view<vxl_byte> dif_cast;
  vil_convert_cast(difference_image, dif_cast);
  vil_save_image_resource(vil_new_image_resource_of_view(
                            dif_cast), "D:\\projects\\vehicle_rec_on_models\\difference_image.png");
  ...
  vil_save_image_resource(vil_new_image_resource_of_view(
                            tr_cast2_byte), "D:\\projects\\vehicle_rec_on_models\\trace_image2.png");
                                           ...
                                           const int no_of_iterations)
{
  ...
  if( (vil_image_view_deep_equality(emptyimg,
                                    current_frame ) ) || (vil_image_view_deep_equality(emptyimg, previous_frame) ) )
    {
...
brip_vil_float_ops::convert_to_byte(vil_image_view<float> const& image)
{
  ...
  for( unsigned y = 0; y < h; y++ )
    ...
    for( unsigned x = 0; x < w; x++ )
      ...
      if( v > 255.0f )
        {
        v = 255.0f;           // could have small round off above 255
        }
...
// \endverbatim
//
double brip_vil_float_ops::bilinear_interpolation(vil_image_view<float> const& input, double x, double y)
{
  // check bounds
  int w = static_cast<int>(input.ni() ), h = static_cast<int>(input.nj() );
                               ...
                               const bool ihs)
{
  if( !intensity && !gradient && !ihs )
    {
    vcl_cout << "In brip_vil_float_ops::minforopy(.) - No computation to do\n";
    return false;
    }
  ...
  if( ni0 < ilimit || nj0 < jlimit || ni1 < ilimit || nj1 < jlimit )
    {
    vcl_cout << "In brip_vil_float_ops::minfo(...) - image too small\n";
    return false;
    }
...
float brip_vil_float_ops::average_in_box(vil_image_view<float> const& v, vgl_box_2d<double> const&  box)
{
                                                  ...
                                                  float& max)
{
  ...
  vcl_cerr << "In brip_vil_float_ops::scan_region() - unknown format\n";
  return pixels;
}
...
// \endverbatim
//
vil_image_view<vxl_byte> brip_vil_float_ops::color_order(vil_image_view<float> const& color_image, float eq_tol)
{
...
void brip_vil_float_ops::gaussian_kernel_mask(float lambda, vbl_array_2d<float>& kernel, vbl_array_2d<bool>& mask,
                                              float cutoff_percentage)
{
  double s1 = lambda;
  double s1sq = 1.0 / (s1 * s1);
  // determine the size of the array
...
void brip_vil_float_ops::gaussian_kernel_square_mask(float lambda, vbl_array_2d<float>& kernel,
...
vil_image_view<float> brip_vil_float_ops::extrema_rotational(vil_image_view<float> const& input, float lambda0,
                                                             ...
                                                             float cutoff_per)
{
  ...
  if( lambda0 < lambda1 )
    ...
    vcl_cout
      <<
    "In brip_vil_float_ops::extrema_rotational() - ERROR! rotational extrema operator requires lambda0 to be larger than lambda1! switch the lambdas and use the output angle map accordingly!\n";
    throw 0;
    }
  ...
  for( float theta = 0.0f; theta < 180.0f; theta += theta_interval )
    {
  ...
  for( ; theta < 180; theta += theta_interval, theta_i++ )
    ...
    for( unsigned j = rj; j < (nj - rj); j++ )
      ...
      for( unsigned i = ri; i < (ni - ri); i++ )
        ...
        if( vcl_fabs(res_img(i, j) ) < vcl_fabs(res) )
          {
          res_img(i, j) = res;
...
// : theta and phi are in radians
void brip_vil_float_ops::max_inscribed_rect(float lambda0, float lambda1, float theta,
...
vil_image_view<float> brip_vil_float_ops::fast_extrema(vil_image_view<float> const& input, float lambda0, float lambda1,
                                                       float theta, bool bright, bool mag_only,
                                                       bool output_response_mask,
                                                       bool signed_response, bool scale_invariant,
