...
public:
  ...
  // in   ... -------------------...edcbabcde...
  // out  ... ---------------------------
  static vil_image_view<float> gaussian(vil_image_view<float> const& input, float sigma,
                                        vcl_string const& boundary_condition = "none", float fill = 0.0f);
  ...
  static vil_image_view<float> gaussian(vil_image_view<float> const& input, float sigma, vil_image_view<float> mask);

#endif

  // : non-maximum suppression on a NxN neighborhood, with sub-pixel location
  static void non_maximum_suppression(vil_image_view<float> const& input, int n, float thresh, vcl_vector<float>& x_pos,
                                      vcl_vector<float>& y_pos, vcl_vector<float>& value);
  ...
  static vil_image_view<float> threshold(vil_image_view<float> const & image, const float thresh,
                                         const float level = 255.0f);

  template <class T_from, class T_to>
  static void normalize_to_interval(const vil_image_view<T_from>& img_inp, vil_image_view<T_to>& img_out, float min,
                                    float max);

  // : sets absolute values greater than \p thresh to specified \p level
  static vil_image_view<float> abs_clip_to_level(vil_image_view<float> const & image, const float thresh,
  ...
  static void gradient_3x3(vil_image_view<float> const& input, vil_image_view<float>& grad_x,
  ...
  static void gradient_mag_comp_3x3(vil_image_view<float> const& input, vil_image_view<float>& mag,
                                    vil_image_view<float>& grad_x, vil_image_view<float>& grad_y);
  ...
  // \endverbatim
  // Larger masks are computed by pre-convolving with a Gaussian
  static void hessian_3x3(vil_image_view<float> const& input, vil_image_view<float>& Ixx, vil_image_view<float>& Ixy,
                          vil_image_view<float>& Iyy);

  static vil_image_view<float> beaudet(vil_image_view<float> const& Ixx, vil_image_view<float> const& Ixy,
                                       vil_image_view<float> const& Iyy, bool determinant = true);
  ...
  //  of the derivative
  static void extrema_kernel_mask(float lambda0, float lambda1, float theta, vbl_array_2d<float>& kernel,
  ...
  static void gaussian_kernel_mask(float lambda, vbl_array_2d<float>& kernel, vbl_array_2d<bool>& mask,
  ...
  static void gaussian_kernel_square_mask(float lambda, vbl_array_2d<float>& kernel, vbl_array_2d<bool>& mask,
                                          float cutoff_percentage = 0.01f);

  // : Compute the standard deviation of an operator response, given the image intensity standard deviation at each pixel
  static  vil_image_view<float> std_dev_operator(vil_image_view<float> const& sd_image,
                                                 vbl_array_2d<float> const& kernel);

  // : Compute the standard deviation of an operator response, given the image intensity standard deviation at each pixel
  //  Uses a modified formula to compute std_dev
  static vil_image_view<float> std_dev_operator_method2(vil_image_view<float> const& sd_image,
                                                        vbl_array_2d<float> const& kernel);
  ...
  static vil_image_view<float> extrema(vil_image_view<float> const& input, float lambda0, float lambda1, float theta,
                                       bool bright = true, bool mag_only = false, bool output_response_mask = true,
                                       bool signed_response = false, bool scale_invariant = false,
                                       bool non_max_suppress = true, float cutoff_per = 0.01f);
  ...
  static vil_image_view<float> extrema_rotational(vil_image_view<float> const& input, float lambda0, float lambda1,
                                                  float theta_interval, bool bright = true, bool mag_only = false,
                                                  bool signed_response = false, bool scale_invariant = false,
                                                  bool non_max_suppress = true, float cutoff_per = 0.01f);
  ...
  static vil_image_view<float> fast_extrema(vil_image_view<float> const& input, float lambda0, float lambda1,
                                            float theta, bool bright = true, bool mag_only = false,
                                            bool output_response_mask = true, bool signed_response = false,
                                            bool scale_invariant = false, bool non_max_suppress = true,
                                            float cutoff = 0.01f);
  ...
  static void grad_matrix_NxN(vil_image_view<float> const& input, unsigned n, vil_image_view<float>& IxIx,
                              vil_image_view<float>& IxIy, vil_image_view<float>& IyIy);
  ...
  static vil_image_view<float> harris(vil_image_view<float> const& IxIx, vil_image_view<float> const& IxIy,
                                      vil_image_view<float> const& IyIy, double scale = 0.04);
  ...
  static vil_image_view<float> max_scale_trace(vil_image_view<float> input, float min_scale, float max_scale,
                                               float scale_inc);
  ...
  static vil_image_view<float> max_scale_trace_value(vil_image_view<float> input, float min_scale, float max_scale,
                                                     float scale_inc);

  // : computes Lucas-Kanade optical flow on a $2n+1$ neighborhood
  static void Lucas_KanadeMotion(vil_image_view<float>& current_frame, vil_image_view<float>& previous_frame, int n,
                                 double thresh, vil_image_view<float>& vx, vil_image_view<float>& vy);

  // : computes velocity of a region(view) using Lucas Kanade
  static void lucas_kanade_motion_on_view(vil_image_view<float> const& curr_frame,
                                          vil_image_view<float> const& prev_frame, const double thresh, float& vx,
                                          float& vy);

  // : computes velocity of a region(view) using correlation
  static void velocity_by_correlation(vil_image_view<float> const& curr_image, vil_image_view<float> const& prev_region,
                                      const unsigned start_i, const unsigned end_i, const unsigned start_j,
                                      const unsigned end_j, const unsigned zero_i, const unsigned zero_j, float& vx,
                                      float& vy);
  ...
  static int Horn_SchunckMotion(vil_image_view<float> const & current_frame,
                                vil_image_view<float> const& previous_frame, vil_image_view<float>& vx,
                                vil_image_view<float>& vy, const float alpha_coef = 10000.0f,
  ...
  static vil_image_view<vxl_byte> convert_to_byte(vil_image_view<vxl_uint_16> const& image, vxl_uint_16 min_val,
                                                  vxl_uint_16 max_val);
  ...
  // Use this instead of convert_to_grey
  static vil_image_view<vxl_uint_16> convert_to_short(vil_image_view<float> const& image, float min_val, float max_val);

  // : converts a float image to an unsigned short (16-bit) image.
  // Range is determined automatically
  static vil_image_view<vxl_uint_16> convert_to_short(vil_image_view<float> const& image);

  // : converts a generic image to an unsigned short (16-bit) image
  // Use this instead of convert_to_grey
  static vil_image_view<vxl_uint_16> convert_to_short(vil_image_resource_sptr const& image);
  ...
  static void convert_to_IHS(vil_image_view<vil_rgb<vxl_byte> > const& image, vil_image_view<float>& I,
                             vil_image_view<float>& H, vil_image_view<float>& S);

  // : converts a byte-pixel image to float (I,H,S) image triple
  static void convert_to_IHS(vil_image_view<vxl_byte> const& image, vil_image_view<float>& I, vil_image_view<float>& H,
                             vil_image_view<float>& S);

  // : somewhat different map to IHS, ratio = (H+1)/(I+1) indicating shadows
  static void convert_to_IHS_tsai(vil_image_view<vxl_byte> const& image, vil_image_view<float>& I,
                                  vil_image_view<float>& H, vil_image_view<float>& S, vil_image_view<float>& ratio);

// : display (I,H,S) image triple as RGB (no conversion from IHS to RGB!)
  static void display_IHS_as_RGB(vil_image_view<float> const& I, vil_image_view<float> const& H,
                                 vil_image_view<float> const& S, vil_image_view<vil_rgb<vxl_byte> >& image);

#if 0 // TODO ?
  // : converting IHS to RGB
  static void convert_IHS_as_RGB(vil_image_view<float> const& I, vil_image_view<float> const& H,
                                 vil_image_view<float> const& S, vil_image_view<vil_rgb<vxl_byte> >& image);
  ...
  static vil_image_view<vil_rgb<vxl_byte> > combine_color_planes(vil_image_view<vxl_byte> const& R,
  ...
  static vil_image_view<vil_rgb<vxl_byte> > combine_color_planes(vil_image_resource_sptr const& R,
  ...
  static bool fourier_transform(vil_image_view<float> const& input, vil_image_view<float>& mag,
                                vil_image_view<float>& phase);

  // : compute the inverse Fourier transform using the vnl FFT algorithm
  static bool inverse_fourier_transform(vil_image_view<float> const& mag, vil_image_view<float> const& phase,
                                        vil_image_view<float>& output);

  // : resize to specified dimensions.
  //  Fill with zeros if output is larger
  static void resize(vil_image_view<float> const& input, unsigned width, unsigned height,
                     vil_image_view<float>& output);
  ...
  static bool spatial_frequency_filter(vil_image_view<float> const& input, float dir_fx, float dir_fy, float f0,
                                       float radius, bool output_fourier_mag, vil_image_view<float>& output);
  ...
  static bool homography(vil_image_view<float> const& input, vgl_h_matrix_2d<double> const& H,
                         vil_image_view<float>& output, bool output_size_fixed = false, float output_fill_value = 0.0f);
  ...
  // If \p roi does not overlap input, return false
  static bool chip(vcl_vector<vil_image_resource_sptr> const& images, brip_roi_sptr const& roi,
  ...
  static float cross_correlate(vil_image_view<float> const& image1, vil_image_view<float> const& image2, float x,
                               float y, int radius = 5, float intensity_thresh = 25.0f);

  // : cross_correlate two images using running sums
  static bool cross_correlate(vil_image_view<float> const& image1, vil_image_view<float> const& image2,
                              vil_image_view<float>& out, int radius = 5, float intensity_thresh = 25.0f);

  // : Compute the intensity entropy of a region about the specified pixel
  //  No bounds check
  static float entropy_i(const unsigned i, const unsigned j, const unsigned i_radius, const unsigned j_radius,
                         vil_image_view<float> const& intensity, const float range = 255.0f, const unsigned bins = 16);

  // : Compute the gradient entropy of a region about the specified pixel
  //  No bounds check
  static float entropy_g(const unsigned i, const unsigned j, const unsigned i_radius, const unsigned j_radius,
                         vil_image_view<float> const& gradx, vil_image_view<float> const& grady,
  ...
  //  No bounds check
  static float entropy_hs(const unsigned i, const unsigned j, const unsigned i_radius, const unsigned j_radius,
                          vil_image_view<float> const& hue, vil_image_view<float> const& sat,
                          const float range = 360.0f, const unsigned bins = 8);

  // : Compute the entropy of the specified region about each pixel
  static vil_image_view<float> entropy(const unsigned i_radius, const unsigned j_radius, const unsigned step,
                                       vil_image_resource_sptr const& img, const float sigma = 1.0f,
                                       const unsigned bins = 16, const bool intensity = true,
                                       const bool gradient = true, const bool ihs = false);

  // : Compute the intensity minfo of a region about the specified pixel
  //  No bounds check
  static float minfo_i(const unsigned i0, const unsigned j0, const unsigned i1, const unsigned j1,
                       const unsigned i_radius, const unsigned j_radius, vil_image_view<float> const& intensity0,
                       vil_image_view<float> const& intensity1, const float range = 255.0f, const unsigned bins = 16);

  // : Compute the gradient minfo of a region about the specified pixel
  //  No bounds check
  static float minfo_g(const unsigned i0, const unsigned j0, const unsigned i1, const unsigned j1,
                       const unsigned i_radius, const unsigned j_radius, vil_image_view<float> const& gradx0,
                       vil_image_view<float> const& grady0, vil_image_view<float> const& gradx1,
                       vil_image_view<float> const& grady1, const float range = 360.0f, const unsigned bins = 8);

  // : Compute the hue and saturation minfo of a region about the specified pixel
  //  No bounds check
  static float minfo_hs(const unsigned i0, const unsigned j0, const unsigned i1, const unsigned j1,
                        const unsigned i_radius, const unsigned j_radius, vil_image_view<float> const& hue0,
                        vil_image_view<float> const& sat0, vil_image_view<float> const& hue1,
                        vil_image_view<float> const& sat1, const float range = 360.0f, const unsigned bins = 8);

  // : Compute the minfo of the specified region about each pixel
  static bool minfo(const unsigned i_radius, const unsigned j_radius, const unsigned step,
                    vil_image_resource_sptr const& img0, vil_image_resource_sptr const& img1,
                    vil_image_view<float>& MI0, vil_image_view<float>& MI1, const float sigma = 1.0f,
                    const bool intensity = true, const bool gradient = true, const bool ihs = false);
...
private:
  ...
  static void double_resolution_1d(const float* input, const unsigned n_input, const float k0, const float k1,
