...
bool bstm_time_block::init_empty_block()
{
  ...
  for( iter = time_trees_.begin(); iter != time_trees_.end(); ++iter )
    ...
    if( init_level_t_ == 1 )
      ...
      {
      vcl_cerr << "Init lvl " << init_level_t_
               << " not supported. Only init level 1 or 6 is supported for time trees currently...\n";
      }
...
{
  return boxm2_array_1d<vnl_vector_fixed<unsigned char, 8> >(sub_block_num_t_,
                                                             &(time_trees_[cell_data_offset * sub_block_num_t_]) );
}
