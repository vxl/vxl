...
public:
  ...
  static bool query_point(bstm_scene_sptr& scene, bstm_cache_sptr& cache, const vgl_point_3d<double>& point,
                          unsigned time, float& prob, float& intensity);

  static bool query_point_color(bstm_scene_sptr& scene, bstm_cache_sptr& cache, const vgl_point_3d<double>& point,
                                unsigned time, float& prob, vnl_vector_fixed<unsigned char, 3>& rgb);

  // verifies that a scene has a valid appearance, spits out data type and appearance type size
  static bool verify_appearance(bstm_scene& scene, const vcl_vector<vcl_string>& valid_types, vcl_string& data_type,
                                int& appTypeSize );
