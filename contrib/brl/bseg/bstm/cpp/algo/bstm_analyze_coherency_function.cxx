...
bstm_analyze_coherency_function::bstm_analyze_coherency_function(bstm_block* blk, bstm_block_metadata blk_data,
                                                                 bstm_time_block* blk_t,
                                                                 bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype * apps,
                                                                 bstm_data_traits<BSTM_ALPHA>::datatype * alphas,
                                                                 double init_local_time, double end_local_time,
                                                                 vgl_box_3d<double> box, float p_threshold,
        ...
        if( !vgl_intersection<double>(tree_box, box).is_empty() )
          ...
          for( iter = leafBits.begin(); iter != leafBits.end(); ++iter )
            ...
            vgl_point_3d<double> cellCenter_global(
              float(cellCenter.x() * blk_data.sub_block_dim_.x() + blk_data.local_origin_.x() ),
            ...
            if( !vgl_intersection<double>(cell_box, box).is_empty() )
              ...
              vnl_vector_fixed<unsigned char, 8> time_treebits = blk_t->get_cell_tt(data_offset,
                                                                                    init_local_time);
              bstm_time_tree time_tree(
                time_treebits.data_block(), blk_data.max_level_t_);
              int bit_index_t = time_tree.traverse(init_local_time - blk_t->tree_index(
                                                     init_local_time) );
              int   data_offset_t = time_tree.get_data_index(bit_index_t);
              float original_prob = 1.0f
                - (float)vcl_exp(-alphas[data_offset_t] * side_len * blk_data.sub_block_dim_.x() );
              ...
              for( double t = init_local_time; t <= end_local_time; t += time_inc )
                ...
                bstm_time_tree                     time_tree(
                  time_treebits.data_block(), blk_data.max_level_t_);
                int bit_index_t = time_tree.traverse(t - blk_t->tree_index(
                                                       t) );
                int data_offset_t =
                  time_tree.get_data_index(bit_index_t);
                float prob = 1.0f - (float)vcl_exp(
                    -alphas[data_offset_t] * side_len * blk_data.sub_block_dim_.x() );
                bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype app = apps[data_offset_t];
                output_file << bstm_similarity_traits<BSTM_MOG6_VIEW_COMPACT, BOXM2_MOG6_VIEW_COMPACT>::kl_div_surf(
                  original_prob, prob) << " ";
                }
