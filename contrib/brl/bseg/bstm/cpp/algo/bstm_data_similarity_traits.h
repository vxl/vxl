...
class bstm_similarity_traits<BSTM_MOG6_VIEW, BOXM2_MOG6_VIEW>
{
 public:
  static bool is_similar(bstm_data_traits<BSTM_MOG6_VIEW>::datatype /*app*/,
                         ...
                         float /*p*/, float /*boxm2_p*/ , double /*p_threshold*/, double /*app_threshold*/)                           { return true; }
};
 ...
 public:
...
#endif
  static bool is_similar(bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype app, boxm2_data_traits<BOXM2_MOG6_VIEW_COMPACT>::datatype boxm2_app, float p, float boxm2_p, double p_threshold, double app_threshold )
  {
    if( ( p_threshold < 0.0f || kl_div_surf(p,boxm2_p) < p_threshold)  &&  (app_threshold < 0.0f || p < APP_COMP_P_THRESHOLD ||  (kl_div_app(app,boxm2_app)  < app_threshold)  ) )
      return true;
    else
      return false;
  }
  ...
  static float kl_div_app(bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype app, boxm2_data_traits<BOXM2_MOG6_VIEW_COMPACT>::datatype boxm2_app )
  {
    ...
    for(int i = 0; i < 8; i++)
    {
      ...
      if(std < EPSILON)
        std = EPSILON;
      kl_div_app += -0.5f + (vcl_log(std /boxm2_std))/ vnl_math::ln2  + (vcl_pow( mean - boxm2_mean, 2.0f) + boxm2_std*boxm2_std) / (2*std*std);
...
class bstm_similarity_traits<BSTM_GAUSS_RGB_VIEW_COMPACT, BOXM2_GAUSS_RGB_VIEW_COMPACT>
{
 public:
  ...
  {
    if( ( p_threshold < 0.0f || kl_div_surf(p,boxm2_p) < p_threshold)  &&  (app_threshold < 0.0f || p < APP_COMP_P_THRESHOLD ||  (app_rms(app,boxm2_app)  < app_threshold)  ) )
      return true;
    else
      return false;
  }
  ...
  static float app_rms(bstm_data_traits<BSTM_GAUSS_RGB_VIEW_COMPACT>::datatype app, boxm2_data_traits<BOXM2_GAUSS_RGB_VIEW_COMPACT>::datatype boxm2_app )
  {
    ...
    for(int i = 0; i < 8; i++) {
      ...
      rms += vcl_sqrt( (vcl_pow(rgb[0] - boxm2_rgb[0],2) + vcl_pow(rgb[1] - boxm2_rgb[1],2) + vcl_pow(rgb[2] - boxm2_rgb[2],2)) / 3);
    }
