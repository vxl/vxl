...
public:
  static bool is_similar(bstm_data_traits<BSTM_MOG3_GREY>::datatype app,
                         boxm2_data_traits<BOXM2_MOG3_GREY>::datatype boxm2_app, float p, float boxm2_p,
                         double /* p_threshold */, double /* app_threshold */ )
  {
    double isabel_measure = (bstm_data_traits<BSTM_MOG3_GREY>::processor::expected_color(app) + 1) * p;
    double isabel_measure_boxm2 = (boxm2_processor_type<BOXM2_MOG3_GREY>::type::expected_color(boxm2_app) + 1)
      * boxm2_p;
...
public:
  static bool is_similar(bstm_data_traits<BSTM_MOG6_VIEW>::datatype /*app*/,
                         ...
                         float /*p*/, float /*boxm2_p*/, double /*p_threshold*/, double /*app_threshold*/)
  { return true; }
};
...
public:
...
#if 0
  static bool old_is_similar(bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype app,
                             boxm2_data_traits<BOXM2_MOG6_VIEW_COMPACT>::datatype boxm2_app, float p, float boxm2_p,
                             double p_threshold, double app_threshold )
  {
...
#endif
  static bool is_similar(bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype app,
                         boxm2_data_traits<BOXM2_MOG6_VIEW_COMPACT>::datatype boxm2_app, float p, float boxm2_p,
                         double p_threshold, double app_threshold )
  {
    return (  p_threshold < 0.0f || kl_div_surf(p, boxm2_p) < p_threshold)
           &&  (app_threshold < 0.0f || p < APP_COMP_P_THRESHOLD ||  (kl_div_app(app, boxm2_app)  < app_threshold)  );
  }
  ...
  static float kl_div_app(bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype app,
                          boxm2_data_traits<BOXM2_MOG6_VIEW_COMPACT>::datatype boxm2_app )
  {
    ...
    for( int i = 0; i < 8; i++ )
      ...
      kl_div_app += -0.5f + (vcl_log(std / boxm2_std) ) / vnl_math::ln2
        + (square_of(mean - boxm2_mean) + square_of(boxm2_std) ) / (2 * std * std);
      }
    return kl_div_app / 8;
  }
...
template <>
class bstm_similarity_traits<BSTM_GAUSS_RGB_VIEW_COMPACT, BOXM2_GAUSS_RGB_VIEW_COMPACT>
{
public:

  static bool is_similar(bstm_data_traits<BSTM_GAUSS_RGB_VIEW_COMPACT>::datatype app,
                         boxm2_data_traits<BOXM2_GAUSS_RGB_VIEW_COMPACT>::datatype boxm2_app, float p, float boxm2_p,
                         double p_threshold, double app_threshold )
  {
    return (  p_threshold < 0.0f || kl_div_surf(p, boxm2_p) < p_threshold)
  ...
  static float app_rms(bstm_data_traits<BSTM_GAUSS_RGB_VIEW_COMPACT>::datatype app,
                       boxm2_data_traits<BOXM2_GAUSS_RGB_VIEW_COMPACT>::datatype boxm2_app )
  {
    ...
    for( int i = 0; i < 8; i++ )
      ...
      rms +=
        vcl_sqrt( (square_of(rgb[0] - boxm2_rgb[0]) + square_of(rgb[1] - boxm2_rgb[1]) + square_of(
                     rgb[2] - boxm2_rgb[2]) ) / 3.0);
      }
...
public:
  ...
  static bool is_similar(bstm_data_traits<BSTM_GAUSS_RGB>::datatype app,
                         boxm2_data_traits<BOXM2_GAUSS_RGB>::datatype boxm2_app, float p, float boxm2_p,
                         double p_threshold, double app_threshold )
  {
    return (  p_threshold < 0.0f || kl_div_surf(p, boxm2_p) < p_threshold)
           &&  (app_threshold < 0.0f || p < APP_COMP_P_THRESHOLD ||  (app_rms(app, boxm2_app)  < app_threshold) );
