...
public:
  ...
  bstm_ingest_boxm2_scene_function(bstm_block* blk, bstm_time_block* blk_t, vcl_map<vcl_string,
                                                                                    bstm_data_base *> & datas,
                                   boxm2_block* boxm2_blk, vcl_map<vcl_string, boxm2_data_base *> & boxm2_datas,
...
private:
  ...
  int move_time_trees(boct_bit_tree& unrefined_tree, boct_bit_tree& refined_tree, bstm_time_block* newTimeBlk,
                      char* depth_diff );
  ...
  int move_all_time_trees_data( boxm2_array_1d<uchar8>& time_trees_blk_copy, int bstm_data_offset,
                                int boxm2_data_offset, int* dataIndex, int& currIndex,
  ...
  int move_data(bstm_time_tree& unrefined_tree, bstm_time_tree& refined_tree,
                bstm_data_traits<BSTM_ALPHA>::datatype*  alpha_cpy,
                typename bstm_data_traits<APM_TYPE>::datatype * apm_cpy);

  // : inserts the current boxm2 data into the bstm scene
  void place_curr_data( bstm_time_tree& refined_tree, int boxm2_data_offset,
                        bstm_data_traits<BSTM_ALPHA>::datatype*  alpha_cpy,
                        typename bstm_data_traits<APM_TYPE>::datatype * apm_cpy, int depth_diff );

  // : refine input tree and return refined tree
  bstm_time_tree refine_time_tree(const bstm_time_tree& input_tree, int bstm_data_offset, int boxm2_data_offset,
                                  int currDepth, int currDepth_boxm2);

  // : function to define similarity measure between bstm data and current boxm2 data.
  bool is_similar(float p, typename bstm_data_traits<APM_TYPE>::datatype mog, float boxm2_p,
