...
bstm_ingest_boxm2_scene_function<APM_TYPE, BOXM2_APM_TYPE>::bstm_ingest_boxm2_scene_function(bstm_block* blk,
                                                                                             bstm_time_block* blk_t,
                                                                                             vcl_map<vcl_string,
                                                                                                     bstm_data_base *> & datas,
                                                                                             boxm2_block* boxm2_blk,
                                                                                             vcl_map<vcl_string,
                                                                                                     boxm2_data_base *> & boxm2_datas,
                                                                                             double local_time, double p_threshold,
...
template <bstm_data_type APM_TYPE, boxm2_data_type BOXM2_APM_TYPE>
bool bstm_ingest_boxm2_scene_function<APM_TYPE, BOXM2_APM_TYPE>::init_data(bstm_block* blk, bstm_time_block* blk_t,
                                                                           vcl_map<vcl_string,
                                                                                   bstm_data_base *> & datas,
                                                                           boxm2_block* boxm2_blk,  vcl_map<vcl_string,
                                                                                                            boxm2_data_base *> & boxm2_datas,
                                                                           double local_time)
{
  ...
  for( vcl_map<vcl_string, boxm2_data_base *>::const_iterator iter = boxm2_datas.begin(); iter != boxm2_datas.end();
       iter++ )
    {
...
bool bstm_ingest_boxm2_scene_function<APM_TYPE, BOXM2_APM_TYPE>::conform()
{
  ...
  for( blk_iter = trees.begin(), boxm2_blk_iter = boxm2_trees.begin(); blk_iter != trees.end();
  ...
  bstm_block_metadata m_data; m_data.init_level_t_ = blk_t_->init_level(); m_data.max_level_t_ = blk_t_->max_level();
  ...
  for( time_trees_iter = new_time_trees.begin(); time_trees_iter != new_time_trees.end();
  ...
  typename bstm_data_traits<APM_TYPE>::datatype
  *  mog_cpy = (typename bstm_data_traits<APM_TYPE>::datatype *)newM->data_buffer();
  ...
  for( time_trees_iter = new_time_trees.begin(); time_trees_iter != new_time_trees.end();
    ...
    vcl_memcpy( &(alpha_cpy[new_data_ptr]), &(alpha_[old_data_ptr]),
                bstm_data_traits<BSTM_ALPHA>::datasize() * new_time_tree.num_leaves() );                                                          // number of leaves, not all cells.
    vcl_memcpy( &(mog_cpy[new_data_ptr]),   &(apm_model_[old_data_ptr]),
                bstm_data_traits<APM_TYPE>::datasize()  * new_time_tree.num_leaves() );                                                          // number of leaves, not all cells.
...
template <bstm_data_type APM_TYPE, boxm2_data_type BOXM2_APM_TYPE>
int bstm_ingest_boxm2_scene_function<APM_TYPE, BOXM2_APM_TYPE>::move_time_trees(boct_bit_tree& unrefined_tree,
  ...
  for( int j = 0; j < MAX_CELLS_ && cellsMoved < newSize; ++j )
    ...
    if( validCellOld && validCellNew )
      ...
      if( !refined_tree.is_leaf(j) ) // if the time trees being copied belong to an inner cell, erase their time trees
        {
        vnl_vector_fixed<unsigned char,
                         8> * erased_old_time_trees = new vnl_vector_fixed<unsigned char, 8>[sub_block_num_t_];
        ...
        newTimeBlk->set_cell_all_tt(newDataPtr,
                                    boxm2_array_1d<vnl_vector_fixed<unsigned char, 8> >(sub_block_num_t_,
...
template <bstm_data_type APM_TYPE, boxm2_data_type BOXM2_APM_TYPE>
boct_bit_tree bstm_ingest_boxm2_scene_function<APM_TYPE, BOXM2_APM_TYPE>::conform_tree(boct_bit_tree curr_tree,
                                                                                       boct_bit_tree boxm2_curr_tree)
{
...
bool bstm_ingest_boxm2_scene_function<APM_TYPE, BOXM2_APM_TYPE>::ingest()
{
  ...
  bstm_block_id    id = blk_->block_id();
  bstm_cache_sptr  cache = bstm_cache::instance();
  bstm_data_base * change_buffer   =   cache->get_data_base_new(id,
                                                                bstm_data_traits<BSTM_CHANGE>::prefix(),
                                                                blk_t_->tree_buff_length()
                                                                * bstm_data_traits<BSTM_CHANGE>::datasize()  );
  ...
  for( blk_iter = trees.begin(), boxm2_blk_iter = boxm2_trees.begin(); blk_iter != trees.end();
    ...
    for( int i = 0; i < MAX_CELLS_ && num_processed_cells < num_cells; ++i )
      ...
      if( (i == 0) || curr_tree.bit_at(pi) )
        ...
        // it might be that the bstm cell is further divided than the boxm2 cell.
        int i_boxm2 = i;
        while( i_boxm2 != 0 && !boxm2_curr_tree.bit_at( boxm2_curr_tree.parent_index(i_boxm2) ) )
          {
        ...
        bool refined_time_tree = this->refine_all_time_trees(bstm_data_offset, boxm2_data_offset, dataIndex, currIndex,
                                                             dataSize,
                                                             curr_tree.depth_at(i), boxm2_curr_tree.depth_at(
                                                               i_boxm2), is_leaf );
        num_processed_cells++;
        }
      }
  ...
  bstm_data_base* newA =
    new bstm_data_base(new char[dataSize * bstm_data_traits < BSTM_ALPHA > ::datasize()],
                       dataSize * bstm_data_traits<BSTM_ALPHA>::datasize(), id);
  bstm_data_base* newM =
    new bstm_data_base(new char[dataSize * bstm_data_traits < APM_TYPE > ::datasize()],
                       dataSize * bstm_data_traits<APM_TYPE>::datasize(), id);
  bstm_data_traits<BSTM_ALPHA>::datatype * alpha_cpy = (bstm_data_traits<BSTM_ALPHA>::datatype *)newA->data_buffer();
  typename bstm_data_traits<APM_TYPE>::datatype
  *  apm_cpy   = (typename bstm_data_traits<APM_TYPE>::datatype *)newM->data_buffer();
  ...
  for( blk_iter = trees.begin(), boxm2_blk_iter = boxm2_trees.begin(); blk_iter != trees.end();
    ...
    for( int i = 0; i < MAX_CELLS_ && num_processed_cells < num_cells; ++i )
      ...
      if( (i == 0) || curr_tree.bit_at(pi) )
        ...
        newInitCount += move_all_time_trees_data(time_trees_blk_copy, bstm_data_offset, boxm2_data_offset, dataIndex,
                                                 currIndex,
...
template <bstm_data_type APM_TYPE, boxm2_data_type BOXM2_APM_TYPE>
int bstm_ingest_boxm2_scene_function<APM_TYPE, BOXM2_APM_TYPE>::move_all_time_trees_data(
  boxm2_array_1d<uchar8>& time_trees_blk_copy,
  int bstm_data_offset,
  int boxm2_data_offset,
  int* dataIndex, int& currIndex,
  ...
  int depth_diff)
{
  ...
  boxm2_array_1d<uchar8> time_trees_copy(sub_block_num_t_,
                                         &(time_trees_blk_copy[bstm_data_offset * sub_block_num_t_]) );

  boxm2_array_1d<uchar8> time_trees_refined =  blk_t_->get_cell_all_tt(bstm_data_offset);

  // zip thru time trees
  int newInitCount = 0;
  for( unsigned int t = 0; t < time_trees_refined.size(); ++t )
    {
    bstm_time_tree refined_tree( time_trees_refined[t].data_block(), max_level_t_);
    ...
    if( t == blk_t_->tree_index(local_time_) ) // if this time tree contains the queried time
      ...
      refined_tree.cell_range(refined_tree.traverse(local_time_ - blk_t_->tree_index(local_time_) ), cell_min,
                              cell_max);
...
template <bstm_data_type APM_TYPE, boxm2_data_type BOXM2_APM_TYPE>
bool bstm_ingest_boxm2_scene_function<APM_TYPE, BOXM2_APM_TYPE>::refine_all_time_trees(int bstm_data_offset,
                                                                                       int boxm2_data_offset,
                                                                                       int* dataIndex,
                                                                                       int& currIndex, int& dataSize,
                                                                                       int currDepth,
                                                                                       int currDepth_boxm2,
                                                                                       bool is_leaf)
{
  ...
  for( unsigned int t = 0; t < all_time_trees.size(); ++t )
    ...
    if( t == blk_t_->tree_index(local_time_) && is_leaf )
      {
      bstm_time_tree refined_t_tree = this->refine_time_tree(tmp_tree, bstm_data_offset, boxm2_data_offset, currDepth,
                                                             currDepth_boxm2);
...
template <bstm_data_type APM_TYPE, boxm2_data_type BOXM2_APM_TYPE>
void bstm_ingest_boxm2_scene_function<APM_TYPE, BOXM2_APM_TYPE>::place_curr_data(bstm_time_tree& refined_tree,
                                                                                 int boxm2_data_offset,
                                                                                 bstm_data_traits<BSTM_ALPHA>::datatype*  alpha_cpy,
                                                                                 typename bstm_data_traits<APM_TYPE>::datatype * apm_cpy,
                                                                                 int
...
template <bstm_data_type APM_TYPE, boxm2_data_type BOXM2_APM_TYPE>
bstm_time_tree bstm_ingest_boxm2_scene_function<APM_TYPE, BOXM2_APM_TYPE>::refine_time_tree(
  const bstm_time_tree& input_tree, int bstm_data_offset, int boxm2_data_offset, int currDepth, int currDepth_boxm2)
{
...
template <bstm_data_type APM_TYPE, boxm2_data_type BOXM2_APM_TYPE>
bool bstm_ingest_boxm2_scene_function<APM_TYPE, BOXM2_APM_TYPE>::is_similar(float p,
                                                                            typename bstm_data_traits<APM_TYPE>::datatype mog,
                                                                            float boxm2_p,
                                                                            typename boxm2_data_traits<BOXM2_APM_TYPE>::datatype boxm2_mog)
{
  return bstm_similarity_traits<APM_TYPE, BOXM2_APM_TYPE>::is_similar(mog, boxm2_mog, p, boxm2_p, p_threshold_,
                                                                      app_threshold_);
}

template <bstm_data_type APM_TYPE, boxm2_data_type BOXM2_APM_TYPE>
int bstm_ingest_boxm2_scene_function<APM_TYPE, BOXM2_APM_TYPE>::move_data(bstm_time_tree& unrefined_tree,
