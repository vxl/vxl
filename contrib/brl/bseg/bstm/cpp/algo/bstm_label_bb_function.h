...
inline void bstm_label_bb(bstm_block* blk, bstm_block_metadata blk_data, bstm_time_block* blk_t,
                          bstm_data_traits<BSTM_LABEL>::datatype * labels,
                          bstm_data_traits<BSTM_ALPHA>::datatype * alpha, double local_time, unsigned char label,
        ...
        if( !vgl_intersection<double>(tree_box, box).is_empty() )
          ...
          for( iter = leafBits.begin(); iter != leafBits.end(); ++iter )
            ...
            if( !vgl_intersection<double>(cell_box, box).is_empty() )
              ...
              int                                bit_index_t =
                time_tree.traverse(local_time - blk_t->tree_index(local_time) );
...
inline void bstm_label_oriented_bb(bstm_block* blk, bstm_block_metadata blk_data, bstm_time_block* blk_t,
                                   bstm_data_traits<BSTM_LABEL>::datatype * labels,
                                   bstm_data_traits<BSTM_ALPHA>::datatype * alpha, double local_time,
                                   unsigned char label, vgl_orient_box_3d<double> box, float p_threshold)
{
...
inline void bstm_label_ball(bstm_block* blk, bstm_block_metadata blk_data, bstm_time_block* blk_t,
                            bstm_data_traits<BSTM_LABEL>::datatype * labels,
                            bstm_data_traits<BSTM_ALPHA>::datatype * alpha, double local_time, unsigned char label,
                            vgl_sphere_3d<double> sphere, float p_threshold)
{
  boxm2_array_3d<uchar16>& trees = blk->trees();
  // iterate through each tree
  for( unsigned int x = 0; x < trees.get_row1_count(); ++x )
