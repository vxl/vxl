...
bool bstm_merge_tt_function::init_data(bstm_time_block* blk_t, bstm_block* blk, vcl_vector<bstm_data_base *> & datas,
                                       float prob_thresh)
{
...
bool bstm_merge_tt_function::merge(vcl_vector<bstm_data_base *>& datas)
{
  ...
  int                              old_dataSize = 0;
  for( old_time_trees_iter = old_time_trees.begin(); old_time_trees_iter != old_time_trees.end();
       ++old_time_trees_iter, ++currIndex )
    {
...
void bstm_merge_tt_function::move_data(bstm_time_tree old_tree, bstm_time_tree merged_tree,  int depth,
                                       bstm_data_traits<BSTM_ALPHA>::datatype* alpha_cpy,
                                       bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype* mog_cpy,
                                       bstm_data_traits<BSTM_NUM_OBS_VIEW_COMPACT>::datatype* numobs_cpy)
{
  ...
  for( vcl_vector<int>::iterator iter = merged_tree_leaves.begin(); iter != merged_tree_leaves.end(); iter++ )
    ...
    if( old_tree.is_leaf(*iter) )   // if they both exist
      ...
      {
      if( old_tree.is_leaf(old_tree.child_index(*iter) ) == 0 ||
          old_tree.is_leaf(old_tree.child_index(*iter) + 1 ) == 0 )
        {
        vcl_cout << "Bit " << *iter << " and childs: " << old_tree.child_index(*iter) << " and "
                 << old_tree.child_index(*iter) + 1 << vcl_endl;
        vcl_cout << "Valid cells " << old_tree.valid_cell(old_tree.child_index(*iter) ) << " and "
                 << old_tree.valid_cell(old_tree.child_index(*iter) + 1 ) << vcl_endl;
        vcl_cout << "Valid leaves " << old_tree.is_leaf(old_tree.child_index(*iter) ) << " and " << old_tree.is_leaf(old_tree.child_index(
                                                                                                                       *
                                                                                                                       iter)
                                                                                                                     + 1 )
        << vcl_endl;
        vcl_cout << "Num leaves in old tree " << old_tree.num_leaves() << " and in the new one: "
                 << merged_tree.num_leaves() << vcl_endl;
        }
