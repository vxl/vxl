...
bool bstm_refine_blk_in_spacetime_function::init_data(bstm_time_block* blk_t, bstm_block* blk,
                                                      vcl_vector<bstm_data_base *> & datas, float prob_thresh)
{
...
bool bstm_refine_blk_in_spacetime_function::refine(vcl_vector<bstm_data_base *>& datas)
{
  ...
  dataSize = 0;
  for( refined_time_trees_iter =
         new_refined_time_trees.begin(); refined_time_trees_iter != new_refined_time_trees.end();
       ++refined_time_trees_iter, ++currIndex )
    {
  ...
  bstm_data_base* newM = new bstm_data_base(
      new char[dataSize * bstm_data_traits < BSTM_MOG6_VIEW_COMPACT > ::datasize()],
      dataSize * bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datasize(), id);
  bstm_data_base* newN =
    new bstm_data_base(new char[dataSize * bstm_data_traits < BSTM_NUM_OBS_VIEW_COMPACT > ::datasize()],
  ...
  bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype * mog_cpy =
    ( bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype *)newM->data_buffer();
  bstm_data_traits<BSTM_NUM_OBS_VIEW_COMPACT>::datatype * numobs_cpy =
    (bstm_data_traits<BSTM_NUM_OBS_VIEW_COMPACT>::datatype *)newN->data_buffer();

  // 5. move data from old data buffers to new data buffers
  currIndex = 0;
  for( refined_time_trees_iter = new_refined_time_trees.begin(),
       unrefined_time_trees_iter = new_unrefined_time_trees.begin();
       refined_time_trees_iter != new_refined_time_trees.end();
       ++refined_time_trees_iter, ++currIndex, ++unrefined_time_trees_iter )
    {
    ...
    // 4. move the data
    this->move_data(unrefined_time_tree, refined_time_tree, alpha_cpy, mog_cpy, numobs_cpy,
                    (int)( depths[currIndex / sub_block_num_t_]) );
    }
...
void bstm_refine_blk_in_spacetime_function::move_data(bstm_time_tree& unrefined_time_tree,
                                                      bstm_time_tree& refined_time_tree,
                                                      bstm_data_traits<BSTM_ALPHA>::datatype* alpha_cpy,
                                                      bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype * mog_cpy,
                                                      bstm_data_traits<BSTM_NUM_OBS_VIEW_COMPACT>::datatype * numobs_cpy,
                                                      int depth)
...
int bstm_refine_blk_in_spacetime_function::move_time_trees(boct_bit_tree& unrefined_tree, boct_bit_tree& refined_tree,
                                                           ...
                                                           bstm_time_block* newRefinedTimeBlk, char* depths )
{
  int newSize = refined_tree.num_cells();
  ...
  for( int j = 0; j < MAX_CELLS_ && cellsMoved < newSize; ++j )
    ...
    if( validCellOld && validCellNew )  // both old and new cell exists
      ...
      if( !refined_tree.is_leaf(j) ) // if the time trees being copied belong to an inner cell, erase their time trees
        ...
        newRefinedTimeBlk->set_cell_all_tt(newDataPtr,
                                           boxm2_array_1d<vnl_vector_fixed<unsigned char, 8> >(sub_block_num_t_,
                                                                                               erased_old_time_trees) );                                // set all tt to new loc
        ...
        boxm2_array_1d<vnl_vector_fixed<unsigned char, 8> > refined_time_trees = newRefinedTimeBlk->get_cell_all_tt(
            newDataPtr);
        for( unsigned int t_idx = 0; t_idx < sub_block_num_t_; ++t_idx )
          {
...
bstm_time_tree bstm_refine_blk_in_spacetime_function::refine_time_tree(bstm_time_tree& unrefined_time_tree,
                                                                       double side_len )
{
