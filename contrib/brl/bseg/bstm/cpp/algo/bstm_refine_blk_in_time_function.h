...
public:
  ...
  // : "default" constructor
  bstm_refine_blk_in_time_function(bstm_time_block* t_blk, bstm_block* blk, vcl_vector<bstm_data_base *> & datas,
                                   float change_prob_t, float time);

  // : initialize generic data base pointers as their data type
  bool init_data(bstm_time_block* t_blk, bstm_block* blk, vcl_vector<bstm_data_base *> & datas,
                 float change_prob_thresh, float local_time);
...
private:
  ...
  void move_data(bstm_time_tree& unrefined_time_tree, bstm_time_tree& refined_time_tree,
                 ...
                 typename bstm_data_traits<NOBS_DATA_TYPE>::datatype * numobs_cpy, int depth);
