...
template <bstm_data_type APM_DATA_TYPE, bstm_data_type NOBS_DATA_TYPE>
bool bstm_refine_blk_in_time_function<APM_DATA_TYPE, NOBS_DATA_TYPE>::init_data(bstm_time_block* blk_t, bstm_block* blk,
                                                                                vcl_vector<bstm_data_base *> & datas,
                                                                                float change_prob_t, float local_time)
{
...
bool bstm_refine_blk_in_time_function<APM_DATA_TYPE, NOBS_DATA_TYPE>::refine(vcl_vector<bstm_data_base *>& datas)
{
  ...
  bstm_time_block* newTimeBlk =
    new bstm_time_block(id, m_data, blk_t_->tree_buff_length() / blk_t_->sub_block_num() );                            // create empty time block
  ...
  *  numobs_cpy = (typename bstm_data_traits<NOBS_DATA_TYPE>::datatype *)newN->data_buffer();
  ...
  for( refined_time_trees_iter = new_time_trees.begin(), unrefined_time_trees_iter = old_time_trees.begin();
       refined_time_trees_iter != new_time_trees.end();
...
template <bstm_data_type APM_DATA_TYPE, bstm_data_type NOBS_DATA_TYPE>
void bstm_refine_blk_in_time_function<APM_DATA_TYPE, NOBS_DATA_TYPE>::move_data(bstm_time_tree& unrefined_time_tree,
  ...
  for( vcl_vector<int>::iterator iter = new_leaves.begin(); iter != new_leaves.end(); iter++ )
    ...
    if( validCellOld )  // if they both exist
      ...
      if(  curr_time_tree_leaf == *iter )     // if the current time is the start of a cell in which new data will be placed
        ...
        while( valid_parent_bit != 0 &&
               !unrefined_time_tree.bit_at( unrefined_time_tree.parent_index(valid_parent_bit) ) )
          {
