...
void ingest(bstm_time_tree& tree, char*  tree_data, float time, char data)
{
  ...
  if( data != tree_data[offset] )
    ...
    for( int i = 0; i < DATA_LEN; tree_data[i] = new_data[i], ++i )
      {
      ;                                                             // copy new data into old data
      }
    }
}
...
void test_time_tree_ingestion()
{
  ...
  for( unsigned int i = 0; i < leaf_bits.size(); ++i )
    {
    vcl_cout << "Data at leaf " << leaf_bits[i] << " is " << tree_data[tree.get_relative_index(leaf_bits[i])]
             << vcl_endl;
    }
