  ...
  for( ; bstm_iter != blocks.end(); bstm_iter++ )
    ...
    if( !vgl_intersection<double>( bstm_metadata.bbox(), box).is_empty() ) // if the two boxes intersect
      ...
      if( bstm_metadata.contains_t(init_time, init_local_time) && bstm_metadata.contains_t(end_time, end_local_time) ) // if the block box contains the given times
        ...
        bstm_data_base * app = cache->get_data_base(bstm_metadata.id_,
        ...
        bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype * app_data =
          (bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datatype *)app->data_buffer();

        bstm_analyze_coherency_function coherency(blk, bstm_metadata, blk_t, app_data, alpha_data, init_local_time,
                                                  end_local_time, box, p_threshold, output_file);
        }
      }
    }
  output_file.close();
