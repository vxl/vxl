...
bool bstm_cpp_box_roc_process(bprb_func_process& pro)
{
  ...
  // create vgl box,
  const vgl_point_3d<double> change_region_center(change_region_center_x, change_region_center_y,
                                                  change_region_center_z);
  vgl_box_3d<double> change_box(change_region_center, change_region_len_x, change_region_len_y,
                                change_region_len_z, vgl_box_3d<double>::centre);
  vgl_box_3d<double> change_box_insides(change_region_center, change_region_len_x, change_region_len_y,
                                        change_region_len_z, vgl_box_3d<double>::centre);
  ...
  for( ; bstm_iter != blocks.end(); bstm_iter++ )
    ...
    bstm_data_base * change_buffer   =   cache->get_data_base(bstm_metadata.id_,
                                                              bstm_data_traits<BSTM_CHANGE>::prefix()  );
    bstm_data_traits<BSTM_CHANGE>::datatype* change_array =
