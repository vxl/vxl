...
bool bstm_cpp_copy_data_to_future_process(bprb_func_process& pro)
{
  ...
  if( !bstm_util::verify_appearance( *scene, valid_types, app_data_type, apptypesize ) )
    ...
    vcl_cout
      <<
    "bstm_cpp_copy_data_to_future_process ERROR: scene doesn't have BSTM_MOG6_VIEW_COMPACT or BSTM_MOG3_GREY or BSTM_GAUSS_RGB data type"
      << vcl_endl;
    return false;
    }
  ...
  if( !bstm_util::verify_appearance( *scene, valid_types, nobs_data_type, nobstypesize ) )
    ...
    vcl_cout
      <<
    "bstm_cpp_copy_data_to_future_process ERROR: scene doesn't have BSTM_NUM_OBS or BSTM_NUM_OBS_VIEW_COMPACT or BSTM_NUM_OBS_SINGLE data type"
      << vcl_endl;
    return false;
    }
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    if( app_data_type == bstm_data_traits<BSTM_MOG3_GREY>::prefix() &&
      ...
      {
      bstm_copy_data_to_future_function<BSTM_MOG6_VIEW_COMPACT, BSTM_NUM_OBS_VIEW_COMPACT>( blk_t, blk, datas,
                                                                                            local_time);
      }
