...
bool bstm_cpp_copy_data_to_future_process(bprb_func_process& pro)
{
  ...
  if ( !bstm_util::verify_appearance( *scene, valid_types, app_data_type, apptypesize ) ) {
    std::cout<<"bstm_cpp_copy_data_to_future_process ERROR: scene doesn't have BSTM_MOG6_VIEW_COMPACT or BSTM_MOG3_GREY or BSTM_GAUSS_RGB data type"<<std::endl;
    return false;
  }
  ...
  if ( !bstm_util::verify_appearance( *scene, valid_types, nobs_data_type, nobstypesize ) ) {
    std::cout<<"bstm_cpp_copy_data_to_future_process ERROR: scene doesn't have BSTM_NUM_OBS or BSTM_NUM_OBS_VIEW_COMPACT or BSTM_NUM_OBS_SINGLE data type"<<std::endl;
    return false;
  }


  std::cout<<"Copying data to future..."<<std::endl;
  ...
  for (blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter)
  {
    ...
    if(!mdata.contains_t(time,local_time))
      continue;
    std::cout<<"Processsing Block: "<<id<<std::endl;
    ...
    if(app_data_type == bstm_data_traits<BSTM_MOG3_GREY>::prefix() &&  nobs_data_type == bstm_data_traits<BSTM_NUM_OBS>::prefix()  )
      ...
      std::cerr << "bstm_cpp_copy_data_to_future_process ERROR! Types don't match...." << std::endl;
      std::cerr << "App type: " << app_data_type << " and nobs: " << nobs_data_type << std::endl;
    }
  }

  std::cout << "Finished copying data scene..." << std::endl;
  return true;
}
