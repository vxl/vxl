...
bool bstm_cpp_export_point_cloud_process(bprb_func_process& pro)
{
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    bstm_data_base * change_prob =  cache->get_data_base(id,
                                                         bstm_data_traits<BSTM_CHANGE>::prefix(),
                                                         num_el *  bstm_data_traits<BSTM_CHANGE>::datasize() );
...
//    bstm_data_base * change_prob_neg =  cache->get_data_base(id,bstm_data_traits<BSTM_CHANGE>::prefix("neg"));
    ...
    bstm_data_traits<BSTM_CHANGE>::datatype * change_data =
      (bstm_data_traits<BSTM_CHANGE>::datatype *)change_prob->data_buffer();
...
//    bstm_data_traits<BSTM_CHANGE>::datatype *    change_data_neg = (bstm_data_traits<BSTM_CHANGE>::datatype*) change_prob_neg->data_buffer();
    ...
    file << vcl_fixed;
    for( unsigned currIdx = 0; currIdx < (points->buffer_length() / bstm_data_traits<BSTM_POINT>::datasize() );
         currIdx++ )
      {
