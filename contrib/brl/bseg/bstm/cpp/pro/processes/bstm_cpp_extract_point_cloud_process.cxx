...
bool bstm_cpp_extract_point_cloud_process(bprb_func_process& pro)
{
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    bstm_data_base * points = cache->get_data_base(id,
                                                   bstm_data_traits<BSTM_POINT>::prefix(),
                                                   num_time_trees * bstm_data_traits<BSTM_POINT>::datasize(), false);

    // 3d array of trees
    boxm2_array_3d<uchar16>&                 trees = cache->get_block(id)->trees();
    bstm_data_traits<BSTM_ALPHA>::datatype * alpha_data =
      (bstm_data_traits<BSTM_ALPHA>::datatype *)alpha->data_buffer();
    bstm_data_traits<BSTM_POINT>::datatype * points_data =
      (bstm_data_traits<BSTM_POINT>::datatype *)points->data_buffer();
      ...
      for( unsigned int y = 0; y < trees.get_row2_count(); ++y )
        ...
        for( unsigned int z = 0; z < trees.get_row3_count(); ++z )
          ...
          for( iter = leafBits.begin(); iter != leafBits.end(); ++iter )
            ...
            bstm_time_tree tt( cache->get_time_block(id)->get_cell_tt(currIdx,
                                                                      local_time).data_block(), data.max_level_t_ );
