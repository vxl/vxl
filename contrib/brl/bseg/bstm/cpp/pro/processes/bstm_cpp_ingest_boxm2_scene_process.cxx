...
bool bstm_cpp_ingest_boxm2_scene_process(bprb_func_process& pro)
{
  ...
  if( !bstm_util::verify_appearance( *scene, valid_types, data_type, apptypesize ) )
    {
    vcl_cout
      << "bstm_cpp_ingest_boxm2_scene_process ERROR: scene doesn't have BSTM_MOG6_VIEW or BSTM_MOG3_GREY data type"
      << vcl_endl;
    return false;
    }
  ...
  if( !boxm2_util::verify_appearance( *boxm2_scene, boxm2_valid_types, boxm2_data_type, boxm2_apptypesize ) )
    ...
    vcl_cout
      <<
    "bstm_cpp_ingest_boxm2_scene_process ERROR: boxm2 scene doesn't have BOXM2_MOG3_GREY or BOXM2_MOG3_GREY_16 data type"
      << vcl_endl;
    return false;
    }
    ...
    for( ; bstm_iter != blocks.end(); bstm_iter++ )
      ...
      if( bstm_id == boxm2_id )
        ...
        if( !(bstm_metadata == boxm2_metadata) )
          {
          vcl_cerr << "bstm scene and boxm2 scene are not consistent! block " << boxm2_id
                   << " metadata not consistent!\n";
        ...
        boxm2_data_base * boxm2_alph    =
          boxm2_cache->get_data_base(boxm2_scene, bstm_metadata.id_, boxm2_data_traits<BOXM2_ALPHA>::prefix() );
        ...
        if( boxm2_data_type == boxm2_data_traits<BOXM2_MOG3_GREY>::prefix() &&
            data_type == bstm_data_traits<BSTM_MOG3_GREY>::prefix()  )
          {
          bstm_ingest_boxm2_scene_function<BSTM_MOG3_GREY, BOXM2_MOG3_GREY>(blk, blk_t, datas, boxm2_blk, boxm2_datas,
                                                                            local_time, p_threshold, app_threshold);
          }
        else if( boxm2_data_type == boxm2_data_traits<BOXM2_MOG6_VIEW>::prefix() &&
                 data_type == bstm_data_traits<BSTM_MOG6_VIEW>::prefix()  )
          {
          bstm_ingest_boxm2_scene_function<BSTM_MOG6_VIEW, BOXM2_MOG6_VIEW>(blk, blk_t, datas, boxm2_blk, boxm2_datas,
                                                                            local_time, p_threshold, app_threshold);
          }
        else if( boxm2_data_type == boxm2_data_traits<BOXM2_MOG6_VIEW_COMPACT>::prefix() &&
                 data_type == bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::prefix()  )
          {
          bstm_ingest_boxm2_scene_function<BSTM_MOG6_VIEW_COMPACT, BOXM2_MOG6_VIEW_COMPACT>(blk, blk_t, datas,
                                                                                            boxm2_blk, boxm2_datas,
                                                                                            local_time, p_threshold,
                                                                                            app_threshold);
          }
        else if( boxm2_data_type == boxm2_data_traits<BOXM2_GAUSS_RGB>::prefix() &&
                 data_type == bstm_data_traits<BSTM_GAUSS_RGB>::prefix()  )
          {
          bstm_ingest_boxm2_scene_function<BSTM_GAUSS_RGB, BOXM2_GAUSS_RGB>(blk, blk_t, datas, boxm2_blk, boxm2_datas,
                                                                            local_time, p_threshold, app_threshold);
          }
        else if( boxm2_data_type == boxm2_data_traits<BOXM2_GAUSS_RGB_VIEW_COMPACT>::prefix() &&
                 data_type == bstm_data_traits<BSTM_GAUSS_RGB_VIEW_COMPACT>::prefix()  )
          {
          bstm_ingest_boxm2_scene_function<BSTM_GAUSS_RGB_VIEW_COMPACT, BOXM2_GAUSS_RGB_VIEW_COMPACT>(blk, blk_t, datas,
                                                                                                      boxm2_blk,
                                                                                                      boxm2_datas,
                                                                                                      local_time,
                                                                                                      p_threshold,
          ...
          {
          vcl_cout << "bstm_cpp_ingest_boxm2_scene_process ERROR! appearance models do not match, boxm2_data_type: "
                   << boxm2_data_type << " bstm_data_type: " <<  data_type << vcl_endl;
          }
        }
      }
    }
