...
bool bstm_cpp_refine_space_process(bprb_func_process& pro)
{
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    bstm_data_base * change =
      cache->get_data_base(id, bstm_data_traits<BSTM_CHANGE>::prefix(),
                           bstm_data_traits<BSTM_CHANGE>::datasize() * tree_buffer_len );
    ...
    if( app_data_type == bstm_data_traits<BSTM_MOG3_GREY>::prefix() &&
      ...
      {
      bstm_refine_blk_in_space_function<BSTM_MOG6_VIEW_COMPACT, BSTM_NUM_OBS_VIEW_COMPACT>( blk_t, blk, datas,
                                                                                            change_prob_t);
      }
