...
bool bstm_cpp_refine_spacetime_process(bprb_func_process& pro)
{
  ...
  if( !foundAppDataType || !foundNumobsDataType )
    ...
    vcl_cout
      <<
    "BSTM_CPP_REFINE_SPACETIME_PROCESS_ERROR: scene doesn't have BSTM_MOG6_VIEW_COMPACT or BSTM_NUM_OBS_VIEW_COMPACT data type"
      << vcl_endl;
    return false;
    }
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    bstm_data_base * mog     = cache->get_data_base(id,
                                                    bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::prefix(),
                                                    bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::datasize() * num_el);
    bstm_data_base * num_obs = cache->get_data_base(id,
                                                    bstm_data_traits<BSTM_NUM_OBS_VIEW_COMPACT>::prefix(),
                                                    bstm_data_traits<BSTM_NUM_OBS_VIEW_COMPACT>::datasize() * num_el );
