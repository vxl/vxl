...
bool bstm_cpp_refine_tt_process(bprb_func_process& pro)
{
  ...
  std::cout<<"Refining in time..."<<std::endl;
  ...
  for (blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter)
  {
    ...
    if(app_data_type == bstm_data_traits<BSTM_MOG3_GREY>::prefix() &&  nobs_data_type == bstm_data_traits<BSTM_NUM_OBS>::prefix()  )
      ...
      bstm_refine_blk_in_time_function<BSTM_MOG6_VIEW_COMPACT, BSTM_NUM_OBS_VIEW_COMPACT> ( blk_t, blk, datas, change_prob_t, local_time);
    else if (app_data_type == bstm_data_traits<BSTM_GAUSS_RGB>::prefix() &&  nobs_data_type == bstm_data_traits<BSTM_NUM_OBS_SINGLE>::prefix()  )
      bstm_refine_blk_in_time_function<BSTM_GAUSS_RGB, BSTM_NUM_OBS_SINGLE> ( blk_t, blk, datas, change_prob_t, local_time);
    else {
