// This is brl/bseg/bstm/cpp/pro/processes/bstm_cpp_refine_tt_process.cxx
...
#include <bstm/cpp/algo/bstm_refine_blk_in_time_function.h>
...
{
  using namespace bstm_cpp_refine_tt_process_globals;
...
bool bstm_cpp_refine_tt_process(bprb_func_process& pro)
{
  ...
  if( !bstm_util::verify_appearance( *scene, valid_types, app_data_type, apptypesize ) )
    ...
    vcl_cout
      <<
    "bstm_cpp_refine_space_process ERROR: scene doesn't have BSTM_MOG6_VIEW_COMPACT or BSTM_MOG3_GREY or BSTM_GAUSS_RGB data type"
      << vcl_endl;
    return false;
    }
  ...
  if( !bstm_util::verify_appearance( *scene, valid_types, nobs_data_type, nobstypesize ) )
    ...
    vcl_cout
      <<
    "bstm_cpp_refine_space_process ERROR: scene doesn't have BSTM_NUM_OBS or BSTM_NUM_OBS_VIEW_COMPACT or BSTM_NUM_OBS_SINGLE data type"
      << vcl_endl;
    return false;
    }

  vcl_cout << "Refining in time..." << vcl_endl;
  ...
  for( blk_iter = blocks.begin(); blk_iter != blocks.end(); ++blk_iter )
    ...
    if( app_data_type == bstm_data_traits<BSTM_MOG3_GREY>::prefix() &&
        nobs_data_type == bstm_data_traits<BSTM_NUM_OBS>::prefix()  )
      {
      bstm_refine_blk_in_time_function<BSTM_MOG3_GREY, BSTM_NUM_OBS>( blk_t, blk, datas, change_prob_t, local_time);
      }
    else if( app_data_type == bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::prefix() &&
             nobs_data_type == bstm_data_traits<BSTM_NUM_OBS_VIEW_COMPACT>::prefix()  )
      {
      bstm_refine_blk_in_time_function<BSTM_MOG6_VIEW_COMPACT, BSTM_NUM_OBS_VIEW_COMPACT>( blk_t, blk, datas,
                                                                                           change_prob_t, local_time);
      }
    else if( app_data_type == bstm_data_traits<BSTM_GAUSS_RGB>::prefix() &&
             nobs_data_type == bstm_data_traits<BSTM_NUM_OBS_SINGLE>::prefix()  )
      {
      bstm_refine_blk_in_time_function<BSTM_GAUSS_RGB, BSTM_NUM_OBS_SINGLE>( blk_t, blk, datas, change_prob_t,
                                                                             local_time);
      }
    else
      {
