...
void bstm_ocl_annealed_particle_filter::track()
{
  for( unsigned t = start_t_ + 1; t <= end_t_; t++ )
    ...
    for( unsigned m = num_annealing_layers_; m > 0; m-- )
      ...
      vcl_cerr << "SURVIVAL DIAGNOSTIC: " << bstm_ocl_particle_filter::survival_diagnostic(t) << " BETA: " << beta
               << vcl_endl;
...
void bstm_ocl_annealed_particle_filter::dump_particles(unsigned cur_time, unsigned cascade)
{
  ...
  for( unsigned particle_no = 0; particle_no < num_particles_; particle_no++ )
    {
#ifndef DUMP_BOX
    myfile << bb[particle_no].centroid().x() << " " << bb[particle_no].centroid().y() << " "
           << bb[particle_no].centroid().z() << " " <<  mi_[cur_time - start_t_][particle_no] << vcl_endl;
#else
    vcl_vector<vgl_point_3d<double> > corners  = bb[particle_no].corners();
    for( int i = 0; i < corners.size(); i++ )
      {
      myfile << corners[i].x() << " " << corners[i].y() << " " << corners[i].z() << " "
             <<  mi_[cur_time - start_t_][particle_no] << vcl_endl;
      }
#endif
    }
  myfile.close();
...
void bstm_ocl_annealed_particle_filter::perturb_particles(unsigned prev_time, unsigned cur_time, unsigned m)
{
  ...
  for( unsigned particle_no = 0; particle_no < num_particles_; particle_no++ )
    ...
    vnl_vector_fixed<double, 3> t(rand_.drand32(-t_sigma_, t_sigma_), rand_.drand32(-t_sigma_, t_sigma_), rand_.drand32(
                                    -t_sigma_, t_sigma_) );
    ...
    new_T[particle_no] = inf_rot * new_infrot[particle_no].transpose()  * (new_T[particle_no] - new_inft[particle_no])
      + new_inft[particle_no] + inf_t;
    ...
    new_bb[particle_no] = vgl_orient_box_3d<double>( box_min_pt, box_min_pt_plus_width, box_min_pt_plus_depth,
...
float bstm_ocl_annealed_particle_filter::survival_diagnostic(unsigned cur_time, double beta)
{
  ...
  for( unsigned particle_no = 0; particle_no < num_particles_; particle_no++ )
    ...
    if( mi_[cur_time - start_t_][particle_no] > 0.0f )
      ...
      sum_weights_sq +=
        vcl_pow(mi_[cur_time - start_t_][particle_no], beta) * vcl_pow(mi_[cur_time - start_t_][particle_no], beta);
      }
    }
