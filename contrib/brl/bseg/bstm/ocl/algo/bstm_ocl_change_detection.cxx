                                               ...
                                               float                     time)
{
  ...
  if( !isColor )
    ...
    vil_image_view<vil_rgba<vxl_byte> >* img_view =
      static_cast<vil_image_view<vil_rgba<vxl_byte> > *>(float_img.ptr() );
  ...
  bocl_mem_sptr mask_image =
    opencl_cache->alloc_mem(cl_ni * cl_nj * sizeof(char), mask_image_buff, "mask image buffer");
                                             ...
                                             float                     time)
{
  ...
  if( !bstm_util::verify_appearance( *scene, valid_types, data_type, apptypesize ) )
    {
    vcl_cout
      << "bstm_ocl_render_gl_expected_image_process ERROR: scene doesn't have correct appearance model data type"
      << vcl_endl;
    return false;
    }
  vcl_string options = bstm_ocl_util::mog_options(data_type);
...
vcl_vector<bocl_kernel *> & bstm_ocl_aux_pass_change::get_kernels(bocl_device_sptr device, vcl_string opts)
{
  ...
  // pass one, seglen
  vcl_string seg_options = opts
    + "  -D CHANGE_SEGLEN -D STEP_CELL=step_cell_seglen(aux_args,data_ptr,data_ptr_tt,d) ";
                                           ...
                                           float                    time)
{
  ...
  if( !bstm_util::verify_appearance( *scene, valid_types, data_type, apptypesize ) )
    {
    vcl_cout << "bstm_ocl_update_change_process ERROR: scene doesn't have correct appearance model data type"
             << vcl_endl;
    return false;
    }
...
//
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    bocl_mem *       cum_seglen = opencl_cache->get_data<BSTM_AUX0>(*id,
                                                                    num_time_trees * bstm_data_traits<BSTM_AUX0>::datasize(),
                                                                    false);
    bocl_mem * cum_vis   = opencl_cache->get_data<BSTM_AUX1>(*id,
                                                             num_time_trees * bstm_data_traits<BSTM_AUX1>::datasize(),
                                                             false);
    bocl_mem * cum_change = opencl_cache->get_data<BSTM_AUX2>(*id,
                                                              num_time_trees * bstm_data_traits<BSTM_AUX2>::datasize(),
                                                              false);
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    bocl_mem * cum_vis   = opencl_cache->get_data<BSTM_AUX1>(*id);
    bocl_mem * cum_change = opencl_cache->get_data<BSTM_AUX2>(*id);
    bocl_mem * change_prob   = opencl_cache->get_data<BSTM_CHANGE>(*id,
                                                                   info_buffer_t->tree_buffer_length
                                                                   * bstm_data_traits<BSTM_CHANGE>::datasize(), false);
  ...
  for( id = vis_order.begin(); id != vis_order.end(); ++id )
    ...
    bocl_mem * cum_vis   = opencl_cache->get_data<BSTM_AUX1>(*id);
    bocl_mem * cum_change = opencl_cache->get_data<BSTM_AUX2>(*id);
    bocl_mem * change_array   = opencl_cache->get_data(*id,
                                                       bstm_data_traits<BSTM_CHANGE>::prefix(),
                                                       info_buffer_t->tree_buffer_length * bstm_data_traits<BSTM_CHANGE>::datasize(),
                                                       false);
    bocl_mem * change_ll   = opencl_cache->get_data(*id, bstm_data_traits<BSTM_CHANGE>::prefix(
                                                      "pos"),
                                                    info_buffer_t->tree_buffer_length * bstm_data_traits<BSTM_CHANGE>::datasize(),
                                                    false);
    bocl_mem * no_change_ll   = opencl_cache->get_data(*id, bstm_data_traits<BSTM_CHANGE>::prefix(
                                                         "neg"),
                                                       info_buffer_t->tree_buffer_length * bstm_data_traits<BSTM_CHANGE>::datasize(),
                                                       false);
