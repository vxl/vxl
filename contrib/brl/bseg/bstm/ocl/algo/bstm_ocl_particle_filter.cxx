...
bstm_ocl_particle_filter::bstm_ocl_particle_filter(bocl_device_sptr device, bstm_scene_sptr scene,
                                                   bstm_cache_sptr cache, bstm_opencl_cache_sptr opencl_cache,
                                                   unsigned start_t, unsigned end_t, vgl_box_3d<double> initial_bb,
                                                   int num_particles, double t_sigma, double w_sigma,
                                                   vcl_string kernel_opt, int nbins, int track_label, double radius ) :
  device_(device), scene_(scene), cache_(cache), opencl_cache_(opencl_cache), num_particles_(num_particles),
  initial_bb_(initial_bb),
    ...
    */
    ...
    for( unsigned view_dir_num = 0; view_dir_num < app_view_dir_num_; view_dir_num++ )
      ...
      for( int i = 0; i < app_nbins_ * app_nbins_; i++ )
        {
        sum_app_joint_hist +=
          app_joint_histogram_buff_[app_view_dir_num_ * app_nbins_ * app_nbins_ * particle_no
                                    + (app_nbins_ * app_nbins_ * view_dir_num) + i];
        }
      ...
      for( int i = 0; i < app_nbins_ * app_nbins_; i++ )
        {
        app_hist_[(app_nbins_ * app_nbins_ * view_dir_num)
                  + i] =
          app_joint_histogram_buff_[app_view_dir_num_ * app_nbins_ * app_nbins_ * particle_no
                                    + (app_nbins_ * app_nbins_ * view_dir_num) + i] / sum_app_joint_hist;
        }
...
void bstm_ocl_particle_filter::propagate_particles(unsigned prev_time, unsigned cur_time)
{
  ...
  for( unsigned particle_no = 0; particle_no < num_particles_; particle_no++ )
    ...
    vnl_vector_fixed<double, 3> t( rand_.drand32(-t_sigma_, t_sigma_), rand_.drand32(-t_sigma_,
                                                                                     t_sigma_), rand_.drand32(-t_sigma_,
                                                                                                              t_sigma_) );
    ...
    */
    ...
    vgl_orient_box_3d<double> new_box( box_min_pt, box_min_pt_plus_width, box_min_pt_plus_depth,
                                       box_min_pt_plus_height );
...
void bstm_ocl_particle_filter::eval_obs_density(unsigned prev_time, unsigned cur_time)
{
  ...
  queue_ = clCreateCommandQueue(device_->context(),
                                *(device_->device_id() ), CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, &status);
  if( status != 0 )
    {
  ...
  vcl_cerr << "FINISHED EVALUATING OBS DENSITY...gpu time: " <<  gpu_time_ / (double)1000 << " seconds. Real time: "
           << timer.real() / double(1000) << " seconds." << vcl_endl;
...
vgl_box_3d<double> bstm_ocl_particle_filter::w_mean_bb( vcl_vector<vgl_orient_box_3d<double> > bb,
                                                        vcl_vector<double> weights)
{
  ...
  vcl_cerr << "MEAN BB: " << weighted_box.centroid() << " dim " << weighted_box.width() << " "
           << weighted_box.height() << " " << weighted_box.depth() << vcl_endl;
...
vcl_vector<double> bstm_ocl_particle_filter::eval_mi(unsigned prev_time, unsigned cur_time)
{
  ...
  for( iter = global_blk_map_.begin(); iter != global_blk_map_.end(); iter++ )
    ...
    global_threads[0] = (unsigned) RoundUp(
        mdata.sub_block_num_.x() * mdata.sub_block_num_.y() * mdata.sub_block_num_.z(), (int)local_threads[0]);
    ...
    for( unsigned particle_no = 0; particle_no < num_particles_; particle_no++ )
      ...
      vcl_vector<bstm_block_id> particle_target_blks =
        blk_map_[cur_time  - start_t_][particle_no][relevant_blk_id];
      vcl_vector<bstm_block_id>::iterator found = vcl_find(particle_target_blks.begin(),
                                                           particle_target_blks.end(), target_blk_id);
...
void bstm_ocl_particle_filter::init_ocl_minfo()
{
  ...
  // target blk's origin
  target_blk_origin_ = new bocl_mem(
      device_->context(), target_blk_origin_buff, 4 * sizeof(cl_float), " scene origin buff" );
  target_blk_origin_->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR );

  // target blk's sub blk num
  target_subblk_num_ = new bocl_mem(
      device_->context(), target_subblk_num_buff, sizeof(cl_int) * 4, " scene sub block num" );
  ...
  for( unsigned particle_no = 0; particle_no < num_particles_; particle_no++ )
    ...
    particle_no_mems_.push_back( new bocl_mem(device_->context(), &(particle_nos_[particle_no]), sizeof(cl_uint),
                                              " particle number" ) );
  ...
  // histograms
  app_joint_hist_mem_  = new bocl_mem(
      device_->context(), app_joint_histogram_buff_, sizeof(cl_uint) * app_nbins_ * app_nbins_ * app_view_dir_num_ * num_particles_,
      " app joint histogram" );
  app_joint_hist_mem_->create_buffer(CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR );

  surf_joint_hist_mem_  = new bocl_mem(
      device_->context(), surf_joint_histogram_buff_, sizeof(cl_float) * 4 * num_particles_, " surf joint histogram" );
...
vgl_orient_box_3d<double> bstm_ocl_particle_filter::best_bb( vcl_vector<vgl_orient_box_3d<double> > bb,
                                                             vcl_vector<double> weights,
                                                             vcl_vector<vgl_vector_3d<double> > T,
                                                             vcl_vector<vgl_rotation_3d<double> > R)
{
  float best_score = 0;
  ...
  vcl_cerr << "BEST BB " << best_bb.centroid() << " dim " << best_bb.width() << " " << best_bb.height() << " "
           << best_bb.depth() << " has MI: " << best_score
           << " and T: " << best_t << " and R: " << best_r.as_euler_angles() << vcl_endl;
  return best_bb;
}

vgl_rotation_3d<double> bstm_ocl_particle_filter::mean_rot( vcl_vector<vgl_rotation_3d<double> > rot,
                                                            vcl_vector<double> weights )
{
...
void bstm_ocl_particle_filter::label(unsigned t)
{
  ...
  if( t  == start_t_ )
    ...
    vcl_cout << "MEAN T: " << mean_T << " and mean R: " << mean_R.as_euler_angles()[0] << " "
             << mean_R.as_euler_angles()[1] << " " << mean_R.as_euler_angles()[2] << vcl_endl;
  ...
  for( vcl_vector<vgl_point_3d<double> >::const_iterator pt_iter = corners.begin(); pt_iter != corners.end();
       pt_iter++ )
    {
    aabb.add(*pt_iter);
    }
  vcl_cout << "AABB: " << aabb.centroid() << " dim " << aabb.width() << " " << aabb.height() << " " << aabb.depth()
           << vcl_endl;
  ...
  for( ; bstm_iter != blocks.end(); bstm_iter++ )
    ...
    if( !vgl_intersection<double>( bstm_metadata.bbox(), aabb).is_empty() ) // if the two boxes intersect
      ...
      if( bstm_metadata.contains_t(t, local_time) ) // if the block box contains the given time
        ...
        bstm_data_base * label_data_base = cache_->get_data_base(bstm_metadata.id_,
                                                                 bstm_data_traits<BSTM_LABEL>::prefix(),
                                                                 alph->buffer_length() / bstm_data_traits<BSTM_ALPHA>::datasize()
                                                                 * bstm_data_traits<BSTM_LABEL>::datasize() );

        bstm_data_traits<BSTM_ALPHA>::datatype * alpha_data =
          (bstm_data_traits<BSTM_ALPHA>::datatype *)alph->data_buffer();
        bstm_data_traits<BSTM_LABEL>::datatype * label_data =
          (bstm_data_traits<BSTM_LABEL>::datatype *)label_data_base->data_buffer();
...
void bstm_ocl_particle_filter::populate_blk_mapping(vcl_map<bstm_block_id, vcl_vector<bstm_block_id> > & mapping,
                                                    const vgl_box_3d<double>& aabb,
                                                    const vgl_rotation_3d<double>& r, const  vgl_vector_3d<double>& t,
                                                    unsigned prev_time, unsigned curr_time)
{
  ...
  for( iter = relevant_blocks.begin(); iter != relevant_blocks.end(); iter++ )
    ...
    vcl_vector<bstm_block_id> query_relevant_blocks = scene_->get_block_ids(aa_intersection_box,
                                                                            curr_time);
    vcl_vector<bstm_block_id>::const_iterator iter_query_blks;
    for( iter_query_blks = query_relevant_blocks.begin(); iter_query_blks != query_relevant_blocks.end();
         iter_query_blks++ )
      {
