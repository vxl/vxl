...
{
public:
  bstm_ocl_particle_filter(bocl_device_sptr device, bstm_scene_sptr scene, bstm_cache_sptr cache,
                           bstm_opencl_cache_sptr opencl_cache, unsigned start_t, unsigned end_t,
                           vgl_box_3d<double> initial_bb, int num_particles, double t_sigma, double w_sigma,
                           vcl_string kernel_opt, int nbins, int label, double radius = 0 );

  virtual void track();

  ~bstm_ocl_particle_filter()
  {
    delete[] app_joint_histogram_buff_; delete[] app_hist_; delete[] app_histA_; delete[] app_histB_;
...
protected:
  ...
  // helper
  void populate_blk_mapping(vcl_map<bstm_block_id, vcl_vector<bstm_block_id> > & mapping,
                            const vgl_box_3d<double>& aabb, const vgl_rotation_3d<double>& r,
  ...
  vgl_orient_box_3d<double> best_bb( vcl_vector<vgl_orient_box_3d<double> > bb,  vcl_vector<double> weights,
                                     vcl_vector<vgl_vector_3d<double> > T, vcl_vector<vgl_rotation_3d<double> > R);
