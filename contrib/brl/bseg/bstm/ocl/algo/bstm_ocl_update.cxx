                             ...
                             vil_image_view_base_sptr mask_sptr)
{
  ...
  bocl_mem_sptr up_changes_mem = new bocl_mem(
      device->context(), up_changes_only, sizeof(up_changes_only), "update changes bool buffer");
...
vcl_vector<bocl_kernel *> & bstm_ocl_update::get_kernels(bocl_device_sptr device, vcl_string opts, bool isRGB)
{
  ...
  vcl_cout << "=== bstm_ocl_update::compiling kernels on device " << identifier << "===" << vcl_endl;
                                           ...
                                           vcl_string& options)
{
  ...
  for( unsigned int i = 0; i < apps.size(); ++i )
    ...
    else if( apps[i] == bstm_data_traits<BSTM_NUM_OBS>::prefix() )
      {
      num_obs_type = apps[i];
  ...
  if( !foundDataType )
    {
    vcl_cout << "BSTM_OPENCL_VIEW_BASED_UPDATE_PROCESS ERROR: scene doesn't have BSTM_MOG3_GREY data type" << vcl_endl;
    return false;
    }
  if( !foundNumObsType )
    {
    vcl_cout << "BSTM_OPENCL_VIEW_BASED_UPDATE_PROCESS ERROR: scene doesn't have BSTM_NUM_OBS data type" << vcl_endl;
    return false;
    }
  return true;
}
