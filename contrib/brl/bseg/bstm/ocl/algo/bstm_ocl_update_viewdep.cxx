...
std::map<std::string,std::vector<bocl_kernel*> > bstm_ocl_update_viewdep::kernels_;

//Main public method, updates color model
bool bstm_ocl_update_viewdep::update(const bstm_scene_sptr&         scene,
...
std::vector<bocl_kernel*>& bstm_ocl_update_viewdep::get_kernels(const bocl_device_sptr& device, const std::string& opts, bool  /*isRGB*/)
{
  ...
  std::cout<<"=== bstm_ocl_update_viewdep::compiling kernels on device "<<identifier<<"==="<<std::endl;
...
bool bstm_ocl_update_viewdep::validate_appearances(const bstm_scene_sptr& scene,
                                            ...
                                            std::string& options)
{
  ...
  if (!foundDataType) {
    std::cout<<"BSTM_OPENCL_VIEW_BASED_UPDATE_PROCESS ERROR: scene doesn't have BSTM_MOG6_VIEW or BSTM_MOG6_VIEW_COMPACT data type"<<std::endl;
    return false;
  }
  if (!foundNumObsType) {
    std::cout<<"BSTM_OPENCL_VIEW_BASED_UPDATE_PROCESS ERROR: scene doesn't have BSTM_NUM_OBS_VIEW or BSTM_NUM_OBS_VIEW_COMPACT type"<<std::endl;
    return false;
  }
  return true;
}
