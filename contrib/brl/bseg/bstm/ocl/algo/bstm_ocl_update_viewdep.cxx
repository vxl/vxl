  ...
  for( unsigned int i = 0; i < kernels.size(); ++i )
    ...
    for( id = vis_order.begin(); id != vis_order.end(); ++id )
      ...
      if( i == UPDATE_SEGLEN )
        ...
        bocl_mem * change   = opencl_cache->get_data<BSTM_CHANGE>(*id,
                                                                  num_time_trees * bstm_data_traits<BSTM_CHANGE>::datasize(),
                                                                  false);
                                                   ...
                                                   vcl_string& options)
{
  ...
  if( !foundDataType )
    ...
    vcl_cout
      <<
    "BSTM_OPENCL_VIEW_BASED_UPDATE_PROCESS ERROR: scene doesn't have BSTM_MOG6_VIEW or BSTM_MOG6_VIEW_COMPACT data type"
      << vcl_endl;
    return false;
    }
  if( !foundNumObsType )
    ...
    vcl_cout
      <<
    "BSTM_OPENCL_VIEW_BASED_UPDATE_PROCESS ERROR: scene doesn't have BSTM_NUM_OBS_VIEW or BSTM_NUM_OBS_VIEW_COMPACT type"
      << vcl_endl;
    return false;
    }
  return true;
