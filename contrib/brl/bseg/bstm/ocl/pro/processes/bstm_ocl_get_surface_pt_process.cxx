...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  // set kernel options
  vcl_string options = opts + "-D SURFACE_PT ";
  options +=
    " -D STEP_CELL=step_cell_surface_pt(aux_args,data_ptr_tt,d*linfo->block_len,posx*linfo->block_len+linfo->origin.x,posy*linfo->block_len+linfo->origin.y,posz*linfo->block_len+linfo->origin.z)";
...
bool bstm_ocl_get_surface_pt_process(bprb_func_process& pro)
{
  ...
  vcl_cout << "prob: " << output_arr[0] << " pt: (" <<  output_arr[1] << "," << output_arr[2] << "," << output_arr[3]
           << ")" << vcl_endl;
