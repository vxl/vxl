...
bocl_kernel * compile_kernel(bocl_device_sptr device, vcl_string opts)
{
  ...
  compute_vis->create_kernel(&device->context(), device->device_id(), src_paths, "label_scene", seg_opts,
                             "label_scene");
  return compute_vis;
}
...
bool bstm_ocl_label_along_ray_process(bprb_func_process& pro)
{
  ...
  bocl_mem_sptr change_prob_t_mem = new bocl_mem(
      device->context(), &cl_change_prob_t, sizeof(cl_change_prob_t), "change probability threshold buffer");
