...
void compile_kernel(bocl_device_sptr device, vcl_vector<bocl_kernel *> & vec_kernels, vcl_string opts)
{
  ...
  options +=
    "-D RENDER_LAMBERT -D RENDER_CHANGE -D STEP_CELL=step_cell_render_change(aux_args,data_ptr,data_ptr_tt,d*linfo->block_len)";
...
bool bstm_ocl_render_expected_change_process(bprb_func_process& pro)
{
  ...
  bocl_mem_sptr exp_image =
    new bocl_mem(device->context(), buff,  4 * cl_ni * cl_nj * sizeof(float), "exp image buffer");
