...
bool bstm_ocl_update_process(bprb_func_process& pro)
{
  ...
  if( scene->has_data_type(bstm_data_traits<BSTM_MOG6_VIEW>::prefix() ) ||
      scene->has_data_type(bstm_data_traits<BSTM_MOG6_VIEW_COMPACT>::prefix() ) )
    {
    return bstm_ocl_update_viewdep::update(scene, device, opencl_cache, cam, img, time, mog_var, mask_img,
                                           update_alpha, update_changes_ony);
    }
  else
    {
    return bstm_ocl_update::update(scene, device, opencl_cache, cam, img, time, mog_var,  update_alpha,
                                   update_changes_ony, mask_img);
    }
}
