        ...
        for( unsigned int y = 0; y < trees.get_row2_count(); ++y )
          ...
          for( unsigned int z = 0; z < trees.get_row3_count(); ++z )
            ...
            vgl_box_3d<double> tree_box(tree_min_pt, bstm_metadata.sub_block_dim_.x(),
                                        ...
                                        bstm_metadata.sub_block_dim_.z(), vgl_box_3d<double>::min_pos);
            if( !vgl_intersection<double>(tree_box, box).is_empty() ) // if the tree intersects the box
              ...
              for( iter = leafBits.begin(); iter != leafBits.end(); ++iter )
                ...
                vgl_point_3d<double> cellCenter_global(
                  float(cellCenter.x() * bstm_metadata.sub_block_dim_.x() + bstm_metadata.local_origin_.x() ),
                  float(cellCenter.y() * bstm_metadata.sub_block_dim_.y() + bstm_metadata.local_origin_.y() ),
                  float(cellCenter.z() * bstm_metadata.sub_block_dim_.z()
                        + bstm_metadata.local_origin_.z() ) );
                vgl_box_3d<double> cell_box(cellCenter_global,
                                            bstm_metadata.sub_block_dim_.x() * side_len,
                                            bstm_metadata.sub_block_dim_.y() * side_len,
                                            bstm_metadata.sub_block_dim_.z() * side_len,
                ...
                if( !vgl_intersection<double>(cell_box, box).is_empty() )
                  ...
                  int data_offset =
                    bit_tree.get_data_index(currBitIndex);                // mdata index
                  boxm2_array_1d<vnl_vector_fixed<unsigned char,
                                                  8> > time_treebits = blk_t->get_cell_all_tt(data_offset);
                  for( vnl_vector_fixed<unsigned char, 8>* time_tree_iter = time_treebits.begin();
                       time_tree_iter != time_treebits.end(); time_tree_iter++ )
                    {
