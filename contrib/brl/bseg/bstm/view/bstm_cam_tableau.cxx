...
bstm_cam_tableau::bstm_cam_tableau() :
...
void bstm_cam_tableau::setup_gl_matrices()
{
  GLint vp[4]; // x,y,w,h
...
bool bstm_cam_tableau::handle(vgui_event const &e)
{
  ...
  else if (e.type == vgui_IDLE)
  {
    if(render_trajectory_) {
      ...
      std::cout << "I also received " << e.type <<std::endl;
      return false;
    }
  }
...
bool bstm_cam_tableau::mouse_down(int x, int y, vgui_button /*button*/, vgui_modifier /*modifier*/)
{
...
bool bstm_cam_tableau::mouse_drag(int x, int y, vgui_button button, vgui_modifier modifier)
{
  ...
  if (c_mouse_rotate(button, modifier)) {
    ...
    double theta = std::acos((cam_center.z()-origin.z())/rad);
    double phi = std::atan2((cam_center.y()-origin.y()), (cam_center.x()-origin.x()));

    //update theta by a function of dy
    double angleScale = .1;
    double newTheta = theta - dy * angleScale;
    double newPhi = phi + dx * angleScale;
    vgl_point_3d<double> newCenter(origin.x()+rad * std::sin(newTheta) * std::cos(newPhi),
                                   origin.y()+rad * std::sin(newTheta) * std::sin(newPhi),
                                   origin.z()+rad * std::cos(newTheta));
    cam_.set_camera_center(newCenter);
    std::cout<<"New center"<<newCenter<<" old center "<<cam_center<<std::endl;
    cam_.look_at(stare_point_);
...
bool bstm_cam_tableau::mouse_up(int x, int y, vgui_button button, vgui_modifier modifier)
{
  // SPINNING
  if (c_mouse_rotate(button, modifier))
  {
