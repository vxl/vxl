...
void bstm_ocl_multi_render_tableau::compute_gpu_assignments()
{
  ...
  for( int scene_blk_id = 0; scene_blk_id < gpu_ids_.size(); scene_blk_id++ )
    {
    vcl_cout << "Is " << (float)scene_blk_id / gpu_ids_.size() << " smaller than "
             << (float)(curr_mem_size  + opencl_caches_[curr_gpu_id]->max_memory_in_cache() ) / total_gpu_mem_
             << vcl_endl;
    if( (float)scene_blk_id / gpu_ids_.size() <
        (float)(curr_mem_size  + opencl_caches_[curr_gpu_id]->max_memory_in_cache() ) / total_gpu_mem_ )
      {
...
bool bstm_ocl_multi_render_tableau::init_clgl()
{
  for( int gpu_no = 0; gpu_no < num_gpus_; gpu_no++ )
    ...
    int status_queue = 0;
    queues_[gpu_no] =  clCreateCommandQueue(devices_[gpu_no]->context(),
                                            *(devices_[gpu_no]->device_id() ), CL_QUEUE_PROFILING_ENABLE,
                                            &status_queue);
    ...
    clgl_buffer_[gpu_no] = clCreateFromGLBuffer(devices_[gpu_no]->context(),
                                                ...
                                                &status);
    exp_img_[gpu_no] =
      new bocl_mem(devices_[gpu_no]->context(),  NULL, RoundUp(ni_, 8) * RoundUp(nj_,
                                                                                 8) * sizeof(GLubyte) * 4,
                   "exp image (gl) buffer");
