...
public:
  ...
  {
    bvpl_subgrid_iterator<float> ocp_iter(ocp_grid, kernel->dim());
    bvpl_subgrid_iterator<bsta_num_obs<bsta_gauss_f1> > app_iter(app_grid, kernel->dim());
    bvpl_subgrid_iterator<float> output_iter(out_grid, kernel->dim());
