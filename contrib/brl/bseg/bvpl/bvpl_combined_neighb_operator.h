...
{
public:
  bvpl_combined_neighb_operator(const OCP_FUNC ocp_functor, const APP_FUNC app_functor) : ocp_func_(ocp_functor),
    app_func_(app_functor) {}
               ...
               bvxm_voxel_grid<float>* out_grid)
  {
    ...
    bvpl_subgrid_iterator<float>                         output_iter(out_grid, kernel->min_point(),
                                                                     kernel->max_point() );
