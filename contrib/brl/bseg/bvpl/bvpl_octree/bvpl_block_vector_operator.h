...
// This class differes from bvpl_octree_kernel_operator in that it loads neighbor block to process blocks appropiatly.
    ...
    for (unsigned k= 0; k< kernel_vector->kernels_.size(); k++)
    {
      ...
      while (!kernel_iter.isDone())
      {
        ...
        if (this_cell) {
          ...
          T_data_out val =(T_data_out)this_cell->data();
          functor.apply(val, d);
        }
        else {
          valid = false;
