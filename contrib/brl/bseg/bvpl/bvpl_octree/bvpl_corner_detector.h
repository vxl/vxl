...
public:
  ...
  void detect_and_threshold( boxm_scene<boct_tree<short, T_data> > * scene_in, F harris_functor,
                             vgl_point_3d<int> min_neigborhood_idx, vgl_point_3d<int> max_neigborhood_idx,
                             unsigned block_i, unsigned block_j, unsigned block_k, boxm_scene<boct_tree<short,
                                                                                                        bool> > * valid_scene, boxm_scene<boct_tree<short, float> > * corner_scene,
                             double cell_length);

  // : Computes the corner measure and stores it in "corner_scene"
  //  Input scene and auxiliary valid scene are not modified
  template <class T_data, class F>
  void compute_C( boxm_scene<boct_tree<short, T_data> > * scene_in, F harris_functor,
                  vgl_point_3d<int> min_neigborhood_idx, vgl_point_3d<int> max_neigborhood_idx, unsigned block_i,
                  unsigned block_j, unsigned block_k, boxm_scene<boct_tree<short, bool> > * valid_scene,
                  boxm_scene<boct_tree<short, float> > * corner_scene, double cell_length);

  // : Compute the determinant of the hessian
  template <class T_data>
  void compute_det_H(boxm_scene<boct_tree<short, T_data> > * scene_in, unsigned block_i, unsigned block_j,
                     unsigned block_k, boxm_scene<boct_tree<short, bool> > * valid_scene, boxm_scene<boct_tree<short,
                                                                                                               float> > * corner_scene);
                                                 ...
                                                 double cell_length)
{
  ...
  for( ; (it_in != cells_in.end() ) && (valid_it != valid_cells.end() ) && (corner_it != corner_cells.end() );
       it_in++, valid_it++, corner_it++ )
    {
    ...
    for( int k = min_neigborhood_idx.z(); k <= max_neigborhood_idx.z(); k++ )
      ...
      for( int j = min_neigborhood_idx.y(); j <= max_neigborhood_idx.y(); j++ )
        ...
        for( int i = min_neigborhood_idx.x(); i <= max_neigborhood_idx.x(); i++ )
          ...
          vgl_point_3d<double> neighbor_cell_centroid(
            center_cell_centroid.x() + ( (double)neighbor_cell_idx.x() ) * cell_length,
            center_cell_centroid.y() + ( (double)neighbor_cell_idx.y() ) * cell_length,
            center_cell_centroid.z() + ( (double)neighbor_cell_idx.z() )
            * cell_length);
