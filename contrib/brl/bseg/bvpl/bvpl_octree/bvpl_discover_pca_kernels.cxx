...
bvpl_discover_pca_kernels::bvpl_discover_pca_kernels(const vcl_string & path)
{
  ...
  if( !has_length || finest_cell_length_ < 0.0 )
    {
    vcl_cout << "Error: Finest cell length read from file: " << finest_cell_length_ << " has length " << has_length
             << vcl_endl;
...
vnl_matrix<double> bvpl_discover_pca_kernels::compute_scatter_matrix( boxm_scene<boct_tree<short, float> > * scene)
{
  ...
  for( it.begin(); !it.end(); ++it )
    ...
    unsigned long                              tree_nsamples =
      (unsigned long)( (float)tree_ncells / scene_ncells * nsamples_);
#ifdef DEBUG
    ...
    for( unsigned i = 0; i < tree_nsamples; ++i )
      ...
      vgl_box_3d<double> roi(finest_cell_length_ * (double)nmin.x(),
                             finest_cell_length_ * (double)nmin.y(), finest_cell_length_ * (double)nmin.z(),
                             finest_cell_length_ * (double)nmax.x(),
                             finest_cell_length_ * (double)nmax.y(), finest_cell_length_ * (double)nmax.z() );
...
void bvpl_discover_pca_kernels::compute_testing_error(vnl_vector<double> & proj_error)
{
  ...
  boxm_scene<boct_tree<short,
                       float> > * scene =
    dynamic_cast<boxm_scene<boct_tree<short, float> > *>(data_scene_base_.as_pointer() );

  compute_mean_feature(scene);

  // iterate through leaves and arrage as vectors.
  boxm_cell_iterator<boct_tree<short, float> > cell_it = scene->cell_iterator(&boxm_scene<boct_tree<short,
                                                                                                    float> >::load_block_and_neighbors,
                                                                              true);
  cell_it.begin();
                                                      ...
                                                      int block_i, int block_j, int block_k)
{
  ...
  // cast the scenes
  boxm_scene<float_tree_type>* data_scene =
    dynamic_cast<boxm_scene<float_tree_type> *>(data_scene_base_.as_pointer() );
  ...
  for( unsigned i = 0; i < data_leaves.size(); ++i )
    ...
    vgl_point_3d<double> min_point_global(centroid.x() + (double)min_point.x() * finest_cell_length_,
                                          centroid.y() + (double)min_point.y() * finest_cell_length_,
                                          centroid.z() + (double)min_point.z() * finest_cell_length_);
    vgl_point_3d<double> max_point_global(centroid.x() + (double)max_point.x() * finest_cell_length_,
                                          centroid.y() + (double)max_point.y() * finest_cell_length_,
                                          centroid.z() + (double)max_point.z() * finest_cell_length_);
