...
public:
  ...
  bvpl_discover_pca_kernels(vgl_box_3d<int> nbbox, unsigned long long nsamples, boxm_scene<boct_tree<short,
                                                                                                     float> > * scene,
                            const vcl_string& path_out = ".", bool use_evd = true) :
  ...
  void compute_testing_error(boxm_scene_base_sptr error_scene_base, unsigned ncomponents, int block_i, int block_j,
                             int block_k);
...
//  friend void *compute_testing_error_thread( void *ptr);
  ...
  void project(boxm_scene<boct_tree<short, bvpl_pca_basis_sample<DIM> > > * proj_scene, int block_i, int block_j,
               int block_k);
