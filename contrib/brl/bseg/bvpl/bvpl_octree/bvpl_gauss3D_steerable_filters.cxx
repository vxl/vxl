...
bool bvpl_gauss3D_steerable_filters::basis_response_at_leaves(boxm_scene<boct_tree<short, vnl_vector_fixed<float,
                                                                                                           bvpl_gauss3D_steerable_filters::DIM_> > > * scene,
                                                              ...
                                                              double cell_length)
{
  ...
  boxm_scene<tree_type> * temp_scene =
    new boxm_scene<tree_type>(scene->lvcs(), scene->origin(), scene->block_dim(), scene->world_dim(),
                              scene->max_level(), scene->init_level() );
  ...
  for( unsigned tap_i = 0; tap_i < max_ntaps; tap_i++ )
    ...
    boxm_cell_iterator<boct_tree<short,
                                 bool> > valid_cell_it = valid_scene->cell_iterator(&boxm_scene<boct_tree<short,
                                                                                                          bool> >::load_block);
    valid_cell_it.begin();
...
bool bvpl_gauss3D_steerable_filters::multiscale_basis_response(boxm_scene<boct_tree<short, vnl_vector_fixed<float,
                                                                                                            bvpl_gauss3D_steerable_filters::DIM_> > > * scene,
                                                               boxm_scene<boct_tree<short, bool> > * valid_scene,
...
//  "scene" should be initialized with a dim-dimensional vector of the real input scene.
//  the taps have size 5, this could be changed later on
bool bvpl_gauss3D_steerable_filters::basis_response(boxm_scene<boct_tree<short, vnl_vector_fixed<float,
                                                                                                 bvpl_gauss3D_steerable_filters::DIM_> > > * scene,
                                                    ...
                                                    double cell_length)
{
  ...
  for( unsigned tap_i = 0; tap_i < max_ntaps; tap_i++ )
    ...
    for( ; (cells_it != cells.end() ) && (temp_it != temp_cells.end() ) && (valid_it != valid_cells.end() );
         cells_it++, valid_it++, temp_it++ )
      {
...
bool bvpl_gauss3D_steerable_filters::rotation_invariant_interpolation(boxm_scene<boct_tree<short,
