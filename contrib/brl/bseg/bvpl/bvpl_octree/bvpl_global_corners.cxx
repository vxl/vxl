...
void bvpl_global_corners::compute_laptev_corners(bvpl_global_taylor_sptr global_taylor, int scene_id, int block_i,
                                                 int block_j, int block_k)
{
  ...
  boxm_scene<taylor_tree_type>* proj_scene =
    dynamic_cast<boxm_scene<taylor_tree_type> *>(proj_scene_base.as_pointer() );
  boxm_scene<boct_tree<short,
                       bool> >* valid_scene =
    dynamic_cast<boxm_scene<boct_tree<short, bool> > *>(valid_scene_base.as_pointer() );
  boxm_scene<boct_tree<short, float> >* corner_scene =
    new boxm_scene<boct_tree<short, float> >(valid_scene->lvcs(), valid_scene->origin(),
                                             valid_scene->block_dim(), valid_scene->world_dim(),
                                             valid_scene->max_level(), valid_scene->init_level() );
  ...
  vcl_cout << "Neighborhood for Harris threshold: " << min_neighborhood_idx << " , " << max_neighborhood_idx
           << vcl_endl;

  corner_detector.compute_C(proj_scene, harris_functor, min_neighborhood_idx, max_neighborhood_idx, block_i, block_j,
                            block_k, valid_scene, corner_scene, finest_cell_length_[scene_id]);
...
void bvpl_global_corners::compute_beaudet_corners(bvpl_global_taylor_sptr global_taylor, int scene_id, int block_i,
                                                  int block_j, int block_k)
{
  typedef boct_tree<short, vnl_vector_fixed<double, 10> >      taylor_tree_type;
  typedef boct_tree_cell<short, vnl_vector_fixed<double, 10> > taylor_cell_type;
...
//  I. Laptev. On space-time interest points. Int. J. Computer Vision, 64(2):107--123, 2005
void bvpl_global_corners::threshold_laptev_corners(bvpl_global_taylor_sptr global_taylor, int scene_id,
                                                   float harris_thresh, vcl_string output_path)
{
  typedef boct_tree<short, vnl_vector_fixed<double, 10> >      taylor_tree_type;
  typedef boct_tree_cell<short, vnl_vector_fixed<double, 10> > taylor_cell_type;
  ...
  boxm_scene<boct_tree<short,
                       float> >* corner_scene =
    dynamic_cast<boxm_scene<boct_tree<short, float> > *>(corner_scene_base.as_pointer() );
  ...
  boxm_scene<boct_tree<short, bool> >* valid_corner_scene =
    new boxm_scene<boct_tree<short, bool> >(valid_scene->lvcs(), valid_scene->origin(),
                                            valid_scene->block_dim(), valid_scene->world_dim(),
                                            valid_scene->max_level(), valid_scene->init_level() );
  ...
  boxm_scene<taylor_tree_type>* copy_proj_scene =
    new boxm_scene<taylor_tree_type>(proj_scene->lvcs(), proj_scene->origin(),
                                     proj_scene->block_dim(), proj_scene->world_dim(),
                                     proj_scene->max_level(), proj_scene->init_level() );
  ...
  boxm_cell_iterator<boct_tree<short, bool> > valid_it = valid_scene->cell_iterator(&boxm_scene<boct_tree<short,
                                                                                                          bool> >::load_block,
                                                                                    true);
  valid_it.begin();

  boxm_cell_iterator<boct_tree<short,
                               float> > corners_it = corner_scene->cell_iterator(&boxm_scene<boct_tree<short,
                                                                                                       float> >::load_block,
                                                                                 true);
  corners_it.begin();

  boxm_cell_iterator<boct_tree<short,
                               bool> > valid_corners_it =
    valid_corner_scene->cell_iterator(&boxm_scene<boct_tree<short, bool> >::load_block);
  ...
  while( !(valid_it.end() || corners_it.end() || valid_corners_it.end() ) )
    ...
    if( (valid_cell->level() != corner_cell->level() ) || (valid_cell->level() != valid_corner_cell->level() ) ||
        !(valid_code.isequal(&corner_code) ) || !(valid_code.isequal(&valid_corner_code) ) )
      {
