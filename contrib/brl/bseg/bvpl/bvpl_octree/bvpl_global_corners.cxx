...
void bvpl_global_corners::compute_laptev_corners(bvpl_global_taylor_sptr global_taylor,int scene_id, int block_i, int block_j, int block_k)
{
  ...
  vgl_point_3d<int> max_neigborhood_idx = global_taylor->kernel_vector_->max();
  vgl_point_3d<int> min_neigborhood_idx = global_taylor->kernel_vector_->min();
  
  vcl_cout << "Neigborhood for harris threshhold: " << min_neigborhood_idx << " , " << max_neigborhood_idx << vcl_endl;
  
  corner_detector.compute_C(proj_scene, harris_functor, min_neigborhood_idx, max_neigborhood_idx, block_i, block_j, block_k, valid_scene, corner_scene, finest_cell_length_[scene_id]);
...
void bvpl_global_corners::threshold_laptev_corners(bvpl_global_taylor_sptr global_taylor,int scene_id, float harris_thresh, vcl_string output_path)
{
  ...
  while ( !(valid_it.end() || corners_it.end() || valid_corners_it.end()) )
  {
    ...
    if ((valid_cell->level() != corner_cell->level()) || (valid_cell->level() != valid_corner_cell->level()) || !(valid_code.isequal(&corner_code)) || !(valid_code.isequal(&valid_corner_code))){
      vcl_cerr << " Error in threshold_laptev_corners: Cells don't have the same structure " << vcl_endl;
      return;
    }
