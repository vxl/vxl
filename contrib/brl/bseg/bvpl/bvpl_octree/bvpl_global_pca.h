...
public:
  // : Constructor
  bvpl_global_pca(vcl_vector<vcl_string> const & scenes, vcl_vector<vcl_string> const & aux_dirs,
                  vcl_vector<double> const & finest_cell_length,
                  vgl_box_3d<int> nbbox,  double training_fraction = 0.1, vcl_string path_out = "./") :
    scenes_(scenes), aux_dirs_(aux_dirs), finest_cell_length_(finest_cell_length), nbbox_(nbbox), training_fraction_(
      training_fraction), path_out_(path_out)
  {
  ...
  bool sample_statistics( int scene_id, int block_i, int block_j, int block_k, vnl_matrix_fixed<double, feature_dim,
                                                                                                feature_dim> & S,
                          vnl_vector_fixed<double, feature_dim> & mean, unsigned long & nfeature);
  ...
  static void combine_pairwise_statistics( const vnl_vector_fixed<double, feature_dim>  & mean1,
                                           ...
                                           double const n1, const vnl_vector_fixed<double, feature_dim>  & mean2,
                                           const vnl_matrix_fixed<double, feature_dim, feature_dim>  & scatter2,
                                           double const n2, vnl_vector_fixed<double, feature_dim> & mean_out,
  ...
  void set_up_pca_evd(const vnl_matrix_fixed<double, feature_dim, feature_dim> & S, const vnl_vector_fixed<double,
                                                                                                           feature_dim> & mean,
