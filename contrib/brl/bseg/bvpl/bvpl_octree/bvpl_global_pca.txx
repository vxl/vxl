...
void bvpl_global_pca<feature_dim>::init(unsigned scene_id)
{
  ...
  boxm_scene<boct_tree<short,
                       float> >* data_scene =
    dynamic_cast<boxm_scene<boct_tree<short, float> > *>(data_scene_base.as_pointer() );
  if( !data_scene )
    {
  ...
  if( !(vul_file::exists(aux_dirs_[scene_id]) && vul_file::is_directory(aux_dirs_[scene_id]) ) )
    ...
    if( !vul_file::exists(aux_scene_path) )
      ...
      boxm_scene<boct_tree<short, bool> > * aux_scene =
        new boxm_scene<boct_tree<short, bool> >(data_scene->lvcs(), data_scene->origin(),
                                                data_scene->block_dim(), data_scene->world_dim(),
                                                data_scene->max_level(), data_scene->init_level() );
    ...
    if( !vul_file::exists(proj_scene_path) )
      ...
      boxm_scene<pca_tree_type> * proj_scene =
        new boxm_scene<pca_tree_type>(data_scene->lvcs(), data_scene->origin(),
                                      data_scene->block_dim(), data_scene->world_dim(),
                                      data_scene->max_level(), data_scene->init_level() );
                                                      ...
                                                      unsigned long & nfeature)
{
  ...
  boxm_scene<boct_tree<short,
                       bool> >* mask_scene =
    dynamic_cast<boxm_scene<boct_tree<short, bool> > *>(mask_scene_base.as_pointer() );
  ...
  vcl_cout << " In block (" << block_i << ", " << block_j << ", " << block_k << "), number of nsamples is: "
           << tree_nsamples << ", cell length is: " << cell_length << vcl_endl;
  for( unsigned long i = 0; i < tree_nsamples; i++ )
    ...
    vgl_box_3d<double> roi(cell_length * (double)nmin.x(), cell_length * (double)nmin.y(),
                           cell_length * (double)nmin.z(),
                           cell_length * (double)nmax.x(), cell_length * (double)nmax.y(),
                           cell_length * (double)nmax.z() );
...
void bvpl_global_pca<feature_dim>::combine_pairwise_statistics( const vnl_vector_fixed<double, feature_dim> & mean1,
                                                                const vnl_matrix_fixed<double, feature_dim,
                                                                                       feature_dim> & scatter1,
                                                                double const n1,
                                                                const vnl_vector_fixed<double, feature_dim>  & mean2,
                                                                const  vnl_matrix_fixed<double, feature_dim,
                                                                                        feature_dim> & scatter2,
                                                                double const n2,
                                                                vnl_vector_fixed<double, feature_dim> & mean_out,
                                                                vnl_matrix_fixed<double, feature_dim,
                                                                                 feature_dim> & scatter_out,
                                                                double & n_out )
{
  n_out = n1 + n2;
...
void bvpl_global_pca<feature_dim>::project(int scene_id, int block_i, int block_j, int block_k)
{
  ...
  boxm_scene<boct_tree<short,
    ...
    dynamic_cast<boxm_scene<boct_tree<short, float> > *>(data_scene_base.as_pointer() );
  boxm_scene<pca_tree_type>* proj_scene =
    dynamic_cast<boxm_scene<pca_tree_type> *>(proj_scene_base.as_pointer() );
  boxm_scene<boct_tree<short,
                       bool> >* aux_scene =
    dynamic_cast<boxm_scene<boct_tree<short, bool> > *>(aux_scene_base.as_pointer() );
  ...
  // get the cells for this block
  if( !(data_scene->valid_index(block_i, block_j,
                                block_k) &&
        proj_scene->valid_index(block_i, block_j, block_k) && aux_scene->valid_index(block_i, block_j, block_k) ) )
    {
    vcl_cerr << "In compute_testing_error: Invalid block\n";
    return;
    }
  ...
  for( unsigned long i = 0; i < data_leaves.size(); i++ )
    ...
    vgl_point_3d<double> min_point_global(centroid.x() + (double)nmin.x() * cell_length,
                                          centroid.y() + (double)nmin.y() * cell_length,
                                          centroid.z() + (double)nmin.z() * cell_length);
    vgl_point_3d<double> max_point_global(centroid.x() + (double)nmax.x() * cell_length,
                                          centroid.y() + (double)nmax.y() * cell_length,
                                          centroid.z() + (double)nmax.z() * cell_length);
...
void bvpl_global_pca<feature_dim>::projection_error(int scene_id, int block_i, int block_j, int block_k)
{
  ...
  if( !(data_scene->valid_index(block_i, block_j,
                                block_k) &&
        proj_scene->valid_index(block_i, block_j, block_k) && error_scene->valid_index(block_i, block_j, block_k) ) )
    {
    vcl_cerr << "In compute_testing_error: Invalid block\n";
    return;
    }
...
boxm_scene_base_sptr bvpl_global_pca<feature_dim>::load_error_scene(int scene_id)
{
  ...
  if( !vul_file::exists(aux_scene_path) )
    ...
    boxm_scene<boct_tree<short, float> > * aux_scene =
      new boxm_scene<boct_tree<short, float> >(data_scene->lvcs(), data_scene->origin(),
                                               data_scene->block_dim(), data_scene->world_dim(),
                                               data_scene->max_level(), data_scene->init_level() );
