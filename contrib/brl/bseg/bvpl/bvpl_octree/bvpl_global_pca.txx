                                                      ...
                                                      unsigned long &nfeature)
{
  ...
  if(!(data_scene))
  {
    vcl_cerr << "Error in bvpl_global_pca<feature_dim>::sample_statistics: Could not cast data scenes" << vcl_endl;
    return false;
  }
  if(!(mask_scene))
  {
    vcl_cerr << "Error in bvpl_global_pca<feature_dim>::sample_statistics: Could not cast mask scenes" << vcl_endl;
    return false;
  }
  ...
  if(!(data_scene->valid_index(block_i, block_j, block_k) && mask_scene->valid_index(block_i, block_j, block_k))){
    vcl_cerr << "In compute_testing_error: Invalid block" << vcl_endl;
    return false;
  }
...
void bvpl_global_pca<feature_dim>::project(int scene_id, int block_i, int block_j, int block_k)
{
  ...
  {
    vcl_cerr << "Error in bvpl_global_pca<feature_dim>::sample_statistics: Could not cast input scenes" << vcl_endl;
    return;
  }
    ...
    for(int z = nbbox_.min_z(); z<=nbbox_.max_z(); z++)
      for(int y = nbbox_.min_y(); y<=nbbox_.max_y(); y++)
        for(int x = nbbox_.min_x(); x<=nbbox_.max_x(); x++)
        {
          ...
          if(!neighbor_cell){
            vcl_cerr << "Error in bvpl_global_pca<feature_dim>::project" <<vcl_endl;
            return;
          }
...
boxm_scene_base_sptr bvpl_global_pca<feature_dim>::load_projection_scene (int scene_id)
{
  if(scene_id<0 || scene_id>((int)scenes_.size() -1))
  {
    vcl_cerr << "Error in bvpl_global_pca::load_projection_scene: Invalid scene id" << vcl_endl;
    return NULL;
  }
  //load scene
  ...
  if (proj_scene_base->appearence_model() == VNL_DOUBLE_10){     
    ...
    vcl_cerr << "Error in bvpl_global_pca::load_proj_scene: Invalid apperance model" << vcl_endl;
    return NULL;
  }
...
boxm_scene_base_sptr bvpl_global_pca<feature_dim>::load_error_scene (int scene_id)
{
  ...
  if (error_scene_base->appearence_model() == BVPL_PCA_BASIS_SAMPLE_10){     
    ...
    vcl_cerr << "Error in bvpl_global_pca::load_error_scene: Invalid apperance model" << vcl_endl;
    return NULL;
  }
