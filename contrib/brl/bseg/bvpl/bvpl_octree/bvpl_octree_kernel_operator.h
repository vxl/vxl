// This is bvpl_octree_kernel_operator.h
...
public:
  ...
  void operate(F functor, bvpl_kernel_sptr kernel, tree_type* out_tree, short level, double cell_length)
  {
    ...
    for(unsigned i = 0; i <  cells.size();i++)
    {
      ...
      //:TO DO: check bounds. Is it more efficient to check bounds for every kernel or ro let is run and  store a dummy value in it?
