// This is bvpl_octree_vrml_util.h
                                                 ...
                                                 bvpl_kernel_vector_sptr kernel_vector)
{
  ...
  for (; !iter.end(); iter++) {
    ...
    for(unsigned i = 0; i < cells.size(); i+=10)
    {
      ...
      if(response > 3.5f){
        bvrml_write::write_vrml_line(vrml_os, coord, dir, response/10.0, 1, 0 , 0);
        lines++;
      }
      //bvrml_write::write_vrml_disk(vrml_os, coord, dir, 2*response - 0.5, 0, 0.5, 0);
      
    }
    vcl_cout << "max response " << max_response;

    vcl_cout << "number of lines" << lines<< vcl_endl;
  }
}
#endif
