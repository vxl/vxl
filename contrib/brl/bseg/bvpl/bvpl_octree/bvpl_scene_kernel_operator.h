...
public:
  ...
  void operate(boxm_scene<boct_tree<short, T_data> > & scene_in, F functor, bvpl_kernel_sptr kernel,
               boxm_scene<boct_tree<short, T_data> > & scene_out);

  // : Performs in-situ non-maxima suppression over the area occupied by the kernel
  template <class T_data, class F>
  void local_non_maxima_suppression(boxm_scene<boct_tree<short, T_data> > & scene_in, F functor,
                                         ...
                                         boxm_scene<boct_tree<short, T_data> > & scene_out)
{
  ...
  boxm_cell_iterator<boct_tree<short, T_data> > iterator = scene_in.cell_iterator(&boxm_scene<boct_tree<short,
                                                                                                        T_data> >::load_block_and_neighbors);
  iterator.begin();

  boxm_cell_iterator<boct_tree<short, T_data> > out_iter = scene_out.cell_iterator(&boxm_scene<boct_tree<short,
                                                                                                         T_data> >::load_block);
  out_iter.begin();
