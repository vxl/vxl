// This is bvpl_scene_neighborhood_operator.h
#ifndef bvpl_scene_neighborhood_operator_h
#define bvpl_scene_neighborhood_operator_h

//:
// \file
// \brief A class to perform neihborhood operations on a boxm_scene. 
...
//: A class to perform neihborhood operations on a bvxm_scene. 
...
public:
  ...
  //: Applies the region-based functor on every leaf cell of the scene. The region is obtatained from scen2.
...
//: Applies the region-based functor on every leaf cell of the scene. The output is stored in situ
template <class T_data, class F>
                                                                    ...
                                                                    F functor,vgl_box_3d<double> &roi)
{
  ...
  boxm_cell_iterator<boct_tree<short, T_data > > iterator = scene_in.cell_iterator();
  ...
  while (cell) {
    ...
    //Isa: Determine if the coordinates of this are correct i.e global or local?
    //Return all cells that intersec the region
    //scene_in.cells_in_region(roi,cells_in_roi);
    
    for(unsigned i = 0; i<cells_in_roi.size(); i++)
    {
      boct_tree_cell<short, T_data> *this_cell;
      if(!this_cell)
        continue;
