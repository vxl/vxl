...
public:
  ...
  void operate(boxm_scene<boct_tree<short, T_data_in> > & scene_in, F functor, bvpl_kernel_vector_sptr kernel_vector,
                                         ...
                                         boxm_scene<boct_tree<short, T_data_out> > & scene_out)
{
  ...
  vcl_cout << "bvpl_scene_kernel_operator: Operating on cells at level: " << finest_level << " and length: "
           << cell_length << vcl_endl;
  ...
  boxm_cell_iterator<boct_tree<short,
                               T_data_in> > iterator = scene_in.cell_iterator(&boxm_scene<boct_tree<short,
                                                                                                    T_data_in> >::load_block_and_neighbors);
  iterator.begin();

  boxm_cell_iterator<boct_tree<short,
                               T_data_out> > out_iter = scene_out.cell_iterator(&boxm_scene<boct_tree<short,
                                                                                                      T_data_out> >::load_block);
  out_iter.begin();
...
template <class T_data>
void bvpl_scene_vector_operator::combine_kernel_responses(const vcl_vector<T_data>& responses, vnl_vector_fixed<T_data,
                                                                                                                3> & response_out)
{
  if( responses.size() != 3 )
    {
...
template <class T_data>
void bvpl_scene_vector_operator::combine_kernel_responses(const vcl_vector<T_data>& responses,
                                                          bvpl_octree_sample<T_data> & response_out)
{
  if( responses.size() != 3 )
    {
    vcl_cerr << "Wrong number of responses" << vcl_endl;
