                                                    ...
                                                    int block_i, int block_j, int block_k, double cell_length)
{
  ...
  boxm_scene<taylor_tree_type>* basis_scene =
    dynamic_cast<boxm_scene<taylor_tree_type> *>(basis_scene_base.as_pointer() );
  boxm_scene<float_tree_type> * error_scene =
    dynamic_cast<boxm_scene<float_tree_type> *>(error_scene_base.as_pointer() );
  ...
  for( unsigned i = 0; i < data_leaves.size(); i++ )
    ...
    if( !(data_scene->locate_point_in_memory(min_point_global) &&
          data_scene->locate_point_in_memory(max_point_global) ) )
      {
      continue;
      }
    ...
    for( int x = min_point.x(); x <= max_point.x(); x++ )
      ...
      for( int y = min_point.y(); y <= max_point.y(); y++ )
        ...
        for( int z = min_point.z(); z <= max_point.z(); z++ )
          {
          vgl_point_3d<double> point2visit(centroid.x() + (double)x * cell_length,
...
bool bvpl_taylor_basis::assemble_basis(bvpl_taylor_scenes_map_sptr taylor_scenes, int block_i, int block_j, int block_k)
{
  ...
  vcl_cerr << "Caution: Using hardcoded size for taylor reconstruction\n";

  // load output scene
  boxm_scene<taylor_tree_type>* basis_scene =
    dynamic_cast<boxm_scene<taylor_tree_type> *>(taylor_scenes->get_scene("basis").as_pointer() );
  if( !basis_scene )
    {
    vcl_cerr << "Error in reconstruct: Faild to cast output scene\n";
    return false;
    }

  // retieve basis scenes
  // zeroth derivative scene
  boxm_scene<float_tree_type>* I0_scene =
    dynamic_cast<boxm_scene<float_tree_type> *>(taylor_scenes->get_scene("I0").as_pointer() );
  boxm_scene<float_tree_type>* Ix_scene =
    dynamic_cast<boxm_scene<float_tree_type> *>(taylor_scenes->get_scene("Ix").as_pointer() );
  boxm_scene<float_tree_type>* Iy_scene =
    dynamic_cast<boxm_scene<float_tree_type> *>(taylor_scenes->get_scene("Iy").as_pointer() );
  boxm_scene<float_tree_type>* Iz_scene =
    dynamic_cast<boxm_scene<float_tree_type> *>(taylor_scenes->get_scene("Iz").as_pointer() );
  boxm_scene<float_tree_type>* Ixx_scene =
    dynamic_cast<boxm_scene<float_tree_type> *>(taylor_scenes->get_scene("Ixx").as_pointer() );
  boxm_scene<float_tree_type>* Ixy_scene =
    dynamic_cast<boxm_scene<float_tree_type> *>(taylor_scenes->get_scene("Ixy").as_pointer() );
  boxm_scene<float_tree_type>* Ixz_scene =
    dynamic_cast<boxm_scene<float_tree_type> *>(taylor_scenes->get_scene("Ixz").as_pointer() );
  boxm_scene<float_tree_type>* Iyy_scene =
    dynamic_cast<boxm_scene<float_tree_type> *>(taylor_scenes->get_scene("Iyy").as_pointer() );
  boxm_scene<float_tree_type>* Iyz_scene =
    dynamic_cast<boxm_scene<float_tree_type> *>(taylor_scenes->get_scene("Iyz").as_pointer() );
  boxm_scene<float_tree_type>* Izz_scene =
    dynamic_cast<boxm_scene<float_tree_type> *>(taylor_scenes->get_scene("Izz").as_pointer() );

  if( !(I0_scene && Ix_scene && Iy_scene && Iz_scene && Ixx_scene && Iyy_scene && Izz_scene && Ixy_scene && Ixz_scene &&
        Iyz_scene) )
    {
    vcl_cerr << "Error in assemble_basis: Faild to load taylor scenes\n";
    return false;
    }
  ...
  vcl_vector<taylor_cell_type *> leaves_basis = basis_tree->leaf_cells();
  vcl_vector<float_cell_type *>  leaves_I0 = treeI0->leaf_cells();
  vcl_vector<float_cell_type *>  leaves_Ix = Ix_scene->get_block(block_i, block_j, block_k)->get_tree()->leaf_cells();
  vcl_vector<float_cell_type *>  leaves_Iy = Iy_scene->get_block(block_i, block_j, block_k)->get_tree()->leaf_cells();
  vcl_vector<float_cell_type *>  leaves_Iz = Iz_scene->get_block(block_i, block_j, block_k)->get_tree()->leaf_cells();
  vcl_vector<float_cell_type *>  leaves_Ixx = Ixx_scene->get_block(block_i, block_j, block_k)->get_tree()->leaf_cells();
  vcl_vector<float_cell_type *>  leaves_Ixy = Ixy_scene->get_block(block_i, block_j, block_k)->get_tree()->leaf_cells();
  vcl_vector<float_cell_type *>  leaves_Ixz = Ixz_scene->get_block(block_i, block_j, block_k)->get_tree()->leaf_cells();
  vcl_vector<float_cell_type *>  leaves_Iyy = Iyy_scene->get_block(block_i, block_j, block_k)->get_tree()->leaf_cells();
  vcl_vector<float_cell_type *>  leaves_Iyz = Iyz_scene->get_block(block_i, block_j, block_k)->get_tree()->leaf_cells();
  vcl_vector<float_cell_type *>  leaves_Izz = Izz_scene->get_block(block_i, block_j, block_k)->get_tree()->leaf_cells();
