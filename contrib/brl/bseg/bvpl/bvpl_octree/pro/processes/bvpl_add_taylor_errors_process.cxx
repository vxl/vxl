...
bool bvpl_add_taylor_errors_process(bprb_func_process& pro)
{
  ...
  if(!error_scene_base){
    vcl_cerr << "Error in bvpl_add_taylor_errors_process: Null error scene" << vcl_endl;
    return false;
  }
  boxm_scene<boct_tree<short, float> >* error_scene = dynamic_cast<boxm_scene<boct_tree<short, float> >*> (error_scene_base.as_pointer());
  if(!error_scene){
    vcl_cerr << "Error in bvpl_add_taylor_errors_process: Error scene is of incorrect type" << vcl_endl;
    return false;
  }
