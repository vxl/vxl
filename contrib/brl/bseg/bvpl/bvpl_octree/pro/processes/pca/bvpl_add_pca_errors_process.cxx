...
bool bvpl_add_pca_errors_process(bprb_func_process& pro)
{
  ...
  boxm_scene<boct_tree<short,
                       float> >* error_scene =
    dynamic_cast<boxm_scene<boct_tree<short, float> > *>(error_scenes->get_scene(dim).as_pointer() );
  if( !error_scene )
    {
  ...
  if( fraction_nsamples < 0.95 )
    ...
    vcl_cout << "Number of samples in  the scene " << scene_ncells << '\n'
             << "Adding errors from " << tree_nsamples << " samples in block: " << block_i << ',' << block_j << ','
             << block_k << '\n';
    error = bvpl_average_value(error_scene, block_i, block_j, block_k, tree_nsamples);
    vcl_cout << "Error at block: (" << block_i << ", " << block_j << ", " << block_k << ") and dim: " << dim
             << "is: " << error << '\n';
    }
  else
    {
