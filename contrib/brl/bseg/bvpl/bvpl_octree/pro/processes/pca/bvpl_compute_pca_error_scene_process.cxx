...
void * pthread_launcher(void * arg)
{
  ...
  pd->pca_extractor->compute_testing_error_thread_safe(pd->error_scene, pd->num_components, block_idx.x(),
                                                       block_idx.y(), block_idx.z() );
  // delete pd;
  return arg;
}
...
bool bvpl_compute_pca_error_scene_process(bprb_func_process& pro)
{
  ...
  boxm_scene<boct_tree<short,
                       float> >* error_scene =
    dynamic_cast<boxm_scene<boct_tree<short, float> > *>(pca_error_scenes->get_scene(num_components).as_pointer() );
...
#  if 0
  for( unsigned i = 0; i < num_threads; ++i )
    {
    pca_extractor.compute_testing_error_thread_safe(error_scene, num_components,
                                                    block_indices[i].x(), block_indices[i].y(), block_indices[i].z() );
    }
