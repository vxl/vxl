...
bool bvpl_compute_taylor_error_process(bprb_func_process& pro)
{
  ...
  if(!data_scene_base){
    vcl_cerr << "Error in bvpl_compute_taylor_error_process: Null data scene" << vcl_endl;
    return false;
  }
  if(!taylor_scenes){
    vcl_cerr << "Error in bvpl_compute_taylor_error_process: Null scenes map" << vcl_endl;
    return false;
  }
