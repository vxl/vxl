...
void test_gauss3D_steerable_filters()
{
  ...
  vcl_cout << "Exploring mean scene \n";
    ...
    boxm_cell_iterator<boct_tree<short,
                                 float> > iterator = mean_scene->cell_iterator(&boxm_scene<boct_tree<short,
                                                                                                     float> >::load_block);
    iterator.begin();
  ...
  boxm_scene<out_tree_type> * scene_out =
    new boxm_scene<out_tree_type>(mean_scene->lvcs(), mean_scene->origin(),
                                  mean_scene->block_dim(), mean_scene->world_dim(),
                                  mean_scene->max_level(), mean_scene->init_level() );
  ...
  boxm_scene<boct_tree<short, bool> > * valid_scene =
    new boxm_scene<boct_tree<short, bool> >(mean_scene->lvcs(), mean_scene->origin(),
                                            mean_scene->block_dim(), mean_scene->world_dim(),
                                            mean_scene->max_level(), mean_scene->init_level() );
