...
void test_global()
{
  ...
  boxm_scene<boct_tree<short, int> > * aux_scene =
    new boxm_scene<boct_tree<short, int> >(scene->lvcs(), scene->origin(), scene->block_dim(),
                                           scene->world_dim(), scene->max_level(), scene->init_level() );
  ...
  bvpl_global_pca<125> pca_global_info(scenes, aux_paths, cell_lengths, neighborhood, training_fraction,
                                       "./pca_global_info");
  ...
  for( unsigned pow_of_2 = 1; pow_of_2 < orig_file; pow_of_2 *= 2, ++nrepeat ) // i.e.: ceil(log_2(file)) iterations
    ...
    for( ; file_p < file; file_p += 2, ++nfiles )
      ...
      bvpl_global_pca<125>::combine_pairwise_statistics(mean1, S1, (double)nfeatures1, mean2, S2, (double)nfeatures2,
                                                        mean_out, S_out, nfeatures_out);
