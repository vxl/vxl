...
void test_assemble_basis(bvpl_taylor_scenes_map_sptr taylor_scenes, vcl_map<vcl_string, float> & values)
{
  ...
  boxm_scene<boct_tree<short,
                       bvpl_taylor_basis2_sample> >* basis_scene =
    dynamic_cast<boxm_scene<boct_tree<short,
                                      bvpl_taylor_basis2_sample> > *>(taylor_scenes->get_scene("basis").as_pointer() );
  basis_scene->load_block(0, 0, 0);

  vcl_vector<boct_tree_cell<short,
                            bvpl_taylor_basis2_sample> *> basis_leaves =
    basis_scene->get_block(0, 0, 0)->get_tree()->leaf_cells();
...
void test_taylor_basis()
{
  ...
  boxm_scene<boct_tree<short,
                       float> >* error_scene =
    dynamic_cast<boxm_scene<boct_tree<short, float> > *>(taylor_scenes->get_scene("error").as_pointer() );
  double cell_length = data_scene->finest_cell_length();
  bvpl_taylor_basis::compute_approximation_error(data_scene, taylor_scenes->get_scene(
                                                   "basis"), error_scene, loader, 0, 0, 0, cell_length);
