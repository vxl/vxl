...
public:
  ...
  //: Non-max suppression among all features, the result is stored in the input grid, as previous results are 
  //  necessary
  template<class T>
  ...
  //: Using appearance and occupancy grids simultaneously, this function applies a one kernel at a time, 
  //  performs non-maxima suppression and combines the top three responses at every  voxel 
...
void bvpl_vector_operator::non_maxima_suppression(bvxm_voxel_grid<T>* grid_in, 
                                                  ...
                                                  bvpl_kernel_vector_sptr kernel_vector)
{
  bvpl_subgrid_iterator<T> response_grid_iter(grid_in, kernel_vector->max(), kernel_vector->min());
   bvpl_subgrid_iterator<unsigned int> kernel_id_iter(id_grid, kernel_vector->max(), kernel_vector->min());
                                                ...
                                                bvxm_voxel_grid<vnl_vector_fixed<int,3> >* id_grid)
{
  ...
  for(unsigned id = 0; id < kernel_vector->kernels_.size(); ++id)
  {
    ...
    // ocal_non_maxima_suppression(bvxm_voxel_grid<T>* grid_in,    int index, bvpl_kernel_sptr kernel)
                                                        ...
                                                        bvpl_kernel_sptr kernel)
{
  ...
  bvpl_subgrid_iterator<T> response_grid_iter(grid_in, kernel->max(), kernel->min());
  bvpl_local_max_functor<T> func_max;
