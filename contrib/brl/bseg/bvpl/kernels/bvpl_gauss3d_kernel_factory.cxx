...
bvpl_gauss3d_kernel_factory::bvpl_gauss3d_kernel_factory(float s1, float s2, float s3)
{
  //set variances of kernel
  sigma1_ = s1;
  sigma2_ = s2;
...
void bvpl_gauss3d_kernel_factory::create_canonical()
{
  ...
  float min_x = -vcl_floor(2.0f*sigma1_+0.01f);
  float max_x =  vcl_floor(2.0f*sigma1_+0.01f);
