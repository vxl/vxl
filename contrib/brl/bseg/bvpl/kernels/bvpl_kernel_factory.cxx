...
bvpl_kernel_factory::create()
{
  ...
  bvpl_kernel kernel(interpolate(final_float_kernel), rotation_axis_, parallel_axis_, angle_,
                     dim(), min_point_, max_point_, factory_name_, voxel_length_);
...
bvpl_kernel_factory::rotate(vgl_rotation_3d<float> R)
{
  ...
  for( ; kernel_it != kernel_.end(); ++kernel_it )
    ...
    kernel.push_back(vcl_make_pair(vgl_point_3d<float>(new_coord[0], new_coord[1], new_coord[2]),
                                   (kernel_it->second) ) );
