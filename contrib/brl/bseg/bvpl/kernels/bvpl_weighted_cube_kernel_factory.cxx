...
bvpl_weighted_cube_kernel_factory::bvpl_weighted_cube_kernel_factory(unsigned length, unsigned width,
                                                                     unsigned thickness)
{
  // set variables
  length_ = length;
  ...
  for( int x = min_x; x <= max_x; x++ )
    ...
    for( int z = min_z; z <= max_z; z++ )
      ...
      for( int y = min_y; y <= max_y; y++ )
        {
        canonical_kernel_.push_back(vcl_pair<point_3d, dispatch>(point_3d(float(x), float(y), float(z) ), dispatch(
                                                                   1.0f) ) );
        }
      }
    }
