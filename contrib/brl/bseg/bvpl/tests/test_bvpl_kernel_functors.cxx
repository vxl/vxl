...
void test_edge_functors()
{
  ...
  result = is_correct_solution<bvpl_edge_algebraic_mean_functor<float>, float>(kernel_vec, data, mean_functor, min_p,
                                                                               max_p, sigma_noise, max_val);
  TEST("Test Algebraic mean functor with no noise ", true, result);
  result = is_correct_solution<bvpl_edge_geometric_mean_functor<float>, float>(kernel_vec, data, geom_functor, min_p,
                                                                               max_p, sigma_noise, max_val);
  ...
  result = is_correct_solution<bvpl_opinion_functor, bvxm_opinion>(kernel_vec, opinion_data, opinion_functor, minp,
                                                                   maxp, sigma_noise, max_val_opinion);
...
void test_gaussian_kernels()
{
  ...
  for( unsigned i = 0; i < 3; i++ )
    ...
    result = is_correct_solution<bvpl_gauss_convolution_functor>(kernel_vec, data, functor, empty, edge, sigma_noise,
                                                                 empty, response);
