                                                           ...
                                                           bvpl_kernel_vector_sptr corner_kernels)
{
  ...
  for( int target_id = 0; target_id < search_kernels->size(); ++target_id )
    ...
    vcl_cout << "Search kernel " << target_id << " with size" << this_search_kernel->max_point() << ','
             << this_search_kernel->min_point() << vcl_endl;

    /*******************************************************************/
    bvpl_subgrid_iterator<int> id_subgrid_iter(id_grid,
                                               this_search_kernel->min_point(), this_search_kernel->max_point() );
          ...
          if( id_subgrid.voxel(idx, this_id) )
            ...
            if( this_id >= 0 )
              ...
              if( this_corner_kernel->axis() == target_corner_kernel->axis() )// same plane now look for opossite diagonal corners
                {
                if( vcl_abs(vcl_abs(this_corner_kernel->angle() - target_corner_kernel->angle() )
                            - vnl_math::pi_over_2) < 1e-7 )
                  {
                                                           ...
                                                           bvpl_kernel_vector_sptr corner_kernels)
{
  ...
  for( int target_id = 0; target_id < search_kernels->size(); ++target_id )
    ...
    bvpl_subgrid_iterator<int> id_subgrid_iter(id_grid,
                                               this_search_kernel->min_point(), this_search_kernel->max_point() );
    bvpl_subgrid_iterator<float> response_subgrid_iter(response_grid,
                                                       this_search_kernel->min_point(),
                                                       this_search_kernel->max_point() );
    vcl_vector<vgl_line_segment_3d<int> > lines;
                ...
                if( this_id >= 0 )
                  ...
                  if( this_corner_kernel->axis() == target_corner_kernel->axis() )
                    ...
                    if( vcl_abs(vcl_sin(this_corner_kernel->angle() - target_corner_kernel->angle() ) - 1.0) < 1e-4 )
                      {
                      vcl_cout << "this angle " << this_corner_kernel->angle() << " target angle "
                               << target_corner_kernel->angle()
                               << " condition " << vcl_abs(
                        (this_corner_kernel->angle() - target_corner_kernel->angle() ) - vnl_math::pi_over_2)
                                                           ...
                                                           bvxm_voxel_grid<bvpl_pair> * pair_grid)
{
  ...
  for( int target_id = 0; target_id < search_kernels->size(); ++target_id )
    ...
    bvpl_subgrid_iterator<float> response_subgrid_iter(response_grid,
                                                       this_search_kernel->min_point(),
                                                       this_search_kernel->max_point() );
    bvpl_subgrid_iterator<bvpl_pair> pair_subgrid_iter(pair_grid,
                                                       this_search_kernel->min_point(),
                                                       this_search_kernel->max_point() );
    vcl_vector<vgl_line_segment_3d<int> > lines;
    vcl_vector<vgl_box_3d<int> >          boxes;
              ...
              if( id_subgrid.voxel(idx, this_id) )
                ...
                if( this_id >= 0 )
                  ...
                  if( this_corner_kernel->axis() == target_corner_kernel->axis() )
                    ...
                    if( vcl_abs(vcl_sin(this_corner_kernel->angle() - target_corner_kernel->angle() ) - 1.0) < 1e-4 )
                      {
                      bvpl_feature f1(p1, target_id, target_corner_kernel->axis(),
                                      target_corner_kernel->angle(), response_subgrid.get_voxel() );
                      vgl_point_3d<int> p2(p1.x() + idx.x(), p1.y() + idx.y(), p1.z() - idx.z() );
                      vgl_point_3d<int> p2_2(p1.x() + idx.x() / 2, p1.y() + idx.y() / 2, p1.z() - idx.z() / 2);
                      lines.push_back(vgl_line_segment_3d<int>(p1, p2) );
                      bvpl_feature f2(p2, this_id, this_corner_kernel->axis(),
                                      this_corner_kernel->angle(), this_response);
                      bvpl_pair this_pair(p2_2, f1, f2, target_id,
                                          this_search_kernel->axis(), this_search_kernel->angle(), 1.0f);
                                                           ...
                                                           int opposite_angle)
{
  ...
  for( int target_id = 0; target_id < search_kernels->size(); ++target_id )
    ...
    vcl_cout << "\n Search kernel " << target_id << " with size" << this_search_kernel->max_point() << ','
             << this_search_kernel->min_point()
             << "Axis " << this_search_kernel->axis() << " Angle " << this_search_kernel->angle() << vcl_endl;

    /*******************************************************************/
    bvpl_subgrid_iterator<bvpl_pair> in_pair_subgrid_iter(pair_grid_in,
                                                          this_search_kernel->min_point(),
                                                          this_search_kernel->max_point() );
    bvpl_subgrid_iterator<bvpl_pair> out_pair_subgrid_iter(pair_grid_out,
                                                           this_search_kernel->min_point(),
                                                           this_search_kernel->max_point() );
    vcl_vector<vgl_line_segment_3d<int> > lines;
    vcl_vector<vgl_box_3d<int> >          boxes;
              ...
              if( this_id >= 0 )
                ...
                if( this_pair.axis() == pair_at_center.axis() )
                  ...
                  if( opposite_angle == 90 )
                    {
                    condition = (vcl_abs(vcl_sin(this_pair.angle() - pair_at_center.angle() ) - 1.0) < 1e-4);
                    }
                  if( opposite_angle == 180 )
                    {
                    condition = (vcl_abs(vcl_sin( (this_pair.angle() - pair_at_center.angle() ) / 2.0f) - 1.0) < 1e-4);
                    }

                  if( condition )
                    {
                    bvpl_feature f1(p1, target_id, pair_at_center.axis(),
                                    pair_at_center.angle(), response_at_center);
                    vgl_point_3d<int> p2(p1.x() + idx.x(), p1.y() + idx.y(), p1.z() - idx.z() );
                    vgl_point_3d<int> p2_2(p1.x() + idx.x() / 2, p1.y() + idx.y() / 2, p1.z() - idx.z() / 2);
                    lines.push_back(vgl_line_segment_3d<int>(p1, p2) );
                    bvpl_feature f2(p2, this_id, this_pair.axis(), this_pair.angle(), this_response);
                    bvpl_pair    out_pair(p2_2, f1, f2, target_id,
                                          this_search_kernel->axis(), this_search_kernel->angle(), 1.0f);
