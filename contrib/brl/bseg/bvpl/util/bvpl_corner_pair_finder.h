...
public:
  ...
  bvpl_feature(vgl_point_3d<int> const & this_location, int const id, vnl_float_3 const& axis, float const angle,
               float const response)
...
public:
  ...
  bvpl_pair(vgl_point_3d<int> this_location, bvpl_feature f1,  bvpl_feature f2, int id, vnl_float_3 axis, float angle,
            float response )
...
public:
  ...
  // : Find pairs that are 90 degrees apart e.g |_ with _|
  static bvpl_corner_pairs_sptr find_pairs(bvxm_voxel_grid<int>* id_grid, bvxm_voxel_grid<float>* response_grid,
  ...
  // : Find pairs that are 90 degrees apart e.g |_ with _|
  static void find_pairs_no_lines(bvxm_voxel_grid<int>* id_grid, bvxm_voxel_grid<float>* response_grid,
                                  bvpl_kernel_vector_sptr search_kernels, bvpl_kernel_vector_sptr corner_kernels,
                                  bvxm_voxel_grid<bvpl_pair> * pair_grid);
  ...
  static bvpl_corner_pairs_sptr find_pairs(bvxm_voxel_grid<bvpl_pair> * pair_grid_in,
                                           bvpl_kernel_vector_sptr search_kernels,
                                           bvxm_voxel_grid<bvpl_pair> * pair_grid_out, int opposite_angle);

  // : Find pairs that are 90 degrees apart e.g _ with |
  static void find_pairs_no_lines(bvxm_voxel_grid<bvpl_pair> * pair_grid_in, bvpl_kernel_vector_sptr search_kernels,
                                  bvxm_voxel_grid<bvpl_pair> * pair_grid_out, int opposite_angle);

};

void bvpl_convert_pair_grid_to_hsv_grid(bvxm_voxel_grid<bvpl_pair> * pair_grid, bvxm_voxel_grid<vnl_float_4> * out_grid,
                                        vcl_vector<float> colors);

void bvpl_convert_pair_grid_to_float_grid(bvxm_voxel_grid<bvpl_pair> * pair_grid, bvxm_voxel_grid<float> * out_grid );
