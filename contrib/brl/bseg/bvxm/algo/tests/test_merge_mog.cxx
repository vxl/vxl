...
void test_compability_read_write()
{
  //define some known mixtures
  typedef bsta_num_obs<bsta_gauss_f1> gauss_type;
  typedef bsta_mixture_fixed<gauss_type, 3> mix_gauss;
  ...
  f.num_observations = 4;
  
  vcl_string storage_fname_ = "./test.vox";
  vil_stream_fstream *fio_ = new vil_stream_fstream(storage_fname_.c_str(),"rw");
  if (!fio_->ok()) {
    vcl_cerr << "error opening " << storage_fname_ << " for read/write\n";
    return;
  }
  fio_->write(reinterpret_cast<char*>(&f),sizeof(mix_gauss_type));
