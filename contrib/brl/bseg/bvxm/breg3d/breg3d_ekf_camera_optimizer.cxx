                                                                        ...
                                                                        vcl_string apm_type, unsigned bin_idx)
{
  ...
  if( !cam_est )
    {
    vcl_cerr
    << "error: current camera estimate must be a vpgl_perspective_camera with at least the calibration matrix set.\n";
    return prev_state;
    }
  ...
  step_vec.push_back(vpgl_perspective_camera<double>(cam_est->get_calibration(), state_og.get_point(),
                                                     state_og.get_rotation() ) );
  step_vec.push_back(vpgl_perspective_camera<double>(cam_est->get_calibration(), step_state.get_point(),
                                                     step_state.get_rotation() ) );
  ...
  if( use_expected_ )
    ...
    while( iterate_again )
      ...
      vpgl_camera_double_sptr step_cam =
        new vpgl_perspective_camera<double>(cam_est->get_calibration(),
                                            substep_state.get_point(), substep_state.get_rotation() );
      // fill in metadata
      ...
      substep_state = substep_optimizer.optimize_once(vox_world, step_expected, step_mask, curr_img, substep_state,
                                                      false);

      step_vec.push_back(vpgl_perspective_camera<double>(cam_est->get_calibration(), substep_state.get_point(),
                                                         substep_state.get_rotation() ) );
...
breg3d_ekf_camera_optimizer_state breg3d_ekf_camera_optimizer::optimize_once(bvxm_voxel_world_sptr const& vox_world,
                                                                             vil_image_view_base_sptr & prev_img,
                                                                             vil_image_view<float> & prev_mask,
                                                                             bvxm_image_metadata & curr_img,
                                                                             breg3d_ekf_camera_optimizer_state & prev_state,
                                                                             bool use_gps)
{
  ...
  vnl_vector_fixed<double, 3> plane_normal(world_plane.normal().x(), world_plane.normal().y(),
                                           world_plane.normal().z() );
...
vnl_vector<double> breg3d_ekf_camera_optimizer::img_homography(vil_image_view_base_sptr & base_img_viewb,
                                                               vil_image_view_base_sptr & img_viewb,
                                                               vil_image_view<float> & mask_view,
                                                               vnl_matrix_fixed<double, 3, 3> K, bool projective)
{
  ...
  switch( base_img_viewb->pixel_format() )
    ...
    case VIL_PIXEL_FORMAT_RGB_BYTE: {
      vil_image_view<vil_rgb<vxl_byte> > * img_rgb_byte =
        dynamic_cast<vil_image_view<vil_rgb<vxl_byte> > *>(base_img_viewb.ptr() );
    ...
    default:
      vcl_cerr << "error: breg3d_ekf_camera_optimizer::img_homography : unsupported pixel type "
               << base_img_viewb->pixel_format() << '\n';
    }
  ...
  switch( img_viewb->pixel_format() )
    ...
    case VIL_PIXEL_FORMAT_RGB_BYTE: {
      vil_image_view<vil_rgb<vxl_byte> > * img_rgb_byte =
        dynamic_cast<vil_image_view<vil_rgb<vxl_byte> > *>(img_viewb.ptr() );
      vil_convert_rgb_to_grey(*img_rgb_byte, base_img_view);
      break;
      }
    case VIL_PIXEL_FORMAT_BYTE: {
...
vnl_matrix_fixed<double, 3,
                 3> breg3d_ekf_camera_optimizer::coeffs_to_matrix_GA2(vnl_vector_fixed<double, 6> const & /*a*/)
{
...
vnl_matrix_fixed<double, 3,
                 3> breg3d_ekf_camera_optimizer::coeffs_to_matrix_P2(vnl_vector_fixed<double, 8> const & /*a*/)
{
