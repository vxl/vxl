...
bool breg3d_ekf_camera_optimize_process::execute()
{
  ...
  if( !parameters()->get_value(vcl_string("position_measurement_variance"), rot_var_measure) )
    {
    vcl_cout
    << "breg3d_init_ekf_camera_optimize_process::execute() -- problem in retrieving parameter rotation_variance\n";
    return false;
    }
  if( !parameters()->get_value(vcl_string("position_measurement_variance"), pos_var_measure) )
    {
    vcl_cout
    << "breg3d_init_ekf_camera_optimize_process::execute() -- problem in retrieving parameter position_variance\n";
    return false;
    }
  ...
  if( !parameters()->get_value(vcl_string("homography_term_variance"), homography_var) )
    {
    vcl_cout
    << "breg3d_init_ekf_camera_optimize_process::execute() -- problem in retrieving parameter homography_variance\n";
    return false;
    }
  ...
  breg3d_ekf_camera_optimizer_state curr_state = optimizer.optimize(vox_world, prev_img, curr_metadata, prev_state,
                                                                    apm_type);
