...
public:
  ...
  breg3d_ekf_camera_optimize_process(const breg3d_ekf_camera_optimize_process& other) : bprb_process(*static_cast<const
                                                                                                                  bprb_process
                                                                                                                  *>(&
                                                                                                                     other) )
  {}
...
private:
  ...
  vnl_matrix<double> homography_jacobian_t(bvxm_voxel_world_sptr world, vpgl_perspective_camera<double>* cam,
                                           vil_image_view_base_sptr & expected_view);

  // : calculate a homography between two images
  ihog_transform_2d calculate_homography(vil_image_view_base_sptr & base_img, vil_image_view_base_sptr & img,
