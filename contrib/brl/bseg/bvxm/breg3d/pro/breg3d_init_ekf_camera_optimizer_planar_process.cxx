...
bool breg3d_init_ekf_camera_optimizer_planar_process::execute()
{
  ...
  if( !parameters()->get_value(vcl_string("position_measurement_variance"), rot_var) )
    ...
    vcl_cout
      <<
    "breg3d_init_ekf_camera_optimizer_planar_process::execute() -- problem in retrieving parameter rotation_variance\n";
    return false;
    }
  if( !parameters()->get_value(vcl_string("position_measurement_variance"), pos_var) )
    ...
    vcl_cout
      <<
    "breg3d_init_ekf_camera_optimizer_planar_process::execute() -- problem in retrieving parameter position_variance\n";
    return false;
    }
  ...
  if( !parameters()->get_value(vcl_string("translation_scale"), t_scale) )
    ...
    vcl_cout
      <<
    "breg3d_init_ekf_camera_optimizer_planar_process::execute() -- problem in retrieving parameter translation_scale\n";
    return false;
    }
  ...
  vcl_cout << "H =\n" << H.get_matrix() << vcl_endl;

  // TEMP
  vpgl_perspective_camera<double> * cam1 = new vpgl_perspective_camera<double>(
      cam0->get_calibration(), cam0->get_camera_center(), cam0->get_rotation() );
