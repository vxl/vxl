...
bool bvxm_edge_ray_processor::init_edges(unsigned scale)
{
  ...
  bvxm_voxel_grid<edges_datatype> * edges_voxel  =
    static_cast<bvxm_voxel_grid<edges_datatype> *>(edges_voxel_base.ptr() );
...
bool bvxm_edge_ray_processor::expected_edge_image(bvxm_image_metadata const& camera,
                                                  vil_image_view_base_sptr & expected, float n_normal, unsigned scale)
{
  // datatype for current appearance model
  typedef bvxm_voxel_traits<EDGES>::voxel_datatype edges_datatype;
  ...
  for( ; expected_edge_image_it != expected_edge_image.end(); ++expected_edge_image_it )
    {
    (*expected_edge_image_it) = sdet_img_edge::convert_edge_statistics_to_probability( (*expected_edge_image_it),
                                                                                       n_normal, dof);
...
bool bvxm_edge_ray_processor::expected_edge_image_and_heights(bvxm_image_metadata const& camera,
                                                              vil_image_view_base_sptr & expected,
                                                              vil_image_view_base_sptr & height_img, float n_normal,
                                                              unsigned scale)
{
  // datatype for current appearance model
  typedef bvxm_voxel_traits<EDGES>::voxel_datatype edges_datatype;
  ...
  for( unsigned z = 0; z < (unsigned)grid_size.z(); ++z, ++edges_voxel_it )
    ...
    for( ; expected_edge_image_it != expected_edge_image.end();
         ++expected_edge_image_it, ++edges_image_it, ++z_image_it )
      {
      // (*expected_edge_image_it) = vnl_math::max((*expected_edge_image_it),(*edges_image_it));
...
bool bvxm_edge_ray_processor::save_edges_raw(vcl_string filename, float n_normal, unsigned scale)
{
  ...
  for( unsigned k = 0; edges_it != edges_grid->end(); ++edges_it, ++k )
    ...
    for( unsigned i = 0; i < (*edges_it).nx(); ++i )
      ...
      for( unsigned j = 0; j < (*edges_it).ny(); ++j )
        {
        edges_array[i * ny * nz + j * nz
                    + k] =
          (unsigned char)(255.0
                          * sdet_img_edge::convert_edge_statistics_to_probability( (*edges_it)(i, j), n_normal, dof) );
        }
      }
    }
  vcl_cout << vcl_endl;
...
bool bvxm_edge_ray_processor::init_von_mises_edge_tangents(bvxm_image_metadata const& metadata0,
...
bool bvxm_edge_ray_processor::update_von_mises_edge_tangents(bvxm_image_metadata const& metadata,
                                                             double /*x0_interval*/,
                                                             double /*cone_angle*/,
                                                             unsigned scale)
{
...
void bvxm_edge_ray_processor::display_edge_tangent_world_vrml(vcl_string const& vrml_path)
{
  // open the file
  ...
  for( unsigned z = 0; z < nz; ++z, ++pos_dist_it, ++dir_dist_it )
    ...
    if( (pos_dist_it == pos_dist_grid->end() ) ||
        (dir_dist_it == dir_dist_grid->end() ) )
      {
      vcl_cerr << "In bvxm_edge_ray_processor::display_edge_tangent_world_vrml- reached end of tangent slabs at z = "
               << z << ".  nz = " << grid_size.z() << '\n';
      return;
      }
    bvxm_voxel_slab<pos_dist_t>& pos_dist_slab = (*pos_dist_it);
...
void bvxm_edge_ray_processor::display_ground_truth(vcl_string const& gnd_truth_path,
