...
public:
  ...
  bool expected_edge_image(bvxm_image_metadata const& camera, vil_image_view_base_sptr & expected, float n_normal,
                           unsigned scale_idx = 0);

  // : generate the expected edge image from the specified viewpoint. the expected image should be allocated by the caller.
  //  return the height of the voxel where the expected edge image probability is greatest
  bool expected_edge_image_and_heights(bvxm_image_metadata const& camera, vil_image_view_base_sptr & expected,
                                       vil_image_view_base_sptr & height_img, float n_normal, unsigned scale_idx = 0);
  ...
  bool init_von_mises_edge_tangents(bvxm_image_metadata const& metadata0, bvxm_image_metadata const& metadata1,
                                    double initial_sd_ratio = 1.0, double initial_kappa = 10000.0, unsigned scale = 0);
  ...
  bool update_von_mises_edge_tangents(bvxm_image_metadata const& metadata, double x0_interval = 1.0,
                                      double cone_angle = 0.1, unsigned scale = 0);
  ...
  void display_ground_truth(vcl_string const& gnd_truth_path, vcl_string const& vrml_path);
