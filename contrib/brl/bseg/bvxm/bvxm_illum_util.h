...
{
public:
  static bool load_surface_nhbds(vcl_string const& path, vcl_vector<vcl_vector<vnl_matrix<float> > >& nhds);

  static bool load_illumination_dirs(vcl_string const& path, vcl_vector<vnl_double_3>& ill_dirs);
  ...
  // are required.
  static bool solve_lambertian_model(vcl_vector<vnl_double_3> const& ill_dirs, vcl_vector<double> const& intensities,
                                     vnl_double_4& model_params, double& fitting_error);

  // :given an illumination direction and Lambertian model, predict the resulting image intensity
  static double expected_intensity(vnl_double_3 const& illum_dir, vnl_double_4 const& model_params);
