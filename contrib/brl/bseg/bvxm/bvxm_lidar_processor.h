...
public:
  ...
  bvxm_voxel_slab<float>  prob_density(float z_dim, bvxm_voxel_slab<float> const& obs, float voxel_width = 0.0 );

  float prob_density(vil_image_view_base_sptr lidar, float z_dim, vnl_vector_fixed<float, 3>& covar,  // sigma vals
                     vgl_box_2d<double> lidar_roi, float voxel_width);
