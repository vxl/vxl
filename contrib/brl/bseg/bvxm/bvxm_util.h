...
public:
  ...
  static bool read_cameras(const vcl_string filename, vcl_vector<vnl_double_3x3> & Ks, vcl_vector<vnl_double_3x3> & Rs,
                           vcl_vector<vnl_double_3x1> & Ts);

  static bool write_cameras(const vcl_string filename, vcl_vector<vnl_double_3x3> & Ks, vcl_vector<vnl_double_3x3> & Rs,
                            vcl_vector<vnl_double_3x1> & Ts);

  static void logical_and(bvxm_voxel_slab<bool> const& s1, bvxm_voxel_slab<bool> const& s2,
                          bvxm_voxel_slab<bool> & result);

  template <class T, class M>
  static void warp_slab_bilinear(bvxm_voxel_slab<M> const& slab_in, vgl_h_matrix_2d<double> invH,
                                 bvxm_voxel_slab<T> & slab_out);

  template <class T>
  static void warp_slab_nearest_neighbor(bvxm_voxel_slab<T> const& slab_in, vgl_h_matrix_2d<double> invH,
                                         bvxm_voxel_slab<T> & slab_out);
  ...
  template <class T, class M>
  static void multiply_slabs(bvxm_voxel_slab<M> const& s1, bvxm_voxel_slab<T> const& s2, bvxm_voxel_slab<T> & product);
  ...
  template <class T>
  static bool generate_test_boxes(T box_min_x, T box_min_y, T box_min_z, T box_dim_x, T box_dim_y, T box_dim_z,
                                  T world_dim_x, T world_dim_y, T world_dim_z, vcl_vector<vgl_box_3d<T> >& boxes,
                                  bool gen_2box = true);
  ...
  static int convert_uncertainty_from_meters_to_pixels(float uncertainty, vpgl_lvcs_sptr lvcs,
...
protected:
  ...
  static void bilinear_weights(vgl_h_matrix_2d<double> invH, unsigned nx_out, unsigned ny_out,
                               vnl_matrix<unsigned> & xvals, vnl_matrix<unsigned> & yvals, vnl_matrix<float> & weights);
...
bool bvxm_util::img_to_slab(vil_image_view_base_sptr const image, bvxm_voxel_slab<vnl_vector_fixed<T, N> > & slab)
{
  ...
  // take care of pixel format issues.
    ...
    {
    if( vil_image_view<vil_rgb<unsigned char> > * img_view =
...
void bvxm_util::smooth_gaussian(bvxm_voxel_slab<T> & slab, float stdx, float stdy)
{
  ...
  for( unsigned i = 0; i < kernel_size_x; ++i )
    {
    kernel_1dx[i] =
      (float)(vnl_math::sqrt1_2 * vnl_math::two_over_sqrtpi * (0.5 / stdx)
              * vcl_exp(-( ( (i - kernel_radius_x) * (i - kernel_radius_x) ) / (2.0 * stdx * stdx) ) ) );
    }
  ...
  for( unsigned i = 0; i < kernel_size_y; ++i )
    {
    kernel_1dy[i] =
      (float)(vnl_math::sqrt1_2 * vnl_math::two_over_sqrtpi * (0.5 / stdy)
              * vcl_exp(-( ( (i - kernel_radius_y) * (i - kernel_radius_y) ) / (2.0 * stdy * stdy) ) ) );
    }
  // normalize kernel in case taps don't sum to exactly one
                                    ...
                                    vcl_vector<vgl_box_3d<T> >& boxes, bool gen_2box)
{
  ...
  if( gen_2box )
    ...
    top_box.set_centroid(vgl_point_3d<T>(top_centroid.x() + dimx / 3., top_centroid.y() + dimx / 3.,
                                         top_centroid.z() ) );
