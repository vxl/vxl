...
vcl_ostream &  operator<<(vcl_ostream& s, bvxm_voxel_world const& vox_world)
{
  ...
  s << "bvxm_voxel_world : " << params->num_voxels().x() << " x " << params->num_voxels().y() << " x "
    << params->num_voxels().z() << vcl_endl;
  return s;
}
...
#  endif
  ...
  for( unsigned k_idx = 0; k_idx < (unsigned)grid_size.z(); ++k_idx, ++PLvisX_slab_it, ++preX_slab_it, ++ocp_slab_it2 )
    ...
    bvxm_voxel_slab<float>::const_iterator preX_it = preX_slab_it->begin(), PLvisX_it = PLvisX_slab_it->begin(),
      preX_sum_it = preX_accum_vox.begin(), visX_sum_it = visX_accum_vox.begin();
                                          ...
                                          unsigned scale)
{
  ...
  bvxm_voxel_grid<edges_datatype> * edges_grid  =
    static_cast<bvxm_voxel_grid<edges_datatype> *>(edges_grid_base.ptr() );
  ...
  for( unsigned k_idx = 0; k_idx < (unsigned)grid_size.z(); ++k_idx, ++edges_slab_it )
    ...
    bvxm_voxel_slab<float> lidar_prob(lidar_height_backproj.nx(), lidar_height_backproj.ny(),
                                      lidar_height_backproj.nz() );
    lidar_prob.fill(0.0);
    ...
    for( unsigned i_idx = 0; i_idx < lidar_height_backproj.nx(); i_idx++ )
      ...
      for( unsigned j_idx = 0; j_idx < lidar_height_backproj.ny(); j_idx++ )
        ...
        lidar_prob(i_idx, j_idx) = lidar_processor.prob_density(lidar_height,
                                                                local_xyz.z(), sigmas, lidar_roi,
                                                                params_->voxel_length(scale) );
        }
      }
    ...
    for( ; lidar_prob_it != lidar_prob.end();
         ++lidar_prob_it, ++edges_slab_it_it, ++lidar_edges_backproj_it, ++lidar_edges_prob_backproj_it )
      {
...
bool bvxm_voxel_world::heightmap(vpgl_camera_double_sptr virtual_camera, vil_image_view<unsigned> & heightmap,
                                 vil_image_view<float> & conf_map, unsigned scale_idx)
{
  typedef bvxm_voxel_traits<OCCUPANCY>::voxel_datatype ocp_datatype;
...
bool bvxm_voxel_world::heightmap_exp(vpgl_camera_double_sptr virtual_camera, vil_image_view<float> & heightmap,
                                     vil_image_view<float> & var, float& max_depth, unsigned scale_idx)
{
  typedef bvxm_voxel_traits<OCCUPANCY>::voxel_datatype ocp_datatype;
  ...
  for( unsigned z = 0; z < (unsigned)grid_size.z(); ++z, ++ocp_slab_it )
    ...
    bvxm_voxel_slab<float>::iterator depth_it = depth.begin(), x_it = slab_x_virtual.begin(),
      y_it = slab_y_virtual.begin(), z_it = slab_z_virtual.begin();
    bvxm_voxel_slab<float>::iterator x_pre_it = slab_x_pre_virtual.begin(), y_pre_it = slab_y_pre_virtual.begin(),
      z_pre_it = slab_z_pre_virtual.begin();
    ...
    bvxm_voxel_slab<ocp_datatype>::const_iterator PX_it = slice_prob_img.begin();
    bvxm_voxel_slab<float>::iterator              exp_depth_it = exp_depth.begin(),
      exp_depth_square_it = exp_depth_square.begin(), visX_it = visX_accum_virtual.begin();
  ...
  bvxm_voxel_slab<float>::iterator exp_depth_it = exp_depth.begin(), exp_depth_square_it = exp_depth_square.begin(),
    visX_it = visX_accum_virtual.begin();
  depth_it = depth.begin();
  vil_image_view<float>::iterator hmap_it = heightmap.begin(), var_it = var.begin();
  for( ; exp_depth_it != exp_depth.end();
       ++exp_depth_it, ++exp_depth_square_it, ++depth_it, ++visX_it, ++hmap_it, ++var_it )
    {
...
bool bvxm_voxel_world::uncertainty(vpgl_camera_double_sptr virtual_camera, vil_image_view<float> & uncertainty,
                                   unsigned scale_idx)
{
  typedef bvxm_voxel_traits<OCCUPANCY>::voxel_datatype ocp_datatype;
...
vgl_point_3d<float> bvxm_voxel_world::voxel_index_to_xyz(unsigned vox_i, unsigned vox_j, int vox_k, unsigned scale_idx)
{
  ...
  vgl_point_3d<float> grid_origin = params_->corner()
    + (vgl_vector_3d<float>(0.5f, 0.5f, (float(num_vox.z() ) - 0.5f) ) * vox_len);
...
void bvxm_voxel_world::compute_plane_image_H(vpgl_camera_double_sptr const& cam, int k_idx,
                                             vgl_h_matrix_2d<double> & H_plane_to_image,
                                             vgl_h_matrix_2d<double> & H_image_to_plane, unsigned scale_idx)
{
  vgl_vector_3d<unsigned int> grid_size = params_->num_voxels(scale_idx);
