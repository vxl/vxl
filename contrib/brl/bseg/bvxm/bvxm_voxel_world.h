...
public:
  ...
  template <bvxm_voxel_type APM_T>
  bool update(bvxm_image_metadata const& observation, vil_image_view<float> & pixel_prob_density,
              vil_image_view<bool> & mask, unsigned bin_index = 0, unsigned scale_idx = 0, bool use_memory = false);
  ...
  template <bvxm_voxel_type APM_T>
  bool update_lidar(bvxm_image_metadata const& observation, vil_image_view<float> & pixel_prob_density,
                    vil_image_view<bool> & mask, unsigned scale_idx = 0);

  // : update voxel grid edge probabilities with data from LIDAR/camera pair
  bool update_edges_lidar(vil_image_view_base_sptr& lidar_height, vil_image_view_base_sptr& lidar_edges,
                          vil_image_view_base_sptr& lidar_edges_prob, vpgl_camera_double_sptr& camera,
  ...
  template <bvxm_voxel_type APM_T>
  bool expected_image(bvxm_image_metadata const& camera, vil_image_view_base_sptr & expected,
  ...
  template <bvxm_voxel_type APM_T>
  bool inv_pixel_range_probability(bvxm_image_metadata const& observation, vil_image_view<float> & inv_prob,
                                   unsigned bin_index = 0, unsigned scale_idx = 0, float pixel_range = 0.008f);
  ...
  template <bvxm_voxel_type APM_T>
  bool pixel_probability_density(bvxm_image_metadata const& observation, vil_image_view<float> & pixel_probability,
                                 vil_image_view<bool> & mask, unsigned bin_index = 0, unsigned scale_idx = 0);

  // : for each pixel in a region specified by mask , return the probability that the observation was produced by the voxel.
  // The returned values are approximate samples of a probability density, with the pixel values being the independent value.
  template <bvxm_voxel_type APM_T>
  bool region_probability_density(bvxm_image_metadata const& observation, vil_image_view_base_sptr const& mask,
                                  bvxm_voxel_grid_base_sptr & pixel_probability, unsigned bin_index,
                                  unsigned scale_idx);

  // : generate the mixture of gaussians slab from the specified viewpoint. the slab should be allocated by the caller.
  template <bvxm_voxel_type APM_T>
  bool mixture_of_gaussians_image(bvxm_image_metadata const& camera, bvxm_voxel_slab_base_sptr& mog_image,
  ...
  template <bvxm_voxel_type APM_T>
  bool mog_most_probable_image(bvxm_image_metadata const& camera, bvxm_voxel_slab_base_sptr& mog_image,
                               unsigned bin_index = 0, unsigned scale_idx = 0);

  // : return the original image, viewed from a new viewpoint
  template <bvxm_voxel_type APM_T>
  bool virtual_view(bvxm_image_metadata const& original_view, const vpgl_camera_double_sptr virtual_camera,
  ...
  bool heightmap(vpgl_camera_double_sptr virtual_camera, vil_image_view<unsigned> & heightmap,
                 vil_image_view<float> & conf_map, unsigned scale_idx = 0);

  // : generate a heightmap from the viewpoint of a virtual camera
  // The pixel values are the expected z values and variance along the corresponding camera ray
  bool heightmap_exp(vpgl_camera_double_sptr virtual_camera, vil_image_view<float> & heightmap,
                     vil_image_view<float> & var, float& max_depth, unsigned scale_idx = 0);
  ...
  template <bvxm_voxel_type APM_T>
  bool heightmap(bvxm_image_metadata const& virtual_camera, vil_image_view<unsigned> & heightmap,
                 unsigned bin_index = 0, unsigned scale_idx = 0);

  // : use the ortho z map (given by method heightmap() ) of the scene to make an input image ortho.
  template <typename T>
  bool orthorectify(vil_image_view_base_sptr z_map, vpgl_camera_double_sptr z_map_camera,
                    vil_image_view<T>& input_image, vpgl_camera_double_sptr input_camera, vil_image_view<T>& out_image,
                    unsigned scale_idx = 0);
  ...
  void compute_plane_image_H(vpgl_camera_double_sptr const& cam, int grid_k, vgl_h_matrix_2d<double> & H_plane_to_image,
...
private:
  ...
  template <bvxm_voxel_type APM_T>
  bool update_impl(bvxm_image_metadata const& metadata, bool return_prob, vil_image_view<float> & pix_prob_density,
  ...
  template <bvxm_voxel_type APM_T>
  bool update_lidar_impl(bvxm_image_metadata const& metadata, bool return_prob,
                         vil_image_view<float> & pix_prob_density, bool return_mask, vil_image_view<bool> & mask,
                         unsigned scale_idx = 0);
...
unsigned int bvxm_voxel_world::num_observations(unsigned int bin_idx, unsigned int scale_idx, bool use_memory)
{
  ...
  bvxm_voxel_grid<vox_datatype> * grid =
    static_cast<bvxm_voxel_grid<vox_datatype> *>(this->get_grid<VOX_T>(bin_idx, scale_idx, use_memory).ptr() );
...
bool bvxm_voxel_world::update(bvxm_image_metadata const& observation,
                              ...
                              unsigned scale_idx,  bool use_momory)
{
  // check image sizes
                                   ...
                                   vil_image_view<bool> & mask, unsigned bin_index, unsigned scale_idx, bool use_momory)
{
  ...
  for( unsigned z = 0; z < (unsigned)grid_size.z();
       ++z, ++ocp_slab_it, ++apm_slab_it, ++preX_slab_it, ++PIvisX_slab_it )
    {
    vcl_cout << '.';
    vcl_cout.flush();
                                    ...
                                    unsigned scale_idx)
{
  ...
  for( unsigned z = 0; z < (unsigned)grid_size.z(); ++z, ++ocp_slab_it )
    ...
    for( ; vframe_it != virtual_view_slab.end();
         ++vframe_it, ++frame_it, ++height_it, ++vis_it, ++visX_accum_vproj_it, ++PX_vproj_it )
      {
...
// This version of the function assumes that there is also image data associated with the virtual camera
template <bvxm_voxel_type APM_T>
bool bvxm_voxel_world::heightmap(bvxm_image_metadata const& virtual_camera, vil_image_view<unsigned> & heightmap,
                                 unsigned bin_index, unsigned scale_index)
{
  typedef bvxm_voxel_traits<OCCUPANCY>::voxel_datatype      ocp_datatype;
  typedef typename bvxm_voxel_traits<APM_T>::voxel_datatype apm_datatype;
  typedef typename bvxm_voxel_traits<APM_T>::obs_datatype   obs_datatype;
