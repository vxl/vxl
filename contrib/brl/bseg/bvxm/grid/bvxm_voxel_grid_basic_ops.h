...
{      
  //multipy
  bvxm_voxel_grid<T>::iterator grid1_it = grid1.begin();
  bvxm_voxel_grid<T>::iterator grid2_it = grid2.begin();
  bvxm_voxel_grid<T>::iterator grid_out_it = grid_out.begin();
  
  for(; grid1_it != grid1.end(); ++grid1_it, ++grid2_it, ++grid_out_it)
  {
    
    bvxm_voxel_slab<T>::iterator slab1_it = (*grid1_it).begin();
    bvxm_voxel_slab<T>::iterator slab2_it = (*grid2_it).begin();
    bvxm_voxel_slab<T>::iterator slab_out_it = (*grid_out_it).begin();
                                         ...
                                         bvxm_voxel_grid<bool> const mask, float min_thresh)
{
  ...
  bvxm_voxel_grid<T>::iterator in_slab_it = grid_in->begin();
  bvxm_voxel_grid<T>::iterator out_slab_it = grid_out->begin();
  ...
  for (unsigned z=0; z<(unsigned)(grid_in->grid_size().z()); ++z, ++in_slab_it, ++mask_slab_it, ++out_slab_it)
  {
    ...
    bvxm_voxel_slab<T>::iterator in_it = (*in_slab_it).begin();
    bvxm_voxel_slab<T>::iterator out_it = (*out_slab_it).begin();
