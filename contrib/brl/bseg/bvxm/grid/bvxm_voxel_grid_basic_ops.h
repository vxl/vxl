...
template <class T>
bool bvxm_voxel_grid_multiply(bvxm_voxel_grid_base_sptr grid1_base, bvxm_voxel_grid_base_sptr grid2_base,
...
// : Multiplies 2 grids. The types of input grids must have a * operator
bool bvxm_voxel_grid_compare(bvxm_voxel_grid<float> * dt_grid, bvxm_voxel_grid<bvxm_opinion> * op_grid,
  ...
  while( dir_slab != dir->end() )
    ...
    for( unsigned i = 0; i < dir->grid_size().x(); ++i )
      ...
      for( unsigned j = 0; j < dir->grid_size().y(); ++j )
        {
        d(i,
          j) = vnl_vector_fixed<float, 3>(directions(i, j, k).R(), directions(i, j, k).G(), directions(i, j, k).B() );
