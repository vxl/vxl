  ...
  : bvxm_voxel_storage<T>(grid_size), storage_fname_(storage_filename), fio_(0), active_slab_start_(-1)
{
  ...
  if( vul_file::exists(storage_fname_) )
    ...
    if( (header.nx_ != grid_size.x() ) || (header.ny_ != grid_size.y() ) || (header.nz_ != grid_size.z() ) )
      {
      vcl_cerr << "error: file on disk: " << storage_fname_ << " has size " << vgl_vector_3d<unsigned>(header.nx_,
                                                                                                       header.ny_,
                                                                                                       header.nz_)
               << vcl_endl
...
bool bvxm_voxel_storage_disk<T>::initialize_data(T const& value)
{
  ...
  bvxm_voxel_slab<T> init_slab(this->grid_size_.x(),
                               this->grid_size_.y(), 1, slab_buffer_, static_cast<T *>(slab_buffer_->data() ) );
...
bvxm_voxel_slab<T> bvxm_voxel_storage_disk<T>::get_slab(unsigned slice_idx, unsigned slab_thickness)
{
  ...
  bvxm_voxel_slab<T> slab(this->grid_size_.x(),
                          this->grid_size_.y(), slab_thickness, slab_buffer_,
                          reinterpret_cast<T *>(slab_buffer_->data() ) );
