#ifndef bvxm_voxel_storage_disk_cached_txx_
...
template <class T>
bvxm_voxel_storage_disk_cached<T>::bvxm_voxel_storage_disk_cached(vcl_string storage_filename,
                                                                  vgl_vector_3d<unsigned int> grid_size,
                                                                  vxl_int_64 max_cache_size)
  :  bvxm_voxel_storage<T>(grid_size), first_cache_slice_(-1), last_cache_slice_(-1), storage_fname_(storage_filename),
  fio_(0)
{
  // set up cache
  ...
  if( vul_file::exists(storage_fname_) )
    ...
    if( (header.nx_ != grid_size.x() ) || (header.ny_ != grid_size.y() ) || (header.nz_ != grid_size.z() ) )
      {
      vcl_cerr << "error: file on disk has size "
               << vgl_vector_3d<unsigned>(header.nx_, header.ny_, header.nz_) << vcl_endl
...
bool bvxm_voxel_storage_disk_cached<T>::purge_cache()
{
  ...
  vil_streampos write_len = (last_cache_slice_ - first_cache_slice_ + 1) * this->grid_size_.x() * this->grid_size_.y()
    * sizeof(T);
...
{
  vil_streampos pos =
    ( (vil_streampos)(this->grid_size_.x() * this->grid_size_.y() * slab_index) ) * sizeof(T)
    + sizeof(bvxm_voxel_storage_header<T> );
...
#endif // bvxm_voxel_storage_disk_cached_txx_
