...
bool bvxm_voxel_storage_slab_mem<T>::initialize_data(const T& value)
{
  ...
  for( unsigned slice_idx = 0; slice_idx < this->nslabs_; slice_idx++ )
    {
    bvxm_voxel_slab<T> grid_slab(this->grid_size_.x(),
                                 this->grid_size_.y(), 1, mem_[slice_idx],
                                 reinterpret_cast<T *>(mem_[slice_idx]->data() ) );
...
bvxm_voxel_slab<T> bvxm_voxel_storage_slab_mem<T>::get_slab(unsigned slice_idx, unsigned slab_thickness)
{
  ...
  bvxm_voxel_slab<T> slab(this->grid_size_.x(),
                          this->grid_size_.y(), slab_thickness_actual, mem_[slice_idx], first_voxel);
  return slab;
}
...
#endif // bvxm_voxel_storage_slab_mem_txx_
