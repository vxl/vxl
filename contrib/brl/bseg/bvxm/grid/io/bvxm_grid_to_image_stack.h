...
template <class T, unsigned N>
bool bvxm_grid_to_image_stack::write_grid_to_image_stack(bvxm_voxel_grid<vnl_vector_fixed<T, N> > * grid,
                                                         vcl_string directory)
{
  ...
  for( ; grid_it != grid->end(); ++grid_it, i++ )
    ...
    filename << directory << "/slab_" << vcl_setw(4) << vcl_setfill('0') <<  i
             << bvxm_image_traits<vnl_vector_fixed<T, N> >::extension();
    vil_image_view_base_sptr img = new vil_image_view<typename bvxm_image_traits<vnl_vector_fixed<T, N> >::pixel_type>(
        ni, nj);
