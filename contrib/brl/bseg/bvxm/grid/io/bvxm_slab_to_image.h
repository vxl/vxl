...
template <class T, unsigned N>
bool bvxm_slab_to_image::slab_to_image(bvxm_voxel_slab<vnl_vector_fixed<T, N> > const & slab,
                                       vil_image_view_base_sptr image)
{
  ...
  switch( image->pixel_format() )
    ...
    case VIL_PIXEL_FORMAT_RGBA_BYTE:
      if( image->nplanes() == 1 )
        {
        if( vil_image_view<vil_rgba<unsigned char> > * img_view =
              dynamic_cast<vil_image_view<vil_rgba<unsigned char> > *>(image.ptr() ) )
          {
...
// used for debugging
template <class T, unsigned N>
void bvxm_slab_to_image::write_slab_as_image(bvxm_voxel_slab<vnl_vector_fixed<T, N> > const& slab_in,
                                             vcl_string filename)
{
