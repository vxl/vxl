            ...
            for (unsigned i=0; i<(*grid_it).nx(); ){
                for (unsigned j=0; j < (*grid_it).ny(); ) {
                    if(((*grid_it)(i,j)[3] > threshold)){
                        ...
                        write_vrml_line(str, pt,dir,4*((*grid_it)(i,j)[3]-0.5),1,0,0);
                        write_vrml_disk(str, pt,dir,2*((*grid_it)(i,j)[3]-0.5),0,0.5,0.0);
                    }
                    j=j+s;
                }
                i=i+s;
                                           ...
                                           const float b)
{
  str << "Transform {\n"
      ...
      << "      coord Coordinate{\n"
      << "       point[\n";
    str << pt.x()-dir.x()*response*0.0 << ' ' << pt.y()-dir.y()*response*0.0 << ' ' << pt.z()-dir.z()*response*0.0 << '\n';

    str << pt.x()+dir.x()*response*0.5 << ' ' << pt.y()+dir.y()*response*0.5 << ' ' << pt.z()+dir.z()*response*0.5 << '\n';
    str << "   ]\n\n }"
        << "   coordIndex [\n";
    str << 0 << ',';
    str << 1 << ','
