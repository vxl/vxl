  ...
  for( unsigned i = 0; i < storage.nz(); i++ )
    ...
    for( vit = slab.begin(); vit != slab.end(); vit++, count++ )
      ...
      if( *vit != init_val )
        {
#ifdef DEBUG
        vcl_cerr << "error: read in value does not match init value! slice = " << i << ", count = " << count
                 << vcl_endl;
#endif
        init_check = false;
        }
  ...
  for( unsigned i = 0; i < storage.nz(); i++ )
    ...
    for( vit = slab.begin(); vit != slab.end(); vit++, count++ )
      ...
      if( *vit != static_cast<float>(count) )
        {
#ifdef DEBUG
        vcl_cerr << "error: read in value does not match written value! slice = " << i << ", count = " << count
                 << vcl_endl;
#endif
