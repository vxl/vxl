...
#include "bvxm_multi_scale_voxel_world.h"
//:
// \file
...
//: Destructor
bvxm_multi_scale_voxel_world::~bvxm_multi_scale_voxel_world()
{
}


//: equality operator
bool bvxm_multi_scale_voxel_world::operator == (bvxm_multi_scale_voxel_world const& that) const
{
  return true;
}
...
vgl_plane_3d<double> bvxm_multi_scale_voxel_world::fit_plane()
{
...
vcl_ostream&  operator<<(vcl_ostream& s, bvxm_multi_scale_voxel_world const& vox_world)
{
  ...
  s << "bvxm_multi_scale_voxel_world : " << params->num_voxels().x() << " x " << params->num_voxels().y() << " x " << params->num_voxels().z() << vcl_endl;
  return s;
}


//: save the occupancy grid in a ".raw" format readable by Drishti volume rendering software
bool bvxm_multi_scale_voxel_world::save_occupancy_raw(vcl_string filename, unsigned int scale)
{
...
bool bvxm_multi_scale_voxel_world::clean_grids()
{
...
bool bvxm_multi_scale_voxel_world::update_edges(bvxm_image_metadata const& metadata, unsigned int scale)
{
...
bool bvxm_multi_scale_voxel_world::expected_edge_image(bvxm_image_metadata const& camera,vil_image_view_base_sptr &expected,unsigned scale)
{
  // datatype for current appearance model
  typedef bvxm_voxel_traits<EDGES>::voxel_datatype edges_datatype;
...
void vsl_b_read(vsl_b_istream& is, bvxm_multi_scale_voxel_world* p)
{
  ...
  if (not_null_ptr)
  {
    p = new bvxm_multi_scale_voxel_world();
    vsl_b_read(is, *p);
  }
  else
    p = 0;
