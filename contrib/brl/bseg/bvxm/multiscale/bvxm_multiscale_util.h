#ifndef bvxm_multiscale_util_h_
#define bvxm_multiscale_util_h_
//:
// \file
...
#if 0 // only used in commented-out section
...
class bvxm_multiscale_util
{
 public:
  ...
  static bool write_cameras(const vcl_string filename,
                            vcl_vector<vnl_double_3x3> &Ks,
                            vcl_vector<vnl_double_3x3> &Rs,
                            vcl_vector<vnl_double_3x1> &Ts);
#endif // 0
  static void smooth_gaussian(vil_image_view_base_sptr img,
                              float stdx, float stdy);

  static void fill_1d_array(vil_image_view<float> const& input,
                            int y, float* output);

  static void half_resolution_1d(const float* input, unsigned width,
                                 float k0, float k1, float k2,
                                 float* output);

  static vil_image_view<float> half_resolution(vil_image_view<float> const& input,
                                               float filter_coef);
...
template<class T>
void bvxm_multiscale_util::warp_slab_bilinear(bvxm_voxel_slab<T> const& slab_in,
                                              vgl_h_matrix_2d<double> invH,
                                              bvxm_voxel_slab<T> &slab_out)
{
...
template<class T>
void bvxm_multiscale_util::warp_slab_nearest_neighbour(bvxm_voxel_slab<T> const& slab_in,
                                                       vgl_h_matrix_2d<double> invH,
                                                       bvxm_voxel_slab<T> &slab_out)
{
  vnl_matrix_fixed<double,3,3> Hd = invH.get_matrix();
  ...
  for (unsigned z=0; z<slab_out.nz(); ++z) {
    for (unsigned y=0; y<slab_out.ny(); ++y) {
      for (unsigned x=0; x<slab_out.nx(); ++x, ++out_it) {
        ...
        //The safe interpolatable region is [-0.5,slab_in.ni()-0.5)*[0,slab_in.nj()-0.5)

        int ix = int (pix_in_x + 0.5);
        int iy = int (pix_in_y + 0.5);

        if (ix >= 0 && ix < (int)slab_in.nx() && iy >= 0 && iy < (int)slab_in.ny())
          (*out_it) = slab_in_smooth(ix,iy,z);
        else
          (*out_it) = 0;
