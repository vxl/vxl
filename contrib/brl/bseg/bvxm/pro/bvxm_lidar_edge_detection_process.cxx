...
bool bvxm_lidar_edge_detection_process::execute()
{
  ...
  for (unsigned i=0; i<ni; i++){
    for (unsigned j=0; j<nj; j++){
      ...
      lidar_edges_prob(i,j) = curr_difference/(threshold_edge_difference+curr_difference);
      lidar_height(i,j) = image_first_return(i,j)*curr_difference/((threshold_edge_difference+image_first_return(i,j))*(threshold_edge_difference+curr_difference));
