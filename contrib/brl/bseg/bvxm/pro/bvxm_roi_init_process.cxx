...
bvxm_roi_init_process::bvxm_roi_init_process()
{
  ...
  if(!parameters()->add( "Uncertainty error" , "error" , (float) 10.0 ))
    vcl_cerr << "ERROR: Adding parameters in bvxm_roi_init_process" << vcl_endl;
                                     ...
                                     vpgl_local_rational_camera<double>& local_camera)
{
 // read the image and extract the camera
  ...
  if (prefix.compare("nitf") != 0) {
    vcl_cout << "bvxm_roi_init_process::execute - The image should be an NITF" << vcl_endl;
    return false;
  }

  vil_nitf2_image* nitf =  static_cast<vil_nitf2_image*> (img.ptr());
 /* if(!nitf->parse_headers()){
      vcl_cerr << "bvxm_roi_init_process::execute - NITF Image header not correct" << vcl_endl;
      return false;
  }
