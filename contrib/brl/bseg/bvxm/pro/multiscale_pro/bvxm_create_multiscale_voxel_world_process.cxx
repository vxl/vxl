#include "bvxm_create_multiscale_voxel_world_process.h"
//:
// \file
...
//: Constructor
bvxm_create_multiscale_voxel_world_process::bvxm_create_multiscale_voxel_world_process()
{
  ...
  output_types_[0]= "bvxm_multi_scale_voxel_world_sptr";
...
//: Execute the process
bool
bvxm_create_multiscale_voxel_world_process::execute()
{
  // Sanity check
  ...
  if (!parameters()->get_value(vcl_string("input_directory"), vox_dir)) {
    vcl_cout << "bvxm_create_multiscale_voxel_world_process::execute() -- problem in retrieving parameter input_directory\n";
    return false;
  }

  if (!vul_file::is_directory(vox_dir) || !vul_file::exists(vox_dir)) {
    vcl_cerr << "In bvxm_create_multiscale_voxel_world_process::execute() -- input directory is not valid!\n";
    return false;
  }

  vcl_cout << "In bvxm_create_multiscale_voxel_world_process::execute() -- input directory is: " << vox_dir << vcl_endl;
  ...
  if (!parameters()->get_value(vcl_string("lvcs"), lvcs_path)) {
    vcl_cout << "bvxm_create_multiscale_voxel_world_process::execute() -- problem in retrieving parameter lvcs_path\n";
    return false;
  }
  ...
  brdb_value_sptr output0 = new brdb_value_t<bvxm_multi_scale_voxel_world_sptr>(vox_world);
