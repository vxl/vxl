...
bool bvxm_create_mog_image_process(bprb_func_process& pro)
{
  ...
  if( voxel_type == "apm_mog_grey" )
    ...
    {
    vcl_cout << "In bvxm_normalize_image_process::execute() -- input appearance model: " << voxel_type
             << " is not supported\n";
    return false;
    }
  ...
  if( verbose_ )
    ...
    switch( mog_creation_method_ )
      ...
      default:
        vcl_cout << "In bvxm_normalize_image_process::norm_parameters() - unrecognized option: "
                 << mog_creation_method_ << " to create mog image\n"; return false;
      }
    }
  ...
  if( voxel_type == "apm_mog_grey" )
    {
    good = mix_gaussian<APM_MOG_GREY>(world, mog_creation_method_, bin_index, scale_index, n_samples, observation,
                                      mog_image);
    }
  else if( voxel_type == "apm_mog_mc_3_3" )
    {
    good = mix_gaussian<APM_MOG_MC_3_3>(world, mog_creation_method_, bin_index, scale_index, n_samples, observation,
                                        mog_image);
    }
  else if( voxel_type == "apm_mog_mc_4_3" )
    {
    good = mix_gaussian<APM_MOG_MC_4_3>(world, mog_creation_method_, bin_index, scale_index, n_samples, observation,
                                        mog_image);
    }
  else
    {
