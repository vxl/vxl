                  ...
                  bvxm_voxel_slab_base_sptr& mog_image)
{
  ...
  switch( mog_creation_method_ )
    ...
    case bvxm_mog_image_creation_methods::SAMPLING:
      done = world->mog_image_with_random_order_sampling<APM_T>(observation, n_samples, mog_image, bin_index,
                                                                scale_index);
      break;
    default:
      vcl_cout << "In bvxm_create_mog_image_process::mix_gaussian() - unrecognized option: " << mog_creation_method_
               << " to create mog image\n";
      return false;
    }
  if( !done )
