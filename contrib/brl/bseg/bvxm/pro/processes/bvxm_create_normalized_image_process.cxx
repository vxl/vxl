...
bool bvxm_create_normalized_image_process(bprb_func_process& pro)
{
  ...
  if( input_img->pixel_format() != VIL_PIXEL_FORMAT_BYTE )
    {
    vcl_cout
    << "In bvxm_create_normalized_image_process::execute() -- Input image pixel format is not VIL_PIXEL_FORMAT_BYTE!\n";
    return false;
    }
  ...
  if( !bvxm_normalization_util::normalize_image(in_image, out_image, a, b, (unsigned char)255) )
    {
    vcl_cout
    << "In bvxm_create_normalized_image_process::execute() -- Problems during normalization with given inputs\n";
    return false;
    }
