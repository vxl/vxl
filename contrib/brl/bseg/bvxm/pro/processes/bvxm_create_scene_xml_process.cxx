...
bool bvxm_create_scene_xml_large_scale_process(bprb_func_process& pro)
{
  ...
  vcl_cout << "pre-defined world size: " << world_size_in << ", voxel size: " << voxel_size
           << ", truncated world size: " << world_size << vcl_endl;
  ...
  for( unsigned i = 0; i < leaves.size(); i++ )
    ...
    if( !volm_io_tools::find_min_max_height(lower_left, upper_right, dem_infos, min, max) )
      {
      vcl_cerr << pro.name() << ": can not find height info for leave " << i << ", lower left: " << lower_left
    ...
    vpgl_lvcs_sptr lvcs = new vpgl_lvcs(lower_left.y(),
                                        lower_left.x(), min - height_sub, vpgl_lvcs::wgs84, vpgl_lvcs::DEG,
                                        vpgl_lvcs::METERS);
