...
void bvxm_camera_estimator::set_world_params(int nx, int ny, int nz, double sx, double sy, double sz, double dx,
                                             double dy, double dz)
{
  nx_ = nx;
  ny_ = ny;
...
void bvxm_camera_estimator::set_estimation_params(double theta_range, double theta_step, double phi_range,
                                                  double phi_step, double rot_range, double rot_step,
                                                  int max_iter_rot_angle)
{
...
double bvxm_camera_estimator::edge_prob_cross_correlation(const vil_image_view<float> & img1,
                                                    ...
                                                    vil_image_view<float> * img_eei)
{
  ...
  vgl_box_3d<double> box_3d(sx_, sy_, (sz_ + ( (double)nz_) * dz_), (sx_ + ( (double)nx_) * dx_),
                            (sy_ + ( (double)ny_) * dy_), sz_);
  ...
  for( int i = 0; i < ni; i++ )
    ...
    for( int j = 0; j < nj; j++ )
      ...
      if( poly_2d.contains(double(i), (double)j) )
        ...
        vgl_homg_line_3d_2_points<double> bp_h(vgl_homg_point_3d<double>(bp.point1() ), vgl_homg_point_3d<double>(
                                                 bp.point2() ) );
