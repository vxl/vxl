...
public:
  ...
  void set_estimation_params(double theta_range, double theta_step, double phi_range, double phi_step, double rot_range,
                             double rot_step, int max_iter_rot_angle);

  vil_image_view<float> convert_to_spherical_coordinates(const vil_image_view<float> & img,
  ...
  static vgl_polygon<double> convert_3d_box_to_2d_polygon(const vgl_box_3d<double> box_3d,
                                                          const vpgl_perspective_camera<double> * cam);
  ...
  void get_expected_edge_image(const bvxm_voxel_slab<float> & data, const vpgl_perspective_camera<double> * cam,
                               vil_image_view<float> * img_eei);

  template <class T1, class T2, class TR>
  vil_image_view<TR> estimate_offsets_fd(const vil_image_view<T1> & img1, const vil_image_view<T2> & img2,
                                         int & offset_x, int & offset_y, float & score);

  double estimate_rotation_angle(const vil_image_view<float> & img1c, const vil_image_view<float> & img2c);

  void estimate_rotation_iterative(const bvxm_voxel_slab<float> & data, const vil_image_view<float>& img_e,
...
public:
    ...
    : vnl_cost_function(2), data_(data), cest_(cest), img_e_(img_e), cam_(cam)
  {
    ...
    vpgl_perspective_camera<double> cam_centered(cam->get_calibration(), vgl_point_3d<double>(0.0, 0.0,
                                                                                              0.0),
                                                 cam->get_rotation() );
    ...
    vgl_line_3d_2_points<double> ln_center_x =
      cam_centered.backproject(vgl_point_2d<double>(im_center_x + 1.0, im_center_y) );
    vgl_line_3d_2_points<double> ln_center_y =
      cam_centered.backproject(vgl_point_2d<double>(im_center_x, im_center_y - 1.0) );
