                                       ...
                                       vil_image_view<float> * img_eei)
{
  ...
  if( camera->type_name() == "vpgl_perspective_camera" )
      ...
      for( int j = 0; j < nj; j++ )
        ...
        if( poly_2d.contains(double(i), (double)j) )
          ...
          vgl_homg_line_3d_2_points<double> bp_h(vgl_homg_point_3d<double>(bp.point1() ), vgl_homg_point_3d<double>(
                                                   bp.point2() ) );
          vgl_point_3d<double> pt_0(vgl_homg_operators_3d<double>::intersect_line_and_plane(bp_h,
                                                                                            plane_0) );
          vgl_point_3d<double> pt_1(vgl_homg_operators_3d<double>::intersect_line_and_plane(bp_h,
                                                                                            plane_1) );
    ...
    {
    vcl_cout << "bvxm_expected_edge_functor::apply() -- The camera type: " << camera->type_name()
             << " is not implemented yet!" << vcl_endl;
    return false;
    }
}

vcl_vector<vgl_point_3d<double> > bvxm_expected_edge_functor::convert_3d_box_to_3d_points(
  const vgl_box_3d<double> box_3d)
{
