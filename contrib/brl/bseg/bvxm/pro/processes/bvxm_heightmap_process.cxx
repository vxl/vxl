...
bool bvxm_heightmap_process(bprb_func_process& pro)
{
  ...
  if( is_negate )
    ...
    lvcs->local_to_global(ray_trace_end.x(), ray_trace_end.y(), ray_trace_end.z(), vpgl_lvcs::wgs84, lon, lat,
                          base_elev);
    // obtain the scene height
    float h = box.depth();
    vcl_cout << "Using scene height: " << h << " and scene floor elevation: " << base_elev
             << " to negate the depth map!\n";
    unsigned               ni = hmap->ni();
    unsigned               nj = hmap->nj();
