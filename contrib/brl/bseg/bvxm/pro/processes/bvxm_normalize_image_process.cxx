...
bool bvxm_normalize_image_process(bprb_func_process& pro)
{
  ...
  if( !mog_image )
    ...
    vcl_cout << pro.name()
             <<
    " bvxm_normalize_image_process(): Warning: -- Input 1 is an empty pointer! Either the world was empty, or a problem occurred during MOG creation, will return input image back.\n";
                                                           ...
                                                           float& a, float& b)
{
  ...
  if( !mog_image )
    {
    vcl_cout
      << "In bvxm_normalize_image_process::norm_parameters() - problems in creating mixture of gaussian image!\n";
    return false;
    }
