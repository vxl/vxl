...
bool bvxm_orthorectify_process(bprb_func_process& pro)
{
  ...
  if( ortho_height_img_sptr->pixel_format() != VIL_PIXEL_FORMAT_FLOAT )
    {
    vcl_cout << pro.name()
             << " -- input ortho height image needs to be a float image with ABSOLUTE heights of the scene!\n";
    return false;
    }
  ...
  if( input_img_sptr->pixel_format() != VIL_PIXEL_FORMAT_BYTE )
    {
    vcl_cout << pro.name()
             << " -- only input images of type BYTE are supported for now! (easy to add support for others)\n";
    return false;
    }
  ...
  vcl_cout << "Using scene height: " << h << " and scene floor absolute height: " << base_elev
           << " to convert the absolute heights to heights from scene ceiling!\n";
