...
bool bvxm_render_virtual_view_process(bprb_func_process& pro)
{
  ...
  switch( img_og->pixel_format() )
    ...
    case VIL_PIXEL_FORMAT_RGB_BYTE:
      ...
      break;
    default:
      vcl_cerr << "error: bvxm_render_virtual_view_process: unknown pixel format " << img_og->pixel_format()
               << vcl_endl;
      return false;
    }
  ...
  if( voxel_type == "apm_mog_grey" )
    ...
    if( img_og->nplanes() == 3 )
      ...
      {
      vcl_cerr << "error: bvxm_render_virtual_view_process: voxel_type(apm_mog_rgb) does not match, input image\n";
      return false;
      }
    }
    ...
    if( img_og->nplanes() == 2 )
      {
      result = world->virtual_view<APM_MOG_MC_2_3>(obs_og, camera_virtual, virtual_img, *vis_prob_view, bin_index,
                                                   scale_index);
      }
    else
      {
      vcl_cerr << "error: bvxm_render_virtual_view_process: voxel_type(apm_mog_mc_2_3) does not match, input image\n";
      return false;
      }
    }
    ...
    if( img_og->nplanes() == 3 )
      {
      result = world->virtual_view<APM_MOG_MC_3_3>(obs_og, camera_virtual, virtual_img, *vis_prob_view, bin_index,
                                                   scale_index);
      }
    else
      {
      vcl_cerr << "error: bvxm_render_virtual_view_process: voxel_type(apm_mog_mc_3_3) does not match, input image\n";
      return false;
      }
    }
    ...
    if( img_og->nplanes() == 4 )
      {
      result = world->virtual_view<APM_MOG_MC_4_3>(obs_og, camera_virtual, virtual_img, *vis_prob_view, bin_index,
                                                   scale_index);
      }
    else
      {
      vcl_cerr << "error: bvxm_render_virtual_view_process: voxel_type(apm_mog_mc_4_3) does not match, input image\n";
      return false;
      }
    }
  else
