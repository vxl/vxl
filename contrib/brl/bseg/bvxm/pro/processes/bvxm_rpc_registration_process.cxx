...
bool bvxm_rpc_registration_process(bprb_func_process& pro)
{
  ...
  int offset_search_size = bvxm_util::convert_uncertainty_from_meters_to_pixels(uncertainty,
                                                                                vox_world->get_params()->lvcs(),
                                                                                camera_inp);
  ...
  if( rpc_shift_3d_flag )
    ...
    vpgl_local_rational_camera<double> * cam_input_temp =
      dynamic_cast<vpgl_local_rational_camera<double> *>(camera_inp.ptr() );
  ...
  if( is_local_cam )
    ...
    vcl_cout << "Estimated changes with motion_mult in offsets (u,v)=(" << best_offset_u * (1.0 - motion_mult) << ','
             << best_offset_v * (1.0 - motion_mult) << ')' << vcl_endl;
