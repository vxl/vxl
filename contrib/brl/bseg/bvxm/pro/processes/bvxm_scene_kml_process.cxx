...
bool bvxm_scene_kml_process(bprb_func_process& pro)
{
  ...
  lvcs->local_to_global(corner.x() + dim_x, corner.y() + dim_y,
                        corner.z() + dim_z, vpgl_lvcs::wgs84, upp_rght_lon, upp_rght_lat, upp_rght_alt);
  ...
  if( vul_file::exists(kml_file) && !is_overwrite )
    ...
    for( unsigned i = 0; i < num_sheet; i++ )
      ...
      bkml_write::write_box(ofs, desc, box_info_t.str(), ul_t, ur_t, ll_t, lr_t, (unsigned char)r, (unsigned char)g,
                            (unsigned char)b);
      }
    bkml_write::write_box(ofs, name,
                          box_info.str(), ul, ur, ll, lr, (unsigned char)r, (unsigned char)g, (unsigned char)b,
                          (unsigned char)a);
