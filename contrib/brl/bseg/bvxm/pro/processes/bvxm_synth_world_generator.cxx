...
int bvxm_synth_world_generator::on_box_surface(vgl_box_3d<double> box, vgl_point_3d<double> v)
{
  ...
  in_box.set_min_point(vgl_point_3d<double>(box.min_x() + thickness, box.min_y() + thickness,
                                            box.min_z() + thickness) );
  in_box.set_max_point(vgl_point_3d<double>(box.max_x() - thickness, box.max_y() - thickness,
                                            box.max_z() - thickness) );
...
bvxm_synth_world_generator::generate_cameras_z(vgl_box_3d<double>& world)
{
  ...
  for( unsigned i = 0; i < centers.size(); i++ )
    ...
    vcl_vector<vgl_point_3d<double> > corners = bvxm_util::corners_of_box_3d(world);
                                           ...
                                           bvxm_voxel_world_sptr /*world*/)
{
  ...
  vcl_string path = world_dir_ + "/lidar";
...
bvxm_voxel_world_sptr bvxm_synth_world_generator::generate_world()
{
  ...
  gen_voxel_world_2box(grid_size, voxel_world, ocp_grid, intensity_grid, minx_, miny_, minz_, dimx_, dimy_, dimz_, nx_,
                       ny_, nz_, gen2_box_);
