...
def project_point(camera, x, y, z):
    ...
    bvxm_batch.set_input_double(3, z)
    bvxm_batch.run_process()
    (id, type) = bvxm_batch.commit_output(0)
    u = bvxm_batch.get_output_double(id)
    (id, type) = bvxm_batch.commit_output(1)
    v = bvxm_batch.get_output_double(id)
    return (u, v)
...
def convert_to_local_coordinates2(lvcs, lat, lon, el):
    ...
    bvxm_batch.set_input_double(3, el)
    bvxm_batch.run_process()
    (id, type) = bvxm_batch.commit_output(0)
    ...
    bvxm_batch.remove_data(id)
    (id, type) = bvxm_batch.commit_output(2)
    z = bvxm_batch.get_output_double(id)
    bvxm_batch.remove_data(id)
    return (x, y, z)
# convert lat,lon,el to local coordinates;
...
def convert_local_to_global_coordinates(lvcs, x, y, z):
    ...
    bvxm_batch.remove_data(id)
    (id, type) = bvxm_batch.commit_output(2)
    el = bvxm_batch.get_output_double(id)
    bvxm_batch.remove_data(id)
    return (lat, lon, el)
# convert lat,lon,el to local coordinates;
...
def find_connected_component(in_img, in_cam, threshold, out_kml, is_above=True):
    ...
    if status:
      ...
      (id, type) = bvxm_batch.commit_output(1)
      num_regions = bvxm_batch.get_output_unsigned(id)
...
def rational_camera_rotate_to_north(in_cam):
    bvxm_batch.init_process("vpglRationalCamRotationToNorthProcess")
    bvxm_batch.set_input_from_db(0, in_cam)
    status = bvxm_batch.run_process()
    if status:
      (id, type) = bvxm_batch.commit_output(0)
      ang_in_deg = bvxm_batch.get_output_double(id)
      return ang_in_deg
    else:
      return 0.0
