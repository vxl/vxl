...
#include "bvxm_bayesian_propagation.h"
...
bool
bvxm_bayesian_propagation::horizontal_pass(bvxm_fg_pair_density& fgp, bvxm_fg_bg_pair_density& fgbgp, bvxm_bg_pair_density& bgp)
{
...
bool 
bvxm_bayesian_propagation::directional_pass(bvxm_fg_pair_density& fgp, bvxm_fg_bg_pair_density& fgbgp, bvxm_bg_pair_density& bgp, int ii, int jj)
{
  //: for now make only one horizontal pass
...
bool 
bvxm_bayesian_propagation::horizontal_pass_using_prob_density_as_bgp(bvxm_fg_pair_density& fgp, 
                                                            bvxm_fg_bg_pair_density& fgbgp)
{
  ...
  for (unsigned i = 0; i < bg_map_.ni()-1; i++) {
    for (unsigned j = 0; j < bg_map_.nj(); j++) {
      ...
      if (i == 572 && j == 335) {
        vcl_cout << "--------- 572, 335 -----------\n";
        vcl_cout << "p_b_current: " << p_b_current << " p_f_current: " << p_f_current << vcl_endl;
        vcl_cout << "p_b_current1: " << p_b_current1 << " p_f_current1: " << p_f_current1 << vcl_endl;
        vcl_cout << " p_ff: " << p_ff << " p_fb: " << p_fb;
        vcl_cout << " p_bf: " << p_bf << " p_bb: " << p_bb << vcl_endl;
        vcl_cout << " m_ff: " << m_ff << " m_bb: " << m_bb << " m_fb: " << m_fb << " m_bf: " << m_bf << vcl_endl;
        vcl_cout << " p_h_bb: " << p_h_bb << " p_h_bf: " << p_h_bf << " p_h_fb: " << p_h_fb << " p_h_ff: " << p_h_ff << vcl_endl;
        vcl_cout << "the highest prob hypothesis was: ";
        if (p_h_bb >= p_h_bf && p_h_bb >= p_h_ff && p_h_bb >= p_h_fb)
          vcl_cout << " p_h_bb\n";
        else if (p_h_bf >= p_h_bb && p_h_bf >= p_h_ff && p_h_bf >= p_h_fb)
          vcl_cout << " p_h_bf\n";
...
bool 
bvxm_bayesian_propagation::vertical_pass_using_prob_density_as_bgp(bvxm_fg_pair_density& fgp, bvxm_fg_bg_pair_density& fgbgp)
{
  //: for now make only one horizontal pass
  for (unsigned i = 0; i < bg_map_.ni(); i++) {
...
//  jj is either -1 or 0 or 1 so can run in all 8 directions
bool 
bvxm_bayesian_propagation::directional_pass_using_prob_density_as_bgp(bvxm_fg_pair_density& fgp, bvxm_fg_bg_pair_density& fgbgp, int ii, int jj)
{
  //: for now make only one horizontal pass
  for (unsigned i = 0; i < bg_map_.ni(); i++) {
    for (unsigned j = 0; j < bg_map_.nj(); j++) {
...
bool 
bvxm_bayesian_propagation::initialize_bg_map(float high_prior, float top_percentile)
{
  //: initialize the map
  bg_map_.set_size(ni_, nj_);
  bvxm_bg_pair_density::initialize_bg_map(prob_density_, bg_map_, high_prior, top_percentile);
  return true;
}

//: use background mixture of gaussian image
bool 
bvxm_bayesian_propagation::run(bvxm_fg_pair_density& fgp, bvxm_fg_bg_pair_density& fgbgp, bvxm_bg_pair_density& bgp, 
                                int npasses)
{
...
bool 
bvxm_bayesian_propagation::pass_using_prob_density_as_bgp(bvxm_fg_pair_density& fgp, bvxm_fg_bg_pair_density& fgbgp, vil_image_view<float>& temp)
{
...
bool 
bvxm_bayesian_propagation::run_using_prob_density_as_bgp(bvxm_fg_pair_density& fgp, 
                                bvxm_fg_bg_pair_density& fgbgp,
                                int npasses)
{
...
float bvxm_area_density::back_density(float val) {
  return back_model_.prob_density(val);
}
float bvxm_area_density::fore_density(float val) {  
  return fore_model_.prob_density(val);
}

bool 
bvxm_bayesian_propagation::run_area(vil_image_view<float>& area_map, float lambda1, float k1, float lambda2, float k2)
{
  vil_image_view<float> temp(bg_map_.ni(), bg_map_.nj(), 1);
  vil_copy_deep(bg_map_, temp);

  //: construct density 
  bvxm_area_density d(lambda1, k1, lambda2, k2);
