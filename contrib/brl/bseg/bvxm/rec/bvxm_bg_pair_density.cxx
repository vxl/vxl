...
double bvxm_bg_pair_density::operator()(const rgb_obs_datatype y0, const rgb_obs_datatype y1)
{
...
bool bvxm_bg_pair_density::generate_mixture_image()
{
  if (!world_)
    return false;
...
bool
bvxm_bg_pair_density::generate_appearance_likelihood_map(vil_image_view<vxl_byte>& img, vil_image_view<float>& prob_map)
{
...
bvxm_bg_pair_density::initialize_bg_map(vil_image_view<float>& prob_density, vil_image_view<float>& map, float high_prior, float top_percentile)
{
  ...
  float val = -1e63f; // dummy initialisation, to avoid compiler warning
