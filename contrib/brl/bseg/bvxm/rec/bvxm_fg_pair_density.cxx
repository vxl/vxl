...
double bvxm_fg_pair_density::operator()(const double y0, const double y1)
{
...
double bvxm_fg_pair_density::negative_log(const double y0, const double y1)
{
  return -vcl_log(this->operator ()(y0, y1));
}

double bvxm_fg_pair_density::gradient_of_log(const double y0, const double y1)
{
  double ss = vcl_pow(sigma_, 2.0);
  double B = vcl_exp(-vcl_pow(y0-y1, 2.0)/(2*ss));
...
double bvxm_fg_pair_density::gradient_of_negative_log(const double y0, const double y1)
{
  double ss = vcl_pow(sigma_, 2.0);
  double B = vcl_exp(-vcl_pow(y0-y1, 2.0)/(2*ss));
  double B1 = vcl_exp(vcl_pow(y0-y1, 2.0)/(2*ss));
