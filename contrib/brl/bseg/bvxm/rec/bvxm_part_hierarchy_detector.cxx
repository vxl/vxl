...
bvxm_part_hierarchy_detector::~bvxm_part_hierarchy_detector()
{
  ...
  vcl_map<unsigned, vcl_vector<bvxm_part_instance_sptr> >::iterator it2 = map_instance_.begin();
...
bool bvxm_part_hierarchy_detector::detect(vil_image_resource_sptr img)
{
  //: start from the primitives
  vcl_vector<bvxm_part_instance_sptr> dumm_ins = h_->get_dummy_primitive_instances();
...
bool bvxm_part_hierarchy_detector::detect(vil_image_resource_sptr img, float angle) {
  //: start from the primitives
  vcl_vector<bvxm_part_instance_sptr> dumm_ins = h_->get_dummy_primitive_instances();
  vcl_vector<bvxm_part_instance_sptr> parts_prims;
  for (unsigned i = 0; i < dumm_ins.size(); i++) {
...
bvxm_part_instance_sptr 
bvxm_part_hierarchy_detector::exists(bvxm_part_base_sptr upper_p, 
                                     bvxm_part_instance_sptr central_p, unsigned ni, unsigned nj,
                                     ...
                                     float det_threshold)
{
  ...
  bvxm_part_hierarchy::edge_iterator eit = upper_p->out_edges_begin();
...
void bvxm_part_hierarchy_detector::extract_upper_layer(vcl_vector<bvxm_part_instance_sptr>& extracted_parts, unsigned ni, unsigned nj, 
                                                       ...
                                                       vcl_vector<bvxm_part_instance_sptr>& extracted_upper_parts)
{
  ...
  for (unsigned i = 0; i < extracted_parts.size(); i++) {
    ...
    bvxm_part_base_sptr hp = h_->get_node(p->layer_, p->type_);
    if (!hp)
      continue;
    ...
    for (bvxm_part_hierarchy::edge_iterator eit = hp->in_edges_begin(); eit != hp->in_edges_end(); eit++) {
      if (hp == (*eit)->source()->central_part()) {
        ...
        bvxm_part_instance_sptr hp_upper_instance = exists(hp_upper, p, ni, nj, extracted_parts_rtree, hp_upper->detection_threshold_); // p will be its central part and map will tell if all the other parts exist
