...
class rtree_bvxm_instance_box_2d 
{
 ...
 public:
  typedef bvxm_part_instance_sptr v_type;
  typedef vgl_box_2d<T> b_type;
  typedef T t_type;

  // Operations------
  static void  init  (vgl_box_2d<T>& b, bvxm_part_instance_sptr const& pi)
  { vgl_point_2d<T> p(pi->x_, pi->y_); b = vgl_box_2d<T>();  b.add(p); }

  static void  update(vgl_box_2d<T>& b, bvxm_part_instance_sptr const& pi)
  ...
  static bool  meet(vgl_box_2d<T> const& b, bvxm_part_instance_sptr const& pi)
  ...
  static bool meets(bvxm_part_instance_sptr const& pi, vgl_polygon<T> poly)
...
// rtree definitions
typedef bvxm_part_instance_sptr V_type;  // the contained object type
typedef vgl_box_2d<float> B_type;    // the bounding object type
typedef rtree_bvxm_instance_box_2d<float> C_type; // the helper class
...
public:
  ...
  void extract_upper_layer(vcl_vector<bvxm_part_instance_sptr>& extracted_parts, unsigned ni, unsigned nj, Rtree_type* extracted_parts_rtree,
...
public:
  ...
  vcl_map<unsigned, vcl_vector<bvxm_part_instance_sptr> > map_instance_;
...
#endif  //bvxm_part_hierarchy_detector_h_
