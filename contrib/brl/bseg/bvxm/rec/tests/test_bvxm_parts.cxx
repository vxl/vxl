...
static void test_bvxm_parts()
{
  ...
  bvxm_part_hierarchy::generate_map(parts_0_1, map, type_map);
  ...
  bvxm_part_hierarchy::generate_map(parts_0_1, part_map);

  vcl_vector<bvxm_part_instance_sptr> parts_1_0;
  bvxm_part_instance_sptr ins = h_8->exists(p_1_0, parts_0_1[0], map, type_map, part_map, 0.1f); // p will be its central part and map will tell if all the other parts exist
  ...
  vcl_cout << "strength is: " << ins->strength_ << vcl_endl;
/*
  bvxm_part_hierarchy::generate_map(parts_1_0, map, type_map);  
  vil_math_value_range(map, min, max);
...
*/
  ...
  //  parts_1_0.push_back(ins);
  parts_1_0.clear();
  h_8->extract_upper_layer(parts_0_1, ni, nj, 0.1f, parts_1_0);
  TEST_NEAR("extracting layer 1", parts_1_0.size(), 34, 0.1);
  ...
  vcl_vector<bvxm_part_instance_sptr> parts_2_0;
  h_8->extract_upper_layer(parts_1_0, ni, nj, 0.1f, parts_2_0);
  TEST_NEAR("extracting layer 1", parts_2_0.size(), 16, 0.1);

  bvxm_part_hierarchy::generate_map(parts_2_0, map, type_map);  
  vil_math_value_range(map, min, max);
  ...
  bvxm_part_hierarchy::generate_output_map(parts_2_0, output_map_float);
