...
void create_regular_world_scene_xml(boxm2_scene& new_scene, bvxm_world_params_sptr params, bvxm_voxel_world_sptr world,
                                    int max_level)
{
  vcl_cout << "boxm2 scene directory: " << new_scene.data_path() << vcl_endl;
  vcl_cout << "setting 1 block only. number of subblocks, dimx: " << params->num_voxels().x() << " dimy: "
           << params->num_voxels().y() << " dimz: " << params->num_voxels().z() << vcl_endl;

  vgl_point_3d<double> local_orig( (double)params->corner().x(), (double)params->corner().y(),
                                   (double)params->corner().z() );
  ...
  boxm2_block_metadata data(block_id, local_orig, sub_block_dim, sub_block_num, 1, max_level, 650.0,
                            params->min_occupancy_prob() );
...
// this method creates a regular boxm2 world, i.e. each subblock/octree is one voxel of the bvxm_world, octrees are not refined/sub-divided.
void initialize_regular_world_scene(boxm2_scene_sptr new_scene, boxm2_cache_sptr cache, bvxm_world_params_sptr params,
...
int main(int argc, char* * argv)
{
  ...
  vul_arg<bool>       only_create_xml("-only_scene_xml",
                                      "if present creates boxm2 scene xml file by converting bvxm scene file and exists",
                                      false);
