  ...
  sdet_linear_curve_model_base(), min_theta(0.0), max_theta(0.0)
{
  ...
  sdet_intersect_angle_range( ( (sdet_simple_linear_curve_model *)cm1)->min_theta,
                              ( (sdet_simple_linear_curve_model *)cm1)->max_theta,
                              ( (sdet_simple_linear_curve_model *)cm2)->min_theta,
                              ( (sdet_simple_linear_curve_model *)cm2)->max_theta,
                              min_theta, max_theta);
...
void sdet_simple_linear_curve_model::compute_curve_bundle(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e,
...
bool sdet_simple_linear_curve_model::curve_fit_is_reasonable(vcl_deque<sdet_edgel *> & edgel_chain,
  ...
  sdet_linear_curve_model_base()
{
  ...
  // compute new curve bundle
  cv_bundle = vgl_clip( ( (sdet_linear_curve_model *)cm1)->cv_bundle, ( (sdet_linear_curve_model *)cm2)->cv_bundle,
...
// : compute the linear curve bundle for an edgel pair at the ref edgel
void sdet_linear_curve_model::compute_curve_bundle(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e,
                                                   double dpos, double dtheta, double /*token_len*/)
{
  // determine the intrinsic parameters for this edgel pair
  sdet_int_params params = sdet_get_intrinsic_params(e1->pt, e2->pt, e1->tangent, e2->tangent);
  ...
  if( ref_e == e1 )
    ...
    if( sdet_intersect_angle_range_mpi_pi(-dtheta, dtheta, -params.t1 - alpha, -params.t1 + alpha, a1, b1) )
      {
      // intersected with domain 3
      if( sdet_intersect_angle_range_mpi_pi(a1, b1, params.t2 - params.t1 - dtheta, params.t2 - params.t1 + dtheta, a2,
                                            b2) )
        {
        cv_bundle.new_sheet();
    ...
    if( sdet_intersect_angle_range_mpi_pi(-dtheta, dtheta, -params.t2 - alpha, -params.t2 + alpha, a1, b1) )
      {
      // intersected with domain 3
      if( sdet_intersect_angle_range_mpi_pi(a1, b1, params.t1 - params.t2 - dtheta, params.t1 - params.t2 + dtheta, a2,
                                            b2) )
        {
        cv_bundle.new_sheet();
        cv_bundle.push_back( dpos, a2 + alpha);
...
vgl_point_2d<double>  sdet_linear_curve_model::compute_best_fit(vcl_deque<sdet_edgel *> & /*edgel_chain*/)
{
  ...
  pt =
    vgl_point_2d<double>(ref_pt.x() + dx * vcl_cos(ref_theta + vnl_math::pi_over_2),
                         ref_pt.y() + dx * vcl_sin(ref_theta + vnl_math::pi_over_2) );
  theta = ref_theta + dt;
...
// : function to check if the curve fit is reasonable
bool sdet_linear_curve_model::curve_fit_is_reasonable(vcl_deque<sdet_edgel *> & edgel_chain, sdet_edgel * /*ref_e*/,
                                                      double /*dpos*/)
{
  compute_best_fit(edgel_chain);
  return true;
...
sdet_CC_curve_model::sdet_CC_curve_model(sdet_curve_model* cm1, sdet_curve_model* cm2)
{
  ...
  cv_bundle = vgl_clip( ( (sdet_CC_curve_model *)cm1)->cv_bundle, ( (sdet_CC_curve_model *)cm2)->cv_bundle,
                        vgl_clip_type_intersect);
}
...
void sdet_CC_curve_model::compute_curve_bundle(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e,
...
void sdet_CC_curve_model::compute_curve_bundle(vgl_polygon<double>& bundle, sdet_int_params & params,
...
// : function to check if the curve fit is reasonable
bool sdet_CC_curve_model::curve_fit_is_reasonable(vcl_deque<sdet_edgel *> & edgel_chain, sdet_edgel * /*ref_e*/,
                                                  double /*dpos*/)
{
  compute_best_fit(edgel_chain);
  return true;
...
// : Constructor 1: From a pair of edgels
sdet_CC_curve_model_new::sdet_CC_curve_model_new(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e,
                                                 double dpos, double dtheta,
                                                 double token_len, double max_k, double /*max_gamma*/,
                                                 bool /*adaptive*/) :
...
// : constructor 3: From the intersection of two curve bundles
sdet_CC_curve_model_new::sdet_CC_curve_model_new(sdet_CC_curve_model_new* cm1, sdet_CC_curve_model_new* cm2) :
...
sdet_CC_curve_model_new::sdet_CC_curve_model_new(sdet_CC_curve_model_new* cm, vgl_point_2d<double> pt, double theta) :
  pt(0.0, 0.0), theta(0), k(0.0)
{
  type = CC2;
...
sdet_curve_model * sdet_CC_curve_model_new::transition(sdet_curve_model* cm, int& trans_k, int& trans_type,
                                                       double & bundle_size)
{
  if( cm->type != CC2 )
    {
  ...
  for( int i = 0; i < NkClasses; i++ )
    ...
    for( int j = 0; j < NkClasses; j++ )
      ...
      if( i == j )
        {
        continue;         // model continuations not transitions
  ...
  for( int i = 0; i < NkClasses; i++ )
    ...
    vgl_polygon<double> cb1(1); cb1.push_back(dx_min, -0.5); cb1.push_back(dx_max, -0.5); cb1.push_back(dx_max, 0.5);
    cb1.push_back(dx_min, 0.5);
...
sdet_curve_model * sdet_CC_curve_model_new::consistent_transition(sdet_curve_model* cm, int& trans_k, int& trans_type,
                                                                  double & gc_cost)
{
  if( cm->type != CC2 )
    {
  ...
  for( int i = 0; i < NkClasses; i++ )
    ...
    for( int j = 0; j < NkClasses; j++ )
      {
      if( next_cm->cv_bundles[j].num_sheets() != 1 )
        {
        continue;                                           // only legal ones
...
// : compute the CC curve bundle for an edgel pair at the ref edgel
void sdet_CC_curve_model_new::compute_curve_bundle(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e,
                                                   double dpos, double dtheta, double token_len, double /*max_k*/)
{
  // compute the curvature bounds due to the token length and dpos
  double dk = 2 * dpos / (token_len * token_len / 4.0 + dpos * dpos);
  ...
  for( int i = 0; i < NkClasses; i++ )
    ...
    if( vcl_fabs(sdet_k_classes[i]) < dk ) // only the legal curve classes
      ...
      bool                sgn_change;
      vgl_polygon<double> trans_CB = transport_bundle_from(omega, sdet_k_classes[i], neigh_e->pt, neigh_e->tangent,
                                                           sgn_change);
...
vgl_polygon<double> sdet_CC_curve_model_new::transport_bundle_from(const vgl_polygon<double> & cv_bundle, double k,
...
// : Transport curve bundle from the given extrinsic point-tangent to the current point-tanget for the given curve model
vgl_polygon<double> sdet_CC_curve_model_new::transport_bundle_to(const vgl_polygon<double> & cv_bundle, double k,
                                                                 vgl_point_2d<double> pt, double theta,
                                                                 bool & sgn_change)
{
...
vgl_polygon<double> sdet_CC_curve_model_new::transport_bundle(const vgl_polygon<double> & cv_bundle, double k,
...
// : function to check if the curve fit is reasonable
bool sdet_CC_curve_model_new::curve_fit_is_reasonable(vcl_deque<sdet_edgel *> & edgel_chain, sdet_edgel * /*ref_e*/,
                                                      double /*dpos*/)
{
...
// : report accuracy of measurement
void sdet_CC_curve_model_new::report_accuracy(double * estimates, double * min_estimates, double * max_estimates)
{
  double theta_min = 1000.0, theta_max = -1000.0, k_min = 1000.0, k_max = -1000.0;
...
sdet_CC_curve_model_perturbed::sdet_CC_curve_model_perturbed(sdet_curve_model* cm1, sdet_curve_model* cm2)
{
  ...
  for( int i = 0; i < NperturbPCC; i++ )
    ...
    cv_bundles[i] =
      vgl_clip( ( (sdet_CC_curve_model_perturbed *)cm1)->cv_bundles[i],
                ( (sdet_CC_curve_model_perturbed *)cm2)->cv_bundles[i], vgl_clip_type_intersect);
    if( cv_bundles[i].num_sheets() > 1 )
      {
...
// : compute the CC curve bundle for an edgel pair at the ref edgel
void sdet_CC_curve_model_perturbed::compute_curve_bundle(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e,
                                                         double dpos, double dtheta, double token_len, double max_k)
{
...
// : constructor 4: By transporting a curve model to another extrinsic anchor point
sdet_CC_curve_model_3d::sdet_CC_curve_model_3d(sdet_CC_curve_model_3d* cm, vgl_point_2d<double> pt, double theta) :
  pt(0.0, 0.0), theta(0), k(0.0)
{
  type = CC3d;
...
sdet_CC_curve_model_3d::sdet_CC_curve_model_3d(sdet_edgel* ref_e, double dpos, double dtheta, double /*token_len*/,
                                               double max_k, double /*max_gamma*/, bool adaptive) :
...
void sdet_CC_curve_model_3d::compute_curve_bundle(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e,
                                                  double dpos, double dtheta, double /*token_len*/, double max_k,
                                                  bool adaptive)
{
  // determine the size of the uncertainty at the reference edgel
  ...
  if( (DX * mycos(T2) + DY * mysin(T2) ) < 0 )
    ...
    for( unsigned i = 0; i < Kmax.rows(); ++i )
      ...
      for( unsigned j = 0; j < Kmax.cols(); ++j )
        ...
        const float k_dt_min =
          static_cast<float>(sin_t2_p_dt2_m_t0_m_dt
                             / (-sin_t2_p_dt2_m_t0_m_dt * dx2 + cos_T0mdt_times_DX + sin_T0mdt_times_DY) );
        // compute the intersection of all the surfaces to compute the final bundle
        ...
        const float k_dt_max =
          static_cast<float>(sin_t2_p_dt2_m_t0_p_dt
                             / (-sin_t2_p_dt2_m_t0_p_dt * dx2 + cos_T0pdt_times_DX + sin_T0pdt_times_DY) );
        // compute the intersection of all the surfaces to compute the final bundle
...
// : function to check if the curve fit is reasonable
bool sdet_CC_curve_model_3d::curve_fit_is_reasonable(vcl_deque<sdet_edgel *> & edgel_chain, sdet_edgel * /*ref_e*/,
                                                     double /*dpos*/)
{
  // compute LG-ratio and store as the quality
...
// : report accuracy of measurement
void sdet_CC_curve_model_3d::report_accuracy(double * estimates, double * min_estimates, double * max_estimates)
{
  double theta_min = 1000.0, theta_max = -1000.0, k_min = 1000.0, k_max = -1000.0;
  ...
  pt( ( (sdet_ES_curve_model *)cm1)->pt), theta( ( (sdet_ES_curve_model *)cm1)->theta), k(0.0), gamma(0.0)
{
  ...
  cv_bundle = vgl_clip( ( (sdet_ES_curve_model *)cm1)->cv_bundle, ( (sdet_ES_curve_model *)cm2)->cv_bundle,
                        vgl_clip_type_intersect);
}
...
void sdet_ES_curve_model::compute_curve_bundle(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e,
                                               double dpos, double dtheta,
                                               double token_len, double max_k, double max_gamma)
{
  // determine the intrinsic parameters for this edgel pair
...
void sdet_ES_curve_model::compute_curve_bundle(vgl_polygon<double>& bundle,
  ...
  sdet_ES_curve_model()
{
  ...
  for( int i = 0; i < Nperturb; i++ )
    ...
    cv_bundles[i] =
      vgl_clip( ( (sdet_ES_curve_model_perturbed *)cm1)->cv_bundles[i],
                ( (sdet_ES_curve_model_perturbed *)cm2)->cv_bundles[i], vgl_clip_type_intersect);
    if( cv_bundles[i].num_sheets() > 1 )
      {
      cv_bundles[i] = vgl_polygon<double>(); // remove these erroneous curve bundles
      }
...
// : compute the ES curve bundle for an edgel pair at the ref edgel
void sdet_ES_curve_model_perturbed::compute_curve_bundle(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e,
                                                         double dpos, double dtheta,
                                                         double token_len, double max_k, double max_gamma)
{
  // for each discrete perturbation compute a curve bundle
  for( int i = 0; i < Nperturb; i++ )
    ...
    if( ref_e == e1 )
      ...
      if( test_passed )
        {
        sdet_ES_curve_model::compute_curve_bundle(cv_bundles[i], params, true, dpos, dtheta, token_len, max_k,
                                                  max_gamma);
        }
      }
    else
      ...
      if( test_passed )
        {
        sdet_ES_curve_model::compute_curve_bundle(cv_bundles[i], params, false, dpos, dtheta, token_len, max_k,
                                                  max_gamma);
        }
      }
    }
...
void sdet_ES_curve_model_perturbed::print_info()
{
  ...
  for( int i = 0; i < Nperturb; i++ )
    {
    vcl_cout << "                             ";
    vcl_cout << "pos: " << i << " : (th=" << tangents[i] << ", k=" << ks[i] << ", gamma=" << gammas[i] << " "
             << vcl_endl;
    }
}
