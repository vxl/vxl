...
public:
  ...
  virtual sdet_curve_model * transition(sdet_curve_model * /*cm*/, int & /*trans_k*/, int & /*trans_type*/,
                                        double & /*bundle_size*/) { return 0; }                                                              // temp :make it virtual
  ...
  virtual vgl_point_2d<double> compute_best_fit(vcl_deque<sdet_edgel *> & edgel_chain) = 0;

  // : function to check if the curve fit is reasonable
  virtual bool curve_fit_is_reasonable(vcl_deque<sdet_edgel *> & edgel_chain, sdet_edgel* ref_e, double dpos) = 0;
...
public:
  ...
  // : constructor 1
  sdet_linear_curve_model_base(vgl_point_2d<double> new_pt, double new_theta) : pt(new_pt),
    theta(new_theta) { type = LINEAR; }
...
public:
  ...
  sdet_simple_linear_curve_model(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e, double dpos, double dtheta,
                                 double token_len, double max_k, double max_gamma, bool adaptive);

  // : constructor 2: From a set of edgels
  sdet_simple_linear_curve_model(vcl_vector<sdet_edgel *> /*edgels*/, sdet_edgel * /*ref_edgel*/) {}
  ...
  void compute_curve_bundle(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e, double dpos, double dtheta,
                            double token_len);

  // : Compute the best fit curve from the curve bundle
  virtual vgl_point_2d<double> compute_best_fit(vcl_deque<sdet_edgel *> & edgel_chain);

  // : function to check if the curve fit is reasonable
  virtual bool curve_fit_is_reasonable(vcl_deque<sdet_edgel *> & edgel_chain, sdet_edgel* ref_e, double dpos);
...
public:
  ...
  // : Constructor 1: From a pair of edgels
  sdet_linear_curve_model(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e, double dpos, double dtheta,
                          double token_len, double max_k, double max_gamma, bool adaptive);

  // : constructor 2: From a set of edgels
  sdet_linear_curve_model(vcl_vector<sdet_edgel *> /*edgels*/, sdet_edgel * /*ref_edgel*/) {}
...
public:
  ...
  // : constructor
  sdet_CC_curve_model(vgl_point_2d<double> new_pt, double new_tangent) : pt(new_pt), tangent(new_tangent), theta(0), k(
      0.0) { type = CC; }
  ...
  // : Constructor 1: From a pair of edgels
  sdet_CC_curve_model(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e, double dpos, double dtheta, double token_len,
                      double max_k, double max_gamma, bool adaptive);

  // : constructor 2: From a set of edgels
  sdet_CC_curve_model(vcl_vector<sdet_edgel *> /*edgels*/, sdet_edgel * /*ref_edgel*/) {}
  ...
  void compute_curve_bundle(vgl_polygon<double>& bundle, sdet_int_params & params, bool first_is_ref, double dpos,
                            double dtheta, double token_len, double max_k);
...
public:
  ...
  // : constructor
  sdet_CC_curve_model_new(vgl_point_2d<double> new_pt, double new_tangent) : ref_pt(new_pt), ref_theta(new_tangent), pt(
      new_pt), theta(new_tangent), k(0.0) { type = CC2; }

  // : destructor
  virtual ~sdet_CC_curve_model_new() {}

  // : Constructor 1: From a pair of edgels
  sdet_CC_curve_model_new(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e, double dpos, double dtheta,
                          double token_len, double max_k, double max_gamma, bool adaptive);

  // : constructor 2: From a set of edgels
  sdet_CC_curve_model_new(vcl_vector<sdet_edgel *> /*edgels*/, sdet_edgel * /*ref_edgel*/) {}
  ...
  vgl_polygon<double> transport_bundle(const vgl_polygon<double> & cv_bundle, double k, vgl_point_2d<double> spt,
                                       double stheta, vgl_point_2d<double> dpt, double dtheta, bool & sgn_change);

  // : Transport a curve bundle from the given anchor to the current anchor
  vgl_polygon<double> transport_bundle_from(const vgl_polygon<double> & cv_bundle, double k, vgl_point_2d<double> pt,
                                            double theta, bool & sgn_change);

  // : Transport a curve bundle from the current anchor to a new anchor
  vgl_polygon<double> transport_bundle_to(const vgl_polygon<double> & cv_bundle, double k, vgl_point_2d<double> pt,
                                          double theta, bool & sgn_change);

  // : Transport a single CC curve
  vgl_point_2d<double> transport_CC(vgl_point_2d<double> dx_dt, double k, vgl_point_2d<double> spt, double stheta,
                                    vgl_point_2d<double> dpt, double dtheta, bool & sgn_change);
...
public:
  ...
  // : Constructor 1: From a pair of edgels
  sdet_CC_curve_model_perturbed(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e, double dpos, double dtheta,
                                double token_len, double max_k, double max_gamma, bool adaptive);

  // : constructor 2: From a set of edgels
  sdet_CC_curve_model_perturbed(vcl_vector<sdet_edgel *> edgels, sdet_edgel* ref_edgel) : sdet_CC_curve_model(edgels,
                                                                                                              ref_edgel)
  {}
  ...
  void compute_perturbed_position_of_an_edgel(sdet_edgel* e, int per_ind, double dpos, double dtheta,
...
public:
  ...
  sdet_CC_curve_model_3d() :
    Kmin(1, 1, 0.0f), Kmax(1, 1, 0.0f), ref_pt(0.0, 0.0), ref_theta(0.0), pt(0.0, 0.0), theta(0),
    k(0.0) { type = CC3d; }

  // : constructor
  sdet_CC_curve_model_3d(vgl_point_2d<double> new_pt, double new_tangent) :
    Kmin(1, 1, 0.0f), Kmax(1, 1, 0.0f), ref_pt(new_pt), ref_theta(new_tangent), pt(new_pt), theta(new_tangent),
    k(0.0) { type = CC3d; }
  ...
  sdet_CC_curve_model_3d(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e,
                         ...
                         bool adaptive) :
    Kmin(1, 1, (float)-max_k), Kmax(1, 1, (float)max_k), ref_pt(ref_e->pt), ref_theta(ref_e->tangent), pt(0.0, 0.0),
    theta(0), k(0.0)
  {
    type = CC3d;
  ...
  // : constructor 2: From a set of edgels
  sdet_CC_curve_model_3d(vcl_vector<sdet_edgel *> /*edgels*/, sdet_edgel * /*ref_edgel*/) {}
  ...
  sdet_CC_curve_model_3d(sdet_edgel* ref_e, double dpos, double dtheta, double token_len, double max_k,
                         double max_gamma, bool adaptive);
  ...
  // : compute the CC curve bundle for an edgel pair given expected errors
  void compute_curve_bundle(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e, double dpos, double dtheta,
                            double token_len, double max_k, bool adaptive);
...
public:
  ...
  // : Constructor 1: From a pair of edgels
  sdet_ES_curve_model(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e, double dpos, double dtheta, double token_len,
                      double max_k, double max_gamma, bool adaptive);

  // : constructor 2: From a set of edgels
  sdet_ES_curve_model(vcl_vector<sdet_edgel *> /*edgels*/, sdet_edgel * /*ref_edgel*/) : k(0.0), gamma(0.0) {}
  ...
  void compute_curve_bundle(vgl_polygon<double>& bundle, sdet_int_params & params, bool first_is_ref, double dpos,
                            double dtheta, double token_len, double max_k, double max_gamma);
  ...
  double compute_distance(vcl_deque<sdet_edgel *> & edgel_chain, vgl_point_2d<double> pt, double theta, double k,
                          double gamma);
...
public:
  ...
  // : Constructor 1: From a pair of edgels
  sdet_ES_curve_model_perturbed(sdet_edgel* e1, sdet_edgel* e2, sdet_edgel* ref_e, double dpos, double dtheta,
                                double token_len, double max_k, double max_gamma, bool adaptive);

  // : constructor 2: From a set of edgels
  sdet_ES_curve_model_perturbed(vcl_vector<sdet_edgel *> edgels, sdet_edgel* ref_edgel) : sdet_ES_curve_model(edgels,
                                                                                                              ref_edgel)
  {}
