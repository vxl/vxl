                                ...
                                vil_image_view<float> const& varv)
{
  ...
  //compute the minium variance along the path between pix0 and pix1
  bool init = true;
  ...
  if(n1>0.0)
    v_avg1 = sum1/n1;
  //the differnce between depth values averaged over the neighbrd
...
bool sdet_denoise_mrf::denoise()
{
  ...
  vcl_cout << "formed incidence and auxillary matrices in "
           << t.real()/1000.0 << " seconds\n";
  t.mark();
  this->compute_F();
  vcl_cout << "solved LU decomposition and back subsitution in "
           << t.real()/1000.0 << " seconds\n";
  int ni = in_resc_->ni(), nj = in_resc_->nj();
