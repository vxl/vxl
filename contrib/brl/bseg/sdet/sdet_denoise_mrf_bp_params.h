...
public:
  ...
  sdet_denoise_mrf_bp_params(unsigned n_labels = 256, unsigned n_iter = 5, unsigned pyramid_levels = 5,
                             float discontinuity_cost = 200.0f, float truncation_cost = 10000.0f, float kappa = 1.0,
                             float lambda = 0.05f);
...
protected:
  void InitParams(unsigned n_labels, unsigned n_iter, unsigned pyramid_levels, float discontinuity_cost,
