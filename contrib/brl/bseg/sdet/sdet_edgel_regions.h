...
public:
  ...
  ~sdet_edgel_regions();
  // Main process method
  bool compute_edgel_regions(vil1_image const& image, vcl_vector<vtol_edge_2d_sptr>& sgrp,
                             vcl_vector<vtol_intensity_face_sptr>& faces);

  bool compute_edgel_regions(vil_image_resource_sptr const& image, vcl_vector<vtol_edge_2d_sptr>& sgrp,
                             vcl_vector<vtol_intensity_face_sptr>& faces);

  bool compute_edgel_regions(gevd_bufferxy* buf, vcl_vector<vtol_edge_2d_sptr>& sgrp,
                             vcl_vector<vtol_intensity_face_sptr>& faces);
...
protected:
  ...
  bool merge_equivalence(vcl_map<unsigned int, vcl_vector<unsigned int> *>& tab, unsigned int cur_label,
                         unsigned int label);

  bool get_next_label(vcl_vector<unsigned int>* labels, unsigned int& label);

  void print_edge_colis(unsigned int x, unsigned int y, sdet_region_edge_sptr const & r1,
