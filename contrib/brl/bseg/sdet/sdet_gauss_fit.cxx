// This is jgl/sdet/sdet_gauss_fit.cxx
...
/*
#if 0
    // Debug:  print out values for this step
    vcl_cout << "plane(" << unknowns[0] << "),  peak(" << unknowns[1] << ")," << vcl_endl;
    vcl_cout << "x(" << unknowns[2] << "),  y(" << unknowns[3] << ")" << vcl_endl;
    vcl_cout << "var x(" << unknowns[4] << "),  var y(" << unknowns[5] << ")" << vcl_endl;
    vcl_cout << "covar(" << unknowns[6] << ")" << vcl_endl;
#endif
*/
...
/*
#if 0
	  // Debug:  print out values of fit error for each point
											...
											double xmin, double ymin)
{
  ...
  for (int i=0; i < n_peaks; i++)  {
	...
	vcl_cout << "Init values:\n-----------------------------------------------\n Peak "
			<< i+1 << "\n -------------------------\nplane= " << plane << vcl_endl;
	//outfile << "peak=" << peak[i] << ",  ux=" << ux[i] 
    //       << ",  uy=" << uy[i] << ",  sxx=" << sxx[i] << ",  syy=" << syy[i] 
    //       << ",  sxy=" << sxy[i] << vcl_endl;
	vcl_cout << "peak=" << peak[i] << ",  ux=" << ux[i] 
           << ",  uy=" << uy[i] << ",  sxx=" << sxx[i] << ",  syy=" << syy[i] 
           << ",  sxy=" << sxy[i] << vcl_endl;
											...
											float y, int i, int n_peaks, vcl_ofstream& outfile)
{
	...
	double theta;						// angle from x axis to ellipse major axis
	double a;							// ellipse major axis std deviation
	double b;							// ellipse minor axis std deviation
	double sxx;
	double syy;
	double sxy;
		...
		a = 0.0;
		b = 0.0;
		sxx = result[6*i+4];			// copy result data into local variables
		syy = result[6*i+5];
		sxy = result[6*i+6];
		...
		bool xxgtyy = (sxx > syy);
		bool xxeqyy = (abs(sxx - syy) < 0.0001);
		bool xygt0 = (sxy > 0.0001);
		bool xyeq0 = (abs(sxy) < 0.0001);
		...
		{
			theta = 0.5 * atan(2.0 * sxy/(sxx - syy));
		...
		//vcl_cout << "sin(theta)= " << sin(theta) << ",   cos(theta)= " 
		//	 << cos(theta) << vcl_endl;
