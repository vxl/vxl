...
public:
  // : Constructor
  sdet_adjust_lsqr( vcl_vector<vgl_point_3d<double> > const& img_pts, unsigned int num_unknowns,
                    unsigned int num_residuals, int n_peaks);
...
public:
  ...
  static vnl_vector<double> adjust(vcl_vector<vgl_point_3d<double> > img_pts, vcl_vector<vsol_point_2d_sptr> ps_list,
                                   int n_peaks, vcl_ofstream& outfile, double xmin, double ymin);

  static vnl_vector<double> calculate_ellipse(vnl_vector<double> result, float x, float y, int i, int n_peaks,
