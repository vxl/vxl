...
template <class T>
void sdet_segment_img(vil_image_view<T> const& img, unsigned margin, int neigh, T weight_thres, float sigma,
                      int min_size, vil_image_view<vil_rgb<vxl_byte> >& out_img)
{
...
template <class T>
void sdet_segment_img_using_edges(vil_image_view<T> const& img, vil_image_view<float> const& edge_img, unsigned margin,
                                  int neigh, T weight_thres, float sigma, int min_size,
...
void sdet_segment_img2(vil_image_view<float> const& img1, vil_image_view<float> const& img2, unsigned margin, int neigh,
                       float weight_thres, float sigma1, float sigma2, int min_size,
                       vil_image_view<vil_rgb<vxl_byte> >& out_img);

// : segment using two features and also respect the edges in the edge image
void sdet_segment_img2_using_edges(vil_image_view<float> const& img1, vil_image_view<float> const& img2,
                                   vil_image_view<float> const& edge_img, unsigned margin, int neigh,
                                   float weight_thres, float sigma1, float sigma2, int min_size,
                                   vil_image_view<vil_rgb<vxl_byte> >& out_img);
