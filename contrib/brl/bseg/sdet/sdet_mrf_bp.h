...
public:
  ...
  sdet_mrf_bp(vil_image_resource_sptr obs_labels, unsigned n_labels, float discontinuity_cost, float truncation_cost,
              float kappa, float lambda);

  // : constructor with observed labels view
  sdet_mrf_bp(vil_image_view<float> const& obs_labels, unsigned n_labels, float discontinuity_cost,
              float truncation_cost, float kappa, float lambda);

  // : constructor with observed labels and variance resources
  sdet_mrf_bp(vil_image_resource_sptr obs_labels, vil_image_resource_sptr var,  unsigned n_labels,
              float discontinuity_cost, float truncation_cost, float kappa, float lambda);

  // : constructor with observed label and variance views.
  sdet_mrf_bp(vil_image_view<float> const& obs_labels, vil_image_view<float> const& var,  unsigned n_labels,
              float discontinuity_cost, float truncation_cost, float kappa, float lambda);
