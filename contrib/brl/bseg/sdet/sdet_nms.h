...
public:
  ...
  sdet_nms_params(double thresh = 1.0, PFIT_TYPE pfit_type = PFIT_3_POINTS, unsigned margin = 1,
                  double rel_thresh = 2.5, bool adaptive_thresh = false) :
    thresh_(thresh), pfit_type_(pfit_type), margin_(margin), rel_thresh_(rel_thresh), use_adaptive_thresh_(
      adaptive_thresh) {}
  ~sdet_nms_params() {}
};
...
public:
  ...
  sdet_nms(const sdet_nms_params& nsp, const vil_image_view<double>& dir_x, const vil_image_view<double>& dir_y,
  ...
  void apply( bool collect_tokens, vcl_vector<vgl_point_2d<double> >& loc, vcl_vector<double>& orientation,
              vcl_vector<double>& mag);

  void apply( bool collect_tokens, vcl_vector<vgl_point_2d<double> >& loc, vcl_vector<double>& orientation,
              vcl_vector<double>& mag, vcl_vector<double>& d2f);

  void apply( bool collect_tokens, vcl_vector<vgl_point_2d<double> >& loc, vcl_vector<double>& orientation,
              vcl_vector<double>& mag, vcl_vector<double>& d2f, vcl_vector<vgl_point_2d<int> >& pix_loc);
