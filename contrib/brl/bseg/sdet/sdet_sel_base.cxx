...
void sdet_sel_base::form_links_from_a_curvelet(sdet_edgel* eA, sdet_curvelet* cvlet, unsigned min_group_size,
                                               int method)
{
  if( method == 0 )
    {
  ...
  for( ; cv_it != curvelet_map_.curvelets(eN->id).end(); cv_it++ )
    ...
    for( unsigned j = 0; j < cvlet->edgel_chain.size(); j++ )
      ...
      if( cvlet->edgel_chain[j] == eN && j < cvlet->edgel_chain.size() - 1 && cvlet->edgel_chain[j + 1] == eC )
        ...
        if( eX && eY && j > 0 && cvlet->edgel_chain[j - 1] == eX && j < cvlet->edgel_chain.size() - 2 &&
            cvlet->edgel_chain[j + 2] == eY )
          {
          cvlet->used = true;
      ...
      if( cvlet->edgel_chain[j] == eN && j > 0 && cvlet->edgel_chain[j - 1] == eC )
        ...
        if( eX && eY && j > 1 && cvlet->edgel_chain[j - 2] == eX && j < cvlet->edgel_chain.size() - 1 &&
            cvlet->edgel_chain[j + 1] == eY )
          {
          cvlet->used = true;
          return true; // found eX-eA-eB-eY grouping
          }
                       ...
                       unsigned min_group_size)
{
  ...
  for( ; cv_it != curvelet_map_.curvelets(eX->id).end() && !cvlet_found; cv_it++ )
    ...
    for( unsigned j = 0; j < cvlet1->edgel_chain.size(); j++ )
      {
      if( cvlet1->edgel_chain[j] == eX && j < cvlet1->edgel_chain.size() - 2 && cvlet1->edgel_chain[j + 1] == eA &&
          cvlet1->edgel_chain[j + 2] == eY )
        {
  ...
  for( ; cv_it != curvelet_map_.curvelets(eY->id).end() && !cvlet_found; cv_it++ )
    ...
    for( unsigned j = 0; j < cvlet2->edgel_chain.size(); j++ )
      {
      if( cvlet2->edgel_chain[j] == eX && j < cvlet2->edgel_chain.size() - 2 && cvlet2->edgel_chain[j + 1] == eA &&
          cvlet2->edgel_chain[j + 2] == eY )
        {
        cvlet_found =  true; // found eX-eA-eY grouping
        break;
        }
...
::determine_accuracy_of_measurements()
{
  ...
  for( unsigned i = 0; i < edgemap_->edgels.size(); i++ )
    ...
    for( ; it != curvelet_map_.curvelets(i).end(); it++ )
      ...
      theta_error(cvlet_cnt) = vcl_min(vcl_abs(estimates[0] - min_estimates[0]), vcl_abs(
                                         estimates[0] - max_estimates[0]) );
      k_error(cvlet_cnt)     = vcl_min(vcl_abs(estimates[1] - min_estimates[1]), vcl_abs(
                                         estimates[1] - max_estimates[1]) );
      gamma_error(cvlet_cnt) = vcl_min(vcl_abs(estimates[2] - min_estimates[2]), vcl_abs(
                                         estimates[2] - max_estimates[2]) );
  ...
  vcl_cout << "spread of theta estimate (mean, std): (" << theta_error_mean << ", " << vcl_sqrt(theta_error_std)
           << ")" << vcl_endl;
  vcl_cout << "spread of  k   estimate (mean, std): (" << k_error_mean << ", " << vcl_sqrt(k_error_std) << ")"
           << vcl_endl;
  vcl_cout << "spread of gamma estimate (mean, std): (" << gamma_error_mean << ", " << vcl_sqrt(gamma_error_std)
           << ")" << vcl_endl;
...
::report_stats()
{
  ...
  vcl_cout
    <<
  "| cvlet size | raw # cvlet | % of edgels | % of edgels with no other larger curvelets | # of consistent cvlets | % of edgels with consistent curvelets |"
    << vcl_endl;
  for( unsigned i = 3; i <= max_size; i++ )
    {
    vcl_cout << i << " " << cvlet_cnt[i] << " " << cvlet_edgel_cnt[i] * 100.0 / edgemap_->edgels.size() << " "
             << min_cvlet_edgel_cnt[i] * 100.0 / edgemap_->edgels.size() << " ";
    vcl_cout <<  consistent_cvlet_cnt[i] << " " << consistent_cvlet_edgel_cnt[i] * 100.0 / edgemap_->edgels.size()
             << " " << min_consistent_cvlet_edgel_cnt[i] * 100.0 / edgemap_->edgels.size() << vcl_endl;
    }
...
// Modify: for each node, consider all the child links and parent links(exact the one linking parent node), as its child node
  vcl_queue<sdet_EHT_node *> BFS_queue;
  ...
  for( unsigned i = 0; i < edgemap_->edgels.size(); i++ )
    ...
    if( EHT1 )
      ...
      for( ; pit != EHT1->path_end(); pit++ )
        {
        vcl_vector<sdet_edgel *>& edgel_chain = pit.get_cur_path();
...
::is_EHT_path_legal(vcl_vector<sdet_edgel *>& edgel_chain)
{
  ...
  vcl_vector<sdet_edgel *> S_link_end_nodes, E_link_end_nodes;
  ...
  if( !cons )
    {
    return false;          // no good at the start point
  ...
  if( !cons )
    {
    return false;          // no good at the end point
                       ...
                       vcl_vector<sdet_edgel *>& Cchain)
{
  ...
  vcl_vector<sdet_edgel *> chain;
  if( Pchain.size() )
    {
    for( unsigned i = 0; i < Pchain.size(); i++ )
      {
      chain.push_back(Pchain[i]);
      }
    }
  if( Tchain.size() )
    {
    for( unsigned i = 0; i < Tchain.size(); i++ )
      {
      chain.push_back(Tchain[i]);
      }
    }
  if( Cchain.size() )
    {
    for( unsigned i = 0; i < Cchain.size(); i++ )
      {
      chain.push_back(Cchain[i]);
      }
    }

  // now compute the metric
  sdet_edgel * eA = 0, * eP = 0;
...
void sdet_sel_base::disambiguate_the_CFTG()
{
  ...
  for( ; l_it != curve_frag_graph_.CFTG.Links.end(); l_it++ )
    ...
    for( ; l_it2 != curve_frag_graph_.CFTG.cLinks[cur_Link->eE->id].end(); l_it2++ )
      ...
      if( (*l_it2)->eE == cur_Link->eS )
        ...
        vcl_vector<sdet_edgel *> chain1(edgel_chain1->edgels.begin(), edgel_chain1->edgels.end() );
        vcl_vector<sdet_edgel *> chain2(edgel_chain2->edgels.begin(), edgel_chain2->edgels.end() );
  ...
  for( ; l_it != curve_frag_graph_.CFTG.Links.end(); l_it++ )
    ...
    int deg_S = curve_frag_graph_.CFTG.cLinks[(*l_it)->eS->id].size()
      + curve_frag_graph_.CFTG.pLinks[(*l_it)->eS->id].size();
    int deg_E = curve_frag_graph_.CFTG.cLinks[(*l_it)->eE->id].size()
      + curve_frag_graph_.CFTG.pLinks[(*l_it)->eE->id].size();
  ...
  while( GD_list.size() > 0 )
    ...
    // clinks from eS
    vcl_vector<sdet_CFTG_link *> links_to_del;
...
static bool is_continue(const sdet_edgel_chain * c1, const sdet_edgel_chain * c2)
{
  ...
  for( int i = c1->edgels.size() - 2; i >= 0; i-- )
    ...
    else if( SM_1 < Theta_2 || s > Strength_Diff2 )
      {
      return false;
      }
    else { continue; }
      ...
      for( unsigned int j = 0; j < new_chain4->edgels.size(); ++j )
        ...
        if( d1 < cost1 )
          ...
          if( costc < cost )
            ...
            double angle =
              ( (dx1 * dx2 + dy1 * dy2) / vcl_sqrt(dx1 * dx1 + dy1 * dy1) / vcl_sqrt(dx2 * dx2 + dy2 * dy2) );
            if( d0 < d9 )
              {
              ++eit1;
              continue;
  ...
  while( GD_list.size() > 0 )
    ...
    for( unsigned int i = 0; i < new_chain5->edgels.size(); ++i )
      ...
      if( p2 == 1 )
        ...
        new_chain6a = new sdet_edgel_chain(); new_chain6a->edgels.push_back(edge2);
        ...
        while( 1 )
          ...
          sdet_edgel_list_iter eit3 = new_chain4->edgels.begin();
          sdet_edgel_list_iter eit4 = new_chain4->edgels.begin();
...
void sdet_sel_base::Disambiguation()
{
  ...
  for( ; l_it != curve_frag_graph_.CFTG.Links.end(); l_it++ )
    ...
    if( (curve_frag_graph_.pFrags[edgel_chain->edgels.front()->id].size()
         + curve_frag_graph_.cFrags[edgel_chain->edgels.front()->id].size() ) == 1                            &&
        (curve_frag_graph_.pFrags[edgel_chain->edgels.back()->id].size()
         + curve_frag_graph_.cFrags[edgel_chain->edgels.back()->id].size() ) == 1                         &&
        edgel_chain->edgels.size() == 2 ) { curve_frag_graph_.insert_fragment(edgel_chain); }
...
void sdet_sel_base::Post_Process()
{
  ...
  for( unsigned int i = 0; i < edgemap_->edgels.size(); ++i )
    {
    sdet_edgel* eA1 = edgemap_->edgels[i];
    if( (curve_frag_graph_.pFrags[eA1->id].size() + curve_frag_graph_.cFrags[eA1->id].size() ) ==
        1 ) { new_chain->edgels.push_back(eA1); }
    }
  for( unsigned int j = 0; j < new_chain->edgels.size(); ++j )
    {
    sdet_edgel* edge = new_chain->edgels[j]; sdet_edgel* edge2 = 0; sdet_edgel_chain* chain = new sdet_edgel_chain();
    sdet_edgel* edge3 = 0;
    ...
    if( n == 1 && chain->edgels.size() > 1 &&
        ( (curve_frag_graph_.pFrags[edge2->id].size() + curve_frag_graph_.cFrags[edge2->id].size() ) >= 1) )
      {
    ...
    else if( number == 1 && chain->edgels.size() > 1 &&
             ( (curve_frag_graph_.pFrags[edge3->id].size() + curve_frag_graph_.cFrags[edge3->id].size() ) >= 1) )
      {
      curve_frag_graph_.extract_fragment(chain);
      if( diff == 1 )
        {
