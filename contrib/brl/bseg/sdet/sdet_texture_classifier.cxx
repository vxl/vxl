...
static unsigned gauss_radius(float sigma, float cutoff_ratio){
  ...
  int r = 
    static_cast<unsigned>(vcl_sqrt((-2.0*vcl_log(cutoff_ratio))/sigma_sq_inv)+0.5);
  return r;
}
...
void sdet_texture_classifier::init_color_map(){
  ...
  unsigned i = 0;
  vcl_map< vcl_string, vcl_vector<vnl_vector<double> > >::const_iterator it=
    texton_dictionary_.begin();
...
void sdet_texture_classifier::print_color_map() const {
  ...
  vcl_map< vcl_string, vnl_vector_fixed<float, 3> >::const_iterator it=
    color_map_.begin();
...
sdet_texture_classifier(sdet_texture_classifier_params const& params):
  ...
  filter_responses_(brip_filter_bank(params.n_scales_,
                                     ...
                                     params.cutoff_per_)), 
  distances_valid_(false),color_map_valid_(false),texton_index_valid_(false),
  inter_prob_valid_(false), texton_weights_valid_(false){}
...
bool sdet_texture_classifier::compute_training_data(vcl_string const& category)
{
  ...
  if(ns>n_samples_){
    for(unsigned i = 0; i<n_samples_; ++i){
      unsigned s = 
        static_cast<unsigned>((n_samples_-1)*(vcl_rand()/(RAND_MAX+1.0)));
...
compute_training_data(vcl_string const& category,
  ...
  vcl_vector<vgl_polygon<double> >::const_iterator pit = 
    texture_regions.begin();
...
void sdet_texture_classifier::compute_distances(){
  ...
  dist_.clear();
  vcl_map< vcl_string, vcl_vector<vnl_vector<double> > >::const_iterator jt=
    texton_dictionary_.begin();
...
void sdet_texture_classifier::compute_texton_weights(){
  ...
  vcl_map<vcl_string, vcl_vector<float> >::iterator hit = 
    category_histograms_.begin();
...
void sdet_texture_classifier::print_distances() const{
  ...
  vcl_map< vcl_string, vcl_map< vcl_string, double> >::const_iterator jt =  
    dist_.begin();
  for(; jt != dist_.end(); ++jt){
    vcl_cout << (*jt).first << " : \n";
...
void sdet_texture_classifier::print_category_histograms() const
{
  ...
  vcl_map<vcl_string, vcl_vector<float> >::const_iterator hit = 
    category_histograms_.begin(); 
  unsigned j = 0;
...
texture_probabilities(vcl_vector<float> const& hist){
  unsigned nc = category_histograms_.size();
  unsigned nt = texton_index_.size();
...
classify_image_blocks(vcl_string const& img_path){
  ...
  unsigned nblocks = ni*nj/block_area;
  int bidxv = 0;
