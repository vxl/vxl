...
vil_image_view<float> sdet_texture_classifier::scale_image(vil_image_resource_sptr const& resc)
{
  ...
  if( !exists || !this->load_filter_responses(img_out_dir) )
    ...
    if( !exists )
      ...
      if( !vul_file::make_directory(img_out_dir) )
        {
        vcl_cerr << " in sdet_texture_classifier::compute_filter_bank_color_img - cannot create directory: "
                 << img_out_dir << vcl_endl;
        return false;
        }
      }
...
// : check the folder if already computed using the image name, otherwise compute and save
bool sdet_texture_classifier::compute_filter_bank_float_img(vcl_string const& filter_folder, vcl_string const& img_name,
                                                            float max_val)
{
  ...
  if( !exists || !this->load_filter_responses(img_out_dir) )
    ...
    if( !exists )
      ...
      if( !vul_file::make_directory(img_out_dir) )
        {
        vcl_cerr << " in sdet_texture_classifier::compute_filter_bank_float_img() - cannot create directory: "
                 << img_out_dir << vcl_endl;
        return false;
        }
      }
                                                   ...
                                                   vcl_string const& img_name, vcl_string const& response_name)
{
  ...
  vcl_cout << "computing filter bank on: " << img_name << " max filter radius: " << filter_responses.invalid_border()
           << vcl_endl;
  ...
  for( unsigned i = 0; i < filter_responses.n_levels(); i++ )
    ...
    vcl_cout << " adding filter response level: " << i << " to other responses with name: " << res_name.str()
             << vcl_endl;
...
vcl_vector<vnl_vector<double> > sdet_texture_classifier::random_centers(
  vcl_vector<vnl_vector<double> > const& training_data,
  unsigned k) const
{
                                                      ...
                                                      vcl_vector<vgl_polygon<double> > const& texture_regions)
{
  ...
  unsigned dim_total = dim + 2 + other_responses_.size();
  vcl_cout << " texton dimension: " << dim_total << " dim: " << dim << " other_responses_ size: "
           << other_responses_.size() << '\n' << vcl_flush;
...
bool sdet_texture_classifier::compute_training_data(vcl_string const& category, vcl_vector<vcl_pair<int,
                                                                                                    int> > const& pixels)
{
  // dimension of filter bank
  unsigned dim = filter_responses_.n_levels();
  ...
  for( unsigned kk = 0; kk < pixels.size(); kk++ )
    ...
    if( ii < 0 || jj < 0 || ii >= ni || jj >= nj )
      {
      continue;                                              // invalid pixels are marked -1, -1 in the input
      }
    // vnl_vector<double> tx(dim+2);
    vnl_vector<double> tx(dim_total);
    for( unsigned f = 0; f < dim; ++f )
...
bool sdet_texture_classifier::compute_data(vcl_vector<vcl_pair<int, int> > const& pixels,
...
{
  for( vcl_map<vcl_string, vcl_vector<vnl_vector<double> > >::iterator iter = training_data_.begin();
       iter != training_data_.end(); iter++ )
    {
...
bool sdet_texture_classifier::load_data(vcl_string const& path)
{
  ...
  filter_responses_ = brip_filter_bank(tcp_ptr->n_scales_, tcp_ptr->scale_interval_, tcp_ptr->lambda0_,
                                       tcp_ptr->lambda1_, tcp_ptr->angle_interval_, tcp_ptr->cutoff_per_);
  maxr_ = this->max_filter_radius();
  return true;
}

void sdet_texture_classifier::add_training_data(vcl_string const& category,
                                                vcl_vector<vnl_vector<double> >& training_data)
{
  vcl_map<vcl_string, vcl_vector<vnl_vector<double> > >::iterator dit;
  dit = training_data_.find(category);
  if( dit == training_data_.end() )
...
{
  vcl_vector<vcl_string> cats;
  for( vcl_map<vcl_string, vcl_vector<vnl_vector<double> > >::iterator iter = texton_dictionary_.begin();
       iter != texton_dictionary_.end(); iter++ )
    {
    cats.push_back(iter->first);
    }
  return cats;
...
void sdet_texture_classifier::create_samples_and_labels_from_textons(vcl_vector<vnl_vector<double> >& samples,
                                                                     vcl_vector<double>& labels)
{
  ...
  for( ; iter != texton_dictionary_.end(); iter++ )
    ...
    vcl_cout << "category: " << iter->first << " with id: " << label_id << " has final sample size: "
             << samples.size() << vcl_endl;
    label_id += 1.0;
    }
...
void sdet_texture_classifier::create_samples_and_labels_from_training_data(vcl_vector<vnl_vector<double> >& samples,
                                                                           vcl_vector<double>& labels)
{
  samples.clear();
  labels.clear();
  ...
  for( ; dit != training_data_.end(); ++dit )
    ...
    vcl_cout << "category: " << cat << " with id: " << label_id << " has final sample size: " << samples.size()
             << vcl_endl;
    label_id += 1.0;
    }
...
void sdet_texture_classifier::update_hist(vnl_vector<double> const& f, float weight, vcl_vector<float>& hist)
{
...
void sdet_texture_classifier::update_hist(vcl_vector<vnl_vector<double> > const& f, float weight,
...
vcl_map<vcl_string, float>  sdet_texture_classifier::texture_probabilities(vcl_vector<float> const& hist)
{
  unsigned nt = texton_index_.size();
...
void sdet_texture_classifier::compute_textons_of_pixels(vil_image_view<int>& texton_img)
{
  ...
  vcl_cout << " sdet_texture_classifier::compute_textons_of_pixels() -- using border: " << border << " for an " << ni
           << " by " << nj << " image to compute a texton per pixel!\n";
