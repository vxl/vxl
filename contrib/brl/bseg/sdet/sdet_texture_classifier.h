...
public:
  ...
  // : check the folder if already computed using the image name, otherwise compute and save
  bool compute_filter_bank_float_img(vcl_string const& filter_folder, vcl_string const& img_name,
                                     float max_val = -1.0f);
  ...
  void add_gauss_response(vil_image_view<float>& img_f, vcl_string const& filter_folder, vcl_string const& img_name,
                          vcl_string const& response_name, bool const& is_smooth = true);

  // : append to the vector of other_responses_
  void add_filter_responses(vil_image_view<float>& img_f, vcl_string const& filter_folder, vcl_string const& img_name,
                            vcl_string const& response_name);
  ...
  bool compute_textons(vcl_vector<vcl_string> const& image_paths, vcl_string const& category, vcl_vector<vcl_string> const& poly_paths =
  ...
  void create_samples_and_labels_from_training_data(vcl_vector<vnl_vector<double> >& samples,
...
protected:
  ...
  vcl_vector<vnl_vector<double> > random_centers(vcl_vector<vnl_vector<double> > const& training_data,
                                                 unsigned k) const;
