...
void sdet_third_order_edge_det::apply(vil_image_view<vxl_byte> const& image)
{
  ...
  switch( grad_op_ )
    ...
    case 0: // Interpolated Gaussian
      ...
      break;
    case 1: // h0-operator
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Ix,   brip_h0_Gx_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Iy,   brip_h0_Gy_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Ixx,  brip_h0_Gxx_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Ixy,  brip_h0_Gxy_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Iyy,  brip_h0_Gyy_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Ixxx, brip_h0_Gxxx_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Ixxy, brip_h0_Gxxy_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Ixyy, brip_h0_Gxyy_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Iyyy, brip_h0_Gyyy_kernel(sigma_),
                              double(), interp_factor_);
      break;
    case 2:  // h1-operator
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Ix,   brip_h1_Gx_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Iy,   brip_h1_Gy_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Ixx,  brip_h1_Gxx_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Ixy,  brip_h1_Gxy_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Iyy,  brip_h1_Gyy_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Ixxx, brip_h1_Gxxx_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Ixxy, brip_h1_Gxxy_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Ixyy, brip_h1_Gxyy_kernel(sigma_),
                              double(), interp_factor_);
      brip_subpix_convolve_2d(greyscale_view, edge_locations, Iyyy, brip_h1_Gyyy_kernel(sigma_),
                              double(), interp_factor_);
      break;
    }
  ...
  for( unsigned i = 0; i < edge_locations.size(); i++ )
    ...
    if( interp_grid )
      ...
      if( reduce_tokens )
        ...
        sdet_edgel* new_edgel = new sdet_edgel(edge_locations[i], edge_orientations[i], mag[i], d2f[i],
                                               edge_uncertainties[i]);
...
bool sdet_third_order_edge_det::apply_color(vil_image_view<vxl_byte> const& image)
{
  ...
  for( unsigned i = 0; i < edge_locations.size(); i++ )
    ...
    double Axx = 2 * If1x[i] * If1xxx[i] + 2 * If1xx[i] * If1xx[i] + 2 * If2x[i] * If2xxx[i] + 2 * If2xx[i] * If2xx[i]
      + 2 * If3x[i] * If3xxx[i] + 2 * If3xx[i] * If3xx[i];
    double Axy = 2 * If1x[i] * If1xxy[i] + 2 * If1xy[i] * If1xx[i] + 2 * If2x[i] * If2xxy[i] + 2 * If2xy[i] * If2xx[i]
      + 2 * If3x[i] * If3xxy[i] + 2 * If3xy[i] * If3xx[i];
    double Ayy = 2 * If1x[i] * If1xyy[i] + 2 * If1xy[i] * If1xy[i] + 2 * If2x[i] * If2xyy[i] + 2 * If2xy[i] * If2xy[i]
      + 2 * If3x[i] * If3xyy[i] + 2 * If3xy[i] * If3xy[i];

    double B   = If1x[i] * If1y[i] + If2x[i] * If2y[i] + If3x[i] * If3y[i];
    double Bx  = If1x[i] * If1xy[i] + If1y[i] * If1xx[i] + If2x[i] * If2xy[i] + If2y[i] * If2xx[i] + If3x[i]
      * If3xy[i] + If3y[i] * If3xx[i];
    double By  = If1x[i] * If1yy[i] + If1y[i] * If1xy[i] + If2x[i] * If2yy[i] + If2y[i] * If2xy[i] + If3x[i]
      * If3yy[i] + If3y[i] * If3xy[i];
    double Bxx = If1x[i] * If1xxy[i] + If1xx[i] * If1xy[i] + If1y[i] * If1xxx[i] + If1xy[i] * If1xx[i] + If2x[i]
      * If2xxy[i] + If2xx[i] * If2xy[i] + If2y[i] * If2xxx[i] + If2xy[i] * If2xx[i] + If3x[i] * If3xxy[i] + If3xx[i]
      * If3xy[i] + If3y[i] * If3xxx[i] + If3xy[i] * If3xx[i];
    double Bxy = If1x[i] * If1xyy[i] + If1xy[i] * If1xy[i] + If1y[i] * If1xxy[i] + If1yy[i] * If1xx[i] + If2x[i]
      * If2xyy[i] + If2xy[i] * If2xy[i] + If2y[i] * If2xxy[i] + If2yy[i] * If2xx[i] + If3x[i] * If3xyy[i] + If3xy[i]
      * If3xy[i] + If3y[i] * If3xxy[i] + If3yy[i] * If3xx[i];
    double Byy = If1x[i] * If1yyy[i] + If1xy[i] * If1yy[i] + If1y[i] * If1xyy[i] + If1yy[i] * If1xy[i] + If2x[i]
      * If2yyy[i] + If2xy[i] * If2yy[i] + If2y[i] * If2xyy[i] + If2yy[i] * If2xy[i] + If3x[i] * If3yyy[i] + If3xy[i]
      * If3yy[i] + If3y[i] * If3xyy[i] + If3yy[i] * If3xy[i];
    ...
    double Cxx = 2 * If1y[i] * If1xxy[i] + 2 * If1xy[i] * If1xy[i] + 2 * If2y[i] * If2xxy[i] + 2 * If2xy[i] * If2xy[i]
      + 2 * If3y[i] * If3xxy[i] + 2 * If3xy[i] * If3xy[i];
    double Cxy = 2 * If1y[i] * If1xyy[i] + 2 * If1yy[i] * If1xy[i] + 2 * If2y[i] * If2xyy[i] + 2 * If2yy[i] * If2xy[i]
      + 2 * If3y[i] * If3xyy[i] + 2 * If3yy[i] * If3xy[i];
    double Cyy = 2 * If1y[i] * If1yyy[i] + 2 * If1yy[i] * If1yy[i] + 2 * If2y[i] * If2yyy[i] + 2 * If2yy[i] * If2yy[i]
      + 2 * If3y[i] * If3yyy[i] + 2 * If3yy[i] * If3yy[i];
    ...
    double lxx =
      ( (lx - Cx) * Ax + (l - C) * Axx + (lx - Ax) * Cx + (l - A) * Cxx + 2 * Bx * Bx + 2 * B * Bxx - lx
        * (2 * lx - Cx - Ax) ) / e;
    double lxy =
      ( (ly - Cy) * Ax + (l - C) * Axy + (ly - Ay) * Cx + (l - A) * Cxy + 2 * Bx * By + 2 * B * Bxy - lx
        * (2 * ly - Cy - Ay) ) / e;
    double lyy =
      ( (ly - Cy) * Ay + (l - C) * Ayy + (ly - Ay) * Cy + (l - A) * Cyy + 2 * By * By + 2 * B * Byy - ly
        * (2 * ly - Cy - Ay) ) / e;
...
bool sdet_third_order_edge_det::save_edg_ascii(const vcl_string& filename, unsigned ni, unsigned nj,
...
sdet_edgemap_sptr sdet_third_order_edge_det::edgemap()
{
  ...
  for( unsigned i = 0; i < edgels_.size(); i++ )
    {
    sdet_edgel* new_edgel =
      new sdet_edgel(edgels_[i].get_pt(), vcl_tan(edgels_[i].get_theta() ), edgels_[i].get_grad() );
    edge_map->insert(new_edgel);
    }
  return edge_map;
}
