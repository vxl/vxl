                                  ...
                                  vil1_image image, double& corr, double& avg_int)
{
  ...
  if(!image)
    return ;
  double face_avg = 0, image_avg = 0;
  double face_sum=0, image_sum=0, cross_sum=0;
  int n = face->Npix();
  int count = 20;
  for(face->reset(); face->next();)
    {
      ...
      //      face_avg += face_int;
      image_avg += image_int;
      cross_sum += vcl_abs(face_int-image_int);
    }
//   face_avg /= n;
  image_avg /= n;
  cross_sum /= n;
...
//--------------------------------------------------------------------------
//: because of sorting the samples will be in decending order of correlation
