// This is sdet/tests/test_gauss_fit
...
void test_gauss_fit()
{
	...
	vcl_cout << "Covariance Matrix: " << vcl_endl;
	vcl_cout << V(0,0) << "  " << V(0,1) << "\n" << vcl_endl;
	vcl_cout << V(1,0) << "  " << V(1,1) << "\n" << vcl_endl;
	...
	vcl_cout << "Covariance Matrix inverse: " << vcl_endl;
	vcl_cout << Vinv(0,0) << "  " << Vinv(0,1) << vcl_endl;
	vcl_cout << Vinv(1,0) << "  " << Vinv(1,1) << "\n" << vcl_endl;
	vcl_cout << "Vdet = " << Vdet << ",  const term =" << const_term << "\n" << vcl_endl;
...
//			double gaussian = peak_delta * const_term * exponential;
	...
	a = params[1];
	b = params[2];
	float angle = theta*180.0/(2.0*pi);
	
	vcl_cout << "\n   Test fit of ellipse at 0 degrees " << vcl_endl;
	vcl_cout << "sin(theta)= " << sin(theta) << ",   cos(theta)= " << cos(theta) << vcl_endl;
	vcl_cout << "theta= " << theta << ",   angle= " << angle << ",   a= " << a 
			  << ",   b= " << b << vcl_endl;
	
	// ---------------------------------------------------------------------------			 
	// now test at theta= 30 degrees
	double rad = 30.0*2.0*PI/360.;
	double sin30 = sin(rad);
	double cos30 = cos(rad);
	sxx = (18.0*cos30*cos30 + 2.0*sin30*sin30)/4.0;
	syy = (18.0*sin30*sin30 + 2.0*cos30*cos30)/4.0;
	sxy = (18.0*sin30*cos30 + 2.0*cos30*sin30)/4.0;
	...
	vcl_cout << "\n   Test fit of ellipse at 30 degrees " << vcl_endl;
	vcl_cout << "sin(theta)= " << sin(theta) << ",   cos(theta)= " << cos(theta) << vcl_endl;
	vcl_cout << "theta= " << theta << ",   angle= " << angle << ",   a= " << a 
			  << ",   b= " << b << vcl_endl;
	...
	vcl_cout << "\n   Test fit of ellipse at 90 degrees " << vcl_endl;
	vcl_cout << "sin(theta)= " << sin(theta) << ",   cos(theta)= " << cos(theta) << vcl_endl;
	vcl_cout << "theta= " << theta << ",   angle= " << angle << ",   a= " << a 
			  << ",   b= " << b << vcl_endl;
#endif //needs work	
