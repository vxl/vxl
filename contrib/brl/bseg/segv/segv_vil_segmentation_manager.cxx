...
vgui_range_map_params_sptr segv_vil_segmentation_manager::range_params(vil_image_resource_sptr const& image)
{
  float gamma = 1.0;
  bool  invert = false;
  bool  gl_map = false;
...
// -----------------------------------------------------------------------------
void segv_vil_segmentation_manager::draw_polylines(vcl_vector<vsol_polyline_2d_sptr> const& polys)
{
  bgui_vtol2D_tableau_sptr t2D = this->selected_vtol2D_tab();
...
// -----------------------------------------------------------------------------
void segv_vil_segmentation_manager::draw_lines(vcl_vector<vsol_line_2d_sptr> const& lines,
...
// -----------------------------------------------------------------------------
void segv_vil_segmentation_manager::draw_conics(vcl_vector<vsol_conic_2d_sptr> const& conics,
                                                const vgui_style_sptr& style)
{
  bgui_vtol2D_tableau_sptr t2D = this->selected_vtol2D_tab();
...
void segv_vil_segmentation_manager::extrema()
{
  ...
  if( fast )
    {
    extr = brip_vil_float_ops::fast_extrema(fimg, lambda0, lambda1, theta, bright, mag_only, output_mask,
                                            output_unclipped, scale_invariant, non_max_suppress);
    }
  else
    {
    extr = brip_vil_float_ops::extrema(fimg, lambda0, lambda1, theta, bright, mag_only, output_mask, output_unclipped,
                                       scale_invariant, non_max_suppress);
    }
