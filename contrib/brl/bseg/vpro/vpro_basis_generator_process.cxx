...
bool vpro_basis_generator_process::execute()
{
  ...
  input_images_.clear();
  vil1_memory_image_of<float> flt = brip_vil1_float_ops::convert_to_float(img);
...
bool vpro_basis_generator_process::finish()
{
  ...
  brip_vil1_float_ops::basis_images(frames_, basis);
  vcl_list<vil1_image> basis_frames;
  for(vcl_vector<vil1_memory_image_of<float> >::iterator iit = basis.begin();
      iit != basis.end(); iit++)
    {
      vil1_image img = brip_vil1_float_ops::convert_to_byte(*iit);
