#include <image/mil_to_vil.h>
#include <image/vil_to_mil.h>
...
void test_mil_conversions()
{
  ...
  mil_image_2d_of<vil_byte>* im2 = (mil_image_2d_of<vil_byte>*)vil_to_mil(im1);
  TEST("vil_to_mil # dimensions", im2->n_dims(), 2);
  TEST("vil_to_mil width", wd, im2->nx());
  TEST("vil_to_mil height", ht, im2->ny());
  char* buf2 = new char[size];
  im2->getRange(*(vil_byte*)buf2, *(vil_byte*)(buf2+wd*ht));
  TEST("vil_to_mil data", 0, memcmp(buf1, buf2, size));

  vil_image im3 = mil_to_vil(*im2);
  TEST("mil_to_vil width", im3.width(), im2->nx())
  TEST("mil_to_vil height", im3.height(), im2->ny())
  im3.get_section(buf1, 0, 0, wd, ht);
  TEST("mil_to_vil data", 0, memcmp(buf1, buf2, size))

  delete[] buf1; delete[] buf2;
}

TESTMAIN(test_mil_conversions);
