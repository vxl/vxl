...
bool bundler_driver(const bundler_routines & routines, const vcl_vector<vil_image_resource_sptr> & imageset,
                    const vcl_vector<double> & exif_tags, vcl_vector<vpgl_perspective_camera<double> > & cameras,
                    vcl_vector<vgl_point_3d<double> > & points, vnl_sparse_matrix<bool> & visibility_graph);

// Creates a file according to the PLY specifications, containing
// the point data held in points.
bool bundler_write_ply_file(const char * filename, const vcl_vector<vgl_point_3d<double> > & points);

bool bundler_write_bundle_file(const char* filename, const vcl_vector<vpgl_perspective_camera<double> > & cameras,
...
public:
  ...
  // yourself.
  virtual void run_feature_stage(const vcl_vector<vil_image_resource_sptr> & imageset,
...
public:
  ...
  // pipeline. Alternatively, you can directly call functors
  // yourself.
  virtual bool run_sfm_stage(bundler_inters_reconstruction & empty_recon,
