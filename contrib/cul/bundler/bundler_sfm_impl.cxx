...
void bundler_sfm_impl_create_initial_recon::operator()(
    ...
    bundler_inters_reconstruction &reconstruction)
{
    ...
    const bundler_inters_match_set *best_match = NULL;
    ...
    vcl_vector<bundler_inters_match_set>::const_iterator ii;
         ...
         ii != track_set.match_sets.end(); ii++)
    {
        ...
        if (ii->num_features()>=settings.min_number_of_matches_homography &&
            ii->image1.focal_length != BUNDLER_NO_FOCAL_LEN &&
            ii->image1.focal_length != BUNDLER_NO_FOCAL_LEN)
        {
            double current_inlier_percentage =
                bundler_utils_get_homography_inlier_percentage(
                    *ii, thresh_sq,
    ...
    for (int i = 0; i < best_match->num_features(); i++) {
        bundler_inters_3d_point new_point;
        ...
        bundler_utils_triangulate_points(
...
static bool is_not_in_list(
    bundler_inters_feature_set_sptr to_check,
    const vcl_vector<bundler_inters_camera> &cameras)
{
    vcl_vector<bundler_inters_camera>::const_iterator ii;
    ...
    vcl_vector<bundler_inters_feature_set_sptr> &to_add)
{
    ...
    bundler_inters_feature_set_sptr next_image;

    vcl_vector<bundler_inters_feature_set_sptr>::iterator ii;
...
void bundler_sfm_default_add_next_images::operator()(
    ...
    const vcl_vector<bundler_inters_feature_set_sptr> &to_add_set)
{
    const double thresh_sq =
        settings.inlier_thresh * settings.inlier_thresh;

    vcl_vector<bundler_inters_feature_set_sptr>::const_iterator to_add;
    for (to_add = to_add_set.begin(); to_add != to_add_set.end(); to_add++)
    {
        ...
        bundler_inters_camera bundler_camera(
...
void bundler_sfm_default_add_new_points::operator()(
    bundler_inters_reconstruction &reconstruction,
    bundler_inters_track_set &track_set,
    const vcl_vector<bundler_inters_camera> &added)
{
