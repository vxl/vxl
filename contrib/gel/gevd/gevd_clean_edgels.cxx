                                           ...
                                           vcl_vector<vtol_edge_2d_sptr>& out_edgels, int steps)
{
  ...
  for (vcl_vector<vtol_edge_2d_sptr>::iterator egit = in_edgels.begin(); egit != in_edgels.end(); egit++)
                                             ...
                                             vcl_vector<vtol_edge_2d_sptr >& deleted_edges)
{
  ...
  for (vcl_vector<vtol_edge_2d_sptr >::iterator e1it = common_edges.begin(); e1it != common_edges.end(); e1it++)
    {
  ...
  for (vcl_vector<vtol_edge_2d_sptr >::iterator eit = temp.begin(); eit != temp.end(); eit++)
    {
...
void gevd_clean_edgels::JumpGaps()
{
  ...
  for (vcl_vector<vtol_vertex_2d*>::iterator vit = verts.begin(); vit != verts.end(); vit++)
    ...
    //We assume that the edges all have digital curves
          int nnn = 1;
    for (vcl_vector<vtol_edge_2d_sptr >::iterator eit = near_edges.begin(); eit != near_edges.end() && nnn == 1; eit++)
      {
      ///      nnn=0;
      vtol_edge_2d_sptr  e = (*eit);
...
void gevd_clean_edgels::DeleteShortEdges()
{
  ...
  for (vcl_vector<vtol_edge_2d_sptr >::iterator eit = deleted_edges.begin(); eit != deleted_edges.end(); eit++)
    {
    vcl_vector<vtol_edge_2d_sptr >::iterator f;
    // (*eit)->unlink();
...
{
  vul_timer t;
  for (vcl_vector<vtol_edge_2d_sptr >::iterator egit = out_edgels_->begin(); egit!=out_edgels_->end(); egit++)
    {
      bool fix_it = false;
      vtol_edge_2d_sptr e = (vtol_edge_2d_sptr )(*egit);
      vdgl_digital_curve_sptr dc = e->curve()->cast_to_digital_curve();
...
void gevd_clean_edgels::RemoveLoops()
{
  ...
  for (vcl_vector<vtol_edge_2d_sptr >::iterator eit = removed_edges.begin(); eit != removed_edges.end(); eit++)
    {
