...
bool gevd_clean_edgels::edge_exists(vtol_vertex_2d_sptr v1, vtol_vertex_2d_sptr v2,
                                    vcl_vector<vtol_edge_2d_sptr>& intersection)
{
  bool found = false;
...
void gevd_clean_edgels::JumpGaps()
{
  ...
  for( vcl_vector<vtol_vertex_2d *>::iterator vit = verts.begin();
    ...
    for( eit = near_edges.begin(); eit != near_edges.end() && nnn == 1; ++eit )
      ...
      if( !new_v )
        {
        continue;           // should always have a new_v
        }
      if( !end_vertex )
        {
...
void gevd_clean_edgels::DeleteShortEdges()
{
  ...
  for( vcl_vector<vtol_edge_2d_sptr>::iterator egit = out_edgels_->begin();
    ...
    int               dx = x2 - x1; if( dx < 0 )
      {
      dx = -dx;                           // dx = vcl_abs(x2-x1);
      }
    int dy = y2 - y1; if( dy < 0 )
      {
      dy = -dy;                           // dy = vcl_abs(y2-y1);
      }
    ...
    if( dx < d_remove && dy < d_remove )
      ...
      for( int t = 0; (t < n_edgels) && all_close; t++ )
        ...
        dx = xe - x1; if( dx < 0 )
          {
          dx = -dx;                       // dx = vcl_abs(xe-x1);
          }
        dy = ye - y1; if( dy < 0 )
          {
          dy = -dy;                       // dy = vcl_abs(ye-y1);
          }
        bool far_from_v1 = (dx > d_close || dy > d_close);
        dx = xe - x2; if( dx < 0 )
          {
          dx = -dx;                       // dx = vcl_abs(xe-x2);
          }
        dy = ye - y2; if( dy < 0 )
          {
          dy = -dy;                       // dy = vcl_abs(ye-y2);
          }
