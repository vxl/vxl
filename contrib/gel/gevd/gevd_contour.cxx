...
NumConnectedRays(vtol_vertex_2d& v)
{
  ...
  for( unsigned int i = 0; i < segs.size(); i++ )
    {
    if( segs[i]->v1()->cast_to_vertex_2d() == &v )
      {
      nray++;                                             // 1 for 1-chain
      }
    if( segs[i]->v2()->cast_to_vertex_2d() == &v )
      {
      nray++;                                             // 2 for 1-cycle
      }
    }
  ...
  for( unsigned int i = 0; i < edges.size(); ++i )
    ...
    if( !edge->v1() )    // vertices not created from 1.
      ...
      vtol_vertex_2d_sptr  v =
        new vtol_vertex_2d( ( (*cxy)[0].x() + (*cxy)[last].x() ) / 2, ( (*cxy)[0].y() + (*cxy)[last].y() ) / 2);
...
EqualizeElements(double* elmts, int n, double v1, double v2)
{
  ...
  if( n > 1 )
    {
    elmts[n - 2] = (p0 + p2) / 2;      // last convolution
    }
  if( n > 0 )
    {
    elmts[n - 1] = (p1 + v2) / 2;      // touching second vertex
    }
}
