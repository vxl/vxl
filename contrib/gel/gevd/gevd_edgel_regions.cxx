...
//                               
//      Permission to use, copy, modify, distribute, and sell this softvware
...
//    _corrupt_edges.push_back(corrupt_edges);
//  }
...
//    _corrupt_vertices.push_back(corrupt_verts);
//  }
//--------------------------------------------------------
...
{
  vcl_cout << vcl_endl << "Label Equivalence:" << vcl_endl
       << "----------------" << vcl_endl;
...
{
  vcl_cout << vcl_endl << "Reverse Label Equivalence:" << vcl_endl
       << "----------------" << vcl_endl;
  vcl_map<unsigned int, vcl_vector<unsigned int>*>::iterator rpf_iterator;
...
{
  vcl_cout << vcl_endl << "Base Label Equivalence:" << vcl_endl
       << "----------------" << vcl_endl;
...
void gevd_edgel_regions::print_intensity_data()
{
  for(vcl_vector<gevd_intensity_face*>::iterator fit =_faces->begin(); fit != _faces->end(); fit++)
    ...
    gevd_intensity_face* f = *fit;
      vcl_cout << "IntFaceAt(" << f->Xo() << " " << f->Yo() << "):" <<vcl_endl;
      f->PrintFit();
#endif
    }
}
...
gevd_edgel_regions::gevd_edgel_regions(bool debug)
{
  ...
  _failed_insertions = new vcl_vector<vtol_edge_sptr>;
  _ubuf = NULL;
  _sbuf = NULL;
}
...
bool gevd_edgel_regions::compute_edgel_regions(vcl_vector<vsol_spatial_object_2d *>& sgrp,
                                               vcl_vector<gevd_intensity_face*>& faces)
{
//    _corrupt_edges.clear();
//    _corrupt_vertices.clear();
#if 0
  if(_debug_data)
    {
      _debug_data->clear();
...
static bool line_gen(bool& init, float xs, float ys, float xe, float ye, 
		     unsigned int& x, unsigned int& y)
{
  float pix_edge = 1.0; //We are working at scale = 1.0
  static float xi, yi;
  static bool done;
  if(init)
    {
  ...
  float delta = (0.5*pix_edge)/mag; //move in 1/2 pixel increments
  //Previous pixel location
  unsigned int xp = (unsigned int)(xi/pix_edge);
  unsigned int yp = (unsigned int)(yi/pix_edge);
  ...
  for(int i = 0; i<3; i++)
      ...
      if((xe>=xs&&xi>xe)||(xe<=xs&&xi<xe)||(ye>=ys&&yi>ye)||(ye<=ys&&yi<ye))
	{
      ...
      x = (unsigned int)(xi/pix_edge);
      y = (unsigned int)(yi/pix_edge);
      if(fabs(x-xp)>(.5*pix_edge)||fabs(y-yp)>(.5*pix_edge))
	return true;
    }
  vcl_cout << "in gevd_edgel_regions line_gen: - shouldn't happen" << vcl_endl;
  return false;
}
//----------------------------------------------------------
...
//-----------------------------------------------------------
// -- Initialize the region label array.  There are three types of
//    region label symbols. 1) UNLABELED - no label has yet been assigned,
//    2) EDGE, the existence of an edgel boundary pixel. 3) An unsigned
//    integer which represents an existing region label.
...
bool gevd_edgel_regions::InitRegionArray(vcl_vector< vsol_spatial_object_2d *>& sg)
{
  ...
  switch(this->bytes_per_pix())
    ...
    default:
      vcl_cout<<"In gevd_intensity_face::get_intensity()"
          <<"bytes/pixel not 1 or 2" << vcl_endl;
    }
  ...
  for(vcl_vector<vsol_spatial_object_2d *>::iterator sgit = sg.begin(); sgit != sg.end(); sgit++)
    {
    vtol_edge_2d* e = (vtol_edge_2d *)(*sgit);
      if(!e)
        continue;
      e->set_id(counter++);
...
//----------------------------------------------------------------
// -- Add a new pair to the forward equivalence list. That is, 
//    a ==> b. Note that there can be multiple equivalences which are
...
//----------------------------------------------------------------
// -- Add a new pair to the reverse equivalence list. That is,
//    b==>a. Note that there can be multiple equivalences which are
//    stored in a vcl_vector
//
...
//-------------------------------------------------------------------
// -- Encode a 2x2 neighbor hood with the state of the region array
...
//-------------------------------------------------------------------
// -- Propagate connected components.  Uses an unsigned char encoding
                                    ...
                                    gevd_region_edge* r1, gevd_region_edge* r2)
{
  if(reg_edges_neq(r1, r2))
    if(_verbose)
      vcl_cout << "Collision at (" << x+_xo << " " << y+_yo << ")" << vcl_endl;
}
...
bool gevd_edgel_regions::
corrupt_boundary(vcl_vector<vtol_edge_sptr>& edges,
                 vcl_vector<vtol_vertex_sptr>& bad_vertices)
{
  bool bad = false;
  //Intialize Markers
  vcl_vector<vtol_edge_sptr>::iterator eit = edges.begin();
  ...
  for(;eit != edges.end(); eit++)
    {
      ...
      vtol_vertex_sptr v2 = (*eit)->v2();
      if((v1!=v2)&&*v1==*v2)
        vcl_cout << "Improper Loop( " << vcl_endl << *v1 << *v2 << ")" << vcl_endl << vcl_endl;
...
//     process.
static bool embedded_T(vtol_vertex_sptr v, vtol_edge_sptr bar, vcl_vector<vtol_edge_sptr>& real_edges)
{
  bool embedded = true;
  ...
  for(vcl_vector<vtol_edge*>::iterator eit = edges->begin();
      eit != edges->end(); eit++)
    {
      if(vcl_find(real_edges.begin(), real_edges.end(),(*eit)) == real_edges.end())
        continue;
...
//--------------------------------------------------------------------
// -- Remove hairs from region boundary.  This condition can occur because
...
//    the input array.
bool gevd_edgel_regions::remove_hairs(vcl_vector<vtol_edge_sptr>& edges)
{
  vcl_vector<vtol_edge_sptr> hairs;
  vcl_vector<vtol_edge_sptr> temp;
  //Intialize Markers
  vcl_vector<vtol_edge_sptr>::iterator eit = edges.begin();
  for(;eit != edges.end(); eit++)
    {
  ...
  for(vcl_vector<vtol_edge_sptr>::iterator hit = hairs.begin();
      hit != hairs.end(); hit++)
...
//
bool gevd_edgel_regions::connect_ends(vcl_vector<vtol_edge_sptr>& edges,
...
void gevd_edgel_regions::repair_failed_insertions(vcl_vector<vtol_edge_sptr>& edges, 
                                            vcl_vector<vtol_vertex_sptr>& bad_verts)
{
  vcl_vector<vtol_vertex_sptr> temp1, temp2;
  for(vcl_vector<vtol_vertex_sptr>::iterator vit = bad_verts.begin();
      vit != bad_verts.end(); vit++)
    for(vcl_vector<vtol_edge_sptr>::iterator eit = _failed_insertions->begin();
...
//-------------------------------------------------------------
// - Get the left region label for an Edge used to construct the boundaries
//   A return corresponding to UNLABELED means the domain outside the ROI
unsigned int gevd_edgel_regions::GetLeftLabel(vtol_edge* e)
{
  vcl_map<int,gevd_region_edge *>::iterator reit = _region_edges.begin();
    for (; reit != _region_edges.end() ; reit++)
      {
      if (e->get_id() == reit->first)
        break;
      }
  if( reit == _region_edges.end())
    return UNLABELED;
  return reit->second->GetLeftRegion();
}

//-------------------------------------------------------------
// - Get the right region label for an Edge used to construct the boundaries
//   A return corresponding to UNLABELED means the domain outside the ROI
unsigned int gevd_edgel_regions::GetRightLabel(vtol_edge* e)
{
  vcl_map<int,gevd_region_edge *>::iterator reit = _region_edges.find(e->get_id());
  if( reit == _region_edges.end())
    return UNLABELED;
  return reit->second->GetRightRegion();
}
//--------------------------------------------------------------------
// -- Insert an Edge into the adjacency list for a region
//
void gevd_edgel_regions::insert_adjacency(unsigned int r, vtol_edge_sptr e)
{
  if(!e) return;
  //  e->Protect();
  vcl_map<unsigned int,vcl_vector<vtol_edge_sptr> *>::iterator reit =_region_edge_adjacency.find(r);
  if(reit == _region_edge_adjacency.end())
    {
      vcl_vector<vtol_edge_sptr>* array = new vcl_vector<vtol_edge_sptr>;
      array->push_back(e);
  ...
  for ( vcl_map<int, gevd_region_edge*>::iterator reit= _region_edges.begin(); reit == _region_edges.end(); reit++)   
    {
      ...
      vtol_edge* e = re->get_edge();
      if(_verbose)
	{
	  vcl_cout << vcl_endl << "Edge:" << e 
	       << "(" << e->v1() <<  " " << e->v2() <<"):(";
	}
...
void gevd_edgel_regions::CollectFaceEdges()
{
  ...
  _face_edge_index = new vcl_vector<vtol_edge_sptr>*[_max_region_label];
  for(i=0; i<_max_region_label; i++)
    _face_edge_index[i] = NULL;

  for(i =_min_region_label; i<_max_region_label; i++)
    {
    vcl_map<unsigned int, vcl_vector<vtol_edge_sptr>* >::iterator  reait;
    reait = _region_edge_adjacency.find(i);
      if(reait == _region_edge_adjacency.end())
        continue;
      
      vcl_vector<vtol_edge_sptr>* edges = reait->second;
          ...
          if(!this->connect_ends(*edges, bad_verts))
            {
              if(_verbose)
                {
                  vcl_cout << "Region [" << i << "] is corrupt " << vcl_endl;
                  vcl_cout << "Bad Vertices " << vcl_endl;
                  ...
                  for(vcl_vector<vtol_edge_sptr>::iterator eit = edges->begin();
                      eit != edges->end(); eit++)
                    vcl_cout << vcl_endl <<"Edge(" << vcl_endl 
                         << *((*eit)->v1()) << *((*eit)->v2())
                         <<")" << vcl_endl;
                }
#if 0
      ...
      if(_verbose)
        vcl_cout << " Building Region [" << i << "]" << vcl_endl;
      len = edges->size();

      vcl_vector<vtol_edge_sptr> EdgeSet;
      for(int j =0; j<len; j++)
        {
          vtol_edge_sptr e = (*edges)[j];
          if(_verbose)
            vcl_cout << "Edge(" << e->v1() <<  " " << e->v2() << vcl_endl;
	  EdgeSet.push_back ( e );
        }
      vcl_vector<vtol_edge_sptr>* edge_list = new vcl_vector<vtol_edge_sptr>;
      for (vcl_vector<vtol_edge_sptr>::iterator esit = EdgeSet.begin(); esit != EdgeSet.end() ; esit++)
	{
  ...
  for(i =_min_region_label; i<_max_region_label; i++)
    {
      ...
      vcl_vector<vtol_edge_sptr>* edge_list = _face_edge_index[i];
...
void gevd_edgel_regions::get_buffer_row(unsigned int row)
{
  ...
  for(int x = x0; x<xsize; x++)
    switch(this->bytes_per_pix())
      ...
      default:
        vcl_cout<<"In gevd_edgel_rgions::get_row()"
            <<"bytes/pixel not 1 or 2" << vcl_endl;
        return;
      }
}
...
void gevd_edgel_regions::get_image_row(unsigned int row)
{
  ...
  switch (this->bytes_per_pix())
    ...
    default:
      vcl_cout<<"In gevd_edgel_regions::get_row()"
          <<"bytes/pixel not 1 or 2" << vcl_endl;
    }
}
//---------------------------------------------------------------
...
unsigned short gevd_edgel_regions::get_intensity(unsigned int x)
{
  ...
  switch (this->bytes_per_pix())
    ...
    default:
      vcl_cout<<"In gevd_edgel_regions::get_intensity()"
          <<"bytes/pixel not 1 or 2" << vcl_endl;
    }
  return intensity;
}
...
//    gevd_intensity_face parameters
