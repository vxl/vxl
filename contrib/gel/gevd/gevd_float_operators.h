...
public:
  ...
  static float Convolve(const gevd_bufferxy& from, gevd_bufferxy *& to, // can mutate in place
                        ...
                        const int xradius, const bool xevenp, const float* ykernel, const int yradius,
                        const bool yevenp, const bool xwrap = false, const bool ywrap = false);

  static float Convolve(const gevd_bufferxy& from, gevd_bufferxy *& to,          // can mutate in place
                        const float* xkernel,                                    // when from==to
                        const int xradius, const bool xevenp, const int yradius, // running sum along y-axis
                        const bool xwrap = false, const bool ywrap = false);

  static float Convolve(const float* from, float *& to, const int len, const float* kernel, const int radius,
  ...
  static float Gaussian(gevd_bufferxy& img, gevd_bufferxy *& smooth, const float sigma = 1.0f, const bool xwrap = false,
  ...
  static float Gradient(const gevd_bufferxy& smooth, // 1st-derivative
                        gevd_bufferxy *& mag, gevd_bufferxy *& dirx, gevd_bufferxy *& diry, const bool xwrap = false,
                        const bool ywrap = false);
  ...
  static float Hessian(const gevd_bufferxy& smooth, // 2nd-derivative
                       ...
                       gevd_bufferxy *& dirx, gevd_bufferxy *& diry, const bool xwrap = false,
                       const bool ywrap = false);

  static float Laplacian(const gevd_bufferxy& smooth, gevd_bufferxy *& mag, // sum of 2 eigenvalue/curvature
                         gevd_bufferxy *& dirx, gevd_bufferxy *& diry, const bool xwrap = false,
                         const bool ywrap = false);

  static float Curvature(float* from, float *& to, const int len, const bool wrap = false);

  static float Orientation(const gevd_bufferxy& smooth, gevd_bufferxy *& theta,  gevd_bufferxy *& coherence,
                           const int frame = 1);

  static void NonMaximumSuppression(const gevd_bufferxy& magnitude, const gevd_bufferxy& directionx,
                                    const gevd_bufferxy& directiony, const float threshold, gevd_bufferxy *& contour,
                                    gevd_bufferxy *& direction, gevd_bufferxy *& locationx, gevd_bufferxy *& locationy,
                                    const bool xwrap = false, const bool ywrap = false);

  static int NonMaximumSuppression(const float* from, const int len, const float threshold, int *& index, float *& mag,
                                   float *& locs, const bool wrap = false);
  ...
  static void SupportAngle(const gevd_bufferxy& dirx, const gevd_bufferxy& diry, const gevd_bufferxy& magnitude,
                           gevd_bufferxy *& angle);
  ...
  static void SurfaceNormalD(const gevd_bufferxy& range, gevd_bufferxy *& normal, float no_value,
  ...
  static void SurfaceCurvatureD(const gevd_bufferxy& normal, gevd_bufferxy *& curvature, float dflt,
                                float pixel_distance = 1.0f);

#endif // 0
  static void SurfaceCurvatureD(const gevd_bufferxy& normal, const gevd_bufferxy& surface, gevd_bufferxy *& curvature,
                                float dflt, float pixel_distance = 1.0f);

  // shrink/expand
  static float ShrinkBy2(const gevd_bufferxy& cfrom, gevd_bufferxy *& to, const float burt_ka = 0.359375f); // Burt filter coeft

  static float ExpandBy2(const gevd_bufferxy& cfrom, gevd_bufferxy *& to, const float burt_ka = 0.359375f); // Burt filter coeft

  static void ShrinkBy2_D(const gevd_bufferxy& from, gevd_bufferxy *& to, float no_value, float burt_ka = 0.359375f );

  static int Pyramid(const float* from, const int length, float *& to, int& nlevels, int trim = 0,
  ...
  static bool WaveletTransform(float* array, const int n, // 1d array
                               const bool forwardp, int nlevels, const int waveletno = 4);

  static bool WaveletTransformByBlock(float* array, // nd imge.
                                      const int* dims, const int ndim, const bool forwardp, int nlevels,
                                      const int waveletno = 4);

  static bool WaveletTransformByIndex(float* array, const int* dims, const int ndim, const bool forwardp, int nlevels,
                                      const int waveletno = 4);

  static bool FindWavelet(const int waveletno, float *& cc, float *& cr, int& ncof);

  static void LowHighPyramid(float* highPass, float* lowPass, int n, int nlevels, const float* lo_filter,
                             const float* hi_filter, int ncof, float* wksp);

#if 0                                           // commented out
  static int DeleteMixedComponents(float* wave, // delete wavelet coefts
                                   const int* dims, const int ndim, const int nlevels)
  static int TruncateHighFrequencies(float* wave, const int* dims, const int ndim, const int nlevels, // throw all small
                                     const float threshold = 0.1f);                                   // components

  static int TruncateLowestFrequency(float* wave, const int* dims, const int ndim, const int nlevels, float& average); // uniform average

#endif

  static bool WaveletTransformByBlock(const gevd_bufferxy& cfrom, gevd_bufferxy *& to, const bool forwardp, int nlevels,
                                      const int waveletno = 4);

  static bool WaveletTransformByIndex(const gevd_bufferxy& cfrom, gevd_bufferxy *& to, const bool forwardp, int nlevels,
                                      const int waveletno = 4);
  ...
  static void ProjectOntoX(const gevd_bufferxy& buf, float *& proj, int sizeX = 0, int sizeY = 0, int origX = 0,
                           int origY = 0);

  static void ProjectOntoY(const gevd_bufferxy& buf, float *& proj, int sizeX = 0, int sizeY = 0, int origX = 0,
                           int origY = 0);
  ...
  static float Correlation(const float* data, const int length,// correlation at index
                           const float* pattern, const int radius, const int index);

  static float * Correlations(const float* data, const int length, // cors around index
                              const float* pattern, const int radius, const int index, const int search);

  static float BestCorrelation(const float* data, const int length, // search max cor
                               const float* pattern, const int radius, int& shift, const int search);

  static float CoarseFineCorrelation(const float* dataPyr, const int dlength, const float* patternPyr,
                                     const int plength, float& shift,  // pattern location in data
...
#if 0 // commented out
  static gevd_bufferxy * Correlations(const gevd_bufferxy& data, const gevd_bufferxy& pattern, const int* indexes,
                                      const int* search);

  static float Correlate(const gevd_bufferxy& data, const gevd_bufferxy& pattern, const int* indexes);

  static float Correlate(const gevd_bufferxy& data, const gevd_bufferxy& pattern, int *& shifts, const int* search);
...
#if 0 // commented out
  static gevd_bufferxy * MakeImpulse(const float mag, int x = 100, int y = 100);

  static gevd_bufferxy * MakeGaussianNoise(const float mag, const float sigma, int x = 100, int y = 100);

  static gevd_bufferxy * MakeCircles(const float mag, float w1, float w2, int x = 100, int y = 100);

  static gevd_bufferxy * MakeEllipses(const float mag, const float w1, const float w2, const float eccentricity,
                                      int x = 100, int y = 100);

  static gevd_bufferxy * MakeSuperellipses(const float mag, const float w1, const float w2, float, float, float,
                                           int x = 100, int y = 100);
  ...
  // utilities
  static gevd_bufferxy * Allocate(gevd_bufferxy* space, const gevd_bufferxy& model, int bits_per_pixel = 0,
  ...
  static gevd_bufferxy * Extract(const gevd_bufferxy& buf, int sizeX = 0, int sizeY = 0, int origX = 0, int origY = 0);

  static void Update(gevd_bufferxy& old_buf, const gevd_bufferxy& new_buf, int origX = 0, int origY = 0);

  static gevd_bufferxy * TransposeExtract(const gevd_bufferxy & buf, int sizeX, int sizeY, int origX, int origY);
...
protected:
  static int SetupPipeline(const float* data, const int len, const int kradius, const bool wrap, float *& cache,
                           float *& pipeline);
  ...
  static float ShrinkBy2AlongX(const gevd_bufferxy& cfrom, const int y, float* yline, const int len, const float ka,
                               const float kb, const float kc);

  static void ShrinkBy2AlongX_D( const gevd_bufferxy& from, int from_sizeX, int sizeX, int y, float kernel[],
                                 float no_value, float* yline, float* wline );

  static float ExpandBy2AlongX(const gevd_bufferxy& cfrom, const int y, float* yline, const int len, const float ka,
                               const float kb, const float kc);

  static void WaveletTransformStep(float* array, const int n, const bool forwardp, const float* lo_filter,
                                   const float* hi_filter, const int ncof, float* wksp);

  static void WaveletTransformStep(float* array, const int* dims, const int ndim, const bool forwardp,
                                   const float* lo_filter, const float* hi_filter, const int ncof, float* buffer,
                                   float* wksp);

  static void CopyNdRecursive(const float* from_array, const int from_size, const int* from_dims, float* to_array,
                              const int to_size, const int* to_dims, const int ndim, const bool fullp = true);
