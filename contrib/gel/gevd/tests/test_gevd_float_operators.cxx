...
#define TEST_(m, i, j, x) TEST_NEAR("gevd_float_operators::"# m, *(float *)buf_out->GetElementAddr(i, j), x, 1e-7f)
  for( int i = 1; i < 7; ++i )
    {
    for( int j = 1; j < 7; ++j )                    // avoid boundaries: undefined behaviour
      {
      if( i == 2 && j == 3 ) { TEST_("Convolve", i, j, 0.0f); }
      else if( i == 2 && j == 2 )
        {
        TEST_("Convolve", i, j, 10.0f);
        }
      else if( i == 1 && j == 2 )
        {
        TEST_("Convolve", i, j, -10.0f);
  ...
  for( int i = 1; i < 7; ++i )
    ...
    for( int j = 1; j < 7; ++j )
      ...
      if( i == 2 && j == 3 ) { TEST_("Correlation", i, j, -3.f / vcl_sqrt(177.f) ); }
        ...
        {
        TEST_("Correlation", i, j, 5.f / vcl_sqrt(177.f) );
        }
      else if( i == 1 && j == 2 )
        {
        TEST_("Correlation", i, j, -11.f / vcl_sqrt(177.f) );
        }
      else if( i == 1 && j == 3 )
        {
        TEST_("Correlation", i, j, 9.f / vcl_sqrt(177.f) );
        }
