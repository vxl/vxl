...
static void test_optimize_camera()
{
  ...
  const double max_r_err = 1.5707963; // maximum rotation error to introduce (radians)
  ...
  vnl_vector<double> w = (3.141592653 - vcl_asin(vcl_sqrt(2.0/3.0)))/vcl_sqrt(2.0)*vnl_double_3(-1.0,1.0,0.0);
  ...
  for(unsigned int i=0; i<world.size(); ++i){
    ...
    vcl_cout <<  err << "\t";
    vcl_cout <<  vgl_point_2d<double>(hpt.x()/hpt.w(), hpt.y()/hpt.w()) << vcl_endl;
  }
  ...
  vcl_cout << "Initial principal ray:" << cam.principal_axis() << vcl_endl;

  vpgl_perspective_camera<double> err_cam(K,c+max_t_err*dc,H);
  vcl_cout << "Perturbed position:" << err_cam.get_camera_center() << vcl_endl;
  vcl_cout << "Perturbed principal ray:" << err_cam.principal_axis() << vcl_endl;
  ...
  vcl_cout << "Estimated principal ray:" << opt_cam.principal_axis() << vcl_endl;
