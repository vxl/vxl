...
{
  for (unsigned i = 0; i < params_indices.size(); i++)
       ...
       vcl_vector<vgl_point_3d<double> >& intersections)
{
  ...
  if (!corrs[0].size())
    return false;
  int cnt_corrs_for_each_cam = corrs[0].size();
  ...
  for (int i = 0; i < cnt_corrs_for_each_cam; i++) { // for each corr
    for (unsigned j = 0; j < corrs.size(); j++) // for each cam (corr size and cams size are equal)
                  ...
                  vcl_vector<vgl_point_3d<double> >& intersections)             // output 3d locations for each correspondence
{
  ...
  for (unsigned i = 0; i < intersections_initial.size(); i++)
