...
vpgl_bundler_tracks_impl_detect_sift::operator ()(
    ...
    vpgl_bundler_inters_feature_set_sptr feature_set( 
        new vpgl_bundler_inters_feature_set);
    ...
    for(kp = keypoints.begin(); kp != keypoints.end(); ++kp){
        ...
        vpgl_bundler_inters_feature_sptr f(
...
void vpgl_bundler_tracks_impl_propose_matches_all::operator ()(
    ...
    vcl_vector<vpgl_bundler_inters_feature_set_sptr>::const_iterator i, j;

    for(i = features.begin(); i != features.end(); i++){
        for(j = i; j != features.end(); j++){
            if(i != j){
                vpgl_bundler_inters_feature_set_pair pair;
...
static void to_rsdl_pt_vector(
    const vcl_vector<vpgl_bundler_inters_feature_sptr> &f,
    vcl_vector<rsdl_point> &v){

    vcl_vector<vpgl_bundler_inters_feature_sptr>::const_iterator i;
...
void vpgl_bundler_tracks_impl_match_ann::operator ()(
...
static void remove_if_true(
    ...
    vpgl_bundler_inters_match_set &to_prune){
...
/*Refine implementation.*/
void vpgl_bundler_tracks_impl_refine_epipolar::operator ()(
    vpgl_bundler_inters_match_set &matches){
...
void create_new_track(
    ...
    vcl_stack<vpgl_bundler_inters_feature_sptr> feature_stack;

    //Mark both the input features as explored, and add them to the
    // stack
    f1->visited = true;
    ...
    //Start the search
    while(not feature_stack.empty()){
        //Get the current element from the stack
        vpgl_bundler_inters_feature_sptr curr;
        ...
        for(match = matches.begin(); match != matches.end(); match++){
            ...
            for(int i = 0; i < match->num_features(); i++){
                if(match->side1[i] == curr and
                    not match->side2[i]->visited){

                    match->side2[i]->visited = true;
                    feature_stack.push(match->side2[i]);

                }else if(match->side2[i] == curr and
                    not match->side1[i]->visited){
