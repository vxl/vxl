// Ths is gel/mrc/vpgl/algo/vpgl_em_compute_5_point.h
...
bool vpgl_em_compute_5_point<T>::compute( 
    ...
    if(right_points.size() != 5 || left_points.size() != 5){
        if(verbose){
            vcl_cerr<<"Wrong number of input points!" << vcl_endl <<
                "right_points has "<<right_points.size() << 
                " and left_points has "<<left_points.size() << vcl_endl;
        }
...
/*-----------------------------------------------------------------------*/
//Returns the coefficentof a term of a vnl_real_npolynomial in three 
