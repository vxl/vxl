        ...
        vgl_point_3d<double> &point_3d)
{
    ...
    for (int i = 0; i < points.size(); i++) {
        ...
        A.put(2 * i, 0,
            rot.get(0, 0) - points[i].x() * rot.get(2, 0) );

        A.put(2 * i, 1,
            rot.get(0, 1) - points[i].x() * rot.get(2, 1) );

        A.put(2 * i, 2,
            rot.get(0, 2) - points[i].x() * rot.get(2, 2) );

        // Set the row for y for this point
        A.put(2*i+1, 0,
            rot.get(1, 0) - points[i].y() * rot.get(2, 0) );

        A.put(2*i+1, 1,
            rot.get(1, 1) - points[i].y() * rot.get(2, 1) );

        A.put(2*i+1, 2,
            rot.get(1, 2) - points[i].y() * rot.get(2, 2) );
