...
class icam_ocl_search_manager : public icam_ocl_manager<icam_ocl_search_manager>
{
 public:
  ...
  icam_ocl_kernel* kernel() {return kernel_;}
  ...
  cl_int* image_para_flag() {return image_para_flag_;}
  ...
  cl_int* rot_para_flag() {return rot_para_flag_;}
 ...
 protected:
  ...
  cl_float* Ks_; // source image K matrix (actually cl_float4)
  ...
  cl_uint * cl_dni_;
  cl_uint * cl_dnj_;
  cl_float* Kdi_;// inverse  K matrix for destination image (actually cl_float4)
  ...
  cl_float* rotation_;  // actually cl_float4
  cl_float* rot_array_; // actually cl_float4
  // search translations
  vcl_vector<vgl_vector_3d<double> > translations_;
  cl_float* translation_; // actually cl_float4

  // the result of the search (image parallel)
  cl_float* image_para_result_; // actually cl_float4
  cl_int  * image_para_flag_;   // actually cl_int4

  // the result of the search (rot parallel)
  // minfo result
  cl_float* minfo_array_;
  cl_int  * rot_para_flag_; // actually cl_int4
