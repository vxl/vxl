                              ...
                              unsigned& dest_nj, icam_transform_2d& mod_trans)
{
  ...
  icam_transform_2d tinv = t.inverse();
  ...
  icam_transform_2d dinv = mod_trans.inverse();
  //revised bounds of destination image
  ul=dinv(0,0);  ur = dinv(ni_d,0); lr = dinv(ni_d, nj_d); ll = dinv(0, nj_d);
  bb.empty();//clear the box
  bb.add(ul); bb.add(ur); bb.add(lr); bb.add(ll);
  dest_ni = static_cast<int>(bb.max_x())+1; 
  dest_nj = static_cast<int>(bb.max_y())+1;
}

icam_image<float> icam_utils::destination_mask(unsigned source_ni,
                                                   unsigned source_nj,
                                                   icam_transform_2d const& t)
{
  unsigned ni, nj;
  icam_transform_2d m, m_inv;
  icam_utils::image_bounds(source_ni, source_nj, t, ni, nj, m); 
  m_inv = m.inverse();
  double ni_d = source_ni-1, nj_d = source_nj-1;
 ...
 //create scan converter
  ...
  icam_image<float>imask(mask, m_inv);
 return imask;
}
#endif