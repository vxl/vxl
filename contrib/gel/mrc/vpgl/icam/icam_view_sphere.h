...
public:
  ...
  //: Constructor, creates a view sphere around the box, centered 
  // at box's center with radius
  icam_view_sphere(vgl_box_3d<double> bb, double radius);

  //: Destructor
  ~icam_view_sphere(){ delete view_sphere_; }

  //: creates uniform view points on the view square and generates 
  // expected and depth images at each view point
