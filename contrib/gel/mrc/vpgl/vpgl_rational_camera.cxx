    ...
    : world_scale_ (false), image_scale_ (false), global_ (true)
{
  ...
  for (i=0 ;i<20 ;i++) matrix.put (0,i,Un[i]) ;
  for (i=0 ;i<20 ;i++) matrix.put (1,i,Ud[i]) ;
  for (i=0 ;i<20 ;i++) matrix.put (2,i,Vn[i]) ;
  for (i=0 ;i<20 ;i++) matrix.put (3,i,Vd[i]) ;
...
#if 0
//------------------------------------------------------------
// -- Computes the intial point as the center of the x, y, z domain
//    as defined by the offset values
    ...
    vnl_vector<double> LL)
{
  // Output
  vcl_cout << "---------------------------------------------------" << vcl_endl ;
  vcl_cout << "           VERIFYING RATIONAL CAMERA               " << vcl_endl ;
