...
void computeborgefors( const vbl_array_2d<bool> & edges, vbl_array_2d<short> & distance)
{
  ...
  if( r <= 2 || c <= 2 )
    {
    return;                     // the iterations below are empty in that case
...
void computeedgemap( vil1_memory_image_of<vxl_byte> imbuf, vbl_array_2d<bool> & edges)
{
  ...
  if( r <= 2 || c <= 2 )
    {
    return;                     // the iteration below is empty in that case
...
int main(int argc, char * * argv)
{
  ...
  // Create voxmap
  VoxmapImagePoints voxmap(iterations() - 1, vnl_double_3( double(sx() ), double(sy() ), double(sz() ) ),
                           double(ss() ), MAXIMAGES);
    ...
    for( unsigned int i = 0; i < voxels.size(); i++ )
      ...
      if( !outsideobject[i] && insideimage[i] && !insideobject[i] )
        ...
        if( count != (int(iterations() ) - 1) )
          ...
          newvoxels.push_back( Voxel( voxels[i].depth + 1, voxels[i].x * 2 + 1, voxels[i].y * 2 + 1,
                                      voxels[i].z * 2  ) );
          newvoxels.push_back( Voxel( voxels[i].depth + 1, voxels[i].x * 2 + 1, voxels[i].y * 2 + 1,
                                      voxels[i].z * 2 + 1) );
          }
