...
//: constructor we send in the parameters that can change,
...
//: the method for evaluating the cost of the parameter values (x) 
//  and getting the residuals (fx)
...
void pop_graph_cost_function::f(vnl_vector<double> const &x, vnl_vector<double> &fx)
{
 ...
 for(i=0;i<params_.size();i++){
  ...
  // sterp 3 collect the residuals 
  for(i=0;i<cfs_.size();i++){
    fx(i) = cfs_[i]->cost();
  }
...
//: get the current paramter valuse
