...
double vdgl_interpolator_cubic::get_x(double index)
{
  ...
  if( index == N - 2 )
    {
    --a;                 // take previous interval if we are exactly on edgel N-2
    }
  int b = a + 1;
  int c = b + 1;
  int d = c + 1;
...
double vdgl_interpolator_cubic::get_grad(double index)
{
  ...
  if( index == N - 2 )
    {
    --a;                 // take previous interval if we are on edgel N-2
    }
  int b = a + 1;
  int c = b + 1;
  int d = c + 1;
...
vsol_point_2d_sptr vdgl_interpolator_cubic::closest_point_on_curve( vsol_point_2d_sptr p )
{
