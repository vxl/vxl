// This is contrib/gel/vifa/vifa_coll_lines.cxx
#include <vcl_cmath.h>
...
bool vifa_coll_lines::
get_discard_flag(void) const
{
...
double vifa_coll_lines::
get_measure(const vtol_edge_2d&	e) const
{
	return get_measure(e, *hypothesized_line_);
}

double vifa_coll_lines::
get_projected_length(const vtol_edge_2d&	e) const
{
	double	v1;
	double	v2;
...
edge_2d_list& vifa_coll_lines::
get_contributors(void)
{
	return contributors_;
}

face_list* vifa_coll_lines::
get_contributor_faces(void)
{
...
void vifa_coll_lines::
lms_fit(const vcl_vector<double>&	x,
...
void vifa_coll_lines::
add_and_update(vtol_edge_2d_sptr	e)
{
	contributors_.push_back(e);
	this->fit_line();
}
...
double vifa_coll_lines::
spanning_length(vgl_point_2d<double>&	p1,
				vgl_point_2d<double>&	p2)
{
	double	min_d;
	double	min_x;
	...
	//vcl_cout << "  -> span: p1 is " << max_x << " , " << max_y << " ( " <<
	//	max_d << " ) " << vcl_endl;
	...
	//vcl_cout << "  -> span: p2 is " << min_x << " , " << min_y << " ( " <<
	//	min_d << " ) " << vcl_endl;
...
bool vifa_coll_lines::
contains(const vtol_edge&	edgeref)
{
	for (edge_2d_iterator e = contributors_.begin();
			e != contributors_.end(); ++e)
	{
...
double vifa_coll_lines::
get_projected_length(const vtol_edge_2d&	e,
...
double vifa_coll_lines::
get_midpt_dist(const vtol_edge_2d&	e,
			   const imp_line&		hyp_line)
{
	const vgl_point_2d<double>&	v1 = e.curve()->p0()->get_p();
	const vgl_point_2d<double>&	v2 = e.curve()->p1()->get_p();
...
double vifa_coll_lines::
get_measure(const vtol_edge_2d&	e,
