	...
	: vifa_group_pgram_params(old_params)
{
	...
	_th_dim = (int)ceil(_angle_range/_angle_increment);
...
//--------------------------------------------------------------
// -- Compute parallel line overlap on a line at the same orientation
//    with midpoint at the center of the region of the line group.
//
...
//--------------------------------------------------------------
// -- Collect implicit line(s) from the angle array at orientations +- the
//    given bin orienation.  Wrap around the end of the array so that
//    0 degrees and 180 degrees are considered parallel.
void vifa_group_pgram::
...
//------------------------------------------------------
// -- Compute the total length of parallel lines
//    normalized by the total edge perimeter
double vifa_group_pgram::
...
AngleLoc(imp_line_sptr	il)
{
	...
	{
		vcl_cerr << "In vifa_group_pgram::AngleLoc(): " <<
			"angle " << angle << " was outside the angle range " <<
			_angle_range << vcl_endl;

		return 0;
	}

	return (int)(floor(angle / _angle_increment));
}

//--------------------------------------------------------------
// --Define a line passing through the center of the Hough ROI
//   and at an angle correspoinding the angle bin
...
LineAtAngle(int	angle_bin)
{
	...
	vgl_vector_2d<double>	d(cos(ang_rad), sin(ang_rad));
...
//    A dominant direction is a local maxium in the coverage distribution.
//
void vifa_group_pgram::
ComputeDominantDirs(void)
{
	...
	vifa_histogram_sptr	h_sup = h->NonMaximumSupress(this->_max_suppress_radius,
													 true);
...
//		<< _dominant_dirs.size() << " dominant directions found" << vcl_endl;
}
