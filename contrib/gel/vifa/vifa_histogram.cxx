...
vifa_histogram* vifa_histogram::NonMaximumSupress(int radius, bool cyclic)
{
  if((2*radius +1)> num/2)
    {
      vcl_cerr << "In vifa_histogram::NonMaximumSupress(): " <<
		"the radius is too large" << vcl_endl;
      return NULL;
    }
...
float vifa_histogram::GetMean()
{
  ...
  if (MEAN_FLAG & stats_consistent)
  ...
  else
    {
      ...
      if(area <= 0.0)
        {
          //vcl_cerr << "vifa_histogram : Area <= 0.0" << vcl_endl;
          return 0.0;
        }
      else
        {
  ...
  for (int j = 0; j < res; j++)
    {
      ...
      printf("%6.1f %5.0f |", vals[j], counts[j]);
      width++;
    }
  
  vcl_cout << vcl_endl << " MaxVal " << this->GetMaxVal() << vcl_endl;
  vcl_cout << " MinVal " << this->GetMinVal() << vcl_endl;
  vcl_cout << " BucketSize " << this->GetBucketSize() << vcl_endl;
  vcl_cout << " Resolution " << this->GetRes() << vcl_endl;
  vcl_cout << " Area "
       << this->ComputeArea(this->GetMinVal(),this->GetMaxVal()) << vcl_endl;
...
void vifa_histogram::Dump(char *dumpfile)
{
  ...
  if (!dumpfp)
    {
      vcl_cerr << "Error opening histogram data file." << vcl_endl;
      return;
    }
...
int vifa_histogram::WritePlot(const char *fname)
{
  ...
  if (!fp)
    {
      vcl_cerr << "Error opening histogram plot file." << vcl_endl;
      return 0;
    }
...
float vifa_histogram::CompareToHistogram(vifa_histogram* h)
{
  ...
  if ( (fabs(v1) < 1.0e-6) || (fabs(v2) < 1.0e-6) ) return 0.0;
