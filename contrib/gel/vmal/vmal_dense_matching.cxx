											   ...
											   vmal_multi_view_data_edge_sptr res)
{
	...
	if (mvd_edge->get_nb_views()>1)
	{
		...
		for(int i=0;i<numlines;i++)
		{
			...
			if(vmal_operators::line_cross_seg(lines1_p[i], lines1_q[i], 
											  epi_line1p, inter1p,alpha))	
				if(vmal_operators::line_cross_seg(lines1_p[i], lines1_q[i], 
											      epi_line1q, inter1q,beta))		
				{ 
					//1 case: the first segment is "included" in the second segment
					vmal_operators::line_cross_seg(lines0_p[i], lines0_q[i], 
											       epi_line0p, pt0p,alpha);	
					vmal_operators::line_cross_seg(lines0_p[i], lines0_q[i], 
											       epi_line0q, pt0q,beta);	
				}
				else
				{	
										...
										int h,int w)
{
	...
	for(int i=0;i<numlines;i++)
	{
		...
		{	if(num0!=0.0)
				if(fabs(((point0[0]+delta0x)*a0+b0)-point0[1])<=0.5)
					point0[0]+=delta0x*2;	   	   	
				else 
					point0[1]+=delta0y*2;	   	   
			else
			...
			if(num1!=0.0)
				if(fabs(((point1[0]+delta1x)*a1+b1)-point1[1])<=0.5)
					point1[0]+=delta1x*2;	   	   	
				else 
					point1[1]+=delta1y*2;	
			else
			...
			t_point0=_H0*point0;
			t_point1=_H1*point1;
			int_t_point0[0]=round(t_point0[0]/t_point0[2]); int_t_point0[1]=round(t_point0[1]/t_point0[2]);
			int_t_point1[0]=round(t_point1[0]/t_point1[2]); int_t_point1[1]=round(t_point1[1]/t_point1[2]);
