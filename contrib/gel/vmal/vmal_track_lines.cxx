            ...
            if( belong(cur_tl, cur_fl) )
              ...
              if( (cur_dist <= min_dist) && (cur_dist != -1) )
                ...
                if( tmp_matches->get_pred_match(view_num, cur_fl, other_match) )
                  {
                  vtol_edge_2d_sptr t_other_match =
                    find_transfo(other_match, *(*fit_lines)[view_num], *(*transformed_lines)[view_num]);
                  double other_dist;
...
int vmal_track_lines::is_cur_best(vtol_edge_2d_sptr trans_line, vtol_edge_2d_sptr fit_line,
