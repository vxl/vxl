...
double vmal_track_lines::dist(vtol_edge_2d_sptr trans_line,vtol_edge_2d_sptr fit_line)
{
	...
	{ 
		...
		{
		//	dist=2*dist;
		//	distover=min(sqrtf((x1-tl2x)*(x1-tl2x)+(y1-tl2y)*(y1-tl2y)),
					    // sqrtf((x1-tl1x)*(x1-tl1x)+(y1-tl1y)*(y1-tl1y)));
			dist=-1;
			distover=1;
		}
...
double vmal_track_lines::min(double x,double y)
{
	if(x>y)
		return y;
	else
...

{
	...
	double corr=lines_correlation(line0, 
							      line1, 
						          homo, i0, i1);
	
	//double dis=dist(t_line0, line1);
	//result=dis;
	//double alpha=0.5;
		result=corr;//dist*alpha+(1-aplha);
}