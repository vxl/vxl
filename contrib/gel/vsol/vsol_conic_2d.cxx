...
{
  return obj.cast_to_curve() && obj.cast_to_curve()->cast_to_conic() &&
...
vsol_conic_2d::vsol_conic_type vsol_conic_2d::real_type() const
{
  if( type() == vgl_conic<double>::real_circle )
    ...
    {
    return invalid;    // 'degenerate' was is not a good name: some of the above are already degenerate!
    }
}
                                         ...
                                         double & height) const
{
  ...
  if( is_zero(b0) )  // axis already horizontal or vertical
    ...
    if( a0 > 0 ) {phi = 0; }
      ...
      {
      phi = vcl_atan2(0.0, 1.0);          // 90 degrees
      }
    }
  else
    {
