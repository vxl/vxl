...
{
  return obj.cast_to_region() && obj.cast_to_region()->cast_to_polygon() &&
...
{
  if( blanking < 0 ) {blanking = 0; } while( blanking-- )
    {
    strm << ' ';
    }
...
vsl_b_read(vsl_b_istream & is, vsol_rectangle_3d * & r)
{
  ...
  if( not_null_ptr )
    {
    r =
      new vsol_rectangle_3d(new vsol_point_3d(0.0, 0.0, 0.0), new vsol_point_3d(0.0, 0.0, 0.0),
                            new vsol_point_3d(0.0, 0.0, 0.0) );
    r->b_read(is);
    }
  else
    {
