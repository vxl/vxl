...
struct vtol_extract_topology_params
  ...
  vtol_extract_topology_params & set_smooth( unsigned s ) { num_for_smooth = s; return *this; }
...
public:  // public methods
  ...
  void add_faces( vcl_vector<vtol_intensity_face_sptr>& faces, finder_type* find, data_image_type const* img,
                  chain_tree_node* node, bool even_level = false ) const;
  ...
  //
  static
  bool contains( region_type_sptr a, region_type_sptr b );
  ...
  //
  static
  unsigned num_crosses_x_pos_ray( double x, double y, vdgl_edgel_chain const& chain );
...
private:    // internal classes and constants
  ...
  //
  bool trace_face_boundary( vcl_vector<unsigned>& markers, unsigned index, unsigned dir, region_type& chain,
  ...
  // (vdgl_digital_region).
  //
  void compute_faces( vcl_vector<region_type_sptr> const& chains, vcl_vector<vtol_intensity_face_sptr>& faces,
