...
template <typename T>
vtol_extract_topology<T>::vtol_extract_topology( label_image_type const& in_image,
...
vtol_extract_topology<T>::vertices() const
{
...
vtol_extract_topology<T>::faces( data_image_type const& data_img ) const
{
...
template <typename T>
vcl_vector<vtol_intensity_face_sptr>
vtol_extract_topology<T>::faces() const
{
  region_collection region_list;
...
template <typename T>
void
vtol_extract_topology<T>::compute_label_range()
{
...
vtol_extract_topology<LABEL_TYPE>::label( unsigned i, unsigned j ) const
{
...
template <typename T>
bool
vtol_extract_topology<T>::is_junction_vertex( unsigned i, unsigned j ) const
{
...
template <typename LABEL_TYPE>
bool
vtol_extract_topology<LABEL_TYPE>::is_boundary_vertex( unsigned i, unsigned j ) const
{
...
template <typename LABEL_TYPE>
bool
vtol_extract_topology<LABEL_TYPE>::is_edge( unsigned i, unsigned j, unsigned dir ) const
{
...
template <typename LABEL_TYPE>
void
vtol_extract_topology<LABEL_TYPE>::edge_labels( unsigned i, unsigned j, unsigned dir,
...
template <typename T>
unsigned
vtol_extract_topology<T>::vertex_index( unsigned i, unsigned j ) const
{
...
template <typename T>
void
vtol_extract_topology<T>::set_vertex_index( unsigned i, unsigned j, unsigned index )
{
  assert( i < index_img_.ni() &&
          j < index_img_.nj() );
...
vtol_extract_topology<T>::node( unsigned index )
{
...
vtol_extract_topology<T>::node( unsigned index ) const
{
  assert( index < node_list_.size() );
...
template <typename T>
void
vtol_extract_topology<T>::move( unsigned dir, unsigned& i, unsigned& j )
{
  assert( dir < 4 );
...
template <typename T>
void
vtol_extract_topology<T>::set_mark( unsigned& marker, unsigned dir ) const
{
  marker |= ( 1 << dir );
}
...
template <typename T>
bool
vtol_extract_topology<T>::is_marked( unsigned marker, unsigned dir ) const
{
...
template <typename T>
void
vtol_extract_topology<T>::trace_edge_chain( unsigned i, unsigned j, unsigned dir )
{
...
template <typename T>
void
vtol_extract_topology<T>::construct_topology()
{
...
template <typename LABEL_TYPE>
bool
vtol_extract_topology<LABEL_TYPE>::trace_face_boundary( vcl_vector<unsigned>& markers,
                                                        unsigned index,
                                                        unsigned dir,
...
template <typename LABEL_TYPE>
void
vtol_extract_topology<LABEL_TYPE>::collect_regions( region_collection& region_list ) const
{
...
template <typename T>
void
vtol_extract_topology<T>::compute_faces( vcl_vector<region_type_sptr> const& chains,
                                         vcl_vector<vtol_intensity_face_sptr>& faces,
...
template <typename T>
void
vtol_extract_topology<T>::add_faces( vcl_vector<vtol_intensity_face_sptr>& faces,
...
template <typename T>
bool
vtol_extract_topology<T>::contains( region_type_sptr a, region_type_sptr b )
{
  assert( b );
  if( !a )
    {
...
template <typename T>
unsigned
vtol_extract_topology<T>::num_crosses_x_pos_ray( double x, double y, vdgl_edgel_chain const& chain )
{
  if( chain.size() < 2 )
    {
    return 0;
...
vtol_extract_topology<T>::smooth_chain( vdgl_edgel_chain_sptr chain,
