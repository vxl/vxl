...
int main(int, char **)
{
  ...
  vtol_vertex_2d_ref v1=new vtol_vertex_2d(1.0,2.0);
  Assert(v1->x()==1.0);
  Assert(v1->y()==2.0);
  
  vsol_point_2d new_point(1.0,2.0);

  vtol_vertex_2d_ref v1a = new vtol_vertex_2d(new_point);
  
  vnl_double_2 vec;
  vec(0)=1.0;
  vec(1)=2.0;

   vtol_vertex_2d_ref v1b = new vtol_vertex_2d(vec);

   vtol_vertex_2d_ref v1c = new vtol_vertex_2d(*v1);
  ...
  vsol_spatial_object_3d_ref so = v1->clone();
  
  vtol_vertex_2d_ref v2 = so->cast_to_topology_object()->cast_to_vertex()->
    cast_to_vertex_2d();
  ...
  vsol_point_2d_ref np = new vsol_point_2d(4.0,5.0);
  v2->set_point(*np);
  ...
  vtol_edge_ref new_edge = v1->new_edge(*v2);
  ...
  vtol_vertex_ref v1v = v1->cast_to_vertex();
  vtol_vertex_ref v2v = v2->cast_to_vertex();
  ...
  vtol_vertex_ref v1v_copy = new vtol_vertex_2d(0.0,0.0);
  (*v1v_copy) = (*v1v);
