...
//---------------------------------------------------------------------------
vtol_zero_chain_ref vtol_edge::zero_chain(void) const
{
...
{
  vertex_list *newl=new vcl_vector<vtol_vertex_ref>();
  if(_v1)
    newl->push_back(_v1);
  if(_v2)
    newl->push_back(_v2);
...
bool vtol_edge::share_vertex_with(vtol_edge &other)
{
  ...
  vcl_vector<vtol_vertex_ref>::iterator i1;
  vcl_vector<vtol_vertex_ref>::iterator i2;
...
vtol_vertex_ref vtol_edge::other_endpoint(const vtol_vertex &overt) const
{
  if (overt==*_v1)
    return _v2;
  else if (overt==*_v2)
