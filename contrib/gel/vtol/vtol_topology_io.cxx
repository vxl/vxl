			     ...
			     vcl_ostream &strm=vcl_cout)
{
  ...
  vcl_list<vtol_topology_object_ref> vertices;
  vcl_list<vtol_topology_object_ref> zero_chains;
  vcl_list<vtol_topology_object_ref> edges;
  vcl_list<vtol_topology_object_ref> one_chains; 
  vcl_list<vtol_topology_object_ref> faces;
  vcl_list<vtol_topology_object_ref> two_chains;
  vcl_list<vtol_topology_object_ref> blocks;


  vcl_list<vtol_topology_object_ref>::iterator ti;
					...
					vcl_ostream &strm)
{
  ...
  const vcl_vector<vtol_topology_object_ref> *inferiors = zc->inferiors();

  vcl_vector<vtol_topology_object_ref>::const_iterator i;
  for(i=inferiors->begin();i!=inferiors->end();++i) {
    strm << " <vxl_vertex_2d_ref id=\"v2d" << (*i)->get_id() << "\"/>" << vcl_endl;
  }
...
void vtol_topology_io::write_edge_2d(vtol_edge_2d *e, vcl_ostream &strm=vcl_cout)
{
  ...
  const vcl_vector<vtol_topology_object_ref> *inferiors = e->inferiors();
  
  vcl_vector<vtol_topology_object_ref>::const_iterator i;
  for(i=inferiors->begin();i!=inferiors->end();++i) {

    strm << " <vxl_zero_chain_2d_ref id=\"zc_" << (*i)->get_id() << "\"/>" << vcl_endl;
  }
...
void vtol_topology_io::write_one_chain(vtol_one_chain *oc, vcl_ostream &strm=vcl_cout)
{
  ...
  const vcl_vector<vtol_topology_object_ref> *inferiors = oc->inferiors();
  ...
  for(i=inferiors->begin();i!=inferiors->end();++i) {
	...
	if(oc->direction(*e) ==1){
	  strm << " <vxl_edge_2d_ref id=\"e2d" << (*i)->get_id() << "\"/>" << vcl_endl;
	}
	else{
	  // we have to signal a flip
	  
	  strm << " <vxl_edge_2d_ref id=\"e2d" << (*i)->get_id() << "\"/><vxl_flip/>"  << vcl_endl;
...
void vtol_topology_io::write_face_2d(vtol_face_2d *f, vcl_ostream &strm=vcl_cout)
{
  ...
  const vcl_vector<vtol_topology_object_ref> *inferiors = f->inferiors();
  
  vcl_vector<vtol_topology_object_ref>::const_iterator i;
  for(i=inferiors->begin();i!=inferiors->end();++i) {
    strm << " <vxl_one_chain_2d_ref id=\"oc_" << (*i)->get_id() << "\"/>" << vcl_endl;
...
void vtol_topology_io::write_two_chain(vtol_two_chain *tc, vcl_ostream &strm=vcl_cout)
{
  ...
  const vcl_vector<vtol_topology_object_ref> *inferiors = tc->inferiors();
  ...
  for(i=inferiors->begin();i!=inferiors->end();++i) {
	...
	// deterimine the direction of the edge
	if(tc->direction(*f) ==1){
	  strm << " <vxl_face_2d_ref id=\"f2d" << (*i)->get_id() << "\"/>" << vcl_endl;
	}
	else{
	  // we have to signal a flip
	  
	  strm << " <vxl_face_2d_ref id=\"f2d" << (*i)->get_id() << "\"/><vxl_flip/>"  << vcl_endl;
...
void vtol_topology_io::write_block(vtol_block *b, vcl_ostream &strm=vcl_cout)
{
  ...
  const vcl_vector<vtol_topology_object_ref> *inferiors = b->inferiors();
  
  vcl_vector<vtol_topology_object_ref>::const_iterator i;
  for(i=inferiors->begin();i!=inferiors->end();++i) {
    strm << " <vxl_two_chain_2d_ref id=\"tc_" << (*i)->get_id() << "\"/>" << vcl_endl;
  }
