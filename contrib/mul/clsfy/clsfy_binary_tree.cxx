...
unsigned clsfy_binary_tree::classify(const vnl_vector<double> &input) const
{
    ...
    if(!pNode)
    {
        vcl_cerr<<"WARNING - empty tree in clsfy_binary_tree::classify"<<vcl_endl;
        vcl_cerr<<"Return default classification zero"<<vcl_endl;
        return 0;
    }
        ...
        case (1):
        {
            ...
            for(unsigned i=0;i<N;++i)
            {
                ...
                if(link.me!= -1)
                {
                    ...
                    {
                        vcl_cerr<<"ERROR - Inconsistent parent in tree set up in clsfy_binary_tree::b_read"<<vcl_endl;
                        assert(0);
                    }
                    if((link.left_child != -1) &&
                       (!left_child || left_child->nodeId_ != link.left_child))
                                        {
                        vcl_cerr<<"ERROR - Inconsistent left child in tree set up in clsfy_binary_tree::b_read"<<vcl_endl;
                        assert(0);
                    }
                    if((link.right_child != -1) &&
                       (!right_child || right_child->nodeId_ != link.right_child))
                                        {
                        vcl_cerr<<"ERROR - Inconsistent right child in tree set up in clsfy_binary_tree::b_read"<<vcl_endl;
                        assert(0);
                    }
