        ...
        
{
    ...
    //Check that this actually managed to produce a split (in case we have homogenous data)
    ...
    clsfy_binary_tree_bnode* pNode) const
{
    ...
    if(nbranch_params_>0) //Random forest style random subset selection
    {
        ...
        if(ndimsUsed<ndims)
        {
            //Note always do a full random permutation as teh ones beyond ndimsUsed
    ...
    //May need a second pass because there may be a subset of homogenous parameters
    ...
    for(unsigned ipass=0;ipass<npasses;++ipass)
    {
        ...
        //Now reapply to all relevant data to constuct the subset split
        vcl_set<unsigned >::const_iterator indIter=subIndices.begin();
        vcl_set<unsigned >::const_iterator indIterEnd=subIndices.end();
