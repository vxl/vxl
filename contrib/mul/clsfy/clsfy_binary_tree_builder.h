...
public:
  ...
  virtual double build(clsfy_classifier_base& classifier, mbl_data_wrapper<vnl_vector<double> >& inputs,
                       unsigned nClasses, const vcl_vector<unsigned> & outputs) const;
...
private:
  void build_children(const vcl_vector<vnl_vector<double> >& vin, const vcl_vector<unsigned>& outputs,
  ...
  void build_a_node(const vcl_vector<vnl_vector<double> >& vin, const vcl_vector<unsigned>& outputs,
                    const vcl_set<unsigned>& subIndices, clsfy_binary_tree_bnode* pNode) const;

  bool isNodePure(const vcl_set<unsigned>& subIndices, const vcl_vector<unsigned>& outputs) const;

  void add_terminator(const vcl_vector<vnl_vector<double> >& vin, const vcl_vector<unsigned>& outputs,
                      clsfy_binary_tree_bnode* parent, bool to_left, bool pure) const;
