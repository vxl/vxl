...
public:
  ...
  friend void merge_sub_forests(const vcl_vector<vcl_string>& filenames, clsfy_random_forest& large_forest);
  ...
  friend clsfy_random_forest operator+(const clsfy_random_forest& forest1, const clsfy_random_forest& forest2);

  // : Provides a probability-like value that the input being in each class.
  // output(i) i<nClasses, contains the probability that the input is in class i
  // Do Out of bag estimates, i.e. only use trees whose vector of indices does not include this_indexs
  virtual void class_probabilities_oob(vcl_vector<double> & outputs, const vnl_vector<double> & input,
                                       const vcl_vector<vcl_vector<unsigned> >& oobIndices, unsigned this_index) const;

  // : Return the classification of the given probe vector using out of bag trees only.
  // See also class_probabilities_oob
  virtual unsigned classify_oob(const vnl_vector<double> & input, const vcl_vector<vcl_vector<unsigned> >& oobIndices,
