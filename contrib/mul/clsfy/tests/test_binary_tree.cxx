...
//            label=1;
    ...
    vcl_cout<<"False Positive Rate "<<double (fpr)/dtn<<vcl_endl;

    vcl_cout<<"True Negative Rate "<<double (tnr)/dtn<<vcl_endl;
    vcl_cout<<"False Negative Rate "<<double (fnr)/dtp<<vcl_endl;
  ...
  // simple test for binary threshold
    ...
    
    {
        ...
        // simple test for binary threshold
        double _tpr=double (tpr)/dtp;
        double _fpr=double (fpr)/dtn;
        TEST("tpr>0.7", _tpr>0.9, true);
        TEST("fpr<0.3", _fpr<0.1, true);
        TEST("Same TPR as pre-IO ",vcl_fabs(testTPR- _tpr)<1.0E-6,true);
        TEST("Same FPR as pre-IO ",vcl_fabs(testFPR- _fpr)<1.0E-6,true);
  ...
  // Train again with +ve and -ve data swapped round + see if get same error
...

    {
        ...
        double _tpr=double (tpr)/dtp;
        double _fpr=double (fpr)/dtn;
        double _tnr=double (tnr)/dtn;
        double _fnr=double (fnr)/dtp;
        
        TEST("Same TPR as initial TNR ",vcl_fabs(testTNR- _tpr)<1.0E-6,true);
        TEST("Same FPR as initial FNR",vcl_fabs(testFNR- _fpr)<1.0E-6,true);
        TEST("Same TNR as initial TPR ",vcl_fabs(testTPR- _tnr)<1.0E-6,true);
        TEST("Same FNR as initial FPR",vcl_fabs(testFPR- _fnr)<1.0E-6,true);
