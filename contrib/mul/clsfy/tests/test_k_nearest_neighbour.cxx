...
void test_k_nearest_neighbour()
{
  ...
  vcl_cout << "\n\n\n";
  vcl_cout << "**************************************************************" << vcl_endl;
  vcl_cout << " Testing clsfy_k_nearest_neighbour and clsfy.rbf_parzen_window" << vcl_endl;
  vcl_cout << "**************************************************************" << vcl_endl;
...
//  vcl_cout << clsfy_test_error(knn, test_set_inputs, testLabels) << vcl_endl;
  ...
  TEST("Test error on clsfy_k_nearest_neighbour close to 0.0", clsfy_test_error(knn, 
    test_set_inputs, testLabels) < 0.02, true);
//  vcl_cout << clsfy_test_error(win, test_set_inputs, testLabels) << vcl_endl;

  
  TEST("test error on clsfy_rbf_parzen_window close to 0.0", clsfy_test_error(win, 
    test_set_inputs, testLabels) < 0.02, true);
...
//  vcl_cout << clsfy_test_error(rc, test_set_inputs, testLabels) << vcl_endl;
  TEST("test error on clsfy_random_classifier close to 0.5", vnl_math_abs(clsfy_test_error(rc, 
    test_set_inputs, testLabels) - 0.5) < 0.05, true);
  ...
  vcl_cout<<"Loaded KNN: " << knn_in << vcl_endl;
  vcl_cout<<"Loaded KNN: " << p_base_class_knn_in << vcl_endl;

  vcl_cout<<"Saved Win: " << win << vcl_endl;
  vcl_cout<<"Loaded Win: " << win_in << vcl_endl;
  vcl_cout<<"Loaded Win: " << p_base_class_win_in << vcl_endl;
