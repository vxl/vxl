...
public:
  ...
  void nearest_clusters(const T& t, double& max_d, const vcl_vector<unsigned>& c_list,
  ...
  //  Returns number of such clusters. If >0, then nearest_c
  //  gives index of cluster with centre nearest to t
  unsigned clusters_within_d(const T& t, double d, const vcl_vector<unsigned>& in_list, vcl_vector<unsigned>& c_list,
                             unsigned& nearest_c, double& min_d);

  // : Finds list of clusters whose keypoint is within max_r of t
  //  Only considers subset of clusters listed in index
  //  Returns number of such clusters. If >0, then nearest_c
  //  gives index of cluster with centre nearest to t
  unsigned clusters_within_max_r(const T& t, const vcl_vector<unsigned>& in_list, vcl_vector<unsigned>& c_list,
                                 unsigned& nearest_c, double& min_d);
