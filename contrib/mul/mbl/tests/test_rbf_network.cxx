...
{
  unsigned k=0;
  vcl_cout<<"We pick the "<<i<<" sample from training."<<vcl_endl;
  vcl_cout<<"The input data "<<data[i]<<vcl_endl;
  rbf.calcWts(w,data[i]);//"It fails here"
  vcl_cout<<w[i]<<vcl_endl;
  vcl_cout<<"Sum of weights = "<<w.sum()<<vcl_endl;
  TEST_NEAR("Sum of weights near unity",w.sum(),1.0,1e-4);
  TEST_NEAR("Target weight near unity",w[i],1.0,1e-4);
  TEST_NEAR("Next weight near zero",w[i+1],0.0,1e-4);
