...
private:
  ...
  void find_overlaps(mfpf_point_finder& pf, const vcl_vector<mfpf_pose>& poses, const mfpf_pose& pose,
  ...
  bool find_near_pose(mfpf_point_finder& pf, vcl_vector<mfpf_pose>& poses, vcl_vector<double>& fits,
                      const mfpf_pose& pose, double fit);

public:
  mfpf_searcher();

  void find_refined_matches(mfpf_point_finder& pf, const vimt_image_2d_of<float>& image, const vgl_point_2d<double>& p,
                            const vgl_vector_2d<double>& u, vcl_vector<mfpf_pose>& pts, vcl_vector<double>& fit);
