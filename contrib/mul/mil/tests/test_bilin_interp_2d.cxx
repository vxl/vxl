...
void test_bilin_interp_2d_byte()
{
  ...
  for (int y=0;y<image0.ny();++y)
...

   {
     ...
     TEST("mil_bilin_interp_2d at grid point",fabs(v1-33)<1e-8,true);
     ...
     TEST("mil_bilin_interp_2d at off-grid point",fabs(v2-33.4)<1e-8,true);
     double v3 = mil_bilin_interp_2d(3.4,3.5,image0.plane(0),image0.ystep());
     TEST("mil_bilin_interp_2d at off-grid point",fabs(v3-38.4)<1e-8,true);
   }

   {
     ...
     TEST("mil_safe_bilin_interp_2d at grid point",fabs(v1-33)<1e-8,true);
     v1 = mil_safe_bilin_interp_2d(3,3,data,nx,ny,xstep,ystep);
     TEST("mil_safe_bilin_interp_2d at grid point",fabs(v1-33)<1e-8,true);

     double v2 = mil_safe_bilin_interp_2d(3.4,3,data,nx,ny,ystep);
     TEST("mil_safe_bilin_interp_2d at off-grid point",fabs(v2-33.4)<1e-8,true);
     double v3 = mil_safe_bilin_interp_2d(3.4,3.5,data,nx,ny,ystep);
     TEST("mil_safe_bilin_interp_2d at off-grid point",fabs(v3-38.4)<1e-8,true);

	 double v_outside = mil_safe_bilin_interp_2d(-1,-1,data,nx,ny,ystep);
     TEST("mil_safe_bilin_interp_2d outside image",fabs(v_outside)<1e-8,true);
   }
