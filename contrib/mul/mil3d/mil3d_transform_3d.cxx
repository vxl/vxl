...
{
//grv	if ((M.rows()!=4) || (M.columns()!=4)) M.resize(4,4);
    M.resize(4,4);
...
void mil3d_transform_3d::angles(double& phi_x, double& phi_y, double& phi_z) const
{
    ...
    if (vcl_fabs(sin(phi_y) + xz_) > 1e-6)
	if (phi_y > 0)
	    phi_y=phi_y - vnl_math::pi;
	else
	    phi_y=phi_y + vnl_math::pi;

    const double cos_y = cos(phi_y);

    if (vcl_fabs(sin(phi_x)*cos_y + yz_) > 1e-6 ||
        vcl_fabs(cos(phi_x)*cos_y - zz_) > 1e-6)
	if (phi_x > 0)
	    phi_x=phi_x - vnl_math::pi;
	else
	    phi_x=phi_x + vnl_math::pi;

    if (vcl_fabs(cos(phi_z)*cos_y - xx_) > 1e-6 ||
        vcl_fabs(sin(phi_z)*cos_y + xy_) > 1e-6)
	if (phi_z > 0)
...
void mil3d_transform_3d::calcInverse()  const
{
    ...
    switch (form_)
    {
        ...
        case Affine : 
		{
            ...
            if (det==0)
            {
                  vcl_cerr<<"mill_transform_3d() : No inverse exists for this affine transform (det==0)"<<vcl_endl;
                  vcl_abort();
            }
...
void mil3d_transform_3d::print_summary(vcl_ostream& o) const
{
	...
	switch (form_) 
	{
		...
		case ZoomOnly:
		{
			...
			params(p);
			o << "ZoomOnly" << vcl_endl;
			o << vsl_indent()<< "scale factor = (" << p(0) << "," << 
						p(1) << "," << p(2) << ")" << vcl_endl;
			o << vsl_indent()<< "translation = (" << p(3) << "," << 
						p(4) << "," << p(5) << ")" << vcl_endl;
			break;
		}
