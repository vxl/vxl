    ...
    while( labelIter != labelIterEnd )
      ...
      while( neighIter != neighIterEnd )
        ...
        if( !foundThisNeighbour )
          ...
          if( verbose_ )
            {
            vcl_cout << "Found no arc linking labels for node " << inode << " label " << label << " to node "
                     << neighIter->first << "along arc ID " << arcId << vcl_endl;
            }
          break;
          }
        ++neighIter;
      ...
      if( !found )
        ...
        if( verbose_ )
          {
          vcl_cout << "Have removed label " << *labelIter << " for node " << inode << " as it has no linking arcs"
                   << vcl_endl;
          }
