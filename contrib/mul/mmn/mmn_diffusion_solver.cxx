...
//IEEE Trans on Pattern Recog & Machin Inte;, July 2007
...
//: Default constructor
mmn_diffusion_solver::mmn_diffusion_solver():nnodes_(0),epsilon_(1.0E-5),min_iterations_(200),
...
void mmn_diffusion_solver::set_arcs(unsigned num_nodes,const vcl_vector<mmn_arc>& arcs)
{
    ...
    {
        vcl_cerr<<"Arcs appear to be inconsistent with number of nodes in mmn_diffusion_solver::set_arcs"<<vcl_endl;
                                  ...
                                  vcl_vector<unsigned>& x)
{
    ...
    for(unsigned inode=0; inode<neighbourhoods.size();++inode)
    {
        ...
        {
            vnl_vector<double>& pencilPhi = phi_[inode][neighIter->first];
        ...
        if(count_ % gACS_CHECK_PERIOD==0)
        {
            ...
            if(ok)
            {
                ...
                if(verbose_)
                {
                    ...
                    vcl_cout<<"\tSolution value= "<<soln_val<<"\tprev soln val= "<<soln_val_prev_<<vcl_endl;
                }
                if(fabs(soln_val-soln_val_prev_)<epsilon_)
                {
                    ++nConverging_;
