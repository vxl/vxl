...
using namespace test_diffusion_bits;

void test_diffusion_solver_a()
{
  vcl_cout << "==== test test_diffusion_solver (chain) =====" << vcl_endl;
  ...
  mmn_diffusion_solver solver;
  solver.set_arcs(n, arc);
  solver.set_verbose(true);
  vcl_cout << "Run solver." << vcl_endl;

  vcl_vector<unsigned>   x;
  vcl_pair<bool, double> status = solver(node_cost, pair_cost, x);
  double                 min_cost = status.second;
  bool                   arcConsistent = status.first;
  vcl_cout << "DIFFUSION Solver Iteration Count Is: " << solver.count() << vcl_endl;
  double cost = 0.0;
  for( unsigned i = 0; i < node_cost.size(); i++ )
    {
    cost += node_cost[i][i];
  ...
  TEST("Arc Consistent Solution", arcConsistent, true);
}

void test_diffusion_solver_b()
{
  vcl_cout << "==== test test_diffusion_solver (reversed chain) =====" << vcl_endl;
...
void test_diffusion_solver_loop_a(unsigned n)
{
  vcl_cout << "==== test test_diffusion_solver (loop) =====\n"
           << n << " nodes." << vcl_endl;
...
void test_diffusion_solver_loop_b(unsigned n)
{
  vcl_cout << "==== test test_diffusion_solver (loop) =====" << vcl_endl;
...
void test_diff_best_xy_line()
{
  ...
  vcl_cout << "==== test test_diffusion_solver best y line =====" << vcl_endl;
...
//                                                    100.0*istage + error[1]);
  ...
  vcl_vector<unsigned>   x;
  vcl_pair<bool, double> status = solver(node_cost, pair_costs_neg, x);
  ...
  for( unsigned i = 0; i < n; ++i )
    ...
    vcl_cout << "x[" << i << "]=" << x[i] << '\t' << locations[i][x[i]] << "\t prior prob: "
             << vcl_exp(-node_cost[i][x[i]]) << vcl_endl;
    }
}
...
void test_diff_5x5grid_easy()
{
  ...
  vcl_cout << "==== test test_diffusion_solver 5x5 grid =====" << vcl_endl;
  ...
  vcl_vector<unsigned>   x;
  vcl_pair<bool, double> status = solver(node_costs, pair_costs_neg, x);
  double                 min_cost = status.second;
  bool                   arcConsistent = status.first;

  vcl_cout << "DIFFUSION Solver Iteration Count Is: " << solver.count() << " solution value is " << min_cost
           << vcl_endl;
  TEST("Correct number of nodes", x.size(), n);
  TEST("Arc Consistent Solution", arcConsistent, true);
  ...
  for( unsigned iy = 0; iy < NSTAGES; ++iy )
    ...
    for( unsigned ix = 0; ix < NSTAGES; ++ix )
      ...
      vcl_cout << "x[" << nodeId << "]=" << x[nodeId] << '\t' << locations[nodeId][x[nodeId]] << "\t prior prob: "
               << vcl_exp(-node_costs[nodeId][x[nodeId]]) << vcl_endl;
...
void test_diff_5x5grid_hard()
{
      ...
      for( unsigned jLabel = 0; jLabel < locations.size(); ++jLabel )
        ...
        if( false )        // if (iLabel==jLabel)
          ...
          if( node1 % NSTAGES == node2 % NSTAGES )          // x coord same so must be a y arc
            ...
            {
            vcl_cout << "WARNING - inconsistent node numbering on arc " << i << "\tlinking\t" << node1 << '\t'
                     << node2 << vcl_endl;
            assert(0);
            }
          delta[0] = d.x() - dgx; delta[1] = d.y() - dgy;
  ...
  for( unsigned iy = 0; iy < NSTAGES; ++iy )
    ...
    for( unsigned ix = 0; ix < NSTAGES; ++ix )
      ...
      vcl_cout << "x[" << nodeId << "]=" << x[nodeId] << '\t' << locations[x[nodeId]] << "\t prior prob: " << vcl_exp(
        -node_costs[nodeId][x[nodeId]]) << vcl_endl;
...
void test_diffusion_solver()
{
  test_diffusion_solver_a();
  test_diffusion_solver_b();
  test_diffusion_solver_loop_a(3);
  test_diffusion_solver_loop_a(4);
  test_diffusion_solver_loop_a(5);
  test_diffusion_solver_loop_b(4);
  test_diff_best_xy_line();
  test_diff_5x5grid_easy();
  test_diff_5x5grid_hard();
}

TESTMAIN(test_diffusion_solver);
