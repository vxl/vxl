...
public:
  ...
  msdi_array_of_marked_images(const vcl_vector<vil_image_view<vxl_byte> >& images, const vcl_vector<msm_points>& points,
                              bool grey_only = true);

  // : Initialise with external vectors of images and points
  //  Pointers retained to both - they must stay in scope.
  void set(const vcl_vector<vil_image_view<vxl_byte> >& images, const vcl_vector<msm_points>& points,
           bool grey_only = true);
