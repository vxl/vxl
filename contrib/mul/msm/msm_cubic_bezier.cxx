                                   ...
                                   vcl_vector<vgl_point_2d<double> >& new_pts) const
{
  ...
  for( unsigned i = 1; i < n_pts - 1; ++i )
    ...
    double t2 = t[j + 1];  if( t2 == 0.0 )
      {
      t2 = 1.0;                              // start of next segment
      }
                                        ...
                                        vcl_vector<unsigned>& control_pt_index) const
{
  ...
  if( closed_ )
    {
    n_new_pts -= 1;          // Avoid double counting first point
  ...
  for( unsigned i = 0; i < n; ++i )
    ...
    if( n_per_seg[i] == 1 )
      {
      continue;                     // No new points in this segment
