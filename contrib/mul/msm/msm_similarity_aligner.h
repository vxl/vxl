#ifndef msm_similarity_aligner_h_
#define msm_similarity_aligner_h_
// :
// \file
// \author Tim Cootes
...
// : Calculate and apply 2D similarity transformations
//  Similarity transformation encoded as (a-1,b,tx,ty).
//  Transformation is then x'=ax-by+tx, y' = bx+ay+ty.
class msm_similarity_aligner : public msm_aligner
{
public:

  virtual ~msm_similarity_aligner() {}
  ...
  virtual void calc_transform_wt_mat(const msm_points& points1, const msm_points& points2,
                                     const vcl_vector<msm_wt_mat_2d>& wt_mat, vnl_vector<double>& trans) const;
  ...
  virtual void align_set(const vcl_vector<msm_points>& points, msm_points& ref_mean_shape,
                         vcl_vector<vnl_vector<double> >& pose_to_ref, vnl_vector<double>& average_pose) const;
...
#endif // msm_similarity_aligner_h_
