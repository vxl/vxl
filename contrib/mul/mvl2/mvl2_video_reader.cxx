...
if (!(*config_file)) 
  ...
  if ((val=vcl_getenv("VIDL2RC"))==0)
    {
    vcl_cerr << "VIDL2RC environment variable not defined." << vcl_endl;
    vcl_cerr << "Cannot find configuration file for video input." << vcl_endl;
...
{
vcl_cout << vcl_endl;
vcl_cout << "Video configurations :" << vcl_endl;
vcl_cout << "======================" << vcl_endl;
for (int i=0;i<config_names_.size();i++)
  {
  vcl_cout << "Configuration " << i<< vcl_endl;
  vcl_cout << "----------------" << vcl_endl;
  vcl_cout << "name = " <<   config_names_[i] << vcl_endl;
  vcl_cout << "size = " <<   config_sizes_[i].first << "x" 
      << config_sizes_[i].second << vcl_endl;
  vcl_cout << "options = " <<   config_strings_[i] << vcl_endl;
  vcl_cout << "filename = " <<   config_filenames_[i] << vcl_endl;
  vcl_cout << vcl_endl;
  }
}
...
{
int position;
position=vcl_find(config_names_.begin(), config_names_.end(), configname)-
    config_names_.begin();
