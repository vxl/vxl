...
void plot_estimate(const std::string& true_pdf_file, const std::string& new_pdf_file,
                   const pdf1d_pdf& pdf,
                   ...
                   int n_samples, double min_x, double max_x, int nx)
{
  ...
  std::cout<<"Range of values: ["<<d.min_value()<<','<<d.max_value()<<']'<<std::endl;
  ...
  if (pdf.write_plot_file(true_pdf_file,min_x,max_x,nx))
    std::cout<<"Wrote original distribution points to "<<true_pdf_file<<std::endl;
  else
    std::cout<<"Failed to write distribution points to "<<true_pdf_file<<std::endl;

  if (new_pdf->write_plot_file(new_pdf_file,min_x,max_x,nx))
    std::cout<<"Wrote distribution points to "<<new_pdf_file<<std::endl;
  else
    std::cout<<"Failed to write distribution points to "<<new_pdf_file<<std::endl;
