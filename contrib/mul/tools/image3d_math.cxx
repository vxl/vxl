...
bool try_string_to_image(const vcl_string& s, vimt3d_image_3d_of<T>& d)
{
  try
    ...
    if( !d.image() )
      {
      return false;               // LEGACY_ERROR_REPORTING
      }
    }
...
public:
  ...
  bool is_image_3d_of_float() const
  {
    ...
    return (operand_type_ == e_string && global_option_load_as_image == image3d_float_t &&
            try_string_to_image(string_, dummy) )
  ...
  bool is_image_3d_of_double() const
  {
    ...
    return (operand_type_ == e_string && global_option_load_as_image == image3d_double_t &&
            try_string_to_image(string_, dummy) )
  ...
  bool is_image_3d_of_int() const
  {
    ...
    return (operand_type_ == e_string && global_option_load_as_image == image3d_int_t &&
            try_string_to_image(string_, dummy) )
...
void print_quantiles__image_3d_of_int__double(opstack_t& s)
{
  ...
  vcl_cout << "     0%: " << vcl_setw(20) << *vcl_min_element(storage.begin(), storage.begin() + vnl_math::rnd(step) )
           << '\n' << vcl_setw(6) << 100.0 / nsteps << "%: " << vcl_setw(20)
           << *(storage.begin() + vnl_math::rnd(step) )
           << '\n';
  for( unsigned i = 1; i + 1 < nsteps; ++i )
    {
    vcl_nth_element(storage.begin() + vnl_math::rnd(i * step),
...
public:
  ...
  operations()
  {
    ...
    add_operation("--box", &box__image_3d_of_float__double__double__double__double__double__double,
                  ...
                  "image x_min y_min z_min x_max y_max z_max", "image",
    "Fill image with a partial-vol image of a box (in mm)");
    add_operation("--box", &box__image_3d_of_int__double__double__double__double__double__double,
                  function_type_t() << operand::e_image_3d_of_int << operand::e_double << operand::e_double
                                    << operand::e_double << operand::e_double << operand::e_double << operand::e_double,
                  "image x_min y_min z_min x_max y_max z_max", "image",
    "Fill image with a binary image of a box (in mm)");
    add_operation("--checkerboard", &checkerboard__image_3d_of_float__double__double__double,
                  ...
                  "image xsize ysize zsize", "image",
    "Set all the pixels in an image to B&W pattern of given spacing (in mm)");
    ...
    add_operation("--copy-coordinate-frame", &copy_coordinate_frame__image_3d_of_int__image_3d_of_int,
                  function_type_t() << operand::e_image_3d_of_int << operand::e_image_3d_of_int,
                  "image_v image_c", "image",
    "create image that is voxels of image_v and the world to image transform of image_c");
    ...
    add_operation("--local-z-normalise", &local_z_normalise__image_3d_of_float__double,
                  function_type_t() << operand::e_image_3d_of_float << operand::e_double,
                  "image half_radius", "image",
    "Normalise each voxel by mean and stddev measured over half_radius (mm) window");
    ...
    add_operation("--print-overlap", &print_overlap__image_3d_of_int__image_3d_of_int,
                  function_type_t() << operand::e_image_3d_of_int << operand::e_image_3d_of_int,
                  "image image", "",
    "Print overlap measures and volumes of two binary mask images. Assuming the first image is the golden data, it prints the error rates.");
    add_operation("--print-overlap", &print_overlap__image_3d_of_float__image_3d_of_float,
                  function_type_t() << operand::e_image_3d_of_float << operand::e_image_3d_of_float,
                  "image image", "",
    "Print generalised overlap measures and volumes of two partial volume mask images.");
    ...
    add_operation("--shuffle", &shuffle__image_3d_of_float__double__double,
                  function_type_t() << operand::e_image_3d_of_float << operand::e_image_3d_of_float << operand::e_double,
                  "image image radius", "image",
    "Apply shuffle differences to images ; radius being the radius of the circle used as structuring element in each z slice ");
