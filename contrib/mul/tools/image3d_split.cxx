...
int main2(int argc, char* argv[])
{
  ...
  vul_arg<vcl_string> max_voxels_arg(0,
                                     "Max number of voxels in an output image. (Suffix k=1000, kb=1024, M=1e6, G=1e9 etc.)");
  vul_arg<vcl_string> replace_str("-I",
                                  "Replace this string in the filestem with the split numbering. Default puts it at end.");
  vul_arg<vcl_string> output_format_arg("-f",
                                        "Format of output images (e.g. v3i, dcm, gipl.) Guesses from filestem by default.");
...
//  vul_arg<bool> ("-t", "Try to keep all images roughly same size - Default keeps all bu last ones identically sized.");
  ...
  MBL_LOG(INFO,
          logger(), "block sizes: " << blockwidth_i << ',' << blockwidth_i
