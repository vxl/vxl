...
static unsigned try_3d_image(const char * filename, float unit_scaling)
{
  try
  {
    ...
    vcl_cout << "size: " << ir->ni() << "x" << ir->nj() << "x" << ir->nk() << " voxels x " << ir->nplanes()
      << "planes\n";
    vcl_cout << "size: " << ir->ni()*voxel.x() << "x" << ir->nj()*voxel.y() << "x" << ir->nk()*voxel.z() << " in units of " <<
      1.0/unit_scaling << "m\n";
    vcl_cout << "voxel size: " << voxel.x() << "x" << voxel.y() << "x" << voxel.z() << " in units of " <<
      1.0/unit_scaling << "m\n";
    vcl_cout << "world_origin: " << w2i.origin().x() << "x" << w2i.origin().y() << "x" << w2i.origin().z() << " voxels\n";
    vcl_cout << "voxel_type: " << ir->pixel_format() << "\n";
    return 0;
  }
