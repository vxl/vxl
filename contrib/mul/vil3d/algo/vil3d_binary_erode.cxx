                        ...
                        const vil3d_structuring_element& element)
{
  ...
  if (ihi<ni-1) vil3d_crop(dest_image,ihi+1,ni-ihi-1,0,nj,0,nk).fill(false);

  // Zero low and high j sides
  if (jlo>0)    vil3d_crop(dest_image,0,ni,0,jlo,0,nk).fill(false);
  if (jhi<nj-1) vil3d_crop(dest_image,0,ni,jhi+1,nj-jhi-1,0,nk).fill(false);

  // Zero low and high k sides
  if (klo>0)    vil3d_crop(dest_image,0,ni,0,nj,0,klo).fill(false);
  if (khi<nk-1) vil3d_crop(dest_image,0,ni,0,nj,khi+1,nk-khi-1).fill(false);
