...
#define create_im_as_type_macro(type) \
  ...
  switch( pixel_format() )
    ...
    case VIL_PIXEL_FORMAT_UINT_16:
      {
      create_im_as_type_macro(vxl_uint_16);
#if VXL_LITTLE_ENDIAN
      swap16_for_big_endian( (char *)(im.origin_ptr() ), ni * nj * nk);
#endif // VXL_LITTLE_ENDIAN
    ...
    case VIL_PIXEL_FORMAT_UINT_32:
      {
      create_im_as_type_macro(vxl_uint_32);
#if VXL_LITTLE_ENDIAN
    ...
    case VIL_PIXEL_FORMAT_INT_32:
      {
      create_im_as_type_macro(vxl_int_32);
#if VXL_LITTLE_ENDIAN
      swap32_for_big_endian( (char *)(im.origin_ptr() ), ni * nj * nk);
#endif // VXL_LITTLE_ENDIAN
    ...
    case VIL_PIXEL_FORMAT_DOUBLE:
      {
      create_im_as_type_macro(double);
#if VXL_LITTLE_ENDIAN
  ...
  if( view.pixel_format() == VIL_PIXEL_FORMAT_BOOL )
    ...
    if( VXL_BIG_ENDIAN )
      ...
      for( unsigned k = 0; k < view.nk(); ++k )
        ...
        for( unsigned j = 0; j < view.nj(); ++j )
          {
          vcl_memcpy(&tempbuf[0],
                     uint_16_im->origin_ptr() + (k * view.nj() * view.ni() ) + (j * view.ni() ), byte_out_width);
    ...
    if( VXL_BIG_ENDIAN )
      ...
      for( unsigned k = 0; k < view.nk(); ++k )
        ...
        for( unsigned j = 0; j < view.nj(); ++j )
          {
          vcl_memcpy(&tempbuf[0],
                     int_16_im->origin_ptr() + (k * view.nj() * view.ni() ) + (j * view.ni() ), byte_out_width);
    ...
    if( VXL_BIG_ENDIAN )
      ...
      for( unsigned k = 0; k < view.nk(); ++k )
        ...
        for( unsigned j = 0; j < view.nj(); ++j )
          {
          vcl_memcpy(&tempbuf[0],
                     uint_32_im->origin_ptr() + (k * view.nj() * view.ni() ) + (j * view.ni() ), byte_out_width);
    ...
    if( VXL_BIG_ENDIAN )
      ...
      for( unsigned k = 0; k < view.nk(); ++k )
        ...
        for( unsigned j = 0; j < view.nj(); ++j )
          {
          vcl_memcpy(&tempbuf[0],
                     int_32_im->origin_ptr() + (k * view.nj() * view.ni() ) + (j * view.ni() ), byte_out_width);
    ...
    if( VXL_BIG_ENDIAN )
      ...
      for( unsigned k = 0; k < view.nk(); ++k )
        ...
        for( unsigned j = 0; j < view.nj(); ++j )
          {
          vcl_memcpy(&tempbuf[0],
                     float_im->origin_ptr() + (k * view.nj() * view.ni() ) + (j * view.ni() ), byte_out_width);
    ...
    if( VXL_BIG_ENDIAN )
      ...
      for( unsigned k = 0; k < view.nk(); ++k )
        ...
        for( unsigned j = 0; j < view.nj(); ++j )
          {
          vcl_memcpy(&tempbuf[0],
                     double_im->origin_ptr() + (k * view.nj() * view.ni() ) + (j * view.ni() ), byte_out_width);
