                              ...
                              const double dz)
{
  ...
  for (unsigned p=0; p<np; ++p, s_plane+=s_pstep, d_plane+=d_pstep)
  {
    ...
    for (unsigned k=0; k<dnk-dz-1; ++k, d_slice+=d_kstep)
    {
      ...
      for (unsigned j=0; j<dnj-dy-1; ++j, d_row+=d_jstep)
      {
        ...
        for (unsigned i=0; i<dni-dx-1; ++i, d_pix+=d_istep)
          *d_pix = (T) vil3d_trilin_interp_raw(i/dx, j/dy, k/dz, s_plane,
                                               s_istep, s_jstep, s_kstep);

        // Process the pixels near the upper i boundary - no interpolation
        for (unsigned i=dni-unsigned(dx)-1; i<dni; ++i, d_pix+=d_istep)
          *d_pix = src_image(unsigned(i/dx), unsigned(j/dy), unsigned(k/dz));
      }

      // Process the pixels near the upper j boundary - no interpolation
      for (unsigned j=dnj-unsigned(dy)-1; j<dnj; ++j, d_row+=d_jstep)
      {
        T* d_pix = d_row;
    ...
    for (unsigned k=dnk-unsigned(dz)-1; k<dnk; ++k, d_slice+=d_kstep)
    {
      T* d_row = d_slice;
