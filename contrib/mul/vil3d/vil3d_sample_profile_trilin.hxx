// This is mul/vil3d/vil3d_sample_profile_trilin.hxx
#ifndef vil3d_sample_profile_trilin_hxx_
#define vil3d_sample_profile_trilin_hxx_
//:
// \file
  ...
  unsigned n)
{
  ...
  const std::ptrdiff_t kstep = image.kstep();
  ...
  unsigned n)
{
  ...
  else
  {
    ...
    {
      for (unsigned h=0;h<n;++h,x+=dx,y+=dy,z+=dz)
      v[h] = vil3d_trilin_interp_safe_edgena(x,y,z,plane0,ni,nj,nk,istep,jstep,kstep);
    }
    else
    {
      for (unsigned h=0;h<n;++h,x+=dx,y+=dy,z+=dz)
      {
        for (unsigned p=0;p<np;++p,++v)
          *v = vil3d_trilin_interp_safe_edgena(x,y,z,plane0+p*pstep,ni,nj,nk,istep,jstep,kstep);
      }
    }
  }
}
...
template void vil3d_sample_profile_trilin_extend(vecType* v, \
  ...
  double dx, double dy, double dz,\
  unsigned n); \
template void vil3d_sample_profile_trilin_edgena(vecType* v, \
  const vil3d_image_view<imType >& image, \
  double x0, double y0, double z0,\
  double dx, double dy, double dz,\
  unsigned n)
#endif // vil3d_sample_profile_trilin_hxx_
