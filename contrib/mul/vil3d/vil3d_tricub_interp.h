...
template <class T>
double vil3d_tricub_interp_raw(double x, double y, double z, const T* data, vcl_ptrdiff_t xstep, vcl_ptrdiff_t ystep,
                               vcl_ptrdiff_t zstep);
                                              ...
                                              vcl_ptrdiff_t xstep, vcl_ptrdiff_t ystep, vcl_ptrdiff_t zstep)
{
  ...
  else if( x > nx - 3.0000001 && x < nx - 1.9999999 )
    {
    x = nx - 2.0;
    }
  else if( x > nx - 2.0000001 )
    {
    x = nx - 1.0;
    }

  if( y < 0.9999999 ) {y = 0.0; }
  else if( y > ny - 3.0000001 && y < ny - 1.9999999 )
    {
    y = ny - 2.0;
    }
  else if( y > ny - 2.0000001 )
    {
    y = ny - 1.0;
    }

  if( z < 0.9999999 ) {z = 0.0; }
  else if( z > nz - 3.0000001 && z < nz - 1.9999999 )
    {
    z = nz - 2.0;
    }
  else if( z > nz - 2.0000001 )
    {
    z = nz - 1.0;
    }
...
//  The safe interpolatable region is [1,nx-3]*[1,ny-3]*[1,nz-3].
template <class T>
double vil3d_tricub_interp_safe_trilinear_extend(double x, double y, double z, const T* data, int nx, int ny, int nz,
                                                 vcl_ptrdiff_t xstep, vcl_ptrdiff_t ystep, vcl_ptrdiff_t zstep);
