// This is mul/vimt/vimt_sample_grid_bilin.hxx
#ifndef vimt_sample_grid_bilin_hxx_
#define vimt_sample_grid_bilin_hxx_
//:
// \file
...
inline bool vimt_grid_bilin_corner_in_image(const vgl_point_2d<double>& p,
                            ...
                            int n1, int n2)
{
  ...
  bool all_in_image =
      vimt_grid_bilin_corner_in_image(im_p0,image.image()) &&
      vimt_grid_bilin_corner_in_image(im_p1,image.image()) &&
      vimt_grid_bilin_corner_in_image(im_p2,image.image()) &&
      vimt_grid_bilin_corner_in_image(w2i(p0+(n1-1)*u+(n2-1)*v),image.image());
                                   ...
                                   int n1, int n2)
{
  ...
  if (image.world2im().form()!=vimt_transform_2d::Projective)
  {
    ...
    vil_sample_grid_bilin_edgena(vec_data,image.image(),im_p0.x(),im_p0.y(),
                                 im_u.x(),im_u.y(),im_v.x(),im_v.y(),n1,n2);
    return;
  }
  ...
  else
  {
    ...
    if (np==1)
    {
      for (int i=0;i<n1;++i,p1+=u)
      {
        ...
        {
          vgl_point_2d<double> im_p = w2i(p);
          *vec_data = (vecType)vil_bilin_interp_safe_edgena(im_p.x(),im_p.y(),plane0,ni,nj,istep,jstep);
        }
      }
    }
    else
    {
      for (int i=0;i<n1;++i,p1+=u)
      {
        ...
        for (int j=0;j<n2;++j,p+=v)
        {
          vgl_point_2d<double> im_p = w2i(p);
          for (int k=0;k<np;++k,++vec_data)
            *vec_data = (vecType)vil_bilin_interp_safe_edgena(im_p.x(),im_p.y(),plane0+k*pstep,ni,nj,istep,jstep);
        }
      }
    }
  }
...
template void vimt_sample_grid_bilin(vnl_vector<vecType >&, \
                                     ...
                                     const vgl_vector_2d<double >&, \
                                     int, int); \
template void vimt_sample_grid_bilin_edgena(vnl_vector<vecType >&, \
                                            const vimt_image_2d_of<imType >&, \
                                            const vgl_point_2d<double >&, \
                                            const vgl_vector_2d<double >&, \
                                            const vgl_vector_2d<double >&, \
...
#endif // vimt_sample_grid_bilin_hxx_
