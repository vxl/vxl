...
vil_image_resource_sptr vimt_vil_v2i_format::make_input_image(vil_stream* vs)
{
  ...
  if( v_i != 1 )
    {
    return 0;             // can only handle version 1.
    }
  vsl_b_read(vslstream, v_i);  // vil_image_view.ni
  if( v_i == 0 )
    {
    return 0;             // can't handle empty images.
    }
  ...
  if( v_i != 2 )
    {
    return 0;             // can only handle version 2.
    }
  vsl_b_read(vslstream, v_b);  // vil_smart_ptr.firsttime
  if( !v_b )
    {
    return 0;         // can't handle multiple images.
    }
  ...
  switch( version )
    ...
    case 1: {
      ...
      if( !vslstream )
        {
        delete p_im;
        vil_exception_warning(vil_exception_corrupt_image_file("Constructor", "v2i", "",
