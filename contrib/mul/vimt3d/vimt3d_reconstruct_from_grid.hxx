// This is mul/vimt3d/vimt3d_reconstruct_from_grid.hxx
#ifndef vimt3d_reconstruct_from_grid_hxx_
#define vimt3d_reconstruct_from_grid_hxx_
//:
// \file
...
template<class T>
inline bool vimt3d_grid_point_in_image(const vgl_point_3d<double>& p, const vil3d_image_view<T>& image)
{
                                    ...
                                    const vil3d_image_view<T>& image)
{
  ...
  if (!vimt3d_grid_point_in_image(im_p,image)) return false;
  if (!vimt3d_grid_point_in_image(im_p+u1,image)) return false;
  if (!vimt3d_grid_point_in_image(im_p+v1,image)) return false;
  if (!vimt3d_grid_point_in_image(im_p+w1,image)) return false;
  if (!vimt3d_grid_point_in_image(im_p+u1+v1,image)) return false;
  if (!vimt3d_grid_point_in_image(im_p+u1+w1,image)) return false;
  if (!vimt3d_grid_point_in_image(im_p+v1+w1,image)) return false;
  if (!vimt3d_grid_point_in_image(im_p+u1+v1+w1,image)) return false;
...
#endif // vimt3d_reconstruct_from_grid_hxx_
