...
void vimt3d_vil3d_v3i_image::load_full_image() const
{
  ...
  if( magic != vimt3d_vil3d_v3i_format::magic_number() )
    ...
    vil_exception_warning(vil_exception_corrupt_image_file(
                            "vimt3d_vil3d_v3i_image::load_full_image", "vimt3d_vil3d_v3i_image", "",
                            "Incorrect V3I magic number detected") );
    return;
    }
  short version;
  ...
  file_(file.release() ), im_(0), dirty_(false)
{
  ...
  switch( version )
    ...
    case 1:
        ...
        unsigned int i = 0;
        while( i < instance.object().size() && !(instance.object()[i]->is_a() == name) )
          {
          ++i;
          }
        ...
        unsigned sizeof_T = vil_pixel_format_sizeof_components(header_.pixel_format);
          ...
          switch( vimt3d_image_3d_of_version )
            ...
            default:
              ...
              is.is().clear(vcl_ios::badbit); // Set an unrecoverable IO error on stream
              vil_exception_warning(vil_exception_invalid_version("vimt3d_vil3d_v3i_image constructor",
                                                                  "vimt3d_vil3d_v3i_image", "", oss.str() ) );
              return;
            }
