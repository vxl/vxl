...
void vimt3d_vil3d_v3m_image::load_full_image() const
{
  ...
  if( magic != vimt3d_vil3d_v3m_format::magic_number() )
    ...
    vil_exception_warning(vil_exception_corrupt_image_file(
                            "vimt3d_vil3d_v3m_image::load_full_image", "vimt3d_vil3d_v3m_image", "",
                            "Incorrect v3m magic number detected") );
    return;
    }
  short version;
  ...
  switch( version )
    ...
    case 1:
      ...
      vil_memory_chunk_sptr chunk_ptr( new vil_memory_chunk(
                                         size * vil_pixel_format_sizeof_components(
                                           header_.pixel_format), header_.pixel_format) );
...
#undef macro
  ...
  if( !is )
    ...
    vil_exception_warning(vil_exception_corrupt_image_file(
                            "vimt3d_vil3d_v3m_image::load_full_image", "vimt3d_vil3d_v3m_image", "",
                            "Failed to read all expected values.") );
    im_ = 0;
    }
}
  ...
  switch( version )
    ...
    default:
      ...
      oss << "I/O ERROR: vimt3d_vil3d_v3m_image::vimt3d_vil3d_v3m_image()\n"
          << "           Unknown version number " << version << '\n';
      vil_exception_warning(vil_exception_invalid_version("vimt3d_vil3d_v3m_image constructor",
                                                          "vimt3d_vil3d_v3m_image", "", oss.str() ) );
      return;
    }
  if( !is )
    {
                                                                 ...
                                                                 unsigned k0, unsigned nk) const
{
  ...
  if( !im_ )
    {
    return 0;         // If load full image failed then im_ will remain null
                                      ...
                                      unsigned i0, unsigned j0, unsigned k0)
{
  ...
  if( !im_ )
    {
    return false;         // If load full image failed then im_ will remain null
