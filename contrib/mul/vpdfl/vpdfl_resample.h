...
void vpdfl_resample(vcl_vector<vnl_vector<double> >& x, const vcl_vector<vnl_vector<double> >& d, int ns = 0);

// : Fill x with ns samples drawn at random from d[0..n-1]
//  If ns not specified (or zero) then draw d.size() samples from d
void vpdfl_resample(vcl_vector<vnl_vector<double> >& x, const vnl_vector<double>* d, int n, int ns = 0);
