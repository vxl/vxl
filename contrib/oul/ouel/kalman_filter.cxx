	...
	P(Pi), K(ns, nm)
{
	...
	{
		cerr << "Error in Kalman constructor:\n";
		cerr << "\tMatrix A must be of size ns*ns\n";
		exit(-1);
	}
	...
	{
		cerr << "Error in Kalman constructor:\n";
		cerr << "\tMatrix H must be of size nm*ns\n";
		exit(-1);
	}
	...
	{
		cerr << "Error in Kalman constructor:\n";
		cerr << "\tMatrix B must be of size ns*nc\n";
		exit(-1);
	}
	...
	{
		cerr << "Error in Kalman constructor:\n";
		cerr << "\tMatrix x must be of size ns*1\n";
		exit(-1);
	}
	...
	{
		cerr << "Error in Kalman constructor:\n";
		cerr << "\tMatrix p_initial must be of size ns*1\n";
		exit(-1);
	}
}
									  ...
									  const vnl_matrix<double> &Rk)
{
	...
	if ((zk.rows()!=num_measurement_dimensions)||
		(zk.cols()!=1))
	{
		cerr << "Error in Kalman measurement_update\n";
		cerr << "\tzk must have dimensions nm*1\n";
		exit(-1);
	}
	...
	{
		cerr << "Error in Kalman measurement_update\n";
		cerr << "\tRk must have dimensions nm*nm\n";
		exit(-1);
	}
...
KalmanFilter::predict(const vnl_matrix<double> &Qk)
{
	...
	{
		cerr << "Error in Kalman predict\n";
		cerr << "\tQk must have dimensions ns*ns\n";
		exit(-1);
	}
					  ...
					  const vnl_matrix<double> &uk)
{
	...
	{
		cerr << "Error in Kalman predict\n";
		cerr << "\tuk must have dimensions nc*1\n";
		exit(-1);
	}
...
ostream &operator<<(ostream &os, const KalmanFilter &kf)
{
	os << "Current state of Kalman Filter is: " << endl;
	os << "estimate = " << kf.x.transpose() ;
	os << "prediction = " << kf.x_pred.transpose() ;
	os << "measurement = " << kf.z.transpose() ;
	os << "error covariance = " << endl << kf.P ;
	os << "Kalman gain = " << endl << kf.K ;
	os << "A = " << endl << kf.A ;
	os << "H = " << endl << kf.H ;
