...
int main(int argc, char *argv[])
{
	...
	while ((opt=getopt(argc, argv, "x:y:n:c:d"))!=EOF)
    {
        ...
        case 'd':
            ...
            break;
        default:
            vcl_cout << "Usage: example1 [-x width] [-y height] "
                     << "[-n num_to_grab] [-c device_name] [-d]" << vcl_endl;
            vcl_cout << "\t-d turn debug info on" << vcl_endl;
            break;
        }
    }
    ...
    vcl_cout << "which means "<< 1000.0*(double)num_to_grab/(double)time_taken 
             << "frames per second\n";
