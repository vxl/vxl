//-*-c++-*--------------------------------------------------------------
//: \file frame_grabber_v4l.h: 
// 
// \brief Classes for video input under linux making use of the video
// for linux stuff. This has been tested using a webcam running under
// the Phillips Web Cam driver.
//
...
class FrameGrabberV4lGrey : public FrameGrabber
{
 public:
	...
	FrameGrabberV4lGrey(int width_=384, int height_=288, bool _debug=false,
	...
	inline vil1_memory_image *get_current_frame(){
        if (debug)
        {
            ...
            vcl_cout << "current = " << current;
            vcl_cout << " im = " << im[current] << vcl_endl;
        }
		return(im[current]);
	};
