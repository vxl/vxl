                                    ...
                                    vgl_homg_point_2d<double>& perfect_point2) const
{
  ...
  coeffs[1] = b * b * b * b + b * b * c * c - a * a * d * d + 2.0 * b * b * d * d * g * g + d * d * d * d * g * g * g
    * g;
  coeffs[2] = 4.0 * a * b * b * b + a * b * c * c - a * a * c * d + 2.0 * b * b * c * d * f * f - 2.0 * a * b * d * d
    * f * f
    + 4.0 * b * b * c * d * g * g + 4.0 * a * b * d * d * g * g + 4.0 * c * d * d * d * g * g * g * g;
  coeffs[3] = 6.0 * a * a * b * b + 2.0 * b * b * c * c * f * f - 2.0 * a * a * d * d * f * f + 2.0 * b * b * c * c
    * g * g
    + 8 * a * b * c * d * g * g + 2.0 * a * a * d * d * g * g + 6.0 * c * c * d * d * g * g * g * g;
  coeffs[4] = 4.0 * a * a * a * b + 2.0 * a * b * c * c * f * f - 2.0 * a * a * c * d * f * f + b * b * c * d * f * f
    * f * f
    - a * b * d * d * f * f * f * f + 4.0 * a * b * c * c * g * g + 4.0 * a * a * c * d * g * g + 4.0 * c * c * c * d
    * g * g * g * g;
  coeffs[5] = a * a * a * a + b * b * c * c * f * f * f * f - a * a * d * d * f * f * f * f + 2.0 * a * a * c * c * g
    * g + c * c * c * c * g * g * g * g;
                                    ...
                                    HomgPoint2D * perfect_point2_ptr) const
{
  ...
  if( perfect_point1_ptr )
    ...
    *perfect_point1_ptr =
      HomgPoint2D(p1_matrix * HomgOperator2D::perp_projection(epipolar_line1, origin).get_vector() );
    *perfect_point2_ptr =
      HomgPoint2D(p2_matrix * HomgOperator2D::perp_projection(epipolar_line2, origin).get_vector() );
    }
}
