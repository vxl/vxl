...
void osl_canny_gradient(int xsize_, int ysize_, float const * const * smooth_, float * const * dx_, float * const * dy_,
                        float * const * grad_);

// : computes doubled central derivatives : df[i] = f[i+1]-f[i-1].
// the boundary pixels are left untouched.
void osl_canny_gradient_central(int xsize_, int ysize_, float const * const * smooth_, float * const * dx_,
                                float * const * dy_, float * const * grad_);
