    ...
    for( unsigned j = rows[i]; j < rows[i + 1]; ++j ) // for each edgel in this row
      ...
      if( rows[i + 1] == rows[ysize_] )
        ...
        if( i > 0 )
          ...
          while( k < rows[i] && col[k] < col[j] + 1 )
            {
            ++k;
            }

          if( k < rows[i] && col[k] == col[j] + 1 )
            {
            continue;                                      // found n-e link
            }
          }
      ...
      while( k < rows[i + 2] && col[k] + 1 < col[j] )
        {
        ++k;                                          // skip the early ones
      ...
      if( k == rows[i + 2] )
        {
        continue;                   // no vertical or diagonal neighbour
      ...
      k = rows[i + 1];
      while( k < rows[i + 2] && col[k] + 2 < col[j] )
        {
        ++k;
        }
      ...
      while( k < rows[i + 2] && col[k] < col[j] + 2 )
        {
        ++k;
        }
      ...
      while( k < rows[i + 3] && col[k] + 1 < col[j] )
        {
        ++k;
        }
...
int osl_canny_ox::Join_dotsOX(int x, int y, int dx, int dy, int& xNew, int& yNew)
{
  ...
  if( !dx && (vcl_abs(dy) == 1) )
    ...
    else if( thin_[x + 1][y + 2 * dy] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x + 1, y + 2 * dy);
      }
    else if( thin_[x - 1][y + 2 * dy] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x - 1, y + 2 * dy);
      }
    else if( thin_[x + 2][y + 2 * dy] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x + 2, y + 2 * dy);
      }
    else if( thin_[x - 2][y + 2 * dy] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x - 2, y + 2 * dy);
      }
    else if( thin_[x + 2][y +  dy] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x + 2, y + dy  );
      }
    else if( thin_[x - 2][y +  dy] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x - 2, y + dy  );
      }
    else if( thin_[x + 2][y] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x + 2, y     );
      }
    else if( thin_[x - 2][y] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x - 2, y     );
    ...
    else if( thin_[x + 2 * dx][y + 1] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x + 2 * dx, y + 1);
      }
    else if( thin_[x + 2 * dx][y - 1] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x + 2 * dx, y - 1);
      }
    else if( thin_[x + 2 * dx][y + 2] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x + 2 * dx, y + 2);
      }
    else if( thin_[x + 2 * dx][y - 2] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x + 2 * dx, y - 2);
      }
    else if( thin_[x +  dx][y + 2] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x +  dx, y + 2);
      }
    else if( thin_[x +  dx][y - 2] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x +  dx, y - 2);
      }
    else if( thin_[x][y + 2] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x, y + 2);
      }
    else if( thin_[x][y - 2] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x, y - 2);
    ...
    else if( thin_[x + 2 * dx][y +  dy] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x + 2 * dx, y +  dy);
      }
    else if( thin_[x +  dx][y + 2 * dy] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x +  dx, y + 2 * dy);
      }
    else if( thin_[x + 2 * dx][y] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x + 2 * dx, y     );
      }
    else if( thin_[x][y + 2 * dy] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x, y + 2 * dy);
      }
    else if( thin_[x + 2 * dx][y -  dy] >= edge_min_OX_ )
      {
      Set_intsOX(xNew, yNew, x + 2 * dx, y -  dy);
