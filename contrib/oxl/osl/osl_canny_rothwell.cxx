...
void osl_canny_rothwell::detect_edges(vil1_image const & image, vcl_list<osl_edge *> * edges, bool adaptive)
{
  ...
  if( adaptive )
    ...
    if( verbose )
      {
      vcl_cerr << xdang_->size() << " dangling edges found initially\n"
    ...
    while( sigma_ > min_sigma )
      ...
      if( verbose )
        {
        vcl_cerr << xdang_->size() << " dangling edges found after scale reduction\n"
                 << "looking for single pixel breaks - ";
        }
      Jump_single_breaks();
      Thin_edges();
