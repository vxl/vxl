...
void osl_ortho_regress::add_points(double const * x, double const * y, unsigned n)
{
...
void osl_ortho_regress::add_points(float const * x, float const * y, unsigned n)
{
  for( unsigned i = 0; i < n; ++i )
    {
    add_point(x[i], y[i]);
...
// eigenvalue.
//
double osl_ortho_regress::cost(double a, double b, double c) const
{
  return (Sxx * a * a + 2 * Sxy * a * b + Syy * b * b + 2 * Sx * a * c + 2 * Sy * b * c + S1 * c * c) / (a * a + b * b);
}

bool osl_ortho_regress::fit(double & a, double & b, double & c) const
{
  assert(S1 >= 2);
  double xbar = Sx / S1;
  double ybar = Sy / S1;
...
// eigenvalue.
//
bool osl_ortho_regress::fit_constrained(double x, double y,
                                        double & a, double & b, double & c) const
{
  assert(S1 >= 1);

  if( !symmetric_2x2_eigen_system(Sxx - 2 * Sx * x + S1 * x * x, Sxy - Sx * y - x * Sy + S1 * x * y,
                                  Syy - 2 * Sy * y + S1 * y * y,
                                  &a, &b, 0,
                                  0, 0, 0) )
    {
