...
public:
  ...
  static void draw_edges(vcl_list<osl_edge *> detected_edges, unsigned col, unsigned row);

  // : Draw straight lines onto the given location.
  static void draw_straight_lines(vcl_vector<float>, vcl_vector<float>, vcl_vector<float>, vcl_vector<float>, unsigned,
                                  unsigned);
  ...
  static bool get_detect_lines_ox_params(float* high, int* edge_min, int* min_length, int* min_fit_length);
