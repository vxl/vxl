...
template <class T>
void vdtop_compute_8_upper_masks(vil_image_view<T> & img, const T& max_value,
                                 vil_image_view<vdtop_8_neighborhood_mask> & masks)
{
  unsigned ni = img.ni(), nj = img.nj(), np = img.nplanes(), nil = ni - 1, njl = nj - 1;
...
template <class T>
void vdtop_remove_non_maximal_direction(const vil_image_view<T> & img,
                                        vil_image_view<vdtop_8_neighborhood_mask> & masks)
{
...
template <class T, class TMap>
void vdtop_set_structure_from_masks(TMap & res, const vil_image_view<vdtop_8_neighborhood_mask> & mask, int plane,
                                    vmap_2_map_tag)
{
