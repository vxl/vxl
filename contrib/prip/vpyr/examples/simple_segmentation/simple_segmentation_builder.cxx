...
void simple_segmentation_builder::initialise(const my_image_type & arg)
{
   ...
   //cout<<"Computing its dual."<<vcl_endl ;
   //_grid.set_dual() ;
#if 0
  ...
  _background=&_top_map.face(_top_map.nb_faces()-1) ;
   ...
   //_chrono.stop() ;
   vcl_cout<<"Initialization done in "<<vcl_endl ;//_chrono.read()<<" seconds."<<vcl_endl
           <<"--------------------------------------------"<<vcl_endl ;
}
...
void simple_segmentation_builder::filter_edge_below(int threshold)
{
   ...
   for (my_top_map::edge_iterator e=_top_map.begin_edge(); e!=_top_map.end_edge(); ++e)
   {
     ...
     {
       rk.add(*e) ; //vcl_cout<<e->value()<<' ' << vcl_flush;
     }
   }
   rk.finalise() ;
   if (!rk.valid())
   ...
   //if (!rk.empty())
   {
     ...
     for (my_top_map::vertex_iterator v=_top_map.begin_vertex(); v!=_top_map.end_vertex(); ++v, ++vp)
     {
       ...
       {
         vcl_cout<<_pyramid.base_map().vertex_position(vp->sequence_index())<<"ii"
    ...
    //vcl_cout<<_top_map.nb_vertices()<<'/'<<_pyramid.top_level().nb_vertices()<<' '
    //        <<_top_map.nb_darts()<<'/'<<_pyramid.top_level().nb_darts()<<vcl_endl ;
#if 0
     ...
     for (my_top_map::vertex_iterator v=_top_map.begin_vertex(); v!=_top_map.end_vertex(); ++v, ++vp)
     {
       if (_pyramid.base_map().vertex_position(vp->sequence_index())!=v->value())
       {
         vcl_cout<<_pyramid.base_map().vertex_position(vp->sequence_index())<<"::"<<v->value()
