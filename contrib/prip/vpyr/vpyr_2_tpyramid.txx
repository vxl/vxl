...
#  endif // 0
      last_vertex = this->base_map().begin_vertex_sequence()
        + (pyr.level(i).end_base_vertex() - pyr.base_map().begin_vertex_sequence() );
      level(i).set_vertex_sequence(first_vertex, last_vertex);
#  if 0
      firstEdge = &this->base_map().edge( (*pyr.level_[i].beginbase_edge() )->index() );
#  endif // 0
      lastEdge = this->base_map().begin_edge_sequence()
        + (pyr.level(i).endbase_edge() - pyr.base_map().begin_edge_sequence() );
      level(i).set_edge_sequence(firstEdge, lastEdge);
#  if 0
      firstFace = &this->base_map().face( (*pyr.level_[i].beginbase_type() )->index() );
#  endif // 0
      lastFace = this->base_map().begin_face_sequence()
        + (pyr.level(i).endbase_type() - pyr.base_map().begin_face_sequence() );
...
template <class TLevel>
void vpyr_2_tpyramid<TLevel>::down_projection_vertices(vmap_level_index arg_level,
                                                       vcl_vector<vmap_vertex_index> & res) const
{
