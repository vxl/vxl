...
void vpyr_top_tmap< TPyramid , V , E , F , D >::contraction(const contraction_kernel & arg) 
{
  ...
  for (int i=0; i<nb_vertices(); ++i)
  {
    ...
    {
      vcl_cout<<_pyramid.base_map().vertex_position(i)<<"ii"<<vertex(i).value()<<' ' << vcl_flush;
    }
  }
  for (int i=0; i<arg.size(); ++i)
  {
    ...
    {
      vcl_cout<<"ICI2"<<_pyramid.base_map().vertex(ck.dart(i)->vertex().sequence_index()).last_level()<<'/'
