...
// for the 3d neurons
double global_scales[3] = { 0.375, 0.375, 0.5 };
...
void save_trans_and_region( unsigned count,
                            rgrl_trans_affine* final_aff,
                            rgrl_mask_box const& r)
{
  {
    vcl_ostringstream s;
    s << "out2-dump-"<<count;
               ...
               rgrl_set_of<rgrl_match_set_sptr> const& matches )
{
  ...
  save_trans_and_region( count, final_aff, view->region() );
...
//    rgrl_estimator_sptr est_p = new rgrl_est_affine;
    ...
    // for 3d vessels 1->2
//     rgrl_mask_box start( vnl_double_3( 150, 100, 60 ),
//                     vnl_double_3( 200, 160, 85 ) );
//     rgrl_mask_box start( vnl_double_3( 170, 100, 60 ),
//                     vnl_double_3( 230, 160, 95 ) );
//     rgrl_mask_box start = from_roi;
//     rgrl_mask_box start( vnl_double_3( 95, 60, 80 ),
//                     vnl_double_3( 105, 70, 130 ) );
    rgrl_mask_box start( vnl_double_3( 0, 0, 0 ),
                    vnl_double_3( 200, 180, 160 ) );
    ...
    save_trans_and_region( 0, rgrl_cast<rgrl_trans_affine*>(init_trans), start );
  }
  ...
  if( reg.final_matches().size() > 0 ) {
    ...
    for( from_iter fitr = ms.from_begin(); fitr != ms.from_end(); ++fitr ) {
      for( to_iter titr = fitr.begin(); titr != fitr.end(); ++titr ) {
        ...
        d[0].push_back( sqr(titr.to_feature()->location()[0] - m->location()[0]) );
        d[1].push_back( sqr(titr.to_feature()->location()[1] - m->location()[1]) );
        d[2].push_back( sqr(titr.to_feature()->location()[2] - m->location()[2]) );
      }
      fout << "\n";
    }
    double e[3];
