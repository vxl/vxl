...
// : For multi-stage
void
rgrl_data_manager::add_data( unsigned stage,
...
// : For single-stage
void
rgrl_data_manager::add_data( rgrl_feature_set_sptr                    from_set,
                             rgrl_feature_set_sptr                    to_set,
                             rgrl_matcher_sptr                        matcher,
                             rgrl_weighter_sptr                       weighter,
                             rgrl_scale_estimator_unwgted_sptr        unwgted_scale_est,
...
void
rgrl_data_manager::add_estimator( unsigned                           stage,
...
// : For single-stage
void
rgrl_data_manager::add_estimator( rgrl_estimator_sptr                estimator)
{
  assert( !multi_stage_ );
...
void
rgrl_data_manager::set_dimension_increase_for_next_stage( unsigned stage, double rate)
{
  data_.set_dimension_increase_for_next_stage(stage, rate);
}

double
rgrl_data_manager::dimension_increase_for_next_stage(unsigned stage) const
{
...
bool
rgrl_data_manager::is_multi_feature() const
{
...
void
rgrl_data_manager::get_data_at_stage( unsigned stage,
...
void
rgrl_data_manager::get_data( vcl_vector<rgrl_feature_set_sptr>             & from_sets,
                             vcl_vector<rgrl_feature_set_sptr>             & to_sets,
                             vcl_vector<rgrl_matcher_sptr>                 & matchers,
                             vcl_vector<rgrl_weighter_sptr>                & weighters,
                             vcl_vector<rgrl_scale_estimator_unwgted_sptr> & unwgted_scale_ests,
...
void
rgrl_data_manager::get_data( rgrl_feature_set_sptr             & from_set,
                             rgrl_feature_set_sptr             & to_set,
                             rgrl_matcher_sptr                 & matcher,
                             rgrl_weighter_sptr                & weighter,
                             rgrl_scale_estimator_unwgted_sptr & unwgted_scale_est,
...
bool
rgrl_data_manager::has_stage(unsigned stage ) const
{
  return data_.has( stage );
}

void
rgrl_data_manager::generate_defaults(  rgrl_matcher_sptr                  & matcher,
                                       rgrl_weighter_sptr                 & weighter,
                                       rgrl_scale_estimator_unwgted_sptr  & unwgted_scale_est )
{
...
// : For multi-stage, multi-type
void
rgrl_data_manager::get_label( unsigned stage,
...
// : For single-stage, multi-type
void
rgrl_data_manager::get_label( vcl_vector<vcl_string>& labels) const
{
  assert( !multi_stage_ );
