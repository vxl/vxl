...
void
rgrl_debug_feature_iteration_print::execute(const rgrl_object* caller, const rgrl_event & event )
{
...
void
rgrl_debug_feature_iteration_save_matches::execute(const rgrl_object* caller, const rgrl_event & event )
{
  ...
  vcl_string xform_name = path_ + '/' + file_prefix_ + '_' + stage_buffer + '_' + iter_buffer
    + ".xform";
  const rgrl_transformation_sptr& trans_sptr = reg_engine->current_transformation();
  if( !trans_sptr )
    {
