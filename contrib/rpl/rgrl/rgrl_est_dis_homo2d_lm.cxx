...
void
rgrl_rad_dis_homo2d_func::f(vnl_vector<double> const& x, vnl_vector<double>& fx)
{
...
void
rgrl_rad_dis_homo2d_func::gradf(vnl_vector<double> const& x, vnl_matrix<double>& jacobian)
{
...
rgrl_est_dis_homo2d_lm::rgrl_est_dis_homo2d_lm( vnl_vector<double> const& from_centre,
...
rgrl_est_dis_homo2d_lm::estimate( rgrl_set_of<rgrl_match_set_sptr> const& matches,
                                  rgrl_transformation const& cur_transform ) const
{
  ...
  return new rgrl_trans_rad_dis_homo2d( init_H.as_ref(), k1_from, k1_to, covar,
                                        from_centre_.as_ref(), to_centre_.as_ref() );
}

rgrl_transformation_sptr
rgrl_est_dis_homo2d_lm::estimate( rgrl_match_set_sptr matches,
                                  rgrl_transformation const& cur_transform ) const
{
...
// : change frome_centre
void
rgrl_est_dis_homo2d_lm::set_centres( vnl_vector<double> const& from_centre,
                                     vnl_vector<double> const& to_centre )
{
  assert( from_centre.size() == 2 && to_centre.size() == 2 );
...
const vcl_type_info &
rgrl_est_dis_homo2d_lm::transformation_type() const
{
