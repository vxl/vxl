...
public:
  ...
  bool projective_estimate( vnl_matrix_fixed<double, Tdim + 1, Fdim + 1>& proj, vnl_matrix<double>& full_covar,
                            vnl_vector_fixed<double, Fdim>& from_centre, vnl_vector_fixed<double, Tdim>& to_centre );
  ...
  vnl_matrix_fixed<double, Tdim + 1,
                   Fdim + 1> uncentre_proj( vnl_matrix_fixed<double, Tdim + 1, Fdim + 1> const& proj ) const;
...
protected:
  // : compute jacobian
  void reduced_proj_jacobian( vnl_matrix_fixed<double, Tdim, (Fdim + 1) * (Tdim + 1) - 1>& base_jac,
                              vnl_matrix_fixed<double, Tdim + 1, Fdim + 1> const& proj, vnl_vector_fixed<double,
                                                                                                         Fdim>           const& from )
  const;

  // : compute jacobian
  void full_proj_jacobian( vnl_matrix_fixed<double, Tdim, (Fdim + 1) * (Tdim + 1)>& base_jac, vnl_matrix_fixed<double,
                                                                                                               Tdim + 1,
                                                                                                               Fdim + 1> const& proj,
                           vnl_vector_fixed<double, Fdim>           const& from ) const;
  ...
  // : convert parameters
  void convert_parameters( vnl_vector<double>& params, vnl_matrix_fixed<double, Tdim + 1, Fdim + 1> proj_matrix,
                           vnl_vector_fixed<double, Fdim> const& fc, vnl_vector_fixed<double, Tdim> const& tc );
