...
public:
  // : ctor
  rgrl_est_proj_rad_func( rgrl_set_of<rgrl_match_set_sptr> const& matches, unsigned int camera_dof,
                          bool with_grad = true );
  ...
  bool projective_estimate( vnl_matrix_fixed<double, Tdim + 1, Fdim + 1>& proj, vcl_vector<double>& rad_dist,
                            vnl_matrix<double>& full_covar, vnl_vector_fixed<double, Fdim>& from_centre,
                            vnl_vector_fixed<double, Tdim>& to_centre, vnl_vector_fixed<double,
                                                                                        Tdim> const& camera_centre);
  ...
  void set_centres( vnl_vector_fixed<double, Fdim> const& from_centre,
  ...
  // : map a location
  inline
  void map_loc( vnl_vector_fixed<double, Tdim>& mapped,
...
protected:
  ...
  // : compute jacobian
  void reduced_proj_rad_jacobian( vnl_matrix<double>                            & base_jac, vnl_matrix_fixed<double,
                                                                                                             Tdim + 1,
                                                                                                             Fdim + 1> const& proj, vcl_vector<double>                       const& rad_k,
                                  vnl_vector_fixed<double,
                                                   Fdim>           const& from )
  const;

  // : compute the full jacobian
  void full_proj_rad_jacobian( vnl_matrix<double>                            & base_jac, vnl_matrix_fixed<double,
                                                                                                          Tdim + 1,
                                                                                                          Fdim + 1> const& proj, vcl_vector<double>                       const& rad_k,
                               vnl_vector_fixed<double, Fdim>           const& from ) const;

  // : compute jacobian w.r.t. location
  void proj_jac_wrt_loc( vnl_matrix_fixed<double, Tdim, Fdim>          & jac_loc, vnl_matrix_fixed<double, Tdim + 1,
                                                                                                   Fdim + 1> const& proj, vcl_vector<double>                       const& rad_k,
                         vnl_vector_fixed<double, Fdim>           const& from ) const;

  // : convert parameters
  void convert_parameters( vnl_vector<double>& params, vnl_matrix_fixed<double, Tdim + 1, Fdim + 1> const& proj_matrix,
                           vcl_vector<double>             const& rad_dist, vnl_vector_fixed<double, Fdim> const& fc,
                           vnl_vector_fixed<double, Tdim> const& tc,
                           vnl_vector_fixed<double, Tdim> const& camera_centre );

  void apply_radial_distortion( vnl_vector_fixed<double, Tdim>      & mapped, vnl_vector_fixed<double, Tdim> const& p,
