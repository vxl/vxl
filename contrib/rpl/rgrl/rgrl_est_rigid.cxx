...
rgrl_est_rigid::rgrl_est_rigid( unsigned int dimension )
{
...
rgrl_est_rigid::estimate( rgrl_set_of<rgrl_match_set_sptr> const& matches,
                          rgrl_transformation const& cur_trans_in ) const
{
  ...
  do
    ...
    for( unsigned int ms = 0; ms < matches.size(); ++ms )
      {
...
void rgrl_est_rigid::determine_covariance( rgrl_set_of<rgrl_match_set_sptr> const& matches,
                                           rgrl_transformation_sptr current_trans) const
{
    ...
    for( FIter fi = match_set.from_begin(); fi != match_set.from_end(); ++fi )
      ...
      for( TIter ti = fi.begin(); ti != fi.end(); ++ti )
        ...
        Hoo(0, 0) = Hoo(0, 0) + wgt * 2 * (
            ...
            + inner_product(from_pt,
                            ( dRdtheta.transpose() * B * dRdtheta + R.transpose() * B * d2Rdtheta2) * from_pt ) );

        Hoo(1, 1) = Hoo(1, 1) + wgt * 2 * (
            inner_product( (t - to_pt), B * d2Rdalpha2 * from_pt)
            + inner_product(from_pt,
                            ( dRdalpha.transpose() * B * dRdalpha + R.transpose() * B * d2Rdalpha2) * from_pt ) );
        ...
        Hoo(0, 1) = Hoo(0, 1) + wgt * 2 * (
            ...
            + inner_product(from_pt,
                            ( dRdtheta.transpose() * B * dRdalpha + R.transpose() * B * d2Rdthetadalpha) * from_pt ) );

        Hoo(0, 2) = Hoo(0, 2) + wgt * 2 * (
            inner_product( (t - to_pt), B * d2Rdphidtheta * from_pt)
            + inner_product(from_pt,
                            ( dRdtheta.transpose() * B * dRdphi + R.transpose() * B * d2Rdphidtheta) * from_pt ) );

        Hoo(1, 2) = Hoo(1, 2) + wgt * 2 * (
            inner_product( (t - to_pt), B * d2Rdphidalpha * from_pt)
            + inner_product(from_pt,
                            ( dRdalpha.transpose() * B * dRdphi + R.transpose() * B * d2Rdphidalpha) * from_pt ) );
        }
      }
    }
...
rgrl_transformation_sptr
rgrl_est_rigid::estimate( rgrl_match_set_sptr matches,
...
const vcl_type_info &
rgrl_est_rigid::transformation_type() const
{
