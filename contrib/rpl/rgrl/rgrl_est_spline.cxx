...
void
rgrl_est_spline::point_in_knots( vnl_vector<double> const& point, vnl_vector<double> & spline_pt ) const
{
...
rgrl_est_spline::estimate( rgrl_set_of<rgrl_match_set_sptr> const& matches,
...
//  vnl_vector<int> num( dim, 0 );
        ...
        if( roi_.inside( from_pt ) && fi.size() != 0 )
          ...
          for( TIter ti = fi.begin(); ti != fi.end(); ++ti )
            ...
            if( !global_xform_ )
              ...
              DebugMacro_abv(2, from_pt << " \t " << ti.to_feature()->location() << " \t " << displacement.get_row(
                               i ) << '\n' );
              }
            else
              {
  ...
  if( optimize_method_ == RGRL_LEVENBERG_MARQUARDT )
    ...
    for( unsigned i = 0; i < dim; ++i )
      {
      spline_least_squares_func f( splines[i], from_pts_in_knots, wgt, displacement.get_column(
                                     i ), free_control_pt_index );
...
rgrl_transformation_sptr
rgrl_est_spline::estimate( rgrl_match_set_sptr matches,
                           rgrl_transformation const& cur_transform ) const
{
...
const vcl_type_info &
rgrl_est_spline::transformation_type() const
{
