...
public:
  ...
  rgrl_est_spline( unsigned dof, rgrl_mask_box const& roi, vnl_vector<double> const& delta,
                   vnl_vector<unsigned> const& m, bool use_thin_plate = false, double lambda = 0.001 );

  // : Constructor
  //
  // global_xform is used as the global xformation. No global
  // estimation will be performed.
  rgrl_est_spline( unsigned dof, rgrl_transformation_sptr global_xform, rgrl_mask_box const& roi,
                   vnl_vector<double> const& delta, vnl_vector<unsigned> const& m, bool use_thin_plate = false,
                   double lambda = 0.001 );
