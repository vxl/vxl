...
public:
  ...
  virtual 
  vcl_string name(void) const=0;

  virtual 
  bool is_same_type(const rgrl_event* e) const=0;
  
};

// Macro for creating new Events (from ITK)
#define EventMacro( classname , super ) \
 class classname : public super { \
   public: \
     classname() {} \
     virtual ~classname() {} \
     virtual vcl_string name() const { return #classname; } \
     virtual bool is_same_type(const rgrl_event* e) const \
...
//
EventMacro( rgrl_event_start, rgrl_event );
EventMacro( rgrl_event_end,   rgrl_event );
EventMacro( rgrl_event_iteration,   rgrl_event );
