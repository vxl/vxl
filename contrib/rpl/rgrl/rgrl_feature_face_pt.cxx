...
rgrl_feature_face_pt::error_projector() const
{
  if( scale_ == 0 )
    {
...
vnl_matrix<double> const &
rgrl_feature_face_pt::error_projector_sqrt() const
{
  if( scale_ == 0 )
    {
    WarningMacro( "The scale is zero." );
...
unsigned int
rgrl_feature_face_pt::num_constraints() const
{
  return 1;
}
...
vnl_matrix<double> const &
rgrl_feature_face_pt::tangent_subspace()
{
  if( subspace_cached_ )
    {
...
rgrl_feature_sptr
rgrl_feature_face_pt::transform( rgrl_transformation const& xform ) const
{
...
double
rgrl_feature_face_pt::transform_scale( rgrl_transformation const& xform ) const
{
...
double
rgrl_feature_face_pt::absolute_signature_weight( rgrl_feature_sptr other ) const
{
  // if other is invalid
  if( !other ) {return 0.0; }
...
vnl_vector<double>
rgrl_feature_face_pt::signature_error_vector( rgrl_feature const& other ) const
{
...
unsigned
rgrl_feature_face_pt::signature_error_dimension( const vcl_type_info& other_feature_type ) const
{
...
// : write out feature
void
rgrl_feature_face_pt::write( vcl_ostream& os ) const
{
  // tag
  os << "FACE\n"
    // dimension
...
// : read in feature
bool
rgrl_feature_face_pt::read( vcl_istream& is, bool skip_tag )
{
  if( !skip_tag )
    {
...
rgrl_feature_sptr
rgrl_feature_face_pt::clone() const
{
