...
rgrl_feature_point::rgrl_feature_point( vnl_vector<double> const& loc, double scale )
  : rgrl_feature( loc, scale )
{
}

rgrl_feature_point::rgrl_feature_point( unsigned size )
  : rgrl_feature()
{
...
unsigned int
rgrl_feature_point::num_constraints() const
{
  return location_.size();
}

vnl_matrix<double> const &
rgrl_feature_point::error_projector() const
{
  if( scale_ == 0 )
    {
...
vnl_matrix<double> const &
rgrl_feature_point::error_projector_sqrt() const
{
  if( scale_ == 0 )
    {
    WarningMacro( "The scale is zero." );
...
rgrl_feature_sptr
rgrl_feature_point::transform( rgrl_transformation const& xform ) const
{
...
double
rgrl_feature_point::transform_scale( rgrl_transformation const& xform ) const
{
...
double
rgrl_feature_point::absolute_signature_weight( rgrl_feature_sptr other ) const
{
  // if other is invalid
  if( !other ) {return 0.0; }
...
// : write out feature
void
rgrl_feature_point::write( vcl_ostream& os ) const
{
  // tag
  os << "POINT" << vcl_endl;
...
// : read in feature
bool
rgrl_feature_point::read( vcl_istream& is, bool skip_tag )
{
  if( !skip_tag )
    {
    // skip empty lines
...
rgrl_feature_sptr
rgrl_feature_point::clone() const
{
