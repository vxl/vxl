...
public:
  ...
  //
  virtual
  void features_in_region( feature_vector& results, rgrl_mask_box const& roi ) const = 0;

  // :  Return the features in a given circle/sphere.
  //
  virtual
  void features_within_radius( feature_vector& results, vnl_vector<double> const& center, double radius ) const = 0;
  ...
  //
  virtual
  rgrl_feature_sptr nearest_feature( const vnl_vector<double>& loc ) const = 0;

  // : Return the nearest feature to the given location.
  virtual
  rgrl_feature_sptr nearest_feature( rgrl_feature_sptr const& feature ) const = 0;

  // :  Return all features within a given distance of the given feature, one per segment (if segmented)
  //
  virtual
  void features_within_radius( feature_vector& results, rgrl_feature_sptr const& feature, double distance ) const = 0;

  // :  Return the k nearest features to the given location, one per segment (if segmented)
  //
  virtual
  void k_nearest_features( feature_vector& results, const vnl_vector<double>& feature_loc, unsigned int k ) const = 0;

  // :  Return the k nearest features, one per segment (if segmented)
  //
  virtual
  void k_nearest_features( feature_vector& results, rgrl_feature_sptr const& feature, unsigned int k ) const = 0;

  // :  Return the bounding box encloses the feature set
  virtual
  rgrl_mask_box bounding_box() const = 0;

  // :  Return the type of feature
  //
  virtual
  const vcl_type_info & type() const = 0;
