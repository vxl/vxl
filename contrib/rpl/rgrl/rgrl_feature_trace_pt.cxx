...
rgrl_feature_trace_pt::rgrl_feature_trace_pt()
  : rgrl_feature(),
...
unsigned int
rgrl_feature_trace_pt::num_constraints() const
{
  return location_.size() - 1;
}

vnl_vector<double> const &
rgrl_feature_trace_pt::tangent() const
{
  return tangent_;
}

vnl_matrix<double> const &
rgrl_feature_trace_pt::error_projector() const
{
  return error_proj_;
}

rgrl_feature_sptr
rgrl_feature_trace_pt::transform( rgrl_transformation const& xform ) const
{
...
vnl_matrix<double> const &
rgrl_feature_trace_pt::normal_subspace()
{
  if( subspace_cached_ )
    {
...
rgrl_feature_trace_pt::boundary_points(vnl_vector<double> const& in_direction) const
{
...
double
rgrl_feature_trace_pt::absolute_signature_weight( rgrl_feature_sptr other ) const
{
  // if other is invalid
  if( !other ) {return 0.0; }
...
// : write out feature
void
rgrl_feature_trace_pt::write( vcl_ostream& os ) const
{
  // tag
  os << "TRACE" << vcl_endl;
...
// : read in feature
bool
rgrl_feature_trace_pt::read( vcl_istream& is, bool skip_tag )
{
  if( !skip_tag )
    {
...
rgrl_feature_sptr
rgrl_feature_trace_pt::clone() const
{
