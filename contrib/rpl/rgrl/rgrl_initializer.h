...
public:
  ...
  virtual bool next_initial( rgrl_view_sptr           & view, rgrl_scale_sptr          & prior_scale) = 0;

  // : Get next set of initial information without the notion of a view.
  //
  bool next_initial( rgrl_mask_sptr           & from_image_roi, rgrl_mask_sptr           & to_image_roi,
                     rgrl_mask_box            & current_region, rgrl_mask_box            & global_region,
                     rgrl_estimator_sptr      & xform_estimator, rgrl_transformation_sptr & xform_estimate,
                     unsigned                 & current_resolution, rgrl_scale_sptr          & prior_scale);

  // : Get next set of initial information, assuming \a current_region is the region of interest for registration.
  //
  bool next_initial( rgrl_mask_box            & current_region, rgrl_estimator_sptr      & xform_estimator,
                     rgrl_transformation_sptr & xform_estimate, unsigned                 & current_resolution,
  ...
  //
  bool next_initial( rgrl_mask_box            & current_region, rgrl_estimator_sptr      & xform_estimator,
                     rgrl_transformation_sptr & xform_estimate, rgrl_scale_sptr          & prior_scale);
