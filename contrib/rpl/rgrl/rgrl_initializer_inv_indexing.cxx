...
void
rgrl_initializer_inv_indexing::add_data( vcl_vector<rgrl_invariant_sptr> const& fixed_set,
...
void
rgrl_initializer_inv_indexing::add_multiple_data( vcl_vector<rgrl_invariant_sptr> const& fixed_set,
  ...
  for( unsigned int m_ind = 0; m_ind < num_moving_sets; ++m_ind )
    ...
    for( unsigned int pt = 0; pt < moving_sets[m_ind].size(); pt++ )
      ...
      for( unsigned int nn_ind = 0; nn_ind < near_neighbor_indices.size(); ++nn_ind )
        {
        matches_[m_ind].push_back( new rgrl_invariant_match(moving_sets[m_ind][pt],
                                                            fixed_set[near_neighbor_indices[nn_ind]]) );
        }
      }
...
void
rgrl_initializer_inv_indexing::set_current_moving_image( unsigned int moving_image_index)
{
...
rgrl_initializer_inv_indexing::matches_for_moving_image( unsigned int moving_image_index)
{
  return matches_[moving_image_index];
}

bool
rgrl_initializer_inv_indexing::next_initial( rgrl_view_sptr           & view,
...
bool
rgrl_initializer_inv_indexing::next_initial( rgrl_invariant_match_sptr& best_match )
{
  if( max_num_matches_tried_ > 0 &&
      num_matches_tried_ == max_num_matches_tried_ )
    {
...
int
rgrl_initializer_inv_indexing::size() const
{
