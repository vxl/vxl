...
public:
  // : Constructor
  rgrl_initializer_inv_indexing( rgrl_view_sptr prior_view, bool should_estimate_global_region = true,
                                 int max_num_matches_tried = -1);

  rgrl_initializer_inv_indexing( rgrl_mask_sptr const&     from_image_roi, rgrl_mask_sptr const&     to_image_roi,
                                 rgrl_estimator_sptr xform_estimator = 0, unsigned             initial_resolution = 0,
                                 bool should_estimate_global_region = true, int max_num_matches_tried = -1);
  ...
  void add_data( vcl_vector<rgrl_invariant_sptr> const& fixed_set, vcl_vector<rgrl_invariant_sptr> const& moving_set,
                 double nn_radius = 0, unsigned int k_nn = 1 );
  ...
  virtual void add_multiple_data( vcl_vector<rgrl_invariant_sptr> const& fixed_set,
                                  vcl_vector<vcl_vector<rgrl_invariant_sptr> > const& moving_sets, double nn_radius = 0,
                                  unsigned int k_nn = 1);
