...
public:
  ...
  rgrl_mask_oriented_box( vnl_vector<double> const& origin, vnl_matrix<double> const& axes,
                          vnl_vector<double> const& len );

  // : Initialize a box from \a x0 to \a x1 using axes specified in columns of \a axes
  rgrl_mask_oriented_box( vnl_vector<double> const& oriented_xmin, vnl_vector<double> const& oriented_xmax,
