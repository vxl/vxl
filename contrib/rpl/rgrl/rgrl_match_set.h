...
public:
  ...
  // :  construct an empty match set, specifying the feature type
  rgrl_match_set( const vcl_type_info& from_type, const vcl_type_info& to_type,
  ...
  const vcl_type_info & from_type() const { return *from_type_; }

  // : type of from features
  const vcl_type_info & to_type() const { return *to_type_; }
