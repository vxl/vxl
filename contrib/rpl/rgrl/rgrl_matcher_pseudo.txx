...
template <class PixelType>
rgrl_matcher_pseudo<PixelType>::rgrl_matcher_pseudo( vil_image_view<PixelType> from_image,
...
template <class PixelType>
rgrl_match_set_sptr
rgrl_matcher_pseudo<PixelType>::compute_matches( rgrl_feature_set const&    from_set,
    ...
    rgrl_feature_sptr mapped_feature = (*fitr)->transform( current_xform );
      ...
      if( (*fitr)->is_type( rgrl_feature_trace_region::type_id() ) )
        ...
        DebugMacro_abv(1, "\n\nfrom :\n" << (*fitr)->location() << " normal: "
                                         << rgrl_cast<rgrl_feature_trace_region *>(
                         *fitr )->normal_subspace().get_column(0)
                                         << "\nmapped :\n" << mapped_feature->location()
                                         << rgrl_cast<rgrl_feature_trace_region *>(
                         mapped_feature )->normal_subspace().get_column(0) << '\n' );
        }
...
template <class PixelType>
void
rgrl_matcher_pseudo<PixelType>::map_region_intensities( rgrl_transformation      const& trans,
...
template <class PixelType>
void
rgrl_matcher_pseudo<PixelType>::map_region_intensities( vcl_vector<vnl_vector<int> > const& pixel_locations,
                                                        ...
                                                        rgrl_mapped_pixel_vector_type   & mapped_pixels) const
{
  ...
  for( unsigned int i = 0; i < pixel_locations.size(); ++i )
    ...
    for( unsigned int j = 0; j < dim; ++j )
      {
      pixel_loc_dbl[j] = pixel_locations[i][j];
      }
...
template <class PixelType>
void
rgrl_matcher_pseudo<PixelType>::match_mapped_region( rgrl_feature_sptr                     mapped_feature,
...
template <class PixelType>
double
rgrl_matcher_pseudo<PixelType>::compute_response( vnl_double_2                  const& mapped_location, // FIXME: unused
                                                  rgrl_mapped_pixel_vector_type const& mapped_pixels,
