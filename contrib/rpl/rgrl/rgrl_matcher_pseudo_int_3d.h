...
public:
  ...
  rgrl_matcher_pseudo_int_3d( vil3d_image_view<PixelType> const& from_image,
                              vil3d_image_view<PixelType> const& to_image, vnl_vector<double> const& from_spacing_ratio,
                              vnl_vector<double> const& to_spacing_ratio, rgrl_evaluator_sptr evaluator,
                              rgrl_mask_sptr mask = 0 );

  // :  Match the features in the "from" image to the intensity in the "to" image.
  //
  rgrl_match_set_sptr compute_matches( rgrl_feature_set const&     from_features,
...
private:
  ...
  void map_region_intensities( rgrl_transformation      const& trans, rgrl_feature_sptr               feature_sptr,
                               rgrl_mapped_pixel_vector_type & mapped_pixels ) const;

  // :  The actual work of mapping the region intensities.
  void map_region_intensities( vcl_vector<vnl_vector<int> > const& pixel_locations,
                               rgrl_transformation           const& trans,
                               rgrl_feature_sptr                    feature_sptr,
                               rgrl_mapped_pixel_vector_type      & mapped_pixels) const;

  // : move window around to find the optimum response
  void slide_window(rgrl_feature_sptr                    mapped_feature,
  ...
  void sample_pixels_along_direction( vcl_vector<discrete_shift_node>& two_dir_shifts, vnl_double_3 dir,
