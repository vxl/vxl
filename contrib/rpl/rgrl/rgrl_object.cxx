...
void
rgrl_object::set_debug_flag(  unsigned int debugFlag ) const
{
  debug_flag_ = debugFlag;
}

unsigned int
rgrl_object::debug_flag() const
{
  return debug_flag_;
}

void
rgrl_object::set_warning( bool flag ) const
{
  warning_ = flag;
}

bool
rgrl_object::warning() const
{
  return warning_;
}

unsigned int
rgrl_object::add_observer( rgrl_event_sptr event, rgrl_command_sptr cmd )
{
...
rgrl_command_sptr
rgrl_object::get_command(unsigned int tag)
{
...
void
rgrl_object::invoke_event( const rgrl_event & event)
{
...
void
rgrl_object::invoke_event( const rgrl_event & event) const
{
...
void
rgrl_object::remove_observer(unsigned int tag)
{
  observer_map::iterator i =  observers_.find( tag );
...
bool
rgrl_object::has_observer( const rgrl_event & event ) const
{
  typedef observer_map::const_iterator const_obs_itr;
  const_obs_itr i = observers_.begin();
  for( ; i != observers_.end(); ++i )
