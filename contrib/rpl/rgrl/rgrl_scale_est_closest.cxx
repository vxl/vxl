...
rgrl_scale_sptr
rgrl_scale_est_closest::estimate_unweighted( rgrl_match_set const& match_set,
...
bool
rgrl_scale_est_closest::compute_geometric_scale( double& return_scale,
                                                 ...
                                                 bool penalize_scaling ) const
{
  ...
  for( from_iter fitr = match_set.from_begin(); fitr != match_set.from_end(); ++fitr )
    ...
    if( fitr.empty() )
      {
      DebugMacro_abv(1,
                     " no matched points for from: " << fitr.from_feature()->location()
                                                     << ", set its error distances = 1.0e30\n");
...
bool
rgrl_scale_est_closest::compute_signature_inv_covar( vnl_matrix<double>& inv_covar,
...
rgrl_scale_sptr
rgrl_scale_est_closest::estimate_weighted( rgrl_match_set const& match_set,
