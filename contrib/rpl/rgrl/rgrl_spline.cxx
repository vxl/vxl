...
rgrl_spline::rgrl_spline( vnl_vector<unsigned> const& m, vnl_vector<double> const& c )
  : m_( m ), c_( c )
{
  unsigned int n = 1;
...
void
rgrl_spline::set_control_points( vnl_vector<double> const& c )
{
  unsigned n = 1;
...
double
rgrl_spline::f_x( vnl_vector<double> const& point ) const
{
...
vnl_vector<double>
rgrl_spline::jacobian( vnl_vector<double> const& point ) const
{
  assert( point.size() == m_.size() );
  // check if it's in the valid region
  for( unsigned i = 0; i < m_.size(); ++i )
...
void
rgrl_spline::basis_response( vnl_vector<double> const& point, vnl_vector<double>& br ) const
{
  basis_response_helper( point, br, &bspline_basis_function );
}

void
rgrl_spline::basis_response_helper( vnl_vector<double> const& point, vnl_vector<double>& br,
                                    func_type basis_func ) const
{
...
double
rgrl_spline::element_1d_thin_plate(unsigned i, unsigned j) const
{
  // a1, b1 : {-1, 0, ..., m_[0] + 1}
  int a1 = i - 1;
  int b1 = j - 1;
...
double
rgrl_spline::element_2d_thin_plate(unsigned i, unsigned j) const
{
  // a1, b1 : {-1, 0, ..., m_[0] + 1}
  // a2, b2 : {-1, 0, ..., m_[1] + 1}
  int a1 = i % ( m_[0] + 3 ) - 1;
...
double
rgrl_spline::element_3d_thin_plate(unsigned i, unsigned j) const
{
  // a1, b1 : {-1, 0, ..., m_[0] + 1}
  // a2, b2 : {-1, 0, ..., m_[1] + 1}
  // a3, b3 : {-1, 0, ..., m_[2] + 1}
...
void
rgrl_spline::thin_plate_regularization(vnl_matrix<double>& regularization) const
{
  const unsigned dim = m_.size();
  // The volume
  double vol = 1;
...
rgrl_spline_sptr
rgrl_spline::refinement( vnl_vector<unsigned> const& m ) const
{
  const unsigned   dim = m_.size();
    ...
    for( unsigned i = 0; i < m_[0] + 2; ++i )
      ...
      for( unsigned j = 0; j < m_[1] + 2; ++j )
        ...
        for( unsigned k = 0; k < m_[2] + 2; ++k )
          ...
          if( i != m_[0] + 1 && j != m_[1] + 1 && k != m_[2] + 1 && 2 * i + 1 < m[0] + 3 && 2 * j + 1 < m[1] + 3 &&
              2 * k + 1 < m[2] + 3 )
            {
...
#if 0
bool
rgrl_spline::is_support( vnl_vector<double> const& pt, unsigned index )
{
