...
public:
  ...
  rgrl_trans_homo2d_proj_rad( vnl_matrix_fixed<double, 3, 3> const& H, vcl_vector<double>             const & k,
  ...
  //  The radial parameters are assumed to be unnormalized.
  rgrl_trans_homo2d_proj_rad( vnl_matrix_fixed<double, 3, 3> const& H, vcl_vector<double>             const & k,
                              vnl_vector_fixed<double, 2>  const & image_centre, vnl_matrix<double> const& covar,
                              vnl_vector<double> const& from_centre, vnl_vector<double> const& to_centre );

  // : Constructor based on an initial transformation and covar estimate
  //  The radial parameters are assumed to be normalized.
  rgrl_trans_homo2d_proj_rad( vcl_vector<double>             const & norm_k, vnl_matrix_fixed<double, 3, 3> const& H,
                              vnl_vector_fixed<double, 2>  const & image_centre );
