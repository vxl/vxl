...
rgrl_trans_homography2d::rgrl_trans_homography2d()
  : H_( 0.0 ),
  from_centre_( 0.0, 0.0 ),
  to_centre_( 0.0, 0.0 )
{}

rgrl_trans_homography2d::rgrl_trans_homography2d( vnl_matrix<double> const& H,
...
rgrl_trans_homography2d::H() const
{
  return uncenter_H_matrix();
}

vnl_matrix_fixed<double, 3, 3>
rgrl_trans_homography2d::uncenter_H_matrix() const
{
...
vnl_matrix<double>
rgrl_trans_homography2d::transfer_error_covar( vnl_vector<double> const& from_loc ) const
{
  assert( is_covar_set() );
...
void
rgrl_trans_homography2d::inv_map( const vnl_vector<double>& to,
...
rgrl_trans_homography2d::inverse_transform() const
{
...
rgrl_trans_homography2d::homo_jacobian( vnl_vector_fixed<double, 2> const& from_loc ) const
{
...
// : Return the jacobian of the transform.
void
rgrl_trans_homography2d::jacobian_wrt_loc( vnl_matrix<double>& jacobian, vnl_vector<double> const& from_loc ) const
{
  ...
  jacobian(0,
           0) =
    H_(0, 0) * ( H_(2, 1) * centered_from[1] + H_(2, 2) ) - H_(2, 0) * ( H_(0, 1) * centered_from[1] + H_(0, 2) );
  jacobian(1,
           0) =
    H_(1, 0) * ( H_(2, 1) * centered_from[1] + H_(2, 2) ) - H_(2, 0) * ( H_(1, 1) * centered_from[1] + H_(1, 2) );
  // w/ respect to y
  jacobian(0,
           1) =
    H_(0, 1) * ( H_(2, 0) * centered_from[0] + H_(2, 2) ) - H_(2, 1) * ( H_(0, 0) * centered_from[0] + H_(0, 2) );
  jacobian(1,
           1) =
    H_(1, 1) * ( H_(2, 0) * centered_from[0] + H_(2, 2) ) - H_(2, 1) * ( H_(1, 0) * centered_from[0] + H_(1, 2) );
...
void
rgrl_trans_homography2d::write(vcl_ostream& os ) const
{
...
// for input
bool
rgrl_trans_homography2d::read(vcl_istream& is )
{
  int dim;
...
void
rgrl_trans_homography2d::map_loc( vnl_vector<double> const& from,
...
void
rgrl_trans_homography2d::map_dir( vnl_vector<double> const& from_loc,
...
rgrl_transformation_sptr
rgrl_trans_homography2d::scale_by( double scale ) const
{
...
// : make a clone copy
rgrl_transformation_sptr
rgrl_trans_homography2d::clone() const
{
