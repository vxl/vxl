...
void rgrl_trans_rigid::set_rotation(double theta, double alpha, double phi)
{
  ...
  R_(0,0) =cos(alpha)*cos(theta) ;                              R_(0,1) = -cos(alpha)*sin(theta);                              R_(0,2)=sin(alpha);
  R_(1,0) =cos(theta)*sin(alpha)*sin(phi)+cos(phi)*sin(theta);  R_(1,1) = -sin(alpha)*sin(phi)*sin(theta)+cos(phi)*cos(theta); R_(1,2)=-cos(alpha)*sin(phi);
  R_(2,0)=-cos(phi)*cos(theta)*sin(alpha)+sin(phi)*sin(theta);  R_(2,1) = cos(phi)*sin(alpha)*sin(theta)+cos(theta)*sin(phi);  R_(2,2)=cos(alpha)*cos(phi);
}
...
void rgrl_trans_rigid::set_rotation(double theta)
{
  ...
  R_(0,0) = cos(theta);    R_(0,1) = sin(theta);
  R_(1,0) = -sin(theta);   R_(1,1) = cos(theta);
}
...
read( vcl_istream& is )
{
  ...
  if( str != "RIGID" ) {
    WarningMacro( "The tag is not RIGID. reading is aborted." << vcl_endl );
    exit(10);
  }
