...
rgrl_trans_similarity::rgrl_trans_similarity( unsigned int dimension )
...
rgrl_trans_similarity::rgrl_trans_similarity( vnl_matrix<double> const& rot_and_scale,
...
rgrl_trans_similarity::rgrl_trans_similarity( vnl_matrix<double> const& in_A,
                                              vnl_vector<double> const& in_trans,
                                              vnl_matrix<double> const& in_covar,
...
void
rgrl_trans_similarity::map_loc( vnl_vector<double> const& from,
...
void
rgrl_trans_similarity::map_dir( vnl_vector<double> const& from_loc,
...
vnl_matrix<double>
rgrl_trans_similarity::transfer_error_covar( vnl_vector<double> const& p  ) const
{
  assert( is_covar_set() );
...
rgrl_trans_similarity::A() const
{
  return A_;
}

double
rgrl_trans_similarity::scaling() const
{
  assert( A_.rows() && A_.cols() );
...
vnl_vector<double>
rgrl_trans_similarity::t() const
{
  return trans_ - A_ * from_centre_;
}

void
rgrl_trans_similarity::inv_map( const vnl_vector<double>& to,
...
rgrl_trans_similarity::inverse_transform() const
{
...
void
rgrl_trans_similarity::jacobian_wrt_loc( vnl_matrix<double>& jac, vnl_vector<double> const & /*from_loc*/ ) const
{
  jac = A_;
}

rgrl_transformation_sptr
rgrl_trans_similarity::scale_by( double scale ) const
{
...
void
rgrl_trans_similarity::write( vcl_ostream& os ) const
{
  // tag
  os << "SIMILARITY\n"
    // parameters
...
bool
rgrl_trans_similarity::read( vcl_istream& is )
{
  int dim;
...
// : make a clone copy
rgrl_transformation_sptr
rgrl_trans_similarity::clone() const
{
