...
rgrl_mask_box rgrl_util_estimate_global_region_with_inverse_xform(rgrl_mask_sptr      const&   from_image_roi,
                                                                  ...
                                                                  bool                         union_with_curr = false, double                       drastic_change_ratio =
                                                                    1.75 );
...
// : Estimate the change between the spreads of the from_feature_set and mapped_feature_set
bool rgrl_util_geometric_scaling_factors( rgrl_match_set const& current_match_set, vnl_vector<double>& factors );
...
bool rgrl_util_irls( rgrl_match_set_sptr              match_set, rgrl_scale_sptr                  scale,
                     rgrl_weighter_sptr               weighter, rgrl_convergence_tester   const& conv_tester,
                     rgrl_estimator_sptr              estimator, rgrl_transformation_sptr       & estimate,
...
//  on re-mapping scales, orientations.  However, the result may be
//  inaccurate due to out-of-date signature weights.
bool rgrl_util_irls( rgrl_set_of<rgrl_match_set_sptr> const& match_sets, rgrl_set_of<rgrl_scale_sptr>     const& scales,
                     vcl_vector<rgrl_weighter_sptr>   const& weighters,
                     rgrl_convergence_tester          const& conv_tester, rgrl_estimator_sptr              estimator,
                     rgrl_transformation_sptr&        estimate, const bool                       fast_remapping = false,
                     unsigned int                     debug_flag = 0);
