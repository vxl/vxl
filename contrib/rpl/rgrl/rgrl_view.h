...
public:
  ...
  rgrl_view( rgrl_mask_sptr      const& from_image_roi, rgrl_mask_sptr      const& to_image_roi,
             rgrl_mask_box       const& region, rgrl_mask_box       const& global_region,
             rgrl_estimator_sptr        xform_estimator, rgrl_transformation_sptr   xform_estimate,
             unsigned                   resolution = 0, rgrl_transformation_sptr   inverse_estimate = 0 );

  // default copy and assignment are good.

  // : return a self copy
  virtual
  rgrl_view_sptr self_copy() const;
  ...
  virtual
  bool features_in_region( feature_vector& features, rgrl_feature_set const& fea_set ) const;
  ...
  virtual
  bool inside_current_region( vnl_vector<double> const& loc );
