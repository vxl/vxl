...
public:
  ...
  //: Return the index of a child of an internal node.  The given cell
  //  must have children.
  ...
  //: Return a link into the leaf data array for the given leaf.  The
  //  given cell must be a leaf.  If the leaf has no data the returned
  //  link will test false.
  link leaf_data_get(cell_index_type c) const;

  //: Insert a leaf data entry for the given leaf.  The given cell
  //  must be a leaf.  If the leaf already has data there is no
  //  effect.  Returns a link into the leaf data array for the given
  //  leaf.
  link leaf_data_insert(cell_index_type c);

  //: Erase a leaf data entry for the given leaf.  The given cell must
  //  be a leaf.  If the leaf has no data there is no effect.
  void leaf_data_erase(cell_index_type c);

  //: Return a link into the node data array for the given node.  The
  //  given cell must be an internal node.  If the node has no data
  //  the returned link will test false.
  link node_data_get(cell_index_type c) const;

  //: Insert a node data entry for the given node.  The given cell
  //  must be an internal node.  If the node already has data there is
  //  no effect.  Returns a link into the node data array for the
  //  given node.
  link node_data_insert(cell_index_type c);

  //: Erase a node data entry for the given node.  The given cell must
  //  be an internal node.  If the node has no data there is no
  //  effect.
  ...
  //: Remove children from the given node thus converting it to a
  //  leaf.  The given cell must be a node with no data and its
  //  children must have no children or data of their own when this
  //  method is invoked.
