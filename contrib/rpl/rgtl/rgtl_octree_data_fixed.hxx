...
public:
  ...
  leaf_data_type const * set_leaf_data(cell_index_type cell_index, leaf_data_type const* leaf_data);

  leaf_data_type const * set_leaf_data(cell_location_type const& cell, leaf_data_type const* leaf_data);

  node_data_type const * set_node_data(cell_index_type cell_index, node_data_type const* node_data);

  node_data_type const * set_node_data(cell_location_type const& cell, node_data_type const* node_data);
  ...
  //  cell.
  void get_start_cell(cell_location_type const& dest_cell, cell_location_type& start_cell,
                      cell_index_type& start_index) const;

  cell_index_type location_to_index(cell_location_type const& cell) const;

  bool get_neighbor(cell_location_type const& cell, unsigned int face, cell_location_type& neighbor,
                    cell_index_type& nidx) const;

  void find_neighbor(cell_location_type const& dest_cell, cell_location_type const& cur_cell, cell_index_type cidx,
                     cell_location_type& out_cell, cell_index_type& out_cidx) const;

  // : Get the index of the child of the a current cell containing a destination cell.
  child_index_type get_child_containing(cell_location_type const& dest_cell, cell_location_type const& cur_cell) const;

private:
  leaf_data_type const * set_leaf_data(cell_location_type const& cell_dest, leaf_data_type const* leaf_data,
                                       cell_location_type const& cell_current, cell_index_type cidx);

  node_data_type const * set_node_data(cell_location_type const& cell_dest, node_data_type const* node_data,
                                       cell_location_type const& cell_current, cell_index_type cidx);

  cell_index_type location_to_index(cell_location_type const& dest_cell, cell_location_type const& cur_cell,
