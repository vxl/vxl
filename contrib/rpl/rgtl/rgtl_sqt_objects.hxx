...
public:
  ...
  //  leaf.
  rgtl_sqt_objects(sqt_object_array_type const& soa, double const origin[D], int ml, int mpl);

  // :
  //  Build with a set of objects, the star-shape guard location,
  //  a maximum subdivision level, and a maximum number of objects per
  //  leaf.
  void build(sqt_object_array_type const& soa, double const origin[D], int ml, int mpl);
  ...
  cell_index_type get_child(unsigned int face, cell_index_type cell_index, child_index_type child) const;

  bool get_depth_range(unsigned int face, cell_index_type cell_index, double& depth_min, double& depth_max) const;
