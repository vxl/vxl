...
private:
  ...
  void build(cell_geometry_type const& cell_geometry, cell_index_type cell_index, sqt_object_set_face_ptr& oa);
...
struct rgtl_sqt_objects_face_next
  {
  template <unsigned int D>
...
template <>
struct rgtl_sqt_objects_face_next<0>
  {
  template <unsigned int D>
...
public:
  ...
  void build(sqt_object_array_type const& objs, double const origin[D], int ml, int mpl);
...
public:
  ...
  double compute_disc_nearest(double query_distance, double query_angle, double query_depth, double cone_angle,
                              double clip_depth) const;

  bool disjoint(cell_data_type const* cell_data, double& nearest_squared) const;
...
rgtl_sqt_objects_face<D, Face>::query_ray(int id,
                                          ...
                                          double* s) const
{
  return this->object_array().object_intersects_ray(id,
