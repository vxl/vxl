...
public:
  ...
  virtual void  homography_to_parameters(const vnl_matrix<double>&  m, vnl_vector<double>&        p) const;

  // : Convert a linear parameter list (from estimation) to a homography.
  virtual void  parameters_to_homography(const vnl_vector<double>&  p, vnl_matrix<double>&        m) const;
