...
rrel_quad_est::rrel_quad_est( const vcl_vector<vgl_point_2d<double> > & from_pts,
...
rrel_quad_est::rrel_quad_est( const vcl_vector<vnl_vector<double> > & from_pts,
...
vnl_matrix<double>
rrel_quad_est::A( const vnl_vector<double>& params ) const
{
...
vnl_vector<double>
rrel_quad_est::trans( const vnl_vector<double>& params ) const
{
...
//
bool
rrel_quad_est::fit_from_minimal_set( const vcl_vector<int>& point_indices,
                                     vnl_vector<double>& params ) const
{
  if( point_indices.size() != min_num_pts_ )
    {
    vcl_cerr << "rrel_quad_est::fit_from_minimal_sample  The number of point "
...
bool
rrel_quad_est::weighted_least_squares_fit( vnl_vector<double>& params,
                                           ...
                                           const vcl_vector<double>* weights ) const
{
  ...
  if( (unsigned int)svd.rank() < min_num_pts_ )
    {
    vcl_cerr << "rrel_quad_est::WeightedLeastSquaresFit --- singularity!\n";
    return false;
    }
  else
    {
