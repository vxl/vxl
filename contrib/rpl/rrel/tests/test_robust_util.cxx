...
static void test_robust_util()
{
  ...
  vcl_vector<double> wgt(10); for( int i = 0; i < 10; ++i )
    {
    wgt[i] = wgt_arr[i];
    }
  double             rs_arr[] =  { 2,   4,    0.5,  7,  6,    5,   3,   2,   5,   2    };
  vcl_vector<double> rs(10); for( int i = 0; i < 10; ++i )
    {
    rs[i] = rs_arr[i];
    }
  double sum_wr = 0;
  ...
  TEST( "rrel_util_intercept_adjustment (pointer) --- sorted?", is_sorted( test_vect2.begin(), test_vect2.end() ),
        true);
