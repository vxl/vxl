...
clear()
{
  typedef vcl_vector< bin_type >::iterator vec_iter;
...
{
  typedef bin_index_vector::iterator ind_iter;
  typedef bin_type::const_iterator entry_iter;
...
template<unsigned N, typename C, typename V>
rsdl_bins<N,C,V>::bin_index_type
rsdl_bins<N,C,V>::
...
template<unsigned N, typename C, typename V>
rsdl_bins<N,C,V>::bin_index_vector
rsdl_bins<N,C,V>::
...
template<unsigned Ndim, typename C, typename V>
void
rsdl_bins<Ndim,C,V>::
                ...
                vcl_vector< point_type >* points ) const
{
  ...
  int bin_lo[Ndim], bin_hi[Ndim], cur[Ndim];
  ...
  // structures.
  //
  typedef vcl_vector< point_dist_entry >::iterator point_dist_iter;
...
// lo[dim..Ndim-1] and hi[dim..Ndim-1], boundary points inclusive. It
...
unsigned
rsdl_bins<Ndim,C,V>::
scan_region( int lo[Ndim], int hi[Ndim], int cur[Ndim], unsigned dim,
...
unsigned
rsdl_bins<Ndim,C,V>::
scan_bdy( int lo[Ndim], int hi[Ndim], int cur[Ndim], unsigned dim,
          bin_index_vector& indices ) const
{
  unsigned found = 0;
