...
template <unsigned N, typename C, typename V>
void
rsdl_bins<N, C, V>::set_distance_tolerance( coord_type const& tol )
{
  dist_tol_ = tol;
}
...
template <unsigned N, typename C, typename V>
void
rsdl_bins<N, C, V>::add_point( point_type const& pt, value_type const& val )
{
...
template <unsigned N, typename C, typename V>
bool
rsdl_bins<N, C, V>::get_value( point_type const& pt, value_type& val ) const
{
...
template <unsigned N, typename C, typename V>
void
rsdl_bins<N, C, V>::n_nearest( point_type const& pt,
                               unsigned n,
...
template <unsigned N, typename C, typename V>
void
rsdl_bins<N, C, V>::n_nearest_exhaustive( point_type const& pt,
                                          unsigned n,
                                          vcl_vector<value_type>& values ) const
{
...
template <unsigned N, typename C, typename V>
bool
rsdl_bins<N, C, V>::is_any_point_within_radius( point_type const& pt,
...
template <unsigned N, typename C, typename V>
void
rsdl_bins<N, C, V>::points_within_radius( point_type const& pt,
...
template <unsigned N, typename C, typename V>
void
rsdl_bins<N, C, V>::points_in_bounding_box( point_type const& min_pt,
...
template <unsigned N, typename C, typename V>
int
rsdl_bins<N, C, V>::coord_to_bin( coord_type x, unsigned d ) const
{
...
template <unsigned N, typename C, typename V>
void
rsdl_bins<N, C, V>::point_to_bin( point_type const& pt, int ind[N] ) const
{
...
rsdl_bins<N, C, V>::bin_index( point_type const& pt ) const
{
...
template <unsigned N, typename C, typename V>
typename rsdl_bins<N, C, V>::bin_index_type
rsdl_bins<N, C, V>::bin_index( int bin[N] ) const
{
  bin_index_type index = 0;
  bin_index_type skip = 1;
...
rsdl_bins<N, C, V>::bin_indices( point_type const& pt ) const
{
...
template <unsigned N, typename C, typename V>
void
rsdl_bins<N, C, V>::closest_face( point_type const& pt,
                                  int bin_lo[N],
                                  int bin_hi[N],
                                  unsigned * face_dim,
                                  unsigned * face_dir,
...
template <unsigned N, typename C, typename V>
void
rsdl_bins<N, C, V>::n_nearest_impl( point_type const& pt,
                                    unsigned n,
                                    vcl_vector<value_type>& values,
  ...
  while( !done )
    ...
    if( add_new_bins )
      ...
      if( bin_rng_empty )
        ...
        for( unsigned i = 0; i < N; ++i )
          {
          bin_rng_hi[i] = bin_rng_lo[i];
          }
        bin_rng_empty = false;
        // the entire bin search range is newly added
        for( unsigned i = 0; i < N; ++i )
          {
          bin_new_lo[i] = bin_rng_lo[i];
          }
        for( unsigned i = 0; i < N; ++i )
          {
          bin_new_hi[i] = bin_rng_hi[i];
          }
        found_new_bins = true;
        }
      else
...
template <unsigned N, typename C, typename V>
void
rsdl_bins<N, C, V>::n_nearest_exhaustive_impl( point_type const& pt,
                                               unsigned n,
                                               vcl_vector<value_type>& values,
                                               vcl_vector<point_type>* points ) const
{
...
template <unsigned N, typename C, typename V>
void
rsdl_bins<N, C, V>::points_within_radius_impl( point_type const& pt,
                                               coord_type const& radius,
                                               vcl_vector<value_type>& values,
                                               vcl_vector<point_type>* points ) const
{
...
template <unsigned N, typename C, typename V>
void
rsdl_bins<N, C, V>::points_in_bounding_box_impl( point_type const& min_pt,
                                                 point_type const& max_pt,
                                                 vcl_vector<value_type>& values,
                                                 vcl_vector<point_type>* points ) const
{
...
template <unsigned N, typename C, typename V>
vcl_size_t
rsdl_bins<N, C, V>::scan_region( int lo[N], int hi[N], int cur[N], unsigned dim,
...
template <unsigned N, typename C, typename V>
unsigned
rsdl_bins<N, C, V>::scan_bdy( int lo[N], int hi[N], int cur[N], unsigned dim,
                              bin_index_vector& indices ) const
{
  unsigned found = 0;
...
template <unsigned N, typename C, typename V>
rsdl_bins_bin_entry_type<N, C, V>::rsdl_bins_bin_entry_type( point_type const& pt, const value_type val )
  : point_( pt ),
  value_( val )
{
}
...
template <unsigned N, typename C, typename V>
bool
rsdl_bins_bin_entry_type<N, C, V>::equal( point_type const& pt, double tol_sqr ) const
{
...
template <unsigned N, typename C, typename V>
bool
rsdl_bins_point_dist_entry<N, C, V>::operator<( rsdl_bins_point_dist_entry const& other ) const
{
