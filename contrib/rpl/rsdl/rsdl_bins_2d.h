...
public:
  ...
  void n_nearest( const point_type & query_pt, int n, vcl_vector<point_type> & points,
  ...
  void points_within_radius( const point_type & query_pt, COORD_T radius, vcl_vector<point_type> & points,
                             vcl_vector<value_type>& values ) const;

  bool is_any_point_in_bounding_box(  const point_type & min_query_pt, const point_type & max_query_pt ) const;

  void points_in_bounding_box( const point_type & min_query_pt, const point_type & max_query_pt,
                               vcl_vector<value_type>& values ) const;

  void points_in_bounding_box( const point_type & min_query_pt, const point_type & max_query_pt,
                               vcl_vector<point_type> & points, vcl_vector<value_type> & values ) const;
