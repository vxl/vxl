...
public:
  ...
  void n_nearest( const rsdl_point& query_point, int n, vcl_vector<rsdl_point>& closest_points,
                  vcl_vector<int>& indices, bool use_heap = false, int max_leaves = -1 );

  // : find all points within a query's bounding box
  void points_in_bounding_box( const rsdl_bounding_box& box, vcl_vector<rsdl_point>& closest_points,
                               vcl_vector<int>& indices );

  // : find all points within a given distance of the query_point.
  void points_in_radius( const rsdl_point& query_point, double radius, vcl_vector<rsdl_point>& points,
                         vcl_vector<int>& indices );
...
private:
  ...
  rsdl_kd_node * build_kd_tree( int points_per_leaf, const rsdl_bounding_box& outer_box, int depth,
                                vcl_vector<int>& indices );
  ...
  void n_nearest_with_stack( const rsdl_point& query_point, int n, rsdl_kd_node* root, vcl_vector<int>& closest_indices,
                             vcl_vector<double>& sq_distances, int & num_found );

  void n_nearest_with_heap( const rsdl_point& query_point, int n, rsdl_kd_node* root, vcl_vector<int>& closest_indices,
                            vcl_vector<double>& sq_distances, int & num_found, int max_leaves);

  void update_closest( const rsdl_point& query_point, int n, rsdl_kd_node* p, vcl_vector<int>& closest_indices,
                       vcl_vector<double>& sq_distances, int & num_found );

  bool bounded_at_leaf( const rsdl_point& query_point, int n, rsdl_kd_node* current,
                        const vcl_vector<double>& sq_distances, int & num_found );
