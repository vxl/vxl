                   ...
                   vcl_string name )
{
  ...
  for( unsigned tn = 0; tn < num_trials; ++tn )
    ...
    if( !pass_match )
      {
      vcl_cout << "debug vals_fst and vals_exh do not match\n"
               << "debug tn: " << tn << '\n'
...
static void test_bins_nearest_all_inst( unsigned n,
                                        ...
                                        vcl_string name )
{
  test_bins_nearest<2, float, int>( n, npoints, square_bins, bound_points, single_bin_dim,
                                    name + ",N=2,C=float,V=int" );
  test_bins_nearest<3, float, int>( n, npoints, square_bins, bound_points, single_bin_dim,
                                    name + ",N=3,C=float,V=int" );
  test_bins_nearest<2, double, int>( n, npoints, square_bins, bound_points, single_bin_dim,
                                     name + ",N=2,C=double,V=int" );
  test_bins_nearest<3, double, int>( n, npoints, square_bins, bound_points, single_bin_dim,
                                     name + ",N=3,C=double,V=int" );
  test_bins_nearest<1, float, unsigned short>( n, npoints, square_bins, bound_points, single_bin_dim,
                                               name + ",N=1,C=float,V=ushort" );
  test_bins_nearest<7, float, unsigned short>( n, npoints, square_bins, bound_points, single_bin_dim,
                                               name + ",N=7,C=float,V=ushort" );
}
