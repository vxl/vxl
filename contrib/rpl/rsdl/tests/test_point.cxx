...
static void test_point()
{
  ...
  testlib_test_begin( "ctor from two arrays" );
    ...
    for( unsigned int i = 0; ok && i < Nc; ++i )
      {
      ok = ok && close(pt2.cartesian(i), cart[i]);
      }
    for( unsigned int i = 0; ok && i < Na; ++i )
      {
      ok = ok && close(pt2.angular(i), ang[i]);
      }
  ...
  testlib_test_begin( "ctor from two vnl_vectors" );
    ...
    for( unsigned int i = 0; ok && i < Nc; ++i )
      {
      ok = ok && close(pt_from_2v.cartesian(i), cv[i]);
      }
    for( unsigned int i = 0; ok && i < Na; ++i )
      {
      ok = ok && close(pt_from_2v.angular(i), av[i]);
      }
    testlib_test_perform( ok );
    }
  ...
  rsdl_point pt3( cart.begin(), cart.end(), ang.begin(), ang.end() );
    ...
    for( unsigned int i = 0; ok && i < Nc; ++i )
      {
      ok = ok && close(pt3.cartesian(i), cart[i]);
      }
    for( unsigned int i = 0; ok && i < Na; ++i )
      {
      ok = ok && close(pt3.angular(i), ang[i]);
      }
    testlib_test_perform( ok );
    }
  ...
  testlib_test_begin( "copy ctor" );
    ...
    for( unsigned int i = 0; ok && i < Nc; ++i )
      {
      ok = ok && close(pt4.cartesian(i), cart[i]);
      }
    for( unsigned int i = 0; ok && i < Na; ++i )
      {
      ok = ok && close(pt4.angular(i), ang[i]);
      }
    testlib_test_perform( ok );
    }
