...
main()
{
  ...
  vbl_test_begin( "square distance between points" );
  vbl_test_perform( close( sq_dist, rsdl_dist_sq( p, q ) ) );

  vbl_test_begin( "distance between points" );
  vbl_test_perform( close( sqrt(sq_dist), rsdl_dist( p, q ) ) );
  ...
  vbl_test_begin( "point-to-box dist sq - point inside" );
  ...
  vbl_test_perform( close( rsdl_dist_sq( test_p, box), 0) );
  vbl_test_begin( "point-to-box dist - point inside" );
  vbl_test_perform( close( rsdl_dist( test_p, box), 0) );
  
  vbl_test_begin( "point-to-box dist sq - point outside cartesian" );
  cart0[0] = -2.5; cart0[1] = 7.5;
  test_p.set_cartesian( cart0.begin() );
  vbl_test_perform( close( rsdl_dist_sq( test_p, box), 3.25) );
  vbl_test_begin( "point-to-box dist sq - point outside cartesian" );
  vbl_test_perform( close( rsdl_dist( test_p, box), sqrt(3.25)) );

  vbl_test_begin( "point-to-box dist sq - angle outside (a)" );
  ...
  vbl_test_perform( close( rsdl_dist_sq( test_p, box), sq_dist ) );
  vbl_test_begin( "point-to-box dist - angle outside (a) " );
  vbl_test_perform( close( rsdl_dist( test_p, box), sqrt(sq_dist)) );

  vbl_test_begin( "point-to-box dist sq - angle outside (b)" );
  ...
  vbl_test_perform( close( rsdl_dist_sq( test_p, box), sq_dist ) );
  vbl_test_begin( "point-to-box dist - angle outside (b) " );
  vbl_test_perform( close( rsdl_dist( test_p, box), sqrt(sq_dist)) );
 ...
 //  Now, both wrap around...
  ...
  inner_a.min_cartesian(0) = vnl_math::pi/4; inner_a.max_cartesian(0) = vnl_math::pi/8;
  rsdl_dist_box_relation( inner_a, outer_a, inside, intersects );
  vbl_test_begin( "rsdl_dist_box_relation - angular only (4a) " );
  vbl_test_perform( !inside && intersects );

  //  between / below (intersect)
  inner_a.min_cartesian(0) = vnl_math::pi; inner_a.max_cartesian(0) = vnl_math::pi/8;
  rsdl_dist_box_relation( inner_a, outer_a, inside, intersects );
  vbl_test_begin( "rsdl_dist_box_relation - angular only (4b) " );
  vbl_test_perform( !inside && intersects );

  //  above / below (inside and intersect)
  inner_a.min_cartesian(0) = 7*vnl_math::pi/4; inner_a.max_cartesian(0) = vnl_math::pi/8;
  rsdl_dist_box_relation( inner_a, outer_a, inside, intersects );
  vbl_test_begin( "rsdl_dist_box_relation - angular only (4c) " );
  vbl_test_perform( inside && intersects );

  //  between / between (intersect)
  inner_a.min_cartesian(0) = 9*vnl_math::pi/8; inner_a.max_cartesian(0) = vnl_math::pi;
  rsdl_dist_box_relation( inner_a, outer_a, inside, intersects );
  vbl_test_begin( "rsdl_dist_box_relation - angular only (4d) " );
  vbl_test_perform( intersects );

  //  above / between (intersect)
  inner_a.min_cartesian(0) = 7*vnl_math::pi/4; inner_a.max_cartesian(0) = vnl_math::pi;
  rsdl_dist_box_relation( inner_a, outer_a, inside, intersects );
  vbl_test_begin( "rsdl_dist_box_relation - angular only (4e) " );
  vbl_test_perform( intersects );

  //  above / above (intersect)
  inner_a.min_cartesian(0) = 7*vnl_math::pi/4; inner_a.max_cartesian(0) = 13*vnl_math::pi/8;
  rsdl_dist_box_relation( inner_a, outer_a, inside, intersects );
  vbl_test_begin( "rsdl_dist_box_relation - angular only (4f) " );
  vbl_test_perform( intersects );
  ...
  vbl_test_begin( "rsdl_dist_box_relation - complete (c) " );
  rsdl_dist_box_relation( inner, outer, inside, intersects );
  vbl_test_perform( !inside && intersects );

  // intersects fails on cartesian
  inner.min_cartesian(0) = 4.75; inner.max_cartesian(0) = 5.0;
  vbl_test_begin( "rsdl_dist_box_relation - complete (d) " );
  rsdl_dist_box_relation( inner, outer, inside, intersects );
  vbl_test_perform( !inside && !intersects );
  ...
  vbl_test_begin( "rsdl_dist_box_relation - complete (e) " );
  rsdl_dist_box_relation( inner, outer, inside, intersects );
  vbl_test_perform( !inside && !intersects );
  ...
  vbl_test_begin( "rsdl_dist_point_in_box (a) " );
  inside = rsdl_dist_point_in_box( pt, outer );
  vbl_test_perform( inside );

  pt.angular(2) = 7*pi/4;
  vbl_test_begin( "rsdl_dist_point_in_box (b) " );
  inside = rsdl_dist_point_in_box( pt, outer );
  vbl_test_perform( inside );

  vbl_test_begin( "rsdl_dist_point_in_box (c) " );
  pt.cartesian(1) = -2;
  inside = rsdl_dist_point_in_box( pt, outer );
  vbl_test_perform( !inside );

  vbl_test_begin( "rsdl_dist_point_in_box (d) " );
  pt.cartesian(1) = 1;
  pt.cartesian(0) = 5;
  inside = rsdl_dist_point_in_box( pt, outer );
  vbl_test_perform( !inside );

  vbl_test_begin( "rsdl_dist_point_in_box (e) " );
  pt.cartesian(0) = 2.5;
  inside = rsdl_dist_point_in_box( pt, outer );
  vbl_test_perform( inside );  // sanity check to make sure things are reset correctly

  vbl_test_begin( "rsdl_dist_point_in_box (f) " );
  double temp = pt.angular(0);
  pt.angular(0) = -3*pi/2;
  inside = rsdl_dist_point_in_box( pt, outer );
  pt.angular(0) = temp;
  bool back_inside = rsdl_dist_point_in_box( pt, outer );
  vbl_test_perform( !inside && back_inside ); 

  vbl_test_begin( "rsdl_dist_point_in_box (g) " );
  temp = pt.angular(0);
  pt.angular(0) = 3*pi/2;
  inside = rsdl_dist_point_in_box( pt, outer );
  pt.angular(0) = temp;
  ...
  vbl_test_begin( "rsdl_dist_point_in_box (h) " );
  temp = pt.angular(2);
  pt.angular(2) = pi;
  inside = rsdl_dist_point_in_box( pt, outer );
  pt.angular(2) = temp;
