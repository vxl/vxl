...
public: virtual ~section__()
  {
  }

};
template <class T>
class section_ : public section__
{
protected: section_() : section__()
  {
  }
...
class section_iterator;                            // forward declaration

template <class T, uint N>
class section : public section_<T>
{
#ifdef __GNUC__
...
public:
  ...
  T      Value(uint pos0, uint pos1, uint pos2) const
  {
    assert(N == 3);
    return buffer[pos0 * offset[0] + pos1 * offset[1] + pos2 * offset[2]];
  }
  ...
  void   Set(T const& val, uint pos0, uint pos1, uint pos2)
  {
    assert(N == 3);
    buffer[pos0 * offset[0] + pos1 * offset[1] + pos2 * offset[2]] = val;
  }
  vcl_size_t Position(const uint pos[N])  const
  {
    vcl_size_t p = 0; for( uint i = 0; i < N; ++i )
      {
      p += pos[i] * Offset(
          i);
      }
  ...
  void   Position(uint ret[N],
                  const vcl_size_t pos)  const { for( uint i = 0; i < N; ++i ) {ret[i] = Position(pos, i); } }
  uint   Position(const vcl_size_t pos, const uint i)  const
  {
    vcl_size_t p = pos % Offset(i + 1); p /= Offset(i);
...
bool section<T, N>::operator==(section<T, N> const& s) const
{
  ...
  for( vcl_size_t i = 0; i < GetSize(); ++i )
    {
...
section<T, N> & section<T, N>::operator=(section<T, N> const& s)
{
  ...
  for( uint i = 0; i < N; ++i )
    {
    ROI_start[i] = s.ROI_start[i], ROI_end[i] = s.ROI_end[i];
    }
  return *this;
}
...
section<T, N - 1> Project(section<T, N> const& s, uint slice = 0, int d = -1, bool copy = false)
{
  ...
  for( i = 0; i < dim; ++i )
    {
    t.ROI_start[i] = s.ROI_start[i], t.ROI_end[i] = s.ROI_end[i];
    }
  for( ; i < N - 1; ++i )
    {
    t.ROI_start[i] = s.ROI_start[i + 1], t.ROI_end[i] = s.ROI_end[i + 1];
    }
  return t;
}
...
public:
  ...
  section_iterator<T, N> & operator++()
  {
    ++pos;
    for( uint i = 0; i < N; ++i )
      {
      if( data->Position(pos, i) >= data->ROI_end[i] )
        {
  ...
  section_iterator<T, N> & operator--()
  {
    ...
    for( uint i = 0; i < N; ++i )
      {
      if( data->Position(pos, i) < data->ROI_start[i] )
        {
