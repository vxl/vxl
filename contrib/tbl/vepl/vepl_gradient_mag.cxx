...
vil_image vepl_gradient_mag(vil_image const& image, double scale, double shift)
{
  // byte greyscale
  if (vil_pixel_format(image) == VIL_BYTE) {
    ...
    vipl_gradient_mag<vil_image,vil_image,ubyte,ubyte,vipl_trivial_pixeliter> op(scale, shift);
  ...
  // float
    ...
    vipl_gradient_mag<vil_image,vil_image,float,float,vipl_trivial_pixeliter> op(scale, shift);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // double
    ...
    vipl_gradient_mag<vil_image,vil_image,double,double,vipl_trivial_pixeliter> op(scale, shift);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
