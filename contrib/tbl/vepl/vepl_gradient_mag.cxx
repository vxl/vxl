...
vil_image vepl_gradient_mag(vil_image const& image, double scale, double shift)
{
  ...
  if (vil_pixel_format(image) == VIL_BYTE) {
    ...
    vipl_gradient_mag<vil_image,vil_image,ubyte,ubyte> op(scale, shift);
  ...
  // 16-bit greyscale
    ...
    vipl_gradient_mag<vil_image,vil_image,ushort,ushort> op(scale, shift);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // float
    ...
    vipl_gradient_mag<vil_image,vil_image,float,float> op(scale, shift);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // double
    ...
    vipl_gradient_mag<vil_image,vil_image,double,double> op(scale, shift);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
