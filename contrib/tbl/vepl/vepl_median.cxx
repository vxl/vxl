...
vil_image vepl_median(vil_image const& image, float radius)
{
  ...
  if (vil_pixel_format(image) == VIL_BYTE) {
    ...
    vipl_median<vil_image,vil_image,ubyte,ubyte> op(radius);
  ...
  // 16-bit greyscale
    ...
    vipl_median<vil_image,vil_image,ushort,ushort> op(radius);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // float
    ...
    vipl_median<vil_image,vil_image,float,float> op(radius);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // double
    ...
    vipl_median<vil_image,vil_image,double,double> op(radius);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
