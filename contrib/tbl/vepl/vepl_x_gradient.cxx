...
vil1_image vepl_x_gradient(vil1_image const& image, double scale, double shift)
{
  ...
  if (vil1_pixel_format(image) == VIL1_BYTE) {
    ...
    vipl_x_gradient<vil1_image,vil1_image,ubyte,ubyte> op(scale, (ubyte)(shift+0.5));
...
#define r_g_b vil1_rgb<ubyte> // cannot use typedef since that may cause ambiguous overload problems
  ...
  // 16-bit greyscale
    ...
    vipl_x_gradient<vil1_image,vil1_image,ushort,ushort> op(scale, (ushort)(shift+0.5));
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
