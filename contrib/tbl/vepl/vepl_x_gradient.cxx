...
vil_image vepl_x_gradient(vil_image const& image, double scale, double shift)
{
  ...
  if (vil_pixel_format(image) == VIL_BYTE) {
    ...
    vipl_x_gradient<vil_image,vil_image,ubyte,ubyte> op(scale, (ubyte)(shift+0.5));
  ...
  // byte rgb
    ...
    vipl_x_gradient<vil_image,vil_image,r_g_b,r_g_b> op(scale, r_g_b((ubyte)(shift+0.5)) );
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // 16-bit greyscale
    ...
    vipl_x_gradient<vil_image,vil_image,ushort,ushort> op(scale, (ushort)(shift+0.5));
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // float
    ...
    vipl_x_gradient<vil_image,vil_image,float,float> op(scale, (float)shift);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // double
    ...
    vipl_x_gradient<vil_image,vil_image,double,double> op(scale, shift);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
