...
vil_image vepl_x_gradient(vil_image const& image, double scale, double shift)
{
  // byte greyscale
  if (vil_pixel_format(image) == VIL_BYTE) {
    ...
    vipl_x_gradient<vil_image,vil_image,ubyte,ubyte,vipl_trivial_pixeliter> op(scale, (ubyte)(shift+0.5));
  ...
  // byte rgb
    ...
    vipl_x_gradient<vil_image,vil_image,vil_rgb<ubyte> ,vil_rgb<ubyte> ,vipl_trivial_pixeliter> op(scale, vil_rgb<ubyte>((ubyte)(shift+0.5)) );
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // float
    ...
    vipl_x_gradient<vil_image,vil_image,float,float,vipl_trivial_pixeliter> op(scale, (float)shift);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // double
    ...
    vipl_x_gradient<vil_image,vil_image,double,double,vipl_trivial_pixeliter> op(scale, shift);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
