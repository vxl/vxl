// This is tbl/vepl1/vepl_gaussian_convolution.cxx
...
vil1_image vepl_gaussian_convolution(vil1_image const& image, double sigma, double cutoff)
{
  ...
  if (vil1_pixel_format(image) == VIL1_BYTE) {
    ...
    vipl_gaussian_convolution<vil1_image,vil1_image,vxl_byte,vxl_byte> op(sigma, cutoff);
...
#define r_g_b vil1_rgb<vxl_byte> // cannot use typedef since that may cause ambiguous overload problems
  ...
  // 16-bit greyscale
    ...
    vipl_gaussian_convolution<vil1_image,vil1_image,vxl_uint_16,vxl_uint_16> op(sigma, cutoff);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
