// This is tbl/vepl1/vepl_gradient_mag.cxx
...
vil1_image vepl_gradient_mag(vil1_image const& image, double scale, double shift)
{
  // byte greyscale
  if (vil1_pixel_format(image) == VIL1_BYTE) {
    ...
    vipl_gradient_mag<vil1_image,vil1_image,vxl_byte,vxl_byte> op(scale, shift);
  ...
  // byte rgb
    ...
    vcl_cerr << __FILE__ ": vepl_gradient_mag() cannot be implemented for colour images\n";
    return 0;
  }
  ...
  // 16-bit greyscale
    ...
    vipl_gradient_mag<vil1_image,vil1_image,vxl_uint_16,vxl_uint_16> op(scale, shift);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // float
    ...
    vipl_gradient_mag<vil1_image,vil1_image,float,float> op(scale, shift);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // double
    ...
    vipl_gradient_mag<vil1_image,vil1_image,double,double> op(scale, shift);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  //
    ...
    vcl_cerr << __FILE__ ": vepl_gradient_mag() not implemented for " << image << vcl_endl;
    return 0;
  }
}
