// This is tbl/vepl1/vepl_median.cxx
...
vil1_image vepl_median(vil1_image const& image, float radius)
{
  // byte greyscale
  if (vil1_pixel_format(image) == VIL1_BYTE) {
    ...
    vipl_median<vil1_image,vil1_image,vxl_byte,vxl_byte> op(radius);
...
#define r_g_b vil1_rgb<vxl_byte> // cannot use typedef since that may cause ambiguous overload problems
  ...
  // 16-bit greyscale
    ...
    vipl_median<vil1_image,vil1_image,vxl_uint_16,vxl_uint_16> op(radius);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // float
    ...
    vipl_median<vil1_image,vil1_image,float,float> op(radius);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
  ...
  // double
    ...
    vipl_median<vil1_image,vil1_image,double,double> op(radius);
    op.put_in_data_ptr(&mem);
    op.put_out_data_ptr(&out);
    op.filter();
    return out;
