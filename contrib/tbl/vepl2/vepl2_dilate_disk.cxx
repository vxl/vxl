...
vil2_image_view_base_sptr vepl2_dilate_disk(vil2_image_view_base const& image, float radius)
{
  // byte rgb
  if (image.nplanes() > 1 || image.pixel_format() == VIL2_PIXEL_FORMAT_RGB_BYTE) {
    vcl_cerr << __FILE__ ": currently no dilation of colour images\n";
    return 0;
  }
  ...
  // byte greyscale
    ...
    vipl_dilate_disk<vil2_image_view_base,vil2_image_view_base,vxl_byte,vxl_byte,vipl_trivial_pixeliter> op(radius);
  ...
  // short
    ...
    vipl_dilate_disk<vil2_image_view_base,vil2_image_view_base,unsigned short,unsigned short,vipl_trivial_pixeliter> op(radius);
    op.put_in_data_ptr(&image);
    op.put_out_data_ptr(out);
    op.filter();
    return out;
  ...
  // int
    ...
    vipl_dilate_disk<vil2_image_view_base,vil2_image_view_base,unsigned,unsigned,vipl_trivial_pixeliter> op(radius);
    op.put_in_data_ptr(&image);
    op.put_out_data_ptr(out);
    op.filter();
    return out;
  ...
  // float
    ...
    vipl_dilate_disk<vil2_image_view_base,vil2_image_view_base,float,float,vipl_trivial_pixeliter> op(radius);
    op.put_in_data_ptr(&image);
    op.put_out_data_ptr(out);
    op.filter();
    return out;
  ...
  // double
    ...
    vipl_dilate_disk<vil2_image_view_base,vil2_image_view_base,double,double,vipl_trivial_pixeliter> op(radius);
    op.put_in_data_ptr(&image);
    op.put_out_data_ptr(out);
    op.filter();
    return out;
  ...
  //
    ...
    vcl_cerr << __FILE__ ": vepl2_dilate_disk() not implemented for " << image.is_a() << '\n';
    return 0;
  }
}
