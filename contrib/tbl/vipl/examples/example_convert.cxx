...
//  Uses vipl_convert<vil1_image,vil1_image,*,ubyte>.
//
...
main(int argc, char** argv) {
  ...
  if (in.component_format() == VIL1_COMPONENT_FORMAT_UNSIGNED_INT) {
    if (in.planes() == 1 && in.components() == 1) { // monochrome
      if (in.bits_per_component() == 8) {
        ...
        vipl_convert<vil1_image,vil1_image,short,ubyte VCL_DFL_TMPL_ARG(vipl_trivial_pixeliter)> op;
        ...
        vipl_convert<vil1_image,vil1_image,int,ubyte VCL_DFL_TMPL_ARG(vipl_trivial_pixeliter)> op;
        op.put_in_data_ptr(&in); op.put_out_data_ptr(&out); op.filter();
      ...
      if (in.bits_per_component() == 8) {
        vipl_convert<vil1_image,vil1_image,rgbcell,ubyte> op;
        op.put_in_data_ptr(&in); op.put_out_data_ptr(&out); op.filter();
  ...
  else if (in.component_format() == VIL1_COMPONENT_FORMAT_IEEE_FLOAT) {
    if (in.components() == 1 && in.planes() == 1) { // monochrome
      if (in.bits_per_component() == 32) {
        vipl_convert<vil1_image,vil1_image,float,ubyte> op;
        op.put_in_data_ptr(&in); op.put_out_data_ptr(&out); op.filter();
        vcl_cout << "vipl_converted float image to PGM image " << argv[2] << vcl_endl;
      } else if (in.bits_per_component() == 64) {
        vipl_convert<vil1_image,vil1_image,double,ubyte> op;
        op.put_in_data_ptr(&in); op.put_out_data_ptr(&out); op.filter();
  ...
  } else if (in.component_format() == VIL1_COMPONENT_FORMAT_COMPLEX) {
      vcl_cerr << "Cannot currently handle complex pixel type images\n";
      return 1;
#if 0
  } else if (in.component_format() == VIL1_COMPONENT_FORMAT_PYRAMID) {
      vcl_cerr << "Cannot currently handle pyramid images\n";
      return 1;
  } else if (in.component_format() == VIL1_COMPONENT_FORMAT_BANDED) {
