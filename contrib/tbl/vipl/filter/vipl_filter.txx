...
const VIPL_FILTER_STATE vipl_filter<ImgIn, ImgOut, DataIn, DataOut, Arity,
                                    PixelItr>::Not_Ready VCL_STATIC_CONST_INIT_INT_DEFN( 0 );
template <class ImgIn, class ImgOut, class DataIn, class DataOut, int Arity, class PixelItr>
const VIPL_FILTER_STATE vipl_filter<ImgIn, ImgOut, DataIn, DataOut, Arity,
                                    PixelItr>::Ready VCL_STATIC_CONST_INIT_INT_DEFN( 1 );
template <class ImgIn, class ImgOut, class DataIn, class DataOut, int Arity, class PixelItr>
const VIPL_FILTER_STATE vipl_filter<ImgIn, ImgOut, DataIn, DataOut, Arity,
                                    PixelItr>::Unchanged VCL_STATIC_CONST_INIT_INT_DEFN( 2 );
template <class ImgIn, class ImgOut, class DataIn, class DataOut, int Arity, class PixelItr>
const VIPL_FILTER_STATE vipl_filter<ImgIn, ImgOut, DataIn, DataOut, Arity,
                                    PixelItr>::Filter_Owned VCL_STATIC_CONST_INIT_INT_DEFN( 4 );
#endif
  ...
  hsis_input_driven(false)
{
  ...
  for( int i = 0; i < ninputs; i++ )
    ...
    if( src_img )
      {
      src_img++;            // if real, go to next one
      }
#if 0
...
::~vipl_filter()
{
  ...
  if( ref_src_section() )
    {
    FILTER_IMPTR_DEC_REFCOUNT(ref_src_section() );                     // dec_refcount or kill it
    }
  if( ref_dst_section() )
    {
    FILTER_IMPTR_DEC_REFCOUNT(ref_dst_section() );                     // dec_refcount or kill it
    }
  if( ref_secp() )
    {
    FILTER_IMPTR_DEC_REFCOUNT(ref_secp() );                     // dec_refcount or kill it
    }
  if( ref_insecp() )
    {
    FILTER_IMPTR_DEC_REFCOUNT(ref_insecp() );                     // dec_refcount or kill it
    }
  if( ref_ROA() )
    {
    FILTER_IMPTR_DEC_REFCOUNT(ref_ROA() );                     // dec_refcount or kill it
    }
  if( ref_inROA() )
    {
    FILTER_IMPTR_DEC_REFCOUNT(ref_inROA() );                     // dec_refcount or kill it
    }
#endif
}
