...
class vipl_section_descriptor {
 ...
 public:
  ...
  vipl_section_descriptor(const vipl_section_descriptor< DataType > &);
    ...
    // This method takes in an argument called \usearg {axis} (i.e. 0
    // means the ``x'' axis, 1 means ``y'' axis) etc... and returns
    // an integer which describes the start coordinate value for
    // ``x'' (or ``y'' etc..) with respect to the ``image''
    // coordinate system
  virtual int curr_sec_start( int axis) const ;
    ...
    // (or ``y'' etc..) with respect to the ``image'' coordinate
    // system
  ...
  virtual int curr_sec_size( int axis) const ;
    ...
    // along the \usearg {axis}.
  virtual int data_offsets( int axis) const ;

    // Returns a referable pointer to the first data item in the
    // current section. If the value returned is null, then the
  ...
  virtual const DataType* data_ptr() const ;
    ...
    // Returns a writable pointer to the ``real'' section
    // descriptor. If this method is called on a concrete instance,
    // it should return 0.
  virtual vipl_section_descriptor< DataType >* inner_descriptor() ;

    // Returns a const pointer to the ``real'' section descriptor. If
    // this method is called on a concrete instance, it should
    // return 0
