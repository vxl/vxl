...
# Purpose: Automate generation of _ref and _new types for tableaux.
...
#
#blah_ref:
#   // Forward declare the _ref types. This could be in a header file.
#   (struct|class) blah;
#   #include <vgui/vgui_tableau_ref.h>
#   typedef vgui_tableau_ref_t<blah> blah_ref;
#
#blah:
...
#   struct blah_new : public blah_ref {
#     ctor1(...) : blah_ref(new blah(...)) { }
#     ctor2(...) : blah_ref(new blah(...)) { }
#   };
#
#
# Missing features and known bugs:
#  1. script could detect tableau base classes and derive 
#     the blah_new class from base_ref?
#
...
sub do_ref {
  ...
  # look for #include of blah_ref.h or typedef of blah_ref:
  my $gotref = 0;
  for (my $i=0; $i<=$#line; ++$i) {
    if ($line[$i] =~ m/^\s*\#\s*include\s*([\"\<].*[\"\>])/) {
      my $h = $1;
      if ($h =~ m/\b$blah\_ref\.h/) {
	print STDERR "$me: found #include of ${blah}_ref.h\n";
	$gotref = 1;
	last;
      }
    }
    if ($line[$i] =~ m/^\s*typedef\s+.*\s+$blah\_ref\s*\;/) {
      print STDERR "$me: found typedef of ${blah}_ref\n";
      $gotref = 1;
      last;
    }
  }
  ...
  ##############################
  # Make the _ref.h header unless the source file is not a header
  # file or the file _ref.h already exists.
  if ($file =~ m/^(.*)\.h$/) {
    my ($stem) = $1;
    if (-f "${stem}_ref.h") {
      print STDERR "$me: file ${stem}_ref.h exists already\n";
    }
    else {
      if (! $dry_run) {
	print STDERR "$me: create ${stem}_ref.h\n";
	die unless open(FD, ">${stem}_ref.h");
	$stem =~ s/[\+\-\.]/_/g;
	print FD "#ifndef ${stem}_ref_h_\n";
	print FD "#define ${stem}_ref_h_\n";
	print FD "\n";
	print FD "// this is a generated file.\n";
	print FD "\n";
	print FD "#include <vgui/vgui_tableau_ref.h>\n";
	print FD "\n";
	if ($template_clause eq "") {
	  print FD "$struct_or_class ${blah};\n";
	  print FD "typedef vgui_tableau_ref_t<${blah}> ${blah}_ref;\n";
	} else {
	  print FD "template <$template_clause>\n";
	  print FD "$struct_or_class ${blah};\n";
	  print FD "\n";
	  print FD "template <$template_clause>\n";
	  print FD "struct ${blah}_ref : public vgui_tableau_ref_t<${blah}<$template_args> > {\n";
	  print FD "  typedef vgui_tableau_ref_t<${blah}<$template_args> > base;\n";
	  print FD "  ${blah}_ref(${blah}<$template_args> *p) : base(p) { }\n";
	  print FD "};\n";
	}
	print FD "\n";
	print FD "#endif\n";
  ...
  my @blah_new_def = ();
  {
    ...
    if ($template_clause eq "") {
      push @blah_new_def, "struct ${blah}_new : public ${blah}_ref {\n";
      push @blah_new_def, "  typedef ${blah}_ref base;\n";
    }
    else {
      push @blah_new_def, "template <$template_clause>\n";
      push @blah_new_def, "struct ${blah}_new : public ${blah}_ref<$template_args > {\n";
      push @blah_new_def, "  typedef ${blah}_ref<$template_args > base;\n";
    }
  ...
  # then declare blah_ref
  if ($file eq "$blah.h") {
    # if it's a header file called "blah.h", insert a #include to 
    # the _ref file unless we've already seen one.
    if (! $gotref) {
      push @newfile, "#include \"${blah}_ref.h\"\n";
      push @newfile, "\n";
    }
  }
  else {
    ...
    if (! $gotref) {
      push @newfile, "#include <vgui/vgui_tableau_ref.h>\n";
      if ($template_clause eq "") {
	push @newfile, "$struct_or_class ${blah};\n";
	push @newfile, "typedef vgui_tableau_ref_t<${blah}> ${blah}_ref;\n";
      } else {
	push @newfile, "template <$template_clause>\n";
	push @newfile, "struct ${blah}_ref : public vgui_tableau_ref_t<${blah}<$template_args> > {\n";
	push @newfile, "  typedef vgui_tableau_ref_t<${blah}<$template_args> > base;\n";
	push @newfile, "  ${blah}_ref(${blah}<$template_args> *p) : base(p) { }\n";
	push @newfile, "};\n";
      }
      push @newfile, "\n";
    }
