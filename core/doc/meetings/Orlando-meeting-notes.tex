...
(Note that it will be difficult to decide on such rules, as tastes differ¨)
...
hacks needed for a specific platform, ¨) is not destroyed by this move?
...
the @assignment@ to something of type |¨_ref| should do.
...
E.g.: edit conic parameters, draw bitangent~¨\ 
...
-- other suggestions are welcome (e.g.: Video; splines/snakes; ¨)
...
- VGUI deals with geometry, not widgets/buttons¨
~(note: this was one of Parmesan's mistakes.)\\
$\Rightarrow$ able to use MFC or Qt or GTK or glut or glX or Motif or ¨ \\
...
draws its children one after another), a point overlay, an image raster ¨
...
Rendering primitives: point, line, polygon, triangular strip, volume, ¨
...
like displaying 16-bit images, colourmapped images, parts of very large images ¨
