  ...
  switch (ExceptionCode)
  {
    case EXCEPTION_ACCESS_VIOLATION:
      std::fprintf(stderr,
                   ...
                   ExceptionAddress,
                   ExceptionInformation[0] ? "write to" : "read",
                   ExceptionInformation[1]);
      break;

    case EXCEPTION_INT_DIVIDE_BY_ZERO:
      std::fprintf(
        stderr, "The instruction at \"0x%.8p\" caused an exception of integer devision by zero.\n\n", ExceptionAddress);
      break;
    default:
      std::fprintf(stderr,
...
testlib_main(int argc, char * argv[])
{
  ...
  else
  {
    ...
    std::cout << "\n\nCombined result of " << testlib_test_name_.size() << " tests: " << (all_pass ? "PASS" : "FAIL")
              << std::endl;
    return all_pass ? 0 : 1;
  }
...
void
testlib_register_test(const std::string & name, TestMainFunction func)
{
