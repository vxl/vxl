...
vbl_test_batch_compact_multimap1()
{
  ...
  TEST("batch_compact_multimap::equal_range",
       bcmmap.equal_range("3"),
...
vbl_test_batch_compact_multimap2()
{
  ...
  TEST("batch_compact_multimap sorted data values are scrabled by default sort",
       issorted(bcmmap.values_begin(), bcmmap.values_end()),
       false);
  ...
  std::stable_sort(
    test_data.begin(), test_data.end(), vbl_batch_compact_multimap<int, std::string>::input_compare(std::less<int>()));
  bcmmap2.assign_sorted(test_data.begin(), test_data.end());
  TEST("Check assign_sorted() kept stable sort value order",
       issorted(bcmmap2.lower_bound(-10), bcmmap2.upper_bound(10)),
       true);
}

static void
vbl_test_batch_compact_multimap()
{
  vbl_test_batch_compact_multimap1();
  vbl_test_batch_compact_multimap2();
}
