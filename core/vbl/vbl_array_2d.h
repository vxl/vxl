...
public:
  ...
  vbl_array_2d(const vbl_array_2d<T> & that)
  {
  ...
  //: Assignment
  vbl_array_2d<T> &
  operator=(const vbl_array_2d<T> & that)
  {
  ...
  //: Comparison
  bool
  operator==(const vbl_array_2d<T> & that) const
  {
  ...
  void
  put(size_type i, size_type j, const T & x)
  {
    rows_[i][j] = x;
  }
  T
  ...
  {
    return rows_[0] + num_cols_ * num_rows_;
  }
...
private:
  ...
  void
  construct(size_type m, size_type n)
  {
    num_rows_ = m;
    num_cols_ = n;
    if (m && n)
...
template <class T>
std::ostream &
operator<<(std::ostream &, const vbl_array_2d<T> &);
