...
public:
  ...
  vbl_array_3d(size_type n1, size_type n2, size_type n3, const T * init_values)
  {
    construct(n1, n2, n3);
    set(init_values);
  }

  vbl_array_3d(size_type n1, size_type n2, size_type n3, const T & fill_value)
  {
    construct(n1, n2, n3);
    fill(fill_value);
  }

  vbl_array_3d(const vbl_array_3d<T> & that)
    : element_(nullptr)
  {
    if (that.element_)
    {
  ...
  vbl_array_3d<T> &
  operator=(const vbl_array_3d<T> & that)
  {
  ...
  //: Comparison
  bool
  operator==(const vbl_array_3d<T> & that) const
  {
    if (row1_count_ != that.row1_count_ || row2_count_ != that.row2_count_ || row3_count_ != that.row3_count_)
      return false;
  ...
  reference
  operator()(size_type i1, size_type i2, size_type i3)
  {
    RANGECHECK(i1, i2, i3);
  ...
  const_reference
  operator()(size_type i1, size_type i2, size_type i3) const
  {
    RANGECHECK(i1, i2, i3);
    return element_[i1][i2][i3];
  }
  ...
  T * const *
  operator[](size_type i1)
  {
    return element_[i1];
  }
  const T * const *
  operator[](size_type i1) const
  {
    return element_[i1];
  }
  ...
  // dimensions
  size_type
  get_row1_count() const
  {
    return row1_count_;
  }
  size_type
  get_row2_count() const
  {
    return row2_count_;
  }
  size_type
  get_row3_count() const
  {
    return row3_count_;
  }
  ...
  void
  resize(size_type n1, size_type n2, size_type n3); // no malloc unless size changes.
  void
  set(const T * array);
  void
  get(T * array) const;
...
template <class T>
std::ostream &
operator<<(std::ostream &, const vbl_array_3d<T> &);

template <class T>
std::istream &
operator>>(std::istream &, vbl_array_3d<T> &);
