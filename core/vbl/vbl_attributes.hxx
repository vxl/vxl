...
minval(const vbl_array_1d<T> & in)
{
  assert(in.size() > 0);
  T mval = *(in.begin());
  for (typename vbl_array_1d<T>::const_iterator it = in.begin(); it != in.end(); ++it)
  {
    T v = *it;
    if (v < mval)
      mval = v;
...
minval(const vbl_array_2d<T> & in)
{
  assert(in.size() > 0);
  T mval = *(in.begin());
  for (typename vbl_array_2d<T>::const_iterator it = in.begin(); it != in.end(); ++it)
  {
    T v = *it;
    if (v < mval)
      mval = v;
...
minval(const vbl_array_3d<T> & in)
{
  assert(in.size() > 0);
  T mval = *(in.begin());
  for (typename vbl_array_3d<T>::const_iterator it = in.begin(); it != in.end(); ++it)
  {
    T v = *it;
    if (v < mval)
      mval = v;
