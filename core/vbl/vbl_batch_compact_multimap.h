...
public:
  ...
  public:
    ...
    bool
    operator()(const input_type & x, const input_type & y) const
    {
  ...
  template <typename CI>
  void
  assign_sorted(CI start, CI finish)
  {
    keys_.clear();
    indices_.clear();
    values_.clear();
    ...
    while (start != finish)
    {
      const typename std::iterator_traits<CI>::value_type::first_type & last_start_val = start->first;
  ...
  void
  swap(vbl_batch_compact_multimap & x)
  {
    keys_.swap(x.keys_);
  ...
  {
    const_key_iterator k_it = std::lower_bound(keys_.begin(), keys_.end(), x, key_compare());
  ...
  {
    const_key_iterator k_it = std::upper_bound(keys_.begin(), keys_.end(), x, key_compare());

    return values_.begin() + indices_[k_it - keys_.begin()];
  }

  //: A more efficient  make_pair(lower_bound(...), upper_bound(...))
  std::pair<const_value_iterator, const_value_iterator>
...
private:
  ...
  bool
  is_sorted(CI start, CI end, CMP comp)
  {
    if (start == end)
      return true;
...
inline void
swap(vbl_batch_compact_multimap<K, T, C> & x, vbl_batch_compact_multimap<K, T, C> & y)
{
  x.swap(y);
}
