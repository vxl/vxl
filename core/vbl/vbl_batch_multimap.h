...
public:
  ...
  public:
    ...
    bool
    compare(const value_type & x, const value_type & y) const
    {
      return comp(x.first, y.first);
    }
  };
  ...
  lower_bound(const key_type & key) const
  {
    return std::lower_bound(
      data_.begin(), data_.end(), std::make_pair(key, mapped_type()), value_compare_t(key_compare()));
  }
...
inline void
swap(vbl_batch_multimap<K, T, C> & x, vbl_batch_multimap<K, T, C> & y)
{
  x.swap(y);
}
