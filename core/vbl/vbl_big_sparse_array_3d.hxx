// This is core/vbl/vbl_big_sparse_array_3d.hxx
...
// locals
inline ulonglong
bigencode(unsigned i, unsigned j, unsigned k)
{
  // Use a map of tuples if you need bigger sparse arrays
#if VXL_HAS_INT_64
  assert(i <= 0x3fffff && j <= 0x1fffff && k <= 0x1fffff);
  return (((ulonglong)i) << 42) | (((ulonglong)j) << 21) | ((ulonglong)k);
#else
  assert(i <= 0x3ff && j <= 0x7ff && k <= 0x7ff);
  return (((ulonglong)i) << 22) | (((ulonglong)j) << 11) | ((ulonglong)k);
#endif
}

inline void
bigdecode(ulonglong v, unsigned & i, unsigned & j, unsigned & k)
{
#if VXL_HAS_INT_64
...
template <class T>
T &
vbl_big_sparse_array_3d<T>::operator()(unsigned i, unsigned j, unsigned k)
{
#ifdef DEBUG
  std::cout << "{vbl_big_sparse_array_3d(" << i << ',' << j << ',' << k << ") - storage[" << bigencode(i, j, k)
...
template <class T>
const T &
vbl_big_sparse_array_3d<T>::operator()(unsigned i, unsigned j, unsigned k) const
{
...
template <class T>
bool
vbl_big_sparse_array_3d<T>::fullp(unsigned i, unsigned j, unsigned k) const
{
#ifdef DEBUG
...
template <class T>
bool
vbl_big_sparse_array_3d<T>::put(unsigned i, unsigned j, unsigned k, const T & t)
{
...
#ifdef DEBUG
  std::cout << "{vbl_big_sparse_array_3d::put(" << i << ',' << j << ',' << k << ") - " << res.second << "}\n";
#endif
  return res.second;
}
...
template <class T>
std::ostream &
vbl_big_sparse_array_3d<T>::print(std::ostream & out) const
{
