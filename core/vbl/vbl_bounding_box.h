...
public:
  ...
  inline void
  update(const T & x, const T & y, const T & z)
  {
    assert(DIM_::value == 3);
    T tmp[3] = { x, y, z };
    update(tmp);
  ...
  inline bool
  inside(const T & x, const T & y) const
  {
    ...
    return initialized_ && min_[xdim] <= x && x <= max_[xdim] && min_[ydim] <= y && y <= max_[ydim];
  }

  //:  is a 3D point inside the bounding box
  inline bool
  inside(const T & x, const T & y, const T & z) const
  {
  ...
  {
    assert(DIM_::value >= 2);
    return (DIM_::value >= 2) ? min_[1] : min_[0];
  }
  inline const T &
  ymax() const
  {
    assert(DIM_::value >= 2);
    return (DIM_::value >= 2) ? max_[1] : max_[0];
  }
  inline const T &
  zmin() const
  {
    assert(DIM_::value >= 3);
    return (DIM_::value >= 3) ? min_[2] : min_[0];
  }
  inline const T &
  zmax() const
  {
    assert(DIM_::value >= 3);
    return (DIM_::value >= 3) ? max_[2] : max_[0];
  }
...
template <class T, class DIM_>
inline bool
nested(const vbl_bounding_box_base<T, DIM_> & a, const vbl_bounding_box_base<T, DIM_> & b)
{
...
template <class T, class DIM_>
inline bool
disjoint(const vbl_bounding_box_base<T, DIM_> & a, const vbl_bounding_box_base<T, DIM_> & b)
{
  for (int i = 0; i < DIM_::value; ++i)
...
template <class T, class DIM_>
inline bool
meet(const vbl_bounding_box_base<T, DIM_> & a, const vbl_bounding_box_base<T, DIM_> & b)
{
  return !disjoint(a, b);
}
...
template <class T, class DIM_>
std::ostream &
operator<<(std::ostream & s, const vbl_bounding_box_base<T, DIM_> & bbox);
