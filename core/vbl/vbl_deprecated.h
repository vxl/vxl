#ifndef vbl_deprecated_h_
#define vbl_deprecated_h_
...
// a debugger!
  ...
  void vbl_deprecated_warn( const char* func_name );
  void vbl_deprecated_abort( const char* func_name );
  #ifdef VXL_WARN_DEPRECATED_ABORT
    #define VXL_DEPRECATED(f) vbl_deprecated_abort( f )
  #else
    #ifdef VXL_WARN_DEPRECATED_ONCE
      #define VXL_DEPRECATED(f) static bool vbl_deprecated_flag = true; \
                                if( vbl_deprecated_flag ) {vbl_deprecated_warn( f ); vbl_deprecated_flag=false;}
    #else
      #define VXL_DEPRECATED(f) vbl_deprecated_warn( f )
    #endif
  #endif
#else
