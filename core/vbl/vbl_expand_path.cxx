// This is vxl/vbl/vbl_expand_path.cxx
...
#endif
#include "vbl_expand_path.h"

#ifdef VCL_WIN32
vcl_string vbl_expand_path         (char const *path_) { return vcl_string(path_); }
vcl_string vbl_expand_path_uncached(char const *path_) { return vcl_string(path_); }
#else
...
static
vcl_string vbl_expand_path_internal(vcl_string path)
{
  // expand ~/ or just ~
  ...
  for (unsigned int i=1; i<=path.size(); ++i) {
    if (i==path.size() || path[i] == '/') {
      ...
      if (len != -1) {
        ...
        return vbl_expand_path_internal(path);
      }
    }
  }
...
vcl_string vbl_expand_path(vcl_string path)
{
  ...
  if (i == the_map->end()) {
    ...
    vcl_string mapped = vbl_expand_path_internal(path);
    // cache it :
