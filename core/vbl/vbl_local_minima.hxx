...
template <class T>
bool
local_minima(const vbl_array_1d<T> & in, vbl_array_1d<T> & minima, T thresh)
{
...
template <class T>
bool
local_minima(const vbl_array_2d<T> & in, vbl_array_2d<T> & minima, T thresh)
{
...
template <class T>
bool
local_minima(const vbl_array_3d<T> & in, vbl_array_3d<T> & minima, T thresh)
{
  const unsigned int n1 = (unsigned int)(in.get_row1_count()), n2 = (unsigned int)(in.get_row2_count()),
                     n3 = (unsigned int)(in.get_row3_count());
  assert(n3 == minima.get_row3_count() && n2 == minima.get_row2_count() && n1 == minima.get_row1_count());
  // special case
  ...
  if (n3 > 2 && n2 > 2)
  { // top and bottom faces of the array vary x3, x2
    // top face
    x1 = 0;
    for (x3 = 1; x3 < n3 - 1; ++x3)
      for (x2 = 1; x2 < n2 - 1; ++x2)
  ...
  if (n3 > 2 && n1 > 2)
  { // front and back faces of the array vary x3, x1
    // front face
    x2 = 0;
    for (x3 = 1; x3 < n3 - 1; ++x3)
      for (x1 = 1; x1 < n1 - 1; ++x1)
  ...
  if (n2 > 2 && n1 > 2)
  { // left and right faces of the array vary x2, x1
    // left face
    x3 = 0;
    for (x2 = 1; x2 < n2 - 1; ++x2)
      for (x1 = 1; x1 < n1 - 1; ++x1)
