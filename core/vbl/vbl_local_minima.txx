...
{
  unsigned nr = in.rows(),nc = in.cols();
  unsigned mnr = in.rows(),mnc = in.cols();
  assert(nr==mnr&&nc==mnc);
  bool  minima_found = false;
...
bool local_minima(vbl_array_3d<T> const& in, vbl_array_3d<T>& minima, T thresh)
{
  ...
  T v, d, mind;
  T mval = vcl_numeric_limits<T>::max();
  int n1 = in.get_row1_count(),n2 = in.get_row2_count(),
    n3 = in.get_row3_count();
  int mn3 = minima.get_row3_count();
  int mn2 = minima.get_row2_count();
  int mn1 = minima.get_row1_count();
  assert(n3==mn3&&n2==mn2&&n1==mn1);
...
template bool local_minima(vbl_array_1d<T> const& in,vbl_array_1d<T>& minima, \
                           T thresh); \
template bool local_minima(vbl_array_2d<T> const& in,vbl_array_2d<T>& minima, \
                           T thresh); \
template bool local_minima(vbl_array_3d<T> const& in,vbl_array_3d<T>& minima, \
                           T thresh);
