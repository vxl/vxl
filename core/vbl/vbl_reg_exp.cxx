...
// private data.  The vbl_reg_exp class  provides a convenient  way  to  represent
...
// only want to find a match at the beginning or end of  a String.  The vbl_reg_exp
...
//
// There are three constructors for vbl_reg_exp.  One  just creates an empty vbl_reg_exp
// object.  Another creates a vbl_reg_exp object  and initializes it with a regular
// expression  that is given  in  the form of a   char*.   The  third  takes a
// reference  to  a vbl_reg_exp  object    as an  argument    and creates an object
// initialized with the information from the given vbl_reg_exp object.
//
...
void vbl_reg_exp::compile (char const* exp) {
    ...
    if (exp == NULL) {
      //RAISE Error, SYM(vbl_reg_exp), SYM(No_Expr),
      printf ("vbl_reg_exp::compile(): No expression supplied.\n");
      return;
    }
    ...
    if(!reg(0, &flags))
      {
        printf ("vbl_reg_exp::compile(): Error in compile.\n");
        return;
      }
