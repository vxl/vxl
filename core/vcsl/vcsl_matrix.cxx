...
//---------------------------------------------------------------------------
void
vcsl_matrix::set_static(const vcsl_matrix_param_sptr & new_matrix)
{
  matrix_.clear();
  matrix_.push_back(new_matrix);
...
//---------------------------------------------------------------------------
vnl_vector<double>
vcsl_matrix::execute(const vnl_vector<double> & v, double time) const
{
  // require
  assert(is_valid());
  assert(v.size() == 3);
...
//---------------------------------------------------------------------------
vnl_vector<double>
vcsl_matrix::inverse(const vnl_vector<double> & v, double time) const
{
  assert(is_valid());
  assert(v.size() == 3);
...
vnl_matrix<double>
vcsl_matrix::matrix_value(double time, bool type) const
{
...
vnl_matrix<double>
vcsl_matrix::param_to_matrix(const vcsl_matrix_param_sptr & from, bool type) const
{
  int coef = 1;
  if (type)
    coef = -1;
  ...
  R(0, 2) = -co * sp * ck + so * sk;
  R(1, 0) = -cp * sk;
  R(1, 1) = -so * sp * sk + co * ck;
  R(1, 2) = co * sp * sk + so * ck;
  R(2, 0) = sp;
  R(2, 1) = -so * cp;
  R(2, 2) = co * cp;
