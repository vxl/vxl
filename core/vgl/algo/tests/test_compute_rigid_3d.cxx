...
void
add_noise(std::vector<vgl_point_3d<double>> & points, double sigma)
{
  vnl_random r;
...
double
alignment_error(const std::vector<vgl_point_3d<double>> & points1, const std::vector<vgl_point_3d<double>> & points2)
{
  double error = 0.0;
...
test_compute_rigid_3d()
{
  ...
  TEST_NEAR("rotation estimate", (est_sim.rotation().as_matrix() - R.as_matrix()).array_inf_norm(), 0.0, 1e-8);
  TEST_NEAR("RMS alignment error",
            vgl_test_compute_rigid_3d::alignment_error(
              vgl_test_compute_rigid_3d::transform_points(points1, est_sim.rotation(), est_sim.translation()), points2),
            0.0,
            1e-8);
  ...
  TEST_NEAR("noisy rotation estimate", (est_sim2.rotation().as_matrix() - R.as_matrix()).array_inf_norm(), 0.0, sigma);
  TEST_NEAR(
    "RMS alignment error",
    vgl_test_compute_rigid_3d::alignment_error(
      vgl_test_compute_rigid_3d::transform_points(points1, est_sim2.rotation(), est_sim2.translation()), points2),
    0.0,
    10 * sigma);
}
