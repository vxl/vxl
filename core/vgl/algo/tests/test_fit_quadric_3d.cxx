...
test_linear_fit_sphere_taubin()
{
  ...
  const vgl_point_3d<double> p0(1.0, 0.0, 0.0);
  const vgl_point_3d<double> p1(0.0, 1.0, 0.0);
  const vgl_point_3d<double> p2(0.0, 0.0, 1.0);
  const vgl_point_3d<double> p3(-1.0, 0.0, 0.0);
  const vgl_point_3d<double> p4(0.0, -1.0, 0.0);
  const vgl_point_3d<double> p5(0.0, 0.0, -1.0);
  const double r = 0.5773503;
  const vgl_point_3d<double> p6(r, r, r);
  const vgl_point_3d<double> p7(-r, -r, -r);
  const vgl_point_3d<double> p8(-r, -r, r);
  const vgl_point_3d<double> p9(-r, r, -r);
  const vgl_point_3d<double> p10(r, -r, -r);
  const vgl_point_3d<double> p11(r, r, -r);
  const vgl_point_3d<double> p12(r, -r, r);
  const vgl_point_3d<double> p13(-r, r, r);
  pts.push_back(p0);
  pts.push_back(p1);
  pts.push_back(p2);
  pts.push_back(p3);
  ...
  const double error = fit_sph.fit_linear_Taubin(&std::cout);
...
test_linear_fit_taubin_sphere_rand_error()
{
  ...
  for (auto & pt : pts)
  {
    const vgl_vector_3d<double> er(ran.drand32(-e, e), ran.drand32(-e, e), ran.drand32(-e, e));
    pt = pt + er;
  }
  vgl_fit_quadric_3d<double> fit_sph(pts);
  const double error = fit_sph.fit_linear_Taubin(&std::cout);
...
test_linear_fit_Taubin_parabolid()
{
  ...
  const double th = 1.047197551;
  // double th = 0.0;
  const vgl_vector_3d<double> off(0.1, 0.2, 0.3);
  ...
  {
    const vgl_point_3d<double> p(RP[0][i], RP[1][i], RP[2][i]);
    pts.push_back(p + off);
    std::cout << p + off << std::endl;
  }
  vgl_fit_quadric_3d<double> fit_parab(pts);
  const double lin_error = fit_parab.fit_linear_Taubin(&std::cout);

  const vgl_quadric_3d<double> q = fit_parab.quadric_Taubin_fit();
  ...
  const vnl_svd<double> svd(T);
  const vnl_matrix<double> Hi = svd.inverse();
...
test_linear_fit_Taubin_parabolid_rand_error()
{
  ...
  std::vector<vgl_point_3d<double>> pts;
  const vgl_point_3d<double> p0(1.0, 1.0, 2.0);
  const vgl_point_3d<double> p1(2.0, 2.0, 8.0);
  const vgl_point_3d<double> p2(3.0, 3.0, 18.0);
  const vgl_point_3d<double> p3(-1.0, -1.0, 2.0);
  const vgl_point_3d<double> p4(-2.0, -2.0, 8.0);
  const vgl_point_3d<double> p5(-3.0, -3.0, 18.0);
  const vgl_point_3d<double> p6(1.0, -1.0, 2.0);
  const vgl_point_3d<double> p7(2.0, -2.0, 8.0);
  const vgl_point_3d<double> p8(3.0, -3.0, 18.0);
  const vgl_point_3d<double> p9(-1.0, 1.0, 2.0);
  const vgl_point_3d<double> p10(-2.0, 2.0, 8.0);
  const vgl_point_3d<double> p11(-3.0, 3.0, 18.0);
  pts.push_back(p0);
  pts.push_back(p1);
  pts.push_back(p2);
  pts.push_back(p3);
...
test_linear_fit_sphere_Taubin_actual_pts()
{
  ...
  std::vector<vgl_point_3d<double>> pts;
  const vgl_point_3d<double> p0(-10.356499671936, 68.164497375488, 48.227199554443);
  const vgl_point_3d<double> p1(-12.030300140381, 68.351600646973, 46.570499420166);
  const vgl_point_3d<double> p2(-12.924599647522, 70.128097534180, 46.037700653076);
  const vgl_point_3d<double> p3(-12.602700233459, 72.161201477051, 47.159500122070);
  const vgl_point_3d<double> p4(-11.404299736023, 75.049201965332, 49.112400054932);
  const vgl_point_3d<double> p5(-9.647660255432, 71.332901000977, 49.860500335693);
  const vgl_point_3d<double> p6(-13.605299949646, 72.556297302246, 46.091999053955);
  const vgl_point_3d<double> p7(-15.126000404358, 71.693496704102, 43.016601562500);
  const vgl_point_3d<double> p8(-16.063600540161, 70.053901672363, 41.029098510742);
  const vgl_point_3d<double> p9(-13.483099937439, 68.981697082520, 44.901100158691);
  const vgl_point_3d<double> p10(-9.131649971008, 69.801300048828, 49.823600769043);
  const vgl_point_3d<double> p11(-8.480529785156, 67.906501770020, 50.200801849365);
  const vgl_point_3d<double> p12(4.875539779663, 71.071899414063, 50.690101623535);
  const vgl_point_3d<double> p13(-3.617100000381, 67.346603393555, 51.243801116943);
  const vgl_point_3d<double> p14(-4.381750106812, 68.257698059082, 51.418098449707);
  const vgl_point_3d<double> p15(-5.963240146637, 68.928100585938, 51.265399932861);
  const vgl_point_3d<double> p16(3.054660081863, 68.043899536133, 51.342098236084);
  const vgl_point_3d<double> p17(-11.062999725342, 72.394500732422, 48.930099487305);
  const vgl_point_3d<double> p18(-11.003100395203, 70.369201660156, 48.363498687744);
  const vgl_point_3d<double> p19(5.466730117798, 69.970596313477, 50.311599731445);
  const vgl_point_3d<double> p20(-11.655500411987, 71.570999145508, 48.103099822998);
  const vgl_point_3d<double> p21(-10.344300270081, 72.406997680664, 49.586498260498);
  const vgl_point_3d<double> p22(-11.970600128174, 72.936203002930, 48.114101409912);
  const vgl_point_3d<double> p23(-13.019499778748, 73.698402404785, 47.480598449707);
  const vgl_point_3d<double> p24(4.039299964905, 69.958900451660, 50.850700378418);
  pts.push_back(p0);
  pts.push_back(p1);
  pts.push_back(p2);
  pts.push_back(p3);
  ...
  const double lin_error = fit_sph_act.fit_linear_Taubin(&std::cout);
...
test_linear_fit_sphere_Alliare()
{
  ...
  pts.push_back(p13);
  vgl_fit_quadric_3d<double> fit_sph(pts);
  const double error = fit_sph.fit_ellipsoid_linear_Allaire(&std::cout);
...
test_linear_fit_Allaire_thin_ellipsoid_rand_error()
{
  ...
  const double th = 1.047197551;
  const vgl_vector_3d<double> off(1.0, 2.0, 3.0);
  double c = cos(th), s = sin(th);
  vnl_matrix<double> R(3, 3), RP;
  R[0][0] = c;
  R[0][1] = 0.0;
  ...
  vgl_fit_quadric_3d<double> fit_sph(pts);
  const double error = fit_sph.fit_ellipsoid_linear_Allaire(&std::cout);
  const vgl_quadric_3d<double> & q = fit_sph.quadric_Allaire_fit();
  ...
  const vnl_svd<double> svd(Hf);
  const vnl_matrix<double> Hr = svd.inverse();
...
test_linear_fit_Allaire_saddle_shape()
{
  ...
  const vgl_point_3d<double> p0(1.0, 0.0, 0.0);
  const vgl_point_3d<double> p1(0.0, 1.0, 0.0);
  const vgl_point_3d<double> p2(-1.0, 0.0, 0.0);
  const vgl_point_3d<double> p3(0.0, -1.0, 0.0);
  const vgl_point_3d<double> p4(2, 0.0, 1.0);
  const vgl_point_3d<double> p5(0.0, 2.0, 1.0);
  const vgl_point_3d<double> p6(-2.0, 0.0, 1.0);
  const vgl_point_3d<double> p7(0.0, -2.0, 1.0);
  const vgl_point_3d<double> p8(2, 0.0, -1.0);
  const vgl_point_3d<double> p9(0.0, 2.0, -1.0);
  const vgl_point_3d<double> p10(-2.0, 0.0, -1.0);
  const vgl_point_3d<double> p11(0.0, -2.0, -1.0);
  ...
  const double error = fit_sad.fit_saddle_shaped_quadric_linear_Allaire(&std::cout);
  const vgl_quadric_3d<double> & q = fit_sad.quadric_Allaire_fit();
  std::cout << "Fit error = " << error << " Quadric type " << q.type_by_number(q.type()) << std::endl;
  std::vector<double> diag;
  std::vector<std::vector<double>> H;
  q.canonical_central_quadric(diag, H);
...
test_linear_fit_Allaire_ellipsoid_actual_pts()
{
  ...
  std::vector<vgl_point_3d<double>> pts;
  const vgl_point_3d<double> p0(11.25000000, 222.75000000, 27.50236130);
  const vgl_point_3d<double> p1(11.55000019, 222.75000000, 27.50236130);
  const vgl_point_3d<double> p2(19.35000038, 220.05000305, 31.17736053);
  const vgl_point_3d<double> p3(19.64999962, 220.05000305, 31.40236092);
  const vgl_point_3d<double> p4(18.45000076, 217.64999390, 29.75236130);
  const vgl_point_3d<double> p5(24.75000000, 214.94999695, 25.25235939);
  const vgl_point_3d<double> p6(23.85000038, 212.55000305, 24.05236053);
  const vgl_point_3d<double> p7(24.14999962, 212.55000305, 25.10235977);
  const vgl_point_3d<double> p8(16.95000076, 211.35000610, 24.57736015);
  const vgl_point_3d<double> p9(17.25000000, 211.35000610, 25.55236053);
  const vgl_point_3d<double> p10(19.04999924, 211.35000610, 23.30236053);
  const vgl_point_3d<double> p11(19.35000038, 211.35000610, 22.10235977);
  const vgl_point_3d<double> p12(21.14999962, 211.35000610, 21.65236092);
  const vgl_point_3d<double> p13(21.45000076, 211.35000610, 21.65236092);
  const vgl_point_3d<double> p14(23.25000000, 211.35000610, 23.45236015);
  const vgl_point_3d<double> p15(23.54999924, 211.35000610, 24.12736130);
  const vgl_point_3d<double> p16(25.35000038, 211.35000610, 27.05236053);
  const vgl_point_3d<double> p17(25.64999962, 211.35000610, 27.05236053);
  const vgl_point_3d<double> p18(27.45000076, 211.35000610, 27.05236053);
  pts.push_back(p0);
  pts.push_back(p1);
  pts.push_back(p2);
  pts.push_back(p3);
  ...
  const double error = fit_ellp.fit_ellipsoid_linear_Allaire(&std::cout);
  if (error != -1.0)
  {
    const vgl_quadric_3d<double> & q = fit_ellp.quadric_Allaire_fit();
    std::cout << "Fit error = " << error << " Quadric type " << q.type_by_number(q.type()) << std::endl;
    std::vector<double> diag;
    std::vector<std::vector<double>> H;
    q.canonical_central_quadric(diag, H);
...
test_linear_fit_xy_paraboloid()
{
  ...
  const double zerr = fxy.fit_linear(&std::cout);
  const std::string type = fxy.quadric_type();
  ...
  const vgl_point_2d<double> etr = fxy.extremum_point();
  const vgl_point_2d<double> actual_extr(-0.42857, -0.14285);
  const double extr_error = (etr - actual_extr).length();
...
test_linear_paraboloid_actual_cost_surface()
{
  ...
  const vgl_point_2d<double> etr = fxy.extremum_point();
  const vgl_point_2d<double> actual_extr(0.63, 0.345);
  const double extr_error = (etr - actual_extr).length();
