...
test_hough_index_2d()
{
  ...
  const vgl_point_2d<double> p0(0, 0);
  const vgl_point_2d<double> p1(100, 100);
  const vgl_point_2d<double> p2(100, 200);
  const vgl_point_2d<double> p3(200, 300);
  const vgl_point_2d<double> p4(300, 300);
  const vgl_line_segment_2d<double> l(p0, p1);
  const vgl_line_segment_2d<double> l1(p0, p2);
  const vgl_line_segment_2d<double> l3(p2, p3);
  const vgl_line_segment_2d<double> l4(p1, p4);
  const bool inserted = hl.insert(l);
  bool found_l = hl.find(l);
  const bool found_l1 = hl.find(l1);
  std::cout << "indexed =" << inserted << '\n' << "found_l = " << found_l << "found_l1 " << found_l1 << '\n';
  TEST("...", inserted && found_l && !found_l1, true);
  const bool remove_l = hl.remove(l);
  found_l = hl.find(l);
  ...
  hl.insert(l3);
  const std::vector<vgl_line_segment_2d<double>> parallel = hl.parallel_lines(45, 0.5);
  const std::vector<vgl_line_segment_2d<double>> non_parallel = hl.parallel_lines(40, 0.5);
  ...
  const vgl_point_2d<double> p0_cut(256, 256);
  const vgl_point_2d<double> p1_cut(356, (256 - 1.74550));
  const vgl_point_2d<double> p2_cut(356, (256 + 1.74550));

  const vgl_line_segment_2d<double> l0_cut(p0_cut, p1_cut);
  const vgl_line_segment_2d<double> l1_cut(p0_cut, p2_cut);
  h_cut.insert(l0_cut);
  h_cut.insert(l1_cut);
