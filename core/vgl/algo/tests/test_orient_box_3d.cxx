...
test_contains()
{
  ...
  const vgl_point_3d<double> p2(1, 1, 1);
  vgl_box_3d<double> abox;
  abox.add(p1);
  abox.add(p2);
  const vnl_double_3 v(1.0, 0, 0);
  const vnl_quaternion<double> q(v, 0.25 * vnl_math::pi);
  const vgl_orient_box_3d<double> obox(abox, q);
  std::cout << "oriented box\n " << obox << '\n' << "Corners\n";
  const std::vector<vgl_point_3d<double>> c = obox.corners();
  for (auto & pit : c)
    std::cout << pit << '\n';
  const vgl_point_3d<double> tp(0.5, 0.4, -0.1);
  const bool inside = obox.contains(tp);
  const bool outside = !(obox.contains(1.0, 1.0, 1.0));
...
test_construct()
{
  ...
  vgl_point_3d<double> p0(1, 2, 3), px(1, 6, 0), py(1, 5, 7), pz(8, 2, 3);
  const vgl_orient_box_3d<double> obox(p0, px, py, pz);
