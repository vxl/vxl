...
static double
d(double x0, double x1, double y0, double y1)
{
  return std::sqrt((x0 - x1) * (x0 - x1) + (y0 - y1) * (y0 - y1));
}

static bool
equals(const double x[12], const double y[12])
{
  for (int i = 0; i < 12; ++i)
    if (x[i] != y[i])
      return false;
...
test_accessors()
{
  ...
  P.get(M1);
  TEST("get(vnl_double_3x4)", M1, M);
  P.get(M2);
  TEST("get(vnl_matrix)", M2, m);
  vnl_vector<double> v1, v2, v3;
  P.get_rows(&v1, &v2, &v3);
  TEST("get_rows(vnl_vector 3x)",
       v1 == vnl_double_4(1, 0, 0, 0) && v2 == vnl_double_4(0, 0.6, 0.8, 0) && v3 == vnl_double_4(0, -0.8, 0.6, 0),
       true);
  vnl_double_4 d1, d2, d3;
  P.get_rows(&d1, &d2, &d3);
  TEST("get_rows(vnl_double_4 3x)", d1 == v1 && d2 == v2 && d3 == v3, true);
  std::stringstream ss;
  ss << P;
  TEST("operator<< to ostream", P, vgl_p_matrix<double>(ss));
  TEST_NEAR("operator()(point projection)",
            vgl_distance(P(vgl_homg_point_3d<double>(1.0, 2.0, 0.0, 0.0)), vgl_homg_point_2d<double>(1.0, 1.2, -1.6)),
            0,
            1e-10);
