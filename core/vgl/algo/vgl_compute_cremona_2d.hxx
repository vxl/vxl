...
bool vgl_compute_cremona_2d<T, deg>::normalize(){
...
template <class T, size_t deg>
bool vgl_compute_cremona_2d<T, deg>::project_linear(T X, T Y, T& x, T& y) const{
  if(!linear_solved_){
...
template <class T, size_t deg>
bool vgl_compute_cremona_2d<T, deg>::compute_linear_solution_error(){
  if(!linear_solved_){
   std::cerr << "no linear solution available" << std::endl;
    return false;
  } 
...
#define VGL_COMPUTE_CREMONA_2D_INSTANTIATE(T, deg) \
  template class vgl_compute_cremona_2d<T, deg>
