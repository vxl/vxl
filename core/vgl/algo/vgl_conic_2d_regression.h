...
//
//  The reqression uses the Bookstein algorithm which constrains the 
...
//                S21   S22
//
//      The conic coefficents are v1 = {a, b, c}^t and v2 = {d, e, f}^t
//      The Bookstein constraint is expressed by the diagonal matrix 
//               D = Diag{1, 0.5, 1, 0, 0, 0}
//      The Lagrangian to be minimized is
//
//  L =  v1^t S11 v1  + 2 v2^t S21 v1  + v2^t S22 v2 - lambda(v1^t D v1 -2)
//
//     Minimizing with respect to v2 gives,
//
//      dL/dv2 = 2 S21 v1 + 2 S22 v2 = 0
//
//    So,   v2 = -S22^-1 S21 v1
//
//    Substituting for v2 in L,
//
//   L = v1^t ( S11 - S12 * S22^-1 * S21) v1 - lambda(v1^t D v1 -2)
//     = v1^t ( (S11 - S12 * S22^-1 * S21) - lambda D ) v1 - 2 lambda

//   dL/dv1 =  ( (S11 - S12 * S22^-1 * S21) - lambda D ) v1 = 0
//
//   So,  lambda v1 = D^-1 (S11 - S12 * S22^-1 * S21) v1
//  
...
class vgl_conic_2d_regression
{
 public:
  ...
  //: get the estimated Euclidean error with respect to the 
  //  fitted conic segment in the original frame
  ...
  vgl_conic<T> conic() {return conic_;}
 ...
 protected:
  ...
  //: The algebraic fitting cost in the normalized framex
  T cost_;
