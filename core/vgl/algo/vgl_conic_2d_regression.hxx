...
template <class T>
void
vgl_conic_2d_regression<T>::init()
{
...
template <class T>
void
vgl_conic_2d_regression<T>::clear_points()
{
  points_.clear();
  npts_ = 0;
}
...
template <class T>
T
vgl_conic_2d_regression<T>::get_rms_error_est(const vgl_point_2d<T> & p) const
{
...
template <class T>
void
vgl_conic_2d_regression<T>::set_sampson_error(T a, T b, T c, T d, T e, T f)
{
  T sum = 0;
...
template <class T>
void
vgl_conic_2d_regression<T>::add_point(const vgl_point_2d<T> & p)
{
  points_.push_back(p);
  ++npts_;
}
...
template <class T>
void
vgl_conic_2d_regression<T>::remove_point(const vgl_point_2d<T> & p)
{
  // remove warnings on implicit typename
...
template <class T>
T
vgl_conic_2d_regression<T>::get_rms_algebraic_error() const
{
  return cost_;
}
...
template <class T>
void
vgl_conic_2d_regression<T>::compute_partial_sums()
{
  hnorm_points_.clear();
  ...
  for (typename std::vector<vgl_homg_point_2d<T>>::iterator pit = hpoints.begin(); pit != hpoints.end(); ++pit)
    hnorm_points_.push_back(trans_(*pit));

  for (typename std::vector<T>::iterator dit = partial_sums_.begin(); dit != partial_sums_.end(); ++dit)
    (*dit) = 0;

  T x2, y2, x3, y3;
  for (typename std::vector<vgl_homg_point_2d<T>>::iterator pit = hnorm_points_.begin(); pit != hnorm_points_.end();
       ++pit)
  {
    T x = pit->x() / pit->w(), y = pit->y() / pit->w();

    x2 = x * x;
    x3 = x2 * x;
    y2 = y * y;
    y3 = y2 * y;
    partial_sums_[0] += x3 * x;
    partial_sums_[1] += x3 * y;
    partial_sums_[2] += x2 * y2;
    partial_sums_[3] += x * y3;
    partial_sums_[4] += y3 * y;
    partial_sums_[5] += x3;
    partial_sums_[6] += x2 * y;
    partial_sums_[7] += x * y2;
    partial_sums_[8] += y3;
    partial_sums_[9] += x2;
    partial_sums_[10] += x * y;
    partial_sums_[11] += y2;
    partial_sums_[12] += x;
    partial_sums_[13] += y;
  }
...
template <class T>
void
vgl_conic_2d_regression<T>::fill_scatter_matrix()
{
...
vgl_conic_2d_regression<T>::fit()
{
  ...
  vnl_matrix_fixed<T, 3, 3> S_lambda = Dinv_ * (S11_ - S12_ * (vnl_inverse(S22_) * S12_T));
  ...
  this->set_sampson_error(conic_.a(), conic_.b(), conic_.c(), conic_.d(), conic_.e(), conic_.f());
...
template <class T>
void
vgl_conic_2d_regression<T>::print_pointset(std::ostream & str)
{
