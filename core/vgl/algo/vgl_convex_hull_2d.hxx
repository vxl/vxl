...
template <class T>
vgl_convex_hull_2d<T>::vgl_convex_hull_2d(const std::vector<vgl_point_2d<T>> & points)
{
  hull_valid_ = false;
  points_ = points;
}
...
template <class T>
void
vgl_convex_hull_2d<T>::compute_hull()
{
...
vgl_convex_hull_2d<T>::min_area_enclosing_rectangle()
{
  if (!hull_valid_)
    this->compute_hull();
  ...
  if (w < h)
  {
    width = h;
    height = w;
    pmaj0.set(c.x(), c.y() - width / T(2));
    pmaj1.set(c.x(), c.y() + width / T(2));
  }
