...
class vgl_ellipsoid_3d
{
 ...
 public:
  ...
  vgl_ellipsoid_3d(vgl_point_3d<T> const& center,
                   T x_halflength,
                   T y_halflength,
                   T z_halflength,
                   vgl_rotation_3d<T> const& orientation = vgl_rotation_3d<T>())
  : center_(center), x_halflength_(x_halflength), y_halflength_(y_halflength), z_halflength_(z_halflength), orientation_(orientation) {}
  ...
  void set_center(vgl_point_3d<T> const& center) { center_ = center; }
  //: sets the rotation that would create this ellipsoid from an axes-aligned one
  void set_orientation(vgl_rotation_3d<T> orientation) { orientation_ = orientation; }
