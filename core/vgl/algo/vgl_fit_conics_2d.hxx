...
//: Constructor
template <class T>
vgl_fit_conics_2d<T>::vgl_fit_conics_2d(const unsigned min_length, const T tol)
...
template <class T>
void
vgl_fit_conics_2d<T>::add_point(const vgl_point_2d<T> & p)
{
  curve_.push_back(p);
#ifdef DEBUG
  std::cout << p << '\n';
...
template <class T>
void
vgl_fit_conics_2d<T>::add_point(T x, T y)
{
...
template <class T>
void
vgl_fit_conics_2d<T>::output(const unsigned start_index, const unsigned end_index, const vgl_conic<T> & conic)
{
...
template <class T>
bool
vgl_fit_conics_2d<T>::fit()
{
  if (curve_.size() < min_length_)
  {
    std::cout << "In vgl_fit_conics_2d<T>::fit() - number of points < min_length " << min_length_ << '\n';
    return false;
  }
    ...
    if (reg.fit() && reg.get_rms_sampson_error() < tol_)
    {
...
#ifdef DEBUG
      std::cout << "Initial fit error " << reg.get_rms_sampson_error() << " for\n" << reg.conic() << '\n';
#endif
      ...
      while (nf < cur_len && below_error_tol)
      {
        ...
        if (below_error_tol)
        {
          ...
          nf++;
#ifdef DEBUG
          std::cout << "Adding point " << p << "with estimated error " << error << '\n';
#endif
        }
      }
