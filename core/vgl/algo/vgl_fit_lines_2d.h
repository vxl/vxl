...
public:
  ...
  //: set parameters
  void
  set_min_fit_length(unsigned int min_fit_length)
  {
    min_length_ = min_fit_length;
  }
  void
  set_rms_error_tol(T rms_error_tol)
  {
    tol_ = rms_error_tol;
  }
  ...
  get_indices()
  {
    return curve_indices_;
  }

protected:
  //: output a line that fits from start to end
  void
  output(unsigned int start_index, unsigned int end_index);
};
