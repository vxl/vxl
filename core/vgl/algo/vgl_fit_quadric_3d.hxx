...
template <class T>
void
vgl_fit_quadric_3d<T>::add_point(const vgl_point_3d<T> & p)
{
...
template <class T>
void
vgl_fit_quadric_3d<T>::add_point(const T x, const T y, const T z)
{
...
vgl_fit_quadric_3d<T>::get_points() const
{
...
template <class T>
T
vgl_fit_quadric_3d<T>::fit_linear_Taubin(std::ostream * errstream)
{
  ...
  if (debug)
    std::cout << quadric_Taubin_.type_by_number(quadric_Taubin_.type()) << std::endl;
...
template <class T>
T
vgl_fit_quadric_3d<T>::fit_ellipsoid_linear_Allaire(std::ostream * errstream)
{
  size_t n = points_.size();
  // actually the minimum number is 9 but require at least 10 to get some amount of regularization
  if (n < 10)
  ...
  if (debug)
    std::cout << "Best ellipsoid is " << min_index << std::endl;
  for (size_t r = 0; r < 10; ++r)
...
template <class T>
T
vgl_fit_quadric_3d<T>::fit_saddle_shaped_quadric_linear_Allaire(std::ostream * errstream)
{
  size_t n = points_.size();
  // actually the minimum number is 9 but require at least 10 to get some amount of regularization
  if (n < 10)
  ...
  vnl_generalized_eigensystem geign(N, M);
  if (debug)
    std::cout << geign.D << std::endl;
  if (debug)
    std::cout << geign.V << std::endl;
  ...
  for (size_t i = 0; i < 5; ++i)
  {
    ...
    if (debug)
      std::cout << "Error =" << e << "for q[" << i << "]\n" << q << std::endl;
    if (debug)
      std::cout << "Type " << quadric_Allaire_.type_by_number(quadric_Allaire_.type()) << std::endl;
    if (e < min_error)
    {
      min_index = i;
      min_error = e;
  ...
  if (debug)
    std::cout << "Best saddle shape is " << min_index << std::endl;
  for (size_t r = 0; r < 10; ++r)
