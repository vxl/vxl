...
{
public:
  sphere_residual_function(const std::vector<vgl_homg_point_3d<double>> & pts)
    : vnl_least_squares_function(4, static_cast<unsigned>(pts.size()), vnl_least_squares_function::use_gradient)
    , pts_(pts)
  {}
  //
  // x = [x0, y0, z0, r]^t
...
template <class T>
void
vgl_fit_sphere_3d<T>::add_point(const vgl_point_3d<T> & p)
{
...
template <class T>
void
vgl_fit_sphere_3d<T>::add_point(const T x, const T y, const T z)
{
...
template <class T>
T
vgl_fit_sphere_3d<T>::fit_linear(std::ostream * errstream)
{
...
template <class T>
T
vgl_fit_sphere_3d<T>::fit(std::ostream * outstream, bool verbose)
{
  ...
  x_init[0] = static_cast<double>(lin_x0);
  x_init[1] = static_cast<double>(lin_y0);
  x_init[2] = static_cast<double>(lin_z0);
  x_init[3] = static_cast<double>(lin_radius);
  ...
  if ((code == vnl_nonlinear_minimizer::CONVERGED_FTOL || code == vnl_nonlinear_minimizer::CONVERGED_XTOL ||
       code == vnl_nonlinear_minimizer::CONVERGED_XFTOL || code == vnl_nonlinear_minimizer::CONVERGED_GTOL))
  {
  ...
  { // non linear optimize failed -- use linear fit result
...
vgl_fit_sphere_3d<T>::get_points() const
{
...
template <class T>
bool
vgl_fit_sphere_3d<T>::fit_linear(const T error_marg, std::ostream * outstream)
{
...
template <class T>
bool
vgl_fit_sphere_3d<T>::fit(const T error_marg, std::ostream * outstream, bool verbose)
{
