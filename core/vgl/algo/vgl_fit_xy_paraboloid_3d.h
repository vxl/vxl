...
class vgl_fit_xy_paraboloid_3d
{
 ...
 public:
  ...
  T fit_linear(std::ostream* errstream);
  ...
  vgl_point_2d<T> extremum_point() const;

  //: the string type name  should be "elliptic_paraboloid"
  std::string quadric_type() const {return paraboloid_linear_.type_by_number(paraboloid_linear_.type());}
};
