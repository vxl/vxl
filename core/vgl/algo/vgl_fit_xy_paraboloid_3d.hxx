...
template <class T>
void vgl_fit_xy_paraboloid_3d<T>::add_point(vgl_point_3d<T> const &p)
{
  points_.push_back(vgl_homg_point_3d<T>(p));
}

template <class T>
void vgl_fit_xy_paraboloid_3d<T>::add_point(const T x, const T y, const T z)
{
  points_.push_back(vgl_homg_point_3d<T>(x, y, z));
}

 template <class T>
 void vgl_fit_xy_paraboloid_3d<T>::clear()
{
  points_.clear();
}

template <class T>
T vgl_fit_xy_paraboloid_3d<T>::fit_linear(std::ostream* errstream)
{
  ...
  for (size_t i=0; i<n; i++) {
    ...
    M[0][0]+= x*x*x*x; M[0][1]+=x*x*x*y; M[0][2]+=x*x*y*y; M[0][3]+=x*x*x; M[0][4]+=x*x*y; M[0][5]+=x*x;
    M[1][0]+= x*x*x*y; M[1][1]+=x*x*y*y; M[1][2]+=x*y*y*y; M[1][3]+=x*x*y; M[1][4]+=x*y*y; M[1][5]+=x*y;
    M[2][0]+= x*x*y*y; M[2][1]+=x*y*y*y; M[2][2]+=y*y*y*y; M[2][3]+=x*y*y; M[2][4]+=y*y*y; M[2][5]+=y*y;
    M[3][0]+= x*x*x;   M[3][1]+=x*x*y;   M[3][2]+=x*y*y;   M[3][3]+=x*x;   M[3][4]+=x*y;   M[3][5]+=x;
    M[4][0]+= x*x*y;   M[4][1]+=x*y*y;   M[4][2]+=y*y*y;   M[4][3]+=x*y;   M[4][4]+=y*y;   M[4][5]+=y;
    M[5][0]+= x*x;     M[5][1]+=x*y;     M[5][2]+=y*y;     M[5][3]+=x;     M[5][4]+=y;     M[5][5]+=1.0;
    b[0][0] += z*x*x;  b[1][0] += z*x*y; b[2][0] += z*y*y; b[3][0] += z*x; b[4][0] += z*y; b[5][0] += z;
  }
  M/=n; b/=n;
  ...
  p_[0][0] = p[0][0]*s;  p_[1][0] = p[1][0]*s; p_[2][0] = p[2][0]*s;
...
template <class T>
T  vgl_fit_xy_paraboloid_3d<T>::z(T x, T y) const{
  vnl_matrix<T> q(6,1);
  q[0][0] = x*x; q[1][0] = x*y; q[2][0] = y*y;
  q[3][0] = x; q[4][0] = y; q[5][0] = 1;
  T v = 0;
...
template <class T>
vgl_point_2d<T> vgl_fit_xy_paraboloid_3d<T>::extremum_point() const{
  ...
  pinv[0][0] = 2.0 * p_[2][0]; pinv[0][1] = -p_[1][0];
  pinv[1][0] = -p_[1][0]; pinv[1][1] = 2.0 * p_[0][0];
  pinv /= det;
...
template class vgl_fit_xy_paraboloid_3d<T >
