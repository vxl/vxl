...
template <class T>
vgl_h_matrix_1d<T>::vgl_h_matrix_1d(const std::vector<vgl_homg_point_1d<T>> & points1,
                                    const std::vector<vgl_homg_point_1d<T>> & points2)
{
  vnl_matrix<T> W;
...
template <class T>
vgl_homg_point_1d<T>
vgl_h_matrix_1d<T>::operator()(const vgl_homg_point_1d<T> & p) const
{
...
template <class T>
vgl_homg_point_1d<T>
vgl_h_matrix_1d<T>::preimage(const vgl_homg_point_1d<T> & q) const
{
...
template <class T>
bool
vgl_h_matrix_1d<T>::read(std::istream & s)
{
...
template <class T>
vgl_h_matrix_1d<T>::vgl_h_matrix_1d(const char * filename)
{
...
template <class T>
bool
vgl_h_matrix_1d<T>::read(const char * filename)
{
  std::ifstream f(filename);
  if (!f.good())
    std::cerr << "vgl_h_matrix_1d::read: Error opening " << filename << std::endl;
...
template <class T>
T
vgl_h_matrix_1d<T>::get(unsigned int row_index, unsigned int col_index) const
{
...
template <class T>
void
vgl_h_matrix_1d<T>::get(T * H) const
{
...
template <class T>
void
vgl_h_matrix_1d<T>::get(vnl_matrix_fixed<T, 2, 2> * H) const
{
  *H = t12_matrix_;
}
...
template <class T>
vgl_h_matrix_1d<T> &
vgl_h_matrix_1d<T>::set(const vnl_matrix_fixed<T, 2, 2> & H)
{
  t12_matrix_ = H;
  return *this;
}
...
template <class T>
vgl_h_matrix_1d<T> &
vgl_h_matrix_1d<T>::set_affine(const vnl_matrix_fixed<T, 1, 2> & M12)
{
...
template <class T>
bool
vgl_h_matrix_1d<T>::projective_basis(const std::vector<vgl_homg_point_1d<T>> & points)
{
  if (points.size() != 3)
    return false;
...
template <class T>
bool
vgl_h_matrix_1d<T>::is_identity() const
{
...
template <class T>
bool
vgl_h_matrix_1d<T>::is_rotation() const
{
...
template <class T>
bool
vgl_h_matrix_1d<T>::is_euclidean() const
{
  // no translational part, and scale 1:
  return t12_matrix_.get(0, 0) == (T)1 && t12_matrix_.get(1, 0) == (T)0 && t12_matrix_.get(1, 1) == (T)1;
}
