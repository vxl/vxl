...
template <class T>
vgl_h_matrix_2d<T>::vgl_h_matrix_2d(const char * filename)
{
...
template <class T>
vgl_h_matrix_2d<T>::vgl_h_matrix_2d(const std::vector<vgl_homg_point_2d<T>> & points1,
                                    const std::vector<vgl_homg_point_2d<T>> & points2)
{
  vnl_matrix<T> W;
...
template <class T>
vgl_h_matrix_2d<T>::vgl_h_matrix_2d(const vnl_matrix_fixed<T, 2, 2> & M, const vnl_vector_fixed<T, 2> & m)
{
  for (int r = 0; r < 2; ++r)
  {
    for (int c = 0; c < 2; ++c)
...
template <class T>
vgl_homg_point_2d<T>
vgl_h_matrix_2d<T>::operator()(const vgl_homg_point_2d<T> & p) const
{
...
template <class T>
vgl_homg_line_2d<T>
vgl_h_matrix_2d<T>::correlation(const vgl_homg_point_2d<T> & p) const
{
  vnl_vector_fixed<T, 3> v2 = t12_matrix_ * vnl_vector_fixed<T, 3>(p.x(), p.y(), p.w());
...
template <class T>
vgl_homg_line_2d<T>
vgl_h_matrix_2d<T>::preimage(const vgl_homg_line_2d<T> & l) const
{
...
template <class T>
vgl_homg_point_2d<T>
vgl_h_matrix_2d<T>::correlation(const vgl_homg_line_2d<T> & l) const
{
...
template <class T>
vgl_conic<T>
vgl_h_matrix_2d<T>::operator()(const vgl_conic<T> & C) const
{
  ...
  return vgl_conic<T>(
    Mp(0, 0), (Mp(0, 1) + Mp(1, 0)), Mp(1, 1), (Mp(0, 2) + Mp(2, 0)), (Mp(1, 2) + Mp(2, 1)), Mp(2, 2));
}

template <class T>
vgl_conic<T>
vgl_h_matrix_2d<T>::preimage(const vgl_conic<T> & C) const
{
  T a = C.a(), b = C.b() / 2, c = C.c(), d = C.d() / 2, e = C.e() / 2, f = C.f();
  vnl_matrix_fixed<T, 3, 3> M, Mp;
  M(0, 0) = a;
...
template <class T>
vgl_homg_point_2d<T>
vgl_h_matrix_2d<T>::preimage(const vgl_homg_point_2d<T> & p) const
{
...
template <class T>
vgl_homg_line_2d<T>
vgl_h_matrix_2d<T>::operator()(const vgl_homg_line_2d<T> & l) const
{
...
template <class T>
bool
vgl_h_matrix_2d<T>::read(std::istream & s)
{
  t12_matrix_.read_ascii(s);
  return s.good() || s.eof();
}
...
template <class T>
bool
vgl_h_matrix_2d<T>::read(const char * filename)
{
  std::ifstream f(filename);
  if (!f.good())
    std::cerr << "vgl_h_matrix_2d::read: Error opening " << filename << std::endl;
...
template <class T>
T
vgl_h_matrix_2d<T>::get(unsigned int row_index, unsigned int col_index) const
{
...
template <class T>
void
vgl_h_matrix_2d<T>::get(T * H) const
{
...
template <class T>
void
vgl_h_matrix_2d<T>::get(vnl_matrix_fixed<T, 3, 3> * H) const
{
  *H = t12_matrix_;
}
...
template <class T>
vgl_h_matrix_2d<T> &
vgl_h_matrix_2d<T>::set(const vnl_matrix_fixed<T, 3, 3> & H)
{
  t12_matrix_ = H;
  return *this;
}
...
template <class T>
bool
vgl_h_matrix_2d<T>::projective_basis(const std::vector<vgl_homg_point_2d<T>> & points)
{
  if (points.size() != 4)
    return false;
...
template <class T>
bool
vgl_h_matrix_2d<T>::is_rotation() const
{
  return t12_matrix_.get(0, 2) == (T)0 && t12_matrix_.get(1, 2) == (T)0 && this->is_euclidean();
}

template <class T>
bool
vgl_h_matrix_2d<T>::is_euclidean() const
{
  if (t12_matrix_.get(2, 0) != (T)0 || t12_matrix_.get(2, 1) != (T)0 || t12_matrix_.get(2, 2) != (T)1)
...
template <class T>
bool
vgl_h_matrix_2d<T>::is_identity() const
{
...
template <class T>
bool
vgl_h_matrix_2d<T>::projective_basis(const std::vector<vgl_homg_line_2d<T>> & lines)
{
  if (lines.size() != 4)
    return false;
...
template <class T>
vgl_h_matrix_2d<T> &
vgl_h_matrix_2d<T>::set_similarity(T s, T theta, T tx, T ty)
{
  T a = s * std::cos(theta);
  T b = s * std::sin(theta);
  t12_matrix_[0][0] = a;
...
template <class T>
vgl_h_matrix_2d<T> &
vgl_h_matrix_2d<T>::set_affine(const vnl_matrix_fixed<T, 2, 3> & M23)
{
  for (unsigned r = 0; r < 2; ++r)
    for (unsigned c = 0; c < 3; ++c)
...
vgl_h_matrix_2d<T>::get_translation() const
{
  ...
  T d = t12_matrix_[2][2];
  assert(d < -1e-9 || d > 1e-9);
  return vgl_homg_point_2d<T>(t12_matrix_[0][2] / d, t12_matrix_[1][2] / d, (T)1);
}
