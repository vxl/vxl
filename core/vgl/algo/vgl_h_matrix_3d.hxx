...
template <class T>
vgl_h_matrix_3d<T>::vgl_h_matrix_3d(const std::vector<vgl_homg_point_3d<T>> & points1,
                                    const std::vector<vgl_homg_point_3d<T>> & points2)
{
  vnl_matrix<T> W;
...
template <class T>
vgl_h_matrix_3d<T>::vgl_h_matrix_3d(const char * filename)
{
...
template <class T>
vgl_h_matrix_3d<T>::vgl_h_matrix_3d(const vnl_matrix_fixed<T, 3, 3> & M, const vnl_vector_fixed<T, 3> & m)
{
  for (int r = 0; r < 3; ++r)
  {
    for (int c = 0; c < 3; ++c)
...
template <class T>
vgl_homg_point_3d<T>
vgl_h_matrix_3d<T>::operator()(const vgl_homg_point_3d<T> & p) const
{
...
template <class T>
vgl_pointset_3d<T>
vgl_h_matrix_3d<T>::operator()(const vgl_pointset_3d<T> & ptset) const
{
...
template <class T>
vgl_homg_plane_3d<T>
vgl_h_matrix_3d<T>::correlation(const vgl_homg_point_3d<T> & p) const
{
  vnl_vector_fixed<T, 4> v2 = t12_matrix_ * vnl_vector_fixed<T, 4>(p.x(), p.y(), p.z(), p.w());
...
template <class T>
vgl_homg_plane_3d<T>
vgl_h_matrix_3d<T>::preimage(const vgl_homg_plane_3d<T> & l) const
{
...
template <class T>
vgl_homg_point_3d<T>
vgl_h_matrix_3d<T>::correlation(const vgl_homg_plane_3d<T> & l) const
{
...
template <class T>
vgl_homg_point_3d<T>
vgl_h_matrix_3d<T>::preimage(const vgl_homg_point_3d<T> & p) const
{
...
template <class T>
vgl_pointset_3d<T>
vgl_h_matrix_3d<T>::preimage(const vgl_pointset_3d<T> & ptset) const
{
...
template <class T>
vgl_homg_plane_3d<T>
vgl_h_matrix_3d<T>::operator()(const vgl_homg_plane_3d<T> & l) const
{
...
template <class T>
bool
vgl_h_matrix_3d<T>::read(std::istream & s)
{
  t12_matrix_.read_ascii(s);
  return s.good() || s.eof();
}
...
template <class T>
bool
vgl_h_matrix_3d<T>::read(const char * filename)
{
  std::ifstream f(filename);
  if (!f.good())
    std::cerr << "vgl_h_matrix_3d::read: Error opening " << filename << std::endl;
...
template <class T>
T
vgl_h_matrix_3d<T>::get(unsigned int row_index, unsigned int col_index) const
{
...
template <class T>
void
vgl_h_matrix_3d<T>::get(T * H) const
{
  for (const T * iter = t12_matrix_.begin(); iter < t12_matrix_.end(); ++iter)
    *H++ = *iter;
}

template <class T>
void
vgl_h_matrix_3d<T>::get(vnl_matrix_fixed<T, 4, 4> * H) const
{
  *H = t12_matrix_;
}
...
template <class T>
vgl_h_matrix_3d<T> &
vgl_h_matrix_3d<T>::set(const T * H)
{
...
template <class T>
vgl_h_matrix_3d<T> &
vgl_h_matrix_3d<T>::set(const vnl_matrix_fixed<T, 4, 4> & H)
{
  t12_matrix_ = H;
  return *this;
}
...
template <class T>
bool
vgl_h_matrix_3d<T>::projective_basis(const std::vector<vgl_homg_point_3d<T>> & /*five_points*/)
{
...
template <class T>
bool
vgl_h_matrix_3d<T>::projective_basis(const std::vector<vgl_homg_plane_3d<T>> & /*five_planes*/)
{
...
template <class T>
vgl_h_matrix_3d<T> &
vgl_h_matrix_3d<T>::set_affine(const vnl_matrix_fixed<T, 3, 4> & M34)
{
  for (unsigned r = 0; r < 3; ++r)
    for (unsigned c = 0; c < 4; ++c)
      t12_matrix_[r][c] = M34[r][c];
  t12_matrix_[3][0] = t12_matrix_[3][1] = t12_matrix_[3][2] = T(0);
  t12_matrix_[3][3] = T(1);
  return *this;
}
...
template <class T>
vgl_h_matrix_3d<T> &
vgl_h_matrix_3d<T>::set_rotation_about_axis(const vnl_vector_fixed<T, 3> & axis, T angle)
{
...
template <class T>
vgl_h_matrix_3d<T> &
vgl_h_matrix_3d<T>::set_rotation_matrix(const vnl_matrix_fixed<T, 3, 3> & R)
{
  for (unsigned r = 0; r < 3; ++r)
...
template <class T>
void
vgl_h_matrix_3d<T>::set_reflection_plane(const vgl_plane_3d<double> & l)
{
...
template <class T>
bool
vgl_h_matrix_3d<T>::is_rotation() const
{
  return t12_matrix_.get(0, 3) == (T)0 && t12_matrix_.get(1, 3) == (T)0 && t12_matrix_.get(2, 3) == (T)0 &&
         this->is_euclidean();
}
...
template <class T>
bool
vgl_h_matrix_3d<T>::is_euclidean() const
{
  T eps = 10 * std::numeric_limits<T>::epsilon();
  if (t12_matrix_.get(3, 0) != (T)0 || t12_matrix_.get(3, 1) != (T)0 || t12_matrix_.get(3, 2) != (T)0 ||
...
template <class T>
bool
vgl_h_matrix_3d<T>::is_affine() const
{
  if (t12_matrix_.get(3, 0) != (T)0 || t12_matrix_.get(3, 1) != (T)0 || t12_matrix_.get(3, 2) != (T)0 ||
...
template <class T>
bool
vgl_h_matrix_3d<T>::is_identity() const
{
...
template <class T>
void
vgl_h_matrix_3d<T>::polar_decomposition(vnl_matrix_fixed<T, 3, 3> & S, vnl_matrix_fixed<T, 3, 3> & R) const
{
...
vgl_h_matrix_3d<T>::get_translation() const
{
  ...
  T d = t12_matrix_[3][3];
  assert(d < -1e-9 || d > 1e-9);
  return vgl_homg_point_3d<T>(t12_matrix_[0][3] / d, t12_matrix_[1][3] / d, t12_matrix_[2][3] / d, (T)1);
}
