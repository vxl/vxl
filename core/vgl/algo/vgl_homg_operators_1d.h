...
public:
  ...
  static vnl_vector_fixed<T, 2>
  get_vector(const vgl_homg_point_1d<T> & p);
  ...
  //
  static double
  cross_ratio(const vgl_homg_point_1d<T> & a,
  ...
  static T
  conjugate(T x1, T x2, T x3, double cr = -1);

  //: Calculate the projective conjugate point of three given points.
  // Or more generally, the point with a given crossratio w.r.t. three other points:
  // The cross ratio ((x1,x2;x3,answer)) is cr (default -1). When cr is -1,
  // the returned value and x3 are conjugate points w.r.t. the pair (x1,x2).
  static vgl_homg_point_1d<T>
  conjugate(const vgl_homg_point_1d<T> & a,
            const vgl_homg_point_1d<T> & b,
            const vgl_homg_point_1d<T> & c,
            double cr = -1);
  ...
  static T
  dot(const vgl_homg_point_1d<T> & a, const vgl_homg_point_1d<T> & b);

  //: Cross product of two homogeneous points
  static T
  cross(const vgl_homg_point_1d<T> & a, const vgl_homg_point_1d<T> & b);

  //: Normalize vgl_homg_point_1d<T> to unit magnitude
  static void
  unitize(vgl_homg_point_1d<T> & a);

  //: Get the distance between the two points.
  static T
  distance(const vgl_homg_point_1d<T> & point1, const vgl_homg_point_1d<T> & point2);

  //: Get the square of the distance between the two points.
  static T
  distance_squared(const vgl_homg_point_1d<T> & point1, const vgl_homg_point_1d<T> & point2);

  //: True if the points are closer than Euclidean distance d.
  static bool
  is_within_distance(const vgl_homg_point_1d<T> & p1, const vgl_homg_point_1d<T> & p2, T d)
  {
  ...
  static vgl_homg_point_1d<T>
  midpoint(const vgl_homg_point_1d<T> & p1, const vgl_homg_point_1d<T> & p2);
};
