// This is core/vgl/algo/vgl_homg_operators_2d.hxx
#ifndef vgl_homg_operators_2d_hxx_
#define vgl_homg_operators_2d_hxx_
//:
// \file
...
template <class T>
static inline vgl_homg_line_2d<T>
cross(const vgl_homg_point_2d<T> & p1, const vgl_homg_point_2d<T> & p2)
{
  return vgl_homg_line_2d<T>(
    p1.y() * p2.w() - p1.w() * p2.y(), p1.w() * p2.x() - p1.x() * p2.w(), p1.x() * p2.y() - p1.y() * p2.x());
}

template <class T>
static inline vgl_homg_point_2d<T>
cross(const vgl_homg_line_2d<T> & l1, const vgl_homg_line_2d<T> & l2)
{
  return vgl_homg_point_2d<T>(
    l1.b() * l2.c() - l1.c() * l2.b(), l1.c() * l2.a() - l1.a() * l2.c(), l1.a() * l2.b() - l1.b() * l2.a());
}

template <class T>
static inline T
dot(const vgl_homg_line_2d<T> & l, const vgl_homg_point_2d<T> & p)
{
...
vgl_homg_operators_2d<T>::get_vector(const vgl_homg_point_2d<T> & p)
{
...
template <class T>
vnl_vector_fixed<T, 3>
vgl_homg_operators_2d<T>::get_vector(const vgl_homg_line_2d<T> & l)
{
  return vnl_vector_fixed<T, 3>(l.a(), l.b(), l.c());
}

template <class T>
vnl_vector_fixed<T, 6>
vgl_homg_operators_2d<T>::get_vector(const vgl_conic<T> & c)
{
...
template <class T>
void
vgl_homg_operators_2d<T>::unitize(vgl_homg_point_2d<T> & a)
{
...
template <class T>
T
vgl_homg_operators_2d<T>::distance(const vgl_homg_point_2d<T> & point1, const vgl_homg_point_2d<T> & point2)
{
...
template <class T>
T
vgl_homg_operators_2d<T>::distance_squared(const vgl_homg_point_2d<T> & p1, const vgl_homg_point_2d<T> & p2)
{
  if (p1 == p2)
    return T(0); // quick return if possible
  ...
  return vnl_math::sqr(p1.x() / p1.w() - p2.x() / p2.w()) + vnl_math::sqr(p1.y() / p1.w() - p2.y() / p2.w());
}
...
template <class T>
T
vgl_homg_operators_2d<T>::perp_dist_squared(const vgl_homg_point_2d<T> & point, const vgl_homg_line_2d<T> & line)
{
...
template <class T>
double
vgl_homg_operators_2d<T>::abs_angle(const vgl_homg_line_2d<T> & line1, const vgl_homg_line_2d<T> & line2)
{
  double angle1 = line_angle(line1);
  double angle2 = line_angle(line2);

  double diff = angle2 - angle1;
  if (diff > vnl_math::pi_over_2)
    diff -= vnl_math::pi;
  if (diff < -vnl_math::pi_over_2)
    diff += vnl_math::pi;
...
template <class T>
vgl_homg_line_2d<T>
vgl_homg_operators_2d<T>::join(const vgl_homg_point_2d<T> & p1, const vgl_homg_point_2d<T> & p2)
{
  return cross(p1, p2);
}
...
template <class T>
vgl_homg_line_2d<T>
vgl_homg_operators_2d<T>::join_oriented(const vgl_homg_point_2d<T> & p1, const vgl_homg_point_2d<T> & p2)
{
  int s1 = p1.w() < 0;
  int s2 = p2.w() < 0;
...
template <class T>
vgl_homg_point_2d<T>
vgl_homg_operators_2d<T>::intersection(const vgl_homg_line_2d<T> & l1, const vgl_homg_line_2d<T> & l2)
{
  return cross(l1, l2);
}
...
template <class T>
vgl_homg_line_2d<T>
vgl_homg_operators_2d<T>::perp_line_through_point(const vgl_homg_line_2d<T> & l, const vgl_homg_point_2d<T> & p)
{
...
template <class T>
vgl_homg_point_2d<T>
vgl_homg_operators_2d<T>::perp_projection(const vgl_homg_line_2d<T> & line, const vgl_homg_point_2d<T> & point)
{
...
template <class T>
vgl_homg_point_2d<T>
vgl_homg_operators_2d<T>::midpoint(const vgl_homg_point_2d<T> & p1, const vgl_homg_point_2d<T> & p2)
{
...
vgl_homg_operators_2d<T>::most_orthogonal_vector(const std::vector<vgl_homg_line_2d<T>> & inpoints)
{
  ...
  for (typename std::vector<vgl_homg_line_2d<T>>::const_iterator i = inpoints.begin(); i != inpoints.end(); ++i)
...
template <class T>
double
vgl_homg_operators_2d<T>::cross_ratio(const vgl_homg_point_2d<T> & a,
...
template <class T>
vgl_conic<T>
vgl_homg_operators_2d<T>::vgl_conic_from_matrix(const vnl_matrix_fixed<T, 3, 3> & mat)
{
  return vgl_conic<T>(
    mat[0][0], mat[1][0] + mat[0][1], mat[1][1], mat[0][2] + mat[2][0], mat[1][2] + mat[2][1], mat[2][2]);
}
...
vgl_homg_operators_2d<T>::matrix_from_conic(const vgl_conic<T> & c)
{
...
template <class T>
vnl_matrix_fixed<T, 3, 3>
vgl_homg_operators_2d<T>::matrix_from_dual_conic(const vgl_conic<T> & c)
{
  vnl_matrix_fixed<T, 3, 3> mat;
  T A = c.a(), B = c.b() / 2, C = c.c(), D = c.d() / 2, E = c.e() / 2, F = c.f();
...
vgl_homg_operators_2d<T>::do_intersect(const vgl_conic<T> & c1, const vgl_conic<T> & c2)
{
  if (c1 == c2)
  {
    std::cerr << __FILE__ << "Warning: the intersection of two identical conics is not a finite set of points.\n"
  ...
  T ab = a * B - A * b, ac = a * C - A * c, ad = a * D - A * d, ae = a * E - A * e, af = a * F - A * f,
    BD = b * D + B * d;
  ...
  if ((ab == 0 && ac == 0)
      ...
      // such that c1-lamda*c2 decomposes into (y-y0)(x+my+n)=0. Which means that lambda=A/a, and the equation
      // satisfies:
  ...
  coef(2) = ae * ae - ab * (b * F - B * f) + ad * (c * D - C * d) - BD * (a * E + A * e) + 2 * a * B * e * D +
            2 * A * b * E * d + 2 * ac * af;
  ...
  { // The equation is actually of 2nd degree
    if (coef(2) == 0 && coef(3) == 0)
      return std::list<vgl_homg_point_2d<T>>();        // no real solutions.
    T dis = coef(3) * coef(3) - 4 * coef(2) * coef(4); // discriminant
    if (dis < 0)
      return std::list<vgl_homg_point_2d<T>>(); // no real solutions.
    T y;
    if (coef(2) == 0)
      dis = 0, y = -coef(4) / coef(3);
    else
      y = -coef(3) / coef(2) / 2;
    T w = y * ab + ad;
    T x = -(y * y * ac + y * ae + af);
    ...
    if (dis == 0)
    {
      return std::list<vgl_homg_point_2d<T>>(2, vgl_homg_point_2d<T>(x, y * w, w));
    }
  ...
  if (coef(0) == 0)
  { // The equation is actually of 3rd degree
    coef /= -coef(1);
    ...
    std::list<vgl_homg_point_2d<T>> solutions;
    for (int i = 0; i < 3; ++i)
      if (std::abs(std::imag(polysolutions(i))) < 1e-3)
      { // only want the real solutions
...
template <class T>
std::list<vgl_homg_point_2d<T>>
vgl_homg_operators_2d<T>::do_intersect(const vgl_conic<T> & q, const vgl_homg_line_2d<T> & l)
{
  ...
  { // line at infinity
    ...
    if (d < 0)
      return std::list<vgl_homg_point_2d<T>>(); // no solutions
    if (d == 0)
  ...
  if (a == 0)
  { // write y in terms of w and solve for (x,w)
    T y = -c / b;
    B = B * y + D;
...
template <class T>
std::list<vgl_homg_point_2d<T>>
vgl_homg_operators_2d<T>::intersection(const vgl_conic<T> & c, const vgl_homg_line_2d<T> & l)
{
  if (c.type() == vgl_conic<T>::no_type || c.type() == vgl_conic<T>::complex_parallel_lines ||
      c.type() == vgl_conic<T>::complex_intersecting_lines || c.type() == vgl_conic<T>::imaginary_ellipse ||
...
template <class T>
std::list<vgl_homg_point_2d<T>>
vgl_homg_operators_2d<T>::intersection(const vgl_conic<T> & c1, const vgl_conic<T> & c2)
{
  if ((c1.type() == vgl_conic<T>::complex_parallel_lines || c1.type() == vgl_conic<T>::complex_intersecting_lines) &&
      c2.contains(c1.centre()))
    return std::list<vgl_homg_point_2d<T>>(2, c1.centre()); // double intersection point
  if ((c2.type() == vgl_conic<T>::complex_parallel_lines || c2.type() == vgl_conic<T>::complex_intersecting_lines) &&
  ...
  if (c1.type() == vgl_conic<T>::coincident_lines || c1.type() == vgl_conic<T>::real_intersecting_lines ||
  ...
  if (c2.type() == vgl_conic<T>::coincident_lines || c2.type() == vgl_conic<T>::real_intersecting_lines ||
...
vgl_homg_operators_2d<T>::tangent_from(const vgl_conic<T> & c, const vgl_homg_point_2d<T> & p)
{
  if (c.is_degenerate())
  {
    if (!c.contains(p))
...
template <class T>
std::list<vgl_homg_line_2d<T>>
vgl_homg_operators_2d<T>::common_tangents(const vgl_conic<T> & c1, const vgl_conic<T> & c2)
{
...
template <class T>
vgl_homg_point_2d<T>
vgl_homg_operators_2d<T>::closest_point(const vgl_conic<T> & c, const vgl_homg_point_2d<T> & pt)
{
  ...
  if (pt.w() == 0)                            // given point is at infinity
  {
    ...
    vgl_homg_line_2d<T> l(
      c.a() * pt.y() * 2 - c.b() * pt.x(), -c.c() * pt.x() * 2 + c.b() * pt.y(), c.d() * pt.y() - c.e() * pt.x());
...
template <class T>
vgl_homg_point_2d<T>
vgl_homg_operators_2d<T>::closest_point(const vgl_conic<T> & c, const vgl_point_2d<T> & pt)
{
...
template <class T>
vgl_box_2d<T>
vgl_homg_operators_2d<T>::compute_bounding_box(const vgl_conic<T> & c)
{
  // Only ellipses have a finite bounding box:

  if (c.real_type() == "complex intersecting lines")
  { // a single point:
    vgl_homg_point_2d<T> pt = c.centre();
    return vgl_box_2d<T>(vgl_point_2d<T>(pt), vgl_point_2d<T>(pt));
  }

  if (c.real_type() == "invalid conic" || c.real_type() == "imaginary ellipse" || c.real_type() == "imaginary circle" ||
      c.real_type() == "complex parallel lines")
    return vgl_box_2d<T>(); // empty box

  if (c.real_type() == "real parallel lines" || c.real_type() == "coincident lines")
  {
...
template <class T>
vgl_homg_point_2d<T>
operator*(const vnl_matrix_fixed<T, 3, 3> & m, const vgl_homg_point_2d<T> & p)
{
...
template <class T>
vgl_homg_line_2d<T>
operator*(const vnl_matrix_fixed<T, 3, 3> & m, const vgl_homg_line_2d<T> & l)
{
...
template <class T>
vgl_homg_point_2d<T>
vgl_homg_operators_2d<T>::closest_point(const vgl_homg_line_2d<T> & l, const vgl_homg_point_2d<T> & p)
{
  // Return p itself, if p lies on l:
  if (l.a() * p.x() + l.b() * p.y() + l.c() * p.w() == 0)
    return p;
...
#endif // vgl_homg_operators_2d_hxx_
