...
{
  return vgl_conic<T>(mat[0][0], mat[1][0] + mat[0][1], mat[1][1], mat[0][2] + mat[2][0], mat[1][2] + mat[2][1],
                      mat[2][2]);
}
                                       ...
                                       vgl_conic<T> const& c2)
{
  ...
  coef(2) = ae * ae - ab * (b * F - B * f) + ad * (c * D - C * d) - BD * (a * E + A * e) + 2 * a * B * e * D + 2 * A
    * b * E * d + 2 * ac * af;
  ...
  if( coef(0) == 0 && coef(1) == 0 )  // The equation is actually of 2nd degree
    ...
    if( coef(2) == 0 && coef(3) == 0 )
      {
      return vcl_list<vgl_homg_point_2d<T> >();                                 // no real solutions.
      }
                                       ...
                                       vgl_homg_line_2d<T> const& l)
{
  ...
  if( a == 0 && b == 0 ) // line at infinity
    ...
    if( d < 0 )
      {
      return vcl_list<vgl_homg_point_2d<T> >();          // no solutions
      }
    if( d == 0 )
      {
