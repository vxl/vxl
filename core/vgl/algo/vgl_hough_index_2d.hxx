...
{
 public:
  bool operator()(const std::vector<vgl_line_segment_2d<T> > & v1,
                  const std::vector<vgl_line_segment_2d<T> > & v2)
  {
...
//
template <class T>
vgl_hough_index_2d<T>::vgl_hough_index_2d(const size_t r_dimension,
                                          const size_t theta_dimension)
{
  xo_ = 0; yo_ = 0;
...
//
template <class T>
vgl_hough_index_2d<T>::vgl_hough_index_2d(const T x0,
                                       const T y0,
                                       const T xsize,
                                       const T ysize,
                                       const T angle_range,
...
template <class T>
T vgl_hough_index_2d<T>::tangent_angle(vgl_line_segment_2d<T> const& line){
...
//
template <class T>
size_t vgl_hough_index_2d<T>::trans_loc(const size_t transx, const size_t transy,
                                     const size_t r, const size_t theta)
{
...
//-----------------------------------------------------------------------------
template <class T>
size_t vgl_hough_index_2d<T>::count(const size_t r, const size_t theta)
{
...
//
template <class T>
bool vgl_hough_index_2d<T>::insert(vgl_line_segment_2d<T> const& line)
{
...
template <class T>
bool vgl_hough_index_2d<T>::insert_new(vgl_line_segment_2d<T> const& line)
{
...
template <class T>
bool vgl_hough_index_2d<T>::find(vgl_line_segment_2d<T> const& line)
{
  size_t r, theta;
  this->array_loc(line, r, theta);
  const std::vector<size_t>& line_ids = index_[r][theta];
...
template <class T>
bool vgl_hough_index_2d<T>::find(T r, T theta, vgl_line_segment_2d<T>& line){
if (r < 0 || theta < 0)
  return false;
...
//
template <class T>
bool vgl_hough_index_2d<T>::remove(vgl_line_segment_2d<T> const& line)
{
  size_t r, theta;
  this->array_loc(line, r, theta);
  if (!(r < r_dim_)||!(theta < th_dim_))
...
//-----------------------------------------------------------------------------
template <class T>
std::vector<vgl_line_segment_2d<T> > vgl_hough_index_2d<T>::
...
//-----------------------------------------------------------------------------
//
//: Return a list of vgl_line_segment_2d<T>(s) which are within a distance(radius) of the r,theta values of a given line.
//
//-----------------------------------------------------------------------------
template <class T>
std::vector<vgl_line_segment_2d<T> >
vgl_hough_index_2d<T>::lines_in_interval(vgl_line_segment_2d<T> const & l,
                                         const T r_dist,
                                         const T theta_dist)
{
  std::vector<vgl_line_segment_2d<T> > out;
...
template <class T>
void
vgl_hough_index_2d<T>::parallel_lines(const T angle,
                                      const T angle_dist,
                                      std::vector<vgl_line_segment_2d<T> >& lines)
{
  lines.clear();
...
template <class T>
void
vgl_hough_index_2d<T>::lines_at_angle(vgl_line_segment_2d<T> const &l,
                                      const T angle,
                                      const T angle_dist,
                                      std::vector<vgl_line_segment_2d<T> >& lines)
{
...
template <class T>
void
vgl_hough_index_2d<T>::parallel_lines(vgl_line_segment_2d<T> const &l,
                                      const T angle_dist,
                                      std::vector<vgl_line_segment_2d<T> >& lines)
{
  lines.clear();
...
//-----------------------------------------------------------------------------
template <class T>
void vgl_hough_index_2d<T>::clear_index()
{
...
//
template <class T>
void vgl_hough_index_2d<T>::init(const size_t r_dimension,
                                 const size_t theta_dimension)
{
  r_dim_ = r_dimension;
...
//
template <class T>
std::vector<size_t> vgl_hough_index_2d<T>::angle_histogram()
{
...
static size_t get_extended_count(int bin, std::vector<size_t> const& ang_hist)
{
...
static void remove_flat_peaks(std::vector<size_t>& angle_hist)
{
...
template <class T>
std::vector<size_t>
vgl_hough_index_2d<T>::non_maximum_suppress(const size_t radius,
...
template <class T>
size_t vgl_hough_index_2d<T>::
dominant_directions(const size_t thresh, const T angle_tol,
                    std::vector<size_t>& dirs)
{
  ...
  std::vector<size_t> suppressed_hist =
...
template <class T>
size_t vgl_hough_index_2d<T>::
dominant_line_groups(const size_t thresh, const T angle_tol,
...
template <class T>
size_t vgl_hough_index_2d<T>::
dominant_group_hough_coords(const size_t thresh, const T angle_tol,std::vector<std::vector<std::pair<T, T> > >& group_h_coords){
...
template <class T>
size_t vgl_hough_index_2d<T>::max_count() const{
  size_t max_cnt = 0;
  for(size_t r = 0; r<r_dim_;++r)
...
template <class T>
size_t vgl_hough_index_2d<T>::min_count() const{
...
template <class T>
T vgl_hough_index_2d<T>::average_count() const{
  T avg_cnt = T(0);
  T ncells = T(0);
  for(size_t r = 0; r<r_dim_;++r)
    for(size_t theta=0; theta<th_dim_; ++theta){
...
#define VGL_HOUGH_INDEX_2D_INSTANTIATE(Type) \
template class vgl_hough_index_2d<Type >
#endif
