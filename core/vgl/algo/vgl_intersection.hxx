...
vgl_intersection(const std::list<vgl_plane_3d<T>> & planes)
{
  ...
  for (typename std::list<vgl_plane_3d<T>>::const_iterator pit = planes.begin(); pit != planes.end(); ++pit)
  {
  ...
  vgl_point_3d<T> pt(static_cast<T>(p0d.x()), static_cast<T>(p0d.y()), static_cast<T>(p0d.z()));

  vgl_vector_3d<T> tv(static_cast<T>(tx), static_cast<T>(ty), static_cast<T>(tz));
...
template <class T>
bool
vgl_intersection(const vgl_box_3d<T> & b, std::list<vgl_point_3d<T>> & poly)
{
  ...
  vgl_point_3d<T> p0 = *it;
  ++it;
  vgl_point_3d<T> p1 = *it;
  ++it;
  vgl_point_3d<T> p2 = *it;
  ++it;
...
vgl_intersection(const vgl_ray_3d<T> & ray0, const vgl_ray_3d<T> & ray1, vgl_point_3d<T> & inter_pt, T & dist)
{
  ...
  A[0][0] = dot_product(v0, v0);
  A[0][1] = -dot_product(v0, v1);
  A[1][0] = A[0][1];
  A[1][1] = dot_product(v1, v1);
  T det = vnl_det(A);
  if (fabs(det) < 1.0e-6)
    return false;
  Ainv = vnl_inverse(A);
