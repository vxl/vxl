// This is core/vgl/algo/vgl_line_2d_regression.hxx
#ifndef vgl_line_2d_regression_hxx_
#define vgl_line_2d_regression_hxx_
//:
// \file
...
template <class T>
void
vgl_line_2d_regression<T>::increment_partial_sums(const T x, const T y)
{
  Sx_ += x;
  Sy_ += y;
  Sxx_ += x * x;
...
template <class T>
void
vgl_line_2d_regression<T>::decrement_partial_sums(const T x, const T y)
{
  assert(npts_ != 0);
  Sx_ -= x;
  Sy_ -= y;
...
template <class T>
void
vgl_line_2d_regression<T>::clear()
{
  Sx_ = 0;
  Sy_ = 0;
  Sxx_ = 0;
...
template <class T>
bool
vgl_line_2d_regression<T>::fit_constrained(T x, T y)
{
  if (npts_ < 1)
  {
...
template <class T>
double
vgl_line_2d_regression<T>::get_rms_error(const T a, const T b, const T c)
{
  if (npts_ == 0)
    return 0;
...
template <class T>
double
vgl_line_2d_regression<T>::get_rms_error()
{
...
template <class T>
void
vgl_line_2d_regression<T>::init_rms_error_est()
{
...
template <class T>
double
vgl_line_2d_regression<T>::get_rms_error_est(const vgl_point_2d<T> & p, bool increment)
{
  if (npts_ == 0)
    return 0;
...
#endif // vgl_line_2d_regression_hxx_
