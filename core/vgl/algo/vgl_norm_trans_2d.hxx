...
template <class T>
vgl_norm_trans_2d<T>::vgl_norm_trans_2d(const char * filename)
...
// Constructor
template <class T>
vgl_norm_trans_2d<T>::vgl_norm_trans_2d(const vnl_matrix_fixed<T, 3, 3> & M)
  : vgl_h_matrix_2d<T>(M)
{}
...
template <class T>
bool
vgl_norm_trans_2d<T>::compute_from_points(const std::vector<vgl_homg_point_2d<T>> & points, bool isotropic)
{
  T cx, cy;
  this->center_of_mass(points, cx, cy);
  vgl_h_matrix_2d<T>::set_identity().set_translation(-cx, -cy);
  std::vector<vgl_homg_point_2d<T>> temp;
  for (typename std::vector<vgl_homg_point_2d<T>>::const_iterator pit = points.begin(); pit != points.end(); pit++)
  {
  ...
  T data[] = { c * scx, -s * scx, -c * scx * cx + s * scx * cy, s * scy, c * scy, -s * scy * cx - c * scy * cy };
...
template <class T>
bool
vgl_norm_trans_2d<T>::compute_from_lines(const std::vector<vgl_homg_line_2d<T>> & lines, bool isotropic)
{
  std::vector<vgl_homg_point_2d<T>> points;
  for (typename std::vector<vgl_homg_line_2d<T>>::const_iterator lit = lines.begin(); lit != lines.end(); lit++)
  {
...
template <class T>
bool
vgl_norm_trans_2d<T>::compute_from_points_and_lines(const std::vector<vgl_homg_point_2d<T>> & pts,
...
template <class T>
void
vgl_norm_trans_2d<T>::center_of_mass(const std::vector<vgl_homg_point_2d<T>> & in, T & cx, T & cy)
{
  T cog_x = 0;
  T cog_y = 0;
  T cog_count = 0.0;
...
template <class T>
bool
vgl_norm_trans_2d<T>::scale_xyroot2(const std::vector<vgl_homg_point_2d<T>> & in, T & radius)
{
  T magnitude = T(0);
  int numfinite = 0;
  T tol = T(1e-06);
...
template <class T>
bool
vgl_norm_trans_2d<T>::scale_aniostropic(const std::vector<vgl_homg_point_2d<T>> & in, T & sdx, T & sdy, T & c, T & s)
{
  T tol = T(1e-06);
  unsigned count = 0;
  unsigned n = in.size();
...
#define VGL_NORM_TRANS_2D_INSTANTIATE(T) template class vgl_norm_trans_2d<T>
