...
//
template <class T>
void vgl_norm_trans_2d<T>::center_of_mass(vcl_vector<vgl_homg_point_2d<T> > const& in, T& cx, T& cy)
{
  T        cog_x = 0;
  T        cog_y = 0;
  T        cog_count = 0.0;
...
//
template <class T>
bool vgl_norm_trans_2d<T>::scale_xyroot2(vcl_vector<vgl_homg_point_2d<T> > const& in, T& radius)
{
  T   magnitude = T(0);
  int numfinite = 0;
  T   tol = T(1e-06);
