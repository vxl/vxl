...
//
template <class T>
void vgl_norm_trans_3d<T>::center_of_mass(vcl_vector<vgl_homg_point_3d<T> > const& in,
                                          T& cx, T& cy, T& cz)
{
  T        cog_x = 0;
  T        cog_y = 0;
...
//
template <class T>
bool vgl_norm_trans_3d<T>::scale_xyzroot2(vcl_vector<vgl_homg_point_3d<T> > const& in, T& radius)
{
  T   magnitude = T(0);
  int numfinite = 0;
  T   tol = T(1e-06);
