...
template <class Type>
vgl_orient_box_3d<Type>::vgl_orient_box_3d(const vgl_point_3d<Type> & p0,
                                           ...
                                           const vgl_point_3d<Type> & pz)
{
  ...
  rotation(0, 0) = vx.x() / lx;
  rotation(0, 1) = vx.y() / lx;
  rotation(0, 2) = vx.z() / lx;
  rotation(1, 0) = vy.x() / ly;
  rotation(1, 1) = vy.y() / ly;
  rotation(1, 2) = vy.z() / ly;
  rotation(2, 0) = vz.x() / lz;
  rotation(2, 1) = vz.y() / lz;
  rotation(2, 2) = vz.z() / lz;
  // If the transformation matrix has negative determinant, invert px and py:
  double det = vnl_det(rotation);
  if (det < 0)
  {
    double t = rotation(0, 0);
  ...
  vgl_point_3d<Type> corner0(
    (Type)(corner_0[0]), (Type)(corner_0[1]), (Type)(corner_0[2])); // explicitly cast back from double
  vnl_vector_fixed<double, 3> corner_1 = centroid - rotation * p0_to_centroid;
  vgl_point_3d<Type> corner1(
    (Type)(corner_1[0]), (Type)(corner_1[1]), (Type)(corner_1[2])); // explicitly cast back from double
...
vgl_orient_box_3d<Type>::corners() const
{
  ...
  corner_points[1] = vgl_point_3d<Type>(corner_points[0].x() + width(), corner_points[0].y(), corner_points[0].z());
  corner_points[2] = vgl_point_3d<Type>(corner_points[0].x(), corner_points[0].y(), corner_points[0].z() + depth());
  corner_points[3] = vgl_point_3d<Type>(corner_points[1].x(), corner_points[1].y(), corner_points[1].z() + depth());
  corner_points[4] = vgl_point_3d<Type>(corner_points[0].x(), corner_points[0].y() + height(), corner_points[0].z());
  ...
  for (unsigned int i = 0; i < corner_points.size(); i++)
  {
    ...
    p = orient_.rotate(p);
    corner_points[i] = vgl_point_3d<Type>(
      Type(p[0]) + box_.centroid_x(), Type(p[1]) + box_.centroid_y(), Type(p[2]) + box_.centroid_z());
  }
...
template <class Type>
vgl_box_3d<Type>
vgl_orient_box_3d<Type>::enclosing_box() const
{
  std::vector<vgl_point_3d<Type>> crns = this->corners();
  vgl_box_3d<Type> ret;
  for (typename std::vector<vgl_point_3d<Type>>::iterator cit = crns.begin(); cit != crns.end(); ++cit)
    ret.add(*cit);
  return ret;
}
...
template <class Type>
bool
vgl_orient_box_3d<Type>::contains(const Type & x, const Type & y, const Type & z) const
{
...
template <class Type>
std::ostream &
vgl_orient_box_3d<Type>::print(std::ostream & s) const
{
...
template <class Type>
std::istream &
vgl_orient_box_3d<Type>::read(std::istream & s)
{
