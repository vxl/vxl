...
public:
  ...
  explicit vgl_p_matrix(const vnl_matrix_fixed<T, 3, 4> & P);
  ...
  vgl_homg_point_2d<T>
  operator()(const vgl_homg_point_3d<T> & X) const
  {
    return p_matrix_ * X;
  }
  //: Return the image point which is the projection of the specified 3D point X
  ...
  vgl_homg_line_2d<T>
  operator()(const vgl_homg_line_3d_2_points<T> & L) const;
  //: Return the image line which is the projection of the specified 3D line L
  vgl_homg_line_2d<T>
  operator*(const vgl_homg_line_3d_2_points<T> & L) const
  {
  ...
  operator()(const vgl_line_segment_3d<T> & L) const;
  //: Return the image linesegment which is the projection of the specified 3D linesegment L
  vgl_line_segment_2d<T>
  operator*(const vgl_line_segment_3d<T> & L) const
  {
  ...
  bool
  is_behind_camera(const vgl_homg_point_3d<T> &);
  ...
  vgl_p_matrix &
  operator=(const vgl_p_matrix &);

  bool
  operator==(const vgl_p_matrix & p) const
  {
    return p_matrix_ == p.get_matrix();
  }

  //: Return the 3x3 matrix and 3x1 column vector of P = [A a].
  void
  get(vnl_matrix_fixed<T, 3, 3> * A, vnl_vector_fixed<T, 3> * a) const;
  //: Deprecated; use the vnl_matrix_fixed variant instead
  void
  get(vnl_matrix<T> * A, vnl_vector<T> * a) const;

  //: Return the rows of P = [a b c]'.
  void
  get_rows(vnl_vector<T> * a, vnl_vector<T> * b, vnl_vector<T> * c) const;
  //: Return the rows of P = [a b c]'.
  void
  get_rows(vnl_vector_fixed<T, 4> * a, vnl_vector_fixed<T, 4> * b, vnl_vector_fixed<T, 4> * c) const;
  //: Set P = [a b c]' from its rows a, b, c.
  vgl_p_matrix &
  set_rows(const vnl_vector_fixed<T, 4> & a, const vnl_vector_fixed<T, 4> & b, const vnl_vector_fixed<T, 4> & c);
  ...
  void
  get(T * c_matrix) const;
  //: Return the 3x4 projection matrix in p_matrix
  void
  get(vnl_matrix_fixed<T, 3, 4> & p_matrix) const
  {
    p_matrix = p_matrix_;
  }
  //: Deprecated; use the vnl_matrix_fixed variant instead
  void
  get(vnl_matrix<T> & p_matrix) const
  {
    p_matrix = p_matrix_.as_ref();
  }

  //: Set the internal matrix using the 3x4 p_matrix.
  vgl_p_matrix &
  set(const vnl_matrix_fixed<T, 3, 4> & p_matrix)
  {
    p_matrix_ = p_matrix;
    clear_svd();
    return *this;
  ...
  //: Deprecated; use the vnl_matrix_fixed variant instead
  vgl_p_matrix &
  set(const vnl_matrix<T> & p_matrix)
  {
    p_matrix_ = p_matrix;
    clear_svd();
    return *this;
  ...
  vgl_p_matrix &
  set(const vnl_matrix_fixed<T, 3, 3> & A, const vnl_vector_fixed<T, 3> & a);
  //: Deprecated; use the vnl_matrix_fixed variant instead
  vgl_p_matrix &
  set(const vnl_matrix<T> & A, const vnl_vector<T> & a);
  //: Set the projective matrix with the matrix in the 3x4 C-array, p_matrix
  vgl_p_matrix &
  set(const T * p_matrix);
  //: Set the projective matrix with the matrix in the 3x4 C-array, p_matrix
  vgl_p_matrix &
  set(const T p_matrix[3][4]);
...
template <class T>
vgl_p_matrix<T>
operator*(const vgl_p_matrix<T> & P, const vgl_h_matrix_3d<T> & H);

//: Print p on an ostream
template <class T>
std::ostream &
operator<<(std::ostream & s, const vgl_p_matrix<T> & p);
//: Load p from an ascii istream
template <class T>
std::istream &
operator>>(std::istream & i, vgl_p_matrix<T> & p);
