...
//
template <class T>
vgl_p_matrix<T>::vgl_p_matrix(const vnl_matrix_fixed<T, 3, 4> & pmatrix)
  : p_matrix_(pmatrix)
  , svd_(nullptr)
{}
...
template <class T>
vgl_p_matrix<T> &
vgl_p_matrix<T>::operator=(const vgl_p_matrix & that)
{
  p_matrix_ = that.get_matrix();
  svd_ = nullptr;
  return *this;
...
template <class T>
vgl_homg_line_2d<T>
vgl_p_matrix<T>::operator()(const vgl_homg_line_3d_2_points<T> & L) const
{
...
vgl_p_matrix<T>::operator()(const vgl_line_segment_3d<T> & L) const
{
...
template <class T>
vgl_homg_point_3d<T>
vgl_p_matrix<T>::backproject_pseudoinverse(const vgl_homg_point_2d<T> & x) const
{
...
vgl_p_matrix<T>::backproject(const vgl_homg_point_2d<T> & x) const
{
...
template <class T>
vgl_homg_plane_3d<T>
vgl_p_matrix<T>::backproject(const vgl_homg_line_2d<T> & l) const
{
...
template <class T>
bool
vgl_p_matrix<T>::read_ascii(std::istream & f)
{
...
template <class T>
vgl_p_matrix<T>
vgl_p_matrix<T>::read(const char * filename)
{
...
template <class T>
vgl_p_matrix<T>
vgl_p_matrix<T>::read(std::istream & s)
{
  vgl_p_matrix<T> P;
  s >> P;
  return P;
...
template <class T>
void
vgl_p_matrix<T>::clear_svd() const
{
  delete svd_;
  svd_ = nullptr;
}
...
template <class T>
vgl_homg_point_3d<T>
vgl_p_matrix<T>::get_focal() const
{
...
template <class T>
vgl_h_matrix_3d<T>
vgl_p_matrix<T>::get_canonical_H() const
{
...
template <class T>
bool
vgl_p_matrix<T>::is_canonical(T tol) const
{
  for (int r = 0; r < 3; ++r)
    for (int c = 0; c < 4; ++c)
    {
...
template <class T>
T
vgl_p_matrix<T>::get(unsigned int row_index, unsigned int col_index) const
{
...
template <class T>
void
vgl_p_matrix<T>::get(T * c_matrix) const
{
  for (int row_index = 0; row_index < 3; row_index++)
    for (int col_index = 0; col_index < 4; col_index++)
...
template <class T>
void
vgl_p_matrix<T>::get_rows(vnl_vector_fixed<T, 4> * a, vnl_vector_fixed<T, 4> * b, vnl_vector_fixed<T, 4> * c) const
{
  a->put(0, p_matrix_(0, 0));
  a->put(1, p_matrix_(0, 1));
  a->put(2, p_matrix_(0, 2));
...
template <class T>
vgl_p_matrix<T> &
vgl_p_matrix<T>::set_rows(const vnl_vector_fixed<T, 4> & a,
...
template <class T>
vgl_p_matrix<T> &
vgl_p_matrix<T>::set(const vnl_matrix_fixed<T, 3, 3> & A, const vnl_vector_fixed<T, 3> & a)
{
  p_matrix_(0, 0) = A(0, 0);
  p_matrix_(1, 0) = A(1, 0);
  p_matrix_(2, 0) = A(2, 0);
...
template <class T>
bool
vgl_p_matrix<T>::is_behind_camera(const vgl_homg_point_3d<T> & hX)
{
...
template <class T>
bool
vgl_p_matrix<T>::looks_conditioned()
{
...
template <class T>
vgl_p_matrix<T>
vgl_p_matrix<T>::postmultiply(const vnl_matrix_fixed<T, 4, 4> & H) const
{
...
template <class T>
vgl_p_matrix<T>
vgl_p_matrix<T>::premultiply(const vnl_matrix_fixed<T, 3, 3> & H) const
{
