...
public:
  ...
  //: Construct from a Rodrigues vector.
  vgl_rotation_3d &
  operator=(const vnl_vector_fixed<T, 3> & rvector)
  {
    T mag = rvector.magnitude();
    if (mag > T(0))
      q_ = vnl_quaternion<T>(rvector / mag, mag);
  ...
  //: Construct from a 3x3 rotation matrix
  vgl_rotation_3d &
  operator=(const vnl_matrix_fixed<T, 3, 3> & matrix)
  {
    q_ = matrix.transpose();
    return *this;
  }
  ...
  {
    assert(h.is_rotation());
  }

  //: Construct from a vgl_h_matrix_3d.
  vgl_rotation_3d &
  operator=(const vgl_h_matrix_3d<T> & h)
  {
    assert(h.is_rotation());
    q_ = h.get_upper_3x3_matrix().transpose();
    return *this;
  ...
  vgl_rotation_3d(const vnl_vector_fixed<T, 3> & a, const vnl_vector_fixed<T, 3> & b)
  {
    ...
    double cmag =
      static_cast<double>(c.magnitude()) / static_cast<double>(a.magnitude()) / static_cast<double>(b.magnitude());
    ...
    if (cmag < vgl_tolerance<double>::position)
    {
      ...
      { // rotation axis not defined for rotation of pi
  ...
  inverse() const
  {
    return vgl_rotation_3d<T>(q_.conjugate());
  }
  ...
  vgl_rotation_3d<T>
  operator*(const vgl_rotation_3d<T> & first_rotation) const
  {
  ...
  vgl_homg_plane_3d<T>
  operator*(const vgl_homg_plane_3d<T> & p) const
  {
  ...
  {
    return vgl_homg_line_3d_2_points<T>(this->operator*(l.point_finite()), this->operator*(l.point_infinite()));
  }
  ...
  operator*(const vgl_line_3d_2_points<T> & l) const
  {
    return vgl_line_3d_2_points<T>(this->operator*(l.point1()), this->operator*(l.point2()));
  }
  ...
  {
    return vgl_line_segment_3d<T>(this->operator*(l.point1()), this->operator*(l.point2()));
  }

  //: Rotate a vgl vector.
  vgl_vector_3d<T>
  operator*(const vgl_vector_3d<T> & v) const
  {
  ...
  //: Rotate a vnl vector.
  vnl_vector_fixed<T, 3>
  operator*(const vnl_vector_fixed<T, 3> & v) const
  {
    return q_.rotate(v);
  }
  ...
  //: comparison operator
  bool
  operator==(const vgl_rotation_3d<T> & r) const
  {
    return q_ == r.as_quaternion();
  }
...
template <class T>
std::istream &
operator>>(std::istream & s, vgl_rotation_3d<T> & R)
{
  vnl_quaternion<T> q;
  s >> q;
...
template <class T>
std::ostream &
operator<<(std::ostream & s, const vgl_rotation_3d<T> & R)
{
...
template <class T>
inline void
vgl_rotate_3d(const vgl_rotation_3d<T> & rot, std::vector<vgl_homg_point_3d<T>> & pts)
{
  vnl_matrix_fixed<T, 3, 3> R = rot.as_3matrix();
  for (typename std::vector<vgl_homg_point_3d<T>>::iterator itr = pts.begin(); itr != pts.end(); ++itr)
  {
