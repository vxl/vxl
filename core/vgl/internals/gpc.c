...
static void
insert_bound(edge_node ** b, edge_node * e)
{
...
static edge_node **
bound_list(lmt_node ** lmt, double y)
{
...
static void
add_to_sbtree(int * entries, sb_tree ** sbtree, double y)
{
  if (!*sbtree)
  {
...
static void
build_sbt(int * entries, double * sbt, sb_tree * sbtree)
{
  if (sbtree->less)
...
static int
count_optimal_vertices(gpc_vertex_list c)
{
  int result = 0, i;
...
static edge_node *
build_lmt(lmt_node ** lmt, sb_tree ** sbtree, int * sbt_entries, gpc_polygon * p, int type, gpc_op op)
{
...
static void
add_edge_to_aet(edge_node ** aet, edge_node * edge, edge_node * prev)
{
  if (!*aet)
  {
...
static void
add_intersection(it_node ** it, edge_node * edge0, edge_node * edge1, double x, double y)
{
...
static void
add_st_edge(st_node ** st, it_node ** it, edge_node * edge, double dy)
{
...
static void
build_intersection_table(it_node ** it, edge_node * aet, double dy)
{
  st_node *st, *stp;
  edge_node * edge;
...
static int
count_contours(polygon_node * polygon)
{
  int nc, nv;
  vertex_node *v, *nextv;
...
static void
add_left(polygon_node * p, double x, double y)
{
  vertex_node * nv;
...
static void
merge_left(polygon_node * p, polygon_node * q, polygon_node * list)
{
  polygon_node * target;
...
static void
add_right(polygon_node * p, double x, double y)
{
  vertex_node * nv;
...
static void
merge_right(polygon_node * p, polygon_node * q, polygon_node * list)
{
  polygon_node * target;
...
static void
add_local_min(polygon_node ** p, edge_node * edge, double x, double y)
{
...
static bbox *
create_contour_bboxes(gpc_polygon * p)
{
  bbox * box;
  int c, v;
...
static void
minimax_test(gpc_polygon * subj, gpc_polygon * clip, gpc_op op)
{
  bbox *s_bbox, *c_bbox;
  ...
  for (s = 0; s < subj->num_contours; s++)
    for (c = 0; c < clip->num_contours; c++)
      o_table[c * subj->num_contours + s] =
        (!((s_bbox[s].xmax < c_bbox[c].xmin) || (s_bbox[s].xmin > c_bbox[c].xmax))) &&
...
void
gpc_free_polygon(gpc_polygon * p)
{
  int c;
...
int
gpc_polygon_clip(gpc_op op, gpc_polygon * subj, gpc_polygon * clip, gpc_polygon * result)
{
  int my_nc;
  sb_tree * sbtree = NULL;
