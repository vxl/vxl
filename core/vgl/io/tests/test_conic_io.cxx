...
test_conic_double_io()
{
  ...
  TEST("Created vgl_conic_test_double_io.bvl.tmp for writing", (!bfs_out), false);
  vsl_b_write(bfs_out, p_out);
  bfs_out.close();

  vsl_b_ifstream bfs_in("vgl_conic_test_double_io.bvl.tmp");
  TEST("Opened vgl_conic_test_double_io.bvl.tmp for reading", (!bfs_in), false);
...
test_conic_float_io()
{
  ...
  TEST("Created vgl_conic_test_float_io.bvl.tmp for writing", (!bfs_out), false);
  vsl_b_write(bfs_out, p_out);
  bfs_out.close();

  vsl_b_ifstream bfs_in("vgl_conic_test_float_io.bvl.tmp");
  TEST("Opened vgl_conic_test_float_io.bvl.tmp for reading", (!bfs_in), false);
  vsl_b_read(bfs_in, p_in);
  TEST("Finished reading file successfully", (!bfs_in), false);
  bfs_in.close();
