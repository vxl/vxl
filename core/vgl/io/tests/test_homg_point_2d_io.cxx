// This is core/vgl/io/tests/test_homg_point_2d_io.cxx
#include <iostream>
#ifdef _MSC_VER
...
test_homg_point_2d_double_io()
{
  ...
  TEST("Created vgl_homg_point_2d_test_double_io.bvl.tmp for writing", (!bfs_out), false);
  vsl_b_write(bfs_out, p_out);
  bfs_out.close();

  vsl_b_ifstream bfs_in("vgl_homg_point_2d_test_double_io.bvl.tmp");
  TEST("Opened vgl_homg_point_2d_test_double_io.bvl.tmp for reading", (!bfs_in), false);
  vsl_b_read(bfs_in, p_in);
  TEST("Finished reading file successfully", (!bfs_in), false);
  bfs_in.close();

  vpl_unlink("vgl_homg_point_2d_test_double_io.bvl.tmp");

  TEST("p_out == p_in", p_out.x() == p_in.x() && p_out.y() == p_in.y() && p_out.w() == p_in.w(), true);
