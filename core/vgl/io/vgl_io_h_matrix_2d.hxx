...
template <class T>
void
vsl_b_write(vsl_b_ostream & bfs, const vgl_h_matrix_2d<T> & t)
{
  static const short io_version_no = 1;
  vsl_b_write(bfs, io_version_no);
  vnl_matrix_fixed<T, 3, 3> m = t.get_matrix();
  vsl_b_write(bfs, m[0][0]);
  vsl_b_write(bfs, m[0][1]);
  vsl_b_write(bfs, m[0][2]);
  vsl_b_write(bfs, m[1][0]);
  vsl_b_write(bfs, m[1][1]);
  vsl_b_write(bfs, m[1][2]);
  vsl_b_write(bfs, m[2][0]);
  vsl_b_write(bfs, m[2][1]);
  vsl_b_write(bfs, m[2][2]);
}

//: Binary load vgl_h_matrix_2d from stream.
template <class T>
void
vsl_b_read(vsl_b_istream & bfs, vgl_h_matrix_2d<T> & v)
{
  if (!bfs)
    return;
