...
template <class T>
void
vsl_b_write(vsl_b_ostream & os, const vgl_homg_line_3d_2_points<T> & p)
{
...
template <class T>
void
vsl_b_read(vsl_b_istream & is, vgl_homg_line_3d_2_points<T> & p)
{
  if (!is)
    return;
...
template <class T>
void
vsl_print_summary(std::ostream & os, const vgl_homg_line_3d_2_points<T> & p)
{
  os << '(';
...
#define VGL_IO_HOMG_LINE_3D_2_POINTS_INSTANTIATE(T)                                      \
  template void vsl_print_summary(std::ostream &, const vgl_homg_line_3d_2_points<T> &); \
