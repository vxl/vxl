...
template <class T>
void
vsl_b_write(vsl_b_ostream & os, const vgl_plane_3d<T> & p)
{
...
template <class T>
void
vsl_b_read(vsl_b_istream & is, vgl_plane_3d<T> & p)
{
  if (!is)
    return;
...
template <class T>
void
vsl_print_summary(std::ostream & os, const vgl_plane_3d<T> & p)
{
  os << '(' << p.a() << ',' << p.b() << ',' << p.c() << ',' << p.d() << ')';
}
