...
test_constructor()
{
  const vgl_vector_3d<double> t(0, 0, 2);
  const vgl_point_3d<double> p(1, 2, 3);
  const vgl_infinite_line_3d<double> inf_l(p, t);
  ...
  const vgl_point_3d<double> p1(1, 2, 4);
  const vgl_infinite_line_3d<double> inf_l1(p, p1);
  x0 = inf_l1.x0();
...
{
  {
    const vgl_vector_3d<double> t(0, 0, 1);
    ...
    const vgl_point_3d<double> x(1.0, 2.0, -3.0);
    const bool con = inf_l.contains(x);
    TEST("Contains ", con, true);
  }

  {
    const vgl_vector_3d<double> t(1, 0, 0);
    vgl_point_3d<double> p(1, 2, 3), p0, pt;
    const vgl_infinite_line_3d<double> inf_l(p, t);
    p0 = inf_l.point();
  ...
  {
    const vgl_vector_3d<double> t(0, 1, 0);
    vgl_point_3d<double> p(1, 2, 3), p0, pt;
    const vgl_infinite_line_3d<double> inf_l(p, t);
    p0 = inf_l.point();
