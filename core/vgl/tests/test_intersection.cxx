...
test_intersection()
{
  ...
  const vgl_box_2d<double> box(pll, pur);
  ...
  const vgl_line_segment_2d<double> l1(p11, p12);
  const bool caseI = vgl_intersection(box, l1, lint);
  TEST("No intersection", caseI, false);
  // case II - line segment completely inside box
  vgl_point_2d<double> p21(0.2, 0.2), p22(0.8, 0.8);
  const vgl_line_segment_2d<double> l2(p21, p22);
  ...
  const vgl_line_segment_2d<double> l3(p31, p32);
  ...
  caseIII = caseIII && (((p1III == pla) && (p2III == plb)) || ((p1III == plb) && (p2III == pla)));
  ...
  const vgl_line_segment_2d<double> l4(p41, p42);
  bool caseIV = vgl_intersection(box, l4, lint);
  vgl_point_2d<double> p1IV(0.5, 0.5), p2IV(0.5, 1.0);
  pla = lint.point1();
  plb = lint.point2();
  caseIV = caseIV && (((p1IV == pla) && (p2IV == plb)) || ((p1IV == plb) && (p2IV == pla)));
  TEST("Lineseg intersects box (1 pt.)", caseIV, true);

  // test intersection of plane with pointset
  const vgl_point_3d<double> p0(-10, 1.0, 1.0);
  const vgl_point_3d<double> p1(-0.1, 2.0, 3.0);
  const vgl_point_3d<double> p2(0.05, 5.0, 6.0);
  const vgl_point_3d<double> p3(1.0, 1.0, 1.0);
  vgl_pointset_3d<double> ptset;
  ptset.add_point(p0);
  ptset.add_point(p1);
  ptset.add_point(p2);
  ptset.add_point(p3);
  const vgl_vector_3d<double> n(1.0, 0.0, 0.0);
  const vgl_point_3d<double> org(0.0, 0.0, 0.0);
  const vgl_plane_3d<double> pl(n, org);
  const vgl_pointset_3d<double> psint = vgl_intersection<double>(pl, ptset, 1.0);
  std::cout << psint << '\n';
  TEST("Pointset intersect plane", psint.npts() == 2, true);
  vgl_box_3d<double> box3;
  box3.add(p1);
  box3.add(p2);
  const vgl_pointset_3d<double> bint = vgl_intersection<double>(box3, ptset);
