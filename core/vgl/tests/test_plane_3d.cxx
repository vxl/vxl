...
test_constructor()
{
  ...
  const vgl_plane_3d<double> plane_abcd(a, b, c, d);
  const vgl_vector_3d<double> n(0.0, 1.0, 0.0);
  const vgl_point_3d<double> p0(0.0, 0.0, 0.0);
  const vgl_plane_3d<double> plane_ptn(n, p0);
  ...
  const vgl_plane_3d<double> plane_coin_rays(r0, r1);
  const vgl_vector_3d<double> ncoin = plane_coin_rays.normal();
  dist = std::fabs(1.0 - dot_product(ncoin, nptn));
  TEST_NEAR("Coincident ray constructor", dist, 0.0, 1e-5);
  const vgl_point_3d<double> p0p(-1.0, 0.0, 0.0);
  const vgl_ray_3d<double> r0p(p0p, dir1);
  const vgl_plane_3d<double> plane_para_rays(r0p, r1);
  const vgl_vector_3d<double> npara = plane_para_rays.normal();
...
test_operations()
{
  ...
  const vgl_plane_3d<double> plane(a, b, c, d);
  const vgl_point_3d<double> p3d(1.0, 1.0, -1.0);
  ...
  a = 0.0, b = 1.0, c = 0.0;
  const vgl_plane_3d<double> plane1(a, b, c, d);
  const vgl_point_3d<double> p3dy(1.0, 0.0, 1.0);
  good = good && plane1.plane_coords(p3dy, p2dy, 1e-5);
  er += std::fabs(p2dy.x() - 1.0) + std::fabs(p2dy.y() - 1.0);
  // shift the plane along the y axis
  d = -10.0;
  const vgl_plane_3d<double> plane1s(a, b, c, d);
  const vgl_point_3d<double> p3dys(1.0, 10.0, 1.0);
  ...
  const vgl_point_3d<double> pt3d(-51.26, -7045.5, -50.0);
  const vgl_plane_3d<double> plane_real(-.162, 0.987, 0.0, 6943.55);
  const vgl_point_3d<double> pt3d_on = vgl_closest_point(pt3d, plane_real);
  vgl_point_2d<double> pt2d;
  good = plane_real.plane_coords(pt3d_on, pt2d, 1e-5);
  TEST("Realistic plane test: world -> plane", good, true);
  const vgl_point_3d<double> prec_3d = plane_real.world_coords(pt2d);
  er = (prec_3d - pt3d_on).length();
  TEST_NEAR("Realistic plane test", er, 0.0, 1e-5);

  // test stream operators
  double as = 0.1, bs = 0.5, cs = 0.86, ds = -1.0;
  const vgl_plane_3d<double> pstr(as, bs, cs, ds);
  std::stringstream ss;
  ss << pstr;
