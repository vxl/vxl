...
public:
  ...
  vgl_box_2d(const Type corner1[2], const Type corner2[2]);

  //: Construct using two corner points
  vgl_box_2d(const vgl_point_2d<Type> & corner1, const vgl_point_2d<Type> & corner2);
  ...
  vgl_box_2d(const Type ref_point[2], Type width, Type height, point_type);

  //: Construct a box sized width x height at a given reference point.
  //  The box will either be centered at ref_point or will have ref_point
  //  as its min-position or max-position, as specified by the 4th argument.
  vgl_box_2d(const vgl_point_2d<Type> & ref_point, Type width, Type height, point_type);

  //: Equality test
  inline bool
  operator==(const vgl_box_2d<Type> & b) const
  {
  ...
  void
  setmin_position(const Type min_position[2]);
  //: Modify top right. Bottom left only changed if necessary to avoid empty box
  void
  setmax_position(const Type max_position[2]);
  //: Modify bottom left. Top right only changed if necessary to avoid empty box
  void
  set_min_point(const vgl_point_2d<Type> & min_pt);
  //: Modify top right. Bottom left only changed if necessary to avoid empty box
  void
  set_max_point(const vgl_point_2d<Type> & max_pt);

  //: Move box so centroid lies at c (width, height unchanged)
  inline void
  set_centroid(const Type c[2])
  {
    set_centroid_x(c[0]);
    set_centroid_y(c[1]);
  }
  //: Move box so centroid lies at c (width, height unchanged)
  inline void
  set_centroid(const vgl_point_2d<Type> & c)
  {
    set_centroid_x(c.x());
    set_centroid_y(c.y());
  }
...
void
vgl_box_2d_bounds(ITER begin, ITER end, vgl_box_2d<T> & bounding_box)
{
  for (; begin != end; ++begin)
