...
template <class T>
bool
vgl_clip_lineseg_to_line(T & x1, T & y1, T & x2, T & y2, T a, T b, T c)
{
  T f1 = a * x1 + b * y1 + c;
  T f2 = a * x2 + b * y2 + c;
  if (f1 < 0)
                     ...
                     T & ey) // end points.
{
  ...
  if (a == 0 && b == 0)
    return false; // then ax+by+c=0 is the line at infinity
...
#  define MALLOC(p, T, c, s)                                        \
    {                                                               \
      if ((c) > 0)                                                  \
      {                                                             \
        p = (T *)std::malloc(c * sizeof(T));                        \
        if (!(p))                                                   \
        {                                                           \
...
add_gpc_to_vgl(vgl_polygon<T> & vgl_poly, const gpc_polygon & gpc_poly)
{
  for (int c = 0; c < gpc_poly.num_contours; ++c)
  {
    ...
    {
      vgl_poly.push_back(T(gpc_poly.contour[c].vertex[p].x), T(gpc_poly.contour[c].vertex[p].y));
    }
  }
}
} // namespace
...
add_clipper_to_vgl(vgl_polygon<T> & vgl_poly, const ClipperLib::Paths & clipper_poly, double scale)
{
  for (const auto & c : clipper_poly)
  {
    ...
    {
      vgl_poly.push_back(T((double)c[p].X / scale), T((double)c[p].Y / scale));
    }
  }
}
} // namespace
...
bounds(vgl_polygon<T> vgl_poly, T & min_x, T & max_x, T & min_y, T & max_y)
{
  for (size_t s = 0; s < vgl_poly.num_sheets(); ++s)
  {
    ...
    {
      if (s == 0 && p == 0)
      { // not the most ideal way to initilize this...
...
vgl_clip(vgl_polygon<T> const & poly1, vgl_polygon<T> const & poly2, vgl_clip_type op, int * p_retval)
{
  ...
  if (poly1.num_sheets() == 0)
  {
    *p_retval = 1;
    switch (op)
    {
      case vgl_clip_type_intersect:
        return poly1;
      case vgl_clip_type_difference:
        return poly1;
      ...
      default:
        *p_retval = -1;
        return vgl_polygon<T>(); // this should not happen...
    }
  }
  ...
  *p_retval = retval;
  ...
  {
    case vgl_clip_type_intersect:
      g_op = ClipperLib::ctIntersection;
      break;
    case vgl_clip_type_difference:
      g_op = ClipperLib::ctDifference;
      break;
    case vgl_clip_type_union:
      g_op = ClipperLib::ctUnion;
      break;
    case vgl_clip_type_xor:
      g_op = ClipperLib::ctXor;
      break;
    default:
      break;
  }
...
template <class T>
vgl_polygon<T>
vgl_clip(const vgl_polygon<T> & poly1, const vgl_polygon<T> & poly2, vgl_clip_type op)
{
  int retval;
