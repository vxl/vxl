...
template <class T>
void
vgl_closest_point_to_linesegment(T & ret_x, T & ret_y, T x1, T y1, T x2, T y2, T x0, T y0)
{
...
template <class T>
void
vgl_closest_point_to_linesegment(T & ret_x, T & ret_y, T & ret_z, T x1, T y1, T z1, T x2, T y2, T z2, T x, T y, T z)
{
  // squared distance between endpoints:
...
template <class T>
int
vgl_closest_point_to_non_closed_polygon(T & ret_x, T & ret_y, const T px[], const T py[], unsigned int n, T x, T y)
{
  assert(n > 1);
...
template <class T>
int
vgl_closest_point_to_non_closed_polygon(T & ret_x,
                                        T & ret_y,
                                        T & ret_z,
                                        const T px[],
                                        const T py[],
                                        ...
                                        T z)
{
  ...
  vgl_closest_point_to_linesegment(
    ret_x, ret_y, ret_z, px[di], py[di], pz[di], px[di + 1], py[di + 1], pz[di + 1], x, y, z);
  return di;
}

template <class T>
int
vgl_closest_point_to_closed_polygon(T & ret_x, T & ret_y, const T px[], const T py[], unsigned int n, T x, T y)
{
  assert(n > 1);
  ...
  if (di == -1)
    di += n, vgl_closest_point_to_linesegment(ret_x, ret_y, px[0], py[0], px[n - 1], py[n - 1], x, y);
  else
    vgl_closest_point_to_linesegment(ret_x, ret_y, px[di], py[di], px[di + 1], py[di + 1], x, y);
  return di;
}
...
template <class T>
int
vgl_closest_point_to_closed_polygon(T & ret_x,
                                    T & ret_y,
                                    T & ret_z,
                                    const T px[],
                                    const T py[],
                                    ...
                                    T z)
{
  ...
  if (di == -1)
    di += n, vgl_closest_point_to_linesegment(
               ret_x, ret_y, ret_z, px[0], py[0], pz[0], px[n - 1], py[n - 1], pz[n - 1], x, y, z);
  else
    vgl_closest_point_to_linesegment(
      ret_x, ret_y, ret_z, px[di], py[di], pz[di], px[di + 1], py[di + 1], pz[di + 1], x, y, z);
  return di;
...
template <class T>
vgl_point_2d<T>
vgl_closest_point_origin(const vgl_line_2d<T> & l)
{
  T d = l.a() * l.a() + l.b() * l.b();
...
template <class T>
vgl_homg_point_2d<T>
vgl_closest_point_origin(const vgl_homg_line_2d<T> & l)
{
  return vgl_homg_point_2d<T>(l.a() * l.c(), l.b() * l.c(), -l.a() * l.a() - l.b() * l.b());
}

template <class T>
vgl_point_3d<T>
vgl_closest_point_origin(const vgl_plane_3d<T> & pl)
{
...
template <class T>
vgl_homg_point_3d<T>
vgl_closest_point_origin(const vgl_homg_plane_3d<T> & pl)
{
  return vgl_homg_point_3d<T>(
    pl.a() * pl.d(), pl.b() * pl.d(), pl.c() * pl.d(), -pl.a() * pl.a() - pl.b() * pl.b() - pl.c() * pl.c());
}

template <class T>
vgl_homg_point_3d<T>
vgl_closest_point_origin(const vgl_homg_line_3d_2_points<T> & l)
{
  const vgl_homg_point_3d<T> & q = l.point_finite();
  // The plane through the origin and orthogonal to l is ax+by+cz=0
  // where (a,b,c,0) is the point at infinity of l.
  T a = l.point_infinite().x(), b = l.point_infinite().y(), c = l.point_infinite().z(), d = a * a + b * b + c * c;
  ...
  return vgl_homg_point_3d<T>(
    d * q.x() - lambda * a * q.w(), d * q.y() - lambda * b * q.w(), d * q.z() - lambda * c * q.w(), d * q.w());
}

template <class T>
vgl_point_3d<T>
vgl_closest_point_origin(const vgl_line_3d_2_points<T> & l)
{
  const vgl_point_3d<T> & q = l.point1();
  // The plane through the origin and orthogonal to l is ax+by+cz=0
  // where (a,b,c) is the direction of l.
  T a = l.point2().x() - q.x(), b = l.point2().y() - q.y(), c = l.point2().z() - q.z(), d = a * a + b * b + c * c;
  // The closest point is then the intersection of this plane with the line l.
...
template <class T>
vgl_point_2d<T>
vgl_closest_point(const vgl_line_2d<T> & l, const vgl_point_2d<T> & p)
{
  T d = l.a() * l.a() + l.b() * l.b();
...
template <class T>
vgl_homg_point_2d<T>
vgl_closest_point(const vgl_homg_line_2d<T> & l, const vgl_homg_point_2d<T> & p)
{
  T d = l.a() * l.a() + l.b() * l.b();
  assert(d != 0); // line should not be the line at infinity
...
template <class T>
vgl_point_3d<T>
vgl_closest_point(const vgl_plane_3d<T> & l, const vgl_point_3d<T> & p)
{
  ...
  return vgl_point_3d<T>(
    ((l.b() * l.b() + l.c() * l.c()) * p.x() - l.a() * l.b() * p.y() - l.a() * l.c() * p.z() - l.a() * l.d()) / d,
...
template <class T>
vgl_homg_point_3d<T>
vgl_closest_point(const vgl_homg_plane_3d<T> & l, const vgl_homg_point_3d<T> & p)
{
  return vgl_homg_point_3d<T>(
    (l.b() * l.b() + l.c() * l.c()) * p.x() - l.a() * l.b() * p.y() - l.a() * l.c() * p.z() - l.a() * l.d(),
...
template <class T>
vgl_point_2d<T>
vgl_closest_point(const vgl_polygon<T> & poly, const vgl_point_2d<T> & point, bool closed)
{
  ...
  for (unsigned int s = 0; s < poly.num_sheets(); ++s)
  {
    ...
    {
      double nd =
        DIST_SQR_TO_LINE_SEG_2D(T, poly[s][i].x(), poly[s][i].y(), poly[s][i + 1].x(), poly[s][i + 1].y(), x, y);
      if (nd < dd)
      {
        dd = nd;
        di = i;
    ...
    {
      double nd =
        DIST_SQR_TO_LINE_SEG_2D(T, poly[s][0].x(), poly[s][0].y(), poly[s][n - 1].x(), poly[s][n - 1].y(), x, y);
      if (nd < dd)
      {
        dd = nd;
        di = -1;
  ...
  if (di == -1)
    vgl_closest_point_to_linesegment(
      ret_x, ret_y, poly[si][0].x(), poly[si][0].y(), poly[si][n - 1].x(), poly[si][n - 1].y(), x, y);
  else
    vgl_closest_point_to_linesegment(
      ret_x, ret_y, poly[si][di].x(), poly[si][di].y(), poly[si][di + 1].x(), poly[si][di + 1].y(), x, y);
  return vgl_point_2d<T>(T(ret_x), T(ret_y));
}

template <class T>
std::pair<vgl_homg_point_3d<T>, vgl_homg_point_3d<T>>
vgl_closest_points(const vgl_homg_line_3d_2_points<T> & line1, const vgl_homg_line_3d_2_points<T> & line2)
{
...
template <class T>
vgl_homg_point_3d<T>
vgl_closest_point(const vgl_homg_line_3d_2_points<T> & l, const vgl_homg_point_3d<T> & p)
{
...
template <class T>
double
vgl_closest_point_t(const vgl_line_3d_2_points<T> & l, const vgl_point_3d<T> & p)
{
  const vgl_point_3d<T> & q = l.point1();
  vgl_vector_3d<T> v = p - q;
  // The plane through p and orthogonal to l is a(x-px)+b(y-py)+c(z-pz)=0
  // where (a,b,c,0) is the direction of l.
  double a = l.point2().x() - q.x(), b = l.point2().y() - q.y(), c = l.point2().z() - q.z(), d = a * a + b * b + c * c;
  // The closest point is then the intersection of this plane with the line l.
  // This point equals l.point1 + lambda * l.direction, with lambda:
...
template <class T>
vgl_point_3d<T>
vgl_closest_point(const vgl_line_3d_2_points<T> & l, const vgl_point_3d<T> & p)
{
  const vgl_point_3d<T> & q = l.point1();
  vgl_vector_3d<T> v = p - q;
  // The plane through p and orthogonal to l is a(x-px)+b(y-py)+c(z-pz)=0
...
template <class T>
vgl_point_3d<T>
vgl_closest_point(const vgl_point_3d<T> & p, const vgl_ray_3d<T> & r)
{
...
vgl_closest_points(const vgl_line_segment_3d<T> & l1, const vgl_line_segment_3d<T> & l2, bool * unique /*=0*/)
{
  ...
  if (denom < SMALL_DOUBLE)
  {
    ...
    t_denom = c;
    if (unique)
      *unique = false; // assume this for now; check below.
  }
  else
  {
...
template <class T>
vgl_point_2d<T>
vgl_closest_point(const vgl_line_segment_2d<T> & l, const vgl_point_2d<T> & p)
{
  vgl_point_2d<T> q;
  vgl_closest_point_to_linesegment(
    q.x(), q.y(), l.point1().x(), l.point1().y(), l.point2().x(), l.point2().y(), p.x(), p.y());
  return q;
}

template <class T>
vgl_point_3d<T>
vgl_closest_point(const vgl_line_segment_3d<T> & l, const vgl_point_3d<T> & p)
{
  vgl_point_3d<T> q;
  vgl_closest_point_to_linesegment(q.x(),
                                   q.y(),
                                   q.z(),
                                   l.point1().x(),
                                   l.point1().y(),
...
template <class T>
vgl_point_3d<T>
vgl_closest_point(const vgl_sphere_3d<T> & s, const vgl_point_3d<T> & p)
{
...
template <class T>
vgl_point_3d<T>
vgl_closest_point(const vgl_cylinder_3d<T> & c, const vgl_point_3d<T> & p)
{
...
template <class T>
vgl_point_3d<T>
vgl_closest_point(const vgl_pointset_3d<T> & ptset, const vgl_point_3d<T> & p, T dist)
{
  ...
  if (std::numeric_limits<T>::is_integer)
  {
    ...
    vgl_vector_3d<double> normd(
      static_cast<double>(norm.x()), static_cast<double>(norm.y()), static_cast<double>(norm.z()));
    ...
    if (dpd > dist)
      return pc;
    vgl_point_3d<T> pc_plane_T(
      static_cast<T>(pc_planed.x()), static_cast<T>(pc_planed.y()), static_cast<T>(pc_planed.z()));
    return pc_plane_T;
  }
  else
  {
...
template <class T>
vgl_point_3d<T>
vgl_closest_point(const vgl_cubic_spline_3d<T> & cspl, const vgl_point_3d<T> & p)
{
  // find the closest knot
  const std::vector<vgl_point_3d<T>> & knots = cspl.const_knots();
...
#define VGL_CLOSEST_POINT_INSTANTIATE(T)                                                                               \
  ...
  template int vgl_closest_point_to_non_closed_polygon(                                                                \
    T &, T &, T &, T const[], T const[], T const[], unsigned int, T, T, T);                                            \
  template int vgl_closest_point_to_closed_polygon(T &, T &, T const[], T const[], unsigned int, T, T);                \
  template int vgl_closest_point_to_closed_polygon(                                                                    \
    T &, T &, T &, T const[], T const[], T const[], unsigned int, T, T, T);                                            \
  ...
  template std::pair<vgl_homg_point_3d<T>, vgl_homg_point_3d<T>> vgl_closest_points(                                   \
    vgl_homg_line_3d_2_points<T> const &, vgl_homg_line_3d_2_points<T> const &);                                       \
  ...
  template std::pair<vgl_point_3d<T>, vgl_point_3d<T>> vgl_closest_points(                                             \
    vgl_line_3d_2_points<T> const &, vgl_line_3d_2_points<T> const &, bool *);                                         \
  template std::pair<vgl_point_3d<T>, vgl_point_3d<T>> vgl_closest_points(                                             \
    vgl_line_segment_3d<T> const &, vgl_line_segment_3d<T> const &, bool *);                                           \
