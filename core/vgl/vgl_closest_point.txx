...
vgl_homg_point_3d<T> vgl_closest_point_origin(vgl_homg_line_3d_2_points<T> const& l)
{
  ...
  return vgl_homg_point_3d<T>(d * q.x() - lambda * a * q.w(), d * q.y() - lambda * b * q.w(),
                              d * q.z() - lambda * c * q.w(), d * q.w() );
}
                                  ...
                                  vgl_point_3d<T> const& p)
{
  ...
  return vgl_point_3d<T>( ( (l.b() * l.b() + l.c() * l.c() ) * p.x() - l.a() * l.b() * p.y() - l.a() * l.c() * p.z()
                            - l.a() * l.d() ) / d,
                          ( (l.a() * l.a() + l.c() * l.c() ) * p.y() - l.a() * l.b() * p.x() - l.b() * l.c() * p.z()
                            - l.b() * l.d() ) / d,
                          ( (l.a() * l.a() + l.b() * l.b() ) * p.z() - l.a() * l.c() * p.x() - l.b() * l.c() * p.y()
                            - l.c() * l.d() ) / d);
}
                                  ...
                                  bool closed)
{
  ...
  for( unsigned int s = 0; s < poly.num_sheets(); ++s )
    ...
    for( unsigned i = 0; i + 1 < n; ++i )
      {
      double nd = DIST_SQR_TO_LINE_SEG_2D(T, poly[s][i].x(), poly[s][i].y(), poly[s][i + 1].x(),
                                          poly[s][i + 1].y(), x, y);
      if( nd < dd ) { dd = nd; di = i; si = s; }
      }
    if( closed )
      {
      double nd = DIST_SQR_TO_LINE_SEG_2D(T, poly[s][0].x(), poly[s][0].y(), poly[s][n - 1].x(),
                                          poly[s][n - 1].y(), x, y);
  ...
  if( di == -1 )
    {
    vgl_closest_point_to_linesegment(ret_x, ret_y, poly[si][0].x(), poly[si][0].y(),
                                     poly[si][n - 1].x(), poly[si][n - 1].y(), x, y);
    }
  else
    {
    vgl_closest_point_to_linesegment(ret_x, ret_y, poly[si][di].x(), poly[si][di].y(), poly[si][di + 1].x(),
                                     poly[si][di + 1].y(), x, y);
    }
                                  ...
                                  vgl_point_3d<T> const& p, T dist)
{
  ...
  if( vcl_numeric_limits<T>::is_integer )
    ...
    vgl_vector_3d<double> normd(static_cast<double>(norm.x() ),
                                static_cast<double>(norm.y() ), static_cast<double>(norm.z() ) );
    ...
    vgl_point_3d<T> pc_plane_T(static_cast<T>(pc_planed.x() ),
                               static_cast<T>(pc_planed.y() ), static_cast<T>(pc_planed.z() ) );
    return pc_plane_T;
    }
  else
    {
...
#define VGL_CLOSEST_POINT_INSTANTIATE(T) \
  ...
  template int vgl_closest_point_to_non_closed_polygon(T &, T &, T &, T const[], T const[], T const[], unsigned int, T, \
                                                       T, T); \
  template int vgl_closest_point_to_closed_polygon(T &, T &, T const[], T const[], unsigned int, T, T); \
  template int vgl_closest_point_to_closed_polygon(T &, T &, T &, T const[], T const[], T const[], unsigned int, T, T, \
                                                   T); \
