...
static const char * vgl_conic_name[] = { "invalid conic",
                                         "real ellipse",
                                         "real circle",
                                         "imaginary ellipse",
                                         "imaginary circle",
...
template <class T>
std::string
vgl_conic<T>::real_type() const
{
  return vgl_conic_name[(int)type_];
}

template <class T>
typename vgl_conic<T>::vgl_conic_type
vgl_conic<T>::type_by_name(const std::string & name)
{
...
template <class T>
std::string
vgl_conic<T>::type_by_number(typename vgl_conic<T>::vgl_conic_type type)
{
  if (type <= 0 || type >= num_conic_types)
...
template <class T>
bool
vgl_conic<T>::operator==(const vgl_conic<T> & that) const
{
  if (type() != that.type())
    return false;
  return a() * that.b() == b() * that.a() && a() * that.c() == c() * that.a() && a() * that.d() == d() * that.a() &&
         a() * that.e() == e() * that.a() && a() * that.f() == f() * that.a() && b() * that.c() == c() * that.b() &&
         b() * that.d() == d() * that.b() && b() * that.e() == e() * that.b() && b() * that.f() == f() * that.b() &&
         c() * that.d() == d() * that.c() && c() * that.e() == e() * that.c() && c() * that.f() == f() * that.c() &&
         d() * that.e() == e() * that.d() && d() * that.f() == f() * that.d() && e() * that.f() == f() * that.e();
}
...
template <class T>
void
vgl_conic<T>::set(T ta, T tb, T tc, T td, T te, T tf)
{
  a_ = ta;
  b_ = tb;
  c_ = tc;
...
template <class T>
vgl_conic<T>::vgl_conic(const T co[])
  : type_(no_type)
  , a_(co[0])
  , b_(co[1])
  , c_(co[2])
...
template <class T>
vgl_conic<T>::vgl_conic(const vgl_homg_point_2d<T> & co, T rx, T ry, T theta)
{
  if (co.w() == 0)
  { // This is a parabola
    a_ = co.y() * co.y();
  ...
  {                                       // hyperbola or ellipse
    ...
    f_ = T((rx * st * st + ry * ct * ct) * u * u + 2 * (rx - ry) * ct * st * u * v +
           (rx * ct * ct + ry * st * st) * v * v - rx * ry);
  }
  set_type_from_equation();
}
...
template <class T>
void
vgl_conic<T>::set_type_from_equation()
{
  ...
  if (det != 0)
  {
    if (J > 0)
    {
      ...
      {
        if (A == C && B == 0)
          type_ = imaginary_circle;
        else
          type_ = imaginary_ellipse;
      }
    }
    else if (J < 0)
      type_ = hyperbola;
  ...
  { // limiting cases
    if (J < 0)
      type_ = real_intersecting_lines;
    else if (J > 0)
      type_ = complex_intersecting_lines;
    else /* J == 0 */
    {
      if (A == 0 && B == 0 && C == 0)
      { // line at infinity is component
        if (D != 0 || E != 0)
          ...
          type_ = coincident_lines; // 2x w=0
        else
          type_ = no_type; // all coefficients are 0
      }
      else if (K < 0)
        type_ = real_parallel_lines;
      else if (K > 0)
        type_ = complex_parallel_lines;
      else
        type_ = coincident_lines;
    }
  }
}
...
template <class T>
bool
vgl_conic<T>::contains(const vgl_homg_point_2d<T> & p) const
{
  return p.x() * p.x() * a_ + p.x() * p.y() * b_ + p.y() * p.y() * c_ + p.x() * p.w() * d_ + p.y() * p.w() * e_ +
           p.w() * p.w() * f_ ==
         0;
}
...
template <class T>
bool
vgl_conic<T>::is_degenerate() const
{
  T A = a_, B = b_ / 2, C = c_, D = d_ / 2, E = e_ / 2, F = f_;
...
template <class T>
bool
vgl_conic<T>::ellipse_geometry(double & xc,
                               double & yc,
                               ...
                               double & angle_in_radians) const
{
  ...
  double A = static_cast<double>(a_), B = static_cast<double>(b_) * 0.5, C = static_cast<double>(c_),
         D = static_cast<double>(d_) * 0.5, F = static_cast<double>(f_), E = static_cast<double>(e_) * 0.5;
  if (A < 0)
  ...
  double cmaj = (trace + disc) * D2 / (2 * det);
  if (cmaj < 0)
    cmaj = -cmaj;
  double cmin = (trace - disc) * D2 / (2 * det);
  if (cmin < 0)
    cmin = -cmin;
...
vgl_conic<T>::components() const
{
  if (!is_degenerate() || type() == complex_intersecting_lines || type() == complex_parallel_lines)
  ...
  if (A == 0 && B == 0 && C == 0)
  { // line at infinity (w=0) is a component
  ...
  if (A == 0 && C == 0)
  { // components are vertical and horizontal, resp.
    vgl_homg_line_2d<T> l1(cntr, vgl_homg_point_2d<T>(0, 1, 0)), l2(cntr, vgl_homg_point_2d<T>(1, 0, 0));
    std::list<vgl_homg_line_2d<T>> v(1, l1);
    v.push_back(l2);
    return v;
  }
...
template <class T>
bool
vgl_conic<T>::is_central() const
{
  return type_ == real_ellipse || type_ == imaginary_ellipse || type_ == hyperbola || type_ == real_circle ||
         type_ == imaginary_circle || type_ == real_intersecting_lines || type_ == complex_intersecting_lines;
}

//--------------------------------------------------------------------------------
template <class T>
void
vgl_conic<T>::translate_by(T x, T y)
{
  d_ += 2 * a_ * x + b_ * y;
...
template <class T>
vgl_conic<T>
vgl_conic<T>::dual_conic() const
{
  T A = a_, B = b_ / 2, C = c_, D = d_ / 2, E = e_ / 2, F = f_;
  return vgl_conic<T>(
    E * E - C * F, 2 * (B * F - D * E), D * D - A * F, 2 * (C * D - B * E), 2 * (A * E - B * D), B * B - A * C);
}

//: return the polar line of the homogeneous 2-D point p.
template <class T>
vgl_homg_line_2d<T>
vgl_conic<T>::polar_line(const vgl_homg_point_2d<T> & p) const
{
...
template <class T>
vgl_homg_point_2d<T>
vgl_conic<T>::polar_point(const vgl_homg_line_2d<T> & l) const
{
  if (!is_degenerate())
  {
...
template <class T>
double
vgl_conic<T>::curvature_at(const vgl_point_2d<T> & p) const
{
  // Shorthands
  const T & a_xx = a_;
  const T & a_xy = b_;
...
template <class T>
std::ostream &
operator<<(std::ostream & s, const vgl_conic<T> & co)
{
  s << "<vgl_conic ";
  if (co.a() == 1)
    s << "X^2";
...
template <class T>
std::istream &
operator>>(std::istream & is, vgl_conic<T> & co)
{
  T ta, tb, tc, td, te, tf;
  is >> ta >> tb >> tc >> td >> te >> tf;
  co.set(ta, tb, tc, td, te, tf);
  return is;
}
