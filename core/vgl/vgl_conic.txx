...
vgl_conic<T>::vgl_conic(vgl_homg_point_2d<T> const& co, T rx, T ry, T theta)
{
  if( co.w() == 0 )  // This is a parabola
    ...
    f_ = T(
        (rx * st * st + ry * ct * ct) * u * u + 2 * (rx - ry) * ct * st * u * v + (rx * ct * ct + ry * st * st) * v * v - rx
        * ry);
    }
  set_type_from_equation();
}
...
template <class T>
bool vgl_conic<T>::ellipse_geometry(double& xc, double& yc, double& major_axis_length,
...
vgl_conic<T> vgl_conic<T>::dual_conic() const
{
  ...
  return vgl_conic<T>(E * E - C * F, 2 * (B * F - D * E), D * D - A * F, 2 * (C * D - B * E), 2 * (A * E - B * D),
                      B * B - A * C);
}
