// This is core/vgl/vgl_convex.hxx
#ifndef vgl_convex_hxx_
...
template <class T>
static T
get_nc_angle(const vgl_vector_2d<T> & last_dir, const vgl_point_2d<T> & current, const vgl_point_2d<T> & next)
{
  vgl_vector_2d<T> v = next - current;
  double eps = std::sqrt(current.x() * current.x() + current.y() * current.y()) * std::numeric_limits<T>::epsilon();
...
vgl_convex_hull(const std::vector<vgl_point_2d<T>> & points)
{
  ...
  while (true)
  {
    ...
    // then we are done.
    if (not_starting && nc_angle_to_first > 1.5)
      return hull;
    not_starting = true;
    ...
    if (nc_angle_to_first <= best_nc_angle)
      return hull;
