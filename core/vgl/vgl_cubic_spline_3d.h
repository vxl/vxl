...
public:
  ...
  n_knots() const
  {
    return static_cast<unsigned>(knots_.size());
  }
  ...
  const std::vector<vgl_point_3d<Type>> &
  const_knots() const
  {
    return knots_;
  }
  ...
  void
  set_knots(const std::vector<vgl_point_3d<Type>> & knots, bool closed)
  {
    knots_ = knots;
    closed_curve_ = closed;
  }
  ...
  max_t() const
  {
    return static_cast<Type>(knots_.size() - 1);
  }

  //: Equality operator
  bool
  operator==(const vgl_cubic_spline_3d<Type> & spl) const;
  ...
  //: The next two accessors are useful for interpolating other quantities than position with the same spatial spline,
  //: e.g. a vector field
  ...
  void
  coefficients(Type vm1, Type v0, Type v1, Type v2, Type & a0, Type & a1, Type & a2, Type & a3) const;

  //: the indices for the knots bounding the interval containing t, u is the local parameter in the interval 0 -> 1
  void
  knot_indices(Type t, unsigned & im1, unsigned & i0, unsigned & i1, unsigned & i2, Type & u) const;
...
template <class Type>
void
vgl_cubic_spline_3d<Type>::coefficients(Type vm1, Type v0, Type v1, Type v2, Type & a0, Type & a1, Type & a2, Type & a3)
  const
{
...
template <class Type>
void
vgl_cubic_spline_3d<Type>::knot_indices(Type t, unsigned & im1, unsigned & i0, unsigned & i1, unsigned & i2, Type & u)
  const
{
...
template <class Type>
bool
vgl_cubic_spline_3d<Type>::operator==(const vgl_cubic_spline_3d<Type> & spl) const
{
  if (spl.closed() != closed_curve_)
    return false;
  if (spl.s() != s_)
    return false;
  std::vector<vgl_point_3d<Type>> knots = spl.knots();
  unsigned n = static_cast<unsigned>(knots.size());
  if (n != static_cast<unsigned>(knots_.size()))
    return false;
...
template <class Type>
vgl_point_3d<Type>
vgl_cubic_spline_3d<Type>::operator()(Type t) const
{
...
template <class Type>
vgl_vector_3d<Type>
vgl_cubic_spline_3d<Type>::tangent(Type t) const
{
...
template <class Type>
std::ostream &
operator<<(std::ostream & ostr, const vgl_cubic_spline_3d<Type> & spl)
{
  if (!ostr)
  {
...
template <class Type>
std::istream &
operator>>(std::istream & istr, vgl_cubic_spline_3d<Type> & spl)
{
  if (!istr)
  {
