...
template <class T>
double
vgl_distance_to_non_closed_polygon(const T px[], const T py[], unsigned n, T x, T y)
{
  double dd = -1;
...
template <class T>
double
vgl_distance_to_closed_polygon(const T px[], const T py[], unsigned n, T x, T y)
{
  double dd = vgl_distance_to_linesegment(px[n - 1], py[n - 1], px[0], py[0], x, y);
  for (unsigned i = 0; i + 1 < n; ++i)
  {
    double nd = vgl_distance_to_linesegment(px[i], py[i], px[i + 1], py[i + 1], x, y);
    if (nd < dd)
...
{
  double dd = vgl_distance_to_linesegment(px[n - 1], py[n - 1], pz[n - 1], px[0], py[0], pz[0], x, y, z);
  for (unsigned i = 0; i + 1 < n; ++i)
  {
    double nd = vgl_distance_to_linesegment(px[i], py[i], pz[i], px[i + 1], py[i + 1], pz[i + 1], x, y, z);
    if (nd < dd)
...
{
  if (l.c() == 0)
    return 0.0; // no call to sqrt if not necessary
  else
    return std::abs(static_cast<double>(l.c())) / std::sqrt(static_cast<double>(l.a() * l.a() + l.b() * l.b()));
}
...
vgl_distance_origin(const vgl_homg_plane_3d<T> & pl)
{
  if (pl.d() == 0)
    ...
    return std::abs(static_cast<double>(pl.d())) /
           std::sqrt(static_cast<double>(pl.a() * pl.a() + pl.b() * pl.b() + pl.c() * pl.c()));
}
...
vgl_distance(const vgl_line_2d<T> & l, const vgl_point_2d<T> & p)
{
  ...
  if (num == 0)
    ...
    return std::abs(static_cast<double>(num)) / std::sqrt(static_cast<double>(l.a() * l.a() + l.b() * l.b()));
}
...
vgl_distance(const vgl_homg_line_2d<T> & l, const vgl_homg_point_2d<T> & p)
{
  ...
  if (num == 0)
    return 0.0; // always return 0 when point on line, even at infinity
  else
    return std::abs(static_cast<double>(num)) / std::sqrt(static_cast<double>(l.a() * l.a() + l.b() * l.b())) /
           p.w(); // could be inf
}
...
vgl_distance(const vgl_plane_3d<T> & l, const vgl_point_3d<T> & p)
{
  ...
  if (num == 0)
    ...
    return std::abs(static_cast<double>(num)) /
           std::sqrt(static_cast<double>(l.nx() * l.nx() + l.ny() * l.ny() + l.nz() * l.nz()));
}
...
vgl_distance(const vgl_homg_plane_3d<T> & l, const vgl_homg_point_3d<T> & p)
{
  ...
  if (num == 0)
    return 0.0; // always return 0 when point on plane, even at infinity
  else
    return std::abs(static_cast<double>(num / p.w())) /
           std::sqrt(static_cast<double>(l.nx() * l.nx() + l.ny() * l.ny() + l.nz() * l.nz()));
}
template <class T>
double
...
template <class T>
double
vgl_distance(const vgl_polygon<T> & poly, const vgl_point_2d<T> & point, bool closed)
{
  double dist = -1;
    ...
    double dd =
      closed
        ? vgl_distance_to_linesegment(
            sheet[n - 1].x(), sheet[n - 1].y(), sheet[0].x(), sheet[0].y(), point.x(), point.y())
        : vgl_distance_to_linesegment(sheet[0].x(), sheet[0].y(), sheet[1].x(), sheet[1].y(), point.x(), point.y());
    for (unsigned int i = 0; i + 1 < n; ++i)
    {
      double nd = vgl_distance_to_linesegment(
        sheet[i].x(), sheet[i].y(), sheet[i + 1].x(), sheet[i + 1].y(), point.x(), point.y());
      if (nd < dd)
        dd = nd;
    }
    if (dist < 0 || dd < dist)
...
{
  std::pair<vgl_homg_point_3d<T>, vgl_homg_point_3d<T>> pp = vgl_closest_points(line1, line2);
  if (pp.first.w() != 0)
...
{
  return vgl_distance_to_linesegment(l.point1().x(), l.point1().y(), l.point2().x(), l.point2().y(), p.x(), p.y());
}
...
{
  return vgl_distance_to_linesegment(l.point1().x(),
                                     l.point1().y(),
                                     l.point1().z(),
                                     l.point2().x(),
                                     l.point2().y(),
