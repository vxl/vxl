...
template <class T>
void
vgl_ellipse_scan_iterator<T>::reset()
{
  // The max value.
  T y0;
...
template <class T>
bool
vgl_ellipse_scan_iterator<T>::next()
{
  --y_;
  if (y_ < min_y_)
    return false;
  ...
  if (A > 0)
  {
    ...
    if (D < 0)
      D = 0; // could be slightly < 0 due to rounding errors
...
#define VGL_ELLIPSE_SCAN_ITERATOR_INSTANTIATE(T) template class vgl_ellipse_scan_iterator<T>
