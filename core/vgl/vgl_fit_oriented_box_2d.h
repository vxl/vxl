...
public:
  ...
  vgl_fit_oriented_box_2d(const vgl_polygon<T> & poly, double dtheta = default_dtheta);
...
private:
  bool fixed_theta_{ false };
  void
  fit_obox(T theta_rad = T(0));
  bool fit_valid_{ false };
  double dtheta_{ default_dtheta };
