...
template <class T>
vgl_fit_oriented_box_2d<T>::vgl_fit_oriented_box_2d(const vgl_polygon<T> & poly, double dtheta)
  : fixed_theta_(false)
  , fit_valid_(false)
  , dtheta_(dtheta)
{
...
template <class T>
void
vgl_fit_oriented_box_2d<T>::fit_obox(T theta_rad)
{
  ...
  for (T theta = low_theta; theta <= high_theta; theta += dtheta_)
  {
    ...
    if (area < min_area)
    {
      ...
      if (w < h)
      {
        width = h;
        height = w;
        pmaj0.set(cent.x(), cent.y() - width / 2.0);
        pmaj1.set(cent.x(), cent.y() + width / 2.0);
      }
...
vgl_fit_oriented_box_2d<T>::fitted_box()
{
  fixed_theta_ = false;
  if (!fit_valid_)
  {
