...
public:
  ...
  //
  vgl_frustum_3d(const std::vector<vgl_ray_3d<Type>> & corner_rays, const vgl_vector_3d<Type> & norm, Type d0, Type d1);

  //: Equality test
  inline bool
  operator==(const vgl_frustum_3d<Type> & other) const;
  ...
  const std::vector<vgl_plane_3d<Type>> &
  surface_planes() const
  {
    return surface_planes_;
  }

  const vgl_plane_3d<Type> &
  near_plane() const
  {
...
template <class Type>
std::ostream &
operator<<(std::ostream & s, const vgl_frustum_3d<Type> & p);
