...
template <class Type>
bool
vgl_homg_line_3d_2_points<Type>::operator==(const vgl_homg_line_3d_2_points<Type> & other) const
{
  if (this == &other)
    return true;
...
template <class Type>
void
vgl_homg_line_3d_2_points<Type>::force_point2_infinite() const
{
  // Require tolerance on ideal point
  Type tol = vgl_tolerance<Type>::position;
  if (point_infinite_.w() < tol && point_infinite_.w() > -tol)
    return;                                                     // already OK
  ...
  Type a = point_finite_.x(), a1 = point_infinite_.x(), b = point_finite_.y(), b1 = point_infinite_.y(),
       c = point_finite_.z(), c1 = point_infinite_.z(), d = point_finite_.w(), d1 = point_infinite_.w();
...
intersection(const vgl_homg_line_3d_2_points<Type> & l1, const vgl_homg_line_3d_2_points<Type> & l2)
{
  assert(concurrent(l1, l2));
  Type a0 = l1.point_finite().x(), a1 = l1.point_infinite().x(), a2 = l2.point_finite().x(),
       a3 = l2.point_infinite().x(), b0 = l1.point_finite().y(), b1 = l1.point_infinite().y(),
       b2 = l2.point_finite().y(), b3 = l2.point_infinite().y(), c0 = l1.point_finite().z(),
       c1 = l1.point_infinite().z(), c2 = l2.point_finite().z(), c3 = l2.point_infinite().z(),
       d0 = l1.point_finite().w(), d1 = l1.point_infinite().w(), d2 = l2.point_finite().w(),
       d3 = l2.point_infinite().w();
...
template <class Type>
std::ostream &
operator<<(std::ostream & s, const vgl_homg_line_3d_2_points<Type> & p)
{
  return s << "<vgl_homg_line_3d_2_points " << p.point_finite() << p.point_infinite() << " >";
}
