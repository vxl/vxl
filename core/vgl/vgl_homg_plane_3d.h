...
public:
  ...
  inline vgl_homg_plane_3d(Type ta, Type tb, Type tc, Type td)
    : a_(ta)
    , b_(tb)
    , c_(tc)
    , d_(td)
  ...
  inline vgl_homg_plane_3d(const Type v[4])
    : a_(v[0])
    , b_(v[1])
    , c_(v[2])
    , d_(v[3])
  ...
  vgl_homg_plane_3d(const vgl_plane_3d<Type> & pl);

  //: Construct from normal and a point
  //  The given point must not be at infinity.
  vgl_homg_plane_3d(const vgl_vector_3d<Type> & n, const vgl_homg_point_3d<Type> & p);

  //: Construct from three non-collinear points
  vgl_homg_plane_3d(const vgl_homg_point_3d<Type> & p1,
                    const vgl_homg_point_3d<Type> & p2,
                    const vgl_homg_point_3d<Type> & p3);

  //: Construct from two concurrent lines
  vgl_homg_plane_3d(const vgl_homg_line_3d_2_points<Type> & l1, const vgl_homg_line_3d_2_points<Type> & l2);
  ...
  inline void
  set(Type ta, Type tb, Type tc, Type td)
  {
    assert(ta || tb || tc || td);
    a_ = ta;
    b_ = tb;
  ...
  bool
  operator==(const vgl_homg_plane_3d<Type> & pl) const;
  inline bool
  operator!=(const vgl_homg_plane_3d<Type> & pl) const
  {
    return !operator==(pl);
...
#define vgl_Abs(x) ((x) < 0 ? -(x) : (x)) // avoid #include of vcl_cmath.h AND vcl_cstdlib.h
    return vgl_Abs(a()) <= tol * vgl_Abs(d()) && vgl_Abs(b()) <= tol * vgl_Abs(d()) &&
           vgl_Abs(c()) <= tol * vgl_Abs(d());
#undef vgl_Abs
  }

  inline vgl_vector_3d<double>
  normal() const
  {
    return normalized(vgl_vector_3d<double>(a(), b(), c()));
  }
...
template <class Type>
inline bool
is_ideal(const vgl_homg_plane_3d<Type> & p, Type tol = (Type)0)
{
  return p.ideal(tol);
}
...
template <class Type>
std::istream &
operator>>(std::istream & is, vgl_homg_plane_3d<Type> & p);
