...
public:
  ...
  inline vgl_homg_point_1d(T px, T pw = T(1))
    : x_(px)
    , w_(pw)
  {}

  //: Construct from homogeneous 2-array.
  inline vgl_homg_point_1d(const T v[2])
    : x_(v[0])
    , w_(v[1])
  {}
...
template <class T>
std::ostream &
operator<<(std::ostream & s, const vgl_homg_point_1d<T> & p);

//: Read x w from stream
// \relatesalso vgl_homg_point_1d
template <class T>
std::istream &
operator>>(std::istream & s, vgl_homg_point_1d<T> & p);
...
template <class T>
inline bool
is_ideal(const vgl_homg_point_1d<T> & p, T tol = T(0))
{
  return p.ideal(tol);
}
...
template <class T>
inline T
operator-(const vgl_homg_point_1d<T> & p1, const vgl_homg_point_1d<T> & p2)
{
  assert(p1.w() && p2.w());
...
// \relatesalso vgl_homg_point_1d
template <class T>
inline vgl_homg_point_1d<T>
operator+(const vgl_homg_point_1d<T> & p, T v)
{
...
template <class T>
inline vgl_homg_point_1d<T> &
operator+=(vgl_homg_point_1d<T> & p, T v)
{
  p.set(p.x() + v * p.w(), p.w());
  return p;
}
...
template <class T>
inline double
cross_ratio(const vgl_homg_point_1d<T> & p1,
            const vgl_homg_point_1d<T> & p2,
            const vgl_homg_point_1d<T> & p3,
            const vgl_homg_point_1d<T> & p4)
{
...
template <class T>
inline double
ratio(const vgl_homg_point_1d<T> & p1, const vgl_homg_point_1d<T> & p2, const vgl_homg_point_1d<T> & p3)
{
  return (p3 - p1) / (p2 - p1);
}
...
template <class T>
inline bool
collinear(const vgl_homg_point_1d<T> &, const vgl_homg_point_1d<T> &, const vgl_homg_point_1d<T> &)
{
  return true;
}
...
template <class T>
inline vgl_homg_point_1d<T>
midpoint(const vgl_homg_point_1d<T> & p1, const vgl_homg_point_1d<T> & p2, T f = 0.5)
{
  return p1 + f * (p2 - p1);
}
...
template <class T>
inline vgl_homg_point_1d<T>
centre(const vgl_homg_point_1d<T> & p1, const vgl_homg_point_1d<T> & p2)
{
  if (p1 == p2)
    return p1;
...
template <class T>
inline vgl_homg_point_1d<T>
centre(const std::vector<vgl_homg_point_1d<T>> & v)
{
  int n = v.size();
