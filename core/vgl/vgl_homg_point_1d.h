...
public:
  ...
  vgl_homg_point_1d() : x_(0), w_(T(1)) {}

  //: Construct from one (nonhomogeneous) or two (homogeneous) T's.
  vgl_homg_point_1d(T px, T pw = T(1)) : x_(px), w_(pw) {}

  //: Construct from homogeneous 2-array.
  vgl_homg_point_1d(const T v[2]) : x_(v[0]), w_(v[1]) {}

#if 0
  // Default copy constructor
  vgl_homg_point_1d(const vgl_homg_point_1d<T>& that) : x_(p.x()), w_(p.w()) {}

  // Destructor
  ~vgl_homg_point_1d() {}

  // Default assignment operator
  vgl_homg_point_1d<T>& operator=(const vgl_homg_point_1d<T>& p) {
  ...
  bool operator!=(vgl_homg_point_1d<T> const& p) const {return !operator==(p);}
...
// The method checks whether |w| <= tol * |x|
template <class T>
inline bool is_ideal(vgl_homg_point_1d<T> const& p, T tol = T(0)) { return p.ideal(); }
...
template <class T>
inline T operator-(vgl_homg_point_1d<T> const& p1,
...
template <class T>
inline vgl_homg_point_1d<T> operator+(vgl_homg_point_1d<T> const& p, T v) {
...
// If the point is at infinity, nothing happens.
template <class T>
inline vgl_homg_point_1d<T>& operator+=(vgl_homg_point_1d<T>& p, T v) {
...
template <class T>
inline double ratio(vgl_homg_point_1d<T> const& p1,
                    vgl_homg_point_1d<T> const& p2,
...
template <class T>
inline bool collinear(vgl_homg_point_1d<T> const&,
...
template <class T>
inline vgl_homg_point_1d<T> midpoint(vgl_homg_point_1d<T> const& p1,
                                     vgl_homg_point_1d<T> const& p2,
                                     T f = 0.5) {
  return p1 + f*(p2-p1);
}
...
template <class T>
inline vgl_homg_point_1d<T> centre(vgl_homg_point_1d<T> const& p1,
                                   vgl_homg_point_1d<T> const& p2) {
  if (p1 == p2) return p1;
...
template <class T>
inline vgl_homg_point_1d<T> centre(vcl_vector<vgl_homg_point_1d<T> > const& v) {
  int n=v.size();
