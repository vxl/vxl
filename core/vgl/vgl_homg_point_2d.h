...
public:
  ...
  inline vgl_homg_point_2d(const T v[3])
    : x_(v[0])
    , y_(v[1])
    , w_(v[2])
  {}
  ...
  inline vgl_homg_point_2d(const vgl_vector_2d<T> & v)
    : x_(v.x())
    , y_(v.y())
    , w_(0)
  {}
  ...
  inline explicit vgl_homg_point_2d(const vgl_point_2d<T> & p)
    : x_(p.x())
    , y_(p.y())
    , w_((T)1)
  {}
  ...
  vgl_homg_point_2d(const vgl_homg_line_2d<T> & l1, const vgl_homg_line_2d<T> & l2);
  ...
  {
    return (this == &p) || (x() * p.w() == w() * p.x() && y() * p.w() == w() * p.y() && y() * p.x() == x() * p.y());
  }

  inline bool
  operator!=(const vgl_homg_point_2d<T> & other) const
  {
...
template <class T>
std::ostream &
operator<<(std::ostream & s, const vgl_homg_point_2d<T> & p);

//: Read x y w from stream
// \relatesalso vgl_homg_point_2d
template <class T>
std::istream &
operator>>(std::istream & s, vgl_homg_point_2d<T> & p);
...
template <class T>
inline bool
is_ideal(const vgl_homg_point_2d<T> & p, T tol = (T)0)
{
  return p.ideal(tol);
}
...
// \relatesalso vgl_homg_point_2d
template <class T>
inline vgl_vector_2d<T>
operator-(const vgl_homg_point_2d<T> & p1, const vgl_homg_point_2d<T> & p2)
{
  assert(p1.w() && p2.w());
  return vgl_vector_2d<T>(p1.x() / p1.w() - p2.x() / p2.w(), p1.y() / p1.w() - p2.y() / p2.w());
}
...
operator+(const vgl_homg_point_2d<T> & p, const vgl_vector_2d<T> & v)
{
...
template <class T>
inline vgl_homg_point_2d<T> &
operator+=(vgl_homg_point_2d<T> & p, const vgl_vector_2d<T> & v)
{
  p.set(p.x() + v.x() * p.w(), p.y() + v.y() * p.w(), p.w());
  return p;
}
...
template <class T>
inline bool
collinear(const vgl_homg_point_2d<T> & p1, const vgl_homg_point_2d<T> & p2, const vgl_homg_point_2d<T> & p3)
{
  return (p1.x() * p2.y() - p1.y() * p2.x()) * p3.w() + (p3.x() * p1.y() - p3.y() * p1.x()) * p2.w() +
           (p2.x() * p3.y() - p2.y() * p3.x()) * p1.w() ==
         0;
}
...
template <class T>
inline double
ratio(const vgl_homg_point_2d<T> & p1, const vgl_homg_point_2d<T> & p2, const vgl_homg_point_2d<T> & p3)
{
  return (p3 - p1) / (p2 - p1);
}
...
template <class T>
inline vgl_homg_point_2d<T>
midpoint(const vgl_homg_point_2d<T> & p1, const vgl_homg_point_2d<T> & p2, T f = (T)0.5)
{
  return p1 + f * (p2 - p1);
}
...
template <class T>
inline vgl_homg_point_2d<T>
centre(const vgl_homg_point_2d<T> & p1, const vgl_homg_point_2d<T> & p2)
{
  return vgl_homg_point_2d<T>(
    p1.x() * p2.w() + p2.x() * p1.w(), p1.y() * p2.w() + p2.y() * p1.w(), p1.w() * p2.w() * 2);
}
...
template <class T>
inline vgl_homg_point_2d<T>
centre(const std::vector<vgl_homg_point_2d<T>> & v)
{
  int n = v.size();
  assert(n > 0); // it is *not* correct to return the point (0,0) when n==0.
  T x = 0, y = 0;
  for (int i = 0; i < n; ++i)
    x += v[i].x() / v[i].w(), y += v[i].y() / v[i].w();
