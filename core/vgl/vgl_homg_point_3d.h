...
public:
  ...
  inline vgl_homg_point_3d(const Type v[4])
    : x_(v[0])
    , y_(v[1])
    , z_(v[2])
    , w_(v[3])
  ...
  inline vgl_homg_point_3d(const vgl_vector_3d<Type> & v)
    : x_(v.x())
    , y_(v.y())
    , z_(v.z())
    , w_(0)
  ...
  inline explicit vgl_homg_point_3d(const vgl_point_3d<Type> & p)
    : x_(p.x())
    , y_(p.y())
    , z_(p.z())
    , w_((Type)1)
  ...
  vgl_homg_point_3d(const vgl_homg_plane_3d<Type> & l1,
                    const vgl_homg_plane_3d<Type> & l2,
                    const vgl_homg_plane_3d<Type> & l3);
  ...
  bool
  operator==(const vgl_homg_point_3d<Type> & other) const;
  inline bool
  operator!=(const vgl_homg_point_3d<Type> & other) const
  {
  ...
  inline void
  set(Type px, Type py, Type pz, Type pw = (Type)1)
  {
    x_ = px;
    y_ = py;
    z_ = pz;
...
#define vgl_Abs(x) ((x) < 0 ? -(x) : (x)) // avoid #include of vcl_cmath.h AND vcl_cstdlib.h
    return vgl_Abs(w()) <= tol * vgl_Abs(x()) || vgl_Abs(w()) <= tol * vgl_Abs(y()) ||
           vgl_Abs(w()) <= tol * vgl_Abs(z());
#undef vgl_Abs
  }

  inline bool
  get_nonhomogeneous(double & vx, double & vy, double & vz) const
  {
    if (w() == 0)
    {
#ifdef DEBUG
...
template <class Type>
std::ostream &
operator<<(std::ostream & s, const vgl_homg_point_3d<Type> & p);

//: Read x y z w from stream
// \relatesalso vgl_homg_point_3d
template <class Type>
std::istream &
operator>>(std::istream & s, vgl_homg_point_3d<Type> & p);
...
template <class Type>
inline bool
is_ideal(const vgl_homg_point_3d<Type> & p, Type tol = (Type)0)
{
  return p.ideal(tol);
}
...
template <class Type>
inline bool
coplanar(const vgl_homg_point_3d<Type> & p1,
         const vgl_homg_point_3d<Type> & p2,
         const vgl_homg_point_3d<Type> & p3,
         const vgl_homg_point_3d<Type> & p4)
{
  return ((p1.x() * p2.y() - p1.y() * p2.x()) * p3.z() + (p3.x() * p1.y() - p3.y() * p1.x()) * p2.z() +
          (p2.x() * p3.y() - p2.y() * p3.x()) * p1.z()) *
             p4.w() -
           ((p4.x() * p1.y() - p4.y() * p1.x()) * p2.z() + (p2.x() * p4.y() - p2.y() * p4.x()) * p1.z() +
            (p1.x() * p2.y() - p1.y() * p2.x()) * p4.z()) *
             p3.w() +
           ((p3.x() * p4.y() - p3.y() * p4.x()) * p1.z() + (p1.x() * p3.y() - p1.y() * p3.x()) * p4.z() +
            (p4.x() * p1.y() - p4.y() * p1.x()) * p3.z()) *
             p2.w() -
           ((p2.x() * p3.y() - p2.y() * p3.x()) * p4.z() + (p4.x() * p2.y() - p4.y() * p2.x()) * p3.z() +
            (p3.x() * p4.y() - p3.y() * p4.x()) * p2.z()) *
             p1.w() ==
         0;
}
...
operator-(const vgl_homg_point_3d<Type> & p1, const vgl_homg_point_3d<Type> & p2)
{
  assert(p1.w() && p2.w());
  return vgl_vector_3d<Type>(
    p1.x() / p1.w() - p2.x() / p2.w(), p1.y() / p1.w() - p2.y() / p2.w(), p1.z() / p1.w() - p2.z() / p2.w());
}
...
operator+(const vgl_homg_point_3d<Type> & p, const vgl_vector_3d<Type> & v)
{
  return vgl_homg_point_3d<Type>(p.x() + v.x() * p.w(), p.y() + v.y() * p.w(), p.z() + v.z() * p.w(), p.w());
}
...
template <class Type>
inline vgl_homg_point_3d<Type> &
operator+=(vgl_homg_point_3d<Type> & p, const vgl_vector_3d<Type> & v)
{
  p.set(p.x() + v.x() * p.w(), p.y() + v.y() * p.w(), p.z() + v.z() * p.w(), p.w());
  return p;
}
...
template <class T>
double
cross_ratio(const vgl_homg_point_3d<T> & p1,
            const vgl_homg_point_3d<T> & p2,
            const vgl_homg_point_3d<T> & p3,
            const vgl_homg_point_3d<T> & p4);

//: Are three points collinear, i.e., do they lie on a common line?
// \relatesalso vgl_homg_point_3d
template <class Type>
bool
collinear(const vgl_homg_point_3d<Type> & p1, const vgl_homg_point_3d<Type> & p2, const vgl_homg_point_3d<Type> & p3);
...
template <class Type>
inline double
ratio(const vgl_homg_point_3d<Type> & p1, const vgl_homg_point_3d<Type> & p2, const vgl_homg_point_3d<Type> & p3)
{
  return (p3 - p1) / (p2 - p1);
}
...
template <class Type>
inline vgl_homg_point_3d<Type>
midpoint(const vgl_homg_point_3d<Type> & p1, const vgl_homg_point_3d<Type> & p2, Type f = (Type)0.5)
{
  return p1 + f * (p2 - p1);
}
...
template <class Type>
inline vgl_homg_point_3d<Type>
centre(const vgl_homg_point_3d<Type> & p1, const vgl_homg_point_3d<Type> & p2)
{
...
template <class Type>
inline vgl_homg_point_3d<Type>
centre(const std::vector<vgl_homg_point_3d<Type>> & v)
{
  int n = v.size();
