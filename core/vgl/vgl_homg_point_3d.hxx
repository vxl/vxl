...
template <class Type>
vgl_homg_point_3d<Type>::vgl_homg_point_3d(const vgl_homg_plane_3d<Type> & l1,
...
template <class Type>
bool
vgl_homg_point_3d<Type>::operator==(const vgl_homg_point_3d<Type> & other) const
{
  return (this == &other) || (x() * other.y() == y() * other.x() && x() * other.z() == z() * other.x() &&
                              x() * other.w() == w() * other.x() && y() * other.z() == z() * other.y() &&
                              y() * other.w() == w() * other.y() && z() * other.w() == w() * other.z());
}
            ...
            const vgl_homg_point_3d<T> & p4)
{
  ...
  if (Den_x == Den_y && Den_y == Den_z)
    return (Num_x + Num_y + Num_z) / 3 / Den_x;
  else
    return (Den_x * Num_x + Den_y * Num_y + Den_z * Num_z) / (Den_x * Den_x + Den_y * Den_y + Den_z * Den_z);
}
...
{
  return s << " <vgl_homg_point_3d (" << p.x() << ',' << p.y() << ',' << p.z() << ',' << p.w() << ") >";
}
...
#define VGL_HOMG_POINT_3D_INSTANTIATE(T)                                                                             \
  ...
  template double cross_ratio(vgl_homg_point_3d<T> const &,                                                          \
