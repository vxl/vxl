...
inline bool
vgl_near_zero(double x)
{
  return x < eps && x > -eps;
}
inline bool
vgl_near_eq(double x, double y)
{
  return vgl_near_zero(x - y);
}
...
template <class T>
vgl_box_2d<T>
vgl_intersection(const vgl_box_2d<T> & b1, const vgl_box_2d<T> & b2)
{
...
vgl_intersection(const vgl_box_3d<T> & box,
                 const vgl_infinite_line_3d<T> & line_3d,
                 vgl_point_3d<T> & p0,
                 vgl_point_3d<T> & p1)
{
  vgl_point_3d<T> lpt = line_3d.point();
  vgl_vector_3d<T> di = line_3d.direction();
  vgl_point_3d<double> dpt(static_cast<double>(lpt.x()), static_cast<double>(lpt.y()), static_cast<double>(lpt.z()));
  vgl_vector_3d<double> dir(static_cast<double>(di.x()), static_cast<double>(di.y()), static_cast<double>(di.z()));
  vgl_infinite_line_3d<double> dline_3d(dpt, dir);
  // expand box by epsilon tolerance
  double xmin = box.min_x(), xmax = box.max_x(), ymin = box.min_y(), ymax = box.max_y(), zmin = box.min_z(),
         zmax = box.max_z();
  ...
  vgl_vector_3d<double> vxmin(-1.0, 0.0, 0.0), vxmax(1.0, 0.0, 0.0), vymin(0.0, -1.0, 0.0), vymax(0.0, 1.0, 0.0),
    vzmin(0.0, 0.0, -1.0), vzmax(0.0, 0.0, 1.0);
  vgl_plane_3d<double> pl_xmin(vxmin, minp), pl_xmax(vxmax, maxp), pl_ymin(vymin, minp), pl_ymax(vymax, maxp),
    pl_zmin(vzmin, minp), pl_zmax(vzmax, maxp);
  ...
  // y-z face at xmin
  if (xmin_good && pt_xmin.y() >= ymin && pt_xmin.y() <= ymax && pt_xmin.z() >= zmin && pt_xmin.z() <= zmax)
  {
    // dp0 = pt_xmin; // not needed: already set when dp0 was initialised
    ++npts;
  }
  // y-z face at xmax
  if (xmax_good && pt_xmax.y() >= ymin && pt_xmax.y() <= ymax && pt_xmax.z() >= zmin && pt_xmax.z() <= zmax)
  {
    if (npts == 0)
      dp0 = pt_xmax;
  ...
  // x-z face at ymin
  if (ymin_good && pt_ymin.x() >= xmin && pt_ymin.x() <= xmax && pt_ymin.z() >= zmin && pt_ymin.z() <= zmax)
  {
    if (npts == 0)
    {
      dp0 = pt_ymin;
    ...
    else if (npts == 1)
    {
      ...
      if (sqr_length(pt_ymin - dp0) > sqr_length(dp1 - dp0))
        dp1 = pt_ymin;
  }
  // x-z face at ymax
  if (ymax_good && pt_ymax.x() >= xmin && pt_ymax.x() <= xmax && pt_ymax.z() >= zmin && pt_ymax.z() <= zmax)
  {
    if (npts == 0)
    {
      dp0 = pt_ymax;
    ...
    else if (npts == 1)
    {
      ...
      if (sqr_length(pt_ymax - dp0) > sqr_length(dp1 - dp0))
        dp1 = pt_ymax;
  }
  // x-y face at zmin
  if (zmin_good && pt_zmin.x() >= xmin && pt_zmin.x() <= xmax && pt_zmin.y() >= ymin && pt_zmin.y() <= ymax)
  {
    if (npts == 0)
    {
      dp0 = pt_zmin;
    ...
    else if (npts == 1)
    {
      ...
      if (sqr_length(pt_zmin - dp0) > sqr_length(dp1 - dp0))
        dp1 = pt_zmin;
  }

  // x-y face at zmax
  if (zmax_good && pt_zmax.x() >= xmin && pt_zmax.x() <= xmax && pt_zmax.y() >= ymin && pt_zmax.y() <= ymax)
  {
    if (npts == 0)
    {
      dp0 = pt_zmax;
    ...
    else if (npts == 1)
    {
      ...
      if (sqr_length(pt_zmax - dp0) > sqr_length(dp1 - dp0))
        dp1 = pt_zmax;
  }

  if (npts == 2)
  {
    p0.set(static_cast<T>(dp0.x()), static_cast<T>(dp0.y()), static_cast<T>(dp0.z()));
    p1.set(static_cast<T>(dp1.x()), static_cast<T>(dp1.y()), static_cast<T>(dp1.z()));
    return true;
  }
  else
    return false;
...
template <class T>
vgl_box_3d<T>
vgl_intersection(const vgl_box_3d<T> & b1, const vgl_box_3d<T> & b2)
{
  T xmin = b1.min_x() > b2.min_x() ? b1.min_x() : b2.min_x();
  T ymin = b1.min_y() > b2.min_y() ? b1.min_y() : b2.min_y();
...
template <class Type>
bool
vgl_intersection(const vgl_box_2d<Type> & box,
                 ...
                 vgl_point_2d<Type> & p1)
{
  ...
  // Case CX
  if (inside_xmin && inside_xmax && !(vgl_near_eq(y_xmin_int, ymin) && vgl_near_eq(y_xmax_int, ymax)))
  {
  ...
  // Case CY
  if (inside_ymin && inside_ymax && !(vgl_near_eq(x_ymin_int, xmin) && vgl_near_eq(x_ymax_int, xmax)))
  {
  ...
  // Case C00
  if (inside_xmin && inside_ymin && !(inside_xmax && inside_ymax))
  {
    p0.set(static_cast<Type>(xmin), static_cast<Type>(y_xmin_int));
  ...
  // Case C01
  if (inside_xmin && inside_ymax && !(inside_xmax && inside_ymin))
  {
    p0.set(static_cast<Type>(xmin), static_cast<Type>(y_xmin_int));
    p1.set(static_cast<Type>(x_ymax_int), static_cast<Type>(ymax));
    return true;
  ...
  // Case C10
  if (inside_ymin && inside_xmax && !(inside_xmin && inside_ymax))
  {
    p0.set(static_cast<Type>(x_ymin_int), static_cast<Type>(ymin));
    p1.set(static_cast<Type>(xmax), static_cast<Type>(y_xmax_int));
    return true;
  ...
  // Case C11
  if (inside_ymax && inside_xmax && !(inside_xmin && inside_ymin))
  {
...
vgl_intersection(const vgl_box_2d<T> & box,
                 const vgl_line_segment_2d<T> & line_seg,
...
vgl_intersection(const vgl_line_3d_2_points<T> & l1, const vgl_line_3d_2_points<T> & l2)
{
  ...
  T a0 = l1.point1().x(), a1 = l1.point2().x(), a2 = l2.point1().x(), a3 = l2.point2().x(), b0 = l1.point1().y(),
    b1 = l1.point2().y(), b2 = l2.point1().y(), b3 = l2.point2().y(), c0 = l1.point1().z(), c1 = l1.point2().z(),
    c2 = l2.point1().z(), c3 = l2.point2().z();
  ...
  return vgl_point_3d<T>(
    ((t1 - t2) * a2 + t2 * a3) / t1, ((t1 - t2) * b2 + t2 * b3) / t1, ((t1 - t2) * c2 + t2 * c3) / t1);
}
...
vgl_intersection(const vgl_line_3d_2_points<T> & line, const vgl_plane_3d<T> & plane)
{
  ...
  else
  {
    ...
    double numer =
      -plane.a() * line.point1().x() - plane.b() * line.point1().y() - plane.c() * line.point1().z() - plane.d();
...
vgl_intersection(const vgl_infinite_line_3d<T> & line, const vgl_plane_3d<T> & plane, vgl_point_3d<T> & i_pt)
{
  ...
  double numer = -plane.a() * pt.x() - plane.b() * pt.y() - plane.c() * pt.z() - plane.d();
...
vgl_intersection(const vgl_plane_3d<T> & plane0, const vgl_plane_3d<T> & plane1, vgl_infinite_line_3d<T> & line)
{
  ...
  vgl_point_3d<T> p0(static_cast<T>(p0d.x()), static_cast<T>(p0d.y()), static_cast<T>(p0d.z()));
  vgl_vector_3d<T> tt(static_cast<T>(t.x()), static_cast<T>(t.y()), static_cast<T>(t.z()));
...
vgl_intersection(const vgl_box_2d<T> & b, const vgl_polygon<T> & poly)
{
  ...
  hit =
    poly.contains(minx, miny) || poly.contains(maxx, maxy) || poly.contains(minx, maxy) || poly.contains(maxx, miny);
  if (hit)
    return true;
...
#define VGL_INTERSECTION_BOX_INSTANTIATE(T)                                                                            \
  ...
  template bool vgl_intersection(                                                                                      \
    vgl_box_3d<T> const &, vgl_infinite_line_3d<T> const &, vgl_point_3d<T> &, vgl_point_3d<T> &);                     \
...
#define VGL_INTERSECTION_INSTANTIATE(T)                                                                                \
  ...
  template vgl_point_3d<T> vgl_intersection(                                                                           \
    vgl_plane_3d<T> const &, vgl_plane_3d<T> const &, vgl_plane_3d<T> const &);                                        \
  template unsigned vgl_intersection(                                                                                  \
    vgl_box_2d<T> const &, vgl_line_segment_2d<T> const &, vgl_point_2d<T> &, vgl_point_2d<T> &);                      \
  template bool vgl_intersection(vgl_line_2d<T> const &, vgl_line_2d<T> const &, vgl_point_2d<T> &);                   \
  template bool vgl_intersection(                                                                                      \
    vgl_point_2d<T> const &, vgl_point_2d<T> const &, vgl_point_2d<T> const &, vgl_point_2d<T> const &, double);       \
