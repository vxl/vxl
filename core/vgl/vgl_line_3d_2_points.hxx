...
template <class Type>
bool
vgl_line_3d_2_points<Type>::operator==(const vgl_line_3d_2_points<Type> & other) const
{
  if (this == &other)
    return true;
  // It suffices to check that the points are collinear:
  return collinear(point1(), point2(), other.point1()) && collinear(point1(), point2(), other.point2());
}
...
template <class Type>
std::ostream &
operator<<(std::ostream & s, const vgl_line_3d_2_points<Type> & p)
{
  return s << "<vgl_line_3d_2_points " << p.point1() << p.point2() << " >";
}
