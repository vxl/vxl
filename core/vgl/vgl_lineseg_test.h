...
//   Mar.2008 - Ibrahim Eden - bug fix: bool vgl_lineseg_test_line(vgl_line_2d<T> const& l1,vgl_line_segment_2d<T>
//   const& l2) Mar.2009 - Dirk Steckhan - bug fix in vgl_lineseg_test_point (missing sqrt)
// \endverbatim
...
template <class T>
bool
vgl_lineseg_test_line(T x1, T y1, T x2, T y2, T x3, T y3, T x4, T y4);

//: true if the linesegment joining [1], [2] meets the linesegment joining [3], [4].
// End points are considered to belong to a line segment.
template <class T>
bool
vgl_lineseg_test_lineseg(T x1, T y1, T x2, T y2, T x3, T y3, T x4, T y4);
...
template <class T>
inline bool
vgl_lineseg_test_line(const vgl_line_2d<T> & l1, const vgl_line_segment_2d<T> & l2)
{
  vgl_point_2d<T> l1_p1, l1_p2;
  l1.get_two_points(l1_p1, l1_p2);
  return vgl_lineseg_test_line(
    l1_p1.x(), l1_p1.y(), l1_p2.x(), l1_p2.y(), l2.point1().x(), l2.point1().y(), l2.point2().x(), l2.point2().y());
}
...
template <class T>
bool
vgl_lineseg_test_point(const vgl_point_2d<T> & p, const vgl_line_segment_2d<T> & lseg);
...
template <class T>
inline bool
vgl_lineseg_test_lineseg(const vgl_line_segment_2d<T> & l1, const vgl_line_segment_2d<T> & l2)
{
  return vgl_lineseg_test_lineseg(l1.point1().x(),
                                  l1.point1().y(),
                                  l1.point2().x(),
                                  l1.point2().y(),
                                  l2.point1().x(),
