...
template <class T>
static inline bool
inbetween(T x1, T y1, T x2, T y2, T x3, T y3)
{
...
vgl_lineseg_test_lineseg(T x1, T y1, T x2, T y2, T x3, T y3, T x4, T y4)
{
  ...
  return (((a <= 0 && b > 0) || (a >= 0 && b < 0) || (a < 0 && b >= 0) || (a > 0 && b <= 0)) &&
         ...
         ( // the above two conditions are only sufficient for noncollinear line segments! - PVr
           a == 0 && b == 0 && c == 0 && d == 0 &&
           (inbetween(px1, py1, px2, py2, px3, py3) || inbetween(px1, py1, px2, py2, px4, py4) ||
            inbetween(px3, py3, px4, py4, px1, py1) || inbetween(px3, py3, px4, py4, px2, py2)));
}
