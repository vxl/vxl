...
public:
  ...
  vgl_oriented_box_2d(T width, T height, const vgl_point_2d<T> & center, T angle_in_rad);

  //: construct from major axis and height
  // the center is the midpoint of the major axis
  vgl_oriented_box_2d(const vgl_point_2d<T> & maj_p1, const vgl_point_2d<T> & maj_p2, T height)
  {
    major_axis_.set(maj_p1, maj_p2);
    half_height_ = height / T(2);
  }

  vgl_oriented_box_2d(const vgl_line_segment_2d<T> & maj, T height)
  {
    major_axis_ = maj;
    half_height_ = height / T(2);
  }
  ...
  vgl_oriented_box_2d(const vgl_point_2d<T> & maj_p1,
                      const vgl_point_2d<T> & maj_p2,
                      const vgl_point_2d<T> & min_p1,
                      const vgl_point_2d<T> & min_p2);

  vgl_oriented_box_2d(const vgl_line_segment_2d<T> & maj, const vgl_line_segment_2d<T> & min)
  {
  ...
  vgl_oriented_box_2d(const vgl_point_2d<T> & p0, const vgl_point_2d<T> & p1, const vgl_point_2d<T> & p2);
  ...
  width() const
  {
    std::pair<T, T> p = this->width_height();
    return p.first;
  }
  ...
  bool
  operator==(const vgl_oriented_box_2d<T> & ob) const
  {
    return (this == &ob) || (ob.major_axis_ == this->major_axis_ && ob.half_height_ == this->half_height_);
  }
  bool
  near_equal(const vgl_oriented_box_2d<T> & ob, T tol = vgl_tolerance<T>::position) const;

  //: Return true if (x,y) is inside this box
  bool
  contains(const T & x, const T & y) const
  {
    return this->contains(vgl_point_2d<T>(x, y));
  }

  //: Return true if point is inside this box
  bool
  contains(const vgl_point_2d<T> & p) const;


  void
  set(const vgl_line_segment_2d<T> & major, T half_height)
  {
    major_axis_ = major;
  ...
  vgl_point_2d<T>
  transform_to_obox(const vgl_point_2d<T> & p) const;

  //: rotate by angle in radians
  vgl_oriented_box_2d<T>
  rotate(const vgl_point_2d<T> & rot_center, T angle_rad) const;
