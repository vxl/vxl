...
template <class T>
vgl_plane_3d<T>::vgl_plane_3d(const vgl_homg_plane_3d<T> & p)
  : a_(p.a())
  , b_(p.b())
  , c_(p.c())
  , d_(p.d())
...
template <class T>
vgl_plane_3d<T>::vgl_plane_3d(const vgl_point_3d<T> & p1, const vgl_point_3d<T> & p2, const vgl_point_3d<T> & p3)
  : a_(p2.y() * p3.z() - p2.z() * p3.y() + p3.y() * p1.z() - p3.z() * p1.y() + p1.y() * p2.z() - p1.z() * p2.y())
  , b_(p2.z() * p3.x() - p2.x() * p3.z() + p3.z() * p1.x() - p3.x() * p1.z() + p1.z() * p2.x() - p1.x() * p2.z())
  , c_(p2.x() * p3.y() - p2.y() * p3.x() + p3.x() * p1.y() - p3.y() * p1.x() + p1.x() * p2.y() - p1.y() * p2.x())
  , d_(p1.x() * (p2.z() * p3.y() - p2.y() * p3.z()) + p2.x() * (p3.z() * p1.y() - p3.y() * p1.z()) +
...
template <class T>
vgl_plane_3d<T>::vgl_plane_3d(const vgl_vector_3d<T> & n, const vgl_point_3d<T> & p)
  : a_(n.x())
  , b_(n.y())
  , c_(n.z())
...
template <class T>
vgl_plane_3d<T>::vgl_plane_3d(const vgl_ray_3d<T> & r0, const vgl_ray_3d<T> & r1)
{
...
template <class T>
bool
vgl_plane_3d<T>::contains(const vgl_point_3d<T> & p, T tol) const
{
...
{
  return (this == &p) ||
         ((a() * p.b() == p.a() * b()) && (a() * p.c() == p.a() * c()) && (a() * p.d() == p.a() * d()) &&
          (b() * p.c() == p.b() * c()) && (b() * p.d() == p.b() * d()) && (c() * p.d() == p.c() * d()));
}
...
template <class T>
std::ostream &
operator<<(std::ostream & os, const vgl_plane_3d<T> & p)
{
  os << "<vgl_plane_3d";
  vp(os, p.a(), "x");
  vp(os, p.b(), "y");
...
operator>>(std::istream & is, vgl_plane_3d<T> & p)
{
  ...
  if (formatted)
  {
    ...
    if (is.peek() == '=')
      ...
      return is; //  = 0  is missing (TODO: throw an exception)
    is >> std::ws;
    if (is.peek() == '0')
      is.ignore();
    else
    ...
    if (!paren && is.peek() == '>')
      ...
      return is; // closing > is missing (TODO: throw an exception)
  }
  p.set(a, b, c, d);
  return is;
}
...
template <class T>
void
vgl_plane_3d<T>::plane_coord_vectors(vgl_vector_3d<T> & uvec, vgl_vector_3d<T> & vvec) const
{
...
template <class T>
bool
vgl_plane_3d<T>::plane_coords(const vgl_point_3d<T> & p3d, vgl_point_2d<T> & p2d, T tol) const
{
...
template <class T>
vgl_point_3d<T>
vgl_plane_3d<T>::world_coords(const vgl_point_2d<T> & p2d) const
{
  vgl_point_3d<T> origin_pt = vgl_closest_point_origin(*this);
  vgl_vector_3d<T> uvec, vvec;
  this->plane_coord_vectors(uvec, vvec);
