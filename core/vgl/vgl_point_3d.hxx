...
template <class Type>
vgl_point_3d<Type>::vgl_point_3d(const vgl_homg_point_3d<Type> & p)
  : x_(p.x() / p.w())
  , y_(p.y() / p.w())
  , z_(p.z() / p.w()) // could be infinite!
{}

//: Construct from 3 planes (intersection).
template <class Type>
vgl_point_3d<Type>::vgl_point_3d(const vgl_plane_3d<Type> & pl1,
...
template <class Type>
bool
vgl_point_3d<Type>::operator==(const vgl_point_3d<Type> & p) const
{
...
{
  Type r = ((p1.x() * p2.y() - p1.y() * p2.x()) * p3.z() + (p3.x() * p1.y() - p3.y() * p1.x()) * p2.z() +
            (p2.x() * p3.y() - p2.y() * p3.x()) * p1.z() + (p1.x() * p4.y() - p1.y() * p4.x()) * p2.z() +
            (p4.x() * p2.y() - p4.y() * p2.x()) * p1.z() + (p2.x() * p1.y() - p2.y() * p1.x()) * p4.z() +
            (p3.x() * p4.y() - p3.y() * p4.x()) * p1.z() + (p1.x() * p3.y() - p1.y() * p3.x()) * p4.z() +
            (p4.x() * p1.y() - p4.y() * p1.x()) * p3.z() + (p3.x() * p2.y() - p3.y() * p2.x()) * p4.z() +
            (p2.x() * p4.y() - p2.y() * p4.x()) * p3.z() + (p4.x() * p3.y() - p4.y() * p3.x()) * p2.z());
...
template <class Type>
std::istream &
vgl_point_3d<Type>::read(std::istream & is)
{
  if (!is.good())
...
template <class Type>
std::istream &
operator>>(std::istream & is, vgl_point_3d<Type> & p)
{
  return p.read(is);
}
...
#define VGL_POINT_3D_INSTANTIATE(T)                                                                      \
  ...
  template double cross_ratio(                                                                           \
    vgl_point_3d<T> const &, vgl_point_3d<T> const &, vgl_point_3d<T> const &, vgl_point_3d<T> const &); \
  template bool coplanar(                                                                                \
    vgl_point_3d<T> const &, vgl_point_3d<T> const &, vgl_point_3d<T> const &, vgl_point_3d<T> const &); \
