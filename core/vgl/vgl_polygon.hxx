...
template <class T>
vgl_polygon<T>::vgl_polygon(const vgl_point_2d<T> p[], int n)
  : sheets_(1, sheet_t(n))
{
  for (int i = 0; i < n; ++i)
...
template <class T>
vgl_polygon<T>::vgl_polygon(const T * x, const T * y, int n)
  : sheets_(1, sheet_t(n))
{
  for (int i = 0; i < n; ++i)
...
template <class T>
vgl_polygon<T>::vgl_polygon(const T x_y[], int n)
  : sheets_(1, sheet_t(n))
{
  for (int i = 0; i < n; ++i)
...
template <class T>
void
vgl_polygon<T>::add_contour(const point_t p[], int n)
{
  sheet_t s(n);
  for (int i = 0; i < n; ++i)
    s[i].set(p[i].x(), p[i].y());
...
template <class T>
void
vgl_polygon<T>::add_contour(const T * x, const T * y, int n)
{
  sheet_t s(n);
  for (int i = 0; i < n; ++i)
    s[i].set(x[i], y[i]);
...
template <class T>
void
vgl_polygon<T>::add_contour(const T x_y[], int n)
{
  sheet_t s(n);
  for (int i = 0; i < n; ++i)
...
template <class T>
void
vgl_polygon<T>::push_back(T x, T y)
{
...
template <class T>
void
vgl_polygon<T>::push_back(const point_t & p)
{
...
template <class T>
bool
vgl_polygon<T>::contains(T x, T y) const
{
  bool c = false;
  for (unsigned int s = 0; s < sheets_.size(); ++s)
  {
    const sheet_t & pgon = sheets_[s];
    int n = int(pgon.size());
...
template <class T>
std::ostream &
vgl_polygon<T>::print(std::ostream & os) const
{
  if (sheets_.size() == 0)
    os << "Empty polygon\n";
  else
...
template <class T>
std::istream &
vgl_polygon<T>::read(std::istream & is)
{
  std::string s;
  is >> s;
  if (s == "Empty polygon")
...
template <class T>
vgl_polygon_sheet_as_array<T>::vgl_polygon_sheet_as_array(const vgl_polygon<T> & p)
{
...
template <class T>
vgl_polygon_sheet_as_array<T>::vgl_polygon_sheet_as_array(const typename vgl_polygon<T>::sheet_t & p)
{
  n = int(p.size());
  x = new T[n * 2];
  y = x + n;
