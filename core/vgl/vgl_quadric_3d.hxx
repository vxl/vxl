...
static const char * quadric_class_name[] = { "invalid_quadric",
                                             "real_ellipsoid",
                                             "imaginary_ellipsoid",
...
template <class T>
std::string
vgl_quadric_3d<T>::type_by_number(const vgl_quadric_type & type)
{
  return quadric_class_name[(size_t)type];
}

template <class T>
typename vgl_quadric_3d<T>::vgl_quadric_type
vgl_quadric_3d<T>::type_by_name(const std::string & name)
{
...
template <class T>
vgl_quadric_3d<T>::vgl_quadric_3d(T a, T b, T c, T d, T e, T f, T g, T h, T i, T j)
  : det_zero_(false)
{
  this->set(a, b, c, d, e, f, g, h, i, j);
}
template <class T>
vgl_quadric_3d<T>::vgl_quadric_3d(const T coeff[])
  : det_zero_(false)
{
...
//
template <class T>
vgl_quadric_3d<T>::vgl_quadric_3d(const std::vector<std::vector<T>> & Q)
  : det_zero_(false)
{
  this->set(Q[0][0],
            Q[1][1],
...
template <class T>
vgl_quadric_3d<T>::vgl_quadric_3d(const std::vector<std::vector<T>> & canonical_quadric,
                                  const std::vector<std::vector<T>> & H)
{
  ...
  std::vector<T> qu(3, T(0)), t(3, T(0)), c(3, T(0)), Rc(3, T(0)), RqRt(3, T(0)), Rtrt(3, T(0));
...
template <class T>
vgl_quadric_3d<T>::vgl_quadric_3d(const std::vector<T> & diag, const std::vector<std::vector<T>> & H)
{
  std::vector<std::vector<T>> Qg(4, std::vector<T>(4, T(0)));
  Qg[0][0] = diag[0];
  Qg[1][1] = diag[1];
...
template <class T>
void
vgl_quadric_3d<T>::set(T a, T b, T c, T d, T e, T f, T g, T h, T i, T j)
{
  a_ = a;
  b_ = b;
  c_ = c;
...
template <class T>
void
vgl_quadric_3d<T>::set(const std::vector<std::vector<T>> & Q)
{
  this->set(Q[0][0],
            Q[1][1],
            Q[2][2],
...
vgl_quadric_3d<T>::coef_matrix() const
{
...
template <class T>
T
vgl_quadric_3d<T>::sampson_dist(const vgl_homg_point_3d<T> & pt) const
{
  T x = pt.x(), y = pt.y(), z = pt.z(), w = pt.w();
  T algebraic_dist = a_ * x * x + b_ * y * y + c_ * z * z + d_ * x * y + e_ * x * z + f_ * y * z + g_ * x * w +
                     h_ * y * w + i_ * z * w + j_ * w * w;
...
template <class T>
bool
vgl_quadric_3d<T>::on(const vgl_homg_point_3d<T> & pt, T tol) const
{
...
template <class T>
bool
vgl_quadric_3d<T>::center(vgl_point_3d<T> & center) const
{
  if (!(type_ == real_ellipsoid || type_ == real_elliptic_cone || type_ == hyperboloid_of_one_sheet ||
        type_ == hyperboloid_of_two_sheets))
    return false;
  T h = T(1) / T(2);
  //              _            _
  //             |  a   d/2  e/2|
  ...
  //             | e/2  f/2   c |
  //              -            -
  T upper_det = a_ * b_ * c_ - a_ * h * f_ * h * f_ - h * d_ * h * d_ * c_ + d_ * h * f_ * h * e_ * h +
                e_ * h * d_ * h * f_ * h - e_ * h * b_ * e_ * h;
...
vgl_quadric_3d<T>::compute_type()
{
  ...
  bool sign = true;
  if (nz == 2)
    sign = (non_zero_vals[0] > z && non_zero_vals[1] > z) || (non_zero_vals[0] < z && non_zero_vals[1] < z);
  else if (nz == 3)
  ...
  bool signu = true;
  if (nz == 2)
    signu = (non_zero_uvals[0] > z && non_zero_uvals[1] > z) || (non_zero_uvals[0] < z && non_zero_uvals[1] < z);
  else if (nz == 3)
  ...
  T det = Q[0][0] * Q[1][1] * Q[2][2] * Q[3][3] - Q[0][0] * Q[1][1] * Q[3][2] * Q[2][3] -
          Q[0][0] * Q[2][1] * Q[1][2] * Q[3][3] + Q[0][0] * Q[2][1] * Q[3][2] * Q[1][3] +
          Q[0][0] * Q[3][1] * Q[1][2] * Q[2][3] - Q[0][0] * Q[3][1] * Q[2][2] * Q[1][3] -
          Q[1][0] * Q[0][1] * Q[2][2] * Q[3][3] + Q[1][0] * Q[0][1] * Q[3][2] * Q[2][3] +
          Q[1][0] * Q[2][1] * Q[0][2] * Q[3][3] - Q[1][0] * Q[2][1] * Q[3][2] * Q[0][3] -
          Q[1][0] * Q[3][1] * Q[0][2] * Q[2][3] + Q[1][0] * Q[3][1] * Q[2][2] * Q[0][3] +
          Q[2][0] * Q[0][1] * Q[1][2] * Q[3][3] - Q[2][0] * Q[0][1] * Q[3][2] * Q[1][3] -
          Q[2][0] * Q[1][1] * Q[0][2] * Q[3][3] + Q[2][0] * Q[1][1] * Q[3][2] * Q[0][3] +
          Q[2][0] * Q[3][1] * Q[0][2] * Q[1][3] - Q[2][0] * Q[3][1] * Q[1][2] * Q[0][3] -
          Q[3][0] * Q[0][1] * Q[1][2] * Q[2][3] + Q[3][0] * Q[0][1] * Q[2][2] * Q[1][3] +
          Q[3][0] * Q[1][1] * Q[0][2] * Q[2][3] - Q[3][0] * Q[1][1] * Q[2][2] * Q[0][3] -
          Q[3][0] * Q[2][1] * Q[0][2] * Q[1][3] + Q[3][0] * Q[2][1] * Q[1][2] * Q[0][3];
  ...
  */
  //  the table above
  if (r3 == 1 && r4 == 1)
    type_ = coincident_planes;
  else if (r3 == 3 && r4 == 4 && gt_0 && signu)
    type_ = imaginary_ellipsoid;
  else if (r3 == 3 && r4 == 4 && !gt_0 && signu)
    type_ = real_ellipsoid;
  else if (r3 == 3 && r4 == 3 && signu)
    type_ = imaginary_elliptic_cone;
  else if (r3 == 3 && r4 == 3 && !signu)
    type_ = real_elliptic_cone;
  else if (r3 == 2 && r4 == 3 && signu && sign)
    type_ = imaginary_elliptic_cylinder;
  else if (r3 == 2 && r4 == 3 && signu && !sign)
    type_ = real_elliptic_cylinder;
  else if (r3 == 2 && r4 == 4 && !gt_0 && signu)
    type_ = elliptic_paraboloid;
  else if (r3 == 2 && r4 == 3 && !signu)
    type_ = hyperbolic_cylinder;
  else if (r3 == 2 && r4 == 4 && gt_0 && !signu)
    type_ = hyperbolic_paraboloid;
  else if (r3 == 3 && r4 == 4 && gt_0 && !signu)
    type_ = hyperboloid_of_one_sheet;
  else if (r3 == 3 && r4 == 4 && !gt_0 && !signu)
    type_ = hyperboloid_of_two_sheets;
  else if (r3 == 2 && r4 == 2 && signu)
    type_ = imaginary_intersecting_planes;
  else if (r3 == 2 && r4 == 2 && !signu)
    type_ = real_intersecting_planes;
  else if (r3 == 1 && r4 == 3)
    type_ = parabolic_cylinder;
  else if (r3 == 1 && r4 == 2 && sign)
    type_ = imaginary_parallel_planes;
  else if (r3 == 1 && r4 == 2 && !sign)
    type_ = real_parallel_planes;
}
template <class T>
bool
vgl_quadric_3d<T>::operator==(const vgl_quadric_3d<T> & that) const
{
  if (type() != that.type())
    return false;
  T mag_coefs =
    (fabs(a_) + fabs(b_) + fabs(c_) + fabs(d_) + fabs(e_) + fabs(f_) + fabs(g_) + fabs(h_) + fabs(i_) + fabs(j_)) /
    T(10);
  T mag_coefs_that = (fabs(that.a()) + fabs(that.b()) + fabs(that.c()) + fabs(that.d()) + fabs(that.e()) +
                      fabs(that.f()) + fabs(that.g()) + fabs(that.h()) + fabs(that.i()) + fabs(that.j())) /
                     T(10);
  T tol = vgl_tolerance<T>::position;
...
template <class T>
void
vgl_quadric_3d<T>::upper_3x3_eigensystem(std::vector<T> & eigenvalues, std::vector<std::vector<T>> & eigenvectors) const
{
  std::vector<std::vector<T>> Q = this->coef_matrix();
  T mu[3][3];
  T lu[3];
...
template <class T>
bool
vgl_quadric_3d<T>::canonical_central_quadric(std::vector<T> & diag, std::vector<std::vector<T>> & H) const
{
  diag.resize(4, T(0));
...
template <class T>
std::vector<std::vector<T>>
vgl_quadric_3d<T>::canonical_quadric(std::vector<std::vector<T>> & H) const
{
...
void
twst(R m[n][n], R c, R s, int i, int j)
{
  int k = n;
  while (k--)
  {
...
#define db 0
  ...
  while (true)
  {
    ...
    if (db)
      std::cout << "mii=" << m2[i][i] << "mjj=" << m2[j][j] << std::endl;
    R th = 0.5 * atan(2 * m2[i][j] / (m2[i][i] - m2[j][j]));
    // if(db) printf("th=%e, i=%d, j=%d\n", th, i, j);
    if (db)
      std::cout << "th=" << th << "i" << i << "j=" << j << std::endl;
    {
      R c = cos(th), s = sin(th);
      {
        int k = n;
...
template <class T>
std::ostream &
operator<<(std::ostream & os, const vgl_quadric_3d<T> & q)
{
  os << "vgl_quadric_3d: a=" << q.a() << " b=" << q.b() << " c=" << q.c() << " d=" << q.d() << " e=" << q.e()
     << " f=" << q.f() << " g=" << q.g() << " h=" << q.h() << " i=" << q.i() << " j=" << q.j() << std::endl;
  return os;
}

template <class T>
std::istream &
operator>>(std::istream & ist, vgl_quadric_3d<T> & q)
{
