...
public:
  ...
  //: Copy constructor
  inline vgl_ray_3d(const vgl_ray_3d<Type> & l)
    : p0_(l.origin())
    , t_(l.direction())
  {}

  //: Construct from orign and direction
  inline vgl_ray_3d(const vgl_point_3d<Type> & p0, const vgl_vector_3d<Type> & direction)
    : p0_(p0)
    , t_(direction)
  {
  ...
  inline vgl_ray_3d(const vgl_point_3d<Type> & origin, const vgl_point_3d<Type> & p)
    : p0_(origin)
    , t_(p - origin)
  {
    t_ = t_ / static_cast<Type>(t_.length());
  ...
  inline vgl_ray_3d(const vgl_line_segment_3d<Type> & ls)
  {
    p0_ = ls.point1();
    t_ = ls.point2() - p0_;
    t_ = t_ / static_cast<Type>(t_.length());
  ...
  inline vgl_ray_3d(const vgl_line_3d_2_points<Type> & ls)
  {
    p0_ = ls.point1();
    t_ = ls.point2() - p0_;
    t_ = t_ / static_cast<Type>(t_.length());
  ...
  inline bool
  operator==(const vgl_ray_3d<Type> & r) const
  {
    return (this == &r) || (direction() == r.direction() && origin() == r.origin());
  }

  inline bool
  operator!=(const vgl_ray_3d<Type> & other) const
  {
  ...
  //: Assignment
  inline void
  set(const vgl_point_3d<Type> & p0, const vgl_vector_3d<Type> & direction)
  {
    p0_ = p0;
    t_ = direction;
    t_ = t_ / static_cast<Type>(t_.length());
...
template <class Type>
std::istream &
operator>>(std::istream & is, vgl_ray_3d<Type> & p);
//: public functions
template <class Type>
//: angle between rays
double
angle(const vgl_ray_3d<Type> & r0, const vgl_ray_3d<Type> & r1)
{
