...
public:
  ...
  //: Default constructor
  inline vgl_sphere_3d()
    : c_(Type(0), Type(0), Type(0))
    , r_(Type(-1))
  {}

  //: Construct from four scalars: centre and radius.
  inline vgl_sphere_3d(Type px, Type py, Type pz, Type rad)
    : c_(px, py, pz)
    , r_(rad)
  {}

  //: Construct from a 4-array, representing centre and radius.
  inline vgl_sphere_3d(const Type v[4])
    : c_(v[0], v[1], v[2])
    , r_(v[3])
  {}

  //: Construct from centre point and radius.
  vgl_sphere_3d(const vgl_point_3d<Type> & cntr, Type rad)
    : c_(cntr)
    , r_(rad)
  {}
  ...
  //: Test for equality
  inline bool
  operator==(const vgl_sphere_3d<Type> & s) const
  {
  ...
  inline const vgl_point_3d<Type> &
  centre() const
  {
    return c_;
  }
  ...
  inline void
  set_centre(const vgl_point_3d<Type> & c)
  {
    c_ = c;
  }
  ...
  bool
  clip(const vgl_line_3d_2_points<Type> & line, vgl_point_3d<Type> & p1, vgl_point_3d<Type> & p2) const;


  //: convert point on sphere to Cartesian coordinates, angles in radians
  void
  spherical_to_cartesian(Type elevation_rad, Type azimuth_rad, Type & x, Type & y, Type & z) const;

  void
  spherical_to_cartesian(Type elevation_rad, Type azimuth_rad, vgl_point_3d<Type> & pt) const;

  //: find elevation and azimuth of closest point on the sphere to x,y,z
  void
  cartesian_to_spherical(Type x, Type y, Type z, Type & elevation_rad, Type & azimuth_rad) const;
  void
  cartesian_to_spherical(const vgl_point_3d<Type> & pt, Type & elevation_rad, Type & azimuth_rad) const;
...
template <class Type>
std::ostream &
operator<<(std::ostream & os, const vgl_sphere_3d<Type> & sph);
...
template <class Type>
std::istream &
operator>>(std::istream & is, vgl_sphere_3d<Type> & sph);
