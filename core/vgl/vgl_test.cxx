...
void vgl_test_start(const char* name = 0) {
  num_test = 0;
  tests_passed = 0;
  tests_failed = 0;
  test_name = name;
...
{
  vcl_cout << msg << " - ";
  vgl_test_perform(expr);
}

void vgl_test_assert_near(const vcl_string& msg, double expr, double target, double tol)
{
  vcl_cout << msg << " should be " << target << ", is " << expr << ", ";
  vgl_test_perform(vcl_abs(expr - target) < tol);
}
