                                 ...
                                 const vgl_point_3d<double> & b_p3)
{
  ...
  std::pair<unsigned, unsigned> e[3] = { std::make_pair(0, 1), std::make_pair(1, 2), std::make_pair(2, 0) };
  ...
  for (unsigned j = 0; j < 3; ++j)
  {
    for (unsigned i = 0; i < 3; ++i)
    {
      ...
      double b1_dist = length(a[e[j].first] - b[e[i].first]) + length(a[e[j].second] - b[e[i].first]);
      double b2_dist = length(a[e[j].first] - b[e[i].second]) + length(a[e[j].second] - b[e[i].second]);

      double e2_len = length(b[e[i].first] - b[e[i].second]);
      double a1_dist = length(b[e[i].first] - a[e[j].first]) + length(b[e[i].second] - a[e[j].first]);
      double a2_dist = length(b[e[i].first] - a[e[j].second]) + length(b[e[i].second] - a[e[j].second]);
