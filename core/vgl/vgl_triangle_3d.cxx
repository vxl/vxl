...
vgl_plane_3d<double>
create_plane_and_ignore_degenerate(const vgl_point_3d<double> & p1,
                                   ...
                                   const vgl_point_3d<double> & p3)
{
  ...
  a[0] = p2.y() * p3.z() - p2.z() * p3.y() + p3.y() * p1.z() - p3.z() * p1.y() + p1.y() * p2.z() - p1.z() * p2.y();

  a[1] = p2.z() * p3.x() - p2.x() * p3.z() + p3.z() * p1.x() - p3.x() * p1.z() + p1.z() * p2.x() - p1.x() * p2.z();

  a[2] = p2.x() * p3.y() - p2.y() * p3.x() + p3.x() * p1.y() - p3.y() * p1.x() + p1.x() * p2.y() - p1.y() * p2.x();

  a[3] = p1.x() * (p2.z() * p3.y() - p2.y() * p3.z()) + p2.x() * (p3.z() * p1.y() - p3.y() * p1.z()) +
...
vgl_triangle_3d_coincident_edges(const vgl_point_3d<double> & a_p1,
                                 ...
                                 const vgl_point_3d<double> & b_p3)
{
  ...
  const vgl_point_3d<double> a[3] = { a_p1, a_p2, a_p3 };
  const vgl_point_3d<double> b[3] = { b_p1, b_p2, b_p3 };
  const std::pair<unsigned, unsigned> e[3] = { std::make_pair(0, 1), std::make_pair(1, 2), std::make_pair(2, 0) };
  ...
  for (unsigned j = 0; j < 3; ++j)
  {
    for (unsigned i = 0; i < 3; ++i)
    {
      ...
      const double e1_len = length(a[e[j].first] - a[e[j].second]);
      const double b1_dist = length(a[e[j].first] - b[e[i].first]) + length(a[e[j].second] - b[e[i].first]);
      const double b2_dist = length(a[e[j].first] - b[e[i].second]) + length(a[e[j].second] - b[e[i].second]);

      const double e2_len = length(b[e[i].first] - b[e[i].second]);
      const double a1_dist = length(b[e[i].first] - a[e[j].first]) + length(b[e[i].second] - a[e[j].first]);
      const double a2_dist = length(b[e[i].first] - a[e[j].second]) + length(b[e[i].second] - a[e[j].second]);

      if ((std::fabs(e1_len - b1_dist) < sqrteps && std::fabs(e1_len - b2_dist) < sqrteps) ||
          (std::fabs(e2_len - a1_dist) < sqrteps && std::fabs(e2_len - a2_dist) < sqrteps))
      {
...
bool
vgl_triangle_3d_test_inside(const vgl_point_3d<double> & i_pnt,
                            ...
                            double coplanar_tolerance)
{
  ...
  { // the triangle is degenerate - its vertices are collinear
    ...
    return vgl_line_segment_3d<double>(p1, p2).contains(i_pnt) || vgl_line_segment_3d<double>(p2, p3).contains(i_pnt) ||
  ...
  const vgl_plane_3d<double> plane = create_plane_and_ignore_degenerate(p1, p2, p3);
  ...
  const double point[3] = { i_pnt.x(), i_pnt.y(), i_pnt.z() };
  const double vert0[3] = { p1.x(), p1.y(), p1.z() };
  const double vert1[3] = { p2.x(), p2.y(), p2.z() };
  const double vert2[3] = { p3.x(), p3.y(), p3.z() };

  double beta = 0.0;
  double alpha = 0.0;

  // compute the barycentric vectors....& ignore numerical roundoff errors
  const double u0 = (std::fabs(point[i1]) < sqrteps ? 0 : point[i1]) - (std::fabs(vert0[i1]) < sqrteps ? 0 : vert0[i1]);
  const double v0 = (std::fabs(point[i2]) < sqrteps ? 0 : point[i2]) - (std::fabs(vert0[i2]) < sqrteps ? 0 : vert0[i2]);

  const double u1 = (std::fabs(vert1[i1]) < sqrteps ? 0 : vert1[i1]) - (std::fabs(vert0[i1]) < sqrteps ? 0 : vert0[i1]);
  const double u2 = (std::fabs(vert2[i1]) < sqrteps ? 0 : vert2[i1]) - (std::fabs(vert0[i1]) < sqrteps ? 0 : vert0[i1]);
  const double v1 = (std::fabs(vert1[i2]) < sqrteps ? 0 : vert1[i2]) - (std::fabs(vert0[i2]) < sqrteps ? 0 : vert0[i2]);
  const double v2 = (std::fabs(vert2[i2]) < sqrteps ? 0 : vert2[i2]) - (std::fabs(vert0[i2]) < sqrteps ? 0 : vert0[i2]);
...
bool
vgl_triangle_3d_test_inside_simple(const vgl_point_3d<double> & i_pnt,
                                   const vgl_point_3d<double> & p1,
                                   const vgl_point_3d<double> & p2,
                                   const vgl_point_3d<double> & p3)
{
  const vgl_vector_3d<double> vec1 = normalized(i_pnt - p1);
  const vgl_vector_3d<double> vec2 = normalized(i_pnt - p2);
  const vgl_vector_3d<double> vec3 = normalized(i_pnt - p3);

  const double int_ang =
    std::acos(dot_product(vec1, vec2)) + std::acos(dot_product(vec2, vec3)) + std::acos(dot_product(vec3, vec1));
  const double test_val = std::fabs(int_ang - (2 * pi));
...
// plane ABC
static vgl_triangle_3d_intersection_t
same_side(const vgl_point_3d<double> & A,
          const vgl_point_3d<double> & B,
          const vgl_point_3d<double> & C,
          const vgl_point_3d<double> & D,
          const vgl_point_3d<double> & E)
{
  const vgl_vector_3d<double> b = B - A;
  const vgl_vector_3d<double> c = C - A;

  const vgl_vector_3d<double> n = cross_product(b, c);

  const vgl_vector_3d<double> d = D - A;
  const double d_dot = dot_product(d, n);
  const vgl_vector_3d<double> e = E - A;
  const double e_dot = dot_product(e, n);
...
vgl_triangle_3d_line_intersection(const vgl_line_segment_3d<double> & line,
                                  const vgl_point_3d<double> & p1,
                                  const vgl_point_3d<double> & p2,
                                  const vgl_point_3d<double> & p3,
...
{
  const vgl_point_3d<double> line_p1 = line.point1();
  const vgl_point_3d<double> line_p2 = line.point2();
  ...
  { // the triangle is degenerate - it's points are collinear
    ...
    const vgl_line_3d_2_points<double> i_line(line_p1, line_p2);
    if (!ignore_coplanar && ((p1 != p2 && concurrent(vgl_line_3d_2_points<double>(p1, p2), i_line) &&
  ...
  const vgl_triangle_3d_intersection_t rv1 = same_side(line.point1(), p1, p2, p3, line.point2());
  if (rv1 == None)
    return None;

  const vgl_triangle_3d_intersection_t rv2 = same_side(line.point1(), p2, p3, p1, line.point2());
  if (rv2 == None)
    return None;

  const vgl_triangle_3d_intersection_t rv3 = same_side(line.point1(), p3, p1, p2, line.point2());
  if (rv3 == None)
    return None;
  ...
  if (rv1 == Coplanar || rv2 == Coplanar || rv3 == Coplanar)
  {
    ...
    const vgl_line_3d_2_points<double> i_line(line_p1, line_p2);
    const vgl_line_segment_3d<double> edge1(p1, p2);

    vgl_point_3d<double> test_pt;
    if (concurrent(vgl_line_3d_2_points<double>(p1, p2), i_line) && vgl_intersection(edge1, line, test_pt))
    {
      i_pnt = test_pt;
      return Coplanar;
    }
    const vgl_line_segment_3d<double> edge2(p1, p3);
    if (concurrent(vgl_line_3d_2_points<double>(p1, p3), i_line) && vgl_intersection(edge2, line, test_pt))
    {
      i_pnt = test_pt;
      return Coplanar;
    }
    const vgl_line_segment_3d<double> edge3(p2, p3);
    if (concurrent(vgl_line_3d_2_points<double>(p2, p3), i_line) && vgl_intersection(edge3, line, test_pt))
    {
      i_pnt = test_pt;
      return Coplanar;
    }
  ...
  i_pnt = vgl_intersection(vgl_line_3d_2_points<double>(line_p1, line_p2), vgl_plane_3d<double>(p1, p2, p3));
  return Skew;
}
...
inline unsigned
calc_edge_index(unsigned v, unsigned w)
{
  const unsigned lookup = v * 3u + w;
  assert(lookup < 8);
  const unsigned edge = calc_edge_index_lookup[lookup];
  assert(edge < 3);
  return edge;
}
} // namespace
...
vgl_triangle_3d_intersection_t
vgl_triangle_3d_triangle_intersection(const vgl_point_3d<double> & a_p1,
                                      const vgl_point_3d<double> & a_p2,
                                      const vgl_point_3d<double> & a_p3,
                                      const vgl_point_3d<double> & b_p1,
                                      const vgl_point_3d<double> & b_p2,
                                      ...
                                      unsigned & i_line_point2_edge)
{
  ...
  if (collinear(a_p1, a_p2, a_p3))
  {
    ...
    if (a_p1 != a_p2 && (ret = vgl_triangle_3d_line_intersection(
                           vgl_line_segment_3d<double>(a_p1, a_p2), b_p1, b_p2, b_p3, i_pnt)) != None)
    ...
    if (a_p2 != a_p3 && (ret = vgl_triangle_3d_line_intersection(
                           vgl_line_segment_3d<double>(a_p2, a_p3), b_p1, b_p2, b_p3, i_pnt)) != None)
    { // half-degenerate tri_b behaves as line. Find line intersection
      i_line.set(i_pnt, i_pnt);
      if (ret == Coplanar)
      {
    ...
    if (a_p1 != a_p3 && (ret = vgl_triangle_3d_line_intersection(
                           vgl_line_segment_3d<double>(a_p1, a_p3), b_p1, b_p2, b_p3, i_pnt)) != None)
    { // half-degenerate tri_b behaves as line. Find line intersection
      i_line.set(i_pnt, i_pnt);
      if (ret == Coplanar)
      {
    ...
    if (!n_tmp_iline_edges)
      return None;              // Found no edges intersecting with triangle
  ...
  if (collinear(b_p1, b_p2, b_p3))
  {
    ...
    unsigned tmp_iline_edges[2];
    unsigned n_tmp_iline_edges = 0;
    if (b_p1 != b_p2 && (ret = vgl_triangle_3d_line_intersection(
                           vgl_line_segment_3d<double>(b_p1, b_p2), a_p1, a_p2, a_p3, i_pnt)) != None)
    { // half-degenerate tri_b behaves as line. Find line intersection
      i_line.set(i_pnt, i_pnt);
      if (ret == Coplanar)
      {
    ...
    if (b_p2 != b_p3 && (ret = vgl_triangle_3d_line_intersection(
                           vgl_line_segment_3d<double>(b_p2, b_p3), a_p1, a_p2, a_p3, i_pnt)) != None)
    { // half-degenerate tri_b behaves as line. Find line intersection
      i_line.set(i_pnt, i_pnt);
      if (ret == Coplanar)
      {
    ...
    if (b_p1 != b_p3 && (ret = vgl_triangle_3d_line_intersection(
                           vgl_line_segment_3d<double>(b_p1, b_p3), a_p1, a_p2, a_p3, i_pnt)) != None)
    { // half-degenerate tri_b behaves as line. Find line intersection
      i_line.set(i_pnt, i_pnt);
      if (ret == Coplanar)
      {
  ...
  const double TRI_TRI_EPS = 1000000 * std::numeric_limits<double>::epsilon();
  ...
  if (std::fabs(d_b[0]) < TRI_TRI_EPS)
    d_b[0] = 0.0;
  if (std::fabs(d_b[1]) < TRI_TRI_EPS)
    d_b[1] = 0.0;
  if (std::fabs(d_b[2]) < TRI_TRI_EPS)
    d_b[2] = 0.0;
  ...
  // coplanarity robustness check
  if (std::fabs(d_a[0]) < TRI_TRI_EPS)
    d_a[0] = 0.0;
  if (std::fabs(d_a[1]) < TRI_TRI_EPS)
    d_a[1] = 0.0;
  if (std::fabs(d_a[2]) < TRI_TRI_EPS)
    d_a[2] = 0.0;
  ...
  if (coplanar)
  {
    ...
    const bool isect1 =
      vgl_triangle_3d_line_intersection(vgl_line_segment_3d<double>(a_p1, a_p2), b_p1, b_p2, b_p3, i_pnt1) != None;
    const bool isect2 =
      vgl_triangle_3d_line_intersection(vgl_line_segment_3d<double>(a_p2, a_p3), b_p1, b_p2, b_p3, i_pnt2) != None;
    const bool isect3 =
      vgl_triangle_3d_line_intersection(vgl_line_segment_3d<double>(a_p3, a_p1), b_p1, b_p2, b_p3, i_pnt3) != None;
  ...
  double tmp =
    d_a[a_ival[0]] / (d_a[a_ival[0]] - d_a[a_ival[1]]); // fraction along edge a_ival[0,1] to plane_b's intersection.
  isect_a[0] = p_a[a_ival[0]] + (p_a[a_ival[1]] - p_a[a_ival[0]]) * tmp;
  const vgl_point_3d<double> a_vs[] = { a_p1, a_p2, a_p3 };
  ...
  const vgl_point_3d<double> b_vs[] = { b_p1, b_p2, b_p3 };
                                      ...
                                      const vgl_point_3d<double> & b_p3)
{
  ...
  if (collinear(a_p1, a_p2, a_p3))
  {
    ...
    if ((a_p1 != a_p2 && (ret = vgl_triangle_3d_line_intersection(
                            vgl_line_segment_3d<double>(a_p1, a_p2), b_p1, b_p2, b_p3, i_pnt)) != None) ||
        (a_p2 != a_p3 && (ret = vgl_triangle_3d_line_intersection(
                            vgl_line_segment_3d<double>(a_p2, a_p3), b_p1, b_p2, b_p3, i_pnt)) != None) ||
        (a_p1 != a_p3 && (ret = vgl_triangle_3d_line_intersection(
                            vgl_line_segment_3d<double>(a_p1, a_p3), b_p1, b_p2, b_p3, i_pnt)) != None))
      return ret;
  ...
  if (collinear(b_p1, b_p2, b_p3))
  {
    ...
    if ((b_p1 != b_p2 && (ret = vgl_triangle_3d_line_intersection(
                            vgl_line_segment_3d<double>(b_p1, b_p2), a_p1, a_p2, a_p3, i_pnt)) != None) ||
        (b_p2 != b_p3 && (ret = vgl_triangle_3d_line_intersection(
                            vgl_line_segment_3d<double>(b_p2, b_p3), a_p1, a_p2, a_p3, i_pnt)) != None) ||
        (b_p1 != b_p3 && (ret = vgl_triangle_3d_line_intersection(
                            vgl_line_segment_3d<double>(b_p1, b_p3), a_p1, a_p2, a_p3, i_pnt)) != None))
      return ret;
  ...
  const double TRI_TRI_EPS = 100000 * std::numeric_limits<double>::epsilon();
  ...
  // coplanarity robustness check
  if (std::fabs(d_b1) < TRI_TRI_EPS)
    d_b1 = 0.0;
  if (std::fabs(d_b2) < TRI_TRI_EPS)
    d_b2 = 0.0;
  if (std::fabs(d_b3) < TRI_TRI_EPS)
    d_b3 = 0.0;
  ...
  // coplanarity robustness check
  if (std::fabs(d_a1) < TRI_TRI_EPS)
    d_a1 = 0.0;
  if (std::fabs(d_a2) < TRI_TRI_EPS)
    d_a2 = 0.0;
  if (std::fabs(d_a3) < TRI_TRI_EPS)
    d_a3 = 0.0;
  ...
  if (coplanar)
  {
    ...
    // finally, test if triangle a is totally contained in triangle b or vice versa
    if (vgl_triangle_3d_test_inside(a_p1, b_p1, b_p2, b_p3) || vgl_triangle_3d_test_inside(b_p1, a_p1, a_p2, a_p3))
    {
      return Coplanar;
    }
...
vgl_triangle_3d_intersection_t
vgl_triangle_3d_plane_intersection(const vgl_point_3d<double> & p1,
                                   const vgl_point_3d<double> & p2,
                                   const vgl_point_3d<double> & p3,
                                   ...
                                   vgl_line_segment_3d<double> & i_line)
{
  ...
  // coplanarity robustness check
  if (std::fabs(p1_d) < sqrteps)
    p1_d = 0.0;
  if (std::fabs(p2_d) < sqrteps)
    p2_d = 0.0;
  if (std::fabs(p3_d) < sqrteps)
    p3_d = 0.0;
  ...
  {
    edge.set(p1, p3);
    const vgl_point_3d<double> i_pnt1 = vgl_intersection(edge, i_plane);
    edge.set(p2, p3);
    const vgl_point_3d<double> i_pnt2 = vgl_intersection(edge, i_plane);
...
//=======================================================================
vgl_point_3d<double>
vgl_triangle_3d_closest_point(const vgl_point_3d<double> & q,
                              const vgl_point_3d<double> & p1,
                              const vgl_point_3d<double> & p2,
                              const vgl_point_3d<double> & p3)
{
  ...
  const vgl_point_3d<double> cp = vgl_closest_point<double>(plane, q);
  ...
  else
  {
    ...
    const vgl_point_3d<double> cp1(cp1x, cp1y, cp1z);
    const double d1 = vgl_distance(cp1, q);

    // Edge 2
    double cp2x = NAN, cp2y = NAN, cp2z = NAN;
    vgl_closest_point_to_linesegment(
      cp2x, cp2y, cp2z, p2.x(), p2.y(), p2.z(), p3.x(), p3.y(), p3.z(), q.x(), q.y(), q.z());
    const vgl_point_3d<double> cp2(cp2x, cp2y, cp2z);
    const double d2 = vgl_distance(cp2, q);

    // Edge 3
    double cp3x = NAN, cp3y = NAN, cp3z = NAN;
    vgl_closest_point_to_linesegment(
      cp3x, cp3y, cp3z, p1.x(), p1.y(), p1.z(), p3.x(), p3.y(), p3.z(), q.x(), q.y(), q.z());
    const vgl_point_3d<double> cp3(cp3x, cp3y, cp3z);
    const double d3 = vgl_distance(cp3, q);
...
//=======================================================================
double
vgl_triangle_3d_distance(const vgl_point_3d<double> & q,
                         const vgl_point_3d<double> & p1,
                         const vgl_point_3d<double> & p2,
                         const vgl_point_3d<double> & p3)
{
  const vgl_point_3d<double> c = vgl_triangle_3d_closest_point(q, p1, p2, p3);
...
//  and \a b_p1, \a b_p2, \a b_p3
bool
vgl_triangle_3d_triangle_coplanar(const vgl_point_3d<double> & a_p1,
                                  const vgl_point_3d<double> & a_p2,
                                  const vgl_point_3d<double> & a_p3,
                                  const vgl_point_3d<double> & b_p1,
                                  const vgl_point_3d<double> & b_p2,
...
//  The triangle is represented by its vertices \a p1, \a p2, \a p3
double
vgl_triangle_3d_area(const vgl_point_3d<double> & p0, const vgl_point_3d<double> & p1, const vgl_point_3d<double> & p2)
{
...
//  The triangle is represented by its vertices \a p1, \a p2, \a p3
double
vgl_triangle_3d_aspect_ratio(const vgl_point_3d<double> & p0,
                             const vgl_point_3d<double> & p1,
                             const vgl_point_3d<double> & p2)
{
