...
template <class T>
static inline void
min_n_max(T a, T b, T c, T * min, T * max)
{
  if (a < b)
  {
    if (a < c)
...
template <class T>
void
vgl_triangle_scan_iterator<T>::reset()
{
#if use_polygon_scan_iterator
  if (data)
    delete data;
...
#  endif
  ...
  data[0][0] = bg.y - cg.y;
  data[0][1] = cg.x - bg.x;
  data[0][2] = bg.x * cg.y - bg.y * cg.x;
  data[1][0] = cg.y - ag.y;
  data[1][1] = ag.x - cg.x;
  data[1][2] = cg.x * ag.y - cg.y * ag.x;
  data[2][0] = ag.y - bg.y;
  data[2][1] = bg.x - ag.x;
  data[2][2] = ag.x * bg.y - ag.y * bg.x;
...
template <class T>
bool
vgl_triangle_scan_iterator<T>::next()
{
#if use_polygon_scan_iterator
  if (data->next())
  {
...
#define VGL_TRIANGLE_SCAN_ITERATOR_INSTANTIATE(T) template class vgl_triangle_scan_iterator<T>
