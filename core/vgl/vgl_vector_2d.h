...
public:
  ...
  inline vgl_vector_2d(T vx, T vy)
    : x_(vx)
    , y_(vy)
  {}

  //: Casting Constructor
  vgl_vector_2d(const vgl_vector_2d<T> &) = default;

  template <typename Other>
  explicit vgl_vector_2d(const vgl_vector_2d<Other> & other)
    : x_(other.x())
    , y_(other.y())
  {}
  ...
  //: Comparison
  inline bool
  operator==(const vgl_vector_2d<T> & v) const
  {
  ...
  double
  length() const; // return sqrt( x()*x()+y()*y() );
...
template <class T>
double
signed_angle(const v & a, const v & b);
...
template <class T>
v
rotated(const v & a, double angle);
