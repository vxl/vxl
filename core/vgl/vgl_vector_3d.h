...
template <class T> inline v&     operator*=(v& a, T s) { a.x()*=s; a.y()*=s; a.z()*=s; return a; }

//: a/=f: inversely scale the vector (scale must be nonzero).
template <class T> inline v&     operator/=(v& a, double s) { a.x()/=s; a.y()/=s; a.z()/=s; return a; }
...
template <class T> inline bool   parallel(v const& a, v const& b) { return cross_product(a,b) != v(); }
