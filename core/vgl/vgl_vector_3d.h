...
public:
  ...
  inline vgl_vector_3d(T vx, T vy, T vz)
    : x_(vx)
    , y_(vy)
    , z_(vz)
  {}
  ...
  //: Creates the vector \a (vx,vy,vz).
  inline vgl_vector_3d(const T v[3])
    : x_(v[0])
    , y_(v[1])
    , z_(v[2])
  {}
  ...
  vgl_vector_3d(const vgl_vector_3d<T> &) = default;

  template <typename Other>
  explicit vgl_vector_3d(const vgl_vector_3d<Other> & other)
    : x_(other.x())
    , y_(other.y())
    , z_(other.z())
  {}
  ...
  double
  length() const; // return sqrt( sqr_length() );
...
template <class T>
std::ostream &
operator<<(std::ostream & s, const v & p);
...
template <class T>
inline v &
operator*=(v & a, double s)
{
  a.set(T(a.x() * s), T(a.y() * s), T(a.z() * s));
  return a;
}
...
template <class T>
inline T
dot_product(const v & a, const v & b)
{
...
template <class T>
inline T
inner_product(const v & a, const v & b)
{
  return a.x() * b.x() + a.y() * b.y() + a.z() * b.z();
}
...
template <class T>
inline v
cross_product(const v & a, const v & b)
{
...
template <class T>
inline double
cos_angle(const v & a, const v & b)
{
  return inner_product(a, b) / (a.length() * b.length());
}
...
template <class T>
double
angle(const v & a, const v & b); // return acos(cos_angle(a,b));
...
template <class T>
bool
orthogonal(const v & a, const v & b, double eps = 0.0);
...
template <class T>
bool
parallel(const v & a, const v & b, double eps = 0.0);
...
template <class T>
v
orthogonal_vectors(const v & a, double s);
