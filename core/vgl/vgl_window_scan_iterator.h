...
public:
  ...
  inline void
  set_window_corners(T x1_, T y1_, T x2_, T y2_)
  {
  ...
  inline vgl_window_scan_iterator(T x1_, T y1_, T x2_, T y2_) { set_window_corners(x1_, y1_, x2_, y2_); }
  ...
  reset() override
  {
    current_y = y1 <= y2 ? y1 - 1 : y1 + 1;
  }
  inline bool
  next() override
  {
    return y1 <= y2 ? ++current_y <= y2 : --current_y >= y2;
  }
  inline int
