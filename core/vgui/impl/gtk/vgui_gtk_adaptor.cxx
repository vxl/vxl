...
vgui_window *
vgui_gtk_adaptor::get_window() const
{
  return win_;
}

void
vgui_gtk_adaptor::swap_buffers()
{
  make_current();
...
void
vgui_gtk_adaptor::make_current()
{
  assert(gtk_gl_area_make_current(GTK_GL_AREA(widget)));
}

void
vgui_gtk_adaptor::post_redraw()
{
  if (!redraw_requested)
  {
...
void
vgui_gtk_adaptor::post_overlay_redraw()
{
  if (!ovl_helper)
...
void
vgui_gtk_adaptor::post_timer(float timeout, int name)
{
...
//: timeout is in milliseconds
void
vgui_gtk_adaptor::kill_timer(int name)
{
...
gint
vgui_gtk_adaptor::handle(GtkWidget * widget, GdkEvent * gev, gpointer context)
{
  ...
  if (debug)
    std::cerr << "vgui_event " << event << std::endl;
...
void
vgui_gtk_adaptor::draw()
{
  if (debug)
    std::cerr << "vgui_gtk_adaptor::draw\n";
  if (GTK_WIDGET_MAPPED(widget))
  {
    make_current();
...
gint
vgui_gtk_adaptor::idle_callback_for_redraw(gpointer data)
{
...
gint
vgui_gtk_adaptor::idle_callback_for_destroy(gpointer data)
{
  vgui_gtk_adaptor * adaptor = static_cast<vgui_gtk_adaptor *>(data);
