...
vgui_button
vgui_gtk_utils::translate_button(int button)
{
  if (button == 1)
    return vgui_LEFT;
  else if (button == 2)
...
vgui_key
vgui_gtk_utils::translate_key(const GdkEventKey * gev)
{
  if (gev->length == 1)
...
void
vgui_gtk_utils::set_modifiers(vgui_event & e, const guint state)
{
  e.modifier =
...
bool
vgui_gtk_utils::is_modifier(const GdkEvent * gev)
{
...
void
vgui_gtk_utils::add_submenu(GtkWidget * widget, const vgui_menu & menu)
{
...
void
vgui_gtk_utils::set_menu(GtkWidget * widget, const vgui_menu & menu, bool is_menubar)
{
  for (unsigned i = 0; i < menu.size(); i++)
  {
    if (menu[i].is_separator())
