...
void
vgui_gtk2_dialog_impl::modal(bool m)
{
  is_modal = m;
}
...
void *
vgui_gtk2_dialog_impl::pushbutton_field_widget(const char * text, const void * filename)
{
  ...
  if (filename)
  { // this button has an image on it
    image = gtk_image_new_from_file((const gchar *)filename);
    gtk_box_pack_start(GTK_BOX(box), image, FALSE, FALSE, 3);
    gtk_widget_show(image);
  }
  if (text)
  { // this button also has text on it
...
void *
vgui_gtk2_dialog_impl::choice_field_widget(const char * /*txt*/, const std::vector<std::string> & labels, int & val)
{
...
void *
vgui_gtk2_dialog_impl::inline_tableau_widget(const vgui_tableau_sptr tab, unsigned width, unsigned height)
{
...
extern "C"
{
  ...
  choose_cb(GtkWidget * /*widget*/, gpointer data)
  {
    ...
    *(ip->val) = ip->tmp;
    if (debug)
      std::cerr << "choose " << (ip->tmp) << '\n';
  }

} // extern "C"

void
vgui_gtk2_dialog_impl::set_ok_button(const char * txt)
{
  if (txt)
...
void
vgui_gtk2_dialog_impl::set_cancel_button(const char * txt)
{
  if (txt)
...
vgui_gtk2_dialog_impl::ask()
{
  ...
  for (std::vector<element>::iterator e_iter = elements.begin(); e_iter != elements.end(); ++e_iter)
  {
    ...
    {
      if (debug)
        std::cerr << "The dialog has line_br\n";
      has_line_break = true;
  ...
  for (std::vector<element>::iterator e_iter = elements.begin(); e_iter != elements.end(); ++e_iter)
  {
    ...
    {
      gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->vbox), current_hbox, TRUE, TRUE, 0);
    ...
    else if (l.type == button_elem)
    {
      ...
      vgui_command_cfunc * cmndc = static_cast<vgui_command_cfunc *>((field->cmnd).ptr());
      g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(cmndc->fn), NULL);
  ...
  for (std::vector<vgui_gtk2_adaptor *>::iterator iter = adaptor_list.begin(); iter != adaptor_list.end(); ++iter)
  {
    delete *iter;
  }
