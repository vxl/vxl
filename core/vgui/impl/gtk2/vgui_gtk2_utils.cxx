...
vgui_button
vgui_gtk2_utils::translate_button(int button)
{
  if (button == 1)
    return vgui_LEFT;
  else if (button == 2)
...
guint
vgui_gtk2_utils::translate_key_reverse(vgui_key key)
{
  guint gdk_key;
...
void
vgui_gtk2_utils::set_coordinates(vgui_event & e, const gdouble x, const gdouble y)
{
  GLint vp[4];
...
void
vgui_gtk2_utils::set_modifiers(vgui_event & e, const guint state)
{
  e.modifier =
    vgui_modifier(((state & 4) ? vgui_CTRL : 0) | ((state & 8) ? vgui_ALT : 0) | ((state & 1) ? vgui_SHIFT : 0));
}
...
vgui_gtk2_utils::is_modifier(const GdkEvent * gev)
{
  ...
  return e->state & GDK_CONTROL_MASK || e->state & GDK_SHIFT_MASK || e->state & GDK_MOD1_MASK;
}


static void
execute_command(GtkWidget *, gpointer c)
{
...
void
vgui_gtk2_utils::add_submenu(GtkWidget * widget, const vgui_menu & menu)
{
  for (unsigned i = 0; i < menu.size(); i++)
  {
    ...
    else if (menu[i].is_command())
    {
      ...
      gtk_signal_connect(
        GTK_OBJECT(item), "activate", GTK_SIGNAL_FUNC(execute_command), (void *)menu[i].cmnd.as_pointer());
...
void
vgui_gtk2_utils::set_menu(GtkWidget * widget, const vgui_menu & menu, bool is_menubar)
{
  for (unsigned i = 0; i < menu.size(); i++)
  {
    if (menu[i].is_separator())
