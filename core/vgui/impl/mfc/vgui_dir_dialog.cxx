...
CDlgWnd::OnOpen()
{
  ...
  else if ((len == 2 && ss[1] == ':') || (len == 3 && ss[1] == ':' && ss[2] == '\\'))
  {
    _TCHAR rootdir[4] = _T("?:\\");
    rootdir[0] = ss[0];
...
void
CDlgWnd::CheckDir(const CString & ss)
{
  ...
  if (strSaved.IsEmpty() || strSaved[0] == '.' || strNew.CompareNoCase(strSaved) == 0 ||
      strNew.CompareNoCase(strSaved + '\\') == 0)
  {
...
void
CDirEdit::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
  ...
  else if (nChar == '\r' || nChar == '\n')
  {
    ...
    else
    {
      ...
      if (attr == 0xFFFFFFFF)
      {
        ...
        if (len == 1 || (len > 1 && ss[1] != ':') || GetDriveType(rootdir) > DRIVE_NO_ROOT_DIR)
        {
          // Appears to be a valid drive (or relative path)
          CString mess(ss);
          mess += _T("\nThis folder does not exist.\n\nDo you want to create it?");
...
void
CDirEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
#if WINVER >= 0x0600
  : CFileDialog(TRUE, NULL, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_PATHMUSTEXIST, NULL, pParentWnd, 0, 0)
  , // Set bVistaStyle to FALSE
#else
...
vgui_dir_dialog::OnInitDone()
{
  ...
  VERIFY(m_Edit.Create(WS_TABSTOP | WS_VISIBLE | WS_CHILD, rct, pp, IDC_DIR));
  ...
  m_Open.Create(_T("Open"), WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON, rct, pp, IDC_OPEN);
  ...
  pp->ScreenToClient(rct);
  pp->GetDlgItem(IDOK)->SetWindowPos(NULL, rct.left, rct.top + 70, 0, 0, SWP_NOZORDER | SWP_NOSIZE);

  ASSERT(pp->GetDlgItem(edt1) != NULL);
  pp->GetDlgItem(edt1)->GetWindowRect(rct);
  pp->ScreenToClient(rct);
  pp->GetDlgItem(edt1)->SetWindowPos(NULL, rct.left, rct.top + 70, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
...
void
vgui_dir_dialog::OnFolderChange()
{
