...
vgui_mfc_adaptor::vgui_mfc_adaptor()
  : m_pDC(0)
  , m_pDC_default_bitmap(0)
  , m_pDC_aux(0)
  , m_pDC_aux_default_bitmap(0)
  , win_(0)
  , redraw_posted_(true)
...
void
vgui_mfc_adaptor::post_overlay_redraw()
{
...
void
vgui_mfc_adaptor::setup_adaptor(CWnd * this_cwnd, HDC oldDC, HGLRC oldContext)
{
  m_pCWnd = this_cwnd;
  hOldDC = oldDC;
  hOldRC = oldContext;
...
#ifdef _DEBUG
void
vgui_mfc_adaptor::AssertValid() const
{
  CView::AssertValid();
}

void
vgui_mfc_adaptor::Dump(CDumpContext & dc) const
{
  CView::Dump(dc);
}
#endif //_DEBUG
...
BOOL
vgui_mfc_adaptor::PreCreateWindow(CREATESTRUCT & cs)
{
...
int
vgui_mfc_adaptor::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
  ...
  if (vgui_mfc_use_bitmap)
    m_hRC = setup_for_gl(m_pDC, PFD_DRAW_TO_BITMAP | PFD_SUPPORT_OPENGL);
  else
    m_hRC = setup_for_gl(m_pDC, PFD_DRAW_TO_WINDOW | PFD_SUPPORT_OPENGL | PFD_DOUBLEBUFFER);
...
HGLRC
vgui_mfc_adaptor::setup_for_gl(CDC * pDC, DWORD dwFlags)
{
...
void
vgui_mfc_adaptor::create_bitmap(int cx, int cy, CDC *& pDC, HBITMAP & defaultBitmapForDC)
{
  ...
  else if (!DeleteObject(old_hbmp)) // Our object - delete it
  {
...
BOOL
vgui_mfc_adaptor::OnEraseBkgnd(CDC * pDC)
{
...
void
vgui_mfc_adaptor::service_redraws()
{
  if (redraw_posted_)
  {
    vgui_macro_report_errors;
...
void
vgui_mfc_adaptor::post_timer(float tm, int id)
{
  CWnd * wnd;
  if (m_pCWnd)
    wnd = m_pCWnd;
...
void
vgui_mfc_adaptor::kill_timer(int id)
{
  CWnd * wnd;
  if (m_pCWnd)
    wnd = m_pCWnd;
...
void
vgui_mfc_adaptor::OnDraw(CDC * pDC)
{
  if (debug)
    std::cerr << "OnDraw\n";
  // post_redraw();
...
void
vgui_mfc_adaptor::OnSize(UINT nType, int cx, int cy)
{
...
void
mfc_key(UINT nChar, UINT nFlags, int * the_key, int * the_ascii_char)
{
  if (nFlags & 256)
  {
    // Extended code
...
vgui_event
vgui_mfc_adaptor::generate_vgui_event(UINT nChar, UINT nRepCnt, UINT nFlags, vgui_event_type evttype)
{
...
void
vgui_mfc_adaptor::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
void
vgui_mfc_adaptor::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
  // Ignore Ctrl and Shift pressed alone:
  if (nChar == VK_SHIFT || nChar == VK_CONTROL)
    return;
...
void
vgui_mfc_adaptor::domouse(vgui_event_type et, UINT nFlags, CPoint point, vgui_button b)
{
#if 0
...
void
vgui_mfc_adaptor::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void
vgui_mfc_adaptor::OnLButtonUp(UINT nFlags, CPoint point)
{
...
void
vgui_mfc_adaptor::OnMButtonDown(UINT nFlags, CPoint point)
{
...
void
vgui_mfc_adaptor::OnMButtonUp(UINT nFlags, CPoint point)
{
...
void
vgui_mfc_adaptor::OnRButtonDown(UINT nFlags, CPoint point)
{
...
void
vgui_mfc_adaptor::OnRButtonUp(UINT nFlags, CPoint point)
{
...
void
vgui_mfc_adaptor::OnMouseMove(UINT nFlags, CPoint point)
{
