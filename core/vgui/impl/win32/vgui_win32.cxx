...
BOOL
vgui_win32::ProcessShellCommand(int argc, char ** argv)
{
  // We set szAppName_ to be the app name.
  char *p, *q = argv[0];

  // Skip path
  while (p = std::strchr(q, '\\'))
    q = ++p;
  // remove ".exe" suffix
  p = std::strstr(q, ".exe");
...
void
vgui_win32::init(int & argc, char ** argv)
{
  ...
  if (!RegisterClass(&wndclass))
  {
    MessageBox(NULL, TEXT("Fail to register window class for dialog box!"), NULL, MB_ICONERROR);
    assert(false); // quit ugly
  }
  ...
  if (!RegisterClass(&wndclass))
  {
    MessageBox(NULL, TEXT("Fail to register window class for inline tableau control!"), NULL, MB_ICONERROR);
    assert(false); // quit ugly
  }
}
...
vgui_win32::produce_dialog(const char * name)
{
  ...
  current_dialog = new vgui_win32_dialog_impl(name, win ? ((vgui_win32_window *)win)->getWindowHandle() : NULL);
...
inline void
vgui_win32::dump_window_stack()
{
  std::cout << "z-top of window stack: ";

  std::vector<vgui_window *>::const_iterator it;
  for (it = windows_to_delete.begin(); it != windows_to_delete.end(); it++)
...
// of windows.
void
vgui_win32::set_current_window(HWND hwnd)
{
...
void
vgui_win32::remove_current_window()
{
...
void
vgui_win32::remove_current_dialog()
{
