...
public:
  ...
  void
  set_width(unsigned w)
  {
    width = w;
  }
  void
  set_height(unsigned h)
  {
    height = h;
  }
  ...
  virtual BOOL
  OnCmdMsg(UINT message, WPARAM wParam, LPARAM lParam);
  // Called within message processing loop.
  void
  menu_dispatcher(int menuId);
  ...
  void
  OnHScroll(UINT message, WPARAM wParam, LPARAM lParam);
  void
  OnVScroll(UINT message, WPARAM wParam, LPARAM lParam);
  void
  OnKeyDown(WPARAM wParam, LPARAM lParam);
  void
  OnKeyUp(WPARAM wParam, LPARAM lParam);
  // void OnChar();
  void
  ...
  void
  OnMButtonDown(WPARAM wParam, LPARAM lParam);
  void
  OnMButtonUp(WPARAM wParam, LPARAM lParam);
  void
  OnRButtonDown(WPARAM wParam, LPARAM lParam);
  void
  OnRButtonUp(WPARAM wParam, LPARAM lParam);
  void
...
protected:
  ...
  vgui_event
  translate_message(WPARAM wParam, LPARAM lParam, vgui_event_type evtype = vgui_EVENT_NULL);
  // Translate a win32 key into the corresponding VGUI key
  void
  translate_key(UINT nChar, UINT nFlags, int * key, int * ascii_char);
  // Handle mouse event
  void
  domouse(vgui_event_type t, vgui_button b, UINT nFlags, int x, int y);
