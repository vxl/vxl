...
{
  for (std::vector<inline_tab_data>::iterator it = inline_tableaus.begin(); it != inline_tableaus.end(); ++it)
    delete it->adaptor;
...
void *
vgui_win32_dialog_impl::choice_field_widget(const char * /*txt*/, const std::vector<std::string> & labels, int & val)
{
...
void *
vgui_win32_dialog_impl::inline_tableau_widget(const vgui_tableau_sptr tab, unsigned width, unsigned height)
{
...
void *
vgui_win32_dialog_impl::pushbutton_field_widget(const char * label, const void * icon)
{
...
void
vgui_win32_dialog_impl::FindDialogSize(int & width,
                                       int & height,
                                       int & max_length,
                                       int & fbsr_count,
                                       int cxChar,
                                       ...
                                       int browser_length)
{
  ...
  for (std::vector<element>::iterator e_iter = elements.begin(); e_iter != elements.end(); ++e_iter)
  {
    ...
    if (!use_line_break)
    {                   // when no line break is present, each control
      field_length = 0; // takes up one line
      field_height = 0;
    }

    if (l.type == int_elem || l.type == long_elem || l.type == float_elem || l.type == double_elem ||
        l.type == string_elem || l.type == choice_elem || l.type == color_csr || l.type == inline_color_csr)
    {
...
vgui_win32_dialog_impl::ask()
{
  ...
  hWnd = CreateWindow(TEXT("vgui_win32_dialog"),
                      name.c_str(),
                      ...
                      NULL);
  if (hWnd == NULL)
    MessageBox(NULL, TEXT("Fail to create dialog box window!"), NULL, MB_ICONERROR);
  ...
  if (cancel_button_text_.size() > 0)
  {
    ...
    CreateWindow(TEXT("BUTTON"),
                 (char *)cancel_button_text_.c_str(),
                 WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                 x,
                 y,
                 cx,
  ...
  for (std::vector<element>::iterator e_iter = elements.begin(); e_iter != elements.end(); ++e_iter)
  {
        ...
        l.type == string_elem)
    {
      ...
      CreateWindow(TEXT("EDIT"),
                   (char *)field->current_value().c_str(),
    ...
    else if (l.type == choice_elem)
    {
      ...
      for (std::vector<std::string>::iterator s_iter = ch->names.begin(); s_iter != ch->names.end(); ++s_iter)
    ...
    else if (l.type == file_bsr || l.type == inline_file_bsr || l.type == color_csr || l.type == inline_color_csr)
    {
      cx = field->label.size() * cxChar;
      cy = edit_height;
      // Set static text first
...
LRESULT
vgui_win32_dialog_impl::DialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  switch (message)
  {
    case WM_CREATE:
...
vgui_win32_dialog_impl::OnOK()
{
  ...
  for (std::vector<element>::iterator e_iter = elements.begin(); e_iter != elements.end(); ++e_iter)
  {
    ...
    else if (l.type == file_bsr || l.type == color_csr || l.type == inline_file_bsr || l.type == inline_color_csr)
...
void
vgui_win32_dialog_impl::OnCancel()
{
  ok_clicked = false;
}
...
// COLORREF format
COLORREF
vgui_win32_dialog_impl::ColorStringToRGB(LPTSTR lpColor)
{
  int r, g, b;
...
inline bool
vgui_win32_dialog_impl::IsFileBrowserButton(unsigned short ctrl_id)
{
...
inline bool
vgui_win32_dialog_impl::IsColorChooserButton(unsigned short ctrl_id)
{
  std::vector<unsigned short>::iterator result;
...
inline bool
vgui_win32_dialog_impl::IsCallbackControl(unsigned short ctrl_id)
{
...
vgui_win32_adaptor *
vgui_win32_dialog_impl::find_adaptor(unsigned short ctrl_id)
{
  for (std::size_t i = 0; i < inline_tableaus.size(); i++)
...
void
vgui_win32_dialog_impl::dialog_dispatcher(int ctrl_id)
{
  for (std::size_t i = 0; i < callback_controls.size(); i++)
  {
    if (ctrl_id == callback_controls[i].child_id)
