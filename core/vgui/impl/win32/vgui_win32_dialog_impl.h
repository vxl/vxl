...
public:
  ...
  OnCancel();
  virtual BOOL
  OnBrowse(HWND hDlg, WORD wCtrlId);
  virtual BOOL
  OnColor(HWND hDlg, WORD wCtrlId, LPTSTR lpColor);

  // Return the number of inline tableaus in the dialog.
  int
  get_inline_tableau_size()
  {
    return inline_tableaus.size();
  }
  ...
  get_current_tab()
  {
    return inline_tableaus.front().adaptor;
  }

protected:
  COLORREF
  ColorStringToRGB(LPTSTR lpColor);
...
private:
  ...
  void
  DrawImageOnButton(HDC hDC, RECT * lprcItem, HBITMAP hBitmap, unsigned w, unsigned h, BOOL isDisabled);

  // Determine if the control with identifier "ctrl_id"
  // is a file browser button.
  bool
  IsFileBrowserButton(unsigned short ctrl_id);
  // Determine if the control with identifier "ctrl_id"
  // is a color chooser button.
  bool
  IsColorChooserButton(unsigned short ctrl_id);
  // Determine if the control with identifier "ctrl_id"
  // has a callback function with it.
  bool
  IsCallbackControl(unsigned short ctrl_id);

  // Find the inline adaptor with identifier "ctrl_id".
  vgui_win32_adaptor *
  find_adaptor(unsigned short ctrl_id);
