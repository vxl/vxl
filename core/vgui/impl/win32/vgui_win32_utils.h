...
public:
  ...
  HMENU vgui_menu_to_win32(vgui_menu const & vguimenu, vcl_vector<vgui_command_sptr> & callbacks, HACCEL * hAccel,
                           bool isPopup = false);

  // Convert a vgui_menu to an extended Win32 menu
  HMENU vgui_menu_to_win32ex(vgui_menu const & vguimenu, vcl_vector<vgui_command_sptr> & callbacks, HACCEL * hAccel,
                             bool isPopup = false);
