...
public:
  ...
  static vgui_win32_utils *
  instance();

  // Convert a vgui_menu to a Win32 menu
  HMENU
  vgui_menu_to_win32(const vgui_menu & vguimenu,
                     std::vector<vgui_command_sptr> & callbacks,
                     HACCEL * hAccel,
                     bool isPopup = false);

  // Convert a vgui_menu to an extended Win32 menu
  HMENU
  vgui_menu_to_win32ex(const vgui_menu & vguimenu,
                       std::vector<vgui_command_sptr> & callbacks,
                       HACCEL * hAccel,
                       bool isPopup = false);
...
private:
  int
  addMenuItems(const vgui_menu & vguimenu, int offset, bool is_popup);
  int
  addMenuItemsEx(const vgui_menu & vguimenu, int offset, bool is_popup);
  void
  addAccelerator(std::string &, const vgui_menu_item &, int);
  std::string vgui_key_to_string(vgui_key);
  UINT vgui_key_to_virt_key(vgui_key);

  // Show explainary message for GetLastError().
  void
  ShowErrorMessage(DWORD dwErrorNo);
