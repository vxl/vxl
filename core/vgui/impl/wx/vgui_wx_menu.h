...
public:
  ...
  wxMenuBar *
  create_wx_menubar(const vgui_menu & menu);

  //: Create a wxMenu that maps to the vgui_menu.
  wxMenu *
  create_wx_menu(const vgui_menu & menu);

private:
  //: Helper class that actually builds the mapped menus.
  wxMenu *
  create_wx_submenu(const vgui_menu & menu);

  //: Create the accelerator substring to add to the menu item name.
  std::string
  create_accelerator_string(const vgui_menu_item & item) const;
  ...
  void
  on_command(wxCommandEvent & event);
