...
{
  if (debug) 
    vcl_cerr << "vgui_accelerate_mfc::vgui_accelerate_mfc() Initializing Windows/MFC acceleration...\n";
  BytesPerPixel = 0;
...
vidfmt::vidfmt()
{
  ...
  if (debug)
    vcl_cerr << "Current video mode is " << dib_hdr->biBitCount << "-bit; ";

  switch(dib_hdr->biCompression) {
  case BI_BITFIELDS: {
      DWORD * fields = (DWORD*) ((char*)dib_hdr + dib_hdr->biSize);
      if (debug)
        vcl_cerr << "masks ["<<vcl_hex<< fields[0]<<' '<< fields[1]<<' '<< fields[2]<<"] ";
      switch (fields[0])
      {
      case 0xf800:
        ...
        gl_format = GL_RGB;
          if (debug)
            vcl_cerr << "    (565 BGR pixel alignment)";
          break;
      case 0x7c00:
        gl_type = GL_UNSIGNED_SHORT_5_5_5_1;
        gl_format = GL_RGB;
          if (debug)
            vcl_cerr << "    (555 BGR pixel alignment)";
          break;
      case 0xff0000:
        gl_type = GL_UNSIGNED_BYTE;
        gl_format = GL_BGR;
          if (debug)
            vcl_cerr << "    (888 BGR pixel alignment)";
          break;
      case 0x0000ff:
        gl_type = GL_UNSIGNED_BYTE;
        gl_format = GL_RGB;
          if (debug)
            vcl_cerr << "    (888 RGB pixel alignment)";
          break;
      default:
          vcl_cerr << "vgui_accelerate_mfc:    (Unknown pixel alignment "
  ...
  if (bpp == 16) {
    ...
    if (debug)
      vcl_cerr << "redmask = "<< vcl_hex << redmask <<'\n';
    ...
    case 0x7c00:
      ...
      break;
    default:
      vcl_cerr << "vgui_accelerate_mfc:    (Unknown redmask "<< vcl_hex << redmask <<")\n";
    }
  }
  ...
  for (int component = 0; component < 3; ++component) {
    ...
    b.bmBits = opx;
    bitmap.GetBitmap(&b);
    vcl_cerr << "BITMAP: w "<< b.bmWidth <<", h "<< b.bmHeight <<", bytesperline "
             << b.bmWidthBytes <<", bitsPixel "<< b.bmBitsPixel <<", bits "<< b.bmBits <<'\n';

    bitmap.GetBitmap(&b);
    bitmap.GetBitmapBits(width*b.bmBitsPixel,opx);
    vcl_cerr << "opx ";
    for (int p = 0; p < width; ++p)
      vcl_cerr  << vcl_hex << (int)opx[2*p + 1] << ' ' << (int)opx[2*p + 0] << " | "
    vcl_cerr << '\n';
  }
#endif
}
