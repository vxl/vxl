...
// These accelerated functions take advantage of the fact the Mesa-X11 uses an XImage for
// its backbuffer. This allows us to determine the optimal pixel-format for image caching, 
// and also to use the Hermes library, with its MMX/PII optimized code, to perform
// surface clearing, pixel conversion and stretching/blitting.
...
gl_to_hermes_format_map gl_to_hermes_formats[] = {
  ...
  {GL_RGB,      GL_UNSIGNED_SHORT_5_6_5, 16,  (0x0000f800),  (0x000007e0),  (0x0000001f), 0}          // capes - this format assumes whole word read/writes
};
#undef s
...
{
  vcl_cerr << "Initializing Mesa/X11 accelerator" << vcl_endl;

#ifdef HAS_HERMES
  vcl_cerr << "Initializing Hermes" << vcl_endl;
  Hermes_Init();
...
vgui_accelerate_x11::~vgui_accelerate_x11()
{
  vcl_cerr << "Destroying Mesa/X11 accelerator" << vcl_endl;
...
bool vgui_accelerate_x11::vgui_glClear( GLbitfield mask )
{
  if (!vgui_no_acceleration) {
	...
	HermesFormat* dest_format = Hermes_FormatNew(backbuffer->bits_per_pixel,
	...
	Hermes_ClearerClear(hermes_clearer,backbuffer->data,
			    ...
			    (int32)(clear_color[0]*255.0F),(int32)(clear_color[1]*255.0F),(int32)(clear_color[2]*255.0F),(int32)(clear_color[3]*255.0F));
...
bool vgui_accelerate_x11::vgui_glDrawPixels( GLsizei width, GLsizei height, GLenum format, GLenum type, const GLvoid *pixels )
{
  if (!vgui_no_acceleration) {
#if defined(VGUI_MESA) && defined(HAS_HERMES)
    GLint render_mode;
	...
	vcl_cerr << "raspos " << raster_pos[0] << " " << raster_pos[1] << " " << raster_pos[2] << " " << raster_pos[3] << vcl_endl;
	...
	*/
	...
	// Find the OUTERMOST boundary of the scaled source image in window coords
	...
	*/
	...
	/*
	vcl_cerr << "clipped dest - " << vcl_endl;
	vcl_cerr << dest_x_min << " " << dest_x_max << vcl_endl;
	vcl_cerr << dest_y_min << " " << dest_y_max << vcl_endl;
	vcl_cerr << "clipped src - " << vcl_endl;
...
bool vgui_accelerate_x11::vgui_choose_cache_format( GLenum* format, GLenum* type)
{
  if (!vgui_no_acceleration) {
    ...
    glGetIntegerv (GL_DRAW_BUFFER, &draw_buffer);
    glGetBooleanv (GL_DEPTH_TEST, &depth_test_enabled);
    if (render_mode == GL_RENDER && (draw_buffer == GL_BACK || draw_buffer == GL_BACK_LEFT) && rgba_mode == GL_TRUE && !depth_test_enabled) {
      XImage* backbuffer;
      XMesaGetBackBuffer(mesabuf, &p_dummy, &backbuffer);
