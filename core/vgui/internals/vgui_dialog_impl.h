...
public:
  ...
  void
  pushbutton_field(vgui_command_sptr cmnd, const char * label, const void * icon);

  //: Pointer to a GUI widget for a bool field.
  virtual void *
  bool_field_widget(const char *, bool &);

  //: Pointer to a GUI widget for an integer field.
  virtual void *
  int_field_widget(const char *, int &);

  //: Pointer to a GUI widget for a long integer field.
  virtual void *
  long_field_widget(const char *, long &);

  //: Pointer to a GUI widget for a float field.
  virtual void *
  float_field_widget(const char *, float &);

  //: Pointer to a GUI widget for a double field.
  virtual void *
  double_field_widget(const char *, double &);

  //: Pointer to a GUI widget for a string field.
  virtual void *
  string_field_widget(const char *, std::string &);

  //: Pointer to a GUI widget for a choice field.
  virtual void *
  choice_field_widget(const char *, const std::vector<std::string> &, int &);

  //: Pointer to a GUI widget for a text message.
  virtual void *
  text_message_widget(const char *);

  //: Pointer to a GUI widget for a file browser.
  virtual void *
  file_browser_widget(const char *, std::string &, std::string &);

  //: Pointer to a GUI widget for an inline file browser.
  virtual void *
  inline_file_browser_widget(const char *, std::string &, std::string &);

  //: Pointer to a GUI widget for a colour chooser.
  virtual void *
  color_chooser_widget(const char *, std::string &);

  //: Pointer to a GUI widget for an inline colour chooser.
  virtual void *
  inline_color_chooser_widget(const char *, std::string &);

  //: Pointer to a GUI widget for a tableau (OpenGL area).
  virtual void *
  inline_tableau_widget(const vgui_tableau_sptr tab, unsigned width, unsigned height);

  //: Pointer to a GUI widget for a push button.
  virtual void *
  pushbutton_field_widget(const char *, const void *);
  ...
  virtual void
  set_cancel_button(const char * msg)
  {
    cancel_button_text_ = msg ? msg : "";
  }

  //: Set the text on the OK button.
  virtual void
  set_ok_button(const char * msg)
  {
    ok_button_text_ = msg ? msg : "";
  }
