...
{
  fp =
    FramePix( g_mapRbyte[map_scale_and_bias(p.R)], g_mapGbyte[map_scale_and_bias(p.G)], g_mapBbyte[map_scale_and_bias(
                                                                                                     p.B)], 0 );
}
...
template <class PixType>
test_it<PixType>::test_it( vcl_string const& name, GLenum in_pack_type, GLenum in_pix_type )
...
template <class PixType>
void
test_it<PixType>::test_pattern( unsigned option )
{
