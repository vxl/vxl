...
public:
  ...
  void
  print_help_message() const;
};

void
front_back_wibbler::print_help_message() const
{
...
bool
front_back_wibbler::handle(const vgui_event & e)
{
  ...
  switch (e.key)
  {
    ...
    case ' ':
                ...
                << "   extensions : " << glGetString(GL_EXTENSIONS) << std::endl;
      {
        ...
        std::cerr << "   "
                  << "size of modelview matrix stack is " << int_v << std::endl;

        glGetIntegerv(GL_MAX_PROJECTION_STACK_DEPTH, &int_v);
        std::cerr << "   "
                  << "size of projection matrix stack is " << int_v << std::endl;
