...
static void
test_image_tableau(int argc, char * argv[])
{
  ...
  else
  {
    ...
    TEST("Size is correct",
         img.width() == img2.width() && img.height() == img2.height() && img.components() == img2.components() &&
           img.planes() == img2.planes() && img.bits_per_component() == img2.bits_per_component(),
         true);

    unsigned buf_size =
      img.width() * img.height() * img.planes() * img.components() * ((img.bits_per_component() + 7) / 8);
    char * img1_buf = new char[buf_size];
    char * img2_buf = new char[buf_size];

    TEST("Get section from img1", !img.get_section(img1_buf, 0, 0, img.width(), img.height()), false);
    TEST("Get section from img2", !img2.get_section(img2_buf, 0, 0, img2.width(), img2.height()), false);
