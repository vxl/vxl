...
void draw_stuff(int window_width, int window_height, double scale=0);
...
END_EVENT_TABLE()
void BasicGLPane::on_maximize(wxMaximizeEvent& event)
{
  parent_frame->SetStatusText("maximize");
}

void BasicGLPane::on_mouse_event(wxMouseEvent& event)
{
...
void BasicGLPane::display(wxPaintEvent& evt)
{
  draw_stuff(getWidth(), getHeight(), GetContentScaleFactor());
  SwapBuffers();
}

void draw_stuff(int window_width, int window_height, double scale)
{
  GLint vp[4];
  ...
  vgui_utils::set_glViewport(topleft_x, topleft_y,
                             bottomright_x-topleft_x, bottomright_y-topleft_y,
                             scale);
  cout << "Logical box: " << topleft_x << "," << topleft_y << "," << bottomright_x-topleft_x << "," << bottomright_y-topleft_y << endl;
...
{
  example_tableau(vgui_tableau_sptr t) : vgui_viewer2D_tableau(t){ }
...
{
  example_tableau_new(vgui_tableau_sptr t) : vgui_viewer2D_tableau_sptr(
    new example_tableau(t)) { }
};
