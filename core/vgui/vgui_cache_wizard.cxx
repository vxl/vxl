...
// wizard won't work.
vgui_cache_wizard::vgui_cache_wizard(int quadrant_width, int quadrant_height)
{
  if (debug)
...
//: Loads an image
int
vgui_cache_wizard::load_image(vil1_image img)
{
  ...
  for (int i = 0; i <= qh_c; i++)
    for (int j = 0; j <= qw_c; j++)
    {
      ...
      else
      {
        ...
        {
          unsigned int k = 0;
          while (k < max_texture_num_ && texture_names_[k] == GLuint(-1))
            ++k;
        ...
        TexImage2D_Brownie(vil1_crop(img,
                                     pos->first * quadrant_width_ + j * quadrant_width_,
