...
public:
  ...
  vgui_debug_tableau(const vgui_tableau_sptr & child);

  //: Returns the type of this tableau ('vgui_debug_tableau').
  std::string
  type_name() const
  {
    return "vgui_debug_tableau";
  }
...
struct vgui_debug_tableau_new : public vgui_debug_tableau_sptr
{
  ...
  vgui_debug_tableau_new(const vgui_tableau_sptr & child)
    : base(new vgui_debug_tableau(child))
  {}
};
