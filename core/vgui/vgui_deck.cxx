...
vgui_deck::vgui_deck(vgui_tableau_ref const& child0, vgui_tableau_ref const& child1) : index_(-1) {
  add(child0);
  add(child1);
}

vgui_deck::vgui_deck(vgui_tableau_ref const& child0, vgui_tableau_ref const& child1, vgui_tableau_ref const& child2) : index_(-1) {
  add(child0);
  add(child1);
  add(child2);
}
...
bool vgui_deck::handle(const vgui_event& event) {
  ...
  vgui_tableau_ref t = children[index_];
  return t->handle(event);
}

bool vgui_deck::help() {
  vcl_cerr << vcl_endl;
  vcl_cerr << "-- vgui_deck ------------------------------------" << vcl_endl;
  vcl_cerr << "|     keys                                      |" << vcl_endl;
  vcl_cerr << "| `PageUp' and `PageDown'  switch current child |" << vcl_endl;
  vcl_cerr << "-------------------------------------------------" << vcl_endl;
  vcl_cerr << vcl_endl;
...
void vgui_deck::add(vgui_tableau_ref const& t) {
  add_child(t);
}

// override virtual base class method
bool vgui_deck::add_child(vgui_tableau_ref const& t) {
...
void vgui_deck::remove(vgui_tableau_ref const& t) {
  if (!remove_child(t))
    vcl_cerr << __FILE__ " no such child tableau : " << t << vcl_endl;
}

// override virtual base class  method
bool vgui_deck::remove_child(vgui_tableau_ref const& t) {
...
vgui_tableau_ref vgui_deck::current() {
  if (index_ok(index_)) 
...
vgui_tableau_ref vgui_deck::get_tableau_at(int tab_pos) {
  if (index_ok(tab_pos))
...
void vgui_deck::get_popup(const vgui_popup_params& params, vgui_menu &menu) {
  ...
  if (params.defaults) {
    ...
    submenu.add(active_label.c_str(),
		new vgui_command_simple<vgui_tableau>(this, &vgui_tableau::toggle_active));
    ...
    submenu.add(visible_label.c_str(),
		new vgui_command_simple<vgui_tableau>(this, &vgui_tableau::toggle_visible)); 
  }
  */
