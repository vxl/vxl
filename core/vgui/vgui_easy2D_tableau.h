...
public:
  ...
  vgui_easy2D_tableau(const vgui_image_tableau_sptr &, const char * n = "unnamed");

  //: Constructor - don't use this, use vgui_easy2D_tableau_new.
  //  Creates an easy2D with the given child tableau.
  vgui_easy2D_tableau(const vgui_tableau_sptr &, const char * n = "unnamed");
  ...
  void
  set_image(const std::string & image);

  //: Set the child tableau to be the given tableau.
  void
  set_child(const vgui_tableau_sptr &);
  ...
  void
  set_foreground(float r, float g, float b, float a = 1.0f);
  ...
  void
  print_psfile(std::string filename, int reduction_factor, bool print_geom_objs, int wd = -1, int ht = -1);
  ...
  // The newer versions that take an image parameter will work more reliably.
  ...
  //
  vgui_soview2D_image *
  add_image(float x, float y, const vil_image_view_base & img);
  ...
  //
  vgui_soview2D_image *
  add_image(float x, float y, const vil1_image & img);
