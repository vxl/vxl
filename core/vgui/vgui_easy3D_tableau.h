...
public:
  ...
  vgui_lineseg3D *
  add_line(float x0, float y0, float z0, float x1, float y1, float z1);

  //: Add a 3D-triangle with the given vertices.
  vgui_triangle3D *
  add_triangle(float x0, float y0, float z0, float x1, float y1, float z1, float x2, float y2, float z2);

  //: Set the colour of objects to the given RGB value.
  void
  set_foreground(float red, float green, float blue);

  //: Set the line width of all objects to the given width.
  void
  set_line_width(float new_width);

  //: Set the radius of all points to the given radius.
  void
  set_point_radius(float new_radius);

  void
  save_e3d(const char * filename);
  void
  load_e3d(const char * filename);
...
struct vgui_easy3D_tableau_new : public vgui_easy3D_tableau_sptr
{
  ...
  vgui_easy3D_tableau_new()
    : base(new vgui_easy3D_tableau())
  {}
};
