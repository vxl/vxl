...
public:
  ...
  vgui_grid_tableau(unsigned initial_columns = 1, unsigned initial_rows = 1) { init(initial_columns, initial_rows); }

  //: Constructor - don't use this, use vgui_grid_tableau_new.
  //  This creates a bi-tab, taking the two tableaux as parameters.
  vgui_grid_tableau(const vgui_tableau_sptr & l, const vgui_tableau_sptr & r);

  //: Constructor - don't use this, use vgui_grid_tableau_new.
  //  This creates a tri-tab, taking the three tableau as parameters.
  vgui_grid_tableau(const vgui_tableau_sptr & l, const vgui_tableau_sptr & m, const vgui_tableau_sptr & r);

  //: Given the column number, returns the x coord for that column.
  float
  get_x(unsigned index);

  //: Given the row number, returns the y coord for that row.
  float
  get_y(unsigned index);
  ...
  void
  add_next(const vgui_tableau_sptr & tab);

  //: Adds a tableau to the next free space and returns the grid location
  void
  add_next(const vgui_tableau_sptr & tab, unsigned & col, unsigned & row);

  //: Add (or replace the tableau at the given position with) the given tableau.
  void
  add_at(const vgui_tableau_sptr & tab, unsigned col_pos, unsigned row_pos);

  //: Removes the tableau at the given grid coordinates from the display
  void
  remove_at(unsigned col_pos, unsigned row_pos);
  ...
  vgui_tableau_sptr
  get_tableau_at(unsigned col_pos, unsigned row_pos);
  ...
  void
  get_active_position(unsigned * col_pos, unsigned * row_pos);

  //: Returns the most recently selected column and row positions.
  void
  get_last_selected_position(unsigned * col_pos, unsigned * row_pos);

  //: Gets the positions and times of selection of the selected tableaux.
  int
  get_selected_positions(std::vector<int> * col_pos, std::vector<int> * row_pos, std::vector<int> * times);

  //: Select a certain tableau
  void
  set_selected(int r, int c, bool onoff = true);

  //: True to allow the grid size to change, false to have fixed size.
  void
  set_grid_size_changeable(bool v)
  {
  ...
  bool
  cell_bounding_box(unsigned col, unsigned row, float & xmin, float & ymin, float & xmax, float & ymax);
...
struct vgui_grid_tableau_new : public vgui_grid_tableau_sptr
{
  ...
  {}
  vgui_grid_tableau_new(const vgui_tableau_sptr & l, const vgui_tableau_sptr & r)
    : base(new vgui_grid_tableau(l, r))
  {}
  vgui_grid_tableau_new(const vgui_tableau_sptr & l, const vgui_tableau_sptr & m, const vgui_tableau_sptr & r)
