...
public:
  ...
  void
  set_file_name(const std::string & fn)
  {
    name_ = fn;
  }
  ...
  virtual void
  set_image_view(const char * filename, const vgui_range_map_params_sptr & rmp = nullptr);

  //: Make image loaded from the given file, the image rendered by this tableau.
  // The image will be stored as a vil1_image, and thus can be
  // retrieved with get_image() but not with get_image_view().
  virtual void
  set_image(const char * filename, const vgui_range_map_params_sptr & rmp = nullptr);
  ...
  virtual void
  get_popup(const vgui_popup_params &, vgui_menu & m);
...
protected:
  ...
  vgui_image_tableau(const vil1_image & img, const vgui_range_map_params_sptr & rmp = nullptr);

  //: Constructor - don't use this, use vgui_image_tableau_new.
  //  Creates a tableau displaying the given image view.
  vgui_image_tableau(const vil_image_view_base & img, const vgui_range_map_params_sptr & rmp = nullptr);

  //: Constructor - don't use this, use vgui_image_tableau_new.
  //  Creates a tableau displaying the given image resource.
  vgui_image_tableau(const vil_image_resource_sptr & img, const vgui_range_map_params_sptr & rmp = nullptr);

  //: Constructor - don't use this, use vgui_image_tableau_new.
  //  Creates a tableau which loads and displays an image from
  //  the given file. File loaded as a vil1_image.
  vgui_image_tableau(const char * filename, const vgui_range_map_params_sptr & rmp = nullptr);
...
struct vgui_image_tableau_new : public vgui_image_tableau_sptr
{
  ...
  vgui_image_tableau_new(const vil1_image & t, const vgui_range_map_params_sptr & rmp = nullptr)
    : vgui_image_tableau_sptr(new vgui_image_tableau(t, rmp))
  {}

  //:  Constructor - creates a tableau displaying the given image.
  vgui_image_tableau_new(const vil_image_view_base & t, const vgui_range_map_params_sptr & rmp = nullptr)
    : vgui_image_tableau_sptr(new vgui_image_tableau(t, rmp))
  {}

  //:  Constructor - creates a tableau displaying the given image.
  vgui_image_tableau_new(const vil_image_resource_sptr & t, const vgui_range_map_params_sptr & rmp = nullptr)
    : vgui_image_tableau_sptr(new vgui_image_tableau(t, rmp))
  {}
  ...
  // The image will be stored as a vil1_image, and thus can be
  // retrieved with get_image() but not with get_image_view().
  vgui_image_tableau_new(const char * f, const vgui_range_map_params_sptr & rmp = nullptr)
