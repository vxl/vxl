...
#include <vgui/vgui_listmanager2D_ref.h>
...
public:
 ...
 ~vgui_listmanager2D();
  ...
  void add(vgui_displaylist2D_ref const&);
  void remove(vgui_displaylist2D_ref const&);
...
protected:
  ...
  vgui_displaylist2D_ref highlight_list;
  vgui_soview2D *highlight_so;
  vgui_event saved_event_;

  vgui_displaylist2D_ref contains_hit(vcl_vector<unsigned> const& names);
...
struct vgui_listmanager2D_new : public vgui_listmanager2D_ref {
  vgui_listmanager2D_new() : 
    vgui_listmanager2D_ref(new vgui_listmanager2D) { }
};
