...
std::string
vgui_listmanager2D_tableau::type_name() const
{
...
void
vgui_listmanager2D_tableau::set_active(int v, bool b)
{
  if (!index_ok(v))
    return;
  active[v] = b;
...
bool
vgui_listmanager2D_tableau::is_active(int v)
{
  if (!index_ok(v))
    return false;
  return active[v];
...
void
vgui_listmanager2D_tableau::set_visible(int v, bool b)
{
  if (!index_ok(v))
    return;
  visible[v] = b;
...
bool
vgui_listmanager2D_tableau::is_visible(int v)
{
  if (!index_ok(v))
    return false;
  return visible[v];
...
bool
vgui_listmanager2D_tableau::index_ok(int v)
{
  return v >= 0 && v < int(children.size());
}

bool
vgui_listmanager2D_tableau::help()
{
  std::cerr << '\n'
...
void
vgui_listmanager2D_tableau::get_hits(float x, float y, std::vector<std::vector<unsigned>> * hits)
{
  GLuint * ptr = vgui_utils::enter_pick_mode(x, y, 100);

  int count = 0;
  for (std::vector<vgui_parent_child_link>::iterator i = this->children.begin(); i != this->children.end();
       ++i, ++count)
  {
...
#endif
  for (std::vector<std::vector<unsigned>>::iterator h_iter = hits->begin(); h_iter != hits->end(); ++h_iter)
  {
...
bool
vgui_listmanager2D_tableau::handle(const vgui_event & event)
{
...
vgui_listmanager2D_tableau::motion(int x, int y)
{
  ...
  if (closest_so && closest_display)
    std::cerr << "vgui_listmanager2D_tableau::motion: hit " << closest_so->get_id() << " in std::list "
              << closest_display->get_id() << std::endl;
#endif
...
bool
vgui_listmanager2D_tableau::mouse_down(int x, int y, vgui_button button, vgui_modifier modifier)
{
...
bool
vgui_listmanager2D_tableau::key_press(int /*x*/, int /*y*/, vgui_key key, vgui_modifier)
{
#ifdef DEBUG
