...
VGUI_PIXEL_CONVERT_SPAN_DO(vgui_pixel_rgb888, vgui_pixel_bgra5551, { d.R = s.R >> 3; d.G = s.G >> 3; d.B = s.B >> 3; SET_FF(
                                                                       d.A); })
...
VGUI_PIXEL_CONVERT_SPAN_DO(vgui_pixel_rgbfloat, vgui_pixel_rgb888,  {d.R = clamp(s.R); d.G = clamp(s.G); d.B = clamp(
                                                                         s.B); })
VGUI_PIXEL_CONVERT_SPAN_DO(vgui_pixel_rgbfloat, vgui_pixel_bgr888,  {d.R = clamp(s.R); d.G = clamp(s.G); d.B = clamp(
                                                                         s.B); })
VGUI_PIXEL_CONVERT_SPAN_DO(vgui_pixel_rgbfloat, vgui_pixel_rgb565,  {d.R = clamp(s.R) >> 3; d.G = clamp(
                                                                         s.G) >> 2; d.B = clamp(s.B) >> 3; })
VGUI_PIXEL_CONVERT_SPAN_DO(vgui_pixel_rgbfloat, vgui_pixel_bgra5551, {d.R = clamp(s.R) >> 3; d.G = clamp(
                                                                          s.G) >> 3; d.B = clamp(s.B) >> 3; })
VGUI_PIXEL_CONVERT_SPAN_DO(vgui_pixel_rgbfloat, vgui_pixel_rgba8888,
 {d.R = clamp(s.R); d.G = clamp(s.G); d.B = clamp(s.B); SET_FF(d.A); })
