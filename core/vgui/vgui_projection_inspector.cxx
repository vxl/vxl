...
void
vgui_projection_inspector::print(std::ostream & strm) const
{
...
// \endverbatim
static bool
is_affine(const vnl_double_4x4 & M)
{
...
vgui_projection_inspector::inspect()
{
  ...
  // [       * ]
  // \endverbatim
  if (is_affine(T) && T(1, 0) == 0 && T(0, 1) == 0 && T(2, 0) == 0 && T(0, 2) == 0 && T(2, 1) == 0 && T(1, 2) == 0 &&
      T(3, 3) != 0)
  {
    //
...
void
vgui_projection_inspector::window_to_image_coordinates(int x, int y, float & xi, float & yi) const
{
...
void
vgui_projection_inspector::image_to_window_coordinates(float ix, float iy, float & wx, float & wy) const
{
...
//-----------------------------------------------------------------------------
bool
vgui_projection_inspector::image_viewport(float & a0, float & b0, float & a1, float & b1)
{
  if (!diagonal_scale_3d)
    return false;
