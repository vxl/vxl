...
bool vgui_projection_inspector::back_project(double const x[3],
  ...
  vnl_matrix<double> T(P*M);

  // get rows of corresponding 3x4 projection matrix :
  vnl_vector<double> a(T.get_row(0));
  vnl_vector<double> b(T.get_row(1));
  vnl_vector<double> c(T.get_row(3));
  ...
  vnl_matrix<double> omega( devx*outer_product(b,c) + devy*outer_product(c,a) + devw*outer_product(a,b) );
  ...
  vnl_matrix<double> omegad(omegad_,4,4);

  // backproject :
  vnl_vector<double> X_( omegad * vnl_vector<double>(p,4) );
...
{
  if (x.size() == 2) {
    vnl_vector<double> x_(3);
    x_[0]=x[0];
    x_[1]=x[1];
    x_[2]=1;
    vnl_vector<double> X_(back_project(x_,p));
...
// * * * *
// 0 0 0 *
static bool is_affine(const vnl_matrix<double> &M)
{
  M.assert_size(4,4);
