...
public:
  ...
  void
  window_to_image_coordinates(int, int, float &, float &) const;

  //: Convert image coords to window coords.
  void
  image_to_window_coordinates(float, float, float &, float &) const;

  //: Returns the corners of the backprojection of the viewport onto z=0.
  bool
  image_viewport(float & bottom_left_x, float & bottom_left_y, float & top_right_x, float & top_right_y);
  ...
  bool
  compute_as_2d_affine(int width, int height, float * offsetX, float * offsetY, float * scaleX, float * scaleY);

  //: Convert window coords (eg. from vgui_event) to image coords.
  //  Some people prefer to put &s on "return value" parameters.
  void
  window_to_image_coordinates(int wx, int wy, float * ix, float * iy) const
  {
  ...
  //  Some people prefer to put &s on "return value" parameters.
  void
  image_to_window_coordinates(float ix, float iy, float * wx, float * wy) const
  {
  ...
  bool
  back_project(const double x[3], const double p[4], double X[4]) const;

  //: Back-projection of a given point onto a given plane p.
  //  Returns a 3-std::vector.
  vnl_vector<double>
  back_project(double x, double y, const vnl_double_4 & p) const;

  //: Back-projection of a given point onto a given plane p.
  //  Returns a 4-std::vector.
  vnl_vector<double>
  back_project(double x, double y, double z, const vnl_double_4 & p) const;

  //: Back-projection of a given point onto a given plane p.
  //  x is a 2-vector (Euclidean coordinates), and the returned 3-vector is also Euclidean.
  vnl_vector<double>
  back_project(const vnl_double_2 & x, const vnl_double_4 & p) const;

  //: Back-projection of a given point onto a given plane p.
  //  x is a 3-vector (projective coordinates), and the returned 4-vector is also projective.
  vnl_vector<double>
  back_project(const vnl_double_3 & x, const vnl_double_4 & p) const;
