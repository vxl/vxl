...
public:
  ...
  {
    return this->map_pixel_byte(pix, Type(min_L_), Type(max_L_), gamma_L_,
                                ratio_L_);
  }
  // : Red channel computed map
  vxl_byte map_R_pixel(const Type pix)
  {
    return this->map_pixel_byte(pix, Type(min_R_), Type(max_R_), gamma_R_,
                                ratio_R_);
  }
  // : Green channel computed map
  vxl_byte map_G_pixel(const Type pix)
  {
    return this->map_pixel_byte(pix, Type(min_G_), Type(max_G_), gamma_G_,
                                ratio_G_);
  }
  // : Blue channel computed map
  vxl_byte map_B_pixel(const Type pix)
  {
    return this->map_pixel_byte(pix, Type(min_B_), Type(max_B_), gamma_B_,
                                ratio_B_);
  }
  // : Alpha or Infrared channel computed map
  vxl_byte map_X_pixel(const Type pix)
  {
    return this->map_pixel_byte(pix, Type(min_X_), Type(max_X_), gamma_X_,
                                ratio_X_);
  }
