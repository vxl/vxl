...
template <class Type>
void
vgui_range_map<Type>::init_map_domain(const Type min, const Type max, long double & ratio)
{
  assert(max >= min);
  assert(!std::numeric_limits<Type>::has_infinity ||
         (-min != std::numeric_limits<Type>::infinity() && max != std::numeric_limits<Type>::infinity()));
  ratio = 1.0;
  if (max != min)
    ratio = 1.0 / (max - min);
}
...
template <>
inline void
vgui_range_map<bool>::init_map_domain(const bool min, const bool max, long double & ratio)
{
  assert(max >= min);
  assert(!std::numeric_limits<bool>::has_infinity ||
         (min != std::numeric_limits<bool>::infinity() && max != std::numeric_limits<bool>::infinity()));
  ratio = 1.0;
}

template <>
inline void
vgui_range_map<unsigned int>::init_map_domain(const unsigned int min, const unsigned int max, long double & ratio)
{
  assert(max >= min);
  assert(!std::numeric_limits<unsigned int>::has_infinity || (min != std::numeric_limits<unsigned int>::infinity() &&
...
template <>
inline void
vgui_range_map<unsigned short>::init_map_domain(const unsigned short min, const unsigned short max, long double & ratio)
{
  assert(max >= min);
  assert(
    !std::numeric_limits<unsigned short>::has_infinity ||
    (min != std::numeric_limits<unsigned short>::infinity() && max != std::numeric_limits<unsigned short>::infinity()));
  ratio = 1.0;
  if (max != min)
    ratio = 1.0 / (max - min);
}
...
template <>
inline void
vgui_range_map<unsigned long>::init_map_domain(const unsigned long min, const unsigned long max, long double & ratio)
{
  assert(max >= min);
  assert(!std::numeric_limits<unsigned long>::has_infinity || (min != std::numeric_limits<unsigned long>::infinity() &&
...
template <class Type>
vxl_byte
vgui_range_map<Type>::map_pixel_byte(const Type pix,
                                     const Type min,
                                     const Type max,
                                     const float gamma,
...
template <class Type>
float
vgui_range_map<Type>::map_pixel_float(const Type pix,
                                      const Type min,
                                      const Type max,
                                      const float gamma,
                                      const long double ratio)
...
vgui_range_map<Type>::compute_byte_table(const Type min, const Type max, const float gamma, const long double ratio)
{
  ...
  else
  {
    ...
    for (long i = 0; i <= range; ++i)
      bmap[i] = map_pixel_byte(static_cast<Type>(i + mint), min, max, gamma, ratio);
  }
  return bmap;
}
...
template <class Type>
vbl_array_1d<float>
vgui_range_map<Type>::compute_float_table(const Type min, const Type max, const float gamma, const long double ratio)
{
...
template <class Type>
vgui_range_map<Type>::vgui_range_map(const vgui_range_map_params & rmp)
...
#define VGUI_RANGE_MAP_INSTANTIATE(T) template class vgui_range_map<T>
