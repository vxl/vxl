...
class vgui_range_map_params : public vbl_ref_count
{
 public:
  ...
  vgui_range_map_params(const long double min_L,
    ...
    min_G_(0), max_G_(0),gamma_B_(1.0),
    min_B_(0), max_B_(0),gamma_G_(1.0),
  ...
  vgui_range_map_params(vgui_range_map_params const& p) :  
    min_L_(p.min_L_), max_L_(p.max_L_), gamma_L_(p.gamma_L_),
    invert_(p.invert_), use_glPixelMap_(p.use_glPixelMap_),
    cache_mapped_pix_(p.cache_mapped_pix_), n_components_(p.n_components_), 
