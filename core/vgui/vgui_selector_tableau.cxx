...
std::string
vgui_selector_tableau::file_name() const
{
...
std::string
vgui_selector_tableau::pretty_name() const
{
  std::stringstream s;
  s << type_name() << "[#kids=" << child_map_.size() << ']';
  return s.str();
}
...
bool
vgui_selector_tableau::handle(const vgui_event & event)
{
...
bool
vgui_selector_tableau::get_bounding_box(float lo[3], float hi[3]) const
{
...
//: Remove given tableau from list of child tableaux.
bool
vgui_selector_tableau::remove(const std::string name)
{
  std::map<std::string, vgui_parent_child_link>::iterator itr = child_map_.find(name);
  if (itr != child_map_.end())
  ...
  std::vector<std::string>::iterator o_itr = std::find(render_order_.begin(), render_order_.end(), name);
  if (o_itr != render_order_.end())
...
vgui_tableau_sptr
vgui_selector_tableau::active_tableau() const
{
  std::map<std::string, vgui_parent_child_link>::const_iterator itr = child_map_.find(active_child_);
  if (itr == child_map_.end())
    return nullptr;
...
vgui_tableau_sptr
vgui_selector_tableau::get_tableau(const std::string & name) const
{
...
vgui_selector_tableau::remove_child(const vgui_tableau_sptr & t)
{
  ...
  for (std::map<std::string, vgui_parent_child_link>::iterator itr = child_map_.begin(); itr != child_map_.end(); ++itr)
...
//----------------------------------------------------------------------------
bool
vgui_selector_tableau::toggle(const std::string & name)
{
...
//----------------------------------------------------------------------------
bool
vgui_selector_tableau::is_visible(const std::string & name) const
{
...
void
vgui_selector_tableau::set_active(const std::string & name)
{
  std::map<std::string, vgui_parent_child_link>::iterator itr = child_map_.find(name);
  if (itr != child_map_.end())
    active_child_ = name;
...
void
vgui_selector_tableau::active_raise()
{
...
void
vgui_selector_tableau::active_lower()
{
  std::vector<std::string>::iterator itr;
  itr = std::find(render_order_.begin(), render_order_.end(), active_child_);
...
void
vgui_selector_tableau::active_to_top()
{
  std::vector<std::string>::iterator itr;
  itr = std::find(render_order_.begin(), render_order_.end(), active_child_);
...
void
vgui_selector_tableau::active_to_bottom()
{
  std::vector<std::string>::iterator itr;
  itr = std::find(render_order_.begin(), render_order_.end(), active_child_);
...
{
public:
  vgui_selector_switch_command(vgui_selector_tableau * s, const std::string & n)
    : selector(s)
    , name(n)
  {}
  void
...
{
public:
  vgui_selector_toggle_command(vgui_selector_tableau * s, const std::string & n)
    : selector(s)
    , name(n)
  {}
  void
...
void
vgui_selector_tableau::get_popup(const vgui_popup_params & params, vgui_menu & menu)
{
  vgui_menu submenu;
  ...
  for (; itr != render_order_.rend(); ++itr)
  {
    ...
    if (*itr == active_child_)
      check = "x";
    else
      check = " ";
    active_menu.add(box_head + check + box_tail + (*itr), new vgui_selector_switch_command(this, *itr));

    if (is_visible(*itr))
      check = "x";
    else
      check = " ";
    visible_menu.add(box_head + check + box_tail + (*itr), new vgui_selector_toggle_command(this, *itr));
  }
  ...
  position_menu.add("Move to Bottom",
                    new vgui_selector_position_command(this, vgui_selector_position_command::TO_BOTTOM));
