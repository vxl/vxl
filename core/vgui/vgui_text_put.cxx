...
void
vgui_text_put(const char * str, unsigned size)
{
...
int
vgui_text_length(const char * str, unsigned size)
{
  if (!glut_was_initialized)
  {
    glut_was_initialized = true;
...
int
vgui_char_width(unsigned char c, unsigned size)
{
  if (!glut_was_initialized)
  {
    glut_was_initialized = true;
...
int
vgui_text_length(const unsigned char * s, unsigned size)
{
  int w = 0;
  int i = 0;
  while (s[i] != 0)
