...
double
wxSliderPanel::sp_to_val(unsigned int idx, int sp) const
{
...
int
wxSliderPanel::val_to_sp(unsigned int idx, double val) const
{
  return static_cast<int>((1000.0 * (val - min_vals_[idx]) / (max_vals_[idx] - min_vals_[idx])));
}


//: Create the controls
void
wxSliderPanel::CreateControls()
{
  ...
  for (unsigned int i = 0; i < vals_.size(); ++i)
  {
    ...
    wxTextCtrl * itemTextCtrl = new wxTextCtrl(this,
                                               base_id_ + 2 * i + 1,
...
void
wxSliderPanel::CreateSliders(const std::vector<double> & init_vals,
...
int
wxSliderPanel::GetWidgets(const wxEvent & event, wxSlider *& slider, wxTextCtrl *& text)
{
  slider = NULL;
  text = NULL;
...
//: Update the data
void
wxSliderPanel::update_data(std::vector<double> & data, bool send_messages)
{
