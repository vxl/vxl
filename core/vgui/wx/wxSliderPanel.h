...
public:
  ...
  void
  CreateSliders(const std::vector<double> & init_vals,
                const std::vector<double> & min_vals,
                const std::vector<double> & max_vals);

  //: Event handler
  void
  OnSliderTrack(wxScrollEvent & event);
  //: Event handler
  void
  OnSliderChange(wxScrollEvent & event);
  //: Event handler
  void
  OnChangeText(wxCommandEvent & event);
  //: Event handler
  void
  OnEnterText(wxCommandEvent & event);

  //: Used by event handles to validate and lookup widgets
  int
  GetWidgets(const wxEvent & event, wxSlider *& slider, wxTextCtrl *& text);
  ...
  //: Update the data
  void
  update_data(std::vector<double> & data, bool send_messages = true);
  //: Update the data
  void
  update_data(unsigned int i, double val, bool send_messages = true);
  ...
  double
  sp_to_val(unsigned int idx, int sp) const;
  //: Convert a double value to a slider position
  int
  val_to_sp(unsigned int idx, double val) const;
