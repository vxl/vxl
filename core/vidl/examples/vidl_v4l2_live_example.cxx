...
int
select_device(vidl_v4l2_devices & devs)
{
...
void
change_width_height(vidl_v4l2_device & dev)
{
  int nw, nh;
  ...
  if (!dev.set_v4l2_format(dev.get_v4l2_format(), nw, nh))
  {
    ...
    std::cout << "New format set" << std::endl; // w,h could be changed by driver
...
// return if success
bool
configure_input(vidl_v4l2_device & dev)
{
  int inum = -1;
  if (dev.n_inputs() == 1)
    inum = 0;
  ...
  else
  {
    ...
    if (!dev.format_is_set())
    {
      ...
      if (dev.set_v4l2_format(V4L2_PIX_FMT_YUV420, 640, 480))
      { // for example
...
void
save_frames(vidl_v4l2_device & dev)
{
