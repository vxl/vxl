...
test_color()
{
  ...
  // correct function pointers
  {
    ...
    cf = vidl_color_converter_func(VIDL_PIXEL_COLOR_YUV, typeid(vxl_byte), VIDL_PIXEL_COLOR_RGB, typeid(vxl_byte));
    vidl_color_conv_fptr cf2 = &vidl_color_converter<VIDL_PIXEL_COLOR_YUV, VIDL_PIXEL_COLOR_RGB>::convert;
    same_func = cf == cf2;
    TEST("vidl_color_converter_func: YUV to RGB", same_func, true);

    cf = vidl_color_converter_func(VIDL_PIXEL_COLOR_MONO, typeid(bool), VIDL_PIXEL_COLOR_RGB, typeid(vxl_byte));
    vidl_color_conv_fptr_1_8 cf_1_8 = &vidl_color_converter<VIDL_PIXEL_COLOR_MONO, VIDL_PIXEL_COLOR_RGB>::convert;
    same_func = cf == reinterpret_cast<vidl_color_conv_fptr>(cf_1_8);
    TEST("vidl_color_converter_func: MONO_1 to RGB", same_func, true);

    cf = vidl_color_converter_func(VIDL_PIXEL_COLOR_MONO, typeid(vxl_uint_16), VIDL_PIXEL_COLOR_MONO, typeid(bool));
    vidl_color_conv_fptr_16_1 cf_16_1 = &vidl_color_converter<VIDL_PIXEL_COLOR_MONO, VIDL_PIXEL_COLOR_MONO>::convert;
...

  {
    ...
    TEST("RGB to RGBA",
         in[0] == out[0] && in[1] == out[1] && in[2] == out[2] && out[3] == 255 && out[0] == out2[0] &&
  ...
  {
    vidl_color_conv_fptr cf =
      vidl_color_converter_func(VIDL_PIXEL_COLOR_RGBA, typeid(vxl_byte), VIDL_PIXEL_COLOR_YUV, typeid(vxl_byte));
    assert(cf);
