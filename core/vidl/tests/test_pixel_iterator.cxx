...
test_pixel_iterator()
{
  ...
  for (unsigned int i = 0; i < VIDL_PIXEL_FORMAT_ENUM_END; ++i)
  {
    ...
    {
      all_found = false;
      std::cout << "Warning: no pixel iterator found for format " << vidl_pixel_format(i) << std::endl;
    }
  }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_RGB_24))
  {
    ...
    for (unsigned int i = 0; i < 3; ++i, ++itr, ++(*pitr))
    {
      ...
      success = success && itr(0) == buffer[3 * i] && itr(1) == buffer[3 * i + 1] && itr(2) == buffer[3 * i + 2];
      psuccess = psuccess && data[0] == buffer[3 * i] && data[1] == buffer[3 * i + 1] && data[2] == buffer[3 * i + 2];
    }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_RGB_24P))
  {
    ...
    for (unsigned int i = 0; i < 3; ++i, ++itr, ++(*pitr))
    {
      ...
      std::cout << "RGB = (" << (int)itr(0) << ',' << (int)itr(1) << ',' << (int)itr(2) << ")\t "
                << "RGB polymorphic = (" << (int)data[0] << ',' << (int)data[1] << ',' << (int)data[2] << ")\n";
      success = success && itr(0) == buffer[i] && itr(1) == buffer[i + 3] && itr(2) == buffer[i + 6];
      psuccess = psuccess && data[0] == buffer[i] && data[1] == buffer[i + 3] && data[2] == buffer[i + 6];
    }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_BGR_24))
  {
    ...
    for (unsigned int i = 0; i < 3; ++i, ++itr, ++(*pitr))
    {
      ...
      std::cout << "RGB = (" << (int)itr(0) << ',' << (int)itr(1) << ',' << (int)itr(2) << ")\t "
                << "RGB polymorphic = (" << (int)data[0] << ',' << (int)data[1] << ',' << (int)data[2] << ")\n";
      success = success && itr(0) == buffer[3 * i + 2] && itr(1) == buffer[3 * i + 1] && itr(2) == buffer[3 * i];
      psuccess = psuccess && data[0] == buffer[3 * i + 2] && data[1] == buffer[3 * i + 1] && data[2] == buffer[3 * i];
    }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_RGBA_32))
  {
    ...
    for (unsigned int i = 0; i < 3; ++i, ++itr, ++(*pitr))
    {
      vxl_byte data[4];
      pitr->get_data(data);
      std::cout << "RGBA = (" << (int)itr(0) << ',' << (int)itr(1) << ',' << (int)itr(2) << ',' << (int)itr(3) << ")\t "
                << "RGBA polymorphic = (" << (int)data[0] << ',' << (int)data[1] << ',' << (int)data[2] << ','
                << (int)data[3] << ")\n";
      success = success && itr(0) == buffer[4 * i] && itr(1) == buffer[4 * i + 1] && itr(2) == buffer[4 * i + 2] &&
                itr(3) == buffer[4 * i + 3];
      psuccess = psuccess && data[0] == buffer[4 * i] && data[1] == buffer[4 * i + 1] && data[2] == buffer[4 * i + 2] &&
                 data[3] == buffer[4 * i + 3];
    }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_RGBA_32P))
  {
    ...
    for (unsigned int i = 0; i < 3; ++i, ++itr, ++(*pitr))
    {
      ...
      std::cout << "RGBA = (" << (int)itr(0) << ',' << (int)itr(1) << ',' << (int)itr(2) << ',' << (int)itr(3) << ")\t "
                ...
                << (int)data[3] << ")\n";
      success =
        success && itr(0) == buffer[i] && itr(1) == buffer[i + 3] && itr(2) == buffer[i + 6] && itr(3) == buffer[i + 9];
      psuccess = psuccess && data[0] == buffer[i] && data[1] == buffer[i + 3] && data[2] == buffer[i + 6] &&
                 data[3] == buffer[i + 9];
    }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_RGB_555))
  {
    ...
    for (unsigned int i = 0; i < 2; ++i, ++itr, ++(*pitr))
    {
      ...
      success = success && itr(0) == vxl_byte((buffer[i] >> 10) & 31) << 3 &&
                itr(1) == vxl_byte((buffer[i] >> 5) & 31) << 3 && itr(2) == vxl_byte(buffer[i] & 31) << 3;
      psuccess = psuccess && data[0] == vxl_byte((buffer[i] >> 10) & 31) << 3 &&
                 data[1] == vxl_byte((buffer[i] >> 5) & 31) << 3 && data[2] == vxl_byte(buffer[i] & 31) << 3;
    }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_RGB_565))
  {
    ...
    for (unsigned int i = 0; i < 2; ++i, ++itr, ++(*pitr))
    {
      ...
      success = success && itr(0) == vxl_byte((buffer[i] >> 11) & 31) << 3 &&
                itr(1) == vxl_byte((buffer[i] >> 5) & 63) << 2 && itr(2) == vxl_byte(buffer[i] & 31) << 3;
      psuccess = psuccess && data[0] == vxl_byte((buffer[i] >> 11) & 31) << 3 &&
                 data[1] == vxl_byte((buffer[i] >> 5) & 63) << 2 && data[2] == vxl_byte(buffer[i] & 31) << 3;
    }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_YUV_444P))
  {
    ...
    for (unsigned int i = 0; i < 4; ++i, ++itr, ++(*pitr))
    {
      ...
      success = success && itr(0) == buffer[i] && itr(1) == buffer[i + 4] && itr(2) == buffer[i + 8];
      psuccess = psuccess && data[0] == buffer[i] && data[1] == buffer[i + 4] && data[2] == buffer[i + 8];
    }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_UYV_444))
  {
    ...
    for (unsigned int i = 0; i < 4; ++i, ++itr, ++(*pitr))
    {
      ...
      std::cout << "YUV = (" << (int)itr(0) << ',' << (int)itr(1) << ',' << (int)itr(2) << ")\t "
                << "YUV polymorphic = (" << (int)data[0] << ',' << (int)data[1] << ',' << (int)data[2] << ")\n";
      success = success && itr(0) == buffer[i * 3 + 1] && itr(1) == buffer[i * 3] && itr(2) == buffer[i * 3 + 2];
      psuccess = psuccess && data[0] == buffer[i * 3 + 1] && data[1] == buffer[i * 3] && data[2] == buffer[i * 3 + 2];
    }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_YUV_422P))
  {
    ...
    for (unsigned int i = 0; i < 16; ++i, ++itr, ++(*pitr))
    {
      ...
      std::cout << "YUV = (" << (int)itr(0) << ',' << (int)itr(1) << ',' << (int)itr(2) << ")\t "
                << "YUV polymorphic = (" << (int)data[0] << ',' << (int)data[1] << ',' << (int)data[2] << ")\n";
      success = success && itr(0) == buffer[i] && itr(1) == buffer[16 + i / 2] && itr(2) == buffer[24 + i / 2];
      psuccess = psuccess && data[0] == buffer[i] && data[1] == buffer[16 + i / 2] && data[2] == buffer[24 + i / 2];
    }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_UYVY_422))
  {
    ...
    for (unsigned int i = 0; i < 4; ++i, ++itr, ++(*pitr))
    {
      ...
      std::cout << "YUV = (" << (int)itr(0) << ',' << (int)itr(1) << ',' << (int)itr(2) << ")\t "
                << "YUV polymorphic = (" << (int)data[0] << ',' << (int)data[1] << ',' << (int)data[2] << ")\n";
      success = success && itr(0) == buffer[2 * i + 1] && itr(1) == buffer[i * 2 - (i % 2) * 2] &&
                itr(2) == buffer[i * 2 + 2 - (i % 2) * 2];
      psuccess = psuccess && data[0] == buffer[2 * i + 1] && data[1] == buffer[i * 2 - (i % 2) * 2] &&
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_YUYV_422))
  {
    ...
    for (unsigned int i = 0; i < 4; ++i, ++itr, ++(*pitr))
    {
      ...
      std::cout << "YUV = (" << (int)itr(0) << ',' << (int)itr(1) << ',' << (int)itr(2) << ")\t "
                << "YUV polymorphic = (" << (int)data[0] << ',' << (int)data[1] << ',' << (int)data[2] << ")\n";
      success = success && itr(0) == buffer[2 * i] && itr(1) == buffer[i * 2 + 1 - (i % 2) * 2] &&
                itr(2) == buffer[i * 2 + 3 - (i % 2) * 2];
      psuccess = psuccess && data[0] == buffer[2 * i] && data[1] == buffer[i * 2 + 1 - (i % 2) * 2] &&
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_YUV_411P))
  {
    ...
    for (unsigned int i = 0; i < 8; ++i, ++itr, ++(*pitr))
    {
      ...
      std::cout << "YUV = (" << (int)itr(0) << ',' << (int)itr(1) << ',' << (int)itr(2) << ")\t "
                << "YUV polymorphic = (" << (int)data[0] << ',' << (int)data[1] << ',' << (int)data[2] << ")\n";
      success = success && itr(0) == buffer[i] && itr(1) == buffer[(i / 4) + 8] && itr(2) == buffer[(i / 4) + 10];
      psuccess = psuccess && data[0] == buffer[i] && data[1] == buffer[(i / 4) + 8] && data[2] == buffer[(i / 4) + 10];
    }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_UYVY_411))
  {
    ...
    for (unsigned int i = 0; i < 8; ++i, ++itr, ++(*pitr))
    {
      ...
      success = success && itr(0) == buffer[6 * (i / 4) + (i & 3) + 1 + ((i >> 1) & 1)] &&
                itr(1) == buffer[6 * (i / 4)] && itr(2) == buffer[6 * (i / 4) + 3];
      psuccess = psuccess && data[0] == buffer[6 * (i / 4) + (i & 3) + 1 + ((i >> 1) & 1)] &&
                 data[1] == buffer[6 * (i / 4)] && data[2] == buffer[6 * (i / 4) + 3];
    }
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_YUV_420P))
  {
    ...
    for (unsigned int i = 0; i < 16; ++i, ++itr, ++(*pitr))
    {
      ...
      if (i % 4 == 3)
        std::cout << '\n';
      success = success && itr(0) == buffer[i] && itr(1) == buffer[16 + (i / 2) % 2 + ((i / 8) << 1)] &&
                itr(2) == buffer[20 + (i / 2) % 2 + ((i / 8) << 1)];
      psuccess = psuccess && data[0] == buffer[i] && data[1] == buffer[16 + (i / 2) % 2 + ((i / 8) << 1)] &&
  ...
  if (vidl_has_pixel_iterator(VIDL_PIXEL_FORMAT_YUV_410P))
  {
    ...
    for (unsigned int i = 0; i < 16; ++i, ++itr, ++(*pitr))
    {
      ...
      if (i % 4 == 3)
        std::cout << '\n';
      success = success && itr(0) == buffer[i] && itr(1) == buffer[16 + (i / 8)] && itr(2) == buffer[18 + (i / 8)];
      psuccess = psuccess && data[0] == buffer[i] && data[1] == buffer[16 + (i / 8)] && data[2] == buffer[18 + (i / 8)];
    }
