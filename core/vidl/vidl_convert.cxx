...
convert_generic(const vidl_frame & in_frame, vidl_frame & out_frame)
{
  ...
  vidl_color_conv_fptr color_conv = vidl_color_converter_func(in_t.color, *in_t.type, out_t.color, *out_t.type);
  if (!color_conv)
    return false;
...
{
  static inline void
  set_entry(converter_func & table_entry)
  {
    ...
    else if (vidl_pixel_iterator_valid<in_Fmt>::value && vidl_pixel_iterator_valid<out_Fmt>::value)
...
{
  static inline void
  populate(converter_func table[VIDL_PIXEL_FORMAT_ENUM_END][VIDL_PIXEL_FORMAT_ENUM_END])
  {
...
vidl_convert_frame(const vidl_frame & in_frame, vidl_frame & out_frame)
{
  ...
  if (in_fmt == VIDL_PIXEL_FORMAT_UNKNOWN || out_fmt == VIDL_PIXEL_FORMAT_UNKNOWN)
    return false;
  ...
  if (out_frame.size() != out_size || out_frame.ni() != ni || out_frame.nj() != nj || !out_frame.data())
    return false;
...
// a new frame buffer
vidl_frame_sptr
vidl_convert_frame(const vidl_frame_sptr & in_frame, vidl_pixel_format format)
{
...
vidl_frame_sptr
vidl_convert_to_frame(const vil_image_view_base_sptr & image)
{
  if (!image)
    return nullptr;
...
// the closest vidl_pixel_format
vidl_frame_sptr
vidl_convert_to_frame(const vil_image_view_base & image)
{
  ...
  if (cmp_format == VIL_PIXEL_FORMAT_UINT_16)
  {
    if (num_channels == 1)
    {
      ...
      return new vidl_memory_chunk_frame(ni, nj, VIDL_PIXEL_FORMAT_MONO_16, img.memory_chunk());
    }
  }
  ...
  else if (cmp_format == VIL_PIXEL_FORMAT_FLOAT)
  {
    if (num_channels == 1 || num_channels == 3)
    {
      ...
      return new vidl_memory_chunk_frame(ni, nj, format, img.memory_chunk());
    }
  }
...
vidl_convert_to_view(const vidl_frame & frame, vil_image_view_base & image, vidl_pixel_color require_color)
{
  ...
  {
    vil_image_view<vxl_uint_16> wrapper(static_cast<const vxl_uint_16 *>(frame.data()), ni, nj, 1, 1, ni, ni * nj);
...
#  undef macro
  ...
  if (image.pixel_format() == VIL_PIXEL_FORMAT_BYTE)
  {
    ...
    switch (require_color)
    {
      case VIDL_PIXEL_COLOR_MONO:
        default_format = VIDL_PIXEL_FORMAT_MONO_8;
        break;
      case VIDL_PIXEL_COLOR_RGB:
        default_format = interleaved ? VIDL_PIXEL_FORMAT_RGB_24 : VIDL_PIXEL_FORMAT_RGB_24P;
        break;
      case VIDL_PIXEL_COLOR_RGBA:
        default_format = interleaved ? VIDL_PIXEL_FORMAT_RGBA_32 : VIDL_PIXEL_FORMAT_RGBA_32P;
        break;
      case VIDL_PIXEL_COLOR_YUV:
        default_format = interleaved ? VIDL_PIXEL_FORMAT_UYV_444 : VIDL_PIXEL_FORMAT_YUV_444P;
        break;
      default:
        break;
    }
  ...
  {
    case VIDL_PIXEL_COLOR_MONO:
      out_fmt = VIDL_PIXEL_FORMAT_MONO_8;
      break;
    case VIDL_PIXEL_COLOR_RGB:
      out_fmt = VIDL_PIXEL_FORMAT_RGB_24P;
      break;
    case VIDL_PIXEL_COLOR_RGBA:
      out_fmt = VIDL_PIXEL_FORMAT_RGBA_32P;
      break;
    case VIDL_PIXEL_COLOR_YUV:
      out_fmt = VIDL_PIXEL_FORMAT_YUV_444P;
      break;
    default:
      return false;
  }
...
vidl_convert_wrap_in_view(const vidl_frame & frame)
{
  ...
  if (const auto * cf = dynamic_cast<const vidl_memory_chunk_frame *>(&frame))
  {
    ...
    else if (format == VIDL_PIXEL_FORMAT_MONO_F32 || format == VIDL_PIXEL_FORMAT_RGB_F32 ||
             format == VIDL_PIXEL_FORMAT_RGB_F32P)
    {
