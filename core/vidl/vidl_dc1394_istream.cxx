...
dc1394_feature_to_vidl(const dc1394feature_info_t & f_old)
{
  ...
  f.active_mode = static_cast<vidl_iidc1394_params::feature_mode_t>(f_old.current_mode - DC1394_FEATURE_MODE_MIN);

  for (unsigned int i = 0; i < f_old.modes.num; ++i)
  {
    f.available_modes.push_back(
      static_cast<vidl_iidc1394_params::feature_mode_t>(f_old.modes.modes[i] - DC1394_FEATURE_MODE_MIN));
  }
...
vidl_feature_to_dc1394(const vidl_iidc1394_params::feature_options & f_old)
{
  ...
  f.current_mode = static_cast<dc1394feature_mode_t>(f_old.active_mode + DC1394_FEATURE_MODE_MIN);

  for (unsigned int i = 0; i < f_old.available_modes.size(); ++i)
  {
    f.modes.modes[i] = static_cast<dc1394feature_mode_t>(f_old.available_modes[i] + DC1394_FEATURE_MODE_MIN);
  }
...
bool
vidl_dc1394_istream::open(unsigned int num_dma_buffers, bool drop_frames, const vidl_iidc1394_params & params)
{
  ...
  for (unsigned int i = 0; i < params.features_.size(); ++i)
  {
    ...
    if (dc1394_feature_get_mode(is_->camera_info_, f.id, &old_mode) == DC1394_SUCCESS && old_mode != f.current_mode)
    {
      if (dc1394_feature_set_mode(is_->camera_info_, f.id, f.current_mode) != DC1394_SUCCESS)
      {
        std::cerr << "Failed to set mode of feature \"" << vidl_iidc1394_params::feature_string(params.features_[i].id)
    ...
    switch (params.features_[i].id)
    {
      ...
      default:
        if (f.abs_control)
        {
          ...
          {
            std::cerr << "Failed to set feature \"" << vidl_iidc1394_params::feature_string(params.features_[i].id)
  ...
  if (dc1394_capture_setup(is_->camera_info_, num_dma_buffers, DC1394_CAPTURE_FLAGS_DEFAULT) != DC1394_SUCCESS)
  {
...
//: Close the stream
void
vidl_dc1394_istream::close()
{
  if (is_->camera_info_)
  {
    // turn off the camera power
    dc1394switch_t pwr;
    if (dc1394_video_get_transmission(is_->camera_info_, &pwr) == DC1394_SUCCESS && pwr == DC1394_ON)
    {
      dc1394_video_set_transmission(is_->camera_info_, DC1394_OFF);
    }
...
vidl_dc1394_istream::valid_params(vidl_iidc1394_params::valid_options & options)
{
  ...
  if (err)
  {
    std::cerr << "error finding cameras: " << dc1394_error_get_string(err) << std::endl;
...
bool
vidl_dc1394_istream::is_open() const
{
...
bool
vidl_dc1394_istream::is_valid() const
{
...
bool
vidl_dc1394_istream::is_seekable() const
{
  return false;
}
...
int
vidl_dc1394_istream::num_frames() const
{
  return -1;
}
...
unsigned int
vidl_dc1394_istream::frame_number() const
{
...
unsigned int
vidl_dc1394_istream::width() const
{
  return is_->width_;
}
...
unsigned int
vidl_dc1394_istream::height() const
{
  return is_->height_;
}
...
vidl_pixel_format
vidl_dc1394_istream::format() const
{
...
double
vidl_dc1394_istream::frame_rate() const
{
...
double
vidl_dc1394_istream::duration() const
{
  return 0.0;
}
...
vidl_dc1394_istream::current_frame()
{
  ...
  if (!is_->cur_frame_valid_)
  {
    ...
    is_->cur_frame_ = new vidl_shared_frame(
      is_->dc1394frame_->image, is_->dc1394frame_->size[0], is_->dc1394frame_->size[1], is_->pixel_format_);
...
bool
vidl_dc1394_istream::seek_frame(unsigned int frame)
{
  return false;
}
