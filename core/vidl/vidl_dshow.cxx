...
public:
  ...
  void
  print_capture_device_names() const;
  std::vector<std::string>
  get_capture_device_names() const;
  CComPtr<IMoniker>
  get_capture_device_moniker(const std::string & name) const;
...
void
com_manager::find_capture_devices(void)
{
  capture_devices_.clear();

  CComPtr<ICreateDevEnum> sys_device_enum;
  DSHOW_ERROR_IF_FAILED(sys_device_enum.CoCreateInstance(CLSID_SystemDeviceEnum));
  ...
  while (cap_device_enum->Next(1, &moniker, 0) == S_OK)
  {
    ...
    hr = moniker->BindToStorage(0, 0, IID_IPropertyBag, reinterpret_cast<void **>(&property_bag));
    if (FAILED(hr))
    {
      std::cerr << "BindToStorage failed for device " << capture_devices_.size() + 1 << '\n'
                << DSHOW_GET_ERROR_TEXT(hr) << std::endl;
      continue;
    }
    else
    {
      ...
      if (SUCCEEDED(property_bag->Read(L"FriendlyName", &var, 0)))
      {
        ...
        capture_devices_.insert(std::pair<std::string, CComPtr<IMoniker>>(std::string(CW2A(var.bstrVal)), moniker));
      }
      else
      {
        ...
        capture_devices_.insert(std::pair<std::string, CComPtr<IMoniker>>("N/A " + oss.str(), moniker));
      }
    }
...
void
com_manager::print_capture_device_names(void) const
{
...
std::vector<std::string>
com_manager::get_capture_device_names(void) const
{
  std::vector<std::string> names;

  std::map<std::string, CComPtr<IMoniker>>::const_iterator iterator = capture_devices_.begin();
...
{
  std::map<std::string, CComPtr<IMoniker>>::const_iterator iterator = capture_devices_.find(name);
...
unsigned int guid_name_list::count = sizeof(guid_name_list::names) / sizeof(guid_name_list::names[0]);

std::string
guid_name_list::get_name(const GUID & guid)
{
  if (guid == GUID_NULL)
  {
    return "GUID_NULL";
...
void
vidl_dshow::initialize_com(void)
{
...
vidl_dshow::get_error_text(const char * file, int line, HRESULT hr)
{
  ...
  std::ostringstream oss;
  oss << file << ':' << line << ':';
  result ? oss << CT2A(err) : oss << "Unknown Error (" << std::hex << hr << ')';
...
void
vidl_dshow::register_in_rot(IUnknown * unknown, DWORD & reg)
{
  assert(unknown != 0);
  ...
  oss << L"FilterGraph " << std::hex << reinterpret_cast<DWORD>(unknown) << L" pid " << std::hex
      << GetCurrentProcessId();
  // std::wcout << oss.str() << std::endl;

  CComPtr<IMoniker> moniker;
  DSHOW_ERROR_IF_FAILED(CreateItemMoniker(L"!", oss.str().c_str(), &moniker));
  DSHOW_ERROR_IF_FAILED(rot->Register(ROTFLAGS_REGISTRATIONKEEPSALIVE, unknown, moniker, &reg));
}

//: Remove a filter graph from ROT.
void
vidl_dshow::remove_from_rot(DWORD reg)
{
  CComPtr<IRunningObjectTable> rot;
  DSHOW_ERROR_IF_FAILED(GetRunningObjectTable(0, &rot));
...
void
vidl_dshow::save_graph_to_file(const CComPtr<IFilterGraph2> & filter_graph, const std::string & filename)
{
  assert(filter_graph != 0);

  CComPtr<IStorage> storage;
  DSHOW_ERROR_IF_FAILED(StgCreateDocfile(
    CA2W(filename.c_str()), STGM_CREATE | STGM_TRANSACTED | STGM_READWRITE | STGM_SHARE_EXCLUSIVE, 0, &storage));

  CComPtr<IStream> stream;
  // const std::wstring stream_name = L"ActiveMovieGraph";
  DSHOW_ERROR_IF_FAILED(
    storage->CreateStream(L"ActiveMovieGraph", STGM_CREATE | STGM_WRITE | STGM_SHARE_EXCLUSIVE, 0, 0, &stream));

  CComPtr<IPersistStream> persist_stream;
  filter_graph->QueryInterface(IID_IPersistStream, reinterpret_cast<void **>(&persist_stream));
...
void
vidl_dshow::load_graph_from_file(const CComPtr<IFilterGraph2> & filter_graph, const std::wstring & filename)
{
  assert(filter_graph != 0);
  ...
  { // might return S_FALSE; can't use FAILED(hr)
    vidl_exception_error(vidl_dshow_exception("Not a storage file.\n" + DSHOW_GET_ERROR_TEXT(E_FAIL)));
  }
  DSHOW_ERROR_IF_FAILED(
    StgOpenStorage(filename.c_str(), 0, STGM_TRANSACTED | STGM_READ | STGM_SHARE_DENY_WRITE, 0, 0, &storage));
  ...
  CComPtr<IStream> stream;
  DSHOW_ERROR_IF_FAILED(storage->OpenStream(L"ActiveMovieGraph", 0, STGM_READ | STGM_SHARE_EXCLUSIVE, 0, &stream));
...
std::string
vidl_dshow::get_guid_name(const GUID & guid)
{
...
GUID
vidl_dshow::get_guid_from_fourcc(const std::string & fourcc)
{
  unsigned long fourcc_cast = static_cast<unsigned long>(fourcc[0]) | static_cast<unsigned long>(fourcc[1]) << 8 |
                              static_cast<unsigned long>(fourcc[2]) << 16 | static_cast<unsigned long>(fourcc[3]) << 24;

  const GUID guid = { fourcc_cast, 0x0000, 0x0010, { 0x80, 0x00, 0x00, 0xAA, 0x00, 0x38, 0x9B, 0x71 } };
                           ...
                           vidl_pixel_format & pixel_format)
{
  ...
  else if (amt.subtype == MEDIASUBTYPE_IYUV || amt.subtype == vidl_dshow::get_guid_from_fourcc("I420"))
  {
...
void
vidl_dshow::delete_media_type(AM_MEDIA_TYPE & amt)
{
  if (amt.cbFormat != 0)
  {
...
void
vidl_dshow::print_capture_device_names(void)
{
...
std::vector<std::string>
vidl_dshow::get_capture_device_names(void)
{
  com_manager & com = com_manager::instance();
                            ...
                            CComPtr<IBaseFilter> & target)
{
  ...
  while (S_OK == source_pins_enum->Next(1, &source_pin, 0))
  {
    ...
    if (FAILED(source_pin->QueryDirection(&pin_dir)) || pin_dir == PINDIR_INPUT)
    {
      source_pin.Release();
      continue;
    }
    ...
    {
      if (FAILED(target_pin->QueryDirection(&pin_dir)) || pin_dir == PINDIR_OUTPUT)
      {
        target_pin.Release();
        continue;
      }
  ...
  vidl_exception_error(vidl_dshow_exception("Couldn't connect the filters." + DSHOW_GET_ERROR_TEXT(E_FAIL)));
}
