...
struct vidl_dshow
  ...
  static void get_media_info(const AM_MEDIA_TYPE& amt, unsigned int& width, unsigned int& height,
  ...
  static void build_filter_graph(/*CComPtr<ICaptureGraphBuilder2>& graph_builder,*/
    CComPtr<IFilterGraph2>& filter_graph, CComPtr<IMoniker>& moniker, CComPtr<ISampleGrabber>& sample_grabber);

#endif

  static void connect_filters(CComPtr<IFilterGraph2>& filter_graph, CComPtr<IBaseFilter>& source,
