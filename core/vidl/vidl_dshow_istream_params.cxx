...
std::map<std::string, vpa_property_wrap>
vpa_properties(void)
{
...
// void print_output_format_help(IAMStreamConfig* asc)
{
  ...
  std::cout << std::setw(w1) << "output_format" << ' ' << vidl_dshow::get_guid_name(amt->subtype) << '\n';
  ...
  {
    DSHOW_ERROR_IF_FAILED(asc->GetStreamCaps(i, &amt, reinterpret_cast<BYTE *>(vscc)));
    ...
    std::cout << std::setw(w1) << i << std::setw(3 * w2) << vidl_dshow::get_guid_name(amt->subtype) << " ("
              << std::setw(4) << vscc->InputSize.cx << 'x' << std::setw(4) << vscc->InputSize.cy << ") "
              << std::setprecision(2) << 1.0 / (max_frame_interval * 100.0e-9) << '-' << std::setprecision(2)
              << 1.0 / (min_frame_interval * 100.0e-9) << " fps\n";
...
void
vidl_dshow_istream_params::configure_filter(const CComPtr<IBaseFilter> & source)
{
  // IAMVideoProcAmp interface.
  CComPtr<IAMVideoProcAmp> am_video_proc_amp;
  if (SUCCEEDED(source->QueryInterface(IID_IAMVideoProcAmp, reinterpret_cast<void **>(&am_video_proc_amp))))
  {
    std::map<std::string, vpa_property_wrap>::const_iterator iter = vpa_properties_.begin();
    for (; iter != vpa_properties_.end(); iter++)
    {
      if (iter->second.is_changed)
      {
        DSHOW_ERROR_IF_FAILED(am_video_proc_amp->Set(iter->second.key, iter->second.value, VideoProcAmp_Flags_Manual));
      }
      else if (load_filter_defaults_)
      {
        long min, max, step, def, flag;
        if (SUCCEEDED(am_video_proc_amp->GetRange(iter->second.key, &min, &max, &step, &def, &flag)))
        {
          DSHOW_ERROR_IF_FAILED(am_video_proc_amp->Set(iter->second.key, def, flag));
        }
      }
    }
  }
  ...
  if (output_format_.is_changed || load_filter_defaults_)
  {
    ...
    DSHOW_ERROR_IF_FAILED(graph_builder->FindInterface(&PIN_CATEGORY_CAPTURE,
                                                       &MEDIATYPE_Video,
                                                       source,
                                                       IID_IAMStreamConfig,
    ...
    int count = 0, size = 0;
    DSHOW_ERROR_IF_FAILED(am_stream_config->GetNumberOfCapabilities(&count, &size));
    ...
    DSHOW_ERROR_IF_FAILED(am_stream_config->GetStreamCaps(
      output_format_.is_changed ? output_format_.value : 0, &amt, reinterpret_cast<BYTE *>(vscc)));
...
void
vidl_dshow_istream_params::print_parameter_help(const std::string & name)
{
...
void
vidl_dshow_istream_params::print_parameter_help(const CComPtr<IBaseFilter> & filter)
{
  ...
  // IAMVideoProcAmp interface.
  CComPtr<IAMVideoProcAmp> am_video_proc_amp;
  if (SUCCEEDED(filter->QueryInterface(IID_IAMVideoProcAmp, reinterpret_cast<void **>(&am_video_proc_amp))))
  {
    std::cout << std::string(w1, ' ') << std::setw(w2) << "curr" << std::setw(w2) << "min" << std::setw(w2) << "max"
              << std::setw(w2) << "step" << std::setw(w2) << "default" << std::setw(w2) << "flags" << '\n'
              << std::string(w1, ' ') << std::string(6 * w2, '-') << '\n';
  ...
  if (SUCCEEDED(graph_builder->FindInterface(&PIN_CATEGORY_CAPTURE,
                                             &MEDIATYPE_Video,
                                             filter,
                                             IID_IAMStreamConfig,
                                             reinterpret_cast<void **>(&am_stream_config))))
  ...
  if (SUCCEEDED(graph_builder->FindInterface(&PIN_CATEGORY_PREVIEW,
                                             &MEDIATYPE_Video,
                                             filter,
                                             IID_IAMStreamConfig,
                                             reinterpret_cast<void **>(&am_stream_config))))
...
vidl_dshow_istream_params ::set_properties(const std::map<std::string, std::string> & props)
{
  std::map<std::string, std::string>::const_iterator iter;
  for (iter = props.begin(); iter != props.end(); iter++)
  {
    std::map<std::string, vpa_property_wrap>::iterator property = vpa_properties_.find(iter->first);
    ...
    else
    {
      ...
      {
        vidl_exception_error(vidl_exception("Target output format not supported yet!"));
      ...
      {
        std::cerr << "DSHOW: vidl_dshow_istream_params param not valid: " << iter->first << std::endl;
      }
    }
  }
