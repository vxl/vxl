...
vidl_ffmpeg_convert(const vidl_frame & in_frame, vidl_frame & out_frame)
{
  ...
  AVPixelFormat out_fmt = vidl_pixel_format_to_ffmpeg(out_frame.pixel_format());
  ...
  if (img_convert(&out_pic, out_fmt, &in_pic, in_fmt, ni, nj) < 0)
    return false;
#else
  SwsContext * ctx = sws_getContext(ni, nj, in_fmt, ni, nj, out_fmt, SWS_BILINEAR, nullptr, nullptr, nullptr);
  sws_scale(ctx, in_pic.data, in_pic.linesize, 0, nj, out_pic.data, out_pic.linesize);
  sws_freeContext(ctx);
#endif
...
vidl_ffmpeg_convert(const vidl_frame_sptr & in_frame, vidl_frame_sptr & out_frame)
{
  ...
  if (out_frame->size() != out_size || out_frame->ni() != ni || out_frame->nj() != nj || !out_frame->data())
    out_frame = new vidl_memory_chunk_frame(
      ni, nj, out_frame->pixel_format(), new vil_memory_chunk(out_size, VIL_PIXEL_FORMAT_BYTE));
