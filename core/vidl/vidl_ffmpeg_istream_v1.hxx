...
#if LIBAVFORMAT_BUILD <= 4616
    av_seek_frame(mutable_this->is_->fmt_cxt_, mutable_this->is_->vid_index_, 0);
#else
...
vidl_ffmpeg_istream::frame_number() const
{
  ...
  return ((is_->last_dts - is_->vid_str_->start_time)
          ...
          AV_TIME_BASE / 2) /
           AV_TIME_BASE
#else
            * is_->vid_str_->r_frame_rate.num / is_->vid_str_->r_frame_rate.den * is_->vid_str_->time_base.num +
...
#if LIBAVFORMAT_BUILD <= 4623
  return static_cast<double>(is_->vid_str_->r_frame_rate) / is_->vid_str_->r_frame_rate_base / AV_TIME_BASE;
#else
...
#if LIBAVFORMAT_BUILD <= 4623
  return static_cast<double>(is_->vid_str_->duration) / AV_TIME_BASE;
#else
...
vidl_ffmpeg_istream::advance()
{
  ...
  while (got_picture == 0)
  {
    ...
    if (pkt.stream_index == is_->vid_index_)
    {
      ...
      {
        avcodec_decode_video(codec, is_->frame_, &got_picture, pkt.data, pkt.size);
      }
    }
  ...
  if (!got_picture)
  {
    avcodec_decode_video(codec, is_->frame_, &got_picture, NULL, 0);
#if LIBAVFORMAT_BUILD <= 4623
...
vidl_ffmpeg_istream::current_frame()
{
  ...
  // dts.
  if (enc->codec_id == CODEC_ID_MPEG2VIDEO && std::string("avi") == is_->fmt_cxt_->iformat->name)
  {
...
#if LIBAVFORMAT_BUILD <= 4623
  int64_t frame_size = int64_t(AV_TIME_BASE) * is_->vid_str_->r_frame_rate_base / is_->vid_str_->r_frame_rate;
  int64_t req_timestamp =
    int64_t(AV_TIME_BASE) * frame * is_->vid_str_->r_frame_rate_base / is_->vid_str_->r_frame_rate +
    is_->vid_str_->start_time;
#else
