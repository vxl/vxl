...
struct vidl_ffmpeg_istream::pimpl
{
  ...
  //  a frame number
  double
  stream_time_base_to_frame()
  {
    assert(this->vid_str_);
    if (this->vid_str_->avg_frame_rate.num == 0.0)
    {
      return av_q2d(av_inv_q(av_mul_q(this->vid_str_->time_base, this->vid_str_->r_frame_rate)));
    }
    return av_q2d(av_inv_q(av_mul_q(this->vid_str_->time_base, this->vid_str_->avg_frame_rate)));
  }
};
...
vidl_ffmpeg_istream::open(const std::string & filename)
{
  ...
  if (is_->vid_str_->codec->codec_id == AV_CODEC_ID_MPEG2VIDEO && std::string("avi") == is_->fmt_cxt_->iformat->name)
  {
...
vidl_ffmpeg_istream::frame_number() const
{
  ...
  unsigned int frame_number = static_cast<unsigned int>(
    (is_->pts_ - is_->start_time_) / is_->stream_time_base_to_frame() - static_cast<int>(is_->frame_number_offset_));
  return frame_number;
}
...
vidl_ffmpeg_istream ::duration() const
{
  ...
  return av_q2d(is_->vid_str_->time_base) * static_cast<double>(is_->vid_str_->duration);
}
...
vidl_ffmpeg_istream::advance()
{
  ...
  while (got_picture == 0 && av_read_frame(is_->fmt_cxt_, &is_->packet_) >= 0)
  {
    ...
    {
      int err = avcodec_decode_video2(is_->video_enc_, is_->frame_, &got_picture, &is_->packet_);
  ...
  if (!got_picture)
  {
    ...
    if (avcodec_decode_video2(is_->video_enc_, is_->frame_, &got_picture, &is_->packet_) >= 0)
    {
...
vidl_ffmpeg_istream::current_frame()
{
  ...
  if (!is_->cur_frame_ && is_->frame_->data[0] != nullptr)
  {
    ...
    if (fmt == VIDL_PIXEL_FORMAT_UNKNOWN)
    {
      ...
      is_->sws_context_ = sws_getCachedContext(is_->sws_context_,
                                               width,
                                               height,
                                               enc->pix_fmt,
                                               width,
...
vidl_ffmpeg_istream::seek_frame(unsigned int frame)
{
  ...
  int64_t req_timestamp =
    static_cast<int64_t>((frame + is_->frame_number_offset_) / is_->stream_time_base_to_frame()) + is_->start_time_;
