...
public:
  ...
  vidl_image_list_ostream(const vcl_string& directory, const vcl_string& name_format = "%05d",
                          const vcl_string& file_format = "tiff", const unsigned int init_index = 0);

  // : Destructor
  virtual ~vidl_image_list_ostream() { close(); }

  // : Open the stream
  virtual bool open(const vcl_string& directory, const vcl_string& name_format = "%05d",
                    const vcl_string& file_format = "tiff", const unsigned int init_index = 0);
