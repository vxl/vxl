...
vidl_v4l2_control *
vidl_v4l2_control::new_control(const v4l2_queryctrl & ctr, int f)
{
...
void
vidl_v4l2_control::set_value(int v) const
{
...
int
vidl_v4l2_control::get_value() const
{
  struct v4l2_control control;
  std::memset(&control, 0, sizeof(control));
  control.id = ctrl_.id;
...
void
vidl_v4l2_control_integer::set(int value) const
{
  if (value < ctrl_.minimum)
    value = ctrl_.minimum;
  else if (value > ctrl_.maximum)
    value = ctrl_.maximum;
  else
    value = ctrl_.minimum + (value - ctrl_.minimum) / ctrl_.step * ctrl_.step;
  set_value(value);
}

void
vidl_v4l2_control_integer::set_100(int value) const
{
  if (value <= 0)
    value = ctrl_.minimum;
  else if (value >= 100)
    ...
    value = ctrl_.minimum + (ctrl_.maximum - ctrl_.minimum) * value / 100;
  set_value(value);
}

std::string
vidl_v4l2_control_integer::description() const
{
  char cad[256];
  std::snprintf(cad,
                256,
                "Control \"%s\": integer (min: %d, max: %d, step: %d, default: %d)",
                (const char *)ctrl_.name,
                minimum(),
                maximum(),
                step(),
                default_value());
...
std::string
vidl_v4l2_control_menu::description() const
{
  char cad[256];
  std::snprintf(
    cad, 256, "Control \"%s\": menu (%d items, default: %d)", (const char *)ctrl_.name, n_items(), default_value());
  return cad;
}
