...
public:
  ...
  static vidl_v4l2_control *
  new_control(const v4l2_queryctrl & ctr, int f);
  //: Type of control
  // \return type as indicated in v4l2 specification
  v4l2_ctrl_type
  type() const
  {
    return (v4l2_ctrl_type)ctrl_.type;
  }
  //: Name of control
...
public:
  ...
  minimum() const
  {
    return ctrl_.minimum;
  }
  //: Maximum value of the control
  int
  maximum() const
  {
    return ctrl_.maximum;
  }
  //: Step size
  ...
  get_100() const
  {
    return (get_value() - ctrl_.minimum) * 100 / (ctrl_.maximum - ctrl_.minimum);
  }
  //: A 1-line brief description
  virtual std::string
  description() const;
