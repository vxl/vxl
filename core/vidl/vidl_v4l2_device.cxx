...
// To set frame rate
void
double2fraction(double value, int & n, int & d)
{
  if (value < 0.0)
    value = -value;
...
void
vidl_v4l2_device::update_controls()
{
...
void
vidl_v4l2_device::reset_controls()
{
  if (is_open())
  {
    if (n_controls() == 0)
...
vidl_v4l2_device::reset()
{
  ...
  update_controls(); // perhaps, we need to iterate over all inputs and reset controls, but probably, user is interested
                     // in current input
  for (int i = 0; i < n_controls(); ++i)
    get_control(i)->reset();
  // Set default values?
...
bool
vidl_v4l2_device::initialize_device()
{
...
bool
vidl_v4l2_device::try_formats(int width, int height)
{
  // change order
  // better select formats implemented in vidl

  if (set_v4l2_format(V4L2_PIX_FMT_BGR24, width, height))
    return true;
  if (set_v4l2_format(V4L2_PIX_FMT_BGR32, width, height))
    return true;
  if (set_v4l2_format(V4L2_PIX_FMT_RGB565, width, height))
    return true;
  if (set_v4l2_format(V4L2_PIX_FMT_RGB555, width, height))
    return true;

  if (set_v4l2_format(V4L2_PIX_FMT_YUYV, width, height))
    return true;
  if (set_v4l2_format(V4L2_PIX_FMT_UYVY, width, height))
    return true;

  if (set_v4l2_format(V4L2_PIX_FMT_YUV422P, width, height))
    return true;
  if (set_v4l2_format(V4L2_PIX_FMT_YVU420, width, height))
    return true;
  if (set_v4l2_format(V4L2_PIX_FMT_YUV420, width, height))
    return true;
  if (set_v4l2_format(V4L2_PIX_FMT_YUV411P, width, height))
    return true;
  if (set_v4l2_format(V4L2_PIX_FMT_YVU410, width, height))
    return true;

  if (set_v4l2_format(V4L2_PIX_FMT_GREY, width, height))
    return true;
...
bool
vidl_v4l2_device::set_v4l2_format(unsigned int fourcode, int width, int height, double fps)
{
  fmt.fmt.pix.width = 0;
  fmt.fmt.pix.height = 0;
  ...
  if (is_open())
  {
    ...
    if (fps != 0.0)
    {
      ...
      {
        if (xioctl(fd, VIDIOC_G_PARM, &sfrate) == 0)
        { // Confirm frame rate
          frame_rate =
            (double)sfrate.parm.capture.timeperframe.denominator / (double)sfrate.parm.capture.timeperframe.numerator;
        }
      }
    }
...
bool
vidl_v4l2_device::init_mmap(int reqbuf)
{
  if (!format_is_set())
    if (!try_formats())
      return false;
  ...
  { // n_buffers is member
    ...
    buffers[n_buffers].start = mmap(NULL /* start anywhere */,
                                    ...
                                    fd,
                                    buffers[n_buffers].buf.m.offset);
...
bool
vidl_v4l2_device::set_number_of_buffers(unsigned int nb)
{
  if (nb == 0)
    return false;
  if (pre_nbuffers == nb)
...
bool
vidl_v4l2_device::start_capturing()
{
  if (capturing)
    return true;
  if (!buffers)
...
bool
vidl_v4l2_device::stop_capturing()
{
  if (!capturing)
    return true;
...
unsigned int
vidl_v4l2_device::current_input() const
{
  if (!is_open())
    return n_inputs();
...
bool
vidl_v4l2_device::set_input(unsigned int i)
{
  if (current_input() == i)
    return true;
...
operator<<(std::ostream & os, const vidl_v4l2_device & dev)
{
  ...
  os << "      Current input: " << dev.current_input() << std::endl
     << "      Current format: " << v4l2_to_vidl(dev.get_v4l2_format()) << " width: " << dev.get_width()
     << " height: " << dev.get_height() << std::endl;
  return os;
}
