...
public:
  ...
  name() const
  {
    return std::string((const char *)input_.name);
  }
  //: Return if the input uses a tuner (RF modulator)
  bool
  is_tuner() const
  {
    return input_.type == V4L2_INPUT_TYPE_TUNER;
  }
};
...
class vidl_v4l2_device
{
  ...
  void
  update_controls(); // must be called after input change
  ...
  bool
  open(); // return true if successful
  bool
  initialize_device(); //  return true if successful
  bool
  init_mmap(int reqbuf);
  bool
  uninit_mmap();
  ...
  bool
  is_prepared_for_capturing() const
  {
    return buffers != 0;
  }
  bool
  ...
  vidl_v4l2_device &
  operator=(const vidl_v4l2_device &);
...
public:
  ...
  bool
  set_input(unsigned int i);

  //: Select a new format.
  // Device try automatically different formats (\see try_formats()). Then this function could be ignored if you don't
  // mind the format. Usually, user wants a concrete pixel format or, simply, change width and height.
  // \param fourcode A four character code defined in v4l2 (see v4l2 specification and vidl_pixel_format.h) indicating
  // pixel encoding
  ...
  // \see format_is_set
  bool
  set_v4l2_format(unsigned int fourcode, int width, int height, double fps = 0.0);
  ...
  bool
  try_formats(int width = 640, int height = 480);

  //: Return if the format is set.
  // Normally, a format is automatically selected or user call set_v4l2_format. User can use this function to know if a
  // format is selected before calling start_capturing.
  // \see set_v4l2_format
  bool
  format_is_set() const
  {
  ...
  // \return the four character code which is being used by driver(see v4l2 specification and vidl_pixel_format.h) or 0
  // if not set
  // \see set_v4l2_format
  unsigned int
  get_v4l2_format() const
  {
    return (fmt.fmt.pix.width != 0) ? fmt.fmt.pix.pixelformat : 0;
  }
  ...
  vidl_v4l2_control_integer *
  get_control_integer_id(int id) const
  {
    vidl_v4l2_control * pc = get_control_id(id);
    return pc ? (pc->type() == V4L2_CTRL_TYPE_INTEGER ? dynamic_cast<vidl_v4l2_control_integer *>(pc) : 0) : 0;
  }

  //: Get control from driver id
  // \param id is control ID from v4l2 specification. For example, V4L2_CID_BRIGHTNESS
  // \return pointer to control. Null if doesn't exist or is not of type V4L2_CTRL_TYPE_MENU
  vidl_v4l2_control_menu *
  get_control_menu_id(int id) const
  {
    vidl_v4l2_control * pc = get_control_id(id);
    return pc ? (pc->type() == V4L2_CTRL_TYPE_MENU ? dynamic_cast<vidl_v4l2_control_menu *>(pc) : 0) : 0;
  }

  //: Get control from driver id
  // \param id is control ID from v4l2 specification. For example, V4L2_CID_BRIGHTNESS
  // \return pointer to control. Null if doesn't exist or is not of type V4L2_CTRL_TYPE_BOOLEAN
  vidl_v4l2_control_boolean *
  get_control_boolean_id(int id) const
  {
    vidl_v4l2_control * pc = get_control_id(id);
    return pc ? (pc->type() == V4L2_CTRL_TYPE_BOOLEAN ? dynamic_cast<vidl_v4l2_control_boolean *>(pc) : 0) : 0;
  }
  ...
  get_control_button_id(int id) const
  {
    vidl_v4l2_control * pc = get_control_id(id);
    return pc ? (pc->type() == V4L2_CTRL_TYPE_BUTTON ? dynamic_cast<vidl_v4l2_control_button *>(pc) : 0) : 0;
  }
  ...
  // \return if successful
  bool
  set_number_of_buffers(unsigned int nb);
  //: Get numbers of buffers
  unsigned int
  get_number_of_buffers() const
  {
    return pre_nbuffers;
  }
