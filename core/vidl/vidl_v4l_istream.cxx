...
//: open a device
bool
vidl_v4l_istream::open(const std::string & device_name)
{
  close();
  frame_number_ = 0;
...
bool
vidl_v4l_istream::is_open() const
{
  return fd_ > 0;
}
...
bool
vidl_v4l_istream::is_valid() const
{
  return is_open();
}
...
bool
vidl_v4l_istream::set_params(const vidl_v4l_params & p)
{
  bool success = true;
  ...
  {
    success = false;
    std::cerr << "vp.brightness = " << vp.brightness << std::endl << "p.brightness = " << p.brightness_ << std::endl;
  }
  if (vp.hue != p.hue_)
  {
    success = false;
    std::cerr << "vp.hue = " << vp.hue << std::endl << "p.hue = " << p.hue_ << std::endl;
  }
  if (vp.colour != p.colour_)
  {
    success = false;
    std::cerr << "vp.colour = " << vp.colour << std::endl << "p.colour = " << p.colour_ << std::endl;
  }
  if (vp.contrast != p.contrast_)
  {
    success = false;
    std::cerr << "vp.contrast = " << vp.contrast << std::endl << "p.contrast = " << p.contrast_ << std::endl;
  }
  if (vp.whiteness != p.whiteness_)
  {
    success = false;
    std::cerr << "vp.whiteness = " << vp.whiteness << std::endl << "p.whiteness = " << p.whiteness_ << std::endl;
  }
  if (vp.depth != p.depth_)
  {
    success = false;
    std::cerr << "vp.depth = " << vp.depth << std::endl << "p.depth = " << p.depth_ << std::endl;
  }
  if (vp.palette != vidl_v4l_params::vidlpf_to_v4lpf(p.pixel_format_))
  {
    success = false;
    std::cerr << "vp.palette = " << vp.palette << std::endl << "p.pixel_format = " << p.pixel_format_ << std::endl;
  }
  ...
  cur_frame_ = new vidl_shared_frame(buf, vw.width, vw.height, vidl_v4l_params::v4lpf_to_vidlpf(vp.palette));
...
double
vidl_v4l_istream::frame_rate() const
{
