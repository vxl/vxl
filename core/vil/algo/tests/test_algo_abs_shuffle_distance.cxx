...
static void
asd_fill_image(vil_image_view<vxl_byte> & image)
{
...
static void
test_algo_abs_shuffle_distance()
{
  std::cout << "**********************************\n"
  ...
  TEST_NEAR(
    "Shuffle dist to self, 2 pixel displacement", vil_abs_shuffle_distance(sub_im1, sub_im3, se, false), 1.0, 1e-6);

  std::cout << "Using larger radius: " << std::endl;
  se.set_to_disk(2.5);
  std::cout << "Structuring element: " << se << std::endl;
  TEST_NEAR(
    "Shuffle dist to self, 2 pixel displacement", vil_abs_shuffle_distance(sub_im1, sub_im3, se, false), 0.0, 1e-6);
