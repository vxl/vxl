...
test_algo_convolve_1d_double()
{
  ...
  std::cout << "Testing vil_convolve_ignore_edge end type\n";
  for (int i = 0; i < n + 2; ++i)
    dest[i] = 999;
  ...
  std::cout << "Testing vil_convolve_no_extend end type\n";
  for (int i = 0; i < n + 2; ++i)
    dest[i] = 999;
  ...
  std::cout << "Testing vil_convolve_zero_extend end type\n";
  for (int i = 0; i < n + 2; ++i)
    dest[i] = 999;
  vil_convolve_1d(&src[0],
                  n,
  ...
  std::cout << "Test vil_convolve_zero_extend end type with 5-tap filter\n";
  for (int i = 0; i < n + 2; ++i)
    dest[i] = 999;
  vil_convolve_1d(&src[0],
                  n,
  ...
  std::cout << "Testing vil_convolve_constant_extend end type\n";
  for (int i = 0; i < n + 2; ++i)
    dest[i] = 999;
  vil_convolve_1d(&src[0],
                  n,
  ...
  std::cout << "Test vil_convolve_constant_extend end type with 5-tap filter\n";
  for (int i = 0; i < n + 2; ++i)
    dest[i] = 999;
  vil_convolve_1d(&src[0],
                  n,
  ...
  std::cout << "Testing vil_convolve_reflect_extend end type\n";
  for (int i = 0; i < n + 2; ++i)
    dest[i] = 999;
  vil_convolve_1d(&src[0],
                  n,
  ...
  std::cout << "Test vil_convolve_reflect_extend end type with 5-tap filter\n";
  for (int i = 0; i < n + 2; ++i)
    dest[i] = 999;
  vil_convolve_1d(&src[0],
                  n,
  ...
  std::cout << "Testing vil_convolve_periodic_extend end type\n";
  for (int i = 0; i < n + 2; ++i)
    dest[i] = 999;
  vil_convolve_1d(&src[0],
                  n,
  ...
  std::cout << "Testing vil_convolve_trim end type\n";
  for (int i = 0; i < n + 2; ++i)
    dest[i] = 999;
  vil_convolve_1d(&src[0],
                  n,
  ...
  std::cout << "Test vil_convolve_trim end type with 5-tap filter\n";
  for (int i = 0; i < n + 2; ++i)
    dest[i] = 999;
  vil_convolve_1d(&src[0],
                  n,
  ...
  std::cout << "Test vil_convolve_trim end type with 4-tap filter\n";
  for (int i = 0; i < n + 2; ++i)
    dest[i] = 999;
  vil_convolve_1d(&src2[0],
                  n,
  ...
  vil_convolve_1d(v, v_out, &kernel[2], -1, 1, int(), vil_convolve_constant_extend, vil_convolve_zero_extend);

  // check they are equal in various regions..
  TEST("convolved resource.get_view() == convolved view.window() top-left corner",
       vil_image_view_deep_equality(vil_crop(v_out, 0, 4, 0, 4), vil_image_view<vxl_byte>(conv->get_view(0, 4, 0, 4))),
       true);
  TEST("convolved resource.get_view() == convolved view.window() centre",
       vil_image_view_deep_equality(vil_crop(v_out, 3, 4, 3, 4), vil_image_view<vxl_byte>(conv->get_view(3, 4, 3, 4))),
       true);
  TEST("convolved resource.get_view() == convolved view.window() bottom-right corner",
       vil_image_view_deep_equality(vil_crop(v_out, n - 4, 4, n - 4, 4),
                                    vil_image_view<vxl_byte>(conv->get_view(n - 4, 4, n - 4, 4))),
       true);
}

static void
test_algo_convolve_1d()
{
  test_algo_convolve_1d_double();
}
