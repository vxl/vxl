...
inline void
vil_cartesian_differential_invariants_3_1plane(const vil_image_view<S> & src,
                                               ...
                                               unsigned max_kernel_width /*=0*/)
{
  ...
  vil_convolve_1d(vil_transpose(LXx), Lx, &filt_0[filt_c], -filt_c, filt_c, double(), bo, bo);
  vil_convolve_1d(vil_transpose(LX), Ly, &filt_1[filt_c], -filt_c, filt_c, double(), bo, bo);

  // construct second order partial derivatives
  vil_convolve_1d(vil_transpose(LXxx), Lxx, &filt_0[filt_c], -filt_c, filt_c, double(), bo, bo);
  vil_convolve_1d(vil_transpose(LXx), Lxy, &filt_1[filt_c], -filt_c, filt_c, double(), bo, bo);
  vil_convolve_1d(vil_transpose(LX), Lyy, &filt_2[filt_c], -filt_c, filt_c, double(), bo, bo);

  // construct third order partial derivatives
  vil_convolve_1d(vil_transpose(LXxxx), Lxxx, &filt_0[filt_c], -filt_c, filt_c, double(), bo, bo);
  vil_convolve_1d(vil_transpose(LXxx), Lxxy, &filt_1[filt_c], -filt_c, filt_c, double(), bo, bo);
  vil_convolve_1d(vil_transpose(LXx), Lxyy, &filt_2[filt_c], -filt_c, filt_c, double(), bo, bo);
  vil_convolve_1d(vil_transpose(LX), Lyyy, &filt_3[filt_c], -filt_c, filt_c, double(), bo, bo);
  ...
  for (unsigned j = 0; j < dest.ni(); ++j)
  {
    for (unsigned i = 0; i < dest.nj(); ++i)
    {
      // I1 = LiLi=LxLx+LyLy
      // cdi(1,:,:)=(Lx.*Lx)+(Ly.*Ly);
      dest(j, i, 0) = Lx(i, j) * Lx(i, j) + Ly(i, j) * Ly(i, j);

      // LiLijLj=LxxLxLx+2LxLxyLy+LyyLyLy
      // cdi(2,:,:)=(Lxx.*Lx.*Lx)+(2.*Lx.*Lxy.*Ly)+(Lyy.*Ly.*Ly);
      dest(j, i, 1) =
        Lxx(i, j) * Lx(i, j) * Lx(i, j) + T(2) * Lx(i, j) * Lxy(i, j) * Ly(i, j) + Lyy(i, j) * Ly(i, j) * Ly(i, j);

      // %LiiLjLj-LijLiLj=LxxLyLy-2LxyLxLy+LyyLxLx
      // cdi(3,:,:)=(Lxx.*Ly.*Ly)-(2.*Lxy.*Lx.*Ly)+(Lyy.*Lx.*Lx);
      dest(j, i, 2) =
        Lxx(i, j) * Ly(i, j) * Ly(i, j) - T(2) * Lxy(i, j) * Lx(i, j) * Ly(i, j) + Lyy(i, j) * Lx(i, j) * Lx(i, j);


      // %-eijLjkLiLk=LxxLxLy+LxyLyLy-LyyLxLy-LxyLxLx
      // cdi(4,:,:)=(Lxx.*Lx.*Ly)+(Lxy.*Ly.*Ly)-(Lyy.*Lx.*Ly)-(Lxy.*Lx.*Lx);
      dest(j, i, 3) = Lxx(i, j) * Lx(i, j) * Ly(i, j) + Lxy(i, j) * Ly(i, j) * Ly(i, j) -
                      Lyy(i, j) * Lx(i, j) * Ly(i, j) - Lxy(i, j) * Lx(i, j) * Lx(i, j);
      ...
      dest(j, i, 4) = T(2) * Lxyy(i, j) * Lx(i, j) * Lx(i, j) * Ly(i, j) -
                      T(2) * Lxxy(i, j) * Lx(i, j) * Ly(i, j) * Ly(i, j) - Lxxy(i, j) * Lx(i, j) * Ly(i, j) * Ly(i, j) -
                      Lyyy(i, j) * Lx(i, j) * Lx(i, j) * Lx(i, j) + Lxyy(i, j) * Ly(i, j) * Lx(i, j) * Lx(i, j) +
      ...
      dest(j, i, 5) = Lxxx(i, j) * Lx(i, j) * Ly(i, j) * Ly(i, j) - T(2) * Lxxy(i, j) * Lx(i, j) * Lx(i, j) * Ly(i, j) +
                      Lxxy(i, j) * Ly(i, j) * Ly(i, j) * Ly(i, j) + Lxyy(i, j) * Lx(i, j) * Lx(i, j) * Lx(i, j) -
                      T(2) * Lxyy(i, j) * Lx(i, j) * Ly(i, j) * Ly(i, j) + Lyyy(i, j) * Lx(i, j) * Lx(i, j) * Ly(i, j);
      ...
      dest(j, i, 6) = Lxxx(i, j) * Lx(i, j) * Lx(i, j) * Ly(i, j) + T(2) * Lxxy(i, j) * Lx(i, j) * Ly(i, j) * Ly(i, j) +
                      Lxyy(i, j) * Ly(i, j) * Ly(i, j) * Ly(i, j) - Lyyy(i, j) * Lx(i, j) * Ly(i, j) * Ly(i, j) -
                      T(2) * Lxyy(i, j) * Lx(i, j) * Lx(i, j) * Ly(i, j) - Lxxy(i, j) * Lx(i, j) * Lx(i, j) * Lx(i, j);
      ...
      dest(j, i, 7) = Lxxx(i, j) * Lx(i, j) * Lx(i, j) * Lx(i, j) + Lyyy(i, j) * Ly(i, j) * Ly(i, j) * Ly(i, j) +
...
#define VIL_CARTESIAN_DIFFERENTIAL_INVARIANTS_INSTANTIATE(S, T) \
  template void vil_cartesian_differential_invariants_3(        \
    const vil_image_view<S> & src, vil_image_view<T> & dest, double, unsigned)
