...
inline void
vil_convolve_edge_1d(const srcT * src,
                     unsigned n,
  ...
  switch (option)
  {
    ...
    case vil_convolve_constant_extend:
    {
      ...
      for (std::ptrdiff_t i = 0; i <= i_max; ++i)
      {
        ...
        {
          if ((i + j) < 0)
            sum += (accumT)(src[0] * kernel[j * (-kstep)]);
          else
    ...
    case vil_convolve_trim:
    {
      ...
      accumT k_sum_all = 0;
      for (std::ptrdiff_t j = -k_hi; j <= -k_lo; ++j)
        k_sum_all += (accumT)(kernel[j * (-kstep)]);
...
template <class srcT, class destT, class kernelT, class accumT>
inline void
vil_convolve_1d(const srcT * src0,
                unsigned nx,
                std::ptrdiff_t s_step,
                destT * dest0,
                std::ptrdiff_t d_step,
                ...
                vil_convolve_boundary_option end_option)
{
  ...
  for (destT *dest = dest0 + d_step * k_hi, *const end_dest = dest0 + d_step * (int(nx) + k_lo); dest != end_dest;
  ...
  vil_convolve_edge_1d(src0 + (nx - 1) * s_step,
                       nx,
                       -s_step,
                       dest0 + (nx - 1) * d_step,
                       -d_step,
...
template <class srcT, class destT, class kernelT, class accumT>
inline void
vil_convolve_1d(const vil_image_view<srcT> & src_im,
    ...
    if (s_istep == 1)
    {
      if (d_istep == 1)
        for (unsigned int j = 0; j < n_j; ++j, src_row += s_jstep, dest_row += d_jstep)
          vil_convolve_1d(src_row, n_i, 1, dest_row, 1, kernel, k_lo, k_hi, ac, start_option, end_option);
      else
        for (unsigned int j = 0; j < n_j; ++j, src_row += s_jstep, dest_row += d_jstep)
          vil_convolve_1d(src_row, n_i, 1, dest_row, d_istep, kernel, k_lo, k_hi, ac, start_option, end_option);
    }
    else
    {
      if (d_istep == 1)
        for (unsigned int j = 0; j < n_j; ++j, src_row += s_jstep, dest_row += d_jstep)
          vil_convolve_1d(src_row, n_i, s_istep, dest_row, 1, kernel, k_lo, k_hi, ac, start_option, end_option);
      else
        for (unsigned int j = 0; j < n_j; ++j, src_row += s_jstep, dest_row += d_jstep)
          vil_convolve_1d(src_row, n_i, s_istep, dest_row, d_istep, kernel, k_lo, k_hi, ac, start_option, end_option);
    }
  }
}
...
vil_convolve_1d(const vil_image_resource_sptr & src_im,
                const destT dt,
                const kernelT * kernel,
                int k_lo,
                int k_hi,
...
public:
  ...
  vil_convolve_1d_resource(const vil_image_resource_sptr & src,
    ...
    , klo_(k_lo)
    , khi_(k_hi)
    , start_option_(start_option)
    , end_option_(end_option)
  {
    // Can't do period extension yet.
    assert(start_option != vil_convolve_periodic_extend || end_option != vil_convolve_periodic_extend);
  }
...
public:
  ...
  {
    if (i0 + n_i > src_->ni() || j0 + n_j > src_->nj())
      return nullptr;
...
#define macro(F, T)                                                                                            \
  case F:                                                                                                      \
    vil_convolve_1d(                                                                                           \
      static_cast<vil_image_view<T> &>(*vs), dest, kernel_, klo_, khi_, accumT(), start_option_, end_option_); \
