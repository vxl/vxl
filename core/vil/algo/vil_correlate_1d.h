...
inline void
vil_correlate_1d(const srcT * src0,
                 unsigned nx,
                 ...
                 vil_convolve_boundary_option end_option)
{
  ...
  for (destT * dest = dest0 - d_step * k_lo; dest != end_dest; dest += d_step, src += s_step)
  {
    accumT sum = 0;
    const srcT * s = src;
    for (const kernelT * k = k_begin; k != k_end; ++k, s += s_step)
      sum += (accumT)((*k) * (*s));
    *dest = destT(sum);
  }
  ...
  vil_convolve_edge_1d(
    src0 + (nx - 1) * s_step, nx, -s_step, dest0 + (nx - 1) * d_step, -d_step, kernel, k_lo, k_hi, 1, ac, end_option);
}
...
template <class srcT, class destT, class kernelT, class accumT>
inline void
vil_correlate_1d(const vil_image_view<srcT> & src_im,
    ...
    if (s_istep == 1)
    {
      if (d_istep == 1)
        for (unsigned int j = 0; j < nj; ++j, src_row += s_jstep, dest_row += d_jstep)
          vil_correlate_1d(src_row, ni, 1, dest_row, 1, kernel, k_lo, k_hi, ac, start_option, end_option);
      else
        for (unsigned int j = 0; j < nj; ++j, src_row += s_jstep, dest_row += d_jstep)
          vil_correlate_1d(src_row, ni, 1, dest_row, d_istep, kernel, k_lo, k_hi, ac, start_option, end_option);
    }
    else
    {
      if (d_istep == 1)
        for (unsigned int j = 0; j < nj; ++j, src_row += s_jstep, dest_row += d_jstep)
          vil_correlate_1d(src_row, ni, s_istep, dest_row, 1, kernel, k_lo, k_hi, ac, start_option, end_option);
      else
        for (unsigned int j = 0; j < nj; ++j, src_row += s_jstep, dest_row += d_jstep)
          vil_correlate_1d(src_row, ni, s_istep, dest_row, d_istep, kernel, k_lo, k_hi, ac, start_option, end_option);
    }
  }
}
...
vil_correlate_1d(const vil_image_resource_sptr & src_im,
                 const destT dt,
                 const kernelT * kernel,
                 std::ptrdiff_t k_lo,
                 std::ptrdiff_t k_hi,
...
public:
  ...
  {
    if (i0 + ni > src_->ni() || j0 + nj > src_->nj())
      return nullptr;
