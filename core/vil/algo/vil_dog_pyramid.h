...
template <class T>
void
vil_dog_pyramid(const vil_image_view<T> & src_image,
                ...
                unsigned min_size)
{
  ...
  for (unsigned i = 1; i < nL; ++i)
  {
    ...
    vil_resample_bilin(smooth_pyramid[i - 1], sub_sampled_image, 0.0, 0.0, scaling, 0.0, 0.0, scaling, ni, nj);

    vil_gauss_filter_5tap(sub_sampled_image, smooth_pyramid[i], smooth_params,
                          dog_pyramid[i]); // Workspace
    vil_math_image_difference(sub_sampled_image, smooth_pyramid[i], dog_pyramid[i]);
  }
}
