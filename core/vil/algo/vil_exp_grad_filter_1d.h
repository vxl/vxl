...
inline void
vil_exp_grad_filter_1d(const srcT * src, std::ptrdiff_t sstep, destT * dest, std::ptrdiff_t dstep, int n, accumT k)
{
  const srcT * s = src;
  ...
  while (s != src_end)
  {
    *dest = (destT)(rt / k_sum); // Set value for -ive half of filter
    ...
    k_sum *= k; // Scale sums
    rt -= *s;
    k_sum += 1.0f; // Increment with next element
    s += sstep;
    dest += dstep; // Move to next element
  }
...
template <class srcT, class destT, class accumT>
inline void
vil_exp_grad_filter_i(const vil_image_view<srcT> & src_im, vil_image_view<destT> & dest_im, accumT k)
{
...
template <class srcT, class destT, class accumT>
inline void
vil_exp_grad_filter_j(const vil_image_view<srcT> & src_im, vil_image_view<destT> & dest_im, accumT k)
{
  unsigned ni = src_im.ni();
  unsigned nj = src_im.nj();
  dest_im.set_size(ni, nj, src_im.nplanes());
