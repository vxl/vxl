...
template <class T>
inline bool
vil_is_peak_3x3(const T * im, std::ptrdiff_t i_step, std::ptrdiff_t j_step)
{
  T v = *im;
  return v > im[i_step] && v > im[-i_step] && v > im[j_step] && v > im[-j_step] && v > im[i_step + j_step] &&
         v > im[i_step - j_step] && v > im[j_step - i_step] && v > im[-i_step - j_step];
}
...
template <class T>
inline void
vil_find_peaks_3x3(std::vector<unsigned> & pi,
...
template <class T>
bool
vil_interpolate_peak(const T * pixel,
                     std::ptrdiff_t istep,
                     std::ptrdiff_t jstep,
                     double & dx,
                     double & dy,
...
template <class T>
inline void
vil_find_peaks_3x3_subpixel(std::vector<double> & px,
