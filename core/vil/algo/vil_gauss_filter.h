...
void
vil_gauss_filter_gen_ntap(double sd, unsigned diff, std::vector<double> & filter_dest);
...
template <class srcT, class destT>
inline void
vil_gauss_filter_1d(const vil_image_view<srcT> & src_im,
                    vil_image_view<destT> & dest_im,
                    double sd,
                    unsigned half_width)
{
...
template <class srcT, class destT>
inline void
vil_gauss_filter_2d(const vil_image_view<srcT> & src_im,
                    vil_image_view<destT> & dest_im,
                    double sd,
                    unsigned half_width,
                    vil_convolve_boundary_option boundary = vil_convolve_zero_extend)
{
  ...
  vil_convolve_1d(src_im, work_im, &filter[half_width], -int(half_width), half_width, float(), boundary, boundary);
  ...
  vil_convolve_1d(work_im_t, dest_im_t, &filter[half_width], -int(half_width), half_width, float(), boundary, boundary);
}
                    ...
                    vil_convolve_boundary_option boundary = vil_convolve_zero_extend)
{
  ...
  vil_image_view<destT> work_im;
  vil_convolve_1d(
    src_im, work_im, &filter_i[half_width_i], -int(half_width_i), half_width_i, float(), boundary, boundary);
  ...
  vil_convolve_1d(
    work_im_t, dest_im_t, &filter_j[half_width_j], -int(half_width_j), half_width_j, float(), boundary, boundary);
}
