// This is core/vil/algo/vil_gauss_filter.hxx
#ifndef vil_gauss_filter_hxx_
#define vil_gauss_filter_hxx_
//:
// \file
// \brief smooth images
// \author Ian Scott
...
vl_round(double x, unsigned char)
{
  return (unsigned char)(x < 0 ? x - 0.5 : x + 0.5);
}
inline signed char
vl_round(double x, signed char)
{
  return (signed char)(x < 0 ? x - 0.5 : x + 0.5);
}
inline unsigned short
vl_round(double x, unsigned short)
{
  return (unsigned short)(x < 0 ? x - 0.5 : x + 0.5);
}
inline signed short
vl_round(double x, signed short)
{
  return (signed short)(x < 0 ? x - 0.5 : x + 0.5);
}
inline unsigned int
vl_round(double x, unsigned int)
{
  return (unsigned int)(x < 0 ? x - 0.5 : x + 0.5);
}
inline signed int
vl_round(double x, signed int)
{
  return (signed int)(x < 0 ? x - 0.5 : x + 0.5);
}
inline unsigned long
vl_round(double x, unsigned long)
{
  return (unsigned long)(x < 0 ? x - 0.5 : x + 0.5);
}
inline signed long
vl_round(double x, signed long)
{
  return (signed long)(x < 0 ? x - 0.5 : x + 0.5);
}
inline double
vl_round(double x, double)
{
  return x;
}
inline float
vl_round(double x, float)
{
  return (float)x;
}
...
void
vil_gauss_filter_5tap(const srcT * src_im,
                      ...
                      std::ptrdiff_t work_jstep)
{
  ...
  for (unsigned int y = 0; y < ny; y++)
  {
    ...
    int nx2 = nx - 2;
    for (x = 2; x < nx2; x++)
      work_row[x] = vl_round(params.filt2() * src_col1[x * src_istep] + params.filt1() * src_col2[x * src_istep] +
                               params.filt0() * src_col3[x * src_istep] + params.filt1() * src_col4[x * src_istep] +
                               params.filt2() * src_col5[x * src_istep],
                             (destT)0);

    // Now deal with edge effects :
    work_row[0] = vl_round(params.filt_edge0() * src_col3[0] + params.filt_edge1() * src_col4[0] +
                             params.filt_edge2() * src_col5[0],
                           (destT)0);

    work_row[1] =
      vl_round(params.filt_pen_edge_n1() * src_col2[src_istep] + params.filt_pen_edge0() * src_col3[src_istep] +
                 params.filt_pen_edge1() * src_col4[src_istep] + params.filt_pen_edge2() * src_col5[src_istep],
               (destT)0);
  ...
  for (unsigned int y = 2; y < ny - 2; y++)
  {
    ...
    for (unsigned int x = 0; x < nx; x++)
      dest_row[x * dest_istep] =
        vl_round(params.filt2() * work_row1[x] + params.filt1() * work_row2[x] + params.filt0() * work_row3[x] +
                   params.filt1() * work_row4[x] + params.filt2() * work_row5[x],
                 (destT)0);
  }
  ...
  for (unsigned int x = 0; x < nx; x++)
  {
    dest_row_top[x * dest_istep] =
      vl_round(params.filt_edge0() * work_row_top_5[x] + params.filt_edge1() * work_row_top_4[x] +
                 params.filt_edge2() * work_row_top_3[x],
               (destT)0);

    dest_row_next_top[x * dest_istep] =
      vl_round(params.filt_pen_edge2() * work_row_top_2[x] + params.filt_pen_edge1() * work_row_top_3[x] +
                 params.filt_pen_edge0() * work_row_top_4[x] + params.filt_pen_edge_n1() * work_row_top_5[x],
               (destT)0);

    dest_row_next_bottom[x * dest_istep] =
      vl_round(params.filt_pen_edge2() * work_row_bottom_4[x] + params.filt_pen_edge1() * work_row_bottom_3[x] +
                 params.filt_pen_edge0() * work_row_bottom_2[x] + params.filt_pen_edge_n1() * work_row_bottom_1[x],
               (destT)0);

    dest_row_bottom[x * dest_istep] =
      vl_round(params.filt_edge2() * work_row_bottom_3[x] + params.filt_edge1() * work_row_bottom_2[x] +
                 params.filt_edge0() * work_row_bottom_1[x],
               (destT)0);
  }
...
template <class srcT, class destT>
void
vil_gauss_filter_5tap(const vil_image_view<srcT> & src_im,
                      ...
                      vil_image_view<destT> & work)
{
  ...
  if (ni > 3 && nj > 3)
    ...
    for (unsigned p = 0; p < n_planes; ++p)
      vil_gauss_filter_5tap(&src_im(0, 0, p),
                            src_im.istep(),
                            src_im.jstep(),
                            &dest_im(0, 0, p),
                            dest_im.istep(),
  ...
  else
  {
    ...
    else if (ni == 4)
    {
      ...
      for (unsigned p = 0; p < n_planes; ++p)
        for (unsigned j = 0; j < nj; ++j)
        {
          ...
          work(1, j, p) = vl_round(
            kpn1 * src_im(0, j, p) + kp0 * src_im(1, j, p) + kp1 * src_im(2, j, p) + kp2 * src_im(3, j, p), destT());
          work(2, j, p) = vl_round(
            kp2 * src_im(0, j, p) + kp1 * src_im(1, j, p) + kp0 * src_im(2, j, p) + kpn1 * src_im(3, j, p), destT());
    ...
    else if (ni > 4)
    {
      ...
      vil_convolve_1d(src_im, work, kernel + 2, -2, +2, double(), vil_convolve_trim, vil_convolve_trim);
    }
    if (nj == 1)
      dest_im = work;
    else if (nj == 2)
    ...
    else if (nj > 4)
    {
      ...
      vil_convolve_1d(vil_transpose(work), rdest, kernel + 2, -2, +2, double(), vil_convolve_trim, vil_convolve_trim);
    }
  }
