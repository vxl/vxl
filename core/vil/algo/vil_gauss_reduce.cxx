...
template <>
void
vil_gauss_reduce_1plane(const vxl_byte * src_im,
                        unsigned src_ni,
                        unsigned src_nj,
                        ...
                        std::ptrdiff_t d_y_step)
{
  ...
  for (unsigned y = 0; y < src_nj; ++y)
  {
    ...
    *d_row = static_cast<vxl_byte>(vnl_math::rnd(0.071f * s_row[sxs2] + 0.357f * s_row[s_x_step] + 0.572f * s_row[0]));
    ...
    for (unsigned x = 0; x < ni2; ++x)
    {
      *d = static_cast<vxl_byte>(
        vnl_math::rnd(0.05 * s[-sxs2] + 0.05 * s[sxs2] + 0.25 * s[-s_x_step] + 0.25 * s[s_x_step] + 0.4 * s[0]));

      d += d_x_step;
      s += sxs2;
    }
    // Set last elements of row
    *d = static_cast<vxl_byte>(vnl_math::rnd(0.071f * s[-sxs2] + 0.357f * s[-s_x_step] + 0.572f * s[0]));
...
template <>
void
vil_gauss_reduce_1plane(const float * src_im,
                        unsigned src_ni,
                        unsigned src_nj,
                        std::ptrdiff_t s_x_step,
                        std::ptrdiff_t s_y_step,
                        ...
                        std::ptrdiff_t d_y_step)
{
  ...
  {
    // Set first element of row
    *d_row = 0.071f * s_row[sxs2] + 0.357f * s_row[s_x_step] + 0.572f * s_row[0];
    float * d = d_row + d_x_step;
    const float * s = s_row + sxs2;
    for (unsigned x = 0; x < ni2; ++x)
    {
      *d = 0.05f * (s[-sxs2] + s[sxs2]) + 0.25f * (s[-s_x_step] + s[s_x_step]) + 0.40f * s[0];

      d += d_x_step;
      s += sxs2;
    }
    // Set last elements of row
    *d = 0.071f * s[-sxs2] + 0.357f * s[-s_x_step] + 0.572f * s[0];
...
template <>
void
vil_gauss_reduce_1plane(const double * src_im,
                        unsigned src_ni,
                        unsigned src_nj,
                        std::ptrdiff_t s_x_step,
                        std::ptrdiff_t s_y_step,
                        ...
                        std::ptrdiff_t d_y_step)
{
  ...
  {
    // Set first element of row
    *d_row = 0.071 * s_row[sxs2] + 0.357 * s_row[s_x_step] + 0.572 * s_row[0];
...
template <>
void
vil_gauss_reduce_1plane(const int * src_im,
                        unsigned src_ni,
                        unsigned src_nj,
                        std::ptrdiff_t s_x_step,
                        std::ptrdiff_t s_y_step,
                        ...
                        std::ptrdiff_t d_y_step)
{
  ...
  {
    // Set first element of row
    *d_row = vnl_math::rnd(0.071f * s_row[sxs2] + 0.357f * s_row[s_x_step] + 0.572f * s_row[0]);

    int * d = d_row + d_x_step;
    const int * s = s_row + sxs2;
    for (unsigned x = 0; x < ni2; ++x)
    {
      *d = vnl_math::rnd(0.05 * s[-sxs2] + 0.25 * s[-s_x_step] + 0.05 * s[sxs2] + 0.25 * s[s_x_step] + 0.4 * s[0]);

      d += d_x_step;
      s += sxs2;
    }
    // Set last elements of row
    *d = vnl_math::rnd(0.071f * s[-sxs2] + 0.357f * s[-s_x_step] + 0.572f * s[0]);
...
template <>
void
vil_gauss_reduce_1plane(const vxl_int_16 * src_im,
                        unsigned src_ni,
                        unsigned src_nj,
                        std::ptrdiff_t s_x_step,
                        std::ptrdiff_t s_y_step,
                        ...
                        std::ptrdiff_t d_y_step)
{
  ...
  for (unsigned y = 0; y < src_nj; ++y)
  {
    // Set first element of row
    *d_row =
      static_cast<vxl_int_16>(vnl_math::rnd(0.071f * s_row[sxs2] + 0.357f * s_row[s_x_step] + 0.572f * s_row[0]));
    ...
    for (unsigned x = 0; x < ni2; ++x)
    {
      ...
      *d = vxl_int_16(0.5 + 0.05 * s[-sxs2] + 0.25 * s[-s_x_step] + 0.05 * s[sxs2] + 0.25 * s[s_x_step] + 0.4 * s[0]);

      d += d_x_step;
      s += sxs2;
    }
    // Set last elements of row
    *d = static_cast<vxl_int_16>(vnl_math::rnd(0.071f * s[-sxs2] + 0.357f * s[-s_x_step] + 0.572f * s[0]));
...
template <>
void
vil_gauss_reduce_2_3_1plane(const float * src_im,
                            unsigned src_ni,
                            unsigned src_nj,
                            std::ptrdiff_t s_x_step,
                            std::ptrdiff_t s_y_step,
...
template <>
void
vil_gauss_reduce_2_3_1plane(const double * src_im,
                            unsigned src_ni,
                            unsigned src_nj,
                            std::ptrdiff_t s_x_step,
                            std::ptrdiff_t s_y_step,
...
template <>
void
vil_gauss_reduce_2_3_1plane(const vxl_byte * src_im,
                            unsigned src_ni,
                            unsigned src_nj,
                            std::ptrdiff_t s_x_step,
                            std::ptrdiff_t s_y_step,
...
template <>
void
vil_gauss_reduce_2_3_1plane(const int * src_im,
                            unsigned src_ni,
                            unsigned src_nj,
                            std::ptrdiff_t s_x_step,
                            std::ptrdiff_t s_y_step,
...
template <>
void
vil_gauss_reduce_2_3_1plane(const vxl_int_16 * src_im,
                            unsigned src_ni,
                            unsigned src_nj,
                            std::ptrdiff_t s_x_step,
                            std::ptrdiff_t s_y_step,
...
template <>
void
vil_gauss_reduce_121_1plane(const vxl_byte * src_im,
                            unsigned src_ni,
                            unsigned src_nj,
                            std::ptrdiff_t s_x_step,
                            std::ptrdiff_t s_y_step,
  ...
  for (unsigned y = 0; y < nj2; ++y)
  {
    ...
    *d_row = *s_row2;
    ...
    for (unsigned x = 0; x < ni2; ++x)
    {
      ...
      *d = vxl_byte(0.0625f * s1[-s_x_step] + 0.125f * s1[0] + 0.0625f * s1[s_x_step] + 0.1250f * s2[-s_x_step] +
                    0.250f * s2[0] + 0.1250f * s2[s_x_step] + 0.0625f * s3[-s_x_step] + 0.125f * s3[0] +
                    0.0625f * s3[s_x_step] + 0.5);
...
template <>
void
vil_gauss_reduce_121_1plane(const float * src_im,
                            unsigned src_ni,
                            unsigned src_nj,
                            std::ptrdiff_t s_x_step,
                            std::ptrdiff_t s_y_step,
                            ...
                            std::ptrdiff_t d_y_step)
{
  ...
  for (unsigned y = 0; y < nj2; ++y)
  {
    ...
    *d_row = *s_row2;
    ...
    {
      // The following is a little inefficient - could group terms to reduce arithmetic
      *d = 0.0625f * s1[-s_x_step] + 0.125f * s1[0] + 0.0625f * s1[s_x_step] + 0.1250f * s2[-s_x_step] +
           0.250f * s2[0] + 0.1250f * s2[s_x_step] + 0.0625f * s3[-s_x_step] + 0.125f * s3[0] + 0.0625f * s3[s_x_step];
...
template <>
void
vil_gauss_reduce_121_1plane(const double * src_im,
                            unsigned src_ni,
                            unsigned src_nj,
                            std::ptrdiff_t s_x_step,
                            std::ptrdiff_t s_y_step,
                            ...
                            std::ptrdiff_t d_y_step)
{
  ...
  for (unsigned y = 0; y < nj2; ++y)
  {
    ...
    *d_row = *s_row2;
    ...
    {
      // The following is a little inefficient - could group terms to reduce arithmetic
      *d = 0.0625 * s1[-s_x_step] + 0.125 * s1[0] + 0.0625 * s1[s_x_step] + 0.1250 * s2[-s_x_step] + 0.250 * s2[0] +
           0.1250 * s2[s_x_step] + 0.0625 * s3[-s_x_step] + 0.125 * s3[0] + 0.0625 * s3[s_x_step];
...
template <>
void
vil_gauss_reduce_121_1plane(const int * src_im,
                            unsigned src_ni,
                            unsigned src_nj,
                            std::ptrdiff_t s_x_step,
                            std::ptrdiff_t s_y_step,
  ...
  for (unsigned y = 0; y < nj2; ++y)
  {
    ...
    *d_row = *s_row2;
    ...
    for (unsigned x = 0; x < ni2; ++x)
    {
      // The following is a little inefficient - could group terms to reduce arithmetic
      // Add 0.5 so that truncating effectively rounds
      *d = int(0.0625f * s1[-s_x_step] + 0.125f * s1[0] + 0.0625f * s1[s_x_step] + 0.1250f * s2[-s_x_step] +
               0.250f * s2[0] + 0.1250f * s2[s_x_step] + 0.0625f * s3[-s_x_step] + 0.125f * s3[0] +
               0.0625f * s3[s_x_step] + 0.5);
...
template <>
void
vil_gauss_reduce_121_1plane(const vxl_int_16 * src_im,
                            unsigned src_ni,
                            unsigned src_nj,
                            std::ptrdiff_t s_x_step,
                            std::ptrdiff_t s_y_step,
  ...
  for (unsigned y = 0; y < nj2; ++y)
  {
    ...
    *d_row = *s_row2;
    ...
    for (unsigned x = 0; x < ni2; ++x)
    {
      // The following is a little inefficient - could group terms to reduce arithmetic
      // Add 0.5 so that truncating effectively rounds
      *d = vxl_int_16(0.0625f * s1[-s_x_step] + 0.125f * s1[0] + 0.0625f * s1[s_x_step] + 0.1250f * s2[-s_x_step] +
                      0.250f * s2[0] + 0.1250f * s2[s_x_step] + 0.0625f * s3[-s_x_step] + 0.125f * s3[0] +
                      0.0625f * s3[s_x_step] + 0.5);
