// This is core/vil/algo/vil_gauss_reduce.hxx
#ifndef vil_gauss_reduce_hxx_
#define vil_gauss_reduce_hxx_
//:
// \file
...
template <class T>
void
vil_gauss_reduce(const vil_image_view<T> & src_im, vil_image_view<T> & dest_im, vil_image_view<T> & work_im)
{
  ...
  for (unsigned int i = 0; i < n_planes; ++i)
  {
    ...
    vil_gauss_reduce_1plane(src_im.top_left_ptr() + i * src_im.planestep(),
                            ni,
                            nj,
                            src_im.istep(),
                            src_im.jstep(),
    ...
    vil_gauss_reduce_1plane(work_im.top_left_ptr(),
                            nj,
                            ni2,
                            work_im.jstep(),
                            work_im.istep(),
...
template <class T>
void
vil_gauss_reduce_2_3(const vil_image_view<T> & src_im, vil_image_view<T> & dest_im, vil_image_view<T> & work_im)
{
  unsigned ni = src_im.ni();
  unsigned nj = src_im.nj();
  unsigned n_planes = src_im.nplanes();
  ...
  {
    // Smooth and subsample in x, result in work_im
    vil_gauss_reduce_2_3_1plane(src_im.top_left_ptr() + i * src_im.planestep(),
                                ni,
                                nj,
                                src_im.istep(),
                                src_im.jstep(),
    ...
    // Smooth and subsample in y (by implicitly transposing work_im)
    vil_gauss_reduce_2_3_1plane(work_im.top_left_ptr(),
                                nj,
                                ni2,
                                work_im.jstep(),
                                work_im.istep(),
...
template <class T>
void
vil_gauss_reduce_121(const vil_image_view<T> & src_im, vil_image_view<T> & dest_im)
{
...
rl_round(double x, unsigned char)
{
  return (unsigned char)(x + 0.5);
}

inline signed char
rl_round(double x, signed char)
{
  return (signed char)(x + 0.5);
}

inline unsigned short
rl_round(double x, unsigned short)
{
  return (unsigned short)(x + 0.5);
}

inline signed short
rl_round(double x, signed short)
{
  return (signed short)(x + 0.5);
}

inline unsigned int
rl_round(double x, unsigned int)
{
  return (unsigned int)(x + 0.5);
}

inline signed int
rl_round(double x, signed int)
{
  return (signed int)(x + 0.5);
}

inline unsigned long
rl_round(double x, unsigned long)
{
  return (unsigned long)(x + 0.5);
}

inline signed long
rl_round(double x, signed long)
{
  return (signed long)(x + 0.5);
}

inline double
rl_round(double x, double)
{
  return x;
}

inline float
rl_round(double x, float)
{
  return (float)x;
}
...
template <class T>
void
vil_gauss_reduce_general_plane(const vil_image_view<T> & src,
                               ...
                               const vil_gauss_reduce_params & params)
{
  ...
  for (unsigned y = 0; y < src.nj(); y++)
  {
    ...
    for (unsigned x = 2; x < ni2; ++x)
      worka(x, y) = rl_round(params.filt2() * (src(x - 2, y) + src(x + 2, y)) +
                               params.filt1() * (src(x - 1, y) + src(x + 1, y)) + params.filt0() * src(x, y),
                             T(0));

    // Now deal with edge effects :
    worka(0, y) = rl_round(
      params.filt_edge0() * src(0, y) + params.filt_edge1() * src(1, y) + params.filt_edge2() * src(2, y), (T)0);

    worka(1, y) = rl_round(params.filt_pen_edge_n1() * src(0, y) + params.filt_pen_edge0() * src(1, y) +
                             params.filt_pen_edge1() * src(2, y) + params.filt_pen_edge2() * src(3, y),
                           (T)0);

    worka(src.ni() - 2, y) =
      rl_round(params.filt_pen_edge2() * src(src.ni() - 4, y) + params.filt_pen_edge1() * src(src.ni() - 3, y) +
                 params.filt_pen_edge0() * src(src.ni() - 2, y) + params.filt_pen_edge_n1() * src(src.ni() - 1, y),
               (T)0);

    worka(src.ni() - 1, y) =
      rl_round(params.filt_edge2() * src(src.ni() - 3, y) + params.filt_edge1() * src(src.ni() - 2, y) +
                 params.filt_edge0() * src(src.ni() - 1, y),
               (T)0);
  }
  ...
  for (unsigned int y = 2; y + 2 < src.nj(); ++y)
  {
    for (unsigned int x = 0; x < src.ni(); x++)
      workb(x, y) = rl_round(params.filt2() * (worka(x, y - 2) + worka(x, y + 2)) +
                               params.filt1() * (worka(x, y - 1) + worka(x, y + 1)) + params.filt0() * worka(x, y),
                             (T)0);
  }
  ...
  {
    workb(x, src.nj() - 1) =
      rl_round(params.filt_edge0() * worka(x, src.nj() - 1) + params.filt_edge1() * worka(x, src.nj() - 2) +
                 params.filt_edge2() * worka(x, src.nj() - 3),
               (T)0);

    workb(x, src.nj() - 2) =
      rl_round(params.filt_pen_edge2() * worka(x, src.nj() - 4) + params.filt_pen_edge1() * worka(x, src.nj() - 3) +
                 params.filt_pen_edge0() * worka(x, src.nj() - 2) + params.filt_pen_edge_n1() * worka(x, src.nj() - 1),
               (T)0);

    workb(x, 1) = rl_round(params.filt_pen_edge2() * worka(x, 3) + params.filt_pen_edge1() * worka(x, 2) +
                             params.filt_pen_edge0() * worka(x, 1) + params.filt_pen_edge_n1() * worka(x, 0),
                           (T)0);

    workb(x, 0) = rl_round(
      params.filt_edge2() * worka(x, 2) + params.filt_edge1() * worka(x, 1) + params.filt_edge0() * worka(x, 0), (T)0);
  }
...
template <class T>
void
vil_gauss_reduce_general(const vil_image_view<T> & src,
                         vil_image_view<T> & dest,
                         vil_image_view<T> & worka,
                         vil_image_view<T> & workb,
                         const vil_gauss_reduce_params & params)
{
  ...
  dest.set_size(
    (unsigned)(src.ni() / params.scale_step() + 0.5), (unsigned)(src.nj() / params.scale_step() + 0.5), src.nplanes());
...
template <class T>
void
vil_gauss_reduce_1plane(const T * src_im,
                        unsigned src_ni,
                        unsigned src_nj,
                        ...
                        std::ptrdiff_t d_y_step)
{
  ...
  for (unsigned y = 0; y < src_nj; ++y)
  {
    ...
    double dsum = 0.071 * static_cast<double>(s_row[sxs2]) + 0.357 * static_cast<double>(s_row[s_x_step]) +
    ...
    dsum = 0.071 * static_cast<double>(s[-sxs2]) + 0.357 * static_cast<double>(s[-s_x_step]) +
...
template <class T>
void
vil_gauss_reduce_121_1plane(const T * src_im,
                            unsigned src_ni,
                            unsigned src_nj,
                            std::ptrdiff_t s_x_step,
                            std::ptrdiff_t s_y_step,
  ...
  for (unsigned y = 0; y < nj2; ++y)
  {
    ...
    *d_row = *s_row2;
    ...
    for (unsigned x = 0; x < ni2; ++x)
    {
      ...
      double ds1 = 0.0625 * static_cast<double>(s1[-s_x_step]) + 0.125 * static_cast<double>(s1[0]) +
                   0.0625 * static_cast<double>(s1[s_x_step]),
             ds2 = 0.1250 * static_cast<double>(s2[-s_x_step]) + 0.250 * static_cast<double>(s2[0]) +
                   0.1250 * static_cast<double>(s2[s_x_step]),
             ds3 = 0.0625 * static_cast<double>(s3[-s_x_step]) + 0.125 * static_cast<double>(s3[0]) +
...
template <class T>
void
vil_gauss_reduce_2_3_1plane(const T * src_im,
                            unsigned src_ni,
                            unsigned src_nj,
                            std::ptrdiff_t s_x_step,
                            std::ptrdiff_t s_y_step,
                            ...
                            std::ptrdiff_t d_y_step)
{
  ...
  for (unsigned y = 0; y < src_nj; ++y)
  {
    ...
    {
      double df =
        0.2 * (static_cast<double>(s[-s_x_step]) + static_cast<double>(s[s_x_step])) + 0.6 * static_cast<double>(s[0]);
      rounder(df, *d);
...
#define VIL_GAUSS_REDUCE_INSTANTIATE(T)                                                        \
  template void vil_gauss_reduce(                                                              \
    const vil_image_view<T> & src, vil_image_view<T> & dest, vil_image_view<T> & work_im);     \
  template void vil_gauss_reduce_2_3(                                                          \
    const vil_image_view<T> & src, vil_image_view<T> & dest, vil_image_view<T> & work_im);     \
  template void vil_gauss_reduce_121(const vil_image_view<T> & src, vil_image_view<T> & dest); \
