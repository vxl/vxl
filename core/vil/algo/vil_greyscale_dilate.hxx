// This is core/vil/algo/vil_greyscale_dilate.hxx
#ifndef vil_greyscale_dilate_hxx_
#define vil_greyscale_dilate_hxx_
//:
// \file
                     ...
                     const vil_structuring_element & element)
{
  ...
  std::ptrdiff_t s_istep = src_image.istep(), s_jstep = src_image.jstep(), d_istep = dest_image.istep(),
                 d_jstep = dest_image.jstep();
  ...
  for (int i = 0; i < ilo; ++i)
    for (unsigned int j = 0; j < nj; ++j)
      dest_image(i, j, 0) = vil_greyscale_dilate(src_image, 0, element, i, j);
  ...
  for (int j = jlo; j <= jhi; ++j)
  {
    ...
    for (int i = ilo; i <= ihi; ++i, src_p += s_istep, dest_p += d_istep)
      *dest_p = vil_greyscale_dilate(src_p, &offset[0], offset.size());
  }
}

#undef VIL_GREYSCALE_DILATE_INSTANTIATE
#define VIL_GREYSCALE_DILATE_INSTANTIATE(T) \
  template void vil_greyscale_dilate(       \
    const vil_image_view<T> & src_image, vil_image_view<T> & dest_image, const vil_structuring_element & element)

#endif // vil_greyscale_dilate_hxx_
