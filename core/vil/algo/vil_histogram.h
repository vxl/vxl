...
template <class T>
inline void
vil_histogram(const vil_image_view<T> & image, std::vector<double> & histo, double min, double max, unsigned n_bins)
{
  histo.resize(n_bins);
...
//  \relatesalso vil_image_view
void
vil_histogram_byte(const vil_image_view<vxl_byte> & image, std::vector<double> & histo);

//: Instantiation macro for other types
#define VIL_HISTOGRAM_INSTANTIATE(T) \
  template void vil_histogram(       \
    const vil_image_view<T> & image, std::vector<double> & histo, double min, double max, unsigned n_bins)
