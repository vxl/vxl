...
inline void
vil_update_parabola_set(std::vector<double> & x,
                        std::vector<double> & y,
                        std::vector<double> & z,
                        double a,
                        double x0,
                        ...
                        double n)
{
  ...
  while (true)
  {
    ...
    if (new_z > n)
      return; // Only useful outsize (0,n) so discard
    if (new_z > z[k])
    {
      x.push_back(x0);
      y.push_back(y0);
...
template <class srcT>
inline void
vil_quad_envelope(const srcT * src,
...
inline void
vil_sample_quad_envelope(const std::vector<double> & x,
...
inline void
vil_sample_quad_envelope_with_pos(const std::vector<double> & x,
                                  const std::vector<double> & y,
                                  const std::vector<double> & z,
                                  double a,
                                  destT * dest,
...
inline void
vil_quad_distance_function_1D(const srcT * src,
                              std::ptrdiff_t s_step,
                              size_t n,
                              double a,
                              destT * dest,
...
template <class srcT, class destT>
inline void
vil_quad_distance_function(const vil_image_view<srcT> & src, double ai, double aj, vil_image_view<destT> & dest)
{
  ...
  std::ptrdiff_t d_istep = dest.istep(), d_jstep = dest.jstep();
...
template <class srcT, class destT, class posT>
inline void
vil_quad_distance_function(const vil_image_view<srcT> & src,
                           double ai,
                           double aj,
                           ...
                           vil_image_view<posT> & pos)
{
  ...
  std::ptrdiff_t t_istep = tmp.istep(), t_jstep = tmp.jstep();
  std::ptrdiff_t d_istep = dest.istep(), d_jstep = dest.jstep();
  std::ptrdiff_t tp_istep = tmp_pos.istep(), tp_jstep = tmp_pos.jstep();
  std::ptrdiff_t p_istep = pos.istep(), p_jstep = pos.jstep();
  ...
  for (unsigned int i = 0; i < ni;
       ++i, d_col += d_istep, t_col += t_istep, pi_col += p_istep, pj_col += p_istep, tp_col += tp_istep)
  {
    vil_quad_envelope(t_col, t_jstep, nj, x, y, z, aj);
