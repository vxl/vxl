...
void
vil_structuring_element::set(const std::vector<int> & v_p_i, const std::vector<int> & v_p_j)
{
  ...
  for (unsigned int k = 1; k < v_p_i.size(); ++k)
  {
    if (v_p_i[k] < min_i_)
      min_i_ = v_p_i[k];
    else if (v_p_i[k] > max_i_)
      max_i_ = v_p_i[k];

    if (v_p_j[k] < min_j_)
      min_j_ = v_p_j[k];
    else if (v_p_j[k] > max_j_)
      max_j_ = v_p_j[k];
  }
}
...
void
vil_structuring_element::set_to_line_i(int ilo, int ihi)
{
...
void
vil_structuring_element::set_to_line_j(int jlo, int jhi)
{
...
std::ostream &
operator<<(std::ostream & os, const vil_structuring_element & element)
{
  os << "Bounds [" << element.min_i() << ',' << element.max_i() << "][" << element.min_j() << ',' << element.max_j()
     << "] Points: ";
