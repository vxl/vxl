...
public:
  ...
  vil_structuring_element(const std::vector<int> & v_p_i, const std::vector<int> & v_p_j) { set(v_p_i, v_p_j); }

  //: Define elements { (p_i[k],p_j[k]) }
  void
  set(const std::vector<int> & v_p_i, const std::vector<int> & v_p_j);

  //: Set to disk of radius r
  //  Select pixels in disk s.t. x^x+y^y<r^r
  void
  set_to_disk(double r);

  //: Set to line along i (ilo,0)..(ihi,0)
  void
  set_to_line_i(int ilo, int ihi);

  //: Set to line along j (jlo,0)..(jhi,0)
  void
  set_to_line_j(int jlo, int jhi);
...
std::ostream &
operator<<(std::ostream &, const vil_structuring_element & element);
