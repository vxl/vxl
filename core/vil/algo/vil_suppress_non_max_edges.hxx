                           ...
                           vil_image_view<destT> & grad_mag)
{
  ...
  for (unsigned j = 2; j <= jhi; ++j, gi_row += gi_jstep, gj_row += gj_jstep, gm_row += gm_jstep)
  {
    const srcT * pgi = gi_row;
    const srcT * pgj = gj_row;
    destT * pgm = gm_row;
    for (unsigned i = 2; i <= ihi; ++i, pgi += gi_istep, pgj += gj_istep, pgm += gm_istep)
    {
...
double
interpolate_parabola(double y_1, double y_0, double y_2, double & y_peak)
{
