...
vil2_image_view<vil2_byte> transpose_view(const vil2_image_view<vil2_byte>& v)
{
  // Create view with i and j switched
  return vil2_image_view<vil2_byte>(v.memory_chunk(),v.top_left_ptr(),
                                    v.nj(),v.ni(),v.nplanes(),
...
int main(int argc, char** argv)
{
  unsigned ni=9;
  unsigned nj=8;
  vil2_image_view<vil2_byte> image(ni,nj);

  // Slow fill
  for (unsigned j=0;j<nj;++j)
    for (unsigned i=0;i<ni;++i)
      image(i,j) = vil2_byte(i+10*j);
  ...
  vil2_image_view<vil2_byte> transpose = transpose_view(image);
