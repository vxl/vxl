...
int main (int argc, char **argv)
{
	...
	levels = vcl_atoi (argv[2]) ;
      ...
      vcl_cout << "Usage: test_generate_pyramid_images.exe <input_file_name>"
	       << " <num_levels>"
	       << vcl_endl ;
      exit (-1) ;
    }
    ...
    vcl_cout << "input opened OK." << vcl_endl ;

    input_stream->ref() ;
    input_stream->seek (0) ;  // RESET STREAM TO BEGINNING

    vcl_cout << "create vil_nitf_file_format" << vcl_endl ;
    vil_nitf_file_format * file_format = new vil_nitf_file_format() ;

    vcl_cout << "call vil_nitf_file_format::make_input_image" << vcl_endl ;
	...
	vcl_cout << method_name << "vil_nitf_image parameters: " << vcl_endl ;
	vcl_cout << "    ni = " << nitf_image->ni()
		 << "  nj = " << nitf_image->nj() 
		 << "  nplanes = " << nitf_image->nplanes() 
		 << vcl_endl ;
	vcl_cout << "    pixel_format = " << nitf_image->pixel_format() ;
	vcl_cout << "  vil_property_quantisation_depth = " << bits_per_component
		 << vcl_endl ;
	...
	if (temp_nitf_image == 0) {
	    vcl_cerr << method_name << "dynamic cast to vil_nitf_image failed." << vcl_endl ;
	}
	else {
