...
public:
   ...
   */
  virtual CNCSError
  Open(vil_stream * stream, bool bWrite = false);

  virtual CNCSError
  Close();

  virtual bool NCS_FASTCALL
  Seek();

  virtual bool NCS_FASTCALL
  Seek(INT64 offset, Origin origin = CURRENT);

  virtual INT64 NCS_FASTCALL
  Tell();
  ...
  virtual bool NCS_FASTCALL
  Read(void * buffer, UINT32 count);

  virtual bool NCS_FASTCALL
  Write(void * buffer, UINT32 count);
