...
vil_bmp_file_format::make_input_image(vil_stream * vs)
{
...
vil_bmp_image::vil_bmp_image(vil_stream * vs, unsigned nx, unsigned ny, unsigned nplanes, vil_pixel_format format)
  : is_(vs)
  , bit_map_start(-1L)
{
  if (format != VIL_PIXEL_FORMAT_BYTE)
  {
    vil_exception_warning(
      vil_exception_pixel_formats_incompatible(VIL_PIXEL_FORMAT_BYTE, format, "vil_bmp_image::vil_bmp_image"));
  ...
  {
    vil_exception_warning(
      vil_exception_unsupported_operation("vil_bmp_image::vil_bmp_image: invalid number of planes"));
    // std::cerr << "Sorry -- pixel format " << format << " not yet supported\n";
    return;
  }
...
vil_bmp_image::read_header()
{
  ...
  else if (core_hdr.header_size == static_cast<unsigned>(vil_bmp_core_header::disk_size) + static_cast<unsigned>(vil_bmp_info_header::disk_size))
  {
...
bool
vil_bmp_image::write_header()
{
#ifdef DEBUG
  std::cerr << "Writing BMP header\n"
            << ni() << 'x' << nj() << '@' << nplanes() << 'x' << vil_pixel_format_sizeof_components(pixel_format())
            << '\n';
#endif
  ...
  // core_hdr.width = ni();
  // core_hdr.height = nj();
  core_hdr.bitsperpixel = nplanes() * vil_pixel_format_sizeof_components(pixel_format()) * 8;
...
vil_bmp_image::get_copy_view(unsigned x0, unsigned nx, unsigned y0, unsigned ny) const
{
  ...
  {
    vil_exception_warning(vil_exception_corrupt_image_file("vil_bmp_image::get_copy_view", "BMP", ""));
    return nullptr;
  }
  ...
  return new vil_image_view<vxl_byte>(buf,
                                      reinterpret_cast<vxl_byte *>(buf->data()) + top_left_plane0_in_mem +
                                        top_left_y0_in_mem,
                                      nx,
                                      ny,
                                      np,
...
bool
vil_bmp_image::put_view(const vil_image_view_base & view, unsigned x0, unsigned y0)
{
  ...
  {
    vil_exception_warning(
      vil_exception_pixel_formats_incompatible(VIL_PIXEL_FORMAT_BYTE, view.pixel_format(), "vil_bmp_image::put_view"));
    return false;
  }
  ...
  if ((view2.planestep() == -1 || nplanes() == 1) && view2.istep() == (int)view2.nplanes())
  {
  ...
  else if (nplanes() == 3)
  {
    ...
    for (unsigned i = rowlen; i < rowlen + padlen; ++i)
      buf[i] = 0;
