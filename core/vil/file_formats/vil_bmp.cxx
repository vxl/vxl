  ...
  is_(vs), bit_map_start(-1L)
{
  if( format != VIL_PIXEL_FORMAT_BYTE )
    {
    vil_exception_warning(vil_exception_pixel_formats_incompatible(VIL_PIXEL_FORMAT_BYTE, format,
  ...
  unsigned x0, unsigned nx, unsigned y0, unsigned ny) const
{
  ...
  if( nx == ni() && want_bytes_per_raster == have_bytes_per_raster )
    ...
    for( unsigned i = 0; i < ny; ++i )
      ...
      bytes_read += is_->read(
          reinterpret_cast<vxl_byte *>(buf->data() ) + want_bytes_per_raster * i, want_bytes_per_raster);
      }
    }
                             ...
                             unsigned x0, unsigned y0)
{
  ...
  if( view.pixel_format() != VIL_PIXEL_FORMAT_BYTE )
    {
    vil_exception_warning(vil_exception_pixel_formats_incompatible(VIL_PIXEL_FORMAT_BYTE, view.pixel_format(),
                                                                   "vil_bmp_image::put_view") );
    return false;
    }
