// This is mul/vil2/file_formats/vil2_bmp.cxx
...
{
  return new vil2_bmp_image(vs, nx, ny, nplanes, format);
}

char const* vil2_bmp_format_tag = "bmp";

char const* vil2_bmp_file_format::tag() const
{
...
char const* vil2_bmp_image::file_format() const
{
  return vil2_bmp_format_tag;
}
...
bool vil2_bmp_image::read_header()
{
...
bool vil2_bmp_image::write_header()
{
#ifdef DEBUG
  ...
  if (nplanes() == 1)
    info_hdr.colorcount = info_hdr.colormapsize = 1<<
      vil2_pixel_format_sizeof_components(pixel_format()) * 8;
  ...
  if (nplanes() == 1) // Need to write a colourmap in this case
    for (int i=0; i<(1<<vil2_pixel_format_sizeof_components(pixel_format())*8); ++i)
      for (int j=0; j<4; ++j)
      {
        unsigned char c = i;
        is_->write(&c,1L);
...
//------------------------------------------------------------
vil2_image_view_base_sptr vil2_bmp_image::get_copy_view(
  unsigned x0, unsigned nx, unsigned y0, unsigned ny) const
{
  ...
  if (nx == ni())
  {
    ...
    is_->read((vxl_byte *)buf->data(), want_bytes_per_raster *ny);
  }
  else
  {
  ...
  return new vil2_image_view<vxl_byte>(buf,
    (vxl_byte *)buf->data()+(ny-1)*have_bytes_per_raster + nplanes()-1,
    nx, ny, nplanes(),
    nplanes(), -have_bytes_per_raster, -1);
}


bool vil2_bmp_image::put_view(const vil2_image_view_base& view,
