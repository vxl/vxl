...
vil_dicom_file_format::make_input_image(vil_stream * vs)
{
  bool is_dicom = false;
...
vil_image_resource_sptr
vil_dicom_file_format::make_output_image(vil_stream * /*vs*/,
                                         unsigned /*ni*/,
                                         unsigned /*nj*/,
                                         unsigned /*nplanes*/,
                                         vil_pixel_format)
...
unsigned
vil_dicom_image::nplanes() const
{
  return header_.pix_samps_;
}

unsigned
vil_dicom_image::ni() const
{
  return header_.size_x_;
}

unsigned
vil_dicom_image::nj() const
{
  return header_.size_y_;
}

enum vil_pixel_format
vil_dicom_image::pixel_format() const
{
  return pixels_ ? pixels_->pixel_format() : VIL_PIXEL_FORMAT_UNKNOWN;
}

vil_image_view_base_sptr
vil_dicom_image::get_copy_view(unsigned x0, unsigned nx, unsigned y0, unsigned ny) const
{
  return pixels_ ? pixels_->get_copy_view(x0, nx, y0, ny) : 0;
}

vil_image_view_base_sptr
vil_dicom_image::get_view(unsigned x0, unsigned nx, unsigned y0, unsigned ny) const
{
  return pixels_ ? pixels_->get_view(x0, nx, y0, ny) : 0;
}

bool
vil_dicom_image::put_view(const vil_image_view_base & view, unsigned x0, unsigned y0)
{
...
void
vil_dicom_image::correct_manufacturer_discrepancies()
{
...
{
  static void
  proc(DcmObject * dset, vxl_uint_16 group, vxl_uint_16 element, std::string & value)
  {
...
template <>
struct try_set<vil_dicom_header_AE> : public try_set_to_string
{};

template <>
struct try_set<vil_dicom_header_AS> : public try_set_to_string
{};

template <>
struct try_set<vil_dicom_header_AT> : public try_set_to_string
{};

template <>
struct try_set<vil_dicom_header_CS> : public try_set_to_string
{};
...
{
  static void
  proc(DcmObject * dset, vxl_uint_16 group, vxl_uint_16 element, long & value)
  {
    DcmElement * e = find_element(dset, group, element);
    if (e)
    {
...
{
  static void
  proc(DcmObject * dset, vxl_uint_16 group, vxl_uint_16 element, float & value)
  {
    DcmElement * e = find_element(dset, group, element);
    if (e)
    {
  ...
  static void
  proc(DcmObject * dset, vxl_uint_16 group, vxl_uint_16 element, std::vector<float> & value)
  {
    DcmElement * e = find_element(dset, group, element);
    if (e)
    {
...
{
  static void
  proc(DcmObject * dset, vxl_uint_16 group, vxl_uint_16 element, vxl_ieee_64 & value)
  {
    DcmElement * e = find_element(dset, group, element);
    if (e)
    {
...
{
  static void
  proc(DcmObject * dset, vxl_uint_16 group, vxl_uint_16 element, vxl_ieee_32 & value)
  {
    DcmElement * e = find_element(dset, group, element);
    if (e)
    {
...
template <>
struct try_set<vil_dicom_header_LO> : public try_set_to_string
{};

template <>
struct try_set<vil_dicom_header_LT> : public try_set_to_string
{};

template <>
struct try_set<vil_dicom_header_OB> : public try_set_to_string
{};

template <>
struct try_set<vil_dicom_header_OW> : public try_set_to_string
{};

template <>
struct try_set<vil_dicom_header_PN> : public try_set_to_string
{};

template <>
struct try_set<vil_dicom_header_SH> : public try_set_to_string
{};
...
{
  static void
  proc(DcmObject * dset, vxl_uint_16 group, vxl_uint_16 element, vxl_sint_32 & value)
  {
    DcmElement * e = find_element(dset, group, element);
    if (e)
    {
...
template <>
struct try_set<vil_dicom_header_SQ> : public try_set_to_string
{};
...
{
  static void
  proc(DcmObject * dset, vxl_uint_16 group, vxl_uint_16 element, vxl_sint_16 & value)
  {
    DcmElement * e = find_element(dset, group, element);
    if (e)
    {
...
template <>
struct try_set<vil_dicom_header_ST> : public try_set_to_string
{};
...
template <>
struct try_set<vil_dicom_header_UI> : public try_set_to_string
{};
...
{
  static void
  proc(DcmObject * dset, vxl_uint_16 group, vxl_uint_16 element, vxl_uint_32 & value)
  {
    DcmElement * e = find_element(dset, group, element);
    if (e)
    {
...
template <>
struct try_set<vil_dicom_header_UN> : public try_set_to_string
{};
...
{
  static void
  proc(DcmObject * dset, vxl_uint_16 group, vxl_uint_16 element, vxl_uint_16 & value)
  {
    DcmElement * e = find_element(dset, group, element);
    if (e)
    {
  ...
  static void
  proc(DcmObject * dset, vxl_uint_16 group, vxl_uint_16 element, std::vector<vxl_uint_16> & value)
  {
    DcmElement * e = find_element(dset, group, element);
    if (e)
    {
      ...
      for (unsigned long pos = 0; pos < e->getVM(); ++pos)
      {
        ...
        {
          std::cerr << "vil_dicom Warning: value of (" << group << ',' << element << ") at " << pos
                    << " is not Uint16\n";
        }
        else
        {
...
template <>
struct try_set<vil_dicom_header_UT> : public try_set_to_string
{};
...
#  define ap_el(X) ap_join(VIL_DICOM_HEADER_, X)
  ...
  try_set<ap_type(ST)>::proc(
    f, group, ap_el(IDINSTITUTIONADDRESS), i.institution_addr_); // It's the institution address
  try_set<ap_type(PN)>::proc(
    f, group, ap_el(IDREFERRINGPHYSICIAN), i.ref_phys_name_);                     // It's the referring physician's name
  ...
  try_set<ap_type(PN)>::proc(
    f, group, ap_el(IDATTENDINGPHYSICIAN), i.att_phys_name_); // It's the name of the attending physician
  try_set<ap_type(PN)>::proc(
    f, group, ap_el(IDOPERATORNAME), i.operator_name_); // It's the name of the scanner operator
  ...
  try_set<ap_type(DS)>::proc(
    f, group, ap_el(AQMAGNETICFIELDSTRENGTH), i.mag_field_strength_);            // It's the magnetic field strength
  ...
  try_set<ap_type(IS)>::proc(
    f, group, ap_el(AQCARDIACNUMBEROFIMAGES), i.card_num_images_);                 // It's the cardiac number of images
  try_set<ap_type(IS)>::proc(f, group, ap_el(AQTRIGGERWINDOW), i.trigger_window_); // It's the trigger window
  try_set<ap_type(DS)>::proc(
    f, group, ap_el(AQRECONTRUCTIONDIAMETER), i.reconst_diameter_);                // It's the reconstruction diameter
  try_set<ap_type(SH)>::proc(f, group, ap_el(AQRECEIVINGCOIL), i.receiving_coil_); // It's the receiving coil
  try_set<ap_type(CS)>::proc(
    f, group, ap_el(AQPHASEENCODINGDIRECTION), i.phase_enc_dir_);                 // It's the phase encoding direction
  ...
  try_set<ap_type(UI)>::proc(
    f, group, ap_el(RSFRAMEOFREFERENCEUID), i.frame_of_ref_); // It's the frame of reference uid
  try_set<ap_type(IS)>::proc(
    f, group, ap_el(RSIMAGESINACQUISITION), i.images_in_acq_); // the number of images in the acquisition
  ...
  try_set<ap_type(CS)>::proc(
    f, group, ap_el(IMPHOTOMETRICINTERP), i.photo_interp_);          // It's the photometric interpretation
  ...
  try_set<ap_type(US)>::proc(
    f, group, ap_el(IMSMALLIMPIXELVALUE), i.small_im_pix_val_); // It's the smallest image pixel value
  try_set<ap_type(US)>::proc(
    f, group, ap_el(IMLARGEIMPIXELVALUE), i.large_im_pix_val_); // It's the largest image pixel value
  try_set<ap_type(US)>::proc(
    f, group, ap_el(IMPIXELPADDINGVALUE), i.pixel_padding_val_);                     // It's the pixel padding value
  ...
  if (ps.size() <= 0)
  {
    ...
    else if (ps_ips.size() > 0)
    {
      ...
      // std::cout<<"DICOM tags y.PixelSpacing and y.ImagerPixelSpacing missing: use y.PixelSpacing =
      // x.ImagerPixelSpacing."<<std::endl;
    }
  }

  group = VIL_DICOM_HEADER_PROCEDUREGROUP;
  try_set<ap_type(FD)>::proc(
    f, group, ap_el(PRREALWORLDVALUEINTERCEPT), i.real_world_value_intercept_); // It's the real world intercept value
  try_set<ap_type(FD)>::proc(
  ...
  try_set<ap_type(DS)>::proc(f,
                             group,
                             ap_el(NSPHILIPSPRIVATEINTERCEPT),
                             i.philips_private_intercept_); // It's the Philips private intercept value
  try_set<ap_type(DS)>::proc(
    f, group, ap_el(NSPHILIPSPRIVATESLOPE), i.philips_private_slope_); // It's the Philips private slope value
...
swap_short(unsigned short v)
{
  return (v << 8) | (v >> 8);
}

static void
swap_shorts(unsigned short * ip, unsigned short * op, int count)
{
  while (count)
  {
    *op++ = swap_short(*ip++);
