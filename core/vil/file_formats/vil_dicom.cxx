...
static
void read_header( DcmObject* dataset, vil_dicom_header_info& i );

static
void read_pixels_into_buffer(DcmPixelData* pixels, unsigned num_samples, Uint16 alloc, Uint16 stored, Uint16 high,
                             Uint16 rep, Float64 slope, Float64 intercept, vil_memory_chunk_sptr& out_buf,
    ...
    if( e )
      ...
      for( unsigned long pos = 0; pos < e->getVM(); ++pos )
        ...
        if( e->getUint16( value_at_pos, pos ) != EC_Normal )
          {
          vcl_cerr << "vil_dicom Warning: value of (" << group << ',' << element << ") at " << pos
                   << " is not Uint16\n";
          }
        else
          {
