...
public:
  ...
  virtual vil_image_resource_sptr
  make_output_image(vil_stream * vs, unsigned ni, unsigned nj, unsigned nplanes, vil_pixel_format format);
};
...
public:
  ...
  virtual vil_image_view_base_sptr
  get_view(unsigned i0, unsigned ni, unsigned j0, unsigned nj) const;
  ...
  file_format() const;
  bool
  get_property(const char * tag, void * prop = 0) const;

  // Dicom specific stuff
  const vil_dicom_header_info &
  header() const
  {
    return header_;
  }
  ...
  virtual bool
  interpret_hologic_header(float & xpixSize, float & ypixSize);
};
