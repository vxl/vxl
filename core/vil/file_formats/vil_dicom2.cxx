// This is core/vil/file_formats/vil_dicom2.cxx
...
char const* vil_dicom2_format_tag = "dicom2";


vil_image_resource_sptr vil_dicom2_file_format::make_input_image(vil_stream* vs)
{
  bool is_dicom = false;
                                                                 ...
                                                                 vil_pixel_format )
{
  ...
  {
    vcl_cerr << "ERROR: vil_dicom2_file_format::make_output_image\n"
             << "       Can only create DICOM images with a single plane\n"
             << "       and 32-bit integer pixels\n";
    return 0;
  }
  return new vil_dicom2_image(vs, ni, nj, nplanes, format);
#endif
}

char const* vil_dicom2_file_format::tag() const
{
  return vil_dicom2_format_tag;
}
    ...
    img_( 0 )
{
  ...
  dcis_ = new vil_dicom2_stream_input( vs );
  ffmt_ = new DcmFileFormat;
  
  ffmt_->transferInit();
  OFCondition cond = ffmt_->read( *dcis_ );
  ffmt_->transferEnd();
  ...
  read_header( ffmt_, header_ );

  img_ = new DicomImage( ffmt_, EXS_Unknown );

  if( img_->getStatus() != EIS_Normal ) {
    vcl_cerr << "vil_dicom2 ERROR: could not form image. status=" << img_->getStatus() << "\n";
    delete img_;
    img_ = 0;
    return;
  }

  switch( img_->getPhotometricInterpretation() ) {
  ...
  default:
    vcl_cerr << "vil_dicom2 ERROR: can't handle photometric interpretation="
             << img_->getPhotometricInterpretation() << "\n";
    return;
  }
}


bool vil_dicom2_image::get_property(char const* tag, void* value) const
{
  if (vcl_strcmp(vil_property_quantisation_depth, tag)==0)
  {
    if (value)
      *static_cast<unsigned int*>(value) = img_->getDepth();
    return true;
  }
...
char const* vil_dicom2_image::file_format() const
{
  return vil_dicom2_format_tag;
}
...
vil_dicom2_image::~vil_dicom2_image()
{
  delete img_;
  delete ffmt_;
  delete dcis_;
...
unsigned vil_dicom2_image::nplanes() const
{
  return header_.pix_samps_;
}

unsigned vil_dicom2_image::ni() const
{
  return header_.size_x_;
}

unsigned vil_dicom2_image::nj() const
{
  return header_.size_y_;
}


enum vil_pixel_format vil_dicom2_image::pixel_format() const
{
  unsigned bytes_read;
...
vil_image_view_base_sptr vil_dicom2_image::get_copy_view(
  unsigned x0, unsigned nx, unsigned y0, unsigned ny) const
{
  ...
  vil_memory_chunk_sptr buf = new vil_memory_chunk(ni() * nj() * nplanes() * bytes_per_comp, fmt );

  // this will write the pixels into the buffer. Non-monochrome images
  // are written in component order.
  if( !img_->getOutputData( buf->data(), buf->size() ) ) {
    vcl_cerr << "vil_dicom2 ERROR: failed to put output in " << buf->size() << " byte buffer\n";
    return 0;
  }
...
#define DOCASE( fmt )                                                   \
      case fmt: {                                                       \
        ...
        return new vil_image_view<T>( buf, (T*)buf->data(),             \
                                      ni(), nj(), nplanes(),            \
