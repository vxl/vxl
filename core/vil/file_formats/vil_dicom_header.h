...
const vxl_uint_16 VIL_DICOM_HEADER_EXPOSEDAREA = 0x0303;               // US
...
#define vr_macro(VR, CPPT)            \
  template <>                         \
  struct vil_dicom_header_type_of<VR> \
  {                                   \
    typedef CPPT type;                \
  }
...
struct vil_dicom_header_info
{
  ...
  vil_dicom_header_type_of<vil_dicom_header_UI>::type
    sop_cl_uid_; /*< The class unique id for the Service/Object Pair */
  ...
  std::vector<vil_dicom_header_type_of<vil_dicom_header_DS>::type>
    image_pos_; /*< The image position relative to the patient */
  std::vector<vil_dicom_header_type_of<vil_dicom_header_DS>::type>
    image_orient_; /*< The image orientation relative to the patient */
  ...
  vil_dicom_header_type_of<vil_dicom_header_US>::type exposedarea_x_;              /*< The exposed area in x */
  vil_dicom_header_type_of<vil_dicom_header_US>::type exposedarea_y_;              /*< The exposed area in y */

  // Non-standard info from Philips scanner
  vil_dicom_header_type_of<vil_dicom_header_DS>::type
    philips_private_intercept_;                                               /*< The philips private intercept value */
...
private:
  ...
  determineFileType(vil_stream & fs);
  ...
  void
  readHeaderElements(vil_stream & fs);
  ...
  void
  readIdentifyingElements(short element, int dblock_size, vil_stream & fs);
  ...
  //      readImageElements(), readDelimiterElements() and last_read_()
  void
  readPatientElements(short element, int dblock_size, vil_stream & fs);
  ...
  //      readImageElements(), readDelimiterElements() and last_read_()
  void
  readAcquisitionElements(short element, int dblock_size, vil_stream & fs);
  ...
  //    and last_read_()
  void
  readRelationshipElements(short element, int dblock_size, vil_stream & fs);
  ...
  //    and last_read_()
  void
  readImageElements(short element, int dblock_size, vil_stream & fs);
  ...
  //    and last_read_()
  void
  readDelimiterElements(short element, int dblock_size, vil_stream & fs);
  ...
  //    and last_read_()
  void
  readProcedureElements(short element, int dblock_size, vil_stream & fs);
  ...
  //    and last_read_()
  void
  readNSPhilipsElements(short element, int dblock_size, vil_stream & fs);
  ...
  bool
  convertValueRepresentation(unsigned int & dblock_size, vil_stream & ifs);
  ...
  //  \sa readHeaderElements()
  bool
  pixelDataFound(short group, short element);
  ...
  vil_dicom_header_endian
  determineMetaInfo(vil_stream & fs);
  ...
  vxl_uint_16
  shortSwap(vxl_uint_16 short_in);
  ...
  void
  charSwap(char * char_in, int val_size);
