...
bool
vil_geotiff_header::PCS_WGS84_UTM_zone(int & zone, GTIF_HEMISPH & hemisph) // hemisph is O for N, 1 for S
{
  modeltype_t type;
...
bool
vil_geotiff_header::PCS_NAD83_UTM_zone(int & zone, GTIF_HEMISPH & hemisph)
{
  modeltype_t type;
  if (gtif_modeltype(type) && type == ModelTypeProjected)
  {
...
bool
vil_geotiff_header::get_key_value(geokey_t key, void ** value, int & size, int & length, tagtype_t & type)
{
