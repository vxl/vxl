...
static vxl_sint_16
get_short(vil_stream * file, int location = -1); // default -1 means: read at current position
static vxl_uint_16
get_ushort(vil_stream * file, int location = -1);
static char
get_char(vil_stream * file, int location = -1);
static vxl_sint_32
get_long(vil_stream * file, int location = -1);
static void
send_char(vil_stream * data, char s);
static void
send_short(vil_stream * data, vxl_sint_16 s);
static void
send_ushort(vil_stream * data, vxl_uint_16 s);
static void
send_long(vil_stream * data, vxl_sint_32 s);
static void
  ...
  , colormap_(0)
{
  ...
  if (vil_pixel_format_sizeof_components(format) <= 2)
  {
    ...
    std::cerr << __FILE__ ": Cannot write iris image, which needs 8 or 16 bits per component\n";
}
...
bool
vil_iris_generic_image::read_header()
{
  is_->seek(0L);
...
vil_iris_generic_image::get_copy_view(unsigned int x0, unsigned int xs, unsigned int y0, unsigned int ys) const
{
...
vxl_sint_16
get_short(vil_stream * file, int location)
{
  if (location >= 0)
    file->seek(location);
...
char
get_char(vil_stream * file, int location)
{
  if (location >= 0)
    file->seek(location);
...
vxl_uint_16
get_ushort(vil_stream * file, int location)
{
  if (location >= 0)
    file->seek(location);
...
vxl_sint_32
get_long(vil_stream * file, int location)
{
  if (location >= 0)
    file->seek(location);

  vxl_byte buff[4];
  if (file->read((void *)buff, 4L) != 4L)
    return 0;
