...
//      Jul-2000 Peter Vanroose - bug fixed in write_header() by adding extra argument to constructor to pass imagename_
//      member
...
public:
  ...
  vil_image_resource_sptr
  make_output_image(vil_stream * vs, unsigned int ni, unsigned int nj, unsigned int planes, vil_pixel_format format)
    override;
};
...
public:
  vil_iris_generic_image(vil_stream * is, const char * imagename = "");
  ...
  get_copy_view(unsigned int x0, unsigned int ni, unsigned int y0, unsigned int nj) const override;
  bool
  put_view(const vil_image_view_base & buf, unsigned int x0, unsigned int y0) override;
...
protected:
  ...
  vil_image_view_base_sptr
  get_section_rle(unsigned int x0, unsigned int ni, unsigned int y0, unsigned int nj) const;

  // Read a plain section
  vil_image_view_base_sptr
  get_section_verbatim(unsigned int x0, unsigned int ni, unsigned int y0, unsigned int nj) const;
};
