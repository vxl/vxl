                                               ...
                                               unsigned int output_height) const
{
  ...
  NCSError setViewError = mFileResource->SetView( nBands, bandMap, output_width, output_height,
                                                  (INT32)sample0, (INT32)line0, (INT32)(sample0 + num_samples - 1),
  ...
  void* * linePtrPtr = (void * *)vcl_malloc(
      nBands * sizeof( int * /*all pointers have same size, so eg char* would work too*/ ) );
  ...
  for( unsigned int currLine = 0; currLine < output_height; currLine++ )
    ...
    for( int currBand = 0; currBand < nBands; currBand++ )
      {
      linePtrPtr[currBand] =
        (void *) ( ( (char *)data_ptr->data() ) + currLine * allBandLineSizeBytes + currBand
                   * singleBandLineSizeBytes );
      }
    NCSEcwReadStatus readStatus = mFileResource->ReadLineBIL(
        mFileResource->GetFileInfo()->eCellType, nBands, linePtrPtr, 0);
...
bool vil_j2k_image::put_view(const vil_image_view_base& im)
{
...
bool vil_j2k_image::view_fits(const vil_image_view_base& im, unsigned i0, unsigned j0)
{
