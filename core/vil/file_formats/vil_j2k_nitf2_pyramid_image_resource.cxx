...
unsigned
vil_j2k_nitf2_pyramid_image_resource::nplanes() const
{
  unsigned ret = 0;
  if (ptr_)
    ret = ptr_->nplanes();
...
unsigned
vil_j2k_nitf2_pyramid_image_resource::ni() const
{
  unsigned ret = 0;
  if (ptr_)
    ret = ptr_->ni();
...
// This method refers to the base (max resolution) image
unsigned
vil_j2k_nitf2_pyramid_image_resource::nj() const
{
  unsigned ret = 0;
  if (ptr_)
    ret = ptr_->nj();
...
//: Pixel Format.
vil_pixel_format
vil_j2k_nitf2_pyramid_image_resource::pixel_format() const
{
  if (ptr_)
...
const char *
vil_j2k_nitf2_pyramid_image_resource::file_format() const
{
  return "j2k_nitf2";
}
...
unsigned
vil_j2k_nitf2_pyramid_image_resource::nlevels() const
{
  if (!ptr_)
    return 0;
...
vil_j2k_nitf2_pyramid_image_resource::get_copy_view(unsigned i0,
                                                    unsigned n_i,
                                                    unsigned j0,
                                                    unsigned n_j,
...
//: for debug purposes
void
vil_j2k_nitf2_pyramid_image_resource::print(const unsigned level)
{}
