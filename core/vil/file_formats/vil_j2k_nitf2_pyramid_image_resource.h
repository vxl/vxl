...
{
public:
  vil_j2k_nitf2_pyramid_image_resource(const vil_image_resource_sptr & nitf2);
  ...
  get_copy_view(unsigned i0, unsigned n_i, unsigned j0, unsigned n_j, unsigned level) const;
  ...
  virtual vil_image_view_base_sptr
  get_copy_view(unsigned i0, unsigned n_i, unsigned j0, unsigned n_j, const float scale, float & actual_scale) const;
  ...
  virtual bool
  put_resource(const vil_image_resource_sptr & resc)
  {
    return false;
  }
  ...
  get_resource(const unsigned level) const;

  //: for debug purposes
  virtual void
  print(const unsigned level);
