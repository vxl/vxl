...
vil_jpeg_init_destination(j_compress_ptr cinfo)
{
  ...
  dest->buffer =
    (JOCTET *)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, vil_jpeg_OUTPUT_BUF_SIZE * SIZEOF(JOCTET));
...
vil_jpeg_stream_dst_set(j_compress_ptr cinfo, vil_stream * vs)
{
  ...
  // allocate
  auto dest = (vil_jpeg_dstptr)(*cinfo->mem->alloc_small)(
    (j_common_ptr)cinfo, JPOOL_PERMANENT, SIZEOF(vil_jpeg_stream_destination_mgr));
...
{
  auto dst = (vil_jpeg_dstptr)(cinfo->dest);
  { // verify
    assert(dst != nullptr);
