// This is core/vil2/file_formats/vil2_jpeg_destination_mgr.cxx
...
typedef vil2_jpeg_stream_destination_mgr *vil2_jpeg_dstptr;
...
vil2_jpeg_init_destination (j_compress_ptr cinfo) {
  vil2_jpeg_dstptr dest = (vil2_jpeg_dstptr) cinfo->dest; // cast to derived class

  // Allocate the output buffer --- it will be released when done with image
  dest->buffer = (JOCTET *)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo,
                                JPOOL_IMAGE,
                                vil2_jpeg_OUTPUT_BUF_SIZE * SIZEOF(JOCTET));

  dest->base.next_output_byte = dest->buffer;
  dest->base.free_in_buffer = vil2_jpeg_OUTPUT_BUF_SIZE;
}
...
vil2_jpeg_empty_output_buffer (j_compress_ptr cinfo) {
  ...
  if (dest->stream->write(dest->buffer, vil2_jpeg_OUTPUT_BUF_SIZE) != (vcl_size_t) vil2_jpeg_OUTPUT_BUF_SIZE)
...
vil2_jpeg_term_destination (j_compress_ptr cinfo) {
  vil2_jpeg_dstptr dest = (vil2_jpeg_dstptr) cinfo->dest; // cast to derived class
  vcl_size_t datacount = vil2_jpeg_OUTPUT_BUF_SIZE - dest->base.free_in_buffer;
...
vil2_jpeg_stream_dst_set (j_compress_ptr cinfo, vil2_stream *vs) {
  ...
  // allocate
  vil2_jpeg_dstptr dest = (vil2_jpeg_dstptr)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo,
                                JPOOL_PERMANENT,
  ...
  dest->base.empty_output_buffer = vil2_jpeg_empty_output_buffer;
  dest->base.term_destination    = vil2_jpeg_term_destination;
...
vil2_jpeg_stream_dst_rewind(j_compress_ptr cinfo, vil2_stream *vs) {
  ...
  cinfo->dest->free_in_buffer = vil2_jpeg_OUTPUT_BUF_SIZE;
