...
enum
{
  vil_jpeg_INPUT_BUF_SIZE = 4096 // choose an efficiently fread'able size
};
...
{
  auto src = (vil_jpeg_srcptr)(cinfo->src);
...
vil_jpeg_stream_src_set(j_decompress_ptr cinfo, vil_stream * vs)
{
  ...
  auto src = (vil_jpeg_srcptr) // allocate
    (*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_PERMANENT, SIZEOF(vil_jpeg_stream_source_mgr));
  ...
  src->buffer = (JOCTET *)(*cinfo->mem->alloc_small)(
    (j_common_ptr)cinfo, JPOOL_PERMANENT, vil_jpeg_INPUT_BUF_SIZE * SIZEOF(JOCTET));
...
{
  // verify
  assert((vil_jpeg_srcptr)(cinfo->src) != nullptr);
