...
vil_mit_file_format::make_input_image(vil_stream * is)
{
  is->seek(0L);
  if (is->file_size() < 8L)
    return nullptr;
...
vil_mit_image::read_header()
{
  ...
  {
    components_ = 1;
    format_ = bpp == 32 ? VIL_PIXEL_FORMAT_FLOAT : bpp == 64 ? VIL_PIXEL_FORMAT_DOUBLE : VIL_PIXEL_FORMAT_UNKNOWN;
  }
  else
    return false;
...
bool
vil_mit_image::write_header()
{
  is_->seek(0L);

  if (format_ == VIL_PIXEL_FORMAT_UINT_32 || format_ == VIL_PIXEL_FORMAT_UINT_16 || format_ == VIL_PIXEL_FORMAT_BYTE ||
  ...
  else if (format_ == VIL_PIXEL_FORMAT_INT_32 || format_ == VIL_PIXEL_FORMAT_INT_16 ||
  ...
  else if (format_ == VIL_PIXEL_FORMAT_RGB_UINT_32 || format_ == VIL_PIXEL_FORMAT_RGB_INT_32 ||
           format_ == VIL_PIXEL_FORMAT_RGB_UINT_16 || format_ == VIL_PIXEL_FORMAT_RGB_INT_16 ||
           format_ == VIL_PIXEL_FORMAT_RGB_BYTE || format_ == VIL_PIXEL_FORMAT_RGB_SBYTE)
  {
    if (components_ == 1)
      type_ = MIT_RGB;
    else
  ...
  else if (format_ == VIL_PIXEL_FORMAT_RGB_FLOAT || format_ == VIL_PIXEL_FORMAT_RGB_DOUBLE)
  {
    if (components_ == 1)
      type_ = MIT_FLOAT;
    else
      std::cerr << __FILE__ " : Can only write grayscale float-pixel MIT images\n"
                << " (format=" << format_ << ", #components=" << components_ << ")\n";
  }
  else
    std::cerr << __FILE__ " : Can only write RGB or grayscale MIT images\n"
...
vil_mit_image::get_copy_view(unsigned int x0, unsigned int xs, unsigned int y0, unsigned int ys) const
{
  assert(x0 + xs <= ni_);
  assert(y0 + ys <= nj_);
  ...
  if (VXL_BIG_ENDIAN && bytes_per_pixel() > 1)
  { // MIT image data is little-endian
...
#define ARGS(T) buf, reinterpret_cast<T *>(buf->data()), xs, ys, components_, components_, xs *components_, 1
  if (format_ == VIL_PIXEL_FORMAT_BOOL)
    return new vil_image_view<bool>(ARGS(bool));
  else if (format_ == VIL_PIXEL_FORMAT_BYTE)
    return new vil_image_view<vxl_byte>(ARGS(vxl_byte));
  else if (format_ == VIL_PIXEL_FORMAT_SBYTE)
    return new vil_image_view<vxl_sbyte>(ARGS(vxl_sbyte));
  else if (format_ == VIL_PIXEL_FORMAT_UINT_16)
    return new vil_image_view<vxl_uint_16>(ARGS(vxl_uint_16));
  else if (format_ == VIL_PIXEL_FORMAT_INT_16)
    return new vil_image_view<vxl_int_16>(ARGS(vxl_int_16));
  else if (format_ == VIL_PIXEL_FORMAT_UINT_32)
    return new vil_image_view<vxl_uint_32>(ARGS(vxl_uint_32));
  else if (format_ == VIL_PIXEL_FORMAT_INT_32)
    return new vil_image_view<vxl_int_32>(ARGS(vxl_int_32));
  else if (format_ == VIL_PIXEL_FORMAT_FLOAT)
    return new vil_image_view<float>(ARGS(float));
  else if (format_ == VIL_PIXEL_FORMAT_DOUBLE)
    return new vil_image_view<double>(ARGS(double));
  else
    return nullptr;
#undef ARGS
}
...
vil_mit_image::put_view(const vil_image_view_base & buf, unsigned int x0, unsigned int y0)
{
  ...
  if (ibuf.istep() == int(components_) && ibuf.jstep() == int(components_ * ni) &&
    ...
    buf_is_planar = false;
  else if (ibuf.istep() == 1 && ibuf.jstep() == int(ni) && (ibuf.planestep() == int(ni * nj) || components_ == 1))
    buf_is_planar = true;
  else
  {
    std::cerr << "ERROR: " << __FILE__ << ":\n"
              << " view does not fit: istep=" << ibuf.istep() << ", jstep=" << ibuf.jstep()
              << ", planestep=" << ibuf.planestep() << " instead of " << components_ << ',' << components_ * ni << ','
              << 1 << " or 1," << ni << ',' << ni * nj << '\n';
