// Source
// Eventually this will be core/vil/file_formats/vil_nitf.cxx ???
//
// $Revision: 1.1 $ 
// $Date: 2003/12/26 00:19:10 $
// $Author: mlaymon $
...
//:
// Implementation of methods for class vil_nitf.
// \file
...
//  bits_per_component_ = vil_pixel_format_sizeof_components (format) ;
...
//    setvil_stream (in_stream_, ifilePos_) ;
        ...
        image_subheader->ICORDS == vil_nitf_image_subheader::GEOGRAPHIC)
    {
      vcl_cerr << method_name << "WARNING: ICORDS = " << image_subheader->ICORDS
	       << vcl_endl ;
      vcl_cerr << "    Code to set GeoPt values in"
	       << " super-class Image not implemented." << vcl_endl ;
    ...
    if (image_subheader && (length = image_subheader->get_data_length()) == 0) {
      vcl_cerr << method_name << "WARNING: data length not set in image subheader."
	       << vcl_endl ;
    }

//     SetStatusGood() ;  // This method is on super-class Tag (through super-class Image of class NITFImage.  See if we actually need status attribute in this class.
    ...
    unsigned nj) const
{
    ...
    vcl_cout << method_name << "image size from get_image_data_length (0) = " 
	     << message_header_->get_image_data_length (0) << vcl_endl ;

    vcl_cout << method_name << "bits_per_pixel = " << get_bits_per_pixel() << vcl_endl ;
    vcl_cout << method_name << "bits_per_component = " << get_bits_per_component() << vcl_endl ;

    vcl_cout << method_name << "pixel_format    = " << pixel_format() << vcl_endl ;
    vcl_cout << method_name << "bytes_per_pixel = " << bytes_per_pixel << vcl_endl ;
    vcl_cout << method_name << "total_bytes = " << total_bytes << vcl_endl ;

    vcl_cout << method_name << "origin = (" << i0 << ", " << j0 << ")" << vcl_endl ;
    vcl_cout << method_name << "size = (" << ni << ", " << nj << ", " 
	     << this->nplanes() << ")" 
	     << vcl_endl ;
    vcl_cout << method_name << "block_size_x = " << get_block_size_x() << vcl_endl ;

    if (image_subheader_->NBANDS == 1) {
      image_view = get_single_band_view (i0, ni, j0, nj) ;
    }
    else {
      vcl_cerr << method_name << "WARNING: logic for NBANDS != 1 not implemented." << vcl_endl ;
    }
    ...
    unsigned nj) const
{
    ...
    unsigned long last_bytes_read = 0 ;
    ...
    vcl_cout << method_name 
	     ...
	     << vcl_endl ;
    vcl_cout << "  block_buffer = " << &block_buffer 
	     << vcl_endl ;
    ...
    vcl_cout << method_name << "start_block_x = " << start_block_x 
	     << "  start_block_x_offset = " << start_block_x_offset
	     << vcl_endl ;
    last_image_col_num = (max_block_x * get_block_size_x()) - 1 ;
    vcl_cout << method_name 
	     << "  max_block_x = " << max_block_x
	     << "  expected last X pixel = " << last_image_col_num
	     << vcl_endl ;
	...
	// CALCULATE OFFSET TO CURRENT BLOCK IN BLOCK ROW
	  ...
	  if (bytes_read != bytes_per_block) {
	    ...
	    vcl_cerr << method_name << "WARNING: "
		     ...
		     << " less than requested = " << bytes_per_read
		     << vcl_endl ;
	    continue ;
	  }
	  if (reverse_bytes()) {
	  ...
	  if ((image_row_offset + within_row_offset) >= total_bytes) {
	    ...
	    vcl_cout << "block_row = " << block_row
		     << "  start_block_y = " << start_block_y
		     << "  block_col = " << block_col
		     << vcl_endl ;
	    vcl_cout << "  row = " << row
		     << "  image_row_num = " << image_row_num
		     << "  within_row_offset = " << within_row_offset
		     << vcl_endl ;
	    vcl_cout << "  image_offset = " << (image_row_offset + within_row_offset)
      ...
      vcl_cout << "last_image_row_num = " << last_image_row_num ;
      vcl_cout << "  last_image_col_num = " << last_image_col_num
	       << vcl_endl ;

      vcl_cout << "blocks_read = " << blocks_read << vcl_endl ;
    ...
    vcl_cout << "##### " << method_name << "finish read loops" << vcl_endl ;

    tms finish ;
    times (&finish) ;

    vcl_cout << "finish reading input = "
             << finish.tms_utime << vcl_endl ;
    ...
    vcl_cout << "buffers read = " << read_count 
	     << "  expected value = " << expected_read_count
	     << vcl_endl ;
    vcl_cout << method_name << "total bytes read = " << total_read_count << vcl_endl ;
    vcl_cout << method_name << "last_image_row_num = " << last_image_row_num
	     << vcl_endl ;
    vcl_cout << method_name << "buffer->size = " << buffer->size()
	     << vcl_endl ;
    vcl_cout << "last_image_offset = " << last_image_offset
...
//		  + buffer->size() - DEFAULT_PIXELS_TO_DISPLAY ;
    ...
    vcl_cout << "buffer size - DEFAULT_PIXELS_TO_DISPLAY = " 
	     << (buffer->size() - DEFAULT_PIXELS_TO_DISPLAY)
	     << vcl_endl ;
    vcl_cout << "last_image_offset + bytes_per_read - DEFAULT_PIXELS_TO_DISPLAY = "
    ...
    vcl_string dir_name)
{
    ...
    vcl_cout << method_name << "vil_image_view parameters:" << vcl_endl ;
    vcl_cout << "    ni = " << image_view->ni()
	     << "  nj = " << image_view->nj()
	     << "  nplanes = " << image_view->nplanes() 
	     << vcl_endl ;
    vcl_cout << "    pixel_format = " << image_view->pixel_format()
	     << "  bits_per_component from get_property = " << bits_per_component
	     << vcl_endl ;
    vcl_cout << method_name << "current directory = " << vul_file::get_cwd() << vcl_endl ;
    ...
    if (dot_pos == vcl_string::npos) {
        vcl_cout << "No dot in file name." << vcl_endl ;
    }
    else {
      if (dot_pos > 0) {
	...
	vcl_cout << method_name << "save_view parameters:" << vcl_endl ;
	vcl_cout << "    ni = " << save_view->ni()
		 << "  nj = " << save_view->nj()
	...
	vcl_cout << "Saved image as 2-byte PNG to file <" << out_file_name
		 << "> using vil_save.  Success = " << write_success
		 << vcl_endl ;
#endif
...
//====================================================================
// -- Method to set padding at the top of the Image.  The equation
...
//====================================================================
// -- Method to set padding at the bottom of the Image.  The equation
// top_pad_ + nj_ + bottom_pad_ == get_num_blocks_y() * get_block_size_y()
// must hold.  
// NOTE: this is in PIXELS, not bytes.
//====================================================================
...
//====================================================================
// -- Method to set the size in BYTES that each line of each block of
...
int vil_nitf_image::check_image_data_offset() const
{
    ...
    // so file postion should be equal to sum of length of these two headers.
	...
	vcl_cout << method_name << "WARNING:" << vcl_endl ;
	vcl_cout << "    file postion and calculated offset differ by " 
		 << diff << " bytes." << vcl_endl ;
	vcl_cout << "    Using current file position." << vcl_endl ;

	vcl_cout << "    message header length = "
		 << message_header_->GetHeaderLength()
		 << vcl_endl ;
	vcl_cout << "    image header length   = "
		 << message_header_->IMAGESH[0]->LISH
		 << vcl_endl ;
	vcl_cout << "    sum           = " << header_length_sum
		 << vcl_endl ;
	vcl_cout << "    file_position = " << get_image_data_offset()
		 << vcl_endl ;
    }
    else {
	vcl_cout << method_name << "file postion and calculated header offset agree"
...
void vil_nitf_image::display_message_attributes (vcl_string caller)
{
    ...
    if (message_header_ == static_cast<vil_nitf_message_header_sptr> (0)) {
        vcl_cout << method_name << "ERROR: Cannot message attributes.  Message header is null." << vcl_endl ;
    }
    else {
...
void vil_nitf_image::display_image_attributes (vcl_string caller)
{
    ...
    if (image_subheader_ == static_cast<vil_nitf_image_subheader_sptr> (0)) {
        vcl_cout << method_name << "ERROR: Cannot display image attributes.  Image subheader is null." << vcl_endl ;
    }
    else {
      image_subheader_->display_size_attributes (caller) ; 
      vcl_cout << "bits_per_component = " << this->bits_per_component_ << vcl_endl ;
      vcl_cout << "pixel_format = " << this->pixel_format() << vcl_endl ;
 ...
 * @param bytes_per_value number of bytes per numeric value
 *
 * @return true if any bits other than 3 least significant bits in first byte 
 *    are being used.
 */
    ...
    unsigned block_col) const
{
	...
	if ((uint_val > 0) && (uint_val != masked_val)) {
	  ...
	  vcl_cout << method_name 
		   ...
		   << vcl_endl ;
	  vcl_cout << vcl_hex << "  mask = " << byte_mask
		   << "  value = " << vcl_hex
		   << uint_val << "  masked value = "
		   << masked_val << vcl_endl ;
    ...
    unsigned int pixel_count) const
{
    ...
    for (unsigned int i = 0 ; i < pixel_count ; ++i) {
	  ...
	  vcl_cout << "ERROR: upper five bits should be zero." << vcl_endl ;
	  vcl_cout << method_name << "i = " << i
	  ...
	  vcl_cout << "ERROR: value from image and value from buffer not equal." << vcl_endl ;
	  vcl_cout << method_name << "i = " << i
		   << "   view value = " << vcl_hex << view_pixel_value
...
//      vxl_uint_16 new_value = reverse_bytes (view_pixel_value) ;
    ...
    vcl_cout << method_name << "invalid error count = " << invalid_error_count
	     << "  value error count = " << value_error_count
	     << vcl_endl ;
    ...
    unsigned int pixel_count) const
{
    ...
    for (unsigned int i = 0 ; i < pixel_count ; ++i) {
	  ...
	  vcl_cout << "ERROR: value from read_buffer and value from memory chunk not equal." << vcl_endl ;
	  vcl_cout << method_name << "row = " << row << "  column = " << offset
		   << "   view value = " << vcl_hex << view_pixel_value
		   << "   buffer value = " << buffer_pixel_value
		   << vcl_dec << vcl_endl ;
...
void vil_nitf_image::display_block_attributes (vcl_string caller)
{
    ...
    vcl_cout << "num blocks X = " << get_num_blocks_x() 
	     ...
	     << vcl_endl ;
    vcl_cout << "num blocks Y = " << get_num_blocks_y() 
	     << "  block size Y = " << get_block_size_y() 
	     << vcl_endl ;

    vcl_cout << "right pad  = " << get_right_pad() << vcl_endl ;
    vcl_cout << "bottom pad = " << get_bottom_pad() << vcl_endl ;
}
...
{
    long elapsed = (end.tms_utime - start.tms_utime) * (1000 / CLK_TCK) ;
    if (msg.length() > 0) {
      vcl_cout << msg << " - " ;
    }
    vcl_cout << "elapsed milli-seconds = " << elapsed
	     << vcl_endl ;
}
    ...
    vcl_string label) const
{
  static vcl_string method_name = "compare_bytes: " ;
	...
	vcl_cout << "ERROR: Difference at position " << count 
		 << "  buffer 1 byte = "
		 << vcl_hex << temp_short ;
	temp_short = (unsigned short) buffer_2[count] ;
	vcl_cout << "  buffer 2 byte = "
		 << temp_short << vcl_dec
		 << vcl_endl ;
