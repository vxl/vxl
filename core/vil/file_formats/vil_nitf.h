// Source
// Eventually this will be core/vil/file_formats/vil_nitf.h ???
...
class vil_nitf_image : public vil_image_resource
{
 ...
 public:
  ...
  virtual unsigned int get_bits_per_component() const {return bits_per_component_ ;}  /// Return actual bits per pixel ( = ABPP in NITF image header)
  ...
  vxl_uint_16 check_max_value (unsigned char * buffer, unsigned long buf_len, unsigned int bytes_per_value, unsigned block_row, unsigned block_col) const ;
  ...
  void static display_elapsed_time (const tms &start, const tms &end, const vcl_string &msg) ;
