...
vil_nitf2_classification::type_field_defs_map &
vil_nitf2_classification::s_field_definitions()
{
    ...
    case V_NITF_21:
      (*defs)
        ...
        .field(tp + "SDG",
               np + "Downgrade",
               NITF_ENUM(
                 1, vil_nitf2_enum_values().value("S", "Secret").value("C", "Confidential").value("R", "Restricted")),
               true,
               nullptr,
               nullptr)
        // this field def is only used for NITF2.1, so we can use NitfDT
        ...
        .field(tp + "SCATP",
               np + "Classification Authority Type",
               NITF_ENUM(1,
                         vil_nitf2_enum_values()
