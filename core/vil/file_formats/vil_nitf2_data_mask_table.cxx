...
vil_nitf2_data_mask_table::parse(vil_stream * stream)
{
  ...
  if (stream->read((void *)(&IMDATOFF), 4) != 4 || stream->read((void *)(&BMRLNTH), 2) != 2 ||
      stream->read((void *)(&TMRLNTH), 2) != 2 || stream->read((void *)(&TPXCDLNTH), 2) != 2)
  {
    return false;
  }
  else
  ...
  if (BMRLNTH != 0)
  {
    ...
    for (i = 0; i < num_blocks_x; i++)
    {
      for (j = 0; j < num_blocks_y; j++)
      {
        ...
        {
          if (stream->read((void *)(&BMR_n_BND_m[i][j][b]), 4) != 4)
            return false;
  ...
  if (TMRLNTH != 0)
  {
    ...
    for (i = 0; i < num_blocks_x; i++)
    {
      for (j = 0; j < num_blocks_y; j++)
      {
        ...
        {
          if (stream->read((void *)(&TMR_n_BND_m[i][j][b]), 4) != 4)
            return false;
...
vxl_uint_32
vil_nitf2_data_mask_table::blocked_image_data_offset() const
{
  return IMDATOFF;
}
...
void
vil_nitf2_data_mask_table::maybe_endian_swap(char * a, unsigned size_of_a_in_bytes, vil_pixel_format pix_format)
{
#if VXL_LITTLE_ENDIAN
...
void
vil_nitf2_data_mask_table::maybe_endian_swap(char * a, unsigned size_of_a_in_bytes, unsigned int bytesPerSample)
{
#if VXL_LITTLE_ENDIAN
