...
{
public:
  vil_nitf2_data_mask_table(unsigned int numBlocksX, unsigned int numBlocksY, unsigned int numBands, std::string imode);
  bool
  parse(vil_stream * stream);

  //: If this function returns true, then you may call \sa block_band_offset()
  bool
  has_offset_table() const
  {
    return !BMR_n_BND_m.empty();
  }

  //: If this function returns true, then you may call \sa pad_pixel()
  bool
  has_pad_pixel_table() const
  {
    return !TMR_n_BND_m.empty();
  }

  vxl_uint_32
  blocked_image_data_offset() const;
  ...
  //  blank.
  vxl_uint_32
  block_band_present(unsigned int block_x, unsigned int block_y, int band = -1) const;
  ...
  vxl_uint_32
  block_band_offset(unsigned int block_x, unsigned int block_y, int band = -1) const;

  //:
  // band argument is ignored if imode != "S"... i.e. all bands have the same
  // pad pixel in that case
  vxl_uint_32
  pad_pixel(unsigned int block_x, unsigned int block_y, int band) const;

  vxl_uint_32
  block_band_has_pad(unsigned int block_x, unsigned int block_y, int band = -1) const;

  static void
  maybe_endian_swap(char * a, unsigned sizeOfAInBytes, vil_pixel_format pixFormat);
  static void
  maybe_endian_swap(char * a, unsigned sizeOfAInBytes, unsigned int bytesPerSample);
