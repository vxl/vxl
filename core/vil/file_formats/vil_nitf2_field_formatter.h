...
public:
  ...
  read_field(vil_nitf2_istream & input, bool & out_blank) = 0;

  // Initializes a vector field of specified dimensionality
  virtual vil_nitf2_array_field *
  create_array_field(int num_dimensions, vil_nitf2_field_definition *) = 0;

  virtual bool
  write_field(vil_nitf2_ostream & output, const vil_nitf2_scalar_field * field) = 0;
  ...
  bool
  write_blank(std::ostream & output) const;

  // Same as above, but writes to a vil_stream.
  bool
  write_blank(vil_stream & output) const;
