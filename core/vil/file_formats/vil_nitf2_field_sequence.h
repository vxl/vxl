...
public:
  ...
  bool
  get_value(std::string tag, vil_nitf2_tagged_record_sequence & out_value) const;
#if VXL_HAS_INT_64
  bool
  ...
  bool
  get_value(std::string tag,
            const vil_nitf2_index_vector & indexes,
            int & out_value,
            bool ignore_extra_indexes = false) const;
  bool
  get_value(std::string tag,
            const vil_nitf2_index_vector & indexes,
            double & out_value,
  ...
  vil_nitf2_field *
  get_field(const std::string & tag) const;
  ...
  bool
  create_array_fields(const vil_nitf2_field_definitions * field_defs, int num_dimensions);

  // Recursively set the specified dimension for vector fields to repeat_count.
  void
  set_array_fields_dimension(const vil_nitf2_field_definitions * field_defs,
                             const vil_nitf2_index_vector & index,
                             int repeat_count);

  // Returns field definition if found
  vil_nitf2_field_definition *
  find_field_definition(const std::string & tag);
  ...
  get_tree(vil_nitf2_field::field_tree * tr = nullptr) const;
