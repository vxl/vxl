...
std::string
vil_nitf2_header::section_num_tag(section_type sec)
{
  switch (sec)
  {
    case enum_file_header:
      assert(0);
      return "";
    case enum_image_segments:
      return "NUMI";
    case enum_graphic_segments:
      return "NUMS";
    case enum_label_segments:
      return "NUML";
    case enum_text_segments:
      return "NUMT";
    case enum_data_extension_segments:
      return "NUMDES";
    case enum_reserved_extension_segments:
      return "NUMRES";
    default:
      return "";
  }
...
std::string
vil_nitf2_header::section_len_header_tag(section_type sec)
{
  switch (sec)
  {
    case enum_file_header:
...
std::string
vil_nitf2_header::section_len_data_tag(section_type sec)
{
  switch (sec)
  {
    case enum_file_header:
...
bool
vil_nitf2_header::read(vil_stream * stream)
{
  ...
  if (success)
  {
    if (m_field_sequence_classification)
      delete m_field_sequence_classification;
...
vil_nitf2_header::get_field_definitions_2(vil_nitf2_classification::file_version version)
{
  // Check if previously computed
  vil_nitf2_field_definitions * field_defs =
    (version == vil_nitf2_classification::V_NITF_20) ? s_field_definitions_20 : s_field_definitions_21;
  if (field_defs)
    return field_defs;
  ...
  {
    (*field_defs).field("ONAME", "Originator's Name", NITF_STR_ECSA(27), true, nullptr, nullptr);
  }
  else
  {
  ...
  (*field_defs)
    ...
    .field(
      "UDHDL", "User Defined Header Data Length", NITF_INT(5), false, nullptr, nullptr) // range [00000,00003-99999]
    ...
    .field("UDHD",
           ...
           NITF_TRES(),
           false,
           new vil_nitf2_max_field_value_plus_offset_and_threshold("UDHDL", -3),
           nullptr)
    ...
    .field("XHD",
           ...
           NITF_TRES(),
           false,
           new vil_nitf2_max_field_value_plus_offset_and_threshold("XHDL", -3),
           nullptr);
...
vil_nitf2_header::get_field_definitions_1()
{
  if (!s_field_definitions_1)
  {
    ...
    field_defs
      .field("FHDR",
             "File Profile Name",
             NITF_ENUM(4, vil_nitf2_enum_values().value("NITF", "NITF File").value("NSIF", "NSIF File")),
             false,
             nullptr,
             nullptr)
      ...
      .field("STYPE",
             "Standard Type",
             NITF_ENUM(4, vil_nitf2_enum_values().value("BF01", "ISO/IEC IS 12087-5")),
             true,
             nullptr,
             nullptr)
...
vil_nitf2_header::file_version() const
{
  std::string file_profile;
  std::string file_version;
  if (get_property("FHDR", file_profile) && get_property("FVER", file_version))
  {
    if (file_profile == "NSIF" && file_version == "01.00")
      return vil_nitf2_classification::V_NSIF_10;
    if (file_profile == "NITF" && file_version == "01.00")
      return vil_nitf2_classification::V_NITF_10;
    if (file_profile == "NITF" && file_version == "02.00")
      return vil_nitf2_classification::V_NITF_20;
    if (file_profile == "NITF" && file_version == "02.10")
      return vil_nitf2_classification::V_NITF_21;
  }
  return vil_nitf2_classification::V_UNKNOWN;
}

vil_nitf2_field::field_tree *
vil_nitf2_header::get_tree() const
{
  auto * t = new vil_nitf2_field::field_tree;
  t->columns.emplace_back("File Header");
  m_field_sequence1.get_tree(t);
  if (m_field_sequence_classification)
    m_field_sequence_classification->get_tree(t);
  if (m_field_sequence2)
    m_field_sequence2->get_tree(t);
  return t;
}
