...
bool vil_nitf2_image::parse_headers()
{
  ...
  for( int j = 0; j < num_des; j++ )
    {
    vil_streampos offset = get_offset_to( vil_nitf2_header::enum_data_extension_segments,
                                          vil_nitf2_header::enum_subheader, j);
    m_stream->seek(offset);
    int data_width;
  ...
  unsigned start_i, unsigned num_i, unsigned start_j, unsigned num_j, double i_factor, double j_factor ) const
{
  ...
  m_stream->seek(get_offset_to( vil_nitf2_header::enum_image_segments, vil_nitf2_header::enum_data,
...
template <>
void right_justify<bool>(bool * /* data */, unsigned int /* num_samples */, unsigned int /* bitsToMove */) {}
template <>
void right_justify<float>(float * /* data */, unsigned int /* num_samples */, unsigned int /* bitsToMove */) {}
template <>
void right_justify<double>(double * /* data */, unsigned int /* num_samples */, unsigned int /* bitsToMove */) {}
template <>
void right_justify<vcl_complex<float> >(vcl_complex<float> * /*data*/, unsigned int /*num_samples*/,
...
vil_image_view_base_sptr get_block_vcl_internal(vil_pixel_format pix_format, vil_memory_chunk_sptr image_memory,
                                                ...
                                                const vil_nitf2_image_subheader * /*image_header*/, T dummy)
{
  ...
  if( data_is_all_blank )
    ...
    if( need_to_right_justify )
      {
      right_justify<T>(static_cast<T *>(image_memory->data() ),
                       (unsigned int)(image_memory->size() / sizeof(T) ), extra_bits);
      }
    // Nitf files store data in big endian... little endian machines need to convert
    vil_nitf2_data_mask_table::maybe_endian_swap(static_cast<char *>(image_memory->data() ),
                                                 (unsigned int)(image_memory->size() ), pix_format);
...
vil_image_view_base_sptr vil_nitf2_image::get_block(unsigned int block_index_x, unsigned int block_index_y) const
{
  ...
  if( bits_per_band % 8 != 0 )
    {
    bytes_per_block_per_band++;                               // round up if remainder vcl_left over
    }
  unsigned int block_size_bytes = bytes_per_block_per_band * nplanes();
  ...
  if( image_mode_type == "S" )
    ...
    for( unsigned int i = 0; i < nplanes(); i++ )
      {
      vil_streampos current_offset = get_offset_to_image_data_block_band(m_current_image_index, block_index_x,
                                                                         block_index_y, i);
      if( current_offset == 0 )
        {
        ...
        if( m_stream->read( (void *)position_to_read_to,
                            bytes_per_block_per_band) != static_cast<int>(bytes_per_block_per_band) )
          {
          return 0;
          }
        }
