...
public:
  ...
  vil_image_resource_sptr
  make_output_image(vil_stream * vs, unsigned nx, unsigned ny, unsigned nplanes, enum vil_pixel_format) override;
...
public:
  ...
  get_copy_view_decimated_j2k(unsigned i0, unsigned ni, unsigned j0, unsigned nj, double i_factor, double j_factor)
    const;
  virtual vil_image_view_base_sptr
  get_copy_view_decimated_j2k(double i_factor, double j_factor) const
  {
  ...
  vil_image_view_base_sptr
  get_block(unsigned int blockIndexX, unsigned int blockIndexY) const override;

  bool
  get_property(const char * tag, void * property_value = nullptr) const override;

  // const vil_nitf2_header& getFileHeader() const;
  const std::vector<vil_nitf2_image_subheader *> &
  get_image_headers() const
  {
    return m_image_headers;
  }
  const vil_nitf2_header &
  get_header() const
  {
    return m_file_header;
  }
  const std::vector<vil_nitf2_des *> &
  get_des() const
  {
    return m_des;
  }
  ...
  virtual void
  set_current_image(unsigned int index);
...
protected:
  virtual vil_image_view_base_sptr
  get_block_j2k(unsigned int blockIndexX, unsigned int blockIndexY) const;
  virtual vil_image_view_base_sptr
  get_copy_view_uncompressed(unsigned i0, unsigned ni, unsigned j0, unsigned nj) const;
  ...
  // you call this.
  vil_streampos
  get_offset_to(vil_nitf2_header::section_type sec, vil_nitf2_header::portion_type por, unsigned int index = 0) const;
  ...
  vil_streampos
  size_to(vil_nitf2_header::section_type sec, vil_nitf2_header::portion_type por, int index) const;
  ...
  // I'll return 0;
  vil_streampos
  get_offset_to_image_data_block_band(unsigned int imageIndex,
...
template <class T>
T
get_bits(const T * in_val, unsigned int i0, unsigned int ni)
{
...
template <class T>
T *
byte_align_data(T * in_data, unsigned int num_samples, unsigned int in_bits_per_sample, T * out_data)
{
