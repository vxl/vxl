...
vil_nitf2_image_subheader::vil_nitf2_image_subheader(vil_nitf2_classification::file_version version)
  : m_field_sequence(version == vil_nitf2_classification::V_NITF_20 ? *get_field_definitions_20()
...
vil_nitf2_image_subheader::read(vil_stream * stream)
{
  ...
  if (success)
  {
    ...
    if (compression_code.find_first_of('M') != std::string::npos)
    {
      ...
      m_data_mask_table = new vil_nitf2_data_mask_table(get_num_blocks_x(), get_num_blocks_y(), nplanes(), imode);
      if (!m_data_mask_table->parse(stream))
        return false;
    }
  }
  return success;
...
vil_nitf2_image_subheader::get_field_definitions_21()
{
...
const vil_nitf2_field_definitions *
vil_nitf2_image_subheader::get_field_definitions_20()
{
...
void
vil_nitf2_image_subheader::add_shared_field_defs_3(vil_nitf2_field_definitions * defs)
{
  (*defs)
    .field("NICOM", "Number of Image Comments", NITF_INT(1), false, nullptr, nullptr)

    // TODO: does it make any sense for a std::vector to have blank entries????  For now, I'm saying no (false
    // parameter)
    .repeat("NICOM",
  ...
  (*defs)
    ...
    .repeat(new vil_nitf2_choose_field_value<int>(
            ...
            vil_nitf2_field_definitions()
              ...
              .repeat(new vil_nitf2_field_value<int>("NLUTS"),
                      vil_nitf2_field_definitions()
                        ...
                        .field("LUTDnm",
                               ...
                               NITF_BIN(1),
                               false,
                               new vil_nitf2_field_value<int>("NELUT"),
                               nullptr)))
    ...
    .field("UDID",
           ...
           NITF_TRES(),
           false,
           new vil_nitf2_max_field_value_plus_offset_and_threshold("UDIDL", -3),
           nullptr)
    ...
    .field("IXSHD",
           ...
           NITF_TRES(),
           false,
           new vil_nitf2_max_field_value_plus_offset_and_threshold("IXSHDL", -3),
           nullptr);
}
...
void
vil_nitf2_image_subheader::add_shared_field_defs_2(vil_nitf2_field_definitions * defs)
{
  (*defs)
    .field("ENCRYP",
           "Encryption",
    ...
    .field("PJUST",
           "Pixel Justification",
           NITF_ENUM(1, vil_nitf2_enum_values().value("L", "Left-justified").value("R", "Right-justified")),
           false,
           nullptr,
           nullptr);
}
...
void
vil_nitf2_image_subheader::add_shared_field_defs_1(vil_nitf2_field_definitions * defs)
{
  (*defs)
    .field("IM", "File Part Type", NITF_ENUM(2, vil_nitf2_enum_values().value("IM", "Image Header")))
...
unsigned
vil_nitf2_image_subheader::nplanes() const
{
...
std::string
vil_nitf2_image_subheader::get_image_source() const
{
  std::string source;
...
std::string
vil_nitf2_image_subheader::get_image_type() const
{
  std::string id;
...
#if 0
void
vil_nitf2_image_subheader::add_rsm_definitions()
{
...
vil_nitf2_image_subheader::add_ICHIPB_definitions()
{
  ...
  if (!tr)
  {
    vil_nitf2_tagged_record_definition::define("ICHIPB", "ICHIPB SUPPORT DATA EXTENSION")
      ...
      //: intelligent data
      .field(
        "OP_ROW_11", "Output product row number component of grid point index (1,1)", NITF_DBL(12, 3, false), false)
      .field(
        "OP_COL_11", "Output product column number component of grid point index (1,1)", NITF_DBL(12, 3, false), false)

      .field(
        "OP_ROW_12", "Output product row number component of grid point index (1,2)", NITF_DBL(12, 3, false), false)
      .field(
        "OP_COL_12", "Output product column number component of grid point index (1,2)", NITF_DBL(12, 3, false), false)

      .field(
        "OP_ROW_21", "Output product row number component of grid point index (2,1)", NITF_DBL(12, 3, false), false)
      .field(
        "OP_COL_21", "Output product column number component of grid point index (2,1)", NITF_DBL(12, 3, false), false)

      .field(
        "OP_ROW_22", "Output product row number component of grid point index (2,2)", NITF_DBL(12, 3, false), false)
      .field(
        "OP_COL_22", "Output product column number component of grid point index (2,2)", NITF_DBL(12, 3, false), false)

      //: full image coordinate system
      .field(
        "FI_ROW_11", "Output product row number component of grid point index (1,1)", NITF_DBL(12, 3, false), false)
      .field(
        "FI_COL_11", "Output product column number component of grid point index (1,1)", NITF_DBL(12, 3, false), false)

      .field(
        "FI_ROW_12", "Output product row number component of grid point index (1,2)", NITF_DBL(12, 3, false), false)
      .field(
        "FI_COL_12", "Output product column number component of grid point index (1,2)", NITF_DBL(12, 3, false), false)

      .field(
        "FI_ROW_21", "Output product row number component of grid point index (2,1)", NITF_DBL(12, 3, false), false)
      .field(
        "FI_COL_21", "Output product column number component of grid point index (2,1)", NITF_DBL(12, 3, false), false)

      .field(
        "FI_ROW_22", "Output product row number component of grid point index (2,2)", NITF_DBL(12, 3, false), false)
      .field(
        "FI_COL_22", "Output product column number component of grid point index (2,2)", NITF_DBL(12, 3, false), false)
...
vil_nitf2_image_subheader::get_correction_offset(double & u_off, double & v_off) const
{
  ...
  for (tres_itr = isxhd_tres.begin(); tres_itr != isxhd_tres.end(); ++tres_itr)
  {
    ...
    if (type == "ICHIPB")
    {
      ...
      if ((*tres_itr)->get_value("FI_ROW_11", r_off) && (*tres_itr)->get_value("FI_COL_11", c_off))
      {
        ulr += r_off;
        ulc += c_off;
      }
    ...
    else if (type == "STDIDC")
    {
      int r_off = 1;
      int c_off = 1;
      if ((*tres_itr)->get_value("START_ROW", r_off) && (*tres_itr)->get_value("START_COLUMN", c_off))
      {
