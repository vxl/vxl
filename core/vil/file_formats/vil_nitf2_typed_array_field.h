...
public:
  ...
  bool
  value(const vil_nitf2_index_vector & indexes, T & out_value) const override;

  //: Reads from input stream the scalar value at specified index.
  // check_index(indexes) must be true, or this will emit an error.
  // Returns success.
  bool
  read_vector_element(vil_nitf2_istream & input, const vil_nitf2_index_vector & indexes, int variable_width) override;
...
protected:
  ...
  // to output stream.
  void
  output_dimension_iterate(std::ostream & os, vil_nitf2_index_vector indexes, bool & output_yet) const;
...
template <class T>
bool
vil_nitf2_typed_array_field<T>::value(const vil_nitf2_index_vector & indexes, T & out_value) const
{
...
template <class T>
bool
vil_nitf2_typed_array_field<T>::read_vector_element(vil_nitf2_istream & input,
                                                    const vil_nitf2_index_vector & indexes,
                                                    int variable_width)
{
...
template <class T>
bool
vil_nitf2_typed_array_field<T>::write_vector_element(vil_nitf2_ostream & output,
                                                     const vil_nitf2_index_vector & indexes,
                                                     int variable_width) const
{
  ...
  T val;
  // To do: make this cast safe!
  vil_nitf2_typed_field_formatter<T> * typed_formatter = (vil_nitf2_typed_field_formatter<T> *)m_definition->formatter;
  if (variable_width > 0)
    typed_formatter->field_width = variable_width;
...
template <class T>
std::ostream &
vil_nitf2_typed_array_field<T>::output(std::ostream & os) const
{
...
template <class T>
void
vil_nitf2_typed_array_field<T>::output_dimension_iterate(std::ostream & os,
...
template <class T>
std::ostream &
operator<<(std::ostream & os, const vil_nitf2_typed_array_field<T> & field)
{
