...
public:
  ...
  create_array_field(int num_dimensions, vil_nitf2_field_definition * field_definition) override;

  // Returns a new field, read from stream.
  vil_nitf2_scalar_field *
  read_field(vil_nitf2_istream & input, bool & out_blank) override;

  // Writes scalar field to the specified stream.
  bool
  write_field(vil_nitf2_ostream & output, const vil_nitf2_scalar_field * field) override;
  ...
  virtual bool
  read(vil_nitf2_istream & input, T & out_value, bool & out_blank);

  // Same as above, but reads value from a std::istream (which, unlike a
  // vil_nitf2_istream, supports formatted I/O).
  virtual bool
  read_vcl_stream(std::istream & input, T & out_value, bool & out_blank);
...
template <typename T>
vil_nitf2_array_field *
vil_nitf2_typed_field_formatter<T>::create_array_field(int num_dimensions,
...
template <typename T>
vil_nitf2_scalar_field *
vil_nitf2_typed_field_formatter<T>::read_field(vil_nitf2_istream & input, bool & out_blank)
{
  T value;
...
template <typename T>
bool
vil_nitf2_typed_field_formatter<T>::write_field(vil_nitf2_ostream & output, const vil_nitf2_scalar_field * field)
{
  if (field)
  {
    T val;
...
template <typename T>
bool
vil_nitf2_typed_field_formatter<T>::read(vil_nitf2_istream & input, T & out_value, bool & out_blank)
{
...
template <typename T>
bool
vil_nitf2_typed_field_formatter<T>::read_vcl_stream(std::istream & /* input */,
                                                    T & /* out_value */,
                                                    bool & /* out_blank */)
{
  assert(0);
...
template <typename T>
bool
vil_nitf2_typed_field_formatter<T>::write_vcl_stream(std::ostream & /* output */, const T & /* value */)
{
  assert(0);
  return false;
}
...
template <typename T>
bool
vil_nitf2_typed_field_formatter<T>::write(vil_nitf2_ostream & output, const T & value)
{
...
public:
  ...
  bool
  read_vcl_stream(std::istream & input, int & out_value, bool & out_blank) override;
  bool
  write_vcl_stream(std::ostream & output, const int & value) override;
...
public:
  ...
  bool
  read_vcl_stream(std::istream & input, vil_nitf2_long & out_value, bool & out_blank) override;
  bool
  write_vcl_stream(std::ostream & output, const vil_nitf2_long & value) override;
...
public:
  ...
  bool
  read_vcl_stream(std::istream & input, double & out_value, bool & out_blank) override;
  bool
  write_vcl_stream(std::ostream & output, const double & value) override;
...
public:
  ...
  bool
  read_vcl_stream(std::istream & input, char & out_value, bool & out_blank) override;
  bool
  write_vcl_stream(std::ostream & output, const char & value) override;
};
...
public:
  ...
  // the stringstream.
  bool
  read(vil_nitf2_istream & input, void *& out_value, bool & out_blank) override;

  /// Overload to write() instead of write_vcl_stream() to write binary data
  // (see preceding comment).
  bool
  write(vil_nitf2_ostream & output, void * const & value) override;
};
...
public:
  ...
  bool
  read_vcl_stream(std::istream & input, std::string & out_value, bool & out_blank) override;
  bool
  write_vcl_stream(std::ostream & output, const std::string & value) override;

  virtual bool
  is_valid(std::string value) const;
...
public:
  ...
  bool
  is_valid_value(const std::string & value) const;
...
public:
  ...
  bool
  read_vcl_stream(std::istream & input, vil_nitf2_date_time & out_value, bool & out_blank) override;
  bool
  write_vcl_stream(std::ostream & output, const vil_nitf2_date_time & value) override;
};
...
public:
  ...
  bool
  read_vcl_stream(std::istream & input, vil_nitf2_location *& out_value, bool & out_blank) override;
  bool
  write_vcl_stream(std::ostream & output, vil_nitf2_location * const & value) override;

  // Converts overall field width to seconds precision for DMSH formatted
  // field, ddmmss.sssHdddmmss.sssH
  static int
  sec_precision(int field_width)
  {
    return (field_width - 17) / 2;
  }

  // Converts overall field width to degrees precision for degrees formatted
  // field, +dd.ddd+ddd.ddd
  static int
  deg_precision(int field_width)
  {
    return (field_width - 9) / 2;
  }
};
...
public:
  ...
  // the stringstream.
  bool
  read(vil_nitf2_istream & input, vil_nitf2_tagged_record_sequence & out_value, bool & out_blank) override;

  /// Overload to write() instead of write_vcl_stream() to write binary data
  // (see preceding comment).
  bool
  write(vil_nitf2_ostream & output, const vil_nitf2_tagged_record_sequence & value) override;
};
