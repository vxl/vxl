...
public:
  ...
  vil_nitf2_typed_scalar_field(T value, vil_nitf2_field_definition * definition)
    : vil_nitf2_scalar_field(definition)
  ...
  bool
  value(T & out_value) const override
  {
    out_value = m_value;
    return true;
  }
  ...
  {
    return vil_nitf2_scalar_field::get_tree();
  }
...
template <>
inline std::ostream &
vil_nitf2_typed_scalar_field<vil_nitf2_location *>::output(std::ostream & os) const
{
  if (m_value == 0)
  {
    os << m_value;
