...
//
// $Revision: 1.1 $ 
// $Date: 2003/12/26 00:19:43 $
// $Author: mlaymon $
//
// Description:	Implementation of extended headers included in an NITF message.
//
...
{
    if (!file || !file->ok() || XHDL < 0) return (STATUS_BAD) ;

    if (!XHDL) return (STATUS_GOOD) ;

    if (!XHD) return (STATUS_BAD) ;
    if (file->write (XHD, XHDL) < XHDL) return (STATUS_BAD) ;
...
int vil_nitf_extended_header::AsciiWrite (vil_stream* f)
{
    ...
    if (!XHDL) return (STATUS_GOOD);
    if (f->write (CRLF, strlen (CRLF)) < 1) return (STATUS_BAD);
