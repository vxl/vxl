...
// PMP added the following 48 lines for aggregation mode processing
      ...
      //  and cannot be used.
      if ((map_size != 346)) {
        vcl_cout << "MPD26A data determined to be invalid -- vil will ignore it.\n"
                 << "Therefore, vil cannot determine if this is a non-1x1 image.\n";
#ifdef DEBUG
        vcl_cout << "WARNING: If this is an anamorphic image, vil weirdness will ensue.\n";
#endif
      ...
      else
      {
        ...
        if ((mpd[45] != '1') || (mpd[47] != '1'))
        {
          ...
          ANAMRPH[1] = 1;
          if (debug_level > 0) {
            vcl_cout << "vil can process this image - it is isotropic (1x1)!\n";
          }
        }
...
void vil_nitf_image_subheader_v20::init()
{
  ...
  for (unsigned int n = 0; n < NBANDS; n++)
  {
    if (bands[n] != NULL)
    {
      ...
      {
        for (unsigned int m=0; m<bands[n]->NLUTS; m++)
        {
...
//char *rpc_tag = XSHD->XHD ? vcl_strstr(XSHD->XHD,"RPC00A") : 0;
  ...
  if (vcl_strncmp(work, "RPC00A", 6) == 0) {
    ...
    RPC_TYPE = UNDEFINED;
    vcl_cerr << "Encountered UNKNOWN RPC00x tag = <" << work << "> !\n";
    return error ? STATUS_BAD : STATUS_GOOD;
  }  // Unknown RPC tag!
