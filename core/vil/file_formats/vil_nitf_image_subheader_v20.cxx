...
//
// Description:	This file implements the NITF image subheader for NITF v2.0.
// NOTE: Ported from TargetJr ImageSubHeader_v20.  (Code in file NITFV20Header.C)
//
//  $Revision: 1.1 $ 
//  $Date: 2003/12/26 00:20:50 $
//  $Author: mlaymon $
//
 ...
 * @return StatusCode == STATUS_GOOD if succeed
 */
StatusCode vil_nitf_image_subheader_v20::Read (vil_stream* file)
{
  ...
  // MAKE SURE FLOAT AND COMPLEX BELOW SHOULD NOT BE DOUBLE.  MAL 8oct2003
	...
	if (!get_unsigned (buffer, &ABPP, 2, file) ||
            (ABPP < 1 || ABPP > 32)) {
	...
	vcl_cout << method_name << "clear ICOM_ OK." << vcl_endl ;
...
// PMP added the following 48 lines for aggregation mode processing
      ...
      if (mpd_tag == NULL) {
        vcl_cout << "Cannot determine aggregation mode" << vcl_endl;
  }// no MPD26A tag (cross your fingers!)
      else {
        // Compute byte offset in UDID to where the 'MPD26A' tag begins.
        int map_off = mpd_tag - XSHD->XHD ;
        vcl_cout << "MPD26A tag found in image's subheader." << vcl_endl;

        map_off += strlen ("MPD26A") ; // advance over extension type identifier
        ...
        //  and cannot be used.
        if ((map_size != 346)) {
          vcl_cout << "MPD26A data determined to be invalid -- TJ will ignore it." << vcl_endl;
          vcl_cout << "Therefore, TJ cannot determine if this is a non-1x1 image." << vcl_endl;
          // vcl_cout << "WARNING: If this is an anamorphic image, TJ-weirdness will ensue." << vcl_endl;
	}  // bad MPD26A data (cross your fingers tighter!)
        else {

          // Okay, the MPD26A data is good. We'll use it.
          vcl_cout << "MPD26A data determined to be valid -- using it." << vcl_endl;
          ...
          if ((mpd[45] != '1') || (mpd[47] != '1')) {
            ...
            vcl_cout << "This image is anamorphic!" << vcl_endl;
            char tmp_ana[2]; tmp_ana[1]=0; //Null terminated string
            tmp_ana[0] = mpd[45];
            ANAMRPH[0] = vcl_atoi (&tmp_ana[0]); 
            tmp_ana[0] = mpd[47];
            ANAMRPH[1] = vcl_atoi (&tmp_ana[0]);
            vcl_cout << "Anamorphic ratio: " << ANAMRPH[0] << "x" << ANAMRPH[1] << "." << vcl_endl;
            if ((ANAMRPH[0] == 5) || (ANAMRPH[0] > 6) || (ANAMRPH[0] == 0)) {
              vcl_cout << "Anamorphic coefficients determined to be invalid!" << vcl_endl;
              vcl_cout << "This NITF file should not be processed any further." << vcl_endl;
            ...
            ANAMRPH[0] = 1;
            ANAMRPH[1] = 1;
            vcl_cout << "TJ can process this image - it is isotropic (1x1)!" << vcl_endl;
	  }
      ...
      if (map_tag == NULL) {
        ...
        vcl_cout << "There is no I2MAPD tag in this image's subheader." << vcl_endl;

	// Removed return, since we can continue if there is an ICHIPB and RPC
        vcl_cout << "Hopefully there is an ICHIPB tag.  Continuing..." << vcl_endl;
      ...
      vcl_cout << "I2MAPD tag found in image's subheader." << vcl_endl;

      map_off += strlen ("I2MAPD") ; // advance over extension type identifier
      ...
      if ((map_size < 256) || (map_size > 3616)) {
        vcl_cout << "I2MAPD data determined to be invalid -- ignoring it." << vcl_endl;
        return(error ? STATUS_BAD : STATUS_GOOD);
      }  // bad I2MAPD data

      // Okay, the I2MAPD data is good. We'll use it.
      vcl_cout << "I2MAPD data determined to be valid -- using it." << vcl_endl;
      I2MAPD_present = true;
      ...
      if (itemp != 0) {
        vcl_cout << "This image is dewarped! (ODS shouldn't do this)." << vcl_endl;
        return(error ? STATUS_BAD : STATUS_GOOD);
      ...
      vcl_cout << "Parsed and stored I2MAPD data." << vcl_endl;
...
StatusCode vil_nitf_image_subheader_v20::Write (vil_stream* file)
{
	...
	if (RPC_present)
	{
		...
		imap << vcl_setw (8) << (1.0/pow(2.0,RRDSset));
		...
		imap << vcl_setw (12) << ULs+0.5;
		imap << vcl_setw (12) << URl+0.5;
		imap << vcl_setw (12) << URs+0.5;
		imap << vcl_setw (12) << LLl+0.5;
		imap << vcl_setw (12) << LLs+0.5;
		imap << vcl_setw (12) << LRl+0.5;
		imap << vcl_setw (12) << LRs+0.5;
		...
		if (ICHIPB_present)
		{
			...
			// int len_ichipb = strlen("ICHIPB") + 5;//The 5 is for CDATA
			// len_ichipb += 224;
	...
	while (!done)
{
        ...
        if ((file->write (ISCODE, 40) < 40) ||
            ...
            (NROWS < 0 || NROWS > NBPC * NPPBV)	||
            (!put_unsigned (buffer, NROWS, 8, file))	||
            (NCOLS < 0 || NCOLS > NBPR * NPPBH)	||
...
int
vil_nitf_image_subheader_v20::GetHeaderLength()
{
...
/*
 *  Intitial header values to default values.
...
StatusCode vil_nitf_image_subheader_v20::extract_rpc00x_extension()
{
  ...
  char * rpc_tag = XSHD->XHD ? vcl_strstr (XSHD->XHD, "RPC00") : 0 ;
      ...
      //  return the 'error' variable to the caller.
      if (rpc_tag == 0) {
        vcl_cout << "There is no RPC tag in this image's subheader." << vcl_endl;
        return (error ? STATUS_BAD : STATUS_GOOD) ;
      }// no RPC tag (there is great sadness)

      // Compute byte offset in XHD to where the 'RPC00X' tag begins.
      int offset = rpc_tag - XSHD->XHD;
      vcl_cout << "RPC tag found in image's subheader." << vcl_endl;

      //      offset += strlen("RPC00A"); // advance over extension type identifier
      ...
      if (vcl_strncmp (work, "RPC00A", 6) == 0) {
        ...
        RPC_TYPE = UNDEFINED ;
        vcl_cout << "Encountered UNKNOWN RPC00x tag!." << vcl_endl;
        return (error ? STATUS_BAD : STATUS_GOOD) ;
      }  // Unknown RPC tag!
      ...
      if ((vcl_atoi (work) != 1041) || (XSHD->XHD[offset] != '1')) {
        vcl_cout << "RPC data determined to be invalid -- ignoring it." << vcl_endl ;
        error = true ;
        return (error ? STATUS_BAD : STATUS_GOOD);
      }  // bad RPC data

      // Okay, the RPC data is good. We'll use it.
      vcl_cout << "RPC data determined to be valid -- using it." << vcl_endl ;
      ...
      // Whew!
      vcl_cout << "Parsed and stored RPC data." << vcl_endl ;
...
StatusCode vil_nitf_image_subheader_v20::extract_ichipb_extension()
{
  ...
  if (ICHIPB_tag == NULL) 
{
      vcl_cout << "There is no ICHIPB tag in this image's subheader." << vcl_endl ;
      return (error ? STATUS_BAD : STATUS_GOOD);
  ...
  vcl_cout << "ICHIPB tag found in image's subheader." << vcl_endl ;

  offset += strlen("ICHIPB"); // advance over extension type identifier
...
{
      vcl_cout << "ICHIPB data determined to be invalid -- ignoring it." << vcl_endl ;
      error = true;
        return (error ? STATUS_BAD : STATUS_GOOD);
}// bad ICHIPB data

  // Okay, the ICHIPB data is good. We'll use it.
  vcl_cout << "ICHIPB data determined to be valid -- using it." << vcl_endl ;
  ...
  offset = 0;  // Let's start clean at the beginning.
      ...
      SCALE_FACTOR = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      OP_ROW_11 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      OP_COL_11 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      OP_ROW_12 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      OP_COL_12 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      OP_ROW_21 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      OP_COL_21 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      OP_ROW_22 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      OP_COL_22 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      FI_ROW_11 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      FI_COL_11 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      FI_ROW_12 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      FI_COL_12 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      FI_ROW_21 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      FI_COL_21 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      FI_ROW_22 = vcl_atof (temp);         // store value into data structure
      ...
      temp[12] = 0;                   // ensure NULL terminator
      offset += 12;                   // advance to next field
      FI_COL_22 = vcl_atof (temp);         // store value into data structure
      ...
      // Whew!
      vcl_cout << "Parsed and stored ICHIPB data." << vcl_endl ;
...
StatusCode vil_nitf_image_subheader_v20::extract_piaimc_extension()
{
	...
	if (PIAIMC_tag == NULL) 
	{
		vcl_cout << "There is no PIAIMC tag in this image's subheader." << vcl_endl ;
		return (error ? STATUS_BAD : STATUS_GOOD);
	...
	vcl_cout << "PIAIMC tag found in image's subheader." << vcl_endl ;

	offset += strlen("PIAIMC"); // advance over extension type identifier
	...
	{
		vcl_cout << "PIAIMC data determined to be invalid -- ignoring it." << vcl_endl ;
		error = true;
		return (error ? STATUS_BAD : STATUS_GOOD);
	}// bad PIAIMC data

	// Okay, the PIAIMC data is good. We'll use it.
	vcl_cout << "PIAIMC data determined to be valid -- using it." << vcl_endl ;
	...
	temp[7] = 0;                                   // ensure NULL terminator
	offset += 7;                                   // advance to next field
	MEANGSD = vcl_atof (temp);                          // store value
	...
	// Whew!
	vcl_cout << "Parsed and stored PIAIMC data." << vcl_endl ;
...
StatusCode vil_nitf_image_subheader_v20::extract_stdid_extension()
{
    ...
    if (STDID_tag == NULL) {
	vcl_cout << "There is no STDIDx tag in this image's subheader." << vcl_endl ;
	status = false;
	return status;
    }
    ...
    offset += strlen("STDID"); // advance over the extension type ID

    // Find out what kind of STDID tag we have...
    char tag_type;
    vcl_strncpy(&tag_type,&XSHD->XHD[offset],1);  // extract the tag type (A,B,C?)
    vcl_cout << "STDID" << tag_type << " tag found in image's subheader." << vcl_endl ;
    offset++; 
...
static void es (vcl_ostringstream& s, double v)
{
    ...
    double l = log10 (fabs(v)) ;
    if (fabs(l) >= 10)
    {
	vcl_cout << "In vil_nitf_v20_header.C - invalid exponent" << vcl_endl ;
	s << "+0.000000E+0";
	return;
     }
    ...
    double x = v/pow (10.0, exp) ;  //gives a single digit 0-9 before .
  //could be 10.xxx etc so back off one
    if (fabs(x) > 9) { 
	exp++ ;
	x = v/pow (10.0, exp);
    }
    ss.setf (vcl_iostream::fixed, vcl_iostream::floatfield) ;  //fixed floating point
    ss.setf (vcl_iostream::showpos) ;  //leading +-
    ss.setf (vcl_iostream::showpoint) ;  //fill in trailing zeros
    ss << vcl_setw (9) << vcl_setprecision (6) << x ;   //use up 6 digits after the .
    ss << "E";
    ss.setf (vcl_iostream::dec, vcl_iostream::basefield) ;  //integer
    ss.setf (vcl_iostream::showpos) ;  //with leading +-
    ss << vcl_setw (2) << exp ;  //force to E+x
    ss << vcl_ends ;

    vcl_string temp_str = ss.str() ;
    s << temp_str ;

    // delete ss.str() ;  // MEMORY FROM UNDERLYING stringstream SHOULD BE RELEASED WHEN temp_str GOES OUT OF SCOPE ?? MAL 10oct2003
}
...
void vil_nitf_image_subheader_v20::encode_rpc00a_extension (vcl_ostringstream& rpc_buf)
{
	...
	rpc_buf << vcl_setw (5) << SAMP_OFF;
	rpc_buf << vcl_setw (8) << LAT_OFF;
	rpc_buf << vcl_setw (9) << LONG_OFF;
	rpc_buf << vcl_setw (5) << HEIGHT_OFF;
	
	// Write SCALES.
	rpc_buf << vcl_setw (6) << LINE_SCALE;
	rpc_buf << vcl_setw (5) << SAMP_SCALE;
	rpc_buf << vcl_setw (8) << vcl_setprecision (3) << LAT_SCALE;
	rpc_buf << vcl_setw (9) << vcl_setprecision (4) << LONG_SCALE;
	rpc_buf << vcl_setw (5) << HEIGHT_SCALE;
