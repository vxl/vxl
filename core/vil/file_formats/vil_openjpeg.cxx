...
vil_openjpeg_file_format ::make_input_image(vil_stream * vs, vil_openjpeg_format opjfmt)
{
...
vil_image_resource_sptr
vil_openjpeg_file_format ::make_output_image(vil_stream * /*vs*/,
                                             unsigned int /*ni*/,
                                             unsigned int /*nj*/,
...
private:
  ...
  static vxl_uint_32
  opj_vil_stream_read(void * p_buffer, vxl_uint_32 p_nb_bytes, void * p_user_data);
  static vxl_uint_32
  opj_vil_stream_write(void * p_buffer, vxl_uint_32 p_nb_bytes, void * p_user_data);
  static vxl_uint_32
  opj_vil_stream_skip(vxl_uint_32 p_nb_bytes, void * p_user_data);
  static bool
  opj_vil_stream_seek(vxl_uint_32 p_nb_bytes, void * p_user_data);
...
bool
vil_openjpeg_decoder ::init_from_stream(unsigned int reduction, void * stream)
{
...
bool
vil_openjpeg_decoder ::init_stream(void * stream)
{
  if (this->stream_)
  {
    opj_stream_destroy(this->stream_);
  ...
  opj_stream_set_read_function(this->stream_, vil_openjpeg_decoder::opj_vil_stream_read);
  opj_stream_set_write_function(this->stream_, vil_openjpeg_decoder::opj_vil_stream_write);
  opj_stream_set_skip_function(this->stream_, vil_openjpeg_decoder::opj_vil_stream_skip);
  opj_stream_set_seek_function(this->stream_, vil_openjpeg_decoder::opj_vil_stream_seek);
...
bool
vil_openjpeg_decoder ::init_decoder(unsigned int reduction)
{
  if (this->codec_)
  {
    opj_destroy_codec(this->codec_);
  ...
  opj_set_info_handler(this->codec_, vil_openjpeg_decoder::opj_event_info, this);
  opj_set_warning_handler(this->codec_, vil_openjpeg_decoder::opj_event_warning, this);
  opj_set_error_handler(this->codec_, vil_openjpeg_decoder::opj_event_error, this);
...
bool
vil_openjpeg_decoder ::set_decode_area(unsigned int x, unsigned int y, unsigned int w, unsigned int h)
{
  this->error_ = false;
...
bool
vil_openjpeg_decoder ::opj_vil_stream_seek(vxl_uint_32 p_nb_bytes, void * p_user_data)
{
  auto * stream = reinterpret_cast<vil_stream *>(p_user_data);
...
void
vil_openjpeg_decoder ::opj_event_info(const char * msg, void * /*data*/)
{
...
void
vil_openjpeg_decoder ::opj_event_warning(const char * msg, void * /*data*/)
{
...
void
vil_openjpeg_decoder ::opj_event_error(const char * msg, void * data)
{
...
vil_openjpeg_image ::vil_openjpeg_image(vil_stream * /*is*/,
                                        unsigned int /*ni*/,
                                        unsigned int /*nj*/,
                                        unsigned int /*nplanes*/,
                                        vil_pixel_format /*format*/,
...
vil_openjpeg_image ::vil_openjpeg_image(vil_stream * is, vil_openjpeg_format opjfmt)
  : impl_(new vil_openjpeg_image_impl)
{
  switch (opjfmt)
  {
    case VIL_OPENJPEG_JP2:
      this->impl_->opj_codec_format_ = CODEC_JP2;
      break;
    case VIL_OPENJPEG_JPT:
      this->impl_->opj_codec_format_ = CODEC_JPT;
      break;
    case VIL_OPENJPEG_J2K:
      this->impl_->opj_codec_format_ = CODEC_J2K;
      break;
    default:
      return;
  }
...
vil_openjpeg_image ::nreductions() const
{
  ...
  if (this->impl_->header_.num_reductions_ == static_cast<vxl_uint_32>(-1))
  {
    ...
    for (;;)
    {
      // This will fail when we ask for a reduction that doesn't exist
      this->impl_->vstream_->seek(this->impl_->vstream_start_);
      if (!decoder.init_from_stream(num_reductions + 1, this->impl_->vstream_.as_pointer()))
        break;
      // If init_from_stream succeeded, that reduction is available
      ++num_reductions;
    }
...
unsigned int
vil_openjpeg_image ::nplanes() const
{
  if (!this->impl_->is_valid_)
    return static_cast<unsigned int>(-1);
...
vil_openjpeg_image ::pixel_format() const
{
  // NOTE:
...
const char *
vil_openjpeg_image ::file_format() const
{
  switch (this->impl_->opj_codec_format_)
  {
    case CODEC_JP2:
...
vil_openjpeg_image ::get_copy_view(unsigned int i0, unsigned int ni, unsigned int j0, unsigned int nj) const
{
...
vil_image_view_base_sptr
vil_openjpeg_image ::get_copy_view_reduced(unsigned int i0,
                                           unsigned int ni,
                                           unsigned int j0,
                                           unsigned int nj,
...
bool
vil_openjpeg_image ::put_view(const vil_image_view_base & /*im*/, unsigned int /*i0*/, unsigned int /*j0*/)
{
  assert(!"openjpeg write support is currently not implemented");
  return false;
}
