...
public:
  ...
  {
    return static_cast<vil_openjpeg_file_format *>(this)->make_input_image(vs, VIL_OPENJPEG_JP2);
  }

  vil_image_resource_sptr
  make_output_image(vil_stream * vs, unsigned int ni, unsigned int nj, unsigned int nplanes, vil_pixel_format format)
    override
  {
    return static_cast<vil_openjpeg_file_format *>(this)->make_output_image(
      vs, ni, nj, nplanes, format, VIL_OPENJPEG_JP2);
  }
};
...
public:
  ...
  make_input_image(vil_stream * vs) override
  {
    return static_cast<vil_openjpeg_file_format *>(this)->make_input_image(vs, VIL_OPENJPEG_J2K);
  }
  ...
  {
    return static_cast<vil_openjpeg_file_format *>(this)->make_output_image(
      vs, ni, nj, nplanes, format, VIL_OPENJPEG_J2K);
  }
};
...
public:
  ...
  get_copy_view(unsigned int i0, unsigned int ni, unsigned int j0, unsigned int nj) const override;
  ...
  get_copy_view_reduced(unsigned i0, unsigned ni, unsigned j0, unsigned nj, unsigned reduction) const;

  bool
  put_view(const vil_image_view_base & im, unsigned int i0, unsigned int j0) override;
...
private:
  ...
  vil_image_view_base_sptr
  opj2vil(void * opj_view, unsigned int i0, unsigned int ni, unsigned int j0, unsigned int nji) const;
