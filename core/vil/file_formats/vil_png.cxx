...
vil_png_file_format::make_input_image(vil_stream * is)
{
...
//
static void
pngtopnm_error_handler(png_structp png_ptr, png_const_charp msg)
{
...
struct vil_png_structures
{
  ...
  vil_png_structures(bool reading)
  {
    ...
    if (reading)
      ...
      png_ptr =
        png_create_write_struct(PNG_LIBPNG_VER_STRING, &pngtopnm_jmpbuf_struct, pngtopnm_error_handler, nullptr);
...
vil_png_image::vil_png_image(vil_stream * s, unsigned nx, unsigned ny, unsigned nplanes, enum vil_pixel_format format)
  : vs_(s)
  , width_(nx)
  , height_(ny)
  , components_(nplanes)
  ...
  , p_(new vil_png_structures(false))
{
  ...
  vs_->ref();
  write_header();
  assert(format == VIL_PIXEL_FORMAT_BYTE || format == VIL_PIXEL_FORMAT_UINT_16);
  // FIXME || format == VIL_PIXEL_FORMAT_BOOL
}
...
vil_png_image::read_header()
{
  ...
  {
    if (bit_depth == 1)
    { // treat 1-bit image as bool image
      is_bool_image = true;
  ...
  if (this->bits_per_component_ == 1)
    format_ = VIL_PIXEL_FORMAT_BOOL;
  else if (this->bits_per_component_ == 8)
    format_ = VIL_PIXEL_FORMAT_BYTE;
  else if (this->bits_per_component_ == 16)
    format_ = VIL_PIXEL_FORMAT_UINT_16;
  else
    return problem("Bad bit depth");
...
vil_png_image::get_copy_view(unsigned x0, unsigned nx, unsigned y0, unsigned ny) const
{
  if (!p_->ok)
    return nullptr;
  ...
  if (nx == png_get_image_width(p_->png_ptr, p_->info_ptr))
  {
    ...
    if (bit_depth == 1)
    {
      ...
      return new vil_image_view<bool>(
        chunk, reinterpret_cast<bool *>(chunk->data()), nx, ny, nplanes(), nplanes(), nplanes() * nx, 1);
    }
    else if (bit_depth == 16)
    {
      ...
      return new vil_image_view<vxl_uint_16>(
        chunk, reinterpret_cast<vxl_uint_16 *>(chunk->data()), nx, ny, nplanes(), nplanes(), nplanes() * nx, 1);
    }
    else if (bit_depth == 8)
    {
      std::memcpy(reinterpret_cast<char *>(chunk->data()), rows[y0], ny * bytes_per_row_dst);
      return new vil_image_view<vxl_byte>(
        chunk, reinterpret_cast<vxl_byte *>(chunk->data()), nx, ny, nplanes(), nplanes(), nplanes() * nx, 1);
    }
    else
      return nullptr;
  }
...
bool
vil_png_image::put_view(const vil_image_view_base & view, unsigned x0, unsigned y0)
{
  ...
  {
    if (view.pixel_format() != VIL_PIXEL_FORMAT_UINT_16)
      return false;
