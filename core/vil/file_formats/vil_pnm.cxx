...
vil2_image_data* vil_pnm_file_format::make_input_image(vil_stream* vs)
{
...
vil2_image_data* vil2_pnm_file_format::make_output_image(vil_stream* vs, int planes,
                                                       int width,
                                                       int height,
                                                       int components,
                                                       int bits_per_component,
                                                       vil2_component_format format)
{
...
vil2_pnm_generic_image::vil2_pnm_generic_image(vil2_stream* vs):
  vs_(vs)
{
  vs_->ref();
  read_header();
...
vil2_pnm_generic_image::vil2_pnm_generic_image(vil2_stream* vs, int planes,
                                             int width,
                                             int height,
                                             int components,
                                             int bits_per_component,
...
bool operator>>(vil2_stream& vs, int& a)
{
  char c; vs.read(&c,1L);
  SkipSpaces(&vs,c);
...
bool vil2_pnm_generic_image::get_section(void* buf, int x0, int y0, int xs, int ys) const
{
...
void operator<<(vil2_stream& vs, int a) {
  char buf[128]; vcl_sprintf(buf, " %d\n", a); vs.write(buf,vcl_strlen(buf));
}

bool vil2_pnm_generic_image::put_section(void const* buf, int x0, int y0, int xs, int ys)
{
