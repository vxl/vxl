...
vil_pnm_file_format::make_input_image(vil_stream * vs)
{
  ...
  : vs_(vs)
{
  ...
  if (nplanes == 1 && (format == VIL_PIXEL_FORMAT_RGB_BYTE || format == VIL_PIXEL_FORMAT_RGB_SBYTE ||
                       ...
                       format == VIL_PIXEL_FORMAT_RGB_FLOAT || format == VIL_PIXEL_FORMAT_RGB_DOUBLE))
    ncomponents_ = 3;
...
static void
SkipSpaces(vil_stream * vs, char & temp)
{
  while (isws(temp) || temp == '#')
  {
    if (temp == '#') // skip this line:
      while (!iseol(temp))
        if (1L > vs->read(&temp, 1L))
...
static int
ReadInteger(vil_stream * vs, char & temp)
{
  int n = 0;
  while ((temp >= '0') && (temp <= '9'))
  {
...
static void
ConvertMSBToHost(void * buf, int num_words)
{
...
static void
ConvertHostToMSB(void * buf, int num_words)
{
  auto * ptr = static_cast<unsigned char *>(buf);
  for (int i = 0; i < num_words; ++i)
  {
...
vil_pnm_image::read_header()
{
  ...
  if (magic_ == 1 || magic_ == 4)
    ...
    assert(!"problem reading maxval from PNM file");
  else if (maxval_ <= 0xFF)
    bits_per_component_ = 8;
  else if (maxval_ <= 0xFFFF)
    bits_per_component_ = 16;
  else if (maxval_ <= 0xFFFFFF)
    bits_per_component_ = 24;
  else if (maxval_ <= 0x7FFFFFFF)
    bits_per_component_ = 32;
  else
    assert(!"vil_pnm_image: maxval is too big");
...
#endif
    if (bits_per_component_ <= 1)
      return new vil_image_view<bool>(
        buf, reinterpret_cast<bool *>(buf->data()), ni, nj, nplanes(), nplanes(), ni * nplanes(), 1);
    if (bits_per_component_ <= 8)
      return new vil_image_view<vxl_byte>(
        buf, reinterpret_cast<vxl_byte *>(buf->data()), ni, nj, nplanes(), nplanes(), ni * nplanes(), 1);
    else if (bits_per_component_ <= 16)
      return new vil_image_view<vxl_uint_16>(
        buf, reinterpret_cast<vxl_uint_16 *>(buf->data()), ni, nj, nplanes(), nplanes(), ni * nplanes(), 1);
    else
      return new vil_image_view<vxl_uint_32>(
        buf, reinterpret_cast<vxl_uint_32 *>(buf->data()), ni, nj, nplanes(), nplanes(), ni * nplanes(), 1);
#if 0  // never return vil_image_view<vil_rgb<T> > : default image representation is planar
    }
    else if (ncomponents_ == 3)
    {
...
bool
vil_pnm_image::put_view(const vil_image_view_base & view, unsigned x0, unsigned y0)
{
  ...
  if ((view.pixel_format() == VIL_PIXEL_FORMAT_UINT_32 && bits_per_component_ < 32) ||
      ...
      view.pixel_format() == VIL_PIXEL_FORMAT_DOUBLE || view.pixel_format() == VIL_PIXEL_FORMAT_FLOAT)
  {
  ...
  {
    std::cerr << "ERROR: " << __FILE__ << ":\n Do not support putting " << view.is_a()
  ...
  else if (magic_ == 4) // pbm (bitmap) raw image
  {
    ...
    for (unsigned y = 0; y < view.nj(); ++y)
    {
      ...
      {
        if ((*bb)(x, y))
          a |= 1 << (7 - s); // single bit; high bit = first
        if (s >= 7)
        {
          vs_->write(&a, 1L);
          ++byte_start;
