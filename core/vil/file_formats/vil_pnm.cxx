  ...
  unsigned x0, unsigned ni, unsigned y0, unsigned nj) const
{
  ...
  if( magic_ > 4 ) // pgm or ppm raw image
    ...
    if( bits_per_component_ <= 1 )
      {
      return new vil_image_view<bool>(buf, reinterpret_cast<bool *>(buf->data() ), ni, nj, nplanes(),
                                      nplanes(), ni * nplanes(), 1);
      }
    if( bits_per_component_ <= 8 )
      {
      return new vil_image_view<vxl_byte>(buf, reinterpret_cast<vxl_byte *>(buf->data() ), ni, nj, nplanes(),
                                          nplanes(), ni * nplanes(), 1);
      }
    else if( bits_per_component_ <= 16 )
      {
      return new
             vil_image_view<vxl_uint_16>(buf, reinterpret_cast<vxl_uint_16 *>(buf->data() ), ni, nj, nplanes(),
                                         nplanes(), ni * nplanes(), 1);
      }
    else
      {
      return new
             vil_image_view<vxl_uint_32>(buf, reinterpret_cast<vxl_uint_32 *>(buf->data() ), ni, nj, nplanes(),
                                         nplanes(), ni * nplanes(), 1);
      }
#if 0 // never return vil_image_view<vil_rgb<T> > : default image representation is planar
    }
    ...
    if( bits_per_component_ <= 8 )
      {
      return new vil_image_view<vil_rgb<vxl_byte> >(buf,
                                                    reinterpret_cast<vil_rgb<vxl_byte> *>(buf->data() ), ni, nj, 1, 1, ni,
                                                    1);
      }
    else if( bits_per_component_ <= 16 )
      {
      return new vil_image_view<vil_rgb<vxl_uint_16> >(buf,
                                                       reinterpret_cast<vil_rgb<vxl_uint_16> *>(buf->data() ), ni, nj, 1, 1, ni,
                                                       1);
      }
    else
      {
      return new vil_image_view<vil_rgb<vxl_uint_32> >(buf,
                                                       reinterpret_cast<vil_rgb<vxl_uint_32> *>(buf->data() ), ni, nj, 1, 1, ni,
                                                       1);
      }
    }
  else { return 0; }
