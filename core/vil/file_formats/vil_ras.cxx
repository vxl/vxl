...
// native format.
bool
read_uint_32(vil_stream * vs, vxl_uint_32 & word)
{
  if (vs->read(&word, 4) < 4)
    return false;
#if VXL_LITTLE_ENDIAN
...
bool
write_uint_32(vil_stream * vs, vxl_uint_32 word)
{
#if VXL_LITTLE_ENDIAN
  swap_endian(word);
#endif
...
vxl_uint_32
compute_length(vxl_uint_32 w, vxl_uint_32 h, vxl_uint_32 d)
{
  w *= (d / 8);
  w += (w % 2);
  return h * w;
...
vil_ras_file_format::make_input_image(vil_stream * vs)
{
  // Check the magic number
  vxl_uint_8 buf[4] = { 0, 0, 0, 0 };
  vs->read(buf, 4);
  if (!(buf[0] == RAS_MAGIC[0] && buf[1] == RAS_MAGIC[1] && buf[2] == RAS_MAGIC[2] && buf[3] == RAS_MAGIC[3]))
    return nullptr;
...
vil_ras_image::vil_ras_image(vil_stream * vs)
  : vs_(vs)
{
  vs_->ref();
  read_header();
...
vil_ras_image::read_header()
{
  ...
  if (!(read_uint_32(vs_, width_) && read_uint_32(vs_, height_) && read_uint_32(vs_, depth_) &&
        read_uint_32(vs_, length_) && read_uint_32(vs_, type_) && read_uint_32(vs_, map_type_) &&
  ...
  if (type_ != RT_OLD && type_ != RT_STANDARD && type_ != RT_BYTE_ENCODED && type_ != RT_FORMAT_RGB)
  {
  ...
  {
    std::cerr << __FILE__ << ": length " << length_
              << " does not match wxhxd = " << compute_length(width_, height_, depth_) << std::endl;
    return false;
  }
  if (map_length_ % 3 != 0)
  {
...
unsigned
vil_ras_image::nj() const
{
  return height_;
}

vil_pixel_format
vil_ras_image::pixel_format() const
{
  if (bits_per_component_ == 8)
...
vil_ras_image::get_copy_view(unsigned i0, unsigned ni, unsigned j0, unsigned nj) const
{
  ...
  if (fmt == VIL_PIXEL_FORMAT_BYTE)
    return new vil_image_view<vxl_byte>(buf, ib, ni, nj, components_, components_, components_ * ni, 1);
  if (fmt == VIL_PIXEL_FORMAT_UINT_16 && components_ == 1)
  {
    ...
    return new vil_image_view<vxl_uint_16>(buf, sib, ni, nj, components_, components_, components_ * ni, 1);
  }
  return nullptr;
}
...
bool
vil_ras_image::put_view(const vil_image_view_base & view, unsigned i0, unsigned j0)
{
