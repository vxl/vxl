...
vil_sgi_file_format::make_input_image(vil_stream * vs)
{
...
vil_sgi_image::vil_sgi_image(vil_stream * vs, unsigned nx, unsigned ny, unsigned nplanes, vil_pixel_format format)
  : is_(vs)
{
  if (format != VIL_PIXEL_FORMAT_BYTE)
  {
    vil_exception_warning(
      vil_exception_pixel_formats_incompatible(VIL_PIXEL_FORMAT_BYTE, format, "vil_sgi_image::vil_sgi_image"));
  ...
  {
    vil_exception_warning(
      vil_exception_unsupported_operation("vil_sgi_image::vil_sgi_image: invalid number of planes"));
    return;
  }
...
unsigned
vil_sgi_image::ni() const
{
  return hdr.xsize;
}

unsigned
vil_sgi_image::nj() const
{
  return hdr.ysize;
}

bool
vil_sgi_image::read_header()
{
  ...
  // There are four colormap modes. Only mode 0 ("normal") is in widespread use. Mode 1 and 2 are obsolete, mode 3 is
  // for SGI hardware.
  if (hdr.colormap != 0)
  {
...
vil_sgi_image::get_copy_view(unsigned x0, unsigned nx, unsigned y0, unsigned ny) const
{
  ...
  return new vil_image_view<vxl_byte>(
    chunk, reinterpret_cast<vxl_byte *>(chunk->data()), nx, ny, nplanes(), nplanes(), nx * nplanes(), 1);
}

bool
vil_sgi_image::put_view(const vil_image_view_base & view, unsigned x0, unsigned y0)
{
  ...
  {
    vil_exception_warning(
      vil_exception_pixel_formats_incompatible(VIL_PIXEL_FORMAT_BYTE, view.pixel_format(), "vil_sgi_image::put_view"));
    return false;
  }
