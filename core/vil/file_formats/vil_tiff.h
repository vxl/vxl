...
public:
  ...
  make_input_pyramid_image(const char * file) override;
  ...
  make_pyramid_image_from_base(const char * filename,
                               ...
                               unsigned nlevels,
                               const char * temp_dir) override;

  vil_image_resource_sptr
  make_output_image(vil_stream * vs, unsigned ni, unsigned nj, unsigned nplanes, enum vil_pixel_format) override;
  ...
  vil_pyramid_image_resource_sptr
  make_pyramid_output_image(const char * file) override;
};
...
struct tif_smart_ptr
{
  ...
  tif_smart_ptr(const tif_smart_ptr & tp)
  {
    tptr_ = tp.tptr_;
    if (tptr_)
      tptr_->ref();
...
public:
  vil_tiff_image(const tif_smart_ptr & tif, vil_tiff_header * th, const unsigned nimages = 1);
  ...
  unsigned
  size_block_i() const override;

  //: Block size in rows (must be a multiple of 16)
  unsigned
  size_block_j() const override;

  //: Number of blocks in image width
  unsigned
  n_block_i() const override;

  //: Number of blocks in image height
  unsigned
  n_block_j() const override;

  vil_image_view_base_sptr
  get_block(unsigned block_index_i, unsigned block_index_j) const override;

  bool
  put_block(unsigned block_index_i, unsigned block_index_j, const vil_image_view_base & blk) override;
  ...
  bool
  set_compression_method(compression_methods cm);

  bool
  set_compression_quality(int quality);
  ...
  void
  set_index(const unsigned int index)
  {
    assert(index < nimages_);
    index_ = index;
  }
...
private:
  ...
  void
  copy_byte_block(vxl_byte * data, const vxl_uint_32 nbytes, vil_memory_chunk_sptr & cnk) const;
  ...
  vil_image_view_base_sptr
  fill_block_from_tile(const vil_memory_chunk_sptr & buf) const;

  vil_image_view_base_sptr
  fill_block_from_strip(const vil_memory_chunk_sptr & buf) const;
...
#endif
  bool
  put_block(unsigned bi, unsigned bj, unsigned i0, unsigned j0, const vil_image_view_base & im);
  ...
  void
  pad_block_with_zeros(unsigned ioff,
                       unsigned joff,
                       unsigned iclip,
                       unsigned jclip,
  ...
  void
  fill_block_from_view(unsigned bi,
                       unsigned bj,
                       unsigned i0,
                       unsigned j0,
                       unsigned ioff,
  ...
  void
  bitpack_block(unsigned bytes_per_block, const vxl_byte * in_block_buf, vxl_byte * out_block_buf);

  bool
  write_block_to_file(unsigned bi, unsigned bj, unsigned block_size_bytes, vxl_byte * block_buf);
...
public:
  ...
  void
  print(const unsigned l) const
  {
    std::cout << "level[" << l << "] hindex " << header_index_ << " scale: " << scale_ << "  width: " << ni_
              << std::endl;
  }
};
...
{
public:
  vil_tiff_pyramid_resource(const tif_smart_ptr & t, bool read = true);
  ...
  vil_image_view_base_sptr
  get_copy_view(unsigned i0, unsigned n_i, unsigned j0, unsigned n_j, unsigned level) const override;
  ...
  vil_image_view_base_sptr
  get_copy_view(unsigned i0, unsigned n_i, unsigned j0, unsigned n_j, const float scale, float & actual_scale)
    const override;
  ...
  bool
  put_resource(const vil_image_resource_sptr & resc) override;

  //: returns the image resource at the specified pyramid level
  vil_image_resource_sptr
  get_resource(const unsigned level) const override;

  //: for debug purposes
  void
  print(const unsigned level) override
  {
...
// and contained the following bits: 0100110010111000 0111101100000000 1111000011110000 (note that this bitstream is
// shown in big endian notation, that will not be the case if you are on a little endian machine -- this is just for
// illustration) in other words: in_data[0] = 19640 (0100110010111000) [shown in big endian for illustrative purposes
// only] in_data[1] = 31488 (0111101100000000) [shown in big endian for illustrative purposes only] in_data[2] = 61680
// (1111000011110000) [shown in big endian for illustrative purposes only] Let's further say, you called this function
// like this byte_align_data( in_data, 4, 12, out_data ). Then out_data would be aligned along two byte (sixteen bit)
// boundaries and would look like this: out_data[0] = 1227 (0000010011001011) [shown in big endian for illustrative
// purposes only] out_data[1] = 2171 (0000100001111011) [shown in big endian for illustrative purposes only] out_data[2]
// = 15   (0000000000001111) [shown in big endian for illustrative purposes only] out_data[3] = 240  (0000000011110000)
// [shown in big endian for illustrative purposes only]
//
...
template <class T>
T *
tiff_byte_align_data(T * in_data, unsigned num_samples, unsigned in_bits_per_sample, T * out_data)
{
