...
public:
  ...
  virtual vil_pyramid_image_resource_sptr make_pyramid_image_from_base(char const* filename,
                                                                       vil_image_resource_sptr const& base_image,
                                                                       unsigned nlevels, char const* temp_dir);

  virtual vil_image_resource_sptr make_output_image(vil_stream* vs, unsigned ni, unsigned nj, unsigned nplanes,
                                                    enum vil_pixel_format);

  virtual vil_blocked_image_resource_sptr make_blocked_output_image(vil_stream* vs, unsigned ni, unsigned nj,
                                                                    unsigned nplanes, unsigned size_block_i,
                                                                    unsigned size_block_j, enum vil_pixel_format);
...
private:
  ...
  vil_image_view_base_sptr view_from_buffer(vil_pixel_format& fmt, vil_memory_chunk_sptr const& buf,
                                            unsigned samples_per_block, unsigned bits_per_sample) const;
...
#if 0
  vil_image_view_base_sptr get_block_internal( unsigned block_index_i, unsigned block_index_j ) const;

  void get_blocks_internal( unsigned start_block_i, unsigned end_block_i, unsigned start_block_j, unsigned end_block_j,
  ...
  void pad_block_with_zeros(unsigned ioff, unsigned joff, unsigned iclip, unsigned jclip, unsigned bytes_per_pixel,
                            vxl_byte* block_buf);

  // : fill the block with view data
  void fill_block_from_view(unsigned bi, unsigned bj, unsigned i0, unsigned j0, unsigned ioff, unsigned joff,
                            unsigned iclip, unsigned jclip, const vil_image_view_base& im, vxl_byte *& block_buf);
...
public:
  ...
  {
    vcl_cout << "level[" << l <<  "] hindex " << header_index_ << " scale: " << scale_ << "  width: " << ni_
             << vcl_endl;
  }
  };
...
public:
  ...
  virtual vil_image_view_base_sptr get_copy_view(unsigned i0, unsigned n_i, unsigned j0, unsigned n_j,
  ...
  virtual vil_image_view_base_sptr get_copy_view(unsigned i0, unsigned n_i, unsigned j0, unsigned n_j,
                                                 const float scale, float& actual_scale) const;
...
T tiff_get_bits( const T* in_val, unsigned i0, unsigned ni )
{
  ...
  if( strip_right > 0 )
    ...
    for( int i = 0; i < new_strip_right; i++ )
      {
      next /= 2;
      }
