...
vil_viff_file_format::make_input_image(vil_stream * is)
{
  ...
  switch (dst)
  {
    ...
    default:
      std::cout << "vil_viff: non supported data type: VFF_TYP " << header.data_storage_type << std::endl;
      return nullptr;
  }
}
...
bool
vil_viff_image::write_header()
{
  is_->seek(0L);
  vil_viff_data_storage type;

  if (format_ == VIL_PIXEL_FORMAT_UINT_32 || format_ == VIL_PIXEL_FORMAT_INT_32)
    type = VFF_TYP_4_BYTE;
  else if (format_ == VIL_PIXEL_FORMAT_UINT_16 || format_ == VIL_PIXEL_FORMAT_INT_16)
    type = VFF_TYP_2_BYTE;
  else if (format_ == VIL_PIXEL_FORMAT_BYTE || format_ == VIL_PIXEL_FORMAT_SBYTE)
    type = VFF_TYP_1_BYTE;
...
vil_viff_image::get_copy_view(unsigned int x0, unsigned int xs, unsigned int y0, unsigned int ys) const
{
  assert(x0 + xs <= ni_);
  assert(y0 + ys <= nj_);
  ...
  for (unsigned int p = 0; p < nplanes_; ++p)
  {
    ...
    {
      is_->seek(start_of_data_ + p * nj_ * ((ni_ * pix_size + 7) / 8) + y * ((ni_ * pix_size + 7) / 8) +
                x0 * pix_size / 8);
      is_->read(ib, rowsize);
      ib += rowsize;
    }
...
#define ARGS(T) buf, reinterpret_cast<T *>(buf->data()), xs, ys, nplanes_, 1, xs, xs * ys
  if (format_ == VIL_PIXEL_FORMAT_BOOL)
    ...
    return new vil_image_view<std::complex<float>>(ARGS(std::complex<float>));
  else if (format_ == VIL_PIXEL_FORMAT_COMPLEX_DOUBLE)
    return new vil_image_view<std::complex<double>>(ARGS(std::complex<double>));
  else
    return nullptr;
#undef ARGS
}
...
vil_viff_image::put_view(const vil_image_view_base & buf, unsigned int x0, unsigned int y0)
{
  ...
  if (ibuf.istep() != 1 || ibuf.jstep() != int(ni) || (ibuf.planestep() != int(ni * nj) && nplanes() != 1))
  {
    std::cerr << "ERROR: " << __FILE__ << ":\n"
              ...
              << ", planestep=" << ibuf.planestep() << " instead of 1," << ni << ',' << ni * nj << '\n';
    return false;
  }
  ...
  { // !endian_consistent_ ==> must swap bytes
