...
template <class T>
inline void
vsl_b_write(vsl_b_ostream & os, const vil_image_view<T> & image)
{
  ...
  if (image.size() > 0)
  {
    ...
    std::ptrdiff_t offset = (image.top_left_ptr() - reinterpret_cast<const T *>(image.memory_chunk()->data()));
...
template <class T>
inline void
vsl_b_read(vsl_b_istream & is, vil_image_view<T> & image)
{
  if (!is)
    return;
  ...
  switch (w)
  {
    case 1:
      ...
      else
      {
        ...
        if (chunk->size() < np * ni * nj * sizeof(T) || offset < 0 || offset * sizeof(T) >= chunk->size())
        {
          std::cerr << "I/O ERROR: vsl_b_read(vsl_b_istream&, vil_image_view<T>&)\n"
        ...
        image = vil_image_view<T>(chunk, data + offset, ni, nj, np, istep, jstep, pstep);
      }
      break;
...
template <class T>
inline void
vsl_b_read(vsl_b_istream & is, vil_image_view<T> *& p)
{
  delete p;
  bool not_null_ptr;
...
template <class T>
inline void
vsl_print_summary(std::ostream & os, const vil_image_view<T> & image)
{
  image.print(os);
}
