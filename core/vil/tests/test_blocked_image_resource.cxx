...
test_blocked_image_resource()
{
  ...
  if (bir)
  {
    std::cout << "Blocked Image Parameters\n"
              << "ni = " << bir->ni() << " nj = " << bir->nj() << " nplanes = " << bir->nplanes() << '\n'
              << "size_block_i = " << bir->size_block_i() << "   size_block_j = " << bir->size_block_j()
              << "   n_block_i = " << bir->n_block_i() << "   n_block_j = " << bir->n_block_j() << '\n';
  ...
  if (bir)
  {
    vil_image_view<unsigned short> lview = bir->get_block(bir->n_block_i() - 1, bir->n_block_j() - 1);
#if 0
  ...
  if (bir2 && good_copy)
  {
    ...
    for (unsigned i = 0; i < bir->ni(); ++i)
      for (unsigned j = 0; j < bir->nj(); ++j)
      {
        ...
        if (v(i, j) != v2(i, j))
          std::cout << "v(" << i << ' ' << j << ") = " << v(i, j) << "vs. " << v2(i, j) << '\n';
      }
  }
  else
  {
  ...
  vil_blocked_image_resource_sptr bif = vil_new_blocked_image_facade(ir, sbi, sbj);
  if (bif)
  {
    std::cout << "Blocked Image Parameters\n"
              << "ni = " << bif->ni() << " nj = " << bif->nj() << " nplanes = " << bif->nplanes() << '\n'
              << "size_block_i = " << bif->size_block_i() << "   size_block_j = " << bif->size_block_j()
              << "   n_block_i = " << bif->n_block_i() << "   n_block_j = " << bif->n_block_j() << '\n';
    nbi = bif->n_block_i();
    nbj = bif->n_block_j();
    TEST("Test blocking parameters",
  ...
  if (cflbir && cfabir)
  {
    ...
    for (unsigned i = 0; i < sbi; ++i)
      for (unsigned j = 0; j < sbj; ++j)
        valid =
          valid && cflview(i, j) == cfaview(i, j) && cflview(i, j) == cflblk(i, j) && cflview(i, j) == cfablk(i, j);
  ...
  if (exists)
  {
    ...
    {
      TEST("NITF blocked resource",
           imgr->get_property(vil_property_size_block_i, &sbi) && imgr->get_property(vil_property_size_block_j, &sbj),
           true);
    }
    else
    {
