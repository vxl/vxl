...
struct CompareRGBA : public Compare<PixelType, vil_rgba<PixelType>>
{
    ...
    const override
  {
    return p == 0 && pixel.size() == 4 && img(i, j).r == pixel[0] && img(i, j).g == pixel[1] &&
           img(i, j).b == pixel[2] && img(i, j).a == pixel[3];
  }
};
...
struct CompareRGBNear : public Compare<PixelType, vil_rgb<PixelType>>
{
    ...
    const override
  {
    if (p == 0 && pixel.size() == 3)
    {
      ...
      auto diff_A = static_cast<PixelType>(pixel[0] * pixel[0] + img(i, j).r * img(i, j).r + pixel[1] * pixel[1] +
                                           img(i, j).g * img(i, j).g + pixel[2] * pixel[2] + img(i, j).b * img(i, j).b);

      auto diff_B = static_cast<PixelType>(2 * pixel[0] * img(i, j).r + tol_sq_ + 2 * pixel[1] * img(i, j).g + tol_sq_ +
...
template <class PixelType>
struct CompareGrey : public ComparePlanes<PixelType, 1>
{};

// Greyscale+Alpha has two (2) planes
template <class PixelType>
struct CompareGreyAlpha : public ComparePlanes<PixelType, 2>
{};
          ...
          if (dl + dr < ir->ni() && dt + db < ir->nj())
          {
            ...
            if (!im)
            {
              std::cout << "[ couldn't read (sub)image data from " << ir << "]\n"
                        << "[ off=(" << dl << ',' << dt << ")x(" << ir->ni() - dl - dr << ',' << ir->nj() - dt - db
                        << " ]" << std::endl;
              return false;
            }
            ...
            {
              std::cout << "[ (sub)image has wrong size (!= " << ir->ni() - dl - dr << 'x' << ir->nj() - dt - db
                        << ") in " << img << std::endl;
              return false;
            }
  ...
  for (int p = 0; p < num_planes; ++p)
  {
    for (int j = 0; j < height; ++j)
    {
      for (int i = 0; i < width; ++i)
      {
        ...
        {
          if (i >= int(offi[v]) && i < int(offi[v] + views[v].ni()) && j >= int(offj[v]) &&
              j < int(offj[v] + views[v].nj()) && !check(views[v], p, i - offi[v], j - offj[v], pixel))
          {
            std::cout << "View " << v << " at offset (" << offi[v] << ',' << offj[v] << ") [ " << views[v]
                      << " ] has a mismatch at pixel (p=" << p << ",i=" << i << ",j=" << j << ')' << std::endl;
            if (views[v].size() < 100)
            {
...
image_equals(const std::string & test_file, const std::string & ref_file)
{
  ...
  auto * view_test = dynamic_cast<vil_image_view<T_PIXEL> *>(view_test_ptr.as_pointer());
  auto * view_ref = dynamic_cast<vil_image_view<T_PIXEL> *>(view_ref_ptr.as_pointer());
...
test_file_format_read(int argc, char * argv[])
{
  ...
  TEST(
    "16-bit pgm ascii", CheckFile(CompareGrey<vxl_uint_16>(), "ff_grey16bit_true.txt", "ff_grey16bit_ascii.pgm"), true);
  ...
  TEST("8-bit ppm ascii as planar",
       CheckFile(ComparePlanes<vxl_byte, 3>(), "ff_planar8bit_true.txt", "ff_rgb8bit_ascii.ppm"),
       true);
  TEST("8-bit ppm raw", CheckFile(CompareRGB<vxl_byte>(), "ff_rgb8bit_true.txt", "ff_rgb8bit_raw.ppm"), true);
  TEST("8-bit ppm raw as planar",
       CheckFile(ComparePlanes<vxl_byte, 3>(), "ff_planar8bit_true.txt", "ff_rgb8bit_raw.ppm"),
       true);
  ...
  std::cout << "JPEG [jpg]\n";
  TEST("8-bit grey, normal image to 4 quanta",
       CheckFile(CompareGreyNear<vxl_byte>(4), "ff_grey8bit_true.txt", "ff_grey8bit_compressed.jpg"),
       true);
  TEST("8-bit RGB, easy image accurate to 3 quanta",
       CheckFile(CompareRGBNear<vxl_byte>(3), "ff_rgb8biteasy_true.txt", "ff_rgb8biteasy_compressed.jpg"),
       true);

  std::cout << "Windows bitmap [bmp]\n";
  TEST("8-bit greyscale (xv created)",
       CheckFile(CompareGrey<vxl_byte>(), "ff_grey8bit_true.txt", "ff_grey8bit.bmp"),
       true);
  // TEST("8-bit GA uncompressed", CheckFile(CompareGreyAlpha<vxl_byte>(), "ff_ga8bit_true.txt", "ff_ga8bit.bmp" ),
  // true);
  TEST("8-bit RGB (xv created)",
       CheckFile(ComparePlanes<vxl_byte, 3>(), "ff_planar8bit_true.txt", "ff_rgb8bit_xv.bmp"),
       true);
  TEST("8-bit RGB (Top-down scans, Photoshop created)",
       CheckFile(ComparePlanes<vxl_byte, 3>(), "ff_planar8bit_true.txt", "ff_rgb8bit_topdown_ps.bmp"),
       true);
  TEST("8-bit RGBA uncompressed (Photoshop created)",
       CheckFile(CompareRGBA<vxl_byte>(), "ff_rgba8bit_true.txt", "ff_rgba8bit_uncompressed_ps.bmp"),
       true);
  ...
  TEST("8-bit RGB uncompressed",
       CheckFile(CompareRGB<vxl_byte>(), "ff_rgb8bit_true.txt", "ff_rgb8bit_uncompressed.png"),
       true);
  TEST("8-bit RGB compressed",
       CheckFile(CompareRGB<vxl_byte>(), "ff_rgb8bit_true.txt", "ff_rgb8bit_compressed.png"),
       true);
  TEST(
    "8-bit RGB Indexed", CheckFile(CompareRGB<vxl_byte>(), "ff_rgb8bit_true.txt", "ff_rgb8bit_indexed_4bpp.png"), true);
  TEST("8-bit GA compressed",
       CheckFile(CompareGreyAlpha<vxl_byte>(), "ff_ga8bit_true.txt", "ff_ga8bit_compressed.png"),
       true);
  TEST("10-bit grey with bitdepth=true (from labview)",
       CheckFile(CompareGrey<vxl_uint_16>(), "ff_grey10bit_true.txt", "ff_grey10bit_labview_bitdepth_true.png"),
       true);
  TEST("10-bit grey with bitdepth=false (from labview)",
       CheckFile(CompareGrey<vxl_uint_16>(), "ff_grey10bit_true.txt", "ff_grey10bit_labview_bitdepth_false.png"),
       true);
  TEST("16-bit grey with bitdepth=true (from labview)",
       CheckFile(CompareGrey<vxl_uint_16>(), "ff_grey16bit_true.txt", "ff_grey16bit_labview_bitdepth_true.png"),
       true);
  TEST("16-bit grey with bitdepth=false (from labview)",
       CheckFile(CompareGrey<vxl_uint_16>(), "ff_grey16bit_true.txt", "ff_grey16bit_labview_bitdepth_false.png"),
       true);

  std::cout << "TIFF [tiff]\n";
  // TEST("1-bit grey uncompressed", CheckFile(CompareGrey<bool>(), "ff_grey1bit_true.txt",
  // "ff_grey1bit_uncompressed.tif" ), true);
  TEST("8-bit RGB uncompressed",
       CheckFile(CompareRGB<vxl_byte>(), "ff_rgb8bit_true.txt", "ff_rgb8bit_uncompressed.tif"),
       true);
  ...
  TEST("8-bit RGBA uncompressed",
       CheckFile(CompareRGBA<vxl_byte>(), "ff_rgba8bit_true.txt", "ff_rgba8bit_uncompressed.tif"),
       true);
  TEST("8-bit RGBA without RowsPerStrip",
       CheckFile(CompareRGBA<vxl_byte>(), "no_rowsperstrip_true.txt", "no_rowsperstrip.tif"),
       true);
  TEST("8-bit GA uncompressed",
       CheckFile(CompareGreyAlpha<vxl_byte>(), "ff_ga8bit_true.txt", "ff_ga8bit_uncompressed.tif"),
       true);
  // TEST("16-bit GA uncompressed", CheckFile(CompareGreyAlpha<vxl_uint_16>(), "ff_ga16bit_true.txt",
  // "ff_ga16bit_uncompressed.tif" ), true);

#if defined(TIFF_VERSION_BIG)
  TEST(
    "8-bit RGB Classic", CheckFile(ComparePlanes<vxl_byte, 3>(), "bigtiff/Classic.txt", "bigtiff/Classic.tif"), true);
  ...
  TEST("8-bit RGB BigTIFF long8Tiles",
       image_equals<vxl_byte>("bigtiff/BigTIFFLong8Tiles.tif", "bigtiff/Classic.tif"),
       true);
  TEST(
    "8-bit RGB BigTIFF Motorola", image_equals<vxl_byte>("bigtiff/BigTIFFMotorola.tif", "bigtiff/Classic.tif"), true);
  ...
  TEST("8-bit unsigned int (IMODE=P)",
       CheckFile(ComparePlanes<vxl_byte, 3>(), "ff_nitf_8bit_true.txt", "ff_nitf_8bit_p.nitf"),
       true);
  TEST("8-bit unsigned int (IMODE=B)",
       CheckFile(ComparePlanes<vxl_byte, 3>(), "ff_nitf_8bit_true.txt", "ff_nitf_8bit_b.nitf"),
       true);
  TEST("8-bit unsigned int (IMODE=R)",
       CheckFile(ComparePlanes<vxl_byte, 3>(), "ff_nitf_8bit_true.txt", "ff_nitf_8bit_r.nitf"),
       true);
  TEST("8-bit unsigned int (IMODE=S)",
       CheckFile(ComparePlanes<vxl_byte, 3>(), "ff_nitf_8bit_true.txt", "ff_nitf_8bit_s.nitf"),
       true);
  TEST("16-bit unsigned int (ABPP=13)",
       CheckFile(CompareGrey<vxl_uint_16>(), "ff_nitf_16bit_true.txt", "ff_nitf_16bit.nitf"),
       true);
  ...
  TEST("64-bit float (double)",
       CheckFile(CompareGreyFloat<double>(), "ff_nitf_float_true.txt", "ff_nitf_double.nitf"),
       true);
  std::cout << "NSIF 1.0 [nitf] (uncompressed)\n";
  TEST("1-bit bool (NSIF w/ LUT to parse)",
       CheckFile(CompareGrey<bool>(), "ff_nitf_1bit_lut_true.txt", "ff_nitf_1bit_lut.nsif"),
       true);
  ...
  // TEST("file6 (768x512 12Bit Grey)", image_diff_peak<vxl_uint_16>("jpeg2000/file6.jp2", "jpeg2000/opj_file6.tif"),
  // 0);
  ...
  TEST("8-bit grey, no colourmap",
       CheckFile(CompareGrey<vxl_byte>(), "ff_grey8bit_true.txt", "ff_grey8bit_nocol.ras"),
       true);
  ...
  TEST("8-bit grey big endian",
       CheckFile(CompareGrey<vxl_uint_8>(), "ff_grey8bit_true.txt", "ff_grey8bit_bigendian.viff"),
       true);
  TEST("8-bit RGB big endian",
       CheckFile(ComparePlanes<vxl_uint_8, 3>(), "ff_planar8bit_true.txt", "ff_rgb8bit_bigendian.viff"),
       true);
  TEST("16-bit grey big endian",
       CheckFile(CompareGrey<vxl_uint_16>(), "ff_grey16bit_true.txt", "ff_grey16bit_bigendian.viff"),
       true);
  TEST("16-bit RGB big endian",
       CheckFile(ComparePlanes<vxl_uint_16, 3>(), "ff_planar16bit_true.txt", "ff_rgb16bit_bigendian.viff"),
       true);
  TEST("32-bit grey big endian",
       CheckFile(CompareGrey<vxl_uint_32>(), "ff_grey32bit_true.txt", "ff_grey32bit_bigendian.viff"),
       true);
  TEST("32-bit float grey big endian",
       CheckFile(CompareGreyFloat<float>(), "ff_grey_float_true.txt", "ff_grey_float_bigendian.viff"),
       true);
  TEST("64-bit float grey big endian",
       CheckFile(CompareGreyFloat<double>(), "ff_grey_float_true.txt", "ff_grey_double_bigendian.viff"),
       true);

  TEST("8-bit grey little endian",
       CheckFile(CompareGrey<vxl_uint_8>(), "ff_grey8bit_true.txt", "ff_grey8bit_littleendian.viff"),
       true);
  TEST("8-bit RGB little endian",
       CheckFile(ComparePlanes<vxl_uint_8, 3>(), "ff_planar8bit_true.txt", "ff_rgb8bit_littleendian.viff"),
       true);
  TEST("16-bit grey little endian",
       CheckFile(CompareGrey<vxl_uint_16>(), "ff_grey16bit_true.txt", "ff_grey16bit_littleendian.viff"),
       true);
  TEST("16-bit RGB little endian",
       CheckFile(ComparePlanes<vxl_uint_16, 3>(), "ff_planar16bit_true.txt", "ff_rgb16bit_littleendian.viff"),
       true);
  TEST("32-bit grey little endian",
       CheckFile(CompareGrey<vxl_uint_32>(), "ff_grey32bit_true.txt", "ff_grey32bit_littleendian.viff"),
       true);
  TEST("32-bit float grey little endian",
       CheckFile(CompareGreyFloat<float>(), "ff_grey_float_true.txt", "ff_grey_float_littleendian.viff"),
       true);
  TEST("64-bit float grey little endian",
       CheckFile(CompareGreyFloat<double>(), "ff_grey_float_true.txt", "ff_grey_double_littleendian.viff"),
       true);
