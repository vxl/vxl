...
template <class T>
void
test_image_resource(const std::string & type, vil_pixel_format format, T /*dummy*/)
{
  std::cout << "*****************************************************************\n"
            << " Testing vil_image_resource objects with pixel type = " << type << '\n'
            << "*****************************************************************\n";

  const vil_image_resource_sptr mem = vil_new_image_resource(10, 8, 1, format);

  TEST("vil_new_image_resource", mem ? true : false, true);

  vil_image_view<T> view1 = mem->get_view(0, mem->ni(), 0, mem->nj());
  TEST("vil_memory_image::get_view()", view1 && view1.ni() == 10 && view1.nj() == 8 && view1.nplanes() == 1, true);

  const vil_image_resource_sptr mem2 = vil_new_image_resource_of_view(view1);
  ...
  const vil_image_resource_sptr crop = vil_crop(mem, 2, 6, 2, 4);

  TEST("vil_crop(image_resource)", crop ? true : false, true);

  vil_image_view<T> view2 = crop->get_copy_view(0, crop->ni(), 0, crop->nj());
  TEST("vil_memory_image::get_copy_view()", view2 && view2.ni() == 6 && view2.nj() == 4 && view2.nplanes() == 1, true);
  ...
  if (format != VIL_PIXEL_FORMAT_COMPLEX_FLOAT && format != VIL_PIXEL_FORMAT_COMPLEX_DOUBLE)
  { // Can't do clamping, or math_value_range on a complex<T>
    ...
    const vil_image_resource_sptr clamp = vil_clamp(mem, 1.0, 9.0); // arguments of type double, not T
  ...
  const vil_image_resource_sptr flip1 = vil_flip_ud(mem);
  const vil_image_resource_sptr flip2 = vil_flip_lr(flip1);
  const vil_image_resource_sptr flip3 = vil_flip_ud(flip2);
  const vil_image_resource_sptr flip4 = vil_flip_lr(flip3);
  ...
  TEST("x == flip_lr(flip_ud(flip_lr(flip_ud(x)))) C", vil_image_view_deep_equality(view1, view2), true);
  flip4->put_view(view2, 0, 0);
  view1 = mem->get_copy_view(0, mem->ni(), 0, mem->nj());
  // Check that the put view works
  TEST("x == flip_lr(flip_ud(flip_lr(flip_ud(x)))) D", vil_image_view_deep_equality(view1, view2), true);

  const vil_image_resource_sptr dec = vil_decimate(flip4, 2, 3);
