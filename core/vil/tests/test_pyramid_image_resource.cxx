...
test_pyramid_image_resource(int argc, char * argv[])
{
  ...
  const std::string d = "pyramid_dir";
  ...
  else
  {
    ...
    const vil_image_view<unsigned short> tview = pir->get_copy_view(15, 10, 10, 20, 0.6f, actual_scale);
  ...
  const vil_image_resource_sptr bir = new vil_blocked_image_facade(ir, 16, 16);
  // test decimation for generating a pyramid level
  const std::string dec_file = "decimated_resource";
  {
    const vil_image_resource_sptr dec_resc = vil_pyramid_image_resource::decimate(bir, dec_file.c_str());
    if (dec_resc)
      std::cout << "Made a successful decimated resource\n";
  } // close open resource files
  {
    const vil_image_resource_sptr reload_dec = vil_load_image_resource(dec_file.c_str());
    if (!reload_dec)
  ...
  {
    const vil_pyramid_image_resource_sptr bpyr =
    ...
    if (bpyr)
    {
      for (unsigned l = 0; l < bpyr->nlevels(); ++l)
        bpyr->print(l);
      float ac = 1.0f;
      const vil_image_view<unsigned short> v25 = bpyr->get_copy_view(0.25f, ac);
      const unsigned ni25 = v25.ni();
      const vil_image_view<unsigned short> v50 = bpyr->get_copy_view(0.5f, ac);
      const unsigned ni50 = v50.ni();
      std::cout << "ni25 = " << ni25 << ",  ni50 = " << ni50 << '\n';
      const bool good = (ni25 == 19 && ni50 == 37) || (ni25 == 18 && ni50 == 36);
  ...
  bool good = true;
  { // scope for dpyr
    const vil_pyramid_image_resource_sptr dpyr = vil_new_pyramid_image_resource(d.c_str(), "pyil");
    if (dpyr)
    {
  ...
  { // open input pyramid rdpyr
    const vil_pyramid_image_resource_sptr rdpyr = vil_load_pyramid_resource(d.c_str());
    TEST("load of pyramid resource rdpyr", !rdpyr, false);
    if (!rdpyr)
      return;
    const unsigned nlevels = rdpyr->nlevels();
  ...
  const std::string file = "tiff_pyramid.tif"; //  create_multi_file_resource(file);
  {                                            // scope for pi
    const vil_pyramid_image_resource_sptr pi = vil_new_pyramid_image_resource(file.c_str(), "tiff");
    if (pi)
    {
  ...
  { // open input pyramid rpi
    ...
    const vil_pyramid_image_resource_sptr rpi = vil_load_pyramid_resource(file.c_str());
    std::cout << "loaded multi-image pyramid " << rpi << '\n';
    const unsigned nlevels = rpi->nlevels();
    std::cout << "Read nlevels = " << nlevels << '\n';
    for (unsigned L = 0; L < nlevels; ++L)
      rpi->print(L);
    ...
    const vil_image_resource_sptr rlev_0 = rpi->get_resource(0);
    auto * tlev_0 = (vil_tiff_image *)rlev_0.ptr();
    const unsigned nimgs = tlev_0->nimages();
    vil_image_view<unsigned short> vl0 = rlev_0->get_view(1, 1, 1, 1);
    std::cout << "view0(0,0) " << vl0(0, 0) << '\n';
    const vil_image_resource_sptr rlev_1 = rpi->get_resource(1);
  ...
  const std::string fb = "tiff_pyramid_from_base.tif";
  { // scope for pyfb
    const vil_pyramid_image_resource_sptr pyfb = vil_new_pyramid_image_from_base(fb.c_str(), bir, 3, "tiff", d.c_str());
    const unsigned nlevels = pyfb->nlevels();
    std::cout << "Read nlevels = " << nlevels << '\n';
    for (unsigned L = 0; L < nlevels; ++L)
      pyfb->print(L);
  ...
  //
#if HAS_OPENJPEG2
  std::string const filepath_open_jpg2 = image_base + "jpeg2000/file1.jp2";
  const vil_image_resource_sptr resc_open_jpeg2 = vil_load_image_resource(filepath_open_jpg2.c_str());
  if (resc_open_jpeg2)
  {
    ...
    const unsigned nlevels = j2k_pyr->nlevels();
