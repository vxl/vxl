...
void test_sample_profile_bicub_byte()
{
  ...
  vil2_sample_profile_bicub(&vec[0],image1,x0,y0,dx,dy,3);
  TEST_NEAR("First value",vec[0],55,1e-6);
  TEST_NEAR("Last value",vec[2],57,1e-6);

  vcl_vector<double> vec2(8);
  vil2_sample_profile_bicub(&vec2[0],image1,x0,y0,dx,dy,8);
  ...
  vil2_sample_profile_bicub(&vec3[0],image3,x0,y0,dx,dy,3);
  ...
  vil2_sample_profile_bicub(&vec4[0],image3,x0,y0,dx,dy,8);
