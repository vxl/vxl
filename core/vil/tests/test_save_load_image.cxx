                 ...
                 unsigned max_bad_pixels = 0)
{
  ...
  if (!image2)
  {
    std::cout << "read back image type has pixel type " << pimage2->pixel_format() << " instead of (as written) "
              << image.pixel_format() << std::endl;
    return false;
  }
  ...
  {
    std::cout << type_name << ": sizes are " << sizex2 << " x " << sizey2 << " instead of " << sizex << " x " << sizey
              << std::endl;
    return false;
  }
  ...
  if (planes != planes2)
  {
    std::cout << type_name << ": nplanes are " << planes2 << " instead of " << planes << std::endl;
    return false;
  }
...
#endif
  std::cout << "istep()=" << image2.istep() << ", jstep()=" << image2.jstep() << ", planestep()=" << image2.planestep()
            << std::endl;
  std::ptrdiff_t i = image2.istep();
  if (i < 0)
    i = -i;
  TEST("|istep| is either 1, or np or height or np*height",
       i == 1 || i == planes2 || i == sizey2 || i == planes2 * sizey2,
       true);
  std::ptrdiff_t j = image2.jstep();
  if (j < 0)
    j = -j;
  TEST("|jstep| is either 1, or np or width or np*width",
       j == 1 || j == planes2 || j == sizex2 || j == 4 * ((sizex2 + 3) / 4) || j == planes2 * sizex2 ||
         j == 4 * ((planes2 * sizex2 + 3) / 4),
       true);
  // The "+3" is there to allow for "row word alignment", e.g. with the BMP format.
  std::ptrdiff_t p = image2.planestep();
  if (p < 0)
    p = -p;
  TEST("|planestep| is either 1 or width or height or width x height",
       p == 1 || p == sizex2 || p == sizey2 || p == sizex2 * sizey2,
       true);
  ...
  {
    std::cout << type_name << ": number of unequal pixels: " << bad << " out of " << planes * sizex * sizey
              << std::endl;
    return false;
  }
  else
...
static bool
create_colour_gif(const char * filename)
{
#ifdef _MSC_VER
#  pragma warning(push)
#  pragma warning(disable : 4305 4309)
  ...
  unsigned char b[] = {
    ...
    176, 113, 3,   71,  14,  53,  107, 216, 180, 113, 243, 6,   78,  28,  57,  115, 232, 212, 177, 115, 7,   79,  30,
    1,   10,  22,  104, 216, 192, 161, 131, 142, 29,  60,  122, 248, 248, 1,   36,  136, 158, 61,  124, 250, 248, 249,
    3,   40,  144, 160, 65,  132, 10,  25,  58,  132, 40,  17,  1,   7,   15,  68,  140, 32,  81,  66,  201, 18,  38,
    77,  156, 60,  129, 18,  69,  211, 38,  78,  157, 60,  125, 2,   21,  74,  212, 40,  82,  165, 76,  157, 66,  149,
    106, 0,   131, 6,   30,  62,  128, 8,   33,  100, 8,   145, 34,  70,  142, 32,  73,  162, 104, 17,  163, 70,  142,
    30,  65,  138, 36,  105, 18,  165, 74,  150, 46,  97,  202, 84,  0,   66,  4,   19,  39,  80,  164, 144, 50,  133,
    74,  21,  43,  87,  176, 100, 81,  181, 138, 85,  43,  87,  175, 96,  197, 146, 53,  139, 86,  45,  91,  183, 112,
    229, 50,  32,  97,  130, 138, 21,  44,  90,  104, 217, 194, 165, 139, 151, 47,  96,  194, 232, 218, 197, 171, 151,
    202, 181, 171, 215, 175, 96,  195, 138, 29,  75,  182, 172, 217, 179, 104, 211, 170, 93,  203, 182, 173, 219, 183,
    112, 227, 202, 157, 75,  183, 174, 221, 187, 120, 243, 234, 221, 203, 183, 175, 223, 191, 128, 3,   11,  30,  76,
    184, 176, 225, 195, 136, 19,  43,  94,  204, 184, 177, 227, 199, 144, 35,  75,  158, 76,  185, 178, 229, 203, 152,
    51,  107, 222, 204, 185, 179, 231, 207, 160, 67,  139, 30,  77,  186, 180, 233, 211, 168, 83,  171, 94,  205, 186,
    181, 235, 215, 176, 99,  203, 158, 77,  187, 182, 237, 219, 184, 115, 235, 222, 205, 187, 183, 239, 223, 192, 131,
    11,  31,  78,  188, 184, 241, 227, 200, 147, 43,  95,  206, 188, 185, 243, 231, 208, 163, 75,  159, 78,  189, 186,
    245, 235, 216, 179, 107, 223, 206, 189, 187, 247, 239, 224, 195, 67,  139, 31,  79,  190, 188, 249, 243, 232, 211,
    171, 95,  207, 190, 189, 251, 247, 240, 227, 203, 159, 79,  191, 190, 253, 251, 248, 243, 235, 223, 207, 191, 191,
    255, 255, 0,   6,   40,  224, 128, 4,   22,  104, 224, 129, 8,   38,  168, 224, 130, 12,  54,  232, 224, 131, 16,
    70,  40,  225, 132, 20,  86,  104, 225, 133, 155, 5,   4,   0,   59
  };
#ifdef _MSC_VER
#  pragma warning(pop)
#endif
                    ...
                    bool fail_save = false)          // expect fail on save if true
{
  ...
  std::cout << "vil_test_image_type: Save " << image.is_a() << " to [" << fname << "]\n" << std::flush;

  // Write image to disk
  if (std::strcmp(type_name, "gif") == 0 && vil_pixel_format_num_components(image.pixel_format()) == 3)
  {
...
vil_image_view<bool>
CreateTest1bitImage(int wd, int ht)
{
...
CreateTest8bitImage(int wd, int ht)
{
...
CreateTest16bitImage(int wd, int ht)
{
...
vil_image_view<vxl_uint_32>
CreateTest32bitImage(int wd, int ht)
{
...
CreateTest24bitImage(int wd, int ht)
{
  vil_image_view<vil_rgb<vxl_byte>> image(wd, ht);
  for (int i = 0; i < wd; i++)
    for (int j = 0; j < ht; j++)
      image(i, j).r = vxl_byte(i % (1 << 8)), image(i, j).g = vxl_byte(((i - wd / 2) * (j - ht / 2) / 16) % (1 << 8)),
...
vil_image_view<vxl_byte>
CreateTest3planeImage(int wd, int ht)
{
...
// create a 24 bit color test image, with 3 planes
vil_image_view<vxl_byte>
CreateTest4planeImage(int wd, int ht)
{
...
vil_image_view<vxl_byte>
CreateTest3ComponentImage(int wd, int ht)
{
...
CreateTestfloatImage(int wd, int ht)
{
...
CreateTestdoubleImage(int wd, int ht)
{
  ...
  // Test small image
  {
    ...
    for (unsigned j = 0; j < nj; ++j)
      for (unsigned i = 0; i < ni; ++i)
        small_greyscale_image(i, j) = vxl_byte((i + j) * 4);
#  ifdef DEBUG
