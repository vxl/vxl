...
template <class imT>
double
method1(const vil_image_view<imT> & src_image, vil_image_view<imT> & dest_image, int n_loops)
{
...
template <class imT>
double
method2(const vil_image_view<imT> & src_image, vil_image_view<imT> & dest_image, int n_loops)
{
  dest_image.set_size(src_image.ni(), src_image.nj(), src_image.nplanes());
  std::time_t t0 = std::clock();
  for (int n = 0; n < n_loops; ++n)
...
template <class imT>
double
method3(const vil_image_view<imT> & src_image, vil_image_view<imT> & dest_image, int n_loops)
{
  std::time_t t0 = std::clock();
  for (int n = 0; n < n_loops; ++n)
  {
...
template <class imT>
double
method4(const vil_image_view<imT> & src_image, vil_image_view<imT> & dest_image, int n_loops)
{
  std::time_t t0 = std::clock();
  for (int n = 0; n < n_loops; ++n)
  {
...
template <class imT>
double
method5(const vil_image_view<imT> & src_image, vil_image_view<imT> & dest_image, int n_loops)
{
...
template <class imT>
double
method6(const vil_image_view<imT> & src_image, vil_image_view<imT> & dest_image, int n_loops)
{
  // Assumes that istep==1 for both!
  std::time_t t0 = std::clock();
  for (int n = 0; n < n_loops; ++n)
    ...
    for (unsigned p = 0; p < np; ++p, splane += spstep, dplane += dpstep)
    {
      ...
      for (unsigned j = 0; j < nj; ++j, srow += sjstep, drow += djstep)
      {
        for (unsigned i = ni; i; --i)
          drow[i] = srow[i] + 1; // Test i against zero
      }
    }
  }
  std::time_t t1 = std::clock();
...
template <class imT>
double
method7(const vil_image_view<imT> & src_image, vil_image_view<imT> & dest_image, int n_loops)
{
...
template <class imT>
double
method(int i, const vil_image_view<imT> & src_image, vil_image_view<imT> & dest_image, int n_loops)
{
  double t;
  switch (i)
  {
    case 1:
      t = method1(src_image, dest_image, n_loops);
      break;
    case 2:
      t = method2(src_image, dest_image, n_loops);
      break;
    case 3:
      t = method3(src_image, dest_image, n_loops);
      break;
    case 4:
      t = method4(src_image, dest_image, n_loops);
      break;
    case 5:
      t = method5(src_image, dest_image, n_loops);
      break;
    case 6:
      t = method6(src_image, dest_image, n_loops);
      break;
    case 7:
      t = method7(src_image, dest_image, n_loops);
      break;
    default:
      t = -1;
  }
...
template <class imT>
void
compute_stats(int i, const vil_image_view<imT> & src_image, vil_image_view<imT> & dest_image, int n_loops)
{
  mbl_stats_1d stats;
  for (int j = 0; j < 10; ++j)
    stats.obs(method(i, src_image, dest_image, n_loops));
  std::cout << "Method " << i << ") Mean: " << int(stats.mean() + 0.5) << "us  +/-"
            << int(0.5 * (stats.max() - stats.min()) + 0.5) << "us\n";
}
