...
template <class imT>
double
method3(vil_image_view<imT> & image, const int * x, const int * y, int n_pts, int n_loops)
{
...
template <class imT>
double
method(int i, vil_image_view<imT> & image, const int * x, const int * y, int n_pts, int n_loops)
{
  double t;
  switch (i)
  {
    case 1:
      t = method1(image, x, y, n_pts, n_loops);
      break;
    case 2:
      t = method2(image, x, y, n_pts, n_loops);
      break;
    case 3:
      t = method3(image, x, y, n_pts, n_loops);
      break;
    default:
      t = -1;
  }
...
template <class imT>
void
compute_stats(int i, vil_image_view<imT> & image, const int * x, const int * y, int n_pts, int n_loops)
{
  mbl_stats_1d stats;
  std::cout << "\t\t\t\t";
  for (int j = 0; j < 10; ++j)
    stats.obs(method(i, image, x, y, n_pts, n_loops));
  std::cout << "\nMethod " << i << ") Mean: " << 0.1 * int(10 * stats.mean() + 5) << "ns  +/-"
            << 0.1 * int(5 * (stats.max() - stats.min()) + 5) << "ns\n";
}
...
template <class imT>
double
bilin_method1(vil_image_view<imT> & image, int n_pts, int n_loops, double & sum)
{
  std::time_t t0 = std::clock();
  sum = 0.0;
  for (int n = 0; n < n_loops; ++n)
...
template <class imT>
double
bilin_method2(vil_image_view<imT> & image, int n_pts, int n_loops, double & sum)
{
  std::time_t t0 = std::clock();
  sum = 0.0;
  for (int n = 0; n < n_loops; ++n)
...
template <class imT>
double
bilin_method3(vil_image_view<imT> & image, int n_pts, int n_loops, double & sum)
{
  assert(image.istep() == 1);
  // Uses row[i] to simulate lookup type access used in original vil1 images
  assert(image.ni() == NI);
...
template <class imT>
double
bilin_method(int i, vil_image_view<imT> & image, int n_pts, int n_loops)
{
  double t, sum;
  switch (i)
  {
    case 1:
      t = bilin_method1(image, n_pts, n_loops, sum);
      break;
    case 2:
      t = bilin_method2(image, n_pts, n_loops, sum);
      break;
    case 3:
      t = bilin_method3(image, n_pts, n_loops, sum);
      break;
    default:
      t = -1;
  }
...
template <class imT>
void
compute_bilin_stats(int i, vil_image_view<imT> & image, int n_pts, int n_loops)
{
  mbl_stats_1d stats;
  std::cout << "\t\t\t\t";
  for (int j = 0; j < 10; ++j)
    stats.obs(bilin_method(i, image, n_pts, n_loops));
  std::cout << "\nBilin. Method " << i << ") Mean: " << 0.1 * int(10 * stats.mean() + 5) << "ns  +/-"
            << 0.1 * int(5 * (stats.max() - stats.min()) + 5) << "ns\n";
}
...
{
  vil_image_view<vxl_byte> byte_image(NI, NJ, 1);
  byte_image.fill(17);
