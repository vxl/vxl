...
enum vil_border_mode {
  ...
  //: Border pixels take the value of the image point which is its
  // symmetric w.r.t. the closest image edge.
  // ex: im(im.ni()+3, 0) = im(im.ni()-4, 0)
  vil_border_reflect = 2,

  //: The image is seen as periodic, after the right (respectively
  //bottom) edge comes the left (respectively top).  
  // ex: im(im.ni()+2,0) == im(2,0) 
  // ex: im(im.ni()+3, im.nj()+2) == im(3,2)
  vil_border_periodic = 3,
};

//: Border class.  Makes pixel access outside image range transparent
// and configurable. Note that a border is not assigned to a specific 
// image, but only holds border properties.
template <class imT>
class vil_border
{
public:
...
public:
  ...
  //: Return read-only reference to pixel at (i,j,p) on the given
  // image.  If the pixel falls out of the image range, the border
  // value is returned.
  inline const pixel_type&
