...
template <class T>
inline void
vil_clamp(const vil_image_view<T> & src, vil_image_view<T> & dest, T lo, T hi)
{
  assert(hi >= lo);
  assert(src.nplanes() == dest.nplanes() && src.nj() == dest.nj() && src.ni() == dest.ni());
...
vil_clamp(const vil_image_resource_sptr & src, double low, double hi);
...
{
  vil_clamp_image_resource(const vil_image_resource_sptr &, double low, double high);
...
template <class T>
inline void
vil_clamp_below(vil_image_view<T> & src, T t, T v)
{
...
template <class T>
inline void
vil_clamp_above(vil_image_view<T> & src, T t, T v)
{
  std::ptrdiff_t istepA = src.istep(), jstepA = src.jstep(), pstepA = src.planestep();
  T * planeA = src.top_left_ptr();
  for (unsigned int p = 0; p < src.nplanes(); ++p, planeA += pstepA)
...
template <class T>
inline void
vil_clamp_below(vil_image_view<T> & src, T t)
{
...
template <class T>
inline void
vil_clamp_above(vil_image_view<T> & src, T t)
{
