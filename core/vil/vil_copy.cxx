...
{
  if (dest->ni() != src->ni() || dest->nj() != src->nj() || dest->nplanes() != src->nplanes() ||
      dest->pixel_format() != src->pixel_format())
    return false;

  if (src->ni() == 0 || src->nj() == 0 || src->nplanes() == 0)
    return true;
  ...
  else
  {
    ...
    while (got_to_line < src->nj())
    {
      ...
      if (!view_ref)
        return false;
      if (!dest->put_view(*view_ref, 0, got_to_line))
        return false;
