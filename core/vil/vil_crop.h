...
vil_crop(const vil_image_view<T> & im, unsigned i0, unsigned n_i, unsigned j0, unsigned n_j)
{
  assert(i0 < im.ni());
  assert(i0 + n_i <= im.ni());
  assert(j0 < im.nj());
  ...
  return vil_image_view<T>(im.memory_chunk(),
                           im.top_left_ptr() + i0 * im.istep() + j0 * im.jstep(),
                           n_i,
                           n_j,
                           im.nplanes(),
                           im.istep(),
...
{
public:
  vil_crop_image_resource(const vil_image_resource_sptr &, unsigned i0, unsigned n_i, unsigned j0, unsigned n_j);
