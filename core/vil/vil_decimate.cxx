...
vil_decimate(const vil_image_resource_sptr & src, unsigned i_factor, unsigned j_factor)
{
...
vil_decimate_image_resource::get_copy_view(unsigned i0, unsigned ni, unsigned j0, unsigned nj) const
{
  ...
  else // do large image case.
  {
    ...
    {
      vil_exception_warning(vil_exception_out_of_bounds("vil_decimate_image_resource::get_copy_view"));
      return nullptr;
    }
        ...
        macro(VIL_PIXEL_FORMAT_UINT_64, vxl_uint_64) macro(VIL_PIXEL_FORMAT_INT_64, vxl_int_64)
          ...
          macro(VIL_PIXEL_FORMAT_UINT_32, vxl_uint_32) macro(VIL_PIXEL_FORMAT_INT_32, vxl_int_32)
            macro(VIL_PIXEL_FORMAT_UINT_16, vxl_uint_16) macro(VIL_PIXEL_FORMAT_INT_16, vxl_int_16)
              macro(VIL_PIXEL_FORMAT_BOOL, bool) macro(VIL_PIXEL_FORMAT_FLOAT, float)
                macro(VIL_PIXEL_FORMAT_DOUBLE, double) macro(VIL_PIXEL_FORMAT_COMPLEX_FLOAT, std::complex<float>)
                  macro(VIL_PIXEL_FORMAT_COMPLEX_DOUBLE, std::complex<double>)
#undef macro
                    default
        : vil_exception_warning(
            vil_exception_unsupported_pixel_format(src_->pixel_format(), "vil_decimate_image_resource::get_copy_view"));
      return nullptr;
    }
  }
}
...
vil_image_view_base_sptr
vil_decimate_image_resource::get_view(unsigned i0, unsigned ni, unsigned j0, unsigned nj) const
{
  if ((unsigned long)i_factor_ * (unsigned long)ni * (unsigned long)j_factor_ * (unsigned long)nj < large_image_limit)
  {
...
vil_image_view_base_sptr
vil_decimate(const vil_image_view_base_sptr & im, unsigned i_factor, unsigned j_factor)
{
  if (j_factor == 0)
    j_factor = i_factor;
...
#undef macro
                  default
      : vil_exception_warning(vil_exception_unsupported_pixel_format(im->pixel_format(), "vil_decimate"));
    return nullptr;
  }
}
...
#if 1
bool
vil_decimate_image_resource::put_view(const vil_image_view_base &, unsigned, unsigned)
{
  vil_exception_warning(vil_exception_unsupported_operation("vil_decimate_image_resource::put_view"));
  return false;
}
...
bool
vil_decimate_image_resource::put_view(const vil_image_view_base & im, unsigned i0, unsigned j0)
{
  if ((unsigned long)i_factor_ * (unsigned long)im.ni() * (unsigned long)j_factor_ * (unsigned long)im.nj() <
      large_image_limit)
  {
    vil_image_view_base_sptr vs =
      src_->get_view(i0 * i_factor_, im.ni() * i_factor_, j0 * j_factor_, im.nj() * j_factor_);
    if (!vs || im.pixel_format() != vs->pixel_format() || im.nplanes() != vs->nplanes())
      return false;
