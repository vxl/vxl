...
vil_decimate(const vil_image_view<T> & im, unsigned i_factor, unsigned j_factor = 0)
{
  if (j_factor == 0)
    j_factor = i_factor;
  ...
  return vil_image_view<T>(im.memory_chunk(),
                           im.top_left_ptr(),
                           (im.ni() + i_factor - 1u) / i_factor,
                           (im.nj() + j_factor - 1u) / j_factor,
                           im.nplanes(),
                           im.istep() * i_factor,
                           im.jstep() * j_factor,
                           im.planestep());
...
vil_decimate(const vil_image_view_base_sptr & im, unsigned i_factor, unsigned j_factor = 0);

//: decimate to a region of src.
// \relatesalso vil_image_resource
vil_image_resource_sptr
vil_decimate(const vil_image_resource_sptr & src, unsigned i_factor, unsigned j_factor = 0);
...
{
public:
  vil_decimate_image_resource(const vil_image_resource_sptr &, unsigned i_factor, unsigned j_factor);
  ...
  ni() const override
  {
    return src_->ni() / i_factor_;
  }
  unsigned
  nj() const override
  {
    return src_->nj() / j_factor_;
  }
  ...
  vil_image_view_base_sptr
  get_view(unsigned i0, unsigned n_i, unsigned j0, unsigned n_j) const override;
