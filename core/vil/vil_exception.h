...
template <class T>
void
vil_exception_error(T exception)
{
...
template <class T>
void
vil_exception_warning(T exception)
{
#if !defined VXL_LEGACY_ERROR_REPORTING
  throw exception;
#else
...
class vil_exception_pixel_formats_incompatible : public std::logic_error
{
public:
  enum vil_pixel_format src_type, dest_type;
  std::string operation_name;
  vil_exception_pixel_formats_incompatible(enum vil_pixel_format src,
...
class vil_exception_unsupported_pixel_format : public std::logic_error
{
public:
...
class vil_exception_out_of_bounds : public std::logic_error
{
public:
  std::string operation_name;
...
// This function is only used in cases where easy of use, and risk of mistakes are high,
// and inefficiency is very low.
class vil_exception_unsupported_operation : public std::logic_error
{
public:
  std::string operation_name;
...
class vil_exception_image_io : public std::runtime_error
{
public:
  ...
  vil_exception_image_io(const std::string & function,
    ...
    : std::runtime_error("Failed to load " + file_name + " in " + function + " using " + type + " loader. " +
                         description)
...
// get_view(), etc.
class vil_exception_corrupt_image_file : public vil_exception_image_io
{
public:
...
class vil_exception_invalid_version : public vil_exception_image_io
{
public:
