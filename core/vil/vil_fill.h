...
template <class T>
void
vil_fill(vil_image_view<T> & view, T value)
{
...
template <class T>
void
vil_fill_line(T * data, unsigned n, std::ptrdiff_t step, T value)
{
...
template <class T>
void
vil_fill_line(vil_image_view<T> & im, int ai, int aj, int bi, int bj, T value)
{
  if (ai == bi && aj == bj)
  {
    if (im.in_range(ai, aj))
      im(ai, aj) = value;
    return;
  }
...
template <class T>
void
vil_fill_row(vil_image_view<T> & view, unsigned j, T value)
{
  unsigned ni = view.ni();
  std::ptrdiff_t istep = view.istep();
  assert(j < view.nj());
...
template <class T>
void
vil_fill_col(vil_image_view<T> & view, unsigned i, T value)
{
  assert(i < view.ni());
  std::ptrdiff_t istep = view.istep();
  unsigned nj = view.nj();
...
template <class srcT>
inline void
vil_fill_mask(vil_image_view<srcT> & image, const vil_image_view<bool> & mask, srcT value, bool b = true)
{
...
template <class T>
inline void
vil_fill_disk(vil_image_view<T> & image, double ci, double cj, double r, T value)
{
  ...
  for (unsigned j = jlo; j <= jhi; ++j)
  {
    ...
    for (unsigned i = ilo; i <= ihi; ++i)
    {
      if ((i - ci) * (i - ci) < t2)
      {
        for (unsigned k = 0; k < image.nplanes(); ++k)
          image(i, j, k) = value;
      }
    }
  }
