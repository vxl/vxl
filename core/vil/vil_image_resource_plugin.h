...
public:
  ...
  virtual bool
  load_the_image(vil_image_view_base_sptr & image, const std::string & path)
  {
  ...
  // \return  true if successful
  virtual bool
  load_the_image(vil_image_view_base_sptr & image,
  ...
  static void
  register_plugin(vil_image_resource_plugin * plugin);
  ...
  virtual bool
  can_be_loaded(const std::string & filename);

  //: Set the colour options
  void
  set_colour(const std::string & colour)
  {
    colour_ = colour;
  }
  ...
  void
  set_filetype(const std::string & filetype)
  {
    filetype_ = filetype;
  }
