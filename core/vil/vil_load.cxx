...
vil_load_image_resource_raw(vil_stream * is, bool verbose)
{
...
vil_load_image_resource_plugin(const char * filename)
{
  ...
  if (im_resource_plugin.can_be_loaded(filename))
  {
    ...
    {
      im = vil_new_image_resource(im_view->ni(), im_view->nj(), im_view->nplanes(), im_view->pixel_format());
...
//: Convenience function for loading an image into an image view.
vil_image_view_base_sptr
vil_load(const wchar_t * file, bool verbose)
{
  vil_image_resource_sptr data = vil_load_image_resource(file, verbose);
  if (!data)
    return 0;
