...
template <class T>
inline void
vil_math_value_range(const vil_image_view<T> & view, T & min_value, T & max_value)
{
  if (view.size() == 0)
  {
    min_value = 0;
...
template <>
inline void
vil_math_value_range(const vil_image_view<vil_rgb<vxl_byte>> & rgb_view,
...
template <>
inline void
vil_math_value_range(const vil_image_view<vil_rgb<float>> & rgb_view,
...
template <class T>
inline void
vil_math_value_range_percentiles(const vil_image_view<T> & im,
                                 ...
                                 bool ignore_nan = false)
{
  ...
  if (ignore_nan)
  {
    data.erase(std::remove_if(data.begin(), data.end(), [](const T & x) { return vil_math::isnan(x); }), data.end());
    if (data.empty())
    {
      return;
    }
...
template <class T>
inline void
vil_math_value_range_percentile(const vil_image_view<T> & im, const double fraction, T & value, bool ignore_nan = false)
{
...
inline sumT
vil_math_ssd(const vil_image_view<imT> & imA, const vil_image_view<imT> & imB, sumT /*dummy*/)
{
...
inline void
vil_math_mean_over_planes(const vil_image_view<aT> & src, vil_image_view<sumT> & dest)
{
...
inline void
vil_math_mean_over_planes(const vil_image_view<inT> & src, vil_image_view<outT> & dest, sumT /*dummy*/)
{
  dest.set_size(src.ni(), src.nj(), 1);
  for (unsigned j = 0; j < src.nj(); ++j)
    for (unsigned i = 0; i < src.ni(); ++i)
...
template <class imT, class sumT>
inline void
vil_math_sum(sumT & sum, const vil_image_view<imT> & im, unsigned p)
{
...
template <class imT, class sumT>
inline void
vil_math_mean(sumT & mean, const vil_image_view<imT> & im, unsigned p)
{
  if (im.size() == 0)
  {
    mean = 0;
...
template <class imT>
inline void
vil_math_median(imT & /*median*/, const vil_image_view<imT> & /*im*/, unsigned /*p*/)
{
  vil_math_median_unimplemented();
}
...
template <class imT, class sumT>
inline void
vil_math_sum_squares(sumT & sum, sumT & sum_sq, const vil_image_view<imT> & im, unsigned p)
{
  const imT * row = im.top_left_ptr() + p * im.planestep();
  std::ptrdiff_t istep = im.istep(), jstep = im.jstep();
  const imT * row_end = row + im.nj() * jstep;
...
template <class imT, class sumT>
inline void
vil_math_mean_and_variance(sumT & mean, sumT & var, const vil_image_view<imT> & im, unsigned p)
{
  if (im.size() == 0)
  {
    mean = 0;
...
template <class imT, class sumT>
inline bool
vil_math_mean_and_variance(sumT & mean,
                           sumT & var,
                           const vil_image_view<imT> & im,
                           const vil_image_view<bool> & mask,
                           unsigned p)
...
#define macro(F, T) \
  case F:           \
    return vil_math_mean_and_variance(mean, var, static_cast<vil_image_view<T> &>(*(im->get_view())), mask, p);

    macro(VIL_PIXEL_FORMAT_BYTE, vxl_byte) macro(VIL_PIXEL_FORMAT_SBYTE, vxl_sbyte)
      macro(VIL_PIXEL_FORMAT_UINT_32, vxl_uint_32) macro(VIL_PIXEL_FORMAT_UINT_16, vxl_uint_16)
        macro(VIL_PIXEL_FORMAT_INT_32, vxl_int_32) macro(VIL_PIXEL_FORMAT_INT_16, vxl_int_16)
          macro(VIL_PIXEL_FORMAT_BOOL, bool) macro(VIL_PIXEL_FORMAT_FLOAT, float) macro(VIL_PIXEL_FORMAT_DOUBLE, double)
#undef macro
...
public:
  vxl_byte
  operator()(vxl_byte x) const
  {
    return static_cast<vxl_byte>(0.5 + std::sqrt(double(x)));
  }
  unsigned
  operator()(unsigned x) const
  {
    return static_cast<unsigned int>(0.5 + std::sqrt(double(x)));
  }
  int
  operator()(int x) const
  {
    return x > 0 ? static_cast<int>(0.5 + std::sqrt(double(x))) : 0;
  }
  short
  operator()(short x) const
  {
    return x > 0 ? static_cast<short>(0.5 + std::sqrt(double(x))) : 0;
  }
  float
...
template <class T>
inline void
vil_math_sqrt(vil_image_view<T> & image)
{
...
template <class T>
inline void
vil_math_truncate_range(vil_image_view<T> & image, T min_v, T max_v)
{
...
public:
  ...
  operator()(vxl_byte x) const
  {
    return vxl_byte(0.5 + s_ * x);
  }
  unsigned
  operator()(unsigned x) const
  {
    return unsigned(0.5 + s_ * x);
  }
  short
  operator()(short x) const
  {
...
public:
  ...
  operator()(vxl_byte x) const
  {
    return vxl_byte(0.5 + s_ * x + t_);
  }
  unsigned
  operator()(unsigned x) const
  {
    return unsigned(0.5 + s_ * x + t_);
  }
  short
  operator()(short x) const
  {
...
public:
  ...
  operator()(vxl_byte x) const
  {
    return static_cast<vxl_byte>(0.5 + std::log(double(x)));
  }
  unsigned
  operator()(unsigned x) const
  {
    return static_cast<unsigned int>(0.5 + std::log(double(x)));
  }
  int
  operator()(int x) const
  {
    return x > 0 ? static_cast<int>(0.5 + std::log(double(x))) : 0;
  }
  short
  operator()(short x) const
  {
    return x > 0 ? static_cast<short>(0.5 + std::log(double(x))) : 0;
  }
  float
  operator()(float x) const
  {
...
template <class T>
inline void
vil_math_scale_values(vil_image_view<T> & image, double scale)
{
...
inline void
vil_math_scale_and_offset_values(vil_image_view<imT> & image, double scale, offsetT offset)
{
  unsigned ni = image.ni(), nj = image.nj(), np = image.nplanes();
  std::ptrdiff_t istep = image.istep(), jstep = image.jstep(), pstep = image.planestep();
...
template <class imT>
inline void
vil_math_normalise(vil_image_view<imT> & image)
{
...
inline void
vil_math_rms(const vil_image_view<srcT> & src, vil_image_view<destT> & dest)
{
  ...
  for (unsigned j = 0; j < nj; ++j, rowA += jstepA, rowB += jstepB)
  {
    ...
    else if (np == 2)
    {
      for (; pixelA != end_pixelA; pixelA += istepA, pixelB += istepB)
      {
        destT sum2 = destT(*pixelA) * (*pixelA) + destT(pixelA[pstepA]) * (pixelA[pstepA]);
    ...
    else
    {
      for (; pixelA != end_pixelA; pixelA += istepA, pixelB += istepB)
      {
        *pixelB = destT(*pixelA) * destT(*pixelA);
        ...
        for (; p != end_p; p += pstepA)
          *pixelB += destT(*p) * destT(*p);
...
template <class srcT, class destT>
inline void
vil_math_rss(const vil_image_view<srcT> & src, vil_image_view<destT> & dest)
{
  unsigned ni = src.ni(), nj = src.nj(), np = src.nplanes();
  dest.set_size(ni, nj, 1);
...
template <class srcT, class destT>
inline void
vil_math_sum_sqr(const vil_image_view<srcT> & src, vil_image_view<destT> & dest)
{
  unsigned ni = src.ni(), nj = src.nj(), np = src.nplanes();
  dest.set_size(ni, nj, 1);
  ...
  for (unsigned j = 0; j < nj; ++j, rowA += jstepA, rowB += jstepB)
  {
    ...
    {
      for (; pixelA != end_pixelA; pixelA += istepA, pixelB += istepB)
        *pixelB = destT(*pixelA) * (*pixelA) + destT(pixelA[pstepA]) * (pixelA[pstepA]);
    }
    else
    {
      for (; pixelA != end_pixelA; pixelA += istepA, pixelB += istepB)
...
inline void
vil_math_image_sum(const vil_image_view<aT> & imA, const vil_image_view<bT> & imB, vil_image_view<sumT> & im_sum)
{
...
template <class aT, class bT, class sumT>
inline void
vil_math_image_product(const vil_image_view<aT> & imA,
                       ...
                       vil_image_view<sumT> & im_product)
{
  ...
  std::ptrdiff_t istepA = imA.istep(), jstepA = imA.jstep(), pstepA = imA.planestep();
  std::ptrdiff_t istepB = imB.istep(), jstepB = imB.jstep(), pstepB = imB.planestep();
  std::ptrdiff_t istepP = im_product.istep(), jstepP = im_product.jstep(), pstepP = im_product.planestep();
...
inline void
vil_math_image_max(const vil_image_view<aT> & imA, const vil_image_view<bT> & imB, vil_image_view<maxT> & im_max)
{
  unsigned ni = imA.ni(), nj = imA.nj(), np = imA.nplanes();
  assert(imB.ni() == ni && imB.nj() == nj && imB.nplanes() == np);
...
inline void
vil_math_image_min(const vil_image_view<aT> & imA, const vil_image_view<bT> & imB, vil_image_view<minT> & im_min)
{
  unsigned ni = imA.ni(), nj = imA.nj(), np = imA.nplanes();
  assert(imB.ni() == ni && imB.nj() == nj && imB.nplanes() == np);
...
template <class aT, class bT, class sumT>
inline void
vil_math_image_ratio(const vil_image_view<aT> & imA, const vil_image_view<bT> & imB, vil_image_view<sumT> & im_ratio)
{
  unsigned ni = imA.ni(), nj = imA.nj(), np = imA.nplanes();
  assert(imB.ni() == ni && imB.nj() == nj);
  assert(imB.ni() == ni && imB.nj() == nj);
  ...
  std::ptrdiff_t istepA = imA.istep(), jstepA = imA.jstep(), pstepA = imA.planestep();
  std::ptrdiff_t istepB = imB.istep(), jstepB = imB.jstep(), pstepB = imB.planestep();
  std::ptrdiff_t istepR = im_ratio.istep(), jstepR = im_ratio.jstep(), pstepR = im_ratio.planestep();
    ...
    for (unsigned j = 0; j < nj; ++j, rowA += jstepA, rowB += jstepB, rowR += jstepR)
    {
      ...
      for (unsigned i = 0; i < ni; ++i, pixelA += istepA, pixelB += istepB, pixelR += istepR)
      {
        if (*pixelB == 0)
          ...
          *pixelR = sumT(*pixelA) / sumT(*pixelB);
      }
    }
  }
}
...
template <class aT, class bT, class sumT>
inline void
vil_math_image_difference(const vil_image_view<aT> & imA, const vil_image_view<bT> & imB, vil_image_view<sumT> & im_sum)
{
  unsigned ni = imA.ni(), nj = imA.nj(), np = imA.nplanes();
  assert(imB.ni() == ni && imB.nj() == nj && imB.nplanes() == np);
  im_sum.set_size(ni, nj, np);
...
inline void
vil_math_image_abs_difference_1d_generic(const aT * pxA,
                                         std::ptrdiff_t isA,
                                         const bT * pxB,
                                         std::ptrdiff_t isB,
                                         dT * pxD,
...
template <class aT, class bT, class dT>
inline void
vil_math_image_abs_difference_1d(const aT * pxA,
                                 std::ptrdiff_t isA,
                                 const bT * pxB,
                                 std::ptrdiff_t isB,
                                 dT * pxD,
                                 ...
                                 unsigned len)
{
  vil_math_image_abs_difference_1d_generic<aT, bT, dT>(pxA, isA, pxB, isB, pxD, isD, len);
}
...
template <class aT, class bT, class dT>
inline void
vil_math_image_abs_difference(const vil_image_view<aT> & imA, const vil_image_view<bT> & imB, vil_image_view<dT> & imD)
{
  unsigned ni = imA.ni(), nj = imA.nj(), np = imA.nplanes();
  assert(imB.ni() == ni && imB.nj() == nj && imB.nplanes() == np);
  ...
  for (unsigned p = 0; p < np; ++p, planeA += psA, planeB += psB, planeD += psD)
  {
    ...
    {
      vil_math_image_abs_difference_1d<aT, bT, dT>(rowA, isA, rowB, isB, rowD, isD, ni);
    }
  }
}
...
inline double
vil_math_image_abs_difference(const vil_image_view<aT> & imA, const vil_image_view<bT> & imB)
{
  double sum = 0.0;
  unsigned ni = imA.ni(), nj = imA.nj(), np = imA.nplanes();
  assert(imB.ni() == ni && imB.nj() == nj && imB.nplanes() == np);
...
inline void
vil_math_image_vector_mag(const vil_image_view<aT> & imA, const vil_image_view<bT> & imB, vil_image_view<magT> & im_mag)
{
  unsigned ni = imA.ni(), nj = imA.nj(), np = imA.nplanes();
  assert(imB.ni() == ni && imB.nj() == nj && imB.nplanes() == np);
...
inline void
vil_math_add_image_fraction(vil_image_view<aT> & imA, scaleT fa, const vil_image_view<bT> & imB, scaleT fb)
{
  unsigned ni = imA.ni(), nj = imA.nj(), np = imA.nplanes();
  assert(imB.ni() == ni && imB.nj() == nj && imB.nplanes() == np);
...
template <class aT, class sumT>
inline void
vil_math_integral_image(const vil_image_view<aT> & imA, vil_image_view<sumT> & im_sum)
{
...
template <class aT, class sumT>
inline void
vil_math_integral_sqr_image(const vil_image_view<aT> & imA,
