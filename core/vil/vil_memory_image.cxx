...
vil_memory_image::get_copy_view(unsigned i0, unsigned n_i, unsigned j0, unsigned n_j) const
{
  if (i0 + n_i > view_->ni() || j0 + n_j > view_->nj())
    return nullptr;
...
// \return 0 if unable to get view of correct size.
vil_image_view_base_sptr
vil_memory_image::get_view(unsigned i0, unsigned n_i, unsigned j0, unsigned n_j) const
{
  if (i0 + n_i > view_->ni() || j0 + n_j > view_->nj())
    return nullptr;
...
bool
vil_memory_image::put_view(const vil_image_view_base & im, unsigned i0, unsigned j0)
{
  if (view_->pixel_format() != im.pixel_format())
    return false;
