...
// \relatesalso vil_image_resource
vil_image_resource_sptr
vil_new_image_resource(unsigned ni, unsigned nj, const vil_image_resource_sptr & prototype)
{
  return vil_new_image_resource(ni, nj, prototype->nplanes(), prototype->pixel_format());
}
                       ...
                       const char * file_format)
{
  ...
  const std::list<vil_file_format *> & l = vil_file_format::all();
  for (auto fmt : l)
  {
...
// \relatesalso vil_image_resource
vil_image_resource_sptr
vil_new_image_resource(const char * filename,
                       unsigned ni,
                       unsigned nj,
                       const vil_image_resource_sptr & prototype,
                       const char * file_format)
{
  ...
  return vil_new_image_resource(
    os, ni, nj, prototype->nplanes(), prototype->pixel_format(), file_format ? file_format : prototype->file_format());
}
                               ...
                               const char * file_format)
{
  ...
  for (auto fmt : l)
  {
    if (std::strcmp(fmt->tag(), file_format) == 0)
    {
      outimage = fmt->make_blocked_output_image(os, ni, nj, nplanes, size_block_i, size_block_j, format);
      if (!outimage)
...
vil_blocked_image_resource_sptr
vil_new_blocked_image_resource(const char * filename,
                               unsigned ni,
                               unsigned nj,
                               unsigned nplanes,
                               vil_pixel_format format,
                               ...
                               const char * file_format)
{
  ...
  auto * os = new vil_stream_fstream(filename, "w");
#endif // VIL_USE_FSTREAM64
  return vil_new_blocked_image_resource(os, ni, nj, nplanes, format, size_block_i, size_block_j, file_format);
}

vil_blocked_image_resource_sptr
vil_new_blocked_image_facade(const vil_image_resource_sptr & src, unsigned size_block_i, unsigned size_block_j)
{
...
vil_blocked_image_resource_sptr
vil_new_cached_image_resource(const vil_blocked_image_resource_sptr & bir, const unsigned cache_size)
{
  return new vil_cached_image_resource(bir, cache_size);
}

vil_pyramid_image_resource_sptr
vil_new_pyramid_image_resource(const char * file_or_directory, const char * file_format)
{
  if (!file_format) // avoid segfault in strcmp()
    file_format = "tiff";
...
vil_pyramid_image_resource_sptr
vil_new_pyramid_image_from_base(const char * filename,
                                ...
                                const char * temp_dir)
{
  ...
  for (auto fmt : l)
  {
    if (std::strcmp(fmt->tag(), file_format) == 0)
    {
      outimage = fmt->make_pyramid_image_from_base(filename, base_image, nlevels, temp_dir);
      if (!outimage)
        std::cerr << "vil_new: Cannot new a pyramid resource to type [" << file_format << "]\n";
      return outimage;
    }
...
vil_pyramid_image_resource_sptr
vil_new_pyramid_image_list_from_base(const char * directory,
                                     const vil_image_resource_sptr & base_image,
                                     unsigned nlevels,
                                     bool copy_base,
...
vil_new_image_view_base_sptr(const vil_image_view_base & src)
{
...
// \relatesalso vil_image_resource
vil_image_resource_sptr
vil_new_image_resource(const wchar_t * filename,
                       unsigned ni,
                       unsigned nj,
                       const vil_image_resource_sptr & prototype,
                       const wchar_t * file_format)
...
#  endif // VIL_USE_FSTREAM64
  ...
  if (!file_format)
  {
    const char * tag = prototype->file_format();
  ...
  return vil_new_image_resource(os, ni, nj, prototype->nplanes(), prototype->pixel_format(), file_format);
}
