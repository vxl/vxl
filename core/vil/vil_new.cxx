...
vil2_image_data_sptr vil2_new_image_data(unsigned nx, unsigned ny, vil2_image_data_sptr const& prototype)
{
  return new vil2_memory_image(nx, ny, prototype->nplanes(),
                  ...
                  char const* file_format)
{
  ...
  for (vil2_file_format** p = vil2_file_format::all(); *p; ++p) {
    ...
    if (vcl_strcmp(fmt->tag(), file_format) == 0) {
      outimage = fmt->make_output_image(os, nx, ny, nplanes, format);
      if (!outimage) {
...
//: Make a new vil2_image_impl, writing to stream "os", size nx x ny, copying pixel format etc from "prototype".
vil2_image_data_sptr vil2_new_image_data(vil_stream* os,
