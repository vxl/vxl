...
// \relatesalso vil_image_resource
vil_image_resource_sptr vil_new_image_resource_interleaved(unsigned ni, unsigned nj, unsigned nplanes,
...
// : Make a new image.
// \relatesalso vil_image_resource
vil_image_resource_sptr vil_new_image_resource(vil_stream* os, unsigned ni, unsigned nj, unsigned nplanes,
                                               vil_pixel_format format, char const* file_format = 0);

// : Make a new image.
// \relatesalso vil_image_resource
vil_image_resource_sptr vil_new_image_resource(char const* filename, unsigned ni, unsigned nj, unsigned nplanes,
                                               vil_pixel_format format, char const* file_format = 0);

// : Make a new vil_image_resource, writing to file "filename", size ni x nj, copying pixel format etc from "prototype".
// \relatesalso vil_image_resource
vil_image_resource_sptr vil_new_image_resource(char const* filename, unsigned ni, unsigned nj, unsigned nplanes,
                                               vil_image_resource_sptr const & prototype, char const* format = 0);

// : Make a new vil_image_resource, writing to stream "os", size ni x nj, copying pixel format etc from "prototype".
// \relatesalso vil_image_resource
vil_image_resource_sptr vil_new_image_resource(vil_stream* os, unsigned ni, unsigned nj, unsigned nplanes,
                                               vil_image_resource_sptr const& prototype, char const* file_format = 0);

// : Make a new blocked resource file
vil_blocked_image_resource_sptr vil_new_blocked_image_resource(vil_stream* os, unsigned ni, unsigned nj,
                                                               unsigned nplanes, vil_pixel_format format,
                                                               unsigned size_block_i, unsigned size_block_j,
                                                               char const* file_format = 0);

// : Make a new blocked resource file
vil_blocked_image_resource_sptr vil_new_blocked_image_resource(char const* filename, unsigned ni, unsigned nj,
                                                               unsigned nplanes, vil_pixel_format format,
                                                               unsigned size_block_i, unsigned size_block_j,
                                                               char const* file_format = 0);
...
vil_pyramid_image_resource_sptr vil_new_pyramid_image_from_base(char const* filename,
                                                                ...
                                                                unsigned nlevels, char const* file_format,
                                                                char const* temp_dir);
...
vil_pyramid_image_resource_sptr vil_new_pyramid_image_list_from_base(char const* directory,
                                                                     vil_image_resource_sptr const& base_image,
                                                                     unsigned nlevels, bool copy_base,
...
// \relatesalso vil_image_resource
vil_image_resource_sptr vil_new_image_resource(vil_stream* os, unsigned ni, unsigned nj, unsigned nplanes,
                                               vil_pixel_format format, wchar_t const* file_format = 0);

// : Make a new image.
// \relatesalso vil_image_resource
vil_image_resource_sptr vil_new_image_resource(wchar_t const* filename, unsigned ni, unsigned nj, unsigned nplanes,
                                               vil_pixel_format format, wchar_t const* file_format = 0);

// : Make a new vil_image_resource, writing to file "filename", size ni x nj, copying pixel format etc from "prototype".
// \relatesalso vil_image_resource
vil_image_resource_sptr vil_new_image_resource(wchar_t const* filename, unsigned ni, unsigned nj, unsigned nplanes,
                                               vil_image_resource_sptr const & prototype, wchar_t const* format = 0);
