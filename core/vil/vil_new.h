...
vil_blocked_image_resource_sptr
vil_new_cached_image_resource(const vil_blocked_image_resource_sptr & bir, const unsigned cache_size = 100);
...
template <class T>
vil_image_view<T>
vil_new_image_view_plane_i_j(unsigned ni, unsigned nj, unsigned nplanes, T /*dummy*/)
{
  vil_pixel_format fmt = vil_pixel_format_of(T());
  vil_memory_chunk_sptr chunk =
    new vil_memory_chunk(ni * nj * nplanes * sizeof(T), vil_pixel_format_component_format(fmt));
...
template <class T>
vil_image_view<T>
vil_new_image_view_i_j_plane(unsigned ni, unsigned nj, unsigned nplanes, T /*dummy*/)
{
  vil_pixel_format fmt = vil_pixel_format_of(T());
  vil_memory_chunk_sptr chunk =
    new vil_memory_chunk(ni * nj * nplanes * sizeof(T), vil_pixel_format_component_format(fmt));
...
vil_new_image_view_base_sptr(const vil_image_view_base &);
