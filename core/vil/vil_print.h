...
template <class T>
void
vil_print_value(std::ostream & s, const T & value, unsigned = 0);
...
template <class T>
inline void
vil_print_all(std::ostream & os, const vil_image_view<T> & view, unsigned width = 0)
{
  if (!width)
    width = static_cast<unsigned>(os.width());
  os << view.is_a() << ' ' << view.nplanes() << " planes, each " << view.ni() << " x " << view.nj()
     << " istep: " << (int)view.istep() << ' ' << " jstep: " << (int)view.jstep() << ' '
