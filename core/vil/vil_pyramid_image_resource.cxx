...
static bool
convert_multi_plane_to_float(vil_image_view_base_sptr & blk, std::vector<vil_image_view<float>> & fblk)
{
  if (!blk)
    return false;
  fblk.clear();
...
vil_pyramid_image_resource::decimate(const vil_image_resource_sptr & resc, const char * filename, const char * format)
{
  ...
  { // file scope to close resource
    ...
    dnj += rnj % 2;
    vil_blocked_image_resource_sptr dec_resc =
      vil_new_blocked_image_resource(filename, dni, dnj, np, fmt, brsc->size_block_i(), brsc->size_block_j(), format);
