...
{
public:
  vil_pyramid_image_resource(const std::vector<vil_image_resource_sptr> & images);
  ...
  get_copy_view(unsigned i0, unsigned n_i, unsigned j0, unsigned n_j, unsigned level) const = 0;

  //: Get a complete view from a specified pyramid level.
  // This method needs no specialisation.
  virtual vil_image_view_base_sptr
  get_copy_view(unsigned level) const
  {
  ...
  // This method needs no specialisation.
  virtual vil_image_view_base_sptr
  get_copy_view(const float scale, float & actual_scale) const
  {
  ...
  decimate(const vil_image_resource_sptr & resc, const char * filename, const char * format = "tiff");

  //: for debug purposes
  virtual void
  print(const unsigned level) = 0;
