...
class vil_pyramid_image_view 
{
 public:
    ...
    //: Creates an pyramid of given image views with associates scales, 
    ...
    //: adds a view to the list of view sorted in the right place based on the 
    // scale in tde descending order
 ...
 protected:
    ...
    inline bool limit_reached(unsigned i, unsigned j) 
      {if (i<2 || j<2 || nlevels_==max_levels_) return true; return false; }
