...
public:
  ...
  vil_pyramid_image_view(const vil_image_view<T> & image)
    ...
    , max_levels_(256)
  {
    images_.push_back(new vil_image_view<T>(image));
    scales_.push_back(1.0);
  }
  ...
  vil_pyramid_image_view(const std::vector<vil_image_view_base_sptr> & images, const std::vector<double> & scales);
  ...
  void
  add_view(vil_image_view_base_sptr & image, double scale);

  vil_image_view_base_sptr
  get_view(unsigned level, double & scale)
  {
    scale = scales_[level];
  ...
  operator=(const vil_pyramid_image_view<T> & rhs);

  vil_image_view<T> &
  operator()(unsigned l)
  {
    return static_cast<vil_image_view<T> &>(*images_[l]);
  }
...
protected:
  ...
  inline bool
  limit_reached(unsigned i, unsigned j)
  {
  ...
  void
  scale_down(const vil_image_view<T> &, vil_image_view_base_sptr & image_out);
};
