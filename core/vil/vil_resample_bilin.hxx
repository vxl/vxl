...
// \relatesalso vil_image_view
template <class sType, class dType>
void vil_resample_bilin_edge_extend(
  ...
  else
  {
    ...
    if (np==1)
    {
      ...
      for (int j=0;j<n2;++j,x1+=dx2,y1+=dy2,row+=d_jstep)
      {
        ...
        dType *dpt = row;
        for (int i=0;i<n1;++i,x+=dx1,y+=dy1,dpt+=d_istep)
          *dpt = (dType) vil_bilin_interp_safe_extend(
    ...
    else
    {
      ...
      for (int j=0;j<n2;++j,x1+=dx2,y1+=dy2,row+=d_jstep)
      {
        ...
        {
          for (unsigned int p=0;p<np;++p)
            dpt[p*d_pstep] = (dType) vil_bilin_interp_safe_extend(
                                    ...
                                    int n1, int n2)
{
  ...
  double dx2=0;
  double dy2=f*(src_image.nj()-1)*1.0/(n2-1);
  vil_resample_bilin_edge_extend(
