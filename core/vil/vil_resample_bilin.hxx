// This is core/vil/vil_resample_bilin.hxx
#ifndef vil_resample_bilin_hxx_
#define vil_resample_bilin_hxx_
//:
// \file
...
inline bool
vil_resample_bilin_corner_in_image(double x0, double y0, const vil_image_view_base & image)
{
  return x0 >= 0.0 && y0 >= 0.0 && x0 + 1 <= image.ni() && y0 + 1 <= image.nj();
}
...
void
vil_resample_bilin(const vil_image_view<sType> & src_image,
                   ...
                   int n2)
{
  bool all_in_image = vil_resample_bilin_corner_in_image(x0, y0, src_image) &&
                      ...
                      vil_resample_bilin_corner_in_image(
                        x0 + (n1 - 1) * dx1 + (n2 - 1) * dx2, y0 + (n1 - 1) * dy1 + (n2 - 1) * dy2, src_image);
#ifdef DEBUG
  // corners
  ...
  else
  {
    ...
    if (np == 1)
    {
      ...
      for (int j = 0; j < n2; ++j, x1 += dx2, y1 += dy2, row += d_jstep)
      {
        ...
        dType * dpt = row;
        for (int i = 0; i < n1; ++i, x += dx1, y += dy1, dpt += d_istep)
          *dpt = (dType)vil_bilin_interp_safe(x, y, plane0, ni, nj, istep, jstep);
      }
    }
    else
    {
      ...
      for (int j = 0; j < n2; ++j, x1 += dx2, y1 += dy2, row += d_jstep)
      {
        ...
        {
          for (unsigned int p = 0; p < np; ++p)
            dpt[p * d_pstep] = (dType)vil_bilin_interp_safe(x, y, plane0 + p * pstep, ni, nj, istep, jstep);
        }
      }
    }
  }
...
template <class sType, class dType>
void
vil_resample_bilin(const vil_image_view<sType> & src_image, vil_image_view<dType> & dest_image, int n1, int n2)
{
...
template <class sType, class dType>
void
vil_resample_bilin_edge_extend(const vil_image_view<sType> & src_image,
                               vil_image_view<dType> & dest_image,
                               double x0,
                               double y0,
                               double dx1,
  ...
  else
  {
    ...
    if (np == 1)
    {
      ...
      for (int j = 0; j < n2; ++j, x1 += dx2, y1 += dy2, row += d_jstep)
      {
        ...
        dType * dpt = row;
        for (int i = 0; i < n1; ++i, x += dx1, y += dy1, dpt += d_istep)
          *dpt = (dType)vil_bilin_interp_safe_extend(x, y, plane0, ni, nj, istep, jstep);
      }
    }
    else
    {
      ...
      for (int j = 0; j < n2; ++j, x1 += dx2, y1 += dy2, row += d_jstep)
      {
        ...
        {
          for (unsigned int p = 0; p < np; ++p)
            dpt[p * d_pstep] = (dType)vil_bilin_interp_safe_extend(x, y, plane0 + p * pstep, ni, nj, istep, jstep);
        }
      }
    }
  }
                               ...
                               int n2)
{
  ...
  double dx2 = 0;
  double dy2 = f * (src_image.nj() - 1) * 1.0 / (n2 - 1);
  vil_resample_bilin_edge_extend(src_image, dest_image, x0, y0, dx1, dy1, dx2, dy2, n1, n2);
}
#define VIL_RESAMPLE_BILIN_INSTANTIATE(sType, dType)                                              \
  template void vil_resample_bilin(const vil_image_view<sType> & src_image,                       \
                                   ...
                                   int n1,                                                        \
                                   int n2);                                                       \
  template void vil_resample_bilin(                                                               \
    const vil_image_view<sType> & src_image, vil_image_view<dType> & dest_image, int n1, int n2); \
  template void vil_resample_bilin_edge_extend(const vil_image_view<sType> & src_image,           \
                                               ...
                                               int n1,                                            \
                                               int n2);                                           \
  template void vil_resample_bilin_edge_extend(                                                   \
    const vil_image_view<sType> & src_image, vil_image_view<dType> & dest_image, int n1, int n2)

#endif // vil_resample_bilin_hxx_
