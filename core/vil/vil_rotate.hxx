...
calc_theta_mod(double theta, double x)
{
  if (x < 0)
    x = -x;
...
#ifdef DEBUG
  std::cout << "theta_90 = " << theta_90 << std::endl << "theta_360= " << theta_360 << std::endl;
#endif
...
#define VIL_ROTATE_INSTANTIATE(sType, dType) \
  template void vil_rotate_image(            \
    const vil_image_view<sType> & src_image, vil_image_view<dType> & dest_image, double theta)
