...
inline bool
vil_grid_bicub_corner_in_image(double x0, double y0, const vil_image_view_base & image)
{
  return x0 >= 2 && y0 >= 2 && x0 + 3 <= image.ni() && y0 + 3 <= image.nj();
}
...
{
  bool all_in_image =
    vil_grid_bicub_corner_in_image(x0, y0, image) &&
    vil_grid_bicub_corner_in_image(x0 + (n1 - 1) * dx1, y0 + (n1 - 1) * dy1, image) &&
    vil_grid_bicub_corner_in_image(x0 + (n2 - 1) * dx2, y0 + (n2 - 1) * dy2, image) &&
    vil_grid_bicub_corner_in_image(x0 + (n1 - 1) * dx1 + (n2 - 1) * dx2, y0 + (n1 - 1) * dy1 + (n2 - 1) * dy2, image);
