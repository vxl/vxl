...
inline bool
vil_profile_bilin_in_image(double x0, double y0, double x1, double y1, const vil_image_view_base & image)
{
  return x0 >= 1 && y0 >= 1 && x1 >= 1 && y1 >= 1 && x0 + 2 <= image.ni() && y0 + 2 <= image.nj() &&
         x1 + 2 <= image.ni() && y1 + 2 <= image.nj();
}
...
#define VIL_SAMPLE_PROFILE_BILIN_INSTANTIATE(imType, vecType)                                              \
  template void vil_sample_profile_bilin(                                                                  \
    vecType * v, const vil_image_view<imType> & image, double x0, double y0, double dx, double dy, int n); \
  template void vil_sample_profile_bilin_edgena(                                                           \
    vecType * v, const vil_image_view<imType> & image, double x0, double y0, double dx, double dy, int n)
