...
vil_save(const vil_image_view_base & im, const char * filename, const char * file_format)
{
  ...
  vil_image_resource_sptr out =
    vil_new_image_resource(os,
                           ...
                           vil_pixel_format_component_format(im.pixel_format()),
                           file_format);
  if (!out)
  {
...
vil_save_image_resource(const vil_image_resource_sptr & ir, const char * filename, const char * file_format)
{
  ...
  vil_image_resource_sptr out =
    vil_new_image_resource(os, ir->ni(), ir->nj(), ir->nplanes(), ir->pixel_format(), file_format);
  if (!out)
  {
    std::cerr << __FILE__ ": (vil_save) Cannot save to type [" << file_format << "]\n";
    return false;
...
#  define macro(ext, fmt) else if (ext_lower_case == L"." L#ext) file_format = L#fmt
    ...
    macro(v2i, v2i);
#  undef macro
    else std::wcerr << __FILE__ ": assuming pnm format for \'" << filename << "\'\n";
  }
