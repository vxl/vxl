...
template <class T>
struct vil_smart_ptr_T_as_string
{
  static const char *
  str()
  {
...
template <class T>
std::ostream &
operator<<(std::ostream & os, const vil_smart_ptr<T> & r)
{
  return os << "vil_smart_ptr<" << vil_smart_ptr_T_as_string<T>::str() << ">("
...
#define VIL_SMART_PTR_INSTANTIATE(T, ...)         \
  ...
  template <>                                     \
  struct __VA_ARGS__ vil_smart_ptr_T_as_string<T> \
  {                                               \
    static char const *                           \
    str()                                         \
    {                                             \
