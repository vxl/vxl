...
{
  vil_streampos curpos_{ 0 }; // current file pointer.
  unsigned blocksize_;
  std::vector<char *> block_;
  vil_streampos tailpos_{ 0 }; // size of file so far
...
public:
  ...
  vil_streampos
  m_transfer(char * buf, vil_streampos pos, vil_streampos n, bool read);
