...
class vil_stream_url : public vil_stream
{
  ...
  vil_stream_url(const vil_stream_url &)
    : vil_stream()
  {}
  vil_stream_url &
  operator=(const vil_stream_url &)
  {
    return *this;
  }

public:
  vil_stream_url(const char *);
  ...
  vil_streampos
  write(const void * buf, vil_streampos n) override
  {
    return u_ ? u_->write(buf, n) : 0L;
  }
  // strictly speaking, writes should fail, but that isn't useful in any way.
  vil_streampos
  read(void * buf, vil_streampos n) override
  {
    return u_ ? u_->read(buf, n) : 0L;
  }
  vil_streampos
  tell() const override
  {
    return u_ ? u_->tell() : (vil_streampos)(-1L);
  }
  void
  seek(vil_streampos position) override
  {
    if (u_)
      u_->seek(position);
  }
