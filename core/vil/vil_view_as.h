...
template <class T>
inline vil_image_view<typename T::value_type>
vil_view_as_planes(const vil_image_view<T> & v)
{
  ...
  return vil_image_view<comp_type>(v.memory_chunk(),
                                   reinterpret_cast<const comp_type *>(v.top_left_ptr()),
                                   v.ni(),
                                   v.nj(),
                                   ncomponents,
                                   v.istep() * ncomponents,
...
// \relatesalso vil_image_view
template <class T>
inline vil_image_view<vil_rgb<T>>
vil_view_as_rgb(const vil_image_view<T> & v)
{
  ...
  return vil_image_view<vil_rgb<T>>(v.memory_chunk(),
                                    reinterpret_cast<const vil_rgb<T> *>(v.top_left_ptr()),
                                    v.ni(),
                                    v.nj(),
                                    1,
                                    v.istep() / 3,
...
// \relatesalso vil_image_view
template <class T>
inline vil_image_view<vil_rgba<T>>
vil_view_as_rgba(const vil_image_view<T> & v)
{
  ...
  return vil_image_view<vil_rgba<T>>(v.memory_chunk(),
                                     static_cast<const vil_rgba<T> *>(v.top_left_ptr()),
                                     v.ni(),
                                     v.nj(),
                                     1,
                                     v.istep() / 3,
...
vil_view_as_complex(const vil_image_view<T> & v)
{
  ...
  return vil_image_view<std::complex<T>>(v.memory_chunk(),
                                         reinterpret_cast<const std::complex<T> *>(v.top_left_ptr()),
                                         v.ni(),
                                         v.nj(),
                                         v.nplanes() / 2,
                                         v.istep() / 2,
...
{
  return vil_image_view<T>(img.memory_chunk(),
