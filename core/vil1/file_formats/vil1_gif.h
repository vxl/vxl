...
struct vil1_gif_file_format : public vil1_file_format
{
  ...
  vil1_image_impl *
  make_output_image(vil1_stream *, int, int, int, int, int, vil1_component_format) override;
};
...
struct vil1_gif_color_map
{
  int size;
  char * cmap;
  vil1_gif_color_map(int sz)
    : size(sz)
    , cmap(new char[3 * size])
  {}
  ~vil1_gif_color_map()
  {
    delete[] cmap;
...
struct vil1_gif_loader_saver : public vil1_image_impl
{
  ...
  bool
  get_section(int image, void * buf, int x0, int y0, int width, int height) const;
  bool
  put_section(const void *, int, int, int, int) override;
  bool
  put_section(int, const void *, int, int, int, int)
  {
    return false;
  }
...
struct vil1_gif_loader_saver_proxy : public vil1_image_impl
{
  ...
  bits_per_component() const override
  {
    return other->bits_per_component();
  }

  enum vil1_component_format
  component_format() const override
  {
    return other->component_format();
  }
