...
public:
  vil1_iris_generic_image(vil1_stream * is, const char * imagename = "");
  ...
  {
    return bytes_per_component_ * 8;
  }
  int
  bytes_per_pixel() const
  {
    return bytes_per_component_ * planes_;
  }
  ...
  bool
  get_section(void * buf, int x0, int y0, int, int) const override;
  bool
  ...
  bool
  get_section_rle(void * ib, int x0, int y0, int xs, int ys) const;

  // Read a plain section
  bool
  get_section_verbatim(void * ib, int x0, int y0, int xs, int ys) const;
};
