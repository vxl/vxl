...
const char * vil1_png_format_tag = "png";
...
vil1_image_impl *
vil1_png_file_format::make_input_image(vil1_stream * is)
{
...
bool
vil1_png_generic_image::read_header()
{
  if (!p->ok)
    return false;
 ...
 if (p->info_ptr->color_type == PNG_COLOR_TYPE_PALETTE) { maxval = 255; }
  ...
  if (png_get_valid(p->png_ptr, p->info_ptr, PNG_INFO_sBIT))
    problem("LAZY AWF! PNG_INFO_sBIT");
...
bool
vil1_png_generic_image::write_header()
{
  if (!p->ok)
    return false;
...
bool
vil1_png_generic_image::get_section(void * buf, int x0, int y0, int xs, int ys) const
{
  if (!p->ok)
    return false;
