...
inline void
swap_endian(vxl_uint_32 &)
{}
#endif

// Equivalent of ntoh
bool
read_uint_32(vil1_stream * vs, vxl_uint_32 & word)
{
  if (vs->read(&word, 4) < 4)
    return false;
  swap_endian(word);
...
// Equivalent of hton
bool
write_uint_32(vil1_stream * vs, vxl_uint_32 word)
{
  swap_endian(word);
...
vil1_image_impl *
vil1_ras_file_format::make_input_image(vil1_stream * vs)
{
  ...
  : vs_(vs)
{
  ...
  if (planes != 1)
  {
    std::cerr << __FILE__ << ": can only handle 1 plane\n";
    return;
  }
  if (components != 3 && components != 1)
  {
    std::cerr << __FILE__ << ": can't handle " << components << " components\n";
    return;
  }
  if (bits_per_component != 8)
  {
    std::cerr << __FILE__ << ": can't handle " << bits_per_component << " bits per component\n";
    return;
  }
...
bool
vil1_ras_generic_image::read_header()
{
  // Go to start of file
  vs_->seek(0);
...
bool
vil1_ras_generic_image::write_header()
{
  vs_->seek(0);
...
bool
vil1_ras_generic_image::get_section(void * buf, int x0, int y0, int xs, int ys) const
{
