  ...
  if( endian_consistent_ )
    ...
    for( int p = 0; p < planes_; ++p )
      ...
      for( int y = y0; y < y0 + ys; ++y )
        ...
        is_->seek(start_of_data_ + p * width_ * height_ * bits_per_component_ / 8 + bits_per_component_
                  * (y * width_ + x0) / 8);
...
bool vil1_viff_generic_image::get_section_rgb_byte(void * /*buf*/, int /*x0*/, int /*y0*/, int /*width*/,
                                                   int /*height*/) const
{
