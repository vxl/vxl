...
test_image_equal(const char * type_name, const vil1_image & image, const vil1_image & image2, bool exact = true)
{
  ...
  {
    std::cout << type_name << ": pixel size is " << cell_bits2 << " instead of " << cell_bits << std::endl
              << std::flush;
    return false;
  }
  ...
  {
    std::cout << type_name << ": pixel format is " << vil1_print(vil1_pixel_format(image2)) << " instead of "
  ...
  {
    std::cout << type_name << ": component format is " << vil1_print(image2.component_format()) << " instead of "
              << vil1_print(image.component_format()) << std::endl
              << std::flush;
    return false;
  }

  TEST("Image bit count", num_bits, num_bits2);
  TEST("Image byte count", image.get_size_bytes() == num_bytes && image2.get_size_bytes() == num_bytes2, true);
  if (num_bits != num_bits2 || image.get_size_bytes() != num_bytes || image2.get_size_bytes() != num_bytes2)
  {
    std::cout << type_name << " in:  " << sizex << 'x' << sizey << ", " << components << " components, " << planes
              << " planes, " << cell_bits << " cell bits.\n"
              << type_name << " out: " << sizex2 << 'x' << sizey2 << ", " << components2 << " components, " << planes2
              << " planes, " << cell_bits2 << " cell bits.\n"
              << type_name << ": " << num_bits << " bits in, " << num_bits2 << " bits out, " << image2.get_size_bytes()
              << " bytes\n"
              << std::flush;
    return false;
  }
...
#ifdef DEBUG
      if (++bad < 20)
        std::cout << "\n byte " << i << " differs: " << (int)image_buf[i] << " --> " << (int)image_buf2[i]
                  << std::flush;
#else
      ++bad;
      std::cout << '.' << std::flush;
                         ...
                         bool exact = true)        //!< require read back image identical
{
...

  {
    ...
    tst = !(std::strcmp(
      type_name,
      image2.file_format())); // NOTE: the analyzer notes it's invalid to pass null to strcmp, and since 'type_name' was
                              // tested above against null, it assumes this could occur. From the looks of it, this test
                              // function is meant to never be passed null, so I added an assert and removed the test.
