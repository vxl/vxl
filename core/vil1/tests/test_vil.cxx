...
test_vil(int argc, char * argv[])
{
  ...
  TEST("vil1_from_vil_image_view sizes",
       vil1_mem.width() == int(image0.ni()) && vil1_mem.height() == int(image0.nj()),
       true);
  ...
  for (int j = 0; j < vil_mem.height(); ++j)
    for (int i = 0; i < vil_mem.width(); ++i)
      if (vil_mem(i, j) != (double)i - (double)j)
        bad_pixels = true;
