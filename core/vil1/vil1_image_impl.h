...
inline const char *
vil1_print(vil1_component_format f)
{
  switch (f)
  {
    case VIL1_COMPONENT_FORMAT_UNKNOWN:
      return "VIL1_COMPONENT_FORMAT=UNKNOWN";
    case VIL1_COMPONENT_FORMAT_UNSIGNED_INT:
      return "VIL1_COMPONENT_FORMAT=unsigned int";
    case VIL1_COMPONENT_FORMAT_SIGNED_INT:
      return "VIL1_COMPONENT_FORMAT=signed int";
    case VIL1_COMPONENT_FORMAT_IEEE_FLOAT:
      return "VIL1_COMPONENT_FORMAT=IEEE float";
    case VIL1_COMPONENT_FORMAT_COMPLEX:
      return "VIL1_COMPONENT_FORMAT=complex";
    default:
      return "VIL1_COMPONENT_FORMAT_INVALID";
  }
}
...
public:
  ...
  virtual enum vil1_component_format
  component_format() const = 0;
  ...
  //   total size of BUF in bytes should be (bits_per_component * components +
  //   7) / 8 i.e. rounding to the next multiple of 8 bits (only correct if 1
  //   byte = 8 bits)
  virtual bool
  get_section(void * buf, int x0, int y0, int width, int height) const = 0;

  //: Copy plane PLANE of BUF to this.
  // The buffer should look like this for each pixel:
  // component0(plane0,plane1,plane2,...),component1(plane0,plane1,plane2,...),...\n
  // total size of BUF in bytes should be (bits_per_component * components + 7)
  // / 8 i.e. rounding to the next multiple of 8 bits (only correct if 1 byte =
  // 8 bits)
  virtual bool
  put_section(const void * buf, int x0, int y0, int width, int height) = 0;
  ...
  virtual bool
  set_property(const char * tag, const void * property_value = nullptr) const;

  //: Return the name of the class;
  virtual std::string
  is_a() const
  {
    return "vil1_image_impl";
  }
