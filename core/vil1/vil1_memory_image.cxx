...
#ifdef DEBUG
  std::cout << "copying " << that.impl() << " to a " << that.planes() << 'x' << that.width() << 'x' << that.height()
            << 'x' << that.components() << " memory image, " << that.bits_per_component() << " bpc, component format "
            << that.component_format() << '\n'
            << std::flush;
#endif
  vil1_memory_image mem(
    that.planes(), that.width(), that.height(), that.components(), that.bits_per_component(), that.component_format());
...
void
vil1_memory_image::resize(int width, int height)
{
  assert(ptr != nullptr);
...
void
vil1_memory_image::resize(int planes, int width, int height)
{
  assert(ptr != nullptr);
  auto * mi = (vil1_memory_image_impl *)ptr;
...
void
vil1_memory_image::assert_size(int width, int height) const
{
...
vil1_memory_image::vil1_memory_image(void * buf, int planes, int w, int h, vil1_pixel_format_t pixel_format)
...
vil1_memory_image::vil1_memory_image(void * buf, int w, int h, vil1_pixel_format_t pixel_format)
...
void
vil1_memory_image::recache_from_impl()
{
  cache_from_impl;
}
