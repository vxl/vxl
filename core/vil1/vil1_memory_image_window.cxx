...
void
vil1_memory_image_window::init(int centre_x, int centre_y, int mask_size)
{
...
float
vil1_memory_image_window::mean_intensity()
{
  int tot = 0;
                                       ...
                                       int early_exit_level)
{
  ...
  int row_start = 0;
  if (row_start < -mask1_row_index_)
    row_start = -mask1_row_index_;
  if (row_start < -mask2_row_index)
    row_start = -mask2_row_index;
  int row_end = mask_size_;
  if (row_end >= int(image1_.width()) - mask1_row_index_)
    row_end = image1_.width() - mask1_row_index_ - 1;
  if (row_end >= int(image2.width()) - mask2_row_index)
    row_end = image2.width() - mask2_row_index - 1;
  int col_start = 0;
  if (col_start < -mask1_col_index_)
    col_start = -mask1_col_index_;
  if (col_start < -mask2_col_index)
    col_start = -mask2_col_index;
  int col_end = mask_size_;
  if (col_end >= int(image1_.width()) - mask1_col_index_)
    col_end = image1_.width() - mask1_col_index_ - 1;
  if (col_end >= int(image2.width()) - mask2_col_index)
    col_end = image2.width() - mask2_col_index - 1;
