...
public:
  ...
  int
  sum_abs_diff(const vil1_memory_image_of<vxl_byte> & image2,
               int centre2_x,
               int centre2_y,
               int early_exit_level = vil1_memory_image_window_maxint);

  int
  sum_sqr_diff(const vil1_memory_image_of<vxl_byte> & image2,
               int centre2_x,
               int centre2_y,
               int early_exit_level = vil1_memory_image_window_maxint);

  int
  normalised_sum_abs_diff(const vil1_memory_image_of<vxl_byte> & image2,
                          int centre2_x,
                          int centre2_y,
  ...
  double
  normalised_cross_correlation(const vil1_memory_image_of<vxl_byte> & image2, int centre2_x, int centre2_y);
...
protected:
  ...
  void
  init(int centre_x, int centre_y, int mask_size);

private:
  // Helpers-------------------------------------------------------------------
  vil1_memory_image_window(const vil1_memory_image_window & that) = delete;
  vil1_memory_image_window &
  operator=(const vil1_memory_image_window & that) = delete;
};
