...
vil1_streampos
vil1_stream_section::write(const void * buf, vil1_streampos n)
{
  ...
  const vil1_streampos nb = underlying_->write(buf, n);
  if (nb != -1L)
    current_ += nb;
  return nb;
}
...
vil1_streampos
vil1_stream_section::read(void * buf, vil1_streampos n)
{
  assert(n >= 0); // wouldn't you want to be told?
  ...
  const vil1_streampos nb = underlying_->read(buf, n);
  if (nb != -1L)
    current_ += nb;
  return nb;
}
...
void
vil1_stream_section::seek(vil1_streampos position)
{
