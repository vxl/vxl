...
template <class T>
vil_image_view<T>
vil1_to_vil_image_view(const vil1_memory_image_of<T> & vil1_im)
{
  ...
  return vil_image_view<T>(
    tl, vil1_im.cols(), vil1_im.rows(), vil1_im.planes(), &vil1_im(1, 0) - tl, &vil1_im(0, 1) - tl, planestep);
}
...
vil1_from_vil_image_view(const vil_image_view<T> & vil_im)
{
  ...
  return vil1_memory_image_of<T>(const_cast<T *>(vil_im.top_left_ptr()), vil_im.ni(), vil_im.nj());
}

//: Create a vil_image_resource from a vil1_image.
inline vil_image_resource_sptr
vil1_to_vil_image_resource(const vil1_image & vil1_im);
  ...
  pixel_format() const override
  {
    ...
    switch (src_.component_format())
    {
      ...
      case VIL1_COMPONENT_FORMAT_UNSIGNED_INT:
        if (src_.bits_per_component() == 1)
          return VIL_PIXEL_FORMAT_BOOL;
        else if (src_.bits_per_component() <= 8)
          return VIL_PIXEL_FORMAT_BYTE;
        else if (src_.bits_per_component() <= 16)
          return VIL_PIXEL_FORMAT_UINT_16;
        else if (src_.bits_per_component() <= 32)
      ...
      case VIL1_COMPONENT_FORMAT_IEEE_FLOAT:
        if (src_.bits_per_component() == 32)
          return VIL_PIXEL_FORMAT_FLOAT;
        else if (src_.bits_per_component() <= 64)
...
#define macro(F, T)                                                                                                 \
  case F:                                                                                                           \
  {                                                                                                                 \
    vil_memory_chunk_sptr chunk = new vil_memory_chunk(ni * nj * nplanes() * sizeof(T), F);                         \
    src_.get_section(chunk->data(), i0, j0, ni, nj);                                                                \
    return new vil_image_view<T>(chunk, (const T *)chunk->data(), ni, nj, nplanes(), nplanes(), ni * nplanes(), 1); \
  }
