...
{
  testlib_test_amoeba_cost1(int n)
    : vnl_cost_function(n)
  {}
...
test_amoeba()
{
  ...
  for (unsigned n = 1; n <= 4; ++n)
  {
    ...
    for (unsigned int i = 0; i < n; ++i)
      err += std::fabs(x[i] - i);
    TEST_NEAR("Quadratic, starting at (1,1,1...)",
              err,
              0.0,
              1e-5); // fails from n=6 onwards: result is 0.379827 for n=6, 0.0812755 for n=7, 7.01668 for n=8.
    ...
    for (unsigned int i = 0; i < n; ++i)
      err += std::fabs(x[i] - i);
    TEST_NEAR("Quadratic, starting at (0,0,0...) using minimise(x,dx)",
              err,
              0,
              1e-5); // fails from n=7 onwards, value approx. 5.0074 for n=7, between 5.95 and 6.05 for n=8
    std::cout << "Number of evaluations: " << amoeba2.get_num_evaluations() << std::endl
    ...
    TEST_NEAR("Quadratic, starting at (0,0,0...) using minimise(x,dx)",
              err,
              0,
              1e-5); // fails from n=6 onwards: result is 0.0000167217 for n=6, 4.80315 for n=7, 6.64319 for n=8.
  }
}
