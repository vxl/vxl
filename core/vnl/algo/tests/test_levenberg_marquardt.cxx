...
struct linear_est_throw : public vnl_least_squares_function
{
  class exception {};
  linear_est_throw(vnl_matrix<double> const& A, vnl_vector<double> const& b, bool with_grad)
  : vnl_least_squares_function(A.cols(), A.rows(), with_grad ? use_gradient : no_gradient),
    A_(A), b_(b)
  { assert(A.rows() == b.size()); }
...
static
void do_linear_exception_test( bool with_grad )
{
  vnl_matrix<double> A(6,2,1.0);
  vnl_vector<double> b(6);
  ...
  linear_est_throw f(A, b, with_grad);
  vnl_levenberg_marquardt lm(f);
  vnl_vector<double> x(2,-1000.0);   // init can be far off
  // since obj function is linear
  // high precision can be achieved
  ...
  TEST("Caught exception", caught, true);
}
...
#if VCL_HAS_EXCEPTIONS
  do_linear_exception_test(false);
#endif //VCL_HAS_EXCEPTIONS
}
