...
void test_sparse_lu()
{
  ...
  vcl_cout << "birth-death upper error bound = " << upbnd << '\n';
  vcl_cout << "mean passage time from adjacent state = " << -(xlarge[n/2-1]-1)/s << '\n';
  vcl_cout << "mean passage time from S0 = " << -(xlarge[0]-1)/s << '\n';
  vcl_cout << "ratio =" << (1.0-xlarge[0])/(1-xlarge[n/2-1])<< '\n';
  cond = lubdl.rcond();
