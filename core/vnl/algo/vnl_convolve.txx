...
vnl_vector<U> vnl_convolve_cyclic_using_fft(vnl_vector<T1> const& v1, vnl_vector<T2> const& v2, U *)
{
  ...
  vnl_vector<C> w1(n, C(0) ); for( unsigned i = 0; i < n; ++i )
    {
    w1[i] = v1[i];
    }
  vnl_vector<C> w2(n, C(0) ); for( unsigned i = 0; i < n; ++i )
    {
    w2[i] = v2[i];
    }
...
vnl_vector<U> vnl_convolve_using_fft(vnl_vector<T1> const& v1, vnl_vector<T2> const& v2, U *, int n)
{
  ...
  vnl_vector<U> w1(n, U(0) ); for( unsigned i = 0; i < v1.size(); ++i )
    {
    w1[i] = U(v1[i]);
    }
  vnl_vector<U> w2(n, U(0) ); for( unsigned i = 0; i < v2.size(); ++i )
    {
    w2[i] = U(v2[i]);
    }
