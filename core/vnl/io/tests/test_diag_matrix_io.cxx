...
test_diag_matrix_double_io()
{
  ...
  TEST("Created vnl_diag_matrix_test_io.bvl.tmp for writing", (!bfs_out), false);
  vsl_b_write(bfs_out, diag_mat_out);
  bfs_out.close();

  vsl_b_ifstream bfs_in("vnl_diag_matrix_test_io.bvl.tmp");
  TEST("Opened vnl_diag_matrix_test_io.bvl.tmp for reading", (!bfs_in), false);
  ...
  TEST("diag_mat_out.diagonal() == diag_mat_in.diagonal()", diag_mat_out.diagonal() == diag_mat_in.diagonal(), true);
