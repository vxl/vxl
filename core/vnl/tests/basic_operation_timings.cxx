                 ...
                 int n_loops)
{
  ...
  std::cout << "  Mean: " << stats.mean() << "ns  +/-"
            << stats((unsigned)(nstests * 0.75)) - stats((unsigned)(nstests * 0.25)) << "ns\n\n";
}
...
template <class T>
void
mat_x_vec(const vnl_matrix<T> & s1, const std::vector<vnl_vector<T>> & s2, int n_loops)
{
  vnl_vector<double> stats(nstests);
  for (unsigned st = 0; st < nstests; ++st)
  {
  ...
  std::cout << "  Mean: " << stats.mean() << "us  +/-"
            << stats((unsigned)(nstests * 0.75)) - stats((unsigned)(nstests * 0.25)) << "us\n\n";
}

template <class T>
void
vec_x_mat(const std::vector<vnl_vector<T>> & s1, const vnl_matrix<T> & s2, int n_loops)
{
  vnl_vector<double> stats(nstests);
  for (unsigned st = 0; st < nstests; ++st)
  {
