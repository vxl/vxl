...
test_operators()
{
  ...
  c = a * b;
  TEST("product", c, std::complex<double>(-35., 5.));
  // Numerically could have small floating point truncation errors
  c = a / b;
  TEST_NEAR("quotient", c, std::complex<double>(-0.7, -0.1), 1e-12);
  // Numerically deterministic double precision
  a += b;
  TEST("+=", a, std::complex<double>(2., -1.));
  a -= b;
  TEST("-=", a, std::complex<double>(-5.));
  a *= b;
