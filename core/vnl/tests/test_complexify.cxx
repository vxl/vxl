...
test_complexify_instance(const ValueType & re, const ValueType & im)
{
  ...
  c_vector = vnl_complexify(r_vector, i_vector);
  for (unsigned int i = 0; i < length; ++i)
  {
    TEST("vnl_vector real", re, c_vector.get(i).real());
  ...
  c_vector_fixed = vnl_complexify(r_vector_fixed, i_vector_fixed);
  for (unsigned int i = 0; i < length; ++i)
  {
    TEST("vnl_vector_fixed real", re, c_vector_fixed.get(i).real());
  ...
  c_matrix = vnl_complexify(r_matrix, i_matrix);
  for (unsigned int c = 0; c < length; ++c)
  {
    for (unsigned int r = 0; r < length; ++r)
    {
  ...
  c_matrix_fixed = vnl_complexify(r_matrix_fixed, i_matrix_fixed);
  for (unsigned int c = 0; c < length; ++c)
  {
    for (unsigned int r = 0; r < length; ++r)
    {
  ...
  c_diag_matrix = vnl_complexify(r_diag_matrix, i_diag_matrix);
  for (unsigned int i = 0; i < length; ++i)
  {
    TEST("vnl_diag_matrix real", re, c_diag_matrix.get(i, i).real());
  ...
  c_diag_matrix_fixed = vnl_complexify(r_diag_matrix_fixed, i_diag_matrix_fixed);
  for (unsigned int i = 0; i < length; ++i)
  {
    TEST("vnl_diag_matrix_fixed real", re, c_diag_matrix_fixed.get(i, i).real());
