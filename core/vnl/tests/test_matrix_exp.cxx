...
void test_matrix_exp()
{
  ...
  vnl_test_assert("expX(0,0)", vcl_abs(expX(0,0)-0.6221833130) < 1e-10);
  vnl_test_assert("expX(0,1)", vcl_abs(expX(0,1)+0.7825192869) < 1e-10);
  vnl_test_assert("expX(1,1)", vcl_abs(expX(1,1)-0.1379544126) < 1e-10);
  vnl_test_assert("expX(2,2)", vcl_abs(expX(2,2)-0.2501918781) < 1e-10);

  vnl_matrix<double> rotv = vnl_rotation_matrix(v);
  vnl_matlab_print(vcl_cout, rotv, "rotate exp([v])");
  vnl_test_assert("rotv == expX", (rotv-expX).fro_norm() < 1e-10);
}
