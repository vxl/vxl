...
int testRNPolyRoots() {
  ...
  vnl_real_npolynomial poly1(f1,p1); cout << poly1; // X^2 +Y^2 -1
  
  vnl_vector<double> f2(2);    f2(0) = 1;   f2(1) = -1;
  vnl_matrix<int> p2(2,2,0.0); p2(0,0) = 1;
  vnl_real_npolynomial monom1(f2,p2); cout << monom1; // X-1
  ...
  poly2 = poly2 + monom3; cout << poly2; // (X-1)^2 +Y^2 -1 = X^2 -2X +Y^2
  ...
  for (rp=roots_r.begin(),ip=roots_i.begin(); rp!=roots_r.end(); ++rp,++ip)
    cout << "  REAL " << *((*rp)) << " IMAG " << *((*ip)) << endl;

  // Real intersection of two ellipses, both centered in (0,0):

  f1(0) = 1;   f1(1) = 2;
  vnl_real_npolynomial poly3(f1,p1); cout << poly3; // X^2 +2 Y^2 -1
  
  f1(0) = 2;   f1(1) = 1;
  vnl_real_npolynomial poly4(f1,p1); cout << poly4; // 2 X^2 +Y^2 -1
  ...
  f1(0) = 2;   f1(1) = 3;
  vnl_real_npolynomial poly5(f1,p1); cout << poly5; // 2 X^2 +3 Y^2 -1
