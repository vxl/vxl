...
template <class T>
// the primary template is empty, by design.
...
VCL_DEFINE_SPECIALIZATION struct vnl_complex_traits<vnl_rational>
  ...
  static vcl_complex<vnl_rational> complexify(vnl_rational x)
  {
    return vcl_complex<vnl_rational>(x,
                                     vnl_rational(0, 1) );
  }
  };
...
VCL_DEFINE_SPECIALIZATION struct vnl_complex_traits<vcl_complex<vnl_rational> >
  {
  enum { isreal = false };
  static vcl_complex<vnl_rational> conjugate(vcl_complex<vnl_rational> x)
  {
    return vcl_complex<vnl_rational>(
      x.real(), -x.imag() );
  }
