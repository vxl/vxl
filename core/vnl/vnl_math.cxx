...
bool isnan(double x)
{
  return bMe(&x, 0x7ff00000L,
             sz_d) && (bMp(&x, 0x000fffffL, sz_d) || bMp(&x, 0xffffffffL, 1 - sz_d) );
}
bool isnan(long double x)
{
  if( sizeof(long double) ==
      8 ) { return bMe(&x, 0x7ff00000L, sz_l) && (bMp(&x, 0x000fffffL, sz_l) || bMp(&x, 0xffffffffL, 1 - sz_d) ); }
...
#  if defined LDBL_MANT_DIG && LDBL_MANT_DIG <= 53
                                                       { return bMe(&x, 0x4001ffffL,
                                                                    sz_l) && bMp(&x, 0x40000000, sz_l - 4); }
#  else
...
bool isfinite(long double x)
{
  if( sizeof(long double) == 8 ) {return !bMe(&x, 0x7ff00000L, sz_l); }
    ...
    {
    return !bMe(&x, 0xbfff7fffL, sz_l) && !bMe(&x, 0x4001ffffL, sz_l);
    }
...
bool isinf(long double x)
{
  if( sizeof(long double) == 8 ) {return bMe(&x, 0x7ff00000L, sz_l) && !bMp(&x, 0x000fffffL, sz_l); }
    ...
    {
    return (bMe(&x, 0xbfff7fffL, sz_l) || bMe(&x, 0x4001ffffL, sz_l) ) && !bMp(&x, 0x40000000, sz_l - 4);
    }
...
double vnl_huge_val(double)
{
  return                             // 2^2047
         ...
         05475440761 932241415603154386836504980458750988751948260533980288191 \
...
OutType hypot(const T x, const T y)
{
  T ret_val;
    {
