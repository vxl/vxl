...
template <class T>
VNL_EXPORT bool
vnl_matlab_read_or_die(std::istream &, vnl_vector<T> &, const char * name = nullptr);
template <class T>
VNL_EXPORT bool
vnl_matlab_read_or_die(std::istream &, vnl_matrix<T> &, const char * name = nullptr);
...
template <class T>
VNL_EXPORT void
vnl_matlab_read_data(std::istream & s, T * ptr, unsigned n);
...
public:
  ...
  operator!() const;
  void
  read_next(); // skip to next header in file
  ...
  is_rowwise() const;
  bool
  is_bigendian() const; // don't use this
  long
  rows() const
  {
    return hdr.rows;
