...
class VNL_EXPORT vnl_matrix_fixed_ref : public vnl_matrix_fixed_ref_const<T,num_rows,num_cols>
{
 ...
 public:
  ...
  vnl_matrix_ref<T> as_ref() const { return vnl_matrix_ref<T>( num_rows, num_cols, const_cast<T*>(data_block()) ); }
  vnl_matrix<T> as_matrix() const { return make_initialized_matrix<T, num_rows, num_cols>(num_rows*num_cols, data_block()); }
  ...
  explicit operator vnl_matrix_ref<T>() const { return vnl_matrix_ref<T>( num_rows, num_cols, const_cast<T*>(data_block()) ); }
