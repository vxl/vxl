...
template <class T>
inline void
vnl_na_extract_type(std::istream & is, T & value)
{
  if (!is)
    return;
...
void
vnl_na_insert(std::ostream & os, double x)
{
  if (vnl_na_isna(x))
    os << "NA";
  else
...
//: Write a floating point number or "NA" to a stream.
void
vnl_na_insert(std::ostream & os, float x)
{
  if (vnl_na_isna(x))
    os << "NA";
  else
