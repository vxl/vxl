...
template <class T>
T
vnl_polynomial<T>::evaluate(const T & x) const
{
...
vnl_polynomial<T>::operator+(const vnl_polynomial<T> & f) const
{
  ...
  while (sum.end() != sum.begin() && sum.back() == T(0))
    sum.pop_back();
...
vnl_polynomial<T>::operator%(const vnl_polynomial<T> & f) const
{
  ...
  // normalise the result, viz. such that the highest order coefficient is zero:
  while (diff.end() != diff.begin() && diff.back() == T(0))
    diff.pop_back();
...
template <class T>
void
vnl_polynomial<T>::print(std::ostream & os) const
{
  ...
  for (int i = degree(); i >= 0; --i)
  {
    if (coeffs_[i] == T(0))
      continue;
    os << ' ';
    if (coeffs_[i] > T(0) && !first_coeff)
      os << '+';
    if (i == 0)
      os << coeffs_[i];
    else if (coeffs_[i] == -T(1))
      os << '-';
    else if (coeffs_[i] != T(1))
      os << coeffs_[i] << ' ';
    if (i == 1)
