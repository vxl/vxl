...
vnl_matrix<T> vnl_power(vnl_matrix<T> const& m, int n)
{
  ...
  if (n == 0) {
    ...
    return vnl_matrix<T>(vnl_power(vnl_matrix_fixed<T,1,1>(m),n));
  else if (n < 0 && m.rows() == 2)
    return vnl_matrix<T>(vnl_power(vnl_matrix_fixed<T,2,2>(m),n));
  else if (n < 0 && m.rows() == 3)
    return vnl_matrix<T>(vnl_power(vnl_matrix_fixed<T,3,3>(m),n));
  else if (n < 0 && m.rows() == 4)
    return vnl_matrix<T>(vnl_power(vnl_matrix_fixed<T,4,4>(m),n));
  else {
