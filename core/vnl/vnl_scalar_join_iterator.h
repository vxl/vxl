...
protected:
  unsigned n1;
  unsigned n2;
  vcl_list<indexed_pair<T> >* pI1;
  vcl_list<indexed_pair<T> >* pI2;
  vcl_list<indexed_pair<T> >& I1;
  vcl_list<indexed_pair<T> >& I2;
  vcl_list<indexed_pair<T> >::iterator index1;
  vcl_list<indexed_pair<T> >::iterator index2;
};
...
public:
  ...
  indexed_pair() {}
  indexed_pair(const T* object_, int original_index_):object(object_), original_index(original_index_) {}

  bool operator == (const indexed_pair<T>& that) const;
  bool operator <  (const indexed_pair<T>& that) const;
