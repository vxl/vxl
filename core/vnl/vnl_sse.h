...
public:
  static VNL_SSE_FORCE_INLINE void element_product(const double* x, const double* y, double* r, unsigned n)
  {
    ...
    if( vcl_ptrdiff_t(x) % 16 || vcl_ptrdiff_t(y) % 16  || vcl_ptrdiff_t(r) % 16 )
      ...
      for( int i = n - 4; i >= 0; i -= 4 )
        ...
        VNL_SSE_HEAP_STORE(pd)(r + i + 2,
                               _mm_mul_pd(VNL_SSE_HEAP_LOAD(pd)(x + i + 2), VNL_SSE_HEAP_LOAD(pd)(y + i + 2) ) );
        }
      }
  }
  ...
  static VNL_SSE_FORCE_INLINE void vector_x_matrix(const double* v, const double* m, double* r, unsigned rows,
                                                   unsigned cols)
  {
  ...
  static VNL_SSE_FORCE_INLINE void matrix_x_vector(const double* m, const double* v, double* r, unsigned rows,
                                                   unsigned cols)
  {
...
public:
  ...
  static VNL_SSE_FORCE_INLINE void vector_x_matrix(const float* v, const float* m, float* r, unsigned rows,
                                                   unsigned cols)
  {
  ...
  static VNL_SSE_FORCE_INLINE void matrix_x_vector(const float* m, const float* v, float* r, unsigned rows,
                                                   unsigned cols)
  {
...
#  endif // 0
      ...
      switch( c_left )
        {
        case 3: accum =
          _mm_add_ps(_mm_mul_ps(_mm_load1_ps(v + j), _mm_set_ps(*(r4 + j), *(r3 + j), *(r2 + j), *(r1 + j) ) ), accum);
          j++;
        case 2: accum =
          _mm_add_ps(_mm_mul_ps(_mm_load1_ps(v + j), _mm_set_ps(*(r4 + j), *(r3 + j), *(r2 + j), *(r1 + j) ) ), accum);
          j++;
