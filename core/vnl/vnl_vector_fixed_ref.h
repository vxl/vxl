...
#if 0 // This version of class vnl_vector_fixed_ref_const is commented out
template <class T, unsigned n>
class vnl_vector_fixed_ref_const
{
 ...
 private:
  ...
  const vnl_vector_fixed_ref_const<T,n> & operator=(const vnl_vector_fixed_ref<T,n> & i_Input) const
  {
    assert(false);
    return *this;
  }
...
#if 0
  //: Copy data from a vector fixed ref
  // The dimensions must match.
  vnl_vector_fixed<T,n>& operator=( const vnl_vector_fixed_ref<T,n>& rhs) {
    assert( n == rhs.size() );
    vcl_memcpy( data_, rhs.data_block(), size() );
    return *this;
  }
#endif // 0
...
//: \relates vnl_vector_fixed
template<class T, unsigned int n>
inline bool operator==( const vnl_vector_fixed_ref_const<T,n>& a, const vnl_vector_fixed_ref_const<T,n>& b )
{
