...
template <class T>
void
test_gaussian_type(T epsilon, const std::string & type_name)
{
  // test dimension, zero variance
  {
    ...
    TEST(("var size <" + type_name + "> variable").c_str(), tmp.rows() == tmp.columns() && tmp.rows() == 0, true);
    tmp = gauss.covariance();
    TEST(("var size <" + type_name + "> variable").c_str(), tmp.rows() == tmp.columns() && tmp.rows() == 3, true);
    tmp = gauss_init.covariance();
    TEST(("var size <" + type_name + "> variable").c_str(), tmp.rows() == tmp.columns() && tmp.rows() == 10, true);
    ...
    TEST(("zero default mean <" + type_name + "> variable").c_str(), gauss.mean(), zero_vector.as_ref());

    vnl_matrix_fixed<T, 3, 3> zero_matrix(T(0));
    TEST(("zero default variance <" + type_name + "> fixed").c_str(), gauss3.covariance(), zero_matrix);
    TEST(("zero default variance <" + type_name + "> scalar").c_str(), gauss1.covariance(), T(0));
    TEST(("zero default variance <" + type_name + "> variable").c_str(), gauss.covariance(), zero_matrix.as_ref());
  ...
  // cumulative probability, box probability
  {
    ...
    TEST_NEAR(("covar <" + type_name + "> fixed").c_str(), (gauss3.covariance() - covar).array_inf_norm(), 0, epsilon);
    TEST(("mean <" + type_name + "> scalar").c_str(), gauss1.mean(), mean[0]);
    TEST_NEAR(("covar <" + type_name + "> scalar").c_str(), gauss1.covariance(), covar(0, 0), epsilon);
    TEST(("mean <" + type_name + "> variable").c_str(), gauss.mean(), mean.as_ref());
    TEST_NEAR(("covar <" + type_name + "> variable").c_str(),
              (gauss.covariance() - covar.as_ref()).array_inf_norm(),
              0,
              epsilon);
    ...
    TEST_NEAR(("compute_covar <" + type_name + "> fixed").c_str(), (S3 - covar).array_inf_norm(), 0, epsilon);
    ...
    TEST_NEAR(("compute_covar <" + type_name + "> scalar").c_str(), v1, covar(0, 0), epsilon);
    ...
    TEST_NEAR(("compute_covar <" + type_name + "> variable").c_str(), (S - covar).array_inf_norm(), 0, epsilon);
    ...
    TEST_NEAR(
      ("mahalanobis dist <" + type_name + "> fixed").c_str(), gauss3.sqr_mahal_dist(test_pt), sqr_mahal_dist, epsilon);
    ...
    T prob3 = static_cast<T>(1.0 / std::sqrt(two_pi * two_pi * two_pi * vnl_determinant(covar)) *
                             std::exp(-sqr_mahal_dist / 2));
    T prob1 = static_cast<T>(1.0 / std::sqrt(two_pi * covar(0, 0)) * std::exp(-d[0] * d[0] / (2 * covar(0, 0))));
