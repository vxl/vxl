...
template <class T>
void
test_gaussian_indep_type(T epsilon, const std::string & type_name)
{
  // test dimension, zero variance
  {
    ...
    TEST(("var size <" + type_name + "> variable").c_str(), gauss_default.covariance().size(), 0);
    TEST(("var size <" + type_name + "> variable").c_str(), gauss.covariance().size(), 3);
    TEST(("var size <" + type_name + "> variable").c_str(), gauss_init.covariance().size(), 10);
    ...
    TEST(("zero default variance <" + type_name + "> fixed").c_str(), gauss3.covariance(), zero_vector);
    TEST(("zero default variance <" + type_name + "> scalar").c_str(), gauss1.covariance(), T(0));
    TEST(("zero default variance <" + type_name + "> variable").c_str(), gauss.covariance(), zero_vector.as_ref());
  ...
  // cumulative probability, box probability
  {
    ...
    T sqr_mahal_dist = d[0] * d[0] / var[0] + d[1] * d[1] / var[1] + d[2] * d[2] / var[2];
    ...
    TEST(
      ("mahalanobis dist <" + type_name + "> scalar").c_str(), gauss1.sqr_mahal_dist(test_pt[0]), d[0] * d[0] / var[0]);
    TEST(("mahalanobis dist <" + type_name + "> variable").c_str(),
         gauss.sqr_mahal_dist(test_pt.as_ref()),
         sqr_mahal_dist);

    T two_pi = static_cast<T>(vnl_math::twopi);
    T prob3 = static_cast<T>(1.0 / std::sqrt(two_pi * two_pi * two_pi * var[0] * var[1] * var[2]) *
                             std::exp(-sqr_mahal_dist / 2));
    T prob1 = static_cast<T>(1.0 / std::sqrt(two_pi * var[0]) * std::exp(-d[0] * d[0] / (2 * var[0])));
