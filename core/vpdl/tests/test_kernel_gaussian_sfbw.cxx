...
template <class T>
void
test_kernel_gaussian_sfbw_type(T epsilon, const std::string & type_name)
{
  ...
  std::cout << "=================== fixed<3> =======================" << std::endl;
  {
    ...
    TEST(("num_components <" + type_name + ">").c_str(), kernel_g3.num_components(), samples.size());

    TEST(("bandwidth <" + type_name + ">").c_str(), kernel_g3.bandwidth(), bandwidth);

    // test probability funnctions
    TEST_NEAR(("prob_density <" + type_name + ">").c_str(), kernel_g3.prob_density(pt1), density, epsilon);
    TEST_NEAR(("cumulative_prob <" + type_name + ">").c_str(), kernel_g3.cumulative_prob(pt1), cum_prob, epsilon);
    TEST_NEAR(("box_prob <" + type_name + ">").c_str(), kernel_g3.box_prob(pt1, pt2), box_prob, epsilon);
    ...
    TEST_NEAR(("compute_mean <" + type_name + ">").c_str(), (m - mean).inf_norm(), T(0), epsilon);
    vnl_matrix_fixed<T, 3, 3> c;
    kernel_g3.compute_covar(c);
    TEST_NEAR(("compute_covar <" + type_name + ">").c_str(), (c - covar).array_inf_norm(), T(0), epsilon);

    kernel_g3.set_bandwidth(T(2.1));
    TEST(("set_bandwidth <" + type_name + ">").c_str(), kernel_g3.bandwidth(), T(2.1));
  }
  ...
  std::cout << "=================== variable =======================" << std::endl;
  {
    ...
    TEST(("init num_components <" + type_name + ">").c_str(), kernel_g.num_components(), 0);
    TEST(("init dimension <" + type_name + ">").c_str(), kernel_g.dimension() == 0 && kernel_g1.dimension() == 3, true);

    TEST(("init bandwidth <" + type_name + ">").c_str(), kernel_g.bandwidth(), T(1));

    kernel_g.set_bandwidth(bandwidth);
    TEST(("set_bandwidth <" + type_name + ">").c_str(), kernel_g.bandwidth(), bandwidth);

    for (unsigned int i = 0; i < samples.size(); ++i)
      kernel_g.add_sample(samples[i].as_ref());
    TEST(("add_sample <" + type_name + ">").c_str(), kernel_g.num_components(), samples.size());

    TEST(("dimension <" + type_name + ">").c_str(), kernel_g.dimension(), 3);

    // test probability funnctions
    TEST_NEAR(("prob_density <" + type_name + ">").c_str(), kernel_g.prob_density(pt1.as_ref()), density, epsilon);
    TEST_NEAR(
      ("cumulative_prob <" + type_name + ">").c_str(), kernel_g.cumulative_prob(pt1.as_ref()), cum_prob, epsilon);
    TEST_NEAR(
  ...
  std::cout << "=================== scalar =======================" << std::endl;
  {
    ...
    TEST(("num_components <" + type_name + ">").c_str(), kernel_g1.num_components(), samples.size());
    ...
    // test probability funnctions
    TEST_NEAR(("prob_density <" + type_name + ">").c_str(), kernel_g1.prob_density(pt1), density, epsilon);
    TEST_NEAR(("cumulative_prob <" + type_name + ">").c_str(), kernel_g1.cumulative_prob(pt1), cum_prob, epsilon);
    TEST_NEAR(("box_prob <" + type_name + ">").c_str(), kernel_g1.box_prob(pt1, pt2), box_prob, epsilon);
    ...
    TEST_NEAR(("compute_mean <" + type_name + ">").c_str(), m, mean[0], epsilon);
    T c;
    kernel_g1.compute_covar(c);
    TEST_NEAR(("compute_covar <" + type_name + ">").c_str(), c, covar(0, 0), epsilon);
  }
}


static void
test_kernel_gaussian_sfbw()
{
