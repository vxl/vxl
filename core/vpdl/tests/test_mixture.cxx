...
bool
weight_less(const vpdl_distribution<T, n> & /*d1*/, const T & w1, const vpdl_distribution<T, n> & /*d2*/, const T & w2)
{
  return w1 < w2;
}
...
template <class T>
void
test_covar_mean(T epsilon, const std::string & type_name)
{
  ...
  for (unsigned int i = 0; i < 15; ++i)
    data.push_back(vnl_vector_fixed<T, 3>(T(3 * rnd.normal64() - 1), T(rnd.normal64() + 3), T(1.5 * rnd.normal64())));
  for (unsigned int i = 0; i < 10; ++i)
  ...
  TEST_NEAR(("compute_mean <" + type_name + ">").c_str(), (gaussT.mean() - mean).inf_norm(), T(0), epsilon);

  vnl_matrix_fixed<T, 3, 3> covar;
  mixture.compute_covar(covar);
  TEST_NEAR(
    ("compute_covar <" + type_name + ">").c_str(), (gaussT.covariance() - covar).array_inf_norm(), T(0), epsilon);
}


template <class T>
void
test_mixture_type(T epsilon, const std::string & type_name)
{
