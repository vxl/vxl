...
bool
weight_less(const dist_t & /*d1*/,
            const typename vpdt_dist_traits<dist_t>::scalar_type & w1,
            const dist_t & /*d2*/,
            const typename vpdt_dist_traits<dist_t>::scalar_type & w2)
{
  return w1 < w2;
}
...
template <class T>
void
test_mixture_of_type(T epsilon, const std::string & type_name)
{
  // test a bunch of instantiations to make sure they compile
  {
    const vpdl_mixture_of<vpdl_gaussian_sphere<T, 3>> mix_gauss_s3;
    const vpdl_mixture_of<vpdl_gaussian_indep<T, 3>> mix_gauss_i3;
    const vpdl_mixture_of<vpdl_gaussian<T, 3>> mix_gauss_f3;
    const vpdl_mixture_of<vpdl_gaussian_sphere<T, 1>> mix_gauss_s1;
    const vpdl_mixture_of<vpdl_gaussian_indep<T, 1>> mix_gauss_i1;
    const vpdl_mixture_of<vpdl_gaussian<T, 1>> mix_gauss_f1;
    const vpdl_mixture_of<vpdl_gaussian_sphere<T>> mix_gauss_s;
    const vpdl_mixture_of<vpdl_gaussian_indep<T>> mix_gauss_i;
    const vpdl_mixture_of<vpdl_gaussian<T>> mix_gauss_f;
  }
  ...
  {
    const vpdl_gaussian_indep<T, 3> gauss1(mean1, var1);
    ...
    TEST(("distribution access <" + type_name + ">").c_str(),
         dist1.mean() == gauss1.mean() && dist1.covariance() == gauss1.covariance(),
         true);
    TEST(("weight access <" + type_name + ">").c_str(), mixture.weight(1), T(0.5));

    valid = mixture.remove_last();
    TEST(("remove last <" + type_name + ">").c_str(), valid && mixture.num_components() == 1, true);

    mixture.insert(gauss2, T(0.3));
    mixture.insert(gauss3, T(0.15));
    mixture.set_weight(0, T(0.05));
    TEST(("set_weight <" + type_name + ">").c_str(), mixture.weight(0), T(0.05));

    mixture.sort();
    TEST(("default sort <" + type_name + ">").c_str(),
         mixture.weight(0) == T(0.3) && mixture.weight(1) == T(0.15) && mixture.weight(2) == T(0.05) &&
           &mixture.distribution(2) == &dist1,
         true);
    ...
    TEST_NEAR(("gradient density <" + type_name + ">").c_str(), (g3 - grad).inf_norm(), 0, dp);
    TEST_NEAR(("density <" + type_name + ">").c_str(), density, den, epsilon);
    ...
    TEST(("sort by increasing weight <" + type_name + ">").c_str(),
         mixture2.weight(0) == T(0.05) && mixture2.weight(1) == T(0.15) && mixture2.weight(2) == T(0.3),
         true);

    mixture2.normalize_weights();
    TEST(("normalize <" + type_name + ">").c_str(),
         mixture2.weight(0) == T(0.1) && mixture2.weight(1) == T(0.3) && mixture2.weight(2) == T(0.6),
         true);
  }

  std::cout << "=================== scalar =======================" << std::endl;
  {
    const vpdl_gaussian<T, 1> gauss1(mean1[0], var1[0]);
    ...
    TEST_NEAR(("gradient density <" + type_name + ">").c_str(), g, grad, dp);
    TEST_NEAR(("density <" + type_name + ">").c_str(), density, den, epsilon);
    ...
    TEST_NEAR(("compute_mean <" + type_name + ">").c_str(), cmp_mean, mean, epsilon);

    T var = T(0.1 * gauss1.covariance() + 0.6 * gauss2.covariance() + 0.3 * gauss3.covariance());
    var += T(0.1 * gauss1.mean() * gauss1.mean() + 0.6 * gauss2.mean() * gauss2.mean() +
             0.3 * gauss3.mean() * gauss3.mean());
    var -= mean * mean;
    T cmp_var;
    mixture2.compute_covar(cmp_var);
    TEST_NEAR(("compute_covar <" + type_name + ">").c_str(), cmp_var, var, epsilon);
  }

  std::cout << "=================== variable =======================" << std::endl;
  {
    const vpdl_gaussian_indep<T> gauss1(mean1.as_ref(), var1.as_ref());
    ...
    TEST(("initial num_components <" + type_name + ">").c_str(), mixture.num_components(), 0);
    TEST(("initial dimension <" + type_name + ">").c_str(), mixture.dimension(), 0);
    ...
    mixture.remove_last();
    TEST(("copy constructor <" + type_name + ">").c_str(),
         mixture2.num_components() == 3 && mixture2.weight(1) == T(0.15) && mixture2.dimension() == 3,
         true);
    ...
    TEST_NEAR(("gradient density <" + type_name + ">").c_str(), (g - grad).inf_norm(), 0, dp);
    TEST_NEAR(("density <" + type_name + ">").c_str(), density, den, epsilon);

    prob = T(0.1 * gauss1.cumulative_prob(pt.as_ref()) + 0.6 * gauss2.cumulative_prob(pt.as_ref()) +
             0.3 * gauss3.cumulative_prob(pt.as_ref()));
    TEST_NEAR(
      ("cumulative probability <" + type_name + ">").c_str(), mixture2.cumulative_prob(pt.as_ref()), prob, epsilon);

    vnl_vector_fixed<T, 3> pt2(T(10), T(5), T(8));
    prob = T(0.1 * gauss1.box_prob(pt.as_ref(), pt2.as_ref()) + 0.6 * gauss2.box_prob(pt.as_ref(), pt2.as_ref()) +
             0.3 * gauss3.box_prob(pt.as_ref(), pt2.as_ref()));
    TEST_NEAR(
      ("box probability <" + type_name + ">").c_str(), mixture2.box_prob(pt.as_ref(), pt2.as_ref()), prob, epsilon);
