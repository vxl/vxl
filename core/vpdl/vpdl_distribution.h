...
public:
  ...
  // \sa prob_density
  virtual T
  density(const vector & pt) const = 0;
  ...
  virtual T
  log_prob_density(const vector & pt) const
  {
  ...
  virtual T
  gradient_density(const vector & pt, vector & g) const = 0;
  ...
  virtual T
  cumulative_prob(const vector & pt) const = 0;
  ...
  virtual void
  compute_mean(vector & mean) const = 0;

  //: Compute the covariance of the distribution.
  // This may be trivial for distributions like Gaussians,
  // but actually involves computation for others.
  virtual void
  compute_covar(matrix & covar) const = 0;
};
...
template <class T, unsigned int n>
inline T
vpdt_box_prob(const vpdl_distribution<T, n> & d,
