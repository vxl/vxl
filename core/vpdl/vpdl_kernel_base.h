...
public:
  ...
  vpdl_kernel_base(std::vector<vector> samplez)
    : samples_(std::move(samplez))
  {}
  ...
  virtual void
  set_samples(const std::vector<vector> & samplez)
  {
    samples_ = samplez;
  }
...
public:
  ...
  virtual void
  add_sample(const vector & s, T bw)
  {
    vpdl_kernel_base<T, n>::add_sample(s);
  ...
  virtual void
  set_samples(const std::vector<vector> & samplez, const std::vector<T> & bandwidthz)
  {
