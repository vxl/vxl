...
template <class T>
inline T & vpdt_index(vnl_matrix<T>& v, unsigned int i, unsigned int j)
{
...
template <class T>
inline const T & vpdt_index(const vnl_matrix<T>& v, unsigned int i, unsigned int j)
{
  assert(
    i < v.rows() && j < v.columns() ); return v(i, j);
}

// : Index into a vnl_matrix_fixed
template <class T, unsigned int n>
inline T & vpdt_index(vnl_matrix_fixed<T, n, n>& v, unsigned int i, unsigned int j)
{
  assert(i < n && j < n); return v(
    i, j);
}
// : Index into a vnl_matrix_fixed (const)
template <class T, unsigned int n>
inline const T & vpdt_index(const vnl_matrix_fixed<T, n, n>& v, unsigned int i,
                            unsigned int j) { assert(i < n && j < n); return v(i, j); }
