...
inline void
vnl_symmetric_eigensystem_compute(const vnl_matrix_fixed<T, n, n> & A,
...
public:
  ...
  vpdt_eigen_sym_matrix(const matrix & m) { set_matrix(m); }

  //: Return the dimension
  unsigned int
  dimension() const
  {
    return eigen_val_.size();
  }
  ...
  void
  set_eigenvectors(const matrix & m)
  {
    eigen_vec_ = m;
    assert(are_evec_orthonormal());
  }
  ...
  //: set the eigenvalues
  void
  set_eigenvalues(const vector & v)
  {
    eigen_val_ = v;
  }
  ...
  //: evaluate y = M * x
  void
  product(const vector & x, vector & y) const
  {
    const unsigned int dim = eigen_val_.size();
    vpdt_set_size(y, dim);
    vpdt_fill(y, T(0));
  ...
  //: evaluate y = M^-1 * x
  void
  inverse_product(const vector & x, vector & y) const
  {
    const unsigned int dim = eigen_val_.size();
    vpdt_set_size(y, dim);
    vpdt_fill(y, T(0));
...
{
  typedef vpdt_eigen_sym_matrix<typename vpdt_field_traits<F>::scalar_type, vpdt_field_traits<F>::dimension> type;
};
...
template <class T>
inline void
vpdt_set_size(vpdt_eigen_sym_matrix<T, 0> & m, unsigned int s)
{
  m.set_size(s);
}
...
template <class T, unsigned int n>
inline void
vpdt_fill(vpdt_eigen_sym_matrix<T, n> & m, const T & val)
{
