...
public:
  ...
  vpdt_gaussian(F m, covar_type c)
    : mean(std::move(m))
    , covar(std::move(c))
  {}
  ...
  T
  gradient_density(const F & pt, vector & g) const
  {
  ...
  norm_const() const
  {
    return T(1) / vpdt_gaussian_integrator<F, Covar, Metric>::domain_integral(*this);
  }

  //: The squared Mahalanobis distance to this point
  // Non-virtual for efficiency
  T
  sqr_mahal_dist(const F & pt) const
  {
  ...
  T
  cumulative_prob(const F & pt) const
  {
    return vpdt_gaussian_integrator<F, Covar, Metric>::partial_integral(*this, pt);
  }

  //: Compute the mean of the distribution.
  void
  compute_mean(vector & m) const
  {
    m = mean;
  }
...
template <class F>
struct vpdt_gaussian_integrator<F,
                                typename vpdt_eigen_sym_matrix_gen<F>::type,
                                ...
                                typename vpdt_field_traits<F>::type_is_vector>
{
  ...
  static inline T
  domain_integral(const vpdt_gaussian<F, Covar> & g)
  {
  ...
  static inline T
  partial_integral(const vpdt_gaussian<F, Covar> & /*g*/, const F & /*pt*/)
  {
...
struct vpdt_gaussian_integrator<F, F, vpdt_norm_metric<F, F>, typename vpdt_field_traits<F>::type_is_vector>
{
  ...
  //: integrate over the entire domain
  static inline T
  domain_integral(const vpdt_gaussian<F, F> & g)
  {
    const unsigned int d = g.dimension();
    const double two_pi = vnl_math::twopi;
    double val = 1.0;
  ...
  //: integrate from -infinity to \c pt
  static inline T
  partial_integral(const vpdt_gaussian<F, F> & g, const F & pt)
  {
    const unsigned int d = g.dimension();
    double val = 1.0;
    for (unsigned int i = 0; i < d; ++i)
...
template <class F>
struct vpdt_gaussian_integrator<F,
                                typename vpdt_field_traits<F>::scalar_type,
                                ...
                                typename vpdt_field_traits<F>::type_is_vector>
{
  ...
  //: integrate over the entire domain
  static inline T
  domain_integral(const vpdt_gaussian<F, T> & g)
  {
    const unsigned int d = g.dimension();
    const double two_pi = vnl_math::twopi;
    double val = 1.0;
  ...
  //: integrate from -infinity to \c pt
  static inline T
  partial_integral(const vpdt_gaussian<F, T> & g, const F & pt)
  {
    const unsigned int d = g.dimension();
    if (g.covar <= T(0))
    {
