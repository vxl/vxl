...
private:
  ...
  struct component
  {
    ...
    bool
    operator<(const component & rhs) const
    {
  ...
  public:
    bool
    operator()(const component * c1, const component * c2) const
    {
  ...
  public:
    ...
    {}
    bool
    operator()(const component * const c1, const component * const c2) const
    {
...
public:
  ...
  //: Assignment operator
  vpdt_mixture_of<dist_t> &
  operator=(const vpdt_mixture_of<dist_t> & rhs)
  {
    if (this != &rhs)
    {
      for (unsigned int i = 0; i < components_.size(); ++i)
  ...
  void
  compute_covar(matrix & covar) const
  {
    const unsigned int d = this->dimension();
    F mean;
  ...
  friend T
  vpdt_box_prob<dist_t>(const vpdt_mixture_of<dist_t> & d, const F & min_pt, const F & max_pt);
};
