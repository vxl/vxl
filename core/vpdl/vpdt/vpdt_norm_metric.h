...
struct vpdt_norm_metric<F, typename vpdt_eigen_sym_matrix_gen<F>::type, typename vpdt_field_traits<F>::type_is_vector>
{
  ...
  static inline T
  distance(const F & pt1, const F & pt2, const covar_type & c)
  {
  ...
  static inline T
  sqr_distance(const F & pt1, const F & pt2, const covar_type & c)
  {
  ...
  static inline T
  sqr_distance_deriv(const F & pt1, const F & pt2, const covar_type & c, vector & g)
  {
    // F must provide operator-(F,F) that returns (or can cast to) the vector type
    vector d(pt1 - pt2);
  ...
  static inline void
  compute_covar(matrix & covar, const F & /*pt*/, const covar_type & c)
  {
    c.form_matrix(covar);
  }
  ...
  // \note this metric is independent of the point
  static inline T
  covar_det(const F & /*pt*/, const covar_type & c)
  {
