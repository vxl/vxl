...
protected:
  ...
  void
  insert(mog_type & mixture, const F & sample, T init_weight) const
  {
  ...
  unsigned int
  match(const mog_type & mix, const F & sample, const T & gt2, T & sqr_dist) const
  {
...
public:
  ...
  //: The main function
  void
  operator()(mog_type & mix, const F & sample) const
  {
...
protected:
  ...
  void
  update(mog_type & mix, const F & sample, T alpha) const
  {
    T sqr_dist;
...
protected:
  ...
  T
  total_num_obs(const mog_type & mix) const
  {
    T num_obs = T(0);
    const unsigned int mix_nc = mix.num_components();
    for (unsigned int i = 0; i < mix_nc; ++i)
  ...
  void
  winner_take_all(std::vector<std::pair<unsigned int, double>> & m) const
  {
...
public:
  ...
  //: The main function
  void
  operator()(mog_type & mix, const F & sample, T sample_weight = T(1)) const
  {
...
protected:
  ...
  T
  total_weight(const mog_type & mix) const
  {
    T tw = T(0);
    const unsigned int mix_nc = mix.num_components();
    for (unsigned int i = 0; i < mix_nc; ++i)
  ...
  //: Update the mixture with \a sample using learning rate \a alpha
  void
  update(mog_type & mix, const F & sample, T samp_weight) const
  {
