...
test_ba_shared_k_lsqr()
{
  ...
  TEST_NEAR("Jacobian A (same as finite diff)", (A[0] - fdA[0]).absolute_value_max(), 0.0, 1e-4);
  TEST_NEAR("Jacobian B (same as finite diff)", (B[0] - fdB[0]).absolute_value_max(), 0.0, 1e-4);
  TEST_NEAR("Jacobian C (same as finite diff)", (C[0] - fdC[0]).absolute_value_max(), 0.0, 1e-4);
}
