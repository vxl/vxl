...
make_noisy_measurements(const std::vector<vgl_point_2d<double>> & image_points, double max_p_err)
{
  vnl_random rnd;
...
void
perturb_points(std::vector<vgl_point_3d<double>> & points, double max_p_err)
{
  vnl_random rnd;
  // add uniform noise in a [-max_p_err/2, max_p_err/2] window
  for (auto & point : points)
  {
    const vgl_vector_3d<double> noise(rnd.drand32() - 0.5, rnd.drand32() - 0.5, rnd.drand32() - 0.5);
...
void
perturb_cameras(std::vector<vpgl_perspective_camera<double>> & cameras, double max_t_err, double max_r_error)
{
  vnl_random rnd;
  // add uniform noise in a [-max_p_err/2, max_p_err/2] window
  for (auto & camera : cameras)
  {
    const vgl_vector_3d<double> tnoise(rnd.drand32() - 0.5, rnd.drand32() - 0.5, rnd.drand32() - 0.5);
    camera.set_camera_center(camera.get_camera_center() + max_t_err * tnoise);
    vnl_double_3 rnoise(rnd.drand32() - 0.5, rnd.drand32() - 0.5, rnd.drand32() - 0.5);
    rnoise.normalize();
    rnoise *= rnd.drand32() * max_r_error;
    camera.set_rotation(camera.get_rotation() * vgl_rotation_3d<double>(rnoise));
  }
}
...
std::vector<vgl_point_2d<double>>
make_occlusions(const std::vector<vgl_point_2d<double>> & image_points, std::vector<std::vector<bool>> & mask)
{
...
test_bundle_adjust()
{
  ...
  const vpgl_calibration_matrix<double> K(2000.0, vgl_homg_point_2d<double>(512, 384));
  ...
  const std::vector<vgl_point_2d<double>> subset_image_points = make_occlusions(noisy_image_points, mask);

  // test optimization with fixed calibration
  {
    // make default cameras
    const vgl_rotation_3d<double> I; // no rotation
  ...
  // test optimization with outliers
  {
    ...
    const std::vector<vgl_point_3d<double>> init_world(unknown_world);
    std::vector<vpgl_perspective_camera<double>> unknown_cameras(cameras);
    perturb_cameras(unknown_cameras, 0.1, 0.0001);
    const std::vector<vpgl_perspective_camera<double>> init_cameras(unknown_cameras);
