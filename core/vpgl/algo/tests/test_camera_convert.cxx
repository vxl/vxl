...
{
  const double n_u[20] = { 3.89528e-006,  -7.41303e-008, 1.25847e-005,  0.00205724,    5.38537e-008,
  ...
  const double d_u[20] = { -1.68064e-006, -9.85769e-007, 3.49708e-005,  4.28561e-006,  2.25889e-007,
  ...
  const double n_v[20] = { -1.21292e-006, 7.83222e-006,  -6.63094e-005, -0.000224627,  -4.85207e-006,
  ...
  const double d_v[20] = { 1.44065e-005,  -2.47213e-006, -8.52173e-006, -1.28085e-005, 4.81707e-006,
  ...
  const vpgl_scale_offset<double> sox(0.0347, -71.4049);
  const vpgl_scale_offset<double> soy(0.0219, 41.8216);
  const vpgl_scale_offset<double> soz(501, -30);
  const vpgl_scale_offset<double> sou(4764, 4693);
  const vpgl_scale_offset<double> sov(4221, 3921);
  ...
  const vpgl_rational_camera<double> rat_cam(cmatrix, scale_offsets);
  return rat_cam;
}
...
{
  const double n_u[20] = { -7.81964e-006, 6.39341e-005,  4.67275e-006,  0.00423585,   -9.69009e-005,
  ...
  const double d_u[20] = { 7.0187e-008,
                           -5.40373e-007,
                           0,
                           -2.08288e-005,
                           -1.0929e-006,
                           ...
                           -0.000537438,
                           1 };
  const double n_v[20] = { 9.93585e-007, 4.86672e-006, 2.26215e-007, -0.000188252, 5.27564e-005,
                           -1.5669e-007, 0.00153582,   6.71638e-007, 2.8673e-005,  -0.0486115,
                           3.90973e-005, 5.30928e-006, -0.00419213,  1.40611e-005, 0.000269255,
                           -1.03953,     1.22934e-007, 3.19812e-006, -0.00884028,  0.00575278 };
  const double d_v[20] = { 8.51615e-007,  6.74855e-007,  1.26254e-007,  -1.59186e-005, 1.97482e-005,
  ...
  // The scales and offsets
  const vpgl_scale_offset<double> sox(0.0986, 44.4692);
  const vpgl_scale_offset<double> soy(0.1058, 33.252);
  const vpgl_scale_offset<double> soz(500, 34);
  const vpgl_scale_offset<double> sou(13982, 12782.0673959);
  const vpgl_scale_offset<double> sov(17213, 13798.1995849);
  std::vector<vpgl_scale_offset<double>> scale_offsets;
  scale_offsets.push_back(sox);
  scale_offsets.push_back(soy);
  scale_offsets.push_back(soz);
  scale_offsets.push_back(sou);
  scale_offsets.push_back(sov);
  const vpgl_lvcs lvcs(33.331465, 44.37697, 0.0, vpgl_lvcs::wgs84, vpgl_lvcs::DEG, vpgl_lvcs::METERS);
  // Construct the rational camera
  const vpgl_rational_camera<double> rat_cam(cmatrix, scale_offsets);
  // Construct the local rational camera
  const vpgl_local_rational_camera<double> loc_rat_cam(lvcs, rat_cam);
  return loc_rat_cam;
}
vpgl_RSM_camera<double>
construct_replacement_sensor_model()
{
  const std::vector<double> RNC = {
  ...
  const std::vector<std::vector<int>> powers(4, std::vector<int>(3, 3));
  const vpgl_scale_offset<double> sox(0.0029411689968438198, 0.77378520899138403);
  const vpgl_scale_offset<double> soy(0.0080796904012988002, 0.58005546402463803);
  const vpgl_scale_offset<double> soz(657.20053824177000, 484.65013863099699);
  const vpgl_scale_offset<double> sou(148.00000000000000, 128.00000000000000);
  const vpgl_scale_offset<double> sov(1696.0000000000000, 1676.0000000000000);
  std::vector<vpgl_scale_offset<double>> scale_offsets;
  scale_offsets.push_back(sox);
  scale_offsets.push_back(soy);
  scale_offsets.push_back(soz);
  scale_offsets.push_back(sou);
  scale_offsets.push_back(sov);
  const vpgl_polycam<double> pcam(1, 1, powers, coeffs, scale_offsets);
  const vpgl_region_selector<double> rsel; // rnis ==1 , cnis == 1
...
{
  const vpgl_rational_camera<double> rat_cam = construct_rational_camera();
  std::cout << rat_cam;
...
#endif
  vgl_point_3d<double> const pmin(sox.offset() - sox.scale(), soy.offset() - soy.scale(), 0);
  const vgl_point_3d<double> pmax(sox.offset() + sox.scale(), soy.offset() + soy.scale(), soz.scale());
  const vgl_box_3d<double> approx_vol(pmin, pmax);
...
test_rational_camera_approx_affine()
{
  ...
  vpgl_local_rational_camera<double> const loc_rat_cam = construct_local_rational_camera();

  // a 500x500x100 roi centered at the lvcs origin
  const vgl_point_3d<double> roi_center(0, 0, 0);
  const vgl_point_3d<double> roi_min = roi_center - vgl_vector_3d<double>(250, 250, 0);
  const vgl_point_3d<double> roi_max = roi_center + vgl_vector_3d<double>(250, 250, 100);
  const vgl_box_3d<double> roi(roi_min, roi_max);

  vpgl_affine_camera<double> ac;
  const bool success = vpgl_affine_camera_convert::convert(loc_rat_cam, roi, ac);

  TEST("vpgl_affine_camera_convert success", success, true);

  // project center and corners of volume into image

  const std::vector<vgl_point_3d<double>> test_points{ roi_center, roi_min, roi_max };
  for (const vgl_point_3d<double> & test_pt : test_points)
  {
    const vgl_point_2d<double> img_pt1 = loc_rat_cam.project(test_pt);
    const vgl_point_2d<double> img_pt2 = ac.project(test_pt);
    const double dist = (img_pt2 - img_pt1).length();
...
{
#if HAS_GEOTIFF // Requires GEOTIFF to succeed
  vpgl_RSM_camera<double> const rsm_cam = construct_replacement_sensor_model();
  // define lvcs
  const vpgl_lvcs lvcs(33.315252, 44.366044, 35.0, vpgl_lvcs::wgs84, vpgl_lvcs::DEG, vpgl_lvcs::METERS);
  // a 500x500x100 roi centered at the lvcs origin
  const vgl_point_3d<double> roi_center(0, 0, 0);
  const vgl_point_3d<double> roi_min = roi_center - vgl_vector_3d<double>(250, 250, 0);
  const vgl_point_3d<double> roi_max = roi_center + vgl_vector_3d<double>(250, 250, 100);
  ...
  vpgl_affine_camera<double> ac;
  const bool success = vpgl_affine_camera_convert::convert(rsm_cam, lvcs, roi, ac);
  ...
  for (const vgl_point_3d<double> & test_pt : test_points)
  {
    ...
    const vgl_point_3d<double> tpg(X, Y, Z);
    const vgl_point_2d<double> img_pt1 = rsm_cam.project(tpg);
    const vgl_point_2d<double> img_pt2 = ac.project(test_pt);
    const double dist = (img_pt2 - img_pt1).length();
    // test to within one pixel
...
test_generic_camera_convert()
{
  ...
  //==================================================//
  const vpgl_local_rational_camera<double> lcam = construct_local_rational_camera();
  ...
  if (success)
  {
    ...
    std::cout << "(u v)=(" << u << ' ' << v << ")\n";
    const double del = std::fabs(u - u0) + std::fabs(v - v0);
    TEST_NEAR("test ray projection at center", del, 0.0, 0.5);
    // four corners
    const int ua[4] = { 0, 832, 0, 832 };
    const int va[4] = { 0, 0, 876, 876 };
    dorg = 0.0;
    dang = 0.0;
    // DEC reduced z range by 1/2 so need the following delta
    const vgl_vector_3d<double> delta(72.792, -51, 250);
  ...
  K[2][2] = 1.0;
  C = K * m;
  const vpgl_proj_camera<double> icam(C);
  ni = 10;
  nj = 10;
  ...
  if (success)
  {
    ...
    const vgl_point_3d<double> org(0, 0, 0);
  ...
  //==================================================//
  const vgl_vector_3d<double> view_dir(0.0, 0.0, -1.0);
  const vgl_vector_3d<double> up(0.0, 0.0, 1.0);
  const vgl_point_3d<double> stare(0.0, 0.0, 0.0);
  ...
  if (success)
  {
    ...
    const double edir = (view_dir - rayc.direction()).length();
...
test_rational_camera_convert()
{
  ...
  //==================================================//
  const vpgl_RSM_camera<double> rsm_cam = construct_replacement_sensor_model();
  const vgl_point_3d<double> min_pt(44.254166, 32.77861, -100);
  const vgl_point_3d<double> max_pt(44.41305, 33.6911, +100);
  ...
  vnl_random rng;
  const double min_x = box.min_x(); // longitude degrees
  const double min_y = box.min_y(); // latitude degrees
  const double min_z = box.min_z(); // elevation meters
  const double max_x = box.max_x();
  const double max_y = box.max_y();
  const double max_z = box.max_z();
  // test n points
