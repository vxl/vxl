            ...
            const std::string &                                test_name)
{
  ...
  for (i = ems.begin(); i != ems.end(); ++i)
  {
    ...
    vnl_matrix_fixed<double, 3, 3> e_et = i->get_matrix() * i->get_matrix().transpose();

    double v = (e_et * i->get_matrix() * 2.0 - vnl_trace(e_et) * i->get_matrix()).get(0, 0);
    ...
    for (unsigned int j = 0; j < corres1.size(); ++j)
    {
      ...
      double result = (vec2.transpose() * i->get_matrix() * vec1).get(0, 0);
