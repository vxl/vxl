...
{
  const vnl_matrix_fixed<double, 3, 3> k_inv = vnl_inverse(k.get_matrix());
  ...
  for (auto point : points)
  {
    ...
    const vnl_matrix_fixed<double, 3, 1> normed = k_inv * vec;
            ...
            const std::string & test_name)
{
  ...
  for (i = ems.begin(); i != ems.end(); ++i)
  {
    ...
    const double det = vnl_determinant(i->get_matrix());
    TEST_NEAR((test_name + std::string(": Test det(essential_matrix) = 0")).c_str(), det, 0, TOL);

    // Now check that 2*e*e^t*e - tr(e * e^t) * e = 0;
    const vnl_matrix_fixed<double, 3, 3> e_et = i->get_matrix() * i->get_matrix().transpose();

    const double v = (e_et * i->get_matrix() * 2.0 - vnl_trace(e_et) * i->get_matrix()).get(0, 0);

    TEST_NEAR((test_name + std::string(": Test 2*E*E^t*E - tr(E*E^t)*E = 0")).c_str(), v, 0, TOL);
    ...
    for (unsigned int j = 0; j < corres1.size(); ++j)
    {
      ...
      const double result = (vec2.transpose() * i->get_matrix() * vec1).get(0, 0);

      TEST_NEAR((test_name + std::string(": Testing that q' * E * q = 0")).c_str(), result, 0, TOL);
    }
  }
}
...
test_unnormed()
{
  ...
  const double focal_length1_mm = 5.4; // In mm
  const double ccd_width1 = 5.312;
  int width1 = 640, height1 = 480;
  const double focal_length1 = width1 * (focal_length1_mm / ccd_width1);

  const vpgl_calibration_matrix<double> k1(focal_length1, vgl_point_2d<double>(width1 / 2, height1 / 2));


  // Define the information in the second camera
  const double focal_length2_mm = 5.4; // In mm
  const double ccd_width2 = 5.312;
  int width2 = 640, height2 = 480;
  const double focal_length2 = width2 * (focal_length2_mm / ccd_width2);

  const vpgl_calibration_matrix<double> k2(focal_length2, vgl_point_2d<double>(width2 / 2, height2 / 2));
  ...
  const std::vector<vgl_point_2d<double>> corres1(corres1_arr,
  ...
  const std::vector<vgl_point_2d<double>> corres2(corres2_arr,
                                                  corres2_arr + sizeof(corres2_arr) / sizeof(vgl_point_2d<double>));

  Assert("Un-normed: Testing that the correspondences are the same size.", corres1.size() == corres2.size());
  //*************End definitions.

  // Now actually do the computation.
  std::vector<vpgl_essential_matrix<double>> ems;
  const vpgl_em_compute_5_point<double> em_5_pt;

  Assert("Un-normed: Testing that the 5 point algorithm returns correctly.",
         em_5_pt.compute(corres1, k1, corres2, k2, ems));

  Assert("Un-normed: Testing that we found the right number of solutions.", ems.size() <= 10);
...
test_normed()
{
  ...
  Assert("Normed: Testing that the correspondences are the same size.", corres1.size() == corres2.size());
  //*************End definitions.
  ...
  Assert("Normed: Testing that we found the right number of solutions.", ems.size() <= 10);

  eval_e_mats(ems, corres1, corres2, std::string("Normed"));
}

static void
test_em_compute_5_point()
{
  test_normed();
  test_unnormed();
}
