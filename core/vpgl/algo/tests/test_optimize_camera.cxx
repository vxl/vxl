                    ...
                    vnl_random & rnd)
{
  ...
  const vgl_rotation_3d<double> dR(dw);

  const vgl_vector_3d<double> dc(rnd.drand32() - 0.5, rnd.drand32() - 0.5, rnd.drand32() - 0.5);
  ...
  const vgl_point_3d<double> new_center = c + max_t_err * dc;
  std::cout << "new center = " << new_center << std::endl;
  const vpgl_perspective_camera<double> err_cam(K, c + max_t_err * dc, dR * R);
  std::cout << "Perturbed position:" << err_cam.get_camera_center() << std::endl
            << "Perturbed principal ray:" << err_cam.principal_axis() << std::endl;

  const vpgl_perspective_camera<double> opt_cam = vpgl_optimize_camera::opt_orient_pos(err_cam, world, image);

  std::cout << "Estimated position:" << opt_cam.get_camera_center() << std::endl
            << "Estimated principal ray:" << opt_cam.principal_axis() << std::endl;

  const double dist = vgl_distance(opt_cam.get_camera_center(), cam.get_camera_center());
                      ...
                      vnl_random & rnd)
{
  ...
  const vpgl_perspective_camera<double> opt_cam = vpgl_optimize_camera::opt_orient_pos_f(err_cam, world, image);
  ...
  const double f_opt = opt_cam.get_calibration().focal_length();
  const double f_cam = cam.get_calibration().focal_length();
  TEST_NEAR_REL("opt_orient_pos_f: focal_len", f_opt, f_cam, 0.1);
}

static void
test_optimize_camera()
{
  ...
  const vgl_homg_point_3d<double> c(4.0, 4.0, 4.0);
  ...
  {
    const vgl_homg_point_2d<double> hpt = cam(i);
