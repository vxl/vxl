  ...
  : vrel_estimation_problem( 8 , 4 /*points to instantiate*/ )
{
  ...
  from_pts_.resize(n); to_pts_.resize(n);
                      ...
                      vnl_vector<double>& params ) const
{
  ...
  for ( unsigned int i=0; i<min_num_pts_; ++i )
  {
    ...
    A[2*i][0]=from_pts_[indx][0];   A[2*i][1]=from_pts_[indx][1];   A[2*i][2]=from_pts_[indx][2];   A[2*i][3]=1; b[2*i] = to_pts_[indx][0];
    A[2*i+1][4]=from_pts_[indx][0]; A[2*i+1][5]=from_pts_[indx][1]; A[2*i+1][6]=from_pts_[indx][2]; A[2*i][7]=1; b[2*i+1] = to_pts_[indx][1];
  }
                            ...
                            const std::vector<double>* weights ) const
{
  ...
  vnl_matrix< double > A(2*num_samples_, 8, 0.0); vnl_vector<double> b(2*num_samples_);
  // if mapping confidence is trustworthy maybe use as weights
  for(size_t i = 0; i<num_samples_; ++i){
    A[2*i][0]=from_pts_[i][0];   A[2*i][1]=from_pts_[i][1];   A[2*i][2]=from_pts_[i][2];   A[2*i][3]=1; b[2*i] = to_pts_[i][0];
    A[2*i+1][4]=from_pts_[i][0]; A[2*i+1][5]=from_pts_[i][1]; A[2*i+1][6]=from_pts_[i][2]; A[2*i][7]=1; b[2*i+1] = to_pts_[i][1];
  }
