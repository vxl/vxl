...
vpgl_affine_fm_robust_est::
vpgl_affine_fm_robust_est( const std::vector< vgl_point_2d<double> > & pr_pts,
                           const std::vector< vgl_point_2d<double> > & pl_pts , unsigned dof)
                      ...
                      vnl_vector<double>& params ) const
{
  ...
  for ( unsigned int i=0; i<min_num_pts_; ++i )
  {
    ...
    A[i][0]=left_pts_[indx][0]; A[i][1]=left_pts_[indx][1]; A[i][2]=right_pts_[indx][0]; A[i][3]=right_pts_[indx][1]; A[i][4] = 1.0;
  }
...
void
vpgl_affine_fm_robust_est::compute_residuals( const vnl_vector<double>& params,
                            ...
                            const std::vector<double>* weights ) const
{
  ...
  for(size_t i = 0; i<num_samples_; ++i){
    A[i][0]=left_pts_[i][0]; A[i][1]=left_pts_[i][1]; A[i][2]=right_pts_[i][0]; A[i][3]=right_pts_[i][1]; A[i][4] = 1.0;
  }
