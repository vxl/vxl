...
public:
  ...
  vpgl_affine_fm_robust_est( const std::vector< vgl_point_2d<double> > & pr_pts,
                             const std::vector< vgl_point_2d<double> > & pl_pts, unsigned int dof = 5);
  ...
  unsigned int min_num_points() const { return min_num_pts_;}
  ...
  void fill_Fa_from_params(vnl_vector<double>& params){
    Fa_.set_from_params(params[0], params[1], params[2], params[3], params[4] );
  }

  vpgl_affine_fundamental_matrix<double> const Fa(){return Fa_;}
protected:
