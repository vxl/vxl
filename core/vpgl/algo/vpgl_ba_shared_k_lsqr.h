...
public:
  ...
  vpgl_ba_shared_k_lsqr(const vpgl_calibration_matrix<double>& K, const vcl_vector<vgl_point_2d<double> >& image_points,
  ...
  virtual void jac_Aij(unsigned int i, unsigned int j, vnl_double_3x4 const& Pi, vnl_vector<double> const& ai,
                       vnl_vector<double> const& bj, vnl_vector<double> const& c, vnl_matrix<double>& Aij);
  ...
  virtual void jac_Bij(unsigned int i, unsigned int j, vnl_double_3x4 const& Pi, vnl_vector<double> const& ai,
                       vnl_vector<double> const& bj, vnl_vector<double> const& c, vnl_matrix<double>& Bij);
  ...
  virtual void jac_Cij(unsigned int i, unsigned int j, vnl_double_3x4 const& Pi, vnl_vector<double> const& ai,
                       vnl_vector<double> const& bj, vnl_vector<double> const& c, vnl_matrix<double>& Cij);

  // : construct the \param j-th perspective camera from a pointer to the j-th parameters of \param b and parameters \param c
  virtual vgl_homg_point_3d<double> param_to_point(int j, const double* bj, const vnl_vector<double>& c) const;

  // : construct the \param j-th perspective camera from a pointer to the j-th parameters of \param b and parameters \param c
  virtual vnl_vector_fixed<double, 4> param_to_pt_vector(int j, const double* bj, const vnl_vector<double>& c) const;

  // : construct the \param i-th perspective camera from a pointer to the i-th parameters of \param a and parameters \param c
  virtual vpgl_perspective_camera<double> param_to_cam(int i, const double* ai, const vnl_vector<double>& c) const;

  // : compute a 3x4 camera matrix of camera \param i from a pointer to the i-th parameters of \param a and parameters \param c
  virtual vnl_double_3x4 param_to_cam_matrix(int i, const double* ai, const vnl_vector<double>& c) const;

  // : Create the parameter vectors \p a and \p c from a vector of cameras
  static void create_param_vector(const vcl_vector<vpgl_perspective_camera<double> >& cameras, vnl_vector<double>& a,
                                  vnl_vector<double>& c);
