...
public:
  ...
  void
  set_use_m_estimator(bool use_m)
  {
    use_m_estimator_ = use_m;
  }
  void
  set_m_estimator_scale(double scale)
  {
    m_estimator_scale_ = scale;
  }
  void
  set_use_gradient(bool use_gradient)
  {
    use_gradient_ = use_gradient;
  }
  void
  set_self_calibrate(bool self_calibrate)
  {
    self_calibrate_ = self_calibrate;
  }
  void
  set_normalize_data(bool normalize)
  {
    normalize_data_ = normalize;
  }
  void
  ...
  void
  set_max_iterations(unsigned maxitr)
  {
    max_iterations_ = maxitr;
  }
  void
  set_x_tolerence(double xtol)
  {
    x_tol_ = xtol;
  }
  void
  set_g_tolerence(double gtol)
  {
    g_tol_ = gtol;
  }
  //: step size for finite differencing operations
  void
  set_epsilon(double eps)
  {
    epsilon_ = eps;
  }
  ...
  static void
  write_vrml(const std::string & filename,
...
private:
  ...
  void
  normalize_points(std::vector<vgl_point_2d<double>> & image_points, double & nx, double & ny, double & ns);

  // reflect the points about a plane
  void
  reflect_points(const vgl_plane_3d<double> & plane, std::vector<vgl_point_3d<double>> & points);

  // rotation the cameras 180 degrees around an axis
  void
  rotate_cameras(const vgl_vector_3d<double> & axis, std::vector<vpgl_perspective_camera<double>> & cameras);
