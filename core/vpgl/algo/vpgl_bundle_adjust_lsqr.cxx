...
void
vpgl_bundle_adjust_lsqr::f(const vnl_vector<double> & a,
...
void
vpgl_bundle_adjust_lsqr::jac_blocks(const vnl_vector<double> & a,
                                    const vnl_vector<double> & b,
                                    const vnl_vector<double> & c,
...
void
vpgl_bundle_adjust_lsqr::compute_weight_ij(int /*i*/,
                                           int /*j*/,
                                           const vnl_vector<double> & /*ai*/,
                                           const vnl_vector<double> & /*bj*/,
                                           const vnl_vector<double> & /*c*/,
...
void
vpgl_bundle_adjust_lsqr::jac_inhomg_3d_point(const vnl_double_3x4 & P,
...
void
vpgl_bundle_adjust_lsqr::jac_camera_center(const vnl_double_3x3 & M,
                                           const vnl_vector<double> & C,
                                           const vnl_vector<double> & pt,
                                           vnl_matrix<double> & J)
{
...
void
vpgl_bundle_adjust_lsqr::jac_camera_rotation(const vnl_double_3x3 & K,
                                             const vnl_vector<double> & C,
                                             ...
                                             vnl_matrix<double> & J)
{
  ...
  else
  {
    ...
    J(1, 0) = (w * (dct * x * vtc + ct * (t[0] * y - 2 * t[1] * x) + dst * x * vts - st * t[2]) - v * dw) / w2;

    dw = dct * y * wtc + ct * (t[1] * z - 2 * t[2] * y) + dst * y * wts - st * t[0];
    J(0, 1) = (w * (dct * y * utc + ct * (t[1] * x - 2 * t[0] * y) + dst * y * uts + st * t[2]) - u * dw) / w2;
    J(1, 1) = (w * (dct * y * vtc + ct * (t[0] * x + t[2] * z) + dst * y * vts) - v * dw) / w2;

    dw = dct * z * wtc + ct * (t[0] * x + t[1] * y) + dst * z * wts;
    J(0, 2) = (w * (dct * z * utc + ct * (t[2] * x - 2 * t[0] * z) + dst * z * uts - st * t[1]) - u * dw) / w2;
    J(1, 2) = (w * (dct * z * vtc + ct * (t[2] * y - 2 * t[1] * z) + dst * z * vts + st * t[0]) - v * dw) / w2;
  }
