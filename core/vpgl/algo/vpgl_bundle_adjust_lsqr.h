...
public:
  ...
  vpgl_bundle_adjust_lsqr(unsigned int num_params_per_a, unsigned int num_params_per_b, unsigned int num_params_c,
  ...
  vpgl_bundle_adjust_lsqr(unsigned int num_params_per_a, unsigned int num_params_per_b, unsigned int num_params_c,
                          const vcl_vector<vgl_point_2d<double> >& image_points,
                          const vcl_vector<vnl_matrix<double> >& inv_covars, const vcl_vector<vcl_vector<bool> >& mask);
  ...
  virtual void jac_Aij(unsigned int i, unsigned int j, vnl_double_3x4 const& Pi, vnl_vector<double> const& ai,
                       vnl_vector<double> const& bj, vnl_vector<double> const& c, vnl_matrix<double>& Aij) = 0;

  // : compute the Jacobian Bij
  virtual void jac_Bij(unsigned int i, unsigned int j, vnl_double_3x4 const& Pi, vnl_vector<double> const& ai,
                       vnl_vector<double> const& bj, vnl_vector<double> const& c, vnl_matrix<double>& Bij) = 0;

  // : compute the Jacobian Cij
  virtual void jac_Cij(unsigned int i, unsigned int j, vnl_double_3x4 const& Pi, vnl_vector<double> const& ai,
                       vnl_vector<double> const& bj, vnl_vector<double> const& c, vnl_matrix<double>& Cij) = 0;

  // : Use an M-estimator to compute weights
  void compute_weight_ij(int i, int j, vnl_vector<double> const& ai, vnl_vector<double> const& bj,
  ...
  vgl_homg_point_3d<double> param_to_point(int j,
  ...
  // : construct the \param j-th 3D point from parameter vector \param b and \param c
  vnl_vector_fixed<double, 4> param_to_pt_vector(int j,
                                                 const vnl_vector<double>& b,
                                                 const vnl_vector<double>& c) const
  {
  ...
  // : construct the \param j-th perspective camera from a pointer to the j-th parameter of \param bj and parameters \param c
  virtual vnl_vector_fixed<double, 4> param_to_pt_vector(int j, const double* bj,
                                                         const vnl_vector<double>& c) const = 0;

  // : construct the \param i-th perspective camera from parameter vector \param a
  vpgl_perspective_camera<double> param_to_cam(int i,
  ...
  static void jac_camera_center(vnl_double_3x3 const& M, vnl_vector<double> const& C, vnl_vector<double> const& pt,
                                vnl_matrix<double>& J);
  ...
  static void jac_camera_rotation(vnl_double_3x3 const& K, vnl_vector<double> const& C, vnl_vector<double> const& r,
                                  vnl_vector<double> const& pt, vnl_matrix<double>& J);
