...
public:
  ...
  void
  jac_blocks(const vnl_vector<double> & a,
             const vnl_vector<double> & b,
             const vnl_vector<double> & c,
  ...
  void
  set_residual_scale(double scale)
  {
    scale2_ = scale * scale;
  }
  ...
  param_to_point(int j, const vnl_vector<double> & b, const vnl_vector<double> & c) const
  {
    return param_to_point(j, b.data_block() + index_b(j), c);
  }

  //: construct the \param j-th perspective camera from a pointer to the j-th parameter of \param bj and parameters
  //: \param c
  virtual vgl_homg_point_3d<double>
  param_to_point(int j, const double * bj, const vnl_vector<double> & c) const = 0;

  //: construct the \param j-th 3D point from parameter vector \param b and \param c
  vnl_vector_fixed<double, 4>
  param_to_pt_vector(int j, const vnl_vector<double> & b, const vnl_vector<double> & c) const
  {
  ...
  param_to_cam(int i, const vnl_vector<double> & a, const vnl_vector<double> & c) const
  {
    return param_to_cam(i, a.data_block() + index_a(i), c);
  }

  //: construct the \param i-th perspective camera from a pointer to the i-th parameter of \param ai and parameters
  //: \param c
  ...
  vnl_double_3x4
  param_to_cam_matrix(int i, const vnl_vector<double> & a, const vnl_vector<double> & c) const
  {
    return param_to_cam_matrix(i, a.data_block() + index_a(i), c);
  }

  //: compute the 3x4 matrix of camera \param i from a pointer to the i-th parameter of \param ai and parameters \param
  //: c
  virtual vnl_double_3x4
  param_to_cam_matrix(int i, const double * ai, const vnl_vector<double> & c) const = 0;
  ...
  static void
  jac_camera_rotation(const vnl_double_3x3 & K,
                      const vnl_vector<double> & C,
                      const vnl_vector<double> & r,
                      const vnl_vector<double> & pt,
                      vnl_matrix<double> & J);
