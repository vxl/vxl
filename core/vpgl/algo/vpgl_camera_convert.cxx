...
static double
pval(const std::vector<double> & pv, const std::vector<double> & coef)
{
  double sum = 0.0;
...
// at the world center.
bool
vpgl_proj_camera_convert::convert(const vpgl_rational_camera<double> & rat_cam,
                                  const vgl_point_3d<double> & world_center,
...
vpgl_proj_camera_convert::norm_trans(const vpgl_rational_camera<double> & rat_cam)
{
  ...
  T.set(1, 1, 1 / yscale);
  T.set(2, 2, 1 / zscale);
  T.set(0, 3, -xoff / xscale);
  T.set(1, 3, -yoff / yscale);
...
bool
vpgl_perspective_camera_convert::convert(const vpgl_rational_camera<double> & rat_cam,
                                         const vgl_box_3d<double> & approximation_volume,
                                         ...
                                         vgl_h_matrix_3d<double> & norm_trans)
{
  ...
  norm_trans.set(0, 0, 1 / sox.scale());
  norm_trans.set(1, 1, 1 / soy.scale());
  ...
  double xrange = maxp.x() - xmin, yrange = maxp.y() - ymin, zrange = maxp.z() - zmin;
...
bool
vpgl_perspective_camera_convert::convert_local(const vpgl_rational_camera<double> & rat_cam,
                                               const vgl_box_3d<double> & approximation_volume,
                                               vpgl_perspective_camera<double> & camera,
                                               vgl_h_matrix_3d<double> & norm_trans)
{
  ...
  norm_trans.set_identity();
  norm_trans.set(0, 0, 2 / dlx);
  norm_trans.set(1, 1, 2 / dly);
  norm_trans.set(2, 2, 2 / dlz);
...
//
static bool
interp_ray(const std::vector<vgl_ray_3d<double>> & ray_nbrs, vgl_ray_3d<double> & intrp_ray)
{
...
static bool
ray_tol(const vpgl_local_rational_camera<double> & rat_cam,
        double mid_u,
        double mid_v,
        const vgl_plane_3d<double> & high,
        const vgl_point_3d<double> & high_guess,
        const vgl_plane_3d<double> & low,
        const vgl_point_3d<double> & low_guess,
        double & org_tol,
        double & dir_tol)
{
...
bool
vpgl_generic_camera_convert::upsample_rays(const std::vector<vgl_ray_3d<double>> & ray_nbrs,
                                           ...
                                           std::vector<vgl_ray_3d<double>> & interp_rays)
{
  ...
  if (nrays != 4)
    return false;
  vgl_ray_3d<double> r00 = ray_nbrs[0], r01 = ray_nbrs[1];
  vgl_ray_3d<double> r10 = ray_nbrs[2], r11 = ray_nbrs[3];
  vgl_point_3d<double> org00 = r00.origin(), org01 = r01.origin();
  vgl_point_3d<double> org10 = r10.origin(), org11 = r11.origin();
...
//
bool
vpgl_generic_camera_convert::convert(const vpgl_local_rational_camera<double> & rat_cam,
                                     int ni,
                                     int nj,
...
bool
vpgl_generic_camera_convert::pyramid_est(vpgl_local_rational_camera<double> const & rat_cam,
                                         int ni,
                                         int nj,
                                         int offseti,
                                         int offsetj,
...

{
  ...
  double ang_tol = 0.0;
  double max_org_err = 0.0, max_ang_err = 0.0;
  if (!ray_tol(rat_cam, offseti + ni / 2.0, offsetj + nj / 2.0, high, org, low, endpt, org_tol, ang_tol))
    return false;
  ...
  for (++lev; lev > 0; --lev)
  {
    ...
    for (unsigned int j = 0; j < nrb; ++j)
    {
      for (unsigned int i = 0; i < ncr; ++i)
      {
        ...
        if (j + 1 < nrb)
          ray_nbrs[2] = clev[j + 1][i];
        if (i + 1 < ncr && j + 1 < nrb)
          ray_nbrs[3] = clev[j + 1][i + 1];
        if (!upsample_rays(ray_nbrs, clev[j][i], interp_rays))
          return false;
        if (2 * i < nlev.cols() && 2 * j < nlev.rows())
        {
          ...
          if (2 * i + 1 < nlev.cols() && 2 * j + 1 < nlev.rows())
                                     ...
                                     unsigned level)
{
  ...
  for (unsigned int bigi = 0; bigi <= numi; bigi++)
  {
    ...
    for (unsigned int bigj = 0; bigj <= numj; bigj++)
    {
      ...
      if (!vpgl_generic_camera_convert::pyramid_est(
            rat_cam, ni, nj, offseti, offsetj, local_z_min, local_z_max, n_levels, nr, nc, scl, ray_pyr))
        return false;
...
#if 1
bool
vpgl_generic_camera_convert::convert_bruteforce(vpgl_local_rational_camera<double> const & rat_cam,
                                                int gni,
                                                int gnj,
                                                vpgl_generic_camera<double> & gen_cam,
                                                double local_z_min,
...
#endif
bool
vpgl_generic_camera_convert::convert(vpgl_proj_camera<double> const & prj_cam,
                                     int ni,
                                     int nj,
                                     vpgl_generic_camera<double> & gen_cam,
                                     unsigned level)
...
bool
vpgl_generic_camera_convert::convert(const vpgl_perspective_camera<double> & per_cam,
                                     int ni,
                                     int nj,
                                     vpgl_generic_camera<double> & gen_cam,
                                     unsigned level)
...
bool
vpgl_generic_camera_convert::convert_with_margin(const vpgl_perspective_camera<double> & per_cam,
                                                 int ni,
                                                 int nj,
                                                 vpgl_generic_camera<double> & gen_cam,
                                                 int margin,
...
bool
vpgl_generic_camera_convert::convert(const vpgl_affine_camera<double> & aff_cam,
                                     int ni,
                                     int nj,
                                     vpgl_generic_camera<double> & gen_cam,
                                     unsigned level)
...
bool
vpgl_generic_camera_convert::convert(const vpgl_camera_double_sptr & camera,
                                     int ni,
                                     int nj,
                                     vpgl_generic_camera<double> & gen_cam,
                                     unsigned level)
{
  if (auto * cam = dynamic_cast<vpgl_local_rational_camera<double> *>(camera.ptr()))
    return vpgl_generic_camera_convert::convert(*cam, ni, nj, gen_cam, level);

  if (auto * cam = dynamic_cast<vpgl_perspective_camera<double> *>(camera.ptr()))
  {
    return vpgl_generic_camera_convert::convert(*cam, ni, nj, gen_cam, level);
  }

  if (auto * cam = dynamic_cast<vpgl_affine_camera<double> *>(camera.ptr()))
    return vpgl_generic_camera_convert::convert(*cam, ni, nj, gen_cam, level);
...
bool
vpgl_generic_camera_convert::convert(vpgl_geo_camera & geocam,
                                     int ni,
                                     int nj,
                                     double height,
                                     vpgl_generic_camera<double> & gen_cam,
...
//: Convert a geocam (transformtaion matrix read from a geotiff header + an lvcs) to a generic camera using the
// specified ray direction (not necessarily nadir rays)
...
bool
vpgl_affine_camera_convert::convert(const vpgl_local_rational_camera<double> & camera_in,
...
#  if 0 // debug test
  for (unsigned i = 0; i < num_points; ++i) {
      double uu, vv;
      vgl_point_3d<double> wp = world_pts[i];
      camera_out.project(wp.x(), wp.y(), wp.z(), uu, vv);
...
#else
bool
vpgl_affine_camera_convert::convert(vpgl_local_rational_camera<double> const & camera_in,
...
bool
vpgl_rational_camera_convert::convert(vpgl_RSM_camera<double> const & camera_in,
                                      vgl_box_3d<double> const & region_of_interest,
