...
{
public:
  //: Find a projective camera that best approximates the rational camera at the world center (lon (deg), lat (deg),
  //: elev (meters) )
  static bool
  convert(const vpgl_rational_camera<double> & rat_cam,
          const vgl_point_3d<double> & world_center,
          vpgl_proj_camera<double> & camera);

  //: An auxiliary matrix that transforms (normalizes) world points prior to projection by a projective camera.  (lon,
  //: lat, elevation)->[-1, 1].
  static vgl_h_matrix_3d<double>
  norm_trans(const vpgl_rational_camera<double> & rat_cam);
...
public:
  ...
  static bool
  convert_local(const vpgl_rational_camera<double> & rat_cam,
                const vgl_box_3d<double> & approximation_volume,
                vpgl_perspective_camera<double> & camera,
                vgl_h_matrix_3d<double> & norm_trans);
...
public:
  ...
  static bool
  convert(const vpgl_local_rational_camera<double> & rat_cam,
          int ni,
          int nj,
  ...
  convert(const vpgl_local_rational_camera<double> & rat_cam,
          ...
          unsigned level = 0);
  static bool
  convert_bruteforce(const vpgl_local_rational_camera<double> & rat_cam,
                     int gni,
                     int gnj,
                     vpgl_generic_camera<double> & gen_cam,
                     double local_z_min,
  ...
  static bool
  convert(const vpgl_proj_camera<double> & prj_cam,
          int ni,
          int nj,
          vpgl_generic_camera<double> & gen_cam,
          unsigned level = 0);
  ...
  static bool
  convert(const vpgl_perspective_camera<double> & per_cam,
          int ni,
          int nj,
          vpgl_generic_camera<double> & gen_cam,
          unsigned level = 0);
  ...
  static bool
  convert_with_margin(vpgl_perspective_camera<double> const & per_cam,
                      int ni,
                      int nj,
                      vpgl_generic_camera<double> & gen_cam,
                      int margin,
  ...
  static bool
  convert(const vpgl_affine_camera<double> & aff_cam,
          int ni,
          int nj,
          vpgl_generic_camera<double> & gen_cam,
          unsigned level = 0);
  ...
  static bool
  convert(const vpgl_camera_double_sptr & camera,
          int ni,
          int nj,
          vpgl_generic_camera<double> & gen_cam,
          unsigned level = 0);
  ...
  static bool
  convert(vpgl_geo_camera & geocam,
          int ni,
          int nj,
          double height,
          vpgl_generic_camera<double> & gen_cam,
...
private:
  ...
  static bool
  upsample_rays(const std::vector<vgl_ray_3d<double>> & ray_nbrs,
                const vgl_ray_3d<double> & ray,
                std::vector<vgl_ray_3d<double>> & interp_rays);
  //: interpolate a span of rays base on a linear interpolation. n_grid is the step distance from r1. r0 and r1 are one
  //: unit apart.
  static vgl_ray_3d<double>
  interp_pair(const vgl_ray_3d<double> & r0, const vgl_ray_3d<double> & r1, double n_grid);
  static bool
  pyramid_est(const vpgl_local_rational_camera<double> & rat_cam,
              int ni,
              int nj,
              int offseti,
              int offsetj,
...
public:
  //: Convert from rational camera using a local Euclidean coordinate system
  static bool
  convert(const vpgl_local_rational_camera<double> & camera_in,
