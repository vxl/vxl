...
void vpgl_camera_transform_f::compute_cams(vnl_vector<double> const& x,
...
void vpgl_camera_transform_f::compute_cams_selective(vnl_vector<double> const& x,
                                                     vcl_vector<vpgl_perspective_camera<double> >& output_cams)
{
...
bool vpgl_camera_transform::compute_fixed_transformation(
  const vcl_vector<vpgl_perspective_camera<double>  >& input_cams,
  const vcl_vector<vcl_vector<vcl_pair<vnl_vector_fixed<double,
                                                        2>, unsigned> > >& cam_ids_img_pts,
  const vcl_vector<vnl_vector_fixed<double, 4> >& pts_3d,
  vcl_vector<vpgl_perspective_camera<double>  >& output_cams)
{
  ...
  for( unsigned i = 0; i < output_cams_denorm.size(); i++ )
    {
    vpgl_calibration_matrix<double> K = input_cams[i].get_calibration();
    vpgl_perspective_camera<double> cnew(K, output_cams_denorm[i].get_rotation(),
                                         output_cams_denorm[i].get_translation() );
...
vcl_vector<vnl_vector_fixed<double, 3> > vpgl_camera_transform::sample_centers(double dim_x, double dim_y, double dim_z,
                                                                               double step)
{
...
bool vpgl_camera_transform::compute_fixed_transformation_sample(
  const vcl_vector<vpgl_perspective_camera<double>  >& input_cams,
  ...
  for( unsigned i = 0; i < offsets.size(); i++ )
    ...
    if( enderror < error_min )
      ...
      for( unsigned j = 0; j < output_cams_denorm.size(); j++ )
        ...
        vpgl_perspective_camera<double> cnew(K,
                                             output_cams_denorm[j].get_rotation(),
                                             output_cams_denorm[j].get_translation() );
        output_cams[j] = cnew;
        }
      }
    }
  vcl_cout << " final error min: " << error_min << " final offset: " << offset_min << " tried: " << offsets.size()
           << " offsets!\n";
  return true;
}

bool vpgl_camera_transform::compute_initial_transformation(
  const vcl_vector<vpgl_perspective_camera<double> >& input_cams,
  const vcl_vector<vcl_vector<vcl_pair<vnl_vector_fixed<double,
                                                        2>, unsigned> > >& cam_ids_img_pts,
  const vcl_vector<vnl_vector_fixed<double, 4> >& pts_3d,
...
bool vpgl_camera_transform::compute_initial_transformation_t(
  const vcl_vector<vpgl_perspective_camera<double> >& input_cams,
  const vcl_vector<vcl_vector<vcl_pair<vnl_vector_fixed<double, 2>, unsigned> > >& cam_ids_img_pts,
  const vcl_vector<vnl_vector_fixed<double, 4> >& pts_3d,
  vcl_vector<vpgl_perspective_camera<double> >& output_cams)
...
bool vpgl_camera_transform::compute_initial_transformation_R(
  const vcl_vector<vpgl_perspective_camera<double> >& input_cams,
  const vcl_vector<vcl_vector<vcl_pair<vnl_vector_fixed<double, 2>, unsigned> > >& cam_ids_img_pts,
  const vcl_vector<vnl_vector_fixed<double, 4> >& pts_3d,
  vcl_vector<vpgl_perspective_camera<double> >& output_cams)
{
  ...
  vcl_cout << " R_fixed_norm: " << R_fixed_norm << vcl_endl; vcl_cout << " det of R_fixed_norm: " << vnl_det(
    R_fixed_norm) << vcl_endl;
  /////////////////////////////////////////////////////////////////////////
  // denormalize and compute R and t
  for( unsigned i = 0; i < input_cams.size(); i++ )
    {
...
void vpgl_camera_transform::K_normalize_img_pts(const vcl_vector<vpgl_perspective_camera<double> >& input_cams,
                                                                               ...
                                                                               unsigned> > >& cam_ids_img_pts_norm)
{
  ...
  for( unsigned j = 0; j < cam_ids_img_pts.size(); j++ )
    ...
    for( unsigned i = 0; i < cam_ids_img_pts[j].size(); i++ )
      ...
      vcl_pair<vnl_vector_fixed<double, 3>, vnl_matrix_fixed<double, 3, 3> > pair(cam_pt,
                                                                                  correspondence_covariance);
      vcl_pair<vcl_pair<vnl_vector_fixed<double, 3>, vnl_matrix_fixed<double, 3, 3> >, unsigned> pair2(pair,
                                                                                                       cam_ids_img_pts[j][i].second);
...
bool vpgl_camera_transform::compute_covariance(unsigned cam_i, unsigned cam_j,
                                               const vcl_vector<vpgl_perspective_camera<double> >& input_cams,
                                               const vcl_vector<vcl_vector<vcl_pair<vcl_pair<vnl_vector_fixed<double,
                                                                                                              3>,
                                                                                             vnl_matrix_fixed<double, 3,
                                                                                                              3> >,
                                               unsigned> > >& cam_ids_img_pts,
