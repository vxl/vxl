...
//:
// \file
// \brief Methods for computing a fixed transformation that moves and orients a camera so that given 3-d points project
// to the given 2-d image positions
...
//  6 dimensional param vector, x[0], x[1], x[2] constitute Rodriguez vector of rotation, x[3], x[4], x[5] constitute
...
{
public:
  //: Constructor. The source image is mapped to the destination frame by dt. nbins is the number of histogram bins used
  //: to compute entropies.
  vpgl_camera_transform_f(unsigned cnt_residuals,
                          unsigned n_unknowns,
  ...
  virtual vnl_matrix_fixed<double, 3, 3>
  rod_to_matrix(double r0, double r1, double r2);
  virtual void
  compute_cams(const vnl_vector<double> & x, std::vector<vpgl_perspective_camera<double>> & output_cams);
  virtual void
  compute_cams_selective(const vnl_vector<double> & x, std::vector<vpgl_perspective_camera<double>> & output_cams);
...
public:
  ...
  static void
  apply_fixed_transformation(const std::vector<vpgl_perspective_camera<double>> & input_cams,
  ...
  static std::vector<vnl_vector_fixed<double, 3>>
  sample_centers(double dim_x, double dim_y, double dim_z, double step);
  ...
  static void
  normalize_img_pts(const std::vector<vpgl_perspective_camera<double>> & input_cams,
                    const std::vector<std::vector<std::pair<vnl_vector_fixed<double, 2>, unsigned>>> & cam_ids_img_pts,
  ...
  // compute both R and t -- // TODO: the formulation for the initial transformation need to be updated to multiple R
  // from left (not right as in the current version)
  static bool
  ...
  // compute only R -- // TODO: the formulation for the initial transformation need to be updated to multiple R from
  // left (not right as in the current version)
  static bool
  ...
  static bool
  normalize_to_rotation_matrix(const vnl_matrix_fixed<double, 3, 3> & R, vnl_matrix_fixed<double, 3, 3> & R_norm);
  // use quaternions
  static bool
  normalize_to_rotation_matrix_q(const vnl_matrix_fixed<double, 3, 3> & R, vnl_matrix_fixed<double, 3, 3> & R_norm);

  //: pass the ids of cams in the input_cams vector, this method computes the variance between these two using their
  //: image correspondences (K normalized image points)
