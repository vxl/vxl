...
//:
// \file
// \brief The 5 point algorithm as described by David Nister for computing an essential matrix from point
// correspondences.
...
public:
  ...
  bool
  compute(const std::vector<vgl_point_2d<T>> & normed_right_points,
  ...
  bool
  compute(const std::vector<vgl_point_2d<T>> & right_points,
...
protected:
  ...
  void
  normalize(const std::vector<vgl_point_2d<T>> & points,
            const vpgl_calibration_matrix<T> & k,
            std::vector<vgl_point_2d<T>> & normed_points) const;

  void
  compute_nullspace_basis(const std::vector<vgl_point_2d<T>> & right_points,
                          const std::vector<vgl_point_2d<T>> & left_points,
                          std::vector<vnl_vector_fixed<T, 9>> & basis) const;

  void
  compute_constraint_polynomials(const std::vector<vnl_vector_fixed<T, 9>> & basis,
                                 std::vector<vnl_real_npolynomial> & constraints) const;

  void
  compute_groebner_basis(const std::vector<vnl_real_npolynomial> & constraints,
                         vnl_matrix<double> & groebner_basis) const;

  void
  compute_action_matrix(const vnl_matrix<double> & groebner_basis, vnl_matrix<double> & action_matrix) const;

  void
  compute_e_matrices(const std::vector<vnl_vector_fixed<T, 9>> & basis,
                     const vnl_matrix<double> & action_matrix,
                     std::vector<vpgl_essential_matrix<T>> & ems) const;

  double
  get_coeff(const vnl_real_npolynomial & p, unsigned int x_p, unsigned int y_p, unsigned int z_p) const;
};
