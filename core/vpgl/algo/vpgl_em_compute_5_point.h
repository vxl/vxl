...
public:
  ...
  bool compute( const vcl_vector<vgl_point_2d<T> > & right_points, const vpgl_calibration_matrix<T> & k_right,
                const vcl_vector<vgl_point_2d<T> > & left_points, const vpgl_calibration_matrix<T> & k_left,
                vcl_vector<vpgl_essential_matrix<T> > & ems) const;
...
protected:
  ...
  void normalize(const vcl_vector<vgl_point_2d<T> > & points, const vpgl_calibration_matrix<T> & k,
                 vcl_vector<vgl_point_2d<T> > & normed_points) const;

  void compute_nullspace_basis(const vcl_vector<vgl_point_2d<T> > & right_points,
                               const vcl_vector<vgl_point_2d<T> > & left_points, vcl_vector<vnl_vector_fixed<T,
                                                                                                             9> > & basis)
  const;

  void compute_constraint_polynomials(const vcl_vector<vnl_vector_fixed<T, 9> > & basis,
                                      vcl_vector<vnl_real_npolynomial> & constraints) const;

  void compute_groebner_basis(const vcl_vector<vnl_real_npolynomial> & constraints,
  ...
  void compute_e_matrices(const vcl_vector<vnl_vector_fixed<T, 9> > & basis, const vnl_matrix<double> & action_matrix,
                          vcl_vector<vpgl_essential_matrix<T> > & ems) const;
...
public:
  ...
  bool compute(vcl_vector<vgl_point_2d<T> > const& right_points, vpgl_calibration_matrix<T> const& right_k,
               vcl_vector<vgl_point_2d<T> > const& left_points, vpgl_calibration_matrix<T> const& left_k,
