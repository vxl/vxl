...
template <class T>
bool
vpgl_em_compute_5_point<T>::compute(const std::vector<vgl_point_2d<T>> & right_points,
...
template <class T>
bool
vpgl_em_compute_5_point<T>::compute(const std::vector<vgl_point_2d<T>> & normed_right_points,
  ...
  if (normed_right_points.size() != 5 || normed_left_points.size() != 5)
  {
    if (verbose)
    {
      std::cerr << "Wrong number of input points!\n"
                << "right_points has " << normed_right_points.size() << " and left_points has "
                << normed_left_points.size() << '\n';
    }
    return false;
  }
...
template <class T>
void
vpgl_em_compute_5_point<T>::normalize(const std::vector<vgl_point_2d<T>> & points,
...
template <class T>
void
vpgl_em_compute_5_point<T>::compute_nullspace_basis(const std::vector<vgl_point_2d<T>> & right_points,
                                                    const std::vector<vgl_point_2d<T>> & left_points,
...
template <class T>
void
vpgl_em_compute_5_point<T>::compute_constraint_polynomials(const std::vector<vnl_vector_fixed<T, 9>> & basis,
                                                           std::vector<vnl_real_npolynomial> & constraints) const
{
  ...
  vnl_real_npolynomial sum_of_squares = entry_polynomials[0] * entry_polynomials[0];

  for (int i = 1; i < 9; ++i)
  {
    sum_of_squares = sum_of_squares + entry_polynomials[i] * entry_polynomials[i];
  }
  ...
  for (int i = 0; i < 9; ++i)
  {
    constraints.push_back(entry_polynomials[i % 3] * (entry_polynomials[0] * entry_polynomials[3 * (i / 3) + 0] * 2 +
  ...
  for (int i = 0; i < 9; ++i)
  {
    constraints[i] += entry_polynomials[(i % 3) + 3] * (entry_polynomials[3] * entry_polynomials[3 * (i / 3) + 0] * 2 +
  ...
  for (int i = 0; i < 9; ++i)
  {
    constraints[i] = (constraints[i] + entry_polynomials[(i % 3) + 6] *
  ...
  // (bf - ec) * g = (1*5 - 4*2) * 4
  vnl_real_npolynomial det_term_1 =
    entry_polynomials[6] * (entry_polynomials[1] * entry_polynomials[5] - entry_polynomials[2] * entry_polynomials[4]);

  // (cd - fa) * h
  vnl_real_npolynomial det_term_2 =
    entry_polynomials[7] * (entry_polynomials[2] * entry_polynomials[3] - entry_polynomials[0] * entry_polynomials[5]);

  // (ae - db) * i
  vnl_real_npolynomial det_term_3 =
    entry_polynomials[8] * (entry_polynomials[0] * entry_polynomials[4] - entry_polynomials[1] * entry_polynomials[3]);
...
template <class T>
double
vpgl_em_compute_5_point<T>::get_coeff(const vnl_real_npolynomial & p,
                                      unsigned int x_p,
                                      unsigned int y_p,
                                      unsigned int z_p) const
{
  ...
  {
    if (x_p == p.polyn().get(i, 0) && y_p == p.polyn().get(i, 1) && z_p == p.polyn().get(i, 2))
    {
...
template <class T>
void
vpgl_em_compute_5_point<T>::compute_groebner_basis(const std::vector<vnl_real_npolynomial> & constraints,
                                                   vnl_matrix<double> & groebner_basis) const
{
  ...
  for (int i = 0; i < 10; ++i)
  {
    ...
    for (int j = i + 1; j < 10; ++j)
    {
      ...
      vnl_vector<double> new_row = A.get_row(j) - A.get_row(i) * leading2;
  ...
  for (int i = 9; i >= 0; --i)
  {
    for (int j = 0; j < i; ++j)
    {
      ...
      vnl_vector<double> new_row = A.get_row(j) - A.get_row(i) * scale;
...
template <class T>
void
vpgl_em_compute_5_point<T>::compute_action_matrix(const vnl_matrix<double> & groebner_basis,
...
template <class T>
void
vpgl_em_compute_5_point<T>::compute_e_matrices(const std::vector<vnl_vector_fixed<T, 9>> & basis,
                                               ...
                                               std::vector<vpgl_essential_matrix<T>> & ems) const
{
  ...
  for (unsigned int i = 0; i < eigs.W.size(); ++i)
  {
    if (std::abs(eigs.W[i].imag()) <= tolerance)
    {
      ...
      linear_e = x * basis[0] + y * basis[1] + z * basis[2] + basis[3];
      linear_e /= linear_e[8];
      // ignore solutions that are non-finite
      if (!linear_e.is_finite())
        continue;

      ems.push_back(vpgl_essential_matrix<T>(vnl_matrix_fixed<T, 3, 3>(linear_e.data_block())));
    }
  }
}
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
static inline void
get_distinct_indices(int n, int * idxs, int number_entries)
{
  for (int i = 0; i < n; ++i)
  {
    bool found = false;
...
template <class T>
bool
vpgl_em_compute_5_point_ransac<T>::compute(const std::vector<vgl_point_2d<T>> & right_points,
                                           const vpgl_calibration_matrix<T> & right_k,

                                           const std::vector<vgl_point_2d<T>> & left_points,
                                           const vpgl_calibration_matrix<T> & left_k,
                                           ...
                                           vpgl_essential_matrix<T> & best_em) const
{
  ...
  if (right_points.size() != left_points.size())
  {
    if (verbose)
    {
      std::cerr << "The two vectors of points must be the same size!\n"
                << "right_points is size " << right_points.size() << " while left_points is size " << left_points.size()
                << ".\n";
    }
    return false;
  }
  else if (right_points.size() < 5)
  {
    if (verbose)
    {
      std::cerr << "There need to be at least 5 points to run the "
                << "five-point algorithm!\n"
                << "Input only contained " << right_points.size() << " points.\n";
    }
    return false;
  }
  ...
  for (unsigned int r = 0; r < num_rounds; ++r)
  {
    ...
    five_point.compute(right_points_to_use, right_k, left_points_to_use, left_k, ems);
    ...
    for (i = ems.begin(); i != ems.end(); ++i)
    {
      ...
      for (unsigned j = 0; j < num_points; ++j)
      {
        ...
        double error = (1.0 / (f_l.get(0, 0) * f_l.get(0, 0) + f_l.get(1, 0) * f_l.get(1, 0)) +
                        1.0 / (f_r.get(0, 0) * f_r.get(0, 0) + f_r.get(1, 0) * f_r.get(1, 0))) *
                       (p * p);
