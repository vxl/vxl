...
public:
  ...
  vpgl_orientation_lsqr(const vpgl_calibration_matrix<double>& K, const vgl_point_3d<double>& c,
...
{
public: ~vpgl_optimize_camera();

  // : optimize orientation for a perspective camera
  static vpgl_perspective_camera<double> opt_orient(const vpgl_perspective_camera<double>& camera,
                                                    const vcl_vector<vgl_homg_point_3d<double> >& world_points,
                                                    const vcl_vector<vgl_point_2d<double> >& image_points );

  // : optimize orientation and position for a perspective camera
  static vpgl_perspective_camera<double> opt_orient_pos(const vpgl_perspective_camera<double>& camera,
                                                        const vcl_vector<vgl_homg_point_3d<double> >& world_points,
                                                        const vcl_vector<vgl_point_2d<double> >& image_points );

  // : optimize orientation, position and internal calibration(no skew)for a perspective camera
  static vpgl_perspective_camera<double> opt_orient_pos_cal(const vpgl_perspective_camera<double>& camera,
                                                            const vcl_vector<vgl_homg_point_3d<double> >& world_points,
                                                            const vcl_vector<vgl_point_2d<double> >& image_points,
