...
void
vpgl_adjust_lsqr::f(const vnl_vector<double> & unknowns, vnl_vector<double> & projection_error)
{
  ...
  std::cout << "x(" << unknowns[0] << ' ' << unknowns[1] << ")\n"
...
#if 0
    projection_error[ir++] = std::fabs(pp.x()-c.x());
    projection_error[ir++] = std::fabs(pp.y()-c.y());

    std::cout << "perror[" << i << "](" << projection_error[ir-2] << ' '
                ...
                double & zoff)
{
  ...
  const vgl_plane_3d<double> pl(0, 0, 1, -zoff);

  // an initial point for the backprojection
  const double xo = initial_rcam.offset(vpgl_rational_camera<double>::X_INDX);
  const double yo = initial_rcam.offset(vpgl_rational_camera<double>::Y_INDX);
  const vgl_point_3d<double> initial_pt(xo, yo, zoff);
  ...
  for (unsigned i = 0; i < npts; ++i)
  {
    ...
    if (!vpgl_backproject::bproj_plane(initial_rcam, img_pts[i], pl, initial_pt, bp_pt))
      return false;
                             ...
                             vpgl_rational_camera<double> & adj_rcam)
{
  ...
  const unsigned num_unknowns = 3;
  const unsigned num_residuals = num_corrs * 2;
  // Initialize the least squares function
  vpgl_adjust_lsqr lsf(initial_rcam, img_pts, geo_pts, num_unknowns, num_residuals);
...
#ifdef ADJUST_DEBUG
  std::cout << "Min error of " << levmarq.get_end_error() << " at the following local minima : " << '\n';
#endif
  ...
  for (unsigned i = 0; i < num_corrs; ++i)
  {
    ...
    const double d = vgl_distance<double>(c, pp);
