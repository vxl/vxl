...
//: to generate the permutations, always increment the one at the very end by one; if it exceeds max, then increment the
// one before as well, etc.
bool
increment_perm(std::vector<unsigned> & params_indices, unsigned size)
{
  ...
  if (params_indices[params_indices.size() - 1] == size)
  { // carry on
  ...
  std::vector<vgl_point_3d<double>> & intersections)
{
  ...
  // find the best intersections for all the correspondences using the given cameras to compute good initial estimates
  // for z of each correspondence
...
vpgl_cam_trans_search_lsqr::vpgl_cam_trans_search_lsqr(
  ...
  : vnl_least_squares_function(2 * (unsigned)cams.size(),
                               (unsigned)(cams.size() * image_pts.size()),
...
void
vpgl_cam_trans_search_lsqr::get_finals(std::vector<vgl_point_3d<double>> & finals)
{
  finals = finals_;
}
  ...
  std::vector<vgl_point_3d<double>> & intersections)            // output 3d locations for each correspondence
{
  ...
  {
    vgl_point_3d<double> final;
    if (!vpgl_rational_adjust_onept::refine_intersection_pt(
          cams, cam_weights, corrs_reformatted[i], intersections_initial[i], final))
      return false;
  ...
  {
    // if cam weight is 1 for one of them, it is a special case, then pass 0 as translation for that one, cause
    // projections still cause a tiny translation, ignore that
  ...
  const double relative_diameter)
{
  ...
  // find the best 3-d intersections for all the correspondences using the given cameras to compute good initial
  // estimates for z of each correspondence
  std::vector<vgl_point_3d<double>> intersections_initial;
  for (const auto & i : corrs_reformatted)
  {
    vgl_point_3d<double> pt;
    if (!vpgl_rational_adjust_onept::find_intersection_point(
          cams, cam_weights, i, initial_pt, zmin, zmax, pt, relative_diameter))
      return false;
    intersections_initial.push_back(pt);
  }
  ...
  for (unsigned i = 0; i < corrs_reformatted.size(); i++)
  {
    vgl_point_3d<double> final;
    if (!vpgl_rational_adjust_onept::refine_intersection_pt(
          cams, cam_weights, corrs_reformatted[i], intersections_initial[i], final, relative_diameter))
      return false;
    intersections_initial[i] = final;
  }
  for (const auto & i : intersections_initial)
