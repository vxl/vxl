...
double error_corr(vpgl_rational_camera<double> const& cam, vgl_point_2d<double> const& corr,
                                               ...
                                               vcl_vector<vgl_point_3d<double> >& intersections)
{
  ...
  while( !done )
    ...
    vcl_vector<vgl_point_3d<double> > finals;
    double                            err = re_projection_error(current_cams, cam_weights, corrs_reformatted,
                                                                intersections_initial, finals);
...
vpgl_cam_trans_search_lsqr::vpgl_cam_trans_search_lsqr(vcl_vector<vpgl_rational_camera<double> > const& cams,
  ...
  :  vnl_least_squares_function(2 * (unsigned)cams.size(),
                                (unsigned)(cams.size() * image_pts.size() ), vnl_least_squares_function::no_gradient),
                                                        ...
                                                        vcl_vector<vgl_point_3d<double> >& intersections)            // output 3d locations for each correspondence
{
  ...
  for( unsigned int i = 0; i < corrs_reformatted.size(); ++i )
    {
    vgl_point_3d<double> final;
    if( !vpgl_rational_adjust_onept::refine_intersection_pt(cams, cam_weights, corrs_reformatted[i],
                                                        ...
                                                        double const relative_diameter)
{
  ...
  for( unsigned i = 0; i < corrs_reformatted.size(); i++ )
    {
    vgl_point_3d<double> pt;
    if( !vpgl_rational_adjust_onept::find_intersection_point(cams, cam_weights, corrs_reformatted[i], initial_pt, zmin,
                                                             zmax, pt, relative_diameter) )
      {
      return false;
      }
    intersections_initial.push_back(pt);
  ...
  for( unsigned i = 0; i < corrs_reformatted.size(); i++ )
    ...
    vgl_point_3d<double> final;
    if( !vpgl_rational_adjust_onept::refine_intersection_pt(cams, cam_weights, corrs_reformatted[i],
                                                            intersections_initial[i], final, relative_diameter) )
      {
      return false;
      }
    intersections_initial[i] = final;
