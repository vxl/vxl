...
public:
  // : Constructor
  vpgl_z_search_lsqr(vcl_vector<vpgl_rational_camera<double> > const& cams, vcl_vector<float> const& cam_weights,
                     vcl_vector<vgl_point_2d<double> > const& image_pts, vgl_point_3d<double> const& initial_pt,
...
public:
  ...
  static bool find_intersection_point(vcl_vector<vpgl_rational_camera<double> > const& cams,
                                      vcl_vector<float> const& cam_weights,
                                      vcl_vector<vgl_point_2d<double> > const& corrs, vgl_point_3d<double>& p_3d);

  static bool find_intersection_point(vcl_vector<vpgl_rational_camera<double> > const& cams,
                                      vcl_vector<float> const& cam_weights,
                                      vcl_vector<vgl_point_2d<double> > const& corrs,
                                      vgl_point_3d<double> const& initial_pt, double const& zmin, double const& zmax,
                                      vgl_point_3d<double> & p_3d, double const& relative_diameter = 1.0);

  static bool refine_intersection_pt(vcl_vector<vpgl_rational_camera<double> > const& cams,
                                     vcl_vector<float> const& cam_weights,
                                     vcl_vector<vgl_point_2d<double> > const& image_pts,
                                     vgl_point_3d<double> const& initial_pt, vgl_point_3d<double>& final_pt,
                                     double const& relative_diameter = 1.0);

  static bool adjust(vcl_vector<vpgl_rational_camera<double> > const& cams,
                     vcl_vector<vgl_point_2d<double> > const& corrs,
                     vcl_vector<vgl_vector_2d<double> >& cam_translations, vgl_point_3d<double>& intersection);

  static bool adjust(vcl_vector<vpgl_rational_camera<double> > const& cams,
                     vcl_vector<vgl_point_2d<double> > const& corrs, vgl_point_3d<double> const& initial_pt,
                     double const& zmin, double const& zmax, vcl_vector<vgl_vector_2d<double> >& cam_translations,
  ...
  static bool adjust_with_weights(vcl_vector<vpgl_rational_camera<double> > const& cams,
                                  vcl_vector<float> const& weights, vcl_vector<vgl_point_2d<double> > const& corrs,
                                  vgl_point_3d<double> const& initial_pt, double const& zmin, double const& zmax,
                                  vcl_vector<vgl_vector_2d<double> >& cam_translations,
                                  vgl_point_3d<double>& intersection, double const& relative_diameter = 1.0);
