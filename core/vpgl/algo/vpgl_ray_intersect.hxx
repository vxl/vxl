...
public:
  ...
  void
  f(const vnl_vector<double> & intersection_point, vnl_vector<double> & image_errors) override;
...
vpgl_ray_intersect_lsqr<T>::vpgl_ray_intersect_lsqr(std::vector<const vpgl_camera<T> *> cams,
                                                    std::vector<vgl_point_2d<T>> image_pts,
                                                    unsigned num_residuals)
  : vnl_least_squares_function(3, num_residuals, vnl_least_squares_function::no_gradient)
...
template <typename T>
void
vpgl_ray_intersect_lsqr<T>::f(const vnl_vector<double> & intersection_point, vnl_vector<double> & image_errors)
{
...
#ifdef RAY_INT_DEBUG
  std::cout << "Error Vector (" << intersection_point_x << ", " << intersection_point_y << ", " << intersection_point_z
            << ") = ";
#endif
  ...
  for (unsigned image_no = 0; image_no < dim; image_no++)
  {
    ...
    double image_u = f_image_pts_[image_no].x(), image_v = f_image_pts_[image_no].y();
    // Compute the image of the intersection vert through this camera
    T cur_image_u, cur_image_v;
    cam->project(intersection_point_x, intersection_point_y, intersection_point_z, cur_image_u, cur_image_v);
    // Compute and store the error with respect to actual image
    image_errors.put(2 * image_no, (cur_image_u - image_u));
    image_errors.put(2 * image_no + 1, (cur_image_v - image_v));
#ifdef RAY_INT_DEBUG
    std::cout << " x_err = " << cur_image_u << '-' << image_u << '=' << image_errors[2 * image_no]
              << " y_err = " << cur_image_v << '-' << image_v << '=' << image_errors[2 * image_no + 1];
#endif
...
template <typename T>
bool
vpgl_ray_intersect<T>::intersect(const std::vector<const vpgl_camera<T> *> & cams,
                                 ...
                                 vgl_point_3d<T> & intersection)
{
...
#ifdef RAY_INT_DEBUG
  std::cout << "Initialized the intersection point " << intersection_pt << " ... minimizing\n";
#endif
...
#ifdef RAY_INT_DEBUG
  std::cout << "Min error of " << levmarq.get_end_error() << " at " << '(' << intersection_pt[0] << ", "
            << intersection_pt[1] << ", " << intersection_pt[2] << ")\n"
  ...
  intersection.set(intersection_pt[0], intersection_pt[1], intersection_pt[2]);
  // Return success
  return true;
}
