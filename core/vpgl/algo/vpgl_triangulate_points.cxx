                                     ...
                                     vgl_point_3d<double> &                               point_3d)
{
  ...
  {
    const vgl_vector_3d<double> & trans = cameras[i].get_translation();

    const vnl_double_3x3 & rot = cameras[i].get_rotation().as_matrix();

    const vgl_point_2d<double> pt = cameras[i].get_calibration().map_to_focal_plane(points[i]);
