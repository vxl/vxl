                                      ...
                                      vpgl_geo_camera *& camera)
{
  ...
  if( gtif->PCS_WGS84_UTM_zone(utm_zone, h) || gtif->PCS_NAD83_UTM_zone(utm_zone, h) )
    ...
    vcl_cout
      <<
    "vpgl_geo_camera::init_geo_camera()-- if UTM only PCS_WGS84_UTM and PCS_NAD83_UTM, if geographic (GCS_WGS_84) only linear units in meters, angular units in degrees are supported, please define otherwise!"
      << vcl_endl;
    return false;
    }
}
...
bool vpgl_geo_camera::init_geo_camera(vcl_string img_name, unsigned ni, unsigned nj, vpgl_lvcs_sptr lvcs,
                                      vpgl_geo_camera *& camera)
{
  ...
  vcl_string n_str = n_coords.substr(n_coords.find_first_of(hemisphere) + 1, n_coords.find_first_of(
                                       direction) - n_coords.find_first_of(hemisphere) - 1);
  ...
  if( direction == "W" )
    {
    vcl_cout << " lower right corner in the image is: " << hemisphere << lat << direction << lon - scale_lon
             << vcl_endl;
    }
  else
    {
  ...
  vcl_cout << " lat: " << lat << " lon: " << lon << " WARNING: using same scale for both ni and nj: scale:" << scale
           << vcl_endl;
...
bool vpgl_geo_camera::init_geo_camera_from_filename(vcl_string img_name, unsigned ni, unsigned nj, vpgl_lvcs_sptr lvcs,
                                                    vpgl_geo_camera *& camera)
{
  // determine the translation matrix from the image file name and construct a geo camera
  vcl_string name = vul_file::strip_directory(img_name);
...
bool vpgl_geo_camera::init_geo_camera(vcl_string tfw_name, vpgl_lvcs_sptr lvcs, int utm_zone, unsigned northing,
                                      vpgl_geo_camera *& camera)
{
...
void vpgl_geo_camera::global_utm_to_img(const double x, const double y, int zone, double elev, double& u,
                                        double& v) const
{
  vnl_vector<double> vec(4), res(4);
  if( is_utm )
...
bool vpgl_geo_camera::img_four_corners_in_utm(const unsigned ni, const unsigned nj, double elev, double& e1, double& n1,
                                              double& e2, double& n2)
{
  if( !is_utm )
    {
...
void vpgl_geo_camera::img_to_wgs(unsigned /*i*/, unsigned /*j*/, unsigned /*k*/, double & /*lon*/, double & /*lat*/,
                                 double & /*elev*/)
{
