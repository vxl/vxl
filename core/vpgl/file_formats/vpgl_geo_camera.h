...
public:
  ...
  static bool init_geo_camera(vil_image_resource_sptr const geotiff_img, vpgl_lvcs_sptr lvcs,
  ...
  static bool init_geo_camera(vcl_string img_name, unsigned ni, unsigned nj, vpgl_lvcs_sptr lvcs,
                              vpgl_geo_camera *& camera);
  ...
  static bool init_geo_camera_from_filename(vcl_string img_name, unsigned ni, unsigned nj, vpgl_lvcs_sptr lvcs,
                                            vpgl_geo_camera *& camera);

  // : init using a tfw file, reads the transformation matrix from the tfw
  static bool init_geo_camera(vcl_string tfw_name, vpgl_lvcs_sptr lvcs, int utm_zone, unsigned northing,
                              vpgl_geo_camera *& camera);
  ...
  {
    if( scale_tag_ ) { return trans_matrix_[0][0]; }
    else { return 1.0; } }

  bool operator ==(vpgl_geo_camera const& rhs) const;

  static bool comp_trans_matrix(double sx1, double sy1, double sz1, vcl_vector<vcl_vector<double> > tiepoints,
                                vnl_matrix<double>& trans_matrix, bool scale_tag = false);
  ...
  bool img_four_corners_in_utm(const unsigned ni, const unsigned nj, double elev, double& e1, double& n1, double& e2,
                               double& n2);
  ...
  void wgs_to_img(double lon, double lat, unsigned& i, unsigned& j);
