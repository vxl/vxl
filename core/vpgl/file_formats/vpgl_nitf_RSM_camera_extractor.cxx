...
void adjustable_parameter_metadata::print(std::ostream& os){
  if(!defined_)
    return;
  ...
  os << "adjustable parameter covariance indices I* image space, G* ground space:"
     << std::endl;
      ...
      for (tres_itr = hdr_ixshd_tres_[header_idx].begin(); tres_itr != hdr_ixshd_tres_[header_idx].end() && !found; ++tres_itr)
        {
          ...
          if (tre_name == "RSMPCA") { // looking for "RSMPCA..."
            if(verbose_) std::cout << "RSMPCA PRESENT IN IMAGE SUBHEADER " << header_idx << std::endl;
            ...
            {//is there a finite amount of data?
              ...
              for (tres_itr = ovfl_ixshd_tres_[header_idx].begin();
                   tres_itr != ovfl_ixshd_tres_[header_idx].end() && !found; ++tres_itr)
                {
                  ...
                  if (type == "RSMIDA") {
                    if(verbose_) std::cout << "RSMPCA PRESENT IN OVERFLOW DES: NOT IN IMAGE SUBHEADER "
                              << header_idx << std::endl;
                    found = true;
                  }
                }
  ...
  std::vector<vil_nitf2_image_subheader*> headers = nitf_image->get_image_headers();
    ...
    for (unsigned i = 0; i < headers.size(); ++i) {
        ...
        if (!hdr->get_property("IGEOLO", igeolo)) {
          if(verbose_) std::cout << "IGEOLO Property not specified in vil_nitf2_image_subheader " << i << "\n";
        }
        else {
        ...
        if(verbose_) print_file_header_summary();
        //return true;
    }
    return true;
}
vpgl_nitf_RSM_camera_extractor::vpgl_nitf_RSM_camera_extractor(std::string const & nitf_image_path, bool verbose):verbose_(verbose)
{
...
vpgl_nitf_RSM_camera_extractor::vpgl_nitf_RSM_camera_extractor(vil_nitf2_image * nitf_image, bool verbose):verbose_(verbose)
{
  RSM_defined_ = false;
  if (!this->init(nitf_image, verbose_))
    throw std::runtime_error("NITF HEADER EXTRACTION FAILED");
    ...
    for (tres_itr = ixshd_tres.begin(); tres_itr != ixshd_tres.end(); ++tres_itr)
      {
        ...
        if (type == "RSMECA") { // looking for "RSMECA..."
          ...
          nt5.get_append(tres_itr, tre_str, v);
          int npar = 0; nt5.get(tres_itr, npar);

          nitf_tre<int> nt6("NPARO", opt, false);
          nt6.get_append(tres_itr, tre_str, v);
          int nparo = 0; nt6.get(tres_itr, nparo);

          nitf_tre<int> nt7("IGN", opt, false);
          nt7.get_append(tres_itr, tre_str, v);
          int n_indp=0; nt7.get(tres_itr, n_indp);
          ...
          nt9.get_append(tres_itr, tre_str, v);
          nt9.get(tres_itr, s); ASC_double(s, xuol);

          nitf_tre<std::string> nt10("YUOL", opt, false);
          nt10.get_append(tres_itr, tre_str, v);
          nt10.get(tres_itr, s); ASC_double(s, yuol);

          nitf_tre<std::string> nt11("ZUOL", opt, false);
          nt11.get_append(tres_itr, tre_str, v);
          nt11.get(tres_itr, s); ASC_double(s, zuol);
          if(!opt){
          ...
          nt12.get_append(tres_itr, tre_str, v);
          if(!opt){ nt12.get(tres_itr, s); ASC_double(s, m[0][0]);}

          nitf_tre<std::string> nt13("XUYL", opt, false);
          nt13.get_append(tres_itr, tre_str, v);
         if(!opt){ nt13.get(tres_itr, s); ASC_double(s, m[0][1]);}

          nitf_tre<std::string> nt14("XUZL", opt, false);
          nt14.get_append(tres_itr, tre_str, v);
          if(!opt){nt14.get(tres_itr, s); ASC_double(s, m[0][2]);}
          ...
          nt17.get_append(tres_itr, tre_str, v);
          if(!opt){ nt17.get(tres_itr, s); ASC_double(s, m[1][2]);}

          nitf_tre<std::string> nt18("ZUXL", opt, false);
          nt18.get_append(tres_itr, tre_str, v);
          if(!opt){nt18.get(tres_itr, s); ASC_double(s, m[2][0]);}
          ...
          nt20.get_append(tres_itr, tre_str, v);
          if(!opt){nt20.get(tres_itr, s); ASC_int(s, idx["IRO"]);}
          ...
          nt23.get_append(tres_itr, tre_str, v);
          if(!opt){nt23.get(tres_itr, s); ASC_int(s, idx["IRZ"]);}

          nitf_tre<std::string> nt24("IRXX", opt, false);
          nt24.get_append(tres_itr, tre_str, v);
          if(!opt){nt24.get(tres_itr, s); ASC_int(s, idx["IRXX"]);}

          nitf_tre<std::string> nt25("IRXY", opt, false);
          nt25.get_append(tres_itr, tre_str, v);
          if(!opt){nt25.get(tres_itr, s); ASC_int(s, idx["IRXY"]);}

          nitf_tre<std::string> nt26("IRXZ", opt, false);
          nt26.get_append(tres_itr, tre_str, v);
          if(!opt){nt26.get(tres_itr, s); ASC_int(s, idx["IRXZ"]);}

          nitf_tre<std::string> nt27("IRYY", opt, false);
          nt27.get_append(tres_itr, tre_str, v);
          if(!opt){nt27.get(tres_itr, s); ASC_int(s, idx["IRYY"]);}

          nitf_tre<std::string> nt28("IRYZ", opt, false);
          nt28.get_append(tres_itr, tre_str, v);
          if(!opt){nt28.get(tres_itr, s); ASC_int(s, idx["IRYZ"]);}
          ...
          nt31.get_append(tres_itr, tre_str, v);
          if(!opt){nt31.get(tres_itr, s); ASC_int(s, idx["ICX"]);}

          nitf_tre<std::string> nt32("ICY", opt, false);
          nt32.get_append(tres_itr, tre_str, v);
          if(!opt){nt32.get(tres_itr, s); ASC_int(s, idx["ICY"]);}

          nitf_tre<std::string> nt33("ICZ", opt, false);
          nt33.get_append(tres_itr, tre_str, v);
          if(!opt){nt33.get(tres_itr, s); ASC_int(s, idx["ICZ"]);}
          ...
          nt36.get_append(tres_itr, tre_str, v);
          if(!opt){nt36.get(tres_itr, s); ASC_int(s, idx["ICXZ"]);}

          nitf_tre<std::string> nt37("ICYY", opt, false);
          nt37.get_append(tres_itr, tre_str, v);
          if(!opt){nt37.get(tres_itr, s); ASC_int(s, idx["ICYY"]);}

          nitf_tre<std::string> nt38("ICYZ", opt, false);
          nt38.get_append(tres_itr, tre_str, v);
          if(!opt){nt38.get(tres_itr, s); ASC_int(s, idx["ICYZ"]);}

          nitf_tre<std::string> nt39("ICZZ", opt, false);
          nt39.get_append(tres_itr, tre_str, v);
          if(!opt){nt39.get(tres_itr, s); ASC_int(s, idx["ICZ"]);}

          nitf_tre<std::string> nt40("GXO", opt, false);
          nt40.get_append(tres_itr, tre_str, v);
          if(!opt){nt40.get(tres_itr, s); ASC_int(s, idx["GXO"]);}
          ...
          nt43.get_append(tres_itr, tre_str, v);
          if(!opt){nt43.get(tres_itr, s); ASC_int(s, idx["GXR"]);}

          nitf_tre<std::string> nt44("GYR", opt, false);
          nt44.get_append(tres_itr, tre_str, v);
          if(!opt){nt44.get(tres_itr, s); ASC_int(s, idx["GYR"]);}

          nitf_tre<std::string> nt45("GZR", opt, false);
          nt45.get_append(tres_itr, tre_str, v);
          if(!opt){nt45.get(tres_itr, s); ASC_int(s, idx["GZR"]);}

          nitf_tre<std::string> nt46("GS", opt, false);
          nt46.get_append(tres_itr, tre_str, v);
          if(!opt){nt46.get(tres_itr, s); ASC_int(s, idx["GS"]);}

          nitf_tre<std::string> nt47("GXX", opt, false);
          nt47.get_append(tres_itr, tre_str, v);
          if(!opt){nt47.get(tres_itr, s); ASC_int(s, idx["GXX"]);}
