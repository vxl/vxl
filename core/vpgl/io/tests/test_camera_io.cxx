...
test_camera_io()
{
  ...
  auto * pcam_r = dynamic_cast<vpgl_proj_camera<double> *>(cam_r);
  if (!pcam_r)
  {
  ...
  {
    auto * local_pcam_r = dynamic_cast<vpgl_proj_camera<double> *>(cam_r_sptr.as_pointer());
    if (!local_pcam_r)
    {
  ...
  auto * percam_r = dynamic_cast<vpgl_perspective_camera<double> *>(cam_r);
  if (!percam_r)
  {
  ...
  random_list[10] = 0.0;
  random_list[11] = 1.0;
  auto * acam = new vpgl_affine_camera<double>(random_matrix);
  ...
  auto * acam_r = dynamic_cast<vpgl_affine_camera<double> *>(cam_r);
  if (!acam_r)
  {
  ...
  auto * rcam = new vpgl_rational_camera<double>(neu_u, den_u, neu_v, den_v, sx, ox, sy, oy, sz, oz, su, ou, sv, ov);
  cam = rcam;
  ...
  auto * rcam_r = dynamic_cast<vpgl_rational_camera<double> *>(cam_r);
  if (!rcam_r)
  {
  ...
  auto * lrcam_r = dynamic_cast<vpgl_local_rational_camera<double> *>(cam_r);
  if (!lrcam_r)
  {
