...
template <class T>
void
vsl_b_write(vsl_b_ostream & os, const vpgl_proj_camera<T> & camera)
{
  if (!os)
    return;
  unsigned version = 1;
...
template <class T>
void
vsl_b_read(vsl_b_istream & is, vpgl_proj_camera<T> & camera)
{
  if (!is)
    return;
  short ver;
...
template <class T>
void
vsl_print_summary(std::ostream & os, const vpgl_proj_camera<T> & c)
{
  os << c << '\n';
}
