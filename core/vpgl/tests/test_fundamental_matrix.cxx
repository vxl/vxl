...
static vpgl_fundamental_matrix<double>
actual_f_matrix()
{
  vnl_double_3x3 m;

  m[0][0] = 0;
  m[0][1] = -9.46971705e-006;
  m[0][2] = 2.31061096e-003;
  m[1][0] = 0;
  m[1][1] = 0;
  m[1][2] = 1.66666667e-002;
  m[2][0] = 0;
  m[2][1] = -6.81088136e-003;
  m[2][2] = -2.33814495e+000;
...
test_fundamental_matrix()
{
  ...
  const vpgl_proj_camera<double> C1r(random_list1r);
  const vpgl_proj_camera<double> C1l(random_list1l);
  const vpgl_fundamental_matrix<double> F1(C1r, C1l);

  const vgl_homg_point_3d<double> p1w(.4, -0.1, 10);
  const vgl_homg_point_2d<double> p1r = C1r.project(p1w);
  const vgl_homg_point_2d<double> p1l = C1l.project(p1w);

  const vnl_vector_fixed<double, 3> p1r_vnl(p1r.x(), p1r.y(), p1r.w());
  vnl_vector_fixed<double, 3> l1r_vnl = F1.get_matrix() * p1r_vnl;
  const vgl_homg_line_2d<double> l1r(l1r_vnl[0], l1r_vnl[1], l1r_vnl[2]);
  ...
  const vnl_matrix_fixed<double, 3, 3> F2_vnl(random_list2);
  const vpgl_fundamental_matrix<double> F2(F2_vnl);
  ...
  const vnl_vector_fixed<double, 3> e1r_vnl(e1r.x(), e1r.y(), e1r.w());
  const vnl_vector_fixed<double, 3> e1l_vnl(e1l.x(), e1l.y(), e1l.w());
  ...
  const vgl_homg_point_3d<double> p2w(-4, 1, 0);
  const vgl_homg_point_2d<double> p2r = C1r.project(p2w);
  const vgl_homg_point_2d<double> p2l = C1l.project(p2w);
  TEST_NEAR("right epipolar line finder", vgl_distance(F1.r_epipolar_line(p2l), p2r), 0, 1e-06);
  ...
  const vpgl_proj_camera<double> C2l(random_list2l);
  const vpgl_proj_camera<double> C2r;
  const vpgl_fundamental_matrix<double> F3(C2r, C2l);
  const vpgl_proj_camera<double> C2l_est = F3.extract_left_camera(vnl_vector_fixed<double, 3>(0, 0, 0), 1);
  const vpgl_fundamental_matrix<double> F3_est(C2r, C2l_est);
  std::cerr << "\nTrue fundamental matrix: " << F3.get_matrix() << '\n'
            << "Estimated fundamental matrix: " << F3_est.get_matrix() << '\n';
  TEST_NEAR("left camera extraction",
            F3.get_matrix()(0, 0) / F3_est.get_matrix()(0, 0) - F3.get_matrix()(0, 1) / F3_est.get_matrix()(0, 1),
            0,
            1e-06);
  ...
  for (const auto & i : p3w)
    p3i.emplace_back(C2l.project(vgl_homg_point_3d<double>(i)));
  const vpgl_proj_camera<double> C2l_est2 = F3.extract_left_camera(p3w, p3i);
  ...
  K1_[2][1] = 0;
  K1_[2][2] = 1;
  const vpgl_calibration_matrix<double> K1(K1_);
  ...
  K2_[2][1] = 0;
  K2_[2][2] = 1;
  const vpgl_calibration_matrix<double> K2(K2_);
  ...
  const vpgl_essential_matrix<double> E(skew_symmetric(t[0], t[1], t[2]) * R);

  const vpgl_fundamental_matrix<double> F4(K1, K2, E);
  const vpgl_fundamental_matrix<double> Fi = actual_f_matrix();

  const vnl_double_3x3 error = Fi.get_matrix() - F4.get_matrix();
  TEST_NEAR("Construct fundamental matrix from essential matrix", error.frobenius_norm(), 0, 1);
}
