...
test_local_rational_camera()
{
  ...
  const vpgl_rational_camera<double> rcam = construct_rational_camera();
  const double xoff = rcam.offset(vpgl_rational_camera<double>::X_INDX);
  const double yoff = rcam.offset(vpgl_rational_camera<double>::Y_INDX);
  const double zoff = rcam.offset(vpgl_rational_camera<double>::Z_INDX);

  const vpgl_lvcs lvcs(yoff, xoff, zoff);
  ...
  std::cout << "Global vnl " << image_point_vnl_g << std::endl << "Local vnl  " << image_point_vnl_l << std::endl;
  ...
  std::cout << "Global " << image_point_vgl_g << std::endl << "Local  " << image_point_vgl_l << std::endl;
