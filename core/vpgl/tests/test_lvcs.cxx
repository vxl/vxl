...
static void test_lvcs()
{
  ...
  TEST_NEAR("wgs84 to utm, pt1 x", x1, 237346.15, 0.1);
  TEST_NEAR("wgs84 to utm, pt1 y", y1, 4199542.5, 0.1);
  
  TEST_NEAR("wgs84 to utm (noaa), pt1 x", x1, 237346.006, 0.1);
  TEST_NEAR("wgs84 to utm (noaa), pt1 y", y1, 4199541.877, 0.1);
  

  u.transform(lat2, lon2, x2,y2,zone2);
  TEST("wgs84 to utm, pt2 zone", zone2, 10);
  TEST_NEAR("wgs84 to utm, pt2 x", x2, 763653.66, 0.1);
  TEST_NEAR("wgs84 to utm, pt2 y", y2, 4199428.45, 0.1);

  u.transform(lat3, lon3, x3,y3,zone3);
  TEST("wgs84 to utm, pt3 zone", zone3, 11);
  TEST_NEAR("wgs84 to utm, pt3 x", x3, 495039.78, 0.1);
  TEST_NEAR("wgs84 to utm, pt3 y", y3, 4314876.95, 0.1);

  TEST_NEAR("wgs84 to utm (noaa), pt3 x", x3, 495039.78, 0.1);
  TEST_NEAR("wgs84 to utm (noaa), pt3 y", y3, 4314876.95, 0.1);
