                ...
                double tolerance)
{
  ...
  vgl_point_3d<double> cc1 = c1.get_camera_center(), cc2 = c2.get_camera_center();
  ...
  for (unsigned r = 0; r < 3; ++r)
    ...
    {
      if (std::fabs(Km1[r][c] - Km2[r][c]) > tolerance)
        return false;
      if (std::fabs(Rm1[r][c] - Rm2[r][c]) > tolerance)
        return false;
    }
  return std::fabs(cc1.x() - cc2.x()) <= tolerance && std::fabs(cc1.y() - cc2.y()) <= tolerance &&
...
test_perspective_camera()
{
  ...
  const vgl_rotation_3d<double> R; // the identity
  const vgl_homg_point_3d<double> center(0, 0, -10.0);
  const vpgl_perspective_camera<double> P(K, center, R);
  std::cout << "Camera " << P;

  const vgl_homg_point_3d<double> hp_3d(0, 0, 0);
  ...
  const vgl_homg_point_3d<double> hpa_3d(10, 20, 10);
  hpa_2d = P(hpa_3d);
  ...
  const vpgl_perspective_camera<double> P_rot = vpgl_perspective_camera<double>::postmultiply(P, tr);
  std::cout << "P_rot\n" << P_rot << '\n';
  ...
  // test look_at
  {
    ...
    const vgl_homg_point_3d<double> center_pt2(8.12, 2.81, 10.0);
    vpgl_perspective_camera<double> P(K, center_pt2, R);
    P.look_at(target);
    const bool infront = !P.is_behind_camera(target);
    vgl_point_2d<double> tgt(P(target));
    vgl_point_2d<double> abv(P(above));
    std::cout << "target projects to " << tgt << std::endl << "this point should be above " << abv << std::endl;
  ...
  const vgl_homg_point_3d<double> X(10., 0, 0);
  ...
  const vpgl_perspective_camera<double> P_rott = vpgl_perspective_camera<double>::postmultiply(P, tr);
  ...
  // Test finite backprojection.
  {
    ...
    for (int i = 0; i < 4; ++i)
    {
      ...
      if (i == 0)
        ...
        p.set(0, -1, -1);
      const vgl_homg_point_2d<double> ip = P0.project(vgl_homg_point_3d<double>(p));
      const vgl_line_3d_2_points<double> l = P0.backproject(vgl_point_2d<double>(ip.x() / ip.w(), ip.y() / ip.w()));
  ...
  const vgl_point_3d<double> c(-25.3302, -31.0114, 1030.63);
  k[0][0] = 2200;
  k[0][1] = 0;
  k[0][2] = 640;
  k[1][0] = 0;
  ...
  const vgl_rotation_3d<double> rot(Rm);
  const vpgl_calibration_matrix<double> Ko(k);
  vpgl_perspective_camera<double> Po;
  Po.set_camera_center(c);
  Po.set_rotation(rot);
  Po.set_calibration(Ko);
  const std::string cam_path = "cam_path";
  ...
  tv = -(Rm * cv);
  const vgl_vector_3d<double> t(tv[0], tv[1], tv[2]);
  const vpgl_perspective_camera<double> Ptc(Ko, rot, t);
  ...
  const vgl_vector_3d<double> tg = Po.get_translation();
  Pin.set_translation(tg);
  eql = cams_near_equal(Po, Pin, 0.01);
  TEST("get translation ", eql, true);
  // test the frustum
  const vgl_frustum_3d<double> fr = frustum(P, 10.0, 20.0);
  std::cout << P << fr;
  vgl_point_3d<double> cnt = fr.centroid();
  std::cout << cnt;
  const double dif = std::fabs(cnt.z() - 5.0) + std::fabs(cnt.x()) + std::fabs(cnt.y());
  ...
  str >> Pact;
  const vgl_frustum_3d<double> fr_act = frustum(Pact, 0.5, 60.0);
  const vgl_point_3d<double> pt_in(20.7763, 0.42938, 0.8249);
  const vgl_point_3d<double> pt_out(-20.0135, -1.3294, 0.7445);
  std::cout << "in " << pt_in << " out " << pt_out << '\n' << Pact << '\n' << fr_act << '\n';
  const bool cont = fr_act.contains(pt_in);
  const bool not_cont = !fr_act.contains(pt_out);
