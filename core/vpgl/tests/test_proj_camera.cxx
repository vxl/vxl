...
test_proj_camera()
{
  ...
  const vnl_matrix_fixed<double, 3, 4> random_matrix(random_list);
  ...
  const vpgl_proj_camera<double> P3(random_matrix2);
  TEST("vnl_fixed_matrix constructor", P3.get_matrix(), random_matrix2);

  // Copy constructor and assignment.
  const vpgl_proj_camera<double> P4(P2);
  ...
  // Point projection.
  const vgl_homg_point_3d<double> x1(1, 4, 5);
  const vgl_homg_point_2d<double> y1 = P1.project(x1);
  ...
  // Line projection.
  const vgl_homg_point_3d<double> x2(4, 3, -10);
  const vgl_line_segment_3d<double> l1w(x1, x2);
  const vgl_line_segment_2d<double> l1i = P1.project(l1w);
  const vgl_line_segment_2d<double> l1ib(P1.project(x1), P1.project(x2));
  TEST_NEAR("line projection", l1i.b() * l1ib.c(), l1ib.b() * l1i.c(), 1e-06);

  // Point backprojection.
  const vgl_homg_point_2d<double> y3(100, 12);
  const vgl_homg_line_3d_2_points<double> l3 = P1.backproject(y3);
  const vgl_homg_point_2d<double> y3b = P1.project(l3.point_finite());
  TEST_NEAR("point backprojection", y3.x() * y3b.w(), y3b.x() * y3.w(), 1e-06);
  const vgl_ray_3d<double> r = P1.backproject_ray(y3);
  const vgl_point_3d<double> c(P1.camera_center());
  const bool org = c == r.origin();
  const vgl_vector_3d<double> dirr = r.direction();
  const double dp = dot_product(l3.direction(), dirr);
  ...
  const vnl_matrix_fixed<double, 3, 4> act_cam_matrix(actual_cam_list);
  const vpgl_proj_camera<double> Pact(act_cam_matrix);
  const vgl_homg_point_3d<double> xc = Pact.camera_center();
  double u = 0, v = 0;
  Pact.project(0.0, 0.0, 0.0, u, v);
  const vgl_homg_point_2d<double> img_pt(u, v, 1.0);
  const vgl_homg_line_3d_2_points<double> line = Pact.backproject(img_pt);
  const vgl_homg_point_3d<double> inf_pt = line.point_infinite();
  vgl_vector_3d<double> dir(inf_pt.x(), inf_pt.y(), inf_pt.z());
  normalize(dir);
  vgl_vector_3d<double> act_dir(-xc.x(), -xc.y(), -xc.z());
  normalize(act_dir);
  const double er = (dir - act_dir).length();
  TEST_NEAR("Ray direction actual", er, 0.0, 1.0e-6);
  // Plane backprojection.
  P2.set_matrix(random_matrix);
  const vgl_homg_line_2d<double> l4(1, -2, 3);
  const vgl_homg_plane_3d<double> plane4 = P2.backproject(l4);
  const vgl_homg_point_3d<double> q1(1, 0, 0, -plane4.a() / plane4.d());
  const vgl_homg_point_3d<double> q2(0, 1, 0, -plane4.b() / plane4.d());
  const vgl_homg_point_3d<double> q3(0, 0, 1, -plane4.c() / plane4.d());
  vgl_homg_point_2d<double> q1i = P2.project(q1);
  vgl_homg_point_2d<double> q2i = P2.project(q2);
  const vgl_homg_point_2d<double> q3i = P2.project(q3);
  TEST_NEAR("plane backprojection1", vgl_distance(l4, q1i) * vgl_distance(l4, q2i) * vgl_distance(l4, q3i), 0, 1e-06);

  const vgl_homg_line_2d<double> l5(-10, 13, 40);
  l5.get_two_points(q1i, q2i);
  const vgl_homg_plane_3d<double> plane5a = P2.backproject(l5);
  const vgl_homg_plane_3d<double> plane5b(
  ...
  const vpgl_proj_camera<double> P6(random_matrix);
  const vgl_h_matrix_3d<double> H = get_canonical_h(P6);
  ...
  // Test camera center
  const vgl_homg_point_2d<double> q6 = P6.project(P6.camera_center());
  TEST_NEAR("camera center computation", q6.x() * q6.y() * q6.w(), 0, 1e-06);

  // Test pre-multiply
  double T1array[9] = { 1, 2, 3, 4, 5, 6, 7, 8, 9 };
  const vnl_matrix_fixed<double, 3, 3> T1(T1array);
  ...
  const vnl_matrix_fixed<double, 4, 4> T2(T2array);
  P1.set_matrix(random_matrix2);
  ...
  for (unsigned int i = 0; i < pts.size(); ++i)
  {
    ...
    const vgl_point_3d<double> pdx(p.x() + eps, p.y(), p.z());
    const vgl_point_3d<double> pdy(p.x(), p.y() + eps, p.z());
    const vgl_point_3d<double> pdz(p.x(), p.y(), p.z() + eps);
    ...
    const double err = (J_diff - Jac[i]).array_inf_norm();
    if (err > eps)
    {
      std::cerr << "Jacobian\n" << J_diff << "\nshould be\n" << Jac[i] << std::endl;
      valid = false;
