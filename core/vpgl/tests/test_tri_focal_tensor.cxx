...
static void
test_tri_focal_tensor()
{
  const vpgl_tri_focal_tensor<double> T0;

  vgl_homg_point_3d<double> X(0.5, 0.5, 2.0, 1.0), X0(1.0, 1.0, 2.0, 1.0);

  // canonical camera
  const vpgl_proj_camera<double> c1;
  vgl_homg_point_2d<double> x0(0, 0, 1), x1 = c1.project(X), x10 = c1.project(X0);
  const vgl_homg_line_2d<double> l1(x1, x10);
  ...
  const vgl_homg_line_2d<double> tl1 = T.image1_transfer(l2, l3);
  const vgl_homg_line_2d<double> tl2 = T.image2_transfer(l1, l3);
  const vgl_homg_line_2d<double> tl3 = T.image3_transfer(l1, l2);
  ...
  const bool good_epipoles = T.compute_epipoles();
  const bool good_f_matrices = T.compute_f_matrices();
  const bool good_f23 = T.compute_f_matrix_23();
  const bool good_p_cams = T.compute_proj_cameras();
  ...
  const double er_e12 = fabs(e12.x() / e12.x() - 1.0) + fabs(e12.y()) + fabs(e12.w());
  const double er_e13 = fabs(e13.y() / e13.y() - 1.0) + fabs(e13.x()) + fabs(e12.w());
  TEST_NEAR("Epipoles", er_e12 + er_e13, 0.0, 0.0001);
  const vpgl_fundamental_matrix<double> f12 = T.fmatrix_12();
  const vpgl_fundamental_matrix<double> f13 = T.fmatrix_13();
  const vpgl_fundamental_matrix<double> f23 = T.fmatrix_23();
  std::cout << "F12\n" << f12 << std::endl;
  std::cout << "F13\n" << f13 << std::endl;
  std::cout << "F23\n" << f23 << std::endl;
  const vgl_homg_line_2d<double> epl2 = f12.r_epipolar_line(x1);
  const double alg_dist_2 = x2.x() * epl2.a() + x2.y() * epl2.b() + x2.w() * epl2.c();
  TEST_NEAR("Fundamental Matrix F12", fabs(alg_dist_2), 0.0, 0.0001);
  const vgl_homg_line_2d<double> epl3 = f13.r_epipolar_line(x1);
  const double alg_dist_3 = x3.x() * epl3.a() + x3.y() * epl3.b() + x3.w() * epl3.c();
  TEST_NEAR("Fundamental Matrix F13", fabs(alg_dist_3), 0.0, 0.0001);
  const vgl_homg_line_2d<double> epl23 = f23.r_epipolar_line(x2);
  const double alg_dist_23 = x3.x() * epl23.a() + x3.y() * epl23.b() + x3.w() * epl23.c();
  TEST_NEAR("Fundamental Matrix F23", fabs(alg_dist_23), 0.0, 0.0001);

  const vgl_h_matrix_2d<double> H13 = T.hmatrix_13(epl3);
  const vgl_h_matrix_2d<double> H12 = T.hmatrix_12(epl2);
  std::cout << "H13\n" << H13 << std::endl;
  std::cout << "H12\n" << H12 << std::endl;
  ...
  const vnl_matrix_fixed<double, 3, 3> zero_3x3 = T.point_constraint_3x3(x1, x2, x3);
  std::cout << "zero 3x3\n" << zero_3x3 << std::endl;
  const double fbz = zero_3x3.frobenius_norm();
  TEST_NEAR("perfect tri point match", fbz, 0.0, 0.000001);

  // scalar three point constraint
  const double scalar_zero = T.point_constraint(x1, x2, x3);
  ...
  const double d = 1.5;
  const vgl_homg_point_2d<double> x2p((x2.x() / x2.w()) + d, (x2.y() / x2.w()) + d);
  const vnl_matrix_fixed<double, 3, 3> zero_3x3_p = T.point_constraint_3x3(x1, x2p, x3);
  std::cout << "zero 3x3 perturbed\n" << zero_3x3_p << std::endl;
  const double fbp = zero_3x3_p.frobenius_norm();
  TEST("perturbed tri point match", fbp > 0.0, true);
  // scalar point constraint
  const double scalar_zero_p = T.point_constraint(x1, x2p, x3);
  ...
  const vgl_h_matrix_2d<double> HL12 = T.hmatrix_12(line_3);
  const vgl_homg_point_2d<double> p2_HL12 = HL12 * x1;
  vgl_point_2d<double> p2(p2_HL12), p2_gt(x2);
  std::cout << "p2 " << p2 << " vs. p2_gt " << p2_gt << std::endl;
  const double er_p2 = (p2 - p2_gt).length();
  ...
  const vgl_h_matrix_2d<double> HL13 = T.hmatrix_13(line_2);
  const vgl_homg_point_2d<double> p3_HL13 = HL13 * x1;
  vgl_point_2d<double> p3(p3_HL13), p3_gt(x3);
  std::cout << "p3 " << p3 << " vs. p3_gt " << p3_gt << std::endl;
  const double er_p3 = (p3 - p3_gt).length();
  ...
  const vnl_vector_fixed<double, 3> lc_3 = T.line_constraint_3(lc1, lc2, lc3);
  std::cout << "line constraint 3 " << lc_3 << std::endl;
  const double er_lc = lc_3.magnitude();
