...
public:
  ...
  // \a p projects to (\a u0, \a v0), \a su and \a sv are calibration scale factors
  vpgl_affine_camera(vnl_vector_fixed<T, 3> ray,
  ...
  bool
  set_matrix(const vnl_matrix_fixed<T, 3, 4> & new_camera_matrix) override;
  bool
  set_matrix(const T * new_camera_matrix) override; // i.e., T new_camera_matrix[12]
  ...
  void
  set_rows(const vnl_vector_fixed<T, 4> & row1, const vnl_vector_fixed<T, 4> & row2);
  ...
  void
  orient_ray_direction(const vgl_vector_3d<T> & look_dir);

  //: Equality test
  inline bool
  operator==(const vpgl_affine_camera<T> & that) const
  {
    return this == &that ||
           (this->get_matrix() == that.get_matrix() && this->viewing_distance() == that.viewing_distance());
  }
  ...
  vgl_ray_3d<T>
  backproject_ray(const vgl_homg_point_2d<T> & image_point) const override;

  //: Find the world plane perpendicular to the camera rays at viewing distance from the origin
  vgl_homg_plane_3d<T>
  principal_plane() const override;
...
template <class T>
vpgl_affine_camera<T>
premultiply_a(const vpgl_affine_camera<T> & in_camera, const vgl_h_matrix_2d<T> & transform)
{
...
template <class T>
vpgl_affine_camera<T>
postmultiply_a(const vpgl_affine_camera<T> & in_camera, const vgl_h_matrix_3d<T> & transform)
{
...
template <class T>
vpgl_affine_camera<T>
postmultiply_a(const vpgl_affine_camera<T> & in_camera, const vgl_vector_3d<T> & translation)
{
