...
public:
  ...
  vpgl_calibration_matrix(T focal_length,
  ...
  void
  set_focal_length(T new_focal_length);
  void
  set_principal_point(const vgl_point_2d<T> & new_principal_point);
  void
  set_x_scale(T new_x_scale);
  void
  set_y_scale(T new_y_scale);
  void
  set_skew(T new_skew);
  ...
  //: Equality tests
  bool
  operator==(const vpgl_calibration_matrix<T> & that) const;
  bool
  operator!=(const vpgl_calibration_matrix<T> & that) const
  {
    return !(*this == that);
  ...
  vgl_point_2d<T>
  map_to_focal_plane(const vgl_point_2d<T> & p_image) const;

  vgl_point_2d<T>
  map_to_image(const vgl_point_2d<T> & p_focal_plane) const;
