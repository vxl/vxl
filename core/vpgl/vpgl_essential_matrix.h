...
public:
  ...
  vpgl_essential_matrix( const vpgl_fundamental_matrix<T>& F, const vpgl_calibration_matrix<T>& K_left,
...
template <class T>
bool extract_left_camera(const vpgl_essential_matrix<T>& E, const vgl_point_2d<T>& left_corr, // focal plane coordinates
                         const vgl_point_2d<T>& right_corr,                                   // focal plane coordinates
                         vpgl_perspective_camera<T>& p_left, const T translation_mag = (T)1);
