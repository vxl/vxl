                                                ...
                                                const vpgl_perspective_camera<T> & cl)
{
  ...
  crc.set_calibration(K);
  clc.set_calibration(K);
                    ...
                    const T translation_mag)
{
  ...
  for (int c = 0; c < 4; c++)
  {
    ...
    vpgl_proj_camera<T> ppl = static_cast<vpgl_proj_camera<T>>(p_left);
    vpgl_proj_camera<T> ppr = static_cast<vpgl_proj_camera<T>>(p_right);
