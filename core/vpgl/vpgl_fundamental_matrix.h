...
public:
  ...
  void
  get_epipoles(vgl_homg_point_2d<T> & er, vgl_homg_point_2d<T> & el) const;

  //: Given a point in one image, find the corresponding epipolar line in the other image.
  vgl_homg_line_2d<T>
  r_epipolar_line(const vgl_homg_point_2d<T> & pl) const;
  vgl_homg_line_2d<T>
  l_epipolar_line(const vgl_homg_point_2d<T> & pr) const;

  //: Given an epipolar line in one image, find the corresponding epipolar line in the other image.
  // H&Z 2nd ed p. 247
  vgl_homg_line_2d<T>
  r_epipolar_line(const vgl_homg_line_2d<T> & epiline_l) const;
  vgl_homg_line_2d<T>
  l_epipolar_line(const vgl_homg_line_2d<T> & epiline_r) const;
  ...
  vpgl_proj_camera<T>
  extract_left_camera(const std::vector<vgl_point_3d<T>> & world_points,
  ...
  void
  set_matrix(const vpgl_proj_camera<T> & cr, const vpgl_proj_camera<T> & cl);

  virtual void
  set_matrix(const vnl_matrix_fixed<T, 3, 3> & F);
