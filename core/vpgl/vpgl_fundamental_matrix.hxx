...
template <class T>
void
vpgl_fundamental_matrix<T>::get_epipoles(vgl_homg_point_2d<T> & er, vgl_homg_point_2d<T> & el) const
{
...
template <class T>
vgl_homg_line_2d<T>
vpgl_fundamental_matrix<T>::r_epipolar_line(const vgl_homg_point_2d<T> & pl) const
{
  vnl_vector_fixed<T, 3> lr = F_.transpose() * vnl_vector_fixed<T, 3>(pl.x(), pl.y(), pl.w());
...
template <class T>
vgl_homg_line_2d<T>
vpgl_fundamental_matrix<T>::l_epipolar_line(const vgl_homg_point_2d<T> & pr) const
{
  vnl_vector_fixed<T, 3> ll = F_ * vnl_vector_fixed<T, 3>(pr.x(), pr.y(), pr.w());
...
template <class T>
vgl_homg_line_2d<T>
vpgl_fundamental_matrix<T>::r_epipolar_line(const vgl_homg_line_2d<T> & epiline_l) const
{
  ...
  vnl_vector_fixed<T, 3> epiline_r =
    ...
    vgl_homg_operators_2d<T>::get_vector(vgl_homg_operators_2d<T>::intersection(el_as_line, epiline_l));
...
template <class T>
vgl_homg_line_2d<T>
vpgl_fundamental_matrix<T>::l_epipolar_line(const vgl_homg_line_2d<T> & epiline_r) const
{
  vgl_homg_point_2d<T> er, el;
  ...
  vnl_vector_fixed<T, 3> epiline_l =
    get_matrix() * vgl_homg_operators_2d<T>::get_vector(vgl_homg_operators_2d<T>::intersection(er_as_line, epiline_r));
...
template <class T>
vpgl_proj_camera<T>
vpgl_fundamental_matrix<T>::extract_left_camera(const vnl_vector_fixed<T, 3> & v, T lambda) const
{
  vgl_homg_point_2d<T> er, el;
  get_epipoles(er, el);
...
template <class T>
vpgl_proj_camera<T>
vpgl_fundamental_matrix<T>::extract_left_camera(const std::vector<vgl_point_3d<T>> & world_points,
                                                const std::vector<vgl_point_2d<T>> & image_points) const
{
  ...
  {
    vnl_vector_fixed<T, 3> wp_vnl(world_points[p].x(), world_points[p].y(), world_points[p].z());
    vnl_vector_fixed<T, 3> ip_vnl(image_points[p].x(), image_points[p].y(), (T)1);
    ...
    for (unsigned i = 0; i < 3; i++)
    {
      ...
      if (i == 0)
        ...
        ei = el.w();
      A(3 * p + i, 0) = ei * wp_vnl[0];
      A(3 * p + i, 1) = ei * wp_vnl[1];
      A(3 * p + i, 2) = ei * wp_vnl[2];
      A(3 * p + i, 3) = ei;
    }
  }
...
template <class T>
void
vpgl_fundamental_matrix<T>::set_matrix(const vpgl_proj_camera<T> & cr, const vpgl_proj_camera<T> & cl)
{
...
template <class T>
void
vpgl_fundamental_matrix<T>::set_matrix(const vnl_matrix_fixed<T, 3, 3> & F)
{
...
template <class T>
std::ostream &
operator<<(std::ostream & s, const vpgl_fundamental_matrix<T> & p)
{
  s << p.get_matrix();
  return s;
}
...
template <class T>
std::istream &
operator>>(std::istream & s, vpgl_fundamental_matrix<T> & p)
{
