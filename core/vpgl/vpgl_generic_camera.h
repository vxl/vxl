...
public:
  ...
  vpgl_generic_camera( vcl_vector<vbl_array_2d<vgl_ray_3d<T> > > const& rays, vcl_vector<int> nrs,
                       vcl_vector<int> ncs  );
...
protected:
  ...
  void nearest_ray(int level, vgl_point_3d<T> const& p, int start_r, int end_r, int start_c, int end_c, int& nearest_r,
                   int& nearest_c) const;
