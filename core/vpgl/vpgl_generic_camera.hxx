...
//------------------------------------------
template <class T>
vpgl_generic_camera<T>::vpgl_generic_camera(const vbl_array_2d<vgl_ray_3d<T>> & rays)
{
  ...
  if (dim * std::pow(0.5, static_cast<double>(n_levels_ - 1)) < 32.0)
    n_levels_--;
  if (n_levels_ <= 0)
    n_levels_ = 1;
...
//------------------------------------------
template <class T>
vpgl_generic_camera<T>::vpgl_generic_camera(const std::vector<vbl_array_2d<vgl_ray_3d<T>>> & rays,
                                            std::vector<int> nrs,
                                            std::vector<int> ncs)
{
...
template <class T>
void
vpgl_generic_camera<T>::nearest_ray(int level,
...
template <class T>
void
vpgl_generic_camera<T>::nearest_ray_to_point(const vgl_point_3d<T> & p, int & nearest_r, int & nearest_c) const
{
  int lev = n_levels_ - 1;
  ...
  for (; lev >= 0; --lev)
  {
    ...
    if (end_c >= nc_[lev])
      end_c = nc_[lev] - 1;
    nearest_ray(lev, p, start_r, end_r, start_c, end_c, nearest_r, nearest_c);
    // compute new bounds
    start_r = 2 * nearest_r - 1;
    start_c = 2 * nearest_c - 1;
    end_r = start_r + 2;
    end_c = start_c + 2;
    // check if the image sizes are odd, so search range is extended
    if ((lev > 0) && (nr_[lev - 1] % 2 != 0))
      end_r++;
    if ((lev > 0) && (nc_[lev - 1] % 2 != 0))
      end_c++;
  }
}
...
template <class T>
void
vpgl_generic_camera<T>::refine_ray_at_point(int nearest_c,
                                            int nearest_r,
                                            const vgl_point_3d<T> & p,
...
template <class T>
vgl_ray_3d<T>
vpgl_generic_camera<T>::ray(const vgl_point_3d<T> & p) const
{
...
template <class T>
void
vpgl_generic_camera<T>::refine_projection(int nearest_c, int nearest_r, const vgl_point_3d<T> & p, T & u, T & v) const
{
...
template <class T>
void
vpgl_generic_camera<T>::project(const T x, const T y, const T z, T & u, T & v) const
{
...
template <class T>
vgl_ray_3d<T>
vpgl_generic_camera<T>::ray(const T u, const T v) const
{
  ...
  vgl_point_3d<T> avg_org(static_cast<T>(ox), static_cast<T>(oy), static_cast<T>(oz));
  vgl_vector_3d<T> avg_dir(static_cast<T>(dx), static_cast<T>(dy), static_cast<T>(dz));
...
template <class T>
void
vpgl_generic_camera<T>::print_orig(int level)
{
...
template <class T>
void
vpgl_generic_camera<T>::print_to_vrml(int level, std::ostream & os)
{
  for (int r = 0; r < nr_[level]; ++r)
  {
    for (int c = 0; c < nc_[level]; ++c)
...
#define vpgl_GENERIC_CAMERA_INSTANTIATE(T) template class vpgl_generic_camera<T>
