...
public:
  ...
  vpgl_lvcs(double orig_lat, double orig_lon, double orig_elev, // !< simplified interface
            cs_names cs_name, AngUnits  ang_unit = DEG, LenUnits len_unit = METERS);
  ...
  vpgl_lvcs(double lat_low, double lon_low,   // !< lower corner bounding geo_rectangle
            ...
            cs_names cs_name = wgs84, AngUnits ang_unit = DEG, LenUnits elev_unit = METERS);

  vpgl_lvcs(const vpgl_lvcs &);
  vpgl_lvcs & operator=(const vpgl_lvcs &);

  // Utility Methods-----------------------------------------------------------
  void local_to_global(const double lx, const double ly, const double lz, cs_names cs_name,        // this is output global cs
                       double& lon, double& lat, double& gz, AngUnits output_ang_unit = DEG,
                       LenUnits output_len_unit = METERS);

  void global_to_local(const double lon, const double lat, const double gz, cs_names cs_name,        // this is input global cs
                       double& lx, double& ly, double& lz, AngUnits output_ang_unit = DEG,
                       LenUnits output_len_unit = METERS);
