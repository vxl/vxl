...
//                                  but it is converted to utm origin during local to global transformations and vice
//                                  versa all the global coords are still either in wgs84, nad27n or wgs72 if a utm zone
//                                  is crossed
//   Yi Dong  July 31, 2013: fix the local_to_global method to correctly transfer the local cooridinates to global wgs84
//   coords
//                           for points located in southern hemisphere.  Note that the input point needs to be at the
//                           same hemisphere as the lvcs origin
//
// \endverbatim
/////////////////////////////////////////////////////////////////////////////
#include <iostream>
...
public:
  ...
  static vpgl_lvcs::cs_names
  str_to_enum(const char *);
  ...
  vpgl_lvcs(double lat_low,
            double lon_low, //!< lower corner bounding geo_rectangle
            double lat_high,
            double lon_high, //!< upper corner bounding geo_rectangle
  ...
  ~vpgl_lvcs() override;
  vpgl_lvcs &
  operator=(const vpgl_lvcs &);

  //: Create a copy on the heap via new and return pointer
  vpgl_lvcs *
  clone() const
  {
    return new vpgl_lvcs(*this);
  }

  // Utility Methods-----------------------------------------------------------
  void
  local_to_global(const double lx,
                  const double ly,
                  const double lz,
  ...
  void
  global_to_local(const double lon,
                  const double lat,
                  const double gz,
  ...
  void
  radians_to_degrees(double & lon, double & lat, double & z) const;
  double
  radians_to_degrees(const double val) const;
  void
  degrees_to_dms(double, int & degrees, int & minutes, double & seconds) const;
  void
  radians_to_dms(double, int & degrees, int & minutes, double & seconds) const;

  // uses the units defined for *this lvcs, e.g. deg and meters. computes cartesian vector (p1 - p0)
  void
  angle_diff_to_cartesian_vector(const double lon0,
                                 const double lat0,
                                 const double lon1,
                                 const double lat1,
                                 double & cart_dx,
                                 double & cart_dy) const
  {
  ...
  // accessors
  void
  get_origin(double & lat, double & lon, double & elev) const;
  void
  get_scale(double & lat, double & lon) const;
  void
  get_transform(double & lox, double & loy, double & theta) const;
  void
  set_transform(const double lox, const double loy, const double theta);
  void
  set_origin(const double lon, const double lat, const double elev);
  cs_names
  get_cs_name() const;
  inline LenUnits
  local_length_unit() const
  {
    return this->localXYZUnit_;
  }
  inline AngUnits
  geo_angle_unit() const
  {
    return this->geo_angle_unit_;
  }
  ...
  void
  write(std::ostream & strm); // write just "read" would read

  friend std::ostream &
  operator<<(std::ostream & os, const vpgl_lvcs & local_coord_sys);
  friend std::istream &
  operator>>(std::istream & os, vpgl_lvcs & local_coord_sys);
  bool
  operator==(const vpgl_lvcs & r) const;

  void
  get_utm_origin(double & x, double & y, double & elev, int & zone) const;
  void
  get_utm_origin(double & x, double & y, double & elev, int & zone, bool & south_flag) const;

  void
  get_utm(int & zone, bool & south_flag) const;
  void
  set_utm(int zone, bool south_flag);
...
protected:
  ...
  compute_scale();
  void
  local_transform(double & x, double & y) const;
  void
  inverse_local_transform(double & x, double & y) const;
  void
  get_angle_conversions(double & to_radians, double & to_degrees) const;
  void
  get_length_conversions(double & to_meters, double & to_feet) const;
...
inline void
vpgl_lvcs::get_scale(double & lat, double & lon) const
{
  lat = lat_scale_;
  lon = lon_scale_;
}
...
inline vpgl_lvcs::cs_names
vpgl_lvcs::get_cs_name() const
{
...
inline void
vpgl_lvcs::get_origin(double & lat, double & lon, double & elev) const
{
...
inline void
vpgl_lvcs::get_utm_origin(double & x, double & y, double & elev, int & zone) const
{
...
inline void
vpgl_lvcs::get_utm_origin(double & x, double & y, double & elev, int & zone, bool & south_flag) const
{
...
inline void
vpgl_lvcs::get_utm(int & zone, bool & south_flag) const
{
  zone = localUTMOrigin_Zone_;
  south_flag = localUTMOrigin_SouthFlag_;
}
...
inline void
vpgl_lvcs::get_transform(double & lox, double & loy, double & theta) const
{
  lox = lox_;
  loy = loy_;
  theta = theta_;
...
inline void
vpgl_lvcs::set_transform(const double lox, const double loy, const double theta)
{
  lox_ = lox;
  loy_ = loy;
  theta_ = theta;
...
inline void
vpgl_lvcs::set_origin(const double lon, const double lat, const double elev)
{
...
inline void
vpgl_lvcs::radians_to_dms(double rad, int & degrees, int & minutes, double & seconds) const
{
