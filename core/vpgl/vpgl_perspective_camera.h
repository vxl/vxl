...
public:
  ...
  backproject(const vgl_homg_point_2d<T> & image_point) const override;
  //: Finite backprojection.
  vgl_line_3d_2_points<T>
  backproject(const vgl_point_2d<T> & image_point) const;
  //: Finite backprojection.
  vgl_line_3d_2_points<T>
  backproject(T u, T v) const
  {
    return backproject(vgl_point_2d<T>(u, v));
  }

  /* suppress
   * warning: 'vpgl_perspective_camera<double>::backproject_ray' hides overloaded virtual function
   * [-Woverloaded-virtual] by explicitly acknowledging the parent class virtual function of the same name with
   * different parameters. that is not overrriden here
   */
  ...
  vgl_vector_3d<T>
  principal_axis() const;

  //: Determine whether the given point lies in front of the principal plane.
  bool
  is_behind_camera(const vgl_homg_point_3d<T> & world_point) const;

  //: Setters and getters.
  void
  set_calibration(const vpgl_calibration_matrix<T> & K);
  void
  set_camera_center(const vgl_point_3d<T> & camera_center);
  void
  set_translation(const vgl_vector_3d<T> & t);
  void
  set_rotation(const vgl_rotation_3d<T> & R);
  const vpgl_calibration_matrix<T> &
  get_calibration() const
  {
    return K_;
  }
  const vgl_point_3d<T> &
  get_camera_center() const
  {
  ...
  void
  look_at(const vgl_homg_point_3d<T> & point, const vgl_vector_3d<T> & up = vgl_vector_3d<T>(0, 0, 1));

  // Redefined virtual functions -------------------------------------------

  //: Return the known camera center instead of computing it in the base class
  vgl_homg_point_3d<T>
  camera_center() const override
  {
  ...
  postmultiply(const vpgl_perspective_camera<T> & in_cam, const vgl_h_matrix_3d<T> & euclid_trans);
  ...
  //: Equality test
  inline bool
  operator==(const vpgl_perspective_camera<T> & that) const
  {
    return this == &that || (K_ == that.K_ && this->get_matrix() == that.get_matrix() &&
  ...
  void
  save(std::string cam_path) override;
  ...
  virtual vpgl_perspective_camera<T> *
  cast_to_perspective_camera()
  {
    return this;
  }
  virtual const vpgl_perspective_camera<T> *
...
//: Write vpgl_perspective_camera to a vrml file, does not write a vrml header, only the camera, the camera center
//: sphere has radius rad
template <class Type>
void
vrml_write(std::ostream & s, const vpgl_perspective_camera<Type> & p, double rad);
