...
public:
  ...
  vpgl_perspective_camera( const vpgl_calibration_matrix<T>& K, const vgl_point_3d<T>& camera_center,
  ...
  static  vpgl_perspective_camera<T> postmultiply( const vpgl_perspective_camera<T>& in_cam,
                                                   const vgl_h_matrix_3d<T>& euclid_trans);

  // : Apply a 3-d Euclidean transformation (transform external cam parameters)
  static vpgl_perspective_camera<T> postmultiply(const vpgl_perspective_camera<T>& camera,
