...
vpgl_perspective_camera<T>::backproject(const vgl_homg_point_2d<T> & image_point) const
{
  // First find a point that projects to "image_point".
  vnl_vector_fixed<T, 4> vnl_wp =
    this->svd()->solve(vnl_vector_fixed<T, 3>(image_point.x(), image_point.y(), image_point.w()).as_ref());
...
vpgl_perspective_camera<T>::backproject(const vgl_point_2d<T> & image_point) const
{
  // First find a point in front of the camera that projects to "image_point".
  vnl_vector_fixed<T, 4> vnl_wp =
    this->svd()->solve(vnl_vector_fixed<T, 3>(image_point.x(), image_point.y(), 1.0).as_ref());
  ...
  if (!wp_homg.ideal())
    ...
    wp.set(camera_center_.x() + wp_homg.x(), camera_center_.y() + wp_homg.y(), camera_center_.z() + wp_homg.z());
...
template <class T>
vgl_ray_3d<T>
vpgl_perspective_camera<T>::backproject_ray(const vgl_point_2d<T> & image_point) const
{
...
template <class T>
vgl_vector_3d<T>
vpgl_perspective_camera<T>::principal_axis() const
{
  // See H&Z pg 147
...
template <class T>
bool
vpgl_perspective_camera<T>::is_behind_camera(const vgl_homg_point_3d<T> & world_point) const
{
  vgl_homg_plane_3d<T> l = this->principal_plane();
  T dot = world_point.x() * l.a() + world_point.y() * l.b() + world_point.z() * l.c() + world_point.w() * l.d();
  if (world_point.w() < (T)0)
    dot = ((T)(-1)) * dot;
  return dot < 0;
}
...
template <class T>
void
vpgl_perspective_camera<T>::set_calibration(const vpgl_calibration_matrix<T> & K)
{
  K_ = K;
  recompute_matrix();
}
...
template <class T>
void
vpgl_perspective_camera<T>::set_camera_center(const vgl_point_3d<T> & camera_center)
{
  camera_center_ = camera_center;
  recompute_matrix();
}
...
template <class T>
void
vpgl_perspective_camera<T>::set_translation(const vgl_vector_3d<T> & t)
{
...
template <class T>
void
vpgl_perspective_camera<T>::set_rotation(const vgl_rotation_3d<T> & R)
{
  R_ = R;
  recompute_matrix();
}
...
template <class T>
vgl_vector_3d<T>
vpgl_perspective_camera<T>::get_translation() const
{
...
template <class T>
void
vpgl_perspective_camera<T>::look_at(const vgl_homg_point_3d<T> & point, const vgl_vector_3d<T> & up)
{
  ...
  else
  {
    ...
    T r[] = { x.x(), x.y(), x.z(), y.x(), y.y(), y.z(), z.x(), z.y(), z.z() };
...
template <class T>
void
vpgl_perspective_camera<T>::recompute_matrix()
{
...
template <class T>
bool
vpgl_perspective_decomposition(const vnl_matrix_fixed<T, 3, 4> & camera_matrix, vpgl_perspective_camera<T> & p_camera)
{
...
template <class T>
vpgl_perspective_camera<T>
vpgl_align_down(const vpgl_perspective_camera<T> & p0, const vpgl_perspective_camera<T> & p1)
{
  ...
  vgl_point_3d<T> new_camera_center(a1.x() - a0.x(), a1.y() - a0.y(), a1.z() - a0.z());
...
template <class T>
vpgl_perspective_camera<T>
vpgl_align_up(const vpgl_perspective_camera<T> & p0, const vpgl_perspective_camera<T> & p1)
{
  vpgl_perspective_camera<T> new_camera;
  new_camera.set_calibration(p0.get_calibration());
  ...
  vgl_point_3d<T> new_camera_center(
    p0.get_camera_center().x() + a.x(), p0.get_camera_center().y() + a.y(), p0.get_camera_center().z() + a.z());
  new_camera.set_camera_center(new_camera_center);
  return new_camera;
}
...
template <class T>
vpgl_perspective_camera<T>
vpgl_perspective_camera<T>::postmultiply(const vpgl_perspective_camera<T> & camera,
...
template <class Type>
std::ostream &
operator<<(std::ostream & s, const vpgl_perspective_camera<Type> & p)
{
...
template <class Type>
std::istream &
operator>>(std::istream & s, vpgl_perspective_camera<Type> & p)
{
...
template <class Type>
void
vpgl_perspective_camera<Type>::save(std::string cam_path)
{
...
template <class Type>
void
vrml_write(std::ostream & str, const vpgl_perspective_camera<Type> & p, double rad)
{
  vgl_point_3d<Type> cent = p.get_camera_center();
  str << "Transform {\n"
      << "translation " << cent.x() << ' ' << cent.y() << ' ' << ' ' << cent.z() << '\n'
      << "children [\n"
      << "Shape {\n"
...
template <class T>
std::vector<vpgl_perspective_camera<T>>
cameras_from_directory(std::string dir, T)
{
...
template <class T>
double
vpgl_persp_cam_distance(const vpgl_perspective_camera<T> & cam1, const vpgl_perspective_camera<T> & cam2)
{
...
template <class T>
vgl_vector_3d<T>
vpgl_persp_cam_base_line_vector(const vpgl_perspective_camera<T> & cam1, const vpgl_perspective_camera<T> & cam2)
{
...
template <class T>
vgl_rotation_3d<T>
vpgl_persp_cam_relative_orientation(const vpgl_perspective_camera<T> & cam1, const vpgl_perspective_camera<T> & cam2)
{
...
template <class T>
vgl_frustum_3d<T>
frustum(const vpgl_perspective_camera<T> & cam, T d_near, T d_far)
{
