...
vpgl_proj_camera<T>::operator=(const vpgl_proj_camera & cam)
{
  P_ = cam.get_matrix();
...
template <class T>
vpgl_proj_camera<T> *
vpgl_proj_camera<T>::clone() const
{
...
template <class T>
vgl_homg_point_2d<T>
vpgl_proj_camera<T>::project(const vgl_homg_point_3d<T> & world_point) const
{
  ...
  vgl_homg_point_2d<T> image_point(
    P_(0, 0) * world_point.x() + P_(0, 1) * world_point.y() + P_(0, 2) * world_point.z() + P_(0, 3) * world_point.w(),

    P_(1, 0) * world_point.x() + P_(1, 1) * world_point.y() + P_(1, 2) * world_point.z() + P_(1, 3) * world_point.w(),

    P_(2, 0) * world_point.x() + P_(2, 1) * world_point.y() + P_(2, 2) * world_point.z() + P_(2, 3) * world_point.w());
  return image_point;
}
...
vpgl_proj_camera<T>::project(const vgl_line_segment_3d<T> & world_line) const
{
...
template <class T>
vgl_line_2d<T>
vpgl_proj_camera<T>::project(const vgl_infinite_line_3d<T> & world_line) const
{
...
vpgl_proj_camera<T>::backproject(const vgl_homg_point_2d<T> & image_point) const
{
  // First find any point in the world that projects to the "image_point".
  vnl_vector_fixed<T, 4> vnl_wp =
    svd()->solve(vnl_vector_fixed<T, 3>(image_point.x(), image_point.y(), image_point.w()).as_ref());
...
template <class T>
vgl_ray_3d<T>
vpgl_proj_camera<T>::backproject_ray(const vgl_homg_point_2d<T> & image_point) const
{
  vnl_vector_fixed<T, 4> vnl_wp =
    svd()->solve(vnl_vector_fixed<T, 3>(image_point.x(), image_point.y(), image_point.w()).as_ref());
  vgl_homg_point_3d<T> wp(vnl_wp[0], vnl_wp[1], vnl_wp[2], vnl_wp[3]);
...
template <class T>
vgl_homg_plane_3d<T>
vpgl_proj_camera<T>::backproject(const vgl_homg_line_2d<T> & image_line) const
{
  ...
  return vgl_homg_plane_3d<T>(world_plane(0), world_plane(1), world_plane(2), world_plane(3));
}
...
template <class T>
vgl_homg_point_3d<T>
vpgl_proj_camera<T>::camera_center() const
{
...
template <class T>
bool
vpgl_proj_camera<T>::is_canonical(T tol) const
{
  if (tol == T(0))
  {
...
template <class T>
bool
vpgl_proj_camera<T>::set_matrix(const vnl_matrix_fixed<T, 3, 4> & new_camera_matrix)
{
...
template <class T>
bool
vpgl_proj_camera<T>::set_matrix(const T * new_camera_matrix)
{
...
template <class T>
void
vpgl_proj_camera<T>::save(std::string cam_path)
{
...
#define vpgl_PROJ_CAMERA_INSTANTIATE(T)                                                                      \
  ...
  template vgl_point_3d<T> triangulate_3d_point(const vpgl_proj_camera<T> & c1,                              \
                                                ...
                                                const vpgl_proj_camera<T> & c2,                              \
                                                const vgl_point_2d<T> & x2);                                 \
  template std::vector<vnl_matrix_fixed<T, 2, 3>> image_jacobians(const vpgl_proj_camera<T> & camera,        \
