...
public:
  ...
  virtual T
  distort_radius_deriv(T radius) const
  {
    T eps = T(0.001);
  ...
  vgl_point_2d<T>
  distorted_center() const
  {
    return distorted_center_;
  }

  //: Set the center of distortion
  void
  set_center(const vgl_point_2d<T> & c)
  {
    center_ = c;
  }
  //: Set the center of distortion in the distorted image
  void
  set_distorted_center(const vgl_point_2d<T> & dc)
  {
    distorted_center_ = dc;
  }
