...
#endif
template <class T>
vgl_vector_2d<T> vpgl_radial_tangential_distortion<T>::apply_distortion(vgl_vector_2d<T> const& x) const{
                                                 ...
                                                 const vgl_homg_point_2d<T>* init ) const
{
  ...
  for (; i<100 && fabs(del.x())>eps && fabs(del.y())>eps ; ++i){
    ...
    vgl_vector_2d<T> del_p0((dvy.y()*del.x() - dvx.y()*del.y()),
                           dvx.x()*del.y() - dvy.x()*del.x());
#if 0
...
#define vpgl_RADIAL_TANGENTIAL_DISTORTION_INSTANTIATE(T) \
template class vpgl_radial_tangential_distortion<T>
