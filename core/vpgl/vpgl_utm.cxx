...
static double adjust_lon2(double x) // Angle in radians
{
  for( long count = 0; count <= MAX_VAL; ++count )
    ...
    else if( ( (long) vnl_math::abs(x / (vcl_numeric_limits<long>::max() * vnl_math::twopi) ) ) <
             vcl_numeric_limits<long>::max() )
      {
      x -=
        ( ( (long)(x / (vcl_numeric_limits<long>::max() * vnl_math::twopi) ) )
          * (vnl_math::twopi * vcl_numeric_limits<long>::max() ) );
      }
...
static double adjust_lat2(double x) // Angle in radians
{
  for( long count = 0; count <= MAX_VAL; ++count )
    ...
    else if( ( (long) vnl_math::abs(x / (vcl_numeric_limits<long>::max() * vnl_math::pi) ) ) <
             vcl_numeric_limits<long>::max() )
      {
      x -=
        ( ( (long)(x / (vcl_numeric_limits<long>::max() * vnl_math::pi) ) )
          * (vnl_math::pi * vcl_numeric_limits<long>::max() ) );
      }
