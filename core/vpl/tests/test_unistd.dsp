...
USERDEP__HACK=\
	...
	"$(IUEROOT)/vxl/vpl/tests/CMakeLists.txt"
# Begin Custom Build

"test_unistd.dsp" :  "$(SOURCE)" "$(INTDIR)" "$(OUTDIR)"
	"cmake.exe" "$(IUEROOT)/vxl/vpl/tests/CMakeLists.txt" -DSP -H"$(IUEROOT)" -S"$(IUEROOT)/vxl/vpl/tests" -O"$(IUEROOT)/vxl/vpl/tests" -B"$(IUEROOT)"
