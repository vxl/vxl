...
//  problem.  This may eventually be moved to vrel/examples.
...
public:
  ...
  image_point_match(const vnl_vector_fixed<double, 2> & from, const vnl_vector_fixed<double, 2> & to, int id)
    : from_loc_(from)
    , to_loc_(to)
    , point_id_(id)
  {}
  ...
  image_point_match &
  operator=(const image_point_match & old) = default;
};

class similarity_from_matches : public vrel_estimation_problem
{
...
public:
  similarity_from_matches()
    : vrel_estimation_problem(2, 2)
  {}
  similarity_from_matches(std::vector<image_point_match> matches);
  ~similarity_from_matches() override = default;
  unsigned int
  num_unique_samples() const override
  {
    return num_points_to_match_;
  }
  unsigned int
  num_samples() const override;
  bool
  fit_from_minimal_set(const std::vector<int> & match_indices, vnl_vector<double> & params) const override;
  void
...
void
generate_similarity_matches(const vnl_vector<double> & params, double sigma, std::vector<image_point_match> & matches);
