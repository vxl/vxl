...
static void
regression_points(const vnl_vector<double> & a, double sigma, std::vector<vnl_vector<double>> & pts)
{
...
check(const vnl_vector<double> & correct_params, vrel_irls * irls)
{
  ...
  std::cout << "Finished:\n"
            << "  estimate = " << irls->estimate() << ", true fit = " << correct_params << '\n'
...
test_irls()
{
  ...
  testlib_test_perform(irls_m.estimate(match_prob, m_est) && check(params.as_vector(), &irls_m));
