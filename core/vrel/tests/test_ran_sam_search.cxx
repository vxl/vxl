// This is core/vrel/tests/test_ran_sam_search.cxx
#include <iostream>
#ifdef _MSC_VER
...
struct null_problem : public vrel_estimation_problem
{
  null_problem(unsigned int num_samples, unsigned int dof, unsigned int min_samples)
    : vrel_estimation_problem(dof, min_samples)
    , ns(num_samples)
  {}
  unsigned int
...
test_ran_sam_residuals()
{
  ...
  vrel_trunc_quad_obj obj_fcn(2);

  vrel_linear_regression est_prob(pts, /*use_intercept=*/true);
  ...
  if (success)
  {
    const std::vector<int> & idx = search.index();
    assert(idx.size() == 1);
    std::cout << "used sample = " << idx[0] << '\n';
    TEST("Used correct sample", idx[0], 2);

    const std::vector<double> & res = search.residuals();
    assert(res.size() == 4);
...
test_ran_sam_search()
{
...
#ifdef DEBUG
  std::cout << "estimate = " << est_params << ", true model = " << true_params << '\n'
            << "scale = " << ransam->scale() << std::endl;
#endif // DEBUG
  ok = vnl_math::abs(est_params[0] - true_params[0]) < 0.2 && vnl_math::abs(est_params[1] - true_params[1]) < 0.025 &&
  ...
  std::cout << "similarity estimate = " << est_params << ", true similarity model = " << sim_params << '\n'
            << "scale = " << ransam->scale() << std::endl;
  ok = vnl_math::abs(est_params[0] - sim_params[0]) < 0.025 && vnl_math::abs(est_params[1] - sim_params[1]) < 0.025 &&
       vnl_math::abs(est_params[2] - sim_params[2]) < 1.0 && vnl_math::abs(est_params[3] - sim_params[3]) < 1.0;
