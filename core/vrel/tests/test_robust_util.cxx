// This is core/vrel/tests/test_robust_util.cxx
#include <iostream>
#include <cmath>
#include <vector>
#ifdef _MSC_VER
...
inline bool
is_sorted(std::vector<double>::iterator first, std::vector<double>::iterator last)
{
  bool ok = true;
...
test_robust_util()
{
  ...
  vrel_util_median_and_scale_copy(test_vect4.begin(), test_vect4.end(), median, scale, dof);
  TEST_NEAR("vrel_util_median_copy", median, corr_median, 1e-6);
  TEST_NEAR("vrel_util_scale_copy", scale, corr_scale, 1e-6);
  vrel_util_median_and_scale(test_vect4.begin(), test_vect4.end(), median, scale, 1);
  TEST_NEAR("vrel_util_median", median, corr_median, 1e-6);
  TEST_NEAR("vrel_util__scale", scale, corr_scale, 1e-6);

  //
  //  vrel_util_intercept_adjustment
  //
  ...
  TEST_NEAR("vrel_util_intercept_adjustment_copy --- correct?", half_width, corr_half_width, 1e-6);
  vrel_util_intercept_adjustment(test_vect2.begin(), test_vect2.end(), center, half_width, 1);
  TEST_NEAR("vrel_util_intercept_adjustment(pointer) --- correct?", center, corr_center, 1e-6);
  TEST_NEAR("vrel_util_intercept_adjustment(pointer) --- correct?", half_width, corr_half_width, 1e-6);
  TEST("vrel_util_intercept_adjustment (pointer) --- sorted?", is_sorted(test_vect2.begin(), test_vect2.end()), true);
  ...
  vrel_util_intercept_adjust_stats_copy(test_vec3.begin(), test_vec3.end(), mean, std, frac, 1);
  TEST_NEAR("vrel_util_intercept_adjust_stats_copy ", mean, corr_mean, 1e-6);
  TEST_NEAR("vrel_util_intercept_adjust_stats_copy ", std, corr_std, 1e-6);
  TEST_NEAR("vrel_util_intercept_adjust_stats_copy ", frac, corr_frac, 1e-6);
  vrel_util_intercept_adjust_stats(test_vec3.begin(), test_vec3.end(), mean, std, frac, 1);
  TEST_NEAR("vrel_util_intercept_adjust_stats (pointer) ", mean, corr_mean, 1e-6);
  TEST_NEAR("vrel_util_intercept_adjust_stats (pointer) ", std, corr_std, 1e-6);
  TEST_NEAR("vrel_util_intercept_adjust_stats (pointer) ", frac, corr_frac, 1e-6);
}
