...
class null_m_est : public vrel_m_est_obj
{
public:
  double
  ...
  void
  wgt(vect_const_iter b, vect_const_iter e, vect_const_iter s, vect_iter w) const override
  {
    vrel_m_est_obj::wgt(b, e, s, w);
  }
  void
  wgt(vect_const_iter b, vect_const_iter e, double s, vect_iter w) const override
  {
    vrel_m_est_obj::wgt(b, e, s, w);
  }
};
...
test_similarity_from_matches()
{
  ...
  TEST("fit_from_minimal_sample -- exact",
       sim.fit_from_minimal_set(indices, est_params) && close(est_params[0], params[0]) &&
         close(est_params[1], params[1]) && close(est_params[2], params[2]) && close(est_params[3], params[3]),
       true);

  std::vector<double> residuals;
  sim.compute_residuals(params.as_ref(), residuals);
  TEST("compute_residuals",
       residuals.size() == 6 && close(residuals[0], 0) && close(residuals[1], 5) && close(residuals[2], 50) &&
         close(residuals[3], 0) && close(residuals[4], 10) && close(residuals[5], 0.5),
       true);
  ...
  TEST("compute_weights",
       close(weights[0], 1.0 * 1.0 / 1.1) && close(weights[1], 0.1 * 0.1 / 1.1) &&
         close(weights[2], 0.02 * 0.02 / 1.14) && close(weights[3], 1.0 * 1.0 / 1.14) &&
         close(weights[4], 0.12 * 0.12 / 1.14) && close(weights[5], 0.9),
       true);
}
