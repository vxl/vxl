// This is core/vrel/vrel_homography2d_est.cxx
#include <cmath>
#include <iostream>
#include <utility>
#include "vrel_homography2d_est.h"
...
vrel_homography2d_est ::vrel_homography2d_est(const std::vector<vgl_homg_point_2d<double>> & from_pts,
                                              const std::vector<vgl_homg_point_2d<double>> & to_pts,
                                              unsigned int homog_dof)
  : vrel_estimation_problem(homog_dof /*dof*/, (homog_dof / 2) /*points to instantiate*/)
{
...
vrel_homography2d_est ::vrel_homography2d_est(std::vector<vnl_vector<double>> from_pts,
                                              std::vector<vnl_vector<double>> to_pts,
                                              unsigned int homog_dof)
  : vrel_estimation_problem(homog_dof /*dof*/, (homog_dof / 2) /*points to instantiate*/)
  , from_pts_(std::move(from_pts))
...
vrel_homography2d_est::~vrel_homography2d_est() = default;


unsigned int
vrel_homography2d_est ::num_samples() const
{
...
bool
vrel_homography2d_est ::fit_from_minimal_set(const std::vector<int> & point_indices, vnl_vector<double> & params) const
{
...
vrel_homography2d_est ::compute_residuals(const vnl_vector<double> & params, std::vector<double> & residuals) const
{
  ...
  for (unsigned int i = 0; i < from_pts_.size(); ++i)
  {
    ...
    if (from_pts_[i][2] == 0 || to_pts_[i][2] == 0 || trans_pt[2] == 0 || inv_trans_pt[2] == 0)
    {
      residuals[i] = 1e10;
    }
    else
    {
      ...
      residuals[i] =
        std::sqrt(vnl_math::sqr(del_x) + vnl_math::sqr(del_y) + vnl_math::sqr(inv_del_x) + vnl_math::sqr(inv_del_y));
    }
  }
}
...
bool
vrel_homography2d_est ::weighted_least_squares_fit(vnl_vector<double> & params,
...
void
vrel_homography2d_est ::homog_to_params(const vnl_matrix<double> & m, vnl_vector<double> & p) const
{
  for (int r = 0; r < 3; ++r)
    for (int c = 0; c < 3; ++c)
      p(3 * r + c) = m(r, c);
...
void
vrel_homography2d_est ::params_to_homog(const vnl_vector<double> & p, vnl_matrix<double> & m) const
{
  for (int r = 0; r < 3; ++r)
    for (int c = 0; c < 3; ++c)
      m(r, c) = p(3 * r + c);
...
void
vrel_homography2d_est ::normalize(const std::vector<vnl_vector<double>> & pts,
...
void
vrel_homography2d_est ::print_points() const
{}
