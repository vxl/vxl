// This is core/vrel/vrel_irls.cxx
#include <iostream>
#include <vector>
#include "vrel_irls.h"
//:
...
vrel_irls::estimate(const vrel_estimation_problem * problem, const vrel_wls_obj * obj)
{
  ...
  if (scale_ <= 1e-8)
  {
    ...
    scale_ = 0.0;
    converged_ = true;
    std::cerr << "vrel_irls::estimate: initial scale is zero - cannot estimate\n";
  ...
  while (true)
  {
    ...
    if (est_scale_during_ && iteration_ <= iterations_for_scale_est_)
    {
      ...
      { //  fit exact enough to yield 0 scale estimate
        ...
        scale_ = 0.0;
        converged_ = true;
        std::cerr << "vrel_irls::estimate:  scale has gone to 0.\n";
        break;
      }
