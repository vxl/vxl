...
{
inline void
shft2(double & a, double & b, const double c)
{
  a = b;
  b = c;
}
...
inline void
shft3(double & a, double & b, double & c, const double d)
{
  a = b;
  b = c;
  c = d;
...
vrel_kernel_density_obj::vrel_kernel_density_obj(vrel_kernel_scale_type scale_type)
...
double
vrel_kernel_density_obj::best_x(vect_const_iter res_begin, vect_const_iter res_end, double prior_scale) const
{
  if (fix_x_)
    return 0;
  ...
  while (vnl_math::abs(x3 - x0) > tol * vnl_math::abs(x1) + vnl_math::abs(x2))
  {
    if (f2 > f1)
    {
