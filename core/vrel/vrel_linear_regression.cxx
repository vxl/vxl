...
#include <utility>
#include <vector>
#include "vrel_linear_regression.h"
...
vrel_linear_regression::vrel_linear_regression(const std::vector<vnl_vector<double>> & pts, bool use_intercept)
  : rand_vars_(pts.size())
  , ind_vars_(pts.size())
{
  ...
  {
    std::cerr << "\nvrel_linear_regression::vrel_linear_regression  WARNING:  DoF is greater than\n"
...
vrel_linear_regression::vrel_linear_regression(const std::vector<vnl_vector<double>> & ind_vars,
                                               std::vector<double> dep_vars)
  : rand_vars_(std::move(dep_vars))
  , ind_vars_(ind_vars)
{
  set_param_dof(ind_vars_[0].size());
  if (param_dof() > ind_vars.size())
  {
    std::cerr << "vrel_linear_regression::vrel_linear_regression  WARNING:  DoF is greater than\n"
...
vrel_linear_regression::~vrel_linear_regression() = default;

unsigned int
vrel_linear_regression::num_samples() const
{
...
//
bool
vrel_linear_regression::fit_from_minimal_set(const std::vector<int> & point_indices, vnl_vector<double> & params) const
{
  if (point_indices.size() != param_dof())
  {
    std::cerr << "vrel_linear_regression::fit_from_minimal_sample  The number of point "
...
void
vrel_linear_regression::compute_residuals(const vnl_vector<double> & params, std::vector<double> & residuals) const
{
...
bool
vrel_linear_regression::weighted_least_squares_fit(vnl_vector<double> & params,
...
void
vrel_linear_regression::print_points() const
{
  std::cout << "\nvrel_linear_regression::print_points:\n"
            << "  param_dof() = " << param_dof() << '\n'
