// This is core/vrel/vrel_muse_table.cxx
#include <iostream>
#include <cmath>
...
{
  return left_t.n_ < right_t.n_ || (left_t.n_ == right_t.n_ && left_t.k_ < right_t.k_);
}
...
void
vrel_muse_table::calculate_all(unsigned int k, unsigned int n, vrel_muse_table_entry & entry)
{
...
double
vrel_muse_table::calculate_expected(unsigned int k, unsigned int n) const
{
  return vrel_misc_gaussian_cdf_inv(0.5 * (1.0 + ((double)k / (double)(n + 1))));
}
...
vrel_muse_table::calculate_standard_dev(unsigned int k, unsigned int n, double expected_kth) const
{
  ...
  */
  ...
  vrk = (pk * qk / (double)(n + 2)) * Qk_prime * Qk_prime +
        (pk * qk / ((double)((n + 2) * (n + 2)))) *
          (2.0 * (qk - pk) * Qk_prime * Qk_dprime + pk * qk * (Qk_prime * Qk_tprime + 0.5 * Qk_dprime * Qk_dprime));
...
double
vrel_muse_table::calculate_divisor(unsigned int /* k */, unsigned int n, double expected_kth) const
{
  return (n + 1) * std::sqrt(2 / vnl_math::pi) * (1.0 - std::exp(-vnl_math::sqr(expected_kth) / 2.0));
}

double
vrel_muse_table::calculate_sq_divisor(unsigned int k, unsigned int n, double expected_kth) const
{
  return k - (n + 1) * expected_kth * std::sqrt(2 / vnl_math::pi) * std::exp(-vnl_math::sqr(expected_kth) / 2.0);
}
