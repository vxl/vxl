// This is core/vrel/vrel_muset_obj.cxx
#include <limits>
#include <iostream>
#include <vector>
#include <algorithm>
#include "vrel_muset_obj.h"
//
#ifdef _MSC_VER
...
vrel_muset_obj::vrel_muset_obj(vrel_muse_table * table, bool use_sk_refine)
  : use_sk_refine_(use_sk_refine)
  , muse_type_(VREL_MUSE_TRIMMED)
  , table_owned_(false)
  , table_(table)
...
double
vrel_muset_obj::fcn(vect_const_iter begin,
                    vect_const_iter end,
...
void
vrel_muset_obj::internal_fcn(vect_const_iter begin, vect_const_iter end, double & sigma_est, int & best_k) const
{
  ...
  switch (muse_type_)
  {
    case VREL_MUSE_TRIMMED:
    {
      ...
      for (double frac = min_frac_; frac <= max_frac_ + 0.00001; frac += frac_inc_)
      {
        ...
        if (k <= 0)
          k = 1;
        if (table_->expected_kth(k, num_residuals) / table_->standard_dev_kth(k, num_residuals) <
            min_exp_kth_to_stddev_ratio)
        {
          if (notwarned)
          {
            std::cerr << "WARNING:  vrel_muset_obj::internal_fcn "
    ...
    case VREL_MUSE_TRIMMED_SQUARE:
    {
      ...
      for (double frac = min_frac_; frac <= max_frac_ + 0.00001; frac += frac_inc_)
      {
            ...
            min_exp_kth_to_stddev_ratio)
        {
          if (notwarned)
          {
            std::cerr << "WARNING:  vrel_muset_obj::internal_fcn attempted evaluation at "
      ...
      else
      {
        ...
        std::cout << "New n = " << new_n << '\n';
#endif
        sigma_est = std::sqrt(best_sum_sq / table_->muset_sq_divisor(best_k, new_n));
      }
      break;
    }
