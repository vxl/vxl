#include <iostream>
#include <vector>
#include "vrel_orthogonal_regression.h"
  ...
  : vars_(pts)
{
  ...
  if (param_dof() > num_pts)
    std::cerr << "\nvrel_orthogonal_regression::vrel_orthogonal_regression  WARNING:\n"
              << "DoF (" << param_dof() << ") is greater than the number of data points (" << num_pts
...
vrel_orthogonal_regression::~vrel_orthogonal_regression() = default;

unsigned int
vrel_orthogonal_regression::num_samples() const
{
  return vars_.rows();
}

bool
vrel_orthogonal_regression::fit_from_minimal_set(const std::vector<int> & point_indices,
                                                 vnl_vector<double> & params) const
{
  if (point_indices.size() != param_dof())
  {
    std::cerr << "vrel_orthogonal_regression::fit_from_minimal_sample  The number of point "
...
void
vrel_orthogonal_regression::compute_residuals(const vnl_vector<double> & params, std::vector<double> & residuals) const
{
...
bool
vrel_orthogonal_regression::weighted_least_squares_fit(vnl_vector<double> & params,
                                                       ...
                                                       const std::vector<double> * weights) const
{
  ...
  {
    std::cerr << "vrel_orthogonal_regression::WeightedLeastSquaresFit --- singularity!\n";
    return false;
  }
  else
  {
...
void
vrel_orthogonal_regression::print_points() const
{
  std::cout << "\nvrel_orthogonal_regression::print_points:\n"
            << "  param_dof() = " << param_dof() << '\n'
            << "  num_pts = " << vars_.rows() << "\n\n"
            << " i   vars_\n"
            << " =   =========\n";
