#ifndef vrel_quad_est_h_
#define vrel_quad_est_h_
...
class vrel_quad_est : public vrel_estimation_problem
{
public:
  ...
  vrel_quad_est(const std::vector<vgl_point_2d<double>> & from_pts, const std::vector<vgl_point_2d<double>> & to_pts);

  //: Constructor with data pre-separated into arrays of independent and dependent variables.
  vrel_quad_est(const std::vector<vnl_vector<double>> & from_pts,
                std::vector<vnl_vector<double>> to_pts,
                unsigned int dim = 2);

  //: Destructor.
  ~vrel_quad_est() override;
