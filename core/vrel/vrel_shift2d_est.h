#ifndef vrel_shift2d_est_h_
#define vrel_shift2d_est_h_
...
//
// This class is an adaptation of vrel_homography2d_est to compute a
// 2D shift instead of a 2D homography.

class vrel_shift2d_est : public vrel_estimation_problem
{
public:
  //: Constructor from vgl_homg_point_2d's
  vrel_shift2d_est(const std::vector<vgl_homg_point_2d<double>> & from_pts,
                   const std::vector<vgl_homg_point_2d<double>> & to_pts);

  //: Constructor from vnl_vectors
  vrel_shift2d_est(std::vector<vnl_vector<double>> from_pts, std::vector<vnl_vector<double>> to_pts);

  //: Destructor.
  ~vrel_shift2d_est() override;
