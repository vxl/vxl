...
public:
  ...
  virtual void
  b_write_by_base(vsl_b_ostream & os, const test2_base_class & base) const = 0;

  virtual void
  b_read_by_base(vsl_b_istream & os, test2_base_class & base) const = 0;

  virtual void
  print_summary_by_base(std::ostream & os, const test2_base_class & base) const = 0;

  virtual test2_base_class_io *
  clone() const = 0;

  virtual std::string
  target_classname() const = 0;

  virtual bool
  is_io_for(const test2_base_class & base) const = 0;
};
...
void
vsl_add_to_binary_loader(const test2_base_class_io & b)
{
  vsl_clipon_binary_loader<test2_base_class, test2_base_class_io>::instance().add(b);
}


//: Binary save to stream.
inline void
vsl_b_write(vsl_b_ostream & os, const test2_base_class * b)
{
  vsl_clipon_binary_loader<test2_base_class, test2_base_class_io>::instance().write_object(os, b);
}

//: Binary read from stream.
inline void
vsl_b_read(vsl_b_istream & is, test2_base_class *& b)
{
  vsl_clipon_binary_loader<test2_base_class, test2_base_class_io>::instance().read_object(is, b);
}

//: Print summary to stream by vnl_nonlinear_minimizer pointer
void
vsl_print_summary(std::ostream & os, const test2_base_class * b)
{
  vsl_clipon_binary_loader<test2_base_class, test2_base_class_io>::instance().print_object_summary(os, b);
}
...
public:
  ...
  //: Print summary
  void
  print_summary(std::ostream & os) const override;
...
void
test2_derived_class::vtable_hack()
{}

//: Return a platform independent string identifying the class
std::string
test2_derived_class::is_a() const
{
...
//: Print summary
void
test2_derived_class::print_summary(std::ostream & os) const
{
  os << is_a() << " Data=" << data_ << std::endl;
}

void
vsl_b_write(vsl_b_ostream & os, const test2_derived_class & d)
{
...
public:
  test2_base_class *
  new_object() const override
  {
    return new test2_derived_class;
  }

  void
  b_write_by_base(vsl_b_ostream & os, const test2_base_class & base) const override
  {
    vsl_b_write(os, (const test2_derived_class &)base);
  }

  void
  b_read_by_base(vsl_b_istream & is, test2_base_class & base) const override
  {
    vsl_b_read(is, (test2_derived_class &)base);
  }

  void
  print_summary_by_base(std::ostream & os, const test2_base_class & base) const override
  {
  ...
  std::string
  target_classname() const override
  {
    return std::string("test2_derived_class");
  }

  bool
  is_io_for(const test2_base_class & base) const override
  {
