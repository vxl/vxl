...
public:
  ...
  {
    add_attribute(attr_name, (long)value);
  }
  void
  add_attribute(std::string attr_name, size_t value)
  {
    add_attribute(attr_name, (long)value);
  }
#if VXL_ADDRESS_BITS != 32 // defined(_WIN32) && defined(_MSC_VER) && !defined(_WIN64) )  // Cannot have this
                           // overloading for MSVC or GCC 32-bit build as it collides with size_t
  void
  add_attribute(std::string attr_name, unsigned int value)
  {
    add_attribute(attr_name, (long)value);
  }
#endif
...
#endif
  void
  append_cdata(const std::string & cdata);
  void
  append_cdata(double cdata);
  void
  append_cdata(int cdata);

  void
  x_write(std::ostream & ostr);
  //: writes the opening tag for this node to the stream
  void
  x_write_open(std::ostream & ostr);

  //: writes the closing tag for this node to the stream
  void
  x_write_close(std::ostream & ostr);
