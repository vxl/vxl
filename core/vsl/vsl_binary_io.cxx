...
void
vsl_b_write(vsl_b_ostream & os, const std::string & str)
{
...
void
vsl_b_ostream::clear_serialisation_records()
{
...
unsigned long
vsl_b_ostream::add_serialisation_record(void * pointer, int other_data /*= 0*/)
{
...
unsigned long
vsl_b_ostream::get_serial_number(void * pointer) const
{
  auto entry = serialisation_records_.find(pointer);
...
// unrecorded.
int
vsl_b_ostream::get_serialisation_other_data(void * pointer) const
{
  auto entry = serialisation_records_.find(pointer);
  if (entry == serialisation_records_.end())
  {
...
int
vsl_b_ostream::set_serialisation_other_data(void * pointer, int /*other_data*/)
{
  auto entry = serialisation_records_.find(pointer);
  if (entry == serialisation_records_.end())
  {
  ...
  : is_(i_s)
{
  ...
  {
    std::cerr << "\nI/O ERROR: vsl_b_istream::vsl_b_istream(std::istream *is)\n"
              << "             The stream's leading version number is " << v << ". Expected value 1.\n";
    is_->clear(std::ios::badbit); // Set an unrecoverable IO error on stream
  }
...
unsigned short
vsl_b_istream::version_no() const
{
  return version_no_;
}
...
void
vsl_b_istream::clear_serialisation_records()
{
  serialisation_records_.clear();
}
...
void
vsl_b_istream::add_serialisation_record(unsigned long serial_number, void * pointer, int other_data /*= 0*/)
{
  assert(pointer != nullptr);
...
void *
vsl_b_istream::get_serialisation_pointer(unsigned long serial_number) const
{
  auto entry = serialisation_records_.find(serial_number);
  if (entry == serialisation_records_.end())
  {
    return nullptr;
  }
...
// Returns 0 if no record has been added.
int
vsl_b_istream::get_serialisation_other_data(unsigned long serial_number) const
{
  auto entry = serialisation_records_.find(serial_number);
  if (entry == serialisation_records_.end())
  {
