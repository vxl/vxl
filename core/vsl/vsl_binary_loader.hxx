...
vsl_binary_loader<BaseClass> &
vsl_binary_loader<BaseClass>::instance()
{
...
template <class BaseClass>
void
vsl_binary_loader<BaseClass>::make_empty()
{
...
template <class BaseClass>
void
vsl_binary_loader<BaseClass>::load_object(vsl_b_istream & is, BaseClass *& b)
{
  if (!is)
    return;
  ...
  unsigned int i = 0;
  while (i < object_.size() && !(object_[i]->is_a() == name))
    ++i;

  if (i < object_.size())
  {
    b = object_[i]
          ->clone(); // If you get a compile error here you are probably trying to load into a non-base class pointer.
    b->b_read(is);
  }
  else
  {
...
#define VSL_BINARY_LOADER_WITH_SPECIALIZATION_INSTANTIATE(T) \
  template <>                                                \
  std::string vsl_binary_loader<T>::is_a() const             \
  {                                                          \
