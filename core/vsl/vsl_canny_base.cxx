...
vsl_canny_base::vsl_canny_base(float sigma, float low, float high, bool v)
  : _xstart(0), _ystart(0)
  , _xsize(0), _ysize(0)
...
void vsl_canny_base::Initial_follow(float * const *thin, int xsize, int ysize, float low,
				    int x, int y,
				    vcl_list<int> *xc,
				    vcl_list<int> *yc,
				    vcl_list<float> *grad)
...
// -- Following routine looking for connectiveness of edgel chains
// and accounts for single pixel gaps in the chains.
void vsl_canny_base::Final_follow(int x, int y,
				  vcl_list<int> *xc,
				  vcl_list<int> *yc,
				  vcl_list<float> *grad,
				  int reverse)
...
//
void vsl_canny_base::Follow_junctions(int * const *junction,
				      int x, int y, 
				      vcl_list<int> *xc, 
				      vcl_list<int> *yc)
{
...
void vsl_canny_base::Cluster_centre_of_gravity(int * const *jx, int * const *jy,
					       vcl_list<int> &xc,
					       vcl_list<int> &yc,
					       int &x0, int &y0)
{
...
//
int vsl_canny_base::Junction_neighbour(int const * const *junction, int x, int y) {
...
template <class T>
void vsl_canny_base_fill_raw_image(T * * image, int sizex, int sizey, T value) {
...
template <class T>
void vsl_canny_base_free_raw_image(T **ptr) {
  T *array = ptr[0];
...
void vsl_canny_base_copy_raw_image(S const * const *image1, T * const *image2, int m, int n) // 
{
  for (int x=0; x<m; ++x) 
    for (int y=0; y<n; ++y) 
...
#define inst(T) \
template T     **vsl_canny_base_make_raw_image(int, int, T *dummy); \
template void    vsl_canny_base_fill_raw_image(T **, int, int, T value); \
template void    vsl_canny_base_free_raw_image(T **); \
template void vsl_canny_base_copy_raw_image(T const * const *, T * const *, int, int);
inst(float);
inst(int);
#undef inst
