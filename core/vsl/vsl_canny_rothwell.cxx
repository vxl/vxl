...
vsl_canny_rothwell::vsl_canny_rothwell(vsl_canny_rothwell_params const &params)
...
vsl_canny_rothwell::~vsl_canny_rothwell() {
  ...
  vsl_canny_base_free_raw_image(_dangling);
...
void vsl_canny_rothwell::detect_edges(vil_image const &image, vcl_list<vsl_Edge*> *edges, bool adaptive) 
{
  ...
  _dangling = vsl_canny_base_make_raw_image(_xsize, _ysize, (int*)0);
  ...
  vsl_canny_base_fill_raw_image(_smooth,_xsize, _ysize, 0.0f);
  vsl_canny_base_fill_raw_image(_dx,    _xsize, _ysize, 0.0f);
  vsl_canny_base_fill_raw_image(_dy,    _xsize, _ysize, 0.0f);
...
// processing.
//
void vsl_canny_rothwell::Initial_hysteresis() {
 
  vcl_list<int> xcoords,ycoords;
  vcl_list<float> grad;
  vcl_list<vsl_EdgelChain*> edges;
  float *thin,*px,*py,*pg;
  vsl_EdgelChain *edgels;
  ...
  for (int x=_w0; x<_xsize-_w0; ++x)  {
	...
	edgels = new vsl_EdgelChain(xcoords.size());
	px = edgels->GetX();
	py = edgels->GetY();
	pg = edgels->GetGrad();
	while ( xcoords.size() ) {
...
extern vsl_Vertex *vsl_find(vcl_list<vsl_Vertex*> const *l, float x, float y);
...
// greater than _high.
//
void vsl_canny_rothwell::Final_hysteresis(vcl_list<vsl_Edge*> *edges) {
  vcl_list<int> xcoords,ycoords;
  vcl_list<float> grad;
...
// genus zero.
//
void vsl_canny_rothwell::Thin_edges() {
  int i,count;
  float threshold;
...
//
void vsl_canny_rothwell::Jump_single_breaks() {
...
// fix any problems.
//
void vsl_canny_rothwell::Adaptive_Canny(vil_image const &image) {
  ...
  float **dy   = vsl_canny_base_make_raw_image(image_size,image_size, (float*)0);
  float **grad = vsl_canny_base_make_raw_image(image_size,image_size, (float*)0);
  ...
  for (it i=_xdang->begin(), j=_ydang->begin(); i!=_xdang->end() && j!=_ydang->end(); ++i, ++j) {
	...
	vsl_canny_smooth_rothwell_adaptive(image, x0,y0,image_size, _kernel, _width, _k_size, dx,dy,grad);
  ...
  vsl_canny_base_free_raw_image(dy);
  vsl_canny_base_free_raw_image(grad);
}
...
// to zero.
void vsl_canny_rothwell::Subtract_thick(int x0, int y0, int image_size, float **grad)
{
...
void vsl_canny_rothwell::Best_eight_way(int x, int y, float **grad, int *xnew, int *ynew)
{
  float max = _low;
...
//
void vsl_canny_rothwell::Find_dangling_ends() {
  // Reset the dangling ends
  _xdang->clear();
  _ydang->clear();
  vsl_canny_base_fill_raw_image(_dangling, _xsize, _ysize, 0);
...
//
int vsl_canny_rothwell::Dangling_end(int x, int y)
{
  if ( _thin[x][y] <= _low )
    return 0;
...
//  -- Searches for the junctions in the image.
//
void vsl_canny_rothwell::Find_junctions() {
...
//
//
void vsl_canny_rothwell::Find_junction_clusters() {
