...
*/
#ifdef __GNUC__
#pragma implementation "vsl_chamfer"
#endif
#include "vsl_chamfer.h"

//: Determines the minimum of five ints.
int vsl_Minimum5(int a, int b, int c, int d, int e)
{
  if ( (a<=b) && (a<=c) && (a<=d) && (a<=e) )
    return 1;
  else if ( (b<=c) && (b<=d) && (b<=e) )
...
//: Determines the minimum of five ints.
int vsl_Minimum4(int a, int b, int c, int d) 
{
  if ( (a<=b) && (a<=c) && (a<=d) )
    return 1;
  else if ( (b<=c) && (b<=d) )
...
void vsl_chamfer_Forward (int m, int n, int **dist, float **param) 
{
  for (int i=1; i<m-1; ++i) {
    for (int j=1; j<n-1; ++j)  {
      
      int val = vsl_Minimum5(dist[i-1][j-1]+4,
			     dist[i-1][j  ]+3,
			     dist[i-1][j+1]+4,
			     dist[i  ][j-1]+3,
			     dist[i  ][j  ]);
...
void vsl_chamfer_Backward(int m, int n, int **dist, float **param) 
{
  for (int i=m-2; i>0; --i) {
    for (int j=n-2; j>0; --j)  {
	     
      int val = vsl_Minimum5(dist[i  ][j],
			     dist[i  ][j+1]+3,
			     dist[i+1][j-1]+4,
			     dist[i+1][j  ]+3,
			     dist[i+1][j+1]+4 );
...
//
void vsl_chamfer_Alt1(int m, int n, int **dist, float **param) 
{
  for (int i=1; i<m-1; ++i) {
    for (int j=n-2; j>0; --j)  {
...
// nearest pixel. The image sizes are mxn.
//
void vsl_chamfer_Alt2(int m, int n, int **dist, float **param)
{
  for (int i=m-2; i>0; --i) {
    for (int j=1; j<n-1; ++j) {
