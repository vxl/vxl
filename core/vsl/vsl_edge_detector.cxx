...
#define Make_float_image(w, h) vsl_canny_base_make_raw_image(w, h, (float*)0)
#define Make_int_image(w, h) vsl_canny_base_make_raw_image(w, h, (int*)0)
#define Free_float_image(i,dummy) vsl_canny_base_free_raw_image(i)
#define Free_int_image(i,dummy) vsl_canny_base_free_raw_image(i)

//-----------------------------------------------------------------------

vsl_edge_detector::vsl_edge_detector(vsl_edge_detector_params const &params)
...
vsl_edge_detector::~vsl_edge_detector() {
...
void vsl_edge_detector::detect_edges(vil_image const &image, 
				     ...
				     bool maintain_topology) 
{
  ...
  vsl_canny_base_fill_raw_image(_thresh, _xsize, _ysize, _low);
  vsl_canny_base_fill_raw_image(_theta, _xsize, _ysize, DUMMYTHETA);
  
  // A suitably large value (FAR) - perhaps should be more. This represents the
  // distance of a point to the nearest edgel
  vsl_canny_base_fill_raw_image(_dist, _xsize, _ysize, FAR);
...
//
//
void vsl_edge_detector::Thicken_threshold(int x, int y) {
...
// vsl_Edgel change detection-JLM May 1995
//
void vsl_edge_detector::Set_thresholds() {
  ...
  vsl_canny_base_copy_raw_image(VCL_OVERLOAD_CAST(int const*const*, _dist), fdist, _xsize, _ysize);
  vsl_canny_base_copy_raw_image(VCL_OVERLOAD_CAST(int const*const*, _dist), bdist, _xsize, _ysize);
  vsl_canny_base_copy_raw_image(VCL_OVERLOAD_CAST(int const*const*, _dist), a1dist, _xsize, _ysize);
  vsl_canny_base_copy_raw_image(VCL_OVERLOAD_CAST(int const*const*, _dist), a2dist, _xsize, _ysize);
  ...
  vsl_canny_base_copy_raw_image(VCL_OVERLOAD_CAST(float const *const*, _thresh), fth, _xsize, _ysize);
  vsl_canny_base_copy_raw_image(VCL_OVERLOAD_CAST(float const *const*, _thresh), bth, _xsize, _ysize);
  vsl_canny_base_copy_raw_image(VCL_OVERLOAD_CAST(float const *const*, _thresh), a1th, _xsize, _ysize);
  vsl_canny_base_copy_raw_image(VCL_OVERLOAD_CAST(float const *const*, _thresh), a2th, _xsize, _ysize);

  vsl_chamfer_Forward (_xsize, _ysize, fdist, fth);
  vsl_chamfer_Backward(_xsize, _ysize, bdist, bth);
  vsl_chamfer_Alt1(_xsize, _ysize, a1dist, a1th);
  vsl_chamfer_Alt2(_xsize, _ysize, a2dist, a2th);
  ...
  for (int x=0; x<_xsize; ++x) {
	...
	int option = vsl_Minimum4(fdist[x][y],
				  bdist[x][y],
				  a1dist[x][y],
				  a2dist[x][y]);
	float num,den;
  ...
  // Noticed that all gradient values are used in vsl_Edgel Strength Histogram - May 1997
...
//
void vsl_edge_detector::Thin_edges() {
...
// see vsl_canny_ox.cxx
extern vsl_Vertex *vsl_find(vcl_list<vsl_Vertex*> const *l, vsl_Vertex const &v);


// -- Follow all edgel chains that have pixel values above their corresponding
// threshold values (_thin[x][y] > _thresh[x][y]).
//
void vsl_edge_detector::Follow_curves(vcl_list<vsl_Edge*> *edges)
{
  ...
  for (int x=_width; x<_xsize-_width; ++x)  {
	...
	continue;
      ...
      // Write the points to the vsl_EdgelChain
      while (count) {
...
//
void vsl_edge_detector::Follow(int x, int y, 
			       vcl_list<int> *xc, 
			       vcl_list<int> *yc,
			       vcl_list<float> *grad,
			       int reverse)
...
#undef smoo
  ...
  // ends of the vsl_EdgelChains when reversal occurs next to the junction
...
//
void vsl_edge_detector::Find_junctions() {
...
//
//
void vsl_edge_detector::Find_junction_clusters() {
  ...
  for (vcl_list<int>::iterator i=xvertices.begin(), j=yvertices.begin(); 
    ...
    vsl_Vertex *v = new vsl_Vertex( (*i)/*xvertices.value()*/+_xstart,
...
//
void vsl_edge_detector::Follow_junctions(int x, int y, vcl_list<int> *xc, vcl_list<int> *yc)
{
...
//
//
void vsl_edge_detector::Cluster_centre(vcl_list<int> &xc, 
				       vcl_list<int> &yc,
				       int &x0, 
				       int &y0)
{
