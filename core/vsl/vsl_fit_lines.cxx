...
vsl_fit_lines::vsl_fit_lines(vsl_fit_lines_params const & params,
			     double scale, double x0, double y0) 
  : vsl_fit_lines_params(params)
{
  float temp_thresh = this->_threshold;
  if (this->_use_square_fit) temp_thresh *= this->_threshold;
  this->_threshold = temp_thresh;
  _data = new vsl_OrthogRegress(scale,x0,y0);
}

vsl_fit_lines::~vsl_fit_lines() {
...
void vsl_fit_lines::MergeLines(vcl_list<vsl_edgel_chain*> *_curves) {
  ...
  // Make sure that b >= 0 to be consistent with vsl_OrthogRegress - though
