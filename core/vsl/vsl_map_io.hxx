...
void
vsl_b_write(vsl_b_ostream & s, const std::map<Key, T, Compare> & v)
{
...
template <class Key, class T, class Compare>
void
vsl_b_read(vsl_b_istream & is, std::map<Key, T, Compare> & v)
{
  if (!is)
    return;
...
template <class Key, class T, class Compare>
void
vsl_print_summary(std::ostream & os, const std::map<Key, T, Compare> & v)
{
  os << "Map size: " << v.size() << '\n';
  unsigned i = 0;
  for (typename std::map<Key, T, Compare>::const_iterator iter = v.begin(); iter != v.end() && i < 5; ++iter, ++i)
  {
    os << vsl_indent() << ' ' << i << ": ";
...
template <class Key, class T, class Compare>
void
vsl_b_write(vsl_b_ostream & s, const std::multimap<Key, T, Compare> & v)
{
  constexpr short version_no = 1;
  vsl_b_write(s, version_no);
  vsl_b_write(s, v.size());
  for (typename std::multimap<Key, T, Compare>::const_iterator iter = v.begin(); iter != v.end(); iter++)
  {
    vsl_b_write(s, (*iter).first);
    vsl_b_write(s, (*iter).second);
  }
...
template <class Key, class T, class Compare>
void
vsl_b_read(vsl_b_istream & is, std::multimap<Key, T, Compare> & v)
{
  if (!is)
    return;
...
template <class Key, class T, class Compare>
void
vsl_print_summary(std::ostream & os, const std::multimap<Key, T, Compare> & v)
{
  os << "multimap size: " << v.size() << '\n';
  unsigned i = 0;
  for (typename std::multimap<Key, T, Compare>::const_iterator iter = v.begin(); iter != v.end() && i < 5; ++iter, ++i)
  {
    os << ' ' << i << ": ";
    vsl_print_summary(os, (*iter).first);
    os << ", ";
