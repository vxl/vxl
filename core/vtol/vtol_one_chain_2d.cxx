...
//:
// constructors for the vtol_one_chain_2d
...
vtol_one_chain_2d::vtol_one_chain_2d(vcl_vector<vtol_edge_2d*>& edges, bool iscycle)
 {
  _cycle_p = iscycle;


  for(vcl_vector<vtol_edge_2d*>::iterator i = edges.begin(); i != edges.end(); i++)
    {
    link_inferior(*i);
...
vtol_one_chain_2d::vtol_one_chain_2d(vcl_vector<vtol_edge_2d*>& edges, vcl_vector<signed char> &dirs, bool iscycle)
{
  _cycle_p = iscycle;
...
vtol_one_chain_2d::vtol_one_chain_2d (vtol_one_chain_2d const& onechain)
{
  vtol_one_chain_2d* el = (vtol_one_chain_2d*)(&onechain);
  vcl_vector<vtol_vertex_2d*>* verts = el->vertices();
  int len = verts->size();
  vcl_vector<vtol_topology_object_2d*> newverts(len);
  int i = 0;
  vcl_vector<vtol_vertex_2d*>::iterator v;
  ...
  for(dir=el->_directions.begin(), inf=el->_inferiors.begin(); dir!=el->_directions.end(); dir++, inf++)
    {
      vtol_edge_2d* e = (*inf)->cast_to_edge_2d();

      vtol_edge_2d* newedge = new vtol_edge_2d(newverts[e->get_v1()->get_id()]->cast_to_vertex_2d(),
                               newverts[e->get_v2()->get_id()]->cast_to_vertex_2d());
      link_inferior(newedge);
      _directions.push_back(*dir);
    }
  set_cycle_p(el->get_cycle_p());
  hierarchy_node_list_2d *hierarchy_infs = el->get_hierarchy_inferiors();
  
  hierarchy_node_list_2d::iterator h;
  for (h=hierarchy_infs->begin();h!= hierarchy_infs->end();h++)
    add_hierarchy_inferior(((vtol_one_chain_2d*)(*h))->copy());
  delete verts;
}
...
vtol_one_chain_2d* vtol_one_chain_2d::copy_with_arrays(vcl_vector<vtol_topology_object_2d*>& verts,
                                   vcl_vector<vtol_topology_object_2d*>& edges)
{
  vtol_one_chain_2d* newone = new vtol_one_chain_2d();
  ...
  for(di=_directions.begin(),ti =_inferiors.begin();ti != _inferiors.end();ti++,di++)
    {
      vtol_edge_2d *e = (*ti)->cast_to_edge_2d();
      vtol_edge_2d *newedge = edges[e->get_id()]->cast_to_edge_2d();
  ...
  for (hi=hierarchy_infs->begin(); hi !=hierarchy_infs->end();hi++)
    {
      vtol_one_chain_2d* oldone = (vtol_one_chain_2d*)(*hi);
...
signed char vtol_one_chain_2d::get_direction(vtol_edge_2d* e)
{
  vcl_vector<signed char>::iterator dit = _directions.begin();
  for(topology_list_2d::iterator toit = _inferiors.begin(); toit != _inferiors.end(); ++toit)
    {
      vtol_edge_2d* ce = (*toit)->cast_to_edge_2d();
      if(ce == e)
        return *dit;
      ++dit;
    }
...
vcl_vector<vtol_vertex_2d*>* vtol_one_chain_2d::outside_boundary_vertices()
{
  
  vcl_vector<vtol_vertex_2d*> *verts = new vcl_vector<vtol_vertex_2d*>();
  vcl_vector<vtol_vertex_2d*> *templist;
  vtol_edge_2d *e;
  ...
  tagged_union((vcl_vector<vsol_spatial_object_2d*> *)verts);
  return verts;
}
...
vcl_vector<vtol_vertex_2d*>* vtol_one_chain_2d::vertices()
{
  ...
  SUBCHAIN_INF_2d(verts, vtol_one_chain_2d, vtol_vertex_2d, vertices);
}
...
vcl_vector<vtol_zero_chain_2d*>* vtol_one_chain_2d::outside_boundary_zero_chains()
{
...
vcl_vector<vtol_zero_chain_2d*>* vtol_one_chain_2d::zero_chains()
{
  ...
  SUBCHAIN_INF_2d(zchs, vtol_one_chain_2d,  vtol_zero_chain_2d, zero_chains);
}

//:
// get the outside boundary edges

vcl_vector<vtol_edge_2d*>* vtol_one_chain_2d::outside_boundary_edges()
{
...
vcl_vector<vtol_edge_2d*>* vtol_one_chain_2d::edges()
{
  vcl_vector<vtol_edge_2d*> *edges;
  edges = outside_boundary_edges();

  // This macro adds the subchain zerochains to the zchs list.
  SUBCHAIN_INF_2d(edges, vtol_one_chain_2d,  vtol_edge_2d, edges);
}
...
vcl_vector<vtol_one_chain_2d*>* vtol_one_chain_2d::one_chains()
{
  vcl_vector<vtol_one_chain_2d*> *onechs = outside_boundary_one_chains();
  
  hierarchy_node_list_2d::iterator i;
  for (i=_hierarchy_inferiors.begin();i!= _hierarchy_inferiors.end(); i++)
    onechs->push_back((vtol_one_chain_2d*)(*i));
...
vcl_vector<vtol_one_chain_2d*>* vtol_one_chain_2d::inferior_one_chains()
{
  vcl_vector<vtol_one_chain_2d*> *onechs = new vcl_vector<vtol_one_chain_2d*>();
...
vcl_vector<vtol_one_chain_2d*>* vtol_one_chain_2d::superior_one_chains()
{
  vcl_vector<vtol_one_chain_2d*> *onechs = new vcl_vector<vtol_one_chain_2d*>();
...
vcl_vector<vtol_one_chain_2d*>* vtol_one_chain_2d::outside_boundary_one_chains()
{
...
vcl_vector<vtol_face_2d*>* vtol_one_chain_2d::faces()
{
  if(this->is_sub_chain())
    {
      vcl_vector<vtol_face_2d*> *new_list = new vcl_vector<vtol_face_2d*>, *sublist;
      vcl_vector<vtol_one_chain_2d*>* onech = this->superior_one_chains();
      
      for(vcl_vector<vtol_one_chain_2d*>::iterator i = onech->begin(); i != onech->end();
	  i++){
	sublist = (*i)->faces();
	if(sublist->size()){
...
vcl_vector<vtol_two_chain_2d*>* vtol_one_chain_2d::two_chains()
{
  if(this->is_sub_chain())
    {
      vcl_vector<vtol_two_chain_2d*> *new_list = new vcl_vector<vtol_two_chain_2d*>, *sublist;
      vcl_vector<vtol_one_chain_2d*>* onech = this->superior_one_chains();
...
vcl_vector<vtol_block_2d*>* vtol_one_chain_2d::blocks()
{
   if(this->is_sub_chain())
    {
...
void vtol_one_chain_2d::compute_bounding_box()
{
  vcl_vector<vtol_edge_2d*>* edges = this->edges();
  ...
  for(vcl_vector<vtol_edge_2d*>::iterator eit = edges->begin();
      eit!=edges->end(); eit++)
    {
      vtol_edge_2d* e = (*eit);
...
void vtol_one_chain_2d::determine_edge_directions()
{
  ...
  vtol_edge_2d *first_edge, *second_edge;
  vtol_vertex_2d *tweeney;
  
  topology_list_2d::iterator i;
  i=_inferiors.begin();

  first_edge = (*i)->cast_to_edge_2d();
  i++;

  second_edge = (*i)->cast_to_edge_2d();
...
void vtol_one_chain_2d::add_inferior_one_chain(vtol_one_chain_2d* onechain )
{
...
void vtol_one_chain_2d::remove_inferior_one_chain(vtol_one_chain_2d* onechain )
{
...
bool vtol_one_chain_2d::add_edge(vtol_edge_2d* new_edge, bool dir )
{
  if (dir == true)
    _directions.push_back((signed char)1);
...
bool vtol_one_chain_2d::remove_edge(vtol_edge_2d* doomed_edge, bool force_it)
{
  if (!force_it && _cycle_p)
    {
      ...
      topology_list_2d::iterator i = vcl_find(_inferiors.begin(),_inferiors.end(),
					      (vtol_topology_object_2d*)doomed_edge);
...
bool vtol_one_chain_2d::operator==(vtol_one_chain_2d& ch)
{
  topology_list_2d* inf1 = this->get_inferiors();
  topology_list_2d* inf2 = ch.get_inferiors();

  if (this == &ch) return true;

  // Check to see if the number of vertices is the same
  vcl_vector<vtol_vertex_2d*>* verts1 = this->vertices();
  vcl_vector<vtol_vertex_2d*>* verts2 = ch.vertices();
  ...
  topology_list_2d::iterator i1;
  topology_list_2d::iterator i2;
  ...
  for(i1=inf1->begin() , i2 = inf2->begin(); i1 != inf1->end(); i1++ , i2++){
    ...
    hierarchy_node_list_2d& lefth = ch._hierarchy_inferiors;
    if(righth.size() != lefth.size())
      return false;
    
    
    hierarchy_node_list_2d::iterator r;
    hierarchy_node_list_2d::iterator l;
...
void  vtol_one_chain_2d::reverse_directions()
{
  ...
  for (hi=_hierarchy_inferiors.begin();hi!= _hierarchy_inferiors.end();hi++ )
    ((vtol_one_chain_2d*)(*hi))->reverse_directions();
}
...
bool vtol_one_chain_2d::operator==(vsol_spatial_object_2d& obj)
{
  vtol_topology_object_2d* topo = (vtol_topology_object_2d*)(obj.cast_to_topology_object_2d());
  if (!topo) return false;
  vtol_one_chain_2d* chainobj = topo->cast_to_one_chain_2d();
...
void vtol_one_chain_2d::print(ostream& strm)
{
...
void vtol_one_chain_2d::describe_directions(ostream& strm, int )
{
...
void vtol_one_chain_2d::describe(ostream& strm, int blanking)
{
  print(strm);
