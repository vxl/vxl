...
# ADD CPP /I "$(IUEROOT)/vcl" /I "$(IUEROOT)/vxl" /I "$(IUEROOT)/vcl" /I "$(IUEROOT)/vcl/config.win32-VC60" /I "$(IUEROOT)/vxl"    -DVXL_WARN_DEPRECATED -DVXL_WARN_DEPRECATED_ONCE /D "vul_sequence_filename_example_EXPORTS"
...
USERDEP__HACK=\
	...
	"$(IUEROOT)/vxl/vul/examples/CMakeLists.txt"
# Begin Custom Build

"vul_sequence_filename_example.dsp" :  "$(SOURCE)" "$(INTDIR)" "$(OUTDIR)"
	"cmake.exe" "$(IUEROOT)/vxl/vul/examples/CMakeLists.txt" -DSP -H"$(IUEROOT)" -S"$(IUEROOT)/vxl/vul/examples" -O"$(IUEROOT)/vxl/vul/examples" -B"$(IUEROOT)"
