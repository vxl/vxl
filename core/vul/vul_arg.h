...
template <class T>
void
print_value(std::ostream &, const vul_arg<T> &);
template <class T>
int
parse(vul_arg<T> *, char **);
...
protected:
  ...
  {}; // see constructors of vul_arg

public:
  static VUL_EXPORT required_option_type is_required;

  static void
  parse_deprecated(int & argc, char **& argv, bool warn_about_unrecognized_arguments = true);
  static void
  include_deprecated(vul_arg_info_list & l);

  static void
  add_to_current(vul_arg_base * a);
  static void
  set_help_option(const char * str);
  static void
  set_help_description(const char * str);
  static void
  set_help_precis(const char * str);
  static void
  display_usage(const char * msg = nullptr);
  static void
  display_usage_and_exit(const char * msg = nullptr);
...
protected:
  ...
  vul_arg_base(vul_arg_info_list & l, const char * option_string, const char * helpstring, bool required = false);
  vul_arg_base(const char * option_string, const char * helpstring, bool required = false);
...
//
void
vul_arg_parse(int & argc, char **& argv, bool warn_about_unrecognized_arguments = true);
...
void
vul_arg_display_usage_and_exit(const char * msg = nullptr);
...
public:
  ...
  // want a default... good)
  vul_arg(const char * option_string = nullptr, const char * helpstring = nullptr, T default_value = T())
  ...
  vul_arg(vul_arg_info_list & l,
          const char * option_string = nullptr,
  ...
  // \seealso is_required
  vul_arg(const char * option_string, const char * helpstring, required_option_type /*dummy*/)
  ...
  //: As above, but add the arg to the list \a l, on which \c parse() can be called later.
  vul_arg(vul_arg_info_list & l, const char * option_string, const char * helpstring, required_option_type /*dummy*/)
  ...
  //: print
  std::ostream &
  print_value(std::ostream & s) override
  {
    ::print_value(s, *this);
    return s; // << flush
  }
...
public:
  ...
  void
  add(vul_arg_base * arg);
  void
  parse(int & argc, char **& argv, bool warn_about_unrecognized_arguments);
  void
  include(vul_arg_info_list & l);
  void
  verbose(bool on)
  {
    verbose_ = on;
...
public: // protected:
  ...
  void
  display_help(const char * progname = nullptr);

private:
  // Disallow assigning to objects of this class:
  vul_arg_info_list(const vul_arg_info_list &) {}
  vul_arg_info_list &
  operator=(const vul_arg_info_list &)
  {
    return *this;
  }
};
