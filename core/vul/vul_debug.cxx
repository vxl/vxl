...
vul_debug_core_dump_in_windows_seh(const char * filename, EXCEPTION_POINTERS * pep)
{
  ...
  HANDLE hFile = CreateFile(buffer, GENERIC_READ | GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
  ...
  if (!MiniDumpWriteDump(
        GetCurrentProcess(), GetCurrentProcessId(), hFile, MiniDumpWithFullMemory, (pep != 0) ? &mdei : 0, 0, 0))
...
bool
vul_debug_core_dump_in_windows_seh(const char * filename, void * pep)
{
...
unsigned
vul_debug_windows_structured_exception::code() const
{
...
void *
vul_debug_windows_structured_exception::address() const
{
  return static_cast<EXCEPTION_POINTERS *>(ex_ptr_)->ExceptionRecord->ExceptionAddress;
}

const char *
vul_debug_windows_structured_exception::what() const throw()
{
  static char buf[100];
  std::snprintf(buf, sizeof(buf), "Caught Windows Structured Exception. Code %lx. Address %px", code(), address());
  return buf;
}
...
void
vul_debug_set_coredump_and_throw_on_windows_se_handler(unsigned code, EXCEPTION_POINTERS * ex_ptr)
{
...
bool
vul_debug_core_dump_in_windows_seh(const char *, void *)
{
...
bool
vul_debug_core_dump(const char *)
{
...
// \throws vul_debug_windows_structured_exception.
void
vul_debug_set_coredump_and_throw_on_windows_se(const char *)
{
