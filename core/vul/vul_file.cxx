...
#if defined(_WIN32) && !defined(__CYGWIN__)
bool
vul_file::is_drive(char const * fn)
{
  // a drive string looks like "c:", "z:"
  return fn && std::isalpha(fn[0]) && fn[1] == ':' && fn[2] == '\0';
}
#endif
...
vul_file::make_directory_path(const char * filename)
{
  ...
  return make_directory_path(dirname(filename)) && (make_directory(filename) || is_directory(filename));
}
...
static unsigned
replace(char from, char to, std::string & s)
{
  unsigned c = 0;
...
vul_file::make_directory_path(const wchar_t * filename)
{
  ...
  return make_directory_path(dirname(filename)) && make_directory(filename);
}
