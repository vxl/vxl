...
public:
  ...
  vul_psfile(const char * filename, bool debug_output = false);
  ~vul_psfile() override;
  operator bool() { return static_cast<bool>(output_filestream); }

  void
  set_paper_type(vul_psfile::paper_type type)
  {
    printer_paper_type = type;
  }
  void
  set_paper_layout(vul_psfile::paper_layout layout)
  {
    printer_paper_layout = layout;
  }
  void
  set_paper_orientation(vul_psfile::paper_orientation o)
  {
    printer_paper_orientation = o;
  }
  void
  set_reduction_factor(int rf)
  {
    reduction_factor = rf;
  }
  //: set the horizontal scaling (in percent); no scaling is 100.
  void
  set_scale_x(float sx)
  {
    scale_x = sx * .01f;
  }
  //: set the vertical scaling (in percent); no scaling is 100.
  void
  set_scale_y(float sy)
  {
    scale_y = sy * .01f;
  }
  void
  set_fg_color(float r, float g, float b)
  {
    fg_r = r;
    fg_g = g;
    fg_b = b;
  ...
  void
  set_bg_color(float r, float g, float b)
  {
    bg_r = r;
    bg_g = g;
    bg_b = b;
  ...
  void
  print_greyscale_image(const unsigned char * data, int sizex, int sizey);
  //: Write 24 bit colour image.
  void
  print_color_image(const unsigned char * data, int sizex, int sizey);
  ...
  void
  point(float x, float y, float point_size = 0);
  //: Add an ellipse to the Postscript file.
  void
  ellipse(float x, float y, float a_axis, float b_axis, int angle = 0);
...
protected:
  void
  set_min_max_xy(float x, float y);
  void
  set_min_max_xy(int x, int y);
  bool
  set_parameters(int sizex, int sizey);
  ...
  void
  sobj_rgb_params(const char * str, bool filled);
