...
void
vul_sequence_filename_map::parse()
{
  ...
  if (image_dir_.empty() && image_extension_.empty())
  {
    ...
    if (!found_match)
    {
      std::cerr << __FILE__ << " : Can't find files matching " << basename_ << index_format_
  ...
  else if (image_dir_.empty())
  {
    ...
    if (!found_match)
    {
      std::cerr << __FILE__ << " : Can't find files matching " << basename_ << index_format_ << image_extension_
                << " in common locations!\n";
      std::abort();
    }
  }
  ...
  else if (image_extension_.empty())
  {
    ...
    if (!found_match)
    {
      std::cerr << __FILE__ << " : Can't find files matching " << image_dir_ << basename_ << index_format_
                << " with common extension!\n";
      std::abort();
    }
  }
  ...
  if (debug)
    std::cerr << seq_template_ << std::endl
              ...
              << "    indices   : " << start_ << ':' << step_ << ':' << end_ << std::endl
              << std::endl;
}

int
vul_sequence_filename_map::get_mapped_index(int real) const
{
  int idx = -1;
...
std::ostream &
vul_sequence_filename_map::print(std::ostream & s) const
{
...
std::ostream &
operator<<(std::ostream & os, const vul_sequence_filename_map & s)
{
  return s.print(os);
}
