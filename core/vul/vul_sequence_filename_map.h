...
public:
  ...
  name(int frame);
  std::string
  pair_name(int i, int j);
  std::string
  triplet_name(int i, int j, int k);

  std::string
  image_name(int frame)
  {
  ...
  int
  get_real_index(int frame) const
  {
    return indices_[frame];
  }
  const std::vector<int> &
  get_real_indices() const
  {
    return indices_;
  }
  ...
  int
  get_mapped_index(int real) const;
...
protected:
  bool
  filter_dirent(const char * name, const std::string & extension);
  int
  extract_index(const char * name);
