...
char *
vul_string_c_upcase(char * s) // Convert entire string to upper case
{
  char * p = s; // Point to beginning of string
  while (*p)
  {                                // While there are still valid characters
...
char *
vul_string_c_downcase(char * s) // Convert entire string to lower case
{
  char * p = s; // Point to beginning of string
  while (*p)
  {                                // While there are still valid characters
...
char *
vul_string_c_capitalize(char * s) // Capitalize each word in string
{
  char * p = s; // Point to beginning of string
  while (true)
  { // Infinite loop
    for (; *p && !std::isalnum(*p); p++)
      ;                          // Skip to first alphanumeric
    if (*p == END_OF_STRING)     // If end of string
      return s;                  // Return string
    *p = (char)std::toupper(*p); // Convert character
    while (*++p && std::isalnum(*p))
...
char *
vul_string_c_trim(char * str, const char * rem) // Trim characters from string
{
  char * s = str;
  char * result = str;
  char c;
...
char *
vul_string_c_right_trim(char * str, const char * rem) // Trim suffix from string
{
...
std::string &
vul_string_reverse(std::string & s)
{
...
std::string &
vul_string_upcase(std::string & s)
{
  for (char & i : s)
...
std::string &
vul_string_downcase(std::string & s)
{
  for (char & i : s)
...
std::string &
vul_string_capitalize(std::string & s)
{
...
template <class IT>
static bool
myequals(IT b1, IT e1, const char * b2, const char * e2)
{
  for (; b1 != e1 && b2 != e2; ++b1, ++b2)
    if (std::toupper(*b1) != *b2)
      return false;
  return b1 == e1 && b2 == e2;
}
...
std::vector<int>
vul_string_to_int_list(std::string str)
{
  std::vector<int> rv;
...
bool
vul_string_replace(std::string & full_str, const std::string & find_str, const std::string & replace_str, int num_times)
{
  bool rep = false;
...
std::string
vul_string_escape_ctrl_chars(const std::string & in)
{
  std::string out;
