...
//       EIGEN_INITIALIZE_COEFFS_IF_THAT_OPTION_IS_ENABLED
  ...
  public:
    ...
    template <class T>
    using eigen_vnl_vector_plain_object_base = Eigen::PlainObjectBase<Eigen::Matrix<T, Eigen::Dynamic, 1>>;
    using eigen_vnl_vector_plain_object_base_same = eigen_vnl_vector_plain_object_base<Scalar>;

    PlainObjectBase &
    copy_in (Scalar const *ptr)
    {
    ...
    using Self = PlainObjectBase;
    Self & update (const PlainObjectBase & v, size_t start)
    {
    ...
    // norms etc
    using abs_t = typename vnl_c_vector<Scalar>::abs_t;

    //: Return sum of squares of elements
    inline abs_t squared_magnitude() const
    {
      return vnl_c_vector<Scalar>::two_nrm2(this->data(), this->size());
    }

    //: Return magnitude (length) of vector
    inline abs_t magnitude() const
    {
      return two_norm();
    }

    //: Return sum of absolute values of the elements
    inline abs_t one_norm() const
    {
      return vnl_c_vector<Scalar>::one_norm(this->data(), this->size());
    }

    //: Return sqrt of sum of squares of values of elements
    inline abs_t two_norm() const
    {
      return vnl_c_vector<Scalar>::two_norm(this->data(), this->size());
    }

    //: Return largest absolute element value
    inline abs_t inf_norm() const
    {
      return vnl_c_vector<Scalar>::inf_norm(this->data(), this->size());
    }

    // These next 6 functions are should really be helper functions since they aren'Scalar
    // really proper functions on a vector in a philosophical sense.
    //: Root Mean Squares of values
    inline abs_t rms() const { return vnl_c_vector<Scalar>::rms_norm(this->data(), this->size()); }

    //: Smallest value
    inline Scalar min_value() const
    {
      return vnl_c_vector<Scalar>::min_value(this->data(), this->size());
    }

    //: Largest value
    inline Scalar max_value() const
    {
      return vnl_c_vector<Scalar>::max_value(this->data(), this->size());
    }

    //: Location of smallest value
    inline size_t arg_min() const
    {
      return vnl_c_vector<Scalar>::arg_min(this->data(), this->size());
    }

    //: Location of largest value
    inline size_t arg_max() const
    {
      return vnl_c_vector<Scalar>::arg_max(this->data(), this->size());
    }
    ...
    //: Access the contiguous block storing the elements in the vector. O(1).
    //  data_block()[0] is the first element of the vector
    inline Scalar * data_block()
    {
      return this->data();
    }
#ifdef NOT_USED_BY_ITK
    //: Flatten row-major (C-style)
    eigen_vnl_vector_plain_object_base_same flatten_row_major() const {
      eigen_vnl_vector_plain_object_base_same v(this->rows() * this->cols());
      v.copy_in(this->data_block());
      return v;
    }

    //: Flatten column-major (Fortran-style)
    eigen_vnl_vector_plain_object_base_same flatten_column_major() const {
      eigen_vnl_vector_plain_object_base_same v(this->rows() * this->cols());
      for (unsigned int c = 0; c < this->cols(); ++c)
        for (unsigned int r = 0; r < this->rows(); ++r) v(c * this->rows() + r) = this->operator()(r, c);
      return v;
    }
#endif
    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
