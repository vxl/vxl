...
/*     compute the norms of the free columns. */
    ...
    for (j = pl; j <= pu; ++j) {
        work[j].r = qraux[j].r = (float)scnrm2_(n, &x[j* *ldx], &c__1),
...
/*           compute the householder transformation for column l. */
        ...
        i__1 = *n - l;
        i__2 = l + l * *ldx;
        nrmxl.r = (float)scnrm2_(&i__1, &x[i__2], &c__1), nrmxl.i = 0.f;
        if (nrmxl.r == 0.f) {
            continue; /* next l */
        }
        if (x[i__2].r != 0.f || x[i__2].i != 0.f) {
            d__1 = c_abs(&nrmxl);
            d__2 = c_abs(&x[i__2]);
            nrmxl.r = (float)(d__1 * x[i__2].r / d__2),
            nrmxl.i = (float)(d__1 * x[i__2].i / d__2);
        }
...
/*              updating the norms. */
        ...
        for (j = l+1; j < *p; ++j) {
            ...
            r__1 = (float)c_abs(&x[l+j* *ldx]) / qraux[j].r;
            tt = 1.f - r__1 * r__1;
            if (tt < 0.f) tt = 0.f;
            t.r = tt, t.i = 0.f;
            ...
            if (tt == 1.f) {
                i__1 = *n - l - 1;
                i__2 = l + 1 + j * *ldx;
                work[j].r = qraux[j].r = (float)scnrm2_(&i__1, &x[i__2], &c__1),
                work[j].i = qraux[j].i = 0.f;
            }
            else {
                r__1 = (float)sqrt(t.r);
